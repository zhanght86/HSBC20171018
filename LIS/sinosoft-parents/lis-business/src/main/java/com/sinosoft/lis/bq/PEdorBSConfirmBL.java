 ...
 * @version 1.0
 */
public class PEdorBSConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(PEdorBSConfirmBL.class);
...
//	private LCContSchema tLCContSchema = new LCContSchema();
	...
	private boolean checkData() {
		...
		//查询是否已经扫描进入保单效力确认说明书，要制定扫描件类型
		String tSQL="select docid from es_doc_relation where bussno = '" + "?bussno?" + "' and subtype = 'BQ006' and  busstype = 'BQ' " ;
		String tHasFLag="";
		SQLwithBindVariables sqlbv=new SQLwithBindVariables();
		sqlbv.sql(tSQL);
		sqlbv.put("bussno", mLPEdorItemSchema.getEdorAcceptNo());
		ExeSQL tExeSQL = new ExeSQL();
		tHasFLag=tExeSQL.getOneValue(sqlbv);
		if("".equals(tHasFLag))
		{
			mErrors.addOneError(new CError("保单效力确认说明书未进行扫描回收，不可以做保全确认！"));
			return false;			
		}		
		// 得到保全批改信息
		return true;
...
//		map.put(tLCContSchema, "UPDATE");
	...
	public static void main(String[] args) {
		...
		try {
			Class tClass = Class
					.forName("com.sinosoft.lis.bq.PEdorBSConfirmBL");
			EdorConfirm tEdorConfirm = (EdorConfirm) tClass.newInstance();
			if (!tEdorConfirm.submitData(tVData, "")) {
				logger.debug("== fail to PEdorBSConfirmBL ==");
			} else {
				logger.debug("== after PEdorBSConfirmBL ==");
			}

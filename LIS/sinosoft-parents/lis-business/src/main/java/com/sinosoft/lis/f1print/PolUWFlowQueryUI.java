...
public class PolUWFlowQueryUI {
private static Logger logger = Logger.getLogger(PolUWFlowQueryUI.class);
	public CErrors mErrors = new CErrors();
	private VData mResult = new VData();

	public PolUWFlowQueryUI() {
	}

	public boolean submitData(VData cInputData, String cOperate) {
		PolUWFlowQueryBL tPolUWFlowQueryBL = new PolUWFlowQueryBL();
		if (!tPolUWFlowQueryBL.submitData(cInputData, cOperate)) {
			mErrors.copyAllErrors(tPolUWFlowQueryBL.mErrors);
			return false;
		} else {
			mResult = tPolUWFlowQueryBL.getResult();
		}
		return true;
	}

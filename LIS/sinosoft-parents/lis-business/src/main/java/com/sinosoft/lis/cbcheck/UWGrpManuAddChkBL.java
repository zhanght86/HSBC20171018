 ...
 * @version 1.0
 */
public class UWGrpManuAddChkBL {
private static Logger logger = Logger.getLogger(UWGrpManuAddChkBL.class);
	...
	private LCGrpPolSchema mLCGrpPolSchema = null;
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("Start UWGrpManuAddChkBL Submit...");

		PubSubmit tSubmit = new PubSubmit();

		if (!cOperate.equals("GrpAdd")) {
		...
		logger.debug("UWGrpManuAddChkBL Submit OK!");
	...
	private boolean checkData() {
		...
		LCGrpPolDB tLCGrpPolDB = new LCGrpPolDB();
		tLCGrpPolDB.setGrpPolNo(mLCPolSchema.getGrpPolNo());
	...
	private boolean prepareAdd() {
		...
		if (mLCPremSet.size() > 0) {
			...
			sqlbv.sql(tsql);
			sqlbv.put("polno", mLCPolSchema.getPolNo().trim());
		...
		sqlbv2.sql(tSQL);
		sqlbv2.put("polno", mPolNo2);
		LCPremDB tLCPremDB = new LCPremDB();
		mOldLCPremSet = tLCPremDB.executeQuery(sqlbv2);
		...
		if ((tSumStandPrem > 0) && (mLCPremSet != null)
				&& (mLCPremSet.size() > 0)) {
			for (int i = 1; i <= mLCPremSet.size(); i++) {
				LCDutyDB tLCDutyDB = new LCDutyDB();
				LCDutySchema tLCDutySchema = new LCDutySchema();
		...
		if (tLCGCUWMasterDB.getInfo() == false) {
			...
			tError.moduleName = "UWGrpManuAddChkBL";
			tError.functionName = "prepareAdd";
			tError.errorMessage = "无团单合同单核保主表信息!";
			this.mErrors.addOneError(tError);

			return false;
		}

		mLCGCUWMasterSchema.setSchema(tLCGCUWMasterDB);

		if ((mAddReason != null) && !mAddReason.trim().equals("")) {
			mLCGCUWMasterSchema.setAddPremReason(mAddReason);
		} else {
			mLCGCUWMasterSchema.setAddPremReason("");
		}

		if ((mLCPremSet != null) && (mLCPremSet.size() > 0)) {
			mLCGCUWMasterSchema.setAddPremFlag("1"); // 有加费标识
		} else {
			mLCGCUWMasterSchema.setAddPremFlag("0"); // 无加费标识
		}

		mLCGCUWMasterSchema.setOperator(mOperater);
		mLCGCUWMasterSchema.setManageCom(mManageCom);
		mLCGCUWMasterSchema.setModifyDate(tCurrentDate);
		mLCGCUWMasterSchema.setModifyTime(tCurrentTime);

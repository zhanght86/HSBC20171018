...
public class FFChargeDayRiskF1PBL {
private static Logger logger = Logger.getLogger(FFChargeDayRiskF1PBL.class);
...
//全局变量
  ...
  private boolean getPrintData()
  {
    ...
    msql="select RiskCode,sum(PayMoney),sum(1) from LJTempFee where MakeDate>='"+"?date1?"+"' and MakeDate<='"+"?date2?"+"' and ManageCom like concat('"+"?ManageCom?"+"','%') Group By RiskCode order by RiskCode";
    //判断操作员位数是否为8位
    ...
    for (int i=1;i<=tSSRS.MaxRow;i++)
    {
      ...
      for (int j=1;j<=tSSRS.MaxCol;j++)
      {
        ...
        if (j==2)
        {
          ...
          strArr[j-1] = strSum;
          //double a = Double.parseDouble(strArr[j-1]);
          //double a = Double.valueOf(strArr[j-1]).doubleValue();
          //SumMoney = SumMoney + a;
    ...
    xmlexport.createDocument("FFChargeDayRisk.vts","printer");//最好紧接着就初始化xml文档

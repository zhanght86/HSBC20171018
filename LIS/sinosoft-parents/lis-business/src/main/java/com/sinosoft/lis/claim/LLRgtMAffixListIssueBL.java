...
public class LLRgtMAffixListIssueBL {
private static Logger logger = Logger.getLogger(LLRgtMAffixListIssueBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		logger.debug("----------LLRgtMAffixListIssueBL Begin----------");
		...
		// 检查数据合法性
		logger.debug("----------LLRgtMAffixListIssueBL checkData begin----------");

		if (!checkData()) {
			return false;
		}
		logger.debug("----------LLRgtMAffixListIssueBL checkData end----------");

		// 进行业务处理
		if (!dealData()) {
			return false;
		}
		logger.debug("----------LLRgtMAffixListIssueBL dealData end----------");
		...
		logger.debug("----------LLRgtMAffixListIssueBL End----------");
		return true;
	}
	...
	private boolean getInputData() {
		logger.debug("----------LLRgtMAffixListIssueBL getInputData begin----------");
		...
		mLLAffixSet = (LLAffixSet) mInputData.getObjectByObjectName(
				"LLAffixSet", 0);
		logger.debug("----------LLRgtMAffixListIssueBL getInputData end----------");
		return true;
	}
	...
	private boolean checkData() {
		...
		if (mGlobalInput == null) {
			...
			tError.moduleName = "LLRgtMAffixListIssueBL";
	...
	private boolean dealData() {
		logger.debug("----------LLRgtMAffixListIssueBL dealData begin----------");
...
//						+ "rgtno='" + mLLAffixSet.get(1).getRgtNo() + "' and "
//						+ "customerno='" + mLLAffixSet.get(1).getCustomerNo()
...
//				}
				...
				String mSQL = "select * from LLRegisterIssue where rgtno='" + "?rgtno?" + "' and IssueConclusion='02' and IssueReplyDate is null";
				SQLwithBindVariables sqlbv = new SQLwithBindVariables();
				sqlbv.sql(mSQL);
				sqlbv.put("rgtno", Rgtno);
				...
				if (tLLRegisterIssueSet.size() != 1 )
				{
					// @@错误处理
					CError tError = new CError();
					tError.moduleName = "LLRgtMAffixListIssueBL";
					tError.functionName = "checkData";
					tError.errorMessage = "当前赔案LLRegisterIssue未核销记录不唯一!";
					this.mErrors.addOneError(tError);
					tLLRegisterIssueSet.clear();
					return false;
				}
				else
				{
					mLLRegisterIssueSchema = tLLRegisterIssueSet.get(1);
					mLLRegisterIssueSchema.setIssueReplyer(mGlobalInput.Operator);
					mLLRegisterIssueSchema.setIssueReplyCom(mGlobalInput.ManageCom);
					mLLRegisterIssueSchema.setIssueReplyDate(CurrentDate);
					mLLRegisterIssueSchema.setIssueReplyTime(CurrentTime);
					mLLRegisterIssueSchema.setModifyDate(CurrentDate);
					mLLRegisterIssueSchema.setModifyTime(CurrentTime);
					map.put(mLLRegisterIssueSchema, "UPDATE");
				}
				tReturn = true;
			} catch (Exception ex) {

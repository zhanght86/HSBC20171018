...
private static Logger logger = Logger.getLogger(LLClaimImportModifyBL.class);
    ...
    private boolean prepareClaimImportModify()
    {
        ...
        String strSQL1 = "delete from LLAppClaimReason where CaseNo='" +mLLCaseSchema.getRgtNo()+ "' and customerno='"+mLLCaseSchema.getCustomerNo()+"'";
	...
	private boolean UpdateAccNo(String mergeAccNo,String oldAccNo,String oldClmNo,String pOperate) {
		try {
			...
			mMMap.put("update llclaimpolicy set caserelano='"+mergeAccNo+"' where clmno='"+oldClmNo+"'", "UPDATE");
			mMMap.put("update llclaimdetail set caserelano='"+mergeAccNo+"' where clmno='"+oldClmNo+"'", "UPDATE");
			mMMap.put("update LLPrepayDetail set caserelano='"+mergeAccNo+"' where clmno='"+oldClmNo+"'", "UPDATE");
			mMMap.put("update llcaserela set caserelano='"+mergeAccNo+"' where caseno='"+oldClmNo+"'", "UPDATE");
			mMMap.put("update LLAccidentSub set accno='"+mergeAccNo+"' where accno='"+oldAccNo+"'", "UPDATE");

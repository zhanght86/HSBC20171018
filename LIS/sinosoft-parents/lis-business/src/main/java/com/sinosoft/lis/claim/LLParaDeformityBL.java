...
private static Logger logger = Logger.getLogger(LLParaDeformityBL.class);
	...
	private boolean getInputData(VData cInputData, String cOperate) {
		...
		// mLCUWSendTraceSchema = (LCUWSendTraceSchema)
		// mTransferData.getValueByName("LCUWSendTraceSchema");

		mLLParaDeformitySchema = (LLParaDeformitySchema) mInputData
				.getObjectByObjectName("LLParaDeformitySchema", 0);
		return true;
	}
	...
	private boolean dealData() {
       ...
       //进行伤残等级参数的修改
		if (this.mOperate.equals("Deformity||UPDATE")) {
			if (!DeformityUpdate()) {
				return false;
			}
		}
	...
	private boolean DeformityInsert() {
		...
		if (tLLParaDeformitySet == null || tLLParaDeformitySet.size() == 0) {
			...
			tLLParaDeformitySchema.setDefoType(mLLParaDeformitySchema
					.getDefoType());
			tLLParaDeformitySchema.setDefoGrade(mLLParaDeformitySchema
					.getDefoGrade());
			tLLParaDeformitySchema.setDefoGradeName(mLLParaDeformitySchema
					.getDefoGradeName());
			tLLParaDeformitySchema.setDefoCode(mLLParaDeformitySchema
					.getDefoCode());
			tLLParaDeformitySchema.setDefoName(mLLParaDeformitySchema
					.getDefoName());
			tLLParaDeformitySchema.setDefoRate(mLLParaDeformitySchema
					.getDefoRate());
	 ...
	 * @return
	 */
	private boolean DeformityUpdate() {
		LLParaDeformitySchema tLLParaDeformitySchema = new LLParaDeformitySchema();
		tLLParaDeformitySchema.setSchema(mLLParaDeformitySchema);

		map.put(tLLParaDeformitySchema, "UPDATE");
		return true;
	}

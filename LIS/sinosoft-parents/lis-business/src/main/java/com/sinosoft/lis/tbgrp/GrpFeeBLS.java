...
import com.sinosoft.lis.vdb.LCGrpFeeTraceDBSet;
import com.sinosoft.lis.vdb.LCGrpFeeParamDBSet;
import com.sinosoft.lis.vdb.LCGrpFeeParamTraceDBSet;
 ...
 * 管理费数据提交类
 * <p>Title: </p>
 * <p>Description: 根据传入类型，执行新增、删除、修改操作 </p>
        ...
        try {
            ...
            tLCGrpFeeParamDBSet.set((LCGrpFeeParamSet) mInputData.
                                    getObjectByObjectName(
                                            "LCGrpFeeParamSet", 0));
            //feifen
            LCGrpFeeTraceDBSet tLCGrpFeeTraceDBSet = new LCGrpFeeTraceDBSet(
                    conn);

            tLCGrpFeeTraceDBSet.set((LCGrpFeeTraceSet) mInputData.
                                    getObjectByObjectName("LCGrpFeeTraceSet",
                    0));
            LCGrpFeeParamTraceDBSet tLCGrpFeeParamTraceDBSet = new
                    LCGrpFeeParamTraceDBSet(conn);
            tLCGrpFeeParamTraceDBSet.set((LCGrpFeeParamTraceSet) mInputData.
                                         getObjectByObjectName(
                    "LCGrpFeeParamTraceSet", 0));
            //删除管理费子表信息
            String tSql = "delete from LCGrpFeeParam Where GrpPolNo = '" +
                          tLCGrpFeeDBSet.get(1).getGrpPolNo() +
                          "' and RiskCode = '" +
                          tLCGrpFeeDBSet.get(1).getRiskCode() +
                          "' and FeeCode = '" +
            ...
            if (!tLCGrpFeeTraceDBSet.insert()) {
                // @@错误处理
                this.mErrors.copyAllErrors(tLCGrpFeeDBSet.mErrors);
                CError tError = new CError();
                tError.moduleName = "GrpFeeBLS";
                tError.functionName = "saveData";
                tError.errorMessage = "插入管理费主表轨迹信息失败!";
                this.mErrors.addOneError(tError);
                conn.rollback();
                conn.close();
                return false;
            ...
            if (!tLCGrpFeeParamTraceDBSet.insert()) {
                // @@错误处理
                this.mErrors.copyAllErrors(tLCGrpFeeDBSet.mErrors);
                CError tError = new CError();
                tError.moduleName = "GrpFeeBLS";
                tError.functionName = "saveData";
                tError.errorMessage = "插入管理费子表轨迹信息失败!";
                this.mErrors.addOneError(tError);
                conn.rollback();
                conn.close();
                return false;

...
public class GrpTranYesLisBL {
private static Logger logger = Logger.getLogger(GrpTranYesLisBL.class);
	public GrpTranYesLisBL() {
		try {
			jbInit();
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	private void buildError(String szFunc, String szErrMsg) {
		...
		cError.moduleName = "GrpTranYesLisBL";
	...
	private boolean getPrintData() {
				...
				for (int k = 0; k < YingSSRS.getMaxRow(); k++) {
					...
					for (int l = 1; l <= i_count; l++) {
						...
						try {
							...
							Sum = Sum + Double.parseDouble(ssrs.GetText(l, 5));
						} catch (Exception ex) {
		...
		if (this.mListType == null || this.mListType == "") {
			...
			xmlexport.createDocument("GrpTranYesRLis.vts", "printer"); // appoint
																		// to a
																		// special
																		// output
																		// .vts
																		// file
		} else if (this.mListType.equals("2")) {
			xmlexport.createDocument("GrpTranYesPLis.vts", "printer");
		}
		...
		xmlexport.outputDocumentToFile("D:\\", "GrpTranYesLisHZM"); // 输出xml文档到文件
		mResult.clear();
	...
	public static void main(String[] args) {
		...
		GrpTranYesLisUI tGrpTranYesLisUI = new GrpTranYesLisUI();
		tGrpTranYesLisUI.submitData(tVData, "PRINT");
	}

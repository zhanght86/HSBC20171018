...
private static Logger logger = Logger.getLogger(EdorSignBL.class);
    ...
    private boolean getInputData()
    {
        try
        {
            ...
            {
                inLCPolSchema = (LCPolSchema) mInputData.getObjectByObjectName("LCPolSchema", 0);
                inLCPremSet = (LCPremSet) mInputData.getObjectByObjectName("LCPremSet", 0);
            }
            else if (mOperate.equals("QUERY||GRPNS"))
            {
                inLCGrpPolSchema = (LCGrpPolSchema) mInputData.getObjectByObjectName("LCGrpPolSchema", 0);
            }
    ...
    private boolean checkFinfee(LJAGetEndorseSet tLJAGetEndorseSet)
    {
        ...
        {
            totalGetMoney = totalGetMoney + tLJAGetEndorseSet.get(i + 1).getGetMoney();
        }
        ...
        try
        {
            ...
            else if (mOperate.equals("QUERY"))
            {
                ...
                if (inLCPolSchema.getGrpPolNo().equals("00000000000000000000"))
                {
                    ...
                    outLJAGetEndorseSet.add(tLJAGetEndorseSet);         //?? 为什么要删除？ asked by zhangrong
                ...
                else
                {
                    ...
                    if (tLJAGetEndorseSet.size() == 0)
                    {
                      ...
                      LJAGetEndorseSchema tLJAGetEndorseSchema = tLJAGetEndorseSet.get(1);

                      //删除LJAGetEndorse表中的数据
                      outLJAGetEndorseSet.add(tLJAGetEndorseSet);

                      //获取实收总表数据
                      /*jixf del 因为可能走内部交退费核销,所以在这里就不校验ljapay,并且把
                                           /*ljapayperson提到前面财务核销时生成!
                                            LJAPayDB tLJAPayDB = new LJAPayDB();
                        ...
                        tLJAPayPersonSchema.setGrpContNo(inLCPolSchema.getGrpContNo());
                      ...
                      //if (inEdorType.equals("NI")) {
                        //新增实收集体交费表
          //                        setLJAPayGrp(tLJAPayDB, inLCPolSchema.getGrpPolNo());        //改到团单批单保全确认时再完成此任务 modified by zhangrong
                      //}
                    }
                }
            ...
            else if (mOperate.equals("QUERY||GRPNS"))
            {
                ...
                LJAGetEndorseSchema tLJAGetEndorseSchema = tLJAGetEndorseSet.get(1);
                if (tLJAGetEndorseSchema.getGetMoney() != inLCGrpPolSchema.getPrem())
                {
    ...
    {
        String strSql = "select * from LJAGetEndorse where EndorsementNo='" + inEdorNo + "'"
                        + " and FeeOperationType='" + inEdorType + "'"
                        + " and PolNo='" + inLCPolSchema.getProposalNo() + "'"
                        + " and FeeFinaType='BF'"
                        + " and operstate='0' ";
        LJAGetEndorseDB tLJAGetEndorseDB = new LJAGetEndorseDB();
        return tLJAGetEndorseDB.executeQuery(strSql);
    }
    ...
    {
        String strSql = "select * from LJAGetEndorse where  EndorsementNo='" + inEdorNo + "'"
                        + " and FeeOperationType='" + inEdorType + "' and operstate='0'";
//                  + " and GrpPolNo='" + inLCGrpPolSchema.getGrpPolNo() + "'";
     ...
     * @throws Exception
     */
//    private void setLJAPayGrp(LJAPayDB tLJAPayDB, String grpContNo) throws Exception
//    {

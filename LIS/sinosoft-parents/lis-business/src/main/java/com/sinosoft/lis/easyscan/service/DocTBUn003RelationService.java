...
public class DocTBUn003RelationService extends BaseService {
private static Logger logger = Logger.getLogger(DocTBUn003RelationService.class);
	...
	public boolean deal(){
		try {
			...
			if (mLWMissionSet == null || mLWMissionSet.size() != 1) {
				CError.buildErr(this, "无发放此体检通知书!");
				return false;
			}
			...
			mES_DOC_RELATIONSchema.setBussNo(mES_DOC_MAINSchema
					.getDocCode());
			mES_DOC_RELATIONSchema.setDocCode(mES_DOC_MAINSchema
					.getDocCode());
			mES_DOC_RELATIONSchema.setBussNoType("15");
			mES_DOC_RELATIONSchema.setDocID(mES_DOC_MAINSchema.getDocID());
			mES_DOC_RELATIONSchema.setBussType(mES_DOC_MAINSchema
					.getBussType());
			mES_DOC_RELATIONSchema.setSubType(mES_DOC_MAINSchema
					.getSubType());
			...
			mES_DOC_RELATIONSchema.setBussNo(mLWMissionSet.get(1)
					.getMissionProp2());
			mES_DOC_RELATIONSchema.setDocCode(mLWMissionSet.get(1)
					.getMissionProp2());

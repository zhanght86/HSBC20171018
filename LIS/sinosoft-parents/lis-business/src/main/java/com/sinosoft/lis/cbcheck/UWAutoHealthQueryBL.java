...
//		mAllLDHealthSet.add( tLDHealthSet );
	...
	*/
	...
	private void prepareSpecHealth(LDHealthSchema tLDHealthSchema,String tItemType,String tSex,int tAge,double tMoney )
	{
		...
		for (int n = 0; n < tSubHealth.length; n++) {
			...
			tSQL = " select * from LDHealthSub where " 
				 + " (" 
				// + //"healthtype = '" + tItemType + "' and "
				 + " (note1 is null or note1<>'X') ) "
				 // 0 表示不区分性别
				 + " and sex in ('" + "?sex?" + "','0') " 
				 + " and ((StartAge <= "+ "?age?" 
				 + " and EndAge >= " + "?age?" +") or (StartAge=0 and EndAge=0)) "
				 + " and ((StartMoney <= " + "?money?" + " " 
				 + " and EndMoney >= " + "?money?" + ") or (StartMoney=0 and EndMoney=0)) "
	...
	/**
	 * 按照代理人以及保单管理机构获取差异核保级别，分为A、B、C、D四档，如果代理人或者机构级别为空，取
	 * 默认级别A，定义在ldsysvar中sysvar ='UWDefaultLevel'
	 * 除02渠道外的其他各个渠道都为“C”级别
	 * @param tLCContSchema
	 * @return
	 */
	private String getUWLevel(LCContSchema tLCContSchema)
	{
		logger.debug("Start getUWLevel....");
		String tUWLevel = "A";
		String tSaleChnl = tLCContSchema.getSaleChnl();
		String tSQL = "select (case when ((case when (select latree.uwlevel from latree where agentcode = '?agentcode?') is null then  "
		            + "(select othersign from ldcode "
		            + "where codetype = 'station' and code = '?code?') else (select latree.uwlevel from latree where agentcode = '?agentcode?') end)) is null then "	
		            + "(select sysvarvalue from ldsysvar where sysvar ='UWDefaultLevel') else ((case when (select latree.uwlevel from latree where agentcode = '?agentcode?') is null then "
		            + "(select othersign from ldcode "
		            + "where codetype = 'station' and code = '?code?') else (select latree.uwlevel from latree where agentcode = '?agentcode?') end)) end) "		 
		            + "from dual";
		logger.debug("tSQL: "+tSQL);
		SQLwithBindVariables sqlbv3 = new SQLwithBindVariables();
		sqlbv3.sql(tSQL);
		sqlbv3.put("agentcode", tLCContSchema.getAgentCode());
		sqlbv3.put("code", tLCContSchema.getManageCom());
		ExeSQL tExeSQL =new ExeSQL();
		String tTempUWLevel = StrTool.cTrim(tExeSQL.getOneValue(sqlbv3));
		logger.debug("***tTempUWLevel: "+tTempUWLevel);
		if(!"".equals(tTempUWLevel)&&"02".equals(tSaleChnl))
		{
			tUWLevel = tTempUWLevel;
		}
		else
		{
			tUWLevel = "C";
		}
		logger.debug("***tUWLevel: "+tUWLevel);
		logger.debug("End getUWLevel....");
		return tUWLevel;
	}
	/**
	...
	private boolean prepareHealthNew()
	{
		...
		tSQL_Flag = "select count(*) from lcpol where insuredno='"+"?insuredno?"+"' "
		          + " and riskcode not in ('112211','121602','121504','121303','112214','121801','121507','121301' "
		          + ",'312201','312202','312203','312204','312205','312206','312205','314301','314302') "
		          + " and appflag<>'4' and uwflag not in ('1','2','a') ";
		SQLwithBindVariables sqlbv4 = new SQLwithBindVariables();
		sqlbv4.sql(tSQL_Flag);
		sqlbv4.put("insuredno", this.mInsuredNo);
		String tFlag = "";
		ExeSQL tExeSQL = new ExeSQL();
		tFlag = tExeSQL.getOneValue(sqlbv4);
...
//		+ "','13','2') from dual";
		if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
				tsql = "select healthyamnt2('" + "?insuredno?" + "','13','2') from dual";
			}else if(SysConst.DBTYPE.equals(SysConst.DBTYPE_MYSQL)){
				tsql ="{ call HEALTHYAMNT2(?#@d#?,'" + "?insuredno?" + "','13','2') }";
			}
		SQLwithBindVariables sqlbv5 = new SQLwithBindVariables();
		sqlbv5.sql(tsql);
		sqlbv5.put("insuredno", mInsuredNo);
		String tempAmnt = riskSql.getOneValue(sqlbv5);			
		RiskAmnt1 = parseFloat(tempAmnt);//寿险体检额度
		
//		tsql = "select healthyamnt2('" + "?insuredno?"
//		+ "','14','2') from dual";
		if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
			tsql = "select healthyamnt2('" + "?insuredno?" + "','14','2') from dual";
		}else if(SysConst.DBTYPE.equals(SysConst.DBTYPE_MYSQL)){
			tsql ="{ call HEALTHYAMNT2(?#@d#?,'" + "?insuredno?" + "','14','2') }";
		}
		SQLwithBindVariables sqlbv6 = new SQLwithBindVariables();
		sqlbv6.sql(tsql);
		sqlbv6.put("insuredno", mInsuredNo);
		tempAmnt = riskSql.getOneValue(sqlbv6);
		RiskAmnt2 = parseFloat(tempAmnt); //重疾体检额度
		
//		tsql = "select healthyamnt2('" + "?insuredno?"
//		+ "','15','2') from dual";
		if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
			tsql = "select healthyamnt2('" + "?insuredno?" + "','15','2') from dual";
		}else if(SysConst.DBTYPE.equals(SysConst.DBTYPE_MYSQL)){
			tsql ="{ call HEALTHYAMNT2(?#@d#?,'" + "?insuredno?" + "','15','2') }";
		}
		SQLwithBindVariables sqlbv7 = new SQLwithBindVariables();
		sqlbv7.sql(tsql);
		sqlbv7.put("insuredno", mInsuredNo);
		tempAmnt = riskSql.getOneValue(sqlbv7);
		RiskAmnt3 = parseFloat(tempAmnt); //医疗体检额度
		...
		if(!tFlag.equals("0"))
		{
			...
			//差异化核保级别
			String tUWLevel = getUWLevel(tLCContSchema);
			//tongmeng 2009-04-28 modify
			//寿险+重疾带寿险表 
			//tMoney = RiskAmnt1;
			...
			sqlbv8.put("Note1", tUWLevel);
					
			tLDHealthSet = tLDHealthDB.executeQuery(sqlbv8);
			if(tLDHealthSet.size()>0&&!tLifeFlag)
			{
			...
			else
			{
				...
				else
				{
					...
					tsql_health = "";
					tsql_health = tsql_health1 + ""
					            + " and healthtype='"+tItemType+"' "
					            + " and StartMoney <= "+tMoney+" "
					            + " and EndMoney >= "+tMoney+" ";
					logger.debug("tsql_health3:"+tsql_health);
					tLDHealthSet = tLDHealthDB.executeQuery(tsql_health);
					if(tLDHealthSet.size()>0)
					{
						//查询有结果.查询LDHealthSub处理特殊体检项目
						//待补充
	...
	*/
	private boolean prepareHealth()
	{
	  ...
	  //计算被保人体检额度
	  String tsql1;
	  tsql1 ="select PECHECK('"+"?contno?"+"','"+"?insuredno?"+"') from dual where 1=1";
	  SQLwithBindVariables sqlbv13 = new SQLwithBindVariables();
	  sqlbv13.sql(tsql1);
	  sqlbv13.put("contno", this.mContNo);
	  sqlbv13.put("insuredno", this.mInsuredNo);
	  String tReSult = new String();
	  ExeSQL tExeSQL=new ExeSQL();
	  tReSult =tExeSQL.getOneValue(sqlbv13);

...
public class PersonPayPlanBLTMThread extends CovBase {
	private static Logger logger = Logger.getLogger(PersonPayPlanBLTMThread.class);
	...
	private LJSGetSet tLJSGetSet = new LJSGetSet();
	...
	public PersonPayPlanBLTMThread() {
	}
	...
	public void run() {
		...
		MMap tMMap = new MMap();
		LCGetSchema hLCGetSchema=new LCGetSchema();
		for(int i=0;i<this.mInputDataNew.size();i++)
		{
			Map tMap = new HashMap();
			tMap = (Map)mInputDataNew.get(i);
			tGI=(GlobalInput)tMap.get("GlobalInput");
			mSerialNo = PubFun1.CreateMaxNo("SERIALNO", PubFun.getNoLimit(tGI.ManageCom));
			VData mInputData = new VData();
			mInputData.add(tMap.get("LCGetSchema"));
			mInputData.add(tMap.get("GlobalInput"));
			LCGetSchema tLCGetSchema=(LCGetSchema)tMap.get("LCGetSchema");
			mTimeEnd =(String)tMap.get("ParamCalDate");
			String thisContNo = tLCGetSchema.getContNo();
			if (i > 1 && thisContNo.equals(hLCGetSchema.getContNo())) {
				continue;// 每次循环处理一个ContNo
			}
			hLCGetSchema=tLCGetSchema;
			if (!getLjsgetPrtM(tLCGetSchema)) {
//				日志监控,状态监控          
				PubFun.logState(tGI,tLCGetSchema.getContNo(),"保单"+tLCGetSchema.getContNo()+"个单催付处理失败，原因是:"+mErrors.getFirstError(),"0");
				ErrCount++;
				continue;
			}else
			{
...
//				日志监控,状态监控          
			...
			//日志监控,结果监控
			PubFun.logResult (tGI,tGI.LogID[1],"共催付产生"+mCount+"笔生存金");
		  ...
		  logger.debug("\t   个单催付处理: " + nSuccCount + " 成功; " + ErrCount + " 失败。");
//			日志监控,结果监控
	      PubFun.logResult (tGI,tGI.LogID[1],"个单催付处理: " + nSuccCount + " 成功; " + ErrCount + " 失败。");
	      logger.debug("\t@> PersonPayPlanBLTMThread.run() : 个单催付处理完毕");
	...
	public boolean getAccGet(LCGetSchema aLCGetSchema)
	{
		String thisContNo = aLCGetSchema.getContNo();
		
		//从这开始处理一个保单号，进行加锁处理
		try {
			
			if (mPubLock.lock(thisContNo, "CFU001", tGI.Operator) == false) {
...
//				return true;
	...
	public boolean getLjsgetPrtM(LCGetSchema tLCGetSchema)
	{
		String thisContNo = tLCGetSchema.getContNo();
		...
		String strsql = initCondGet(tGI,sbv) + " and ContNo='?thisContNo?' order by ContNo,PolNo,getdutycode,dutycode";
		sbv.sql(strsql);
...
//		 日志监控,状态监控                 		
			PubFun.logState(tGI,stateLJSGetSchema.getGetNoticeNo(), "生存金(通知书号：" +stateLJSGetSchema.getGetNoticeNo()+ ")催付成功","2");
		}
		mCount++;
	}
		}
...
//			}
	...
	public LOPRTManagerSet getPrintData(LJSGetSet tLJSGetSet,
		...
		for (int i = 1; i <= tLJSGetSet.size(); i++) {
			...
			if (tLCGetSchema.getGetIntv() == 1) {
				...
				sqlbv1.sql(strsql);
				sqlbv1.put("Date1", tLJSGetSchema.getGetDate());
				sqlbv1.put("Date2", tLJSGetSchema.getStartGetDate());
				logger.debug("strsql:" + strsql);
				
				String month = tExeSQL.getOneValue(sqlbv1);
			...
			for(int j=1;j<=tLCBnfSet.size();j++)
			{
				...
				sqlbv2.sql(sql);
				sqlbv2.put("polno", tLCGetSchema.getPolNo());
				tLOPRTManagerSchema.setStandbyFlag5(tExeSQL.getOneValue(sqlbv2));
				tLOPRTManagerSchema.setStandbyFlag6(tLJSGetSchema.getGetDate());
				tLOPRTManagerSet.add(tLOPRTManagerSchema);			 
			}
			   //日志监控,过程监控        
	    	PubFun.logTrack(tGI,tLJSGetSchema.getGetNoticeNo(),"生成催付通知书打印数据");
		}		
	...
	public LJSGetSet getLJSGet(LJSGetDrawSet aLJSGetDrawSet) {
		...
		for (int k = 1; k <= aLJSGetDrawSet.size(); k++) {
			...
			tLJSGetSchema.setManageCom(tGI.ManageCom);
...
//	 领取时给付校验
	private boolean checkDataToGet(LCGetSchema aLCGetSchema) {
		...
		sqlbv3.sql(strSQL);
		sqlbv3.put("polno", aLCGetSchema.getPolNo());
		sqlbv3.put("dutycode", aLCGetSchema.getDutyCode());
		sqlbv3.put("getdutycode", aLCGetSchema.getGetDutyCode());
		sqlbv3.put("getdutykind", aLCGetSchema.getGetDutyKind());
		logger.debug("strSQL:" + strSQL);
...
//		}
	 ...
	 * @return
	 */
	private String initCondGet(GlobalInput tGI,SQLwithBindVariables sqlbv) {
		String aSql = "select * from LCGet where 1 = 1";
		logger.debug("inLCGetSchema.getManageCom:"+inLCGetSchema.getManageCom() +"    mManageCom:"+tGI.ManageCom);
		...
		if ((inLCGetSchema.getManageCom() == null)
			...
			sqlbv.put("ManageCom", tGI.ManageCom);
		} else {
			aSql = aSql + " and ManageCom like concat('?ManageCom?','%')";

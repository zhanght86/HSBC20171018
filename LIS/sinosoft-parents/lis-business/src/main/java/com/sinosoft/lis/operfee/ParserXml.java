...
{
private static Logger logger = Logger.getLogger(ParserXml.class);
    ...
    public void setPayType(String tPayType)
    {
    	mPayType = tPayType;
    }
    ...
    //setConfigFilePathName
    // 设置配置文件名
    public void setConfigFilePathName(String strConfigFileName)
    {
        ...
        mConfigFileName = strConfigFileName;
    }

    // 设置文件名
    public void setFilePathName(String strFileName)
    {
        File file = new File(strFileName);

        if (!file.exists())
        {
            buildError("setFileName", "指定的路径" + strFileName + "不存在！");
        }
    ...
    //准备要插入数据表的数据，准备要检验的数据
    public boolean insertIntoPhyTable(Element tXMLTargetRoot, VData tVData)
    {
        String strFileName = "";
        int nCount = 0;
            ...
            try
            {
                         ...
                         nSheetIndex++)
                {
                             ...
                             nPartIndex++)
                    {
                        ...
                        String tPhysicalTableAttributeValue = tPartElement.getAttributeValue("physicaltable");
                        ...
                        {
                            //进入数据较验模块
                            String tableName = "LCPol";
                            String mOtherSQL = " and appflag='1' and payintv=-1 and paytodate<payenddate";

                            VData mOtherSQLVData = new VData();
                            mOtherSQLVData.add(mOtherSQL);

                            //if(checkData(tPartElement,tPhysicalTableAttributeValue,tVData)==false)
                            //检查从模板中传入的且相关xml节点的参数
                            if (checkData(tPartElement, tableName, tVData,
                                              mOtherSQLVData) == false)
                            {
                                CError tError = new CError();
                                tError.moduleName = "ParseXml";
                                tError.functionName = "checkData";
                                tError.errorMessage = "指定团单下不存在该个单;"+"错误行数在第"+ErrorRow+"行;";
                                this.mErrors.addOneError(tError);

                                return false;
                            }

                            String msql ="select max(riskcode) from lcpol where grpcontno='" +
                                "?mGrpPolNo?" + "'";
                            SQLwithBindVariables sqlbv = new SQLwithBindVariables();
                            sqlbv.sql(msql);
                            sqlbv.put("mGrpPolNo", mGrpPolNo);
                            ExeSQL mExeSQL = new ExeSQL();
                            String mvalue = mExeSQL.getOneValue(sqlbv); //得到险种编码

                            //检查dutycode
                            tableName = "LMRiskDuty";

                            VData mVdata1 = new VData();
                            mVdata1.add("dutycode");
                            mOtherSQL = " and riskcode='" + mvalue + "' ";

                            VData mOtherSQLVData1 = new VData();
                            mOtherSQLVData1.add(mOtherSQL);

                            if (checkData(tPartElement, tableName, mVdata1,
                                              mOtherSQLVData1) == false)
                            {
                                CError tError = new CError();
                                tError.moduleName = "ParseXml";
                                tError.functionName = "checkData";
                                tError.errorMessage = "责任编码数据检查出错;"+"错误行数在第"+ErrorRow+"行;";
                                this.mErrors.addOneError(tError);
                            ...
                            //检查payplancode
                            tableName = "lmdutypayrela";

                            VData mVdata2 = new VData();
                            mVdata2.add("dutycode");
                            mVdata2.add("payplancode");
                            mOtherSQL = "";

                            VData mOtherSQLVData2 = new VData();
                            mOtherSQLVData2.add(mOtherSQL);

                            if (checkData(tPartElement, tableName, mVdata2,
                                              mOtherSQLVData2) == false)
                            {
                                CError tError = new CError();
                                tError.moduleName = "ParseXml";
                                tError.functionName = "checkData";
                                tError.errorMessage = "缴费计划编码数据检查出错;"+"错误行数在第"+ErrorRow+"行;";
                                this.mErrors.addOneError(tError);
                            ...
                            VData mVdata3 = new VData();
                            mVdata3.add("SumActuPayMoney");
                            mVdata3.add("InputFlag");
                            mOtherSQL = ">0";

                            String inputFlag = " in (0,1)";
                            VData mOtherSQLVData3 = new VData();
                            mOtherSQLVData3.add(mOtherSQL);
                            mOtherSQLVData3.add(inputFlag);

                            if (checkData(tPartElement, tableName, mVdata3,
                                              mOtherSQLVData3) == false)
                            {
                                CError tError = new CError();
                                tError.moduleName = "ParseXml";
                                tError.functionName = "checkData";
                                tError.errorMessage = "金额<=0,保存标志不等于0或1;"+"错误行数在第"+ErrorRow+"行;";
                                this.mErrors.addOneError(tError);
    ...
    //增加数据校验功能(将xml的节点传递到一个容器中)tOtherSQLVData为附带的条件
    public boolean checkData(Element tPartElement, String tTableName,
                             VData tVData, VData tOtherSQLVData)
    {
        ...
        for (int nRowIndex = 0; nRowIndex < tRowNum; nRowIndex++)
        {
          errorRow++;
          ErrorRow=errorRow;
            if (!tTableName.equals(""))
            {
                TransferData mTransferData = new TransferData();
                Element tRowElement = (Element) tRowList.get(nRowIndex);
                String tselectSQL = "select count(*) from " + tTableName +
                                    " where ";
                String tValuesSQL = "";
                ...
                for (int nAttributeIndex = 0;
                         ...
                         nAttributeIndex++)
                {
                    org.jdom.Attribute tAttribute = (org.jdom.Attribute) tRowAttributeList.get(nAttributeIndex);
                    String tAttributeName = tAttribute.getName();

                    //            logger.debug("tAttributeName:"+tAttributeName);
                    String tAttributeValue = tAttribute.getValue();

                    //            logger.debug("tAttributeValue:"+tAttributeValue);
                    mTransferData.setNameAndValue(tAttributeName,
                                                  tAttributeValue);
                }

                int checkSize = tVData.size();
                logger.debug("传入的要检查的字段个数（不包括附带条件）:" + checkSize);
                SQLwithBindVariables sqlbva = new SQLwithBindVariables();
                for (int i = 0; i < checkSize; i++)
                {
                    String checkAttributeName = (String) tVData.getObject(i);

                    for (int j = 0; j < tAttributeListNum; j++)
                    {
                        if ((checkAttributeName.toLowerCase() == mTransferData.getValueNames()
                                                                                  .get(j)
                                                                                  .toString()
                                                                                  .toLowerCase()) ||
                                checkAttributeName.toLowerCase().equals(mTransferData.getValueNames()
                                                                                         .get(j)
                                                                                         .toString()
                                                                                         .toLowerCase()))
                        {
                            if (i == 0)
                            {
                                tValuesSQL = tValuesSQL + " " +
                                             checkAttributeName + "=" +" '"+"?var1?"+j+ "' ";
                                sqlbva.put("var1"+j, (String)mTransferData.getValueByName(mTransferData.getValueNames().get(j).toString()));
                            }
                            else
                            {
                                tValuesSQL = tValuesSQL + " and " +
                                             checkAttributeName + "=" +" '"+"?var2?"+j+"' ";
                                sqlbva.put("var2"+j, (String)mTransferData.getValueByName(mTransferData.getValueNames().get(j).toString()));
                            }
                        }
                    }
                }
                ...
                //如果有具体的表名则只能带一个附加条件
                String tAllSQL = tselectSQL + tValuesSQL +
                                 tOtherSQLVData.get(0);
                logger.debug("要执行的sql：" + tAllSQL);
                
                sqlbva.sql(tAllSQL);
                ExeSQL tExeSQL = new ExeSQL();
                String sign = tExeSQL.getOneValue(sqlbva);

                if ((sign == "0") || sign.equals("0"))
                {
//                    CError tError = new CError();
//                    tError.moduleName = "ParseXml";
//                    tError.functionName = "checkData";
//                    tError.errorMessage = "要导入的团单下不存在该个单;错误行数在第"+errorRow+"行;";
//                    this.mErrors.addOneError(tError);
            ...
            {
                //表名为空则判断字段取值是否符合条件，这时tOtherSQL为条件(单个字段的校验)
                TransferData mTransferData = new TransferData();
                Element tRowElement = (Element) tRowList.get(nRowIndex);
                String tselectSQL = "select count(*) from dual " + " where ";
                String tValuesSQL = "";
...
//                    logger.debug("tAttributeValue:" + tAttributeValue);
                ...
                for (int i = 0; i < checkSize; i++)
                {
                    ...
                    for (int j = 0; j < tAttributeListNum; j++)
                    {
                                                                                         ...
                                                                                         .toLowerCase()))
                        {
                            ...
                            {
                                tValuesSQL = tValuesSQL + " " +
                                             mTransferData.getValueByName(mTransferData.getValueNames()
                                                                                       .get(j)
                                                                                       .toString()) +
                                             " " +
                                             (String) tOtherSQLVData.get(i);
                            }
                            else
                            {
                                tValuesSQL = tValuesSQL + " and " +
                ...
                String tAllSQL = tselectSQL + tValuesSQL;
                logger.debug("要执行的校验：" + tAllSQL);
                ExeSQL tExeSQL = new ExeSQL();
                String sign = tExeSQL.getOneValue(tAllSQL);
...
//                    this.mErrors.addOneError(tError);
                                   ...
                                   MMap tMap)
    {
        ...
        for (int nRowIndex = 0; nRowIndex < tRowNum; nRowIndex++)
        {
            ...
            for (int nAttributeIndex = 0; nAttributeIndex < tAttributeListNum;
                     nAttributeIndex++)
            {
                org.jdom.Attribute tAttribute = (org.jdom.Attribute) tRowAttributeList.get(nAttributeIndex);
                String tAttributeName = tAttribute.getName();
    ...
    private boolean loadConfigXML()
    {
        try
        {
            FileInputStream tFileInputStream = new FileInputStream(mConfigFileName +
                                                                   mConfigXmlFileName +
                                                                   ".xml");
    ...
    private String[] getParseTargetPositon(String tSrcAttributeValueString)
    {
        try
        {
            ...
            tColName = tStrRowIndex.trim().substring(i + 1,
                                                     tStrRowIndex.length());
    ...
    /**
      ...
      *
     */
    private String getExcelValue(int tSheetIndex, int tRowIndex, int tColIndex)
                          throws Exception
    {
                                        ...
                                        TransferData tempTransferData)
    {
        try
        {
            ...
            int a = Integer.parseInt((String) tTransferData.getValueByName("PARAM0"));
            int b = Integer.parseInt((String) tTransferData.getValueByName("PARAM1"));
                                     ...
                                     String tParamsAttributeValue)
    {
        ...
        else if (tTypeAttributeValue.trim().equals("2")) //属性src值是通过计算取得
        {
                    ...
                    !tParamsAttributeValue.trim().equals(""))
            {
                        ...
                        (tParamsName.length > 0) && (tParamsType.length > 0))
                {
                    for (int i = 0; i < tParamsName.length; i++)
                    {
                        ...
                        {
                            tParamsValue = getMiddleParamAttributeValue(tParamsName[i],
                                                                        tParamsType[i],
                                                                        tTransferData,
                                                                        tempTransferData,
                                                                        tSheetIndex,
            ...
            if (tVector != null)
            {
                ...
                {
                    String tName = (String) tVector.get(i);
                    String tValue = (String) tempTransferData.getValueByName((Object) tName)
                                                             .toString();
                    tPubCalculator.addBasicFactor(tName, tValue);
                }
            }
            ...
            if (tVector != null)
            {
                ...
                {
                    String tName = (String) tVector.get(i);
                    String tValue = (String) temppTransferData.getValueByName((Object) tName)
                                                              .toString();
                    tPubCalculator.addBasicFactor(tName, tValue);
                }
            }
            ...
            logger.debug("计算元素的Src属性值SQL : " + strSQL);
        ...
        {
            String tString = (String) tTransferData.getValueByName(tSrcAttributeValue);
                                     ...
                                     String tParamsAttributeValue)
    {
        ...
        else if (tTypeAttributeValue.trim().equals("2")) //属性src值是通过计算取得
        {
                    ...
                    !tParamsAttributeValue.trim().equals(""))
            {
                        ...
                        (tParamsName.length > 0) && (tParamsType.length > 0))
                {
                    for (int i = 0; i < tParamsName.length; i++)
                    {
                        ...
                        {
                            tParamsValue = getTargetParamAttributeValue(tMiddleRowList,
                                                                        tParamsName[i],
                                                                        tParamsType[i],
                                                                        tTransferData,
                                                                        tempTransferData,
            ...
            String strSQL = tPubCalculator.calculate();
            logger.debug("计算ColConfig元素的Field元素的Src属性值SQL : " + strSQL);
        ...
        else if (tTypeAttributeValue.trim().equals("5")) //属性src值是中间结果集合中直接取得（如Row(2).FirstSubjectNo）
        {
            ...
            String tString = ((Element) tMiddleRowList.get(tMiddleRowIndex + 1)).getAttributeValue(tSubString[1]);
                                    ...
                                    int tSheetIndex, int tPartIndex)
    {
        ...
        else if (tTypeAttributeValue.trim().equals("2")) //属性src值是通过计算取得
        {
                    ...
                    !tParamsAttributeValue.trim().equals(""))
            {
                        ...
                        (tParamsName.length > 0) && (tParamsType.length > 0))
                {
                    for (int i = 0; i < tParamsName.length; i++)
                    {
                        ...
                        if (tParamsType[i].equals("1"))
                        {
                            tParamsValue = getExcelParamAttributeValue(tParamsName[i],
                                                                       tParamsType[i],
                                                                       tTransferData,
                                                                       tSheetIndex);
            ...
            String strSQL = tPubCalculator.calculate();
            logger.debug("计算PARAMS中的Src属性SQL : " + strSQL);
                     ...
                     nSheetIndex++)
            {
                         ...
                         nPartIndex++)
                {
                    ...
                    if (tPartName.trim().toUpperCase().equals("PART"))
                    {
                        ...
                        Element tParamsElement = (Element) (((List) ((((Element) (((List) tPartElement.getChildren()).get(0)))).getChildren())).get(0));
                        String tParamsElementName = tParamsElement.getName();
                        List tParamsFieldList = (List) ((tParamsElement).getChildren());
                        int tParamsFieldNum = tParamsFieldList.size();

                        //////////////////////////////////////////////////////////////////////////////
                        //生成参数信息
                        //生成系统级参数-增加操作员信息（从页面提取）
                        nParamsValueSet.setNameAndValue("SYS_OPERATOR",
                                                        mOperator);

                        //生成参数节点
                        Element tSysParamRootElement = new Element("PARAM");
                        tSysParamRootElement.addAttribute("name", "SYS_OPERATOR");
                        tSysParamRootElement.addAttribute("src", mOperator);
                        ...
                        ///////////////////////////////////////////////////////////////////////////////                        
                        //生成系统级参数-增加团单号（从页面提取） 
                        nParamsValueSet.setNameAndValue("SYS_GRPPOLNO",
                                                        mGrpPolNo);

                        //生成参数节点
                        Element tSysParamRootElement1 = new Element("PARAM");
                        tSysParamRootElement1.addAttribute("name",
                                                           "SYS_GRPPOLNO");
                        tSysParamRootElement1.addAttribute("src", mGrpPolNo);
                        
                        //添加一系统级参数节点
                        Element tParamsRootElement1 = new Element("PARAMS");
                        tParamsRootElement1.addContent(tSysParamRootElement1);
                        
                        //add by liuqh 2009-01-14
                        //生成系统级参数-增加团体合同号
                        nParamsValueSet.setNameAndValue("SYS_GRPCONTNO",
                                                        mGrpContNo);
                        
                        //生成参数节点
                        Element tSysParamRootElement2 = new Element("PARAM");
                        tSysParamRootElement2.addAttribute("name",
                                                           "SYS_GRPCONTNO");
                        tSysParamRootElement2.addAttribute("src", mGrpContNo);
                        Element tParamsRootElement2 = new Element("PARAMS");
                        tParamsRootElement2.addContent(tSysParamRootElement2);
                        
                        
                        nParamsValueSet.setNameAndValue("SYS_PAYTYPE",
                        		mPayType);

						//生成参数节点
						Element tSysParamRootElement3 = new Element("PARAM");
						tSysParamRootElement3.addAttribute("name",
						                                   "SYS_PAYTYPE");
						tSysParamRootElement3.addAttribute("src", mPayType);
						Element tParamsRootElement3 = new Element("PARAMS");
						tParamsRootElement3.addContent(tSysParamRootElement3);
                                 ...
                                 nParamsFieldIndex++)
                        {
                            ...
                            String tStrMaxRow = (String) mExcelSheetMaxRowSet.getValueByName(tSheetMaxRow);
                            nMaxRow = Integer.parseInt(tStrMaxRow);

                            int nEndRow = nMaxRow;
                            int nStartRow = 0;

                            Element tFieldElement = ((Element) tParamsFieldList.get(nParamsFieldIndex));
                            String tFieldName = tFieldElement.getName();

                            //计算出所有参数
                            String tTypeAttributeValue = tFieldElement.getAttributeValue("type");
                            String tSrcAttributeValue = tFieldElement.getAttributeValue("src");
                            String tNameAttributeValue = tFieldElement.getAttributeValue("name");
                            String tParamsAttributeValue = tFieldElement.getAttributeValue("params");
                            String tRealSrcAttributeValue = getExcelSrcValue(nParamsValueSet,
                                                                             tSrcAttributeValue,
                                                                             tTypeAttributeValue,
                        ...
                        int tStartRow = Integer.parseInt((String) nParamsValueSet.getValueByName("PARAM0"));
                        int tEndRow = Integer.parseInt((String) nParamsValueSet.getValueByName("PARAM1"));
                        Element tColConfigElement = (Element) (((List) ((((Element) (((List) tPartElement.getChildren()).get(0)))).getChildren())).get(1));
                        String tColConfigName = tColConfigElement.getName();
                        List tColConfigFieldList = (List) (tColConfigElement.getChildren());
                                 ...
                                 nRowIndex++)
                        {
                                     ...
                                     nColConfigFieldIndex++)
                            {
                                ...
                                Element tFieldElement = ((Element) tColConfigFieldList.get(nColConfigFieldIndex));
                                String tFieldName = tFieldElement.getName();
                                String tTypeAttributeValue = tFieldElement.getAttributeValue("type");
                                String tNameAttributeValue = tFieldElement.getAttributeValue("name");
                                String tSrcAttributeValue = tFieldElement.getAttributeValue("src");
                                ...
                                String tTrueSrcAttributeValue = getMiddleSrcValue(nParamsValueSet,
                                                                                  tempParamsValueSet,
                                                                                  nSheetIndex,
                                                                                  nPartIndex,
                                                                                  nRowIndex,
                                ...
                                tOneRowRootElement.addAttribute(tNameAttributeValue,
                                                                tTrueSrcAttributeValue);
                                ...
                                tempParamsValueSet.setNameAndValue(tNameAttributeValue,
                                                                   tTrueSrcAttributeValue);
                            }
    ...
    private boolean createTargetXML()
    {
        ...
        try
        {
                     ...
                     nSheetIndex++)
            {
                //XML配置模板描述中Sheet元素本身
                Element tSheetElement = ((Element) tSheetList.get(nSheetIndex));
                Element tMiddleSheetElement = ((Element) tMiddleSheetList.get(nSheetIndex));
                         ...
                         nPartIndex++)
                {
                    ...
                    //XML配置模板描述中Sheet元素内部Part元素本身校验
                    Element tPartElement = ((Element) tPartList.get(nPartIndex));
                    Element tMiddlePartElement = ((Element) tMiddlePartList.get(nPartIndex));
                    ...
                    tPartRootElement.addAttribute("physicaltable",
                                                  String.valueOf(tPhysicalTableAttributeValue));
                    ...
                    if (tPartName.trim().toUpperCase().equals("PART"))
                    {
                        ...
                        Element tDataDestinationSetsElement = (Element) ((((Element) (((List) tPartElement.getChildren()).get(1)))));
                        String tDataDestinationSetsElementElementName = tDataDestinationSetsElement.getName();

                        //预先取得中间结果集合中的直接参数Params到nParamsValueSet
                        Element tParamsElement = (Element) ((((Element) (((List) tMiddlePartElement.getChildren()).get(0)))));
                        ...
                        for (int nParamFieldIndex = 0;
                                 ...
                                 nParamFieldIndex++)
                        {
                            Element tFieldElement = ((Element) tParamList.get(nParamFieldIndex));
                            String tFieldName = tFieldElement.getName();
                            String tNameAttributeValue = tFieldElement.getAttributeValue("name");
                            String tFieldContent = tFieldElement.getAttributeValue("src");
                            nParamsValueSet.setNameAndValue(tNameAttributeValue,
                                                            tFieldContent);
                        }

                        //更据XML配置模板描述中DataDestinationSet中的Row元素的描述逐一生成行数据
                        List tRowList = (List) ((tDataDestinationSetsElement).getChildren());
                        List tMiddleRowList = (List) tMiddlePartElement.getChildren(); //从第二个孩子节点开始是Row节点
                        int tRowNum = tRowList.size();
                        int tTotalRowCount = 0;

                        for (int nRowIndex = 0; nRowIndex < tRowNum;
                                 nRowIndex++)
                        {
                            //XML配置模板描述中DataDestinationSet中的Row元素的校验只针对本身校验，而不对其内部再进行校验
                            //XML配置模板描述中DataDestinationSet中的Row元素本身校验
                            Element tDataDestinationSetRowElement = ((Element) tRowList.get(nRowIndex));
                            String tDataDestinationSetRowName = tDataDestinationSetRowElement.getName();
                            String tTypeAttributeValue = tDataDestinationSetRowElement.getAttributeValue("type");

                            if (tTypeAttributeValue.trim().equals("1")) //Row值直接到中间数据源中取得的连续行ROW(startRowIndex,endRowIndex)
                            {
                                String tFieldAttributeValue = tDataDestinationSetRowElement.getAttributeValue("src");

                                String[] tSubString = getParseMiddlePosition(tFieldAttributeValue);
                                int tStartRowIndex = Integer.parseInt(tSubString[0]);
                                int tEndRowIndex = Integer.parseInt(tSubString[1]);
                                ...
                                for (; tStartRowIndex <= tEndRowIndex;
                                         tStartRowIndex++)
                                {
                                    Element tRowElement = (Element) (tMiddleRowList.get(tStartRowIndex +
                                                                                        1));

                                    Element tRowRootElement = new Element("ROW");
                                    List tList = (List) tRowElement.getAttributes();
                            ...
                            {
                                List tDataDestinationSetRowFieldList = tDataDestinationSetRowElement.getChildren();
                                int tDataDestinationSetRowFieldListNum = tDataDestinationSetRowFieldList.size();
                                ...
                                //for(int nRowFieldIndex=0;nRowFieldIndex<tDataDestinationSetRowFieldListNum;nRowFieldIndex++)
                                {
                                    ...
                                    for (int nFieldIndex = 0;
                                             nFieldIndex < tDataDestinationSetRowFieldListNum;
                                             nFieldIndex++)
                                    {
                                        //XML配置模板描述中Row中的Field元素本身校验
                                        Element tFieldElement = ((Element) tDataDestinationSetRowFieldList.get(nFieldIndex));
                                        String tFieldName = tFieldElement.getName();
                                        String tTypeFieldAttributeValue = tFieldElement.getAttributeValue("type");
                                        String tNameFieldAttributeValue = tFieldElement.getAttributeValue("name");
                                        String tSrcFieldAttributeValue = tFieldElement.getAttributeValue("src");
                                        String tParamsFieldAttributeValue = tFieldElement.getAttributeValue("params");
                                        ...
                                        //生成该Field节点的根节点及属性name
                                        String tTrueSrcAttributeValue = getTargetSrcValue(tMiddleRowList,
                                                                                          nParamsValueSet,
                                                                                          tempParamsValueSet,
                                                                                          nSheetIndex,
                                                                                          nPartIndex,
                                        ...
                                        //添加一个Field节点的到父节点Row某以属性中
                                        tOneRowRootElement.addAttribute(tNameFieldAttributeValue,
                                                                        tTrueSrcAttributeValue);
                                        ...
                                        //               tempParamsValueSet.setNameAndValue("COL"+nColConfigFieldIndex,tTrueSrcAttributeValue) ;
                                        tempParamsValueSet.setNameAndValue(tNameFieldAttributeValue,
                                                                           tTrueSrcAttributeValue);
                                    }
                                       ...
                                       int tColIndex) throws Exception
    {
        ...
        String tStrMaxRow = (String) mExcelSheetMaxRowSet.getValueByName(tStrSheetIndex);
        String tStrMaxCol = (String) mExcelSheetMaxColSet.getValueByName(tStrSheetIndex);
     ...
     *
     */
    private boolean checkExcelParamsAttributeDesc(String tParamsSrcAttributeValue,
                                                  TransferData tTransferData,
                                                  int tSheetIndex)
    {
        try
        {
            ...
            for (int i = 0; i < tSubString.length; i++)
            {
                ...
                if (tType == 3) //直接引用前驱已定义的param
                {
                    ...
                    String tString = (String) tTransferData.getValueByName(tParam);
    ...
    private boolean checkMiddleParamsAttributeDesc(TransferData tTransferData,
                                                   ...
                                                   int tSheetIndex,
                                                   int tRowIndex,
                                                   String tParamsSrcAttributeValue)
                                            throws Exception
    {
        try
        {
            ...
            for (int i = 0; i < tSubString.length; i++)
            {
                ...
                if (tType == 4) //直接引用前驱已定义的当前行辅助列参数
                {
                    ...
                    String tString = (String) tempTransferData.getValueByName(tParam);
                                                ...
                                                int tSheetIndex, int tRowIndex)
    {
        try
        {
            ...
            {
                tParamValue = (String) tempTransferData.getValueByName(tParamName);
     ...
     *
     */
    private String[] getSpliter(String tString, String tSpliter)
                         throws Exception
    {
        String[] tSpliterString = null;
        StringTokenizer tStringTokenizer = new StringTokenizer(tString, tSpliter);
                                     ...
                                     throws Exception
    {
                ...
                !tDataSourceSetName.trim().toUpperCase().equals("DATASOURCESET"))
        {
            buildError("checkMatch",
                       ...
                       nPartIndex + "个Part元素的DataSourceSet元素名描述有误");
                 ...
                 nParamsFieldIndex++)
        {
            ...
            if (nParamsFieldIndex == 0)
            {
                ...
                String tStrNameAttributeValue = tFieldElement.getAttributeValue("name");
                ...
                if (nStartRow > nMaxRow)
                {
                    buildError("checkMatch",
                               "XML配置模板描述中第" + nSheetIndex + "Sheet元素中的第" +
                               nPartIndex +
                               "个Part元素的DataSourceSet元素中第一元素:Params中的第一个Feild元素（指定起始行元素）的Scr属性值的描述行数大于该Sheet的最大行数");
                ...
                if (tTypeAttributeValue.trim().equals("0"))
                {
                    ...
                    if (nEndRow > nMaxRow)
                    {
                        buildError("checkMatch",
                                   "XML配置模板描述中第" + nSheetIndex + "Sheet元素中的第" +
                                   nPartIndex +
                                   "个Part元素的DataSourceSet元素中第一元素:Params中的第二个Feild元素（指定终止行元素）的Scr属性值的描述行数大于该Sheet的最大行数");
            ...
            else
            {
                ...
                else if (tTypeAttributeValue.trim().equals("2"))
                {
                    ...
                    {
                        if (!checkExcelParamsAttributeDesc(tParamsAttributeValue,
                                                               nParamsValueSet,
                                                               nSheetIndex))
                        {
                            buildError("checkMatch",
                ...
                //记录前驱已定义的参数，以便检验后续参数引用的校验
                nParamsValueSet.setNameAndValue(tStrNameAttributeValue, "DEFAUT");
            }
        }
        ...
        {
            //XML配置模板描述中Row中的Field元素本身校验
            Element tFieldElement = ((Element) tColConfigFieldList.get(nRowFieldIndex));
            String tFieldName = tFieldElement.getName();
    ...
    private boolean checkDataDestinationSetMatch(int nSheetIndex,
                                                 int nPartIndex,
                                                 int nDestSheetIndex,
                                                 Element tDataDestinationSetElement,
                                                 TransferData nParamsValueSet)
                                          throws Exception
    {
        ...
        if ((tDataDestinationSetName == null) ||
                !tDataDestinationSetName.trim().toUpperCase().equals("DATADESTINATIONSET"))
        {
            buildError("checkMatch",
                       "XML配置模板描述中第" + nSheetIndex + "个Sheet元素中的第" +
                       nPartIndex + "个Part元素的DataDestinationSet元素名描述有误");

            return false;
        }

        //XML配置模板描述中该Sheet元素中内部该Part元素中的第二孩子元素：DataDestinationSet的内部元素的校验
        List tDataDestinationSetChildenList = tDataDestinationSetElement.getChildren();
        ...
        if (tDataDestinationSetChildenNum < 1)
        {
            buildError("checkMatch",
                       "XML配置模板描述中第" + nSheetIndex + "个Sheet元素中的第" +
                       nPartIndex + "个Part元素的DataDestinationSet元素缺少ROW元素的描述");
                 ...
                 nRowIndex++)
        {
            //XML配置模板描述中DataDestinationSet中的Row元素的校验只针对本身校验，而不对其内部再进行校验
            //XML配置模板描述中DataDestinationSet中的Row元素本身校验
            Element tDataDestinationSetRowElement = ((Element) tDataDestinationSetChildenList.get(nRowIndex));
            String tDataDestinationSetRowName = tDataDestinationSetRowElement.getName();
            ...
            if (tTypeAttributeValue.trim().equals("1")) //Row值直接到物理数据源Excel表的中间级数据中取得的连续行ROW(startRowIndex,endRowIndex)
            {
                ...
                int tStarRowIndex = Integer.parseInt((String) nParamsValueSet.getValueByName("PARAM0"));
                int tEndRowIndex = Integer.parseInt((String) nParamsValueSet.getValueByName("PARAM1"));
                                             ...
                                             tEndRowIndex - tStarRowIndex))
                {
                    buildError("checkMatch",
                               ...
                               nPartIndex + "个Part元素的DataDestinationSet元素中第" +
                               nRowIndex +
                               "个Row元素Src属性值描述的参数取值不符和中间Row结果集合的范围");
            ...
            else if (tTypeAttributeValue.trim().equals("2")) //Row值通过计算到物理数据源Excel表的中间级数据中获得
            {
                ...
                for (int nRowFieldIndex = 0;
                         nRowFieldIndex < tDataDestinationSetRowFieldListNum;
                         nRowFieldIndex++)
                {
                    //XML配置模板描述中Row中的Field元素本身校验
                    Element tFieldElement = ((Element) tDataDestinationSetRowFieldList.get(nRowFieldIndex));
                    String tFieldName = tFieldElement.getName();
                    ...
                    String tRowTypeAttributeValue = tFieldElement.getAttributeValue("type");
                    String tRowNameAttributeValue = tFieldElement.getAttributeValue("name");
                    ...
                    nCurrentRowParamsValueSet.setNameAndValue(tRowNameAttributeValue,
                                                              "DEFAUT");
                }
            }
            else
    ...
    private boolean checkMatch() throws Exception
    {
        ...
        logger.debug("sheet的个数：" + tExcelSheetNum);
                     ...
                     nSheetIndex++)
            {
                         ...
                         nPartIndex++)
                {
                    ...
                    if (tPartName.trim().toUpperCase().equals("PART"))
                    {
                        ...
                        Element tDataSourceSetElement = ((Element) tPartChildenList.get(0));

                        if (!checkDataSourceSetMatch(nSheetIndex, nPartIndex,
                                                         nDestIndex,
                                                         tDataSourceSetElement,
                                                         nParamsValueSet))
                        {
                            return false;
                        }
                        ...
                        Element tDataDestinationSetElement = ((Element) tPartChildenList.get(1));
                ...
                logger.debug("XML配置模板检第" + nSheetIndex + "Sheet检验成功结束");
            }

            logger.debug("XML配置模板检验成功结束");
        }
        catch (Exception e)
        {
            System.err.println("XML配置数据读入失败:" + e.getMessage());

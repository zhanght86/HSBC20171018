 ...
 * @author pst
 * @version 1.0
 * @CreateDate：2009-04-11 清单名称：保单迁移清单
 */

public class EdorPMBillPrintBL implements BqBill {
private static Logger logger = Logger.getLogger(EdorPMBillPrintBL.class);
	...
	public static final String VTS_NAME = "EdorPMBill.vts";
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (!mOperate.equals("PRINT")) {
			...
			tError.moduleName = "EdorPMBillPrintBL";
	...
	private boolean preparePrintData() {
		...
		if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
			strsql = " select a.managecom,"
					...
					+ " (select n.indate"
					+ " from lpmove n"
					+ " where n.edorno = a.edorno"
					+ " and n.contnoold = a.contno and rownum=1),"
					+ " (select concat(n.managecomold , n.oldconame)"
					+ " from lpmove n"
					+ " where n.edorno = a.edorno"
					+ " and n.contnoold = a.contno and rownum=1),"
					+ " (select concat(n.managecomnew , n.newconame)"
					+ " from lpmove n"
					+ " where n.edorno = a.edorno"
					+ " and n.contnoold = a.contno and rownum=1),"
					+ " a.operator,"
			...
			strsql = " select a.managecom,"
					...
					+ " from lcpol d"
					+ " where d.contno = a.contno"
					+ " and d.polno = d.mainpolno limit 0,1),"
					+ " (select prem"
					+ " from lcpol d"
					+ " where d.contno = a.contno"
					+ " and d.polno = d.mainpolno limit 0,1),"
					...
					+ " from lpmove n"
					+ " where n.edorno = a.edorno"
					+ " and n.contnoold = a.contno limit 0,1),"
					+ " (select concat(n.managecomold , n.oldconame)"
					+ " from lpmove n"
					+ " where n.edorno = a.edorno"
					+ " and n.contnoold = a.contno limit 0,1),"
		...
		//tXmlExport.outputDocumentToFile("c:\\XML\\", "EdorPMBill.vts");
		mResult.clear();

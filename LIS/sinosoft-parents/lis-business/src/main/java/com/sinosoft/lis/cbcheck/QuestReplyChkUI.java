 ...
 * @version 1.0
 */
public class QuestReplyChkUI implements BusinessService {
private static Logger logger = Logger.getLogger(QuestReplyChkUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		QuestReplyChkBL tQuestReplyChkBL = new QuestReplyChkBL();

		logger.debug("---QuestReplyChkBL UI BEGIN---");

		if (tQuestReplyChkBL.submitData(cInputData, mOperate) == false) {
			// @@错误处理
			this.mErrors.copyAllErrors(tQuestReplyChkBL.mErrors);

			CError tError = new CError();
			tError.moduleName = "QuestReplyChkUI";
	...
	public static void main(String[] args) {
		...
		tLCPENoticeSchema.setContNo("86110020030110003579");
		...
		tLCIssuePolSchema.setReplyResult("OK!!");
		...
		QuestReplyChkUI ui = new QuestReplyChkUI();

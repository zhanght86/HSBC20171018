 ...
 * @version 3.0
 */
public class RIAttachMainEdorProcess implements RIProcessType {
	...
	public RIAttachMainEdorProcess() {
	}
	...
	public boolean dealData(RIPolRecordSchema aRIPolRecordSchema,
		...
		try {
			...
			// else if ("CM".equals(aRIPolRecordSchema.getFeeOperationType())) {
			// // 客户资料变更
			// if (!DealEdorCM(aRIPolRecordSchema, aRIIncomeCompanySet,
			// aRIDivisionLineDefSet, aRIRiskDivideSet)) {
			// return false;
			// }
			// }
	 ...
	 * 保全CM分保处理
	 * 
	 * @param polRecordSchema
	 * @param incomeCompanySet
	 * @param divisionLineDefSet
	 * @param riskDivideSet
	 * @return
	 */
	private boolean DealEdorCM(RIPolRecordSchema polRecordSchema,
			RIIncomeCompanySet incomeCompanySet,
			RIDivisionLineDefSet divisionLineDefSet,
			RIRiskDivideSet riskDivideSet) {
		...
		try {
			...
			double AccumulateAmnt = polRecordSchema.getAccAmnt();
			double riLevel = mRIPrecept.getLevelone(divisionLineDefSet,
					polRecordSchema);
			double riLimit = mRIPrecept.getRILimit(divisionLineDefSet,
					polRecordSchema);
			if (riLevel == -1 || riLimit == -1) {
				// 计算动态限额系数时再保接口表对应字段为空(只能校验字符类型的字段)时返回-1
				this.mErrors.addOneError(mRIPrecept.mErrors.getFirstError());
				return false;
			}
			System.out.println(" eventno: " + polRecordSchema.getEventNo());
			if (AccumulateAmnt > riLimit) {
				RIPubFun tRIPubFun = new RIPubFun();
				RIRecordTraceTempSet tRIRecordTraceTempSet = mRIPrecept
						.DealAddDiv(incomeCompanySet, divisionLineDefSet,
								riskDivideSet);
				;
				if (tRIRecordTraceTempSet.size() > 0) {
					for (int i = 1; i <= tRIRecordTraceTempSet.size(); i++) {
						String serialNo = tRIPubFun.getCessRecordSerialNo();
						tRIRecordTraceTempSet.get(i).setSerialNo(serialNo);
						tRIRecordTraceTempSet.get(i).setBatchNo(
								polRecordSchema.getBatchNo());
						tRIRecordTraceTempSet.get(i).setAccumulateDefNO(
								polRecordSchema.getAccumulateDefNO());
						tRIRecordTraceTempSet.get(i).setOtherNo(
								polRecordSchema.getInsuredNo());
						tRIRecordTraceTempSet.get(i).setContNo(
								polRecordSchema.getContNo());
						tRIRecordTraceTempSet.get(i).setEventNo(
								polRecordSchema.getEventNo());
						tRIRecordTraceTempSet.get(i).setEventType(
								polRecordSchema.getEventType());
						tRIRecordTraceTempSet.get(i).setCurentAmnt(
								polRecordSchema.getRiskAmnt());
						tRIRecordTraceTempSet.get(i).setRIDate(
								polRecordSchema.getGetDate());
						tRIRecordTraceTempSet.get(i).setAddSubFlag("CM");
						tRIRecordTraceTempSet.get(i).setRiskCode(
								polRecordSchema.getRiskCode());
						tRIRecordTraceTempSet.get(i).setDutyCode(
								polRecordSchema.getDutyCode());
						...
						tRIRecordTraceTempSet.get(i)
								.setCessionAmount(mCessAmnt);
						// 分保保费、分保佣金
						if (!getCessFeeAndCessComm(
								tRIRecordTraceTempSet.get(i),
								polRecordSchema.getInsuredYear())) {
							System.out
									.println("RIStandEdorProcess.getCessFeeAndCessComm(): 取得历史分保保费、佣金出错 !!");
							return false;
						}
			...
			System.out.println("保全CM分保处理类出错：" + ex.getMessage());
			buildError("DealEdorCM", "保全CM分保处理类出错!!");
			return false;
		}
		return true;
	}
		...
		try {
			...
			for (int i = 1; i <= tRIRecordTraceTempSet.size(); i++) {
				...
				tRIRecordTraceTempSet.get(i).setEventType(
						aRIPolRecordSchema.getEventType());
				tRIRecordTraceTempSet.get(i).setCurentAmnt(
						aRIPolRecordSchema.getRiskAmnt()); // 当前风险保额
				tRIRecordTraceTempSet.get(i).setRIDate(
						aRIPolRecordSchema.getGetDate());
				tRIRecordTraceTempSet.get(i).setRiskCode(
						aRIPolRecordSchema.getRiskCode());
				...
				// 分保变化量
				tRIRecordTraceTempSet.get(i).setAmountChang(
						(aRIPolRecordSchema.getRiskAmnt() - aRIPolRecordSchema
								.getPreRiskAmnt()) * mCessRate);
				tRIRecordTraceTempSet.get(i).setCessionRate(mCessRate);
	...
	private boolean DealEdorTB(RIPolRecordSchema aRIPolRecordSchema,
		...
		for (int i = 1; i <= tRIRecordTraceTempSet.size(); i++) {
			...
			// 分保变化量
			mCessAmnt = aRIPolRecordSchema.getChRiskAmnt()-aRIPolRecordSchema.getPreChRiskAmnt();
			tRIRecordTraceTempSet.get(i).setAmountChang(mCessAmnt*mCessRate);
			tRIRecordTraceTempSet.get(i).setCessionRate(mCessRate);
			tRIRecordTraceSchema = new RIRecordTraceSchema();
			tReflections.transFields(tRIRecordTraceSchema,
					tRIRecordTraceTempSet.get(i));
			//if(mCessRate==0){
				//tRIRecordTraceTempSet.remove(tRIRecordTraceTempSet.get(i));
				//continue;
			//}
			// 保存新分保保额分保比例到内存
			if (!setRecord(tRIRecordTraceSchema)) {
	...
	private boolean getAmntChagAndCessRate(
		...
		try {
			...
			strBuffer
					.append(" select nvl(x.A2,0) from (select c.cessionrate A1,c.CessionAmount A2 from rirecordtrace c where c.serialno=(select max(b.serialno) from rirecordtrace b where b.accumulatedefno='");
			...
			if (tSSRS.MaxRow == 0) {
				...
				mCessAmnt = Double.parseDouble(tSSRS.GetText(1, 1));
			}
		} catch (Exception ex) {
	...
	private boolean getRecord(RIRecordTraceSchema riRecordTraceSchema) {
		...
		for (; i <= tRIRecordSet.size(); i++) {
			System.out.println(" ContNo: " + tRIRecordSet.get(i).getContNo()
					+ " OtherNo: " + tRIRecordSet.get(i).getOtherNo()
					+ " AccumulateDefNO: "
					+ tRIRecordSet.get(i).getAccumulateDefNO() + " AreaID: "
					+ tRIRecordSet.get(i).getAreaID() + " RiskCode: "
					+ tRIRecordSet.get(i).getRiskCode() + " CessionRate: "
		...
		if (i > tRIRecordSet.size()) {
			...
			//if (mCessRate != 0) {
				...
				riRecordSchema.setRIPreceptNo(riRecordTraceSchema
						.getRIPreceptNo());

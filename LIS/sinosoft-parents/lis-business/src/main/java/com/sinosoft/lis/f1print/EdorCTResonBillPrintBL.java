...
public class EdorCTResonBillPrintBL implements BqBill {
private static Logger logger = Logger.getLogger(EdorCTResonBillPrintBL.class);
	...
	public static final String VTS_NAME = "EdorCTReson.vts";
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (!mOperate.equals("PRINT")) {
			...
			tError.moduleName = "EdorCTResonBillPrintBL";
	...
	private boolean getInputData(VData cInputData) {
		...
		if (mEdorType != null && !"".equals(mEdorType)) {
			...
			mEdorTypeSQL = " and  a.edortype in ('CT','XT','WT')";
		}
	...
	private boolean preparePrintData() {
		...
		if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
			strsql = " select a.managecom,"
				...
				//modify by jiaqiangli 2009-11-18 协议退保原因与退保原因不一样 a.edorreason删除
				//+ " (select codename from ldcode where codetype='surrordereason' and code= a.edorreasoncode) || a.edorreason, "
				+ " (case when a.edortype in ('XT','XS') then (select codename from ldcode where codetype='xsurrordereason' and code= a.edorreasoncode) else (select codename from ldcode where codetype='surrordereason' and code= a.edorreasoncode) end), "
				...
				+ " and a.edorstate in ('0','b')" + " and a.makedate >= '"
				+ "?mStartDate?" + "'" + " and a.makedate <= '" + "?mEndDate?" + "'"
		...
		//tXmlExport.outputDocumentToFile("c:\\XML\\", "EdorCTReson.vts"); 
		mResult.clear();

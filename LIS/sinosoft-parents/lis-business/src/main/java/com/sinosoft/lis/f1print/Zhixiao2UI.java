...
public class Zhixiao2UI {
private static Logger logger = Logger.getLogger(Zhixiao2UI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			Zhixiao2BL tZhixiao2BL = new Zhixiao2BL();
			logger.debug("Start Zhixiao2 UI Submit ...");
			if (!tZhixiao2BL.submitData(vData, cOperate)) {
				if (tZhixiao2BL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tZhixiao2BL.mErrors);
					return false;
				} else {
					buildError("submitData", "Zhixiao2BL发生错误，但是没有提供详细的出错信息");
					return false;
				}
			} else {
				mResult = tZhixiao2BL.getResult();
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
			CError cError = new CError();
			cError.moduleName = "Zhixiao2UI";

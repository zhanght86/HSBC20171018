 ...
 * @version  : 1.00, 1.01
 * @date     : 2006-04-18, 2006-08-19
 * @direction: 团单保全缴费方式及期限变更确认
 ******************************************************************************/
...
public class GrpEdorFMConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(GrpEdorFMConfirmBL.class);
	// public GrpEdorFMConfirmBL() {}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (cInputData == null) {
			...
			logger.debug("\t@> GrpEdorFMConfirmBL.submitData() : 无法获取 InputData 数据！");
			return false;
		} else {
		...
		// logger.debug("\t@> GrpEdorFMConfirmBL.submitData() 结束");
		return true;
	...
	private boolean getInputData() {
		// logger.debug("\t@> GrpEdorFMConfirmBL.getInputData() 开始");
		...
		if (rGlobalInput == null) {
			...
			logger.debug("\t@> GrpEdorFMConfirmBL.getInputData() : 无法获取 GlobalInput 数据！");
			return false;
		}
		...
		if (rLPGrpEdorItemSchema == null) {
			...
			logger.debug("\t@> GrpEdorFMConfirmBL.getInputData() : 无法获取 LPGrpEdorItemSchema 数据！");
			return false;
		}
		...
		// logger.debug("\t@> GrpEdorFMConfirmBL.getInputData() 结束");
		return true;
	...
	private boolean checkData() {
		...
		// logger.debug("\t@> GrpEdorFMConfirmBL.checkData() 结束");
		return true;
	...
	private boolean dealData() {
		...
		if (tLPEdorItemSet == null || tLPEdorItemSet.size() <= 0) {
			...
			for (int i = 1; i <= tLPEdorItemSet.size(); i++) {
				...
				// 调用个单保全项目
				PEdorFMConfirmBL tPEdorFMConfirmBL = new PEdorFMConfirmBL();
				if (!tPEdorFMConfirmBL.submitData(tVData, rOperation)) {
					CError.buildErr(this, "处理提交的数据失败！");
					mErrors.copyAllErrors(tPEdorFMConfirmBL.mErrors);
					logger.debug("\t@> GrpEdorFMConfirmBL.dealData() : PEdorFMConfirmBL.submitData() 失败！");
					logger.debug("\t   错误原因 : " + mErrors.getFirstError());
					return false;
				} else {
					VData tTempVData = new VData();
					tTempVData = tPEdorFMConfirmBL.getResult();
		...
		// logger.debug("\t@> GrpEdorFMConfirmBL.dealData() 结束");
		return true;
	...
	private boolean outputData() {
		// logger.debug("\t@> GrpEdorFMConfirmBL.outputData() 开始");

		rResultData = new VData();
		rResultData.add(rMap);

		// logger.debug("\t@> GrpEdorFMConfirmBL.outputData() 结束");
		return true;

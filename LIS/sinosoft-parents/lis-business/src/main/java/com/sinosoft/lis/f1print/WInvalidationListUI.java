...
public class WInvalidationListUI implements PrintService {
private static Logger logger = Logger.getLogger(WInvalidationListUI.class);
	...
	public WInvalidationListUI() {
	}

	public static void main(String[] args) { // String tAppntNo = "0000498010";
		TransferData tTransferData = new TransferData();
		tTransferData.setNameAndValue("StartDate", "2004-07-01");
		tTransferData.setNameAndValue("EndDate", "2005-07-02");
		...
		tVData.add(tG);
		WInvalidationListUI u = new WInvalidationListUI();
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		WInvalidationListBL tWInvalidationListBL = new WInvalidationListBL();
		logger.debug("Start ConFeeF1P UI Submit ...");

		if (!tWInvalidationListBL.submitData(cInputData, "PRINT")) {
			logger.debug("真的不好意思，我没有得到任何的值");
			if (tWInvalidationListBL.mErrors.needDealError()) {
				logger.debug("你要调用我就好，我就可以把我的真挚给你");
				mErrors.copyAllErrors(tWInvalidationListBL.mErrors);
				return false;
			} else {
			...
			mResult = tWInvalidationListBL.getResult();
			return true;
		}

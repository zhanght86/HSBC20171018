 ...
 */
public class PEdorIPCancelBL implements EdorCancel
{
private static Logger logger = Logger.getLogger(PEdorIPCancelBL.class);
    ...
    private boolean dealData(){
        ...
        String stredorno = mLPEdorItemSchema.getEdorNo(); 
        //投连保全项目AR需要删除的表
        String[] tableForDel =
        {
        "lpinsuaccout", "lpinsuaccin","LPPerInvestPlan","lpinsureaccfeetrace"
         };
        for (int i = 0; i < tableForDel.length; i++)
        {
        	String delSql = "delete from  "+tableForDel[i]+" where edorno = '?stredorno?'";
        	SQLwithBindVariables sqlbv=new SQLwithBindVariables();
        	sqlbv.sql(delSql);
        	sqlbv.put("stredorno", stredorno);

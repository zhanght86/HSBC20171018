...
public class PEdorLRPrintBL implements EdorPrint {
	private static Logger logger = Logger.getLogger(PEdorLRPrintBL.class);
	...
	private boolean dealData() {
		...
		if (mLPEdorItemSchema.getGetMoney() > 0) {
			// 补发工本费信息
			xmlexport.addDisplayControl("displayLRAdd");
			mTextTag.add("GBMoney", PubFun.getChnMoney(mLPEdorItemSchema
					.getGetMoney())
					+ "(￥" + mLPEdorItemSchema.getGetMoney() + ")");

		}
		// 补发的次数
		int tLostTimes = tLPContSchema.getLostTimes();
		mTextTag.add("LostTimes", tLostTimes);

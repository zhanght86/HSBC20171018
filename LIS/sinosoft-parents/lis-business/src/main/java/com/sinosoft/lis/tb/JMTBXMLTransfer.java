...
public class JMTBXMLTransfer
{
private static Logger logger = Logger.getLogger(JMTBXMLTransfer.class);
  ...
  /**投资账户信息列表*/
  private static final String XML_LCINVEST = "LCInvest";//new
  private static final String XML_INVESTFLAG = "InvestFlag";//new
  private static final String XML_INVESTSET = "InvestSet";//new
  private static final String XML_EFFTIME = "EffTime";    //领取(起领)年龄年期标志 1字节  Y-年,A-岁
  private static final String XML_INVEST = "Invest";            //领取(起领)年龄
  private static final String XML_ACCTORISK = "AccToRisk";          //领取期间
  ...
  private static final String XML_ADDITFLAG = "AdditFlag";//new
  private static final String XML_ADDITSET = "AdditSet";//new
  private static final String XML_ADDIT = "Addit";    //领取(起领)年龄年期标志 1字节  Y-年,A-岁
  private static final String XML_ADDITTORISK = "AdditToRisk";            //领取(起领)年龄
  private static final String XML_ADDITPREM = "AdditPrem";          //领取期间
	  ...
	  String tSuccFlag = ""; //0-正常件, 1-外包方可处理的异常件（如扫描件上有一两处无法识别），2-外包方无法录入的异常件（如整个扫描件无法识别）
	    ...
	    try
	    {
	      ...
	      if("".equals(StrTool.cTrim(LCPol.getChildText(XML_MANAGECOM))))
	      {
	        ...
	        sqlbv1.sql(tSQL);
	        sqlbv1.put("DocCode", tLCPolSchema.getPrtNo());
	        tLCPolSchema.setManageCom(StrTool.cTrim(tExeSQL.getOneValue(sqlbv1)));
	      }
	      else
	      {
	    ...
	    //2009-1-21 ln add --自动垫交标志为空则默认为不垫交
	      ...
	      tBPOMissionStateSchema.setBussNoType("JM");

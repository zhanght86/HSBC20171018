...
public class LLAdvancdPaymentDayCheckBL implements PrintService {
private static Logger logger = Logger.getLogger(LLAdvancdPaymentDayCheckBL.class);
	...
	public LLAdvancdPaymentDayCheckBL() {
	...
	public static void main(String[] args) {
		...
		if (tLLAdvancdPaymentDayCheckBL.submitData(tVData, "") == false) {
			logger.debug("----------理赔日结-->理赔预付赔款日结出错---------------");
		}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("-----理赔日结-->理赔－预付赔款日结开始---------LLAdvancdPaymentDayCheckBL---");
		// 得到外部传入的数据,将数据备份到本类中
		...
		logger.debug("-----理赔日结--理赔－预付赔款日结结束----LLAdvancdPaymentDayCheckBL---");
	...
	private boolean checkInputData() {
		if (mStatiCode == null || mStatiCode.equals("")) {
			...
			tError.moduleName = "LLAdvancdPaymentDayCheckBL";
	...
	private boolean dealData() {
		...
		tXmlExport.createDocument("AdvancdPaymentDayCheck.vts", "");
		...
		String strSQL = "select j.riskcode,(select riskname from lmrisk where riskcode=j.riskcode),sum(j.pay)"
				+ ",("
				+ " select sum(jj.pay) from ljagetclaim jj where 1=1 and jj.riskcode=j.riskcode "
				+ " and jj.feeoperationtype='B' and jj.subfeeoperationtype='B' and jj.feefinatype='B' "
				...
				+ "' "
				+ ")"
				+ " from ljagetclaim j where 1=1 "
				+ " and j.feeoperationtype='B' and j.subfeeoperationtype='B' and j.feefinatype='B' "
				// +" and j.pay>=0 "Modify by zhaorx 2006-04-19
				...
				+ tNCLType
				+ " "
				+ " group by j.riskcode order by j.riskcode ";
		logger.debug("==================================================");

 ...
 * @version 1.0
 */
public class RINewDistillRN implements RIDistill {
	String mEventType;
	...
	private String getUnSQL() {
		...
		strBuffer.append(" ceil(months_between('" + mRIWFLogSchema.getEndDate()
				+ "', d.cvalidate - 1) / 12) as InsuredYear, ");
		...
		// 业务日期按 保单生效周年日 提取
		strBuffer
				.append(" add_months(d.cvalidate,ceil(months_between(to_date('"
		...
		System.out.println("续期提数公共部分sql: " + strBuffer.toString());
	...
	private String getSpecSQL() {
		...
		strBuffer
				.append(" nvl((select x.Smokeflag from Lcinsured x where x.Contno=a.Contno and x.Insuredno=a.Insuredno),'N') as SmokeFlag, ");
		strBuffer
				.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=d.DutyCode) as AdditionalRate, ");
		strBuffer
				.append(" (select max(AddFeeDirect) from LCPrem where polno=a.polno and DutyCode=d.DutyCode) as ExtType, ");
		strBuffer
				.append(" (select sum(Prem) from LCPrem where polno=a.polno and DutyCode=d.DutyCode) as ExtPrem, ");
		strBuffer.append(" '' as BegAccountValue, ");
		strBuffer.append(" '' as EndUnit, ");
		strBuffer.append(" '' as EndPrice, ");
		strBuffer.append(" '' as EndAccountValue, ");
		...
		strBuffer.append(" '' as StandbyDate5 ");
		strBuffer.append(" from lcpol a,lcduty d ");
		System.out.println("续期临分提数公共部分sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	private String getDistillSQL() {
		 ...
		 * 续期日期(add_months(d.cvalidate,floor(decode(months_between(
		 * '"+mRIWFLogSchema.getEndDate()+"',d.cvalidate),0,1,months_between('"+mRIWFLogSchema.getEndDate()+"',a.cvalid
		 * a t e ) ) / 1 2 ) *12))之前 且一直为失效状态(lccontstate.enddate is
		 * null)，说明保单在提数的上一个缴费期后就已经失效，这时不提续期数据， 如果失效日期在续期日期之后或失效状态已经结束这时可以提续期数据
		 */
		// strBuffer.append(" and not exists (select 'X' from lccontstate where  statetype ='Available' and state='1' and enddate is null and startdate<add_months(d.cvalidate,ceil(months_between('"+mRIWFLogSchema.getEndDate()+"',d.cvalidate-1)/12)*12) and polno =a.polno )");
		// strBuffer.append(" and not exists (select 'X' from lccontstate where  statetype ='Available' and state='1' and enddate is not null and add_months(d.cvalidate,ceil(months_between('"+mRIWFLogSchema.getEndDate()+"',d.cvalidate-1)/12)*12) between  startdate and enddate and polno =a.polno )");
		strBuffer
		...
		strBuffer
				.append(" and exists(select 'X' from RIAccumulateGetDuty r where a.riskcode = r.associatedcode and d.Dutycode like r.Getdutycode||'%'  and r.accumulatedefno='"
						+ mAccumulateDefNo + "') ");
		strBuffer
				.append(" and add_months(d.cvalidate,ceil(months_between(to_date('"
						+ mRIWFLogSchema.getEndDate()
						+ "','yyyy-MM-dd'),d.cvalidate-1)/12-1)*12) between '");// 保单周年日
		...
		strBuffer.append(") ");
		System.out.println("续期提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	private String getDistillSQL1() {
		...
		strBuffer
				.append(" and a.riskcode not in('IBC02','IBC01','IRD04','IRA03')");
		strBuffer.append(") ");
		System.out.println("续期临分提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	public static void main(String[] args) {
		...
		mRIWFLogSchema.setStartDate("2012-1-1");
		mRIWFLogSchema.setEndDate("2012-12-31");
		mRIWFLogSchema.setTaskCode("L01010010010");
		cInputData.add(mRIWFLogSchema);
		RINewDistillRN tRIDistillRN = new RINewDistillRN();

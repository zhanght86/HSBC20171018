...
/**
 * <p>Title: Web业务系统</p>
 * <p>Description: 单证打印：理赔案件审核报告  vtAudiReportC000200.vts</p>
...
private static Logger logger = Logger.getLogger(LLPRTClaimAuditiReportBL.class);
    ...
    private boolean dealData()
    {
        ...
        LDPersonSet tLDPersonSet = tLDPersonDB.executeQuery(insuredSql) ;
        String tName = tLDPersonSet.get(1).getName() ;
        //年龄 llcase中的customerage --------------------------------------------
        String ageSQL = "select customerage from llcase where caseno='"+mClmNo+"'"
                        +"and customerno='"+mCusNo+"'" ;
        ExeSQL ageExeSQL = new ExeSQL() ;
        String age = ageExeSQL.getOneValue(ageSQL);
        ...
        //预估金额
        String standSQL = "select StandPay from llclaim where clmno='"+mClmNo+"'" ;
        ExeSQL standExeSQL = new ExeSQL() ;
        String standPay = standExeSQL.getOneValue(standSQL) ;
        tTextTag.add("LLClaim.StandPay",standPay);
        //审核结论---------------------------------------------------------------
        ...
        String auditConclusion = audiExeSQL.getOneValue(audiSQL) ;
        tTextTag.add("LLClaimUWMain.AuditConclusion",auditConclusion) ;
        //出险情况---------------------------------------------------------------
        String caseSQL = "select AccdentDesc from llcase where caseno='"+mClmNo+"'"
                         +" and CustomerNo='"+mCusNo+"'";
        ExeSQL caseExeSQL = new ExeSQL() ;
        String accdentDesc = caseExeSQL.getOneValue(caseSQL) ;
        tTextTag.add("RiskCase",accdentDesc);
        //责任认定---------------------------------------------------------------
        String auditIdeaSQL = "select AuditIdea from LLClaimUWMain where clmno='"+mClmNo+"'" ;
        ExeSQL auditIdeaExeSQL = new ExeSQL() ;
        String auditIdea = auditIdeaExeSQL.getOneValue(auditIdeaSQL) ;
        ...
        if (tLLClaimUWMainDB.getInfo())
        {
            ...
            tAuditPer = tExeSQL.getOneValue(tSql);
            ...
            tTextTag.add("LLClaimUWMain.AuditIdea",tLLClaimUWMainSchema.getAuditIdea());
        ...
        String contSQL = "" ;
        contSQL = "select * from lccont where contno in (select distinct contno from llclaimdetail where clmno='"+mClmNo+"' and givetype !='1')";
        ...
        for (int i=1;i<=tLCContSet.size();i++)
        {
            ...
            //查出保项数目--------------------------------------------------------
            String bxSQL = "";
            bxSQL = "select * from LLClaimDetail where clmno='"+mClmNo+"' and contno='"+contNo+"'";
            logger.debug(bxSQL);
            ...
            for(int j=1;j<=bxLLClaimDetailSet.size();j++)//内层循环保项的数目
            {
                //有效保额-------------------------------------------------------
                String feeSQL = "";
                feeSQL = "select * from lcget where polno='"+bxLLClaimDetailSet.get(j).getPolNo()+"'"
                         +" and getdutycode='"+bxLLClaimDetailSet.get(j).getGetDutyCode()+"'"
                         +" and dutycode='"+bxLLClaimDetailSet.get(j).getDutyCode()+"'";
                //logger.debug(feeSQL);
                ...
                stra[0] = "出险保项: "+bxName;//保项
                stra[1] = "有效保额: "+String.valueOf(fee);//lcget表中standmoney-summoney
                tListTable.add(stra);//将保项信息加入ListTable
            }

        }
        //理赔调查取证材料---------------------------------------------------------
        String inqSQL = "select * from LLInqCertificate where clmno='"+mClmNo+"'" ;
        LLInqCertificateDB tLLInqCertificateDB = new LLInqCertificateDB() ;
        LLInqCertificateSet tLLInqCertificateSet = tLLInqCertificateDB.executeQuery(inqSQL);

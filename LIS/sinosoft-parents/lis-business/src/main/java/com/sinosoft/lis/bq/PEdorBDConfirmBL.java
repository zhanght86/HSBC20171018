...
public class PEdorBDConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(PEdorBDConfirmBL.class);
	...
	private boolean getInputData() {
		try {
			...
			tError.moduleName = "PEdorBDConfirmBL";
	...
	public static void main(String[] args) {
		...
		tLPEdorItemSchema.setEdorAcceptNo("6120050812000070");
		tLPEdorItemSchema.setEdorNo("6020050812000055");
		tLPEdorItemSchema.setEdorType("BD");
		tLPEdorItemSchema.setContNo("230110000002980");
		...
		PEdorBDConfirmBL tPEdorBDConfirmBL = new PEdorBDConfirmBL();

		if (!tPEdorBDConfirmBL.submitData(tVData, "")) {
			logger.debug(tPEdorBDConfirmBL.mErrors.getError(0).errorMessage);
		}

		PubSubmit tSubmit = new PubSubmit();
		if (!tSubmit.submitData(tPEdorBDConfirmBL.mResult, "")) {
			// @@错误处理
			tPEdorBDConfirmBL.mErrors.copyAllErrors(tSubmit.mErrors);
			CError tError = new CError();
			tError.moduleName = "PEdorBLDetailBLF";
			tError.functionName = "submitData";
			tError.errorMessage = "数据提交失败!";
			tPEdorBDConfirmBL.mErrors.addOneError(tError);
			// return false;
		}
		logger.debug("ok!");
	}

...
public class LDIndUWUserBL {
private static Logger logger = Logger.getLogger(LDIndUWUserBL.class);
	...
	private boolean dealData() {
		...
		if (this.mOperate.equals("INSERT||MAIN")) {
			...
			if (tLDUWUserDB.getCount() > 0) {
				CError.buildErr(this,"该核保师记录已经存在！不能在此增加！");
				return false;
			}
					...
					if (mLDUWUserSchema.getUWType() == "2") {
						...
						if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
							...
							Sql = "insert into LDUWAmntGrade "
									+ "(select '"
									+ "?value1?"
									+ "',UWPopedom,UWPopedomName,UWType,UWKind,MaxAmnt,0,0,0,'"
									+ "?value2?" + "','"
									+ "?value3?" + "'," + "'"
									+ "?value4?" + "','"
									+ "?value5?" + "','"
			...
			if (mLDUWAmntGradeSet != null && mLDUWAmntGradeSet.size() > 0) {
				...
				if (mmLDUWAmntGradeSet != null) {
					...
					/*	String Sql = "";
						...
						sqlbv5.put("vallue4",PubFun.getCurrentTime());

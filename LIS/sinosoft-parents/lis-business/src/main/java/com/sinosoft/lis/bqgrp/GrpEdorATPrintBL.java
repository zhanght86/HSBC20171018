...
public class GrpEdorATPrintBL implements EdorPrint {
private static Logger logger = Logger.getLogger(GrpEdorATPrintBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "AT数据提取失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "AT数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		xmlexport.addDisplayControl("displayAT");
        ...
        ListTable tATListTable = new ListTable();
        tATListTable.setName("AT");
        String tSql = "select (select riskname from lmrisk where riskcode = a.riskcode),"
        			+ " nvl(sum(b.getmoney), 0)"
        			+ " from lcgrppol a"
        			+ " left outer join ljsgetendorse b on a.riskcode = b.riskcode"
        			+ "  and a.grpcontno = b.grpcontno"
        			+ " and b.endorsementno = '"+mLPGrpEdorItemSchema.getEdorNo()+"'"
        			+ " where a.grpcontno = '"+mLPGrpEdorItemSchema.getGrpContNo()+"'"
        			+ " and a.appflag = '1'"
        			+ " group by a.riskcode";
		SSRS rSSRS = new SSRS();
		...
		for (int i = 1; i <= tArrLen; i++) {
			...
			for(int k = 1;k<=strAT.length;k++){
				strAT[k-1] = rSSRS.GetText(i, k);
			}
			strAT[2] = "退保";
			strAT[3] = "0";
			tATListTable.add (strAT) ;
		}
		...
		String dATSumMoney = "";//退保的人数
		ExeSQL tExeSQL = new ExeSQL();
		tSql = "select sum(getmoney) from ljsgetendorse where GrpContNo ='"+mLPGrpEdorItemSchema.getGrpContNo()+"' and endorsementno = '"+mLPGrpEdorItemSchema.getEdorNo()+"'";
		dATSumMoney = tExeSQL.getOneValue(tSql);
		if (dATSumMoney == null || dATSumMoney.equals("")) {
			dATSumMoney = "0.0";
		}
		mTextTag.add("ATSUM", dATSumMoney);
		...
		String[] b_strAT = new String[4];
		xmlexport.addListTable(tATListTable, b_strAT);

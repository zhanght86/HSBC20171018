           ...
           try
           {
               ...
               if (mLLAffixSet.size()>0)
               {
                   for (int i = 1; i <= mLLAffixSet.size(); i++)
                   {
                    ...
                    tLLAffixDB.setCustomerNo(mLLAffixSchema.getCustomerNo());
                    ...
                    tLLAffixSchema.setShortCount(mLLAffixSchema.getShortCount());//缺少件数
                    tLLAffixSchema.setProperty(mLLAffixSchema.getProperty());//提交形式
                    tLLAffixSchema.setAffixConclusion(mLLAffixSchema.getAffixConclusion());//齐全标志
                    tLLAffixSchema.setAffixReason(mLLAffixSchema.getAffixReason());//不齐全原因
                    tLLAffixSchema.setReturnFlag(mLLAffixSchema.getReturnFlag());//是否退还原件
...
//            	 @@错误处理
                   ...
                   tError.errorMessage = "准备回销的单证数目=="+mLLAffixSet.size();
                   this.mErrors.addOneError(tError);
                   return false;
               }
               //检测如果单证已经全部提交,修改立案分案（LLCase）表中 修改单证检查结论（AffixConclusion=0-齐全）（此段程序2005-7-13 添加）
               //检查该（赔案，客户）的所有单证是否齐全，如果齐全则在立案分案（LLCase）表中 修改单证检查结论（AffixConclusion=0）
               String strSQL = "select * from llaffix where  1=1 and "
                    + "rgtno='"+mLLAffixSet.get(1).getRgtNo()+"' and "
                    + "customerno='"+mLLAffixSet.get(1).getCustomerNo()+"' and "
                    + "(SubFlag is null or SubFlag='1')";
               logger.debug("--strSQL=" + strSQL);

               LLAffixSet ttLLAffixSet = ((new LLAffixDB()).executeQuery(strSQL));
               ...
               if( ttLLAffixSet.size()==mLLAffixSet.size())
               {
                   ...
                   tLLCaseSchema.setAffixConclusion("1");//0--不齐全、1--齐全

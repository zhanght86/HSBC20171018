...
public class GrpEdorBRPrintBL implements EdorPrint {
private static Logger logger = Logger.getLogger(GrpEdorBRPrintBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "EI数据提取失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "EI数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		xmlexport.addDisplayControl("displayBR");
        ...
        ListTable tBRListTable = new ListTable();
        tBRListTable.setName("BR");

//		String tNewEndDate = "";//新的止期
//		ExeSQL tExeSQL = new ExeSQL();
//		String tSql = "select max(payenddate) from lcgrppol where grpcontno = '"+tLCGrpContSchema.getGrpContNo()+"'";
//		tNewEndDate = tExeSQL.getOneValue(tSql);
//		if (tNewEndDate == null || tNewEndDate.equals("")) {
//			CError.buildErr(this, "统计变更日期失败!");
//			return false;
//		}
		String tOldEndDate = "";//旧的止期
		String tSql = "select min(startdate) from lpgrpcontstate where grpcontno = '"+tLCGrpContSchema.getGrpContNo()+"' and edorno = '"+mLPGrpEdorItemSchema.getEdorNo()+"' and edortype = '"+mLPGrpEdorItemSchema.getEdorType()+"' and enddate is null and statetype = 'Available' and state = '0'";
		ExeSQL tExeSQL = new ExeSQL();
		...
		//String[] sNewEndDate = tNewEndDate.substring(0, 10).split("-");
		mTextTag.add("OldEndDate", sOldEndDate[0]+"年"+sOldEndDate[1]+"月"+sOldEndDate[2]+"日");
		//mTextTag.add("NewEndDate", sNewEndDate[0]+"年"+sNewEndDate[1]+"月"+sNewEndDate[2]+"日");
		...
		String[] b_strBR = new String[2];
		xmlexport.addListTable(tBRListTable, b_strBR);

...
public class RnewUWManuAddChkBL {
private static Logger logger = Logger.getLogger(RnewUWManuAddChkBL.class);
	...
	private boolean checkData() {
		...
		//校验稍后加~
		//ln 2008-12-9 add
		String tSQL = "select * from RnewIssuePol where contno='?contno?' and state in ('0','1') and needprint = 'Y' and prtseq is not null"
		...
		RnewIssuePolDB tRnewIssuePolDB = new RnewIssuePolDB();
		 ...
		 tRnewIssuePolSet = tRnewIssuePolDB.executeQuery(sqlbv);
		//是否有问题件
		if (tRnewIssuePolSet==null || tRnewIssuePolSet.size() < 0) 
		{
			// @@错误处理
	...
	private boolean prepareAdd() {
		...
		for(int i=1;i<=this.mLCPolSet.size();i++)
		{
			...
			if(tempLCPremSet.size()>0)
			{
				...
				String tsql = "select count(*) from LCPrem where  polno = '?polno?'  and state in ('1','3')";
				...
				if (tempLCDutySet.size() > 0) {
					for (int m = 1; m <= tempLCDutySet.size(); m++) {
						...
						// 减去该责任的原本次加费金额
						String sql = "select * from LCPrem where payplancode  like '000000%' and polno = '?polno?' and dutycode = '?dutycode?' and state = '1'";
						SQLwithBindVariables sqlbv2=new SQLwithBindVariables();
						sqlbv2.sql(sql);
						sqlbv2.put("polno", tempLCPolSchema.getPolNo().trim());
						sqlbv2.put("dutycode", tLCDutySchema.getDutyCode().trim());
			...
			sqlbv3.sql(tSQL);
			sqlbv3.put("polno", tempLCPolSchema.getPolNo());
			LCPremDB tLCPremDB = new LCPremDB();
			LCPremSet oldPremSet = new LCPremSet();
			oldPremSet = tLCPremDB.executeQuery(sqlbv3);
			if (oldPremSet == null) {
				// @@错误处理
			...
			String tSQL_Duty = "update lcduty a set prem = (select (case when sum(prem) is not null then sum(prem) else 0 end) from lcprem where polno=a.polno) "
				             + " where polno='?polno?' "; 
			SQLwithBindVariables sqlbv5=new SQLwithBindVariables();
			sqlbv5.sql(tSQL_Duty);
			sqlbv5.put("polno", tempLCPolSchema.getPolNo());
			logger.debug("tSQL_Duty:"+tSQL_Duty);
			String tSQL_LCPol = "update lcpol a set prem = (select (case when sum(prem) is not null then sum(prem) else 0 end) from lcduty where polno=a.polno) "
				         //     + " ,sumprem = (select nvl(sum(prem),0) from lcprem where polno=a.polno) "
				              + " where polno = '?polno?' ";
			SQLwithBindVariables sqlbv6=new SQLwithBindVariables();
			sqlbv6.sql(tSQL_LCPol);
			sqlbv6.put("polno", tempLCPolSchema.getPolNo());
...
*/
	...
	private boolean prepareOutputData() {
		...
		String tSQL_Cont = "update lccont a set prem=(select (case when sum(prem) is not null then sum(prem) else 0 end) from lcpol where contno=a.contno) "
			            // + " ,sumprem = (select nvl(sum(prem),0) from lcpol where contno=a.contno) " 
						 + " where contno='?contno?' ";
		SQLwithBindVariables sqlbv7=new SQLwithBindVariables();
		sqlbv7.sql(tSQL_Cont);

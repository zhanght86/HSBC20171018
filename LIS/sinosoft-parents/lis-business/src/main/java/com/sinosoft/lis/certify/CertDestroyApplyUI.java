 ...
 * @version 1.0
 */
public class CertDestroyApplyUI implements BusinessService {
private static Logger logger = Logger.getLogger(CertDestroyApplyUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			boolean bReturn = tCertDestroyApplyBL.submitData(cInputData,
					mOperate);
			mResult = tCertDestroyApplyBL.getResult();

			if (!bReturn) {
				if (tCertDestroyApplyBL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tCertDestroyApplyBL.mErrors);
				} else {

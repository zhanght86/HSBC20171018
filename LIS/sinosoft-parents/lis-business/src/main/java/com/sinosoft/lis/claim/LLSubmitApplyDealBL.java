...
private static Logger logger = Logger.getLogger(LLSubmitApplyDealBL.class);
	...
	private boolean getInputData() {
		...
		// mIsReportExist =(String)
		// mInputData.getObjectByObjectName("IsReportExist", 0);
	...
	private boolean dealData(String cOperate) {
		...
		if (cOperate.equals("INSERT||Reporthead")) {
			...
			if (mLLSubmitApplySchema.getSubCount().equals("0")) {
				...
				// 更新原记录DispType的SQL语句
				String sSql = "update LLSubmitApply set DispType='" + "?disptype?"
						+ "'" + " ,DispPer='" + "?operator?" + "'"
						+ " ,DispDept='" + "?comcode?" + "'" + " ,DispDate='"
						+ "?curdate?" + "'" + " where ClmNO='" + "?ClmNO?" + "'"
						+ " and SubNO='" + "?SubNO?" + "' " + " and SubCount='"
						+ "?SubCount?" + "'";
				SQLwithBindVariables sqlbv = new SQLwithBindVariables();
				sqlbv.sql(sSql);
				sqlbv.put("disptype", mDispType);
				sqlbv.put("operator", mG.Operator);
				sqlbv.put("comcode", mG.ComCode);
				sqlbv.put("curdate", CurrentDate);
				sqlbv.put("ClmNO", mClmNO);
				sqlbv.put("SubNO", mSubNO);
				sqlbv.put("SubCount", mSubCount);
				map.put(sqlbv, "UPDATE"); // 提交语句修改原记录
		...
		if (cOperate.equals("UPDATE||Replyport")) // 回复意见处理模块
		{
			...
			if (tLLSubmitApplyDB.getSubCount().equals("1")) {
				...
				ttLLSubmitApplyDB.setSubCount(mLLSubmitApplySchema
						.getSubCount());
				ttLLSubmitApplyDB.setSubNo(mLLSubmitApplySchema.getSubNo());
				ttLLSubmitApplyDB.getInfo();
				LLSubmitApplySchema ttLLSubmitApplySchema = new LLSubmitApplySchema();

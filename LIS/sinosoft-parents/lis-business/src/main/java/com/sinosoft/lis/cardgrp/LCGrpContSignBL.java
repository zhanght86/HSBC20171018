...
private static Logger logger = Logger.getLogger(LCGrpContSignBL.class);
	...
	private boolean signGrpCont() {
		...
		if (tSignLCContSet == null || tSignLCContSet.size() == 0) {
			...
			for (int k = 1; k <= tSignLCContSet.size(); k++) {
	...
	private boolean SignOneGrpCont(LCGrpContSchema signSchema,
		...
		boolean result = true;
		logger.debug(tSignLCContSet.size());
		if (tSignLCContSet != null && tSignLCContSet.size() > 0) {
	...
	private boolean dealLCGrppol(LCGrpContSchema tLCGrpContSchema,
		...
		for (int i = 1; i <= tLCGrpPolSet.size(); i++) {
			...
			if (tLMRiskAppSet.get(1).getBonusFlag() != null) {
			...
			if(!"L".equals(tLMRiskAppSet.get(1).getRiskPeriod())){
	...
	private boolean dealBackMoney(LCGrpContSchema tLCGrpContSchema,
		...
		if (sumPay > sumPrem) {
			...
			tLJAGetSchema.setOtherNoType("6");
			...
			tLJAGetOtherSchema.setOtherNoType("1");
...
//		}
	...
	private boolean checkSignCont(String tGrpContNo) {
		...
		String tSql = "select * From lccont where grpcontno='" + tGrpContNo
				+ "' and (uwflag='4' or uwflag='9') and appflag not in('1','4')";
	...
	private static MMap prepareOtherUpdateGrpCont(String inNewGrpContNo,
		...
		String[] grpCont = { "LCGrpCont", "LCGrpAppnt", "LCGrpPol", "LCCont",
				...
				"LCAscriptionRuleFactory", "lcprem", "lcget" };
		...
		try {
			...
			if (sumcount > 0) {
				...
				if (tLCGrpPolSet != null && tLCGrpPolSet.size() > 0) {
					...
					tsql = "select * From lcgrpfee where feecode in (select substr(riskcode,2,3)||'0002' from lmriskapp where risktype6 in ('1')) and grpcontno='"
							+ tLCGrpContSchema.getGrpContNo() + "'";
					logger.debug(tsql);
	...
	private LCContSet queryLCContSet(LCGrpContSchema tLCGrpContSchema) {
		...
		String sql = "select * from lccont where prtno='"
				+ tLCGrpContSchema.getPrtNo()
				+ "' and appflag='0' order by poltype desc ";
		logger.debug(sql);

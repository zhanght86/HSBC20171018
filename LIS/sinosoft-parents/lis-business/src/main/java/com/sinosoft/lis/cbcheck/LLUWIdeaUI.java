...
public class LLUWIdeaUI implements BusinessService {
private static Logger logger = Logger.getLogger(LLUWIdeaUI.class);
	/** 传入数据的容器 */
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		LLUWIdeaBL tLLUWIdeaBL = new LLUWIdeaBL();

		logger.debug("----UI BEGIN---");
		if (tLLUWIdeaBL.submitData(cInputData, mOperate) == false) {
			// @@错误处理
			this.mErrors.copyAllErrors(tLLUWIdeaBL.mErrors);
			return false;
		}
		logger.debug("----UI END---");
		return true;

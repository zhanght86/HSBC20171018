 ...
 * @version 1.0
 */
public class RIRecordErrLog {
	public RIRecordErrLog() {
	}
	...
	public static void main(String[] args) {
		...
		tTransferData.setNameAndValue("ErrorInfo", "aaaaaaaaaaaaaaaaaaaaaaaaa");
		...
		tRIRecordErrLog.submitData(tVData, "");
	...
	private boolean recordLog() {
		String tErrorInfo = (String) mTransferData.getValueByName("ErrorInfo");
		RIWFErrLogSchema tRIWFErrLogSchema = new RIWFErrLogSchema();
		tRIWFErrLogSchema.setBatchNo(mRIWFLogSchema.getBatchNo());
		tRIWFErrLogSchema.setSerialNo(PubFun1.CreateMaxNo("RIWFERRLOG", 10));
		tRIWFErrLogSchema.setTaskCode(mRIWFLogSchema.getTaskCode());
		tRIWFErrLogSchema.setTaskType(mRIWFLogSchema.getTaskType());
		tRIWFErrLogSchema.setNodeState(mRIWFLogSchema.getNodeState());
		tRIWFErrLogSchema.setErrInfo("出错信息是: " + tErrorInfo);
		tRIWFErrLogSchema.setOperator(mGlobalInput.Operator);
		tRIWFErrLogSchema.setManageCom(mGlobalInput.ManageCom);
		tRIWFErrLogSchema.setMakeDate(PubFun.getCurrentDate());
		tRIWFErrLogSchema.setMakeTime(PubFun.getCurrentTime());
		mMap.put(tRIWFErrLogSchema, "INSERT");
		mResult.clear();
		mResult.add(tRIWFErrLogSchema);
		return true;
	}
	...
	private boolean getInputData(VData cInputData) {
		this.mRIWFLogSchema = (RIWFLogSchema) cInputData.getObjectByObjectName(
				"RIWFLogSchema", 0);
		...
		try {
			if (!mPubSubmit.submitData(this.mInputData, "")) {
				if (mPubSubmit.mErrors.needDealError()) {
					buildError("BL",
							"保存错误日志时出现错误:" + mPubSubmit.mErrors.getFirstError());
					return false;
				}
			}
			mMap = null;
			...
			tError.errorMessage = "保存错误日志时出现错误：" + ex.getMessage();

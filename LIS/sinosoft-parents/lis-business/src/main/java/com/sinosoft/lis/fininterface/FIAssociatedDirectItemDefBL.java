...
/**
 * <p>
 * ClassName: FIAssociatedDirectItemDefBL
 * </p>
 * <p>
 * Description: 财务接口-财务规则参数管理-科目固定专项定义BL
 * </p>
 * <p>
...
public class FIAssociatedDirectItemDefBL
{
private static Logger logger = Logger.getLogger(FIAssociatedDirectItemDefBL.class);
	...
	public FIAssociatedDirectItemDefBL()
	{}
	...
	public boolean submitData(VData cInputData, String cOperate)
	{
		...
		if (!dealData())
		{
			...
			tError.errorMessage = "数据处理失败FIAssociatedDirectItemDefBL-->dealData!";
...
//		mFIAssociatedDirectItemDefSchema.setSourceTableID(tFITableColumnDefSet.get(1).getColumnID());
		...
		if (this.mFIAssociatedDirectItemDefSchema == null)
		{
			CError tError = new CError();
			tError.moduleName = "FIAssociatedDirectItemDefBL";
			tError.functionName = "getInputData";
			tError.errorMessage = "mFIAssociatedDirectItemDefSchema为空！";
			this.mErrors.addOneError(tError);
			return false;
		}
		return true;
	...
	private boolean dealData()
	{
		...
		{
			logger.debug("进入新增模块！");
			mMMap.put(mFIAssociatedDirectItemDefSchema, "INSERT");
		}
		...
		else if (this.mOperate.equals("UPDATE||MAIN"))
		{
			...
			logger.debug("进入修改模块！");
			mMMap.put(mFIAssociatedDirectItemDefSchema, "UPDATE");
		...
		else if (this.mOperate.equals("DELETE||MAIN"))
		{
			...
			logger.debug("进入删除模块！");
			mMMap.put(mFIAssociatedDirectItemDefSchema, "DELETE");
	...
	// {
	// this.mResult.clear();
	// logger.debug("Start FIAssociatedDirectItemDefBLQuery Submit...");
	...
	// logger.debug("End FIAssociatedDirectItemDefBLQuery Submit...");
	// // 如果有需要处理的错误，则返回
	...
	private boolean pubSubmit()
	{
		...
		if (!tPubSubmit.submitData(mInputData, ""))
		{
			...
			tError.errorMessage = "数据提交失败FIAssociatedDirectItemDefBL-->pubSubmit!";
			this.mErrors.addOneError(tError);
			return false;
		}
		return true;

 ...
 * @version 1.0
 */
public class LLGrpReportBLF
{
private static Logger logger = Logger.getLogger(LLGrpReportBLF.class);
...
//    private LLReportReasonSet mLLReportReasonSet = new LLReportReasonSet();
    ...
    private boolean dealData()
    {
        ...
        {
            // @@错误处理
            this.mErrors.copyAllErrors(tLLGrpReportBL.mErrors);
            CError tError = new CError();
            tError.moduleName = "LLReportBLF";
            tError.functionName = "submitData";
            tError.errorMessage = "数据提交失败!";
        ...
        else
        {
            ...
            tempVData = tLLGrpReportBL.getResult();
    ...
    private boolean updateMyMission()
    {
        ...
        tLWMissionDB.setActivityID("0000009005");
        
		if (tLWMissionDB.getInfo() == false) {
			// @@错误处理
			CError.buildErr(this,"查询工作流轨迹失败!");
			return false;
		}
		else{ 

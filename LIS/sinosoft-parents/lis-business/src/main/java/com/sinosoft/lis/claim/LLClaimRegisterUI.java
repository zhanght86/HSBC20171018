 ...
 * @version 1.0
 */
public class LLClaimRegisterUI {
private static Logger logger = Logger.getLogger(LLClaimRegisterUI.class);
	...
	public static void main(String[] args) {
		...
		tLLClaimRegisterUI.submitData(tVData, transact);
	}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (tLLClaimRegisterBLF.submitData(mInputData, mOperate) == false) {
			// @@错误处理
			CError.buildErr(this, "数据提交失败!");
			this.mErrors.addOneError(tLLClaimRegisterBLF.mErrors.getLastError());
			mResult.clear();
			return false;
		} else {
			mResult = tLLClaimRegisterBLF.getResult();
		}
		mInputData = null;
		return true;
	}

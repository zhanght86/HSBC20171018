...
//		}
		...
		if (tLCIndUWMasterDB.getInfo() == false) {
			...
			CError.buildErr(this,"请先进行客户承保结论变更录入!");
			return false;
		}
		...
		String tSql = "select * from LCIndUWSub where contno='"+"?contno?"+"' and insuredno='"+"?insuredno?"+"' order by uwno desc";
		SQLwithBindVariables sqlbv = new SQLwithBindVariables();
		sqlbv.sql(tSql);
		sqlbv.put("contno", mContNo);
		sqlbv.put("insuredno", mCustomerNo);
		LCIndUWSubSet tLCIndUWSubSet = new LCIndUWSubSet();
		tLCIndUWSubSet = tLCIndUWSubDB.executeQuery(sqlbv);
		...
		if (mLCCSpecSchema != null) {
			...
			} else // INSERT
			{
				...
				tLCCSpecSchema.setGrpContNo(mLCIndUWMasterSchema.getGrpContNo());
				tLCCSpecSchema.setProposalContNo(mLCIndUWMasterSchema.getProposalContNo());

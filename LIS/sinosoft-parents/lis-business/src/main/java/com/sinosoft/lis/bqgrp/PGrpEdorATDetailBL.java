...
/**
 * <p>Title: Web业务系统</p>
 * <p>Description: 保全年金险退保AT业务逻辑处理类</p>
 * <p>Copyright: Copyright (c) 2008</p>
 * <p>Company: Sinosoft</p>
 * @author sunsx 2009-02-09; modified on 2009-02-13
 * @version 1.0
 */
public class PGrpEdorATDetailBL {
private static Logger logger = Logger.getLogger(PGrpEdorATDetailBL.class);
	...
	private boolean dealData() {
		...
		double appTotalGet = 0.0;//公共帐户合计领的金额
		...
		for(int i = 1; i<=tLCContSet.size();i++ )
		{
			...
			if("2".equals(tLCContSchema.getPolType()))
			{
				//公共帐户单独处理
				continue;
			}
			...
			for(int j = 1;j <= tLCPolSet.size();j++)
			{
				...
				String tAccFlag = tGrpEdorCalZT.getCalAccFlag();
				
				if("Y".equals(tAccFlag))
				{
					String tAnnuityFlag = tGrpEdorCalZT.getAnnuityFlag();
					//如果该保单进入领取期，即转为年金操作了，不能进行整单退保
					if("Y".equals(tAnnuityFlag))
					{
						CError.buildErr(this, "已经有被保人进入领取期，不能做退保!请去做减人操作");
						return false;
					}
					//帐户型的退何标志;
					double tAppGet = tGrpEdorCalZT.getCompanyGet();//投保人领取
					tInsureGet = tGrpEdorCalZT.getPersonalGet();//被保人领取
					appTotalGet += tAppGet; //将单位缴费的部分累加到投保人领取的部份;
					
					mLPInsureAccSet.add(tGrpEdorCalZT.getLPInsureAccSchema());
					mLPInsureAccClassSet.add(tGrpEdorCalZT.getLPInsureAccClassSet());
					mLPInsureAccTraceSet.add(tGrpEdorCalZT.getLPInsureAccTraceSet());
				}
		...
		//公共帐户的险种退保处理
		LCContDB pubLCContDB = new LCContDB();
		pubLCContDB.setAppFlag("1");
		pubLCContDB.setPolType("2");
		pubLCContDB.setGrpContNo(mLPGrpEdorItemSchema.getGrpContNo());
		LCContSet pubLCContSet = pubLCContDB.query();
		if(pubLCContSet!= null && pubLCContSet.size() >0 )
		{
			//公共帐户只有一个
			if(pubLCContSet.size()>1)
			{
				CError.buildErr(this, "计算失败,公共帐户只能有一个!");
				return false; 
			}
			tLPEdorMainSchema = new LPEdorMainSchema();
			tLCContSchema = pubLCContSet.get(1);
			tLPEdorMainSchema.setContNo(tLCContSchema.getContNo());
			...
			for(int j = 1;j <= tLCPolSet.size();j++)
			{
				...
				//退保金计算;
				double tInsureGet = 0.0;//被保人领取(暂定公共帐户的也为被保人)
				GrpEdorCalZT tGrpEdorCalZT = new GrpEdorCalZT();
				tGrpEdorCalZT.setGlobalInput(mGlobalInput);
				try {
				...
				if("Y".equals(tAccFlag))
				{
					...
					//帐户型的退保标志;
					double tAppGet = tGrpEdorCalZT.getCompanyGet();//公共帐户只有CompanyGet
					appTotalGet += tAppGet; //将单位缴费的部分累加到投保人领取的部份;
				...
				if(tInsureGet>=0){
					...
					tLJSGetEndorseSchemaNew.setPayPlanCode("000000");
					tLJSGetEndorseSchemaNew.setGetDate(mLPGrpEdorItemSchema.getEdorValiDate());
					tLJSGetEndorseSchemaNew.setGetMoney(appTotalGet);//投保人领取所有被保人帐户的单位交费部份与公共帐户之和
					tLJSGetEndorseSchemaNew.setAgentCode(tLPPolSchema.getAgentCode());
					tLJSGetEndorseSchemaNew.setAgentCom(tLPPolSchema.getAgentCom());
					tLJSGetEndorseSchemaNew.setAgentGroup(tLPPolSchema.getAgentGroup());
					tLJSGetEndorseSchemaNew.setAgentType(tLPPolSchema.getAgentType());
				...
				tLPEdorItemSchema.setGetMoney(appTotalGet);
				tLPEdorItemSchema.setManageCom(mLPGrpEdorItemSchema.getManageCom());
				tLPEdorItemSchema.setOperator(mGlobalInput.Operator);
				tLPEdorItemSchema.setUWFlag("0");
				tLPEdorItemSchema.setEdorState("1");

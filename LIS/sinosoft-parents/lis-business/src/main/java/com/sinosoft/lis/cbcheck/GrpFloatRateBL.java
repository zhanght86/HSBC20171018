		...
		for (int i = 1; i <= tLCPolSet.size(); i++) {
			...
			for (int j = 1; j <= tLCDutySet.size(); j++) {
				...
				String tSql = "select sum(Prem) from LCPrem where PolNo='"
						...
						+ "' and PayPlanType='03' and PayPlanCode like '000000%' "
				...
				newFloatRate = Double.parseDouble(mFRDecimalFormat
						.format(tFloatRate)); // 转换费率规定的精度

				double tAddPrem = Double.parseDouble(mAddPrem);
				double tcFloatRate = mLCDutySchema.getFloatRate(); // 录单时的费率折扣比例
				if (tcFloatRate == 0) {
					tcFloatRate = 1;
				}
				...
				// 责任项下的新保费金额
				mNewPrem = Double.parseDouble(mDecimalFormat.format(mOldPrem
						* tFloatRate + tAddPrem)); // 转换计算后的保费（规定的精度）
				...
				// 修改保费表保费值
				// 先计算出标准保费，再乘新的费率折扣，即为现在的值
				// prem = prem * newFloatRate / oldFloatRate
				// 对于原费率折扣，在做浮动费率调整时，会记入lcprem的SuppRiskScore；
				// 如果SuppRiskScore为0，证明没有做浮动费率调整，则以录单时的费率折扣计算
				tUpdatePremSQL = "update lcprem set prem=prem/decode(SuppRiskScore,0,"
						+ tcFloatRate
						+ ",SuppRiskScore)*"
						+ newFloatRate
						+ ",rate="
						...
						+ "' where contno='"
						+ mLCDutySchema.getContNo()
						+ "' and polno='"
						+ mLCDutySchema.getPolNo()
						+ "' and dutycode='"
						+ mLCDutySchema.getDutyCode() + "' and payplantype='0'";

				mMap.put(tUpdatePremSQL, "UPDATE");
			...
			// mLCPolSchema.setStandbyFlag1(Double.toString(newFloatRate));

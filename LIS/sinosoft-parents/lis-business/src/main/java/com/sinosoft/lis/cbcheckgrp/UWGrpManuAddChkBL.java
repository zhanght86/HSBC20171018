...
private static Logger logger = Logger.getLogger(UWGrpManuAddChkBL.class);
	...
	private boolean checkData() {
		...
		String sql = "select * from lcgrppol where grpcontno='" + mGrpContNo
		...
		if (tLCGrpPolSet.size() == 0) {
			...
			tError.moduleName = "UWGrpManuAddChkBL";
			tError.functionName = "checkData";
			tError.errorMessage = "保单" + mPolNo + "集体险种信息查询失败!";
			this.mErrors.addOneError(tError);
					...
					if (tLCPremSet.size() > 0) {
						for (int j = 1; j <= tLCPremSet.size(); j++) {
							...
							mLCGrpContSchema.setPrem(mLCGrpContSchema.getPrem()
									- tLCPremSchema.getPrem());
							tLCGrpPolSchema.setPrem(tLCGrpPolSchema.getPrem()
									- tLCPremSchema.getPrem());
						}
					}

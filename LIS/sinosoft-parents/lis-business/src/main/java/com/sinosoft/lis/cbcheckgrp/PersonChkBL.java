...
private static Logger logger = Logger.getLogger(PersonChkBL.class);
	...
	private boolean prepareAppnt() {
		...
		String tsql = "";
		if (mLoadFlag.equals("3")) {
			tsql = " select standbyflag1 from lcissuepol where 1=1 "
					+ " and issuetype = '99' and contno = '"
					+ tLCContSchema.getContNo()
					+ "'"
					+ " and operatepos = 'A'"
					+ " and serialno in (select max(serialno) from lcissuepol "
					+ " where issuetype = '99' and operatepos = 'A' and contno = '"
					+ tLCContSchema.getContNo() + "')";
		} else {
			tsql = "select distinct customerno from LDPerson where name = '"
					+ tLCAppntDB.getAppntName()
					+ "'"
					+ " and Birthday = '"
					+ tLCAppntDB.getAppntBirthday()
					+ "'"
					+ " and Sex = '"
					...
					+ tAppntNo
					+ "' "
					+ " union select distinct customerno from ldperson where IDType = '"
					+ tLCAppntDB.getIDType() + "'" + " and IDNo = '"
					+ tLCAppntDB.getIDNo() + "'" + " and IDType is not null"
					+ " and IDNo is not null" + " and CustomerNo <> '"
					+ tAppntNo + "' ";
		}
	...
	private boolean prepareInsured() {
		...
		if (mLoadFlag.equals("3")) {
			tsql = " select standbyflag1 from lcissuepol where 1=1 "
					...
					+ " and operatepos = 'I'"
					+ " and serialno in (select max(serialno) from lcissuepol "
					+ " where issuetype = '99' and operatepos = 'I' and contno = '"
					+ tLCContDB.getContNo() + "')";
		} else {
			tsql = "select distinct customerno from LDPerson where name = '"
					+ tLCInsuredDB.getName()
					+ "' and Birthday = '"
					+ tLCInsuredDB.getBirthday()
					+ "' and Sex = '"
					+ tLCInsuredDB.getSex()
					+ "' and CustomerNo <> '"
					+ mInsuredNo
					+ "'"
					+ " union select distinct customerno from ldperson where IDType = '"
					+ tLCInsuredDB.getIDType() + "'" + " and IDNo = '"
					+ tLCInsuredDB.getIDNo() + "'" + " and IDType is not null"
					+ " and IDNo is not null" + " and CustomerNo <> '"
					+ mInsuredNo + "'";

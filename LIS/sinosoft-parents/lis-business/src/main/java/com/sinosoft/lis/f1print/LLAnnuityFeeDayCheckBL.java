...
/**
 * <p>
 * Title:财务收付-->财务日结-->理赔日结-->理赔－年金给付日结:
 * </p>
 * <p>
 * Description:
 * </p>
...
public class LLAnnuityFeeDayCheckBL implements PrintService {
private static Logger logger = Logger.getLogger(LLAnnuityFeeDayCheckBL.class);
	...
	public static void main(String[] args) {
		...
		tTransferData.setNameAndValue("StartDate", "2005-09-1"); // 开始时间
		tTransferData.setNameAndValue("EndDate", "2005-09-5"); // 结束时间
		...
		LLAnnuityFeeDayCheckBL tLLAnnuityFeeDayCheckBL = new LLAnnuityFeeDayCheckBL();
		if (tLLAnnuityFeeDayCheckBL.submitData(tVData, "") == false) {
			logger.debug("----------理赔日结-->理赔年金给付出错---------------");
		}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("-----理赔日结-->理赔－年金给付日结开始---------LLAnnuityFeeDayCheckBL---");
		// 得到外部传入的数据,将数据备份到本类中
		...
		logger.debug("-----理赔日结--理赔－年金给付日结结束----LLAnnuityFeeDayCheckBL---");
	...
	private boolean checkInputData() {
		if (mStatiCode == null || mStatiCode.equals("")) {
			...
			tError.moduleName = "LLAnnuityFeeDayCheckBL";
	...
	private boolean dealData() {
		...
		tXmlExport.createDocument("AnnuityFeeDayCheck.vts", "");
		...
		// 保单结算生存金、养老金----业务类型：C04；子业务类型：C0401；财务类型：EF ；
		// 以下主要是 日期的问题《》
		String strSQL = "select distinct j.riskcode,(select riskname from lmrisk where riskcode=j.riskcode),t.payyears,t.payendyearflag, sum(j.pay) "
				+ ",("
				+ " select sum(jj.pay)  from ljagetclaim jj ,lcpol ss where 1=1 and jj.polno=ss.polno "
				+ " and ss.riskcode= j.riskcode and ss.payyears= t.payyears and ss.payendyearflag=t.payendyearflag "
				+ " and jj.feefinatype = 'EF'  and jj.feeoperationtype = 'C04' and jj.managecom like concat('"
				+ "?mStatiCode?"
				+ "','%') "
				+ " and jj.opconfirmdate between (last_day(add_months('"
				+ "?mStartDate?"
				+ "', -1)) + 1)  and '"
				+ "?mEndDate?"
				+ "' "
				+ ")"
				+ " from lcpol t,ljagetclaim j where 1 = 1  and t.polno = j.polno "
				+ " and j.feefinatype = 'EF' and j.feeoperationtype = 'C04' and j.managecom like concat('"
				+ "?mStatiCode?"
				+ "','%') "

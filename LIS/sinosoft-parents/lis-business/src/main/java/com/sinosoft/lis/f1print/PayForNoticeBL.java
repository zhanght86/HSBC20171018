...
public class PayForNoticeBL {
private static Logger logger = Logger.getLogger(PayForNoticeBL.class);
	...
	// private static int compareInt = 0;

	private TransferData mTransferData = new TransferData();
	String mAppntNo = "";
	String mAppntName = "";

	public PayForNoticeBL() {
	}
	...
	private boolean getBatchInputData(VData cInputData) {
		...
		mGlobalInput.setSchema((GlobalInput) cInputData.getObjectByObjectName(
				"GlobalInput", 0));
		// mSchemaNum = ((Integer)
		// cInputData.getObjectByObjectName("Integer",0)).intValue();
		// mLJSPaySchema.setSchema((LJSPaySchema)cInputData.getObjectByObjectName("LJSPaySchema",0));
		...
		// mLOPRTManagerSchema.setSchema((LOPRTManagerSchema) cInputData.
		// getObjectByObjectName(
		// "LOPRTManagerSchema", i));
		// mLOPRTManagerSchemas.addElement(mLOPRTManagerSchema);
		// mLOPRTManagerSchema = null;
		// }
		mTransferData = (TransferData) cInputData.getObjectByObjectName(
				"TransferData", 0);
		mAppntNo = (String) mTransferData.getValueByName("AppntNo");
	...
	private boolean getPrintData() {
		...
		// testSSRS = exeSql.execSQL(
		// "select SysVarValue from ldsysvar where Sysvar='VTSFilePath'");
		// String strVTSFilePath = testSSRS.GetText(1, 1);
		// testSSRS = exeSql.execSQL(
		// "select SysVarValue from ldsysvar where Sysvar='VTSRealPath'");
		...
		// xmlExport.createDocument("HastenPersonal.vts", "printer"); //最好紧接着就初始化xml文
		...
		for (int j = 0; j < page; j++) {
			...
			sqlbv.sql("select appntno from lccont where contno = '"
					+ "?contno?" + "'");
			sqlbv.put("contno", tsLOPRTManagerDB.getOtherNo());
			tSrs = exeSql.execSQL(sqlbv);

			logger.debug("=========================tSrs======================="
							+ tSrs.GetText(1, 1));

			String tappntinfo = " select  concat((select placename from ldaddress where placetype = '03' and trim(placecode) = t.county),t.postaladdress), ' ' ,' ',"
					+ "        ' ',t.ZipCode,"
					+ "        n.appntname, (select GetAppntPhone(n.contno,'') from dual) ,n.ManageCom ,"
					+ "        n.ContNo ,n.appntno ,n.appntsex,"
					+ "        n.IDType,n.IDNo"
					+ "   from lcaddress t,lcappnt n "
					+ "  where 1=1 and n.addressno = t.addressno"
					+ " and t.customerno = n.appntno  and n.contno = '"
					+ "?contno?" + "' ";
			logger.debug("==========" + tappntinfo);
			if (tSrs.getMaxRow() != 0) {
				tappntinfo += " and n.appntno = '" + "?appntno?" + "'";
			}
			SQLwithBindVariables sqlbv1 = new SQLwithBindVariables();
			sqlbv1.sql(tappntinfo);
			sqlbv1.put("contno", tsLOPRTManagerDB.getOtherNo());
			sqlbv1.put("appntno", tSrs.GetText(1, 1));
			...
			// String taddress = "";
			// String tzipcode = "";
			// if(!testSSRS.GetText(1,9).equals("")&&testSSRS.GetText(1,9) != null&&
			// !testSSRS.GetText(1,9).equals("null")){
			// tzuSSRS = exeSql.execSQL(" select b.zipcode, b.Province, b.City, b.County
			// b.PostalAddress from lcappnt a ,lcaddress b where a.AddressNo = b.addressno
			// and a.appntno = b.customerno and a.Contno ='"+testSSRS.GetText(1,9)+"'");
			// taddress =
			// tzuSSRS.GetText(1,2)+tzuSSRS.GetText(1,3)+tzuSSRS.GetText(1,4)+tzuSSRS.GetText(1,5);
			// tzipcode = tzuSSRS.GetText(1,1);
			// }
			FDate tfDate = new FDate();
			texttag.add("Address", testSSRS.GetText(1, 1));

			// texttag.add("Address",
			// getPlaceName(testSSRS.GetText(1, 1), "01") +
			...
			// //计算组别的
			// tzuSSRS = exeSql.execSQL( "select labranchgroup.name ,laagent.name from
			// labranchgroup, laagent where trim(laagent.branchcode) =
			// trim(labranchgroup.agentgroup) and trim(laagent.agentcode) in (select
			// trim(agentcode) from lccont where contno
			// ='"+mLOPRTManagerSet.get(1).getOtherNo()+"')");

			for (int i = j * 13 + 1; i <= j * 13 + 13; i++) {
				...
				// String exsql = " select paytodate from lcpol where mainpolno = polno and
				// contno = '"+tLCContSchema.getContNo()+"'";
				// ExeSQL exe = new ExeSQL();
				// SSRS daySSRS = new SSRS();
				// testSSRS = exeSql.execSQL(exsql);
				// tComDate = testSSRS.GetText(1, 1);
				FDate fa = new FDate();

				// 取得loprtmanagerSub表数据
				// 正确提取本期交费的险种个数
				logger.debug("=============提取正确的该保单下的险种信息=================");
				String tRISKSql = "select distinct b.* "
						+ "from ljspayperson a, loprtmanagersub b "
						+ "where a.getnoticeno = b.getnoticeno "
						+ "and a.riskcode = b.riskcode " + "and b.prtseq = '"
						+ "?prtseq?" + "'";
				...
				sqlbv2.sql(tRISKSql);
				sqlbv2.put("prtseq", tLOPRTManagerSchema.getPrtSeq());
				LOPRTManagerSubSet tLOPRTManagerSubSet = new LOPRTManagerSubSet();
				tLOPRTManagerSubSet = tLOPRTManagerSubDB.executeQuery(sqlbv2);
				...
				sqlbv3.sql("select a.riskcode , a.sumactupaymoney from ljapayperson a , lcpol b where lastpaytodate in ( "
				...
				tljsSSRS = exeSql.execSQL(sqlbv3);

				SQLwithBindVariables sqlbv4 = new SQLwithBindVariables();
				sqlbv4.sql("select a.riskcode, "
				...
				tljaSSRS = exeSql.execSQL(sqlbv4);
				...
				for (int m = 1; m <= tLOPRTManagerSubSet.size(); m++) {
					...
					sqlbv5.sql("select riskshortname from lmrisk where 1=1 "
							+ " and riskcode = '"
							+ "?riskcode?"
							+ "'");
					sqlbv5.put("riskcode", tLOPRTManagerSubSchema.getRiskCode());
					...
					sqlbv6.sql("select sum(b.SumduePayMoney) from ljspayperson b  where b.contno = '"
							+ "?contno?"
							+ "' "
							+ " and b.riskcode = '"
							+ "?riskcode?"
							+ "'");
					sqlbv6.put("contno", tLOPRTManagerSchema.getOtherNo());
					sqlbv6.put("riskcode", tLOPRTManagerSubSchema.getRiskCode());
					tsdSSRS = exeSql.execSQL(sqlbv6);
					...
					if (tPayToDate == null) {
						...
						sqlbv7.sql(tSql);
						sqlbv7.put("otherno", tLOPRTManagerSchema.getOtherNo());
						sqlbv7.put("getnoticeno", tLOPRTManagerSchema.getStandbyFlag2());
						tdateSSRS = tExeSQL.execSQL(sqlbv7);
						logger.debug("====================================");
						logger.debug("====================================");
						logger.debug("====================================");
						logger.debug("====================================");
				...
				sqlbv8.sql("select managecom , agentcode  from lccont where contno = '"
						+ "?contno?" + "'");
				sqlbv8.put("contno", tLCContSchema.getContNo());
				tySSRS = exeSql.execSQL(sqlbv8);
				...
				sqlbv9.sql("select (select name from labranchgroup where managecom ='"
						...
						+ "?agentcode?" + "' ))) from dual ");
				sqlbv9.put("managecom", tySSRS.GetText(1, 1));
				sqlbv9.put("agentcode", tySSRS.GetText(1, 2));
				t1SSRS = exeSql.execSQL(sqlbv9);
				String tAgentManage = t1SSRS.GetText(1, 1)
						+ t1SSRS.GetText(1, 2) + t1SSRS.GetText(1, 3);
				SQLwithBindVariables sqlbv10 = new SQLwithBindVariables();
				sqlbv10.sql("select name ,phone from laagent where agentcode = '"
						+ "?agentcode?" + "'");
				sqlbv10.put("agentcode", tySSRS.GetText(1, 2));
				tzuSSRS = exeSql.execSQL(sqlbv10);
				...
				sqlbv11.sql("select sum(summoney) from loloan where loantype ='1' and polno in (select polno from lcpol where contno ='"
						+ "?contno?" + "') ");
				sqlbv11.put("contno", tLCContSchema.getContNo());
				tfinSSRS = exeSql.execSQL(sqlbv11);
				logger.debug(exeSql);
			...
			PayForNoticeBL pay = new PayForNoticeBL();
			...
			// strVFPathName[j] = strVTSFilePath + j + "HastenPersonal.vts";
			// //把dataStream存储到磁盘文件
		...
		// {
		// }
		// xmlExportAll.setTemplateName("HastenPersonal.vts");
		// mResult.clear();
		// mResult.addElement(xmlExport);
	...
	private String getPlaceName(String tPlaceCode, String tPlaceType) {
		...
		// if (!tLDAddressDB.getInfo())
		// {
		// mErrors.copyAllErrors(tLDAddressDB.mErrors);
		// buildError("getPlaceName", "在取得ldaddress的数据时发生错误");
		// return "";
		// }
		...
		for (int i = 0; i < mSchemaNum; i++) {
			...
			if (flag.equals("GrpNo")) {
				...
				if (xmlexport == null) {
					...
					xmlexport.createDocument("HastenPersonal.vts", "printer"); // 最好紧接着就初始化xml文档
				}
			...
			if (flag.equals("PerNo")) {
				...
				if (xmlexport == null) {
					xmlexport = new XmlExport(); // 新建一个XmlExport的实例
					// xmlexport.createDocuments("PConFeeNotice.vts",
					// "printer"); //最好紧接着就初始化xml文档
				}
	...
	public static void main(String[] args) {
		PayForNoticeBL payForNoticeBL = new PayForNoticeBL();
	}
}

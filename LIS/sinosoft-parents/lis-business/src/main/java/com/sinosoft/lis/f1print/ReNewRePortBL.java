 ...
 * @version 1.0
 */
public class ReNewRePortBL {
private static Logger logger = Logger.getLogger(ReNewRePortBL.class);
	...
	private boolean getPrintData() {
		...
		for (int i = 1; i <= mLOPRTManagerSet.size(); i++) {
			...
			xmlExport.createDocument("RenewReport.vts", "PRINT");
			...
			for (int t = 0; t < RiskInt; t++) {
				...
				if (tRiskcode.equals("00237000")) {
					...
					sqlbv.sql(LLSql);
					sqlbv.put("contno", tLOPRTManagerDB.getOtherNo());
					...
					tLLRegisterSet = tLLRegisterDB.executeQuery(sqlbv);
			...
			sqlbv1.sql(t1SQL);
			sqlbv1.put("placecode", tLCAddressDB.getCounty());
			String tAddressEX = "";
			t1SSRS = t1ExeSQL.execSQL(sqlbv1);
			if (t1SSRS.MaxRow > 0) {
			...
			String PostalAddress = tAddressEX + tLCAddressDB.getPostalAddress();
			String ZipCode = tLCAddressDB.getZipCode();
			String Phone = " ";
			SQLwithBindVariables sqlbv2 = new SQLwithBindVariables();
			sqlbv2.sql("select GetAppntPhone('" + "?contno?"
					+ "','') from dual");
			sqlbv2.put("contno", contno);
			t1SSRS = t1ExeSQL.execSQL(sqlbv2);
			if (t1SSRS.MaxRow > 0) {
				Phone = t1SSRS.GetText(1, 1);
			}
	...
	public static void main(String arg[]) {
		...
		ReNewRePortBL tReNewRePortBL = new ReNewRePortBL();
		if (!tReNewRePortBL.submitData(tVData, "")) {
			logger.debug("false!!!");
		}
	}

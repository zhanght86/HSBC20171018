...
public class BonusCountBL {
private static Logger logger = Logger.getLogger(BonusCountBL.class);
	...
	/**根据主调函数传入的动作，作相应处理,BONUS--红利系数计算,BONUSM--红利金额计算 */
	...
	private boolean dealData() {
		...
		if ("BONUS".equals(mOperate)) {
			...
			mCalBase.setBonusYear(Integer.parseInt(mFiscalYear)); //加入会计年度考虑
			...
			mCalculator.addBasicFactor("BonusYear", mFiscalYear); //会计年度
			...
			if (mValue > 0) {
				//格式化，取两位精度-校验
//				String strCalPrem = mDecimalFormat.format(mValue);//转换计算后的保费(规定的精度)// 不再对红利系数进行格式化
...
//			String strBonusMoney = mDecimalFormat.format(tBonusMoney);//转换计算后的保费(规定的精度)
			String strBonusMoney = mDecimalFormat.format(tBonusMoney+0.00000000001);//转换计算后的保费(规定的精度)，解决取偶问题
			...
			String strSQL = "update LOBonusPol set BonusMoney = ?tBonusMoney?,BonusInterest =?tInterest?,BonusMakeDate = '?tCurMakeDate?',ModifyDate = '?tCurMakeDate?',ModifyTime='?tCurMakeTime?' "
					+ "where PolNo = '?PolNo?' and FiscalYear = ?FiscalYear? and GroupID = ?mGroupID?";

			SQLwithBindVariables sqlbv1=new SQLwithBindVariables();
			sqlbv1.sql(strSQL);
			sqlbv1.put("tBonusMoney", tBonusMoney);
			sqlbv1.put("tInterest", tInterest);
	...
	private double calInterest(String cPolNo, double cBonusMoney,
			String cSGetDate) {
		//如果没有到应分配日，则不计算利息
		if (PubFun.calInterval(cSGetDate, PubFun.getCurrentDate(), "D") <= 0)
			return 0;
		...
		if (!tssrs.GetText(1, 1).equals("2")) //只针对现金领取的保单随红利进行批量计算；
			return 0;
		...
		double tInterest = tAccountManage.getInterest(cBonusMoney, cSGetDate,
				PubFun.getCurrentDate(), tRate, "Y", "C", "D");

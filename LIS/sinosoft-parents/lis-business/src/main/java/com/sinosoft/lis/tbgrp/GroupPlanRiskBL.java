 ...
 * @date 2002-09-25
 */
public class GroupPlanRiskBL {
private static Logger logger = Logger.getLogger(GroupPlanRiskBL.class);
	/** 传入数据的容器 */
	...
	private boolean dealData() {
		...
		if (mOperate.equals("INSERT||GROUPRISK")) {
			for (int i = 1; i <= mLCGrpPolSet.size(); i++) {
				...
				String tLimit = PubFun.getNoLimit(mLCGrpContSchema
						.getManageCom());
				...
				tLCGrpPolSchema.setRiskCode(mLCGrpPolSet.get(i).getRiskCode());
				tLCGrpPolSchema
						.setCValiDate(mLCGrpPolSet.get(i).getCValiDate());
				tLCGrpPolSchema.setPayIntv(mLCGrpPolSet.get(i).getPayIntv());
				tLCGrpPolSchema.setExpPeoples(mLCGrpPolSet.get(i)
						.getExpPeoples());
				tLCGrpPolSchema.setDistriFlag(mLCGrpPolSet.get(i)
						.getDistriFlag());
				...
				mOLDLCGrpPolSet.add(tLCGrpPolSchema);
				...
				tTransferData.setNameAndValue("RiskName", tLMRiskDB
						.getRiskName());
			}
			map.put(mOLDLCGrpPolSet, "INSERT");
		}

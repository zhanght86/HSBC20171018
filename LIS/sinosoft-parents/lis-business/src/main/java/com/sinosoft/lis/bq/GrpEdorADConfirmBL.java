 ...
 * @version 1.0
 */
public class GrpEdorADConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(GrpEdorADConfirmBL.class);
	...
	public GrpEdorADConfirmBL() {
	}

	public boolean submitData(VData cInputData, String cOperate) {
		logger.debug("-------Grp AD Confirm----");
		...
		logger.debug("aftergetInputData Grp AD:::");
		...
		logger.debug("afterdealDate Grp AD:::");
	...
	private boolean dealData() {
		...
		for (int i = 1; i <= tLPGrpContSet.size(); i++) {
			...
			String ModifyInfo = "modifydate = '" + CurrDate
					+ "',modifytime = '" + CurrTime + "',operator = '"
					+ mLPGrpEdorItemSchema.getOperator() + "'";

			String[] tab1 = { "LCInsureAccFee", "LCInsureAccClassFee",
					"LCInsureAcc", "LCInsureAccClass", "LCPrem", "LCInsured" };
			for (int t = 0; t < tab1.length; t++) {
				mMap.put("insert into " + tab1[t].replaceFirst("C", "P")
						+ " (select " + EdorInfo + ",a.* from " + tab1[t]
						+ " a where grpcontno = '"
						+ tLCGrpContSchema.getGrpContNo() + "')", "INSERT");
				mMap.put("update " + tab1[t] + " set appntno = '"
						+ tLCGrpContSchema.getAppntNo() + "'," + ModifyInfo
						+ " where grpcontno = '"
						+ tLCGrpContSchema.getGrpContNo() + "'", "UPDATE");
			}

			String[] tab2 = { "LCPol", "LCCont", "LCGrpPol" };
			for (int t = 0; t < tab2.length; t++) {
				mMap.put("insert into " + tab2[t].replaceFirst("C", "P")
						+ " (select " + EdorInfo + ",a.* from " + tab2[t]
						+ " a where grpcontno = '"
						+ tLCGrpContSchema.getGrpContNo() + "')", "INSERT");
			}

			mMap.put("update lcgrppol set CustomerNo = '"
					+ tLCGrpContSchema.getAppntNo() + "',AddressNo = '"
					+ tLCGrpContSchema.getAddressNo() + "',GrpName = '"
					+ tLCGrpContSchema.getGrpName() + "'," + ModifyInfo
					+ " where appflag = '1' and grpcontno = '"
					+ tLCGrpContSchema.getGrpContNo() + "'", "UPDATE");
			mMap.put("update lccont set appntno = '"
					+ tLCGrpContSchema.getAppntNo() + "',appntname = '"
					+ tLCGrpContSchema.getGrpName() + "'," + ModifyInfo
					+ " where grpcontno = '" + tLCGrpContSchema.getGrpContNo()
					+ "'", "UPDATE");
			mMap.put("update lcpol set appntno = '"
					+ tLCGrpContSchema.getAppntNo() + "',appntname = '"
					+ tLCGrpContSchema.getGrpName() + "'," + ModifyInfo
					+ " where grpcontno = '" + tLCGrpContSchema.getGrpContNo()
					+ "'", "UPDATE");
	...
	private String UpAppntNo(String aGrpContNo, String nAppntNo) {
		String SQL = "";
		return SQL;
	}

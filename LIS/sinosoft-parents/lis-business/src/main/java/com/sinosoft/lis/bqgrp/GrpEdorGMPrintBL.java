...
private static Logger logger = Logger.getLogger(GrpEdorGMPrintBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "GM数据提取失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "GM数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		ListTable tGMListTable = new ListTable();
		tGMListTable.setName("GM");
		...
		String tSql = "select distinct c.name,"
					...
					+ " '月领几何增额型养老金',"
					+ " '未知')"
					+ "from lcget a, lpget b, lcinsured c"
		...
		String[] b_strGM = new String[3];
		xmlexport.addListTable(tGMListTable, b_strGM);

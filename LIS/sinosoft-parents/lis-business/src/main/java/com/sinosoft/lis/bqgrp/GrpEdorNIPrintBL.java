...
public class GrpEdorNIPrintBL implements EdorPrint {
	...
	private boolean dealData() {
		...
		ListTable tNIListTable = new ListTable();
		tNIListTable.setName("NI");
		...
		tSql = "SELECT RiskCode,Sum(Prem) FROM LCPol WHERE ContNo IN (SELECT ContNo FROM LPEdorItem WHERE EdorNo = '"
				+ mLPGrpEdorItemSchema.getEdorNo() + "') GROUP BY RiskCode";
		SSRS rSSRS = new SSRS();
		...
		for (int i = 1; i <= tArrLen; i++) {
			...
			String tSumPrem = rSSRS.GetText(i, 2);
			...
			String[] strNI = new String[1];
			if (i == tArrLen) {
				strNI[0] = tLMRiskAppSchema.getRiskName() + "险种保费合计" + tSumPrem
						+ "元。";
			} else {
				strNI[0] = tLMRiskAppSchema.getRiskName() + "险种保费合计" + tSumPrem
						+ "元，";
			}
			tNIListTable.add(strNI);
		}
		LPEdorAppDB tLPEdorAppDB = new LPEdorAppDB();
		tLPEdorAppDB.setEdorAcceptNo(mLPGrpEdorItemSchema.getEdorAcceptNo());
		mLPEdorAppSchema = tLPEdorAppDB.query().get(1);

		mTextTag.add("NINUM", dNum);
		...
		String[] b_strNI = new String[2];
		xmlexport.addListTable(tNIListTable, b_strNI);
	...
	private boolean bulidBill() {
		...
		tXmlExport.addDisplayControl("displayNI");
		...
		String strsql1 = "";
		strsql1 = "select rownum,b.name, "
				+ "(select codename from ldcode  where code = b.sex and codetype = 'sex'), "
				+ "b.birthday, "
				+ "b.idno, "
				+ "(select occupationname from ldoccupation where occupationcode = b.occupationcode), "
				+ "(select riskname from lmrisk where riskcode = a.riskcode), "
				+ "a.amnt,a.cvalidate,a.payenddate, "
				+ "(select name from lcbnf where bnfgrade = '1' and polno = a.polno and rownum = 1), "
				+ "(select idno from lcbnf where bnfgrade = '1' and polno = a.polno and rownum = 1), "
				+ "a.prem,'0',decode(b.socialinsuflag,'1','有','无') "
		...
		texttag.add("NISUM", mSumPrem);
		texttag.add("NINUM", mNum);

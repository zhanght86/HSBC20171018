 ...
 * @version 1.0
 */
public class RINewDistillBQTB implements RIDistill {
	String mEventType;
	...
	private String getUnSQL() {
		...
		// 变更前风险保额=风险保额
		strBuffer
				.append("RISK_AMNT2(InsuredYear,preAmnt,DutyCode,PolNo,Currency,Accumulatedefno) as PreRiskAmnt,");
		// 变更前风险保额转换
		strBuffer
				.append(" ( RISK_AMNT2(InsuredYear,preAmnt, DutyCode, PolNo, Currency,Accumulatedefno) *ChangeRate) as PreChRiskAmnt,");
		// 风险保额转换
		strBuffer.append("0 as  ChRiskAmnt,");
		strBuffer
				.append("Retention,Faceamount,Currency,PayIntv,PayYears,PayEndYearFlag,PayEndYear,GetYearFlag,GetYear,InsuYearFlag,InsuYear,AcciYearFlag,AcciYear,AcciEndDate,GetStartDate,GetStartType,PeakLine,GetLimit,GetIntv,PayNo,PayCount,PayMoney,LastPayToDate,CurPayToDate,EdorNo,FeeOperationType,FeeFinaType,ChangeRate,AccCurrency,AccAmnt,PreStandPrem,PrePrem,PreAmnt,ClmNo,ClmFeeOperationType,ClmFeeFinaType,StandGetMoney,GetRate,ClmGetMoney,AccDate,AccumulateDefNO,RIContNo,RIPreceptNo,ReinsreFlag,GetDate,StandbyString1,StandbyString2,StandbyString3,StandbyDouble1,StandbyDouble2,StandbyDouble3,StandbyDate1,StandbyDate2,ManageCom,MakeDate,MakeTime, ");
		strBuffer
		...
		strBuffer
				.append(" a.PolNo||','|| d.Dutycode ||','||b.edorno||','||b.edortype||','||'"
						+ mAccumulateDefNo + "'||','||'TB' as UnionKey, ");
		...
		strBuffer.append(" from lcpol a ,lpedoritem b,Lcduty d ");
		System.out.println("保全退保提数公共部分sql: " + strBuffer.toString());
	...
	private String getSpecSQL() {
		...
		strBuffer.append("0 as  RiskAmnt,");
		strBuffer
				.append("PayIntv,PayYears,PayEndYearFlag,PayEndYear,GetYearFlag,GetYear,InsuYearFlag,InsuYear,AcciYearFlag,AcciYear,AcciEndDate,GetStartDate,GetStartType,PeakLine,GetLimit,GetIntv,PayNo,PayCount,PayMoney,LastPayToDate,CurPayToDate,EdorNo,FeeOperationType,FeeFinaType,ChangeRate,AccCurrency,AccAmnt,PreStandPrem,PrePrem,PreAmnt,ClmNo,ClmFeeOperationType,ClmFeeFinaType,StandGetMoney,GetRate,ClmGetMoney,AccDate,AccumulateDefNO,RIContNo,RIPreceptNo,ReinsreFlag ,GetDate,");
		strBuffer
		...
		strBuffer
				.append(" a.PolNo||','|| d.Dutycode ||','||b.edorno||','||b.edortype||','||'"
						+ mAccumulateDefNo + "'||','||'TB'  as UnionKey, ");// 联合主键：polno||","||edorno||","||edortype
		strBuffer.append(" a.GrpContNo as GrpContNo, ");
		...
		strBuffer.append(" a.managecom as managecom ,");
		strBuffer
				.append(" date'" + PubFun.getCurrentDate() + "' as MakeDate, ");
		strBuffer.append(" '" + PubFun.getCurrentTime() + "' as MakeTime, ");
		strBuffer.append(" 0 as PreChangeRate, ");
		strBuffer.append(" a.Mainpolno as MainPolNo, ");
		strBuffer.append(" '' as RIPreceptType, ");
		...
		strBuffer.append(" '' as StandbyDate5 ");
		strBuffer.append(" from lcpol a ,lpedoritem b ,Lcduty d");
		System.out.println("保全退保临分提数公共部分sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	private String getDistillSQL() {
		...
		// edortype CF:犹豫期退保 CT:退保 EA:解约 RE:复效 IS:加保 RS:减保
		// CM:客户资料变更(客户资料变更提数令配置类RIDistillBQCM.java) IO:被保人职业类别变更 FT:缴费年期变更
		// LN:保单贷款
		strBuffer
				.append(" and a.Polno=d.Polno and a.contno = d.Contno and a.appflag='4' and b.edortype in('CF','CT','ES') and b.edorstate='0' ");
		strBuffer
		...
		strBuffer.append(") ");
		System.out.println("保全退保提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	private String getDistillSQL1() {
		...
		strBuffer
				.append(" and exists(select 'X' from Lpedorapp x where x.Edoracceptno=b.Edoracceptno and x.Confdate between '"
						...
						+ mRIWFLogSchema.getEndDate() + "')");
		strBuffer.append(") ");
		System.out.println("保全临分退保提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	public static void main(String[] args) {
		...
		mRIWFLogSchema.setStartDate("2013-1-1");
		mRIWFLogSchema.setEndDate("2013-11-19");
		mRIWFLogSchema.setTaskCode("L01010010010");
		cInputData.add(mRIWFLogSchema);
		RINewDistillBQTB tRIDistillBQ = new RINewDistillBQTB();

...
public class GrpEdorOGConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(GrpEdorOGConfirmBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (cInputData == null) {
			...
			logger.debug("\t@> GrpEdorOGConfirmBL.submitData() : 无法获取 InputData 数据！");
			return false;
		} else {
	...
	private boolean getInputData(VData cInputData) {
		// logger.debug("\t@> GrpEdorOGConfirmBL.getInputData() 开始");
		// GlobalInput
		...
		if (mGlobalInput == null) {
			...
			logger.debug("\t@> GrpEdorOGConfirmBL.getInputData() : 无法获取 GlobalInput 数据！");
			return false;
		}
		...
		if (mLPGrpEdorItemSchema == null) {
			...
			logger.debug("\t@> GrpEdorOGConfirmBL.getInputData() : 无法获取 LPGrpEdorItemSchema 数据！");
			return false;
		}
		// ----------------------------------------------------------------------
	...
	private boolean checkData() {
		...
		// logger.debug("\t@> GrpEdorOGConfirmBL.checkData() 成功");
		return true;
	...
	private boolean dealData() {
		...
		for (int i = 1; i <= tLPEdorItemSet.size(); i++) {
			...
			if (!tPEdorOGConfirmBL.submitData(tVData, mOperate)) {
				mErrors.copyAllErrors(tPEdorOGConfirmBL.mErrors);
				CError.buildErr(this, "处理提交的数据失败！");
				logger.debug("\t@> GrpEdorOGConfirmBL.dealData() : PEdorOGConfirmBL.submitData() 失败！");
				logger.debug("\t   出错的被保人号是 : "
						+ tLPEdorItemSchema.getInsuredNo());
				return false;
			} else {
				VData tTempVData = new VData();
				tTempVData = tPEdorOGConfirmBL.getResult();
		...
		// logger.debug("\t@> GrpEdorOGConfirmBL.dealData() 成功");
		return true;
	...
	private boolean prepareData() {
		mResult = new VData();
		mResult.add(mMap);
		// logger.debug("\t@> GrpEdorOGConfirmBL.prepareData() 成功");
		return true;

...
//	private static String PATH_INS_RELA_ID = "RelaId";
	...
	private static LCPolSchema getLCPolSchema(Node node) {
		...
		// 增加缴费规则与归属规则
		tLCPolSchema.setAscriptionRuleCode(parseNode(node,
				PATH_PolAscriptionRule));
		tLCPolSchema.setPayRuleCode(parseNode(node, PATH_PolPayRuleCode));
		// 借用[复核状态]缓存 免赔额

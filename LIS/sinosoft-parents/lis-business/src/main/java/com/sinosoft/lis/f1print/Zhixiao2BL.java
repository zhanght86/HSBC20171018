...
public class Zhixiao2BL {
private static Logger logger = Logger.getLogger(Zhixiao2BL.class);
	...
	private boolean getPrintData() {
			...
			for (int j = 0; j < ZhongSSRS.getMaxRow(); j++) {
				...
				for (int k = 0; k < YingSSRS.getMaxRow(); k++) {
					...
					if (mListType != null && mListType.equals("1")) {
						tSQL = "select aa, ab, ac, ad, ae, af, ag, ah, ai, aj, ak, al"
								...
								+ " a.polno as ad,"
								+ " nvl(a.accname, '') as ae,"
								+ " (select agentcode from laagent where agentcode in (select agentcode from lacommisiondetail where grpcontno = a.polno) and branchtype = '5' and branchtype2 = '01' and rownum = '1' ) as af,"
								+ " (select name from laagent where agentcode in (select agentcode from lacommisiondetail where grpcontno = a.polno) and branchtype = '5' and branchtype2 = '01' and rownum = '1' ) as ag,"
								+

								" (select agentcode from laagent where agentcode in (select agentcode from lacommisiondetail where grpcontno = a.polno) and branchtype = '5' and branchtype2 = '02' and rownum = '1' ) as ah,"
								+ " (select name from laagent where agentcode in (select agentcode from lacommisiondetail where grpcontno = a.polno) and branchtype = '5' and branchtype2 = '02' and rownum = '1' ) as ai,"
								+
								...
								+ " and codetype = 'bankerror'),'不成功') as al,"
								...
								+ " and exists(select 'x' from ljtempfee oo where oo.otherno = a.polno and oo.riskcode in ((select riskcode from lmriskapp where riskprop = 'T')))";
					...
					if (mListType != null && mListType.equals("2")) {
						tSQL = "select aa, ab, ac, ad, ae, af, ag, ah, ai, aj, ak, al"
								...
								+ " and a.dealtype='F'"
								+ " and a.notype='4'"
								+ " and exists(select 'x' from lcpol oo where oo.polno = a.polno and oo.riskcode in ((select riskcode from lmriskapp where riskprop = 'T')))";
						if (strReturnDate != null && !strReturnDate.equals("")) {
							tSQL = tSQL + " and a.bankdealdate = '"
									+ strReturnDate + "'";
						}
		...
		if (this.mListType != null && mListType.equals("1")) {
			xmlexport.createDocument("Zhixiao2.vts", "printer"); // appoint
																	// to a
																	// special
																	// output
																	// .vts file
	...
	public static void main(String[] args) {
		...
		Zhixiao2UI tZhixiao2UI = new Zhixiao2UI();
		tZhixiao2UI.submitData(tVData, "PRINT");
	}
}

...
public class GetReturnFromBankDealBL {
private static Logger logger = Logger.getLogger(GetReturnFromBankDealBL.class);
	/** 传入数据的容器 */
	...
	public GetReturnFromBankDealBL() {
	}
	...
	private boolean dealData() {
		...
		try {
			...
			if (mOperate.equals("GETMONEY")) {
				...
				logger.debug(tSql);
				if (!rsWrapper.prepareData(tLYReturnFromBankSet, sqlbv2)) {
...
//			sqlbv3.put("serialno1", Serialno);
	...
	private void getLYSendToBank(String paycode) {
		...
		String tSql = "select * from lysendtobank where paycode = '" + "?paycode1?"
				+ "' and serialno = '" + "?serialno1?" + "' ";
		SQLwithBindVariables sqlbv6=new SQLwithBindVariables();
		sqlbv6.sql(tSql);
		sqlbv6.put("paycode1", paycode);
		sqlbv6.put("serialno1", serialNo);
		outDelLYSendToBankSet = tLYSendToBankSchema.getDB().executeQuery(sqlbv6);
	}
	...
	private void getLYReturnFromBank(String paycode) {
		...
		String tSql = "select * from lyreturnfrombank where paycode = '"
				+ "?paycode2?" + "' and serialno = '" + "?serialNo2?" + "' ";
		SQLwithBindVariables sqlbv7=new SQLwithBindVariables();
		sqlbv7.sql(tSql);
		sqlbv7.put("paycode2", paycode);
		sqlbv7.put("serialno2", serialNo);
	...
	private void setLJTempFeeCont(
				...
				|| tLJSPaySchema.getOtherNoType().equals("5")) // 集体保单号
		{
			...
			for (int i = 1; i <= tLJSPayGrpSet.size(); i++) {
				...
				|| tLJSPaySchema.getOtherNoType().equals("03")) // 个人保单号
		{
			ExeSQL tExeSQL = new ExeSQL();
			SSRS tSSRS = new SSRS();
			...
			sqlbv8.sql(tSql);
			sqlbv8.put("GetNoticeNo", tLJSPaySchema.getGetNoticeNo());
			...
			for (int i = 1; i <= tSSRS.MaxRow; i++) {
				...
				if (tLYReturnFromBankBSchema.getNoType().equals("2")
						||
						// tLYReturnFromBankSchema.getNoType().equals("02") ||
						tLYReturnFromBankBSchema.getNoType().equals("3")
						|| tLYReturnFromBankBSchema.getNoType().equals("8")
				// tLYReturnFromBankSchema.getNoType().equals("03")
				) {
	...
	private void setLCContb(LYReturnFromBankBSchema tLYReturnFromBankBSchema) {
		...
		sqlbv9.sql(tSql);
		sqlbv9.put("customerno", tLCContSchema.getAppntNo());
	...
	private void updateLJTempFee(
		...
		sqlbv10.sql(tSql);
		sqlbv10.put("tempfeeno",tLYReturnFromBankBSchema.getPayCode());
		...
		sqlbv11.sql(strSql);
		sqlbv11.put("TempFeeNo",tLYReturnFromBankBSchema.getPayCode());
	...
	private boolean verifyLJSPay(
		...
		try {
			...
			if ((inLYReturnFromBankBSchema.getNoType().equals("6")
					...
					|| inLYReturnFromBankBSchema.getNoType().equals("03") || inLYReturnFromBankBSchema
					.getNoType().equals("08"))
					&& !inLYReturnFromBankBSchema.getConvertFlag().equals("1")) // 首期交费需要核销暂收费表
			{
				// 更新暂交费表
				...
				// setLOPRTManager(tLYReturnFromBankSchema, "48");
					...
					|| inLYReturnFromBankBSchema.getNoType().equals("3") || inLYReturnFromBankBSchema
					.getNoType().equals("8"))
...
//		sqlbv13.sql(tSQL);
//		sqlbv13.put("AccTotalMoney",totalMoney);
//		sqlbv13.put("BankSuccMoney", totalMoney);
//		sqlbv13.put("BankSuccNum", tSSRS.GetText(1, 1));
//		sqlbv13.put("ModifyTime", PubFun.getCurrentDate() );
//		sqlbv13.put("ModifyTime1", PubFun.getCurrentTime());
//		sqlbv13.put("serialno", serialNo);
	...
	public String sendBankCount(LYReturnFromBankBSchema tLYReturnFromBankBSchema) {
		...
		SQLwithBindVariables sqlbv13=new SQLwithBindVariables();
		sqlbv13.sql(sql);
		sqlbv13.put("getnoticeno", tLYReturnFromBankBSchema.getPayCode());
		ExeSQL tExeSQL = new ExeSQL();
		SSRS ssrs = new SSRS();
		ssrs = tExeSQL.execSQL(sqlbv13);

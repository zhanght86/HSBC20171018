...
public class LLUWIdeaBL {
private static Logger logger = Logger.getLogger(LLUWIdeaBL.class);
	...
	/** 业务处理类 */
//	private LLCUWMasterSchema mLCInsuredSchema = new LCInsuredSchema();
	private LLCUWMasterSchema mLLCUWMasterSchema = new LLCUWMasterSchema();
	private LLCUWMasterSchema mLLCUWMasterSchemaUpdate = new LLCUWMasterSchema();
...
//	private LCIndUWSubSchema mLCIndUWSubSchema = new LCIndUWSubSchema();
	...
	private boolean prepareOutputData() {
		...
		map.put(mLLCUWMasterSchemaUpdate, "UPDATE");
...
//		mLCInsuredSchema = tLCInsuredDB.getSchema();
	...
	private boolean getInputData(VData cInputData, String cOperate) {
		...
		// 获得核保结论
		mLLCUWMasterSchema = (LLCUWMasterSchema) mTransferData
				.getValueByName("LLCUWMasterSchema");
		if (mLLCUWMasterSchema == null) {
			// @@错误处理
			// this.mErrors.copyAllErrors( tLCPolDB.mErrors );
			CError.buildErr(this, "前台传输业务数据中LCIndUWMasterSchema失败!");
			return false;
		}
...
//		}
	...
	private boolean prepareIndUW() {
		...
		if (!tLLCUWMasterDB.getInfo()) {
			CError.buildErr(this, "查询合同核保主表失败！");
		}
			// 准备被保人核保主表数据
		mLLCUWMasterSchemaUpdate = tLLCUWMasterDB.getSchema();
//			int uwno = mLLCUWMasterSchemaNew.getUWNo();
		//	mLCIndUWMasterSchemaNew.setPassFlag(mLCIndUWMasterSchema
		//			.getPassFlag());
		mLLCUWMasterSchemaUpdate.setChangePolReason(mLLCUWMasterSchema.getChangePolReason());
//		mLLCUWMasterSchemaUpdate.setUWNo(uwno + 1);
		//	mLCIndUWMasterSchemaNew.setSugPassFlag(mLCIndUWMasterSchema
		//			.getSugPassFlag());
		//	mLCIndUWMasterSchemaNew.setSugUWIdea(mLCIndUWMasterSchema
		//			.getSugUWIdea());
		mLLCUWMasterSchemaUpdate.setOperator(mOperator); // 操作员		
		mLLCUWMasterSchemaUpdate.setModifyDate(PubFun.getCurrentDate());
		mLLCUWMasterSchemaUpdate.setModifyTime(PubFun.getCurrentTime());

			// 准备被保人核保子表数据
//			LCIndUWSubDB tLCIndUWSubDB = new LCIndUWSubDB();
//			String tSql = "select * from LCIndUWSub where contno='"+mContNo+"' and insuredno='"+mInsuredNo+"' order by uwno desc";
...
//			mLCIndUWSubSchema.setContNo(mLCIndUWMasterSchema.getContNo());
//			mLCIndUWSubSchema.setInsuredNo(mLCIndUWMasterSchema.getInsuredNo());

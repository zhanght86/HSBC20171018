 ...
 * @version 1.0
 */
public class RIIBCoDistillBQWD implements RIDistill {
	String mEventType;
	...
	private boolean dealData() {
		try {
			...
			buildError("dealData", " 提取共保保全領取业务数据失败 " + ex.getMessage());
			return false;
		}
		return true;
	}
	...
	private String getDistillSQL() {
		...
		strBuffer
				.append(" a.PolNo||','||b.edorno||','||b.edortype||','||'"
						+ mAccumulateDefNo + "'||','||'GBBQTB' as UnionKey, ");// 联合主键
		...
		strBuffer.append(" (SELECT max(Planlevel) FROM lcduty  WHERE polno=a.polno ) as PlanType, ");
		strBuffer.append(" '' as RiskPeriod, ");
		strBuffer.append(" '' as RiskType, ");
		strBuffer.append(" a.RiskCode as RiskCode, ");
		strBuffer.append(" (select min(dutycode) from lcduty where prem>0 and polno=a.polno ) as DutyCode, ");
		...
		strBuffer
				.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno  and AddFeeDirect is not null) as AdditionalRate, ");
		strBuffer
				.append(" (select max(AddFeeDirect) from LCPrem where polno=a.polno  and AddFeeDirect is not null) as ExtType, ");
		strBuffer
				.append(" (select sum(Prem) from LCPrem where polno=a.polno  and AddFeeDirect is not null) as ExtPrem, ");
		...
		strBuffer
				.append(" rigetplancode(a.contno,a.polno,(select max(dutycode) from lcduty where polno=a.polno )) as StandbyString1, ");
		strBuffer.append(" '' as standbystring2, ");
		strBuffer
				.append(" decode(b.edortype,'CT','CT','ZC') as StandbyString3, ");// a.PLANTYPE
		strBuffer
				.append(" nvl((SELECT sum(x.insuaccbala) FROM Lcinsureacc x where x.polno=a.polno),0) as StandbyDouble1, ");
		strBuffer
				.append(" 0 as StandbyDouble2, ");
		strBuffer
		...
		strBuffer.append(" b.GetMoney  as EdorPrem,");
		...
		/*strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno  and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno  and AddFeeDirect='02' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate2, ");
		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno  and AddFeeDirect='03' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate3, ");*/
		strBuffer.append("(SELECT CASE WHEN(A.POLNO IN (SELECT POLNO FROM RI_LOADING_TABLE)) THEN (SELECT MAX(M.SUPPRISKSCORE) FROM RI_LOADING_TABLE M WHERE M.POLICY = A.CONTNO AND M.POLNO = A.POLNO AND M.LOADINGTYPE = '01') ELSE (SELECT SUM(SUPPRISKSCORE) FROM LCPREM WHERE POLNO = A.POLNO AND ADDFEEDIRECT = '01' AND PAYENDDATE > '"+ mRIWFLogSchema.getEndDate()+ "') END FROM DUAL) AS EXTRATE1,");
		strBuffer.append("(SELECT CASE WHEN(A.POLNO IN (SELECT POLNO FROM RI_LOADING_TABLE)) THEN (SELECT MAX(M.SUPPRISKSCORE) FROM RI_LOADING_TABLE M WHERE M.POLICY = A.CONTNO AND M.POLNO = A.POLNO AND M.LOADINGTYPE = '02') ELSE (SELECT SUM(SUPPRISKSCORE) FROM LCPREM WHERE POLNO = A.POLNO AND ADDFEEDIRECT = '02' AND PAYENDDATE > '"+ mRIWFLogSchema.getEndDate()+ "') END FROM DUAL) AS EXTRATE2,");
		strBuffer.append("(SELECT CASE WHEN(A.POLNO IN (SELECT POLNO FROM RI_LOADING_TABLE)) THEN (SELECT MAX(M.SUPPRISKSCORE) FROM RI_LOADING_TABLE M WHERE M.POLICY = A.CONTNO AND M.POLNO = A.POLNO AND M.LOADINGTYPE = '03') ELSE (SELECT SUM(SUPPRISKSCORE) FROM LCPREM WHERE POLNO = A.POLNO AND ADDFEEDIRECT = '03' AND PAYENDDATE > '"+ mRIWFLogSchema.getEndDate()+ "') END FROM DUAL) AS EXTRATE3,");
		strBuffer.append(" (select sum(Prem) from LCPrem where polno=a.polno  and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtPrem1, ");
		strBuffer.append(" (select sum(Prem) from LCPrem where polno=a.polno  and AddFeeDirect='02' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtPrem2, ");
		strBuffer.append(" (select sum(Prem) from LCPrem where polno=a.polno  and AddFeeDirect='03' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtPrem3, ");
		...
		strBuffer
				.append("from lcpol a,lpedoritem b,LJAGetEndorse d ");
		strBuffer.append("where a.ContType='1' ");
		strBuffer.append("and a.contno = b.contno  ");
		strBuffer.append("and a.polno = d.polno and b.edorno=D.endorsementno and d.feefinatype='TF' ");
		...
		strBuffer
				.append(" and exists(select 'X' from RIAccumulateRDCode r where a.riskcode = r.associatedcode  and r.accumulatedefno='"
						+ mAccumulateDefNo + "') ");
		// 有效保单
		//strBuffer.append(" and exists(select 'X' from lccontstate where  StateType = 'Available' and State = '0' and enddate is null and polno=a.polno ) ");
		// 领取
		strBuffer
				.append(" and b.edortype   in('WD')  and b.edorstate='0'");
		// 不临分
		strBuffer
		...
		strBuffer.append(") ");
		System.out.println("共保退保提数sql: " + strBuffer.toString());
	...
	private String getDistillSQL1() {
		...
		strBuffer.append(" '" + mAccumulateDefNo + "' as AccumulateDefNO, ");
		strBuffer
				.append(" nvl((select RIContNo from RITempContLink where proposalno=a.polno ),'000000') as RIContNo, ");
		strBuffer
				.append(" nvl((select RIPreceptNo from RITempContLink where proposalno=a.polno ),'000000') as RIPreceptNo, ");
		strBuffer
				.append(" nvl((select State from ridutystate where proposalno=a.polno ),'02') as ReinsreFlag, ");
		// 业务日期
		strBuffer.append(" d.getdate as GetDate, ");
		strBuffer
				.append(" rigetplancode(a.contno,a.polno,(select max(dutycode) from lcduty where polno=a.polno )) as StandbyString1, ");
		...
		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno  and AddFeeDirect='02' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate2, ");
		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno  and AddFeeDirect='03' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate3, ");*/
		strBuffer.append("(select case when (a.polno in (select polno from lcprem where polno= a.polno and AddFeeDirect = '01' and PayEndDate > '"+ mRIWFLogSchema.getEndDate()+ "' and suppriskscore>0)) then (case when (a.polno in (select polno from RI_LOADING_TABLE)) then (select max(m.SuppRiskScore) FROM RI_LOADING_TABLE m where m.polno = A.polno and m.loadingtype = '01') else (select sum(SuppRiskScore) FROM lcprem where polno= a.polno AND AddFeeDirect = '01' and PayEndDate > '"+ mRIWFLogSchema.getEndDate()+ "') end) else 0 end from dual) as ExtRate1,"); 
		strBuffer.append("(select case when (a.polno in (select polno from lcprem where polno= a.polno and AddFeeDirect = '02' and PayEndDate > '"+ mRIWFLogSchema.getEndDate()+ "' and suppriskscore>0)) then (case when (a.polno in (select polno from RI_LOADING_TABLE)) then (select max(m.SuppRiskScore) FROM RI_LOADING_TABLE m where m.polno = A.polno and m.loadingtype = '02') else (select sum(SuppRiskScore) FROM lcprem where polno= a.polno AND AddFeeDirect = '02' and PayEndDate > '"+ mRIWFLogSchema.getEndDate()+ "') end) else 0 end from dual) as ExtRate2,"); 
		strBuffer.append("(select case when (a.polno in (select polno from lcprem where polno= a.polno and AddFeeDirect = '03' and PayEndDate > '"+ mRIWFLogSchema.getEndDate()+ "' and suppriskscore>0)) then (case when (a.polno in (select polno from RI_LOADING_TABLE)) then (select max(m.SuppRiskScore) FROM RI_LOADING_TABLE m where m.polno = A.polno and m.loadingtype = '03') else (select sum(SuppRiskScore) FROM lcprem where polno= a.polno AND AddFeeDirect = '03' and PayEndDate > '"+ mRIWFLogSchema.getEndDate()+ "') end) else 0 end from dual) as ExtRate3,");
		strBuffer.append(" (select sum(Prem) from LCPrem where polno=a.polno  and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtPrem1, ");
		strBuffer.append(" (select sum(Prem) from LCPrem where polno=a.polno  and AddFeeDirect='02' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtPrem2, ");
		strBuffer.append(" (select sum(Prem) from LCPrem where polno=a.polno  and AddFeeDirect='03' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtPrem3, ");
		strBuffer.append(" '' as StandbyString4, ");
		...
		strBuffer.append(" and a.Currency='01'");
		strBuffer.append(") ");
		System.out.println("共保領取临分提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	public static void main(String[] args) {
		...
		mRIWFLogSchema.setStartDate("2001-11-02");
		mRIWFLogSchema.setEndDate("2022-11-23");
		mRIWFLogSchema.setTaskCode("L01010060010");
		cInputData.add(mRIWFLogSchema);
		RIIBCoDistillBQWD tRIDistillRN = new RIIBCoDistillBQWD();
		tRIDistillRN.test(cInputData, "01");
	}
}

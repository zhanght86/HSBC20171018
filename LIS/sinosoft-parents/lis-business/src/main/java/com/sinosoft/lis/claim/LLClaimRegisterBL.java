 ...
 * @version 1.0
 */
public class LLClaimRegisterBL {
...
//				}
		...
		if (cOperate.equals("UPDATE")) {
			...
			{
				// @@错误处理
				CError.buildErr(this, "查询"+mLLAccidentSchema.getAccNo()+"事件信息失败!");
				return false;
			}

			tLLAccidentDB.setAccDate(mLLRegisterSchema.getAccidentDate());// 事故日期
			tLLAccidentDB.setModifyDate(PubFun.getCurrentDate());
			tLLAccidentDB.setModifyTime(PubFun.getCurrentTime());
			mLLAccidentSchema.setSchema(tLLAccidentDB.getSchema());
			mLLAccidentSchema.setAccPlace(mLLRegisterSchema.getAccidentSite());
			mLLAccidentSchema.setAccDesc(mLLCaseSchema.getAccdentDesc());
			...
			mLLRegisterSchema.setRgtClass(tLLRegisterDB.getRgtClass());
			...
			mLLRegisterSchema.setRgtSources(tLLRegisterDB.getRgtSources());
			...
			mLLRegisterSchema.setModifyDate(mLLAccidentSchema.getModifyDate());
			mLLRegisterSchema.setModifyTime(mLLAccidentSchema.getModifyTime());
			...
			mLLCaseSchema.setModifyDate(mLLAccidentSchema.getModifyDate());
			mLLCaseSchema.setModifyTime(mLLAccidentSchema.getModifyTime());

			// ------------------------------------------------------------------BEG
			// 理赔类型表,首先删除所有该分案下的所有理赔类型,然后再添加前台更改后数据
			// ------------------------------------------------------------------
			deleteSql = " delete from LLAppClaimReason where " + " RgtNo = '"
					+ "?RgtNo?" + "'" + " and CustomerNo = '"
					+ "?CustomerNo?" + "'";
			SQLwithBindVariables sqlbv = new SQLwithBindVariables();
			sqlbv.sql(deleteSql);
			sqlbv.put("RgtNo", mLLCaseSchema.getRgtNo());
			sqlbv.put("CustomerNo", mLLCaseSchema.getCustomerNo());
			// 先删除伤残打印数据
			String tsql = "delete from LOPRTManager where " + " otherno = '"
					+ "?otherno?" + "'" + " and code = 'PCT001'";
			SQLwithBindVariables sqlbv1 = new SQLwithBindVariables();
			sqlbv1.sql(tsql);
			sqlbv1.put("otherno", mRptNo);
			...
			for (int i = 1; i <= mLLAppClaimReasonSet.size(); i++) {
				...
				if (tLLAppClaimReasonDB.getMakeDate() == null) {
					...
					mLLAppClaimReasonSet.get(i).setSchema(
							tLLAppClaimReasonDB.getSchema());
				}
			}
	...
	private boolean CreatStartMission(){
		ExeSQL tExeSQL = new ExeSQL();
		SQLwithBindVariables sqlbv2 = new SQLwithBindVariables();
		sqlbv2.sql("select busitype,activityid from lwactivity where functionid='10030003'");
		SSRS tSSRS = tExeSQL.execSQL(sqlbv2);
		if(tSSRS == null || tSSRS.MaxRow == 0){
			CError.buildErr(this, "查询工作流节点失败!");
			return false;
		}
		String BusiType = (String)mTransferData.getValueByName("BusiType");
		...
		this.mTransferData.setNameAndValue("Version", Version);
		...
		try{
			...
			if(tActivityOperatorNode.Create(tMissionID, tSubMissionID, tVData)){
				...
				if(tMap == null){
					CError.buildErr(this, "接受立案确认工作流节点信息失败!");
					return false;
				}else{
					this.map.add(tMap);						
				}
	...
	private void IsExist() {
		...
		//      mIsReportExist = true;
				...
				sqlbv5.sql(strSql);
				sqlbv5.put("RgtNo", mLLRegisterSchema.getRgtNo());
				ExeSQL exesql = new ExeSQL();
				String tResult = exesql.getOneValue(sqlbv5);
				if (tResult.length() > 0) {

...
//  private LLCaseReceiptDB mLLCaseReceiptDB = new LLCaseReceiptDB();
    ...
    public boolean submitData(VData cInputData, String cOperate)
    {
        ...
        if (!tPubSubmit.submitData(mInputData, ""))
        {
            ...
            tError.moduleName = "LLMedicalFeeInp1BL";
    ...
    private boolean dealData()
    {
        ...
        if (this.mOperate.equals("DELETE"))
        {
            ...
            tLLCaseReceiptDB.setMainFeeNo(mLLCaseReceiptSchema.getMainFeeNo()); //客户号
            ...
            if (tLLCaseReceiptSet == null && tLLCaseReceiptSet.size() == 0)
            {
                ...
                tError.errorMessage = "查询LLCaseReceipt表失败!";
                this.mErrors.addOneError(tError);
                return false;
            }
            else
    ...
    private boolean medicalInsert()
    {
        ...
        tLLFeeMainDB.setMainFeeNo(mLLFeeMainSchema.getMainFeeNo()); //账单号
        ...
        if (tLLFeeMainSet == null || tLLFeeMainSet.size() == 0)
        {
            ...
            tLLFeeMainSchema.setMainFeeNo(mLLFeeMainSchema.getMainFeeNo());
            ...
            LLCommendHospitalSchema tLLCommendHospitalSchema =
                    tLLCommendHospitalSet.get(1);
            tLLFeeMainSchema.setHospitalName(tLLCommendHospitalSchema.
                                             getHospitalName());
            tLLFeeMainSchema.setHospitalGrade(tLLCommendHospitalSchema.
                                              getHosAtti());
...
//            tLLFeeMainSchema.setMainFeeNo(tMainFeeNo);
            ...
            tLLCaseReceiptSchema.setCustomerNo(mLLFeeMainSchema.getCustomerNo());
            //住院起付线
            tLLCaseReceiptSchema.setHospLineAmnt(mLLCaseReceiptSchema.getHospLineAmnt());
            //  生成账单费用明细流水号
            String tFeeDetailNo = PubFun1.CreateMaxNo("FeeDetailNo", 10);
            tLLCaseReceiptSchema.setFeeDetailNo(tFeeDetailNo);

            tLLCaseReceiptSchema.setFeeItemType(mLLCaseReceiptSchema.
                                                getFeeItemType());
            tLLCaseReceiptSchema.setFeeItemCode(mLLCaseReceiptSchema.
                                                getFeeItemCode());
            tLLCaseReceiptSchema.setFeeItemName(mLLCaseReceiptSchema.
                                                getFeeItemName());
            ...
            logger.debug("----AdjReason----"+mLLCaseReceiptSchema.getAdjReason());
            logger.debug("----AdjRemark----"+mLLCaseReceiptSchema.getAdjRemark());
            logger.debug("----securityeamnt----"+mLLCaseReceiptSchema.getSecurityAmnt());
            logger.debug("----securityamnt----"+mLLCaseReceiptSchema.getCutApartAmnt());
            logger.debug("----refuseamnt----"+mLLCaseReceiptSchema.getRefuseAmnt());
            double adjsum=mLLCaseReceiptSchema.getFee()-mLLCaseReceiptSchema.getRefuseAmnt();
            double cutamnt=adjsum-mLLCaseReceiptSchema.getSecurityAmnt();
            logger.debug("==========="+adjsum);
            logger.debug("==========="+cutamnt);
            //tLLCaseReceiptSchema.setAdjSum(mLLCaseReceiptSchema.getAdjSum());
            tLLCaseReceiptSchema.setAdjSum(adjsum);
            tLLCaseReceiptSchema.setSecurityAmnt(mLLCaseReceiptSchema.getSecurityAmnt());
            //tLLCaseReceiptSchema.setCutApartAmnt(mLLCaseReceiptSchema.getCutApartAmnt());
            tLLCaseReceiptSchema.setCutApartAmnt(cutamnt);
            tLLCaseReceiptSchema.setRefuseAmnt(mLLCaseReceiptSchema.getRefuseAmnt());
            // 设置住院天数
            ...
            {
                tLLCaseReceiptSchema.setDayCount(mLLCaseReceiptSchema.getDayCount());
            }
//            tLLCaseReceiptSchema.setDealFlag("01");
        ...
        else if (tLLFeeMainSet.size() == 1)
        {
            ...
            tLLCaseReceiptSchema.setMainFeeNo(mLLCaseReceiptSchema.getMainFeeNo());
            tLLCaseReceiptSchema.setCustomerNo(mLLCaseReceiptSchema.getCustomerNo());
            //住院起付线
            tLLCaseReceiptSchema.setHospLineAmnt(mLLCaseReceiptSchema.getHospLineAmnt());
            ...
            */
            ...
            logger.debug("----feeno----"+mLLCaseReceiptSchema.getFee());
            logger.debug("----Adjsum----"+mLLCaseReceiptSchema.getAdjSum());
            logger.debug("----securityeamnt----"+mLLCaseReceiptSchema.getSecurityAmnt());
            logger.debug("----securityamnt----"+mLLCaseReceiptSchema.getCutApartAmnt());
            logger.debug("----refuseamnt----"+mLLCaseReceiptSchema.getRefuseAmnt());
            double adjsum=mLLCaseReceiptSchema.getFee()-mLLCaseReceiptSchema.getRefuseAmnt();
...
//            tLLCaseReceiptSchema.setDealFlag("01");
            tLLCaseReceiptSchema.setDealFlag(mLLCaseReceiptSchema.getDealFlag());
            //初始化时设置调整金额为初始金额
//            tLLCaseReceiptSchema.setAdjSum(mLLCaseReceiptSchema.getFee());

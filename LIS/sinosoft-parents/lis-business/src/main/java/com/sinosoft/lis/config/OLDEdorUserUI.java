...
public class OLDEdorUserUI implements BusinessService{
private static Logger logger = Logger.getLogger(OLDEdorUserUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		 ...
		 */
		OLDEdorUserBL tOLDEdorUserBL = new OLDEdorUserBL();

		// logger.debug("Start OLDUWUser UI Submit...");
		tOLDEdorUserBL.submitData(cInputData, mOperate);
		// logger.debug("End OLDUWUser UI Submit...");
		// 如果有需要处理的错误，则返回
		if (tOLDEdorUserBL.mErrors.needDealError()) {
			// @@错误处理
			this.mErrors.copyAllErrors(tOLDEdorUserBL.mErrors);
		...
		if (mOperate.equals("INSERT||MAIN")) {
			this.mResult.clear();
			this.mResult = tOLDEdorUserBL.getResult();
		}
		mInputData = null;
		this.mResult.clear();
		return true;

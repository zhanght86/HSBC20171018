...
/**
 * <p>Title: </p>
 * <p>Description: 单证打印：PCT018-----调查任务通知书------InquiryNoticeC000220.vts</p>
...
private static Logger logger = Logger.getLogger(LLPRTInteInqTaskBL.class);
    ...
    private boolean dealData()
    {
      ...
      //#############################################################################
       ...
       String tManageComNameSql = "select name from LDCom where ComCode='"+mngCom+"'";
//       String tManageComNameSql="select codename from ldcode where codetype='station' and code='"+mngCom+"'";
       ExeSQL comExeSQL = new ExeSQL();
       String tManageComName =comExeSQL.getOneValue(tManageComNameSql);

       //是否异地调查-----0---本地（否）；1----异地（是）
       String LocFlag=tLLInqApplyDB.getLocFlag();
       if(LocFlag==null){LocFlag="";}
       String tLocFlag="";
       if(LocFlag.equals("0"))
       {
           tLocFlag="否";
...
//       tLLCaseDB.getInfo();
       ...
       String tAccNoSql="select CaseRelaNo from LLCaseRela where CaseNo='"+mClmNo+"'";
       ExeSQL tAccNoExeSQL = new ExeSQL();
       String tAccNo=tAccNoExeSQL.getOneValue(tAccNoSql);
       //查询 事件表（LLAccident）----查询  出险时间，申请人陈述出险情况（事故描述）
       String tAccidentSql="select *  from LLAccident where accno ='"+tAccNo+"'";
...
//        contSQL = "select * from lccont where contno in (select distinct contno from llclaimdetail where clmno='"+mClmNo+"')";
        ...
        tLCContSet.set(tLCContDB.executeQuery(contSQL));
...
//            }
       ...
       //根据 出险人客户号 查询“赔案号”
        String tClmSql="select * from LLRegister a where 1=1"
                       + "and a.RgtNo !='"+mClmNo+"'"
                       +" and a.RgtNo in (select t.CaseNo from LLCase t where t.CaseNo!='"+mClmNo+"' and t.CustomerNo = '"+ mCusNo +"')";
       logger.debug("--------“既往赔案情况”查询语句===----"+tClmSql);
       LLRegisterDB tLLRegisterDB = new LLRegisterDB();
       LLRegisterSet tLLRegisterSet = tLLRegisterDB.executeQuery(tClmSql);
       //查询出“赔案”数目 作为外层循环
...
//               ttLLCaseDB.getInfo();
              //查询事件号-----用“赔案号”查询 分案事件关联---LLCaseRela
              String ttAccNoSql = "select CaseRelaNo from LLCaseRela where CaseNo='" + ttRgtNo + "'";
              ExeSQL ttAccNoExeSQL = new ExeSQL();
              String ttAccNo =  ttAccNoExeSQL.getOneValue(ttAccNoSql);
              //查询 事件表（LLAccident）----查询  出险时间，申请人陈述出险情况（事故描述）
              ...
              if (ttAccdentDesc == null)
              {
               ...
               String ttreason = ttLLPRTPubFunBL.getSLLReportReason(ttRgtNo,mCusNo);
...
//        tXmlExport.outputDocumentToFile("D:/ui/f1print/NCLtemplate/", "testHZM"); //输出xml文档到文件
        mResult.clear();

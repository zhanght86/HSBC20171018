...
private static Logger logger = Logger.getLogger(NewGrpPolFeeWithdrawBLS.class);
	...
	private boolean save(VData mInputData) {
		...
		try {
			...
			/*
			 * // 打印管理表 logger.debug("Start 打印管理表..."); LOPRTManagerDBSet
			 * tLOPRTManagerDBSet=new LOPRTManagerDBSet(conn);
			 * tLOPRTManagerDBSet.set((LOPRTManagerSet)mInputData.getObjectByObjectName("LOPRTManagerSet",0));
			 * logger.debug("Get LOPRTManager"); if
			 * (!tLOPRTManagerDBSet.insert()) { // @@错误处理
			 * this.mErrors.copyAllErrors(tLOPRTManagerDBSet.mErrors); CError
			 * tError = new CError(); tError.moduleName =
			 * "NewGrpPolFeeWithdrawBLS"; tError.functionName = "saveData";
			 * tError.errorMessage = "打印管理表数据保存失败!"; this.mErrors
			 * .addOneError(tError) ; conn.rollback(); conn.close(); return
			 * false; }
			 */
			conn.commit();
			conn.close();

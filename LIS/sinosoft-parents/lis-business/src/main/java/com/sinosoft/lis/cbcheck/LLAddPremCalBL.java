...
public class LLAddPremCalBL {
private static Logger logger = Logger.getLogger(LLAddPremCalBL.class);
	...
	// 保费项表
	private LLUWPremMasterSchema mLLUWPremMasterSchema = new LLUWPremMasterSchema();
	// 责任表
	...
	private boolean checkData() {
		double FirstScore = mLLUWPremMasterSchema.getSuppRiskScore();
		double SecondScore = mLLUWPremMasterSchema.getSecInsuAddPoint();
	...
	private boolean getInputData(VData cInputData, String cOperate) {
		...
		mLCPolSchema = (LCPolSchema) cInputData.getObjectByObjectName(
				"LCPolSchema", 0);
		mLLUWPremMasterSchema = (LLUWPremMasterSchema) cInputData.getObjectByObjectName(
				"LLUWPremMasterSchema", 0);
		...
		if (mBatNo == null || "".equals(mBatNo)) {
			CError.buildErr(this,"前台传输被保险人BatNo失败!");
			return false;
			
		}
		if (mClmNo == null || "".equals(mClmNo)) {
			CError.buildErr(this,"前台传输被保险人ClmNo失败!");
			return false;
...
//		准备BOMCont数据
		...
		insured = DealBOMInsured(tLCInsuredDB.getSchema(),mLLUWPremMasterSchema);
		...
		mCalBase.setInsuredNo(mLCPolSchema.getInsuredNo());
		mCalBase.setPayEndYear(mLCPolSchema.getPayEndYear());
		mCalBase.setPayEndYearFlag(mLCPolSchema.getPayEndYearFlag());
		mCalBase.setSuppRiskScore(mLLUWPremMasterSchema.getSuppRiskScore());
		...
		mCalBase.setFirstScore(mLLUWPremMasterSchema.getSuppRiskScore());
		...
		if (mLCPolSchema.getInsuredSex() != null
			...
			mCalBase.setHusbandScore(mLLUWPremMasterSchema.getSuppRiskScore());
			mCalBase.setWifeScore(mLLUWPremMasterSchema.getSecInsuAddPoint());
		} else {
			mCalBase.setHusbandScore(mLLUWPremMasterSchema.getSecInsuAddPoint());
			mCalBase.setWifeScore(mLLUWPremMasterSchema.getSuppRiskScore());
...
*/
	...
	private BOMInsured DealBOMInsured(LCInsuredSchema tLCInsuredSchema,LLUWPremMasterSchema mLLUWPremMasterSchema){
		BOMInsured Insured = new BOMInsured();
		ExeSQL tempExeSQL = new ExeSQL();
		try{
			...
			sqlbv1.put("contno", tLCInsuredSchema.getContNo());
			...
			Insured.setSecInsuAddPoint(mLLUWPremMasterSchema.getSecInsuAddPoint());
			// 针对险种144
			if (mLCPolSchema.getInsuredSex() != null
					&& mLCPolSchema.getInsuredSex().equals("0")) {
				Insured.setEM(mLLUWPremMasterSchema.getSuppRiskScore());
				Insured.setSecInsuAddPoint(mLLUWPremMasterSchema.getSecInsuAddPoint());
			} else {
				Insured.setEM(mLLUWPremMasterSchema.getSecInsuAddPoint());
				Insured.setSecInsuAddPoint(mLLUWPremMasterSchema.getSuppRiskScore());
	...
	private double getVPU(String sDutyCode) {
		...
		sqlbv2.sql(sql);
		sqlbv2.put("dutycode", sDutyCode);
		ExeSQL tExeSQL = new ExeSQL();
		String sVPU = tExeSQL.getOneValue(sqlbv2);
	...
	private String getAppAg2(String tPolNo, String tInsuredNo) {
		...
		sqlbv3.put("insuredno", tInsuredNo);
		ExeSQL tExeSQL = new ExeSQL();
		AppAg2 = tExeSQL.getOneValue(sqlbv3);
		if (tExeSQL.mErrors.needDealError()) {

 ...
 * @date 2002-09-25
 */
public class GroupPlanRiskUI {
private static Logger logger = Logger.getLogger(GroupPlanRiskUI.class);
	/** 传入数据的容器 */
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		GroupPlanRiskBL tGroupPlanRiskBL = new GroupPlanRiskBL();

		logger.debug("---UI BEGIN---");
		if (tGroupPlanRiskBL.submitData(cInputData, mOperate) == false) {
			// @@错误处理
			this.mErrors.copyAllErrors(tGroupPlanRiskBL.mErrors);
			return false;
		} else {
			mResult = tGroupPlanRiskBL.getResult();
		}

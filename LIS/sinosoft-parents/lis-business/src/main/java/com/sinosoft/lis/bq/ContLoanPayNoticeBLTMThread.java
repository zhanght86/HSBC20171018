...
public class ContLoanPayNoticeBLTMThread extends CovBase {
	private static Logger logger = Logger.getLogger(ContLoanPayNoticeBLTMThread.class);
	...
	public ContLoanPayNoticeBLTMThread() {
	}
		...
		for(int i=0;i<this.mInputDataNew.size();i++)
		{
			...
			if (!DealOneLoan(tLOLoanSchema)) {
				logger.debug(mErrors.getFirstError()
						+ " 保单借款催付批处理，保单号：" + tLOLoanSchema.getContNo());
				tFail++;
				// 日志监控,状态监控                 		
     			PubFun.logState(tGI,tLOLoanSchema.getEdorNo(), "保单"+tLOLoanSchema.getContNo()+"的"+tLOLoanSchema.getEdorNo()+"保单借款催付批处理，催付处理失败","0");  
				continue;
			}
...
//			 日志监控,状态监控                 		
		...
		logger.debug("\t@> PEdorValidTask.EdorTestFinish() : 保单借款催付批处理");
	    logger.debug("\t   保单借款催付批处理: " + tSuc + " 成功; " + tFail + " 失败。");
//		日志监控,结果监控
		PubFun.logResult (tGI,tGI.LogID[1],"保单借款催付批处理完毕: " + tSuc + " 成功; " + tFail + " 失败。");
		this.close();

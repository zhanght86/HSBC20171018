...
public class HospitalQualityQuery1UI {
private static Logger logger = Logger.getLogger(HospitalQualityQuery1UI.class);
	public CErrors mErrors = new CErrors();
	private VData mResult = new VData();

	public HospitalQualityQuery1UI() {
	}

	public boolean submitData(VData cInputData, String cOperate) {
		HospitalQualityQuery1BL tHospitalQualityQuery1BL = new HospitalQualityQuery1BL();
		if (!tHospitalQualityQuery1BL.submitData(cInputData, cOperate)) {
			mErrors.copyAllErrors(tHospitalQualityQuery1BL.mErrors);
			return false;
		} else {
			mResult = tHospitalQualityQuery1BL.getResult();

/*
 * <p>ClassName: LLGrpMedicalFeeInpForAccBL </p>
 * <p>Description: LLGrpMedicalFeeInpForAccBL类文件 </p>
...
public class LLGrpMedicalFeeInpForAccBL
{
private static Logger logger = Logger.getLogger(LLGrpMedicalFeeInpForAccBL.class);
    ...
    public LLGrpMedicalFeeInpForAccBL()
    {
    }
    ...
    public boolean submitData(VData cInputData, String cOperate)
    {
        ...
        if (!getInputData(cInputData))
        {
            ...
            tError.moduleName = "llgrpmedicalfeeinpforaccbl";
            tError.functionName = "submitData";
            tError.errorMessage = "数据处理失败llgrpmedicalfeeinpforaccbl-->getInputData!";
        ...
        if (!prepareOutputData())
        {
            ...
            tError.moduleName = "llgrpmedicalfeeinpforaccbl";
            tError.functionName = "submitData";
            tError.errorMessage = "数据处理失败llgrpmedicalfeeinpforaccbl-->submitquery!";
            this.mErrors.addOneError(tError);
            return false;
        }
        logger.debug(
    ...
    private boolean dealData()
    {
        ...
        if (this.mOperate.equals("INSERT")||this.mOperate.equals("IMPORT||MAIN"))
        {
                     ...
                     mLLFeeMainSchema = mLLFeeMainSet.get(i);
                        ...
                        LLCommendHospitalSet tLLCommendHospitalSet = tLLCommendHospitalDB.query();
                         ...
                         if (tLLCommendHospitalSet != null &&
                             ...
                             // @@错误处理
                             this.mErrors.copyAllErrors(tLLCommendHospitalDB.
                                     mErrors);
                             CError tError = new CError();
                             tError.moduleName =
                                     "LLClaimConfirmAfterInitService";
        ...
        /******************************************************************************************/
               logger.debug("-------得到帐户信息：---------"+mLLCaseReceiptSet.size());
               /*SQL2 = "delete from LLCaseReceipt where ClmNo = '" +
                          mLLCaseReceiptSet.get(1).getClmNo()+ "' and mainfeeno != '0000000000'";
               map.put(SQL2, "DELETE");*/
          ...
          if (mLLCaseReceiptSet!=null&&mLLCaseReceiptSet.size() > 0)
          {
              for(int i=1;i<=mLLCaseReceiptSet.size();i++)
              {
                  ...
                  // 设置住院天数
                    ...
                    mLLCaseReceiptSchema.setAdjSum(Fee-RefuseAmnt);//合理费用
                    //if(SecurityAmnt > 0){
                        mLLCaseReceiptSchema.setCutApartAmnt(Fee-RefuseAmnt-SecurityAmnt);//合理费用
                    //}
                    mLLCaseReceiptSchema.setOperator(this.mGlobalInput.Operator);
                    mLLCaseReceiptSchema.setMngCom(this.mGlobalInput.ManageCom);
                    mLLCaseReceiptSchema.setMakeDate(PubFun.getCurrentDate());
    ...
    private boolean getInputData(VData cInputData)
    {
        this.mLLFeeMainSet.set((LLFeeMainSet) cInputData.getObjectByObjectName(
                "LLFeeMainSet", 0));
        this.mLLCaseReceiptSet.set((LLCaseReceiptSet) cInputData.getObjectByObjectName(
                    "LLCaseReceiptSet", 0));

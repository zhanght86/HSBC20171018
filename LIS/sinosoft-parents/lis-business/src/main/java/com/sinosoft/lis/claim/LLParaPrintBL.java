...
private static Logger logger = Logger.getLogger(LLParaPrintBL.class);
	...
	private String mPrtType;// 打印单证类型  PCT014---预付
	...
	private boolean getInputData() {
		...
		mTransferData = (TransferData) mInputData.getObjectByObjectName(
				"TransferData", 0);
		// mLLParaPrintSchema =
		// (LLParaPrintSchema)mInputData.getObjectByObjectName("LLParaPrintSchema", 0);
		if (mG == null) {
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLParaPrintBL";
		...
		mPrtType = (String) mTransferData.getValueByName("PrtType");
		return true;
	}
	...
	private boolean dealData(String cOperate) {
		...
		try {
			...
			logger.debug("单证" + tLLParaPrintDB.getPrtName() + "的打印服务类为"
					+ tPrtBL);
			...
			tTransferData.setNameAndValue("PrtType", mPrtType);//单证类型
			...
			if (!ps.submitData(vData, "")) {
				...
				map =(MMap) mResult.getObjectByObjectName("MMap", 1);
			}
	...
	public VData getResult() {
		...
		XmlExportNew tXmlExportNew=(XmlExportNew)mResult.getObjectByObjectName("XmlExportNew",0);
		mResult.clear();

...
public class PersonBillBL implements PrintService {
private static Logger logger = Logger.getLogger(PersonBillBL.class);
	...
	private boolean getPrintData() {
		...
		String remark = "PersonBill.vts";

		newXmlExport.createDocuments(remark, mGlobalInput); // 最好紧接着就初始化xml文档
		newXmlExport.setTemplateName("PersonBill.vts");
		...
		if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
			strSQL =
					...
					"select '',a.proposalcontno,a.contno,a.appntname,a.insuredname,a.prem, "
							+ " a.agentcode,d.Name,h.name,d.branchcode,to_char(a.signdate,'yyyy-mm-dd'),h.branchattr,"
							+ " (Select comcode From ldcom where a.managecom like concat(trim(comcode),'%') and comgrade = '02' and rownum = 1) subcom,"
							+ " (Select Name From ldcom where a.managecom like concat(trim(comcode),'%') and comgrade = '02' and rownum = 1) subcomname,"
							+ " (select comcode from ldcom where a.managecom like concat(trim(comcode),'%') and comgrade = '03' and rownum = 1) subcenter,"
							+ " (select name from ldcom where a.managecom like concat(trim(comcode),'%') and comgrade = '03' and rownum = 1) subcentername,"
							...
							+ " from lccont a,laagent d,labranchgroup h,ldcontinvoicemap l where l.batchno = '"
							+ "?prtSeq?"
			...
			strSQL =
					...
					"select '',a.proposalcontno,a.contno,a.appntname,a.insuredname,a.prem, "
							+ " a.agentcode,d.Name,h.name,d.branchcode,to_char(a.signdate,'yyyy-mm-dd'),h.branchattr,"
							+ " (Select comcode From ldcom where a.managecom like concat(trim(comcode),'%') and comgrade = '02' limit 0,1) subcom,"
							+ " (Select Name From ldcom where a.managecom like concat(trim(comcode),'%') and comgrade = '02' limit 0,1) subcomname,"
							+ " (select comcode from ldcom where a.managecom like concat(trim(comcode),'%') and comgrade = '03' limit 0,1) subcenter,"
							+ " (select name from ldcom where a.managecom like concat(trim(comcode),'%') and comgrade = '03' limit 0,1) subcentername,"
							+ " (select b.branchattr from labranchgroup b,labranchgroup c"
							+ " where b.agentgroup = c.upbranch and c.agentgroup = h.upbranch) branchattr1, (select b.name from labranchgroup b,labranchgroup c"
							+ " where b.agentgroup = c.upbranch and c.agentgroup = h.upbranch) branchattrname1,"
							+ " (select e.branchattr from labranchgroup e where e.agentgroup = h.upbranch) branchattr2,"
		...
		try {
			...
			for (int i = 1; i <= i_count; i++) {
						...
						|| !newGroup.equals(oldGroup)) // 区总数
				{
					...
					xmlexport.createDocument("PersonBill.vts", "printer");
					if (i == 1) // 加标题，日期，总数
					{
			...
			logger.debug("清单打印成功！");
			// newXmlExport.outputDocumentToFile("d:\\","PersonBillSucc"+PubFun.getCurrentTime().toString().replaceAll(":","_"));//输出xml文档到文件
	...
	public static void main(String[] args) {
		...
		PersonBillBL tPersonBillBL = new PersonBillBL();

...
public class RIItemCalDetailBL implements BusinessService {
	...
	private boolean getInputData(VData cInputData) {
		globalInput.setSchema((GlobalInput) cInputData.getObjectByObjectName(
				"GlobalInput", 0));
		this.cRIItemCalSchema.setSchema((RIItemCalSchema) cInputData
				.getObjectByObjectName("RIItemCalSchema", 0));
		return true;
	}
	...
	private boolean dealData(String cOperate) {
		if (cOperate.equals("INSERT")) {
			mMap.put(cRIItemCalSchema, "INSERT");
			this.mInputData.clear();
			this.mInputData.add(mMap);
			if (!tPubSubmit.submitData(this.mInputData, "")) {
				if (tPubSubmit.mErrors.needDealError()) {
					// @@错误处理
					buildError("insertData", "保存算法信息时出现错误!");
					return false;
				}
			}
			mMap = null;
		...
		if (cOperate.equals("UPDATE")) {
			String sql = "delete from RIItemCal r where r.ArithmeticDefID='"
					+ cRIItemCalSchema.getArithmeticDefID() + "' "
					+ "and r.ArithmeticID='"
					+ cRIItemCalSchema.getArithmeticID()
					+ "' and r.CalItemOrder='"
					+ cRIItemCalSchema.getCalItemOrder() + "'";
			mMap.put(sql, "DELETE");
			mMap.put(cRIItemCalSchema, "INSERT");
			this.mInputData.clear();
			this.mInputData.add(mMap);

...
public class GrpChequeNoLisBL {
private static Logger logger = Logger.getLogger(GrpChequeNoLisBL.class);
	...
	private void buildError(String szFunc, String szErrMsg) {
		...
		cError.moduleName = "GrpChequeNoLisBL";
	...
	private boolean getPrintData() {
			...
			for (int j = 0; j < ZhongSSRS.getMaxRow(); j++) {
				...
				for (int k = 0; k < YingSSRS.getMaxRow(); k++) {
					...
					strSQL = "select a.MakeDate,(select codename"
							...
							+ " a.paymoney,"
							+ " nvl((select ldcode1.codename from ldcode1 where trim(ldcode1.code1) = d.banksuccflag and ldcode1.code = d.bankcode and codetype = 'bankerror'),'不成功') "
							+ " from ljtempfeeclass a, ljtempfee b,"
							+ " LYBankLog c,lyreturnfrombankb d "
							+ " where a.paymode = '3' "
							...
							+ " And d.PolNo=a.otherno "
							+ " And a.ChequeDate=c.MakeDate "
		...
		xmlexport.createDocument("GrpChequeNoLis.vts", "printer"); // appoint
																	// to a
																	// special
																	// output
																	// .vts file

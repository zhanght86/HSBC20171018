...
private static Logger logger = Logger.getLogger(GrpEdorGBPrintBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "GB数据提取失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "GB数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		ListTable tGBListTable = new ListTable();
		tGBListTable.setName("GB");

		String tSql = " select distinct d.name,"
					+ " c.GetYear,"
					+ " decode(c.getyearflag,'A','岁','Y','年','其它'),"
					+ " b.getyear,"
					+ " decode(b.getyearflag,'A','岁','Y','年','其它')"
					+ " from lpget a, lppol b, lcpol c, lcinsured d"
					+ " where b.polno = a.polno"
					+ " and b.polno = b.polno"
					+ " and a.polno = c.polno "
					+ " and b.contno = d.contno"
					+ " and a.contno = d.contno"
					+ " and c.contno = c.contno"
					+ " and a.edorno = b.edorno "
					+ " and a.edortype = b.edortype"
					+ " and b.edorno = '"+mLPGrpEdorItemSchema.getEdorNo()+"'"
					+ " and b.edortype = '"+mLPGrpEdorItemSchema.getEdorType()+"' order by d.name";
		SSRS rSSRS = new SSRS();
		...
		String[] b_strGB = new String[5];
		xmlexport.addListTable(tGBListTable, b_strGB);

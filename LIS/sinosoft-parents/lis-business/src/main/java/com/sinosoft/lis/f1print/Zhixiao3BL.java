...
public class Zhixiao3BL implements PrintService {
private static Logger logger = Logger.getLogger(Zhixiao3BL.class);
		...
		if (strManagecom.length() == 8) {
			...
			if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
				strSQL = "select a.proposalcontno,"
						...
						+ " and exists(select 'x' from lmriskapp where riskprop = 'T' and riskcode = (select p.riskcode from lcpol p where p.contno = a.contno and p.polno = p.mainpolno ))"
						+ " order by a.makedate";
	...
	public static void main(String[] args) {
		...
		tZhixiao3BL.submitData(tVData, "PRINT");
	}
}

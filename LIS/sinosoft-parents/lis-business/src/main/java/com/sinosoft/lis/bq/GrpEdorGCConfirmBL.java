...
public class GrpEdorGCConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(GrpEdorGCConfirmBL.class);
	// public GrpEdorGCConfirmBL() {}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		// logger.debug("\t@> GrpEdorGCConfirmBL.submitData() 开始");

		// 接收参数
		if (cInputData == null) {
			CError.buildErr(this, "无法获取提交的数据, 请确认数据不为空！");
			logger.debug("\t@> GrpEdorGCConfirmBL.submitData() : 无法获取 InputData 数据！");
			return false;
		} else {
		...
		// logger.debug("\t@> GrpEdorGCConfirmBL.submitData() 结束");
		return true;
	...
	private boolean getInputData() {
		// logger.debug("\t@> GrpEdorGCConfirmBL.getInputData() 开始");
		...
		if (rGlobalInput == null) {
			logger.debug("\t@> GrpEdorGCConfirmBL.getInputData() : 无法获取 GlobalInput 数据！");
		...
		if (rLPGrpEdorItemSchema == null) {
			logger.debug("\t@> GrpEdorGCConfirmBL.getInputData() : 无法获取 LPGrpEdorItemSchema 数据！");
		...
		// logger.debug("\t@> GrpEdorGCConfirmBL.getInputData() 结束");
		return true;
	...
	private boolean checkData() {
		// logger.debug("\t@> GrpEdorGCConfirmBL.checkData() 开始");
		...
		// logger.debug("\t@> GrpEdorGCConfirmBL.checkData() 结束");
		return true;
	...
	private boolean dealData() {
		// logger.debug("\t@> GrpEdorGCConfirmBL.dealData() 开始");
		...
		if (tLPEdorItemSet == null || tLPEdorItemSet.size() <= 0) {
			...
			for (int i = 1; i <= tLPEdorItemSet.size(); i++) {
				...
				if (!tPEdorGCConfirmBL.submitData(tVData, rOperation)) {
					mErrors.copyAllErrors(tPEdorGCConfirmBL.mErrors);
					logger.debug("\t@> GrpEdorGCConfirmBL.dealData() : PEdorGCConfirmBL.submitData() 失败！");
					logger.debug("\t   错误原因 : " + mErrors.getFirstError());
					CError.buildErr(this, "处理提交的数据失败！");
					return false;
				} else {
					VData tTempVData = new VData();
					tTempVData = tPEdorGCConfirmBL.getResult();
		...
		// logger.debug("\t@> GrpEdorGCConfirmBL.dealData() 结束");
		return true;
	...
	private boolean outputData() {
		// logger.debug("\t@> GrpEdorGCConfirmBL.outputData() 开始");

		rResultData = new VData();
		rResultData.add(rMap);

		// logger.debug("\t@> GrpEdorGCConfirmBL.outputData() 结束");
		return true;
	...
	// {
	// GrpEdorGCConfirmBL tGrpEdorGCConfirmBL = new GrpEdorGCConfirmBL();
	// } //function main end
	// ==========================================================================

} // class GrpEdorGCConfirmBL end

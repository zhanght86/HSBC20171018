...
public class RnewMeetF1PUI  implements BusinessService{
private static Logger logger = Logger.getLogger(RnewMeetF1PUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		RnewMeetF1PBL tRnewMeetF1PBL = new RnewMeetF1PBL();
		logger.debug("Start RefuseAppF1P UI Submit ...");

		if (!tRnewMeetF1PBL.submitData(vData, cOperate)) {
			if (tRnewMeetF1PBL.mErrors.needDealError()) {
				mErrors.copyAllErrors(tRnewMeetF1PBL.mErrors);
				return false;
			} else {
				buildError("submitData", "MeetF1PBL发生错误，但是没有提供详细的出错信息");
				return false;
			}
		} else {
			mResult = tRnewMeetF1PBL.getResult();
			return true;
		}
	}

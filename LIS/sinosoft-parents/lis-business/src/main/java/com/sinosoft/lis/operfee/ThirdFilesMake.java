...
public class ThirdFilesMake {
private static Logger logger = Logger.getLogger(ThirdFilesMake.class);
		...
		try {
			...
			if (listColWidth != null) {
				for (int i = 0; i < listColWidth.size(); i++) {
					...
					int nCol = (short) Integer.parseInt(para[0]);
					int nWidth = (short) Integer.parseInt(para[1]);
...
//							tcell.setEncoding(HSSFCell.ENCODING_UTF_16);
	...
	public static void main(String[] args) {
		...
		try {
			...
			//
			// tLB1.colName=new
			// String[]{"集体合同号码","被保人客户号","姓名","性别","出生日期","证件类型","证件号码","保险计划"};
			// tLB1.sql="select
			// Grpcontno,InsuredNo,Name,Sex,Birthday,IDType,IDNo,ContPlanCode From LCInsured
			// where Grpcontno='140110000000041'";

 ...
 * @version 1.0
 */
public class PEdorLTConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(PEdorLTConfirmBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("---PEdorLTConfirmBL BL BEGIN---");
		PEdorZTConfirmBL tPEdorZTConfirmBL = new PEdorZTConfirmBL();

		if (tPEdorZTConfirmBL.submitData(cInputData, cOperate) == false) {
			// @@错误处理
			this.mErrors.copyAllErrors(tPEdorZTConfirmBL.mErrors);
			mResult.clear();
			mResult.add(mErrors.getFirstError());
			return false;
		} else {
			mResult = tPEdorZTConfirmBL.getResult();
		}

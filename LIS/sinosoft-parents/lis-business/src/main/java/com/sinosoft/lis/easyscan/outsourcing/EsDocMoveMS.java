...
public class EsDocMoveMS implements EsDocMoveInterface {
private static Logger logger = Logger.getLogger(EsDocMoveMS.class);

	public static final String TEMP_PATH = "TempPath";
	public static final String CON_XML_ROOT = "NewCont";
	//
	...
	public static final String CON_XML_CONTNO = "ContNo";
	// private static final String CON_XML_PRTSEQ = "PrtSeq";
	public static final String CON_XML_MANAGECOM = "Managecom";
	// private static final String CON_XML_MAKEDATE = "MakeDate";
	public static final String CON_XML_BPO = "BPO";
	...
	public Document GetXMLDoc(VData cInputData) {
		try {
			...
			// String tMissionid = (String) tTransferData.getValueByName(
			// CON_XML_MISSIONID);
			// String tSubMissionid = (String) tTransferData.getValueByName(
			// CON_XML_SUBMISSIONID);
			String tContNo = (String) tTransferData
					.getValueByName(CON_XML_CONTNO);
			// String tActivityId = (String) tTransferData.getValueByName(
			// CON_XML_ACTIVITYID);
			String tManagecom = (String) tTransferData
					.getValueByName(CON_XML_MANAGECOM);
			// String tMakeDate = (String) tTransferData.getValueByName(
			// CON_XML_MAKEDATE);
			// String tPrtSeq = (String) tTransferData.getValueByName(
			// CON_XML_PRTSEQ);
			...
			// root.addContent(new
			// Element(CON_XML_MISSIONID).setText(tMissionid));
			// root.addContent(new Element(CON_XML_SUBMISSIONID).setText(
			// tSubMissionid));
			// root.addContent(new
			// Element(CON_XML_ACTIVITYID).setText(tActivityId));
			...
			// root.addContent(new
			// Element(CON_XML_MAKEDATE).setText(tMakeDate));
			...
			for (int i = 0; i < tES_DOC_PAGESSet.size(); i++) {
				...
				String tPageSuffix = ".tif"; // tES_DOC_PAGESSchema.getPageSuffix();
	...
	public VData getTempFolder(VData cInputData) {
		...
		String tTempFolder = (String) tTransferData.getValueByName(TEMP_PATH);
	...
	public VData getDesFolder(VData cInputData) {
		// 取TransferData
		BPOServerInfoSchema tBPOServerInfoSchema = getBPOServerInfo(cInputData);

		String tDesFolder = tBPOServerInfoSchema.getScanPicPath();
		// 调用接口
	...
	BPOServerInfoSchema mBPOServerInfoSchema = null;

	private BPOServerInfoSchema getBPOServerInfo(VData cInputData) {
		if (mBPOServerInfoSchema != null)
			return mBPOServerInfoSchema;
		TransferData tTransferData = (TransferData) cInputData
				.getObjectByObjectName("TransferData", 0);

		String bpo = (String) tTransferData.getValueByName(CON_XML_BPO);
		if (bpo.equals("")) {
			throw new RuntimeException("缺少ｂｐｏ");
		}
		BPOServerInfoDB tBPOServerInfoDB = new BPOServerInfoDB();
		tBPOServerInfoDB.setBPOID(bpo);
		if (!tBPOServerInfoDB.getInfo()) {
			throw new RuntimeException("缺少ｂｐｏ描述");
		}
		return tBPOServerInfoDB.getSchema();
	}
}

...
private static Logger logger = Logger.getLogger(GrpEdorCTPrintBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "CT数据提取失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "CT数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		for (int i = 1; i <= tArrLen; i++) {
			...
			for(int k = 1;k<=strCT.length;k++){
				strCT[k-1] = rSSRS.GetText(i, k);
			}
			strCT[2] = "退保";
			strCT[3] = "0";
		...
		dCTSumMoney = tExeSQL.getOneValue(tSql);
		if (dCTSumMoney == null || dCTSumMoney.equals("")) {
			dCTSumMoney = "0.0";
		}
		mTextTag.add("CTSUM", dCTSumMoney);

...
public class HospitalQualityQuery2UI {
private static Logger logger = Logger.getLogger(HospitalQualityQuery2UI.class);
	public CErrors mErrors = new CErrors();
	private VData mResult = new VData();

	public HospitalQualityQuery2UI() {
	}

	public boolean submitData(VData cInputData, String cOperate) {
		HospitalQualityQuery2BL tHospitalQualityQuery2BL = new HospitalQualityQuery2BL();
		if (!tHospitalQualityQuery2BL.submitData(cInputData, cOperate)) {
			mErrors.copyAllErrors(tHospitalQualityQuery2BL.mErrors);
			return false;
		} else {
			mResult = tHospitalQualityQuery2BL.getResult();

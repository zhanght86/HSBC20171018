 ...
 * @version 3.0
 */
public class RIStandEdorProcess_cs implements RIProcessType {
							...
							.setCessionAmount(
									mCessAmnt
											+ tRIRecordTraceSet.get(i)
													.getAmountChang());
					// 成数分保分保比例不变
					tRIRecordTraceSet.get(i).setCessionRate(mCessRate);
					// 保存新分保保额分保比例到内存
	...
	private boolean DealEdorDelAmnt(RIPolRecordSchema aRIPolRecordSchema,
		...
		try {
			...
			double newAmnt = aRIPolRecordSchema.getRiskAmnt()
					- aRIPolRecordSchema.getPreRiskAmnt();
			// 如果减额前已经分保且减额后小于等于最低分处保额，按退保计算
			if ((AccumulateAmnt - newAmnt) > riLimit
					&& AccumulateAmnt <= riLimit) {
				if (!DealEdorTB(aRIPolRecordSchema, aRIIncomeCompanySet,
						aRIDivisionLineDefSet, aRIRiskDivideSet)) {
					buildError("dealData", "累计风险保额低于最低分保保额的分保保数据处理时出错"
	...
	public static void main(String[] args) {
		...
		// RIStandEdorProcess_cs tRIStandEdorProcess_cs = new
		// RIStandEdorProcess_cs();
		// tRIStandEdorProcess_cs.getAmntChagAndCessRate(riRecordTraceSchema);

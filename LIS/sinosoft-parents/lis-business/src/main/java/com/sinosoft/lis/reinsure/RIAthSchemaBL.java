...
public class RIAthSchemaBL {
	...
	/** 业务处理相关变量 */
	private RICalDefSchema tRICalDefSchema = new RICalDefSchema();
	private PubSubmit tPubSubmit = new PubSubmit();
	private MMap map = new MMap();
	public RIAthSchemaBL() {
	...
	private boolean getInputData() {
		this.mGlobalInput = (GlobalInput) mInputData.getObjectByObjectName(
				"GlobalInput", 0);
		this.tRICalDefSchema = (RICalDefSchema) mInputData
				.getObjectByObjectName("RICalDefSchema", 0);
		return true;
	}
	...
	private boolean deleteData() {
		...
		String sql = "delete from RICalDef r where r.ArithmeticDefID='"+tRICalDefSchema.getArithmeticDefID()+"' ";
		map.put(sql, "DELETE");
		if (!prepareOutputData()) {
			return false;
		}
	...
	private boolean updateData() {
		// TODO Auto-generated method stub
		map.put(tRICalDefSchema, "UPDATE");
		if (!prepareOutputData()) {
			return false;
		}
		if (!tPubSubmit.submitData(this.mInputData, "")) {
	...
	private boolean insertData() {
		// TODO Auto-generated method stub
		// 插入再保方案表
		map.put(tRICalDefSchema, "INSERT");
		if (!prepareOutputData()) {
			return false;
		}
		if (!tPubSubmit.submitData(this.mInputData, "")) {

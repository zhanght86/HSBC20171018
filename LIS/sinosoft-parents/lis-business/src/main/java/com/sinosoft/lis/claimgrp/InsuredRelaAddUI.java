...
public class InsuredRelaAddUI implements BusinessService{
private static Logger logger = Logger.getLogger(InsuredRelaAddUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		tInsuredRelaAddBL.submitData(cInputData, mOperate);
		// logger.debug("End OLCInsured UI Submit...");
		// 如果有需要处理的错误，则返回
		if (tInsuredRelaAddBL.mErrors.needDealError()) {
			// @@错误处理
			this.mErrors.copyAllErrors(tInsuredRelaAddBL.mErrors);
			return false;
		}
		if (!mOperate.equals("DELETE||CONTINSURED")) {
			this.mResult.clear();
			this.mResult = tInsuredRelaAddBL.getResult();
		}
		// mInputData = null;
		return true;
	}

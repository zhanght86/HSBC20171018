...
public class SingleTranNoLisUI {
private static Logger logger = Logger.getLogger(SingleTranNoLisUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			SingleTranNoLisBL tSingleTranNoLisBL = new SingleTranNoLisBL();
			logger.debug("Start SingleTranNoLis UI Submit ...");
			if (!tSingleTranNoLisBL.submitData(vData, cOperate)) {
				if (tSingleTranNoLisBL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tSingleTranNoLisBL.mErrors);
					return false;
				} else {
					buildError("submitData",
							"SingleTranNoLisBL发生错误，但是没有提供详细的出错信息");
					return false;
				}
			} else {
				mResult = tSingleTranNoLisBL.getResult();
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
			CError cError = new CError();
			cError.moduleName = "SingleTranNoLisUI";

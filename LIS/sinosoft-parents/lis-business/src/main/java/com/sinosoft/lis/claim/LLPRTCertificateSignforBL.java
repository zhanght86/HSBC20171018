 ...
 * </p>
 * <p>
 * Description: 单证打印：索赔文件签收清单--PCT002,SpwjqsqdGrC000100.vts
 * </p>
 * <p>
 * 此清单为客户向公司提交的资料签收清单
 * </p>
...
private static Logger logger = Logger.getLogger(LLPRTCertificateSignforBL.class);
	...
	private boolean getInputData(VData cInputData, String cOperate) {
		...
		// this.claimTag = (String) mTransferData.getValueByName("ClaimTag");
		// //案件状态
		return true;
	}
...
//		tXmlExport.createDocument("SpwjqsqdGrC000100.vts", "");
		tXmlExport.createDocument("单证签收清单", "", "", "0");
		...
		tTextTag.add("ClaimTag", "收到并留存");// 用于客户向公司提交资料清单时
		...
		String affixSQL = "select * from Llaffix where rgtno='" + "?clmno?"
				+ "' and customerno='" + "?cusno?" + "'";
		SQLwithBindVariables sqlbv1 = new SQLwithBindVariables();
		sqlbv1.sql(affixSQL);
		sqlbv1.put("clmno", mClmNo);
		sqlbv1.put("cusno", mCusNo);
		LLAffixSet tLLAffixSet = tLLAffixDB.executeQuery(sqlbv1);
		...
		if (tLLAffixSet.size() > 0) {
			...
			sqlbv2.sql(operatorSQL);
			sqlbv2.put("usercode", tLLAffixSet.get(1).getOperator());
			ExeSQL cExeSQL = new ExeSQL();
			operator = cExeSQL.getOneValue(sqlbv2);
		} else {
			operator = "";
		}

		tTextTag.add("HangUp", "");// 提交人----暂时“置空”
		tTextTag.add("Operator", operator);// 签收人----操作员，由数据库提取
		for (int i = 1; i <= tLLAffixSet.size(); i++) {
			String[] stra = new String[5];
			stra[0] = tLLAffixSet.get(i).getAffixName();

			if (tLLAffixSet.get(i).getAffixConclusion() == null)// Modify by
																// zhaorx
																// 2006-12-04
			{
				stra[1] = "否";
				...
				if (tLLAffixSet.get(i).getAffixConclusion().equals("0")) {
					stra[1] = "是";
				} else if (tLLAffixSet.get(i).getAffixConclusion().equals("1")) {
					stra[1] = "否";
				}
			}
        ...
        String updateStateSql="update loprtmanager set stateflag='1' where otherno='"+"?clmno?"+"' and code='PCT004'";

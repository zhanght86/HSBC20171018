...
public class WaitPaymentLisUI {
private static Logger logger = Logger.getLogger(WaitPaymentLisUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			if (!tWaitPaymentLisBL.submitData(vData, cOperate)) {
				if (tWaitPaymentLisBL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tWaitPaymentLisBL.mErrors);
					return false;
				} else {
					buildError("submitData",
							"WaitPaymentLisBL发生错误，但是没有提供详细的出错信息");
					return false;
				}
			} else {
				mResult = tWaitPaymentLisBL.getResult();
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();

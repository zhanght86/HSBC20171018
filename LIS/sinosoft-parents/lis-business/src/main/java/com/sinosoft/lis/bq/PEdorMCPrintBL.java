...
public class PEdorMCPrintBL implements EdorPrint {
	private static Logger logger = Logger.getLogger(PEdorMCPrintBL.class);
	...
	private boolean dealData() {
		...
		if (!tLPContDB.getInfo()) {
			mErrors.addOneError("打印生成数据时，取备注信息失败!");
			return false;
		}
		tLPContSchema = tLPContDB.getSchema();
		mTextTag.add("Content", tLPContSchema.getRemark());// 备注信息
		mTextTag.add("ManageComName", tCodeNameQuery.getName("station",
				mLPEdorItemSchema.getManageCom()));// 管理机构
		mTextTag.add("SysDate", strCurDay);// 打印日期
		// 其它信息
		...
		// mTextTag.add("ManageCom",allArr[BqNameFun.COM]);

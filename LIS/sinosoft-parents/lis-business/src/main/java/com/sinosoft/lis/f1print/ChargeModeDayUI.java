...
public class ChargeModeDayUI implements PrintService

{
private static Logger logger = Logger.getLogger(ChargeModeDayUI.class);
	...
	public static void main(String[] args) {
		...
		tTransferData.setNameAndValue("PolicyCom", "86320000");
		...
		tVData.add(tG);
		ChargeModeDayUI u = new ChargeModeDayUI();
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (!tChargeModeDayBL.submitData(cInputData, "PRINT")) {
			logger.debug("真的不好意思，我没有得到任何的值");
			if (tChargeModeDayBL.mErrors.needDealError()) {
				logger.debug("你要调用我就好，我就可以把我的真挚给你");
				mErrors.copyAllErrors(tChargeModeDayBL.mErrors);
				return false;
			} else {
			...
			mResult = tChargeModeDayBL.getResult();
			return true;
		}
	}

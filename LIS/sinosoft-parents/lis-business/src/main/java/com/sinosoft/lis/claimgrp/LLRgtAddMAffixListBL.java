    ...
    private boolean dealData()
    {
        ...
        if (this.mOperate.equals("RgtAdd||Insert"))
        {
           try
           {
               ...
               {
                   //删除原来未回销的记录
                   String strSql="delete from LLAffix where RgtNo='"+tRgtNo+"' and CustomerNo='"+tCustomerNo+"' and (SubFlag is null or SubFlag='1')";
                   ...
                   for (int i = 1; i <= mSetSize; i++)
                   {
                       ...
                       tLLAffixSchema.setCustomerNo(mLLAffixSchema.getCustomerNo()); //客户号
                       tLLAffixSchema.setAffixCode(mLLAffixSchema.getAffixCode()); //单证代码
                       tLLAffixSchema.setAffixName(mLLAffixSchema.getAffixName()); //单证名称
                       tLLAffixSchema.setSubFlag(mLLAffixSchema.getSubFlag()); //是否已提交标志
                       tLLAffixSchema.setNeedFlag(mLLAffixSchema.getNeedFlag()); //是否必需标志
                       tLLAffixSchema.setReadyCount(mLLAffixSchema.getReadyCount()); //单证件数
                       tLLAffixSchema.setProperty(mLLAffixSchema.getProperty()); //单证属性标志
                       //缺少件数
                       tLLAffixSchema.setReturnFlag(mLLAffixSchema.getReturnFlag()); //是否退还原件标志
                      // tLLAffixSchema.setAffixNo(tGrid9[index]); //附件号码
                       if (mLLAffixSchema.getAffixNo().equals("")||mLLAffixSchema.getAffixNo().equals(null))
                       {
                           //生成AffixNo
                           tAffixNo = PubFun1.CreateMaxNo("AffixNo", 10); //生成AffixNo号 附件号码
                           logger.debug("----------tAffixNo======="+tAffixNo);
                           tLLAffixSchema.setAffixNo(tAffixNo);
                       }
                       else
                       {
                           tLLAffixSchema.setAffixNo(mLLAffixSchema.getAffixNo());  //附件号码
                       }
                   ...
                   //[补充单证时]修改立案分案（LLCase）表中 修改单证检查结论（AffixConclusion=0-齐全）（此段程序2005-7-22 添加）
                  ...
                  map.put(tLLCaseSchema, "DELETE&INSERT");
      			  ...
      			  // 单证补充通知单[问题件]
			      if (!insertLOPRTManager("PCT003", "0")) {
						return false;
				  }

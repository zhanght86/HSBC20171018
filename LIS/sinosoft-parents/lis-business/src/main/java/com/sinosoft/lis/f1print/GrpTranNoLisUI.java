...
public class GrpTranNoLisUI {
private static Logger logger = Logger.getLogger(GrpTranNoLisUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			GrpTranNoLisBL tGrpTranNoLisBL = new GrpTranNoLisBL();
			logger.debug("Start GrpTranNoLis UI Submit ...");
			if (!tGrpTranNoLisBL.submitData(vData, cOperate)) {
				if (tGrpTranNoLisBL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tGrpTranNoLisBL.mErrors);
					return false;
				} else {
					buildError("submitData", "GrpTranNoLisBL发生错误，但是没有提供详细的出错信息");
					return false;
				}
			} else {
				mResult = tGrpTranNoLisBL.getResult();
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
			CError cError = new CError();
			cError.moduleName = "GrpTranNoLisUI";

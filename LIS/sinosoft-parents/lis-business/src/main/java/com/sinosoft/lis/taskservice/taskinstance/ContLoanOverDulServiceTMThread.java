 ...
 * @version 1.0
 */
public class ContLoanOverDulServiceTMThread extends TaskThread {
	/** 公共锁定号码类 */
	private PubConcurrencyLock mPubLock = new PubConcurrencyLock();	
	private static Logger logger = Logger.getLogger(ContLoanOverDulServiceTMThread.class);
	...
	String mContNo;
	public ContLoanOverDulServiceTMThread() {
	}

	public boolean dealMain() {
		
		/* 业务处理逻辑 */
		logger.debug("进入业务逻辑处理!--保单欠款逾期催付处理" + PubFun.getCurrentDate()
		...
		PubFun.logPerformance (tG,tG.LogID[1],"保单欠款逾期催付处理运行程序开始","1");
		...
		int tConutMax = 100;
		String tResult = "";
		String tSQL_Count = "select sysvarvalue from ldsysvar where sysvar='ContLoanOverDulServiceCount' ";
		...
		logger.debug("保单欠款逾期催付处理开始：RnewPolAuto...");

//		处理的机构
//        tGlobalInput = (GlobalInput)mParameters.get("GlobalInput");

        logger.debug("The CalDate is :"+mCalDate);
		...
		if (!"".equals(mContNo) && mContNo != null) {
				...
				for (int k = 1; k <= tSSRS.MaxRow; k++) {
			    	...
			    	PubFun.logTrack(tG,tG.LogID[1],"欠款逾期催付处理开始");
					 ...
					 String tContSql = "select * from loloan a where 1=1"
						...
						sqlbv2.sql(tComSQL);
						sqlbv2.put("ReportPubFun", ReportPubFun.getWherePart("ContNo", mContNo));
						sqlbv2.put("tCom", tCom);
						sqlbv2.put("tParamCalDate", tParamCalDate);
						...
						// 采取长连接的方式
						if (!rsWrapper.prepareData(tLOLoanSet, sqlbv2)) {
			...
			int tThreadCount = 5;
			tExeSQL = new ExeSQL();
			String tSQL_ThreadCount = "select (case when sysvarvalue is not null then sysvarvalue else '0' end) from ldsysvar where sysvar ='ContLoanOverDulServiceTMThread'";
			...
			this.mServiceA = new ServiceA(
					"com.sinosoft.lis.bq.ContLoanOverDulBLTMThread", mTaskWaitList,
			...
			PubFun.logTrack(tG, tG.LogID[1], "保全试算数据清理批处理完毕");
	        //完成时间
	    	Date  dend = new Date();         	
    	...
    	PubFun.logTrack(tG,tG.LogID[1],"欠款逾期催付处理开始");
			...
			logger.debug(tContSql);
			  SQLwithBindVariables sqlbv4=new SQLwithBindVariables();
				...
				sqlbv4.put("tCom", tCom);
				sqlbv4.put("mCalDate", mCalDate);
			LOLoanSet tLOLoanSet = new LOLoanSet();
			RSWrapper rsWrapper = new RSWrapper();
			// 采取长连接的方式
			if (!rsWrapper.prepareData(tLOLoanSet, sqlbv4)) {
				this.mErrors.copyAllErrors(rsWrapper.mErrors);
				return false;
			}
			do {
...
//		日志监控,结果监控
	...
	public static void main(String[] args) {
		ContLoanOverDulServiceTMThread tContLoanOverDulServiceTMThread = new ContLoanOverDulServiceTMThread();
		tContLoanOverDulServiceTMThread.dealMain();
	}
}

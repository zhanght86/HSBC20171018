 ...
 * </p>
 * <p>
 * Description:再保有效保单(业务理赔数据)通过可参考提数程序
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class RINewDistillYWCL implements RIDistill {
	String mEventType;
	...
	private String getDistillSQL() {
		...
		strBuffer
				.append(" a.PolNo || ','||g.dutycode||','||h.clmno||', '||'YWCL'||','||'"
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=g.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
		...
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecord m where m.unionkey=(a.PolNo || ','||g.dutycode||','||h.clmno||', '||'YWCL'||','||'"
						+ mAccumulateDefNo + "')) ");
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecordBake n where n.unionkey=(a.PolNo || ','||g.dutycode||','||h.clmno||', '||'YWCL'||','||'"
						+ mAccumulateDefNo + "')) ");
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=g.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
	...
	private String getDistillSQL3() {
		...
		// strBuffer.append(" AccumulateAmnt(a.InsuredNo,b.DutyCode,a.ContNo) as AccAmnt, ");
		strBuffer
				.append(" (select x.Exchangerate from RIExchangeRate x where x.Currency = a.Currency and x.Startdate  =(select max(d.Startdate) from RIExchangeRate d where d.Currency = a.Currency)and x.Enddate is null) from RIExchangeRate) and x.Enddate is null) as ChangeRate, ");
		strBuffer.append(" '13' as AccCurrency, ");
		strBuffer.append(" 0 as AccAmnt, ");
		strBuffer.append(" 0 as PreStandPrem, ");
		strBuffer.append(" 0 as PrePrem, ");
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=g.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
	...
	public static void main(String[] args) {
		...
		RINewDistillYWCL tRIDistillCL = new RINewDistillYWCL();

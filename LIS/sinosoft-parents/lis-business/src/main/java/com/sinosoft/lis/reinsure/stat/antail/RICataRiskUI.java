...
public class RICataRiskUI {
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		RICataRiskBL tRICataRiskBL = new RICataRiskBL();
		if (!tRICataRiskBL.submitData(cInputData, cOperate)) {
			if (tRICataRiskBL.mErrors.needDealError()) {
				this.mErrors.copyAllErrors(tRICataRiskBL.mErrors);
			} else {
				buildError("submitData", "tCardPlanBL发生错误，但是没有提供详细信息！");
			}
			return false;
		}
		mResult = tRICataRiskBL.getResult();
		return true;
	}

...
public class FacuAudit_Limited_Sucess_56To60 implements FacuAudit {
	...
	private boolean dealData() {
		...
		int age = 0;
		SSRS ageSSRS = new SSRS();
		String tSql = "select floor(months_between(sysdate,a.Insuredbirthday)/12) from lcpol a where a.Contno = '"
				+ mContNo + "' and a.Polno ='" + mPolNo + "' ";
		ageSSRS = tExeSQL.execSQL(tSql);
		if (Double.parseDouble(ageSSRS.getAllData()[0][0]) < 56
				|| Double.parseDouble(ageSSRS.getAllData()[0][0]) > 60) {
			uwFlag = true;
			return true;
		}

...
public class EdorAutoPayBillPrintBL implements BqBill {
private static Logger logger = Logger.getLogger(EdorAutoPayBillPrintBL.class);
	...
	public static final String VTS_NAME = "EdorAutoPayBill.vts";
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (!mOperate.equals("PRINT")) {
			...
			tError.moduleName = "EdorAutoPayBillPrintBL";
	...
	private boolean preparePrintData() {
		...
		if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
			...
			+"   (select s.appntno, s.addressno from lcappnt s where s.contno = a.contno)),  " 
			+" b.cvalidate,b.contno,a.loandate, "  
			+" (select (select riskname from lmriskapp m  where m.riskcode = d.riskcode) from lcpol d "
			+"	  where d.contno = a.contno and d.polno = d.mainpolno and rownum = 1), "
			+" (select sum(prem) from lcpol d where d.contno = a.contno and d.appflag='1' ), "
			+" a.summoney,a.loantodate-a.loandate+1,a.loantodate+1, "
			...
			+ "?mEndDate?"
			+ "'"
			+ " and  exists ( select 'X' from lccont b where b.contno = a.contno and b.ManageCom like concat('"+"?mManageCom?"+"','%'))"
			+ mSaleChlSQL +" order by b.managecom,sale";
			...
			strsql = " select substr(b.managecom, 1, 6), "
					...
					+" b.cvalidate,b.contno,a.loandate, "  
					+" (select (select riskname from lmriskapp m  where m.riskcode = d.riskcode) from lcpol d "
					+"	  where d.contno = a.contno and d.polno = d.mainpolno limit 1), "
					+" (select sum(prem) from lcpol d where d.contno = a.contno and d.appflag='1' ), "
					+" a.summoney,a.loantodate-a.loandate+1,a.loantodate+1, "
					+" (select f.name from laagent f where f.agentcode = b.agentcode), "
					+" (select f.agentcode from laagent f where f.agentcode = b.agentcode),"
		...
		//tXmlExport.outputDocumentToFile("c:\\XML\\", "EdorAutoPayBill.vts");
		mResult.clear();

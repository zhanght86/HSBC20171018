...
/*
 * <p>ClassName: GrpOutMedicUndoTabBL </p>
 * <p>Description: GrpOutMedicUndoTabBL类文件 </p>
...
public class GrpOutMedicUndoTabBL
{
    ...
    private boolean getPrintData() {
      ...
      /**以下表示在 报表中生成 18列，*/
        ...
        try {
            SSRS tSSRS = new SSRS();
            String sql = "";
            /**sql 查询18列，目的 往报表的18列里面要插入的数据进行 提数*/
            sql = " select X.A17,X.A0,X.A1,X.A2,X.A3,X.A4,X.A5,X.A6,X.A7,X.A8,X.A9,X.A10,X.A11,X.A12,X.A13,X.A14,X.A15,X.A16,X.A14-X.A15 "
                  + " from (select (select shortname from ldcom where comcode=a.StandbyString3) A0 ,max(a.InsuredName) A1,(case max(a.InsuredSex) when '0' then '男' else '女' end) A2,max(a.InsuredBirthday) A3,max(a.InsuredIDNo) A4,max(a.OccupationType) A5, a.GrpContno A6,max(a.CValiDate) A7,max(a.enddate) A8,a.RiskCode  A9,(select riskname from lmrisk where riskcode=a.RiskCode) A10,(select nvl(sum(c.Amnt),0) from ripolrecordbake c where c.riskcode=a.riskcode and c.insuredno=a.insuredno and c.SerialNo=a.SerialNo and c.eventtype='03' and c.FeeOperationType in('ZT','WT','CT') )  A11, nvl(sum(b.cessionamount),0) A12,nvl(max(b.CessionRate),0)  A13,nvl(sum(b.PremChang),0) A14,nvl(sum(b.ReturnComm),0) A15,nvl(max(b.StandbyDoubleC),0) A16,(select ComPanyName from ReInsuranceComPanyInfo  where companyno=b.companyno) A17 from RIPolRecordBake a, reinsurerecordtrace b where a.eventno=b.eventno and a.SerialNo=b.SerialNo and a.eventtype='03' and a.FeeOperationType in('ZT','WT','CT') and a.dutycode in (select distinct associatedcode from accumulaterdcode where accumulatedefno in (select Accumulatedefno from riprecept where ricontno = 'C000000003')) and a.cvalidate >= '"+mRValidate+"' and a.enddate<='"+mRInvalidate+"' ";
            ...
            String sql2 = " group by a.Serialno,b.companyno,a.StandbyString3,a.GrpContno,a.InsuredNo,a.RiskCode order by a.StandbyString3,a.GrpContno,a.InsuredNo,a.RiskCode) X where rownum <=1000 ";
            sql = sql + sql2;
        ...
        /**LRExportNewUndo.vts//新建一个XmlExport的实例*/
        XmlExport xmlexport = new XmlExport();
        /**最好紧接着就初始化xml文档*/
        xmlexport.createDocument("LRExportNewUndo.vts", "printer");
        SSRS cSSRS = new SSRS();

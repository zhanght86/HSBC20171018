 ...
 * @version 3.0
 */
public class RIStandEdorProcess_DD implements RIProcessType {
			...
			if (AccumulateAmnt > riLimit) {
				...
				if (tRIRecordTraceTempSet.size() > 0) {
					for (int i = 1; i <= tRIRecordTraceTempSet.size(); i++) {
						...
						// tRIRecordTraceTempSet.get(i).setAssociateCode(polRecordSchema.getRiskCode());
	...
	private boolean getCessFeeAndCessComm(
		...
		try {
			...
			// strBuffer.append(riRecordTraceTempSchema.getAssociateCode());
			...
			strBuffer.append(riRecordTraceTempSchema.getContNo());
			strBuffer.append("' and AssociateCode='");
			// strBuffer.append(riRecordTraceTempSchema.getAssociateCode());

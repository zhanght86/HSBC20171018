...
private static Logger logger = Logger.getLogger(GrpEdorNIDetailBL.class);
	...
	private boolean dealData() {
		...
		LCInsuredSet aLCInsuredSet = new LCInsuredSet();// 待更新C表数据使用
		LPInsuredSchema aLPInsuredSchema;
		LCInsuredSchema aLCInsuredSchema;
		...
		for (int i = 1; i <= tLCContSet.size(); i++) {
			...
			//保单生效日期以客户录入为准   wk modify 2009-12-10
//			if (!CertifyDate()) {
//				return false;
//			}

			String ContNo = tLCContSet.get(i).getContNo();
			String tInsuredNo = tLCContSet.get(i).getInsuredNo();
			LCInsuredDB tLCInsuredDB = new LCInsuredDB();
			tLCInsuredDB.setInsuredNo(tInsuredNo);
			tLCInsuredDB.setContNo(ContNo);
			aLCInsuredSet = tLCInsuredDB.query();
			for (int j = 1; j <= aLCInsuredSet.size(); j++) {
				aLCInsuredSchema = aLCInsuredSet.get(j);
				aLPInsuredSchema = new LPInsuredSchema();
				mRef.transFields(aLPInsuredSchema, aLCInsuredSchema);
				aLPInsuredSchema.setEdorNo(mLPGrpEdorItemSchema.getEdorNo());
				aLPInsuredSchema.setEdorType(mLPGrpEdorItemSchema.getEdorType());
				// aLPContSchema.setOperator(mGlobalInput.Operator);
				aLPInsuredSchema.setModifyDate(CurrDate);
				aLPInsuredSchema.setModifyTime(CurrTime);
			...
			for (int j = 1; j <= tLCPolSet.size(); j++) {
				...
				//获取客户录入日期  wk modify 2009-12-10
				this.CValidate=aLCPolSchema.getCValiDate();
				...
				if (aLCPolSchema.getInsuYearFlag() != null
						&& aLCPolSchema.getInsuYearFlag().equals("M")||(aLCPolSchema.getInsuYearFlag().equals("Y")&&aLCPolSchema.getInsuYear()==1)) {
					// 取出该险种原始被保人的保险止期
					String sEnddate = getEndDate(aLCPolSchema.getGrpPolNo());
					if (sEnddate != null && !sEnddate.equals("")) {
						int cIntv = PubFun.calInterval(Enddate, sEnddate, "D");
						if(cIntv<=0){
							Enddate = sEnddate; // 新增被保人保险止期与原有被保人取齐
							Payenddate = Enddate; // 缴费止期即为保险止期
							Paytodate = Payenddate; // 交至日期即为缴费止期
						}else{
							CError.buildErr(this, "新增被保人险种的止期小于团体险种的止期!不能添加!\n被保人号："
				...
				tLCGrpPolDB.setGrpPolNo(aLCPolSchema.getGrpPolNo());
				LCGrpPolSchema tLCGrpPolSchema = tLCGrpPolDB.query().get(1);
				if(tLCGrpPolSchema!= null && tLCGrpPolSchema.getAppFlag().equals("4") )
				{
					CError.buildErr(this, "新增被保人所增团单险种已经失效终止!险种号:"+tLCGrpPolSchema.getRiskCode());
					return false;
				}
	...
	private LJSPayPersonSchema initLJSPayPerson(
		...
		cLJSPayPersonSchema.setEdorNo(this.mLPGrpEdorItemSchema.getEdorNo());
	...
	private SSRS getOldEndDate(String sGrpContNo) {
		String sql = " select grppolno, "
				+ " paytodate "
				+ " from lcgrppol g where g.grpcontno = '" + sGrpContNo + "' and appflag = '1' ";
	 ...
	 * @return boolean
	 */
	private String getEndDate(String sGrpPolNo) {
		String sEndDate = null;
		LCGrpPolDB tLCGrpPolDB = new LCGrpPolDB();
		tLCGrpPolDB.setGrpPolNo(sGrpPolNo);
		LCGrpPolSet tLCGrpPolSet = tLCGrpPolDB.query();
		if (tLCGrpPolSet != null && tLCGrpPolSet.size() > 0) {
			sEndDate = tLCGrpPolSet.get(1).getPayEndDate();
		}
		return sEndDate;
	}

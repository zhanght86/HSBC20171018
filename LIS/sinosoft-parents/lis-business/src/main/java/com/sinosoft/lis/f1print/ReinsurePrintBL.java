 ...
 * @version 1.0
 */
public class ReinsurePrintBL implements PrintService {
private static Logger logger = Logger.getLogger(ReinsurePrintBL.class);
	...
	private void buildError(String szFunc, String szErrMsg) {
		...
		cError.moduleName = "ReinsurePrintBL";
	...
	private boolean getPrintData() {
		...
		String tSql = "select ContNo,(select codename from ldcode where codetype='uqreportstate'"
				+ "and code = ReinsuredResult),"
				+ "ModifyDate,reinsudesc from LCReinsureReport where contno in "
				+ "(select Proposalcontno from lccont where ManageCom like concat('"
				+ "?ManageCom?"
		...
		if (tempSSRS != null && tempSSRS.getMaxRow() > 0
			...
			for (int i = 1; i <= tempSSRS.getMaxRow(); i++) {
				...
				// tSql = "select a.contno,a.insuredname,(select b.IDNo from
				// ldperson b where b.customerno=a.insuredno )," +
				// "a.riskcode,a.amnt,(select codename from ldcode where
				// codetype='uwstate' and code=a.uwflag)," +
				// "(select sum(suppriskscore) from lcprem where polno=a.polno)
				// from lcpol a where a.prtno='" +
				// tempSSRS.GetText(i, 1) +
				// "' and a.signdate is not null order by insuredname";
		...
		xmlexport.createDocument("ReInsurelis.vts", "");

...
private static Logger logger = Logger.getLogger(LLBalExemptFeeBL.class);
	...
	private boolean getLLExempt() {
		 ...
		 * －－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
		 */
		String tSql = "select * from LLExempt where 1=1 " + " and ClmNo = '"
				+ "?ClmNo?" + "'" + " and PolNo = '"
				+ "?PolNo?" + "'" + " and FreeFlag = '1' ";// 1表示免交
		SQLwithBindVariables sqlbv = new SQLwithBindVariables();
		sqlbv.sql(tSql);
		sqlbv.put("ClmNo", this.mClmNo);
		sqlbv.put("PolNo", mLCPolSchema.getPolNo());
		LLExemptDB tLLExemptDB = new LLExemptDB();
		LLExemptSet tLLExemptSet = tLLExemptDB.executeQuery(sqlbv);
		...
		for (int i = 1; i <= tLLExemptSet.size(); i++) {
			String tDate = tLLExemptSet.get(i).getFreeStartDate().substring(0,
					10);
	...
	private boolean getLJTempFee(LCPolSchema pLCPolSchema, String pDate) {
		...
		tSql = "select * from LJTempFee where 1=1 "
				...
				+ " and LastPayToDate >= to_date('" + "?pDate?"
				+ "','yyyy-mm-dd') " + " and PolNo = '"
				+ "?PolNo?" + "')" + " and confflag not in ('1')";
		SQLwithBindVariables sqlbv1 = new SQLwithBindVariables();
		sqlbv1.sql(tSql);
		sqlbv1.put("pDate", pDate);
		sqlbv1.put("PolNo", pLCPolSchema.getPolNo());
		...
		if (tLJTempFeeSet == null || tLJTempFeeSet.size() == 0) {
			...
			this.mErrors.addOneError("合同号:[" + pLCPolSchema.getContNo()
					+ "],保单险种号:[" + pLCPolSchema.getPolNo()
	...
	private boolean getLLJSPay(LCPolSchema pLCPolSchema, String pDate) {
		...
		/**
		 * －－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－ No1.0 查询
		 * 在LJAPayPerson应收总表判断是否有信息 根据保单险种号,豁免开始日期 > LastPayToDate原交至日期为条件进行判断
		...
		sqlbv2.sql(tSql);
		sqlbv2.put("PolNo", pLCPolSchema.getPolNo());
		sqlbv2.put("pDate", pDate);
		LJSPayDB tLJSPayDB = new LJSPayDB();
		LJSPaySet tLJSPaySet = tLJSPayDB.executeQuery(sqlbv2);

		if (tLJSPayDB.mErrors.needDealError()) {
			this.mErrors.addOneError("查询出险日期以后的个人应收汇总信息发生错误!");
			return false;
		}
	...
	private boolean getLJSPayPerson(LCPolSchema pLCPolSchema, String pDate) {
		...
		/**
		 * －－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－ No1.0 查询
		 * 在LJAPayPerson个人应收表判断是否有信息 根据保单险种号,豁免开始日期 > LastPayToDate原交至日期为条件进行判断
		 * 2005.05.01 > 2004.10.01
		 * 
		 * －－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
		 */
		...
		if (tLJSPayPersonDB.mErrors.needDealError()) {
			this.mErrors.addOneError("查询出险日期以后的个人应收信息发生错误!");
			return false;
		}
	...
	private boolean getLJAPayPerson(LCPolSchema pLCPolSchema, String pDate) {
		/**
		 * －－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－ No1.0
		 * 在LJAPayPerson实收个人交费表表 根据保单险种号,豁免开始日期 > LastPayToDate原交至日期为条件进行判断
		 * 2005.05.01 > 2004.10.01
		 * 
		 * －－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－－
		 */
		...
		String tSql = "";
		tSql = "select (case when sum(LJAPayPerson.SumActuPayMoney) is null then 0 else sum(LJAPayPerson.SumActuPayMoney) end) from LJAPayPerson where 1=1 "
				+ " and PolNo = '"
				+ "?PolNo?"
				+ "'"
				+ " and LastPayToDate < to_date('" + "?pDate?" + "','yyyy-mm-dd') ";
		...
		sqlbv4.sql(tSql);
		sqlbv4.put("PolNo", pLCPolSchema.getPolNo());
		sqlbv4.put("pDate", pDate);
		double t_Sum_Return = 0;
		t_Sum_Return = Double.parseDouble(new DecimalFormat("0.00")
				.format(t_Sum_Return));

		ExeSQL tExeSQL = new ExeSQL();
		String tCal = tExeSQL.getOneValue(sqlbv4);

		if (tExeSQL.mErrors.needDealError()) {
			this.mErrors.addOneError("计算退豁免开始日期以后的保费发生错误!");
			return false;
		}
		...
		if (tCal == null || StrTool.cTrim(tCal).equals("")
				|| tCal.length() == 0) {

 ...
 * @version 1.0
 */
public class RICoDistillFX implements RIDistill
{
	String mEventType;
	...
	private String getDistillSQL()
	{
		...
		/*strBuffer
				.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=c.DutyCode and AddFeeDirect='01' and PayEndDate>'"
						+ mRIWFLogSchema.getEndDate() + "') as ExtRate1, ");
		strBuffer
				.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=c.DutyCode and AddFeeDirect='02' and PayEndDate>'"
						+ mRIWFLogSchema.getEndDate() + "') as ExtRate2, ");
		strBuffer
				.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=c.DutyCode and AddFeeDirect='03' and PayEndDate>'"
						+ mRIWFLogSchema.getEndDate() + "') as ExtRate3, ");*/
		...
		strBuffer
				.append(" (select sum(Prem) from LCPrem where polno=a.polno and DutyCode=c.DutyCode and AddFeeDirect='01' and PayEndDate>'"
						+ mRIWFLogSchema.getEndDate() + "') as ExtPrem1, ");
		strBuffer
				.append(" (select sum(Prem) from LCPrem where polno=a.polno and DutyCode=c.DutyCode and AddFeeDirect='02' and PayEndDate>'"
						+ mRIWFLogSchema.getEndDate() + "') as ExtPrem2, ");
		strBuffer
				.append(" (select sum(Prem) from LCPrem where polno=a.polno and DutyCode=c.DutyCode and AddFeeDirect='03' and PayEndDate>'"
						+ mRIWFLogSchema.getEndDate() + "') as ExtPrem3, ");
		...
		// 增额，减额，部分退保
		strBuffer
				.append(" and a.contno = c.Contno and b.edortype in ('RE') and b.edorstate='0'");
		// 不为临分的保单
		strBuffer
	...
	public static void main(String[] args)
	{
		...
		mRIWFLogSchema.setBatchNo("0000300025");
		mRIWFLogSchema.setStartDate("2012-10-01");
		mRIWFLogSchema.setEndDate("2012-11-30");
		mRIWFLogSchema.setTaskCode("L01010010010");
		cInputData.add(mRIWFLogSchema);
		RICoDistillFX tRIDistillRN = new RICoDistillFX();

...
private static Logger logger = Logger.getLogger(GrpPolImpInfo.class);
	...
	public LCBnfSet getBnfSetByPolId(String strPolId) {
		...
		LCBnfSet tBnfSet = new LCBnfSet();
		LCBnfSchema tSchema = null;
		logger.debug("mLCBnfSet");
		for (int i = 1; i <= this.mLCBnfSet.size(); i++) {
			tSchema = mLCBnfSet.get(i);
			// 险种ID 被缓存在 BnfNo
			String tStrId = tSchema.getContNo().trim() + "-"
					+ tSchema.getPolNo().trim();
			if (tStrId.equals(strPolId)) {
				LCBnfSchema newSchema = new LCBnfSchema();
				newSchema.setSchema(tSchema);
				tBnfSet.add(newSchema);
			}
	...
	public VData formatLCPol(LCPolSchema tLCPolSchema, String strID,
		...
		logger.debug("after findeLcgrpPolSchema");
		this.mISPubAcc = tISPubAcc;
	...
	public boolean checkLCInsured(LCInsuredSchema tLCInsuredSchema) {
		String tBirthday = "";
		String tSex = "";
		...
		if (tLCInsuredSchema.getName() == null
				|| "".equals(tLCInsuredSchema.getName().trim())) {
			buildError("formatLCInsured", "用户姓名不能为空!");
			return false;
		}

		// if (tLCInsuredSchema.getOccupationType() == null ||
		// "".equals(tLCInsuredSchema.getOccupationType().trim())) {
		// buildError("formatLCInsured",
		// "用户职业类别不能为空,用户姓名：" + tLCInsuredSchema.getName() + "!");
		// return null;
		// }

		if (tLCInsuredSchema.getIDType() == null
				|| "".equals(tLCInsuredSchema.getIDType().trim())) {
			buildError("formatLCInsured", "用户证件类型不能为空,用户姓名："
					+ tLCInsuredSchema.getName() + "!");
			return false;
		}

		if (tIDNo == null || "".equals(tIDNo.trim())) {
			buildError("formatLCInsured", "用户证件号码不能为空,用户姓名："
					+ tLCInsuredSchema.getName() + "!");
			return false;
		}
		...
		if (tLCInsuredSchema.getIDType().equals("0")) {
			...
			if ((tIDNo.length() != 15) && (tIDNo.length() != 18)) {
				buildError("formatLCInsured", "用户身份证号位数错误,身份证号为：" + tIDNo + "!");
				return false;
			}

			if (tIDNo.length() == 15) {
				tTempStr = "19" + tIDNo.substring(6, 12);
				tTempStr = tTempStr.substring(0, 4) + "-"
						+ tTempStr.substring(4, 6) + "-"
						+ tTempStr.substring(6);
				tBirthday = tTempStr;
				tSex = tIDNo.substring(14);
			} else {
				tTempStr = tIDNo.substring(6, 14);
				tTempStr = tTempStr.substring(0, 4) + "-"
						+ tTempStr.substring(4, 6) + "-"
						+ tTempStr.substring(6);
				tBirthday = tTempStr;
				tSex = tIDNo.substring(16, 17);
			}

			if ((Integer.parseInt(tSex) % 2) == 0) {
				tSex = "1";
			} else {
				tSex = "0";
			}
		...
		if (tLCInsuredSchema.getBirthday() == null
				|| tLCInsuredSchema.getBirthday().trim().equals("")) {
			buildError("formatLCInsured", "用户出生日期不能为空,用户姓名："
					+ tLCInsuredSchema.getName() + "!");
			return false;
		} else {
			if (tLCInsuredSchema.getIDType().equals("0")) {
				if (!tLCInsuredSchema.getBirthday().equals(tBirthday)) {
					buildError("formatLCInsured", "用户输入出生日期和身份证不符,用户姓名："
							+ tLCInsuredSchema.getName() + "!");
					return false;
				}
			}
		...
		if (tLCInsuredSchema.getSex() == null
				|| tLCInsuredSchema.getSex().equals("")) {
			buildError("formatLCInsured", "用户性别不能为空,用户姓名："
					+ tLCInsuredSchema.getName());
			return false;
		} else {
			if (tLCInsuredSchema.getIDType().equals("0")) {
				...
				logger.debug(tSex);
				if (!tLCInsuredSchema.getSex().equals(tSex)) {
					buildError("formatLCInsured", "用户输入性别和身份证不符,用户姓名："
							+ tLCInsuredSchema.getName() + "!");
					return false;
				}
			}
		...
		if (tLCInsuredSchema.getOccupationCode() != null
				&& !tLCInsuredSchema.getOccupationCode().equals("")) {
			String tStrSQL = "select distinct 1 from LDOccupation where OccupationCode='"
					+ tLCInsuredSchema.getOccupationCode().trim()
					+ "' and OccupationType='"
					+ tLCInsuredSchema.getOccupationType().trim() + "'";
			ExeSQL tExeSQL = new ExeSQL();
			String rs = tExeSQL.getOneValue(tStrSQL);
			if (rs == null || rs.length() == 0) {
				buildError("formatLCInsured", "所输入用户职业代码和职业类别不符,用户姓名："
						+ tLCInsuredSchema.getName() + "!");
				return false;
			}
		}
	...
	public LCInsuredRelatedSet getInsuredRelaData(String mainInsuredNo,
		...
		// LCInsuredSchema mainInsured =
		// this.findInsuredfromCache(mainInsureId);
		// if ( mainInsured ==null ){
	...
	public boolean logError(String batchNo, String grpContNo, String contId,
			...
			GlobalInput globalInput, String fileName) {
		boolean res = true;
		...
		if (mBQFlag != null && mBQFlag.equals("Y")) // add by zhangtao
													// 2006-12-05
		{
			tLCGrpImportLogSchema.setOtherNoType("10");
			tLCGrpImportLogSchema.setOtherNo(mEdorAcceptNo);
			tLCGrpImportLogSchema.setStandbyFlag1(mEdorType);
		}
		String errMess = "";
	 ...
	 * @return boolean
	 */
	public boolean delImportLog(String cGrpContNo, String cBatchNo) {
		MMap tMap = new MMap();
		VData tInputData = new VData();
		tMap.put("delete from lcgrpimportlog where grpcontno='" + cGrpContNo
				+ "' and batchno='" + cBatchNo + "'", "DELETE");
		tInputData.add(tMap);

		PubSubmit tPubSubmit = new PubSubmit();
		// 如果有需要处理的错误，则返回
		if (!tPubSubmit.submitData(tInputData, "DELETE")) {
			// @@错误处理
	...
	public VData getContData(String contId, String insuredId,
		...
		if (tLCInsuerdSchema == null) {
			...
			if (!mISPubAcc) {
				if (!checkLCInsured(tLCInsuerdSchema)) {
					return null;
				}
			}
		...
		logger.debug("tLCInsuredDB....... set end");
		...
		// map.put(rInsureSchema,"DELETE&INSERT");

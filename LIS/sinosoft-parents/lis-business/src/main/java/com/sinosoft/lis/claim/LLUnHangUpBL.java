...
public class LLUnHangUpBL {
private static Logger logger = Logger.getLogger(LLUnHangUpBL.class);
	...
	private boolean getInputData() {
		...
		mContent = (String) mTransferData.getValueByName("Content"); // 赔案号
	...
	private boolean dealData(String cOperate) {
		...
		LLContHangUpStateSet tLLContHangUpStateSet = new LLContHangUpStateSet();//备份表
		...
		for (int i = 1; i <= mLCContHangUpStateSet.size(); i++) {
			...
			LLContHangUpStateSchema tLLContHangUpStateSchema = new LLContHangUpStateSchema();
			...
			LCContHangUpStateSchema tLCContHangUpStateSchema = tLCContHangUpBL
					.getHungUp();
			mReflections.transFields(tLLContHangUpStateSchema, tLCContHangUpStateSchema);
			String tSerielNo = PubFun1.CreateMaxNo("LLHangUpSeriel", 20);
			tLLContHangUpStateSchema.setClaimFlag("0");//理赔解挂
			tLLContHangUpStateSchema.setSerialNo(tSerielNo);
			tLLContHangUpStateSchema.setReason(mContent);
		...
		map.put(tLLContHangUpStateSet, "INSERT");
...
//			return false;
//		}
//		tLLReportDB.setAssigneeAddr(mLLReportSchema.getAssigneeAddr());
//		map.put(tLLReportDB.getSchema(), "UPDATE");
		return true;
	}

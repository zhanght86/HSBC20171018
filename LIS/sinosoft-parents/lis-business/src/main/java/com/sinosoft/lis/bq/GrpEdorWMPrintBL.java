 ...
 * @author QianLy
 * @version 6.0
 * @Rewrited By QianLy on 2006-10-14
 */
public class GrpEdorWMPrintBL implements EdorPrint {
private static Logger logger = Logger.getLogger(GrpEdorWMPrintBL.class);
	...
	public static final String VTS_NAME = "GrpEdorNameListWM.vts"; // 模板名称
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 数据传输
		if (!getInputData()) {
			mErrors.addOneError("WM数据传输失败!");
			return false;
		}

		// 数据校验
		if (!checkData()) {
			mErrors.addOneError("WM传入数据无效!");
			return false;
		}

		// 打印数据处理
		if (!dealData()) {
			mErrors.addOneError("WM数据打印失败!");
			return false;
		}

		// 打印人名清单数据处理
		if (!bulidBill()) {
			mErrors.addOneError("WM人名清单数据打印失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			mErrors.addOneError("WM准备数据失败!");
			return false;
		}
	...
	private boolean dealData() {
		...
		xmlexport.addDisplayControl("displayWM");
		...
		mTextTag.add("PT.SumMoney", BqNameFun.chgMoney(tssrs.GetText(1, 1))); // 总退保金额
	...
	private boolean bulidBill() {
		...
		if (isMulProd.toUpperCase().equals("FALSE")) {
			Sql = "select c1,c2,c3,c4,c5,c6,c7,c8,c9,c10,c11,c12,c13,c14,c15"
					...
					+ "                  end) c8,"
					+ "               a.prem c9,"
					+ "               nvl((select ob.AvailableAmnt - pb.AvailableAmnt from loengbonuspol ob,lpengbonuspol pb where ob.polno = pb.polno and ob.FiscalYear = (select max(FiscalYear) from loengbonuspol where polno = ob.polno) and ob.polno =a.polno),0) c10,"
					+ "               (select nvl(sum(getmoney), 0)"
					+ "                  from ljsgetendorse"
					+ "                 where endorsementno = a.edorno"
					+ "                   and feeoperationtype = a.edortype"
		...
		for (int i = 1; i <= SS.MaxRow; i++) {
			...
			for (int j = 0; j < VTS_COLUMN_RISK; j++) {
				if (j >= 4 && j <= 13) {
					strLine[j] = BqNameFun.getRoundMoney(SS.GetText(i, j + 1));
				} else {
			...
			for (int p = 4; p <= 13; p++) {
				if (!BqNameFun.dealHashTable(RiskTable, strLine[3] + "小计" + p,
						strLine[p])) {
					return false;
				}
			}
		...
		for (int i = 1; i <= vct.size(); i++) {
			...
			for (int p = 4; p <= 13; p++) {
				riskLine[p - 3] = BqNameFun.getRoundMoney((String) RiskTable
						.get(riskLine[0] + "小计" + p) == null ? "0.00"
						: (String) RiskTable.get(riskLine[0] + "小计" + p));
			}
		...
		// 全部应退金额
		aTextTag.add("SumMoney", BqNameFun.getRoundMoney((String) RiskTable
	...
	//
	// LPGrpEdorItemDB tLPGrpEdorItemDB=new LPGrpEdorItemDB();
	// tLPGrpEdorItemDB.setEdorAcceptNo("6120071210000011");
	...
	//
	// GrpEdorWMPrintBL tGrpEdorWMPrintBL = new GrpEdorWMPrintBL();
	// if(tGrpEdorWMPrintBL.submitData(tVData,"PRINTWM")){

...
public class LLHurtMedDayCheckBL implements PrintService {
private static Logger logger = Logger.getLogger(LLHurtMedDayCheckBL.class);
	...
	private String mNCLType = ""; // 申请类型private String mNCLType = ""; //申请类型

	public LLHurtMedDayCheckBL() {
	...
	public static void main(String[] args) {
		...
		tTransferData.setNameAndValue("NCLType", "1");// 日结类型

		VData tVData = new VData();
		tVData.add(tG);
		tVData.add(tTransferData);

		LLHurtMedDayCheckBL tLLHurtMedDayCheckBL = new LLHurtMedDayCheckBL();
		if (tLLHurtMedDayCheckBL.submitData(tVData, "") == false) {
			logger.debug("----------理赔日结-->理赔医疗给付日结出错---------------");
		}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("-----理赔日结-->理赔－医疗给付日结开始---------LLHurtMedDayCheckBL---");
		// 得到外部传入的数据,将数据备份到本类中
		...
		logger.debug("-----理赔日结--理赔－医疗给付日结结束----LLHurtMedDayCheckBL---");
	...
	private boolean checkInputData() {
		if (mStatiCode == null || mStatiCode.equals("")) {
			...
			tError.moduleName = "LLHurtMedDayCheckBL";
	...
	private boolean dealData() {
		...
		tXmlExport.createDocument("HurtMedDayCheck.vts", "");
		...
		// 医疗给付日结<长险非身故赔付>----业务类型：A；子业务类型：not %02；财务类型：PK ；
		// 以下主要是 日期的问题《》
		...
		String strSQL = "select distinct j.riskcode,(select riskname from lmrisk where riskcode = j.riskcode),sum(j.pay)"
				...
				// and jj.subfeeoperationtype not in('102','202')"
				...
				+ " and ((j.feefinatype = 'PK' and j.feeoperationtype = 'A' and j.subfeeoperationtype not in ('102', '202')) "
				// Modifyed by niuzj 2005-11-15,QC4695号BUG,在原基础上统计"年度红利和终了红利"
				+ " or (j.feefinatype = 'EF' and j.feeoperationtype = 'C05' "
				+ " and (exists (select b.clmno from llclaimdutykind b where b.clmno = j.otherno and b.getdutykind not in ('102','202','109','209'))) "// Modify
																																						// by
																																						// zhaorx
																																						// 2006-12-29
																																						// QC8209
				...
				// t.caseno=j.otherno and t.reasoncode not in('102','202')) "

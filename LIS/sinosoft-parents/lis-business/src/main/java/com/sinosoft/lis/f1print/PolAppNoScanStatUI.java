...
public class PolAppNoScanStatUI {
private static Logger logger = Logger.getLogger(PolAppNoScanStatUI.class);
	public CErrors mErrors = new CErrors();
	private VData mResult = new VData();

	public PolAppNoScanStatUI() {
	}

	public boolean submitData(VData cInputData, String cOperate) {
		PolAppNoScanStatBL tPolAppNoScanStatBL = new PolAppNoScanStatBL();
		if (!tPolAppNoScanStatBL.submitData(cInputData, cOperate)) {
			mErrors.copyAllErrors(tPolAppNoScanStatBL.mErrors);
			return false;
		} else {
			mResult = tPolAppNoScanStatBL.getResult();
		}
		return true;
	}

...
public class Zhixiao4BL implements PrintService {
private static Logger logger = Logger.getLogger(Zhixiao4BL.class);
	...
	private boolean getPrintData() {
		...
		if (strManagecom.length() == 8) {
			...
			if (strSalechnl == null || strSalechnl.equals("")) {
				//用lwactivity.functionid替换activityid，此时processid可以去掉 2013-04-23 lzf
...
//						+ " and processid = '0000000003' and missionprop7 like '"
				strSQL = "select missionprop1 from (select missionprop1,missionprop7 from lwmission where activityid in (select activityid from lwactivity  where functionid ='10010042')"
...
//						+ " and processid = '0000000003' and missionprop7 like '"
				strSQL = "select missionprop1 from lwmission where activityid in (select activityid from lwactivity  where functionid ='10010042')"
						+ " and missionprop7 like concat('"
						+ "?strManagecom?"
				...
				for (int p = 0; p < i_count; p++) { // 对每张单子进行查询相关信息.
					if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
						strSQL = "select a.contno,a.polapplydate,(case when to_char(a.cvalidate,'YYYY-MM-DD') is not null then to_char(a.cvalidate,'YYYY-MM-DD')  else '无' end),a.insuredname,a.insuredbirthday,"
								...
								+
								"(select agentcode from laagent where agentcode in (select agentcode from lacommisiondetail where grpcontno = a.contno) and branchtype = '5' and branchtype2 = '01' and rownum = '1' ),"
						...
						strSQL = "select a.contno,a.polapplydate,(case when to_char(a.cvalidate,'YYYY-MM-DD') is not null then to_char(a.cvalidate,'YYYY-MM-DD')  else '无' end),a.insuredname,a.insuredbirthday,"
								...
								+
								"(select agentcode from laagent where agentcode in (select agentcode from lacommisiondetail where grpcontno = a.contno) and branchtype = '5' and branchtype2 = '01' limit 0,1 ),"
					...
					sqlbv2.sql(strSQL);
					sqlbv2.put("otherno", ssrs.GetText(p + 1, 1));
					...
					cols[8] = Onessrs.GetText(1, 8); // 话务员
					cols[9] = Onessrs.GetText(1, 9); // 话务员姓名
					cols[10] = getUpComName(Onessrs.GetText(1, 10)); // 直销部组

					cols[11] = Onessrs.GetText(1, 11); // 外务员
					cols[12] = Onessrs.GetText(1, 12); // 外务员姓名
					cols[13] = getUpComName(Onessrs.GetText(1, 13)); // 直销部组

					cols[14] = Onessrs.GetText(1, 14); // 不成功原因
					alistTable.add(cols);

				}
				String[] colsss = new String[15];
				colsss[0] = "合计：";
				colsss[1] = "" + scount;
					...
					for (int p = 0; p < i_count; p++) { // 对每张单子进行查询相关信息.
						if(SysConst.DBTYPE.equals(SysConst.DBTYPE_ORACLE)){
							...
							strSQL = "select a.contno,a.polapplydate,(case when to_char(a.cvalidate,'YYYY-MM-DD') is not null then to_char(a.cvalidate,'YYYY-MM-DD')  else '无' end),a.insuredname,a.insuredbirthday,"
									...
									"(select agentcode from laagent where agentcode in (select agentcode from lacommisiondetail where grpcontno = a.contno) and branchtype = '5' and branchtype2 = '01' and limit 0,1 ),"
									+ " (select name from laagent where agentcode in (select agentcode from lacommisiondetail where grpcontno = a.contno) and branchtype = '5' and branchtype2 = '01' limit 0,1 ),"
									+ "(select r.branchcode from laagent r where r.agentcode in (select agentcode from lacommisiondetail where grpcontno = a.contno) and branchtype = '5' and branchtype2 = '01' limit 0,1 ),"
						...
						sqlbv5.sql(strSQL);
						sqlbv5.put("otherno", ssrs.GetText(p + 1, 1));
...
//							+ " and processid = '0000000003' and missionprop7 like '"
	...
	public static void main(String[] args) {
		...
		tVData.addElement("8626");
		tVData.addElement("1");
		Zhixiao4BL tZhixiao4BL = new Zhixiao4BL();
		tZhixiao4BL.submitData(tVData, "PRINT");
	}
}

...
public class PolAppOtherStatUI {
private static Logger logger = Logger.getLogger(PolAppOtherStatUI.class);
	public CErrors mErrors = new CErrors();
	private VData mResult = new VData();

	public PolAppOtherStatUI() {
	}

	public boolean submitData(VData cInputData, String cOperate) {
		PolAppOtherStatBL tPolAppOtherStatBL = new PolAppOtherStatBL();
		if (!tPolAppOtherStatBL.submitData(cInputData, cOperate)) {
			mErrors.copyAllErrors(tPolAppOtherStatBL.mErrors);
			return false;
		} else {
			mResult = tPolAppOtherStatBL.getResult();
		}
		return true;
	}

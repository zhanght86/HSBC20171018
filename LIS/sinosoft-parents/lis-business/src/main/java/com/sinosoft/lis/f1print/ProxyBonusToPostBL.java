 ...
 * @version  : 1.00, 1.01, 1.02, 1.03, 1.04, 1.05, 1.06, 1.06, 1.07, 1.08, 1.09, 1.10
 * @date     : 2005-10-26, 2005-11-10, 2006-01-24, 2006-02-28, 2006-03-25, 2006-05-29, 2006-07-18, 2006-07-25, 2006-08-18, 2006-09-07, 2006-10-20, 2007-07-02
 * @direction: 代理分红给邮局的清单
 ******************************************************************************/

// ******************************************************************************

public class ProxyBonusToPostBL {
private static Logger logger = Logger.getLogger(ProxyBonusToPostBL.class);
	// ==========================================================================

	// public ProxyBonusToPostBL() {}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// logger.debug("\t@> ProxyBonusToPostBL.submitData() 结束");
		return true;
	...
	private boolean getInputData() {
		// logger.debug("\t@> ProxyBonusToPostBL.getInputData() 开始");
		...
		if (rGlobalInput == null) {
			...
			logger.debug("\t@> ProxyBonusToPostBL.getInputData() : 无法获取 GlobalInput 数据！");
			return false;
		}
		...
		if (rTransferData == null) {
			...
			logger.debug("\t@> ProxyBonusToPostBL.getInputData() : 无法获取 TransferData 数据！");
			return false;
		}
		...
		// logger.debug("\t@> ProxyBonusToPostBL.getInputData() 结束");
		return true;
	...
	private boolean dealData() {
		// logger.debug("\t@> ProxyBonusToPostBL.dealData() 开始");
		...
		QuerySQL = "select (select ZipCode "
				...
				+ "end), "
				// 银行编码
				+ "(select BankCode from LACom where AgentCom = b.AgentCom), "
				...
				+ "where AgentCode = trim(b.AgentCode)))), "
				// 网点名称
				+ "(select Name from LACom where AgentCom = b.AgentCom), "
				// 网点编码
				+ "b.AgentCom, "
				// 公司名称
		...
		// 剩下的 SQL
		QuerySQL += "and b.PolNo = a.PolNo " + "and b.SaleChnl = '3' "
				+ "and exists (select 'X' " + "from LOPrtManager "
				+ "where 1 = 1 "
				+ "and OtherNo = a.PolNo "
				+ "and OtherNoType = '00'"
				+ "and Code = '30'"
		...
		if (tSSRS == null || tSSRS.getMaxRow() <= 0) {
			CError.buildErr(this, "该时间段内无代理分红纪录！");
			...
			// "累计现金价值", "有效保额", "年度分红率", "终了分红率", "银行编码", "营业区", "营销部",
			// "网点名称", "网点编码", "公司名称", "公司地址", "公司邮编", "咨询电话"
			...
			for (int i = 0; i < tSSRS.getMaxRow(); i++) {
				...
				sPolNo = tSSRS.GetText(i + 1, 22);
				sDatePoint = tSSRS.GetText(i + 1, 31);
				...
				if (!tBqPolBalBL.getSumPremPerPol(sPolNo, sDatePoint)) {
					logger.debug("\t@> ProxyBonusToPostBL.dealData() : 计算已交保费失败 : "
									+ sPolNo);
					sSumPrem = "";
				} else {
				...
				if (dSumdBonusCashValue == -1) {
					logger.debug("\t@> ProxyBonusToPostBL.dealData() : 计算累计红利保额现金价值失败 : "
									+ sPolNo);
					sSumdBonusCashValue = "";
				} else {
				...
				// 红利保额和累计红利保额
				try {
					if (tSSRS.GetText(i + 1, 14) != null
							&& !tSSRS.GetText(i + 1, 14).trim().equals("")) {
						dBonusAmnt = Double.parseDouble(tSSRS
								.GetText(i + 1, 14));
					} else {
						dBonusAmnt = 0.0;
					}
					if (tSSRS.GetText(i + 1, 16) != null
							&& !tSSRS.GetText(i + 1, 16).trim().equals("")) {
						dSumdBonusAmnt = Double.parseDouble(tSSRS.GetText(
								i + 1, 16));
					} else {
						dSumdBonusAmnt = 0.0;
					}
				} catch (Exception ex) {
					logger.debug("\t@> ProxyBonusToPostBL.dealData() : 数据类型转换异常 : "
									+ sPolNo);
				}
				...
				// 红利保额现金价值
				try {
					dBonusCashValue = dBonusAmnt / dSumdBonusAmnt
							* dSumdBonusCashValue;
					sBonusCashValue = BqNameFun.getRound(dBonusCashValue);
				} catch (Exception ex) {
					logger.debug("\t@> ProxyBonusToPostBL.dealData() : 红利保额现金价值计算失败 : "
									+ sPolNo);
					sBonusCashValue = "";
				}
				...
				try {
					...
					arrOutputData[i][16] = sSumdBonusCashValue; // 累计现金价值(#)
					...
					arrOutputData[i][20] = tSSRS.GetText(i + 1, 21); // 银行编码
					arrOutputData[i][21] = tSSRS.GetText(i + 1, 23); // 营业区
					arrOutputData[i][22] = tSSRS.GetText(i + 1, 24); // 营销部
					arrOutputData[i][23] = tSSRS.GetText(i + 1, 25); // 网点名称
					arrOutputData[i][24] = tSSRS.GetText(i + 1, 26); // 网点编码
					arrOutputData[i][25] = tSSRS.GetText(i + 1, 27); // 公司名称
					arrOutputData[i][26] = tSSRS.GetText(i + 1, 28); // 公司地址
					arrOutputData[i][27] = tSSRS.GetText(i + 1, 29); // 公司邮编
					arrOutputData[i][28] = tSSRS.GetText(i + 1, 30); // 咨询电话
				} catch (Exception ex) {
					logger.debug("\t@> ProxyBonusToPostBL.dealData() : 赋值给数组出现异常 : "
									+ sPolNo);
				}
			} // end for
		}
		...
		// logger.debug("\t@> ProxyBonusToPostBL.dealData() 结束");
		return true;
	...
	private boolean outputData() {
		// logger.debug("\t@> ProxyBonusToPostBL.outputData() 开始");
		...
		if (sFileType != null && sFileType.trim().equalsIgnoreCase("txt")) {
			...
			logger.debug("代理分红给邮局的 Txt 清单输出文件名: " + sExportTxtName);
			tTxtExport = null;
		}

		// logger.debug("\t@> ProxyBonusToPostBL.outputData() 结束");
		return true;
	...
	public String[] getTitles() {
		String[] arrOutputTitle = new String[] { "邮编", "地址", "姓名", "称谓",
				...
				"年度分红率", "终了分红率", "银行编码", "营业区", "营销部", "网点名称", "网点编码", "公司名称",
				"公司地址", "公司邮编", "咨询电话" };

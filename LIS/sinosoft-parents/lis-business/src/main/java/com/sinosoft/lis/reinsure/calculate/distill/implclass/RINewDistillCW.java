 ...
 * @version 1.0
 */
public class RINewDistillCW implements RIDistill {
	String mEventType;
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=d.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
		...
		strBuffer.append(" from lcpol a , lcduty d ");
		strBuffer
				.append(" where a.conttype='1' and exists(select 'X' from lccontstate where enddate is null and StateType = 'Available' and State = '0' and polno=a.polno ) ");
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecord m where m.unionkey=(a.PolNo||','||d.DutyCode||',CW,'||'"
						+ mAccumulateDefNo
						+ "'||','||'"
	...
	public static void main(String[] args) {
		...
		mRIWFLogSchema.setStartDate("1999-10-30");
		mRIWFLogSchema.setEndDate("2011-11-3");
		mRIWFLogSchema.setTaskCode("L01010070010");
		cInputData.add(mRIWFLogSchema);
		RINewDistillCW tRIDistillNB = new RINewDistillCW();

 ...
 */
public class GEdorTypeXTForLoadBL implements BusinessService{
private static Logger logger = Logger.getLogger(GEdorTypeXTForLoadBL.class);
    public GEdorTypeXTForLoadBL() {
        bulidDocument();
    }
...
//            mBatchNo = PubFun1.CreateMaxNo("SERIALNO", tLimit);
        ...
        for (int i = 0; i < nNodeCount; i++) {
            ...
            if (!tPGrpEdorXTDetailUI.submitData(mInputData, "INSERT||EDOR")) {
            	CError tError = new CError();
            	tError.moduleName = "PEdorBBDetailUI";
            	tError.functionName = "parserXml";
            	tError.errorMessage = "数据提交失败！";
            	this.mErrors.addOneError(tError);
            	this.mErrors.copyAllErrors(tPGrpEdorXTDetailUI.mErrors);
            }

 ...
 * @version 1.0
 */
public class PEdorRSDetailBL {
private static Logger logger = Logger.getLogger(PEdorRSDetailBL.class);
	...
	private boolean getInputData(VData cInputData) {
		...
		tName = mLPEdorItemSchema.getStandbyFlag1();
		logger.debug("****************");
		logger.debug(tName);
		if (tName.equals("") || tName == null) {
			CError tError = new CError();
			tError.moduleName = "PEdorRSDetailBL";
			tError.functionName = "checkdata";
			tError.errorMessage = "补签名人姓名不能为空!";
	...
	private boolean dealData() {
		...
		rLPEdorItemSchema.setStandbyFlag1(tName);
	...
	public static void main(String[] args) {
		logger.debug("-------------PEdorRSDetailBL test start---------------");
		...
		if (!tPEdorRSDetailBL.submitData(tVData, "")) {
			logger.debug(tPEdorRSDetailBL.mErrors.getError(0).errorMessage);
		}
		logger.debug("-------------PEdorRSDetailBL test end---------------");
	}
}

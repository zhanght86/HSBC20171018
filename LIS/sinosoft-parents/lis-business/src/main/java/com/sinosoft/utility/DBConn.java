...
private static Logger logger = Logger.getLogger(DBConn.class);
	...
	public boolean createConnection() {
		...
		if(JUrl.jndi){
			bIsPool = true;
			if (getJNDIPoolConnection()) {
				return true;
			} else {
				logger.debug("JNDI Failed");
				return false;
			}
		}
		...
		try {
			if (con != null) {
				if (!con.isClosed()) {
					try {
						...
						stmt.execute("SELECT 'x' FROM DUAL");
						stmt.close();
						return true;
			...
			if (JUrl.getDBType().equalsIgnoreCase("ORACLE")) {
				...
				dbType = 5;
			}else if (JUrl.getDBType().equalsIgnoreCase("MYSQL")) {
		        dbType = 6;
		    }
...
//				Class.forName("com.p6spy.engine.spy.P6SpyDriver");
	...
	public boolean getJNDIPoolConnection() {
		try {
			DataSource ds = null;
			if (singleDataSource != null) {
				...
				ds = (DataSource) ctx.lookup(JUrl.getDBName());
				singleDataSource = ds;
			}
			...
			e.printStackTrace();
			logger.debug("查找JNDI连接池失败!" + e.getMessage());
			return false;
		}
		return true;
	}
...
//		conn.createConnection();
		DBConn conn = new DBConn("Local");
		conn.createConnection();
		ExeSQL tExeSQL = new ExeSQL(conn);
		String tSQL_Test = "select 1 from dual ";
		String tSQL_UPDATE = "update ldsysvar set remark='test' where sysvar='test'";
		tExeSQL.execUpdateSQL(tSQL_UPDATE);
		logger.debug(tExeSQL.getOneValue(tSQL_Test));
	}

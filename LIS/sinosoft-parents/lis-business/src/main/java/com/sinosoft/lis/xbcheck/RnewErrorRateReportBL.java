 ...
 * 
 */
public class RnewErrorRateReportBL {
private static Logger logger = Logger.getLogger(RnewErrorRateReportBL.class);
	...
	private RnewIssuePolSchema mRnewIssuePolSchema = new RnewIssuePolSchema();
	...
	private boolean dealData() {
		...
		if (mActivityID != null) {
			...
			sqlbv.sql("select activityid from lwactivity  where functionid ='10010001'");
			SQLwithBindVariables sqlbv1=new SQLwithBindVariables();
			sqlbv1.sql("select activityid from lwactivity  where functionid ='10010002'");
			SQLwithBindVariables sqlbv2=new SQLwithBindVariables();
			sqlbv2.sql("select activityid from lwactivity  where functionid ='10010003'");
			String ActivityID1=tExeSQL.getOneValue(sqlbv);
			String ActivityID2=tExeSQL.getOneValue(sqlbv1);
			String ActivityID3=tExeSQL.getOneValue(sqlbv2);
			// 如果是录单发出问题件
//			if (mActivityID.equals("0000001098")
//					|| mActivityID.equals("0000001099")) 
			if (mActivityID.equals(ActivityID1)
						|| mActivityID.equals(ActivityID2)) 
			{
				// 记为初审岗错误
...
//			else if (mActivityID.equals("0000001001")) 
			else if (mActivityID.equals(ActivityID3)) 
			{
...
//								+ "' and activityid in ('0000001098','0000001099','0000001094')";
								+ "' and activityid in (select activityid from lwactivity  where functionid in('10010001','10010002','10010020'))";
						SQLwithBindVariables sqlbv3=new SQLwithBindVariables();
						sqlbv3.sql(tSQL);
						sqlbv3.put("mMissionID", mMissionID);
						tSSRS = tExeSQL.execSQL(sqlbv3);
						// 判断问题件类型1-内部问题件2-外部问题件
						if (mRnewIssuePolSchema.getBackObjType() != null
								&& mRnewIssuePolSchema.getBackObjType().equals(
										"1")) 
						{
...
//								+ " and activityid in ('0000001098', '0000001099', '0000001001','0000001094'))"
								+ " and activityid in (select activityid from lwactivity  where functionid in('10010001','10010002','10010020','10010003'))) g"
								+ " where rn = 1";
				...
				// 先查找新契约的 MissionID
				tSQL = "select missionprop2 from lwmission where activityid = '0000000009' and missionid = '?mMissionID?'";
				SQLwithBindVariables sqlbv5=new SQLwithBindVariables();
				sqlbv5.sql(tSQL);
				sqlbv5.put("mMissionID", mMissionID);
				SSRS contnoSSRS = new SSRS();
				contnoSSRS = tExeSQL.execSQL(sqlbv5);
				if (contnoSSRS != null && contnoSSRS.getMaxRow() > 0) {
					tSQL = "select proposalcontno from lccont where contno = '?contno?'";
					SQLwithBindVariables sqlbv6=new SQLwithBindVariables();
					sqlbv6.sql(tSQL);
					sqlbv6.put("contno", contnoSSRS.GetText(1, 1));
					SSRS ProposalcontnoSSRS = new SSRS();
					ProposalcontnoSSRS = tExeSQL.execSQL(sqlbv6);
					if (ProposalcontnoSSRS != null
							&& ProposalcontnoSSRS.getMaxRow() > 0) {
						tSQL = "select missionid from lbmission where missionprop1 = '?missionprop1?'";
						SQLwithBindVariables sqlbv7=new SQLwithBindVariables();
						sqlbv7.sql(tSQL);
						sqlbv7.put("missionprop1", ProposalcontnoSSRS.GetText(1, 1));
						SSRS missionidSSRS = new SSRS();
						missionidSSRS = tExeSQL.execSQL(sqlbv7);
...
//						+ "' and activityid in ('0000001001') order by makedate desc";
			...
			for (int i = 0; i < tErrorReportCont.length; i++) {
				...
				tLCErrorReportSchema.setotherno(mRnewIssuePolSchema
						.getProposalContNo());
				tLCErrorReportSchema.setErrorPos(tErrorReportCont[i][0]);
				tLCErrorReportSchema.setErrorOperator(tErrorReportCont[i][1]);
				tLCErrorReportSchema.setSerialNo(mRnewIssuePolSchema
						.getSerialNo());
				tLCErrorReportSchema.setOperatorPos(mActivityID);
				tLCErrorReportSchema.setErrorCode(mRnewIssuePolSchema
						.getIssueType());
				tLCErrorReportSchema.setErrorContent(mRnewIssuePolSchema
						.getIssueCont());
	...
	private boolean getInputData(VData cInputData) {
		...
		mRnewIssuePolSchema = (RnewIssuePolSchema) cInputData
				.getObjectByObjectName("RnewIssuePolSchema", 0);
	...
	private boolean checkData() {
		...
		if (mRnewIssuePolSchema == null) {
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "ErrorRateReportBL";
			tError.functionName = "checkData";
			tError.errorMessage = "传入RnewIssuePolSchema数据失败!";
			this.mErrors.addOneError(tError);
			return false;
	...
	public static void main(String[] args) {
		...
		tRnewIssuePolSchema.setProposalContNo("20051229000002");
		tRnewIssuePolSchema.setIssueCont("aa");
		tRnewIssuePolSchema.setIssueType("01");
		tRnewIssuePolSchema.setSerialNo("1");

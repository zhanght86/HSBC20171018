 ...
 * @version 1.0
 */
public class CertSendOutApplyUI implements BusinessService {
private static Logger logger = Logger.getLogger(CertSendOutApplyUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			CertSendOutApplyBL tCertSendOutApplyBL = new CertSendOutApplyBL();
			boolean bReturn = tCertSendOutApplyBL.submitData(cInputData,
					mOperate);
			mResult = tCertSendOutApplyBL.getResult();

			if (!bReturn) {
				if (tCertSendOutApplyBL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tCertSendOutApplyBL.mErrors);
				} else {

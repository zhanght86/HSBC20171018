...
public class JMBPODealInputDataBL {
private static Logger logger = Logger.getLogger(JMBPODealInputDataBL.class);
	/** 数据处理类 */
	...
	public JMBPODealInputDataBL() {
	}

	/**
	 * 处理界面提交数据的公共方法
	 * 
	 * @param: tBussNo 印刷号 cOperate 数据操作
	 * @return:
	 */
	public boolean submitData(String tBussNo, GlobalInput tGI) {
		...
		Element tOneContData;
		try {
			String tSQL = " and BussNo='" + tBussNo + "' and BussNoType='JM'";
			Blob tBlob  = null;
			...
			tOneContData = domBuilder.build(tBlob.getBinaryStream())
					.getRootElement();
			conn.close();
	    	...
	    	{
	    		logger.debug("******处理投保单（印刷号为"+tBussNo+"）时终止 ，原因是:超过自动运行处理时间范围！ " );
	    		return false;
	    	}
			

			// 处理数据
			if (!DealOneFile1(tOneContData, tGI))
				;
			return false;
	...
	private boolean getAllBOPPolData(BPODataContainer tAllBOPPol,
			Element tOnePolData) {
		try {
			JMTBXMLTransfer JMTBXMLTransfer = new JMTBXMLTransfer(); // 将XML转换为VData
			...
			if (tOnePolData != null) {
				...
				schemaData = JMTBXMLTransfer.XMLToVData(tOnePolData);
				if (schemaData == null) {
					mErrors.copyAllErrors(JMTBXMLTransfer.mErrors);
					return false;
				}
	...
	private boolean getImportedBOPPolData(BPODataContainer tAllBOPPol,
		...
		try {
			...
			for (int i = 0; i < tBussNoData.size(); i++) {
				...
				String SQL = "select 1 from LCCont where PrtNo='" + "?BussNo?"
						+ "'" + " union "
						+ " select 1 from BPOMissionState where BussNo='"
						+ "?BussNo?" + "' and BussNoType='JM' and State <>'2'"; // 不考虑已删除的异常件
				// 查看工作流节点是否是处于投保单录入外包状态,如果不在此状态,当作已处理保单 weikai
				// String SQL = "select 1 from lwmission where missionprop1='"
				// + tBussNo
				// + "' and processid='0000000003'and activityid='0000001403'";
				SQLwithBindVariables sqlbv = new SQLwithBindVariables();
				sqlbv.sql(SQL);
				sqlbv.put("BussNo", tBussNo);
	...
	private boolean getQuestBOPPolData(BPODataContainer tAllBOPPol,
		...
		try {
			...
			for (int i = 0; i < tBussNoData.size(); i++) {
				...
				if ("02".equals(tBPOMissionStateSchema.getDealType())
	...
	private boolean getNeedCheckBOPPolData(String tBPOID,
		...
		try {
			...
			tBPOCheckCalModeDB.setBussNoType("JM");
			// 如果没有抽检规则，则不需要抽检
			...
			sqlbv1.sql(tSQL);
			sqlbv1.put("bpoid", tBPOID);
			sqlbv1.put("MakeDate", PubFun.getCurrentDate());
			int tAlreadyCheckCount = Integer
					.parseInt(tExeSQL.getOneValue(sqlbv1));// 当天已经确定为抽检件的保单数量
	 ...
	 */
	private boolean dealImportedAndNeedCheckData(BPODataContainer tImportedBOPPol, BPODataContainer tNeedCheckBOPPol,
			String serNo, GlobalInput tGI, String tBPOID) {
		try {
			// 1 处理已经导入过的保单数据
			...
			ex.printStackTrace();
			CError.buildErr(this, "处理已经导入件的保单发生异常！");
			return false;
		}
		return true;
	}
	...
	public boolean redoInputedPol(String tBussNo) {
		try {
			String tSQL = "select count(1) from LCPol where PrtNo='" + "?tBussNo?"
					+ "'";
			...
			tSQL = "select count(1) from LCCont where ContNo='" + "?tBussNo?" + "'";
			SQLwithBindVariables sqlbv7 = new SQLwithBindVariables();
			sqlbv7.sql(tSQL);
			sqlbv7.put("tBussNo", tBussNo);
			tExeSQL = new ExeSQL();
			int tExistContCount = Integer.parseInt(tExeSQL.getOneValue(sqlbv7));
	...
	public boolean redoInputedCont(String tBussNo, String tDealType,
		...
		try {
			...
			sqlbv29.sql("delete from LCInsuredRelated where "
					+ tPolWherePart);
			sqlbv29.put("tBussNo", tBussNo);
			mapDel.put(sqlbv29,"DELETE");
			SQLwithBindVariables sqlbv30 = new SQLwithBindVariables();
			sqlbv30.sql("delete from LCSpec where "
					+ tPolWherePart);
			sqlbv30.put("tBussNo", tBussNo);
			mapDel.put(sqlbv30,"DELETE");
			SQLwithBindVariables sqlbv31 = new SQLwithBindVariables();
			sqlbv31.sql("delete from LCPremToAcc where "
					+ tPolWherePart);
			sqlbv31.put("tBussNo", tBussNo);
			mapDel.put(sqlbv31,"DELETE");
			SQLwithBindVariables sqlbv32 = new SQLwithBindVariables();
			sqlbv32.sql("delete from LCGetToAcc where "
					+ tPolWherePart);
			sqlbv32.put("tBussNo", tBussNo);
		...
		try {
			...
			for (int i = 0; i < tRightBussNoData.size(); i++) {
				...
				if (!DealOneRightPol(tVData, 0)) // 如果导入失败，则将该清洁件视为导入时的异常件
				{
					...
					tBPOMissionDetailErrorSchema.setBussNoType("JM");
				...
				} else // 如果成功，则记录该保单的状态
				{
					...
					sqlbv33.sql(tSQL);
					sqlbv33.put("BussNo", tBPOMissionStateSchema.getBussNo());
					int tDealCount = Integer
							.parseInt(tExeSQL.getOneValue(sqlbv33));
					logger.debug("tDealCount: " + tDealCount);
					tBPOMissionDetailStateSchema.setDealCount(tDealCount + 1);
					tVData.add(tBPOMissionDetailStateSchema);
	...
	private MMap prepareDelMap(String tPrtNo, Vector tImoprtedPol) {
		...
		try {
			...
			sqlbv34.sql("delete from LCCont where ContNo='" + "?tPrtNo?" + "'");
			sqlbv34.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv34,
					"DELETE");
			SQLwithBindVariables sqlbv35 = new SQLwithBindVariables();
			sqlbv35.sql("delete from LCAppnt where ContNo='" + "?tPrtNo?" + "'");
			sqlbv35.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv35,
					"DELETE");
			SQLwithBindVariables sqlbv36 = new SQLwithBindVariables();
			sqlbv36.sql("delete from LCInsured where ContNo='" + "?tPrtNo?" + "'");
			sqlbv36.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv36,
					"DELETE");
			SQLwithBindVariables sqlbv37 = new SQLwithBindVariables();
			sqlbv37.sql("delete from LCInsureAcc where ContNo='" + "?tPrtNo?" + "'");
			sqlbv37.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv37,
					"DELETE");
			SQLwithBindVariables sqlbv38 = new SQLwithBindVariables();
			sqlbv38.sql("delete from LCInsureAccClass where ContNo='" + "?tPrtNo?" + "'");
			sqlbv38.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv38,
					"DELETE");
			SQLwithBindVariables sqlbv39 = new SQLwithBindVariables();
			sqlbv39.sql("delete from LCInsureAccTrace where ContNo='" + "?tPrtNo?" + "'");
			sqlbv39.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv39,
					"DELETE");
			SQLwithBindVariables sqlbv40 = new SQLwithBindVariables();
			sqlbv40.sql("delete from LCInsureAccFee where ContNo='" + "?tPrtNo?" + "'");
			sqlbv40.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv40,
					"DELETE");
			SQLwithBindVariables sqlbv41 = new SQLwithBindVariables();
			sqlbv41.sql("delete from LCInsureAccClassFee where ContNo='" + "?tPrtNo?" + "'");
			sqlbv41.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv41,
					"DELETE");
			SQLwithBindVariables sqlbv42 = new SQLwithBindVariables();
			sqlbv42.sql("delete from LCPol where ContNo='" + "?tPrtNo?" + "'");
			sqlbv42.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv42,
					"DELETE");
			//删除投保单原始数据 2008-12-27 ln add 
			SQLwithBindVariables sqlbv43 = new SQLwithBindVariables();
			sqlbv43.sql("delete from LCPolOriginal where ContNo='" + "?tPrtNo?" + "'");
			sqlbv43.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv43,
					"DELETE");
			SQLwithBindVariables sqlbv44 = new SQLwithBindVariables();
			sqlbv44.sql("delete from LCDuty where ContNo='" + "?tPrtNo?" + "'");
			sqlbv44.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv44,
					"DELETE");
			SQLwithBindVariables sqlbv45 = new SQLwithBindVariables();
			sqlbv45.sql("delete from LCPrem where ContNo='" + "?tPrtNo?" + "'");
			sqlbv45.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv45,
					"DELETE");
			SQLwithBindVariables sqlbv46 = new SQLwithBindVariables();
			sqlbv46.sql("delete from LCGet where ContNo='" + "?tPrtNo?" + "'");
			sqlbv46.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv46,
					"DELETE");
			SQLwithBindVariables sqlbv47 = new SQLwithBindVariables();
			sqlbv47.sql("delete from LCBnf where ContNo='" + "?tPrtNo?" + "'");
			sqlbv47.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv47,
					"DELETE");
			SQLwithBindVariables sqlbv48 = new SQLwithBindVariables();
			sqlbv48.sql("delete from LCCustomerImpart where ContNo='" + "?tPrtNo?" + "'");
			sqlbv48.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv48,
					"DELETE");
			SQLwithBindVariables sqlbv49 = new SQLwithBindVariables();
			sqlbv49.sql("delete from LCCustomerImpartParams where ContNo='" + "?tPrtNo?" + "'");
			sqlbv49.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv49,
					"DELETE");
			SQLwithBindVariables sqlbv50 = new SQLwithBindVariables();
			sqlbv50.sql("delete from LCCustomerImpartDetail where ContNo='" + "?tPrtNo?" + "'");
			sqlbv50.put("tPrtNo", tPrtNo);
			mapDel.put(sqlbv50,
					"DELETE");
			SQLwithBindVariables sqlbv51 = new SQLwithBindVariables();
			sqlbv51.sql("delete from LACommisionDetail where ContNo='" + "?tPrtNo?" + "'");
			...
			if (tImoprtedPol != null && tImoprtedPol.size() > 0) {
				...
				for (int i = 0; i < tImoprtedPol.size(); i++) {
					...
					tResult.append("'").append((String) tImoprtedPol.get(i))
							.append("'");
					arr.add((String) tImoprtedPol.get(i));
				}
				tResult.append(")");
				...
				sqlbv52.sql("delete from LCInsuredRelated where  PolNo in ("+"?list?"+")");
				sqlbv52.put("list", arr);
				mapDel.put(sqlbv52, "DELETE");
				SQLwithBindVariables sqlbv53 = new SQLwithBindVariables();
				sqlbv53.sql("delete from LCSpec where  PolNo in ("+"?list?"+")");
				sqlbv53.put("list", arr);
				mapDel.put(sqlbv53, "DELETE");
				SQLwithBindVariables sqlbv54 = new SQLwithBindVariables();
				sqlbv54.sql("delete from LCPremToAcc where  PolNo in ("+"?list?"+")");
				sqlbv54.put("list", arr);
				mapDel.put(sqlbv54, "DELETE");
				SQLwithBindVariables sqlbv55 = new SQLwithBindVariables();
				sqlbv55.sql("delete from LCGetToAcc where  PolNo in ("+"?list?"+")");
				sqlbv55.put("list", arr);
				mapDel.put(sqlbv55, "DELETE");
			}
		} catch (Exception ex) {
			CError.buildErr(this, ex.toString());
			ex.printStackTrace();
	 ...
	 * @return 如果处理出现异常，则返回false,否则返回true
	 */
	public boolean DealOneFile1(Element tOnePolData, GlobalInput tGI) {
		try {
			logger.debug("Start  ...");
			...
			String tOperator = tGI.Operator;// 得到操作员的编码
//			LDUserDB tLDUserDB = new LDUserDB();
//			tLDUserDB.setUserCode(tOperator);
//			if (!tLDUserDB.getInfo()) {
//				CError.buildErr(this, "错误的操作员编码！");
//				return false;
//			}

			logger.debug("...Start getAllBOPPolData ...");
			// 1.获取保单信息
			if (!getAllBOPPolData(tAllBOPPol, tOnePolData)) {
				return false;
			}
			...
			// 4.NeedCheckBOPPol
			 if(!getNeedCheckBOPPolData(tOperator,tAllBOPPol,tNeedCheckBOPPol))
			 {
				 return false;
			 }
			...
			logger.debug("...Start dealImportedData ...");
			if (!dealImportedAndNeedCheckData(tImportedBOPPol, tNeedCheckBOPPol, serNo, tGI, tOperator)) {
				return false;
			}

			// 6.清洁件系统导入功能
			logger.debug("...Start dealRightData ...");
			if (!dealRightData(tAllBOPPol, tQuestBOPPol, serNo, tGI, tOperator)) {
				return false;
			}

			// 7.处理异常件
			logger.debug("...Start dealQuestData ...");
			if (!dealQuestData(tQuestBOPPol, serNo, tGI, tOperator)) {
				return false;
			}
		...
		try {
			...
			if (tOldLCPolSchema.getSaleChnl() != null
				...
				else
				{
					...
					{
						String tSQL = "select Managecom from ES_Doc_Main where DocCode='"+"?DocCode?"+"' and subtype='UA001' and rownum=1";
						ExeSQL tExeSQL = new ExeSQL();
						SQLwithBindVariables sqlbv56 = new SQLwithBindVariables();
						sqlbv56.sql(tSQL);
						sqlbv56.put("DocCode", tOldLCPolSchema.getPrtNo());
						String tManageCom = tExeSQL.getOneValue(sqlbv56);//扫描机构
			...
			tLCContSchema.setNewPayMode(StrTool.cTrim((String) tOldTransferData
			...
			tLCContSchema.setPayLocation(StrTool.cTrim((String) tOldTransferData
			...
			//双岗录入增加【初审日期】 && 【续期缴费提示】 --add hanbin 2010-04-16
...
//					.getValueByName("DealType");
			...
			**/
			...
			for (int i = 1; i < tOldRiskBasicInfoSet.size(); i++) {
				...
				sqlbv57.sql("select (case when risktype7 is null then '0' else risktype7 end) from LMRiskApp where RiskCode='"
						+ "?RiskCode?" + "'");
				sqlbv57.put("RiskCode", tRiskBasicInfo1.getRiskCode());				
            ...
            **/
			...
			for (int i = 0; i < tOldRiskBasicInfoSet.size(); i++) {
		        ...
		        if(!tRiskBasicInfo.isMainRisk())
		        {
		        	...
		        	sqlbv58.sql("select RiskPeriod from LMRiskApp where RiskCode='"
							+ "?RiskCode?" + "'");
		        	sqlbv58.put("RiskCode", tLCPolSchema.getRiskCode());
				...
				if (tRiskBasicInfo.isMainRisk()) {
					...
					if (tLCBnfSet != null && tLCBnfSet.size() > 0) {
						...
						//2009-05-09 ln modify 受益比例必须为100%，且收益顺序必须连续
...
//****************************************************************************************
				...
				if (tFlag == 0) {
					...
					sqlbv59.sql(str1);
					sqlbv59.put("CurrentDate", PubFun.getCurrentDate());
					sqlbv59.put("CurrentTime", PubFun.getCurrentTime());
					sqlbv59.put("PolNo", tReturnLCPolSchema.getPolNo());
...
//**********************************对投保人豁免、被保人豁免分别准备主险保单数据和校验保额********************************************
			...
			{				
				...
				sqlbv60.sql(tSql);
				sqlbv60.put("prtno", tLCPolSchema.getPrtNo());
				sqlbv60.put("insuredno", tInsuredNo);
				LCPolSet tLCPolSet = tLCPolDB.executeQuery(sqlbv60);
				if (tLCPolSet.size()!=1) {
			...
			else if(tSpecRiskFlag.equals("appnt"))
			{
				...
				String tSql = "select * from lcpol where prtno='"+"?prtno?"+"' and insuredno="
				           +" (select insuredno from lcinsured where prtno='"+"?prtno?"+"' and sequenceno in('1','-1'))"
				           +" and polno=mainpolno and (RiskSequence is null or RiskSequence in ('1','-1'))";
				SQLwithBindVariables sqlbv61 = new SQLwithBindVariables();
				sqlbv61.sql(tSql);
				sqlbv61.put("prtno", tLCPolSchema.getPrtNo());
				LCPolSet tLCPolSet = tLCPolDB.executeQuery(sqlbv61);
				if (tLCPolSet.size()!=1) {
					CError.buildErr(this, "lcpol表查询失败！");
					return false;
				}
				...
				sqlbv62.sql(tSql);
				sqlbv62.put("prtno", tLCPolSchema.getPrtNo());
				LCInsuredSet tLCInsuredSet = tLCInsuredDB.executeQuery(sqlbv62);
				if (tLCInsuredSet.size()!=1) {
			...
			sqlbv63.sql("select RiskPeriod from LMRiskApp where RiskCode='"
					+ "?RiskCode?" + "'");
			sqlbv63.put("RiskCode", tLCPolSchema.getRiskCode());
			String tRiskPeriod = tExeSQL
						.getOneValue(sqlbv63);
			 ...
			 if(tSpecRiskFlag.equals("insured"))
				{
					...
					sqlbv64.sql(tSql);
					sqlbv64.put("prtno", tLCPolSchema.getPrtNo());
					sqlbv64.put("insuredno", tLCContSchema.getInsuredNo());
					sqlbv64.put("code", tLCPolSchema.getRiskCode());
					tExeSQL = new ExeSQL();
					...
					if (tSubAmnt==0) {
				        ...
				        sqlbv65.sql(tSql);
				        sqlbv65.put("riskcode", tLCPolSchema.getRiskCode());
						tExeSQL = new ExeSQL();
				        String tRiskName = tExeSQL.getOneValue(sqlbv65);
				        ...
				        sqlbv66.sql(tSql);
				        sqlbv66.put("riskcode", tLCPolSchema.getRiskCode());
						tExeSQL = new ExeSQL();
				        String tRiskName = tExeSQL.getOneValue(sqlbv66);
				...
				else if(tSpecRiskFlag.equals("appnt"))
				{
					...
					sqlbv67.sql(tSql);
					sqlbv67.put("prtno", tLCPolSchema.getPrtNo());
					sqlbv67.put("code", tLCPolSchema.getRiskCode());
					tExeSQL = new ExeSQL();
					double tSubAmnt = Double.parseDouble(tExeSQL
							.getOneValue(sqlbv67));
					if (tSubAmnt==0) {
						...
						sqlbv68.sql(tSql);
						sqlbv68.put("riskcode", tLCPolSchema.getRiskCode());
						tExeSQL = new ExeSQL();
				        String tRiskName = tExeSQL.getOneValue(sqlbv68);
						CError.buildErr(this, "该主险不能附加"+ tRiskName);
						return false;
					}
					else if (tLCPolSchema.getAmnt() != tSubAmnt) {
						...
						sqlbv69.sql(tSql);
						sqlbv69.put("riskcode", tLCPolSchema.getRiskCode());
						tExeSQL = new ExeSQL();
				        String tRiskName = tExeSQL.getOneValue(sqlbv69);
						CError.buildErr(this, tRiskName+"的保额有误，应为"+tSubAmnt+"！");
						return false;
					}
				}
...
//****************************************************************************************
				...
				if (tFlag == 0) {
					...
					sqlbv70.sql(str1);
					sqlbv70.put("CurrentDate", PubFun.getCurrentDate());
					sqlbv70.put("CurrentTime", PubFun.getCurrentDate());
					sqlbv70.put("PolNo", tReturnLCPolSchema.getPolNo());
					map.put(sqlbv70, "UPDATE");
					tResult.add(map);
					if (!tPubSubmit.submitData(tResult, "")) {
						CError.buildErr(this, "保单复核状态修改失败，原因是: "
								+ tPubSubmit.mErrors.getError(0).errorMessage);
...
//			// 如果异常件或者抽检件处理成功，修改任务处理表的状态
//			if (tFlag == 1) {
//				if (!updateBPOState(tMainLCPolSchema.getPrtNo(), tDealType,
//						"1", tGI)) {
//					return false;
//				}
//			}
	...
	public boolean DealAddFee(VData tAddFeeData) {
		try {
			...
			String tSQL = "select b.riskcode,a.dutycode,firstpaydate,payenddate "
	                   + " from lcduty a left join lmriskduty b " 
	                   + " on a.dutycode=b.dutycode where polno ='"+"?polno?"+"'";
			SQLwithBindVariables sqlbv71 = new SQLwithBindVariables();
			sqlbv71.sql(tSQL);
			sqlbv71.put("polno", tLCPolSchema.getPolNo());
			ExeSQL tExeSQL = new ExeSQL();
			SSRS tSSRS = tExeSQL.execSQL(sqlbv71);
			...
			LCContSchema tLCContSchema = tLCContSet.get(1);
				...
				//********************************************* 生成加费信息		
					...
					sqlbv72.sql(tsql);
					sqlbv72.put("", tLCPolSchema.getPolNo().trim());
					String tReSult = new String();
					tExeSQL = new ExeSQL();
					tReSult = tExeSQL.getOneValue(sqlbv72);
	...
	**/		
	...
	public boolean updateBPOState(String tBussNo, String tDealType,
		...
		try {
			...
			tBPOMissionStateDB.setBussNoType("JM");
			...
			sqlbv73.sql(tSQL);
			sqlbv73.put("BussNo", tBPOMissionStateSchema.getBussNo());
			int tDealCount = Integer.parseInt(tExeSQL.getOneValue(sqlbv73));
			logger.debug("tDealCount: " + tDealCount);
			tBPOMissionDetailStateSchema.setDealCount(tDealCount + 1);
	...
	public boolean updateLCCont(String tPrtNo) {
		try {
			...
			//查询第一被保人信息
			LCInsuredDB tLCInsuredDB = new LCInsuredDB();
			String tSql = "select * from lcinsured where prtno='"+"?prtno?"+"' and sequenceno in ('1','-1')";
			SQLwithBindVariables sqlbv74 = new SQLwithBindVariables();
			sqlbv74.sql(tSql);
			sqlbv74.put("prtno", tPrtNo);
			LCInsuredSet tLCInsuredSet = tLCInsuredDB.executeQuery(sqlbv74);
			if (tLCInsuredSet.size()!=1) {
	...
	private boolean checkRiskRelation(String tMainPolNo, String cRiskCode) {
		...
		sqlbv75.sql(strSql);
		sqlbv75.put("PolNo", tMainPolNo);
		sqlbv75.put("Code", cRiskCode);
		String tResult = tExeSQL.getOneValue(sqlbv75);
	...
	private boolean checkSaleChnl(LCPolSchema tLCPolSchema) {
		try {	
			...
			sqlbv76.sql(tSQL);
			sqlbv76.put("Code", tLCPolSchema.getSaleChnl());
			ExeSQL tExeSQL = new ExeSQL();
			String tRestlt = tExeSQL.getOneValue(sqlbv76);
			if (tRestlt != null && !"".equals(tRestlt)) {
				...
				sqlbv77.sql(tSubSQL);
				sqlbv77.put("Code", tLCPolSchema.getSaleChnl());
				sqlbv77.put("Code1", tLCPolSchema.getRiskCode());
				String tSubResult = tExeSQL.getOneValue(sqlbv77);
	...
	private boolean checkContData(LCPolSchema tLCPolSchema,
		...
		try {
			// 校验保单的管理机构与扫描件的管理机构
			String tSQL = "SELECT managecom FROM es_doc_main where doccode='"
			...
			sqlbv78.sql(tSQL);
			sqlbv78.put("doccode", tLCPolSchema.getPrtNo());
			ExeSQL tExeSQL = new ExeSQL();
			String tManageCom = tExeSQL.getOneValue(sqlbv78);
			if (tManageCom != null && !"".equals(tManageCom)) {
			...
			sqlbv79.sql(tSQL);
			sqlbv79.put("code", tLCPolSchema.getSaleChnl());
			tExeSQL = new ExeSQL();
			String tRestlt = tExeSQL.getOneValue(sqlbv79);
			if (tRestlt == null || "".equals(tRestlt)) {
				...
				if (!"05".equals(tLCPolSchema.getSaleChnl())
				       ...
				       if("7".equals(tLAComDB.getBranchType()))
				       {
					     ...
					     SQLwithBindVariables sqlbv81 = new SQLwithBindVariables();
							sqlbv81.sql(tSQL);
							sqlbv81.put("agentcom",tLCPolSchema.getAgentCom());
							sqlbv81.put("agentcode",tLCPolSchema.getAgentCode());
					     if("".equals(StrTool.cTrim(tExeSQL.getOneValue(sqlbv81))))
					     {
			...
			sqlbv80.sql(tSQL);
			sqlbv80.put("agentcode", tLAAgentSchema.getAgentCode());
			String tBranchType = tExeSQL.getOneValue(sqlbv80);
			...
			sqlbv82.sql(tSQL);
			sqlbv82.put("code", tLCPolSchema.getSaleChnl());
			sqlbv82.put("code1", tBranchType);
			// 表示销售渠道和代理人无校验
			 String tResult = tExeSQL.getOneValue(sqlbv82);
			 if(tResult == null || "".equals(tResult))
			 {
...
//					.getValueByName("AppntIDEndDate"));
	...
	public static void main(String[] args) {
		...
		JMBPODealInputDataBL BPODealInputDataBL1 = new JMBPODealInputDataBL();
		String tBussNo = "86250100149999";
		GlobalInput tGI = new GlobalInput();
		tGI.Operator = "021";
		tGI.ManageCom = "86";
		tGI.ComCode = "86";
		BPODealInputDataBL1.submitData(tBussNo, tGI);
	}
}

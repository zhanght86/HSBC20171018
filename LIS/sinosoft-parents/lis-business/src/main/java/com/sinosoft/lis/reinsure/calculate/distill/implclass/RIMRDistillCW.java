 ...
 * </p>
 * <p>
 * Description:再保有效保单(财务数据)通过可参考提数程序
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class RIMRDistillCW implements RIDistill {
	String mEventType;
	...
	private boolean dealData() {
		try {
			...
			mowMonth = mRIWFLogSchema.getEndDate().substring(5, 7);
			if (mowMonth.equals("1")) {
				mult = String.valueOf("0");
			} else {
				mult = String.valueOf(Integer.parseInt(mowMonth) % 3);
			}
			// 合同分保提数
			tSql = getDistillSQL();
	...
	private String getDistillSQL() {
		...
		strBuffer.append(" a.PolNo||','||d.DutyCode||',CW,'||'"
				+ mAccumulateDefNo + "'||','||'"
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=d.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
		...
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecord m where m.unionkey=(a.PolNo||','||d.DutyCode||',CW,'||'"
						+ mAccumulateDefNo
						+ "'||','||'"
						+ mRIWFLogSchema.getEndDate().substring(0, 7) + "')) ");
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecordBake n where n.unionkey=(a.PolNo||','||d.DutyCode||',CW'||','||'"
						+ mAccumulateDefNo
						+ "'||','||'"
						+ mRIWFLogSchema.getEndDate().substring(0, 7) + "')) ");
		// 不为临分的保单
		...
		// strBuffer.append(" and greatest(a.signdate,a.cvalidate) between '");//
		// 保单生效日与签单日
		// strBuffer.append(mRIWFLogSchema.getStartDate());
		// strBuffer.append("' and '");
		// strBuffer.append(mRIWFLogSchema.getEndDate());
		// strBuffer.append(" and b.EndDate >= '"+ mRIWFLogSchema.getEndDate() +
		// "'");
		...
		strBuffer.append(mRIWFLogSchema.getEndDate());
		strBuffer.append("' ) ");
		System.out.println("财务数据提数sql: " + strBuffer.toString());
	...
	private String getDistillSQL1() {
		...
		strBuffer.append(" a.InsuredNo as InsuredNo, ");
		strBuffer
				.append(" (select x.Nameen from Lcinsured x where x.Contno=a.Contno and x.Insuredno=a.Insuredno) as InsuredName, ");
		strBuffer.append(" decode(a.InsuredSex,'0','0','1') as InsuredSex, ");
		strBuffer.append(" a.InsuredBirthday as InsuredBirthday, ");
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=d.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
		...
		strBuffer.append(" from lcpol a,lcduty d ");
		strBuffer
				.append(" where a.polno = d.Polno and a.conttype='1' and exists(select 'X' from lccontstate where enddate is null and StateType = 'Available' and State = '0' and polno=a.polno ) ");
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecord m where m.unionkey=(a.PolNo||','||d.DutyCode||',CW,'||'"
						+ mAccumulateDefNo
						+ "'||','||'"
		...
		strBuffer.append(mRIWFLogSchema.getEndDate());
		strBuffer.append("' ) ");
		System.out.println("财务数据临分提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	public static void main(String[] args) {
		...
		mRIWFLogSchema.setStartDate("2012-02-27");
		mRIWFLogSchema.setEndDate("2012-02-27");
		mRIWFLogSchema.setTaskCode("L01010010013");
		cInputData.add(mRIWFLogSchema);
		RIMRDistillCW tRIDistillNB = new RIMRDistillCW();

...
private static Logger logger = Logger.getLogger(LLInqConclusionBL.class);
	...
	private boolean getInputData() {
		...
		mMasFlag = mLLInqConclusionSchema.getMasFlag();
	...
	private boolean dealData(String cOperate) {
		...
		if (cOperate.equals("UPDATE")) {
			...
			tLLInqConclusionSchema.setMasFlag(mMasFlag);
		...
		if (cOperate.equals("UPDATE&&INSERT")) {
			...
			ttLLInqConclusionDB.setBatNo("000000");// 调查批次
			ttLLInqConclusionDB.setFiniFlag("0");
			ttLLInqConclusionSet.set(ttLLInqConclusionDB.query());
			String tConNo = "";
			if (ttLLInqConclusionSet.size() == 1) {
				...
				ttLLInqConclusionSchema = ttLLInqConclusionSet.get(1);
				...
				ttLLInqConclusionSchema.setInqConclusion("调查赔案结论已由"
						+ mG.Operator + "汇总，结论序号"
						+ mLLInqConclusionSchema.getConNo());
				...
				// 查询已完成的调查中是否有阳性标志
				String tSQLF = " select count(*) from llinqconclusion "
						+ " where clmno='" + "?clmno?"
						+ "' " + " and finiflag='1' and masflag='1' ";
				SQLwithBindVariables sqlbv = new SQLwithBindVariables();
				sqlbv.sql(tSQLF);
				sqlbv.put("clmno", mLLInqConclusionSchema.getClmNo());
				ExeSQL tExeSQL = new ExeSQL();
				String tMasFlagS = tExeSQL.getOneValue(sqlbv);
				if (tMasFlagS.equals("") || tMasFlagS == null) {
					// @@错误处理
					CError tError = new CError();
					tError.moduleName = "LLInqConclusionBL";
					tError.functionName = "submitData";
					tError.errorMessage = "查询阳性标志时出错！";
					this.mErrors.addOneError(tError);
					return false;
				}
				int tMasFlagI = Integer.parseInt(tMasFlagS);
				logger.debug("=========已完成的调查中有阳性标志的案件数为==========="
						+ tMasFlagI);
				if (tMasFlagI > 0
						|| mLLInqConclusionSchema.getMasFlag().equals("1")) {
					ttLLInqConclusionSchema.setMasFlag("1");
				}
				map.put(ttLLInqConclusionSchema, "DELETE&INSERT");
			} else {
				// @@错误处理
				CError tError = new CError();
				tError.moduleName = "LLInqConclusionBL";
				tError.functionName = "prepareOutputData";
				tError.errorMessage = "自动汇总赔案调查结论记录时出错！";
				this.mErrors.addOneError(tError);
				return false;
			}

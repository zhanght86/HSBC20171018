 ...
 * @version 1.0
 */
public class RINewDistillCL implements RIDistill {
	String mEventType;
	...
	private String getUnSQL() {
		...
		strBuffer.append(" g.Years as Years, ");
		// 保单年度
		strBuffer
				.append(" ceil(months_between(h.endcasedate,g.cvalidate-1)/12) as InsuredYear, ");
		strBuffer.append(" a.SaleChnl as SaleChnl, ");
		strBuffer.append(" g.CValiDate as CValiDate, ");
		strBuffer.append(" g.EndDate as EndDate, ");
		...
		strBuffer.append(" floor(months_between(g.cvalidate, a.insuredbirthday) / 12) as InsuredAge, ");
		...
		// strBuffer.append(" Risk_Amnt(g.PayYears,ceil(months_between(h.endcasedate,g.cvalidate-1)/12),a.amnt,a.InsuredAppAge,a.years,decode(a.InsuredSex,'0','0','1'),a.prem,a.riskcode,g.PayEndYear,g.PayEndYearFlag,g.InsuYear,g.InsuYearFlag,g.PayIntv) as RiskAmnt, ");
		// 追朔出险时点的保额
		...
		strBuffer.append(" g.currency as Currency, ");
		strBuffer.append(" g.PayIntv as PayIntv, ");
		strBuffer.append(" g.PayYears as PayYears, ");
		strBuffer.append(" g.PayEndYearFlag as PayEndYearFlag, ");
		strBuffer.append(" g.PayEndYear as PayEndYear, ");
		strBuffer.append(" '' as GetYearFlag, ");
		strBuffer.append(" '' as GetYear, ");
		strBuffer.append(" g.InsuYearFlag as InsuYearFlag, ");
		strBuffer.append(" g.InsuYear as InsuYear, ");
		...
		strBuffer
				.append(" (select x.Exchangerate from RIExchangeRate x where x.Currency = g.currency and x.Startdate  =(select max(d.Startdate) from RIExchangeRate d where d.Currency = g.currency)and x.Enddate is null)  as ChangeRate, ");
		...
		System.out.println("理赔非临分SQL公共部分sql: " + strBuffer.toString());
	...
	private String getSpecSQL() {
		...
		strBuffer.append(" from lcpol a,lcduty g,( ");
		System.out.println("理赔临分提数公共部分sql: " + strBuffer.toString());
		return strBuffer.toString();
	}

	// 理赔回退非临分SQL公共部分
	private String getUnHtSQL() {
		StringBuffer strBuffer = new StringBuffer();
		strBuffer
				.append("select BatchNo,EventNo,EventType,RecordType,NodeState,DataFlag,UnionKey,GrpContNo,ProposalGrpContNo,GrpPolNo,GrpProposalNo,ContNo,PolNo,ProposalNo,ContPlanCode,PlanType,RiskPeriod,RiskType,RiskCode,DutyCode,Years,InsuredYear,SaleChnl,CValiDate,EndDate,InsuredNo,InsuredName,InsuredSex,InsuredBirthday,InsuredAge,(InsuredAge+InsuredYear-1) as CurrentAge,IDType,IDNo,OccupationType,HealthTime1,HealthTime2,OccupationCode,SuppRiskScore,SmokeFlag,AdditionalRate,ExtType,ExtPrem,BegAccountValue,EndUnit,EndPrice,EndAccountValue,FeeType,FeeMoney,NonCashFlag,StandPrem,Prem,Amnt,Mult,");
		// 准备金
		...
		strBuffer.append(" '01' as RecordType, ");
		strBuffer.append(" '01' as DataFlag, ");// 01:个人险种 02：个人险种责任
		strBuffer.append(" a.PolNo || ','||h.clmno||','||h.accdate||','||'"
				+ mAccumulateDefNo + "' as UnionKey, ");
		strBuffer.append(" a.GrpContNo as GrpContNo, ");
		strBuffer.append(" '00000000000000000000' as ProposalGrpContNo, ");
		strBuffer.append(" a.GrpPolNo as GrpPolNo, ");
		...
		// 保单年度
		strBuffer
				.append(" ceil(months_between(h.accdate,g.cvalidate-1)/12) as InsuredYear, ");
		strBuffer.append(" a.SaleChnl as SaleChnl, ");
		strBuffer.append(" g.CValiDate as CValiDate, ");
		strBuffer.append(" g.EndDate as EndDate, ");
		strBuffer.append(" a.InsuredNo as InsuredNo, ");
		...
		strBuffer.append(" '' as ClmFeeOperationType, ");
		strBuffer.append(" '' as ClmFeeFinaType, ");
		strBuffer.append(" '' as StandGetMoney, ");
		strBuffer.append(" g.getrate as GetRate, ");
		...
		strBuffer.append(" '01' as NodeState, ");
		strBuffer.append(" '' as ReinsreFlag, ");
		strBuffer.append(" h.accdate as GetDate, ");
		strBuffer.append(" rigetplancode(g.contno,g.polno,g.dutycode) as StandbyString1, ");
		strBuffer.append(" '' as standbystring2, ");
		strBuffer.append(" null as StandbyString3, ");
		strBuffer.append(" 0 as StandbyDouble1, ");
		...
		System.out.println("理赔回退非临分SQL公共部分sql: " + strBuffer.toString());
		return strBuffer.toString();
	}

	// 理赔回退临分提数公共部分
	private String getSpecHtSQL() {
		StringBuffer strBuffer = new StringBuffer();
		strBuffer
				.append("select BatchNo,EventNo,EventType,RecordType,NodeState,DataFlag,UnionKey,GrpContNo,ProposalGrpContNo,GrpPolNo,GrpProposalNo,ContNo,PolNo,ProposalNo,ContPlanCode,PlanType,RiskPeriod,RiskType,RiskCode,DutyCode,Years,InsuredYear,SaleChnl,CValiDate,EndDate,InsuredNo,InsuredName,InsuredSex,InsuredBirthday,InsuredAge,(InsuredAge+InsuredYear-1) as CurrentAge,IDType,IDNo,OccupationType,HealthTime1,HealthTime2,OccupationCode,SuppRiskScore,SmokeFlag,AdditionalRate,ExtType,ExtPrem,BegAccountValue,EndUnit,EndPrice,EndAccountValue,FeeType,FeeMoney,NonCashFlag,StandPrem,Prem,Amnt,Mult,");
		// 准备金
		...
		strBuffer.append(" '' as StandbyDate5 ");
		strBuffer.append(" from lcduty g,lcpol a,( ");
		System.out.println("理赔回退临分提数公共部分sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	private boolean dealData() {
		try {
			...
			// 理赔回退
			tSql = getUnHtSQL() + getDistillSQL2();
			if (!rsWrapper.prepareData(tRIPolRecordSet, tSql)) {
				mErrors.copyAllErrors(rsWrapper.mErrors);
				System.out.println(rsWrapper.mErrors.getFirstError());
				return false;
			...
			// 理赔回退临时分保提数
			tSql = getSpecHtSQL() + getDistillSQL3();
			if (!rsWrapper.prepareData(tRIPolRecordSet, tSql)) {
				mErrors.copyAllErrors(rsWrapper.mErrors);
				System.out.println(rsWrapper.mErrors.getFirstError());
				return false;
			...
			tSql = getUnHtSQL() + getDistillSQL6();
			if (!rsWrapper.prepareData(tRIPolRecordSet, tSql)) {
				mErrors.copyAllErrors(rsWrapper.mErrors);
				System.out.println(rsWrapper.mErrors.getFirstError());
				return false;
			...
			tSql = getSpecHtSQL() + getDistillSQL7();
			if (!rsWrapper.prepareData(tRIPolRecordSet, tSql)) {
				mErrors.copyAllErrors(rsWrapper.mErrors);
				System.out.println(rsWrapper.mErrors.getFirstError());
				return false;
			...
			tSql = getSpecSQL() + getDistillSQL9();
			if (!rsWrapper.prepareData(tRIPolRecordSet, tSql)) {
				mErrors.copyAllErrors(rsWrapper.mErrors);
				System.out.println(rsWrapper.mErrors.getFirstError());
				return false;
			...
			tSql = getUnHtSQL() + getDistillSQL10();
			if (!rsWrapper.prepareData(tRIPolRecordSet, tSql)) {
				mErrors.copyAllErrors(rsWrapper.mErrors);
				System.out.println(rsWrapper.mErrors.getFirstError());
				return false;
			...
			tSql = getSpecHtSQL() + getDistillSQL11();
			if (!rsWrapper.prepareData(tRIPolRecordSet, tSql)) {
				mErrors.copyAllErrors(rsWrapper.mErrors);
				System.out.println(rsWrapper.mErrors.getFirstError());
				return false;
	...
	private String getDistillSQL() {
		...
		if (mAccumulateDefNo.equals("L01010010050")) {
			...
			strBuffer.append(" and g.currency='01'");
		}
	...
	private String getDistillSQL1() {
		...
		strBuffer.append(") ");
		System.out.println("理赔提数临分sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	private String getDistillSQL2() {
		...
		strBuffer
				.append(" and not exists (select 'X' from ridutystate where proposalno=a.polno and state in ('00','02','03','04')) ");
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecord m where m.unionkey=(a.PolNo || ','||h.clmno||','||h.accdate||','||'"
						+ mAccumulateDefNo + "')) ");
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecordBake n where n.unionkey=(a.PolNo || ','||h.clmno||','||h.accdate||','||'"
						+ mAccumulateDefNo + "')) ");
		strBuffer
				.append(" and a.riskcode not in('IBC02','IBC01','IRD04','IRA03')");
		if (mAccumulateDefNo.equals("L01010010050")) {
		...
		strBuffer.append(") ");
		System.out.println("理赔回退提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	private String getDistillSQL3() {
		...
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecordBake n where n.unionkey=(a.PolNo || ','||h.clmno||','||h.accdate||','||'"
						+ mAccumulateDefNo + "')) ");
		strBuffer.append(") ");
		System.out.println("理赔回退临分提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	private String getDistillSQL4() {
		...
		strBuffer
				.append(" and exists(select 'X' from RIAccumulateGetDuty r where a.riskcode = r.associatedcode and g.Dutycode like r.Getdutycode||'%' and r.accumulatedefno='"
						+ mAccumulateDefNo
	...
	// 理赔回退未领取
	private String getDistillSQL10() {
		StringBuffer strBuffer = new StringBuffer();
	...
	// 理赔回退临分未领取
	private String getDistillSQL11() {
		StringBuffer strBuffer = new StringBuffer();
	...
	public static void main(String[] args) {
		...
		mRIWFLogSchema.setStartDate("2010-09-12");
		mRIWFLogSchema.setEndDate("2011-11-14");
		mRIWFLogSchema.setTaskCode("L01010010040");
		cInputData.add(mRIWFLogSchema);
		RINewDistillCL tRIDistillCL = new RINewDistillCL();

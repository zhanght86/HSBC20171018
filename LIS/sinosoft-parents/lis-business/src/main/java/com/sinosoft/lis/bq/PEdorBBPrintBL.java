 ...
 * 
 * <p>
 * Description:生成保全项目客户基本信息变更的批单数据
 * </p>
 * 
 * <p>
...
public class PEdorBBPrintBL implements EdorPrint {
	@SuppressWarnings("unused")
	private static Logger logger = Logger.getLogger(PEdorBBPrintBL.class);
	...
	private boolean getAllSchema() {
		...
		for (int k = 1; k <= mLPEdorItemSet.size(); k++) {
			...
			if ("1".equals(tAppntIsInsuredFlag)
				...
				mTextTag.add("BBLPInsured.CustomerNo" + k, tLPInsuredSchema
						.getInsuredNo());
				if (!StrTool.compareString(tLPInsuredSchema.getName(),
						tLCInsuredSchema.getName())
				/* && !StrTool.cTrim(tLPInsuredSchema.getName()).equals("") */) {
					mTextTag.add("BBLPInsured.NativeNameNew" + k,
							tLPInsuredSchema.getName());
					mTextTag.add("BBLPInsured.NativeName" + k, tLCInsuredSchema
							.getName());
				 ...
				 */) {
					mTextTag.add("BBLPInsured.NativePlaceNew" + k, BqNameFun
							.getCodeName(tLPInsuredSchema.getNativePlace(),
									"NativePlace"));
					mTextTag.add("BBLPInsured.NativePlace" + k, BqNameFun
							.getCodeName(tLCInsuredSchema.getNativePlace(),
									"NativePlace"));
				...
				/* && !StrTool.cTrim(tLPInsuredSchema.getMarriage()).equals("") */) {
					mTextTag.add("BBLPInsured.MarriageNew" + k, BqNameFun
							.getCodeName(tLPInsuredSchema.getMarriage(),
									"Marriage"));
					mTextTag.add("BBLPInsured.Marriage" + k, BqNameFun
							.getCodeName(tLCInsuredSchema.getMarriage(),
									"Marriage"));
				 ...
				 */) {
					mTextTag.add("BBLPInsured.RgtAddressNew" + k,
							tLPInsuredSchema.getRgtAddress());
					mTextTag.add("BBLPInsured.RgtAddress" + k, tLCInsuredSchema
							.getRgtAddress());
					xmlexport.addDisplayControl("displayBBRgtAddress" + k);
				}
				if (!StrTool.compareString(
						tLPInsuredSchema.getSocialInsuFlag(), tLCInsuredSchema
								.getSocialInsuFlag())
				/*
				 * &&
				 ...
				 */) {
					String tPFlag = "";
					if ("0".equals(tLPInsuredSchema.getSocialInsuFlag())
							|| "".equals(tLPInsuredSchema.getSocialInsuFlag())) {
						tPFlag = "无";
					} else {
						tPFlag = "有";
					}
					String tCFlag = "";
					if ("0".equals(tLCInsuredSchema.getSocialInsuFlag())
							|| "".equals(tLCInsuredSchema.getSocialInsuFlag())) {
						tCFlag = "无";
					} else {
						tCFlag = "有";
					}
			...
			if ("0".equals(tAppntIsInsuredFlag)
				...
				mTextTag.add("BBLPInsured.CustomerNo" + k, tLPAppntSchema
						.getAppntNo());
				...
				/* && !StrTool.cTrim(tLPAppntSchema.getAppntName()).equals("") */) {
					mTextTag.add("BBLPInsured.NativeNameNew" + k,
							tLPAppntSchema.getAppntName());
					mTextTag.add("BBLPInsured.NativeName" + k, tLCAppntSchema
							.getAppntName());
					xmlexport.addDisplayControl("displayBBName" + k);
				}
				...
				/* && !StrTool.cTrim(tLPAppntSchema.getNativePlace()).equals("") */) {
					mTextTag.add("BBLPInsured.NativePlaceNew" + k, BqNameFun
							.getCodeName(tLPAppntSchema.getNativePlace(),
									"NativePlace"));
					mTextTag.add("BBLPInsured.NativePlace" + k, BqNameFun
							.getCodeName(tLCAppntSchema.getNativePlace(),
									"NativePlace"));
					xmlexport.addDisplayControl("displayBBNativePlace" + k);
				}
				...
				/* && !StrTool.cTrim(tLPAppntSchema.getMarriage()).equals("") */) {
					mTextTag.add("BBLPInsured.MarriageNew" + k, BqNameFun
							.getCodeName(tLPAppntSchema.getMarriage(),
									"Marriage"));
					mTextTag.add("BBLPInsured.Marriage" + k, BqNameFun
							.getCodeName(tLCAppntSchema.getMarriage(),
									"Marriage"));
					xmlexport.addDisplayControl("displayBBMarriage" + k);
				}
				...
				/* && !StrTool.cTrim(tLPAppntSchema.getRgtAddress()).equals("") */) {
					mTextTag.add("BBLPInsured.RgtAddressNew" + k,
							tLPAppntSchema.getRgtAddress());
					mTextTag.add("BBLPInsured.RgtAddress" + k, tLCAppntSchema
							.getRgtAddress());
					xmlexport.addDisplayControl("displayBBRgtAddress" + k);
				}
			}

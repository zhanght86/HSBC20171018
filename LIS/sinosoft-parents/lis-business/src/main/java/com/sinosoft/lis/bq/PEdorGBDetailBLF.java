 ...
 * @version 1.0
 */
public class PEdorGBDetailBLF implements EdorDetail {
private static Logger logger = Logger.getLogger(PEdorGBDetailBLF.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (!tSubmit.submitData(mResult, "")) {
			...
			tError.moduleName = "PEdorGBDetailBLF";
	...
	private boolean getInputData() {
		try {
			...
			if (mLPEdorItemSchema == null || mGlobalInput == null
					|| mTransferData == null || mLPDutySchema == null
					|| mLPGetSchema == null || mLPPolSchema == null) {
				return false;
			}
		} catch (Exception e) {
	...
	/**
	 * 准备需要保存的数据 调用业务逻辑处理类PEdorGBDetailBL进行处理
	 */
	private boolean dealData() {
		...
		if (!tPEdorGBDetailBL.submitData(mInputData, mOperate)) {
			this.mErrors.copyAllErrors(tPEdorGBDetailBL.mErrors);
			CError tError = new CError();
			tError.moduleName = "PEdorGBDetailBLF";
		...
		mResult.clear();
		VData tResult = tPEdorGBDetailBL.getResult();
	...
	public static void main(String[] args) {
		PEdorGBDetailBLF pedorgbdetailblf = new PEdorGBDetailBLF();
	}
}

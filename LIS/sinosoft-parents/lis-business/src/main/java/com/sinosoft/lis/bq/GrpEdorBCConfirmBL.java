...
public class GrpEdorBCConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(GrpEdorBCConfirmBL.class);
	...
	private boolean dealData() {
		...
		if (tLPEdorItemSet == null || tLPEdorItemSet.size() <= 0) {
			...
			for (int i = 1; i <= tLPEdorItemSet.size(); i++) {
				...
				if (!tPEdorBCConfirmBL.submitData(tVData, rOperation)) {
					mErrors.copyAllErrors(tPEdorBCConfirmBL.mErrors);
					...
					tTempVData = tPEdorBCConfirmBL.getResult();

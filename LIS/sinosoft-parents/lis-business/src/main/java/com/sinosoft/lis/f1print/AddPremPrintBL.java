 ...
 * @version 1.0
 */
public class AddPremPrintBL implements PrintService {
private static Logger logger = Logger.getLogger(AddPremPrintBL.class);
	...
	private boolean getPrintData() {
		...
		xmlexport.createDocument("AddCharge.vts", "printer"); // 最好紧接着就初始化xml文档
		...
		PolSSRS = tempExeSQL.execSQL(sqlbv11);
		if (PolSSRS != null) {
		...
		PeriodSSRS = tempExeSQL.execSQL(sqlbv12);

		if (temp7SSRS.getMaxRow() > 0 || temp8SSRS.getMaxRow() > 0
				|| temp2SSRS.getMaxRow() > 0 || temp5SSRS.getMaxRow() > 0
				|| tempASSRS.getMaxRow() > 0) {
			if (temp7SSRS.getMaxRow() > 0) {
				logger.debug("temp7SSRS");
			} else if (temp8SSRS.getMaxRow() > 0) {
				logger.debug("temp8SSRS");
			} else if (temp2SSRS.getMaxRow() > 0) {
				logger.debug("temp2SSRS");
			} else if (temp5SSRS.getMaxRow() > 0) {
				logger.debug("temp5SSRS");
			} else if (tempASSRS.getMaxRow() > 0) {
				logger.debug("tempASSRS");
			}
	...
	public static void main(String[] args) {
		AddPremPrintBL tAddPremPrintUI = new AddPremPrintBL();

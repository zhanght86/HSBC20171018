 ...
 * @version 1.0
 */
public class PEdorPreAutoUWBL {
private static Logger logger = Logger.getLogger(PEdorPreAutoUWBL.class);
	...
	private boolean dealData() {
		...
		for (int i = 1; i <= n; i++) {
			...
			if (!checkData()) {
				mErrors.addOneError(new CError("申请批单"
						+ mLPEdorMainSchema.getEdorNo() + "自动初次核保失败！"));
				continue;
			}
			...
			EdorPreAutoUWBL tEdorPreAutoUWBL = new EdorPreAutoUWBL();
			if (!tEdorPreAutoUWBL.submitData(pInputData, "")) {
				mErrors.copyAllErrors(tEdorPreAutoUWBL.mErrors);
			...
			MMap map = new MMap();
			map = (MMap) tEdorPreAutoUWBL.getResult().getObjectByObjectName(
					"MMap", 0);
			TransferData tTransferData = new TransferData();
			tTransferData = (TransferData) tEdorPreAutoUWBL.getResult()
					.getObjectByObjectName("TransferData", 0);
			String tPreUWState = (String) tTransferData
					.getValueByName("PreUWFlag");
			String tPreUWGrade = (String) tTransferData
					.getValueByName("PreUWGrade");
			if (!"9".equals(tPreUWState)) {
				mPreUWState = tPreUWState;
			}
			if (tPreUWGrade.compareTo(mPreUWGrade) > 0) {
				mPreUWGrade = tPreUWGrade;
			}

			mMap.add(map);
			//若为客户层保全，存在多条lpedormain记录时，一旦其中一单初核不过则直接跳出
			if (!"9".equals(tPreUWState)) 
			{
				break;
			}
		...
		TransferData tTransferData = new TransferData();
		tTransferData.setNameAndValue("PreUWFlag", mPreUWState);
		tTransferData.setNameAndValue("PreUWGrade", mPreUWGrade);

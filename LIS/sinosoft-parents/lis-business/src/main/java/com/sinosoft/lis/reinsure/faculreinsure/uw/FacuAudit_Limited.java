...
public class FacuAudit_Limited implements FacuAudit {

	protected String mContNo;
	protected String mInsuredNo;
	protected String mFactorID;
	protected String mFactorValue;
	protected String mCurrency;
	protected String mAccumulateDefNO;
	protected String mAccumulateMode;
	protected String mPolNo;
	protected String mRiskCode;
	protected boolean uwFlag = true;
	...
	protected boolean dealData() {
		...
		if (!mAccumulateMode.equals("01")) {
			...
			System.out.println("cccc: " + mFactorID);
			for (int i = 0; i < amntSSRS.getMaxRow(); i++) {
			...
			contBuffer
					.append(" select nvl(risk_amnt2(ceil(months_between(date'"
							...
							+ mAccumulateDefNO
							+ "'),0),b.Currency from lcpol a,lcduty b  ");
			contBuffer.append(" where a.Polno = b.Polno and a.Insuredno='");
			contBuffer.append(mInsuredNo);
			contBuffer
					.append("' and exists (select * from Riaccumulategetduty x where x.Accumulatedefno = '"
							+ mAccumulateDefNO
							+ "' and x.Associatedcode = a.Riskcode and x.Getdutycode = b.Dutycode) ");
			contBuffer
					.append(" and appflag ='0' and not exists(select 'X' from lccontstate where  StateType = 'Available' and State = '0' and enddate is null and polno=a.polno ) and a.contno <>'"
							+ mContNo + "' ");
			...
			for (int i = 0; i < contSSRS.getMaxRow(); i++) {
				if (contSSRS.getAllData()[i][1].equals("13")) {
					accAmnt = accAmnt
							+ Double.parseDouble(contSSRS.getAllData()[i][0]);
				} else {
					for (int j = 1; j <= mRIExchangeRateSet.size(); j++) {
						if (mRIExchangeRateSet.get(j).getCurrency()
								.equals(contSSRS.getAllData()[i][1])) {
							accAmnt = accAmnt
									+ Double.parseDouble(contSSRS.getAllData()[i][0])
									* mRIExchangeRateSet.get(j)
											.getExchangeRate();
		...
		StringBuffer contBuffer = new StringBuffer();
		contBuffer
				.append(" select sum(nvl(risk_amnt2(ceil(months_between(date'"
						+ PubFun.getCurrentDate()
						+ "',a.cvalidate-1)/12),a.Amnt ,a.DutyCode ,a.PolNo,a.Currency,'"
						+ mAccumulateDefNO
						+ "'),0)),max(a.Currency) from lcduty a where a.Polno = '"
						+ mPolNo + "' group by a.polno");

		System.out.println("sql3: " + contBuffer.toString());
		contSSRS = tExeSQL.execSQL(contBuffer.toString());
		if (tExeSQL.mErrors.needDealError()) {
			buildError("InitInfo", "获取累计风险保额明细出错！");
			return false;
	...
	protected void buildError(String szFunc, String szErrMsg) {

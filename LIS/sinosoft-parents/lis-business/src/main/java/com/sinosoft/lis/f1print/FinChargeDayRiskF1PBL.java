...
public class FinChargeDayRiskF1PBL {
private static Logger logger = Logger.getLogger(FinChargeDayRiskF1PBL.class);
...
//全局变量
  ...
  private boolean getPrintData()
  {
    ...
    msql="select RiskCode,sum(PayMoney),sum(1) from LJTempFee where MakeDate>='"+"?date1?"+"' and MakeDate<='"+"?date2?"+"' and Operator='"+"?Operator?"+"' Group By RiskCode order by RiskCode";
    //判断操作员位数是否为8位
    sqlbv1.sql(msql);
    ...
    xmlexport.createDocument("FinChargeDayRisk.vts","printer");//最好紧接着就初始化xml文档

...
public class LCContSignBL extends CovBase {
private static Logger logger = Logger.getLogger(LCContSignBL.class);
	...
	public void run() {
		...
		try {
			...
			signSet = (LCContSet)mInputDataNew.getObjectByObjectName("LCContSet", 0);
			tBatchNo =(String)mInputDataNew.get(0);
			tG =(GlobalInput)mInputDataNew.getObjectByObjectName("GlobalInput", 0);
			if(signSet.size()>0){
				//tGrpContNo =signSet.get(1).getGrpContNo();
				LCContDB tLCContDB = new LCContDB();
				tLCContDB.setContNo(signSet.get(1).getContNo());
				if(!tLCContDB.getInfo()){
					CError.buildErr(this, "查询合同号："+signSet.get(1).getContNo()+"错误！");
				}
				else
				{
					tGrpContNo =tLCContDB.getPrtNo();
					signSet.clear();
					signSet.add(tLCContDB.getSchema());
			
				if (this.submitData(mInputDataNew, "")) {
					// 处理签单
					VData contData = new VData();
					contData.add(mGlobalInput);
					MMap tmpMap = (MMap) this.mVResult
		...
		//CError.buildErr(this,"test error");
	...
	private boolean signLCContPol(LCContSet inLCContSet) {
		...
		for (int i = 1; i <= inLCContSet.size(); i++) {
			...
			contMap.add(aMMap);
			FDate fDate = new FDate();		// 处理日期
			contMap.put("update lccont set paytodate='"+fDate.getString(tPayToDate)+"' where contno ='"+newContNo+"'", "UPDATE");

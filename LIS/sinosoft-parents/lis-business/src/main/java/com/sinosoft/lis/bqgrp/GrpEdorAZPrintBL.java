...
public class GrpEdorAZPrintBL implements EdorPrint {
	@SuppressWarnings("unused")
	private static Logger logger = Logger.getLogger(GrpEdorAZPrintBL.class);
	...
	public static final String VTS_NAME = "GrpEdorNameListAZ.vts"; // 模板名称

	public GrpEdorAZPrintBL() {
	}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "AZ数据提取失败!");
			return false;
		}
		// 打印人名清单数据处理
		if (!bulidBill()) {
			mErrors.addOneError("AZ人名清单数据打印失败!");
			return false;
		}
		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "AZ数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		xmlexport.addDisplayControl("displayAZ");
		...
		ListTable tAZListTable = new ListTable();
		tAZListTable.setName("AZ");
		...
		if (rSSRS == null || rSSRS.MaxRow < 1) {
			String[] strAZ = new String[4];
			for (int k = 1; k <= strAZ.length; k++) {
				strAZ[k - 1] = "--";
			}
			tAZListTable.add(strAZ);
		} else {
		...
		mTextTag.add("AZNUM", dNum);
		mTextTag.add("AZSUM", dSumMoney);
		...
		String[] b_strAZ = new String[2];
		xmlexport.addListTable(tAZListTable, b_strAZ);
	...
	private boolean bulidBill() {
		...
		tXmlExport.addDisplayControl("displayAZ");
		...
		strsql1 = "select a.name,"
				...
				+ "',"
				+ " nvl2(c.bankcode, (select bankname from ldbank where bankcode = c.bankcode),'--'), "
				+ " nvl(c.BankAccNo, '--'), "
				+ " nvl(c.AccName, '--') "
				+ " from lcinsured a, lppol b, ljsgetendorse d, lpbnf c "
				+ " where a.contno = b.contno and a.insuredno = b.insuredno and a.grpcontno = b.grpcontno and d.endorsementno = b.edorno and d.polno = b.polno"
				+ " and c.edorno = b.edorno and c.contno = b.contno and b.edorno = '"
				+ mLPGrpEdorItemSchema.getEdorNo() + "'";
		...
		texttag.add("AZSUM", mSumPrem);
		texttag.add("AZNUM", mNum);

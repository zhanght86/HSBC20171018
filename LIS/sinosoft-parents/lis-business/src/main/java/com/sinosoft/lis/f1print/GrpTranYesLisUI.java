...
public class GrpTranYesLisUI {
private static Logger logger = Logger.getLogger(GrpTranYesLisUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			GrpTranYesLisBL tGrpTranYesLisBL = new GrpTranYesLisBL();
			logger.debug("Start GrpTranYesLis UI Submit ...");
			if (!tGrpTranYesLisBL.submitData(vData, cOperate)) {
				if (tGrpTranYesLisBL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tGrpTranYesLisBL.mErrors);
					return false;
				} else {
					buildError("submitData",
							"GrpTranYesLisBL发生错误，但是没有提供详细的出错信息");
					return false;
				}
			} else {
				mResult = tGrpTranYesLisBL.getResult();
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
			CError cError = new CError();
			cError.moduleName = "GrpTranYesLisUI";

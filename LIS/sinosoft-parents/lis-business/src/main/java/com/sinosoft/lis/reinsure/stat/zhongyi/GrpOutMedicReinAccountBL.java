...
public class GrpOutMedicReinAccountBL
{
    ...
    private String seasonStr="";  /*业务帐单期---季度数*/
    /**前台传入的数据*/
    ...
    //业务处理相关变量
    /** 全局数据 */
    public GrpOutMedicReinAccountBL()
    {
    }
    /**
    * 提交数据处理方法
   ...
   public static void main(String[] args) {
       ...
       GrpOutMedicReinAccountBL t = new GrpOutMedicReinAccountBL();
       t.getPrintData();
   }
    ...
    private boolean getPrintData() {
      ...
      try {
          ...
          String sql = "select '分保费' ,nvl(sum(b.premchang),0) ,null from ripolrecordbake a, reinsurerecordtrace b where a.eventno=b.eventno and a.SerialNo=b.SerialNo and a.eventtype in('01','03','04') and b.ComPanyNo = '"+mRIcomCode+"'  and a.dutycode in (select distinct associatedcode from accumulaterdcode where accumulatedefno in (select Accumulatedefno from riprecept where ricontno = '"+mContNo+"')) and a.serialno in ("+thisSerialNo+") "+sql1
                       +" union all select '分保手续费',nvl(sum(b.CommChang),0),nvl(sum(returncomm),0) from ripolrecordbake a, reinsurerecordtrace b where a.eventno=b.eventno and a.SerialNo=b.SerialNo and a.eventtype in('01','03','04') and b.ComPanyNo = '"+mRIcomCode+"'  and a.dutycode in (select distinct associatedcode from accumulaterdcode where accumulatedefno in (select Accumulatedefno from riprecept where ricontno = '"+mContNo+"')) and a.serialno in ("+thisSerialNo+")"+sql1
                       +" union all select '赔款' , null, nvl(sum(b.returnpay),0) from ripolrecordbake a, reinsurerecordtrace b where a.eventno=b.eventno and a.SerialNo=b.SerialNo and a.eventtype in('01','03','04') and b.ComPanyNo = '"+mRIcomCode+"' and a.dutycode in (select distinct associatedcode from accumulaterdcode where accumulatedefno in (select Accumulatedefno from riprecept where ricontno = '"+mContNo+"')) and a.serialno in ("+thisSerialNo+")"+sql1
                       +" union all select '其它',null,null  from  ldsysvar where sysvar='onerow'";
          ...
          if(temp[i][1]!=null && !temp[i][1].equals(""))
                sumBorrow = sumBorrow + Double.parseDouble(temp[i][1]);/**合计除了 分保余额 一行 种"借"的值*/
          if(temp[i][2]!=null && !temp[i][2].equals(""))
                sumLend = sumLend + Double.parseDouble(temp[i][2]);  /**合计除了 分保余额 一行 种"贷"的值*/
         }
     ...
     if(balance>=0){
     ...
     else{balance=sumLend-sumBorrow;
          texttag.add("negative",balance);
          sumBorrow=sumBorrow+balance;//如果 在 "贷"处的 分保余额有值 这 加到 合计贷变量 sumBorrow中
      }
      System.out.println("合计借的值为"+sumBorrow);
      System.out.println("合计贷的值为"+sumLend);
      ...
      texttag.add("AccountSession",seasonStr);
      ...
      xmlexport.createDocument("GrpOutMedicReinAccount.vts", "printer");
      if (texttag.size() > 0) {

...
public class Zhixiao6BL {
private static Logger logger = Logger.getLogger(Zhixiao6BL.class);
				...
				for (int k = 0; k < YingSSRS.getMaxRow(); k++) {
					...
					if (strStatType != null && strStatType.equals("1")) {
						tSql = " Select a.agentcode,"
								...
								// + " where c.agentgroup =
								// trim(a.agentgroup)),"
								+ " (select cnm.BranchCode from laagent cnm where cnm.agentcode = trim(a.agentcode)),"
								+ " a.contno,"
								+ " a.appntname,"
								+ " (select riskcode"
								+ " from lcpol"
								+ " where lcpol.contno = a.contno"
								+ " and lcpol.polno = lcpol.mainpolno) as riskcode,"
								+ " cc.md as md,"
								+ " a.customgetpoldate,"
								+ " d.makedate,"
								+ " d.operator"
								+ " From lccont a, labranchgroup b, lzsyscertify d,"
								+ " (select contno, max(makedate) as md"
								+ " from ldcontinvoicemap f"
								+ " Where 1 = 1"
								// + " And a.contno = d.certifyno"
								+ " and f.opertype = '1'"
								...
								+ "?strEndDate?"
								+ "' group by contno) cc"
								+ " Where a.contno = cc.contno"
								+ " and b.agentgroup = trim(a.agentgroup)"
								+ " and a.contno = rpad(d.certifyno,lislen('lccont','contno'),' ')"
								+ " And trim(d.certifycode) = '9995'"
								+ " and a.managecom = '"
								+ "?managecom?"
								+ "'"
								+ " and exists(select 'x' from lcpol oo where oo.contno = a.contno and oo.riskcode in (select riskcode from lmriskapp where riskprop = 'T'))"
								// + " and a.SaleChnl = '" + strSaleChnl + "'"
								+ " order by d.makedate,d.operator,d.maketime";
						// + " And d.certifycode = '9995'"
						// + " and a.managecom = '"
						// + YingSSRS.GetText(k + 1, 1) + "'"
						// + " and a.SaleChnl = '" + strSaleChnl + "'"
						// + " and e.rowid = (select min(rowid)\n" +
						// " from ldcontinvoicemap g\n" +
						// " where g.contno = a.contno\n" +
						// " and g.opertype = '1'\n" +
						// " and g.makedate = (select max(makedate)\n" +
						// " from ldcontinvoicemap f\n" +
						// " where f.contno = a.contno\n" +
						// " and f.opertype = '1'))"
						// + " and e.makedate between '"
						...
						tSql = " Select a.agentcode,"
								...
								+ " (select c.BranchCode"
								+ " from laagent c"
								+ " where c.agentcode = trim(a.agentcode)),"
								+ " a.contno,"
								+ " a.appntname,"
								+ " (select riskcode"
								+ " from lcpol"
								+ " where lcpol.contno = a.contno"
								+ " and lcpol.polno = lcpol.mainpolno) as riskcode,"
								+ "(select e.makedate from ldcontinvoicemap e where "
								+ "e.rowid = (select min(rowid)\n"
								+ "                    from ldcontinvoicemap g\n"
								+ "                   where g.contno = a.contno\n"
								+ "                     and g.opertype = '1'\n"
								+ "                     and g.makedate = (select max(makedate)\n"
	...
	public static void main(String[] args) {
		...
		Zhixiao6UI tZhixiao6UI = new Zhixiao6UI();
		tZhixiao6UI.submitData(tVData, "PRINT");
	}
}

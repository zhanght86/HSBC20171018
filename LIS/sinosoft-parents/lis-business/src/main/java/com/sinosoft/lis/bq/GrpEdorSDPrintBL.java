 ...
 * @version 6.0
 */
public class GrpEdorSDPrintBL implements EdorPrint {
	private static Logger logger = Logger.getLogger(GrpEdorSDPrintBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 数据传输
		if (!getInputData()) {
			mErrors.addOneError("SD数据传输失败!");
			return false;
		}

		// 数据校验
		if (!checkData()) {
			mErrors.addOneError("SD传入数据无效!");
			return false;
		}

		// 打印数据处理
		if (!dealData()) {
			mErrors.addOneError("SD数据打印失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			mErrors.addOneError("SD准备数据失败!");
			return false;
		}
	...
	private boolean dealData() {
		...
		xmlexport.addDisplayControl("displaySD");
		...
		mTextTag.add("SD.EndDate", tssrs.GetText(1, 1)); // 中止日期
		mTextTag.add("SD.SumPeople", tssrs.GetText(1, 2)); // 总人数

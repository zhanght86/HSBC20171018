 ...
 * @version 6.0
 */
public class ApplyPlanBL {
	private static Logger logger = Logger.getLogger(ApplyPlanBL.class);
	...
	private boolean dealData() {
		if ("0000002802".equals(mOperate)) {
			mLDPlanSchema.setState(PlanFunc.PLAN_STATE_AUDITING);// 审核中
		} else {
			mLDPlanSchema.setState(PlanFunc.PLAN_STATE_APPROVE);// 审批中
		}
		...
		mLWMissionSchema.setLastOperator(mOperator);
		mLWMissionSchema.setDefaultOperator(mOperator);
		mLWMissionSchema.setInDate(PubFun.getCurrentDate());
		mLWMissionSchema.setInTime(PubFun.getCurrentTime());
		mLWMissionSchema.setModifyDate(mLWMissionSchema.getInDate());
		mLWMissionSchema.setModifyTime(mLWMissionSchema.getInTime());
		...
		tLWLockSchema.setProcessID(mLWMissionSchema.getProcessID());
		...
		String sql1 = "select * from lwmission where activityid='" + "?mActivityID?"
				+ "' and missionid='" + "?mMissionID?" + "' and submissionid='"
				+ "?mSubMissionID?" + "' and DefaultOperator is not null";
		SQLwithBindVariables sqlbv1 = new SQLwithBindVariables();
		sqlbv1.sql(sql1);
		sqlbv1.put("mActivityID",mActivityID);
		sqlbv1.put("mMissionID",mMissionID);
		sqlbv1.put("mSubMissionID",mSubMissionID);
		map.put(sqlbv1, "SELECT");
		map.put(mLWMissionSchema, "UPDATE");
		map.put(mLDPlanSchema, "UPDATE");
		return true;
	}
	...
	private boolean checkData() {
		...
		if (!tLWMissionDB.getInfo()) {
			...
			CError tError = new CError();
			tError.moduleName = "ApplyPlanBL";
			tError.functionName = "dealData";
			tError.errorMessage = "工作流任务查询失败!";
			this.mErrors.addOneError(tError);
			return false;
		}
		this.mLWMissionSchema = tLWMissionDB.getSchema();

		LDPlanDB tLDPlanDB = new LDPlanDB();
		LDPlanSet tLDPlanSet = new LDPlanSet();
		String sql2 = "select * from ldplan "
				+ "where contplancode='" + "?contplancode?"
				+ "'";
		SQLwithBindVariables sqlbv2 = new SQLwithBindVariables();
		sqlbv2.sql(sql2);
		sqlbv2.put("contplancode",mLWMissionSchema.getMissionProp1());
		tLDPlanSet = tLDPlanDB.executeQuery(sqlbv2);
		if (tLDPlanSet == null || tLDPlanSet.size() <= 0) {
			this.mErrors.copyAllErrors(tLWMissionDB.mErrors);
			CError tError = new CError();
			tError.moduleName = "ApplyPlanBL";
			tError.functionName = "dealData";
			tError.errorMessage = "产品组合查询失败!";
			this.mErrors.addOneError(tError);
			return false;
		}
		this.mLDPlanSchema = tLDPlanSet.get(1);
		return true;
	}
	...
	public static void main(String[] args) {
		ApplyPlanBL applyplanbl = new ApplyPlanBL();
	}
}

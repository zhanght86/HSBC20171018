...
public class CBgrqfqdUI {
private static Logger logger = Logger.getLogger(CBgrqfqdUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			logger.debug("Start CBgrqfqd UI Submit ...");
			if (!tCBgrqfqdBL.submitData(vData, cOperate)) {
				if (tCBgrqfqdBL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tCBgrqfqdBL.mErrors);
					return false;
				} else {
					buildError("submitData", "CBgrqfqdBL发生错误，但是没有提供详细的出错信息");
					return false;
				}
			} else {
				mResult = tCBgrqfqdBL.getResult();
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void main(String[] args) {
		...
		CBgrqfqdUI tCBgrqfqdUI = new CBgrqfqdUI();
		tCBgrqfqdUI.submitData(tVData, "PRINT");
	}
}

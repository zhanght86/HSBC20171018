...
//							tcell.setEncoding(HSSFCell.ENCODING_UTF_16);
					...
					if (currCell.height > 1 || currCell.width > 1)
						sheet.addMergedRegion(new Region(currCell.row,
								(short) currCell.col, currCell.row
										+ currCell.height - 1,
	...
	public static String[][] exeSQL(String sql, String companyCode) {
		...
		System.out.println("RIExportExcel.exeSQL() : " + sql.trim());
		Connection conn = null;
		conn = DBConnPool.getConnection();
		if (conn == null) {
			System.out.println("数据库连接失败！");
		...
		try {
			...
			if (dataType == Types.NUMERIC) {
				if (dataScale == 0) {
					...
					System.out.println("BigDecimal: The Numeric is = "
							+ strValue);
				}
			}
			if (strValue == null)
	...
	public static void main(String[] args) {
		...
		try {
			...
			// RIExportExcel.Cell tCell = new RIExportExcel.Cell();
			// tCell.row=0;
			// tCell.col=0;
			// tCell.bBorder=true;
			// tCell.height=1;
			...
			// RIExportExcel.ListBlock tLB = new RIExportExcel.ListBlock("001");
			...
			RIExportExcel.ListBlock tLB1 = new RIExportExcel.ListBlock("001");
			// tLB1.colName=new

 ...
 * @version 1.0
 */
public class GrpEdorGMConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(GrpEdorGMConfirmBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("after GrpEdorGMConfirmBL...");
	...
	private boolean dealData() {
		...
		for (int i = 1; i <= tLPEdorItemSet.size(); i++) {
			...
			PEdorGMConfirmBL tPEdorGMConfirmBL = new PEdorGMConfirmBL();
			if (!tPEdorGMConfirmBL.submitData(cVData, "")) {
				mErrors.copyAllErrors(tPEdorGMConfirmBL.mErrors);
				mErrors.addOneError("个人被保人信息变更保全生效失败!被保人号:"
						+ tLPEdorItemSet.get(i).getInsuredNo());
				return false;
			}
			MMap map = (MMap) tPEdorGMConfirmBL.getResult()

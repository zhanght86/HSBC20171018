...
public class WorkAmountPrintUI {
private static Logger logger = Logger.getLogger(WorkAmountPrintUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			WorkAmountPrintBL tWorkAmountPrintBL = new WorkAmountPrintBL();
			logger.debug("Start PNoticeBill UI Submit ...");
			if (!tWorkAmountPrintBL.submitData(vData, cOperate)) {
				if (tWorkAmountPrintBL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tWorkAmountPrintBL.mErrors);
					return false;
				} else {
				...
				mResult = tWorkAmountPrintBL.getResult();
				return true;
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public static void main(String[] args) {
		WorkAmountPrintUI tWorkAmountPrintUI = new WorkAmountPrintUI();

		VData tVData = new VData();
		tVData.addElement("863200");
		tVData.addElement("2005-11-30");
		tVData.addElement("2005-11-30");
		tVData.addElement("1");
		tVData.addElement("2");

		tWorkAmountPrintUI.submitData(tVData, "PRINT");

...
public class PEdorRFBackDetailBL implements EdorBack {
private static Logger logger = Logger.getLogger(PEdorRFBackDetailBL.class);
	...
	private boolean dealData() {
		try {
			logger.debug("---保单质押贷款清偿保全回退处理---");

			// ***说明：LJAGetDraw、LJAGetEndorse、LJAPayPerson 在公共部分处理，因此以下注掉
			/*
			 * //获得此时的日期和时间 String strCurrentDate = PubFun.getCurrentDate();
			 * String strCurrentTime = PubFun.getCurrentTime(); ExeSQL tExeSQL =
			 * new ExeSQL(); //查询贷款金额和利息 String tSql = "SELECT
			 * ReturnMoney,ReturnInterest FROM LOReturnLoan WHERE" + " ContNo='" +
			 * mLPEdorItemSchema.getContNo() + "'" + " and EdorNo='" +
			 * mLPEdorItemSchema.getEdorNo() + "'" + " and LoanType='0'"; SSRS
			 * tSSRS = tExeSQL.execSQL(tSql); if (tSSRS == null && tSSRS.MaxRow <=
			 * 0) { // @@错误处理 return this.makeError("dealData", "查询还款信息失败！"); }
			 * double tReturnMoney = Double.parseDouble(tSSRS.GetText(1,1));
			 * double tReturnInterest = Double.parseDouble(tSSRS.GetText(1,2));
			 * 
			 * LCContDB tLCContDB = new LCContDB();
			 * tLCContDB.setContNo(mLPEdorItemSchema.getContNo()); if
			 * (!tLCContDB.getInfo()) { // @@错误处理 return
			 * this.makeError("dealData","查询保单信息失败！"); }
			 * //生成“批改补退费表（应收/应付）”数据-------------------->
			 * //本金记录和利息记录，利息为初始利息和逾期利息的和 //查询“单位名称” String tGrpName = ""; tSql =
			 * "SELECT a.GrpName" + " FROM LCAddress a,LCInsured b,LCCont c" + "
			 * WHERE a.CustomerNo=b.InsuredNo" + " and a.AddressNo=b.AddressNo" + "
			 * and b.InsuredNo=c.InsuredNo" + " and b.ContNo=c.ContNo" + " and
			 * c.ContNo='" + tLCContDB.getContNo() + "'";
			 * 
			 * tSSRS = tExeSQL.execSQL(tSql); if (tSSRS != null && tSSRS.MaxRow >
			 * 0) { tGrpName = tSSRS.GetText(1,1); }
			 * 
			 ...
			 * 
			 * //查询“补/退费财务类型” //组织SQL语句 tSql = "SELECT CodeName" + " FROM
			 * ((select '1' as flag,CodeName" + " from LDCode1 a" + " where
			 * CodeType='RF' and Code='HK' and exists(select 'x' from LCPol
			 * where RiskCode=a.Code1 and MainPolNo=PolNo and ContNo='" +
			 * tLCContDB.getContNo() + "'))" + " union" + " (select '2' as
			 * flag,CodeName" + " from LDCode1" + " where CodeType='RF' and
			 * Code='HK' and Code1='000000'))" + " WHERE rownum=1" + " ORDER BY
			 * flag"; //查询 //tSSRS = new SSRS(); tSSRS = tExeSQL.execSQL(tSql);
			 * if (tSSRS == null && tSSRS.MaxRow <= 0) { // @@错误处理 return
			 * this.makeError("dealData","查询还款的“补/退费财务类型”时产生错误！"); }
			 ...
			 * tLJSGetEndorseSchema.setGetMoney(-tReturnMoney); //
			 ...
			 * //查询“补/退费财务类型” //组织SQL语句 tSql = "SELECT CodeName" + " FROM
			 * ((select '1' as flag,CodeName" + " from LDCode1 a" + " where
			 * CodeType='RF' and Code='LX' and exists(select 'x' from LCPol
			 * where RiskCode=a.Code1 and MainPolNo=PolNo and ContNo='" +
			 * tLCContDB.getContNo() + "'))" + " union" + " (select '2' as
			 * flag,CodeName" + " from LDCode1" + " where CodeType='RF' and
			 * Code='LX' and Code1='000000'))" + " WHERE rownum=1" + " ORDER BY
			 * flag"; //查询 //tSSRS = new SSRS(); tSSRS = tExeSQL.execSQL(tSql);
			 * if (tSSRS == null && tSSRS.MaxRow <= 0) { // @@错误处理 return
			 * this.makeError("dealData","查询利息的“补/退费财务类型”时产生错误！"); }
			 * tLJSGetEndorseSchema.setFeeFinaType(tSSRS.GetText(1, 1));
			 * 
			 * tLJSGetEndorseSchema.setGrpContNo(tLCContDB.getGrpContNo());
			 * tLJSGetEndorseSchema.setContNo(tLCContDB.getContNo());
			 ...
			 * tLJSGetEndorseSchema.setInsuredNo(tLCContDB.getInsuredNo());
			 * tLJSGetEndorseSchema.setGetDate(strCurrentDate);
			 * tLJSGetEndorseSchema.setGetMoney(-tReturnInterest); //
			 * tLJSGetEndorseSchema.setKindCode(tLCPolSchema.getKindCode()); //
			 * tLJSGetEndorseSchema.setRiskCode(tLCPolSchema.getRiskCode()); //
			 * tLJSGetEndorseSchema.setRiskVersion(tLCPolSchema.getRiskVersion());
			 * tLJSGetEndorseSchema.setManageCom(mGlobalInput.ManageCom);
	...
	private boolean makeError(String vFuncName, String vErrMsg) {
		...
		tError.moduleName = "PEdorRFBackDetailBL";
	...
	public static void main(String[] args) {
		PEdorRFBackDetailBL tPEdorRFBackDetailBL = new PEdorRFBackDetailBL();
	}
}

 ...
 * @version 1.0
 */
public class LLRgtMAffixListIssueUI implements BusinessService{
private static Logger logger = Logger.getLogger(LLRgtMAffixListIssueUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		LLRgtMAffixListIssueBL tLLRgtMAffixListIssueBL = new LLRgtMAffixListIssueBL();

		logger.debug("----------LLRgtMAffixListIssueUI Begin----------");
		if (tLLRgtMAffixListIssueBL.submitData(mInputData, mOperate) == false) {
			// @@错误处理
			CError.buildErr(this, "单证回销失败,"+tLLRgtMAffixListIssueBL.mErrors.getLastError());
			mResult.clear();
			return false;
		} else {
			mResult = tLLRgtMAffixListIssueBL.getResult();
		}
		logger.debug("----------LLRgtMAffixListIssueUI End----------");

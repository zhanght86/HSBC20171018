...
/**
 * <p>Title: Web业务系统</p>
 * <p>Description:提交理赔审核赔付保项结论信息类</p>
...
private static Logger logger = Logger.getLogger(LLClaimAuditGiveTypeUI.class);
    ...
    public boolean submitData(VData cInputData, String cOperate)
    {
        ...
        LLClaimAuditGiveTypeBL tLLClaimAuditGiveTypeBL = new
                LLClaimAuditGiveTypeBL();

        logger.debug("----------UI BEGIN----------");
        if (tLLClaimAuditGiveTypeBL.submitData(mInputData, mOperate) == false)
        {
            // @@错误处理
            this.mErrors.copyAllErrors(tLLClaimAuditGiveTypeBL.mErrors);
            CError tError = new CError();
            tError.moduleName = "LLClaimAuditGiveTypeUI";

 ...
 * @version 1.0
 */
public class DocLPBankService extends BaseService {
private static Logger logger = Logger.getLogger(DocLPBankService.class);
	public boolean deal() {
		logger.debug("====DocLPBankService start====");
		MMap mMap = new MMap();
		...
		if (!"LP01".equals(tLOPRTManagerDB.getCode())) {
		...
		mES_DOC_RELATIONSchema.setBussNoType("25");
		...
		mES_DOC_RELATIONSchema.setBussNo(tLOPRTManagerDB.getOtherNo());
		mES_DOC_RELATIONSchema.setDocCode(tLOPRTManagerDB.getOtherNo());
		mES_DOC_RELATIONSchema.setBussNoType("22");
		mES_DOC_RELATIONSchema.setDocID(mES_DOC_MAINSchema.getDocID());
		mES_DOC_RELATIONSchema.setBussType(mES_DOC_MAINSchema.getBussType());
		mES_DOC_RELATIONSchema.setSubType(mES_DOC_MAINSchema.getSubType());
		mES_DOC_RELATIONSchema.setRelaFlag("0");

...
public class GTReasonStatUI {
private static Logger logger = Logger.getLogger(GTReasonStatUI.class);
    ...
    public boolean submitData(VData cInputData, String cOperate)
    {
        try
        {
            ...
            logger.debug("Start GTReasonStatBL Submit ...");
            if( !tGTReasonStatBL.submitData(cInputData, cOperate) )
            {
                if( tGTReasonStatBL.mErrors.needDealError() )
                {
                    mErrors.copyAllErrors(tGTReasonStatBL.mErrors);
                    return false;
                }
                else
                {
                    buildError("submitData", "GTReasonStatBL发生错误，但是没有提供详细的出错信息");
                    return false;
                }
            }
            else
            {
                mResult = tGTReasonStatBL.getResult();
                return true;
            }
        }catch(Exception e)
        {
            ...
            cError.moduleName = "GTReasonStatUI";

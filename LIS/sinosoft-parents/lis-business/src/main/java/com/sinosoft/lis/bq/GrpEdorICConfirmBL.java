 ...
 * @version  : 1.00, 1.01
 * @date     : 2006-04-10, 2006-08-19
 * @direction: 团单保全客户重要资料变更确认
 ******************************************************************************/
...
public class GrpEdorICConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(GrpEdorICConfirmBL.class);
	// public GrpEdorICConfirmBL() {}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		// logger.debug("\t@> GrpEdorICConfirmBL.submitData() 开始");

		// 接收参数
		if (cInputData == null) {
			CError.buildErr(this, "无法获取提交的数据, 请确认数据不为空！");
			logger.debug("\t@> GrpEdorICConfirmBL.submitData() : 无法获取 InputData 数据！");
			return false;
		} else {
		...
		// logger.debug("\t@> GrpEdorICConfirmBL.submitData() 结束");
		return true;
	...
	private boolean getInputData() {
		// logger.debug("\t@> GrpEdorICConfirmBL.getInputData() 开始");
		...
		if (rGlobalInput == null) {
			logger.debug("\t@> GrpEdorICConfirmBL.getInputData() : 无法获取 GlobalInput 数据！");
		...
		if (rLPGrpEdorItemSchema == null) {
			logger.debug("\t@> GrpEdorICConfirmBL.getInputData() : 无法获取 LPGrpEdorItemSchema 数据！");
		...
		// logger.debug("\t@> GrpEdorICConfirmBL.getInputData() 结束");
		return true;
	...
	private boolean checkData() {
		// logger.debug("\t@> GrpEdorICConfirmBL.checkData() 开始");
		...
		// logger.debug("\t@> GrpEdorICConfirmBL.checkData() 结束");
		return true;
	...
	private boolean dealData() {
		// logger.debug("\t@> GrpEdorICConfirmBL.dealData() 开始");
		...
		if (tLPEdorItemSet == null || tLPEdorItemSet.size() <= 0) {
			...
			for (int i = 1; i <= tLPEdorItemSet.size(); i++) {
				...
				if (!tPEdorICConfirmBL.submitData(tVData, rOperation)) {
					mErrors.copyAllErrors(tPEdorICConfirmBL.mErrors);
					logger.debug("\t@> GrpEdorICConfirmBL.dealData() : PEdorICConfirmBL.submitData() 失败！");
					logger.debug("\t   错误原因 : " + mErrors.getFirstError());
					CError.buildErr(this, "处理提交的数据失败！");
					return false;
				} else {
					VData tTempVData = new VData();
					tTempVData = tPEdorICConfirmBL.getResult();
		...
		// logger.debug("\t@> GrpEdorICConfirmBL.dealData() 结束");
		return true;
	...
	private boolean outputData() {
		// logger.debug("\t@> GrpEdorICConfirmBL.outputData() 开始");

		rResultData = new VData();
		rResultData.add(rMap);

		// logger.debug("\t@> GrpEdorICConfirmBL.outputData() 结束");
		return true;
	...
	// {
	// GrpEdorICConfirmBL tGrpEdorICConfirmBL = new GrpEdorICConfirmBL();
	// } //function main end
	// ==========================================================================

} // class GrpEdorICConfirmBL end

...
public class GrpPolImpInfo
{
	 ...
	 * @return boolean
	 */
	public boolean init(String strBatchNo, GlobalInput GlobalInput, TransferData TransferData,
			LCInsuredSet tLCInsuredSet, LCBnfSet tLCBnfSet, LCInsuredRelatedSet tLCInsuredRelatedSet,
			LCGrpContSchema tLCGrpContSchema)
	{
		...
		{
			if (!this.initLDGrpAppnt(tLCGrpContSchema.getGrpContNo(), tLCGrpContSchema.getAppntNo()))
			{
				return false;
			}
		}
		...
		if (needquery)
		{
			...
			for (int t = 1; t <= mLCContPlanSet.size(); t++)
			{
				this.queryLCContPlanRisk(grpcontNo, mLCContPlanSet.get(t).getContPlanCode());
			}
		}
		return true;
	}
	...
	public boolean removeCaceh(String contId)
	{
		...
		for (int i = 1; i <= this.mLCInsuredSet.size(); i++)
		{
			...
			{
				polKey = contId + "-" + this.mLCInsuredSet.get(i).getInsuredNo();
				this.mInsuredMap.remove(this.mLCInsuredSet.get(i).getInsuredNo());
				...
				while (it.hasNext())
				{
					...
					if (key.indexOf("-") > 0)
					{
						if (polKey.equals(key.substring(0, key.lastIndexOf("-"))))
						{
							tPol = (LCPolSchema) this.mMainPolMap.get(key);
							if (tPol != null)
							{
								this.substractMoney(tPol.getRiskCode(), tPol.getPrem(), tPol.getAmnt());
	 ...
	 * @return String[]
	 */
	public String[] getInsuredRela(String contId, String mainInsured, String riskcode)
	{
		...
		for (int i = 1; i <= mLCInsuredRelatedSet.size(); i++)
		{
			...
			// &&
			// mainInsured.equals(mLCInsuredRelatedSet.get(i).getMainCustomerNo())
			// // 借用段
	...
	public LCBnfSet getBnfSet(String strPolId)
	{
		...
		for (int i = 1; i <= this.mLCBnfSet.size(); i++)
		{
			...
			if (getPolKey(tSchema.getContNo(), tSchema.getInsuredNo(), tSchema.getPolNo()).equals(strPolId))
			{
	 ...
	 */
	public LCContPlanRiskSchema findLCContPlanRiskSchema(String riskcode, String contPlanCode)
	{
		for (int i = 1; i < mLCContPlanRiskSet.size(); i++)
		{
			if (mLCContPlanRiskSet.get(i).getRiskCode().equals(riskcode)
					&& mLCContPlanRiskSet.get(i).getContPlanCode().equals(contPlanCode))
			{
	...
	public LCContPlanRiskSet findLCContPlanRiskSet(String contPlanCode)
	{
		...
		for (int i = 1; i <= mLCContPlanRiskSet.size(); i++)
		{
			if (mLCContPlanRiskSet.get(i).getContPlanCode().equals(contPlanCode))
			{
	...
	public VData formatLCPol(LCPolSchema tLCPolSchema, String strID)
	{
		...
		if (("Y".equals(tLCPolSchema.getSpecifyValiDate()) && tLCPolSchema.getCValiDate() != null)
				|| (tLCPolSchema.getCValiDate() != null && !"".equals(tLCPolSchema.getCValiDate())))
		{
		...
		else
		{
			...
			if (mTransferData.getValueByName("BQFlag") != null && mTransferData.getValueByName("BQFlag") != "")
			{
		...
		tLCInsuredSchema = this.findInsuredfromCache(tLCPolSchema.getInsuredNo());
		if (tLCInsuredSchema == null)
		{
			// 需要创建新被保险人
			VData tData = this.getContData(tLCPolSchema.getContNo(), tLCPolSchema.getInsuredNo(), tLCGrpPolSchema);
			if (tData == null)
			{
				return null;
			}
			...
			if (map != null && map.keySet().size() > 0)
				tReturnData.add(map);
			tLCInsuredSchema = (LCInsuredSchema) tData.getObjectByObjectName("LCInsuredSchema", 0);

		}
		// }
		if (tLCInsuredSchema.getContNo() != null && !"".equals(tLCInsuredSchema.getContNo()))
		{
			tLCPolSchema.setContNo(tLCInsuredSchema.getContNo().trim());
		}
		else
		{
			tLCPolSchema.setContNo(tLCInsuredSchema.getContNo());
		}
	...
	public String getMainRiskCode(String planId, String riskcode)
	{
		...
		for (int i = 1; i <= this.mLCContPlanRiskSet.size(); i++)
		{
			...
			if (schema.getContPlanCode().equals(planId) && schema.getRiskCode().equals(riskcode))
			{
				// return this.mLCContPlanRiskSet.get(i).getMainRiskCode()
				if (schema.getMainRiskCode().equals("000000") || "".equals(StrTool.cTrim(schema.getMainRiskCode())))
				{
	 ...
	 */
	public LCInsuredRelatedSet getInsuredRelaData(String mainInsuredNo, String[] relaIns, String tBatchNo)
	{
		...
		for (int i = 0; i < relaIns.length; i++)
		{
			...
			if (tSchema == null)
			{
				...
				tSchema = (LCInsuredSchema) tInsData.getObjectByObjectName("LCInsuredSchema", 0);
			}
			...
			if (tSchema != null)
			{
				if (mRiskCode != null)
				{
					for (int k = 0; k < mRiskCode.length; k++)
					{
						...
						if (RelationToInsured != null && !"".equals(RelationToInsured)
								&& !RelationToInsured.equals("00"))
						{
	...
	private String queryLCInsuRelaNo(String SequenceNo, String PrtNo, String tBatchNo, String tRiskCode, String tFlag)
	{
		...
		tStrSQL = "select * from LCInsuredRelated where 1=1 "
				+ "and CustomerNo in (select a.InsuredNo from LCInsured a,LCPol b where 1=1 and a.InsuredNo=b.InsuredNo and a.PrtNo=b.PrtNo and b.PolTypeFlag='0' and b.PrtNo='"
				+ PrtNo + "') " + "and SequenceNo='" + SequenceNo + "'"
	 ...
	 * @param globalInput GlobalInput
	 */
	public boolean logError(String batchNo, String grpContNo, String contId, String insuredId, String polId,
			String contPlanCode, String riskCode, LCInsuredSchema insuredSchema, CErrors errInfo,
	 ...
	 * @param aPolSchema LCPolSchema
	 * @param errInfo CErrors
	 * @param globalInput GlobalInput
	 */
	public MMap logSucc(String batchNo, String grpContNo, String contId, String prtNo, String contNo,
			GlobalInput globalInput)
	{
	...
	public LCGrpImportLogSet getErrors()
	{
		...
		String strSQL = "SELECT * FROM LCGrpImportLog" + " WHERE BatchNo = '" + m_strBatchNo + "'"
	 ...
	 * @return boolean
	 */
	public VData getContData(String contId, String insuredId, LCGrpPolSchema tLCGrpPolSchema)
	{
		...
		if (tLCInsuerdSchema != null && contSchema != null && !"".equals(StrTool.cTrim(contSchema.getContNo())))
		{ // 已经创建，直接返回
		...
		if (createCont)
		{
			...
			{
				// 借用入机时间字段保存 被保人数目
				contSchema.setPeoples(String.valueOf(tLCInsuerdSchema.getMakeTime()));
			}
			else
			{
			...
			if (mTransferData.getValueByName("BQFlag") != null && mTransferData.getValueByName("BQFlag") != ""
					&& "2".equals(StrTool.cTrim(mTransferData.getValueByName("BQFlag").toString())))
		...
		transferData.setNameAndValue("InsuredPeoples", String.valueOf(contSchema.getPeoples()));
		transferData.setNameAndValue("EdorType", mTransferData.getValueByName("EdorType"));
		...
		logger.debug("rrrrrrrrrrrrrrrrrrrrr:" + tLCInsuerdSchema.getOccupationType());
		logger.debug("rrrrrrrrrrrrrrrrrrrrr:" + tLCInsuerdSchema.getOccupationCode());
		...
		boolean cRes = contInsuredBl.preparesubmitData(tInputData, "INSERT||CONTINSURED");
		if (!cRes)
		{
		...
		LCAddressSchema tLCAddressSchema = (LCAddressSchema) tResult.getObjectByObjectName("LCAddressSchema", 0);
		if (tLCAddressSchema != null && "" + tLCAddressSchema.getAddressNo() != "")
		{
			// tLCAddressSet.add( tLCAddressSchema );
			// map.put(tLCAddressSchema, "INSERT");
		}
		LCInsuredSchema rInsureSchema = (LCInsuredSchema) tResult.getObjectByObjectName("LCInsuredSchema", 0);
		LDPersonSchema tLDPersonSchema = (LDPersonSchema) tResult.getObjectByObjectName("LDPersonSchema", 0);
		LCContSchema rContSchema = (LCContSchema) tResult.getObjectByObjectName("LCContSchema", 0);
		LLAccountSchema rLLAccountSchema = (LLAccountSchema) tResult.getObjectByObjectName("LLAccountSchema", 0);
		...
		// if
		// (!StrTool.cTrim(cacheInsuredSchema.getOccupationType()).equals(""))
		// rInsureSchema.setOccupationType(cacheInsuredSchema.
		// getOccupationType());
		// if
		// (!StrTool.cTrim(cacheInsuredSchema.getOccupationCode()).equals(""))
	...
	/**
	 * 受益人信息：查找被保险人，当受益人跟被保险人为同一人，则利用被保险人的信息 如果不是同一人，查找被保险人是否已经创建，若没需要创建 ，
	 * 创建受益人信息
	 * @param tLCBnfSet LCBnfSet
	 * @param insuredSchema LCInsuredSchema
	 ...
	 * @return VData
	 */
	public VData perareBnf(LCBnfSet tLCBnfSet, LCInsuredSchema insuredSchema, LCGrpPolSchema tLCGrpPolSchema,
			String insureIndex)
	{
		MMap tmpMap = null;
		// 校验处理受益人的被保险人信息
		...
		for (int t = 1; t <= tLCBnfSet.size(); t++)
		{
			...
			if ("1".equals(StrTool.cTrim(tBnfSchema.getBnfType()))
					&& "00".equals(StrTool.cTrim(tBnfSchema.getRelationToInsured())))
			{
				CError.buildErr(this, "合同[" + tBnfSchema.getContNo() + "]有受益人类型为'死亡受益人'，不能指定为被保人本人");
				return null;
			}
			// 受益人为被保险人本人
			else if ("00".equals(StrTool.cTrim(tBnfSchema.getRelationToInsured())))
			{
			...
			{
				// 查询被保险人
				LCInsuredSchema tInsuredSchema = findInsuredfromCache(tBnfSchema.getInsuredNo());
				if (tInsuredSchema == null)
				{
					CError.buildErr(this, "受益人信息没有查找到被保险人[" + tBnfSchema.getInsuredNo() + "]信息");
					return null;
				}
	...
	private LDPersonSchema checkInSystemPerson(LDPersonSchema schema)
	{
		...
		if (!"".equals(StrTool.cTrim(schema.getSex())) && !"".equals(StrTool.cTrim(schema.getBirthday()))
				&& !"".equals(StrTool.cTrim(schema.getIDType())) && !"".equals(StrTool.cTrim(schema.getIDNo())))
		{
			/*
			 * if ("".equals(StrTool.cTrim(schema.getSex()))) {
			 * CError.buildErr(this, "客户性别不能为空！"); err = true; } if
			 * ("".equals(StrTool.cTrim(schema.getBirthday()))) {
			 * CError.buildErr(this, "客户生日不能为空！"); err = true; } if
			 * ("".equals(StrTool.cTrim(schema.getBirthday()))) {
			 * CError.buildErr(this, "客户证件类型不能为空！"); err = true; } if
			 * ("".equals(StrTool.cTrim(schema.getBirthday()))) {

...
public class GrpEdorESPrintBL implements EdorPrint {
private static Logger logger = Logger.getLogger(GrpEdorESPrintBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "ES数据提取失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "ES数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		xmlexport.addDisplayControl("displayES");
        ...
        ListTable tESListTable = new ListTable();
        tESListTable.setName("ES");
        String tSql = "select b.name,"
        			...
        			+"a.amnt,"
        			+"nvl((select getmoney from ljsgetendorse where polno = a.polno and endorsementno = a.edorno),'0'),'"+mLPGrpEdorItemSchema.getEdorValiDate()+"'"
		...
		for (int i = 1; i <= tArrLen; i++) {
			String[] strES = new String[8] ;
			for(int k = 1;k<=strES.length;k++){
				strES[k-1] = rSSRS.GetText(i, k);
			}
			tESListTable.add (strES) ;
		}
		...
		mTextTag.add("ESNUM", dNum);
		mTextTag.add("ESSUM", dSum);
		...
		String[] b_strES = new String[2];
		xmlexport.addListTable(tESListTable, b_strES);

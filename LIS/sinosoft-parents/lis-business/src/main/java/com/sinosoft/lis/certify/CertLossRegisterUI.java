 ...
 * @version 1.0
 */
public class CertLossRegisterUI implements BusinessService {
private static Logger logger = Logger.getLogger(CertLossRegisterUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			CertLossRegisterBL tCertLossRegisterBL = new CertLossRegisterBL();
			boolean bReturn = tCertLossRegisterBL.submitData(cInputData,
					mOperate);
			mResult = tCertLossRegisterBL.getResult();

			if (!bReturn) {
				if (tCertLossRegisterBL.mErrors.needDealError()) {
					mErrors.copyAllErrors(tCertLossRegisterBL.mErrors);
				} else {

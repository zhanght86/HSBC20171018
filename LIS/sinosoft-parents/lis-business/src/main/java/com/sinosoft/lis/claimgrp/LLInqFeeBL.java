...
private static Logger logger = Logger.getLogger(LLInqFeeBL.class);
    ...
    */
    ...
    private boolean getInputData()
    {
        ...
        logger.debug("---getInputData===>mIsReportExist="+mIsReportExist);
        mLLInqFeeSchema = (LLInqFeeSchema) mInputData.getObjectByObjectName("LLInqFeeSchema", 0);
    ...
    private boolean dealData(String cOperate)
    {
        ...
        if (mIsReportExist.equals(""))
        {
            ...
            String strSql = "select * from LLInqFee where "
                          + "ClmNo = '"+mLLInqFeeSchema.getClmNo()+"'"
                          + " and InqNo = '"+mLLInqFeeSchema.getInqNo()+"'"
                          + " and InqDept = '"+mLLInqFeeSchema.getInqDept()+"'"
                          + " and FeeType = '"+mLLInqFeeSchema.getFeeType()+"'";
            ExeSQL exesql = new ExeSQL();
            String tResult = exesql.getOneValue(strSql);
            logger.debug("-----判断事件是否存在tResult.length()= "+tResult.length());
            if (tResult.length() > 0)
            {
                mIsReportExist = "true";
            }
            ...
            logger.debug("-----after dealData==>mIsReportExist= "+mIsReportExist);
        }
        //添加纪录
        if (cOperate.equals("INSERT"))
        {
            logger.debug("---start deal insert and mIsReportExist="+mIsReportExist);

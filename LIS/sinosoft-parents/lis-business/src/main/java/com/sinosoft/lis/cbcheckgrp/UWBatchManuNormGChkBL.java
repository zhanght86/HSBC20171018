...
private static Logger logger = Logger.getLogger(UWBatchManuNormGChkBL.class);
	...
	private String specFlag = "";// 批量通过标志
	...
	private boolean getInputData(VData cInputData) {
		...
		if (n == 1) {
			...
			mUWFlag = tLCGrpContSchema.getUWFlag();
			specFlag = tLCGrpContSchema.getSpecFlag();// 批量通过标志
			// 校
	...
	private boolean checkUWGrade() {
		...
		String tUWPopedom = tLDUWUserSet.get(1).getUWPopedom();
	...
	private void prepareOutputData() {
		mMap.put(
				"update LCGUWMaster set passflag='9',state='9' where grpcontno='"
						+ mGrpContNo + "'", "UPDATE");
	...
	private boolean prepareCont() {
		String tsql = "select * from LCCont where  GrpContNo='" + mGrpContNo
				+ "' and UWFlag not in ('0', '1', '4', '9')";
		LCContDB tLCContDB = new LCContDB();
		mLCContSet = tLCContDB.executeQuery(tsql);
		if ((mLCContSet == null) || (mLCContSet.size() <= 0)) {
			CError tError = new CError();
			tError.moduleName = "UWBatchManuNormGChkBL";
			tError.functionName = "prepareCont";
			tError.errorMessage = "没有未通过的个人合同!";
			this.mErrors.addOneError(tError);
	...
	private boolean preparePol() {
		String t1sql = "select * from LCPol where  GrpContNo='" + mGrpContNo
				+ "' and UWFlag not in ('0', '1', '4', '9')" + " and ContNo ='"
				+ mLCContSchema.getContNo() + "'";

		LCPolDB tLCPolDB = new LCPolDB();
		mLCPolSet = tLCPolDB.executeQuery(t1sql);
		if ((mLCPolSet == null) || (mLCPolSet.size() <= 0)) {
			CError tError = new CError();
			tError.moduleName = "UWBatchManuNormGChkBL";
			tError.functionName = "preparePol";
			tError.errorMessage = "该合同" + mLCContSchema.getContNo()
					+ "下的险种保单没有必要批量通过!";
			this.mErrors.addOneError(tError);
	...
	private boolean callCont() {
		...
		// 合同的险种都通过啦，只把合同的UWFLAG修改并下结论就可以啦
	...
	private boolean callPol() {
		...
		for (int j = 1; j <= m; j++) {
			...
			tLCPolSchema.setStandbyFlag3(specFlag);

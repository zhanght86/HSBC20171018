...
import com.sinosoft.lis.claimgrp.LLSubmitApplyHeadDealBL;
 ...
 * @version 1.0
 */
public class LLSubmitApplyHeadDealBLF
{
private static Logger logger = Logger.getLogger(LLSubmitApplyHeadDealBLF.class);
    ...
    public LLSubmitApplyHeadDealBLF()
    {
    }
    ...
    private boolean getInputData()
    {
        ...
        if (mMissionID == null)
        {
            //@@错误处理
            CError tError = new CError();
            tError.moduleName = "LLSubmitApplyHeadDealBLF";
    ...
    {
        logger.debug("----------LLSubmitApplyHeadDealBLF dealData BEGIN----------");
        boolean tReturn = false;

        LLSubmitApplyHeadDealBL tLLSubmitApplyHeadDealBL = new LLSubmitApplyHeadDealBL();
        if (!tLLSubmitApplyHeadDealBL.submitData(mInputData,mOperate))
        {
            // @@错误处理
            this.mErrors.copyAllErrors(tLLSubmitApplyHeadDealBL.mErrors);
            CError tError = new CError();
            tError.moduleName = "LLSubmitApplyHeadDealBLF";
            tError.functionName = "submitData";
            tError.errorMessage = "数据提交失败!";
        ...
        {
            logger.debug("-----start LLSubmitApplyHeadDealBLF getData from BL");
            VData tempVData = new VData();
            tempVData = tLLSubmitApplyHeadDealBL.getResult();
    ...
    private boolean updateMyMission()        //更新LWMission表
    {
            ...
            mLWMissionSchema.setMissionProp6("2");//更新为"2--总公司向分公司"
            mLWMissionSchema.setLastOperator(mG.Operator);//最后操作员代码
            mLWMissionSchema.setModifyDate(CurrentDate);
            mLWMissionSchema.setModifyTime(CurrentTime);

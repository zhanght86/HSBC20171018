...
public class GrpEdorLNPrintBL implements EdorPrint {
private static Logger logger = Logger.getLogger(GrpEdorLNPrintBL.class);
	...
	private boolean dealData() {
        ...
        LPGrpContSchema tLPGrpContSchema = tLPGrpContDB.query().get(1);
        
    	double dGrpContCashValue = 0.0;//合同总的现金价值
    	double dLastLoanValue = 0.0;//历史借款的本息和
    	double dGrpNowCashValue = 0.0;//合同现在的现金价值
    	@SuppressWarnings("unused")
		double dGrpNowCanLoanMoney = 0.0;//合同现在的可贷额度
    	
		//合同总的现金价值
		dGrpContCashValue = tLPGrpContSchema.getAmnt();
		//历史借款的本息和
		dLastLoanValue = tLPGrpContSchema.getPrem();
		//合同现在的现金价值
		dGrpNowCashValue = dGrpContCashValue - dLastLoanValue;
		//合同现在的可贷额度 
		dGrpNowCanLoanMoney = tLPGrpContSchema.getSumPrem();

        ListTable tLNListTable = new ListTable();
        tLNListTable.setName("LN");
        String tSql = " select r.riskname,"
        			+ " decode(m.feefinatype, 'DK', '借款', 'RV', '印花税', '其它'), "
		...
		for (int i = 1; i <= tArrLen; i++) {
			String[] strLN = new String[4] ;
			for(int k = 1;k<=strLN.length;k++){
				strLN[k-1] = rSSRS.GetText(i, k);
			}
			tLNListTable.add (strLN) ;
		}
		
	    tSql = " select nvl(sum(CurLoanMoney), 0)"
             + " from LPLoan "
             + " where 1 = 1 "
             + " and EdorNo = '"+mLPGrpEdorItemSchema.getEdorNo()+"'"
             + " and EdorType = '"+mLPGrpEdorItemSchema.getEdorType()+"'"
             + " and ContNo = '"+mLPGrpEdorItemSchema.getGrpContNo()+"'"
             + " and PolNo = '000000' "
             + " and LoanType = '0' "
             + " and PayOffFlag = '0'";
	    
		String dSum = rExeSQL.getOneValue(tSql);
		if (dSum == null || dSum.equals("")) {
			CError.buildErr(this, "统计借款失败!");
			return false;
		}
		...
		String dLNSumMoney = String.valueOf(mLPGrpEdorItemSchema.getGetMoney());
		
		mTextTag.add("GrpCashValue", dGrpNowCashValue);
		mTextTag.add("LNGetMoney", dLNSumMoney);
		mTextTag.add("LoanMoney", dSum);
		...
		String[] b_strLN = new String[4];
		xmlexport.addListTable(tLNListTable, b_strLN);

 ...
 * @version 1.0
 */
public class LLClaimScanRegisterBL {
private static Logger logger = Logger.getLogger(LLClaimScanRegisterBL.class);
...
//				}
		...
		if (cOperate.equals("UPDATE")) {
			...
			sqlbv2.put("RgtNo", mLLCaseSchema.getRgtNo());
	...
	private void IsExist() {
		...
		//      mIsReportExist = true;
				...
				sqlbv4.sql(strSql);
				sqlbv4.put("RgtNo", mLLRegisterSchema.getRgtNo());
				ExeSQL exesql = new ExeSQL();
				String tResult = exesql.getOneValue(sqlbv4);
				if (tResult.length() > 0) {

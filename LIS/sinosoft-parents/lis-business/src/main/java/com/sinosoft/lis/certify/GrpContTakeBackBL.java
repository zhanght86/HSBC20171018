 ...
 * @version 1.0
 */
public class GrpContTakeBackBL implements BusinessService {
...
//	private String mReasonDesc = "";//延迟送达原因
	...
	private boolean getInputData(VData cInputData) {
		...
		mGrpContno = mLZSysCertifySchema.getCertifyNo();
...
//			mReasonDesc = (String) mTransferData.getValueByName("reasondesc");
	...
	private boolean checkDate() {
		...
		tLCGrpContDB.setGrpContNo(mLZSysCertifySchema.getCertifyNo());
		tLCGrpContDB.getInfo();
		int date = PubFun.calInterval(tLCGrpContDB.getSignDate(),
				mLZSysCertifySchema.getTakeBackDate(), "D");
		String CurDate = PubFun.getCurrentDate();
		int date2 = PubFun.calInterval(CurDate, mLZSysCertifySchema
				.getTakeBackDate(), "D");
		logger.debug("Cvalidate=" + tLCGrpContDB.getSignDate());
...
//		}
	...
	private boolean dealData() {
		...
		String TakeBackSQLb = "update lpgrpcont set GetPolDate='" + "?CurDate?"
	            + "',GetPolTime='" + "?CurTime?" + "'," + " CustomGetPolDate='"
	            + "?CustomGetPolDate?" + "' "
		        +",ModifyDate='" + "?CurDate?" + "',ModifyTime='" + "?CurTime?" + "'"
		        + "  where grpcontno='" + "?grpcontno?" + "' ";
		SQLwithBindVariables sqlbv1 = new SQLwithBindVariables();
		sqlbv1.sql(TakeBackSQLb);
		...
		String TakeBackSQl = "update lccont set GetPolDate='" + "?CurDate?"
				...
				+ "?CustomGetPolDate?" + "' "
				+",ModifyDate='" + "?CurDate?" + "',ModifyTime='" + "?CurTime?" + "'"
//				+",DelayReasonCode='" + mReasonCode + "',DelayReasonDesc='" + mReasonDesc + "'"
				+ "  where grpcontno='" + "?grpcontno?" + "' ";
		SQLwithBindVariables sqlbv2 = new SQLwithBindVariables();
		sqlbv2.sql(TakeBackSQl);
		sqlbv2.put("CurDate", CurDate);
		sqlbv2.put("CurTime", CurTime);
		sqlbv2.put("CustomGetPolDate", mLZSysCertifySchema.getTakeBackDate());
...
//				+",DelayReasonCode='" + mReasonCode + "',DelayReasonDesc='" + mReasonDesc + "'"
		...
		sqlbv3.sql(TakeBackSQlP);
		sqlbv3.put("CurDate", CurDate);
		sqlbv3.put("CurTime", CurTime);
		sqlbv3.put("CustomGetPolDate", mLZSysCertifySchema.getTakeBackDate());
		sqlbv3.put("grpcontno", mGrpContno);

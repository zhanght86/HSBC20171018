 ...
 */
public class FinaDayEdorNSAAPrintUI implements PrintService {
private static Logger logger = Logger.getLogger(FinaDayEdorNSAAPrintUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (finaType.equals(PrintManagerBL.CODE_PEdorFina)) { // 个险日结
			FinaDayEdorNSAAPrintBL tFinaDayEdorNSAAPrintBL = new FinaDayEdorNSAAPrintBL();
			...
			GrpFinaDayEdorWSWAPrintBL tGrpFinaDayEdorWSWAPrintBL = new GrpFinaDayEdorWSWAPrintBL();
			if (!tGrpFinaDayEdorWSWAPrintBL.submitData(mInputData, cOperate)) {
				// @@错误处理
				this.mErrors.copyAllErrors(tGrpFinaDayEdorWSWAPrintBL.mErrors);
				mResult.clear();
				return false;
			} else {
				mResult = tGrpFinaDayEdorWSWAPrintBL.getResult();
			}
		}
		return true;
	}

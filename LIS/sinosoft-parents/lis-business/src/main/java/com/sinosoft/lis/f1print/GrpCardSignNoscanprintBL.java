...
public class GrpCardSignNoscanprintBL implements PrintService {
private static Logger logger = Logger.getLogger(GrpCardSignNoscanprintBL.class);
	...
	private String strSQL01 = "";
	private String strSQL02 = "";
	private String strSQL03 = "";
	...
	public GrpCardSignNoscanprintBL() {
	}
	...
	private boolean getPrintData() {
		...
		String remark = "GrpCardSignNoscan.vts";
		xmlexport.createDocument(remark, "");
		newXmlExport.createDocument(remark, ""); // 最好紧接着就初始化xml文档
		newXmlExport.setTemplateName("GrpCardSignNoscan.vts");
		...
		if (strManagecom.length() == 8) {
			...
			try {
				...
				for (int i = 1; i <= i_count; i++) {
					...
					logger.debug("cols[2]==" + cols[2] + ":::cols[4]=="
							+ cols[4]);
					if (cols[4] == "") {
						strSQL01 = "select balanceno from lxbalancesub "
								+ "where startno<= '" + ssrs.GetText(i, 1)
								+ "' and endno>= '" + ssrs.GetText(i, 1) + "'";
						SSRS ssrs01 = exeSql.execSQL(strSQL01);
						int i_count01 = ssrs01.getMaxRow();
						if (i_count01 == 0) {
							logger.debug("没有在lxbalancesub找到相关信息");
						} else {
							logger.debug("结算号码==" + ssrs01.GetText(1, 1));
							strSQL02 = "select makedate from ljtempfee "
									+ "where otherno= '" + ssrs01.GetText(1, 1)
									+ "'";
							SSRS ssrs02 = exeSql.execSQL(strSQL02);
							int i_count02 = ssrs02.getMaxRow();
							if (i_count02 == 0) {
								logger.debug("没有在ljtempfee找到相关信息");
							} else {
								logger.debug("收费日期=="
										+ ssrs02.GetText(1, 1));
								cols[4] = ssrs02.GetText(1, 1); // 收费日期
							}

							strSQL03 = "select codename from ldcode "
									+ "where codetype = 'paymode ' "
									+ "and code = (select paymode from ljtempfeeclass "
									+ "where otherno = '"
									+ ssrs01.GetText(1, 1)
									+ "' and rownum = 1) ";
							SSRS ssrs03 = exeSql.execSQL(strSQL03);
							int i_count03 = ssrs03.getMaxRow();
							if (i_count03 == 0) {
								logger.debug("没有在ljtempfeeclass找到相关信息");
							} else {
								logger.debug("交费方式=="
										+ ssrs03.GetText(1, 1));
								cols[9] = ssrs03.GetText(1, 1); // 交费方式
							}
			...
			strSQL = "select comcode from ldcom where comcode like'"
					+ strManagecom + "%' and length(trim(comcode))=8";
			SSRS tSSRS = new SSRS();
			...
			alistTable.setName("RISK");
			strSQL = "select comcode from ldcom where comcode like'"
					+ strManagecom + "%' and length(trim(comcode))=6";
			SSRS tSSRS = new SSRS();
			tSSRS = exeSql.execSQL(strSQL);
			for (int p = 1; p <= tSSRS.getMaxRow(); p++) {
				logger.debug(tSSRS.GetText(p, 1));
	...
	public static void main(String[] args) {
		...
		tGrpCardSignNoscanprintBL.submitData(tVData, "PRINT");
	}
}

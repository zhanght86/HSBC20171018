...
public class NewPolQueryUI {
private static Logger logger = Logger.getLogger(NewPolQueryUI.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		try {
			...
			logger.debug("Start ScanLis UI Submit ...");
				// 保全扫描清单
				NewPolQueryBL tNewPolQueryBL = new NewPolQueryBL();
				if (!tNewPolQueryBL.submitData(cInputData, cOperate)) {
					if (tNewPolQueryBL.mErrors.needDealError()) {
						mErrors.copyAllErrors(tNewPolQueryBL.mErrors);
						return false;
					} else {
					...
					mResult = tNewPolQueryBL.getResult();
					return true;
				}
		} catch (Exception e) {
			e.printStackTrace();

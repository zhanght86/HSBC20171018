 ...
 * Title: 虚拟工作流任务申请
 * </p>
 * 发起外包录入时,更改外包录入标志missionprop11为1
 * <p>
 ...
 * </p>
 * 
 * @author niuzj,2005-10-26
 * @version 1.0
 */

public class LLClaimFaQiWaiBaoBL {
private static Logger logger = Logger.getLogger(LLClaimFaQiWaiBaoBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("---LLClaimFaQiWaiBaoBL dealData Start---");
		...
		logger.debug("---LLClaimFaQiWaiBaoBL dealData End---");
		...
		if (!tPubSubmit.submitData(mInputData, "")) {
			...
			tError.moduleName = "LLClaimFaQiWaiBaoBL";
	...
	private boolean dealData() {
		...
		if (tLWMissionSchema.getMissionProp10() != null
				...
				&& tLWMissionSchema.getMissionProp10() == "1") {
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLClaimFaQiWaiBaoBL";
			tError.functionName = "dealData";
		...
		// if(tLWMissionSchema.getMissionProp20()!= mManageCom)
		// {
		// // @@错误处理
		// CError tError = new CError();
		// tError.moduleName = "LLClaimFaQiWaiBaoBL";
		// tError.functionName = "dealData";
		// tError.errorMessage = "你没有权利对该机构下的任务发起外包录入!";
		// this.mErrors.addOneError(tError);
		// return false;
		// }
		...
		tLWMissionSchema.setMissionProp11("1");
		map.put(tLWMissionSchema, "UPDATE");

		// 发起外包录入时,也要把表llregister中的FeeInputFlag字段置为1
		String tRgtNo = tLWMissionSchema.getMissionProp1();
		String strSql = " update llregister b set b.feeinputflag='1' "
				+ " where b.rgtno='" + "?rgtno?" + "' ";

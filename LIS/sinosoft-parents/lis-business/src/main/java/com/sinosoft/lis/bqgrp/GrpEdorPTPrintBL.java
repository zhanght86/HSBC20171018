...
private static Logger logger = Logger.getLogger(GrpEdorPTPrintBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "PT数据提取失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "PT数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		ListTable tPTListTable = new ListTable();
		...
		for (int i = 1; i <= tArrLen; i++) {
			String[] strPT = new String[8] ;
			for(int k = 1;k<=strPT.length;k++){
				strPT[k-1] = rSSRS.GetText(i, k);
			}
		...
		mTextTag.add("PTNUM", dNum);
		mTextTag.add("PTSUM", dSum);
		...
		String[] b_strPT = new String[8];
		xmlexport.addListTable(tPTListTable, b_strPT);

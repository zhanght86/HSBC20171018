...
public class BQWorkStatUI {
private static Logger logger = Logger.getLogger(BQWorkStatUI.class);
    ...
    public boolean submitData(VData cInputData, String cOperate)
    {
        try
        {
            ...
            if( !tBQWorkStatBL.submitData(cInputData, cOperate) )
            {
                if( tBQWorkStatBL.mErrors.needDealError() )
                {
                    mErrors.copyAllErrors(tBQWorkStatBL.mErrors);
                    return false;
                }
                else
                {
                    buildError("submitData", "BQWorkStatBL发生错误，但是没有提供详细的出错信息");
                    return false;
                }
            }
            else
            {
                mResult = tBQWorkStatBL.getResult();
                return true;
            }
        }catch(Exception e)
        {
            ...
            cError.moduleName = "BQWorkStatUI";

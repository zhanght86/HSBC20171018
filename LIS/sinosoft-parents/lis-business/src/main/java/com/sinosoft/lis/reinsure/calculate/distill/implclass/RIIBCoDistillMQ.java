 ...
 * @version 1.0
 */
public class RIIBCoDistillMQ implements RIDistill {
	String mEventType;
	...
	private boolean dealData() {
		try {
			...
			buildError("dealData", " 提取共保满期数据失败 " + ex.getMessage());
			return false;
		}
		return true;
	}
	...
	private String getDistillSQL() {
		...
		strBuffer.append(" '03' as EventType, ");// 01-新单数据
		...
		strBuffer.append(" a.PolNo || ',' ||'GB'||','||b.startdate||','||'"
				+ mAccumulateDefNo + "'||','||'MQ' as UnionKey, ");
		...
		// 保单年度
		strBuffer
				.append(" ceil(months_between(b.StartDate,a.cvalidate-1)/12) as InsuredYear, ");
		...
		strBuffer.append(" 'MQ' as FeeOperationType, ");
		...
		strBuffer.append(" 0 as AccAmnt, ");// 累计风险保额
		...
		strBuffer.append(" b.StartDate as GetDate, ");
		strBuffer
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=d.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
		...
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecord m where m.unionkey=(a.PolNo || ',' ||'GB'||','||b.startdate||','||'"
						+ mAccumulateDefNo + "'||','||'MQ')) ");
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecordBake n where n.unionkey=(a.PolNo || ',' ||'GB'||','||b.startdate||','||'"
						+ mAccumulateDefNo + "'||','||'MQ')) ");
		strBuffer
		...
		// 终止类型包括：01：满期终止
		strBuffer
				.append(" and b.statereason in('01') and b.enddate is null and b.statetype='Terminate' and b.state='1' and a.appflag ='4' and a.polno=b.polno ");
		// 保单终止日期落在区间内
		...
		strBuffer.append("') ");
		System.out.println("共保满期提数sql: " + strBuffer.toString());
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=d.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
		...
		strBuffer.append(mRIWFLogSchema.getEndDate());
		strBuffer.append("') ");
		System.out.println("共保满期临分提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
	...
	public static void main(String[] args) {
		...
		mRIWFLogSchema.setStartDate("2011-11-1");
		mRIWFLogSchema.setEndDate("2011-11-5");
		mRIWFLogSchema.setTaskCode("L01010070010");
		cInputData.add(mRIWFLogSchema);
		RIIBCoDistillMQ tRIDistillNB = new RIIBCoDistillMQ();

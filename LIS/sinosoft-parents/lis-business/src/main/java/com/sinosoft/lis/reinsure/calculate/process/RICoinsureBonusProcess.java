 ...
 * @version 3.0
 */
public class RICoinsureBonusProcess implements RIProcessType {
	public CErrors mErrors = new CErrors();
	private VData mVData = new VData();
	private MMap mMap;

	public RICoinsureBonusProcess() {
	}
	...
	public boolean dealData(RIPolRecordSchema aRIPolRecordSchema,
		...
		try {
			...
			double AccumulateAmnt = 2; // 本系统的累计风险保额定义为包括：历史与本单的风险保额。累计币种
			double newAmnt = 1; // 新增风险保额。累计币种
					...
					tRIRecordTraceSet.get(i)
							.setCessionRate(
									tRIRecordTraceSet.get(i).getAmountChang()
											/ newAmnt);
					// 分保保额
					// tRIRecordTraceSet.get(i).setCessionAmount(aRIPolRecordSchema.getRiskAmnt()*
					// tRIRecordTraceSet.get(i).getCessionRate());
					tRIRecordTraceSet.get(i).setCessionAmount(0);

					// 分保保额变量
					tRIRecordTraceSet.get(i).setAmountChang(0);
					// 业务日期

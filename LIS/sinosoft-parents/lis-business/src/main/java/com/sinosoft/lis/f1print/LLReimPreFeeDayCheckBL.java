...
public class LLReimPreFeeDayCheckBL implements PrintService {
private static Logger logger = Logger.getLogger(LLReimPreFeeDayCheckBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("-----理赔日结-->退还未满保费日结开始---------LLReimPreFeeDayCheckBL---");
		// 得到外部传入的数据,将数据备份到本类中
		...
		logger.debug("-----理赔日结-->退还未满保费日结结束----LLReimPreFeeDayCheckBL---");
	...
	private boolean checkInputData() {
		if (mStatiCode == null || mStatiCode.equals("")) {
			...
			tError.moduleName = "LLReimPreFeeDayCheckBL";
	...
	private boolean dealData() {
		...
		tXmlExport.createDocument("ReimPreFeeDayCheck.vts", "");
		...
		// 序号 险种 缴费期限 本日金额 本月累计
		// D05 险种终止退未满期保费 D0501 退附加险的未满期保费 TF
		// C09 退未满期保费 C0901 退未满期保费 TF
		...
		String strSQL = " select distinct b.riskcode,(select riskname from lmrisk where riskcode=b.riskcode) "
				...
				+ "?mStatiCode?"
				+ "','%') and j.feefinatype = 'TF' and j.feeoperationtype in('D05','C09')"
				...
				+ " and a.feefinatype = 'TF' and a.feeoperationtype  in('D05','C09') and a.managecom like concat('"
				+ "?mStatiCode?"
				+ "','%') "
		...
		for (int i = 1; i <= tMaxRow; i++) {
			...
			/** 查询--本日应付--** */
			String tMonthly = tSSRS.GetText(i, 6);
			/** 查询--本月应付--** */
		...
		// tTextTag.add("MonthMoney",str_tSumMonthly);//$=$=/MonthMoney--总计--本月累计

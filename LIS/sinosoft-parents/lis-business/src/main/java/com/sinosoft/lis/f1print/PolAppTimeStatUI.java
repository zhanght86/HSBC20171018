...
public class PolAppTimeStatUI {
private static Logger logger = Logger.getLogger(PolAppTimeStatUI.class);
	public CErrors mErrors = new CErrors();
	private VData mResult = new VData();

	public PolAppTimeStatUI() {
	}

	public boolean submitData(VData cInputData, String cOperate) {
		PolAppTimeStatBL tPolAppTimeStatBL = new PolAppTimeStatBL();
		if (!tPolAppTimeStatBL.submitData(cInputData, cOperate)) {
			mErrors.copyAllErrors(tPolAppTimeStatBL.mErrors);
			return false;
		} else {
			mResult = tPolAppTimeStatBL.getResult();
		}
		return true;
	}

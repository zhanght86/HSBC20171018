...
public class GrpEdorYBConfirmBL implements EdorConfirm {
private static Logger logger = Logger.getLogger(GrpEdorYBConfirmBL.class);
	...
	private boolean dealData() {
		...
		if (tLPEdorItemSet == null || tLPEdorItemSet.size() <= 0) {
			...
			for (int i = 1; i <= tLPEdorItemSet.size(); i++) {
				...
				PEdorYBConfirmBL tPEdorYBConfirmBL = new PEdorYBConfirmBL();
				if (!tPEdorYBConfirmBL.submitData(tVData, rOperation)) {
					mErrors.copyAllErrors(tPEdorYBConfirmBL.mErrors);
					...
					tTempVData = tPEdorYBConfirmBL.getResult();

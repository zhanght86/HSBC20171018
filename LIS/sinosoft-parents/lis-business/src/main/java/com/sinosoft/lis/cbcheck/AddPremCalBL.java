...
//		准备BOMCont数据
		...
		//准备险种BOMCalInsured数据
		BOMCalInsured CalInsured = new BOMCalInsured();//一个险种
		CalInsured = DealBOMCalInsured(mLCPolSchema,mLCPremSchema,insured);
		
		
		logger.debug("############3insured"+insured.getInsuredNo()+":"+pol.getInsuredNo()+":"+insured.getSecInsuAddPoint());
		//准备受益人BOMBnf数据
		...
		mCalBase.setOccupation(mLCInsuredSchema.getOccupationCode());
	...
	private BOMCont DealBOMCont(LCContSchema tLCContSchema){
		...
		try{			
			...
			cont.setPolApplyDate(sdf.parse(tLCContSchema.getPolApplyDate()+ " 00:00:00"));
		}catch(Exception e){
	...
	private BOMInsured DealBOMInsured(LCInsuredSchema tLCInsuredSchema,LCPremSchema tLCPremSchema){
		...
		try{
			...
			Insured.setSecInsuAddPoint(mLCPremSchema.getSuppRiskScore());		
			logger.debug("#############mLCPremSchema.getSecInsuAddPoint():"+mLCPremSchema.getSuppRiskScore()+":"+Insured.getSecInsuAddPoint());
	...
	private BOMCalInsured DealBOMCalInsured(LCPolSchema mLCPolSchema,
			LCPremSchema mLCPremSchema ,BOMInsured insured) {
		BOMCalInsured bomCalInsured = new BOMCalInsured();
		
		bomCalInsured.setEM(mLCPremSchema.getSuppRiskScore());//加费评点，加费百分比，千元保额
		
		bomCalInsured.setAppAge(insured.getInsuredAppAge());//被保险人投保年龄
		
		bomCalInsured.setSex(insured.getSex());//被保险人年龄
	...
	private BOMPol DealBOMPol(LCPolSchema tLCPolSchema,LCDutySchema tLCDutySchema){
		...
		try{			
			...
			Pol.setYears(mLCPolSchema.getYears()+0.0);

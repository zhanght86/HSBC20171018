 ...
 */
public class AutoPayEndNoticePrintBL implements PrintService {
private static Logger logger = Logger.getLogger(AutoPayEndNoticePrintBL.class);
	...
	private String mName; // 给付通知书上的投保人姓名，不带称谓
	public static final String VTS_NAME = "ZdhtzztzsBzyzfbfP000840.vts"; // 模板名称

	public AutoPayEndNoticePrintBL() {
	}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if (!mOperate.equals("PRINT")) {
			...
			tError.moduleName = "AutoPayEndNoticePrintBL";
	...
	private boolean dealData() {
		...
		// FDate().getDate(mPaytoDate), 1, "D", null));
		...
		// if(tDouble < 0)
		// {
		// CError.buildErr(this,"现金价值净额为负！原因可能是：没有每天运行自垫终止批处理程序");
		// return false;
		// }
		mCCashValue = PubFun.getChnMoney(tDouble);
		mCashValue = BqNameFun.getRound(tDouble) + "元";

		logger.debug(mContNo + "自垫本息和为：" + mAutoPay + "，中文金额为："
				+ mCAutoPay);
	...
	private boolean preparePrintData() {
		...
		texttag.add("NotifyNo", "");
		texttag.add("LPAppnt.AppntName", mName);
	...
	public static void main(String[] args) {
		...
		AutoPayEndNoticePrintBL tAutoPayEndNoticePrintBL = new AutoPayEndNoticePrintBL();
		...
		// if(!tAutoPayEndNoticePrintBL.submitData(tVData, "PRINT"))
		// {
		// logger.debug("test failed");
		// }
		EdorCalZT tEdorCalZT = new EdorCalZT(tGlobalInput);
		double td = tEdorCalZT.getCashValue("210320000000204", null,
				"2006-01-02");

		BqPolBalBL tBqPolBalBL = new BqPolBalBL();
		tBqPolBalBL.calAutoPayPremAddInterest("210320000000204", "2006-01-01");
		double mSum = tBqPolBalBL.getCalResult();
		logger.debug("mSum:" + mSum);
		tBqPolBalBL.calLoanCorpusAddInterest("000000009083", "2006-01-01");
		mSum += tBqPolBalBL.getCalResult();
		logger.debug("mSum:" + mSum);
		logger.debug("td:" + td);

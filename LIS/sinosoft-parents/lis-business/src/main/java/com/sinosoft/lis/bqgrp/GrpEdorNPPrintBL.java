...
public class GrpEdorNPPrintBL implements EdorPrint {
private static Logger logger = Logger.getLogger(GrpEdorNPPrintBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "NP数据提取失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "NP数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		xmlexport.addDisplayControl("displayNP");
		...
		String driskName = "";//退保的人数
		ExeSQL tExeSQL = new ExeSQL();
		String tSql = "select b.riskname from Lpgrppol a,lmrisk b where a.GrpContNo = '"+mLPGrpEdorItemSchema.getGrpContNo()+"' and a.riskcode = b.riskcode and a.EdorNo = '"+ mLPGrpEdorItemSchema.getEdorNo()+ "' and rownum = 1";
		driskName = tExeSQL.getOneValue(tSql);
		if (driskName == null || driskName.equals("")) {
			CError.buildErr(this, "查询新增险种名称失败!");
			return false;
		}
		mTextTag.add("NPRiskName", driskName);

...
import com.sinosoft.lis.claim.LLReportBLF;
...
/**
 * <p>Title: Web业务系统</p>
 * <p>Description:提交团险理赔报案信息类</p>
 * <p>Copyright: Copyright (c) 2005</p>
 * <p>Company: Sinosoft</p>
 * @author
 * @version 1.0
 */
public class LLGrpReportUI implements BusinessService
{
private static Logger logger = Logger.getLogger(LLGrpReportUI.class);
    ...
    public static void main(String[] args)
    {
        ...
        LLGrpReportUI tLLGrpReportUI = new LLGrpReportUI();
        try
        {
            if (tLLGrpReportUI.submitData(tVData, "UPDATE"))
            {
                VData tResult = new VData();
            }
            else
            {
                logger.debug(tLLGrpReportUI.mErrors.getError(0).
                                   errorMessage);
            }
        }
        catch (Exception ex)
    ...
    public boolean submitData(VData cInputData,String cOperate)
    {
        ...
        LLGrpReportBLF tLLGrpReportBLF = new LLGrpReportBLF();

        logger.debug("----------BLF BEGIN----------");
        if (tLLGrpReportBLF.submitData(mInputData,mOperate) == false)
        {
            // @@错误处理
        ...
        {
            mResult = tLLGrpReportBLF.getResult();
        }
        mInputData = null;
        return true;
    }

...
public class SysMaxNoMinSheng implements com.sinosoft.lis.pubfun.SysMaxNo {
private static Logger logger = Logger.getLogger(SysMaxNoMinSheng.class);

	public SysMaxNoMinSheng() {
	}
	...
	public String CreateMaxNo(String cNoType, String cNoLimit, VData cVData) {
		...
		if (cNoLimit.trim().toUpperCase().equals("SN")) {
			if (cNoType.equals("GRPNO")
					|| cNoType.equals("CUSTOMERNO")
					|| cNoType.equals("SUGDATAITEMCODE")
					|| cNoType.equals("SUGITEMCODE")
					|| cNoType.equals("SUGMODELCODE")
					|| cNoType.equals("SUGCODE") || cNoType.equals("ACCNO")
					|| cNoType.equals("RPTONLYNO")) {
				serialLen = 10;
			} 
			else if 
			(cNoType.equals("COMMISIONSN") )
			{
				serialLen = 10;
			}
			else if 
		...
		if (cNoType.equals("SCANNO")) {
			...
			if (tMng.length() == 2) { // 2位管理机构,前面补6个0
				tReturn = "000000" + tMng + tStr.substring(0, 4)
						+ tStr.substring(5, 7) + tStr.substring(8, 10)
						+ tBusstype;
		...
		try {
			...
			String result = null;
			result = exeSQL.getOneValue(sqlbv1);
			...
			if ((result == null) || result.equals("")) {
				strSQL = "insert into ldmaxno(notype, nolimit, maxno) values('"
				 ...
				 SQLwithBindVariables sqlbv2=new SQLwithBindVariables();
		            sqlbv2.sql(strSQL);
		            sqlbv2.put("Type", cNoType);
		            sqlbv2.put("Limit", cNoLimit);
				...
				strSQL = "update ldmaxno set maxno = maxno + 1"
						+ " where notype = '" + "?type?" + "' and nolimit = '"
						+ "?limit?" + "'";
				exeSQL = new ExeSQL(conn);
				 SQLwithBindVariables sqlbv3=new SQLwithBindVariables();
		            sqlbv3.sql(strSQL);
		            sqlbv3.put("type", cNoType);
		            sqlbv3.put("limit", cNoLimit);
				if (!exeSQL.execUpdateSQL(sqlbv3)) {
	...
	public String CreateMaxNo(String cNoType, String cNoLimit) {
		...
		if (cNoLimit.trim().toUpperCase().equals("SN")) { // modify by yt
			...
			if ( cNoType.equals("GRPNO")
					...
					|| cNoType.equals("SUGCODE") || cNoType.equals("ACCNO")
					|| cNoType.equals("RPTONLYNO")
					|| cNoType.equals("APPNTNO")
					|| cNoType.equals("BANKCODE")
					|| cNoType.equals("LCPOSITION")
		...
		if (tReturn.length() >= 4
				&& !(cNoType.toUpperCase().equals("CONTNO")
						...
						|| cNoType.equals("GETNOTICENO") || cNoType.equals("GETNO")
						|| cNoType.equals("PAYNOTICENO")
						|| cNoType.equals("GRPPROPOSALNO")
						|| cNoType.equals("GRPRPTNO")|| cNoType.equals("GRPRGTNO02")|| cNoType.equals("GRPRGTNO11")
						|| cNoType.equals("RPTNO")|| cNoType.equals("RGTNO")|| cNoType.equals("GRPRGTNO03")
...
//			tReturn = "31" + tCom;
		...
		// 保全受理号码 //6+4+1+2+1+7（机构+年份 +校验位 + 特殊字符 +防充字符 +流水号）
			...
			String tStr = PubFun.getCurrentDate();
			tReturn = cNoLimit.substring(0, 6) + tStr.substring(0, 4) 
					+ "41" ;
			// tStr.substring(8, 10);
			serialLen = 7;
		}
...
//		}
				...
				&& "ORACLE".equalsIgnoreCase(SysConst.DBTYPE))
	   {
			try 
			{
					String tSQL_SEQ = " select "
							        + "SEQ_"+ cNoType +".nextval from dual ";
		...
		try {
			...
			sqlbv6.sql(tSBql.toString()); 
			ExeSQL exeSQL = new ExeSQL(conn);
			String result = null;
			result = exeSQL.getOneValue(sqlbv6);
			...
			if ((result == null) || result.equals("")) {
				...
				sqlbv7.sql(tSBql.toString()); 
				exeSQL = new ExeSQL(conn);
				if (!exeSQL.execUpdateSQL(sqlbv7)) {
					logger.debug("CreateMaxNo 插入失败，请重试!");
					conn.rollback();
					conn.close();
				...
				sqlbv8.sql(tSBql.toString()); 
				exeSQL = new ExeSQL(conn);
				if (!exeSQL.execUpdateSQL(sqlbv8)) {
					logger.debug("CreateMaxNo 更新失败，请重试!");
					conn.rollback();
					conn.close();
...
//				//|| cNoType.toUpperCase().equals("PAYNO") 修改成5.3 的生成逻辑 modify on 2009-01-09
...
//		}
		...
		if (tReturn.equals("SN")) {
			if (cNoType.toUpperCase().equals("CUSTOMERNO")
					||cNoType.toUpperCase().equals("APPNTNO")
					||cNoType.toUpperCase().equals("BANKCODE")
					||cNoType.toUpperCase().equals("LCPOSITION")
					||cNoType.toUpperCase().equals("LCACCASCRIPTION")
					||cNoType.toUpperCase().equals("GRPPROPNO")
...
//				tReturn = "9" + tStr.trim();
	...
	public String CreateMaxNo(String cNoType, int cNoLength) {
		...
		if (cNoType.equals("COMMISIONSN") || cNoType.equals("GRPNO")
				|| cNoType.equals("CUSTOMERNO")
				|| cNoType.equals("SUGDATAITEMCODE")
				|| cNoType.equals("SUGITEMCODE")
				|| cNoType.equals("SUGMODELCODE") || cNoType.equals("SUGCODE")) {

			logger.debug("该类型流水号，请采用CreateMaxNo('" + cNoType
					+ "','SN')方式生成");
			return null;
		}
	...
	public static void main(String[] args) {
		...
		//String tPaySerialNo = PubFun1.CreateMaxNo("PAYNOTICENO", "8611002009");

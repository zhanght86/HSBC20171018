...
public class UWSendNoticeBL implements BusinessService {
private static Logger logger = Logger.getLogger(UWSendNoticeBL.class);
...
//	private String mCustomerNoType = "";// 发放对象类型(A 投保人,业务员,机构问题件,操作员问题件),I(被保人)
	...
	private LCInsuredSet mLCInsuredSet=new LCInsuredSet();//有通知书的被保人集合
	public UWSendNoticeBL() {
	}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("---UWSendNoticeBL prepareOutputData---");
		// 数据提交
		...
		logger.debug("UWSendNoticeBL Submit OK!");
		return true;
	}
	...
	private boolean checkData() {
		...
		String strSql = "select count(1) from lcissuepol where 1=1"
				...
				// 判断有没有发放给业务员问题件
				+ " and ((backobjtype = '3' and (replyresult is null or replyresult='' ) and (prtseq is null or prtseq='') "
				+ " )"
				// 判断有没有发放给保户的问题件
				+ " or (backobjtype = '2' and (replyresult is null or replyresult='') and (prtseq is null or prtseq='' ) ) "
				// 判断有没有发放给问题件修改岗的问题件
				+ " or (backobjtype = '1' and (replyresult is null or replyresult='') ) "
				// 判断有没有发放给机构的问题件
...
//		}
		...
		tExeSQL = new ExeSQL();
		int rs_change = Integer.parseInt(tExeSQL.getOneValue(sqlbv1));
		if (rs_change > 0) {		    
			flag = 1;
			mChangePolFlag = "1";
		}	
		...
		tExeSQL = new ExeSQL();
		int rs_addfee = Integer.parseInt(tExeSQL.getOneValue(sqlbv2));
		if (rs_addfee > 0) {		    
			flag = 1;
			mAddFeeFlag = "1";
		}	
		...
		strSql = "select count(1) from lccspec where proposalcontno="
			...
			+ " and (customerno is null or (customerno is not null and (prtflag is null or prtflag='')))";
		SQLwithBindVariables sqlbv3 = new SQLwithBindVariables();
		sqlbv3.sql(strSql);
		sqlbv3.put("contno", mContNo);
		tExeSQL = new ExeSQL();
		int rs_spec = Integer.parseInt(tExeSQL.getOneValue(sqlbv3));
		if (rs_spec > 0) {		    
			flag = 1;
			mSpecFlag = "1";
		}
		...
		String tSQL_exist = "select count(1) from LCRReport where contno='"
			+ "?contno?" + "' and replyflag is null";		
		SQLwithBindVariables sqlbv4 = new SQLwithBindVariables();
		sqlbv4.sql(tSQL_exist);
		sqlbv4.put("contno", mContNo);
		logger.debug("判断是否有需要放送的生调通知书 tSQL_exist:" + tSQL_exist);
		tExeSQL = new ExeSQL();
		int rs_exist = Integer.parseInt(tExeSQL.getOneValue(sqlbv4));
		if (rs_exist > 0) {		    
			flag = 1;
		}	
		...
		strSql = "select count(1) from LCPENotice where contno='"
			+ "?contno?" + "' and printflag is null ";			
		logger.debug("判断是否有需要放送的体检通知书 strSql:" + strSql);
		SQLwithBindVariables sqlbv5 = new SQLwithBindVariables();
		sqlbv5.sql(strSql);
		sqlbv5.put("contno", mContNo);
		tExeSQL = new ExeSQL();
		int rs_health = Integer.parseInt(tExeSQL.getOneValue(sqlbv5));
		if (rs_health > 0) {		    
			flag = 1;
		}
...
//		}
	...
	private boolean dealData() {
		...
		// 准备被保人核保表信息
		if (mLCInsuredSet.size()>0 && prepareIndUW() == false)
			return false;
		return true;
	}
	...
	private boolean prepareContUW() {
		...
		mLCCUWMasterSchema.setUWState("4");//核保状态更新为核保未回复
		...
		if (m > 0) {
			...
			tLCCUWSubSchema.setUWState(mLCCUWMasterSchema.getUWState());
		} else {
			// @@错误处理
			CError.buildErr(this, "LCCUWSub表取数失败!");
			return false;
	...
	private boolean prepareIndUW() {
		...
		for(int i=1;i<=this.mLCInsuredSet.size();i++){
			LCIndUWMasterDB tLCIndUWMasterDB = new LCIndUWMasterDB();
			tLCIndUWMasterDB.setContNo(mContNo);
			tLCIndUWMasterDB.setInsuredNo(mLCInsuredSet.get(i).getInsuredNo());
			if (tLCIndUWMasterDB.getInfo()) {
				LCIndUWMasterSchema tLCIndUWMasterSchema=new LCIndUWMasterSchema(); 
				tLCIndUWMasterSchema.setSchema(tLCIndUWMasterDB);
				tLCIndUWMasterSchema.setPassFlag(mUWFlag);
				tLCIndUWMasterSchema.setOperator(mOperater);
				tLCIndUWMasterSchema.setModifyDate(PubFun.getCurrentDate());
				tLCIndUWMasterSchema.setModifyTime(PubFun.getCurrentTime());
				this.mLCIndUWMasterSet.add(tLCIndUWMasterSchema);
				...
				tLCIndUWSubDB.setInsuredNo(mLCInsuredSet.get(i).getInsuredNo());
				...
				if (m > 0) {
					...
					tLCIndUWSubSchema.setInsuredNo(mLCInsuredSet.get(i).getInsuredNo());
	...
	private boolean chooseActivity() {
		...
		tsql = "select * from LCRReport where contno='"
			+ "?contno?" + "' and (replyflag is null or replyflag='')";	
		...
		tLCRReportSet = tLCRReportDB.executeQuery(sqlbv6);
		...
		tsql = "select * from lcissuepol where 1=1" + " and Contno = '"
				+ "?contno?" + "' and backobjtype = '3' and (prtseq is null or prtseq='')"
				+ " and needprint = 'Y' ";
		...
		// 查询发送给机构的问题件
		tsql = "select * from lcissuepol where 1=1" + " and Contno = '"
				+ "?contno?" + "' and backobjtype = '4' and (replyresult is null or replyresult='')"
				+ " and needprint = 'Y' ";
		...
		tsql = "select * from lcissuepol where 1=1"
				...
				+ " and (prtseq is null or prtseq='')" + " and standbyflag2 = 'Y' ";
		logger.debug("tSql==" + tsql);
...
//		}
		...
		tsql = "select * from lcissuepol where 1=1"
				+ " and Contno = '"
				+ "?contno?"
				+ "' and backobjtype = '1' and (state is null or  state='') and (replyresult is null or replyresult='')  "
				+ " and needprint = 'Y' ";
		logger.debug("tSql==" + tsql);
		tLCIssuePolSet = new LCIssuePolSet();
		...
		tsql = "select * from LCPENotice where 1=1 and Contno = '"
				+ "?contno?" + "' and  (printflag is null or printflag='') ";
		...
		tLCPENoticeSet = tLCPENoticeDB.executeQuery(sqlbv12);
	...
	private boolean preparePrt() {
		...
		if (mUWSendFlag.equals("1")) {
			for (int i = 1; i <= this.mLCIssuePolSet_UWSend.size(); i++) {
				...
				//增加备用字段，以便于在核保打印、回收时查询，
				if("0".equals(mLCIssuePolSet_UWSend.get(i).getQuestionObjType())){//发送对象是投保人
					mLOPRTManagerSchema.setStandbyFlag4("A");//客户类型
				}
				else{//发送对象时被保人
					mLOPRTManagerSchema.setStandbyFlag4("I");//客户类型
					LCInsuredSchema tLCInsuredSchema=new LCInsuredSchema();
					tLCInsuredSchema.setInsuredNo(mLCIssuePolSet_UWSend.get(i).getQuestionObj());
					...
					String tSQL_spec = "select * from lccspec where proposalcontno="
						+ "(select proposalcontno from lccont where contno='" + "?contno?" + "') and  needprint='Y' "
						+ " and (customerno='"+"?customerno?"+"' and (prtflag is null or prtflag=''))";
					SQLwithBindVariables sqlbv13 = new SQLwithBindVariables();
					sqlbv13.sql(tSQL_spec);
					sqlbv13.put("contno", mContNo);
					sqlbv13.put("customerno", mLCIssuePolSet_UWSend.get(i).getQuestionObj());
					LCCSpecDB tLCCSpecDB = new LCCSpecDB();
					LCCSpecSet tLCCSpecSet = new LCCSpecSet();
					tLCCSpecSet = tLCCSpecDB.executeQuery(sqlbv13);
					for (int j = 1; j <= tLCCSpecSet.size(); j++) {
						LCCSpecSchema tempLCCSpecSchema = new LCCSpecSchema();
						tempLCCSpecSchema.setSchema(tLCCSpecSet.get(j));
				...
				mLOPRTManagerSchema.setStandbyFlag5(mLCIssuePolSet_UWSend.get(i).getQuestionObj());//客户号
				mLOPRTManagerSchema.setStandbyFlag6(this.mChangePolFlag);//承保计划变更标记
				mLOPRTManagerSchema.setStandbyFlag7(this.mAddFeeFlag);//加费标记
				mLOPRTManagerSchema.setStandbyFlag2(this.mSpecFlag);//特约标记
			...
			LCCSpecDB tLCCSpecDB = new LCCSpecDB();
			LCCSpecSet tLCCSpecSet = new LCCSpecSet();
				tLCCSpecSet = tLCCSpecDB.executeQuery(sqlbv14);
				for (int j = 1; j <= tLCCSpecSet.size(); j++) {
					LCCSpecSchema tempLCCSpecSchema = new LCCSpecSchema();
					tempLCCSpecSchema.setSchema(tLCCSpecSet.get(j));
					tempLCCSpecSchema.setPrtFlag("0");
		...
		if (mUWSendFlag_UnPrint.equals("1")) {
			...
			for (int i = 1; i <= this.mLCIssuePolSet_UWSend_UnPrint.size(); i++) {
				...
				//增加备用字段，以便于在核保打印、回收时查询，
				if("0".equals(mLCIssuePolSet_UWSend_UnPrint.get(i).getQuestionObjType())){//发送对象是投保人
					mLOPRTManagerSchema.setStandbyFlag4("A");//客户类型
				}
				else{//发送对象是投保人
					mLOPRTManagerSchema.setStandbyFlag4("I");//客户类型
					LCInsuredSchema tLCInsuredSchema=new LCInsuredSchema();
					tLCInsuredSchema.setInsuredNo(mLCIssuePolSet_UWSend_UnPrint.get(i).getQuestionObj());
					this.mLCInsuredSet.add(tLCInsuredSchema);
				}
				mLOPRTManagerSchema.setStandbyFlag5(mLCIssuePolSet_UWSend_UnPrint.get(i).getQuestionObj());//客户号
				mLOPRTManagerSchema.setStandbyFlag6(this.mChangePolFlag);//承保计划变更标记
				mLOPRTManagerSchema.setStandbyFlag7(this.mAddFeeFlag);//加费标记
				mLOPRTManagerSchema.setStandbyFlag2(this.mSpecFlag);//特约标记
		...
		String tsql = "select * from lcissuepol where 1=1" + " and Contno = '"
				+ "?contno?" + "' and backobjtype = '1' and ( state is null or state='') "
				+ " and needprint='Y' ";
		...
		tempLCIssuePolSet = tempLCIssuePolDB.executeQuery(sqlbv15);
		...
		if (mSendReportNoticeFlag.equals("1")) {
			...
			if(this.mLCRReportSet_org.size()>0) {
				for(int i=1;i<=mLCRReportSet_org.size();i++){
				...
				// mLOPRTManagerSchema.setDoneDate() ;
				// mLOPRTManagerSchema.setDoneTime();
				mLOPRTManagerSchema.setStandbyFlag1(mLCRReportSet_org.get(1).getCustomerNo()); // 被保险人编码
		...
		if (mSendPENoticeFlag.equals("1")) {
			...
			for (int i = 1; i <= mLCPENoticeSet.size(); i++) {
				...
				// mLOPRTManagerSchema.setDoneDate() ;
				// mLOPRTManagerSchema.setDoneTime();
				mLOPRTManagerSchema.setStandbyFlag1(mLCPENoticeSet.get(i).getCustomerNo()); // 接收对象编码
				mLOPRTManagerSchema.setStandbyFlag2(mLCContSchema.getPrtNo());  
				mLOPRTManagerSchema.setStandbyFlag3(mMissionID);
...
//		.setNameAndValue("CustomerNoType", this.mCustomerNoType);
	...
	private boolean prepareOutputData() {
		...
		map.put(mLCIndUWMasterSet, "UPDATE");

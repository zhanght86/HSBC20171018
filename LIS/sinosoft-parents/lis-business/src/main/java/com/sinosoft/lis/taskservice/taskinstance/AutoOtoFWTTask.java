...
import com.sinosoft.lis.otof.OtoFWTBL;
 ...
 */
public class AutoOtoFWTTask extends TaskThread {
private static Logger logger = Logger.getLogger(AutoOtoFWTTask.class);
	// private static String[] Otofargs;
	public CErrors mErrors = new CErrors();

	public AutoOtoFWTTask() {
	}

	public boolean dealMain() {
		logger.debug("＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝  OtoFTask Execute !!! ＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝");
		/* 业务处理逻辑 */
		logger.debug("进入业务逻辑处理AutoOtoFWTTask!");
...
//		bdate = "2008-11-26";
		...
		if (flag.equals("true")) {
			...
			for (int i = 1; i <= 8; i++) {
				...
				if (!tOtoFWTBL.submitData(vData, "Buss")) {
					CErrors cerr = tOtoFWTBL.mErrors;
	...
	public static void main(String[] args) {
		...
		AutoOtoFWTTask tOtoFTask = new AutoOtoFWTTask();

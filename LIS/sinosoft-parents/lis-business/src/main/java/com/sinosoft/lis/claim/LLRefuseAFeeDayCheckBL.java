 ...
 * 
 * @author
 * @version 1.0 niuzj,2006-01-17,增加“制表时间”的显示
 */

public class LLRefuseAFeeDayCheckBL implements PrintService {
private static Logger logger = Logger.getLogger(LLRefuseAFeeDayCheckBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("-----理赔日结-->犹豫期外拒赔全额退费日结开始---------LLRefuseAFeeDayCheckBL---");
		// 得到外部传入的数据,将数据备份到本类中
		...
		logger.debug("-----理赔日结-->犹豫期外拒赔全额退费日结结束----LLRefuseAFeeDayCheckBL---");
	...
	private boolean dealData() {
		...
		tXmlExport.createDocument("RefuseAFeeDayCheck.vts", "");
		...
		String strSQL = " select distinct b.riskcode,(select riskname from lmrisk where riskcode=b.riskcode) "
				...
				+ " ,("
				+ " select sum((case when j.pay is null then 0 else j.pay end)) from ljagetclaim j where 1=1  and  j.managecom like concat('"
				+ "?staticode?"
				+ "','%')"
				+ " and j.feefinatype = 'TF' and j.feeoperationtype  in('D01','D06')"
				...
				+ " and a.feefinatype = 'TF' and a.feeoperationtype in('D01','D06') and a.managecom like concat('"
				+ "?staticode?"
				+ "','%') "
		...
		tTextTag.add("AgenTime", SysDate);// $=/AgenTime$----当前时间
		...
		// tTextTag.add("SumMonthly",str_tSumMonthly);//$=/SumMonthly---总计----本月应付
		tTextTag.add("StartDate", mStartDate);
		tTextTag.add("EndDate", mEndDate);
		tTextTag.add("Operator", tOperatorName);
		tTextTag.add("CurrentDate", SysDate);

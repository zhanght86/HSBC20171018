...
public class MeetF1PBL {
	...
	private void getPrintData4() throws Exception {
		...
		if (tLOPRTManagerDB.getStateFlag().equals("0")) {
			...
			logger.debug("test by yaory:" + mLOPRTManagerSchema.getOtherNo());
			...
			sqlbv2.sql("select askcode from LCRReportPrt where prtseq='"
					+ "?prtseq?" + "'");
	...
	private void getPrintData() throws Exception {
		...
		if (tLOPRTManagerDB.getStateFlag().equals("0")) {
			...
			if (mOldPrtSeq != null && !mOldPrtSeq.equals("")) {
				...
				sqlbv4.sql("select askcode from LCRReportPrt where contno='"
						+ "?mPrtSeq?" + "'");
	...
	private void getPrintDataMeet() throws Exception {
		...
		if (tLOPRTManagerDB.getStateFlag().equals("0")) {
			...
			texttag.add("UWOperator", mLOPRTManagerSchema.getReqOperator());// 核保员2008-11-18
			// ln
			// add

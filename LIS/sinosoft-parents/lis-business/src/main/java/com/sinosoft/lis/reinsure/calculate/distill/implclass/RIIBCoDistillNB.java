 ...
 * @version 1.0
 */
public class RIIBCoDistillNB implements RIDistill {
	String mEventType;
	...
	private String getDistillSQL() {
		...
		strBuffer.append(" a.PolNo||','||d.DutyCode||',1,'||'"
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=d.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
		...
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecord m where m.unionkey=(a.PolNo||','||d.DutyCode||',1'||','||'"
						+ mAccumulateDefNo + "'||','||'NB')) ");
		strBuffer
				.append(" and not exists (Select 'X' From RIPolRecordBake n where n.unionkey=(a.PolNo||','||d.DutyCode||',1'||','||'"
						+ mAccumulateDefNo + "'||','||'NB')) ");
		// 不为临分的保单
		strBuffer
		...
		strBuffer.append(" and greatest(a.cvalidate,a.Signdate) between '");// 保单生效日与签单日
		strBuffer.append(mRIWFLogSchema.getStartDate());
		strBuffer.append("' and '");
		strBuffer.append(mRIWFLogSchema.getEndDate());
		strBuffer.append("' ) ");
		System.out.println("共保IBU00新契约提数sql: " + strBuffer.toString());
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=d.DutyCode and AddFeeDirect='01' and PayEndDate>'"+mRIWFLogSchema.getEndDate()+"') as ExtRate1, ");
		...
		strBuffer.append(mRIWFLogSchema.getEndDate());
		strBuffer.append("' ) ");
		System.out.println("共保IBU00新契约临分提数sql: " + strBuffer.toString());
		return strBuffer.toString();
	}
...
/*		strBuffer.append(" (select sum(SuppRiskScore) from LCPrem where polno=a.polno and DutyCode=d.DutyCode and AddFeeDirect='01' and PayEndDate>'"+monthDate[1]+"') as ExtRate1, ");
	...
	public static void main(String[] args) {
		...
		RIIBCoDistillNB tRIDistillNB = new RIIBCoDistillNB();
		tRIDistillNB.submitData(cInputData, null);
		// System.out.print(PubFun.getCurrentDate().substring(0,4));
	}
}

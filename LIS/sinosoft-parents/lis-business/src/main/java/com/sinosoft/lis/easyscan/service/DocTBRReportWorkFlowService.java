...
public class DocTBRReportWorkFlowService extends BaseService {
private static Logger logger = Logger.getLogger(DocTBRReportWorkFlowService.class);
	...
	public boolean deal(){
		...
		String MissionSQL="select * from lwmission where missionprop3='"+"?missionprop3?"+"' and activityid in(select activityid from lwactivity where functionid in('10010031'))";
		...
		tTransferData.setNameAndValue("CertifyNo", mES_DOC_MAINSchema
				.getDocCode());
		tTransferData.setNameAndValue("CertifyCode", PrintManagerBL.CODE_MEET);// guomy
																				// modified
																				// 20050809
		...
		tTransferData.setNameAndValue("PrtSeq", mLWMissionSet.get(1)
				.getMissionProp3());
		tTransferData.setNameAndValue("OldPrtSeq", mLWMissionSet.get(1)
				.getMissionProp14());

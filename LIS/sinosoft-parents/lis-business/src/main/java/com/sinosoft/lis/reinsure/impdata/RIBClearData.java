...
public class RIBClearData {
    ...
    private boolean dealData() {
    	try{
	    	String sql = " delete from ripolrecordbake a where a.batchno='0000000000'";
	        ExeSQL tExeSQL = new ExeSQL();
	        if(!tExeSQL.execUpdateSQL(sql)){
	        	buildError("dealData", "删除业务数据失败 ");
	        	return false;
	        }
    	}catch(Exception ex){
    		buildError("dealData", "删除业务数据失败 "+ex.getMessage());
    	}       
        return true;
    }
    ...
    private void buildError(String szFunc, String szErrMsg) {
        ...
        cError.moduleName = "RIBClearData";

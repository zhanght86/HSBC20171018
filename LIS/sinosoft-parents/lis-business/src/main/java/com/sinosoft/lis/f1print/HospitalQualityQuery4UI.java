...
public class HospitalQualityQuery4UI {
private static Logger logger = Logger.getLogger(HospitalQualityQuery4UI.class);
	public CErrors mErrors = new CErrors();
	private VData mResult = new VData();

	public HospitalQualityQuery4UI() {
	}

	public boolean submitData(VData cInputData, String cOperate) {
		HospitalQualityQuery4BL tHospitalQualityQuery4BL = new HospitalQualityQuery4BL();
		if (!tHospitalQualityQuery4BL.submitData(cInputData, cOperate)) {
			mErrors.copyAllErrors(tHospitalQualityQuery4BL.mErrors);
			return false;
		} else {
			mResult = tHospitalQualityQuery4BL.getResult();

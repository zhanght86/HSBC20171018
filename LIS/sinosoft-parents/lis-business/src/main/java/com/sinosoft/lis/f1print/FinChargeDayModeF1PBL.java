...
public class FinChargeDayModeF1PBL {
private static Logger logger = Logger.getLogger(FinChargeDayModeF1PBL.class);
...
*/
  ...
  public static void main(String[] args)
  {
    ...
    tFinChargeDayModeF1PBL.submitData(tVData,"PRINT");
  }
...
//全局变量
  ...
  private boolean getPrintData()
  {
    ...
    msql="select LDCode.CodeName,sum(LJTempFeeClass.PayMoney),sum(1) from LJTempFeeClass,LDCode where LDCode.CodeType='paymode' and LDCode.Code=LJTempFeeClass.PayMode" 
    + " and LJTempFeeClass.MakeDate>='"+"?date1?"+"' and LJTempFeeClass.MakeDate<='"+"?date2?"+"' and LJTempFeeClass.Operator='"+"?Operator?"+"' Group By LDCode.CodeName ";
    //判断操作员位数是否为8位
    sqlbv1.sql(msql);
    ...
    nsql="select Name from LDCom where ComCode='" + "?ComCode?" +"'";
    sqlbv2.sql(nsql);
    ...
    xmlexport.createDocument("FinChargeDayMode.vts","printer");//最好紧接着就初始化xml文档

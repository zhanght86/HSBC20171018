...
public class PEdorBCDetailBLF implements EdorDetail {
private static Logger logger = Logger.getLogger(PEdorBCDetailBLF.class);
	...
	private boolean dealData() {
		...
		if (!tPEdorBCDetailBL.submitData(rInputData, rOperation)) {
			mErrors.copyAllErrors(tPEdorBCDetailBL.getErrors());
			CError.buildErr(this, "处理提交的数据失败！");
			logger.debug("\t@> PEdorBCDetailBLF.dealData() : PEdorBCDetailBL.submitData() 失败！");
			return false;
		}
		rResultData = new VData();
		rResultData = tPEdorBCDetailBL.getResult();
		tPEdorBCDetailBL = null;

		// logger.debug("\t@> PEdorBCDetailBLF.dealData() 成功");
		return true;
	...
	private boolean pubSubmit() {
		// logger.debug("\t@> PEdorBCDetailBLF.pubSubmit() 开始");

		PubSubmit tPubSubmit = new PubSubmit();
		if (!tPubSubmit.submitData(rResultData, rOperation)) {
			CError.buildErr(this, "保存提交的信息到数据库失败！");
			logger.debug("\t@> PEdorBCDetailBLF.pubSubmit() : PubSubmit.submitData() 失败！");
			return false;
		}
		tPubSubmit = null;

		// logger.debug("\t@> PEdorBCDetailBLF.pubSubmit() 成功");
		return true;

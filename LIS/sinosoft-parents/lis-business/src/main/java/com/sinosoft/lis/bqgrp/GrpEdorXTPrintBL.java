...
public class GrpEdorXTPrintBL implements EdorPrint {
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		// 打印数据处理
		if (!dealData()) {
			CError.buildErr(this, "XT数据提取失败!");
			return false;
		}

		// 准备数据
		if (!prepareData()) {
			CError.buildErr(this, "XT数据生成失败!");
			return false;
		}
		return true;
	}
	...
	private boolean dealData() {
		...
		ListTable tXTListTable = new ListTable();
		tXTListTable.setName("XT");
		String tSql = "Select distinct b.Name, "
				+ "decode(b.Sex, '0', '男', '1', '女', '不详'), " + "b.Birthday, "
				+ "b.Idno,c.RiskName, " + "a.Amnt, " + "d.GetMoney, " + "'"
				+ mLPGrpEdorItemSchema.getEdorValiDate() + "' "
				+ "from LPPol a, LCInsured b, LMRiskApp c, LPEdorItem d "
				+ "where a.InsuredNo = b.InsuredNo "
				+ "and a.ContNo = b.ContNo " + "and a.RiskCode = c.RiskCode "
				+ "and a.EdorNo = d.EdorNo " + "and a.PolNo = d.PolNo "
				+ "and a.EdorType = d.EdorType " + "and a.GrpContNo = '"
		...
		for (int i = 1; i <= tArrLen; i++) {
			String[] strXT = new String[8];
			for (int k = 1; k <= strXT.length; k++) {
				strXT[k - 1] = rSSRS.GetText(i, k);
			}
			tXTListTable.add(strXT);
		}
		...
		mTextTag.add("XTNUM", dNum);
		mTextTag.add("XTSUM", dSum);
		...
		String[] b_strXT = new String[2];
		xmlexport.addListTable(tXTListTable, b_strXT);

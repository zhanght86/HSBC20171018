...
public class NoBackPolQueryLisBL {
private static Logger logger = Logger.getLogger(NoBackPolQueryLisBL.class);
	...
	private boolean getPrintData() {
		...
		xmlexport.createDocument("NoBackPolQueryLis.vts", "printer");
		TextTag aTextTag = new TextTag();
		ListTable aListTable = new ListTable();
		aListTable.setName("LIST");
		String strSQL ="";
		if(mBPOID==null||mBPOID.equals("")){
			strSQL = " order by makedate,maketime ";
		}else{
			strSQL = " and substr(managecom,1,4) in (select(managecom) "
				+" from bpoallotrate where (BPOID) = '"+"?mBPOID?"+"') "
		...
		if (tComLength <= 6) {
			...
			strComSQL = "select a.comcode,a.name,a.upcomcode,"
					+ "(select name from ldcom b where b.comcode=a.upcomcode)"
					+ " from ldcom a where a.comgrade='03'"
					+ " and a.comcode like concat('" + "?mManageCom1?" + "','%')" + " order by a.comcode";
		}
		...
		for (int i = 1; i <= Count_ComSSRS; i++) {
			...
			String strListSQL ="select doccode,managecom,to_char(makedate,'yyyy-mm-dd'),maketime "
							   + " from es_doc_main "
							   ...
							   +ReportPubFun.getWherePartLike("ManageCom", ReportPubFun.getParameterStr(managecom,"?managecom?"))
				              ;
			strListSQL = strListSQL +strSQL;
			SQLwithBindVariables sqlbv2 = new SQLwithBindVariables();
			sqlbv2.sql(strListSQL);
			sqlbv2.put("mBPOID", mBPOID);

 ...
 * @version 6.0
 */
public class UWAgentQualityRecordUI implements BusinessService {
private static Logger logger = Logger.getLogger(UWAgentQualityRecordUI.class);
	/** 传入数据的容器 */
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		UWAgentQualityRecordBL tUWAgentQualityRecordBL = new UWAgentQualityRecordBL();

		logger.debug("---UI BEGIN---");
		if (tUWAgentQualityRecordBL.submitData(cInputData, mOperate) == false) {
			// @@错误处理
			this.mErrors.copyAllErrors(tUWAgentQualityRecordBL.mErrors);
			return false;
		} else {
			mResult = tUWAgentQualityRecordBL.getResult();
		}

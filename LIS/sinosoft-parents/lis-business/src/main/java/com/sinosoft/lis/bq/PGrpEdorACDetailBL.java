...
import com.sinosoft.lis.bl.LPGrpBL;
...
private static Logger logger = Logger.getLogger(PGrpEdorACDetailBL.class);
	...
	private boolean prepareData() {
		...
		// tLPGrpAppntBL.setGrpContNo(mLPGrpAppntSchema.getGrpContNo());
		// //集体投保单号码
		...
		tLPGrpContBL.setRgtMoney(mLPGrpSchema.getRgtMoney()); // 注册资本
		tLPGrpContBL.setAsset(mLPGrpSchema.getAsset()); // 资产总额
		tLPGrpContBL.setNetProfitRate(mLPGrpSchema.getNetProfitRate()); // 净资产收益率
		tLPGrpContBL.setMainBussiness(mLPGrpSchema.getMainBussiness()); // 主营业务
		tLPGrpContBL.setCorporation(mLPGrpSchema.getCorporation()); // 法人
		tLPGrpContBL.setComAera(mLPGrpSchema.getComAera()); // 机构分布区域
		tLPGrpContBL.setPhone(mLPGrpSchema.getPhone()); // 总机
		tLPGrpContBL.setFax(mLPGrpSchema.getFax()); // 传真
		tLPGrpContBL.setFoundDate(mLPGrpSchema.getFoundDate()); // 成立时间
		...
		if (!StrTool.compareString(mLPGrpAddressSchema.getAddressNo(), "")) {
			...
			if (!tExeSQL.mErrors.needDealError()
				...
				sql = " select max(AddressNo) from LPGrpAddress a,LPGrpEdorItem b where a.CustomerNo='"
						+ mLPGrpAddressSchema.getCustomerNo()
						+ "' and a.edorno=b.edorno" + " and b.EdorState='2'";
				tSSRS = tExeSQL.execSQL(sql);
				if (!tExeSQL.mErrors.needDealError()
						&& StrTool.compareString(tSSRS.GetText(1, 1), "")) {
		...
		if ("MOD".equals(addrFlag)) {
			...
			// tLPGrpAddressBL.setAddressNo(mLPGrpAddressSchema.getAddressNo());
			// //地址号码
			...
			tLPGrpAddressBL
					.setDepartment1(mLPGrpAddressSchema.getDepartment1());
			...
			tLPGrpAddressBL
					.setDepartment2(mLPGrpAddressSchema.getDepartment2());

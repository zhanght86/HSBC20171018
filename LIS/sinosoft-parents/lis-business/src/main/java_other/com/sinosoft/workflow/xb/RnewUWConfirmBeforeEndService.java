...
public class RnewUWConfirmBeforeEndService implements BeforeEndService {
private static Logger logger = Logger.getLogger(RnewUWConfirmBeforeEndService.class);
	...
	public RnewUWConfirmBeforeEndService() {
	}

	public static void main(String[] args) {
		RnewUWConfirmBeforeEndService UWConfirmBeforeEndService1 = new RnewUWConfirmBeforeEndService();
	}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		if(mNoticeFlag.equals("0")) //没有发放过拒保通知书才需要处理
		{
			// 进行业务处理
			if (!dealData()) //发放拒保通知书
			{
	...
	private boolean getInputData(VData cInputData, String cOperate) {
		...
		this.mNoticeFlag = (String) mTransferData.getValueByName("NoticeFlag");
	...
	private boolean dealData() {
		...
		tLOPRTManagerSchema.setStandbyFlag7("XBJB");// 投保拒保类型
		...
		tLOPRTManagerSchema.setStandbyFlag5(mCvaliDate); //置上该期投保单生效日期，生成拒保通知书时会用该信息
		...
		if (this.mUWFlag.equals("1")) {
			// 拒保
			tLOPRTManagerSchema.setCode("RE00");// 拒保通知书
			//ln 2008-10-18 add

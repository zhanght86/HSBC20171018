...
public class RnewPrintAutoHealthAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(RnewPrintAutoHealthAfterInitService.class);
	...
	public RnewPrintAutoHealthAfterInitService() {
	}
	...
	private boolean prepareOutputData() {
		...
		if (mPatchFlag == false) {
			...
			map.put(mRnewPENoticeSchema, "UPDATE");
		}
...
//			return false;
	...
	private boolean dealMission(){
		...
		xLWMissionDB.setActivityID("0000007007");
...
//			if(!tLWMissionDB.getInfo()){
	...
	private boolean preparePolUW() {
		...
		for (int i = 1; i <= mLCPolSet.size(); i++) {
			...
			tLCUWMasterSchema.setHealthFlag("2");
	...
	private boolean preparePrintAutoHealth() {
		...
		if (mPatchFlag == false) // 不是补打，则更新数据状态
		{
			...
			tRnewPENoticeDB.setPrintFlag("0");
			tRnewPENoticeDB.setCustomerNo(mLOPRTManagerSchema.getStandbyFlag1()); // 备用字段1存放客户号
			...
			mRnewPENoticeSchema = tRnewPENoticeSet.get(1);
			mRnewPENoticeSchema.setPrintFlag("1");

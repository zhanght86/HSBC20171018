...
import com.sinosoft.lis.claim.LLBnfGatherBL;
...
private static Logger logger = Logger.getLogger(ClaimWorkFlowBL.class);
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("---ClaimWorkFlowBL After checkData---");
	...
	/**
	 * 1、报案确认时，对于年金及分红型类年金产品，系统给予提示：先进行帐户结息及分红，在进行理赔操作
	 * 
	 * 2、报案确认时，对借款未还款保单给予提示：有未还款不能理赔操作，并不予确认
	 */
	private boolean checkData() {
		if (mOperate.trim().equals("0000009005")) {// 报案确认
			// 1、对于年金及分红型类年金产品，系统给予提示：先进行帐户结息及分红，在进行理赔操作
			String tGrpContNo = (String) mTransferData.getValueByName("GrpContNo");
			String CurrentDate = PubFun.getCurrentDate();
			ExeSQL tExeSQL = new ExeSQL();
			String sql = "select a.grppolno,a.cvalidate from lcgrppol a where a.grpcontno='" + tGrpContNo
					+ "'";
			SSRS tssrs = tExeSQL.execSQL(sql);
			for (int i = 1; i <= tssrs.getMaxRow(); i++) {
//				int tPolYear = Integer.parseInt(CurrentDate.substring(0, 4))
//						- Integer.parseInt(tssrs.GetText(i, 2).substring(0, 4)) + 1;
				String cGrpPolNo = tssrs.GetText(i, 1);
//				sql = "select '1' from lcgrppol a where grppolno='"
//						+ cGrpPolNo
//						+ "' and ('"
//						+ tPolYear
//						+ "'<=1 or exists (select '1' from lmriskapp where bonusflag='0' and riskcode=a.riskcode)) "
//						+ " union "
//						+ " select '1' from lcgrppol a where grppolno='"
//						+ cGrpPolNo
//						+ "' and '"
//						+ tPolYear
//						+ "'>1 "
//						+ " and exists (select '1' from lmriskapp where bonusflag='1' and riskcode=a.riskcode) "
//						+ " and exists (select '1' from lobonusgrppolparm where grppolno=a.grppolno and fiscalyear="
//						+ CurrentDate.substring(0, 4) + "-1 " + " and computestate='2' ) ";
				//ssx 2009-09-28 调整校验分红的逻辑
				sql = "select '1' from lcgrppol a where grppolno='"
					+ cGrpPolNo
					+ "' and ("
					+ " (months_between(to_date('"+CurrentDate+"','YYYY-MM-DD'),a.cvalidate)/12)"
					+ "<=1 or exists (select '1' from lmriskapp where bonusflag='0' and riskcode=a.riskcode)) "
					+ " union "
					+ " select '1' from lcgrppol a where grppolno='"
					+ cGrpPolNo
					+ "' and (months_between(to_date('"+CurrentDate+"','YYYY-MM-DD'),a.cvalidate)/12)>1 "
					+ " and exists (select '1' from lmriskapp where bonusflag='1' and riskcode=a.riskcode) "
					+ " and exists (select '1' from lobonusgrppolparm where grppolno=a.grppolno and fiscalyear="
					+ " (substr(a.cvalidate,1,4)+floor(months_between(to_date('"+CurrentDate+"','YYYY-MM-DD'),a.cvalidate)/12)-1) and computestate='2' )";
				...
				if (tFlag == null || tFlag.equals("")) {
					...
					tError.errorMessage = "该团单没有做过上一年度分红，不能进行报案操作！";
					this.mErrors.addOneError(tError);
					return false;
				}

				sql = "select PolNo,InsuAccNo,BalaDate,InsuAccBala,PayPlanCode from LCInsureAccClass a "
						+ "where grppolno='"
						+ cGrpPolNo
						+ "' and insuaccno in ('000001','000002','000003') "
						...
						+ " and payplancode = a.payplancode and fiscalyear = '" + CurrentDate.substring(0, 4)
						+ "') " + "order by polno";
				SSRS tSSRS = tExeSQL.execSQL(sql);
				for (int j = 1; j <= tSSRS.getMaxRow(); j++) {
					String PolNo = tSSRS.GetText(i, 1);
					String InsuAccNo = tSSRS.GetText(i, 2);
					String strBalaDate = tSSRS.GetText(i, 3);
					String calDate = String.valueOf( Integer.parseInt(CurrentDate.substring(0, 4))-1) + "-12-31";
					if (PubFun.calInterval(strBalaDate, calDate, "D") > 0) {
						CError tError = new CError();
						tError.moduleName = "ClaimWorkFlowBL";
						tError.functionName = "checkData";
						tError.errorMessage = "保单 " + PolNo + " 的 " + InsuAccNo + " 帐户还没有计算上一年度的利息，不能进行报案操作！";
						this.mErrors.addOneError(tError);
						return false;
					}
				}
			...
			// 2、对借款未还款保单给予提示：有未还款不能理赔操作，并不予确认
			sql = "select 1 from LOLoan where contno = '" + tGrpContNo
					+ "' and LoanType = '0' and PayOffFlag = '0'";
			String tLoanFlag = tExeSQL.getOneValue(sql);
			if ("1".equals(tLoanFlag)) {
				CError tError = new CError();
				tError.moduleName = "ClaimWorkFlowBL";
				tError.functionName = "checkData";
				tError.errorMessage = "团单 " + tGrpContNo + "还有未还借款，不能进行报案操作！";
				this.mErrors.addOneError(tError);
				return false;
			}
		}
	...
	private boolean dealData() {
		...
		} else if (mOperate.trim().equals("0000009125")) {
...
//				}
	...
	private boolean Execute9999999999() {
		...
		if (tLLReportBL.submitData(mInputData, mOperate)) {
			...
			try {
				// 产生报案确认节点
				if (tActivityOperator.CreateStartMission_NoScan("0000000005", "0000005005", tempVData1)) {
	...
	private boolean Execute9899999999() {
		...
		if (tLLClaimRegisterBLF.submitData(mInputData, "insert||first")) {
			VData tempVData1 = new VData();
			tempVData1 = tLLClaimRegisterBLF.getResult();
			cInputData.add(tempVData1);
			// 向前台返回处理完的参数以便查询工作流
			mTransferData = null;
			mTransferData = (TransferData) tempVData1.getObjectByObjectName("TransferData", 0);
			...
			try {
				// 产生报案确认节点
				if (tActivityOperator.CreateStartMission_NoScan("0000000009", "0000009015", tempVData1)) {
					VData tempVData2 = new VData();
					tempVData2 = tActivityOperator.getResult();
					cInputData.add(tempVData2);
					tempVData2 = null;
	...
	private boolean Execute8999999999() {
		...
		if (tLLSubmitApplyBL.submitData(mInputData, "INSERT")) {
			...
			try {
				// 产生报案确认节点
				if (tActivityOperator.CreateStartMission_NoScan("0000000005", "0000005105", tempVData1)) {
					VData tempVData2 = new VData();
					tempVData2 = tActivityOperator.getResult();
					cInputData.add(tempVData2);
					tempVData2 = null;
	...
	/**
	 * 创建理赔调查报告任务节点,同时调用业务处理类保存业务数据输出：如果没有得到足够的业务数据对象，则返回false,否则返回true
	 */
	private boolean Execute8899999999() {
		...
		if (tLLInqApplyBL.submitData(mInputData, "INSERT")) {
			...
			try {
				// 产生报案确认节点
				if (tActivityOperator.CreateStartMission("0000000009", "0000009125", tempVData1)) {
					VData tempVData2 = new VData();
					tempVData2 = tActivityOperator.getResult();
					cInputData.add(tempVData2);
					tempVData2 = null;
	...
	private boolean Execute0000005035() {
		...
		// tongmeng 2009-01-08 add
		// 增加审核结论
		String tAuditConclusion = mTransferData.getValueByName("AuditConclusion") == null ? ""
				: (String) mTransferData.getValueByName("AuditConclusion");
		logger.debug("@@@@@tAuditConclusion:" + tAuditConclusion);
		if (tClmNo == null) {
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "ClaimWorkFlowBL";
		...
		try {
			...
			mTransferData.setNameAndValue("RptNo", tClmNo);
			mTransferData.setNameAndValue("PrepayFlag", tPrepayFlag); // 是否预付，0-非预付，1-预付
			...
			logger.debug("审核结论为: " + tAuditConclusion);
			// if (tResult.equals("0") || tResult.equals("1") ||
			// tBudgetFlag.equals("1"))
			// 需要审批的案件才生成下个节点 2005-11-14 pd
			if (tAuditConclusion.equals("0")||tAuditConclusion.equals("4")|| tAuditConclusion.equals("1")) {
				// 审核结论为4 - 审批管理 需要指定审核人，所以单独执行Execute02()方法
				if (!Execute02()) {
					// @@错误处理
					this.mErrors.copyAllErrors(mActivityOperator.mErrors);
					return false;
	...
	private boolean Execute0000005145() {
		...
		try {
			...
			strSQL = "select ClmNO,InqNo,BatNo,CustomerNo,Customername,VIPFlag,InqDept,InitPhase,InqRCode,InqItem from llInqapply"
					...
					+ "'"
					+ " and BatNo='"
					+ tBatNo + "'" + " and InqDept='" + tInqDept + "'" + " and InqState='0'";
			logger.debug("--strSQL=" + strSQL);
			ExeSQL exesql = new ExeSQL();
			String tResult = exesql.getOneValue(strSQL);
			logger.debug("-----判断事件是否可以生成下一个节点tResult.length()= " + tResult.length());
			if ((tResult.length() == 0)) {
				// 消亡调查过程录入节点，并生成下一个节点
				logger.debug("-----可以生成下一个节点------");
				if (!Execute()) {
	...
	private boolean Execute0000005165() {
		...
		try {
			...
			String strSql = "select * from llinqconclusion where 1=1  " + " and ClmNO='" + tClmNo + "'"
					+ " and ConNo!='" + tConNo + "'" + " and FiniFlag='0' and ColFlag='2' "
					+ " order by clmno";
			...
			String strlwmisSql = "select * from lwmission where 1=1" + " and processid='0000000005'"
					+ " and activityid='0000005175'" + " and missionprop6='0'" + " and missionprop1='"
					+ tClmNo + "'";
		...
		if (tLLSecondUWBL.submitData(mInputData, mOperate)) {
			...
			try {
				...
				if (tClaimRelFlag.equals("0") || tClaimRelFlag.equals("1")) {
					...
					// 产生[发起二核]节点
					ActivityOperator tActivityOperator = new ActivityOperator();
					if (tActivityOperator.CreateStartMission("0000000005", "0000005505", tempVData1)) {
						VData tempVData2 = new VData();
						tempVData2 = tActivityOperator.getResult();
						// MMap tMap2 = new MMap();
						// tMap2 = (MMap)
						// tempVData2.getObjectByObjectName("MMap",
						// 0);
				...
				if (tClaimRelFlag.equals("2")) {
					...
					// 产生[发起二核]节点
					logger.debug("------相关标志------" + mTransferData.getValueByName("ClaimRelFlag"));
					if (tActivityOperator.CreateStartMission("0000000005", "0000005505", tempVData1)) {
						VData tempVData2 = new VData();
						tempVData2 = tActivityOperator.getResult();
						// MMap tMap2 = new MMap();
					...
					logger.debug("------相关标志------" + mTransferData.getValueByName("ClaimRelFlag"));
					// 产生[发起二核]节点
					if (ttActivityOperator.CreateStartMission("0000000005", "0000005505", tempVData1)) {
				...
				if (tVData != null) {
					for (int i = 0; i < tVData.size(); i++) {
						...
						for (int j = 0; j < tempVData.size(); j++) {
							...
							tActivityOperator = new ActivityOperator();
							if (tActivityOperator.CreateStartMission("0000000000", "0000000000", tempVData1)) {
								VData tempVData2 = new VData();
								tempVData2 = tActivityOperator.getResult();
	...
	private boolean Execute02() {
		...
		try {
			...
			// 产生下一个任务节点，并置上默认操作人
			if (tActivityOperator.CreateNextMissionSpecial(tMissionID, tSubMissionID, mOperate, mInputData,
					tOperator)) {
				VData tempVData = new VData();
				tempVData = tActivityOperator.getResult();
				if ((tempVData != null) && (tempVData.size() > 0)) {

...
public class UWPrintAllNoticeAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(UWPrintAllNoticeAfterInitService.class);
	...
	public UWPrintAllNoticeAfterInitService() {
	}
	...
	private boolean checkData() {
		...
		if (!tLCContDB.getInfo()) {
			...
			tError.moduleName = "UWPrintAllNoticeAfterInitServer";
	...
	/**
	 * prepareBL 准备打印业务逻辑处理
	 * 
	 * @return boolean
	 */
	private boolean prepareBL() {
		return true;
	}

	/**
	 * prepareAutoSysCertSendOut 准备机打单证数据
	 * 
	 * @return boolean
	 */
	private boolean prepareAutoSysCertSendOut() {
		...
		if (mAutoSysCertSendOutFlag == true) { // 需要自动发放，则更新数据状态
			mLZSysCertifySchema.setCertifyCode(tLDCodeDB.getCodeName());
			mLZSysCertifySchema.setCertifyNo(mPrtSeq);
			mLZSysCertifySchema.setSendOutCom("A" + mGlobalInput.ManageCom);
			mLZSysCertifySchema.setReceiveCom("D"
					+ mLOPRTManagerSchema.getAgentCode());
			logger.debug("D" + mLOPRTManagerSchema.getAgentCode());
			mLZSysCertifySchema.setHandler("SYS");
			mLZSysCertifySchema.setStateFlag("0");
			mLZSysCertifySchema.setHandleDate(PubFun.getCurrentDate());
			mLZSysCertifySchema.setTakeBackNo(PubFun1.CreateMaxNo("TAKEBACKNO",
					PubFun.getNoLimit(mManageCom.substring(1))));
			mLZSysCertifySchema.setSendNo(PubFun1.CreateMaxNo("TAKEBACKNO",
					PubFun.getNoLimit(mManageCom.substring(1))));
			mLZSysCertifySchema.setOperator(mOperater);
			mLZSysCertifySchema.setMakeDate(PubFun.getCurrentDate());
			mLZSysCertifySchema.setMakeTime(PubFun.getCurrentTime());
			mLZSysCertifySchema.setModifyDate(PubFun.getCurrentDate());
			mLZSysCertifySchema.setModifyTime(PubFun.getCurrentTime());
		}
		return true;
	}

	/**
	 * preparePrint 准备打印管理表数据
	 * 
	 * @return boolean
	 */
	...
	private boolean prepareTransferData() {
		...
		mTransferData.setNameAndValue("Code", PrintManagerBL.CODE_DECLINE);

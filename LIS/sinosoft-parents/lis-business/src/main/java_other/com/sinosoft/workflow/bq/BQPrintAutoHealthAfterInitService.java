...
public class BQPrintAutoHealthAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(BQPrintAutoHealthAfterInitService.class);
	...
	public BQPrintAutoHealthAfterInitService() {
	}
	...
	private boolean checkData() {
		...
		tLPContDB.setEdorNo(tLWMissionSchema.getMissionProp8());
		tLPContDB.setEdorType(tLWMissionSchema.getMissionProp9());
	...
	private boolean prepareAutoSysCertSendOut() {
		...
		if (!tLMCertifySubDB.getInfo()) {
			...
			CError.buildErr(this, "查询单证回收描述信息表失败!") ;
			return false;
		}

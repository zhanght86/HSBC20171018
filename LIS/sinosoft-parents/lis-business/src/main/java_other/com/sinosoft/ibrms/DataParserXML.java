...
{
private static Logger logger = Logger.getLogger(DataParserXML.class);
     ...
     *
     */
    public DataParserXML(int sheetCount)
    {
    ...
    public boolean transform()
    {
        ...
        try
        {
            ...
            logger.debug("=befor==verify== nCurRow ==" + nCurRow);
            logger.debug("=befor==verify== nMaxRow ==" + nMaxRow);
            ...
            logger.debug("=after==verify== nCurRow ==" + nCurRow2);
            logger.debug("=after==verify== nMaxRow ==" + nMaxRow2);
            
            //判断转换是否完毕
//            while (hasMoreData())
//            {
//                strFileName = m_strPathName + File.separator + m_strBatchNo + String.valueOf(nCount++) + ".xml";
//                logger.debug("-----准备生成的Xml文件名:" + strFileName);
//                genPart(strFileName);
//
//                //用来保存每一个小部分生成的XML文件名
...
//        }
        ...
        for (int nIndex = 0; nIndex < SHEET_COUNT; nIndex++)
        {
            ...
            logger.debug("正在生成的Sheet页是=="+sheetId);
            try
            {

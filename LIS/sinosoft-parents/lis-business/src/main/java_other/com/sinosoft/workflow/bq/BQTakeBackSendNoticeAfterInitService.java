...
public class BQTakeBackSendNoticeAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(BQTakeBackSendNoticeAfterInitService.class);
	...
	public BQTakeBackSendNoticeAfterInitService() {
	}
...
//		map.put(this.mLPCSpecSet, "UPDATE");
	...
	private boolean checkData(String cOperate) {
		...
		if (cOperate.equals("0000000322")) {
			...
			if (tTempCode.equals("25")) {
				tLOPRTManagerDB.setCode(PrintManagerBL.CODE_PEdorSend);
			} else if (tTempCode.equals("27")) {
		...
		for (int i = 1; i <= mLOPRTManagerSet.size(); i++) {
			if (mLOPRTManagerSet.get(i).getStateFlag() != null
				...
				if (this.mOperate.equals("0000000322"))
					tLZSysCertifyDB.setCertifyCode("2011"); // 核保通知书标识
				else
		...
		this.mLCIssuePolSet.add(tLCIssuePolDB.executeQuery(sqlbv3));
...
//			mLPCUWMasterSchema.setSpecFlag("2");
	...
	private boolean preparePrint() {
		...
		for (int i = 1; i <= this.mLCIssuePolSet.size(); i++) {
			mLCIssuePolSet.get(i).setState("2");// 回收完毕
...
//		}
	...
	private boolean chooseActivity() {
		...
		String tSQL = "select (case when sum(A.x) is not null then sum(A.x) else 0 end) from ("
				...
				+ " where otherno='"
				+ "?otherno?"
				+ "' and code in ('25','24','23') "
				+ " and oldprtseq<>'"
				+ "?oldprtseq?"
				+ "' and stateflag in ('0','1') )a";
//				+ " union "
//				// 判断是否有管理中心未回复的问题件
//				+ " select decode(count(*),0,0,1) x from lcissuepol where  backobjtype='4' and needprint = 'Y'"
...
//				+ this.mContNo + "' " + "	) A";
	...
	private boolean prepareTransferData() {
		...
		//加入保全受理号
		String tTemp_SQL = "select missionprop1 from lwmission where missionid='"+"?missionid?"+"' and activityid='0000000008'";
		logger.debug("tTemp_SQL:" + tTemp_SQL);
		ExeSQL tExeSQL = new ExeSQL();
		SQLwithBindVariables sqlbv5 = new SQLwithBindVariables();
		sqlbv5.sql(tTemp_SQL);
		sqlbv5.put("missionid",this.mMissionID);
		String t = tExeSQL.getOneValue(sqlbv5);
		if (t == null || t.equals("")) {
			CError.buildErr(this, "查询保全受理号信息失败!") ;
			return false;
		}
		else
			mTransferData.setNameAndValue("EdorAcceptNo", t);

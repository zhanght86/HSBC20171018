...
/**
 * <p>
 * Title: 工作流节点任务:新契约人工核保团单核保结论通知书回收服务类
 * </p>
 * <p>
 * Description: 回收团单核保结论通知书AfterInit工作流服务类
 * </p>
 * <p>
...
public class GrpTakeBackUWResultAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(GrpTakeBackUWResultAfterInitService.class);
	...
	public GrpTakeBackUWResultAfterInitService() {
	}
	...
	private boolean checkData() {
		...
		if (tLOPRTManagerSet.size() != 1) {
			...
			tError.functionName = "preparePrint";
			tError.errorMessage = "在打印队列中没有处于已打印待回收状态的团单核保结论通知书!";
			this.mErrors.addOneError(tError);
			return false;
		}
		...
		if (mPatchFlag == true) {
			...
			if (tempLOPRTManagerSet.size() == 1) {
				...
				tError.moduleName = "UWPrintTakeBackAutoHealthAfterInitService";
				tError.functionName = "preparePrint";
				tError.errorMessage = "查询在打印队列中没有该补打团单核保结论通知书的原通知书记录信息出错!";
				this.mErrors.addOneError(tError);
				return false;
			}
		...
		for (int i = 1; i <= mLOPRTManagerSet.size(); i++) {
			if (mLOPRTManagerSet.get(i).getStateFlag() != null
				...
				tLZSysCertifyDB.setCertifyCode("0076"); // 团单核保结论通知书标识
				...
				if (tLZSysCertifySet == null || tLZSysCertifySet.size() != 1) {
					...
					tError.moduleName = "UWPrintTakeBackAutoHealthAfterInitService";
					tError.functionName = "preparePrint";
					tError.errorMessage = "回收团单核保结论通知书时,LZSysCertifySchema表信息查询失败!";
					this.mErrors.addOneError(tError);
					return false;
				}

...
//            Connection tConn = DBConnPool.getConnection();
            tConn = DBConnPool.getConnection();
            if (tConn == null)
            {
                // @@错误处理
                CError tError = new CError();
        ...
        try
        {
            ...
            for (int i = 0; i < nRow; i++)
            {
                ...
                if (flag == true)
                {
                    ...
                    tValue = tValue + ",'" + mGlobalInput.Operator + "','?curDate?','?curTime?','?curDate?','?curTime?'";
                    sbv.put("Operator", mGlobalInput.Operator);
                    sbv.put("curTime", curTime);
                    sbv.put("curDate", curDate);
                }

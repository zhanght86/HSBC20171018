...
public class BQRePrintRReportAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(BQRePrintRReportAfterInitService.class);
	...
	public BQRePrintRReportAfterInitService() {
	}
	...
	private boolean checkData() {
		...
		if (tLOPRTManagerSet == null || tLOPRTManagerSet.size() != 1) {
			CError.buildErr(this, "生调通知书" + mLOPRTManagerSchema.getPrtSeq()
					+ "信息查询失败!") ;
			return false;
		}		
	...
	private boolean prepareTransferData() {
		...
		mTransferData
				.setNameAndValue("PrtSeqRR", mLOPRTManagerSchema.getPrtSeq());
		mTransferData.setNameAndValue("OldPrtSeqRR", mLOPRTManagerSchema
				.getOldPrtSeq());
		mTransferData
				.setNameAndValue("PrtNo", mLPContSchema.getPrtNo());
		mTransferData
				.setNameAndValue("SaleChnl", mLPContSchema.getSaleChnl());
		mTransferData
	...
	private boolean dealLwmission() {
		...
		String tSQL = "select * from lwmission where missionid='"
				+ "?missionid?" + "' " + " and activityid='0000000316' and missionprop14='"+"?missionprop14?"+"'";

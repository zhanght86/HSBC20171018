...
private static Logger logger = Logger.getLogger(UWTakeBackAutoHealthAfterInitService.class);
	...
	private boolean checkData() {
		...
		String StrSQL = "select * from LOPRTManager where prtseq = '"
				...
				+ "' and code in ('" + "?var1?"
				+ "','BQ90','LP03') and stateflag = '1'";
		sqlbv.sql(StrSQL);
		sqlbv.put("mCertifyNo", mCertifyNo);
		sqlbv.put("mContNo", mContNo);
		sqlbv.put("othernotype", PrintManagerBL.ONT_CONT);
		sqlbv.put("var1", PrintManagerBL.CODE_PE);
		...
		LOPRTManagerSet tLOPRTManagerSet = tLOPRTManagerDB.executeQuery(sqlbv);
	...
	private boolean prepareAutoHealth() {
		...
		tLCPENoticeDB.setPrtSeq(mOldPrtSeq); // 单证管理表中主键CertifyNo为打印管理表中的打印流水号
		...
		for (int i = 1; i <= tLCPENoticeSet.size(); i++) {
			mLCPENoticeSchema = tLCPENoticeSet.get(i);
			...
			mLCPENoticeSet.add(mLCPENoticeSchema);
		}

...
private static Logger logger = Logger.getLogger(TakeBackSendNoticeAfterInitService.class);
	...
	private String mState="1";//现在问题件修改不区分状态，所以之力初始化为1
...
//		}
	...
	private boolean checkData(String cOperate) {
		...
		if (mPatchFlag == true) {
			...
			sqlbv1.sql(tStr);
			sqlbv1.put("PrtSeq", mOldPrtSeq);
			sqlbv1.put("OldPrtSeq", mOldPrtSeq);
		...
		for (int i = 1; i <= mLOPRTManagerSet.size(); i++) {
			if (mLOPRTManagerSet.get(i).getStateFlag() != null
				...
				tLZSysCertifyDB.setCertifyCode(this.mCertifyCode);
		...
		// tLCIssuePolSet =
		this.mLCIssuePolSet.add(tLCIssuePolDB.executeQuery(sqlbv2));
		...
		if (mLOPRTManagerSchema.getCode().equals("TB89")) {
			...
			sqlbv3.put("contno1", this.mContNo);
			sqlbv3.put("prtseq1", mLOPRTManagerSchema.getOldPrtSeq());
			LCCSpecDB tLCCSpecDB = new LCCSpecDB();
			LCCSpecSet tLCCSpecSet = new LCCSpecSet();
			tLCCSpecSet = tLCCSpecDB.executeQuery(sqlbv3);
		...
		//判断是否有加费
			...
			String tSQL_Add = "select count(*) from LCUWMaster where contno='"
						+ mContNo + "' and AddPremFlag='1' ";
			tExeSQL = new ExeSQL();
	...
	private boolean dealData() {
		...
		if (prepareAutoSysCertSendOut() == false)
			return false;
		// 工作流处理通过流程图和工作流引擎完成，这里不通过业务控制流程
	...
	private boolean preparePrint() {
		...
		for (int i = 1; i <= this.mLCIssuePolSet.size(); i++) {
			...
			mLCIssuePolSet.get(i).setReplyDate(PubFun.getCurrentDate());
			mLCIssuePolSet.get(i).setReplyTime(PubFun.getCurrentTime());
		}
		// tongmeng 2007-12-28 add
		// 增加对特约的维护

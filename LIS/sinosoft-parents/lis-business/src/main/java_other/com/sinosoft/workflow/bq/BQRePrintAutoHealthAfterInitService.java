...
public class BQRePrintAutoHealthAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(BQRePrintAutoHealthAfterInitService.class);
	...
	public BQRePrintAutoHealthAfterInitService() {
	}
	...
	private boolean checkData() {
		...
		if (tLOPRTManagerSet == null || tLOPRTManagerSet.size() != 1) {
			CError.buildErr(this, "体检通知书" + mLOPRTManagerSchema.getPrtSeq()
					+ "信息查询失败!") ;
			return false;
		}
	...
	private boolean dealData() {
		...
		// 准备需要删除的数据 已修改为单一聚合模式together=3
...
//		}
	...
	private boolean dealLwmission() {
		...
		String tSQL = "select * from lwmission where missionid='"
				+ "?missionid?" + "' " 
				+ " and activityid='0000000303' and missionprop14='"+"?missionprop14?"+"'";

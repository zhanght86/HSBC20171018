...
public class LLPrintAutoHealthAfterInitService implements AfterInitService  {
private static Logger logger = Logger.getLogger(LLPrintAutoHealthAfterInitService.class);
	...
	public LLPrintAutoHealthAfterInitService() {
	}
	...
	private boolean prepareOutputData() {
		...
		if (mPatchFlag == false) {
			...
			map.put(mLLUWPENoticeSchema, "UPDATE");
		}
	...
	private boolean checkData() {
		...
		if (tLOPRTManagerSet == null) {
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLPrintAutoHealthAfterInitService";
	...
	private boolean preparePrintAutoHealth() {
		...
		if (mPatchFlag == false) // 不是补打，则更新数据状态
		{
			...
			tLLUWPENoticeDB.setContNo(mContNo);
			tLLUWPENoticeDB.setPrtSeq(mPrtSeq);
			tLLUWPENoticeDB.setPrintFlag("0");
			tLLUWPENoticeDB.setCustomerNo(mLOPRTManagerSchema.getStandbyFlag1()); // 备用字段1存放客户号
			LLUWPENoticeSet tLLUWPENoticeSet = tLLUWPENoticeDB.query();
			if (tLLUWPENoticeSet == null || tLLUWPENoticeSet.size() <= 0) {
				mErrors.copyAllErrors(tLLUWPENoticeDB.mErrors);
				return false;
			}
			mLLUWPENoticeSchema = tLLUWPENoticeSet.get(1);
			mLLUWPENoticeSchema.setPrintFlag("1");
	...
	private boolean prepareTransferData() {
		...
		mTransferData.setNameAndValue("CustomerNo", mLOPRTManagerSchema
				.getStandbyFlag1());
		//09-06-11 配合扫描回收 增加查询赔案号
		mTransferData.removeByName("ClmNo");
		
		mTransferData.setNameAndValue("ClmNo", mLOPRTManagerSchema.getStandbyFlag4());

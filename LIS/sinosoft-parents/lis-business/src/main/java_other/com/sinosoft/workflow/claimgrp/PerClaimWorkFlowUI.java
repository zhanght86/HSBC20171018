...
public class PerClaimWorkFlowUI
{
private static Logger logger = Logger.getLogger(PerClaimWorkFlowUI.class);
...
//        tVData.add(mTransferData);
      ...
      PerClaimWorkFlowUI tPerClaimWorkFlowUI = new PerClaimWorkFlowUI();
      try
      {
          if (tPerClaimWorkFlowUI.submitData(tVData, "0000005005"))
          {
              VData tResult = new VData();
          }
          else
          {
              logger.debug(tPerClaimWorkFlowUI.mErrors.getError(0).
                                 errorMessage);
          }
      }
      catch (Exception ex)
  ...
  public boolean submitData(VData cInputData, String cOperate)
  {
      ...
      PerClaimWorkFlowBL tPerClaimWorkFlowBL = new PerClaimWorkFlowBL();

      logger.debug("---PerClaimWorkFlowBL UI BEGIN---");
      if (!tPerClaimWorkFlowBL.submitData(cInputData, mOperate))
      {
          // @@错误处理
          this.mErrors.copyAllErrors(tPerClaimWorkFlowBL.mErrors);
          mResult.clear();
          return false;
      }
      mResult = tPerClaimWorkFlowBL.getResult();
      logger.debug("---PerClaimWorkFlowBL mResult="+mResult);
      logger.debug("---PerClaimWorkFlowBL UI END---");
      return true;
  }

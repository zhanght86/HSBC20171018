 ...
 * @version 6.0
 */
public class PlanApproveAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(PlanApproveAfterInitService.class);
	...
	private String mApproveFlag; // 审批结论
	private String mApproveIdea; // 审批意见
	...
	public PlanApproveAfterInitService() {
	}
	...
	private boolean getInputData(VData cInputData, String cOperate) {
		...
		if (mApproveFlag == null || "".equals(mApproveFlag)) {
			this.buildError("getInputDate", "没有传入产品组合审批结论！");
		}

		mApproveIdea = (String) mTransferData.getValueByName("ApproveIdea");
	...
	private boolean checkData() {
		...
		if (!(mLDPlanSchema.getState()).equals(PlanFunc.PLAN_STATE_APPROVE)) {
			this.buildError("checkData", "产品组合没有处在审批状态！");
			return false;
		}
	...
	private boolean dealData() {
		if ("9".equals(mApproveFlag)) {
			mLDPlanSchema.setState(PlanFunc.PLAN_STATE_APPROVE_YES);// 如果审批结论为“通过”，则置state为“审批通过”
		} else {
			mLDPlanSchema.setState(PlanFunc.PLAN_STATE_APPROVE_NO);// 如果审批结论为“不通过”，则置state为“审批不通过”
		}

		/**
		 * @todo 设置产品组合审批结论审批意见(等待表结构申请)
		 */
		mLDPlanSchema.setModifyDate(PubFun.getCurrentDate());
		mLDPlanSchema.setModifyTime(PubFun.getCurrentTime());
		mLDPlanSchema.setOperator(this.mOperator);
		...
		mLDPlanApproveMasterSchema
				.setOperateType(PlanFunc.PLAN_OPERATE_APPROVE);// 操作类型:03-审批
		mLDPlanApproveMasterSchema.setOperateNo(tOperateNo);// 操作顺序号
		mLDPlanApproveMasterSchema.setOperateResult(mApproveFlag);
		mLDPlanApproveMasterSchema.setRemark(mApproveIdea);
		...
		mLDPlanApproveSubSchema.setOperateType(PlanFunc.PLAN_OPERATE_APPROVE); // 操作类型:03-审批
		mLDPlanApproveSubSchema.setOperateNo(tOperateNo); // 操作顺序号
		mLDPlanApproveSubSchema.setOperateResult(mApproveFlag);
		mLDPlanApproveSubSchema.setRemark(mApproveIdea);

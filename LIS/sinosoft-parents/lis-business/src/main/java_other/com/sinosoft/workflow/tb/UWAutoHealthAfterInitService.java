...
private static Logger logger = Logger.getLogger(UWAutoHealthAfterInitService.class);
	...
	private String mFlag; // 判断是否保全调用的标志，为"1"时表示保全调用
	...
	private boolean dealData() {
		...
		// 准备操作轨迹
		if (mFlag != null
				&& (mFlag.trim().equals("1") || mFlag.trim().equals("2"))) {
			// 保全和理赔暂不需要此功能
		} else {
	...
	private boolean getInputData(VData cInputData, String cOperate) {
		...
		// 获得业务体检通知数据
		mLCPENoticeSchema = (LCPENoticeSchema) mTransferData
				.getValueByName("LCPENoticeSchema");
	...
	private boolean prepareHealth() {
		...
		String sCustomerName = new String("");
		// 取体检人姓名
		LDPersonDB tLDPersonDB = new LDPersonDB();
		tLDPersonDB.setCustomerNo(mCustomerNo);
		if (tLDPersonDB.getInfo()) {
			sCustomerName = tLDPersonDB.getName();
		} else {
			// <!-- XinYQ modified on 2006-07-20 : 保全复用新契约体检 : BGN -->
			...
			if (tLPPersonSet == null || tLPPersonSet.size() <= 0) {
				...
				sCustomerName = tLPPersonSchema.getName();
				tLPPersonSchema = null;
			}
			tLPPersonDB = null;
			tLPPersonSet = null;
			// <!-- XinYQ modified on 2006-07-20 : 保全复用新契约体检 : END -->
		}
		...
		mLCPENoticeSchema.setName(sCustomerName);
	...
	private boolean preparePrint() {
		...
		logger.debug("Flag to judge:" + mFlag);
		if (mFlag != null && mFlag.trim().equals("1")) {
			tLOPRTManagerDB.setCode(PrintManagerBL.CODE_EdorUWTJ); // 保全体检
		} else {
			tLOPRTManagerDB.setCode(PrintManagerBL.CODE_PE); // 新契约体检
		}
		...
		// 准备打印管理表数据
		if (mFlag != null && mFlag.trim().equals("1")) {
			mLOPRTManagerSchema.setCode(PrintManagerBL.CODE_EdorUWTJ); // 保全体检
		} else if (mFlag != null && mFlag.trim().equals("2")) {
			mLOPRTManagerSchema.setCode(PrintManagerBL.CODE_LPPE); // 理赔体检
		} else {
			mLOPRTManagerSchema.setCode(PrintManagerBL.CODE_PE); // 新契约体检
		}
	...
	private boolean saveUWOperateTrack() {
		...
		if (m > 0) {
			...
			tLCCUWSubSchema.setUWIdea("核保员发送体检通知书。（通知书号：" + mPrtSeq + "）");

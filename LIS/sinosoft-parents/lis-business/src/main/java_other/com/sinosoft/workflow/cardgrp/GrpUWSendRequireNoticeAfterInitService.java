...
import com.sinosoft.lis.cbcheckgrp.GrpUWManuChangPlanBL;
...
/**
 * <p>
 * Title: 工作流节点任务:新契约发团体要求通知书
 * </p>
 * <p>
 * Description: 工作流发团体要求通知书AfterInit服务类
 * </p>
 * <p>
...
public class GrpUWSendRequireNoticeAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(GrpUWSendRequireNoticeAfterInitService.class);
	...
	public GrpUWSendRequireNoticeAfterInitService() {
	}
	...
	private boolean checkData() {
		...
		String ss = " select distinct 1 from loprtmanager where 1=1"
				...
				+ PrintManagerBL.CODE_GRP_UWREQUIRE + "'"
		...
		if (tSqlResult.equals("1")) {
			...
			tError.errorMessage = "该保单下团体核保要求通知书已经在打印队列中，不能再次发送!";
			this.mErrors.addOneError(tError);
			return false;
	...
	private boolean dealData() {
		GrpUWManuChangPlanBL tGrpUWManuChangPlanBL = new GrpUWManuChangPlanBL();

		VData aVData = new VData();
		aVData.add(this.mGlobalInput);
		aVData.add(this.mLCGrpContSet);
		boolean tResult = tGrpUWManuChangPlanBL.submitData(aVData, "");
		logger.debug("after!tResult===" + tResult);
		if (tResult) {
			mMap = (MMap) tGrpUWManuChangPlanBL.getResult()
					.getObjectByObjectName("MMap", 0);
			mLOPRTManagerSchema = (LOPRTManagerSchema) tGrpUWManuChangPlanBL
					.getResult().getObjectByObjectName("LOPRTManagerSchema", 0);

		} else {
			this.mErrors.copyAllErrors(tGrpUWManuChangPlanBL.mErrors);
		}

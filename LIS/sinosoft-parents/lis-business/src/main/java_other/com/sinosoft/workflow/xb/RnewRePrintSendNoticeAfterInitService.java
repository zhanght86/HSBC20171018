...
public class RnewRePrintSendNoticeAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(RnewRePrintSendNoticeAfterInitService.class);
	...
	public RnewRePrintSendNoticeAfterInitService() {
	}
	...
	private boolean dealLwmission() {
		...
		String tSQL = "select * from lwmission where missionid='"
				+ "?missionid?" + "' " + " and activityid='0000007006' ";
...
//		}
	...
	private boolean getInputData(VData cInputData, String cOperate) {
		...
		if (this.mMissionID.equals("")) {
			...
			tError.moduleName = "UWAutoHealthAfterInitService";
			tError.functionName = "getInputData";
			tError.errorMessage = "前台传输业务数据中mMissionID失败!";
			this.mErrors.addOneError(tError);
			return false;
		}
	...
	private boolean preparePrint() {
		...
		//MD at 2009-07-21 核保通知书流水号应该为10位。
//		String strNoLimit = PubFun.getNoLimit(mGlobalInput.ComCode);
//		mLOPRTManagerSchema.setPrtSeq(PubFun1.CreateMaxNo("PRTSEQNO",
//				strNoLimit));
		
		mLOPRTManagerSchema.setPrtSeq(PubFun1.CreateMaxNo("UWPRTSEQ", PrintManagerBL.CODE_PRnewUW));
	...
	private boolean prepareTransferData() {
		...
		// 目前续期二核只会发一次核保通知书，产生一条打印核保通知书工作流，即类型为被保人的核保通知书
		// tongmeng 2007-11-29 modify
		//获取当前核保信息
		LBMissionSchema xLBMissionSchema = new LBMissionSchema();
		LBMissionDB tLBMissionDB = new LBMissionDB();
		tLBMissionDB.setMissionID(this.mMissionID);
		tLBMissionDB.setActivityID("0000007004");
		if(tLBMissionDB.query().size()==0)
		{
            //@@错误处理
			CError tError = new CError();
			tError.moduleName = "UWAutoHealthAfterInitService";
			tError.functionName = "prepareTransferData";
			tError.errorMessage = "任务号"+mMissionID+"查询打印核保通知书B表记录失败";
			this.mErrors.addOneError(tError);
		}
		xLBMissionSchema =tLBMissionDB.query().get(1);
		mTransferData.setNameAndValue("ContNo", mLCContSchema.getContNo());
		mTransferData.setNameAndValue("PrtSeqUW", mLOPRTManagerSchema
				.getPrtSeq());
		mTransferData.setNameAndValue("PrtSeqUW_UnPrint", mLOPRTManagerSchema
				.getPrtSeq());
		mTransferData.setNameAndValue("PrtSeqOper", mLOPRTManagerSchema
				.getPrtSeq());
		mTransferData.setNameAndValue("CustomerNo", xLBMissionSchema.getMissionProp8());
		mTransferData.setNameAndValue("CustomerNoType", xLBMissionSchema.getMissionProp9());
		mTransferData.setNameAndValue("SpecFlag", xLBMissionSchema.getMissionProp10());
		mTransferData.setNameAndValue("ChangePolFlag", xLBMissionSchema.getMissionProp11());
		mTransferData.setNameAndValue("AddFeeFlag", xLBMissionSchema.getMissionProp12());

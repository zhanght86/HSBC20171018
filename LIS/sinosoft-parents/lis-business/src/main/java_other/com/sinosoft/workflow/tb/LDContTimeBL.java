...
public class LDContTimeBL {
private static Logger logger = Logger.getLogger(LDContTimeBL.class);
	...
	private boolean dealData() {
		...
		if (IsLDContTime.equals("true")) {
			...
			.append("?mSubBusinessType?").append("', '")
			.append("?mTimeSeq?").append("', '")
			.append("?mOperator?").append("', '")
			.append("?mMakeDate?").append("', '")
			.append("?mMakeTime?").append("', '")
			.append("?mBackDate?").append("', '")
			.append("?mBackTime?").append("' )");

			logger.debug("== LDContTimeBL == sbSQL \n" + sbSQL.toString());
			SQLwithBindVariables sqlbv1=new SQLwithBindVariables();
			sqlbv1.sql(sbSQL.toString());
			sqlbv1.put("mSerialNo", PubFun1.CreateMaxNo("TraceNo", 10));
			sqlbv1.put("mContNo", mContNo);
			sqlbv1.put("mPrtNo", mPrtNo);
			sqlbv1.put("mBusinessType", mBusinessType);
			sqlbv1.put("mSubBusinessType", mSubMissionID);
			sqlbv1.put("mTimeSeq", mTimeSeq);
			sqlbv1.put("mOperator", mOperator);
			sqlbv1.put("mMakeDate", mLWMissionSchema.getInDate());
			sqlbv1.put("mMakeTime", mLWMissionSchema.getInTime());
			sqlbv1.put("mBackDate", PubFun.getCurrentDate());
			sqlbv1.put("mBackTime", PubFun.getCurrentTime());
	...
	public static void main(String[] args) {
		...
		if (tLDCodeSet.size() == 1) {
			...
			if (tLDContTimeBL.submitData(tVData, "")) {

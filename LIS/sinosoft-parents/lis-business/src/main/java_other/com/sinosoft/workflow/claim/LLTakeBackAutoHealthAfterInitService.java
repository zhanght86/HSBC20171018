...
public class LLTakeBackAutoHealthAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(LLTakeBackAutoHealthAfterInitService.class);
	...
	public LLTakeBackAutoHealthAfterInitService() {
	}
	...
	private boolean prepareOutputData() {
		...
		// 添加体检表数据
		if (mLLUWPENoticeSet != null && mLLUWPENoticeSet.size() > 0) {
			map.put(mLLUWPENoticeSet, "UPDATE");
		}
	...
	private boolean checkData() {
		...
		String StrSQL = "select * from LOPRTManager where prtseq = '"
				...
				+ "' and code in ('LP03') and stateflag = '1'";
	...
	private boolean prepareAutoHealth() {
		...
		tLLUWPENoticeDB.setPrtSeq(mOldPrtSeq); // 单证管理表中主键CertifyNo为打印管理表中的打印流水号
		...
		if (tLLUWPENoticeSet == null || tLLUWPENoticeSet.size() <= 0) {
			mErrors.copyAllErrors(tLLUWPENoticeSet.mErrors);
			return false;
		}
		for (int i = 1; i <= tLLUWPENoticeSet.size(); i++) {
			mLLUWPENoticeSchema = tLLUWPENoticeSet.get(i);
			mLLUWPENoticeSchema.setPrintFlag("2"); // 体检通知书已回收
			mLLUWPENoticeSchema.setModifyDate(PubFun.getCurrentDate());
			mLLUWPENoticeSchema.setModifyTime(PubFun.getCurrentTime());
			mLLUWPENoticeSet.add(mLLUWPENoticeSchema);
		}
	...
	private boolean prepareTransferData() {
		mTransferData.setNameAndValue("ContNo", mLLUWPENoticeSchema.getContNo());
		mTransferData.setNameAndValue("PrtNo", mLLUWPENoticeSchema.getContNo());
		mTransferData.setNameAndValue("PrtSeq", mLLUWPENoticeSchema.getPrtSeq());
		mTransferData.setNameAndValue("CustomerNo", mLLUWPENoticeSchema
				.getCustomerNo());

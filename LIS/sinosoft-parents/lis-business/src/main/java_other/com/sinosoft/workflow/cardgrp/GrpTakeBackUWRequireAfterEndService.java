...
/**
 * <p>
 * Title: 工作流节点任务:新契约人工核保回收团单核保要求通知书工作流服务类
 * </p>
 * <p>
 * Description:新契约人工核保团单核保要求通知书回收工作流AfterEnd服务类 对回收成功后工作流节点的流转进行控制
 * </p>
 * <p>
...
public class GrpTakeBackUWRequireAfterEndService implements AfterEndService {
private static Logger logger = Logger.getLogger(GrpTakeBackUWRequireAfterEndService.class);
	...
	public GrpTakeBackUWRequireAfterEndService() {
	}
	...
	private boolean checkData() {
		...
		tLWMissionDB.setActivityID("0000011311");
	...
	private boolean prepareMission() {
		// 查询出同一团单核保要求通知书(原打印流水号相同者)待回收的任务节点,并回收

		String tStr = "Select * from LWMission where MissionID = '"
				+ mMissionID + "'" + "and ActivityID = '0000011311'"
				...
				+ "'" + " and ActivityID = '0000011314'"
				+ " and MissionProp14 = '"
				+ mLWMissionSchema.getMissionProp14() + "'" + " union "
				+ "Select * from LWMission where MissionID = '" + mMissionID
				+ "'" + " and ActivityID = '0000011306'"
				+ " and MissionProp14 = '"
		...
		// 判段该团单核保要求通知书回收后,是否该承保申请已处于人工核保已回复状态.
		tStr = "Select count(*) from LWMission where MissionID = '"
				+ mMissionID + "'"
				+ "and ActivityID in ('0000011111','0000011106','0000011114')";

...
/**
 * <p>
 * Title:工作流服务类:新契约打印生调通知书
 * </p>
 * <p>
 * Description: 打印生调通知书工作流AfterInit服务类
 * </p>
 * <p>
...
public class RnewPrintRReportAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(RnewPrintRReportAfterInitService.class);
	...
	public RnewPrintRReportAfterInitService() {
	}
...
//			return false;
	...
	private boolean preparePolUW() {
		...
		for (int i = 1; i <= mLCPolSet.size(); i++) {
			...
			tLCUWMasterSchema.setReportFlag("2");
	...
	private boolean dealMission(){
		...
		xLWMissionDB.setActivityID("0000007010");
...
//			if(!tLWMissionDB.getInfo()){
	...
	private boolean prepareTransferData() {
		...
		String tsql = "select max(PrtSeq) from RnewRReport where Contno = '"
				+ "?mContNo?" + "'";
		SQLwithBindVariables sqlbv = new SQLwithBindVariables();
		sqlbv.sql(tsql);
		sqlbv.put("mContNo", mContNo);
		ExeSQL tExeSQL = new ExeSQL();
		String tPrtSeq = tExeSQL.getOneValue(sqlbv);

		tRnewRReportDB.setPrtSeq(tPrtSeq);
		tRnewRReportDB.setContNo(mContNo);
		tRnewRReportSet = tRnewRReportDB.query();
		if (tRnewRReportSet == null || tRnewRReportSet.size() != 1) {
			// @@错误处理
			CError tError = new CError();
		...
		tRnewRReportSchema = tRnewRReportSet.get(1);

		mTransferData.setNameAndValue("InsuredNo", tRnewRReportSchema
				.getCustomerNo());
		mTransferData
				.setNameAndValue("InsuredName", tRnewRReportSchema.getName());
		mTransferData.setNameAndValue("RROperator", tRnewRReportSchema
				.getOperator());
		// mTransferData.setNameAndValue("HandleDate",
		// tRnewRReportSchema.getMakeDate());
		// mTransferData.setNameAndValue("PrtSeq",
		// tRnewRReportSchema.getPrtSeq());
		mTransferData
		...
		mTransferData
				.setNameAndValue("AgentCode", mLCContSchema.getAgentCode());
		mTransferData.setNameAndValue("Code", this.mCode);
		...
		mTransferData.setNameAndValue("ExeOperator", mLOPRTManagerSchema
				.getExeOperator());
		mTransferData.setNameAndValue("CustomerNo", tRnewRReportSchema
				.getCustomerNo());

...
//import com.sinosoft.workflowengine.ActivityOperatorWjm;

public class PerClaimWorkFlowBL
{
private static Logger logger = Logger.getLogger(PerClaimWorkFlowBL.class);
    ...
    {
        logger.debug("------PerClaimWorkFlowBL BEG------");
        //得到外部传入的数据,将数据备份到本类中
        if (!getInputData(cInputData, cOperate))
        {
            return false;
        }
        logger.debug("---PerClaimWorkFlowBL After getInputData---");

        // 数据操作业务处理
        if (!dealData())
        {
            logger.debug("---PerClaimWorkFlowBL dealData  fali------");
            return false;
        }
        logger.debug("---PerClaimWorkFlowBL After dealData---");
        ...
        logger.debug("---PerClaimWorkFlowBL After prepareOutputData---");

        //如果置相应的标志位，不提交
        if (mFlag)
        {

            logger.debug("Start PerClaimWorkFlowBL Submit......");
            ...
            if (!tPubSubmit.submitData(mInputData, ""))
            {
                ...
                tError.moduleName = "PerClaimWorkFlowBL";
            ...
            logger.debug("------PerClaimWorkFlowBL END------");
        }
        return true;
    }
    ...
    private boolean dealData()
    {
        ...
        {
            logger.debug("----------mOperate===9999999999");
            if (!Execute9999999999())
            {
                // @@错误处理
                logger.debug("----------mOperate===9999999999--------fail");
        ...
        {
             logger.debug("----------mOperate===9899999999");
            //创建立案起始节点
            if (!Execute9899999999())
            {
                // @@错误处理
                logger.debug("----------mOperate===9899999999--------fail");
                this.mErrors.copyAllErrors(mActivityOperator.mErrors);
                return false;
            }
        }
        ...
        {
            //创建呈报起始节点
            logger.debug("----------mOperate===8999999999");
            if (!Execute8999999999())
            {
                // @@错误处理
                logger.debug("----------mOperate===8999999999   --------fail");
                this.mErrors.copyAllErrors(mActivityOperator.mErrors);
                return false;
            }
        }
        ...
        {
            //创建调查报告申请起始节点
            logger.debug("----------mOperate===8899999999");
            if (!Execute8899999999())
            {
                // @@错误处理
                logger.debug("----------mOperate===8899999999--------fail");
                this.mErrors.copyAllErrors(mActivityOperator.mErrors);
                return false;
            }
        }
        ...
        {
            //立案确认
            logger.debug("----------mOperate===0000005015");
            if (!Execute0000005015())
            {
                // @@错误处理
                 logger.debug("----------mOperate===0000005015   --------fail");
                this.mErrors.copyAllErrors(mActivityOperator.mErrors);
                return false;
            }
        }
        ...
        {
            //审核确认
             logger.debug("----------mOperate===0000005035");
            if (!Execute0000005035())
            {
                // @@错误处理
                 logger.debug("----------mOperate===0000005035 --------fail");
                this.mErrors.copyAllErrors(mActivityOperator.mErrors);
                return false;
            }
        }
        ...
        {
            //消亡呈报节点
             logger.debug("----------mOperate===0000005105 ");
            if (!Execute01())
            {
                // @@错误处理
                logger.debug("----------mOperate===0000005105-------------fail ");
                this.mErrors.copyAllErrors(mActivityOperator.mErrors);
                return false;
            }
        }
        else if (mOperate.trim().equals("0000005145"))
        {
            //消亡调查过程节点，并根据判断是否生成结论节点
             logger.debug("----------mOperate===0000005145 ");
            if (!Execute0000005145())
            {
                // @@错误处理
                 logger.debug("----------mOperate===0000005145-------------fail ");
                this.mErrors.copyAllErrors(mActivityOperator.mErrors);
                return false;
            }
        }
        else if (mOperate.trim().equals("0000005165"))
        {
            //消亡调查结论节点<机构层面>,并判断是否生成下一个节点<赔案层面>
                logger.debug("----------mOperate===0000005165 ");
            if (!Execute0000005165()) {
                // @@错误处理
                 logger.debug("----------mOperate===0000005165-------------fail ");
                this.mErrors.copyAllErrors(mActivityOperator.mErrors);
                return false;
            }
        }
...
//            }
    ...
    private boolean Execute9899999999()
    {
        ...
        //调用业务逻辑处理类，返回处理完数据
        LLPerClaimRegisterBL tLLPerClaimRegisterBL = new LLPerClaimRegisterBL();
        if (tLLPerClaimRegisterBL.submitData(mInputData, mOperate))
        {
            VData tempVData1 = new VData();
            tempVData1 = tLLPerClaimRegisterBL.getResult();
//            if(tempVData1==null||tempVData1.size()<1)
//            {
//                logger.debug("tempVData1 have no data !");
//            }
//            logger.debug("tempVData1==========="+tempVData1.size());
            cInputData.add(tempVData1);
            //向前台返回处理完的参数以便查询工作流
            mTransferData = null;
            mTransferData = (TransferData) tempVData1.getObjectByObjectName(
...
//            logger.debug("mResult==========="+mResult.size());
            //首先生成节点，由于第一次创建，没有触发服务类，在下面直接调用业务处理类
            ActivityOperator tActivityOperator = new ActivityOperator();
        //    ActivityOperatorWjm tActivityOperator=new ActivityOperatorWjm();
            try
            {
                //产生报案确认节点
                if (tActivityOperator.CreateStartMission_NoScan("0000000005",
                        "0000005015", tempVData1))
                {
                    ...
                    tempVData2 = null;
                    tReturn = true;
                    logger.debug("tActivityOperator===========Success");
                }
                else
                {
                    // @@错误处理
                    logger.debug("tActivityOperator===========fail");
                    this.mErrors.copyAllErrors(mActivityOperator.mErrors);
                    tReturn = false;
                }
            }
            ...
            tempVData1 = null;
            logger.debug(" tempVData1===============null");
        }
        else
        {
            // @@错误处理
            this.mErrors.copyAllErrors(tLLPerClaimRegisterBL.mErrors);
            CError tError = new CError();
            tError.moduleName = "PerClaimWorkFlowBL";
            tError.functionName = "submitData";
            tError.errorMessage = "数据提交失败!";

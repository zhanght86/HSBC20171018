 ...
 * @version 6.0
 */
public class TakeBackNBNoticeAfterInitService implements AfterInitService {
private static Logger logger = Logger.getLogger(TakeBackNBNoticeAfterInitService.class);
	...
	public TakeBackNBNoticeAfterInitService() {
	}
	...
	private boolean prepareOutputData() {
		...
		// 添加问题件表数据
		if (mLCIssuePolSet != null && mLCIssuePolSet.size() > 0) {
	...
	private boolean checkData() {
		...
		if (mPatchFlag == true) {
			...
			sqlbv3.sql(tStr);
			sqlbv3.put("PrtSeq", mOldPrtSeq);
			sqlbv3.put("OldPrtSeq", mOldPrtSeq);
	...
	/**
	 * 准备问题件表信息 输出：如果发生错误则返回false,否则返回true
	 * 
	 * @return boolean
	 */
	private boolean prepareSendNotice() {
		// //准备问题件主表信息
		for (int i = 0; i < mLCIssuePolSet.size(); i++) {
			mLCIssuePolSet.get(i + 1).setModifyDate(PubFun.getCurrentDate());
			mLCIssuePolSet.get(i + 1).setModifyTime(PubFun.getCurrentTime());
			mLCIssuePolSet.get(i + 1).setState("2"); // 回收契约内容变更通知书
		}
		return true;
	}

 ...
 * @version 6.0
 */
public class RePrintNBCustomerNoticeAfterInitService implements
		AfterInitService {
private static Logger logger = Logger.getLogger(RePrintNBCustomerNoticeAfterInitService.class);
	...
	public RePrintNBCustomerNoticeAfterInitService() {
	}
	...
	private boolean checkData() {
		...
		if (tLOPRTManagerSet == null) {
			...
			tError.functionName = "checkData";
			tError.errorMessage = "客户合并通知书" + mLOPRTManagerSchema.getPrtSeq()
					+ "信息查询失败!";
			this.mErrors.addOneError(tError);
			return false;
		}
		...
		if (!mLOPRTManagerSchema.getCode().equals("17")) {
			CError tError = new CError();
			tError.moduleName = "RePrintAutoHealthAfterInitService";
			tError.functionName = "checkData";
			tError.errorMessage = "客户合并通知书" + mLOPRTManagerSchema.getPrtSeq()

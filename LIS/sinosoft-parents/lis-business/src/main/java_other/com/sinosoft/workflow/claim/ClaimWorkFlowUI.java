...
public class ClaimWorkFlowUI implements BusinessService{
private static Logger logger = Logger.getLogger(ClaimWorkFlowUI.class);
	...
	public static void main(String[] args) {
		...
		// mTransferData.setNameAndValue("ComeWhere", "A");
		// //来自:A立案进入审核，B审批进入审核因为不通过，C审批进入审核因为赔案为负数，D为预付进入审核
		// //------------------------------------------------------------------------------BEG
		...
		// 审核调试
		// TransferData mTransferData = new TransferData();
		// mTransferData.setNameAndValue("RptNo", "90000014131");
		// mTransferData.setNameAndValue("RptorState", "40");
		// mTransferData.setNameAndValue("CustomerNo", "000003480");
		// mTransferData.setNameAndValue("CustomerName", "1");
		...
		// mTransferData.setNameAndValue("RgtObjNo", "1"); //其他号码
		// mTransferData.setNameAndValue("MngCom", mGlobalInput.ManageCom); //机构
		// mTransferData.setNameAndValue("PrepayFlag", "1");
		// mTransferData.setNameAndValue("AuditPopedom", "A02");//审核权限
		// mTransferData.setNameAndValue("Auditer", "zhangpa");//审核人
		// //------------------------------------------------------------------------------BEG
		...
		//
		// mTransferData.setNameAndValue("MissionID","00000000000000068253");
		...
		tLLClaimUWMainSchema.setClmNo("90000015278");// 赔案号
		tLLClaimUWMainSchema.setExamConclusion("1");// 审批结论
		tLLClaimUWMainSchema.setExamIdea("12");// 审批意见
		tLLClaimUWMainSchema.setExamNoPassReason("02");// 不通过原因
		tLLClaimUWMainSchema.setExamNoPassDesc("");// 不通过依据

		// String使用TransferData打包后提交
		TransferData mTransferData = new TransferData();
		mTransferData.setNameAndValue("RptNo", "90000015278");
		mTransferData.setNameAndValue("RptorState", "40");
		mTransferData.setNameAndValue("CustomerNo", "90000015278");
		mTransferData.setNameAndValue("CustomerName", "1");
		mTransferData.setNameAndValue("CustomerSex", "1");
		mTransferData.setNameAndValue("AccDate", "1");
		mTransferData.setNameAndValue("RgtClass", "1"); // 申请类型
		...
		// 转移条件
		mTransferData.setNameAndValue("Reject", "1"); // 审批是否通过

		mTransferData.setNameAndValue("MissionID", "00000000000000115420");
		...
		// mTransferData.setNameAndValue("MngCom", mGlobalInput.ManageCom); //机构
		// mTransferData.setNameAndValue("PrepayFlag", "0"); //预付标志
		// mTransferData.setNameAndValue("Auditer", "0");
		// //审核操作人,为审批不通过时返回个人工作队列用
		//
		...
		try {
			if (tClaimWorkFlowUI.submitData(tVData, "0000005055")) {
				VData tResult = new VData();
			} else {
				logger.debug(tClaimWorkFlowUI.mErrors.getError(0).errorMessage);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}
	...
	public boolean submitData(VData cInputData, String cOperate) {
		...
		logger.debug("---ClaimWorkFlowBL UI BEGIN---");
		if (!tClaimWorkFlowBL.submitData(cInputData, mOperate)) {
			// @@错误处理
			this.mErrors.copyAllErrors(tClaimWorkFlowBL.mErrors);
			mResult.clear();
			return false;
		}
		mResult = tClaimWorkFlowBL.getResult();
		logger.debug("---UI mResult=" + mResult);
		logger.debug("---ClaimWorkFlowBL UI END---");
		return true;
	}

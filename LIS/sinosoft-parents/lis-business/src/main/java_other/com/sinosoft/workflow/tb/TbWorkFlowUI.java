...
public class TbWorkFlowUI implements BusinessService {
private static Logger logger = Logger.getLogger(TbWorkFlowUI.class);
	...
	public static void main(String[] args) {
		...
		try {
			if (tTbWorkFlowUI.submitData(tVData, "0000001250")) {
				VData tResult = new VData();

			} else {
				logger.debug(tTbWorkFlowUI.mErrors.getError(0).errorMessage);
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		}

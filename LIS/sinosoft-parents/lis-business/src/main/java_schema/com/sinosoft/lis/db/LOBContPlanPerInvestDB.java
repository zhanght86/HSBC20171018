...
/*
 * <p>ClassName: LOBContPlanPerInvestDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LOBContPlanPerInvestDB extends LOBContPlanPerInvestSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LOBContPlanPerInvestDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LOBContPlanPerInvestSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBContPlanPerInvestDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getInsuAccNo() == null || this.getInsuAccNo().equals("null")) {
				...
				pstmt.setString(9, this.getInsuAccNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBContPlanPerInvest WHERE  BussType = ? AND BussNo = ? AND PolicyNo = ? AND SysPlanCode = ? AND PlanCode = ? AND RiskCode = ? AND DutyCode = ? AND PayPlanCode = ? AND InsuAccNo = ?", 
	...
	public LOBContPlanPerInvestSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBContPlanPerInvestSet aLOBContPlanPerInvestSet = new LOBContPlanPerInvestSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBContPlanPerInvest");
			LOBContPlanPerInvestSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBContPlanPerInvestSchema s1 = new LOBContPlanPerInvestSchema();
				s1.setSchema(rs,i);
				aLOBContPlanPerInvestSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBContPlanPerInvestSet;
	}

	public LOBContPlanPerInvestSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBContPlanPerInvestSet aLOBContPlanPerInvestSet = new LOBContPlanPerInvestSet();
	...
	public LOBContPlanPerInvestSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBContPlanPerInvestSet aLOBContPlanPerInvestSet = new LOBContPlanPerInvestSet();
	...
	public LOBContPlanPerInvestSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBContPlanPerInvestSet aLOBContPlanPerInvestSet = new LOBContPlanPerInvestSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBContPlanPerInvest " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBContPlanPerInvestSet
 */
public LOBContPlanPerInvestSet getData()
{
    int tCount = 0;
    LOBContPlanPerInvestSet tLOBContPlanPerInvestSet = new LOBContPlanPerInvestSet();
    LOBContPlanPerInvestSchema tLOBContPlanPerInvestSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBContPlanPerInvestDB";
    ...
    {
        tCount = 1;
        tLOBContPlanPerInvestSchema = new LOBContPlanPerInvestSchema();
        tLOBContPlanPerInvestSchema.setSchema(mResultSet, 1);
        tLOBContPlanPerInvestSet.add(tLOBContPlanPerInvestSchema);
    ...
    return tLOBContPlanPerInvestSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LOBContPlanPerInvestSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBContPlanPerInvestSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

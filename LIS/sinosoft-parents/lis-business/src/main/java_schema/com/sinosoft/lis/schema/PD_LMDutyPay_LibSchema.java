...
import com.sinosoft.lis.db.PD_LMDutyPay_LibDB;

/*
 * <p>ClassName: PD_LMDutyPay_LibSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 产品定义平台_PDM
 */
public class PD_LMDutyPay_LibSchema implements Schema, Cloneable
{
	// @Field
	/** 责任缴费库代码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		PD_LMDutyPay_LibSchema cloned = (PD_LMDutyPay_LibSchema)super.clone();
	...
	public void setPayPlanCode2(String aPayPlanCode2)
	{
		PayPlanCode2 = aPayPlanCode2;
	}
	/**
	...
	*/
	...
	/**
	* 使用另外一个 PD_LMDutyPay_LibSchema 对象给 Schema 赋值
	* @param: aPD_LMDutyPay_LibSchema PD_LMDutyPay_LibSchema
	**/
	public void setSchema(PD_LMDutyPay_LibSchema aPD_LMDutyPay_LibSchema)
	{
		this.PayPlanCode2 = aPD_LMDutyPay_LibSchema.getPayPlanCode2();
		this.CalCode = aPD_LMDutyPay_LibSchema.getCalCode();
		this.PayPlanName = aPD_LMDutyPay_LibSchema.getPayPlanName();
		this.Type = aPD_LMDutyPay_LibSchema.getType();
		this.PayIntv = aPD_LMDutyPay_LibSchema.getPayIntv();
		this.PayEndYearFlag = aPD_LMDutyPay_LibSchema.getPayEndYearFlag();
		this.PayEndYear = aPD_LMDutyPay_LibSchema.getPayEndYear();
		this.PayEndDateCalRef = aPD_LMDutyPay_LibSchema.getPayEndDateCalRef();
		this.PayEndDateCalMode = aPD_LMDutyPay_LibSchema.getPayEndDateCalMode();
		this.DefaultVal = aPD_LMDutyPay_LibSchema.getDefaultVal();
		this.CnterCalCode = aPD_LMDutyPay_LibSchema.getCnterCalCode();
		this.OthCalCode = aPD_LMDutyPay_LibSchema.getOthCalCode();
		this.Rate = aPD_LMDutyPay_LibSchema.getRate();
		this.MinPay = aPD_LMDutyPay_LibSchema.getMinPay();
		this.AssuYield = aPD_LMDutyPay_LibSchema.getAssuYield();
		this.FeeRate = aPD_LMDutyPay_LibSchema.getFeeRate();
		this.PayToDateCalMode = aPD_LMDutyPay_LibSchema.getPayToDateCalMode();
		this.UrgePayFlag = aPD_LMDutyPay_LibSchema.getUrgePayFlag();
		this.PayLackFlag = aPD_LMDutyPay_LibSchema.getPayLackFlag();
		this.PayOverFlag = aPD_LMDutyPay_LibSchema.getPayOverFlag();
		this.PayOverDeal = aPD_LMDutyPay_LibSchema.getPayOverDeal();
		this.AvoidPayFlag = aPD_LMDutyPay_LibSchema.getAvoidPayFlag();
		this.GracePeriod = aPD_LMDutyPay_LibSchema.getGracePeriod();
		this.PubFlag = aPD_LMDutyPay_LibSchema.getPubFlag();
		this.ZeroFlag = aPD_LMDutyPay_LibSchema.getZeroFlag();
		this.NeedAcc = aPD_LMDutyPay_LibSchema.getNeedAcc();
		this.PayAimClass = aPD_LMDutyPay_LibSchema.getPayAimClass();
		this.Operator = aPD_LMDutyPay_LibSchema.getOperator();
		this.MakeDate = fDate.getDate( aPD_LMDutyPay_LibSchema.getMakeDate());
		this.MakeTime = aPD_LMDutyPay_LibSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aPD_LMDutyPay_LibSchema.getModifyDate());
		this.ModifyTime = aPD_LMDutyPay_LibSchema.getModifyTime();
		this.Standbyflag1 = aPD_LMDutyPay_LibSchema.getStandbyflag1();
		this.Standbyflag2 = aPD_LMDutyPay_LibSchema.getStandbyflag2();
		this.Standbyflag3 = aPD_LMDutyPay_LibSchema.getStandbyflag3();
		this.Standbyflag4 = aPD_LMDutyPay_LibSchema.getStandbyflag4();
		this.Standbyflag5 = aPD_LMDutyPay_LibSchema.getStandbyflag5();
		this.Standbyflag6 = aPD_LMDutyPay_LibSchema.getStandbyflag6();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			//rs.absolute(i);		// 非滚动游标
			if( rs.getString("PayPlanCode2") == null )
				this.PayPlanCode2 = null;
			else
				this.PayPlanCode2 = rs.getString("PayPlanCode2").trim();
		...
		{
			System.out.println("数据库中的PD_LMDutyPay_Lib表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "PD_LMDutyPay_LibSchema";
	...
	public PD_LMDutyPay_LibSchema getSchema()
	{
		PD_LMDutyPay_LibSchema aPD_LMDutyPay_LibSchema = new PD_LMDutyPay_LibSchema();
		aPD_LMDutyPay_LibSchema.setSchema(this);
		return aPD_LMDutyPay_LibSchema;
	}

	public PD_LMDutyPay_LibDB getDB()
	{
		PD_LMDutyPay_LibDB aDBOper = new PD_LMDutyPay_LibDB();
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(PayPlanCode2)); strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpPD_LMDutyPay_Lib>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			PayPlanCode2 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 1, SysConst.PACKAGESPILTER );
			...
			PayEndDateCalRef = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			PayEndDateCalMode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			DefaultVal = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,10,SysConst.PACKAGESPILTER))).doubleValue();
	...
	**/
	...
	{
		String strReturn = "";
		if (FCode.equalsIgnoreCase("PayPlanCode2"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(PayPlanCode2));
		}
	...
	**/
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			case 0:
				strFieldValue = StrTool.GBKToUnicode(PayPlanCode2);
				break;
			case 1:
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		PD_LMDutyPay_LibSchema other = (PD_LMDutyPay_LibSchema)otherObject;
		return
			PayPlanCode2.equals(other.getPayPlanCode2())
	...
	**/
	...
	{
		if( strFieldName.equals("PayPlanCode2") ) {
			return 0;
		}

...
import com.sinosoft.lis.db.FIBusinessRuleDealLogDB;
import com.sinosoft.lis.pubfun.FDate;
import com.sinosoft.utility.*;

/**
 * <p>ClassName: FIBusinessRuleDealLogSchema </p>
 ...
 */
public class FIBusinessRuleDealLogSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(FIBusinessRuleDealLogSchema.class);
	...
	// @Constructor
	public FIBusinessRuleDealLogSchema()
	{
		mErrors = new CErrors();
		...
		PK = pk;
            ...
            public Object clone()
                    throws CloneNotSupportedException
            {
                FIBusinessRuleDealLogSchema cloned = (FIBusinessRuleDealLogSchema)super.clone();
	...
	/**
	* 使用另外一个 FIBusinessRuleDealLogSchema 对象给 Schema 赋值
	* @param: aFIBusinessRuleDealLogSchema FIBusinessRuleDealLogSchema
	**/
	public void setSchema(FIBusinessRuleDealLogSchema aFIBusinessRuleDealLogSchema)
	{
		this.CheckSerialNo = aFIBusinessRuleDealLogSchema.getCheckSerialNo();
		this.RuleDealBatchNo = aFIBusinessRuleDealLogSchema.getRuleDealBatchNo();
		this.RuleID = aFIBusinessRuleDealLogSchema.getRuleID();
		this.RuleDealResult = aFIBusinessRuleDealLogSchema.getRuleDealResult();
		this.DealOperator = aFIBusinessRuleDealLogSchema.getDealOperator();
		this.RuleDealDate = fDate.getDate( aFIBusinessRuleDealLogSchema.getRuleDealDate());
		this.RuleDealTime = aFIBusinessRuleDealLogSchema.getRuleDealTime();
		this.LogFilePath = aFIBusinessRuleDealLogSchema.getLogFilePath();
		this.LogFileName = aFIBusinessRuleDealLogSchema.getLogFileName();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的FIBusinessRuleDealLog表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "FIBusinessRuleDealLogSchema";
	...
	public FIBusinessRuleDealLogSchema getSchema()
	{
		FIBusinessRuleDealLogSchema aFIBusinessRuleDealLogSchema = new FIBusinessRuleDealLogSchema();
		aFIBusinessRuleDealLogSchema.setSchema(this);
		return aFIBusinessRuleDealLogSchema;
	}

	public FIBusinessRuleDealLogDB getDB()
	{
		FIBusinessRuleDealLogDB aDBOper = new FIBusinessRuleDealLogDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpFIBusinessRuleDealLog描述/A>表字段
...
strReturn.append(StrTool.cTrim(LogFileName));
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpFIBusinessRuleDealLog>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			RuleDealBatchNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 2, SysConst.PACKAGESPILTER );
			RuleID = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			RuleDealResult = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			DealOperator = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			RuleDealDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6,SysConst.PACKAGESPILTER));
			RuleDealTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			LogFilePath = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			LogFileName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		FIBusinessRuleDealLogSchema other = (FIBusinessRuleDealLogSchema)otherObject;
		return

...
/*
 * <p>ClassName: LPGeneralSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPGeneralSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPGeneralSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPGeneralSchema cloned = (LPGeneralSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LPGeneralSchema 对象给 Schema 赋值
	* @param: aLPGeneralSchema LPGeneralSchema
	**/
	public void setSchema(LPGeneralSchema aLPGeneralSchema)
	{
		this.EdorNo = aLPGeneralSchema.getEdorNo();
		this.EdorType = aLPGeneralSchema.getEdorType();
		this.GrpContNo = aLPGeneralSchema.getGrpContNo();
		this.PrtNo = aLPGeneralSchema.getPrtNo();
		this.ExecuteCom = aLPGeneralSchema.getExecuteCom();
		this.ManageCom = aLPGeneralSchema.getManageCom();
		this.CustomerNo = aLPGeneralSchema.getCustomerNo();
		this.AddressNo = aLPGeneralSchema.getAddressNo();
		this.EmployeePeoples = aLPGeneralSchema.getEmployeePeoples();
		this.RelationPepoles = aLPGeneralSchema.getRelationPepoles();
		this.IsPayManageCom = aLPGeneralSchema.getIsPayManageCom();
		this.Operator = aLPGeneralSchema.getOperator();
		this.MakeDate = fDate.getDate( aLPGeneralSchema.getMakeDate());
		this.MakeTime = aLPGeneralSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLPGeneralSchema.getModifyDate());
		this.ModifyTime = aLPGeneralSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPGeneral表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPGeneralSchema";
	...
	public LPGeneralSchema getSchema()
	{
		LPGeneralSchema aLPGeneralSchema = new LPGeneralSchema();
		aLPGeneralSchema.setSchema(this);
		return aLPGeneralSchema;
	}

	public LPGeneralDB getDB()
	{
		LPGeneralDB aDBOper = new LPGeneralDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPGeneral描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPGeneral>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			EmployeePeoples= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,9,SysConst.PACKAGESPILTER))).intValue();
			RelationPepoles= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,10,SysConst.PACKAGESPILTER))).intValue();
			IsPayManageCom = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPGeneralSchema other = (LPGeneralSchema)otherObject;
		return

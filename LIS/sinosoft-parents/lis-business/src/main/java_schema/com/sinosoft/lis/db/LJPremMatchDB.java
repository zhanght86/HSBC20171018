...
/*
 * <p>ClassName: LJPremMatchDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LJPremMatchDB extends LJPremMatchSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LJPremMatchDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJPremMatchSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJPremMatchDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getMatchType() == null || this.getMatchType().equals("null")) {
				...
				pstmt.setString(2, this.getMatchType());
			}
			...
			pstmt.setDouble(4, this.getSumMoney());
			pstmt.setDouble(5, this.getHisOutPayMoney());
			pstmt.setDouble(6, this.getDuePayMoney());
			pstmt.setDouble(7, this.getCurOutPayMoney());
			if(this.getMatchDate() == null || this.getMatchDate().equals("null")) {
				pstmt.setNull(8, 91);
			} else {
				pstmt.setDate(8, Date.valueOf(this.getMatchDate()));
			}
			if(this.getMatchTime() == null || this.getMatchTime().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getMatchTime());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJPremMatch WHERE  MatchSerialNo = ?", 
	...
	public LJPremMatchSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJPremMatchSet aLJPremMatchSet = new LJPremMatchSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJPremMatch");
			LJPremMatchSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJPremMatchSchema s1 = new LJPremMatchSchema();
				s1.setSchema(rs,i);
				aLJPremMatchSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJPremMatchSet;
	}

	public LJPremMatchSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJPremMatchSet aLJPremMatchSet = new LJPremMatchSet();
	...
	public LJPremMatchSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJPremMatchSet aLJPremMatchSet = new LJPremMatchSet();
	...
	public LJPremMatchSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJPremMatchSet aLJPremMatchSet = new LJPremMatchSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJPremMatch " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJPremMatchSet
 */
public LJPremMatchSet getData()
{
    int tCount = 0;
    LJPremMatchSet tLJPremMatchSet = new LJPremMatchSet();
    LJPremMatchSchema tLJPremMatchSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJPremMatchDB";
    ...
    {
        tCount = 1;
        tLJPremMatchSchema = new LJPremMatchSchema();
        tLJPremMatchSchema.setSchema(mResultSet, 1);
        tLJPremMatchSet.add(tLJPremMatchSchema);
...
public boolean closeData()
{
	...
	public LJPremMatchSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LJPremMatchSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LMDutyGetDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_2
 */
public class LMDutyGetDB extends LMDutyGetSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LMDutyGetDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LMDutyGetSchema tSchema = this.getSchema();
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LMDutyGet WHERE  GetDutyCode = ?", 
	...
	public LMDutyGetSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMDutyGetSet aLMDutyGetSet = new LMDutyGetSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("LMDutyGet");
			LMDutyGetSchema aSchema = this.getSchema();
			...
			{
				i++;
				LMDutyGetSchema s1 = new LMDutyGetSchema();
				s1.setSchema(rs,i);
				aLMDutyGetSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aLMDutyGetSet;
	}

	public LMDutyGetSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LMDutyGetSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMDutyGetSet aLMDutyGetSet = new LMDutyGetSet();
	...
	public LMDutyGetSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMDutyGetSet aLMDutyGetSet = new LMDutyGetSet();
	...
	public LMDutyGetSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMDutyGetSet aLMDutyGetSet = new LMDutyGetSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LMDutyGet " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LMDutyGetSet
 */
public LMDutyGetSet getData()
{
    int tCount = 0;
    LMDutyGetSet tLMDutyGetSet = new LMDutyGetSet();
    LMDutyGetSchema tLMDutyGetSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LMDutyGetDB";
    ...
    try
    {
        ...
        tLMDutyGetSchema.setSchema(mResultSet, 1);

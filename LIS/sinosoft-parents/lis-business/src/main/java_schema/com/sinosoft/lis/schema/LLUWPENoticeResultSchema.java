...
import com.sinosoft.lis.db.LLUWPENoticeResultDB;

/*
 * <p>ClassName: LLUWPENoticeResultSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 泰康未整理PDM
 */
public class LLUWPENoticeResultSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLUWPENoticeResultSchema.class);
	// @Field
	/** 赔案号 */
	...
	// @Constructor
	public LLUWPENoticeResultSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLUWPENoticeResultSchema cloned = (LLUWPENoticeResultSchema)super.clone();
	...
	public void setCustomerNo(String aCustomerNo)
	{
		if(aCustomerNo!=null && aCustomerNo.length()>24)
			throw new IllegalArgumentException("体检客户号码CustomerNo值"+aCustomerNo+"的长度"+aCustomerNo.length()+"大于最大值24");
	...
	public void setName(String aName)
	{
		if(aName!=null && aName.length()>120)
			throw new IllegalArgumentException("体检客户姓名Name值"+aName+"的长度"+aName.length()+"大于最大值120");
		Name = aName;
	}
	...
	public void setSerialNo(String aSerialNo)
	{
		if(aSerialNo!=null && aSerialNo.length()>20)
			throw new IllegalArgumentException("内部子序列号SerialNo值"+aSerialNo+"的长度"+aSerialNo.length()+"大于最大值20");
		SerialNo = aSerialNo;
	}
	...
	{
		if(aDisDesb!=null && aDisDesb.length()>90)
			throw new IllegalArgumentException("疾病症状DisDesb值"+aDisDesb+"的长度"+aDisDesb.length()+"大于最大值90");
		DisDesb = aDisDesb;
	}
	...
	{
		if(aDisResult!=null && aDisResult.length()>90)
			throw new IllegalArgumentException("疾病结论DisResult值"+aDisResult+"的长度"+aDisResult.length()+"大于最大值90");
	...
	{
		if(aICDCode!=null && aICDCode.length()>20)
			throw new IllegalArgumentException("Icd编码ICDCode值"+aICDCode+"的长度"+aICDCode.length()+"大于最大值20");
		ICDCode = aICDCode;
	}
	...
	/**
	* 使用另外一个 LLUWPENoticeResultSchema 对象给 Schema 赋值
	* @param: aLLUWPENoticeResultSchema LLUWPENoticeResultSchema
	**/
	public void setSchema(LLUWPENoticeResultSchema aLLUWPENoticeResultSchema)
	{
		this.ClmNo = aLLUWPENoticeResultSchema.getClmNo();
		this.GrpContNo = aLLUWPENoticeResultSchema.getGrpContNo();
		this.ContNo = aLLUWPENoticeResultSchema.getContNo();
		this.ProposalContNo = aLLUWPENoticeResultSchema.getProposalContNo();
		this.PrtSeq = aLLUWPENoticeResultSchema.getPrtSeq();
		this.CustomerNo = aLLUWPENoticeResultSchema.getCustomerNo();
		this.Name = aLLUWPENoticeResultSchema.getName();
		this.SerialNo = aLLUWPENoticeResultSchema.getSerialNo();
		this.DisDesb = aLLUWPENoticeResultSchema.getDisDesb();
		this.DisResult = aLLUWPENoticeResultSchema.getDisResult();
		this.ICDCode = aLLUWPENoticeResultSchema.getICDCode();
		this.Remark = aLLUWPENoticeResultSchema.getRemark();
		this.Operator = aLLUWPENoticeResultSchema.getOperator();
		this.MakeDate = fDate.getDate( aLLUWPENoticeResultSchema.getMakeDate());
		this.MakeTime = aLLUWPENoticeResultSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLLUWPENoticeResultSchema.getModifyDate());
		this.ModifyTime = aLLUWPENoticeResultSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLUWPENoticeResult表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLUWPENoticeResultSchema";
	...
	public LLUWPENoticeResultSchema getSchema()
	{
		LLUWPENoticeResultSchema aLLUWPENoticeResultSchema = new LLUWPENoticeResultSchema();
		aLLUWPENoticeResultSchema.setSchema(this);
		return aLLUWPENoticeResultSchema;
	}

	public LLUWPENoticeResultDB getDB()
	{
		LLUWPENoticeResultDB aDBOper = new LLUWPENoticeResultDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLUWPENoticeResult描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(DisDesb)); strReturn.append(SysConst.PACKAGESPILTER);
		strReturn.append(StrTool.cTrim(DisResult)); strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLUWPENoticeResult>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			DisDesb = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			DisResult = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			ICDCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LLUWPENoticeResultSchema other = (LLUWPENoticeResultSchema)otherObject;
		return

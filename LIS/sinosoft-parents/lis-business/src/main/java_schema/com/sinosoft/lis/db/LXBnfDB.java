...
/*
 * <p>ClassName: LXBnfDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 兼业系统
 */
public class LXBnfDB extends LXBnfSchema
{
private static Logger logger = Logger.getLogger(LXBnfDB.class);
	...
	**/
	...
	// @Constructor
	public LXBnfDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LXBnfSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getBnfType() == null || this.getBnfType().equals("null")) {
				...
				pstmt.setString(2, StrTool.space1(this.getBnfType(), 1));
			}
			pstmt.setInt(3, this.getBnfNo());
			if(this.getBnfGrade() == null || this.getBnfGrade().equals("null")) {
				pstmt.setNull(4, 1);
			} else {
				pstmt.setString(4, StrTool.space1(this.getBnfGrade(), 1));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getBnfType() == null || this.getBnfType().equals("null")) {
				...
				pstmt.setString(2, this.getBnfType());
			}
			pstmt.setInt(3, this.getBnfNo());
			if(this.getBnfGrade() == null || this.getBnfGrade().equals("null")) {
				pstmt.setNull(4, 1);
			} else {
				pstmt.setString(4, this.getBnfGrade());
			}
			...
			pstmt.setDouble(6, this.getBnfLot());
			...
			if(this.getCardNo() == null || this.getCardNo().equals("null")) {
				...
				pstmt.setString(17, this.getCardNo());
			}
			if(this.getBnfType() == null || this.getBnfType().equals("null")) {
				pstmt.setNull(18, 1);
			} else {
				pstmt.setString(18, StrTool.space1(this.getBnfType(), 1));
			}
			pstmt.setInt(19, this.getBnfNo());
			if(this.getBnfGrade() == null || this.getBnfGrade().equals("null")) {
				pstmt.setNull(20, 1);
			} else {
				pstmt.setString(20, StrTool.space1(this.getBnfGrade(), 1));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LXBnf WHERE  CardNo = ? AND BnfType = ? AND BnfNo = ? AND BnfGrade = ?", 
	...
	public LXBnfSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LXBnfSet aLXBnfSet = new LXBnfSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LXBnf");
			LXBnfSchema aSchema = this.getSchema();
			...
			{
				i++;
				LXBnfSchema s1 = new LXBnfSchema();
				s1.setSchema(rs,i);
				aLXBnfSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LXBnfSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LXBnfSet aLXBnfSet = new LXBnfSet();
	...
	public LXBnfSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LXBnfSet aLXBnfSet = new LXBnfSet();
	...
	public LXBnfSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LXBnfSet aLXBnfSet = new LXBnfSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LXBnf " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LXBnfSet
 */
public LXBnfSet getData()
{
    int tCount = 0;
    LXBnfSet tLXBnfSet = new LXBnfSet();
    LXBnfSchema tLXBnfSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LXBnfDB";
    ...
    {
        tCount = 1;
        tLXBnfSchema = new LXBnfSchema();
        tLXBnfSchema.setSchema(mResultSet, 1);
        tLXBnfSet.add(tLXBnfSchema);
...
public boolean closeData()
{
	...
	public LXBnfSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LXBnfSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCRReportDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCRReportDB extends LCRReportSchema
{
private static Logger logger = Logger.getLogger(LCRReportDB.class);
	...
	**/
	...
	// @Constructor
	public LCRReportDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCRReportSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getSendDate() == null || this.getSendDate().equals("null")) {
				...
				pstmt.setDate(25, Date.valueOf(this.getSendDate()));
			}
			if(this.getSendTime() == null || this.getSendTime().equals("null")) {
				pstmt.setNull(26, 1);
			} else {
				pstmt.setString(26, this.getSendTime());
			}
			// set where condition
			if(this.getProposalContNo() == null || this.getProposalContNo().equals("null")) {
				pstmt.setNull(27, 12);
			...
			if(this.getPrtSeq() == null || this.getPrtSeq().equals("null")) {
				...
				pstmt.setString(28, this.getPrtSeq());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCRReport WHERE  ProposalContNo = ? AND PrtSeq = ?", 
	...
	public LCRReportSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCRReportSet aLCRReportSet = new LCRReportSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCRReport");
			LCRReportSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCRReportSchema s1 = new LCRReportSchema();
				s1.setSchema(rs,i);
				aLCRReportSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCRReportSet;

	}

	public LCRReportSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCRReportSet aLCRReportSet = new LCRReportSet();
	...
	public LCRReportSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCRReportSet aLCRReportSet = new LCRReportSet();
	...
	public LCRReportSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCRReportSet aLCRReportSet = new LCRReportSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCRReport " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LCRReportSet
 */
public LCRReportSet getData()
{
    int tCount = 0;
    LCRReportSet tLCRReportSet = new LCRReportSet();
    LCRReportSchema tLCRReportSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCRReportDB";
    ...
    {
        tCount = 1;
        tLCRReportSchema = new LCRReportSchema();
        tLCRReportSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCRReportSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCRReportSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

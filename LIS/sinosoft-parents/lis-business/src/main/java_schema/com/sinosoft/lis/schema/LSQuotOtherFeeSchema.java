...
import com.sinosoft.lis.db.LSQuotOtherFeeDB;

/*
 * <p>ClassName: LSQuotOtherFeeSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LSQuotOtherFeeSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LSQuotOtherFeeSchema.class);
	// @Field
	/** 序号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LSQuotOtherFeeSchema cloned = (LSQuotOtherFeeSchema)super.clone();
	...
	public void setFeeType(String aFeeType)
	{
		if(aFeeType!=null && aFeeType.length()>2)
			throw new IllegalArgumentException("费用类型FeeType值"+aFeeType+"的长度"+aFeeType.length()+"大于最大值2");
		FeeType = aFeeType;
	}
	...
	public void setOtherFeeDesc(String aOtherFeeDesc)
	{
		if(aOtherFeeDesc!=null && aOtherFeeDesc.length()>30)
			throw new IllegalArgumentException("其他类型描述OtherFeeDesc值"+aOtherFeeDesc+"的长度"+aOtherFeeDesc.length()+"大于最大值30");
		OtherFeeDesc = aOtherFeeDesc;
	}
	...
	public void setFeeValue(String aFeeValue)
	{
		if(aFeeValue!=null && aFeeValue.length()>20)
			throw new IllegalArgumentException("费用值FeeValue值"+aFeeValue+"的长度"+aFeeValue.length()+"大于最大值20");
		FeeValue = aFeeValue;
	}
	...
	/**
	* 使用另外一个 LSQuotOtherFeeSchema 对象给 Schema 赋值
	* @param: aLSQuotOtherFeeSchema LSQuotOtherFeeSchema
	**/
	public void setSchema(LSQuotOtherFeeSchema aLSQuotOtherFeeSchema)
	{
		this.SerialNo = aLSQuotOtherFeeSchema.getSerialNo();
		this.QuotNo = aLSQuotOtherFeeSchema.getQuotNo();
		this.QuotBatNo = aLSQuotOtherFeeSchema.getQuotBatNo();
		this.FeeType = aLSQuotOtherFeeSchema.getFeeType();
		this.OtherFeeDesc = aLSQuotOtherFeeSchema.getOtherFeeDesc();
		this.FeeValue = aLSQuotOtherFeeSchema.getFeeValue();
		this.Remark = aLSQuotOtherFeeSchema.getRemark();
		this.Segment1 = aLSQuotOtherFeeSchema.getSegment1();
		this.Segment2 = aLSQuotOtherFeeSchema.getSegment2();
		this.Segment3 = aLSQuotOtherFeeSchema.getSegment3();
		this.MakeOperator = aLSQuotOtherFeeSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLSQuotOtherFeeSchema.getMakeDate());
		this.MakeTime = aLSQuotOtherFeeSchema.getMakeTime();
		this.ModifyOperator = aLSQuotOtherFeeSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLSQuotOtherFeeSchema.getModifyDate());
		this.ModifyTime = aLSQuotOtherFeeSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			if( rs.getString("OtherFeeDesc") == null )
				this.OtherFeeDesc = null;
			else
				this.OtherFeeDesc = rs.getString("OtherFeeDesc").trim();
		...
		{
			logger.debug("数据库中的LSQuotOtherFee表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LSQuotOtherFeeSchema";
	...
	public LSQuotOtherFeeSchema getSchema()
	{
		LSQuotOtherFeeSchema aLSQuotOtherFeeSchema = new LSQuotOtherFeeSchema();
		aLSQuotOtherFeeSchema.setSchema(this);
		return aLSQuotOtherFeeSchema;
	}

	public LSQuotOtherFeeDB getDB()
	{
		LSQuotOtherFeeDB aDBOper = new LSQuotOtherFeeDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotOtherFee描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(OtherFeeDesc)); strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotOtherFee>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			OtherFeeDesc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			FeeValue = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("OtherFeeDesc"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(OtherFeeDesc));
		}
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 3:
				...
				break;
			case 4:
				strFieldValue = StrTool.GBKToUnicode(OtherFeeDesc);
				break;
			case 5:
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LSQuotOtherFeeSchema other = (LSQuotOtherFeeSchema)otherObject;
		return
			SerialNo.equals(other.getSerialNo())
			&& QuotNo.equals(other.getQuotNo())
			&& QuotBatNo == other.getQuotBatNo()
			&& FeeType.equals(other.getFeeType())
			&& OtherFeeDesc.equals(other.getOtherFeeDesc())
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("OtherFeeDesc") ) {
			return 4;
		}

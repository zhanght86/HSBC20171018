...
/*
 * <p>ClassName: LDAutoAssignTraceSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 自动分配
 */
public class LDAutoAssignTraceSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LDAutoAssignTraceSchema.class);
	...
	// @Constructor
	public LDAutoAssignTraceSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LDAutoAssignTraceSchema cloned = (LDAutoAssignTraceSchema)super.clone();
	...
	public void setActuallyAmount(int aActuallyAmount)
	{
		ActuallyAmount = aActuallyAmount;
	}
	public void setActuallyAmount(String aActuallyAmount)
	{
		if (aActuallyAmount != null && !aActuallyAmount.equals(""))
		{
			Integer tInteger = new Integer(aActuallyAmount);
			int i = tInteger.intValue();
			ActuallyAmount = i;
		}
	}
	...
	/**
	* 使用另外一个 LDAutoAssignTraceSchema 对象给 Schema 赋值
	* @param: aLDAutoAssignTraceSchema LDAutoAssignTraceSchema
	**/
	public void setSchema(LDAutoAssignTraceSchema aLDAutoAssignTraceSchema)
	{
		this.Serialno = aLDAutoAssignTraceSchema.getSerialno();
		this.TaskNo = aLDAutoAssignTraceSchema.getTaskNo();
		this.Activityid = aLDAutoAssignTraceSchema.getActivityid();
		this.ServiceName = aLDAutoAssignTraceSchema.getServiceName();
		this.AssignNo = aLDAutoAssignTraceSchema.getAssignNo();
		this.TaskStartTime = aLDAutoAssignTraceSchema.getTaskStartTime();
		this.TaskEndTime = aLDAutoAssignTraceSchema.getTaskEndTime();
		this.PlanAmount = aLDAutoAssignTraceSchema.getPlanAmount();
		this.ActuallyAmount = aLDAutoAssignTraceSchema.getActuallyAmount();
		this.Operator = aLDAutoAssignTraceSchema.getOperator();
		this.MakeTime = aLDAutoAssignTraceSchema.getMakeTime();
		this.MakeDate = fDate.getDate( aLDAutoAssignTraceSchema.getMakeDate());
		this.UWPopedom = aLDAutoAssignTraceSchema.getUWPopedom();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			this.ActuallyAmount = rs.getInt("ActuallyAmount");
		...
		{
			logger.debug("数据库中的LDAutoAssignTrace表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LDAutoAssignTraceSchema";
	...
	public LDAutoAssignTraceSchema getSchema()
	{
		LDAutoAssignTraceSchema aLDAutoAssignTraceSchema = new LDAutoAssignTraceSchema();
		aLDAutoAssignTraceSchema.setSchema(this);
		return aLDAutoAssignTraceSchema;
	}

	public LDAutoAssignTraceDB getDB()
	{
		LDAutoAssignTraceDB aDBOper = new LDAutoAssignTraceDB();
	...
	**/
	public String encode()
	{
		...
		strReturn.append( ChgData.chgData(ActuallyAmount));strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDAutoAssignTrace>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			AssignNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			TaskStartTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			TaskEndTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			PlanAmount= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,8,SysConst.PACKAGESPILTER))).intValue();
			ActuallyAmount= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,9,SysConst.PACKAGESPILTER))).intValue();
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("ActuallyAmount"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(ActuallyAmount));
		}
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 7:
				...
				break;
			case 8:
				strFieldValue = String.valueOf(ActuallyAmount);
				break;
			case 9:
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LDAutoAssignTraceSchema other = (LDAutoAssignTraceSchema)otherObject;
		return
			...
			&& ActuallyAmount == other.getActuallyAmount()
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("ActuallyAmount") ) {
			return 8;
		}

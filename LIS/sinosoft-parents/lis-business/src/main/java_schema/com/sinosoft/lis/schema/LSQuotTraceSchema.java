...
import com.sinosoft.lis.db.LSQuotTraceDB;

/*
 * <p>ClassName: LSQuotTraceSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LSQuotTraceSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LSQuotTraceSchema.class);
	// @Field
	/** 流水号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LSQuotTraceSchema cloned = (LSQuotTraceSchema)super.clone();
	...
	public void setActivityID(String aActivityID)
	{
		if(aActivityID!=null && aActivityID.length()>10)
			throw new IllegalArgumentException("记录节点ActivityID值"+aActivityID+"的长度"+aActivityID.length()+"大于最大值10");
	...
	/**
	* 使用另外一个 LSQuotTraceSchema 对象给 Schema 赋值
	* @param: aLSQuotTraceSchema LSQuotTraceSchema
	**/
	public void setSchema(LSQuotTraceSchema aLSQuotTraceSchema)
	{
		this.SerialNo = aLSQuotTraceSchema.getSerialNo();
		this.ActivityID = aLSQuotTraceSchema.getActivityID();
		this.QuotNo = aLSQuotTraceSchema.getQuotNo();
		this.QuotBatNo = aLSQuotTraceSchema.getQuotBatNo();
		this.MakeOperator = aLSQuotTraceSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLSQuotTraceSchema.getMakeDate());
		this.MakeTime = aLSQuotTraceSchema.getMakeTime();
		this.ModifyOperator = aLSQuotTraceSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLSQuotTraceSchema.getModifyDate());
		this.ModifyTime = aLSQuotTraceSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LSQuotTrace表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LSQuotTraceSchema";
	...
	public LSQuotTraceSchema getSchema()
	{
		LSQuotTraceSchema aLSQuotTraceSchema = new LSQuotTraceSchema();
		aLSQuotTraceSchema.setSchema(this);
		return aLSQuotTraceSchema;
	}

	public LSQuotTraceDB getDB()
	{
		LSQuotTraceDB aDBOper = new LSQuotTraceDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotTrace描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotTrace>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			QuotNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			QuotBatNo= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,4,SysConst.PACKAGESPILTER))).intValue();
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LSQuotTraceSchema other = (LSQuotTraceSchema)otherObject;
		return

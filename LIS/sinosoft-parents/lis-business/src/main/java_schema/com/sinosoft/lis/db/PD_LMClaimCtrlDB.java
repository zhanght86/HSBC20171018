...
/*
 * <p>ClassName: PD_LMClaimCtrlDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 产品定义平台_PDM
 */
public class PD_LMClaimCtrlDB extends PD_LMClaimCtrlSchema
{
private static Logger logger = Logger.getLogger(PD_LMClaimCtrlDB.class);
	...
	**/
	...
	// @Constructor
	public PD_LMClaimCtrlDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		PD_LMClaimCtrlSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "PD_LMClaimCtrlDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getClaimCtrlCode() == null || this.getClaimCtrlCode().equals("null")) {
				...
				pstmt.setString(31, this.getClaimCtrlCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM PD_LMClaimCtrl WHERE  ClaimCtrlCode = ?", 
	...
	public PD_LMClaimCtrlSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		PD_LMClaimCtrlSet aPD_LMClaimCtrlSet = new PD_LMClaimCtrlSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("PD_LMClaimCtrl");
			PD_LMClaimCtrlSchema aSchema = this.getSchema();
			...
			{
				i++;
				PD_LMClaimCtrlSchema s1 = new PD_LMClaimCtrlSchema();
				s1.setSchema(rs,i);
				aPD_LMClaimCtrlSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aPD_LMClaimCtrlSet;

	}

	public PD_LMClaimCtrlSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMClaimCtrlSet aPD_LMClaimCtrlSet = new PD_LMClaimCtrlSet();
	...
	public PD_LMClaimCtrlSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		PD_LMClaimCtrlSet aPD_LMClaimCtrlSet = new PD_LMClaimCtrlSet();
	...
	public PD_LMClaimCtrlSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMClaimCtrlSet aPD_LMClaimCtrlSet = new PD_LMClaimCtrlSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update PD_LMClaimCtrl " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return PD_LMClaimCtrlSet
 */
public PD_LMClaimCtrlSet getData()
{
    int tCount = 0;
    PD_LMClaimCtrlSet tPD_LMClaimCtrlSet = new PD_LMClaimCtrlSet();
    PD_LMClaimCtrlSchema tPD_LMClaimCtrlSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "PD_LMClaimCtrlDB";
    ...
    {
        tCount = 1;
        tPD_LMClaimCtrlSchema = new PD_LMClaimCtrlSchema();
        tPD_LMClaimCtrlSchema.setSchema(mResultSet, 1);
        tPD_LMClaimCtrlSet.add(tPD_LMClaimCtrlSchema);
...
public boolean closeData()
{
	...
	public PD_LMClaimCtrlSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public PD_LMClaimCtrlSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

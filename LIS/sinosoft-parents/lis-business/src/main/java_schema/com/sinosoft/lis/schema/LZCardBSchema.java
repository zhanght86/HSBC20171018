...
/*
 * <p>ClassName: LZCardBSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 单证管理
 */
public class LZCardBSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LZCardBSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LZCardBSchema cloned = (LZCardBSchema)super.clone();
	...
	*/
	...
	/**
	* 1、待入库    处于待入库池中，需要进行入库确认。<p>
	* 2、已入库（库存）    入库确认后的处于已入库(库存)。<p>
	* 3、已发放未核销    发放至B(部门)与D(代理人)下的未核销单证。<p>
	* 4、自动缴销    交费使用的单证，由系统在交费时自动实现。<p>
	* 5、手工缴销    交费使用的单证，人工触发。<p>
	* 6、使用作废<p>
	* 7、停用作废<p>
	* 8、逾期    超过使用截止日期的单证，由系统自动置为此状态。<p>
	* 9、挂失<p>
	* 10、遗失<p>
	* 11、销毁
	*/
	...
	*/
	...
	/**
	* 使用另外一个 LZCardBSchema 对象给 Schema 赋值
	* @param: aLZCardBSchema LZCardBSchema
	**/
	public void setSchema(LZCardBSchema aLZCardBSchema)
	{
		this.CertifyCode = aLZCardBSchema.getCertifyCode();
		this.SubCode = aLZCardBSchema.getSubCode();
		this.RiskCode = aLZCardBSchema.getRiskCode();
		this.RiskVersion = aLZCardBSchema.getRiskVersion();
		this.StartNo = aLZCardBSchema.getStartNo();
		this.EndNo = aLZCardBSchema.getEndNo();
		this.SendOutCom = aLZCardBSchema.getSendOutCom();
		this.ReceiveCom = aLZCardBSchema.getReceiveCom();
		this.SumCount = aLZCardBSchema.getSumCount();
		this.Prem = aLZCardBSchema.getPrem();
		this.Amnt = aLZCardBSchema.getAmnt();
		this.Handler = aLZCardBSchema.getHandler();
		this.HandleDate = fDate.getDate( aLZCardBSchema.getHandleDate());
		this.InvaliDate = fDate.getDate( aLZCardBSchema.getInvaliDate());
		this.TakeBackNo = aLZCardBSchema.getTakeBackNo();
		this.SaleChnl = aLZCardBSchema.getSaleChnl();
		this.StateFlag = aLZCardBSchema.getStateFlag();
		this.OperateFlag = aLZCardBSchema.getOperateFlag();
		this.PayFlag = aLZCardBSchema.getPayFlag();
		this.EnterAccFlag = aLZCardBSchema.getEnterAccFlag();
		this.Reason = aLZCardBSchema.getReason();
		this.State = aLZCardBSchema.getState();
		this.Operator = aLZCardBSchema.getOperator();
		this.MakeDate = fDate.getDate( aLZCardBSchema.getMakeDate());
		this.MakeTime = aLZCardBSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLZCardBSchema.getModifyDate());
		this.ModifyTime = aLZCardBSchema.getModifyTime();
		this.ApplyNo = aLZCardBSchema.getApplyNo();
		this.AgentCom = aLZCardBSchema.getAgentCom();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LZCardB表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LZCardBSchema";
	...
	public LZCardBSchema getSchema()
	{
		LZCardBSchema aLZCardBSchema = new LZCardBSchema();
		aLZCardBSchema.setSchema(this);
		return aLZCardBSchema;
	}

	public LZCardBDB getDB()
	{
		LZCardBDB aDBOper = new LZCardBDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLZCardB描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLZCardB>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LZCardBSchema other = (LZCardBSchema)otherObject;
		return

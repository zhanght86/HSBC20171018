...
/*
 * <p>ClassName: PD_LMRiskAppDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PD_LMRiskApp
 */
public class PD_LMRiskAppDB extends PD_LMRiskAppSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public PD_LMRiskAppDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		PD_LMRiskAppSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "PD_LMRiskAppDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getStandbyflag1() == null || this.getStandbyflag1().equals("null")) {
				...
				pstmt.setString(73, this.getStandbyflag1());
			}
			if(this.getStandbyflag2() == null || this.getStandbyflag2().equals("null")) {
				pstmt.setNull(74, 12);
			} else {
				pstmt.setString(74, this.getStandbyflag2());
			}
			pstmt.setInt(75, this.getStandbyflag3());
			pstmt.setInt(76, this.getStandbyflag4());
			pstmt.setDouble(77, this.getStandbyflag5());
			pstmt.setDouble(78, this.getStandbyflag6());
			if(this.getCancleForeGetSpecFlag() == null || this.getCancleForeGetSpecFlag().equals("null")) {
				pstmt.setNull(79, 1);
			} else {
				pstmt.setString(79, this.getCancleForeGetSpecFlag());
			}
			pstmt.setInt(80, this.getSignDateCalMode2());
			pstmt.setInt(81, this.getSignDateCalMode3());
			if(this.getHealthType() == null || this.getHealthType().equals("null")) {
				pstmt.setNull(82, 12);
			} else {
				pstmt.setString(82, this.getHealthType());
			}
			if(this.getRiskTypeAcc() == null || this.getRiskTypeAcc().equals("null")) {
				pstmt.setNull(83, 12);
			} else {
				pstmt.setString(83, this.getRiskTypeAcc());
			}
			if(this.getRiskPeriodAcc() == null || this.getRiskPeriodAcc().equals("null")) {
				pstmt.setNull(84, 12);
			} else {
				pstmt.setString(84, this.getRiskPeriodAcc());
			}
			if(this.getAutoPayType() == null || this.getAutoPayType().equals("null")) {
				pstmt.setNull(85, 12);
			} else {
				pstmt.setString(85, this.getAutoPayType());
			}
			if(this.getAutoETIFlag() == null || this.getAutoETIFlag().equals("null")) {
				pstmt.setNull(86, 12);
			} else {
				pstmt.setString(86, this.getAutoETIFlag());
			}
			if(this.getAutoETIType() == null || this.getAutoETIType().equals("null")) {
				pstmt.setNull(87, 12);
			} else {
				pstmt.setString(87, this.getAutoETIType());
			}
			if(this.getAutoCTFlag() == null || this.getAutoCTFlag().equals("null")) {
				pstmt.setNull(88, 12);
			} else {
				pstmt.setString(88, this.getAutoCTFlag());
			}
			if(this.getNonParFlag() == null || this.getNonParFlag().equals("null")) {
				pstmt.setNull(89, 12);
			} else {
				pstmt.setString(89, this.getNonParFlag());
			}
			if(this.getBackDateFlag() == null || this.getBackDateFlag().equals("null")) {
				pstmt.setNull(90, 12);
			} else {
				pstmt.setString(90, this.getBackDateFlag());
			}
			if(this.getHoneymoonFlag() == null || this.getHoneymoonFlag().equals("null")) {
				pstmt.setNull(91, 12);
			} else {
				pstmt.setString(91, this.getHoneymoonFlag());
			}
			if(this.getNLGFlag() == null || this.getNLGFlag().equals("null")) {
				pstmt.setNull(92, 12);
			} else {
				pstmt.setString(92, this.getNLGFlag());
			}
			// set where condition
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(93, 12);
			} else {
				pstmt.setString(93, this.getRiskCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM PD_LMRiskApp WHERE  RiskCode = ?", 
	...
	public PD_LMRiskAppSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskAppSet aPD_LMRiskAppSet = new PD_LMRiskAppSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("PD_LMRiskApp");
			PD_LMRiskAppSchema aSchema = this.getSchema();
			...
			{
				i++;
				PD_LMRiskAppSchema s1 = new PD_LMRiskAppSchema();
				s1.setSchema(rs,i);
				aPD_LMRiskAppSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aPD_LMRiskAppSet;
	}

	public PD_LMRiskAppSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskAppSet aPD_LMRiskAppSet = new PD_LMRiskAppSet();
	...
	public PD_LMRiskAppSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskAppSet aPD_LMRiskAppSet = new PD_LMRiskAppSet();
	...
	public PD_LMRiskAppSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskAppSet aPD_LMRiskAppSet = new PD_LMRiskAppSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update PD_LMRiskApp " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return PD_LMRiskAppSet
 */
public PD_LMRiskAppSet getData()
{
    int tCount = 0;
    PD_LMRiskAppSet tPD_LMRiskAppSet = new PD_LMRiskAppSet();
    PD_LMRiskAppSchema tPD_LMRiskAppSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "PD_LMRiskAppDB";
    ...
    {
        tCount = 1;
        tPD_LMRiskAppSchema = new PD_LMRiskAppSchema();
        tPD_LMRiskAppSchema.setSchema(mResultSet, 1);
        tPD_LMRiskAppSet.add(tPD_LMRiskAppSchema);

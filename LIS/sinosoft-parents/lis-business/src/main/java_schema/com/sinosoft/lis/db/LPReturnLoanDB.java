...
/*
 * <p>ClassName: LPReturnLoanDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保
 */
public class LPReturnLoanDB extends LPReturnLoanSchema
{
private static Logger logger = Logger.getLogger(LPReturnLoanDB.class);
	...
	**/
	...
	// @Constructor
	public LPReturnLoanDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LPReturnLoan" );
		mflag = true;
	}

	public LPReturnLoanDB()
	{
		con = null;
		db = new DBOper( "LPReturnLoan" );
		mflag = false;
	}
	...
	{
		LPReturnLoanSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPReturnLoanDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LPReturnLoan WHERE  ContNo = ? AND PolNo = ? AND EdorType = ? AND EdorNo = ? AND LoanNo = ? AND Currency = ?");
			...
			if(this.getLoanNo() == null || this.getLoanNo().equals("null")) {
				...
				pstmt.setString(5, this.getLoanNo());
			}
		...
		SQLString sqlObj = new SQLString("LPReturnLoan");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LPReturnLoan SET  ContNo = ? , PolNo = ? , EdorType = ? , SerialNo = ? , ActuGetNo = ? , LoanType = ? , OrderNo = ? , LoanDate = ? , PayOffDate = ? , SumMoney = ? , InputFlag = ? , InterestType = ? , InterestRate = ? , InterestMode = ? , RateCalType = ? , RateCalCode = ? , SpecifyRate = ? , LeaveMoney = ? , PayOffFlag = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , EdorNo = ? , LoanNo = ? , ReturnMoney = ? , ReturnInterest = ? , ClmNo = ? , Currency = ? WHERE  ContNo = ? AND PolNo = ? AND EdorType = ? AND EdorNo = ? AND LoanNo = ? AND Currency = ?");
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getContNo());
			...
			if(this.getLoanNo() == null || this.getLoanNo().equals("null")) {
				...
				pstmt.setString(26, this.getLoanNo());
			}
			pstmt.setDouble(27, this.getReturnMoney());
			pstmt.setDouble(28, this.getReturnInterest());
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getClmNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(30, 12);
			} else {
			...
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				...
				pstmt.setString(31, this.getContNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(32, 12);
			} else {
			...
			if(this.getLoanNo() == null || this.getLoanNo().equals("null")) {
				...
				pstmt.setString(35, this.getLoanNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LPReturnLoan VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPReturnLoan WHERE  ContNo = ? AND PolNo = ? AND EdorType = ? AND EdorNo = ? AND LoanNo = ? AND Currency = ?", 
	...
	public LPReturnLoanSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPReturnLoanSet aLPReturnLoanSet = new LPReturnLoanSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPReturnLoan");
			LPReturnLoanSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPReturnLoanSchema s1 = new LPReturnLoanSchema();
				s1.setSchema(rs,i);
				aLPReturnLoanSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPReturnLoanSet;

	}

	public LPReturnLoanSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPReturnLoanSet aLPReturnLoanSet = new LPReturnLoanSet();
	...
	public LPReturnLoanSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPReturnLoanSet aLPReturnLoanSet = new LPReturnLoanSet();
	...
	public LPReturnLoanSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPReturnLoanSet aLPReturnLoanSet = new LPReturnLoanSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPReturnLoan " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPReturnLoanSet
 */
public LPReturnLoanSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLPReturnLoanSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LPReturnLoanSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPReturnLoanSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
{
private static Logger logger = Logger.getLogger(LBPremSchema.class);
	...
	**/
	public void setSchema(LBPremSchema aLBPremSchema)
	{
		...
		this.Currency = aLBPremSchema.getCurrency();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBPrem表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			SecInsuAddPoint = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,34,SysConst.PACKAGESPILTER))).doubleValue();

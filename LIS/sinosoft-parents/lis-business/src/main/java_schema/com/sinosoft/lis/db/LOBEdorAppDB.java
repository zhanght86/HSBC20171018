...
/*
 * <p>ClassName: LOBEdorAppDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LOBEdorAppDB extends LOBEdorAppSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LOBEdorAppDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LOBEdorAppSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBEdorAppDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getEdorAcceptNo() == null || this.getEdorAcceptNo().equals("null")) {
				...
				pstmt.setString(48, this.getEdorAcceptNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBEdorApp WHERE  EdorAcceptNo = ?", 
	...
	public LOBEdorAppSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBEdorAppSet aLOBEdorAppSet = new LOBEdorAppSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBEdorApp");
			LOBEdorAppSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBEdorAppSchema s1 = new LOBEdorAppSchema();
				s1.setSchema(rs,i);
				aLOBEdorAppSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBEdorAppSet;
	}

	public LOBEdorAppSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBEdorAppSet aLOBEdorAppSet = new LOBEdorAppSet();
	...
	public LOBEdorAppSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBEdorAppSet aLOBEdorAppSet = new LOBEdorAppSet();
	...
	public LOBEdorAppSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBEdorAppSet aLOBEdorAppSet = new LOBEdorAppSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBEdorApp " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBEdorAppSet
 */
public LOBEdorAppSet getData()
{
    int tCount = 0;
    LOBEdorAppSet tLOBEdorAppSet = new LOBEdorAppSet();
    LOBEdorAppSchema tLOBEdorAppSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBEdorAppDB";
    ...
    {
        tCount = 1;
        tLOBEdorAppSchema = new LOBEdorAppSchema();
        tLOBEdorAppSchema.setSchema(mResultSet, 1);
        tLOBEdorAppSet.add(tLOBEdorAppSchema);
...
public boolean closeData()
{
	...
	public LOBEdorAppSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBEdorAppSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCGetDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCGetDB extends LCGetSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCGetDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCGetSchema tSchema = this.getSchema();
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCGet WHERE  PolNo = ? AND DutyCode = ? AND GetDutyCode = ?", 
	...
	public LCGetSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCGet");
			LCGetSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCGetSchema s1 = new LCGetSchema();
				s1.setSchema(rs,i);
				aLCGetSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LCGetSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGetSet aLCGetSet = new LCGetSet();
	...
	public LCGetSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGetSet aLCGetSet = new LCGetSet();
	...
	public LCGetSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGetSet aLCGetSet = new LCGetSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCGet " + sqlObj.getUpdPart() + " where " + strWherePart;
...
public LCGetSet getData()
{
    ...
    try
    {
        ...
        tLCGetSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCGetSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCGetSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

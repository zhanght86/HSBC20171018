...
/*
 * <p>ClassName: PD_LMCheckFieldDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 产品定义平台_PDM
 */
public class PD_LMCheckFieldDB extends PD_LMCheckFieldSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public PD_LMCheckFieldDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		PD_LMCheckFieldSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "PD_LMCheckFieldDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getFieldName() == null || this.getFieldName().equals("null")) {
				...
				pstmt.setString(25, this.getFieldName());
			}
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM PD_LMCheckField WHERE  RiskCode = ? AND FieldName = ? AND SerialNo = ?", 
	...
	public PD_LMCheckFieldSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMCheckFieldSet aPD_LMCheckFieldSet = new PD_LMCheckFieldSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("PD_LMCheckField");
			PD_LMCheckFieldSchema aSchema = this.getSchema();
			...
			{
				i++;
				PD_LMCheckFieldSchema s1 = new PD_LMCheckFieldSchema();
				s1.setSchema(rs,i);
				aPD_LMCheckFieldSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aPD_LMCheckFieldSet;
	}

	public PD_LMCheckFieldSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMCheckFieldSet aPD_LMCheckFieldSet = new PD_LMCheckFieldSet();
	...
	public PD_LMCheckFieldSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMCheckFieldSet aPD_LMCheckFieldSet = new PD_LMCheckFieldSet();
	...
	public PD_LMCheckFieldSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMCheckFieldSet aPD_LMCheckFieldSet = new PD_LMCheckFieldSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update PD_LMCheckField " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public PD_LMCheckFieldSet getData()
{
    int tCount = 0;
    PD_LMCheckFieldSet tPD_LMCheckFieldSet = new PD_LMCheckFieldSet();
    PD_LMCheckFieldSchema tPD_LMCheckFieldSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "PD_LMCheckFieldDB";
    ...
    {
        tCount = 1;
        tPD_LMCheckFieldSchema = new PD_LMCheckFieldSchema();
        tPD_LMCheckFieldSchema.setSchema(mResultSet, 1);
        tPD_LMCheckFieldSet.add(tPD_LMCheckFieldSchema);

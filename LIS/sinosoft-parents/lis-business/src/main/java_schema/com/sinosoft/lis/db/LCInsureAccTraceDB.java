...
/*
 * <p>ClassName: LCInsureAccTraceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCInsureAccTraceDB extends LCInsureAccTraceSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCInsureAccTraceDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCInsureAccTrace" );
		mflag = true;
	}

	public LCInsureAccTraceDB()
	{
		con = null;
		db = new DBOper( "LCInsureAccTrace" );
		mflag = false;
	}
	...
	{
		LCInsureAccTraceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCInsureAccTraceDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCInsureAccTrace WHERE  SerialNo = ?");
		...
		SQLString sqlObj = new SQLString("LCInsureAccTrace");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCInsureAccTrace SET  GrpContNo = ? , GrpPolNo = ? , ContNo = ? , PolNo = ? , SerialNo = ? , InsuAccNo = ? , RiskCode = ? , PayPlanCode = ? , OtherNo = ? , OtherType = ? , AccAscription = ? , MoneyType = ? , Money = ? , UnitCount = ? , PayDate = ? , State = ? , ManageCom = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , FeeCode = ? , AccAlterNo = ? , AccAlterType = ? , BusyType = ? , ShouldValueDate = ? , ValueDate = ? , PayNo = ? , EdorType = ? , DutyCode = ? , Currency = ? , InvestType = ? , UnitCountPrice = ? , PreMoney = ? , ConfDate = ? , AllocationDate = ? , PricingDate = ? , ComCode = ? , ModifyOperator = ? WHERE  SerialNo = ?");
			...
			if(this.getInvestType() == null || this.getInvestType().equals("null")) {
				...
				pstmt.setString(33, this.getInvestType());
			}
			pstmt.setDouble(34, this.getUnitCountPrice());
			pstmt.setDouble(35, this.getPreMoney());
			if(this.getConfDate() == null || this.getConfDate().equals("null")) {
				pstmt.setNull(36, 91);
			} else {
				pstmt.setDate(36, Date.valueOf(this.getConfDate()));
			}
			if(this.getAllocationDate() == null || this.getAllocationDate().equals("null")) {
				pstmt.setNull(37, 91);
			} else {
				pstmt.setDate(37, Date.valueOf(this.getAllocationDate()));
			}
			if(this.getPricingDate() == null || this.getPricingDate().equals("null")) {
				pstmt.setNull(38, 91);
			} else {
				pstmt.setDate(38, Date.valueOf(this.getPricingDate()));
			}
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCInsureAccTrace VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCInsureAccTrace WHERE  SerialNo = ?", 
	...
	public LCInsureAccTraceSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCInsureAccTrace");
			LCInsureAccTraceSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCInsureAccTraceSchema s1 = new LCInsureAccTraceSchema();
				s1.setSchema(rs,i);
				aLCInsureAccTraceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCInsureAccTraceSet;
	}

	public LCInsureAccTraceSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCInsureAccTraceSet aLCInsureAccTraceSet = new LCInsureAccTraceSet();
	...
	public LCInsureAccTraceSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCInsureAccTraceSet aLCInsureAccTraceSet = new LCInsureAccTraceSet();
	...
	public LCInsureAccTraceSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCInsureAccTraceSet aLCInsureAccTraceSet = new LCInsureAccTraceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCInsureAccTrace " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public LCInsureAccTraceSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLCInsureAccTraceSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCInsureAccTraceSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCInsureAccTraceSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

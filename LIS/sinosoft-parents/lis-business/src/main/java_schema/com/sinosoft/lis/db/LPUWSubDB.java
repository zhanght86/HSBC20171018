...
/*
 * <p>ClassName: LPUWSubDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPUWSubDB extends LPUWSubSchema
{
private static Logger logger = Logger.getLogger(LPUWSubDB.class);
	...
	**/
	...
	// @Constructor
	public LPUWSubDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LPUWSub" );
		mflag = true;
	}
	...
	{
		LPUWSubSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LPUWSub WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ? AND UWNo = ?");
		...
		SQLString sqlObj = new SQLString("LPUWSub");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LPUWSub SET  EdorNo = ? , EdorType = ? , GrpContNo = ? , ContNo = ? , ProposalContNo = ? , PolNo = ? , ProposalNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ? AND UWNo = ?");
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorNo());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LPUWSub(EdorNo ,EdorType ,GrpContNo ,ContNo ,ProposalContNo ,PolNo ,ProposalNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPUWSub WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ? AND UWNo = ?", 
	...
	public LPUWSubSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPUWSubSet aLPUWSubSet = new LPUWSubSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPUWSub");
			LPUWSubSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPUWSubSchema s1 = new LPUWSubSchema();
				s1.setSchema(rs,i);
				aLPUWSubSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LPUWSubSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPUWSubSet aLPUWSubSet = new LPUWSubSet();
	...
	public LPUWSubSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPUWSubSet aLPUWSubSet = new LPUWSubSet();
	...
	public LPUWSubSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPUWSubSet aLPUWSubSet = new LPUWSubSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPUWSub " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LPUWSubSet
 */
public LPUWSubSet getData()
{
    int tCount = 0;
    LPUWSubSet tLPUWSubSet = new LPUWSubSet();
    LPUWSubSchema tLPUWSubSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPUWSubDB";
    ...
    {
        tCount = 1;
        tLPUWSubSchema = new LPUWSubSchema();
        tLPUWSubSchema.setSchema(mResultSet, 1);
        tLPUWSubSet.add(tLPUWSubSchema);
...
public boolean closeData()
{
	...
	public LPUWSubSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPUWSubSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

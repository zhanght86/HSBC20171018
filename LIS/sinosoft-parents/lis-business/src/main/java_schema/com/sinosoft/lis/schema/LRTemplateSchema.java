...
/*
 * <p>ClassName: LRTemplateSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LRTemplateSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LRTemplateSchema.class);
	// @Field
	/** 模板号 */
	private String Id;
	/** 版本号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LRTemplateSchema cloned = (LRTemplateSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LRTemplateSchema 对象给 Schema 赋值
	* @param: aLRTemplateSchema LRTemplateSchema
	**/
	public void setSchema(LRTemplateSchema aLRTemplateSchema)
	{
		this.Id = aLRTemplateSchema.getId();
		this.Version = aLRTemplateSchema.getVersion();
		this.RuleName = aLRTemplateSchema.getRuleName();
		this.RuleCh = aLRTemplateSchema.getRuleCh();
		this.SQLStatement = aLRTemplateSchema.getSQLStatement();
		this.BOMs = aLRTemplateSchema.getBOMs();
		this.SQLParameter = aLRTemplateSchema.getSQLParameter();
		this.Valid = aLRTemplateSchema.getValid();
		this.State = aLRTemplateSchema.getState();
		this.TemplateLevel = aLRTemplateSchema.getTemplateLevel();
		this.Business = aLRTemplateSchema.getBusiness();
		this.Type = aLRTemplateSchema.getType();
		this.TableName = aLRTemplateSchema.getTableName();
		this.StartDate = fDate.getDate( aLRTemplateSchema.getStartDate());
		this.EndDate = fDate.getDate( aLRTemplateSchema.getEndDate());
		this.Description = aLRTemplateSchema.getDescription();
		this.Creator = aLRTemplateSchema.getCreator();
		this.Modifier = aLRTemplateSchema.getModifier();
		this.Approver = aLRTemplateSchema.getApprover();
		this.Deployer = aLRTemplateSchema.getDeployer();
		this.MakeTime = aLRTemplateSchema.getMakeTime();
		this.MakeDate = fDate.getDate( aLRTemplateSchema.getMakeDate());
		this.ModifyTime = aLRTemplateSchema.getModifyTime();
		this.ModifyDate = fDate.getDate( aLRTemplateSchema.getModifyDate());
		this.AuthorTime = aLRTemplateSchema.getAuthorTime();
		this.AuthorDate = fDate.getDate( aLRTemplateSchema.getAuthorDate());
		this.DeclTime = aLRTemplateSchema.getDeclTime();
		this.DeclDate = fDate.getDate( aLRTemplateSchema.getDeclDate());
		this.ViewParameter = aLRTemplateSchema.getViewParameter();
		this.RuleCalCh = aLRTemplateSchema.getRuleCalCh();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LRTemplate表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LRTemplateSchema";
	...
	public LRTemplateSchema getSchema()
	{
		LRTemplateSchema aLRTemplateSchema = new LRTemplateSchema();
		aLRTemplateSchema.setSchema(this);
		return aLRTemplateSchema;
	}

	public LRTemplateDB getDB()
	{
		LRTemplateDB aDBOper = new LRTemplateDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLRTemplate描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLRTemplate>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LRTemplateSchema other = (LRTemplateSchema)otherObject;
		return

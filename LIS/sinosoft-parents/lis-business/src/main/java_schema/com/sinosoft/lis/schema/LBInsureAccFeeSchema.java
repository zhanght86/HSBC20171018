...
/*
 * <p>ClassName: LBInsureAccFeeSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LBInsureAccFeeSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBInsureAccFeeSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBInsureAccFeeSchema cloned = (LBInsureAccFeeSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LBInsureAccFeeSchema 对象给 Schema 赋值
	* @param: aLBInsureAccFeeSchema LBInsureAccFeeSchema
	**/
	public void setSchema(LBInsureAccFeeSchema aLBInsureAccFeeSchema)
	{
		this.EdorNo = aLBInsureAccFeeSchema.getEdorNo();
		this.GrpContNo = aLBInsureAccFeeSchema.getGrpContNo();
		this.GrpPolNo = aLBInsureAccFeeSchema.getGrpPolNo();
		this.PolNo = aLBInsureAccFeeSchema.getPolNo();
		this.InsuAccNo = aLBInsureAccFeeSchema.getInsuAccNo();
		this.ContNo = aLBInsureAccFeeSchema.getContNo();
		this.ManageCom = aLBInsureAccFeeSchema.getManageCom();
		this.PrtNo = aLBInsureAccFeeSchema.getPrtNo();
		this.RiskCode = aLBInsureAccFeeSchema.getRiskCode();
		this.AccType = aLBInsureAccFeeSchema.getAccType();
		this.InvestType = aLBInsureAccFeeSchema.getInvestType();
		this.FundCompanyCode = aLBInsureAccFeeSchema.getFundCompanyCode();
		this.InsuredNo = aLBInsureAccFeeSchema.getInsuredNo();
		this.AppntNo = aLBInsureAccFeeSchema.getAppntNo();
		this.Owner = aLBInsureAccFeeSchema.getOwner();
		this.AccComputeFlag = aLBInsureAccFeeSchema.getAccComputeFlag();
		this.AccFoundDate = fDate.getDate( aLBInsureAccFeeSchema.getAccFoundDate());
		this.AccFoundTime = aLBInsureAccFeeSchema.getAccFoundTime();
		this.FeeRate = aLBInsureAccFeeSchema.getFeeRate();
		this.Fee = aLBInsureAccFeeSchema.getFee();
		this.FeeUnit = aLBInsureAccFeeSchema.getFeeUnit();
		this.BalaDate = fDate.getDate( aLBInsureAccFeeSchema.getBalaDate());
		this.BalaTime = aLBInsureAccFeeSchema.getBalaTime();
		this.Operator = aLBInsureAccFeeSchema.getOperator();
		this.MakeDate = fDate.getDate( aLBInsureAccFeeSchema.getMakeDate());
		this.MakeTime = aLBInsureAccFeeSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLBInsureAccFeeSchema.getModifyDate());
		this.ModifyTime = aLBInsureAccFeeSchema.getModifyTime();
		this.Currency = aLBInsureAccFeeSchema.getCurrency();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBInsureAccFee表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBInsureAccFeeSchema";
	...
	public LBInsureAccFeeSchema getSchema()
	{
		LBInsureAccFeeSchema aLBInsureAccFeeSchema = new LBInsureAccFeeSchema();
		aLBInsureAccFeeSchema.setSchema(this);
		return aLBInsureAccFeeSchema;
	}

	public LBInsureAccFeeDB getDB()
	{
		LBInsureAccFeeDB aDBOper = new LBInsureAccFeeDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBInsureAccFee>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			Fee = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,20,SysConst.PACKAGESPILTER))).doubleValue();
			FeeUnit = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,21,SysConst.PACKAGESPILTER))).doubleValue();
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBInsureAccFeeSchema other = (LBInsureAccFeeSchema)otherObject;
		return

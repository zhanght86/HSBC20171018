...
import com.sinosoft.lis.db.LJPremMatchDB;

/*
 * <p>ClassName: LJPremMatchSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LJPremMatchSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LJPremMatchSchema.class);
	// @Field
	/** 保费匹配流水号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LJPremMatchSchema cloned = (LJPremMatchSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LJPremMatchSchema 对象给 Schema 赋值
	* @param: aLJPremMatchSchema LJPremMatchSchema
	**/
	public void setSchema(LJPremMatchSchema aLJPremMatchSchema)
	{
		this.MatchSerialNo = aLJPremMatchSchema.getMatchSerialNo();
		this.MatchType = aLJPremMatchSchema.getMatchType();
		this.GrpName = aLJPremMatchSchema.getGrpName();
		this.SumMoney = aLJPremMatchSchema.getSumMoney();
		this.HisOutPayMoney = aLJPremMatchSchema.getHisOutPayMoney();
		this.DuePayMoney = aLJPremMatchSchema.getDuePayMoney();
		this.CurOutPayMoney = aLJPremMatchSchema.getCurOutPayMoney();
		this.MatchDate = fDate.getDate( aLJPremMatchSchema.getMatchDate());
		this.MatchTime = aLJPremMatchSchema.getMatchTime();
		this.CheckFlag = aLJPremMatchSchema.getCheckFlag();
		this.State = aLJPremMatchSchema.getState();
		this.ManageCom = aLJPremMatchSchema.getManageCom();
		this.ComCode = aLJPremMatchSchema.getComCode();
		this.MakeOperator = aLJPremMatchSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLJPremMatchSchema.getMakeDate());
		this.MakeTime = aLJPremMatchSchema.getMakeTime();
		this.ModifyOperator = aLJPremMatchSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLJPremMatchSchema.getModifyDate());
		this.ModifyTime = aLJPremMatchSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LJPremMatch表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LJPremMatchSchema";
	...
	public LJPremMatchSchema getSchema()
	{
		LJPremMatchSchema aLJPremMatchSchema = new LJPremMatchSchema();
		aLJPremMatchSchema.setSchema(this);
		return aLJPremMatchSchema;
	}

	public LJPremMatchDB getDB()
	{
		LJPremMatchDB aDBOper = new LJPremMatchDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLJPremMatch>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			MatchType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 2, SysConst.PACKAGESPILTER );
			GrpName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			SumMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,4,SysConst.PACKAGESPILTER))).doubleValue();
			HisOutPayMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,5,SysConst.PACKAGESPILTER))).doubleValue();
			DuePayMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,6,SysConst.PACKAGESPILTER))).doubleValue();
			CurOutPayMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,7,SysConst.PACKAGESPILTER))).doubleValue();
			MatchDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8,SysConst.PACKAGESPILTER));
			MatchTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LJPremMatchSchema other = (LJPremMatchSchema)otherObject;
		return

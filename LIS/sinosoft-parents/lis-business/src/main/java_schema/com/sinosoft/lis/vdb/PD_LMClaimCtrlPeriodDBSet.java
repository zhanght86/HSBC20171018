...
import java.sql.*;
import com.sinosoft.lis.schema.PD_LMClaimCtrlPeriodSchema;
import com.sinosoft.lis.vschema.PD_LMClaimCtrlPeriodSet;
import com.sinosoft.lis.pubfun.*;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: PD_LMClaimCtrlPeriodDBSet </p>
 * <p>Description: DB层多记录数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 产品定义平台_PDM
 */
public class PD_LMClaimCtrlPeriodDBSet extends PD_LMClaimCtrlPeriodSet
{
private static Logger logger = Logger.getLogger(PD_LMClaimCtrlPeriodDBSet.class);
	...
	**/
	...
	// @Constructor
	public PD_LMClaimCtrlPeriodDBSet(Connection tConnection)
	{
		con = tConnection;
		db = new DBOper(con,"PD_LMClaimCtrlPeriod");
		mflag = true;
	}

	public PD_LMClaimCtrlPeriodDBSet()
	{
		db = new DBOper( "PD_LMClaimCtrlPeriod" );
	}
	// @Method
	public boolean deleteSQL()
	{
		...
		else
		{
			...
			tError.moduleName = "PD_LMClaimCtrlPeriodDBSet";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt = con.prepareStatement("DELETE FROM PD_LMClaimCtrlPeriod WHERE  ClaimCtrlCode = ?");
		...
		SQLString sqlObj = new SQLString("PD_LMClaimCtrlPeriod");
...
ex.printStackTrace();
	...
	public boolean update()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("UPDATE PD_LMClaimCtrlPeriod SET  ClaimCtrlCode = ? , ClmPeriodStart = ? , ClmPeriodStartFlag = ? , ClmPeriodInterval = ? , ClmPeriodFlag = ? , CalCode = ? , CalResultType = ? , DefaultValue = ? , CalCtrlFlag = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Standbyflag1 = ? , Standbyflag2 = ? , Standbyflag3 = ? , Standbyflag4 = ? , Standbyflag5 = ? , Standbyflag6 = ? WHERE  ClaimCtrlCode = ?");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getClaimCtrlCode() == null || this.get(i).getClaimCtrlCode().equals("null")) {
				pstmt.setString(1,null);
			...
			if(this.get(i).getClaimCtrlCode() == null || this.get(i).getClaimCtrlCode().equals("null")) {
				...
				pstmt.setString(21, this.get(i).getClaimCtrlCode());
			}
...
ex.printStackTrace();
	...
	public boolean insert()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("INSERT INTO PD_LMClaimCtrlPeriod(ClaimCtrlCode ,ClmPeriodStart ,ClmPeriodStartFlag ,ClmPeriodInterval ,ClmPeriodFlag ,CalCode ,CalResultType ,DefaultValue ,CalCtrlFlag ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,Standbyflag1 ,Standbyflag2 ,Standbyflag3 ,Standbyflag4 ,Standbyflag5 ,Standbyflag6) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getClaimCtrlCode() == null || this.get(i).getClaimCtrlCode().equals("null")) {
				pstmt.setString(1,null);

...
/*
 * <p>ClassName: LPGCUWMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPGCUWMasterDB extends LPGCUWMasterSchema
{
private static Logger logger = Logger.getLogger(LPGCUWMasterDB.class);
	...
	**/
	...
	// @Constructor
	public LPGCUWMasterDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LPGCUWMaster" );
		mflag = true;
	}

	public LPGCUWMasterDB()
	{
		con = null;
		db = new DBOper( "LPGCUWMaster" );
		mflag = false;
	}
	...
	{
		LPGCUWMasterSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPGCUWMasterDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LPGCUWMaster WHERE  EdorNo = ? AND EdorType = ? AND GrpContNo = ?");
		...
		SQLString sqlObj = new SQLString("LPGCUWMaster");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LPGCUWMaster(EdorNo ,EdorType ,GrpContNo ,ProposalGrpContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPGCUWMaster WHERE  EdorNo = ? AND EdorType = ? AND GrpContNo = ?", 
	...
	public LPGCUWMasterSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGCUWMasterSet aLPGCUWMasterSet = new LPGCUWMasterSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPGCUWMaster");
			LPGCUWMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPGCUWMasterSchema s1 = new LPGCUWMasterSchema();
				s1.setSchema(rs,i);
				aLPGCUWMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPGCUWMasterSet;

	}

	public LPGCUWMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGCUWMasterSet aLPGCUWMasterSet = new LPGCUWMasterSet();
	...
	public LPGCUWMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGCUWMasterSet aLPGCUWMasterSet = new LPGCUWMasterSet();
	...
	public LPGCUWMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGCUWMasterSet aLPGCUWMasterSet = new LPGCUWMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPGCUWMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPGCUWMasterSet
 */
public LPGCUWMasterSet getData()
{
    int tCount = 0;
    LPGCUWMasterSet tLPGCUWMasterSet = new LPGCUWMasterSet();
    LPGCUWMasterSchema tLPGCUWMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPGCUWMasterDB";
    ...
    {
        tCount = 1;
        tLPGCUWMasterSchema = new LPGCUWMasterSchema();
        tLPGCUWMasterSchema.setSchema(mResultSet, 1);
        tLPGCUWMasterSet.add(tLPGCUWMasterSchema);
...
public boolean closeData()
{
	...
	public LPGCUWMasterSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPGCUWMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LJFIGetDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务付费
 */
public class LJFIGetDB extends LJFIGetSchema
{
	private static Logger logger = Logger.getLogger(LJFIGetDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJFIGetDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJFIGetSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getShouldDate() == null || this.getShouldDate().equals("null")) {
				...
				pstmt.setDate(6, Date.valueOf(this.getShouldDate()));
			}
			...
			if(this.getChequeNo() == null || this.getChequeNo().equals("null")) {
				...
				pstmt.setString(10, this.getChequeNo());
			}
			if(this.getBankAccNo() == null || this.getBankAccNo().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getBankAccNo());
			}
			if(this.getAccName() == null || this.getAccName().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getAccName());
			}
			if(this.getConfMakeDate() == null || this.getConfMakeDate().equals("null")) {
				pstmt.setNull(13, 91);
			} else {
				pstmt.setDate(13, Date.valueOf(this.getConfMakeDate()));
			}
			if(this.getConfMakeTime() == null || this.getConfMakeTime().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getConfMakeTime());
			}
			...
			if(this.getPolicyCom() == null || this.getPolicyCom().equals("null")) {
				...
				pstmt.setString(17, this.getPolicyCom());
			}
			...
			if(this.getDrawer() == null || this.getDrawer().equals("null")) {
				...
				pstmt.setString(24, this.getDrawer());
			}
			if(this.getDrawerID() == null || this.getDrawerID().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
				pstmt.setString(25, this.getDrawerID());
			}
			...
			if(this.getActuGetNo() == null || this.getActuGetNo().equals("null")) {
				...
				pstmt.setString(36, this.getActuGetNo());
			}
			if(this.getPayMode() == null || this.getPayMode().equals("null")) {
				pstmt.setNull(37, 1);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJFIGet WHERE  ActuGetNo = ? AND PayMode = ? AND Currency = ?", 
	...
	public LJFIGetSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJFIGetSet aLJFIGetSet = new LJFIGetSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJFIGet");
			LJFIGetSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJFIGetSchema s1 = new LJFIGetSchema();
				s1.setSchema(rs,i);
				aLJFIGetSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LJFIGetSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJFIGetSet aLJFIGetSet = new LJFIGetSet();
	...
	public LJFIGetSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJFIGetSet aLJFIGetSet = new LJFIGetSet();
	...
	public LJFIGetSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJFIGetSet aLJFIGetSet = new LJFIGetSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJFIGet " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LJFIGetSet
 */
public LJFIGetSet getData()
{
    int tCount = 0;
    LJFIGetSet tLJFIGetSet = new LJFIGetSet();
    LJFIGetSchema tLJFIGetSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJFIGetDB";
    ...
    try
    {
        ...
        tLJFIGetSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LJFIGetSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJFIGetSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

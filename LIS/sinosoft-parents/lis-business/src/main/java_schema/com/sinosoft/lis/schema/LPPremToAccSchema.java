...
import com.sinosoft.lis.db.LPPremToAccDB;

/*
 * <p>ClassName: LPPremToAccSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPPremToAccSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPPremToAccSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPPremToAccSchema cloned = (LPPremToAccSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LPPremToAccSchema 对象给 Schema 赋值
	* @param: aLPPremToAccSchema LPPremToAccSchema
	**/
	public void setSchema(LPPremToAccSchema aLPPremToAccSchema)
	{
		this.EdorNo = aLPPremToAccSchema.getEdorNo();
		this.EdorType = aLPPremToAccSchema.getEdorType();
		this.PolNo = aLPPremToAccSchema.getPolNo();
		this.DutyCode = aLPPremToAccSchema.getDutyCode();
		this.PayPlanCode = aLPPremToAccSchema.getPayPlanCode();
		this.InsuAccNo = aLPPremToAccSchema.getInsuAccNo();
		this.Rate = aLPPremToAccSchema.getRate();
		this.NewFlag = aLPPremToAccSchema.getNewFlag();
		this.CalCodeMoney = aLPPremToAccSchema.getCalCodeMoney();
		this.CalCodeUnit = aLPPremToAccSchema.getCalCodeUnit();
		this.CalFlag = aLPPremToAccSchema.getCalFlag();
		this.Operator = aLPPremToAccSchema.getOperator();
		this.MakeDate = fDate.getDate( aLPPremToAccSchema.getMakeDate());
		this.MakeTime = aLPPremToAccSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLPPremToAccSchema.getModifyDate());
		this.ModifyTime = aLPPremToAccSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPPremToAcc表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPPremToAccSchema";
	...
	public LPPremToAccSchema getSchema()
	{
		LPPremToAccSchema aLPPremToAccSchema = new LPPremToAccSchema();
		aLPPremToAccSchema.setSchema(this);
		return aLPPremToAccSchema;
	}

	public LPPremToAccDB getDB()
	{
		LPPremToAccDB aDBOper = new LPPremToAccDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPPremToAcc>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			NewFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			CalCodeMoney = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			CalCodeUnit = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPPremToAccSchema other = (LPPremToAccSchema)otherObject;
		return

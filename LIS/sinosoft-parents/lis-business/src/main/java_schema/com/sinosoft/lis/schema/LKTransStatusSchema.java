...
{
private static Logger logger = Logger.getLogger(LKTransStatusSchema.class);
	...
	*/
	...
	/**
	* 明细对账时，该编号记录此次对账的一个编号，可通过此字段去关联对账明细表的AppntName
	*/
	...
	*/
	...
	/**
	* 记录每一个原子交易，一个TransCode可对应多个TransNo
	*/
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LKTransStatus表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(IssueWay));
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			OutServiceCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 29, SysConst.PACKAGESPILTER );
			IssueWay = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 30, SysConst.PACKAGESPILTER );
		}

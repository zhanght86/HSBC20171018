...
/*
 * <p>ClassName: LPPerInvestPlanDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 泰康未整理PDM
 */
public class LPPerInvestPlanDB extends LPPerInvestPlanSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPPerInvestPlanDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPPerInvestPlanSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPPerInvestPlanDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(11, this.getInvestMinRate());
			pstmt.setDouble(12, this.getInvestMaxRate());
			if(this.getInputMode() == null || this.getInputMode().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getInputMode());
			}
			pstmt.setDouble(14, this.getInvestRate());
			pstmt.setDouble(15, this.getInvestMoney());
			...
			pstmt.setDouble(24, this.getRealInvestMoney());
			...
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				...
				pstmt.setString(27, this.getEdorNo());
			}
			if(this.getEdorType() == null || this.getEdorType().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getEdorType());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPPerInvestPlan WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ? AND PayPlanCode = ? AND InsuAccNo = ?", 
	...
	public LPPerInvestPlanSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPPerInvestPlanSet aLPPerInvestPlanSet = new LPPerInvestPlanSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPPerInvestPlan");
			LPPerInvestPlanSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPPerInvestPlanSchema s1 = new LPPerInvestPlanSchema();
				s1.setSchema(rs,i);
				aLPPerInvestPlanSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPPerInvestPlanSet;
	}

	public LPPerInvestPlanSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPPerInvestPlanSet aLPPerInvestPlanSet = new LPPerInvestPlanSet();
	...
	public LPPerInvestPlanSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPPerInvestPlanSet aLPPerInvestPlanSet = new LPPerInvestPlanSet();
	...
	public LPPerInvestPlanSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPPerInvestPlanSet aLPPerInvestPlanSet = new LPPerInvestPlanSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPPerInvestPlan " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPPerInvestPlanSet
 */
public LPPerInvestPlanSet getData()
{
    int tCount = 0;
    ...
    {
        tCount = 1;
        tLPPerInvestPlanSchema = new LPPerInvestPlanSchema();
        tLPPerInvestPlanSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LPPerInvestPlanSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPPerInvestPlanSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

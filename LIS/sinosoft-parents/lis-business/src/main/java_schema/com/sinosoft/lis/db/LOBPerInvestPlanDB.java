...
/*
 * <p>ClassName: LOBPerInvestPlanDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: bug2
 */
public class LOBPerInvestPlanDB extends LOBPerInvestPlanSchema
{
	private static Logger logger = Logger.getLogger(LOBPerInvestPlanDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LOBPerInvestPlanDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LOBPerInvestPlanSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBPerInvestPlanDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBPerInvestPlan WHERE  PolNo = ? AND PayPlanCode = ? AND InsuAccNo = ?", 
	...
	public LOBPerInvestPlanSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBPerInvestPlanSet aLOBPerInvestPlanSet = new LOBPerInvestPlanSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBPerInvestPlan");
			LOBPerInvestPlanSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBPerInvestPlanSchema s1 = new LOBPerInvestPlanSchema();
				s1.setSchema(rs,i);
				aLOBPerInvestPlanSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBPerInvestPlanSet;
	}

	public LOBPerInvestPlanSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBPerInvestPlanSet aLOBPerInvestPlanSet = new LOBPerInvestPlanSet();
	...
	public LOBPerInvestPlanSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBPerInvestPlanSet aLOBPerInvestPlanSet = new LOBPerInvestPlanSet();
	...
	public LOBPerInvestPlanSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBPerInvestPlanSet aLOBPerInvestPlanSet = new LOBPerInvestPlanSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBPerInvestPlan " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBPerInvestPlanSet
 */
public LOBPerInvestPlanSet getData()
{
    int tCount = 0;
    LOBPerInvestPlanSet tLOBPerInvestPlanSet = new LOBPerInvestPlanSet();
    LOBPerInvestPlanSchema tLOBPerInvestPlanSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBPerInvestPlanDB";
    ...
    {
        tCount = 1;
        tLOBPerInvestPlanSchema = new LOBPerInvestPlanSchema();
        tLOBPerInvestPlanSchema.setSchema(mResultSet, 1);
        tLOBPerInvestPlanSet.add(tLOBPerInvestPlanSchema);
...
public boolean closeData()
{
	...
	public LOBPerInvestPlanSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LOBPerInvestPlanSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

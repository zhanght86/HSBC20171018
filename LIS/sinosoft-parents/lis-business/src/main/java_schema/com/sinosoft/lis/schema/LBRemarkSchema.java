...
/*
 * <p>ClassName: LBRemarkSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统缺失模型
 */
public class LBRemarkSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBRemarkSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBRemarkSchema cloned = (LBRemarkSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LBRemarkSchema 对象给 Schema 赋值
	* @param: aLBRemarkSchema LBRemarkSchema
	**/
	public void setSchema(LBRemarkSchema aLBRemarkSchema)
	{
		this.EdorNo = aLBRemarkSchema.getEdorNo();
		this.PolNo = aLBRemarkSchema.getPolNo();
		this.OperatePos = aLBRemarkSchema.getOperatePos();
		this.PrtNo = aLBRemarkSchema.getPrtNo();
		this.SerialNo = aLBRemarkSchema.getSerialNo();
		this.RemarkCont = aLBRemarkSchema.getRemarkCont();
		this.Operator = aLBRemarkSchema.getOperator();
		this.ManageCom = aLBRemarkSchema.getManageCom();
		this.MakeDate = fDate.getDate( aLBRemarkSchema.getMakeDate());
		this.MakeTime = aLBRemarkSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLBRemarkSchema.getModifyDate());
		this.ModifyTime = aLBRemarkSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBRemark表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBRemarkSchema";
	...
	public LBRemarkSchema getSchema()
	{
		LBRemarkSchema aLBRemarkSchema = new LBRemarkSchema();
		aLBRemarkSchema.setSchema(this);
		return aLBRemarkSchema;
	}

	public LBRemarkDB getDB()
	{
		LBRemarkDB aDBOper = new LBRemarkDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBRemark描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBRemark>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBRemarkSchema other = (LBRemarkSchema)otherObject;
		return

...
{
private static Logger logger = Logger.getLogger(LPEdorItemSchema.class);
	// @Field
	/** 保全受理号 */
	...
	* 25 其他<p>
	* <p>
	* 对应LDCode.CodeType = 'surrordereason'<p>
	* <p>
	* 挂失(PL)<p>
	* 1  客户丢失<p>
	* 2  业务员丢失<p>
	* 3  其它<p>
	* <p>
	* 对应LDCode.CodeType = 'lostreason'<p>
	* <p>
	* 重新出单(RN)<p>
	* 0 录入错误<p>
	* 1 客户填写不清<p>
	* <p>
	* 对应LDCode.CodeType = 'edorreasonrn'
	*/
	...
	*/
	...
	{
		if(aStandbyFlag3!=null && aStandbyFlag3.length()>200)
			throw new IllegalArgumentException("备用属性字段3StandbyFlag3值"+aStandbyFlag3+"的长度"+aStandbyFlag3.length()+"大于最大值200");
	...
	**/
	public void setSchema(LPEdorItemSchema aLPEdorItemSchema)
	{
		...
		this.ApproveGrade = aLPEdorItemSchema.getApproveGrade();
		this.EdorTypeCal = aLPEdorItemSchema.getEdorTypeCal();
		this.Currency = aLPEdorItemSchema.getCurrency();
		this.ComCode = aLPEdorItemSchema.getComCode();
		this.ModifyOperator = aLPEdorItemSchema.getModifyOperator();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPEdorItem表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ApproveGrade = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 37, SysConst.PACKAGESPILTER );
			EdorTypeCal = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 38, SysConst.PACKAGESPILTER );

...
import com.sinosoft.lis.db.LOBInsureAccFeeTraceDB;

/*
 * <p>ClassName: LOBInsureAccFeeTraceSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: bug
 */
public class LOBInsureAccFeeTraceSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LOBInsureAccFeeTraceSchema.class);
	// @Field
	/** 集体合同号码 */
	...
	// @Constructor
	public LOBInsureAccFeeTraceSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LOBInsureAccFeeTraceSchema cloned = (LOBInsureAccFeeTraceSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LOBInsureAccFeeTraceSchema 对象给 Schema 赋值
	* @param: aLOBInsureAccFeeTraceSchema LOBInsureAccFeeTraceSchema
	**/
	public void setSchema(LOBInsureAccFeeTraceSchema aLOBInsureAccFeeTraceSchema)
	{
		this.GrpContNo = aLOBInsureAccFeeTraceSchema.getGrpContNo();
		this.GrpPolNo = aLOBInsureAccFeeTraceSchema.getGrpPolNo();
		this.ContNo = aLOBInsureAccFeeTraceSchema.getContNo();
		this.PolNo = aLOBInsureAccFeeTraceSchema.getPolNo();
		this.SerialNo = aLOBInsureAccFeeTraceSchema.getSerialNo();
		this.InsuAccNo = aLOBInsureAccFeeTraceSchema.getInsuAccNo();
		this.RiskCode = aLOBInsureAccFeeTraceSchema.getRiskCode();
		this.PayPlanCode = aLOBInsureAccFeeTraceSchema.getPayPlanCode();
		this.OtherNo = aLOBInsureAccFeeTraceSchema.getOtherNo();
		this.OtherType = aLOBInsureAccFeeTraceSchema.getOtherType();
		this.AccAscription = aLOBInsureAccFeeTraceSchema.getAccAscription();
		this.MoneyType = aLOBInsureAccFeeTraceSchema.getMoneyType();
		this.FeeRate = aLOBInsureAccFeeTraceSchema.getFeeRate();
		this.Fee = aLOBInsureAccFeeTraceSchema.getFee();
		this.FeeUnit = aLOBInsureAccFeeTraceSchema.getFeeUnit();
		this.PayDate = fDate.getDate( aLOBInsureAccFeeTraceSchema.getPayDate());
		this.State = aLOBInsureAccFeeTraceSchema.getState();
		this.FeeCode = aLOBInsureAccFeeTraceSchema.getFeeCode();
		this.InerSerialNo = aLOBInsureAccFeeTraceSchema.getInerSerialNo();
		this.ManageCom = aLOBInsureAccFeeTraceSchema.getManageCom();
		this.Operator = aLOBInsureAccFeeTraceSchema.getOperator();
		this.MakeDate = fDate.getDate( aLOBInsureAccFeeTraceSchema.getMakeDate());
		this.MakeTime = aLOBInsureAccFeeTraceSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLOBInsureAccFeeTraceSchema.getModifyDate());
		this.ModifyTime = aLOBInsureAccFeeTraceSchema.getModifyTime();
		this.PayNo = aLOBInsureAccFeeTraceSchema.getPayNo();
		this.Currency = aLOBInsureAccFeeTraceSchema.getCurrency();
		this.InvestType = aLOBInsureAccFeeTraceSchema.getInvestType();
		this.AccalterNo = aLOBInsureAccFeeTraceSchema.getAccalterNo();
		this.AccalterType = aLOBInsureAccFeeTraceSchema.getAccalterType();
		this.ConfDate = fDate.getDate( aLOBInsureAccFeeTraceSchema.getConfDate());
		this.AllocationDate = fDate.getDate( aLOBInsureAccFeeTraceSchema.getAllocationDate());
		this.PricingDate = fDate.getDate( aLOBInsureAccFeeTraceSchema.getPricingDate());
		this.ComCode = aLOBInsureAccFeeTraceSchema.getComCode();
		this.ModifyOperator = aLOBInsureAccFeeTraceSchema.getModifyOperator();
		this.NetAmount = aLOBInsureAccFeeTraceSchema.getNetAmount();
		this.TaxAmount = aLOBInsureAccFeeTraceSchema.getTaxAmount();
		this.Tax = aLOBInsureAccFeeTraceSchema.getTax();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LOBInsureAccFeeTrace表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LOBInsureAccFeeTraceSchema";
	...
	public LOBInsureAccFeeTraceSchema getSchema()
	{
		LOBInsureAccFeeTraceSchema aLOBInsureAccFeeTraceSchema = new LOBInsureAccFeeTraceSchema();
		aLOBInsureAccFeeTraceSchema.setSchema(this);
		return aLOBInsureAccFeeTraceSchema;
	}

	public LOBInsureAccFeeTraceDB getDB()
	{
		LOBInsureAccFeeTraceDB aDBOper = new LOBInsureAccFeeTraceDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBInsureAccFeeTrace描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBInsureAccFeeTrace>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LOBInsureAccFeeTraceSchema other = (LOBInsureAccFeeTraceSchema)otherObject;
		return

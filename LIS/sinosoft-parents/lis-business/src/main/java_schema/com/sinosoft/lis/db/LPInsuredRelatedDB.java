...
/*
 * <p>ClassName: LPInsuredRelatedDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPInsuredRelatedDB extends LPInsuredRelatedSchema
{
private static Logger logger = Logger.getLogger(LPInsuredRelatedDB.class);
	...
	**/
	...
	// @Constructor
	public LPInsuredRelatedDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPInsuredRelatedSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPInsuredRelatedDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getMainCustomerNo() == null || this.getMainCustomerNo().equals("null")) {
				...
				pstmt.setString(5, this.getMainCustomerNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getMainCustomerNo() == null || this.getMainCustomerNo().equals("null")) {
				...
				pstmt.setString(23, this.getMainCustomerNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPInsuredRelated WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ? AND CustomerNo = ? AND MainCustomerNo = ?", 
	...
	public LPInsuredRelatedSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPInsuredRelatedSet aLPInsuredRelatedSet = new LPInsuredRelatedSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPInsuredRelated");
			LPInsuredRelatedSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPInsuredRelatedSchema s1 = new LPInsuredRelatedSchema();
				s1.setSchema(rs,i);
				aLPInsuredRelatedSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPInsuredRelatedSet;

	}

	public LPInsuredRelatedSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPInsuredRelatedSet aLPInsuredRelatedSet = new LPInsuredRelatedSet();
	...
	public LPInsuredRelatedSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPInsuredRelatedSet aLPInsuredRelatedSet = new LPInsuredRelatedSet();
	...
	public LPInsuredRelatedSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPInsuredRelatedSet aLPInsuredRelatedSet = new LPInsuredRelatedSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPInsuredRelated " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPInsuredRelatedSet
 */
public LPInsuredRelatedSet getData()
{
    int tCount = 0;
    LPInsuredRelatedSet tLPInsuredRelatedSet = new LPInsuredRelatedSet();
    LPInsuredRelatedSchema tLPInsuredRelatedSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPInsuredRelatedDB";
    ...
    {
        tCount = 1;
        tLPInsuredRelatedSchema = new LPInsuredRelatedSchema();
        tLPInsuredRelatedSchema.setSchema(mResultSet, 1);
        tLPInsuredRelatedSet.add(tLPInsuredRelatedSchema);
...
public boolean closeData()
{
	...
	public LPInsuredRelatedSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPInsuredRelatedSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

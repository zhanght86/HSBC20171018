...
import com.sinosoft.lis.db.LBGrpFeeDB;

/*
 * <p>ClassName: LBGrpFeeSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 投连改造
 */
public class LBGrpFeeSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBGrpFeeSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBGrpFeeSchema cloned = (LBGrpFeeSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LBGrpFeeSchema 对象给 Schema 赋值
	* @param: aLBGrpFeeSchema LBGrpFeeSchema
	**/
	public void setSchema(LBGrpFeeSchema aLBGrpFeeSchema)
	{
		this.EdorNo = aLBGrpFeeSchema.getEdorNo();
		this.GrpPolNo = aLBGrpFeeSchema.getGrpPolNo();
		this.GrpContNo = aLBGrpFeeSchema.getGrpContNo();
		this.RiskCode = aLBGrpFeeSchema.getRiskCode();
		this.FeeCode = aLBGrpFeeSchema.getFeeCode();
		this.InsuAccNo = aLBGrpFeeSchema.getInsuAccNo();
		this.PayPlanCode = aLBGrpFeeSchema.getPayPlanCode();
		this.PayInsuAccName = aLBGrpFeeSchema.getPayInsuAccName();
		this.FeeCalMode = aLBGrpFeeSchema.getFeeCalMode();
		this.FeeCalModeType = aLBGrpFeeSchema.getFeeCalModeType();
		this.FeeCalCode = aLBGrpFeeSchema.getFeeCalCode();
		this.FeeValue = aLBGrpFeeSchema.getFeeValue();
		this.CompareValue = aLBGrpFeeSchema.getCompareValue();
		this.FeePeriod = aLBGrpFeeSchema.getFeePeriod();
		this.MaxTime = aLBGrpFeeSchema.getMaxTime();
		this.DefaultFlag = aLBGrpFeeSchema.getDefaultFlag();
		this.Operator = aLBGrpFeeSchema.getOperator();
		this.MakeDate = fDate.getDate( aLBGrpFeeSchema.getMakeDate());
		this.MakeTime = aLBGrpFeeSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLBGrpFeeSchema.getModifyDate());
		this.ModifyTime = aLBGrpFeeSchema.getModifyTime();
		this.FeeStartDate = fDate.getDate( aLBGrpFeeSchema.getFeeStartDate());
		this.PeriodType = aLBGrpFeeSchema.getPeriodType();
		this.FeeType = aLBGrpFeeSchema.getFeeType();
		this.InterfaceClassName = aLBGrpFeeSchema.getInterfaceClassName();
		this.FeeBaseType = aLBGrpFeeSchema.getFeeBaseType();
		this.FeeNum = aLBGrpFeeSchema.getFeeNum();
		this.FeeTakePlace = aLBGrpFeeSchema.getFeeTakePlace();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBGrpFee表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBGrpFeeSchema";
	...
	public LBGrpFeeSchema getSchema()
	{
		LBGrpFeeSchema aLBGrpFeeSchema = new LBGrpFeeSchema();
		aLBGrpFeeSchema.setSchema(this);
		return aLBGrpFeeSchema;
	}

	public LBGrpFeeDB getDB()
	{
		LBGrpFeeDB aDBOper = new LBGrpFeeDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBGrpFee描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(FeeTakePlace));
		return strReturn.toString();
	}

	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBGrpFee>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			FeeStartDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22,SysConst.PACKAGESPILTER));
			PeriodType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 23, SysConst.PACKAGESPILTER );
			FeeType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 24, SysConst.PACKAGESPILTER );
			InterfaceClassName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 25, SysConst.PACKAGESPILTER );
			FeeBaseType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 26, SysConst.PACKAGESPILTER );
			FeeNum= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,27,SysConst.PACKAGESPILTER))).intValue();
			FeeTakePlace = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 28, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBGrpFeeSchema other = (LBGrpFeeSchema)otherObject;
		return

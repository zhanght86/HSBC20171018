...
/*
 * <p>ClassName: LLOperationDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLOperationDB extends LLOperationSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LLOperationDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLOperationSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLOperationDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOperationCode() == null || this.getOperationCode().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getOperationCode());
			}
			if(this.getOperationName() == null || this.getOperationName().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getOperationName());
			}
			pstmt.setInt(10, this.getOpLevel());
			pstmt.setInt(11, this.getOpGrade());
			pstmt.setDouble(12, this.getOpFee());
			if(this.getUnitNo() == null || this.getUnitNo().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getUnitNo());
			}
			if(this.getUnitName() == null || this.getUnitName().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getUnitName());
			}
			if(this.getDiagnoseDate() == null || this.getDiagnoseDate().equals("null")) {
				pstmt.setNull(15, 91);
			} else {
				pstmt.setDate(15, Date.valueOf(this.getDiagnoseDate()));
			}
			pstmt.setDouble(16, this.getAdjSum());
			if(this.getAdjReason() == null || this.getAdjReason().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getAdjReason());
			}
			if(this.getAdjRemark() == null || this.getAdjRemark().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
				pstmt.setString(18, this.getAdjRemark());
			}
			if(this.getMainOp() == null || this.getMainOp().equals("null")) {
				pstmt.setNull(19, 1);
			} else {
				pstmt.setString(19, this.getMainOp());
			}
			...
			pstmt.setDouble(26, this.getSelfAmnt());
			...
			if(this.getFeeType() == null || this.getFeeType().equals("null")) {
				...
				pstmt.setString(28, this.getFeeType());
			}
			...
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				...
				pstmt.setString(32, this.getCaseNo());
			}
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLOperation WHERE  ClmNo = ? AND CaseNo = ? AND SerialNo = ? AND CustomerNo = ?", 
	...
	public LLOperationSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLOperationSet aLLOperationSet = new LLOperationSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLOperation");
			LLOperationSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLOperationSchema s1 = new LLOperationSchema();
				s1.setSchema(rs,i);
				aLLOperationSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
	...
	public LLOperationSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLOperationSet aLLOperationSet = new LLOperationSet();
	...
	public LLOperationSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLOperationSet aLLOperationSet = new LLOperationSet();
	...
	public LLOperationSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLOperationSet aLLOperationSet = new LLOperationSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLOperation " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLOperationSet
 */
public LLOperationSet getData()
{
    int tCount = 0;
    LLOperationSet tLLOperationSet = new LLOperationSet();
    LLOperationSchema tLLOperationSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLOperationDB";
    ...
    {
        tCount = 1;
        tLLOperationSchema = new LLOperationSchema();
        tLLOperationSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLOperationSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLOperationSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

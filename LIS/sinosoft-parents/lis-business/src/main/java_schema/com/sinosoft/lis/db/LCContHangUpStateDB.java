...
/*
 * <p>ClassName: LCContHangUpStateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCContHangUpStateDB extends LCContHangUpStateSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCContHangUpStateDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCContHangUpStateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCContHangUpStateDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getHangUpType() == null || this.getHangUpType().equals("null")) {
				...
				pstmt.setString(4, this.getHangUpType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getStandFlag4() == null || this.getStandFlag4().equals("null")) {
				...
				pstmt.setString(20, this.getStandFlag4());
			}
			if(this.getStandFlag5() == null || this.getStandFlag5().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getStandFlag5());
			}
			...
			if(this.getHangUpType() == null || this.getHangUpType().equals("null")) {
				...
				pstmt.setString(26, this.getHangUpType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCContHangUpState WHERE  ContNo = ? AND InsuredNo = ? AND PolNo = ? AND HangUpType = ?", 
	...
	public LCContHangUpStateSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCContHangUpStateSet aLCContHangUpStateSet = new LCContHangUpStateSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCContHangUpState");
			LCContHangUpStateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCContHangUpStateSchema s1 = new LCContHangUpStateSchema();
				s1.setSchema(rs,i);
				aLCContHangUpStateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCContHangUpStateSet;
	}

	public LCContHangUpStateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCContHangUpStateSet aLCContHangUpStateSet = new LCContHangUpStateSet();
	...
	public LCContHangUpStateSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCContHangUpStateSet aLCContHangUpStateSet = new LCContHangUpStateSet();
	...
	public LCContHangUpStateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCContHangUpStateSet aLCContHangUpStateSet = new LCContHangUpStateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCContHangUpState " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCContHangUpStateSet
 */
public LCContHangUpStateSet getData()
{
    int tCount = 0;
    LCContHangUpStateSet tLCContHangUpStateSet = new LCContHangUpStateSet();
    LCContHangUpStateSchema tLCContHangUpStateSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCContHangUpStateDB";
    ...
    try
    {
        ...
        tLCContHangUpStateSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCContHangUpStateSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCContHangUpStateSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

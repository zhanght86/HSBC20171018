...
/*
 * <p>ClassName: LYSendToBankBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 银行业务
 */
public class LYSendToBankBDB extends LYSendToBankBSchema
{
private static Logger logger = Logger.getLogger(LYSendToBankBDB.class);
	...
	**/
	...
	// @Constructor
	public LYSendToBankBDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LYSendToBankB" );
		mflag = true;
	}

	public LYSendToBankBDB()
	{
		con = null;
		db = new DBOper( "LYSendToBankB" );
		mflag = false;
	}
	...
	{
		LYSendToBankBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LYSendToBankBDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LYSendToBankB WHERE  SerialNo = ? AND DealType = ? AND PayCode = ? AND PolNo = ?");
		...
		SQLString sqlObj = new SQLString("LYSendToBankB");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LYSendToBankB(SerialNo ,DealType ,DataType ,PayCode ,PayType ,Name ,BankCode ,AccType ,AccName ,AccNo ,PolNo ,NoType ,ComCode ,AgentCode ,PayMoney ,SendDate ,BankDealDate ,BankDealTime ,BankSuccFlag ,VertifyFlag ,Remark ,ConvertFlag ,DoType ,ModifyDate ,ModifyTime ,RiskCode ,SerNo ,BankName ,InBankCode ,InBankName ,InAccType ,InAccName ,InAccNo ,IDType ,IDNo ,StandByFlag1 ,StandByFlag2 ,StandByFlag3 ,StandByFlag4 ,StandByFlag5) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getSerialNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LYSendToBankB WHERE  SerialNo = ? AND DealType = ? AND PayCode = ? AND PolNo = ?", 
	...
	public LYSendToBankBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LYSendToBankBSet aLYSendToBankBSet = new LYSendToBankBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LYSendToBankB");
			LYSendToBankBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LYSendToBankBSchema s1 = new LYSendToBankBSchema();
				s1.setSchema(rs,i);
				aLYSendToBankBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLYSendToBankBSet;

	}

	public LYSendToBankBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LYSendToBankBSet aLYSendToBankBSet = new LYSendToBankBSet();
	...
	public LYSendToBankBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LYSendToBankBSet aLYSendToBankBSet = new LYSendToBankBSet();
	...
	public LYSendToBankBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LYSendToBankBSet aLYSendToBankBSet = new LYSendToBankBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LYSendToBankB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LYSendToBankBSet
 */
public LYSendToBankBSet getData()
{
    int tCount = 0;
    LYSendToBankBSet tLYSendToBankBSet = new LYSendToBankBSet();
    LYSendToBankBSchema tLYSendToBankBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LYSendToBankBDB";
    ...
    {
        tCount = 1;
        tLYSendToBankBSchema = new LYSendToBankBSchema();
        tLYSendToBankBSchema.setSchema(mResultSet, 1);
        tLYSendToBankBSet.add(tLYSendToBankBSchema);
...
public boolean closeData()
{
	...
	public LYSendToBankBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LYSendToBankBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

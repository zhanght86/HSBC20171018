...
/*
 * <p>ClassName: LCInformationSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCInformationSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LCInformationSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LCInformationSchema cloned = (LCInformationSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LCInformationSchema 对象给 Schema 赋值
	* @param: aLCInformationSchema LCInformationSchema
	**/
	public void setSchema(LCInformationSchema aLCInformationSchema)
	{
		this.GrpContNo = aLCInformationSchema.getGrpContNo();
		this.PrtNo = aLCInformationSchema.getPrtNo();
		this.PrtSeq = aLCInformationSchema.getPrtSeq();
		this.CustomerNo = aLCInformationSchema.getCustomerNo();
		this.CustomerNoType = aLCInformationSchema.getCustomerNoType();
		this.PrintFlag = aLCInformationSchema.getPrintFlag();
		this.Remark = aLCInformationSchema.getRemark();
		this.AgentName = aLCInformationSchema.getAgentName();
		this.AgentCode = aLCInformationSchema.getAgentCode();
		this.Operator = aLCInformationSchema.getOperator();
		this.ManageCom = aLCInformationSchema.getManageCom();
		this.MakeDate = fDate.getDate( aLCInformationSchema.getMakeDate());
		this.MakeTime = aLCInformationSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLCInformationSchema.getModifyDate());
		this.ModifyTime = aLCInformationSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LCInformation表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LCInformationSchema";
	...
	public LCInformationSchema getSchema()
	{
		LCInformationSchema aLCInformationSchema = new LCInformationSchema();
		aLCInformationSchema.setSchema(this);
		return aLCInformationSchema;
	}

	public LCInformationDB getDB()
	{
		LCInformationDB aDBOper = new LCInformationDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCInformation>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			CustomerNoType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LCInformationSchema other = (LCInformationSchema)otherObject;
		return

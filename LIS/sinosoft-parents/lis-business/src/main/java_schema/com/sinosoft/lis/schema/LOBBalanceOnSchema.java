...
import com.sinosoft.lis.db.LOBBalanceOnDB;

/*
 * 与数据库结构不一致，暂不处理
 * <p>ClassName: LOBBalanceOnSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LOBBalanceOnSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LOBBalanceOnSchema.class);
	// @Field
	/** 业务类型 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LOBBalanceOnSchema cloned = (LOBBalanceOnSchema)super.clone();
	...
	*/
	...
	{
		if(aBalanceOnState!=null && aBalanceOnState.length()>1)
			throw new IllegalArgumentException("定期结算开通状态BalanceOnState值"+aBalanceOnState+"的长度"+aBalanceOnState.length()+"大于最大值1");
	...
	*/
	...
	/**
	* 使用另外一个 LOBBalanceOnSchema 对象给 Schema 赋值
	* @param: aLOBBalanceOnSchema LOBBalanceOnSchema
	**/
	public void setSchema(LOBBalanceOnSchema aLOBBalanceOnSchema)
	{
		this.BussType = aLOBBalanceOnSchema.getBussType();
		this.BussNo = aLOBBalanceOnSchema.getBussNo();
		this.GrpContNo = aLOBBalanceOnSchema.getGrpContNo();
		this.GrpPropNo = aLOBBalanceOnSchema.getGrpPropNo();
		this.BalanceOnDate = fDate.getDate( aLOBBalanceOnSchema.getBalanceOnDate());
		this.BalanceOnOperator = aLOBBalanceOnSchema.getBalanceOnOperator();
		this.BalanceOnState = aLOBBalanceOnSchema.getBalanceOnState();
		this.BalancePeriod = aLOBBalanceOnSchema.getBalancePeriod();
		this.BalanceSumLimit = aLOBBalanceOnSchema.getBalanceSumLimit();
		this.BalanceSum = aLOBBalanceOnSchema.getBalanceSum();
		this.BalanceType = aLOBBalanceOnSchema.getBalanceType();
		this.LastBalanceDate = fDate.getDate( aLOBBalanceOnSchema.getLastBalanceDate());
		this.LastBalanceTime = aLOBBalanceOnSchema.getLastBalanceTime();
		this.NextBalanceDate = fDate.getDate( aLOBBalanceOnSchema.getNextBalanceDate());
		this.GracePeriod = aLOBBalanceOnSchema.getGracePeriod();
		this.ManageCom = aLOBBalanceOnSchema.getManageCom();
		this.ComCode = aLOBBalanceOnSchema.getComCode();
		this.MakeOperator = aLOBBalanceOnSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLOBBalanceOnSchema.getMakeDate());
		this.MakeTime = aLOBBalanceOnSchema.getMakeTime();
		this.ModifyOperator = aLOBBalanceOnSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLOBBalanceOnSchema.getModifyDate());
		this.ModifyTime = aLOBBalanceOnSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LOBBalanceOn表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LOBBalanceOnSchema";
	...
	public LOBBalanceOnSchema getSchema()
	{
		LOBBalanceOnSchema aLOBBalanceOnSchema = new LOBBalanceOnSchema();
		aLOBBalanceOnSchema.setSchema(this);
		return aLOBBalanceOnSchema;
	}

	public LOBBalanceOnDB getDB()
	{
		LOBBalanceOnDB aDBOper = new LOBBalanceOnDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBBalanceOn描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBBalanceOn>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LOBBalanceOnSchema other = (LOBBalanceOnSchema)otherObject;
		return

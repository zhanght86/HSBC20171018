...
/*
 * <p>ClassName: LPLoanDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保
 */
public class LPLoanDB extends LPLoanSchema
{
private static Logger logger = Logger.getLogger(LPLoanDB.class);
	...
	**/
	...
	// @Constructor
	public LPLoanDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPLoanSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(26, this.getPreInterest());
			pstmt.setDouble(27, this.getCurLoanMoney());
			pstmt.setDouble(28, this.getLastSumLoanMoney());
			if(this.getLoanToDate() == null || this.getLoanToDate().equals("null")) {
				pstmt.setNull(29, 91);
			} else {
				pstmt.setDate(29, Date.valueOf(this.getLoanToDate()));
			}
			if(this.getNewLoanDate() == null || this.getNewLoanDate().equals("null")) {
				pstmt.setNull(30, 91);
			} else {
				pstmt.setDate(30, Date.valueOf(this.getNewLoanDate()));
			}
			if(this.getZqFlag() == null || this.getZqFlag().equals("null")) {
				pstmt.setNull(31, 1);
			} else {
				pstmt.setString(31, this.getZqFlag());
			}
			...
			if(this.getEdorType() == null || this.getEdorType().equals("null")) {
				...
				pstmt.setString(36, this.getEdorType());
			}
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPLoan WHERE  ContNo = ? AND PolNo = ? AND EdorType = ? AND EdorNo = ? AND Currency = ?", 
	...
	public LPLoanSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPLoan");
			LPLoanSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPLoanSchema s1 = new LPLoanSchema();
				s1.setSchema(rs,i);
				aLPLoanSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LPLoanSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPLoanSet aLPLoanSet = new LPLoanSet();
	...
	public LPLoanSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPLoanSet aLPLoanSet = new LPLoanSet();
	...
	public LPLoanSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPLoanSet aLPLoanSet = new LPLoanSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPLoan " + sqlObj.getUpdPart() + " where " + strWherePart;
...
public LPLoanSet getData()
{
    ...
    try
    {
        ...
        tLPLoanSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LPLoanSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPLoanSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LZCardBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 单证管理
 */
public class LZCardBDB extends LZCardBSchema
{
private static Logger logger = Logger.getLogger(LZCardBDB.class);
	...
	**/
	...
	// @Constructor
	public LZCardBDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LZCardB" );
		mflag = true;
	}
	...
	{
		LZCardBSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LZCardB WHERE  CertifyCode = ? AND SubCode = ? AND RiskCode = ? AND RiskVersion = ? AND StartNo = ? AND EndNo = ?");
		...
		SQLString sqlObj = new SQLString("LZCardB");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LZCardB SET  CertifyCode = ? , SubCode = ? , RiskCode = ? , RiskVersion = ? , StartNo = ? , EndNo = ? , SendOutCom = ? , ReceiveCom = ? , SumCount = ? , Prem = ? , Amnt = ? , Handler = ? , HandleDate = ? , InvaliDate = ? , TakeBackNo = ? , SaleChnl = ? , StateFlag = ? , OperateFlag = ? , PayFlag = ? , EnterAccFlag = ? , Reason = ? , State = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , ApplyNo = ? , AgentCom = ? WHERE  CertifyCode = ? AND SubCode = ? AND RiskCode = ? AND RiskVersion = ? AND StartNo = ? AND EndNo = ?");
			if(this.getCertifyCode() == null || this.getCertifyCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCertifyCode());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LZCardB(CertifyCode ,SubCode ,RiskCode ,RiskVersion ,StartNo ,EndNo ,SendOutCom ,ReceiveCom ,SumCount ,Prem ,Amnt ,Handler ,HandleDate ,InvaliDate ,TakeBackNo ,SaleChnl ,StateFlag ,OperateFlag ,PayFlag ,EnterAccFlag ,Reason ,State ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,ApplyNo ,AgentCom) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getCertifyCode() == null || this.getCertifyCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCertifyCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LZCardB WHERE  CertifyCode = ? AND SubCode = ? AND RiskCode = ? AND RiskVersion = ? AND StartNo = ? AND EndNo = ?", 
	...
	public LZCardBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardBSet aLZCardBSet = new LZCardBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LZCardB");
			LZCardBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LZCardBSchema s1 = new LZCardBSchema();
				s1.setSchema(rs,i);
				aLZCardBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LZCardBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardBSet aLZCardBSet = new LZCardBSet();
	...
	public LZCardBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardBSet aLZCardBSet = new LZCardBSet();
	...
	public LZCardBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardBSet aLZCardBSet = new LZCardBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LZCardB " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LZCardBSet
 */
public LZCardBSet getData()
{
    int tCount = 0;
    LZCardBSet tLZCardBSet = new LZCardBSet();
    LZCardBSchema tLZCardBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LZCardBDB";
    ...
    try
    {
        ...
        tLZCardBSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LZCardBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LZCardBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

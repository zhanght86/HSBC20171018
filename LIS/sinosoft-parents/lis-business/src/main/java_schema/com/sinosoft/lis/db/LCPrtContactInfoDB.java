...
/*
 * <p>ClassName: LCPrtContactInfoDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 新单管理
 */
public class LCPrtContactInfoDB extends LCPrtContactInfoSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCPrtContactInfoDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCPrtContactInfoSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCPrtContactInfoDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOfferListNo() == null || this.getOfferListNo().equals("null")) {
				...
				pstmt.setString(3, this.getOfferListNo());
			}
			...
			if(this.getGrpPropNo() == null || this.getGrpPropNo().equals("null")) {
				...
				pstmt.setString(23, this.getGrpPropNo());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPrtContactInfo WHERE  GrpPropNo = ? AND ContactNo = ?", 
	...
	public LCPrtContactInfoSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCPrtContactInfoSet aLCPrtContactInfoSet = new LCPrtContactInfoSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPrtContactInfo");
			LCPrtContactInfoSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCPrtContactInfoSchema s1 = new LCPrtContactInfoSchema();
				s1.setSchema(rs,i);
				aLCPrtContactInfoSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCPrtContactInfoSet;
	}

	public LCPrtContactInfoSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPrtContactInfoSet aLCPrtContactInfoSet = new LCPrtContactInfoSet();
	...
	public LCPrtContactInfoSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCPrtContactInfoSet aLCPrtContactInfoSet = new LCPrtContactInfoSet();
	...
	public LCPrtContactInfoSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPrtContactInfoSet aLCPrtContactInfoSet = new LCPrtContactInfoSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPrtContactInfo " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCPrtContactInfoSet
 */
public LCPrtContactInfoSet getData()
{
    int tCount = 0;
    LCPrtContactInfoSet tLCPrtContactInfoSet = new LCPrtContactInfoSet();
    LCPrtContactInfoSchema tLCPrtContactInfoSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCPrtContactInfoDB";
    ...
    {
        tCount = 1;
        tLCPrtContactInfoSchema = new LCPrtContactInfoSchema();
        tLCPrtContactInfoSchema.setSchema(mResultSet, 1);
        tLCPrtContactInfoSet.add(tLCPrtContactInfoSchema);
...
public boolean closeData()
{
	...
	public LCPrtContactInfoSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPrtContactInfoSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LMCertifyDesDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 单证管理
 */
public class LMCertifyDesDB extends LMCertifyDesSchema
{
private static Logger logger = Logger.getLogger(LMCertifyDesDB.class);
	...
	**/
	...
	// @Constructor
	public LMCertifyDesDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LMCertifyDes" );
		mflag = true;
	}

	public LMCertifyDesDB()
	{
		con = null;
		db = new DBOper( "LMCertifyDes" );
		mflag = false;
	}
	...
	{
		LMCertifyDesSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LMCertifyDesDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LMCertifyDes WHERE  CertifyCode = ?");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LMCertifyDes SET  CertifyCode = ? , CertifyName = ? , SubCode = ? , RiskCode = ? , RiskVersion = ? , CertifyClass = ? , HavePrice = ? , CertifyClass2 = ? , ImportantLevel = ? , HaveNumber = ? , CertifyLength = ? , TackBackFlag = ? , CertifyPrice = ? , Unit = ? , MaxPrintNo = ? , SplitOnSend = ? , HaveLimit = ? , MaxUnit1 = ? , MaxUnit2 = ? , HaveValidate = ? , Validate1 = ? , Validate2 = ? , State = ? , Note = ? , ManageCom = ? , Operator = ? , MakeDate = ? , MakeTime = ? , JugAgtFlag = ? WHERE  CertifyCode = ?");
			if(this.getCertifyCode() == null || this.getCertifyCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCertifyCode());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LMCertifyDes(CertifyCode ,CertifyName ,SubCode ,RiskCode ,RiskVersion ,CertifyClass ,HavePrice ,CertifyClass2 ,ImportantLevel ,HaveNumber ,CertifyLength ,TackBackFlag ,CertifyPrice ,Unit ,MaxPrintNo ,SplitOnSend ,HaveLimit ,MaxUnit1 ,MaxUnit2 ,HaveValidate ,Validate1 ,Validate2 ,State ,Note ,ManageCom ,Operator ,MakeDate ,MakeTime ,JugAgtFlag) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getCertifyCode() == null || this.getCertifyCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCertifyCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LMCertifyDes WHERE  CertifyCode = ?", 
	...
	public LMCertifyDesSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LMCertifyDesSet aLMCertifyDesSet = new LMCertifyDesSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LMCertifyDes");
			LMCertifyDesSchema aSchema = this.getSchema();
			...
			{
				i++;
				LMCertifyDesSchema s1 = new LMCertifyDesSchema();
				s1.setSchema(rs,i);
				aLMCertifyDesSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLMCertifyDesSet;

	}

	public LMCertifyDesSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMCertifyDesSet aLMCertifyDesSet = new LMCertifyDesSet();
	...
	public LMCertifyDesSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LMCertifyDesSet aLMCertifyDesSet = new LMCertifyDesSet();
	...
	public LMCertifyDesSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMCertifyDesSet aLMCertifyDesSet = new LMCertifyDesSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LMCertifyDes " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LMCertifyDesSet
 */
public LMCertifyDesSet getData()
{
    int tCount = 0;
    LMCertifyDesSet tLMCertifyDesSet = new LMCertifyDesSet();
    LMCertifyDesSchema tLMCertifyDesSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LMCertifyDesDB";
    ...
    {
        tCount = 1;
        tLMCertifyDesSchema = new LMCertifyDesSchema();
        tLMCertifyDesSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LMCertifyDesSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LMCertifyDesSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

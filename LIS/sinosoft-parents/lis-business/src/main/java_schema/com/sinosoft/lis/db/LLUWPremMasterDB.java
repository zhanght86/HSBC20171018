...
/*
 * <p>ClassName: LLUWPremMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_19
 */
public class LLUWPremMasterDB extends LLUWPremMasterSchema
{
	private static Logger logger = Logger.getLogger(LLUWPremMasterDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LLUWPremMasterDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLUWPremMasterSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLUWPremMasterDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAddFeeType() == null || this.getAddFeeType().equals("null")) {
				...
				pstmt.setString(37, this.getAddFeeType());
			}
			if(this.getAddForm() == null || this.getAddForm().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getAddForm());
			}
			// set where condition
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(39, 12);
			...
			if(this.getBatNo() == null || this.getBatNo().equals("null")) {
				...
				pstmt.setString(40, this.getBatNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(41, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLUWPremMaster WHERE  ClmNo = ? AND BatNo = ? AND PolNo = ? AND DutyCode = ? AND PayPlanCode = ?", 
	...
	public LLUWPremMasterSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLUWPremMasterSet aLLUWPremMasterSet = new LLUWPremMasterSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLUWPremMaster");
			LLUWPremMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLUWPremMasterSchema s1 = new LLUWPremMasterSchema();
				s1.setSchema(rs,i);
				aLLUWPremMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLUWPremMasterSet;
	}

	public LLUWPremMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWPremMasterSet aLLUWPremMasterSet = new LLUWPremMasterSet();
	...
	public LLUWPremMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLUWPremMasterSet aLLUWPremMasterSet = new LLUWPremMasterSet();
	...
	public LLUWPremMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWPremMasterSet aLLUWPremMasterSet = new LLUWPremMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLUWPremMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLUWPremMasterSet
 */
public LLUWPremMasterSet getData()
{
    int tCount = 0;
    LLUWPremMasterSet tLLUWPremMasterSet = new LLUWPremMasterSet();
    LLUWPremMasterSchema tLLUWPremMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLUWPremMasterDB";
    ...
    {
        tCount = 1;
        tLLUWPremMasterSchema = new LLUWPremMasterSchema();
        tLLUWPremMasterSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLUWPremMasterSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LLUWPremMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

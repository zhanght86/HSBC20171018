...
/*
 * <p>ClassName: LJSGetOtherDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_5
 */
public class LJSGetOtherDB extends LJSGetOtherSchema
{
	private static Logger logger = Logger.getLogger(LJSGetOtherDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJSGetOtherDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJSGetOtherSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJSGetOtherDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getOtherNoType() == null || this.getOtherNoType().equals("null")) {
				...
				pstmt.setString(3, StrTool.space1(this.getOtherNoType(), 1));
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getFeeOperationType() == null || this.getFeeOperationType().equals("null")) {
				...
				pstmt.setString(13, this.getFeeOperationType());
			}
			if(this.getFeeFinaType() == null || this.getFeeFinaType().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getFeeFinaType());
			}
			...
			pstmt.setDouble(23, this.getNetAmount());
			pstmt.setDouble(24, this.getTaxAmount());
			pstmt.setDouble(25, this.getTax());
			// set where condition
			if(this.getGetNoticeNo() == null || this.getGetNoticeNo().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
			...
			if(this.getOtherNoType() == null || this.getOtherNoType().equals("null")) {
				...
				pstmt.setString(28, StrTool.space1(this.getOtherNoType(), 1));
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJSGetOther WHERE  GetNoticeNo = ? AND OtherNo = ? AND OtherNoType = ? AND Currency = ?", 
	...
	public LJSGetOtherSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSGetOtherSet aLJSGetOtherSet = new LJSGetOtherSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJSGetOther");
			LJSGetOtherSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJSGetOtherSchema s1 = new LJSGetOtherSchema();
				s1.setSchema(rs,i);
				aLJSGetOtherSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJSGetOtherSet;
	}

	public LJSGetOtherSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSGetOtherSet aLJSGetOtherSet = new LJSGetOtherSet();
	...
	public LJSGetOtherSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSGetOtherSet aLJSGetOtherSet = new LJSGetOtherSet();
	...
	public LJSGetOtherSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSGetOtherSet aLJSGetOtherSet = new LJSGetOtherSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJSGetOther " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJSGetOtherSet
 */
public LJSGetOtherSet getData()
{
    int tCount = 0;
    LJSGetOtherSet tLJSGetOtherSet = new LJSGetOtherSet();
    LJSGetOtherSchema tLJSGetOtherSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJSGetOtherDB";
    ...
    try
    {
        ...
        tLJSGetOtherSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LJSGetOtherSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJSGetOtherSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

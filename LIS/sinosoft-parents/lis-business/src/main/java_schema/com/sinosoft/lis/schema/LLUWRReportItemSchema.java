...
import com.sinosoft.lis.db.LLUWRReportItemDB;

/*
 * <p>ClassName: LLUWRReportItemSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 泰康未整理PDM
 */
public class LLUWRReportItemSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLUWRReportItemSchema.class);
	// @Field
	/** 赔案号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLUWRReportItemSchema cloned = (LLUWRReportItemSchema)super.clone();
	...
	{
		if(aRReportItemCode!=null && aRReportItemCode.length()>10)
			throw new IllegalArgumentException("生调项目编号RReportItemCode值"+aRReportItemCode+"的长度"+aRReportItemCode.length()+"大于最大值10");
	...
	{
		if(aRReportItemName!=null && aRReportItemName.length()>60)
			throw new IllegalArgumentException("生调项目名称RReportItemName值"+aRReportItemName+"的长度"+aRReportItemName.length()+"大于最大值60");
	...
	/**
	* 使用另外一个 LLUWRReportItemSchema 对象给 Schema 赋值
	* @param: aLLUWRReportItemSchema LLUWRReportItemSchema
	**/
	public void setSchema(LLUWRReportItemSchema aLLUWRReportItemSchema)
	{
		this.ClmNo = aLLUWRReportItemSchema.getClmNo();
		this.GrpContNo = aLLUWRReportItemSchema.getGrpContNo();
		this.ContNo = aLLUWRReportItemSchema.getContNo();
		this.ProposalContNo = aLLUWRReportItemSchema.getProposalContNo();
		this.PrtSeq = aLLUWRReportItemSchema.getPrtSeq();
		this.RReportItemCode = aLLUWRReportItemSchema.getRReportItemCode();
		this.RReportItemName = aLLUWRReportItemSchema.getRReportItemName();
		this.ModifyDate = fDate.getDate( aLLUWRReportItemSchema.getModifyDate());
		this.ModifyTime = aLLUWRReportItemSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLUWRReportItem表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLUWRReportItemSchema";
	...
	public LLUWRReportItemSchema getSchema()
	{
		LLUWRReportItemSchema aLLUWRReportItemSchema = new LLUWRReportItemSchema();
		aLLUWRReportItemSchema.setSchema(this);
		return aLLUWRReportItemSchema;
	}

	public LLUWRReportItemDB getDB()
	{
		LLUWRReportItemDB aDBOper = new LLUWRReportItemDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLUWRReportItem描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLUWRReportItem>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			RReportItemCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			RReportItemName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LLUWRReportItemSchema other = (LLUWRReportItemSchema)otherObject;
		return

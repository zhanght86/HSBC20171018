...
/*
 * <p>ClassName: LCGrpBonusDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_2
 */
public class LCGrpBonusDB extends LCGrpBonusSchema
{
	private static Logger logger = Logger.getLogger(LCGrpBonusDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LCGrpBonusDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCGrpBonusSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCGrpBonusDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAppntNo() == null || this.getAppntNo().equals("null")) {
				...
				pstmt.setString(2, this.getAppntNo());
			}
			if(this.getDegree() == null || this.getDegree().equals("null")) {
				pstmt.setNull(3, 12);
			} else {
				pstmt.setString(3, this.getDegree());
			}
			if(this.getGrpPolNo() == null || this.getGrpPolNo().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
			...
			pstmt.setInt(6, this.getInsuYear());
			if(this.getInsuYearFlag() == null || this.getInsuYearFlag().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getInsuYearFlag());
			}
			if(this.getRewardType() == null || this.getRewardType().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getRewardType());
			}
			pstmt.setDouble(9, this.getRewardValue());
			...
			pstmt.setDouble(12, this.getPayMoney());
			...
			if(this.getStandbyFlag3() == null || this.getStandbyFlag3().equals("null")) {
				...
				pstmt.setString(16, this.getStandbyFlag3());
			}
			...
			if(this.getGrpPolNo() == null || this.getGrpPolNo().equals("null")) {
				...
				pstmt.setString(22, this.getGrpPolNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCGrpBonus WHERE  GrpPolNo = ?", 
	...
	public LCGrpBonusSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGrpBonusSet aLCGrpBonusSet = new LCGrpBonusSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCGrpBonus");
			LCGrpBonusSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCGrpBonusSchema s1 = new LCGrpBonusSchema();
				s1.setSchema(rs,i);
				aLCGrpBonusSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCGrpBonusSet;
	}

	public LCGrpBonusSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpBonusSet aLCGrpBonusSet = new LCGrpBonusSet();
	...
	public LCGrpBonusSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGrpBonusSet aLCGrpBonusSet = new LCGrpBonusSet();
	...
	public LCGrpBonusSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpBonusSet aLCGrpBonusSet = new LCGrpBonusSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCGrpBonus " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCGrpBonusSet
 */
public LCGrpBonusSet getData()
{
    int tCount = 0;
    LCGrpBonusSet tLCGrpBonusSet = new LCGrpBonusSet();
    LCGrpBonusSchema tLCGrpBonusSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCGrpBonusDB";
    ...
    {
        tCount = 1;
        tLCGrpBonusSchema = new LCGrpBonusSchema();
        tLCGrpBonusSchema.setSchema(mResultSet, 1);
        tLCGrpBonusSet.add(tLCGrpBonusSchema);
...
public boolean closeData()
{
	...
	public LCGrpBonusSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LCGrpBonusSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

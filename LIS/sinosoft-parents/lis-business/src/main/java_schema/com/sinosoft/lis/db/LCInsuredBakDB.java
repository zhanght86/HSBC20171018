...
/*
 * <p>ClassName: LCInsuredBakDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LCInsuredBakDB extends LCInsuredBakSchema
{
private static Logger logger = Logger.getLogger(LCInsuredBakDB.class);
	...
	**/
	...
	// @Constructor
	public LCInsuredBakDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCInsuredBakSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCInsuredBakDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getWorkNo() == null || this.getWorkNo().equals("null")) {
				...
				pstmt.setString(56, this.getWorkNo());
			}
			if(this.getJoinContDate() == null || this.getJoinContDate().equals("null")) {
				pstmt.setNull(57, 91);
			} else {
				pstmt.setDate(57, Date.valueOf(this.getJoinContDate()));
			}
			...
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				...
				pstmt.setString(66, this.getContNo());
			}
			if(this.getInsuredNo() == null || this.getInsuredNo().equals("null")) {
				pstmt.setNull(67, 12);
			} else {
				pstmt.setString(67, this.getInsuredNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCInsuredBak WHERE  ContNo = ? AND InsuredNo = ?", 
	...
	public LCInsuredBakSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCInsuredBakSet aLCInsuredBakSet = new LCInsuredBakSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCInsuredBak");
			LCInsuredBakSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCInsuredBakSchema s1 = new LCInsuredBakSchema();
				s1.setSchema(rs,i);
				aLCInsuredBakSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCInsuredBakSet;

	}

	public LCInsuredBakSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCInsuredBakSet aLCInsuredBakSet = new LCInsuredBakSet();
	...
	public LCInsuredBakSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCInsuredBakSet aLCInsuredBakSet = new LCInsuredBakSet();
	...
	public LCInsuredBakSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCInsuredBakSet aLCInsuredBakSet = new LCInsuredBakSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCInsuredBak " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCInsuredBakSet
 */
public LCInsuredBakSet getData()
{
    int tCount = 0;
    LCInsuredBakSet tLCInsuredBakSet = new LCInsuredBakSet();
    LCInsuredBakSchema tLCInsuredBakSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCInsuredBakDB";
    ...
    {
        tCount = 1;
        tLCInsuredBakSchema = new LCInsuredBakSchema();
        tLCInsuredBakSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCInsuredBakSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCInsuredBakSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

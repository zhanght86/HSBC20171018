...
{
private static Logger logger = Logger.getLogger(LMCalModeDB.class);
	...
	**/
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LMCalMode WHERE  CalCode = ?");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LMCalMode SET  CalCode = ? , RiskCode = ? , Type = ? , CalSQL = ? , Remark = ? WHERE  CalCode = ?");
			if(this.getCalCode() == null || this.getCalCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCalCode());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LMCalMode(CalCode ,RiskCode ,Type ,CalSQL ,Remark) VALUES( ? , ? , ? , ? , ?)");
			if(this.getCalCode() == null || this.getCalCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCalCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LMCalMode WHERE  CalCode = ?", 
...
public LMCalModeSet getData()
{
    ...
    try
    {
        ...
        tLMCalModeSchema.setSchema(mResultSet, 1);
        tLMCalModeSet.add(tLMCalModeSchema);
...
public boolean closeData()
{
	...
	public LMCalModeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LMCalModeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

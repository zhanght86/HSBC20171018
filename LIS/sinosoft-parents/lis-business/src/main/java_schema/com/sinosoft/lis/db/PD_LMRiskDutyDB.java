...
/*
 * <p>ClassName: PD_LMRiskDutyDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 产品定义平台_PDM
 */
public class PD_LMRiskDutyDB extends PD_LMRiskDutySchema
{
private static Logger logger = Logger.getLogger(PD_LMRiskDutyDB.class);
	...
	**/
	...
	// @Constructor
	public PD_LMRiskDutyDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		PD_LMRiskDutySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "PD_LMRiskDutyDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getChoFlag() == null || this.getChoFlag().equals("null")) {
				pstmt.setNull(4, 1);
			} else {
				pstmt.setString(4, this.getChoFlag());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM PD_LMRiskDuty WHERE  RiskCode = ? AND DutyCode = ?", 
	...
	public PD_LMRiskDutySet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		PD_LMRiskDutySet aPD_LMRiskDutySet = new PD_LMRiskDutySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("PD_LMRiskDuty");
			PD_LMRiskDutySchema aSchema = this.getSchema();
			...
			{
				i++;
				PD_LMRiskDutySchema s1 = new PD_LMRiskDutySchema();
				s1.setSchema(rs,i);
				aPD_LMRiskDutySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aPD_LMRiskDutySet;

	}

	public PD_LMRiskDutySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskDutySet aPD_LMRiskDutySet = new PD_LMRiskDutySet();
	...
	public PD_LMRiskDutySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		PD_LMRiskDutySet aPD_LMRiskDutySet = new PD_LMRiskDutySet();
	...
	public PD_LMRiskDutySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskDutySet aPD_LMRiskDutySet = new PD_LMRiskDutySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update PD_LMRiskDuty " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public PD_LMRiskDutySet getData()
{
    int tCount = 0;
    PD_LMRiskDutySet tPD_LMRiskDutySet = new PD_LMRiskDutySet();
    PD_LMRiskDutySchema tPD_LMRiskDutySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "PD_LMRiskDutyDB";
    ...
    {
        tCount = 1;
        tPD_LMRiskDutySchema = new PD_LMRiskDutySchema();
        tPD_LMRiskDutySchema.setSchema(mResultSet, 1);
        tPD_LMRiskDutySet.add(tPD_LMRiskDutySchema);
...
public boolean closeData()
{
	...
	public PD_LMRiskDutySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public PD_LMRiskDutySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

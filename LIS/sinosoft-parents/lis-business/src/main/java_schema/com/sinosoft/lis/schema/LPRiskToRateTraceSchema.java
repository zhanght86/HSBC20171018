...
import com.sinosoft.lis.db.LPRiskToRateTraceDB;

/*
 * <p>ClassName: LPRiskToRateTraceSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 费率表维护
 */
public class LPRiskToRateTraceSchema implements Schema, Cloneable
{
	// @Field
	/** 险种编码 */
	...
	// @Constructor
	public LPRiskToRateTraceSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPRiskToRateTraceSchema cloned = (LPRiskToRateTraceSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LPRiskToRateTraceSchema 对象给 Schema 赋值
	* @param: aLPRiskToRateTraceSchema LPRiskToRateTraceSchema
	**/
	public void setSchema(LPRiskToRateTraceSchema aLPRiskToRateTraceSchema)
	{
		this.RiskCode = aLPRiskToRateTraceSchema.getRiskCode();
		this.RiskName = aLPRiskToRateTraceSchema.getRiskName();
		this.RateType = aLPRiskToRateTraceSchema.getRateType();
		this.RateTable = aLPRiskToRateTraceSchema.getRateTable();
		this.SerialNo = aLPRiskToRateTraceSchema.getSerialNo();
		this.Remark = aLPRiskToRateTraceSchema.getRemark();
		this.Operator = aLPRiskToRateTraceSchema.getOperator();
		this.MakeDate = fDate.getDate( aLPRiskToRateTraceSchema.getMakeDate());
		this.MakeTime = aLPRiskToRateTraceSchema.getMakeTime();
		this.AuditFlag = aLPRiskToRateTraceSchema.getAuditFlag();
		this.AuditConclusion = aLPRiskToRateTraceSchema.getAuditConclusion();
		this.Auditor = aLPRiskToRateTraceSchema.getAuditor();
		this.ModifyDate = fDate.getDate( aLPRiskToRateTraceSchema.getModifyDate());
		this.ModifyTime = aLPRiskToRateTraceSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			System.out.println("数据库中的LPRiskToRateTrace表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			sqle.printStackTrace();
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPRiskToRateTraceSchema";
	...
	public LPRiskToRateTraceSchema getSchema()
	{
		LPRiskToRateTraceSchema aLPRiskToRateTraceSchema = new LPRiskToRateTraceSchema();
		aLPRiskToRateTraceSchema.setSchema(this);
		return aLPRiskToRateTraceSchema;
	}

	public LPRiskToRateTraceDB getDB()
	{
		LPRiskToRateTraceDB aDBOper = new LPRiskToRateTraceDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPRiskToRateTrace描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPRiskToRateTrace>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			AuditFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			AuditConclusion = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			Auditor = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPRiskToRateTraceSchema other = (LPRiskToRateTraceSchema)otherObject;
		return

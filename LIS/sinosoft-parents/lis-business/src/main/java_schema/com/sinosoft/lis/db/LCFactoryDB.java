...
/*
 * <p>ClassName: LCFactoryDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCFactoryDB extends LCFactorySchema
{
private static Logger logger = Logger.getLogger(LCFactoryDB.class);
	...
	**/
	...
	// @Constructor
	public LCFactoryDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCFactorySchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getCalRemark() == null || this.getCalRemark().equals("null")) {
				...
				pstmt.setString(12, this.getCalRemark());
			}
			...
			if(this.getFactoryType() == null || this.getFactoryType().equals("null")) {
				...
				pstmt.setString(20, this.getFactoryType());
			}
			if(this.getOtherNo() == null || this.getOtherNo().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getOtherNo());
			}
			if(this.getFactoryCode() == null || this.getFactoryCode().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getFactoryCode());
			}
			pstmt.setInt(23, this.getFactorySubCode());
			if(this.getInerSerialNo() == null || this.getInerSerialNo().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getInerSerialNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCFactory WHERE  ContNo = ? AND GrpPolNo = ? AND PolNo = ? AND FactoryType = ? AND OtherNo = ? AND FactoryCode = ? AND FactorySubCode = ? AND InerSerialNo = ?", 
	...
	public LCFactorySet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCFactorySet aLCFactorySet = new LCFactorySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCFactory");
			LCFactorySchema aSchema = this.getSchema();
			...
			{
				i++;
				LCFactorySchema s1 = new LCFactorySchema();
				s1.setSchema(rs,i);
				aLCFactorySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCFactorySet;

	}

	public LCFactorySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCFactorySet aLCFactorySet = new LCFactorySet();
	...
	public LCFactorySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCFactorySet aLCFactorySet = new LCFactorySet();
	...
	public LCFactorySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCFactorySet aLCFactorySet = new LCFactorySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCFactory " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LCFactorySet
 */
public LCFactorySet getData()
{
    int tCount = 0;
    LCFactorySet tLCFactorySet = new LCFactorySet();
    LCFactorySchema tLCFactorySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCFactoryDB";
    ...
    try
    {
        ...
        tLCFactorySchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCFactorySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCFactorySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LPCUWMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPCUWMasterDB extends LPCUWMasterSchema
{
private static Logger logger = Logger.getLogger(LPCUWMasterDB.class);
	...
	**/
	...
	// @Constructor
	public LPCUWMasterDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPCUWMasterSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPCUWMasterDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getCustomerIdea() == null || this.getCustomerIdea().equals("null")) {
				pstmt.setNull(41, 1);
			} else {
				pstmt.setString(41, this.getCustomerIdea());
			}
			...
			if(this.getCommonReason() == null || this.getCommonReason().equals("null")) {
				...
				pstmt.setString(43, this.getCommonReason());
			}
			if(this.getCommonReasonCode() == null || this.getCommonReasonCode().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
				pstmt.setString(44, this.getCommonReasonCode());
			}
			// set where condition
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(45, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPCUWMaster WHERE  EdorNo = ? AND EdorType = ? AND ContNo = ?", 
	...
	public LPCUWMasterSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPCUWMasterSet aLPCUWMasterSet = new LPCUWMasterSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPCUWMaster");
			LPCUWMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPCUWMasterSchema s1 = new LPCUWMasterSchema();
				s1.setSchema(rs,i);
				aLPCUWMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPCUWMasterSet;

	}

	public LPCUWMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPCUWMasterSet aLPCUWMasterSet = new LPCUWMasterSet();
	...
	public LPCUWMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPCUWMasterSet aLPCUWMasterSet = new LPCUWMasterSet();
	...
	public LPCUWMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPCUWMasterSet aLPCUWMasterSet = new LPCUWMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPCUWMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPCUWMasterSet
 */
public LPCUWMasterSet getData()
{
    int tCount = 0;
    LPCUWMasterSet tLPCUWMasterSet = new LPCUWMasterSet();
    LPCUWMasterSchema tLPCUWMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPCUWMasterDB";
    ...
    {
        tCount = 1;
        tLPCUWMasterSchema = new LPCUWMasterSchema();
        tLPCUWMasterSchema.setSchema(mResultSet, 1);
        tLPCUWMasterSet.add(tLPCUWMasterSchema);
...
public boolean closeData()
{
	...
	public LPCUWMasterSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPCUWMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

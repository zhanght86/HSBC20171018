...
/*
 * <p>ClassName: LYDupGetDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 银行业务
 */
public class LYDupGetDB extends LYDupGetSchema
{
private static Logger logger = Logger.getLogger(LYDupGetDB.class);
	...
	**/
	...
	// @Constructor
	public LYDupGetDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LYDupGetSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getNoType() == null || this.getNoType().equals("null")) {
				...
				pstmt.setString(36, this.getNoType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LYDupGet WHERE  SerialNo = ? AND DataType = ? AND PayCode = ? AND NoType = ?", 
	...
	public LYDupGetSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LYDupGetSet aLYDupGetSet = new LYDupGetSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LYDupGet");
			LYDupGetSchema aSchema = this.getSchema();
			...
			{
				i++;
				LYDupGetSchema s1 = new LYDupGetSchema();
				s1.setSchema(rs,i);
				aLYDupGetSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LYDupGetSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LYDupGetSet aLYDupGetSet = new LYDupGetSet();
	...
	public LYDupGetSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LYDupGetSet aLYDupGetSet = new LYDupGetSet();
	...
	public LYDupGetSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LYDupGetSet aLYDupGetSet = new LYDupGetSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LYDupGet " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LYDupGetSet
 */
public LYDupGetSet getData()
{
    int tCount = 0;
    LYDupGetSet tLYDupGetSet = new LYDupGetSet();
    LYDupGetSchema tLYDupGetSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LYDupGetDB";
    ...
    {
        tCount = 1;
        tLYDupGetSchema = new LYDupGetSchema();
        tLYDupGetSchema.setSchema(mResultSet, 1);
        tLYDupGetSet.add(tLYDupGetSchema);
...
public boolean closeData()
{
	...
	public LYDupGetSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LYDupGetSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

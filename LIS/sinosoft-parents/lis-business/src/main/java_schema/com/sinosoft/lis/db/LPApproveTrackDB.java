...
/*
 * <p>ClassName: LPApproveTrackDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: lpapprovetrack
 */
public class LPApproveTrackDB extends LPApproveTrackSchema
{
private static Logger logger = Logger.getLogger(LPApproveTrackDB.class);
	...
	**/
	...
	// @Constructor
	public LPApproveTrackDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPApproveTrackSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPApproveTrackDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt.setInt(2, this.getApproveTimes());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getApproveFlag() == null || this.getApproveFlag().equals("null")) {
				...
				pstmt.setString(3, this.getApproveFlag());
			}
			if(this.getModifyReason() == null || this.getModifyReason().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getModifyReason());
			}
			...
			if(this.getApproveOperator() == null || this.getApproveOperator().equals("null")) {
				...
				pstmt.setString(9, this.getApproveOperator());
			}
			...
			if(this.getErrorFlag() == null || this.getErrorFlag().equals("null")) {
				...
				pstmt.setString(15, this.getErrorFlag());
			}
			// set where condition
			if(this.getEdorAcceptNo() == null || this.getEdorAcceptNo().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
				pstmt.setString(16, this.getEdorAcceptNo());
			}
			pstmt.setInt(17, this.getApproveTimes());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPApproveTrack WHERE  EdorAcceptNo = ? AND ApproveTimes = ?", 
	...
	public LPApproveTrackSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPApproveTrackSet aLPApproveTrackSet = new LPApproveTrackSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPApproveTrack");
			LPApproveTrackSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPApproveTrackSchema s1 = new LPApproveTrackSchema();
				s1.setSchema(rs,i);
				aLPApproveTrackSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPApproveTrackSet;

	}

	public LPApproveTrackSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPApproveTrackSet aLPApproveTrackSet = new LPApproveTrackSet();
	...
	public LPApproveTrackSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPApproveTrackSet aLPApproveTrackSet = new LPApproveTrackSet();
	...
	public LPApproveTrackSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPApproveTrackSet aLPApproveTrackSet = new LPApproveTrackSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPApproveTrack " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPApproveTrackSet
 */
public LPApproveTrackSet getData()
{
    int tCount = 0;
    LPApproveTrackSet tLPApproveTrackSet = new LPApproveTrackSet();
    LPApproveTrackSchema tLPApproveTrackSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPApproveTrackDB";
    ...
    {
        tCount = 1;
        tLPApproveTrackSchema = new LPApproveTrackSchema();
        tLPApproveTrackSchema.setSchema(mResultSet, 1);
        tLPApproveTrackSet.add(tLPApproveTrackSchema);
...
public boolean closeData()
{
	...
	public LPApproveTrackSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPApproveTrackSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

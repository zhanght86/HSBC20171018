...
/*
 * <p>ClassName: LOBonusPolDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 分红管理
 */
public class LOBonusPolDB extends LOBonusPolSchema
{
private static Logger logger = Logger.getLogger(LOBonusPolDB.class);
	...
	**/
	...
	// @Constructor
	public LOBonusPolDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LOBonusPolSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBonusPolDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBonusPol WHERE  PolNo = ? AND FiscalYear = ?", 
	...
	public LOBonusPolSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LOBonusPolSet aLOBonusPolSet = new LOBonusPolSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBonusPol");
			LOBonusPolSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBonusPolSchema s1 = new LOBonusPolSchema();
				s1.setSchema(rs,i);
				aLOBonusPolSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBonusPolSet;

	}

	public LOBonusPolSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBonusPolSet aLOBonusPolSet = new LOBonusPolSet();
	...
	public LOBonusPolSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LOBonusPolSet aLOBonusPolSet = new LOBonusPolSet();
	...
	public LOBonusPolSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBonusPolSet aLOBonusPolSet = new LOBonusPolSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBonusPol " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBonusPolSet
 */
public LOBonusPolSet getData()
{
    int tCount = 0;
    LOBonusPolSet tLOBonusPolSet = new LOBonusPolSet();
    LOBonusPolSchema tLOBonusPolSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBonusPolDB";
    ...
    try
    {
        ...
        tLOBonusPolSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LOBonusPolSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBonusPolSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

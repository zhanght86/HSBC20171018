...
/*
 * <p>ClassName: LYSendToBankDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 银行业务
 */
public class LYSendToBankDB extends LYSendToBankSchema
{
private static Logger logger = Logger.getLogger(LYSendToBankDB.class);
	...
	**/
	...
	// @Constructor
	public LYSendToBankDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LYSendToBank" );
		mflag = true;
	}

	public LYSendToBankDB()
	{
		con = null;
		db = new DBOper( "LYSendToBank" );
		mflag = false;
	}
	...
	{
		LYSendToBankSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LYSendToBankDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LYSendToBank WHERE  SerialNo = ? AND DealType = ? AND PayCode = ? AND PolNo = ?");
		...
		SQLString sqlObj = new SQLString("LYSendToBank");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LYSendToBank(SerialNo ,DealType ,DataType ,PayCode ,PayType ,Name ,BankCode ,AccType ,AccName ,AccNo ,PolNo ,NoType ,ComCode ,AgentCode ,PayMoney ,SendDate ,BankDealDate ,BankDealTime ,BankSuccFlag ,VertifyFlag ,Remark ,ConvertFlag ,DoType ,ModifyDate ,ModifyTime ,RiskCode ,SerNo ,BankName ,InBankCode ,InBankName ,InAccType ,InAccName ,InAccNo ,IDType ,IDNo ,StandByFlag1 ,StandByFlag2 ,StandByFlag3 ,StandByFlag4 ,StandByFlag5) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getSerialNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LYSendToBank WHERE  SerialNo = ? AND DealType = ? AND PayCode = ? AND PolNo = ?", 
	...
	public LYSendToBankSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LYSendToBankSet aLYSendToBankSet = new LYSendToBankSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LYSendToBank");
			LYSendToBankSchema aSchema = this.getSchema();
			...
			{
				i++;
				LYSendToBankSchema s1 = new LYSendToBankSchema();
				s1.setSchema(rs,i);
				aLYSendToBankSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLYSendToBankSet;

	}

	public LYSendToBankSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LYSendToBankSet aLYSendToBankSet = new LYSendToBankSet();
	...
	public LYSendToBankSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LYSendToBankSet aLYSendToBankSet = new LYSendToBankSet();
	...
	public LYSendToBankSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LYSendToBankSet aLYSendToBankSet = new LYSendToBankSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LYSendToBank " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LYSendToBankSet
 */
public LYSendToBankSet getData()
{
    int tCount = 0;
    LYSendToBankSet tLYSendToBankSet = new LYSendToBankSet();
    LYSendToBankSchema tLYSendToBankSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LYSendToBankDB";
    ...
    {
        tCount = 1;
        tLYSendToBankSchema = new LYSendToBankSchema();
        tLYSendToBankSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LYSendToBankSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LYSendToBankSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

 ...
 */
public class LQBenefitDB extends LQBenefitSchema
{
private static Logger logger = Logger.getLogger(LQBenefitDB.class);
	...
	**/
	...
	// @Constructor
	public LQBenefitDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LQBenefitSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(14, this.getPeoples3());
			if(this.getRemark2() == null || this.getRemark2().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getRemark2());
			}
			pstmt.setDouble(16, this.getPeoples2());
			// set where condition
			if(this.getAskPriceNo() == null || this.getAskPriceNo().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getAskPriceNo());
			}
			if(this.getAskNo() == null || this.getAskNo().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
				pstmt.setString(18, this.getAskNo());
			}
			if(this.getContPlanCode() == null || this.getContPlanCode().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getContPlanCode());
			}
			if(this.getPlanType() == null || this.getPlanType().equals("null")) {
				pstmt.setNull(20, 1);
			} else {
				pstmt.setString(20, StrTool.space1(this.getPlanType(), 1));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LQBenefit WHERE  AskPriceNo = ? AND AskNo = ? AND ContPlanCode = ? AND PlanType = ?", 
	...
	public LQBenefitSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LQBenefitSet aLQBenefitSet = new LQBenefitSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LQBenefit");
			LQBenefitSchema aSchema = this.getSchema();
			...
			{
				i++;
				LQBenefitSchema s1 = new LQBenefitSchema();
				s1.setSchema(rs,i);
				aLQBenefitSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLQBenefitSet;

	}

	public LQBenefitSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LQBenefitSet aLQBenefitSet = new LQBenefitSet();
	...
	public LQBenefitSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LQBenefitSet aLQBenefitSet = new LQBenefitSet();
	...
	public LQBenefitSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LQBenefitSet aLQBenefitSet = new LQBenefitSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LQBenefit " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LQBenefitSet
 */
public LQBenefitSet getData()
{
    int tCount = 0;
    LQBenefitSet tLQBenefitSet = new LQBenefitSet();
    LQBenefitSchema tLQBenefitSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LQBenefitDB";
    ...
    {
        tCount = 1;
        tLQBenefitSchema = new LQBenefitSchema();
        tLQBenefitSchema.setSchema(mResultSet, 1);
        tLQBenefitSet.add(tLQBenefitSchema);
...
public boolean closeData()
{
	...
	public LQBenefitSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LQBenefitSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

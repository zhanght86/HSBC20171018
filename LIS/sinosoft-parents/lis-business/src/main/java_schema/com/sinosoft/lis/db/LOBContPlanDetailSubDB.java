...
import com.sinosoft.lis.schema.LOBContPlanDetailSubSchema;
import com.sinosoft.lis.vschema.LOBContPlanDetailSubSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LOBContPlanDetailSubDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LOBContPlanDetailSubDB extends LOBContPlanDetailSubSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LOBContPlanDetailSubDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LOBContPlanDetailSub" );
		mflag = true;
	}

	public LOBContPlanDetailSubDB()
	{
		con = null;
		db = new DBOper( "LOBContPlanDetailSub" );
		mflag = false;
	}
	...
	{
		LOBContPlanDetailSubSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBContPlanDetailSubDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LOBContPlanDetailSub WHERE  BussType = ? AND BussNo = ? AND PolicyNo = ? AND SysPlanCode = ? AND PlanCode = ? AND RiskCode = ? AND DutyCode = ?");
		...
		SQLString sqlObj = new SQLString("LOBContPlanDetailSub");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LOBContPlanDetailSub SET  BussType = ? , BussNo = ? , PolicyNo = ? , PropNo = ? , SysPlanCode = ? , PlanCode = ? , RiskCode = ? , DutyCode = ? , P1 = ? , P2 = ? , P3 = ? , P4 = ? , P5 = ? , P6 = ? , P7 = ? , P8 = ? , P9 = ? , P10 = ? , P11 = ? , P12 = ? , P13 = ? , P14 = ? , P15 = ? , P16 = ? , P17 = ? , P18 = ? , P19 = ? , P20 = ? , P21 = ? , P22 = ? , P23 = ? , P24 = ? , P25 = ? , P26 = ? , P27 = ? , P28 = ? , P29 = ? , P30 = ? WHERE  BussType = ? AND BussNo = ? AND PolicyNo = ? AND SysPlanCode = ? AND PlanCode = ? AND RiskCode = ? AND DutyCode = ?");
			if(this.getBussType() == null || this.getBussType().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getBussType());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LOBContPlanDetailSub VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getBussType() == null || this.getBussType().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getBussType());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBContPlanDetailSub WHERE  BussType = ? AND BussNo = ? AND PolicyNo = ? AND SysPlanCode = ? AND PlanCode = ? AND RiskCode = ? AND DutyCode = ?", 
	...
	public LOBContPlanDetailSubSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBContPlanDetailSubSet aLOBContPlanDetailSubSet = new LOBContPlanDetailSubSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBContPlanDetailSub");
			LOBContPlanDetailSubSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBContPlanDetailSubSchema s1 = new LOBContPlanDetailSubSchema();
				s1.setSchema(rs,i);
				aLOBContPlanDetailSubSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBContPlanDetailSubSet;
	}

	public LOBContPlanDetailSubSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBContPlanDetailSubSet aLOBContPlanDetailSubSet = new LOBContPlanDetailSubSet();
	...
	public LOBContPlanDetailSubSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBContPlanDetailSubSet aLOBContPlanDetailSubSet = new LOBContPlanDetailSubSet();
	...
	public LOBContPlanDetailSubSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBContPlanDetailSubSet aLOBContPlanDetailSubSet = new LOBContPlanDetailSubSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBContPlanDetailSub " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBContPlanDetailSubSet
 */
public LOBContPlanDetailSubSet getData()
{
    int tCount = 0;
    LOBContPlanDetailSubSet tLOBContPlanDetailSubSet = new LOBContPlanDetailSubSet();
    LOBContPlanDetailSubSchema tLOBContPlanDetailSubSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBContPlanDetailSubDB";
    ...
    {
        tCount = 1;
        tLOBContPlanDetailSubSchema = new LOBContPlanDetailSubSchema();
        tLOBContPlanDetailSubSchema.setSchema(mResultSet, 1);
        tLOBContPlanDetailSubSet.add(tLOBContPlanDetailSubSchema);
    ...
    return tLOBContPlanDetailSubSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LOBContPlanDetailSubSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBContPlanDetailSubSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LJABonusGetDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: bug2
 */
public class LJABonusGetDB extends LJABonusGetSchema
{
	private static Logger logger = Logger.getLogger(LJABonusGetDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJABonusGetDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJABonusGetSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJABonusGetDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getOtherNoType() == null || this.getOtherNoType().equals("null")) {
				...
				pstmt.setString(3, StrTool.space(this.getOtherNoType(), 1));
			}
			if(this.getBonusYear() == null || this.getBonusYear().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getBonusYear());
			}
			if(this.getPayMode() == null || this.getPayMode().equals("null")) {
				pstmt.setNull(5, 1);
			} else {
				pstmt.setString(5, StrTool.space(this.getPayMode(), 1));
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAPPntName() == null || this.getAPPntName().equals("null")) {
				...
				pstmt.setString(13, this.getAPPntName());
			}
			...
			if(this.getGrpPolNo() == null || this.getGrpPolNo().equals("null")) {
				...
				pstmt.setString(28, this.getGrpPolNo());
			}
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getContNo());
			}
			...
			pstmt.setDouble(32, this.getNetAmount());
			pstmt.setDouble(33, this.getTaxAmount());
			pstmt.setDouble(34, this.getTax());
			// set where condition
			if(this.getActuGetNo() == null || this.getActuGetNo().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getActuGetNo());
			}
			if(this.getOtherNo() == null || this.getOtherNo().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
			...
			if(this.getOtherNoType() == null || this.getOtherNoType().equals("null")) {
				...
				pstmt.setString(37, StrTool.space(this.getOtherNoType(), 1));
			}
			if(this.getBonusYear() == null || this.getBonusYear().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getBonusYear());
			}
			if(this.getPayMode() == null || this.getPayMode().equals("null")) {
				pstmt.setNull(39, 1);
			} else {
				pstmt.setString(39, StrTool.space(this.getPayMode(), 1));
			}
			if(this.getFeeFinaType() == null || this.getFeeFinaType().equals("null")) {
				pstmt.setNull(40, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJABonusGet WHERE  ActuGetNo = ? AND OtherNo = ? AND OtherNoType = ? AND BonusYear = ? AND PayMode = ? AND FeeFinaType = ?", 
	...
	public LJABonusGetSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJABonusGetSet aLJABonusGetSet = new LJABonusGetSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJABonusGet");
			LJABonusGetSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJABonusGetSchema s1 = new LJABonusGetSchema();
				s1.setSchema(rs,i);
				aLJABonusGetSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJABonusGetSet;
	}

	public LJABonusGetSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJABonusGetSet aLJABonusGetSet = new LJABonusGetSet();
	...
	public LJABonusGetSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJABonusGetSet aLJABonusGetSet = new LJABonusGetSet();
	...
	public LJABonusGetSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJABonusGetSet aLJABonusGetSet = new LJABonusGetSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJABonusGet " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public LJABonusGetSet getData()
{
    int tCount = 0;
    LJABonusGetSet tLJABonusGetSet = new LJABonusGetSet();
    LJABonusGetSchema tLJABonusGetSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJABonusGetDB";
    ...
    try
    {
        ...
        tLJABonusGetSchema.setSchema(mResultSet, 1);
        tLJABonusGetSet.add(tLJABonusGetSchema);
...
public boolean closeData()
{
	...
	public LJABonusGetSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJABonusGetSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

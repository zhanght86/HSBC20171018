...
/*
 * <p>ClassName: LPInsureAccClassFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPInsureAccClassFeeDB extends LPInsureAccClassFeeSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPInsureAccClassFeeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPInsureAccClassFeeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPInsureAccClassFeeDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getFeeCode() == null || this.getFeeCode().equals("null")) {
				...
				pstmt.setString(8, this.getFeeCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOtherType() == null || this.getOtherType().equals("null")) {
				...
				pstmt.setString(7, this.getOtherType());
			}
			if(this.getAccAscription() == null || this.getAccAscription().equals("null")) {
				pstmt.setNull(8, 1);
			} else {
			...
			pstmt.setDouble(22, this.getFeeRate());
			pstmt.setDouble(23, this.getFee());
			pstmt.setDouble(24, this.getFeeUnit());
			...
			if(this.getInvestType() == null || this.getInvestType().equals("null")) {
				...
				pstmt.setString(32, this.getInvestType());
			}
			if(this.getIsInitFee() == null || this.getIsInitFee().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
				pstmt.setString(33, this.getIsInitFee());
			}
			...
			if(this.getFeeCode() == null || this.getFeeCode().equals("null")) {
				...
				pstmt.setString(43, this.getFeeCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPInsureAccClassFee WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ? AND InsuAccNo = ? AND PayPlanCode = ? AND OtherNo = ? AND AccAscription = ? AND FeeCode = ?", 
	...
	public LPInsureAccClassFeeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPInsureAccClassFee");
			LPInsureAccClassFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPInsureAccClassFeeSchema s1 = new LPInsureAccClassFeeSchema();
				s1.setSchema(rs,i);
				aLPInsureAccClassFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPInsureAccClassFeeSet;
	}

	public LPInsureAccClassFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPInsureAccClassFeeSet aLPInsureAccClassFeeSet = new LPInsureAccClassFeeSet();
	...
	public LPInsureAccClassFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPInsureAccClassFeeSet aLPInsureAccClassFeeSet = new LPInsureAccClassFeeSet();
	...
	public LPInsureAccClassFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPInsureAccClassFeeSet aLPInsureAccClassFeeSet = new LPInsureAccClassFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPInsureAccClassFee " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPInsureAccClassFeeSet
 */
public LPInsureAccClassFeeSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLPInsureAccClassFeeSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LPInsureAccClassFeeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPInsureAccClassFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

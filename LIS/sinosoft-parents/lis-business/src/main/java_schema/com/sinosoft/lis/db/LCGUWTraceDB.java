...
/*
 * <p>ClassName: LCGUWTraceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LCGUWTraceDB extends LCGUWTraceSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCGUWTraceDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCGUWTraceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCGUWTraceDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getRuleLevel() == null || this.getRuleLevel().equals("null")) {
				...
				pstmt.setString(3, this.getRuleLevel());
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getUpReport() == null || this.getUpReport().equals("null")) {
				...
				pstmt.setString(14, this.getUpReport());
			}
			if(this.getUpReportContent() == null || this.getUpReportContent().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getUpReportContent());
			}
			...
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				...
				pstmt.setString(31, this.getGrpContNo());
			}
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(32, 12);
			} else {
			...
			if(this.getRuleLevel() == null || this.getRuleLevel().equals("null")) {
				...
				pstmt.setString(33, this.getRuleLevel());
			}
			pstmt.setInt(34, this.getUWNo());
			if(this.getRuleCode() == null || this.getRuleCode().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getRuleCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCGUWTrace WHERE  GrpContNo = ? AND RiskCode = ? AND RuleLevel = ? AND UWNo = ? AND RuleCode = ?", 
	...
	public LCGUWTraceSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGUWTraceSet aLCGUWTraceSet = new LCGUWTraceSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCGUWTrace");
			LCGUWTraceSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCGUWTraceSchema s1 = new LCGUWTraceSchema();
				s1.setSchema(rs,i);
				aLCGUWTraceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCGUWTraceSet;
	}

	public LCGUWTraceSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGUWTraceSet aLCGUWTraceSet = new LCGUWTraceSet();
	...
	public LCGUWTraceSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGUWTraceSet aLCGUWTraceSet = new LCGUWTraceSet();
	...
	public LCGUWTraceSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGUWTraceSet aLCGUWTraceSet = new LCGUWTraceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCGUWTrace " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCGUWTraceSet
 */
public LCGUWTraceSet getData()
{
    int tCount = 0;
    LCGUWTraceSet tLCGUWTraceSet = new LCGUWTraceSet();
    LCGUWTraceSchema tLCGUWTraceSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCGUWTraceDB";
    ...
    {
        tCount = 1;
        tLCGUWTraceSchema = new LCGUWTraceSchema();
        tLCGUWTraceSchema.setSchema(mResultSet, 1);
        tLCGUWTraceSet.add(tLCGUWTraceSchema);
...
public boolean closeData()
{
	...
	public LCGUWTraceSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCGUWTraceSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

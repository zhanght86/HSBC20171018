...
import com.sinosoft.lis.schema.LOBInsureAccFeeTraceSchema;
import com.sinosoft.lis.vschema.LOBInsureAccFeeTraceSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LOBInsureAccFeeTraceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: bug
 */
public class LOBInsureAccFeeTraceDB extends LOBInsureAccFeeTraceSchema
{
	private static Logger logger = Logger.getLogger(LOBInsureAccFeeTraceDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LOBInsureAccFeeTraceDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LOBInsureAccFeeTrace" );
		mflag = true;
	}

	public LOBInsureAccFeeTraceDB()
	{
		con = null;
		db = new DBOper( "LOBInsureAccFeeTrace" );
		mflag = false;
	}
	...
	{
		LOBInsureAccFeeTraceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBInsureAccFeeTraceDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LOBInsureAccFeeTrace WHERE  SerialNo = ?");
		...
		SQLString sqlObj = new SQLString("LOBInsureAccFeeTrace");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LOBInsureAccFeeTrace VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBInsureAccFeeTrace WHERE  SerialNo = ?", 
	...
	public LOBInsureAccFeeTraceSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBInsureAccFeeTraceSet aLOBInsureAccFeeTraceSet = new LOBInsureAccFeeTraceSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBInsureAccFeeTrace");
			LOBInsureAccFeeTraceSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBInsureAccFeeTraceSchema s1 = new LOBInsureAccFeeTraceSchema();
				s1.setSchema(rs,i);
				aLOBInsureAccFeeTraceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBInsureAccFeeTraceSet;
	}

	public LOBInsureAccFeeTraceSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBInsureAccFeeTraceSet aLOBInsureAccFeeTraceSet = new LOBInsureAccFeeTraceSet();
	...
	public LOBInsureAccFeeTraceSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBInsureAccFeeTraceSet aLOBInsureAccFeeTraceSet = new LOBInsureAccFeeTraceSet();
	...
	public LOBInsureAccFeeTraceSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBInsureAccFeeTraceSet aLOBInsureAccFeeTraceSet = new LOBInsureAccFeeTraceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBInsureAccFeeTrace " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBInsureAccFeeTraceSet
 */
public LOBInsureAccFeeTraceSet getData()
{
    int tCount = 0;
    LOBInsureAccFeeTraceSet tLOBInsureAccFeeTraceSet = new LOBInsureAccFeeTraceSet();
    LOBInsureAccFeeTraceSchema tLOBInsureAccFeeTraceSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBInsureAccFeeTraceDB";
    ...
    {
        tCount = 1;
        tLOBInsureAccFeeTraceSchema = new LOBInsureAccFeeTraceSchema();
        tLOBInsureAccFeeTraceSchema.setSchema(mResultSet, 1);
        tLOBInsureAccFeeTraceSet.add(tLOBInsureAccFeeTraceSchema);
    ...
    return tLOBInsureAccFeeTraceSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LOBInsureAccFeeTraceSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LOBInsureAccFeeTraceSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

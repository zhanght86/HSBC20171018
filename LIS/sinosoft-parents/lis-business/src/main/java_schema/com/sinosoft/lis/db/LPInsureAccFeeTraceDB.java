...
/*
 * <p>ClassName: LPInsureAccFeeTraceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: bug2
 */
public class LPInsureAccFeeTraceDB extends LPInsureAccFeeTraceSchema
{
	private static Logger logger = Logger.getLogger(LPInsureAccFeeTraceDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LPInsureAccFeeTraceDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LPInsureAccFeeTrace" );
		mflag = true;
	}

	public LPInsureAccFeeTraceDB()
	{
		con = null;
		db = new DBOper( "LPInsureAccFeeTrace" );
		mflag = false;
	}
	...
	{
		LPInsureAccFeeTraceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPInsureAccFeeTraceDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LPInsureAccFeeTrace WHERE  SerialNo = ? AND EdorNo = ? AND EdorType = ?");
		...
		SQLString sqlObj = new SQLString("LPInsureAccFeeTrace");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LPInsureAccFeeTrace SET  GrpContNo = ? , GrpPolNo = ? , ContNo = ? , PolNo = ? , SerialNo = ? , InsuAccNo = ? , RiskCode = ? , PayPlanCode = ? , OtherNo = ? , OtherType = ? , AccAscription = ? , MoneyType = ? , FeeRate = ? , Fee = ? , FeeUnit = ? , PayDate = ? , State = ? , FeeCode = ? , InerSerialNo = ? , ManageCom = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , EdorNo = ? , EdorType = ? , PayNo = ? , Currency = ? , INvesttype = ? , AccAlterNo = ? , AccAlterType = ? , ConfDate = ? , AllocationDate = ? , PricingDate = ? , ComCode = ? , ModifyOperator = ? , NetAmount = ? , TaxAmount = ? , Tax = ? WHERE  SerialNo = ? AND EdorNo = ? AND EdorType = ?");
			...
			if(this.getPayNo() == null || this.getPayNo().equals("null")) {
				...
				pstmt.setString(28, this.getPayNo());
			}
			...
			if(this.getINvesttype() == null || this.getINvesttype().equals("null")) {
				...
				pstmt.setString(30, this.getINvesttype());
			}
			if(this.getAccAlterNo() == null || this.getAccAlterNo().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
				pstmt.setString(31, this.getAccAlterNo());
			}
			if(this.getAccAlterType() == null || this.getAccAlterType().equals("null")) {
				pstmt.setNull(32, 12);
			} else {
				pstmt.setString(32, this.getAccAlterType());
			}
			if(this.getConfDate() == null || this.getConfDate().equals("null")) {
				pstmt.setNull(33, 91);
			} else {
				pstmt.setDate(33, Date.valueOf(this.getConfDate()));
			}
			if(this.getAllocationDate() == null || this.getAllocationDate().equals("null")) {
				pstmt.setNull(34, 91);
			} else {
				pstmt.setDate(34, Date.valueOf(this.getAllocationDate()));
			}
			if(this.getPricingDate() == null || this.getPricingDate().equals("null")) {
				pstmt.setNull(35, 91);
			} else {
				pstmt.setDate(35, Date.valueOf(this.getPricingDate()));
			}
			if(this.getComCode() == null || this.getComCode().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
				pstmt.setString(36, this.getComCode());
			}
			if(this.getModifyOperator() == null || this.getModifyOperator().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getModifyOperator());
			}
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LPInsureAccFeeTrace VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPInsureAccFeeTrace WHERE  SerialNo = ? AND EdorNo = ? AND EdorType = ?", 
	...
	public LPInsureAccFeeTraceSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPInsureAccFeeTrace");
			LPInsureAccFeeTraceSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPInsureAccFeeTraceSchema s1 = new LPInsureAccFeeTraceSchema();
				s1.setSchema(rs,i);
				aLPInsureAccFeeTraceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPInsureAccFeeTraceSet;
	}

	public LPInsureAccFeeTraceSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPInsureAccFeeTraceSet aLPInsureAccFeeTraceSet = new LPInsureAccFeeTraceSet();
	...
	public LPInsureAccFeeTraceSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPInsureAccFeeTraceSet aLPInsureAccFeeTraceSet = new LPInsureAccFeeTraceSet();
	...
	public LPInsureAccFeeTraceSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPInsureAccFeeTraceSet aLPInsureAccFeeTraceSet = new LPInsureAccFeeTraceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPInsureAccFeeTrace " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPInsureAccFeeTraceSet
 */
public LPInsureAccFeeTraceSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLPInsureAccFeeTraceSchema.setSchema(mResultSet, 1);
    ...
    return tLPInsureAccFeeTraceSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LPInsureAccFeeTraceSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LPInsureAccFeeTraceSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

...
/*
 * <p>ClassName: LCUWMasterDBSet </p>
 * <p>Description: DB层多记录数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCUWMasterDBSet extends LCUWMasterSet
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCUWMasterDBSet(Connection tConnection)
	{
		con = tConnection;
		db = new DBOper(con,"LCUWMaster");
		mflag = true;
	}

	public LCUWMasterDBSet()
	{
		db = new DBOper( "LCUWMaster" );
	...
	public boolean deleteSQL()
	{
		...
		else
		{
			...
			tError.moduleName = "LCUWMasterDBSet";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt = con.prepareStatement("DELETE FROM LCUWMaster WHERE  ProposalNo = ?");
		...
		SQLString sqlObj = new SQLString("LCUWMaster");
...
ex.printStackTrace();
	...
	public boolean update()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("UPDATE LCUWMaster SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , PolNo = ? , ProposalNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , UpReport = ? , BatchNo = ? , UWType = ? , AutoUWDate = ? , UWConclu = ? , UWDate = ? , ComCode = ? , ModifyOperator = ? WHERE  ProposalNo = ?");
            for (int i = 1; i <= tCount; i++)
            {
			...
			if(this.get(i).getProposalNo() == null || this.get(i).getProposalNo().equals("null")) {
				...
				pstmt.setString(49, this.get(i).getProposalNo());
			}
...
ex.printStackTrace();
	...
	public boolean insert()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("INSERT INTO LCUWMaster VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getGrpContNo() == null || this.get(i).getGrpContNo().equals("null")) {
				pstmt.setString(1,null);

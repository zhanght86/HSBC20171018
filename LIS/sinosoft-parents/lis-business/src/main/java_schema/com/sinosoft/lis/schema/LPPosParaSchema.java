...
import com.sinosoft.lis.db.LPPosParaDB;

/*
 * <p>ClassName: LPPosParaSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LPPosParaSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPPosParaSchema.class);
	// @Field
	/** 序号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPPosParaSchema cloned = (LPPosParaSchema)super.clone();
	...
	*/
	...
	public String getBalanType()
	{
		return BalanType;
	}
	public void setBalanType(String aBalanType)
	{
		if(aBalanType!=null && aBalanType.length()>2)
			throw new IllegalArgumentException("结算频率BalanType值"+aBalanType+"的长度"+aBalanType.length()+"大于最大值2");
		BalanType = aBalanType;
	}
	/**
	* 续期为提前抽档天数
	*/
	...
	/**
	* 使用另外一个 LPPosParaSchema 对象给 Schema 赋值
	* @param: aLPPosParaSchema LPPosParaSchema
	**/
	public void setSchema(LPPosParaSchema aLPPosParaSchema)
	{
		this.SerialNo = aLPPosParaSchema.getSerialNo();
		this.BussNo = aLPPosParaSchema.getBussNo();
		this.BussType = aLPPosParaSchema.getBussType();
		this.ManageCom = aLPPosParaSchema.getManageCom();
		this.PayIntv = aLPPosParaSchema.getPayIntv();
		this.BalanType = aLPPosParaSchema.getBalanType();
		this.PumpDays = aLPPosParaSchema.getPumpDays();
		this.GracePeriod = aLPPosParaSchema.getGracePeriod();
		this.Segment1 = aLPPosParaSchema.getSegment1();
		this.Segment2 = aLPPosParaSchema.getSegment2();
		this.Segment3 = aLPPosParaSchema.getSegment3();
		this.MakeOperator = aLPPosParaSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLPPosParaSchema.getMakeDate());
		this.MakeTime = aLPPosParaSchema.getMakeTime();
		this.ModifyOperator = aLPPosParaSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLPPosParaSchema.getModifyDate());
		this.ModifyTime = aLPPosParaSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			if( rs.getString("BalanType") == null )
				this.BalanType = null;
			else
				this.BalanType = rs.getString("BalanType").trim();
		...
		{
			logger.debug("数据库中的LPPosPara表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPPosParaSchema";
	...
	public LPPosParaSchema getSchema()
	{
		LPPosParaSchema aLPPosParaSchema = new LPPosParaSchema();
		aLPPosParaSchema.setSchema(this);
		return aLPPosParaSchema;
	}

	public LPPosParaDB getDB()
	{
		LPPosParaDB aDBOper = new LPPosParaDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPPosPara描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(BalanType)); strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPPosPara>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			BalanType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			PumpDays= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,7,SysConst.PACKAGESPILTER))).intValue();
			GracePeriod = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			Segment1 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			Segment2 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			Segment3 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("BalanType"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(BalanType));
		}
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 4:
				...
				break;
			case 5:
				strFieldValue = StrTool.GBKToUnicode(BalanType);
				break;
			case 6:
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPPosParaSchema other = (LPPosParaSchema)otherObject;
		return
			...
			&& BalanType.equals(other.getBalanType())
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("BalanType") ) {
			return 5;
		}

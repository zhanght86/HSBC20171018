...
/*
 * <p>ClassName: LLPrepayDetailDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LLPrepayDetailDB extends LLPrepayDetailSchema
{
private static Logger logger = Logger.getLogger(LLPrepayDetailDB.class);
	...
	**/
	...
	// @Constructor
	public LLPrepayDetailDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLPrepayDetailSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLPrepayDetailDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getPrepayNo() == null || this.getPrepayNo().equals("null")) {
				...
				pstmt.setString(7, this.getPrepayNo());
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRgtNo() == null || this.getRgtNo().equals("null")) {
				...
				pstmt.setString(8, this.getRgtNo());
			}
			if(this.getDeclineNo() == null || this.getDeclineNo().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getDeclineNo());
			}
			...
			if(this.getRiskVer() == null || this.getRiskVer().equals("null")) {
				...
				pstmt.setString(15, this.getRiskVer());
			}
			...
			pstmt.setDouble(18, this.getPrepaySum());
			...
			if(this.getMngCom() == null || this.getMngCom().equals("null")) {
				...
				pstmt.setString(21, this.getMngCom());
			}
			...
			if(this.getNBPolNo() == null || this.getNBPolNo().equals("null")) {
				...
				pstmt.setString(30, this.getNBPolNo());
			}
			if(this.getCaseRelaNo() == null || this.getCaseRelaNo().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
				pstmt.setString(31, this.getCaseRelaNo());
			}
			...
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				...
				pstmt.setString(34, this.getCaseNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
			...
			if(this.getGetDutyCode() == null || this.getGetDutyCode().equals("null")) {
				...
				pstmt.setString(37, this.getGetDutyCode());
			}
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
			...
			if(this.getPrepayNo() == null || this.getPrepayNo().equals("null")) {
				...
				pstmt.setString(39, this.getPrepayNo());
			}
			if(this.getDutyCode() == null || this.getDutyCode().equals("null")) {
				pstmt.setNull(40, 12);
			} else {
			...
			if(this.getCaseRelaNo() == null || this.getCaseRelaNo().equals("null")) {
				...
				pstmt.setString(41, this.getCaseRelaNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLPrepayDetail WHERE  ClmNo = ? AND CaseNo = ? AND PolNo = ? AND GetDutyKind = ? AND GetDutyCode = ? AND SerialNo = ? AND PrepayNo = ? AND DutyCode = ? AND CaseRelaNo = ?", 
	...
	public LLPrepayDetailSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLPrepayDetailSet aLLPrepayDetailSet = new LLPrepayDetailSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLPrepayDetail");
			LLPrepayDetailSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLPrepayDetailSchema s1 = new LLPrepayDetailSchema();
				s1.setSchema(rs,i);
				aLLPrepayDetailSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLPrepayDetailSet;

	}

	public LLPrepayDetailSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLPrepayDetailSet aLLPrepayDetailSet = new LLPrepayDetailSet();
	...
	public LLPrepayDetailSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLPrepayDetailSet aLLPrepayDetailSet = new LLPrepayDetailSet();
	...
	public LLPrepayDetailSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLPrepayDetailSet aLLPrepayDetailSet = new LLPrepayDetailSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLPrepayDetail " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLPrepayDetailSet
 */
public LLPrepayDetailSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLLPrepayDetailSchema.setSchema(mResultSet, 1);
        tLLPrepayDetailSet.add(tLLPrepayDetailSchema);
...
public boolean closeData()
{
	...
	public LLPrepayDetailSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLPrepayDetailSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LLContHangUpStateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLContHangUpStateDB extends LLContHangUpStateSchema
{
private static Logger logger = Logger.getLogger(LLContHangUpStateDB.class);
	...
	**/
	...
	// @Constructor
	public LLContHangUpStateDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLContHangUpStateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLContHangUpStateDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getHangUpType() == null || this.getHangUpType().equals("null")) {
				...
				pstmt.setString(5, this.getHangUpType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getStandFlag4() == null || this.getStandFlag4().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getStandFlag4());
			}
			if(this.getStandFlag5() == null || this.getStandFlag5().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getStandFlag5());
			}
			if(this.getHangUpType() == null || this.getHangUpType().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
			...
			if(this.getHangUpNo() == null || this.getHangUpNo().equals("null")) {
				...
				pstmt.setString(22, this.getHangUpNo());
			}
			...
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				...
				pstmt.setString(24, this.getContNo());
			}
			if(this.getInsuredNo() == null || this.getInsuredNo().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
			...
			if(this.getHangUpType() == null || this.getHangUpType().equals("null")) {
				...
				pstmt.setString(28, this.getHangUpType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLContHangUpState WHERE  ContNo = ? AND InsuredNo = ? AND PolNo = ? AND SerialNo = ? AND HangUpType = ?", 
	...
	public LLContHangUpStateSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLContHangUpStateSet aLLContHangUpStateSet = new LLContHangUpStateSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLContHangUpState");
			LLContHangUpStateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLContHangUpStateSchema s1 = new LLContHangUpStateSchema();
				s1.setSchema(rs,i);
				aLLContHangUpStateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLContHangUpStateSet;

	}

	public LLContHangUpStateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLContHangUpStateSet aLLContHangUpStateSet = new LLContHangUpStateSet();
	...
	public LLContHangUpStateSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLContHangUpStateSet aLLContHangUpStateSet = new LLContHangUpStateSet();
	...
	public LLContHangUpStateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLContHangUpStateSet aLLContHangUpStateSet = new LLContHangUpStateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLContHangUpState " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLContHangUpStateSet
 */
public LLContHangUpStateSet getData()
{
    int tCount = 0;
    LLContHangUpStateSet tLLContHangUpStateSet = new LLContHangUpStateSet();
    LLContHangUpStateSchema tLLContHangUpStateSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLContHangUpStateDB";
    ...
    {
        tCount = 1;
        tLLContHangUpStateSchema = new LLContHangUpStateSchema();
        tLLContHangUpStateSchema.setSchema(mResultSet, 1);
        tLLContHangUpStateSet.add(tLLContHangUpStateSchema);
...
public boolean closeData()
{
	...
	public LLContHangUpStateSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLContHangUpStateSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCIndUWMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCIndUWMasterDB extends LCIndUWMasterSchema
{
private static Logger logger = Logger.getLogger(LCIndUWMasterDB.class);
	...
	**/
	...
	// @Constructor
	public LCIndUWMasterDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCIndUWMaster" );
		mflag = true;
	}

	public LCIndUWMasterDB()
	{
		con = null;
		db = new DBOper( "LCIndUWMaster" );
		mflag = false;
	}
	...
	{
		LCIndUWMasterSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCIndUWMasterDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCIndUWMaster WHERE  ContNo = ? AND InsuredNo = ?");
		...
		SQLString sqlObj = new SQLString("LCIndUWMaster");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCIndUWMaster SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , UpReport = ? , BatchNo = ? , UWType = ? WHERE  ContNo = ? AND InsuredNo = ?");
			...
			pstmt.setInt(40, this.getBatchNo());
			if(this.getUWType() == null || this.getUWType().equals("null")) {
				pstmt.setNull(41, 12);
			} else {
				pstmt.setString(41, this.getUWType());
			}
			// set where condition
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(42, 12);
			...
			if(this.getInsuredNo() == null || this.getInsuredNo().equals("null")) {
				...
				pstmt.setString(43, this.getInsuredNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCIndUWMaster(GrpContNo ,ContNo ,ProposalContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea ,UpReport ,BatchNo ,UWType) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCIndUWMaster WHERE  ContNo = ? AND InsuredNo = ?", 
	...
	public LCIndUWMasterSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCIndUWMasterSet aLCIndUWMasterSet = new LCIndUWMasterSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCIndUWMaster");
			LCIndUWMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCIndUWMasterSchema s1 = new LCIndUWMasterSchema();
				s1.setSchema(rs,i);
				aLCIndUWMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCIndUWMasterSet;

	}

	public LCIndUWMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCIndUWMasterSet aLCIndUWMasterSet = new LCIndUWMasterSet();
	...
	public LCIndUWMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCIndUWMasterSet aLCIndUWMasterSet = new LCIndUWMasterSet();
	...
	public LCIndUWMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCIndUWMasterSet aLCIndUWMasterSet = new LCIndUWMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCIndUWMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCIndUWMasterSet
 */
public LCIndUWMasterSet getData()
{
    int tCount = 0;
    LCIndUWMasterSet tLCIndUWMasterSet = new LCIndUWMasterSet();
    LCIndUWMasterSchema tLCIndUWMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCIndUWMasterDB";
    ...
    {
        tCount = 1;
        tLCIndUWMasterSchema = new LCIndUWMasterSchema();
        tLCIndUWMasterSchema.setSchema(mResultSet, 1);
        tLCIndUWMasterSet.add(tLCIndUWMasterSchema);
...
public boolean closeData()
{
	...
	public LCIndUWMasterSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCIndUWMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

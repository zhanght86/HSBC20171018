...
/*
 * <p>ClassName: LLUWMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLUWMasterDB extends LLUWMasterSchema
{
private static Logger logger = Logger.getLogger(LLUWMasterDB.class);
	...
	**/
	...
	// @Constructor
	public LLUWMasterDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LLUWMaster" );
		mflag = true;
	}
	...
	{
		LLUWMasterSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLUWMasterDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LLUWMaster WHERE  CaseNo = ? AND PolNo = ?");
		...
		SQLString sqlObj = new SQLString("LLUWMaster");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LLUWMaster(CaseNo ,GrpContNo ,BatNo ,ContNo ,ProposalContNo ,PolNo ,ProposalNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCaseNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLUWMaster WHERE  CaseNo = ? AND PolNo = ?", 
	...
	public LLUWMasterSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLUWMaster");
			LLUWMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLUWMasterSchema s1 = new LLUWMasterSchema();
				s1.setSchema(rs,i);
				aLLUWMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLUWMasterSet;

	}

	public LLUWMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWMasterSet aLLUWMasterSet = new LLUWMasterSet();
	...
	public LLUWMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLUWMasterSet aLLUWMasterSet = new LLUWMasterSet();
	...
	public LLUWMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWMasterSet aLLUWMasterSet = new LLUWMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLUWMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLUWMasterSet
 */
public LLUWMasterSet getData()
{
    int tCount = 0;
    LLUWMasterSet tLLUWMasterSet = new LLUWMasterSet();
    LLUWMasterSchema tLLUWMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLUWMasterDB";
    ...
    try
    {
        ...
        tLLUWMasterSchema.setSchema(mResultSet, 1);
        tLLUWMasterSet.add(tLLUWMasterSchema);
...
public boolean closeData()
{
	...
	public LLUWMasterSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLUWMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

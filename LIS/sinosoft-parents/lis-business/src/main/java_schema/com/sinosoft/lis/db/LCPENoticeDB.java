...
/*
 * <p>ClassName: LCPENoticeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCPENoticeDB extends LCPENoticeSchema
{
private static Logger logger = Logger.getLogger(LCPENoticeDB.class);
	...
	**/
	...
	// @Constructor
	public LCPENoticeDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCPENotice" );
		mflag = true;
	}
	...
	{
		LCPENoticeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCPENoticeDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCPENotice WHERE  ProposalContNo = ? AND PrtSeq = ? AND CustomerNo = ?");
		...
		SQLString sqlObj = new SQLString("LCPENotice");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCPENotice SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , PrtSeq = ? , AppName = ? , RiskName = ? , PEDate = ? , CustomerNo = ? , Name = ? , PEAddress = ? , PEBeforeCond = ? , PrintFlag = ? , ManageCom = ? , AgentName = ? , AgentCode = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Remark = ? , PEResult = ? , GrpPrtSeq = ? , PEDoctor = ? , REPEDate = ? , PEReason = ? , MasculineFlag = ? , CustomerType = ? , HospitCode = ? , PETimes = ? , SendDate = ? , SendTime = ? WHERE  ProposalContNo = ? AND PrtSeq = ? AND CustomerNo = ?");
			...
			if(this.getSendDate() == null || this.getSendDate().equals("null")) {
				...
				pstmt.setDate(31, Date.valueOf(this.getSendDate()));
			}
			if(this.getSendTime() == null || this.getSendTime().equals("null")) {
				pstmt.setNull(32, 1);
			} else {
				pstmt.setString(32, this.getSendTime());
			}
			// set where condition
			if(this.getProposalContNo() == null || this.getProposalContNo().equals("null")) {
				pstmt.setNull(33, 12);
			...
			if(this.getPrtSeq() == null || this.getPrtSeq().equals("null")) {
				...
				pstmt.setString(34, this.getPrtSeq());
			}
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getCustomerNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCPENotice(GrpContNo ,ContNo ,ProposalContNo ,PrtSeq ,AppName ,RiskName ,PEDate ,CustomerNo ,Name ,PEAddress ,PEBeforeCond ,PrintFlag ,ManageCom ,AgentName ,AgentCode ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,Remark ,PEResult ,GrpPrtSeq ,PEDoctor ,REPEDate ,PEReason ,MasculineFlag ,CustomerType ,HospitCode ,PETimes ,SendDate ,SendTime) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPENotice WHERE  ProposalContNo = ? AND PrtSeq = ? AND CustomerNo = ?", 
	...
	public LCPENoticeSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPENoticeSet aLCPENoticeSet = new LCPENoticeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPENotice");
			LCPENoticeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCPENoticeSchema s1 = new LCPENoticeSchema();
				s1.setSchema(rs,i);
				aLCPENoticeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCPENoticeSet;

	}

	public LCPENoticeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPENoticeSet aLCPENoticeSet = new LCPENoticeSet();
	...
	public LCPENoticeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPENoticeSet aLCPENoticeSet = new LCPENoticeSet();
	...
	public LCPENoticeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPENoticeSet aLCPENoticeSet = new LCPENoticeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPENotice " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCPENoticeSet
 */
public LCPENoticeSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLCPENoticeSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCPENoticeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPENoticeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

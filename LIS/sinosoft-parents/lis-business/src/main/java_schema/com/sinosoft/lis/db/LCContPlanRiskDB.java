...
/*
 * <p>ClassName: LCContPlanRiskDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LCContPlanRiskDB extends LCContPlanRiskSchema
{
private static Logger logger = Logger.getLogger(LCContPlanRiskDB.class);
	...
	**/
	...
	// @Constructor
	public LCContPlanRiskDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCContPlanRisk" );
		mflag = true;
	}

	public LCContPlanRiskDB()
	{
		con = null;
		db = new DBOper( "LCContPlanRisk" );
		mflag = false;
	}
	...
	{
		LCContPlanRiskSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCContPlanRiskDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCContPlanRisk WHERE  ProposalGrpContNo = ? AND MainRiskCode = ? AND RiskCode = ? AND ContPlanCode = ? AND PlanType = ?");
		...
		SQLString sqlObj = new SQLString("LCContPlanRisk");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCContPlanRisk SET  GrpContNo = ? , ProposalGrpContNo = ? , MainRiskCode = ? , MainRiskVersion = ? , RiskCode = ? , RiskVersion = ? , ContPlanCode = ? , ContPlanName = ? , Remark = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , PlanType = ? , RiskPrem = ? , SysContPlanCode = ? , SysContPlanMult = ? , SysContPlanRate = ? , Currency = ? WHERE  ProposalGrpContNo = ? AND MainRiskCode = ? AND RiskCode = ? AND ContPlanCode = ? AND PlanType = ?");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCContPlanRisk(GrpContNo ,ProposalGrpContNo ,MainRiskCode ,MainRiskVersion ,RiskCode ,RiskVersion ,ContPlanCode ,ContPlanName ,Remark ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,PlanType ,RiskPrem ,SysContPlanCode ,SysContPlanMult ,SysContPlanRate ,Currency) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCContPlanRisk WHERE  ProposalGrpContNo = ? AND MainRiskCode = ? AND RiskCode = ? AND ContPlanCode = ? AND PlanType = ?", 
	...
	public LCContPlanRiskSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCContPlanRisk");
			LCContPlanRiskSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCContPlanRiskSchema s1 = new LCContPlanRiskSchema();
				s1.setSchema(rs,i);
				aLCContPlanRiskSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCContPlanRiskSet;

	}

	public LCContPlanRiskSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCContPlanRiskSet aLCContPlanRiskSet = new LCContPlanRiskSet();
	...
	public LCContPlanRiskSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCContPlanRiskSet aLCContPlanRiskSet = new LCContPlanRiskSet();
	...
	public LCContPlanRiskSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCContPlanRiskSet aLCContPlanRiskSet = new LCContPlanRiskSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCContPlanRisk " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public LCContPlanRiskSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLCContPlanRiskSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCContPlanRiskSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCContPlanRiskSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

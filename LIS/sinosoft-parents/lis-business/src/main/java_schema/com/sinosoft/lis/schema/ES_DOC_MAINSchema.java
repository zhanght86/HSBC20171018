...
/*
 * <p>ClassName: ES_DOC_MAINSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: bug4
 */
public class ES_DOC_MAINSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(ES_DOC_MAINSchema.class);
	// @Field
	/** 单证编号 */
	private int DocID;
	/** 单证号码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		ES_DOC_MAINSchema cloned = (ES_DOC_MAINSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 ES_DOC_MAINSchema 对象给 Schema 赋值
	* @param: aES_DOC_MAINSchema ES_DOC_MAINSchema
	**/
	public void setSchema(ES_DOC_MAINSchema aES_DOC_MAINSchema)
	{
		this.DocID = aES_DOC_MAINSchema.getDocID();
		this.DocCode = aES_DOC_MAINSchema.getDocCode();
		this.BussType = aES_DOC_MAINSchema.getBussType();
		this.SubType = aES_DOC_MAINSchema.getSubType();
		this.NumPages = aES_DOC_MAINSchema.getNumPages();
		this.DocFlag = aES_DOC_MAINSchema.getDocFlag();
		this.DocRemark = aES_DOC_MAINSchema.getDocRemark();
		this.ScanOperator = aES_DOC_MAINSchema.getScanOperator();
		this.ManageCom = aES_DOC_MAINSchema.getManageCom();
		this.InputState = aES_DOC_MAINSchema.getInputState();
		this.Operator = aES_DOC_MAINSchema.getOperator();
		this.InputStartDate = fDate.getDate( aES_DOC_MAINSchema.getInputStartDate());
		this.InputStartTime = aES_DOC_MAINSchema.getInputStartTime();
		this.InputEndDate = fDate.getDate( aES_DOC_MAINSchema.getInputEndDate());
		this.InputEndTime = aES_DOC_MAINSchema.getInputEndTime();
		this.MakeDate = fDate.getDate( aES_DOC_MAINSchema.getMakeDate());
		this.MakeTime = aES_DOC_MAINSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aES_DOC_MAINSchema.getModifyDate());
		this.ModifyTime = aES_DOC_MAINSchema.getModifyTime();
		this.Version = aES_DOC_MAINSchema.getVersion();
		this.ScanNo = aES_DOC_MAINSchema.getScanNo();
		this.PrintCode = aES_DOC_MAINSchema.getPrintCode();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的ES_DOC_MAIN表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "ES_DOC_MAINSchema";
	...
	public ES_DOC_MAINSchema getSchema()
	{
		ES_DOC_MAINSchema aES_DOC_MAINSchema = new ES_DOC_MAINSchema();
		aES_DOC_MAINSchema.setSchema(this);
		return aES_DOC_MAINSchema;
	}

	public ES_DOC_MAINDB getDB()
	{
		ES_DOC_MAINDB aDBOper = new ES_DOC_MAINDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpES_DOC_MAIN>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		ES_DOC_MAINSchema other = (ES_DOC_MAINSchema)otherObject;
		return

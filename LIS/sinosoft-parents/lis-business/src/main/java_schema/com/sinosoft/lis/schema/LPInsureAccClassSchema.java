 ...
 * @Database: 保全核心
 */
public class LPInsureAccClassSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPInsureAccClassSchema.class);
	// @Field
	/** 批单号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPInsureAccClassSchema cloned = (LPInsureAccClassSchema)super.clone();
	...
	**/
	public void setSchema(LPInsureAccClassSchema aLPInsureAccClassSchema)
	{
		...
		this.AccFoundDate = fDate.getDate( aLPInsureAccClassSchema.getAccFoundDate());
		...
		this.AscriptType = aLPInsureAccClassSchema.getAscriptType();
		this.AscriptRate = aLPInsureAccClassSchema.getAscriptRate();
		this.FeeDate = fDate.getDate( aLPInsureAccClassSchema.getFeeDate());
		this.UnitPrice = aLPInsureAccClassSchema.getUnitPrice();
		this.Currency = aLPInsureAccClassSchema.getCurrency();
		this.InvestType = aLPInsureAccClassSchema.getInvestType();
		this.ComCode = aLPInsureAccClassSchema.getComCode();
		this.ModifyOperator = aLPInsureAccClassSchema.getModifyOperator();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPInsureAccClass表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			SumPay = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,22,SysConst.PACKAGESPILTER))).doubleValue();
			SumPaym = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,23,SysConst.PACKAGESPILTER))).doubleValue();
			LastAccBala = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,24,SysConst.PACKAGESPILTER))).doubleValue();
			LastUnitCount = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,25,SysConst.PACKAGESPILTER))).doubleValue();
			LastUnitPrice = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,26,SysConst.PACKAGESPILTER))).doubleValue();
			InsuAccBala = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,27,SysConst.PACKAGESPILTER))).doubleValue();
			UnitCount = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,28,SysConst.PACKAGESPILTER))).doubleValue();
			...
			AscriptRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,38,SysConst.PACKAGESPILTER))).doubleValue();
			FeeDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 39,SysConst.PACKAGESPILTER));
			UnitPrice = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,40,SysConst.PACKAGESPILTER))).doubleValue();
			Currency = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 41, SysConst.PACKAGESPILTER );
			InvestType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 42, SysConst.PACKAGESPILTER );
			ComCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 43, SysConst.PACKAGESPILTER );

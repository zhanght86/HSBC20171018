...
/*
 * <p>ClassName: LCGrpFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCGrpFeeDB extends LCGrpFeeSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCGrpFeeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCGrpFeeSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPayInsuAccName() == null || this.getPayInsuAccName().equals("null")) {
				...
				pstmt.setString(7, this.getPayInsuAccName());
			}
			if(this.getFeeCalMode() == null || this.getFeeCalMode().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getFeeCalMode());
			}
			if(this.getFeeCalModeType() == null || this.getFeeCalModeType().equals("null")) {
				pstmt.setNull(9, 1);
			} else {
				pstmt.setString(9, this.getFeeCalModeType());
			}
			if(this.getFeeCalCode() == null || this.getFeeCalCode().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getFeeCalCode());
			}
			pstmt.setDouble(11, this.getFeeValue());
			pstmt.setDouble(12, this.getCompareValue());
			if(this.getFeePeriod() == null || this.getFeePeriod().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getFeePeriod());
			}
			pstmt.setInt(14, this.getMaxTime());
			if(this.getDefaultFlag() == null || this.getDefaultFlag().equals("null")) {
				pstmt.setNull(15, 1);
			} else {
				pstmt.setString(15, this.getDefaultFlag());
			}
			...
			if(this.getGrpPolNo() == null || this.getGrpPolNo().equals("null")) {
				...
				pstmt.setString(34, this.getGrpPolNo());
			}
			if(this.getFeeCode() == null || this.getFeeCode().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
			...
			if(this.getInsuAccNo() == null || this.getInsuAccNo().equals("null")) {
				...
				pstmt.setString(36, this.getInsuAccNo());
			}
			if(this.getPayPlanCode() == null || this.getPayPlanCode().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getPayPlanCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCGrpFee WHERE  GrpPolNo = ? AND FeeCode = ? AND InsuAccNo = ? AND PayPlanCode = ?", 
	...
	public LCGrpFeeSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCGrpFee");
			LCGrpFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCGrpFeeSchema s1 = new LCGrpFeeSchema();
				s1.setSchema(rs,i);
				aLCGrpFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LCGrpFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpFeeSet aLCGrpFeeSet = new LCGrpFeeSet();
	...
	public LCGrpFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGrpFeeSet aLCGrpFeeSet = new LCGrpFeeSet();
	...
	public LCGrpFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpFeeSet aLCGrpFeeSet = new LCGrpFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCGrpFee " + sqlObj.getUpdPart() + " where " + strWherePart;
...
public LCGrpFeeSet getData()
{
    ...
    try
    {
        ...
        tLCGrpFeeSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCGrpFeeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCGrpFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

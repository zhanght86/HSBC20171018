...
/*
 * <p>ClassName: LDImpartSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 基础信息
 */
public class LDImpartSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LDImpartSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LDImpartSchema cloned = (LDImpartSchema)super.clone();
	...
	*/
	...
	public String getDSImpartParam()
	{
		return DSImpartParam;
	}
	public void setDSImpartParam(String aDSImpartParam)
	{
		DSImpartParam = aDSImpartParam;
	}

	/**
	* 使用另外一个 LDImpartSchema 对象给 Schema 赋值
	* @param: aLDImpartSchema LDImpartSchema
	**/
	public void setSchema(LDImpartSchema aLDImpartSchema)
	{
		this.ImpartVer = aLDImpartSchema.getImpartVer();
		this.ImpartCode = aLDImpartSchema.getImpartCode();
		this.ImpartContent = aLDImpartSchema.getImpartContent();
		this.ImpartParamModle = aLDImpartSchema.getImpartParamModle();
		this.UWClaimFlg = aLDImpartSchema.getUWClaimFlg();
		this.PrtFlag = aLDImpartSchema.getPrtFlag();
		this.Remark = aLDImpartSchema.getRemark();
		this.MakeDate = fDate.getDate( aLDImpartSchema.getMakeDate());
		this.MakeTime = aLDImpartSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLDImpartSchema.getModifyDate());
		this.ModifyTime = aLDImpartSchema.getModifyTime();
		this.DSImpartParam = aLDImpartSchema.getDSImpartParam();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			if( rs.getString("DSImpartParam") == null )
				this.DSImpartParam = null;
			else
				this.DSImpartParam = rs.getString("DSImpartParam").trim();

		}
		catch(SQLException sqle)
		{
			logger.debug("数据库中的LDImpart表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LDImpartSchema";
	...
	public LDImpartSchema getSchema()
	{
		LDImpartSchema aLDImpartSchema = new LDImpartSchema();
		aLDImpartSchema.setSchema(this);
		return aLDImpartSchema;
	}

	public LDImpartDB getDB()
	{
		LDImpartDB aDBOper = new LDImpartDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDImpart描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(DSImpartParam));
		return strReturn.toString();
	}

	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDImpart>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ImpartContent = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			ImpartParamModle = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			UWClaimFlg = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			PrtFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			...
			DSImpartParam = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
		}
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("DSImpartParam"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(DSImpartParam));
		}
		if (strReturn.equals(""))
		{
			strReturn = "null";
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 10:
				...
				break;
			case 11:
				strFieldValue = StrTool.GBKToUnicode(DSImpartParam);
				break;
			default:
				strFieldValue = "";
		};
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LDImpartSchema other = (LDImpartSchema)otherObject;
		return
			...
			&& DSImpartParam.equals(other.getDSImpartParam());
	}
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("DSImpartParam") ) {
			return 11;
		}
		return -1;
	}

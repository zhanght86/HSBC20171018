...
/*
 * <p>ClassName: RIBsnsBillBatchDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保系统模型
 */
public class RIBsnsBillBatchDB extends RIBsnsBillBatchSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public RIBsnsBillBatchDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		RIBsnsBillBatchSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "RIBsnsBillBatchDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getIncomeCompanyNo() == null || this.getIncomeCompanyNo().equals("null")) {
				...
				pstmt.setString(4, this.getIncomeCompanyNo());
			}
			...
			if(this.getBillTimes() == null || this.getBillTimes().equals("null")) {
				...
				pstmt.setString(7, this.getBillTimes());
			}
			...
			if(this.getUWOperator() == null || this.getUWOperator().equals("null")) {
				...
				pstmt.setString(10, this.getUWOperator());
			}
			if(this.getIdea() == null || this.getIdea().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getIdea());
			}
			...
			if(this.getStandbyString1() == null || this.getStandbyString1().equals("null")) {
				...
				pstmt.setString(18, this.getStandbyString1());
			}
			if(this.getStandbyString2() == null || this.getStandbyString2().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getStandbyString2());
			}
			if(this.getStandbyString3() == null || this.getStandbyString3().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getStandbyString3());
			}
			// set where condition
			if(this.getBatchNo() == null || this.getBatchNo().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getBatchNo());
			}
			if(this.getBillNo() == null || this.getBillNo().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getBillNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM RIBsnsBillBatch WHERE  BatchNo = ? AND BillNo = ? AND Currency = ?", 
	...
	public RIBsnsBillBatchSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIBsnsBillBatchSet aRIBsnsBillBatchSet = new RIBsnsBillBatchSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("RIBsnsBillBatch");
			RIBsnsBillBatchSchema aSchema = this.getSchema();
			...
			{
				i++;
				RIBsnsBillBatchSchema s1 = new RIBsnsBillBatchSchema();
				s1.setSchema(rs,i);
				aRIBsnsBillBatchSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aRIBsnsBillBatchSet;
	}

	public RIBsnsBillBatchSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIBsnsBillBatchSet aRIBsnsBillBatchSet = new RIBsnsBillBatchSet();
	...
	public RIBsnsBillBatchSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIBsnsBillBatchSet aRIBsnsBillBatchSet = new RIBsnsBillBatchSet();
	...
	public RIBsnsBillBatchSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIBsnsBillBatchSet aRIBsnsBillBatchSet = new RIBsnsBillBatchSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update RIBsnsBillBatch " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return RIBsnsBillBatchSet
 */
public RIBsnsBillBatchSet getData()
{
    int tCount = 0;
    RIBsnsBillBatchSet tRIBsnsBillBatchSet = new RIBsnsBillBatchSet();
    ...
    try
    {
        ...
        tRIBsnsBillBatchSchema.setSchema(mResultSet, 1);
        tRIBsnsBillBatchSet.add(tRIBsnsBillBatchSchema);

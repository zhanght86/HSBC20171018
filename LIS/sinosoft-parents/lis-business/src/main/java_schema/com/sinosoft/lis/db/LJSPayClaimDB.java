...
/*
 * <p>ClassName: LJSPayClaimDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务收费
 */
public class LJSPayClaimDB extends LJSPayClaimSchema
{
	private static Logger logger = Logger.getLogger(LJSPayClaimDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJSPayClaimDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJSPayClaimSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJSPayClaimDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getFeeOperationType() == null || this.getFeeOperationType().equals("null")) {
				...
				pstmt.setString(36, this.getFeeOperationType());
			}
			if(this.getSubFeeOperationType() == null || this.getSubFeeOperationType().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getSubFeeOperationType());
			}
			if(this.getFeeFinaType() == null || this.getFeeFinaType().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getFeeFinaType());
			}
			if(this.getDutyCode() == null || this.getDutyCode().equals("null")) {
				pstmt.setNull(39, 12);
			} else {
			...
			if(this.getGetDutyKind() == null || this.getGetDutyKind().equals("null")) {
				...
				pstmt.setString(40, this.getGetDutyKind());
			}
			if(this.getGetDutyCode() == null || this.getGetDutyCode().equals("null")) {
				pstmt.setNull(41, 12);
			} else {
				pstmt.setString(41, this.getGetDutyCode());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJSPayClaim WHERE  GetNoticeNo = ? AND FeeOperationType = ? AND SubFeeOperationType = ? AND FeeFinaType = ? AND DutyCode = ? AND GetDutyKind = ? AND GetDutyCode = ? AND PolNo = ?", 
	...
	public LJSPayClaimSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSPayClaimSet aLJSPayClaimSet = new LJSPayClaimSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJSPayClaim");
			LJSPayClaimSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJSPayClaimSchema s1 = new LJSPayClaimSchema();
				s1.setSchema(rs,i);
				aLJSPayClaimSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJSPayClaimSet;
	}

	public LJSPayClaimSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSPayClaimSet aLJSPayClaimSet = new LJSPayClaimSet();
	...
	public LJSPayClaimSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSPayClaimSet aLJSPayClaimSet = new LJSPayClaimSet();
	...
	public LJSPayClaimSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSPayClaimSet aLJSPayClaimSet = new LJSPayClaimSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJSPayClaim " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJSPayClaimSet
 */
public LJSPayClaimSet getData()
{
    int tCount = 0;
    LJSPayClaimSet tLJSPayClaimSet = new LJSPayClaimSet();
    LJSPayClaimSchema tLJSPayClaimSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJSPayClaimDB";
    ...
    {
        tCount = 1;
        tLJSPayClaimSchema = new LJSPayClaimSchema();
        tLJSPayClaimSchema.setSchema(mResultSet, 1);
        tLJSPayClaimSet.add(tLJSPayClaimSchema);
...
public boolean closeData()
{
	...
	public LJSPayClaimSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJSPayClaimSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

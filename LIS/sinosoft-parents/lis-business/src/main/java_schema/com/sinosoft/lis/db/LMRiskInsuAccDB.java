...
/*
 * <p>ClassName: LMRiskInsuAccDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LMRiskInsuAccDB extends LMRiskInsuAccSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LMRiskInsuAccDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LMRiskInsuAccSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LMRiskInsuAccDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAccBonusFlag() == null || this.getAccBonusFlag().equals("null")) {
				...
				pstmt.setString(24, this.getAccBonusFlag());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LMRiskInsuAcc WHERE  InsuAccNo = ?", 
	...
	public LMRiskInsuAccSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMRiskInsuAccSet aLMRiskInsuAccSet = new LMRiskInsuAccSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("LMRiskInsuAcc");
			LMRiskInsuAccSchema aSchema = this.getSchema();
			...
			{
				i++;
				LMRiskInsuAccSchema s1 = new LMRiskInsuAccSchema();
				s1.setSchema(rs,i);
				aLMRiskInsuAccSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aLMRiskInsuAccSet;
	}

	public LMRiskInsuAccSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LMRiskInsuAccSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMRiskInsuAccSet aLMRiskInsuAccSet = new LMRiskInsuAccSet();
	...
	public LMRiskInsuAccSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMRiskInsuAccSet aLMRiskInsuAccSet = new LMRiskInsuAccSet();
	...
	public LMRiskInsuAccSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMRiskInsuAccSet aLMRiskInsuAccSet = new LMRiskInsuAccSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LMRiskInsuAcc " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public LMRiskInsuAccSet getData()
{
    int tCount = 0;
    LMRiskInsuAccSet tLMRiskInsuAccSet = new LMRiskInsuAccSet();
    LMRiskInsuAccSchema tLMRiskInsuAccSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LMRiskInsuAccDB";
    ...
    try
    {
        ...
        tLMRiskInsuAccSchema.setSchema(mResultSet, 1);

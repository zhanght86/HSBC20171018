...
/*
 * <p>ClassName: LLUWPENoticeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLUWPENoticeDB extends LLUWPENoticeSchema
{
private static Logger logger = Logger.getLogger(LLUWPENoticeDB.class);
	...
	**/
	...
	// @Constructor
	public LLUWPENoticeDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LLUWPENotice" );
		mflag = true;
	}

	public LLUWPENoticeDB()
	{
		con = null;
		db = new DBOper( "LLUWPENotice" );
		mflag = false;
	}
	...
	{
		LLUWPENoticeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLUWPENoticeDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LLUWPENotice WHERE  ClmNo = ? AND BatNo = ? AND ContNo = ? AND PrtSeq = ? AND CustomerNo = ?");
		...
		SQLString sqlObj = new SQLString("LLUWPENotice");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LLUWPENotice SET  ClmNo = ? , BatNo = ? , GrpContNo = ? , ContNo = ? , ProposalContNo = ? , PrtSeq = ? , AppName = ? , RiskName = ? , PEDate = ? , CustomerNo = ? , Name = ? , PEAddress = ? , PEBeforeCond = ? , PrintFlag = ? , ManageCom = ? , AgentName = ? , AgentCode = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Remark = ? , PEResult = ? , GrpPrtSeq = ? , PEDoctor = ? , REPEDate = ? , PEReason = ? , MasculineFlag = ? , CustomerType = ? , HospitCode = ? WHERE  ClmNo = ? AND BatNo = ? AND ContNo = ? AND PrtSeq = ? AND CustomerNo = ?");
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getClmNo());
			...
			if(this.getPEDoctor() == null || this.getPEDoctor().equals("null")) {
				...
				pstmt.setString(26, this.getPEDoctor());
			}
			if(this.getREPEDate() == null || this.getREPEDate().equals("null")) {
				pstmt.setNull(27, 91);
			} else {
				pstmt.setDate(27, Date.valueOf(this.getREPEDate()));
			}
			if(this.getPEReason() == null || this.getPEReason().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getPEReason());
			}
			if(this.getMasculineFlag() == null || this.getMasculineFlag().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getMasculineFlag());
			}
			if(this.getCustomerType() == null || this.getCustomerType().equals("null")) {
				pstmt.setNull(30, 12);
			} else {
				pstmt.setString(30, this.getCustomerType());
			}
			if(this.getHospitCode() == null || this.getHospitCode().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
				pstmt.setString(31, this.getHospitCode());
			}
			// set where condition
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(32, 12);
			} else {
				pstmt.setString(32, this.getClmNo());
			}
			if(this.getBatNo() == null || this.getBatNo().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
				pstmt.setString(33, this.getBatNo());
			}
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(34, 12);
			} else {
				pstmt.setString(34, this.getContNo());
			}
			if(this.getPrtSeq() == null || this.getPrtSeq().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getPrtSeq());
			}
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LLUWPENotice(ClmNo ,BatNo ,GrpContNo ,ContNo ,ProposalContNo ,PrtSeq ,AppName ,RiskName ,PEDate ,CustomerNo ,Name ,PEAddress ,PEBeforeCond ,PrintFlag ,ManageCom ,AgentName ,AgentCode ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,Remark ,PEResult ,GrpPrtSeq ,PEDoctor ,REPEDate ,PEReason ,MasculineFlag ,CustomerType ,HospitCode) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getClmNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLUWPENotice WHERE  ClmNo = ? AND BatNo = ? AND ContNo = ? AND PrtSeq = ? AND CustomerNo = ?", 
	...
	public LLUWPENoticeSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLUWPENoticeSet aLLUWPENoticeSet = new LLUWPENoticeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLUWPENotice");
			LLUWPENoticeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLUWPENoticeSchema s1 = new LLUWPENoticeSchema();
				s1.setSchema(rs,i);
				aLLUWPENoticeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLUWPENoticeSet;

	}

	public LLUWPENoticeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWPENoticeSet aLLUWPENoticeSet = new LLUWPENoticeSet();
	...
	public LLUWPENoticeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLUWPENoticeSet aLLUWPENoticeSet = new LLUWPENoticeSet();
	...
	public LLUWPENoticeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWPENoticeSet aLLUWPENoticeSet = new LLUWPENoticeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLUWPENotice " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLUWPENoticeSet
 */
public LLUWPENoticeSet getData()
{
    int tCount = 0;
    LLUWPENoticeSet tLLUWPENoticeSet = new LLUWPENoticeSet();
    LLUWPENoticeSchema tLLUWPENoticeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLUWPENoticeDB";
    ...
    try
    {
        ...
        tLLUWPENoticeSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLUWPENoticeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLUWPENoticeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LBGeneralDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 泰康未整理PDM
 */
public class LBGeneralDB extends LBGeneralSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LBGeneralDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBGeneralSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getExecuteCom() == null || this.getExecuteCom().equals("null")) {
				...
				pstmt.setString(4, this.getExecuteCom());
			}
			...
			pstmt.setInt(8, this.getEmployeePeoples());
			pstmt.setInt(9, this.getRelationPepoles());
			if(this.getIsPayManageCom() == null || this.getIsPayManageCom().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getIsPayManageCom());
			}
			...
			if(this.getExecuteCom() == null || this.getExecuteCom().equals("null")) {
				...
				pstmt.setString(17, this.getExecuteCom());
			}
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBGeneral WHERE  GrpContNo = ? AND ExecuteCom = ? AND CustomerNo = ?", 
	...
	public LBGeneralSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBGeneral");
			LBGeneralSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBGeneralSchema s1 = new LBGeneralSchema();
				s1.setSchema(rs,i);
				aLBGeneralSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBGeneralSet;
	}

	public LBGeneralSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBGeneralSet aLBGeneralSet = new LBGeneralSet();
	...
	public LBGeneralSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LBGeneralSet aLBGeneralSet = new LBGeneralSet();
	...
	public LBGeneralSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBGeneralSet aLBGeneralSet = new LBGeneralSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBGeneral " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LBGeneralSet
 */
public LBGeneralSet getData()
{
    int tCount = 0;
    LBGeneralSet tLBGeneralSet = new LBGeneralSet();
    LBGeneralSchema tLBGeneralSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBGeneralDB";
    ...
    try
    {
        ...
        tLBGeneralSchema.setSchema(mResultSet, 1);
        tLBGeneralSet.add(tLBGeneralSchema);
...
public boolean closeData()
{
	...
	public LBGeneralSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBGeneralSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

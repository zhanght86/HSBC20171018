...
/*
 * <p>ClassName: LCGrpImportLogDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_17
 */
public class LCGrpImportLogDB extends LCGrpImportLogSchema
{
	private static Logger logger = Logger.getLogger(LCGrpImportLogDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LCGrpImportLogDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCGrpImportLogSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCGrpImportLogDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getContPlanCode() == null || this.getContPlanCode().equals("null")) {
				...
				pstmt.setString(12, this.getContPlanCode());
			}
			if(this.getErrorType() == null || this.getErrorType().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getErrorType());
			}
			if(this.getErrorInfo() == null || this.getErrorInfo().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getErrorInfo());
			}
			if(this.getErrorState() == null || this.getErrorState().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getErrorState());
			}
			...
			if(this.getStandbyFlag3() == null || this.getStandbyFlag3().equals("null")) {
				...
				pstmt.setString(20, this.getStandbyFlag3());
			}
			...
			if(this.getContID() == null || this.getContID().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getContID());
			}
			// set where condition
			if(this.getOtherNoType() == null || this.getOtherNoType().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
				pstmt.setString(25, this.getOtherNoType());
			}
			if(this.getBatchNo() == null || this.getBatchNo().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getBatchNo());
			}
			if(this.getID() == null || this.getID().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCGrpImportLog WHERE  OtherNoType = ? AND BatchNo = ? AND ID = ?", 
	...
	public LCGrpImportLogSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGrpImportLogSet aLCGrpImportLogSet = new LCGrpImportLogSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCGrpImportLog");
			LCGrpImportLogSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCGrpImportLogSchema s1 = new LCGrpImportLogSchema();
				s1.setSchema(rs,i);
				aLCGrpImportLogSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCGrpImportLogSet;
	}

	public LCGrpImportLogSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpImportLogSet aLCGrpImportLogSet = new LCGrpImportLogSet();
	...
	public LCGrpImportLogSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGrpImportLogSet aLCGrpImportLogSet = new LCGrpImportLogSet();
	...
	public LCGrpImportLogSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpImportLogSet aLCGrpImportLogSet = new LCGrpImportLogSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCGrpImportLog " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public LCGrpImportLogSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLCGrpImportLogSchema.setSchema(mResultSet, 1);
        tLCGrpImportLogSet.add(tLCGrpImportLogSchema);
...
public boolean closeData()
{
	...
	public LCGrpImportLogSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LCGrpImportLogSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

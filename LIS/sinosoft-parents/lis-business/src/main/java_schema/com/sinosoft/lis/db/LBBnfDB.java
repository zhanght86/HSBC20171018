...
{
private static Logger logger = Logger.getLogger(LBBnfDB.class);
	...
	**/
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getBnfType() == null || this.getBnfType().equals("null")) {
				...
				pstmt.setString(5, this.getBnfType());
			}
			pstmt.setInt(6, this.getBnfNo());
			if(this.getBnfGrade() == null || this.getBnfGrade().equals("null")) {
				pstmt.setNull(7, 1);
			} else {
				pstmt.setString(7, this.getBnfGrade());
			}
			if(this.getRelationToInsured() == null || this.getRelationToInsured().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getRelationToInsured());
			}
			pstmt.setDouble(9, this.getBnfLot());
			...
			if(this.getPostalAddress() == null || this.getPostalAddress().equals("null")) {
				...
				pstmt.setString(21, this.getPostalAddress());
			}
			if(this.getZipCode() == null || this.getZipCode().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getZipCode());
			}
			...
			if(this.getIDExpDate() == null || this.getIDExpDate().equals("null")) {
				...
				pstmt.setDate(27, Date.valueOf(this.getIDExpDate()));
			}
			// set where condition
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(28, 12);
			...
			if(this.getInsuredNo() == null || this.getInsuredNo().equals("null")) {
				...
				pstmt.setString(29, this.getInsuredNo());
			}
			if(this.getBnfType() == null || this.getBnfType().equals("null")) {
				pstmt.setNull(30, 1);
			} else {
				pstmt.setString(30, StrTool.space1(this.getBnfType(), 1));
			}
			pstmt.setInt(31, this.getBnfNo());
			if(this.getBnfGrade() == null || this.getBnfGrade().equals("null")) {
				pstmt.setNull(32, 1);
			} else {
				pstmt.setString(32, StrTool.space1(this.getBnfGrade(), 1));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBBnf WHERE  PolNo = ? AND InsuredNo = ? AND BnfType = ? AND BnfNo = ? AND BnfGrade = ?", 
 ...
 * @return LBBnfSet
 */
public LBBnfSet getData()
{
    int tCount = 0;
    LBBnfSet tLBBnfSet = new LBBnfSet();
    LBBnfSchema tLBBnfSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBBnfDB";
    ...
    try
    {
        ...
        tLBBnfSchema.setSchema(mResultSet, 1);
        tLBBnfSet.add(tLBBnfSchema);
...
public boolean closeData()
{
	...
	public LBBnfSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBBnfSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
import com.sinosoft.lis.db.LCGUWTraceDB;

/*
 * <p>ClassName: LCGUWTraceSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LCGUWTraceSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LCGUWTraceSchema.class);
	// @Field
	/** 团体保单号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LCGUWTraceSchema cloned = (LCGUWTraceSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LCGUWTraceSchema 对象给 Schema 赋值
	* @param: aLCGUWTraceSchema LCGUWTraceSchema
	**/
	public void setSchema(LCGUWTraceSchema aLCGUWTraceSchema)
	{
		this.GrpContNo = aLCGUWTraceSchema.getGrpContNo();
		this.RiskCode = aLCGUWTraceSchema.getRiskCode();
		this.RuleLevel = aLCGUWTraceSchema.getRuleLevel();
		this.UWNo = aLCGUWTraceSchema.getUWNo();
		this.RuleCode = aLCGUWTraceSchema.getRuleCode();
		this.PostponeDay = aLCGUWTraceSchema.getPostponeDay();
		this.PostponeDate = fDate.getDate( aLCGUWTraceSchema.getPostponeDate());
		this.AutoUWConclu = aLCGUWTraceSchema.getAutoUWConclu();
		this.AutoUWDate = fDate.getDate( aLCGUWTraceSchema.getAutoUWDate());
		this.UWError = aLCGUWTraceSchema.getUWError();
		this.UWConclu = aLCGUWTraceSchema.getUWConclu();
		this.UWIdea = aLCGUWTraceSchema.getUWIdea();
		this.UWDate = fDate.getDate( aLCGUWTraceSchema.getUWDate());
		this.UpReport = aLCGUWTraceSchema.getUpReport();
		this.UpReportContent = aLCGUWTraceSchema.getUpReportContent();
		this.HealthFlag = aLCGUWTraceSchema.getHealthFlag();
		this.QuesFlag = aLCGUWTraceSchema.getQuesFlag();
		this.SpecFlag = aLCGUWTraceSchema.getSpecFlag();
		this.AddPremFlag = aLCGUWTraceSchema.getAddPremFlag();
		this.AddPremReason = aLCGUWTraceSchema.getAddPremReason();
		this.ReportFlag = aLCGUWTraceSchema.getReportFlag();
		this.PrintFlag = aLCGUWTraceSchema.getPrintFlag();
		this.ManageCom = aLCGUWTraceSchema.getManageCom();
		this.ComCode = aLCGUWTraceSchema.getComCode();
		this.MakeOperator = aLCGUWTraceSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLCGUWTraceSchema.getMakeDate());
		this.MakeTime = aLCGUWTraceSchema.getMakeTime();
		this.ModifyOperator = aLCGUWTraceSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLCGUWTraceSchema.getModifyDate());
		this.ModifyTime = aLCGUWTraceSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LCGUWTrace表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LCGUWTraceSchema";
	...
	public LCGUWTraceSchema getSchema()
	{
		LCGUWTraceSchema aLCGUWTraceSchema = new LCGUWTraceSchema();
		aLCGUWTraceSchema.setSchema(this);
		return aLCGUWTraceSchema;
	}

	public LCGUWTraceDB getDB()
	{
		LCGUWTraceDB aDBOper = new LCGUWTraceDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCGUWTrace描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCGUWTrace>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			RuleLevel = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			UWNo= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,4,SysConst.PACKAGESPILTER))).intValue();
			RuleCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			PostponeDay = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			PostponeDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7,SysConst.PACKAGESPILTER));
			AutoUWConclu = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			AutoUWDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9,SysConst.PACKAGESPILTER));
			UWError = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			UWConclu = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			UWIdea = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			UWDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13,SysConst.PACKAGESPILTER));
			UpReport = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			UpReportContent = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LCGUWTraceSchema other = (LCGUWTraceSchema)otherObject;
		return

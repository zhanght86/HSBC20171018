...
/*
 * <p>ClassName: LJSGetClaimDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务付费
 */
public class LJSGetClaimDB extends LJSGetClaimSchema
{
	private static Logger logger = Logger.getLogger(LJSGetClaimDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJSGetClaimDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LJSGetClaim" );
		mflag = true;
	}

	public LJSGetClaimDB()
	{
		con = null;
		db = new DBOper( "LJSGetClaim" );
		mflag = false;
	}
	...
	{
		LJSGetClaimSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJSGetClaimDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LJSGetClaim WHERE  GetNoticeNo = ? AND FeeOperationType = ? AND SubFeeOperationType = ? AND FeeFinaType = ? AND PolNo = ? AND DutyCode = ? AND GetDutyKind = ? AND GetDutyCode = ?");
		...
		SQLString sqlObj = new SQLString("LJSGetClaim");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LJSGetClaim SET  GetNoticeNo = ? , FeeOperationType = ? , SubFeeOperationType = ? , FeeFinaType = ? , OtherNo = ? , OtherNoType = ? , SaleChnl = ? , GrpContNo = ? , ContNo = ? , GrpPolNo = ? , PolNo = ? , DutyCode = ? , GetDutyKind = ? , GetDutyCode = ? , KindCode = ? , RiskCode = ? , RiskVersion = ? , AgentCode = ? , AgentGroup = ? , GetDate = ? , Pay = ? , ManageCom = ? , AgentCom = ? , AgentType = ? , SerialNo = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Currency = ? , ComCode = ? , ModifyOperator = ? , NetAmount = ? , TaxAmount = ? , Tax = ? WHERE  GetNoticeNo = ? AND FeeOperationType = ? AND SubFeeOperationType = ? AND FeeFinaType = ? AND PolNo = ? AND DutyCode = ? AND GetDutyKind = ? AND GetDutyCode = ?");
			if(this.getGetNoticeNo() == null || this.getGetNoticeNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGetNoticeNo());
			...
			if(this.getSaleChnl() == null || this.getSaleChnl().equals("null")) {
				...
				pstmt.setString(7, this.getSaleChnl());
			}
			...
			if(this.getGetDate() == null || this.getGetDate().equals("null")) {
				...
				pstmt.setDate(20, Date.valueOf(this.getGetDate()));
			}
			pstmt.setDouble(21, this.getPay());
			...
			pstmt.setDouble(34, this.getNetAmount());
			pstmt.setDouble(35, this.getTaxAmount());
			pstmt.setDouble(36, this.getTax());
			// set where condition
			if(this.getGetNoticeNo() == null || this.getGetNoticeNo().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getGetNoticeNo());
			}
			if(this.getFeeOperationType() == null || this.getFeeOperationType().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getFeeOperationType());
			}
			if(this.getSubFeeOperationType() == null || this.getSubFeeOperationType().equals("null")) {
				pstmt.setNull(39, 12);
			} else {
				pstmt.setString(39, this.getSubFeeOperationType());
			}
			if(this.getFeeFinaType() == null || this.getFeeFinaType().equals("null")) {
				pstmt.setNull(40, 12);
			} else {
				pstmt.setString(40, this.getFeeFinaType());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(41, 12);
			} else {
				pstmt.setString(41, this.getPolNo());
			}
			if(this.getDutyCode() == null || this.getDutyCode().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
			...
			if(this.getGetDutyKind() == null || this.getGetDutyKind().equals("null")) {
				...
				pstmt.setString(43, this.getGetDutyKind());
			}
			if(this.getGetDutyCode() == null || this.getGetDutyCode().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
				pstmt.setString(44, this.getGetDutyCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LJSGetClaim VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGetNoticeNo() == null || this.getGetNoticeNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGetNoticeNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJSGetClaim WHERE  GetNoticeNo = ? AND FeeOperationType = ? AND SubFeeOperationType = ? AND FeeFinaType = ? AND PolNo = ? AND DutyCode = ? AND GetDutyKind = ? AND GetDutyCode = ?", 
	...
	public LJSGetClaimSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJSGetClaim");
			LJSGetClaimSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJSGetClaimSchema s1 = new LJSGetClaimSchema();
				s1.setSchema(rs,i);
				aLJSGetClaimSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJSGetClaimSet;
	}

	public LJSGetClaimSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSGetClaimSet aLJSGetClaimSet = new LJSGetClaimSet();
	...
	public LJSGetClaimSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSGetClaimSet aLJSGetClaimSet = new LJSGetClaimSet();
	...
	public LJSGetClaimSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSGetClaimSet aLJSGetClaimSet = new LJSGetClaimSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJSGetClaim " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJSGetClaimSet
 */
public LJSGetClaimSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLJSGetClaimSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LJSGetClaimSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJSGetClaimSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

...
/*
 * <p>ClassName: LJPremMatchConfirmDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LJPremMatchConfirmDB extends LJPremMatchConfirmSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LJPremMatchConfirmDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJPremMatchConfirmSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJPremMatchConfirmDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getMatchSerialNo() == null || this.getMatchSerialNo().equals("null")) {
				...
				pstmt.setString(23, this.getMatchSerialNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJPremMatchConfirm WHERE  MatchSerialNo = ?", 
	...
	public LJPremMatchConfirmSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJPremMatchConfirmSet aLJPremMatchConfirmSet = new LJPremMatchConfirmSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJPremMatchConfirm");
			LJPremMatchConfirmSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJPremMatchConfirmSchema s1 = new LJPremMatchConfirmSchema();
				s1.setSchema(rs,i);
				aLJPremMatchConfirmSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJPremMatchConfirmSet;
	}

	public LJPremMatchConfirmSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJPremMatchConfirmSet aLJPremMatchConfirmSet = new LJPremMatchConfirmSet();
	...
	public LJPremMatchConfirmSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJPremMatchConfirmSet aLJPremMatchConfirmSet = new LJPremMatchConfirmSet();
	...
	public LJPremMatchConfirmSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJPremMatchConfirmSet aLJPremMatchConfirmSet = new LJPremMatchConfirmSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJPremMatchConfirm " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJPremMatchConfirmSet
 */
public LJPremMatchConfirmSet getData()
{
    int tCount = 0;
    LJPremMatchConfirmSet tLJPremMatchConfirmSet = new LJPremMatchConfirmSet();
    LJPremMatchConfirmSchema tLJPremMatchConfirmSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJPremMatchConfirmDB";
    ...
    {
        tCount = 1;
        tLJPremMatchConfirmSchema = new LJPremMatchConfirmSchema();
        tLJPremMatchConfirmSchema.setSchema(mResultSet, 1);
        tLJPremMatchConfirmSet.add(tLJPremMatchConfirmSchema);
...
public boolean closeData()
{
	...
	public LJPremMatchConfirmSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LJPremMatchConfirmSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

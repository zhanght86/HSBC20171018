...
/*
 * <p>ClassName: LOPRTManager2DB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统lis5模型
 */
public class LOPRTManager2DB extends LOPRTManager2Schema
{
private static Logger logger = Logger.getLogger(LOPRTManager2DB.class);
	...
	**/
	...
	// @Constructor
	public LOPRTManager2DB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LOPRTManager2Schema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOPRTManager2DB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPrtSeq() == null || this.getPrtSeq().equals("null")) {
				...
				pstmt.setString(23, this.getPrtSeq());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOPRTManager2 WHERE  PrtSeq = ?", 
	...
	public LOPRTManager2Set query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOPRTManager2");
			LOPRTManager2Schema aSchema = this.getSchema();
			...
			{
				i++;
				LOPRTManager2Schema s1 = new LOPRTManager2Schema();
				s1.setSchema(rs,i);
				aLOPRTManager2Set.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOPRTManager2Set;

	}

	public LOPRTManager2Set executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOPRTManager2Set aLOPRTManager2Set = new LOPRTManager2Set();
	...
	public LOPRTManager2Set query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LOPRTManager2Set aLOPRTManager2Set = new LOPRTManager2Set();
	...
	public LOPRTManager2Set executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOPRTManager2Set aLOPRTManager2Set = new LOPRTManager2Set();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOPRTManager2 " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOPRTManager2Set
 */
public LOPRTManager2Set getData()
{
    int tCount = 0;
    LOPRTManager2Set tLOPRTManager2Set = new LOPRTManager2Set();
    LOPRTManager2Schema tLOPRTManager2Schema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOPRTManager2DB";
    ...
    {
        tCount = 1;
        tLOPRTManager2Schema = new LOPRTManager2Schema();
        tLOPRTManager2Schema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LOPRTManager2Set executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOPRTManager2Set executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

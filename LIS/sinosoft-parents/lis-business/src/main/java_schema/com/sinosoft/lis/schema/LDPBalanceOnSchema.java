...
import com.sinosoft.lis.db.LDPBalanceOnDB;

/*
 * <p>ClassName: LDPBalanceOnSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LDPBalanceOnSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LDPBalanceOnSchema.class);
	// @Field
	/** 团体保单号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LDPBalanceOnSchema cloned = (LDPBalanceOnSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LDPBalanceOnSchema 对象给 Schema 赋值
	* @param: aLDPBalanceOnSchema LDPBalanceOnSchema
	**/
	public void setSchema(LDPBalanceOnSchema aLDPBalanceOnSchema)
	{
		this.GrpContNo = aLDPBalanceOnSchema.getGrpContNo();
		this.GrpPropNo = aLDPBalanceOnSchema.getGrpPropNo();
		this.BalanceOnDate = fDate.getDate( aLDPBalanceOnSchema.getBalanceOnDate());
		this.BalanceOnOperator = aLDPBalanceOnSchema.getBalanceOnOperator();
		this.BalanceOnState = aLDPBalanceOnSchema.getBalanceOnState();
		this.BalancePeriod = aLDPBalanceOnSchema.getBalancePeriod();
		this.BalanceSumLimit = aLDPBalanceOnSchema.getBalanceSumLimit();
		this.BalanceSum = aLDPBalanceOnSchema.getBalanceSum();
		this.BalanceType = aLDPBalanceOnSchema.getBalanceType();
		this.LastBalanceDate = fDate.getDate( aLDPBalanceOnSchema.getLastBalanceDate());
		this.LastBalanceTime = aLDPBalanceOnSchema.getLastBalanceTime();
		this.NextBalanceDate = fDate.getDate( aLDPBalanceOnSchema.getNextBalanceDate());
		this.GracePeriod = aLDPBalanceOnSchema.getGracePeriod();
		this.ManageCom = aLDPBalanceOnSchema.getManageCom();
		this.ComCode = aLDPBalanceOnSchema.getComCode();
		this.MakeOperator = aLDPBalanceOnSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLDPBalanceOnSchema.getMakeDate());
		this.MakeTime = aLDPBalanceOnSchema.getMakeTime();
		this.ModifyOperator = aLDPBalanceOnSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLDPBalanceOnSchema.getModifyDate());
		this.ModifyTime = aLDPBalanceOnSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LDPBalanceOn表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LDPBalanceOnSchema";
	...
	public LDPBalanceOnSchema getSchema()
	{
		LDPBalanceOnSchema aLDPBalanceOnSchema = new LDPBalanceOnSchema();
		aLDPBalanceOnSchema.setSchema(this);
		return aLDPBalanceOnSchema;
	}

	public LDPBalanceOnDB getDB()
	{
		LDPBalanceOnDB aDBOper = new LDPBalanceOnDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDPBalanceOn描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDPBalanceOn>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			BalanceOnDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3,SysConst.PACKAGESPILTER));
			BalanceOnOperator = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			BalanceOnState = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			BalancePeriod = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			BalanceSumLimit = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,7,SysConst.PACKAGESPILTER))).doubleValue();
			BalanceSum = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,8,SysConst.PACKAGESPILTER))).doubleValue();
			BalanceType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			LastBalanceDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10,SysConst.PACKAGESPILTER));
			LastBalanceTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			NextBalanceDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12,SysConst.PACKAGESPILTER));
			GracePeriod = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LDPBalanceOnSchema other = (LDPBalanceOnSchema)otherObject;
		return

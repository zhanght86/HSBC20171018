...
/*
 * <p>ClassName: LPGrpAddressDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPGrpAddressDB extends LPGrpAddressSchema
{
private static Logger logger = Logger.getLogger(LPGrpAddressDB.class);
	...
	**/
	...
	// @Constructor
	public LPGrpAddressDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPGrpAddressSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPGrpAddressDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getGrpAddress() == null || this.getGrpAddress().equals("null")) {
				...
				pstmt.setString(5, this.getGrpAddress());
			}
			if(this.getGrpZipCode() == null || this.getGrpZipCode().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getGrpZipCode());
			}
			...
			if(this.getPhone1() == null || this.getPhone1().equals("null")) {
				...
				pstmt.setString(10, this.getPhone1());
			}
			if(this.getE_Mail1() == null || this.getE_Mail1().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getE_Mail1());
			}
			...
			if(this.getMobilePhone1() == null || this.getMobilePhone1().equals("null")) {
				...
				pstmt.setString(24, this.getMobilePhone1());
			}
			if(this.getMobilePhone2() == null || this.getMobilePhone2().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
				pstmt.setString(25, this.getMobilePhone2());
			}
			// set where condition
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(26, 12);
			...
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				...
				pstmt.setString(28, this.getCustomerNo());
			}
			if(this.getAddressNo() == null || this.getAddressNo().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getAddressNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPGrpAddress WHERE  EdorNo = ? AND EdorType = ? AND CustomerNo = ? AND AddressNo = ?", 
	...
	public LPGrpAddressSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGrpAddressSet aLPGrpAddressSet = new LPGrpAddressSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPGrpAddress");
			LPGrpAddressSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPGrpAddressSchema s1 = new LPGrpAddressSchema();
				s1.setSchema(rs,i);
				aLPGrpAddressSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPGrpAddressSet;

	}

	public LPGrpAddressSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGrpAddressSet aLPGrpAddressSet = new LPGrpAddressSet();
	...
	public LPGrpAddressSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGrpAddressSet aLPGrpAddressSet = new LPGrpAddressSet();
	...
	public LPGrpAddressSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGrpAddressSet aLPGrpAddressSet = new LPGrpAddressSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPGrpAddress " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPGrpAddressSet
 */
public LPGrpAddressSet getData()
{
    int tCount = 0;
    LPGrpAddressSet tLPGrpAddressSet = new LPGrpAddressSet();
    LPGrpAddressSchema tLPGrpAddressSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPGrpAddressDB";
    ...
    {
        tCount = 1;
        tLPGrpAddressSchema = new LPGrpAddressSchema();
        tLPGrpAddressSchema.setSchema(mResultSet, 1);
        tLPGrpAddressSet.add(tLPGrpAddressSchema);
...
public boolean closeData()
{
	...
	public LPGrpAddressSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPGrpAddressSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

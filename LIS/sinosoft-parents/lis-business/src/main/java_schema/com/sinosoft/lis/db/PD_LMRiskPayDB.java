...
/*
 * <p>ClassName: PD_LMRiskPayDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 产品定义平台_PDM
 */
public class PD_LMRiskPayDB extends PD_LMRiskPaySchema
{
private static Logger logger = Logger.getLogger(PD_LMRiskPayDB.class);
	...
	**/
	...
	// @Constructor
	public PD_LMRiskPayDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		PD_LMRiskPaySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "PD_LMRiskPayDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getGraceCalCode() == null || this.getGraceCalCode().equals("null")) {
				...
				pstmt.setString(26, this.getGraceCalCode());
			}
			if(this.getOverdueDeal() == null || this.getOverdueDeal().equals("null")) {
				pstmt.setNull(27, 1);
			} else {
				pstmt.setString(27, this.getOverdueDeal());
			}
			// set where condition
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(28, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM PD_LMRiskPay WHERE  RiskCode = ?", 
	...
	public PD_LMRiskPaySet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		PD_LMRiskPaySet aPD_LMRiskPaySet = new PD_LMRiskPaySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("PD_LMRiskPay");
			PD_LMRiskPaySchema aSchema = this.getSchema();
			...
			{
				i++;
				PD_LMRiskPaySchema s1 = new PD_LMRiskPaySchema();
				s1.setSchema(rs,i);
				aPD_LMRiskPaySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aPD_LMRiskPaySet;

	}

	public PD_LMRiskPaySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskPaySet aPD_LMRiskPaySet = new PD_LMRiskPaySet();
	...
	public PD_LMRiskPaySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		PD_LMRiskPaySet aPD_LMRiskPaySet = new PD_LMRiskPaySet();
	...
	public PD_LMRiskPaySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskPaySet aPD_LMRiskPaySet = new PD_LMRiskPaySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update PD_LMRiskPay " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return PD_LMRiskPaySet
 */
public PD_LMRiskPaySet getData()
{
    int tCount = 0;
    PD_LMRiskPaySet tPD_LMRiskPaySet = new PD_LMRiskPaySet();
    PD_LMRiskPaySchema tPD_LMRiskPaySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "PD_LMRiskPayDB";
    ...
    {
        tCount = 1;
        tPD_LMRiskPaySchema = new PD_LMRiskPaySchema();
        tPD_LMRiskPaySchema.setSchema(mResultSet, 1);
        tPD_LMRiskPaySet.add(tPD_LMRiskPaySchema);
...
public boolean closeData()
{
	...
	public PD_LMRiskPaySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public PD_LMRiskPaySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

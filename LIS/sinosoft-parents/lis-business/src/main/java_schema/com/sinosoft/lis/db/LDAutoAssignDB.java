...
/*
 * <p>ClassName: LDAutoAssignDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 自动分配
 */
public class LDAutoAssignDB extends LDAutoAssignSchema
{
private static Logger logger = Logger.getLogger(LDAutoAssignDB.class);
	...
	**/
	...
	// @Constructor
	public LDAutoAssignDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDAutoAssignSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LDAutoAssignDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setInt(8, this.getActuallyAmount());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDAutoAssign WHERE  TaskNo = ? AND Activityid = ? AND AssignNo = ?", 
	...
	public LDAutoAssignSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LDAutoAssignSet aLDAutoAssignSet = new LDAutoAssignSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDAutoAssign");
			LDAutoAssignSchema aSchema = this.getSchema();
			...
			{
				i++;
				LDAutoAssignSchema s1 = new LDAutoAssignSchema();
				s1.setSchema(rs,i);
				aLDAutoAssignSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLDAutoAssignSet;

	}

	public LDAutoAssignSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDAutoAssignSet aLDAutoAssignSet = new LDAutoAssignSet();
	...
	public LDAutoAssignSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LDAutoAssignSet aLDAutoAssignSet = new LDAutoAssignSet();
	...
	public LDAutoAssignSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDAutoAssignSet aLDAutoAssignSet = new LDAutoAssignSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDAutoAssign " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDAutoAssignSet
 */
public LDAutoAssignSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLDAutoAssignSchema.setSchema(mResultSet, 1);
        tLDAutoAssignSet.add(tLDAutoAssignSchema);
...
public boolean closeData()
{
	...
	public LDAutoAssignSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDAutoAssignSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

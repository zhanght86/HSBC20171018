...
{
	private static Logger logger = Logger.getLogger(LJAGetOtherDB.class);
	// @Field
	...
	**/
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOperState() == null || this.getOperState().equals("null")) {
				...
				pstmt.setString(25, this.getOperState());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
			...
			pstmt.setDouble(27, this.getNetAmount());
			pstmt.setDouble(28, this.getTaxAmount());
			pstmt.setDouble(29, this.getTax());
			// set where condition
			if(this.getActuGetNo() == null || this.getActuGetNo().equals("null")) {
				pstmt.setNull(30, 12);
			} else {
				pstmt.setString(30, this.getActuGetNo());
			}
			if(this.getOtherNo() == null || this.getOtherNo().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
			...
			if(this.getOtherNoType() == null || this.getOtherNoType().equals("null")) {
				...
				pstmt.setString(32, StrTool.space1(this.getOtherNoType(), 1));
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJAGetOther WHERE  ActuGetNo = ? AND OtherNo = ? AND OtherNoType = ? AND Currency = ?", 
...
public boolean closeData()
{
	...
	public LJAGetOtherSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJAGetOtherSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

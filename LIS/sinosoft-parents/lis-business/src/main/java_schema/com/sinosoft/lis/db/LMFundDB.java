...
/*
 * <p>ClassName: LMFundDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 投连改造
 */
public class LMFundDB extends LMFundSchema
{
private static Logger logger = Logger.getLogger(LMFundDB.class);
	...
	**/
	...
	// @Constructor
	public LMFundDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LMFundSchema tSchema = this.getSchema();
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LMFund WHERE  FundNo = ?", 
	...
	public LMFundSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LMFundSet aLMFundSet = new LMFundSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LMFund");
			LMFundSchema aSchema = this.getSchema();
			...
			{
				i++;
				LMFundSchema s1 = new LMFundSchema();
				s1.setSchema(rs,i);
				aLMFundSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LMFundSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMFundSet aLMFundSet = new LMFundSet();
	...
	public LMFundSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LMFundSet aLMFundSet = new LMFundSet();
	...
	public LMFundSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMFundSet aLMFundSet = new LMFundSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LMFund " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LMFundSet
 */
public LMFundSet getData()
{
    int tCount = 0;
    LMFundSet tLMFundSet = new LMFundSet();
    LMFundSchema tLMFundSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LMFundDB";
    ...
    {
        tCount = 1;
        tLMFundSchema = new LMFundSchema();
        tLMFundSchema.setSchema(mResultSet, 1);
        tLMFundSet.add(tLMFundSchema);
...
public boolean closeData()
{
	...
	public LMFundSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LMFundSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: RIRecordTraceTempDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保系统模型
 */
public class RIRecordTraceTempDB extends RIRecordTraceTempSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public RIRecordTraceTempDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "RIRecordTraceTemp" );
		mflag = true;
	}

	public RIRecordTraceTempDB()
	{
		con = null;
		db = new DBOper( "RIRecordTraceTemp" );
		mflag = false;
	}
	...
	{
		RIRecordTraceTempSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "RIRecordTraceTempDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM RIRecordTraceTemp WHERE  SerialNo = ?");
		...
		SQLString sqlObj = new SQLString("RIRecordTraceTemp");
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM RIRecordTraceTemp WHERE  SerialNo = ?", 
	...
	public RIRecordTraceTempSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIRecordTraceTempSet aRIRecordTraceTempSet = new RIRecordTraceTempSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("RIRecordTraceTemp");
			RIRecordTraceTempSchema aSchema = this.getSchema();
			...
			{
				i++;
				RIRecordTraceTempSchema s1 = new RIRecordTraceTempSchema();
				s1.setSchema(rs,i);
				aRIRecordTraceTempSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aRIRecordTraceTempSet;
	}

	public RIRecordTraceTempSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIRecordTraceTempSet aRIRecordTraceTempSet = new RIRecordTraceTempSet();
	...
	public RIRecordTraceTempSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIRecordTraceTempSet aRIRecordTraceTempSet = new RIRecordTraceTempSet();
	...
	public RIRecordTraceTempSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIRecordTraceTempSet aRIRecordTraceTempSet = new RIRecordTraceTempSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update RIRecordTraceTemp " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return RIRecordTraceTempSet
 */
public RIRecordTraceTempSet getData()
{
    int tCount = 0;
    RIRecordTraceTempSet tRIRecordTraceTempSet = new RIRecordTraceTempSet();
    RIRecordTraceTempSchema tRIRecordTraceTempSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "RIRecordTraceTempDB";
    ...
    try
    {
        ...
        tRIRecordTraceTempSchema.setSchema(mResultSet, 1);

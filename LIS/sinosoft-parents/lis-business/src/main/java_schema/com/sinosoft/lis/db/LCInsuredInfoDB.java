...
/*
 * <p>ClassName: LCInsuredInfoDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LCInsuredInfoDB extends LCInsuredInfoSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCInsuredInfoDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCInsuredInfoSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCInsuredInfoDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getInsuredGenderCode() == null || this.getInsuredGenderCode().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getInsuredGenderCode());
			}
			if(this.getInsuredGenderName() == null || this.getInsuredGenderName().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getInsuredGenderName());
			}
			if(this.getInsuredBirthday() == null || this.getInsuredBirthday().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getInsuredBirthday());
			}
			if(this.getIDTypeCode() == null || this.getIDTypeCode().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getIDTypeCode());
			}
			if(this.getIDTypeName() == null || this.getIDTypeName().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getIDTypeName());
			}
			...
			if(this.getDeathDate() == null || this.getDeathDate().equals("null")) {
				...
				pstmt.setString(13, this.getDeathDate());
			}
			...
			if(this.getBatchNo() == null || this.getBatchNo().equals("null")) {
				...
				pstmt.setString(24, this.getBatchNo());
			}
			pstmt.setInt(25, this.getSerialNo());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCInsuredInfo WHERE  BatchNo = ? AND SerialNo = ?", 
	...
	public LCInsuredInfoSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCInsuredInfoSet aLCInsuredInfoSet = new LCInsuredInfoSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCInsuredInfo");
			LCInsuredInfoSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCInsuredInfoSchema s1 = new LCInsuredInfoSchema();
				s1.setSchema(rs,i);
				aLCInsuredInfoSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCInsuredInfoSet;
	}

	public LCInsuredInfoSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCInsuredInfoSet aLCInsuredInfoSet = new LCInsuredInfoSet();
	...
	public LCInsuredInfoSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCInsuredInfoSet aLCInsuredInfoSet = new LCInsuredInfoSet();
	...
	public LCInsuredInfoSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCInsuredInfoSet aLCInsuredInfoSet = new LCInsuredInfoSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCInsuredInfo " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCInsuredInfoSet
 */
public LCInsuredInfoSet getData()
{
    int tCount = 0;
    LCInsuredInfoSet tLCInsuredInfoSet = new LCInsuredInfoSet();
    LCInsuredInfoSchema tLCInsuredInfoSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCInsuredInfoDB";
    ...
    {
        tCount = 1;
        tLCInsuredInfoSchema = new LCInsuredInfoSchema();
        tLCInsuredInfoSchema.setSchema(mResultSet, 1);
        tLCInsuredInfoSet.add(tLCInsuredInfoSchema);
...
public boolean closeData()
{
	...
	public LCInsuredInfoSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCInsuredInfoSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LPCGrpSpecDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPCGrpSpecDB extends LPCGrpSpecSchema
{
private static Logger logger = Logger.getLogger(LPCGrpSpecDB.class);
	...
	**/
	...
	// @Constructor
	public LPCGrpSpecDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPCGrpSpecSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPCGrpSpecDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getProposalGrpContNo() == null || this.getProposalGrpContNo().equals("null")) {
				...
				pstmt.setString(20, this.getProposalGrpContNo());
			}
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPCGrpSpec WHERE  EdorNo = ? AND EdorType = ? AND ProposalGrpContNo = ? AND SerialNo = ?", 
	...
	public LPCGrpSpecSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPCGrpSpecSet aLPCGrpSpecSet = new LPCGrpSpecSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPCGrpSpec");
			LPCGrpSpecSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPCGrpSpecSchema s1 = new LPCGrpSpecSchema();
				s1.setSchema(rs,i);
				aLPCGrpSpecSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPCGrpSpecSet;

	}

	public LPCGrpSpecSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPCGrpSpecSet aLPCGrpSpecSet = new LPCGrpSpecSet();
	...
	public LPCGrpSpecSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPCGrpSpecSet aLPCGrpSpecSet = new LPCGrpSpecSet();
	...
	public LPCGrpSpecSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPCGrpSpecSet aLPCGrpSpecSet = new LPCGrpSpecSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPCGrpSpec " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPCGrpSpecSet
 */
public LPCGrpSpecSet getData()
{
    int tCount = 0;
    LPCGrpSpecSet tLPCGrpSpecSet = new LPCGrpSpecSet();
    LPCGrpSpecSchema tLPCGrpSpecSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPCGrpSpecDB";
    ...
    {
        tCount = 1;
        tLPCGrpSpecSchema = new LPCGrpSpecSchema();
        tLPCGrpSpecSchema.setSchema(mResultSet, 1);
        tLPCGrpSpecSet.add(tLPCGrpSpecSchema);
...
public boolean closeData()
{
	...
	public LPCGrpSpecSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPCGrpSpecSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

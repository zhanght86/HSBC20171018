...
/*
 * <p>ClassName: LCAirPolSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 短期险平台接口表
 */
public class LCAirPolSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LCAirPolSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LCAirPolSchema cloned = (LCAirPolSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LCAirPolSchema 对象给 Schema 赋值
	* @param: aLCAirPolSchema LCAirPolSchema
	**/
	public void setSchema(LCAirPolSchema aLCAirPolSchema)
	{
		this.PolNo = aLCAirPolSchema.getPolNo();
		this.CertifyCode = aLCAirPolSchema.getCertifyCode();
		this.ManageCom = aLCAirPolSchema.getManageCom();
		this.AgentCom = aLCAirPolSchema.getAgentCom();
		this.Password = aLCAirPolSchema.getPassword();
		this.InsuredName = aLCAirPolSchema.getInsuredName();
		this.InsuredIDType = aLCAirPolSchema.getInsuredIDType();
		this.InsuredIDNo = aLCAirPolSchema.getInsuredIDNo();
		this.RelationToInsured = aLCAirPolSchema.getRelationToInsured();
		this.BnfAddress = aLCAirPolSchema.getBnfAddress();
		this.BnfName = aLCAirPolSchema.getBnfName();
		this.BnfZipCode = aLCAirPolSchema.getBnfZipCode();
		this.BnfPhone = aLCAirPolSchema.getBnfPhone();
		this.Mult = aLCAirPolSchema.getMult();
		this.Prem = aLCAirPolSchema.getPrem();
		this.Amnt = aLCAirPolSchema.getAmnt();
		this.PolState = aLCAirPolSchema.getPolState();
		this.EmplaneDate = fDate.getDate( aLCAirPolSchema.getEmplaneDate());
		this.FlightNo = aLCAirPolSchema.getFlightNo();
		this.AgentCode = aLCAirPolSchema.getAgentCode();
		this.AgentGroup = aLCAirPolSchema.getAgentGroup();
		this.InsuYearFlag = aLCAirPolSchema.getInsuYearFlag();
		this.InsuYear = aLCAirPolSchema.getInsuYear();
		this.EndDate = fDate.getDate( aLCAirPolSchema.getEndDate());
		this.CValiDate = fDate.getDate( aLCAirPolSchema.getCValiDate());
		this.Operator = aLCAirPolSchema.getOperator();
		this.MakeDate = fDate.getDate( aLCAirPolSchema.getMakeDate());
		this.MakeTime = aLCAirPolSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLCAirPolSchema.getModifyDate());
		this.ModifyTime = aLCAirPolSchema.getModifyTime();
		this.TSignDate = fDate.getDate( aLCAirPolSchema.getTSignDate());
		this.RiskCode = aLCAirPolSchema.getRiskCode();
		this.State = aLCAirPolSchema.getState();
		this.RiskType = aLCAirPolSchema.getRiskType();
		this.Destination = aLCAirPolSchema.getDestination();
		this.Reason = aLCAirPolSchema.getReason();
		this.InsuredSex = aLCAirPolSchema.getInsuredSex();
		this.InsuredBirthday = fDate.getDate( aLCAirPolSchema.getInsuredBirthday());
		this.OccupationType = aLCAirPolSchema.getOccupationType();
		this.PrtNo = aLCAirPolSchema.getPrtNo();
		this.StandByFlag1 = aLCAirPolSchema.getStandByFlag1();
		this.StandByFlag2 = aLCAirPolSchema.getStandByFlag2();
		this.StandByFlag3 = aLCAirPolSchema.getStandByFlag3();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LCAirPol表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LCAirPolSchema";
	...
	public LCAirPolSchema getSchema()
	{
		LCAirPolSchema aLCAirPolSchema = new LCAirPolSchema();
		aLCAirPolSchema.setSchema(this);
		return aLCAirPolSchema;
	}

	public LCAirPolDB getDB()
	{
		LCAirPolDB aDBOper = new LCAirPolDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCAirPol描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCAirPol>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LCAirPolSchema other = (LCAirPolSchema)otherObject;
		return

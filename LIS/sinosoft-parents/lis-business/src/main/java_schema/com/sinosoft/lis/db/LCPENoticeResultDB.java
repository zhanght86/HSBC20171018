...
/*
 * <p>ClassName: LCPENoticeResultDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCPENoticeResultDB extends LCPENoticeResultSchema
{
private static Logger logger = Logger.getLogger(LCPENoticeResultDB.class);
	...
	**/
	...
	// @Constructor
	public LCPENoticeResultDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCPENoticeResultSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCPENoticeResultDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPENoticeResult WHERE  ProposalContNo = ? AND PrtSeq = ? AND SerialNo = ?", 
	...
	public LCPENoticeResultSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPENoticeResultSet aLCPENoticeResultSet = new LCPENoticeResultSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPENoticeResult");
			LCPENoticeResultSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCPENoticeResultSchema s1 = new LCPENoticeResultSchema();
				s1.setSchema(rs,i);
				aLCPENoticeResultSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCPENoticeResultSet;

	}

	public LCPENoticeResultSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPENoticeResultSet aLCPENoticeResultSet = new LCPENoticeResultSet();
	...
	public LCPENoticeResultSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPENoticeResultSet aLCPENoticeResultSet = new LCPENoticeResultSet();
	...
	public LCPENoticeResultSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPENoticeResultSet aLCPENoticeResultSet = new LCPENoticeResultSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPENoticeResult " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCPENoticeResultSet
 */
public LCPENoticeResultSet getData()
{
    int tCount = 0;
    LCPENoticeResultSet tLCPENoticeResultSet = new LCPENoticeResultSet();
    LCPENoticeResultSchema tLCPENoticeResultSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCPENoticeResultDB";
    ...
    try
    {
        ...
        tLCPENoticeResultSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCPENoticeResultSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPENoticeResultSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

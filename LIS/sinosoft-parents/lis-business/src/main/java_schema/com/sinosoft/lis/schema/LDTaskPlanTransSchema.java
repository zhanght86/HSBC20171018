...
/*
 * <p>ClassName: LDTaskPlanTransSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 任务计划转移
 */
public class LDTaskPlanTransSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LDTaskPlanTransSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LDTaskPlanTransSchema cloned = (LDTaskPlanTransSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LDTaskPlanTransSchema 对象给 Schema 赋值
	* @param: aLDTaskPlanTransSchema LDTaskPlanTransSchema
	**/
	public void setSchema(LDTaskPlanTransSchema aLDTaskPlanTransSchema)
	{
		this.TaskPlanCode = aLDTaskPlanTransSchema.getTaskPlanCode();
		this.OldServerIP = aLDTaskPlanTransSchema.getOldServerIP();
		this.OldServerPort = aLDTaskPlanTransSchema.getOldServerPort();
		this.NewServerIP = aLDTaskPlanTransSchema.getNewServerIP();
		this.NewServerPort = aLDTaskPlanTransSchema.getNewServerPort();
		this.State = aLDTaskPlanTransSchema.getState();
		this.MakeDate = fDate.getDate( aLDTaskPlanTransSchema.getMakeDate());
		this.MakeTime = aLDTaskPlanTransSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLDTaskPlanTransSchema.getModifyDate());
		this.ModifyTime = aLDTaskPlanTransSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LDTaskPlanTrans表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LDTaskPlanTransSchema";
	...
	public LDTaskPlanTransSchema getSchema()
	{
		LDTaskPlanTransSchema aLDTaskPlanTransSchema = new LDTaskPlanTransSchema();
		aLDTaskPlanTransSchema.setSchema(this);
		return aLDTaskPlanTransSchema;
	}

	public LDTaskPlanTransDB getDB()
	{
		LDTaskPlanTransDB aDBOper = new LDTaskPlanTransDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDTaskPlanTrans>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			OldServerIP = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 2, SysConst.PACKAGESPILTER );
			OldServerPort = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			NewServerIP = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			NewServerPort = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LDTaskPlanTransSchema other = (LDTaskPlanTransSchema)otherObject;
		return

...
import com.sinosoft.lis.db.LLBExemptDB;

/*
 * <p>ClassName: LLBExemptSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLBExemptSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLBExemptSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLBExemptSchema cloned = (LLBExemptSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LLBExemptSchema 对象给 Schema 赋值
	* @param: aLLBExemptSchema LLBExemptSchema
	**/
	public void setSchema(LLBExemptSchema aLLBExemptSchema)
	{
		this.BackNo = aLLBExemptSchema.getBackNo();
		this.ClmNo = aLLBExemptSchema.getClmNo();
		this.GrpContNo = aLLBExemptSchema.getGrpContNo();
		this.ContNo = aLLBExemptSchema.getContNo();
		this.PolNo = aLLBExemptSchema.getPolNo();
		this.DutyCode = aLLBExemptSchema.getDutyCode();
		this.PayPlanCode = aLLBExemptSchema.getPayPlanCode();
		this.PayPlanType = aLLBExemptSchema.getPayPlanType();
		this.AppntType = aLLBExemptSchema.getAppntType();
		this.AppntNo = aLLBExemptSchema.getAppntNo();
		this.UrgePayFlag = aLLBExemptSchema.getUrgePayFlag();
		this.NeedAcc = aLLBExemptSchema.getNeedAcc();
		this.PayTimes = aLLBExemptSchema.getPayTimes();
		this.Rate = aLLBExemptSchema.getRate();
		this.PayStartDate = fDate.getDate( aLLBExemptSchema.getPayStartDate());
		this.PayEndDate = fDate.getDate( aLLBExemptSchema.getPayEndDate());
		this.PaytoDate = fDate.getDate( aLLBExemptSchema.getPaytoDate());
		this.PayIntv = aLLBExemptSchema.getPayIntv();
		this.StandPrem = aLLBExemptSchema.getStandPrem();
		this.Prem = aLLBExemptSchema.getPrem();
		this.SumPrem = aLLBExemptSchema.getSumPrem();
		this.SuppRiskScore = aLLBExemptSchema.getSuppRiskScore();
		this.FreeFlag = aLLBExemptSchema.getFreeFlag();
		this.FreeRate = aLLBExemptSchema.getFreeRate();
		this.FreeStartDate = fDate.getDate( aLLBExemptSchema.getFreeStartDate());
		this.FreeEndDate = fDate.getDate( aLLBExemptSchema.getFreeEndDate());
		this.State = aLLBExemptSchema.getState();
		this.ManageCom = aLLBExemptSchema.getManageCom();
		this.Operator = aLLBExemptSchema.getOperator();
		this.MakeDate = fDate.getDate( aLLBExemptSchema.getMakeDate());
		this.MakeTime = aLLBExemptSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLLBExemptSchema.getModifyDate());
		this.ModifyTime = aLLBExemptSchema.getModifyTime();
		this.AddFeeDirect = aLLBExemptSchema.getAddFeeDirect();
		this.ExemptPeriod = aLLBExemptSchema.getExemptPeriod();
		this.NBPolNo = aLLBExemptSchema.getNBPolNo();
		this.ExemptReason = aLLBExemptSchema.getExemptReason();
		this.ExemptDesc = aLLBExemptSchema.getExemptDesc();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLBExempt表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLBExemptSchema";
	...
	public LLBExemptSchema getSchema()
	{
		LLBExemptSchema aLLBExemptSchema = new LLBExemptSchema();
		aLLBExemptSchema.setSchema(this);
		return aLLBExemptSchema;
	}

	public LLBExemptDB getDB()
	{
		LLBExemptDB aDBOper = new LLBExemptDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLBExempt描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLBExempt>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ExemptPeriod = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 35, SysConst.PACKAGESPILTER );
			NBPolNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 36, SysConst.PACKAGESPILTER );
			ExemptReason = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 37, SysConst.PACKAGESPILTER );
			ExemptDesc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 38, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LLBExemptSchema other = (LLBExemptSchema)otherObject;
		return

...
/*
 * <p>ClassName: LZCardImportDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 单证管理
 */
public class LZCardImportDB extends LZCardImportSchema
{
private static Logger logger = Logger.getLogger(LZCardImportDB.class);
	...
	**/
	...
	// @Constructor
	public LZCardImportDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LZCardImportSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LZCardImportDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getEndNo() == null || this.getEndNo().equals("null")) {
				...
				pstmt.setString(3, this.getEndNo());
			}
			...
			if(this.getMakeTime() == null || this.getMakeTime().equals("null")) {
				...
				pstmt.setString(5, StrTool.space1(this.getMakeTime(), 8));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getApplyOperator() == null || this.getApplyOperator().equals("null")) {
				...
				pstmt.setString(17, this.getApplyOperator());
			}
			if(this.getReason() == null || this.getReason().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
				pstmt.setString(18, this.getReason());
			}
			// set where condition
			if(this.getCertifyCode() == null || this.getCertifyCode().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getCertifyCode());
			}
			if(this.getStartNo() == null || this.getStartNo().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getStartNo());
			}
			if(this.getEndNo() == null || this.getEndNo().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getEndNo());
			}
			if(this.getMakeDate() == null || this.getMakeDate().equals("null")) {
				pstmt.setNull(22, 91);
			} else {
			...
			if(this.getMakeTime() == null || this.getMakeTime().equals("null")) {
				...
				pstmt.setString(23, StrTool.space1(this.getMakeTime(), 8));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LZCardImport WHERE  CertifyCode = ? AND StartNo = ? AND EndNo = ? AND MakeDate = ? AND MakeTime = ?", 
	...
	public LZCardImportSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardImportSet aLZCardImportSet = new LZCardImportSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LZCardImport");
			LZCardImportSchema aSchema = this.getSchema();
			...
			{
				i++;
				LZCardImportSchema s1 = new LZCardImportSchema();
				s1.setSchema(rs,i);
				aLZCardImportSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLZCardImportSet;

	}

	public LZCardImportSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardImportSet aLZCardImportSet = new LZCardImportSet();
	...
	public LZCardImportSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardImportSet aLZCardImportSet = new LZCardImportSet();
	...
	public LZCardImportSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardImportSet aLZCardImportSet = new LZCardImportSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LZCardImport " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LZCardImportSet
 */
public LZCardImportSet getData()
{
    int tCount = 0;
    LZCardImportSet tLZCardImportSet = new LZCardImportSet();
    LZCardImportSchema tLZCardImportSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LZCardImportDB";
    ...
    try
    {
        ...
        tLZCardImportSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LZCardImportSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LZCardImportSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

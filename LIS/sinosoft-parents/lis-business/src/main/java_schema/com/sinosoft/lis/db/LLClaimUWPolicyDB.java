...
/*
 * <p>ClassName: LLClaimUWPolicyDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLClaimUWPolicyDB extends LLClaimUWPolicySchema
{
private static Logger logger = Logger.getLogger(LLClaimUWPolicyDB.class);
	...
	**/
	...
	// @Constructor
	public LLClaimUWPolicyDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLClaimUWPolicySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLClaimUWPolicyDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPolState() == null || this.getPolState().equals("null")) {
				...
				pstmt.setString(23, this.getPolState());
			}
			...
			if(this.getMngCom() == null || this.getMngCom().equals("null")) {
				...
				pstmt.setString(42, this.getMngCom());
			}
			...
			if(this.getBackNo() == null || this.getBackNo().equals("null")) {
				...
				pstmt.setString(48, this.getBackNo());
			}
			// set where condition
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(49, 12);
			...
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				...
				pstmt.setString(50, this.getCaseNo());
			}
			if(this.getCaseRelaNo() == null || this.getCaseRelaNo().equals("null")) {
				pstmt.setNull(51, 12);
			} else {
				pstmt.setString(51, this.getCaseRelaNo());
			}
			if(this.getClmUWNo() == null || this.getClmUWNo().equals("null")) {
				pstmt.setNull(52, 12);
			} else {
				pstmt.setString(52, this.getClmUWNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(53, 12);
			} else {
				pstmt.setString(53, this.getPolNo());
			}
			if(this.getGetDutyKind() == null || this.getGetDutyKind().equals("null")) {
				pstmt.setNull(54, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLClaimUWPolicy WHERE  ClmNo = ? AND CaseNo = ? AND CaseRelaNo = ? AND ClmUWNo = ? AND PolNo = ? AND GetDutyKind = ?", 
	...
	public LLClaimUWPolicySet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLClaimUWPolicySet aLLClaimUWPolicySet = new LLClaimUWPolicySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLClaimUWPolicy");
			LLClaimUWPolicySchema aSchema = this.getSchema();
			...
			{
				i++;
				LLClaimUWPolicySchema s1 = new LLClaimUWPolicySchema();
				s1.setSchema(rs,i);
				aLLClaimUWPolicySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLClaimUWPolicySet;

	}

	public LLClaimUWPolicySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLClaimUWPolicySet aLLClaimUWPolicySet = new LLClaimUWPolicySet();
	...
	public LLClaimUWPolicySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLClaimUWPolicySet aLLClaimUWPolicySet = new LLClaimUWPolicySet();
	...
	public LLClaimUWPolicySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLClaimUWPolicySet aLLClaimUWPolicySet = new LLClaimUWPolicySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLClaimUWPolicy " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLClaimUWPolicySet
 */
public LLClaimUWPolicySet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLLClaimUWPolicySchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLClaimUWPolicySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLClaimUWPolicySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

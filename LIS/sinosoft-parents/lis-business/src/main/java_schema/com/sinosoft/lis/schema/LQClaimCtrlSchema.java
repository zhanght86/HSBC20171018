...
import com.sinosoft.lis.db.LQClaimCtrlDB;

/*
 * <p>ClassName: LQClaimCtrlSchema </p>
 ...
 */
public class LQClaimCtrlSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LQClaimCtrlSchema.class);
	...
	public LQClaimCtrlSchema()
	{
		...
		PK = pk;
            ...
            public Object clone()
                    throws CloneNotSupportedException
            {
                LQClaimCtrlSchema cloned = (LQClaimCtrlSchema)super.clone();
	...
	/**
	* 使用另外一个 LQClaimCtrlSchema 对象给 Schema 赋值
	* @param: aLQClaimCtrlSchema LQClaimCtrlSchema
	**/
	public void setSchema(LQClaimCtrlSchema aLQClaimCtrlSchema)
	{
		this.AskPriceNo = aLQClaimCtrlSchema.getAskPriceNo();
		this.AskNo = aLQClaimCtrlSchema.getAskNo();
		this.ClaimCtrlCode = aLQClaimCtrlSchema.getClaimCtrlCode();
		this.ClaimCtrlName = aLQClaimCtrlSchema.getClaimCtrlName();
		this.ClaimCtrlType = aLQClaimCtrlSchema.getClaimCtrlType();
		this.ClaimEngineDesc = aLQClaimCtrlSchema.getClaimEngineDesc();
		this.PeriodFlag = aLQClaimCtrlSchema.getPeriodFlag();
		this.DefPeriodFlag = aLQClaimCtrlSchema.getDefPeriodFlag();
		this.FamilyFlag = aLQClaimCtrlSchema.getFamilyFlag();
		this.InsPeriodFlag = aLQClaimCtrlSchema.getInsPeriodFlag();
		this.ClmPeriodMAX = aLQClaimCtrlSchema.getClmPeriodMAX();
		this.ClmPeriodMAXFlag = aLQClaimCtrlSchema.getClmPeriodMAXFlag();
		this.ClmPeriodMAXCtrl = aLQClaimCtrlSchema.getClmPeriodMAXCtrl();
		this.ClmPeriodMIN = aLQClaimCtrlSchema.getClmPeriodMIN();
		this.ClmPeriodMINFlag = aLQClaimCtrlSchema.getClmPeriodMINFlag();
		this.ClmPeriodMINCtrl = aLQClaimCtrlSchema.getClmPeriodMINCtrl();
		this.CalCode = aLQClaimCtrlSchema.getCalCode();
		this.CalResultType = aLQClaimCtrlSchema.getCalResultType();
		this.DefaultValue = aLQClaimCtrlSchema.getDefaultValue();
		this.CalCtrlFlag = aLQClaimCtrlSchema.getCalCtrlFlag();
		this.FeeCalCode = aLQClaimCtrlSchema.getFeeCalCode();
		this.MakeDate = fDate.getDate( aLQClaimCtrlSchema.getMakeDate());
		this.MakeTime = aLQClaimCtrlSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLQClaimCtrlSchema.getModifyDate());
		this.ModifyTime = aLQClaimCtrlSchema.getModifyTime();
		this.Currency = aLQClaimCtrlSchema.getCurrency();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LQClaimCtrl表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LQClaimCtrlSchema";
	...
	public LQClaimCtrlSchema getSchema()
	{
		LQClaimCtrlSchema aLQClaimCtrlSchema = new LQClaimCtrlSchema();
		aLQClaimCtrlSchema.setSchema(this);
		return aLQClaimCtrlSchema;
	}

	public LQClaimCtrlDB getDB()
	{
		LQClaimCtrlDB aDBOper = new LQClaimCtrlDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLQClaimCtrl描述/A>表字段
...
strReturn.append(StrTool.cTrim(Currency));
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLQClaimCtrl>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LQClaimCtrlSchema other = (LQClaimCtrlSchema)otherObject;
		return

...
/*
 * <p>ClassName: LCPlanUWSubDBSet </p>
 * <p>Description: DB层多记录数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统未整理模型
 */
public class LCPlanUWSubDBSet extends LCPlanUWSubSet
{
private static Logger logger = Logger.getLogger(LCPlanUWSubDBSet.class);
	...
	**/
	...
	// @Constructor
	public LCPlanUWSubDBSet(Connection tConnection)
	{
		con = tConnection;
		db = new DBOper(con,"LCPlanUWSub");
		mflag = true;
	}

	public LCPlanUWSubDBSet()
	{
		db = new DBOper( "LCPlanUWSub" );
	}
	// @Method
	public boolean deleteSQL()
	{
		...
		else
		{
			...
			tError.moduleName = "LCPlanUWSubDBSet";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt = con.prepareStatement("DELETE FROM LCPlanUWSub WHERE  ProposalContNo = ? AND ContPlanCode = ? AND PlanType = ? AND UWNo = ? AND InsuredNo = ?");
			...
			if(this.get(i).getInsuredNo() == null || this.get(i).getInsuredNo().equals("null")) {
				...
				pstmt.setString(5, StrTool.space1(this.get(i).getInsuredNo(), 24));
			}

		// only for debug purpose
		SQLString sqlObj = new SQLString("LCPlanUWSub");
...
ex.printStackTrace();
	...
	public boolean update()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("UPDATE LCPlanUWSub SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , ContPlanCode = ? , PlanType = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , SugPassFlag = ? , SugUWIdea = ? , UpReport = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? WHERE  ProposalContNo = ? AND ContPlanCode = ? AND PlanType = ? AND UWNo = ? AND InsuredNo = ?");
            for (int i = 1; i <= tCount; i++)
            {
			...
			pstmt.setInt(45, this.get(i).getUWNo());
			if(this.get(i).getInsuredNo() == null || this.get(i).getInsuredNo().equals("null")) {
				pstmt.setString(46,null);
			} else {
				pstmt.setString(46, StrTool.space1(this.get(i).getInsuredNo(), 24));
			}
...
ex.printStackTrace();
	...
	public boolean insert()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("INSERT INTO LCPlanUWSub(GrpContNo ,ContNo ,ProposalContNo ,ContPlanCode ,PlanType ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,SugPassFlag ,SugUWIdea ,UpReport ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getGrpContNo() == null || this.get(i).getGrpContNo().equals("null")) {
				pstmt.setString(1,null);

...
/*
 * <p>ClassName: LCGrpBudgetDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 核保测算
 */
public class LCGrpBudgetDB extends LCGrpBudgetSchema
{
private static Logger logger = Logger.getLogger(LCGrpBudgetDB.class);
	...
	**/
	...
	// @Constructor
	public LCGrpBudgetDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCGrpBudgetSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCGrpBudgetDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOccupationType() == null || this.getOccupationType().equals("null")) {
				...
				pstmt.setString(8, this.getOccupationType());
			}
			...
			pstmt.setInt(11, this.getExpPeoples());
			pstmt.setDouble(12, this.getPrem());
			pstmt.setDouble(13, this.getAmnt());
			pstmt.setInt(14, this.getPayIntv());
			if(this.getCValiDate() == null || this.getCValiDate().equals("null")) {
				pstmt.setNull(15, 91);
			} else {
				pstmt.setDate(15, Date.valueOf(this.getCValiDate()));
			}
			...
			if(this.getBudgetFlag() == null || this.getBudgetFlag().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getBudgetFlag());
			}
			if(this.getBudgetResult() == null || this.getBudgetResult().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getBudgetResult());
			}
			...
			if(this.getBudgetNo() == null || this.getBudgetNo().equals("null")) {
				...
				pstmt.setString(28, this.getBudgetNo());
			}
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
			...
			if(this.getPayPlanCode() == null || this.getPayPlanCode().equals("null")) {
				...
				pstmt.setString(30, this.getPayPlanCode());
			}
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCGrpBudget WHERE  BudgetNo = ? AND GrpContNo = ? AND PayPlanCode = ? AND RiskCode = ?", 
	...
	public LCGrpBudgetSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCGrpBudgetSet aLCGrpBudgetSet = new LCGrpBudgetSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCGrpBudget");
			LCGrpBudgetSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCGrpBudgetSchema s1 = new LCGrpBudgetSchema();
				s1.setSchema(rs,i);
				aLCGrpBudgetSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCGrpBudgetSet;

	}

	public LCGrpBudgetSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpBudgetSet aLCGrpBudgetSet = new LCGrpBudgetSet();
	...
	public LCGrpBudgetSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCGrpBudgetSet aLCGrpBudgetSet = new LCGrpBudgetSet();
	...
	public LCGrpBudgetSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpBudgetSet aLCGrpBudgetSet = new LCGrpBudgetSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCGrpBudget " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCGrpBudgetSet
 */
public LCGrpBudgetSet getData()
{
    int tCount = 0;
    LCGrpBudgetSet tLCGrpBudgetSet = new LCGrpBudgetSet();
    LCGrpBudgetSchema tLCGrpBudgetSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCGrpBudgetDB";
    ...
    {
        tCount = 1;
        tLCGrpBudgetSchema = new LCGrpBudgetSchema();
        tLCGrpBudgetSchema.setSchema(mResultSet, 1);
        tLCGrpBudgetSet.add(tLCGrpBudgetSchema);
...
public boolean closeData()
{
	...
	public LCGrpBudgetSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCGrpBudgetSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: RIGUWErrorDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保系统模型
 */
public class RIGUWErrorDB extends RIGUWErrorSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public RIGUWErrorDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		RIGUWErrorSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "RIGUWErrorDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM RIGUWError WHERE  SerialNo = ?", 
	...
	public RIGUWErrorSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIGUWErrorSet aRIGUWErrorSet = new RIGUWErrorSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("RIGUWError");
			RIGUWErrorSchema aSchema = this.getSchema();
			...
			{
				i++;
				RIGUWErrorSchema s1 = new RIGUWErrorSchema();
				s1.setSchema(rs,i);
				aRIGUWErrorSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aRIGUWErrorSet;
	}

	public RIGUWErrorSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIGUWErrorSet aRIGUWErrorSet = new RIGUWErrorSet();
	...
	public RIGUWErrorSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIGUWErrorSet aRIGUWErrorSet = new RIGUWErrorSet();
	...
	public RIGUWErrorSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIGUWErrorSet aRIGUWErrorSet = new RIGUWErrorSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update RIGUWError " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return RIGUWErrorSet
 */
public RIGUWErrorSet getData()
{
    int tCount = 0;
    RIGUWErrorSet tRIGUWErrorSet = new RIGUWErrorSet();
    RIGUWErrorSchema tRIGUWErrorSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "RIGUWErrorDB";
    ...
    {
        tCount = 1;
        tRIGUWErrorSchema = new RIGUWErrorSchema();
        tRIGUWErrorSchema.setSchema(mResultSet, 1);
        tRIGUWErrorSet.add(tRIGUWErrorSchema);

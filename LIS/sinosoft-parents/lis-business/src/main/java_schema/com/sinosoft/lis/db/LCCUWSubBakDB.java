...
/*
 * <p>ClassName: LCCUWSubBakDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LCCUWSubBakDB extends LCCUWSubBakSchema
{
private static Logger logger = Logger.getLogger(LCCUWSubBakDB.class);
	...
	**/
	...
	// @Constructor
	public LCCUWSubBakDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCCUWSubBak" );
		mflag = true;
	}

	public LCCUWSubBakDB()
	{
		con = null;
		db = new DBOper( "LCCUWSubBak" );
		mflag = false;
	}
	...
	{
		LCCUWSubBakSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCCUWSubBakDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCCUWSubBak WHERE  ContNo = ? AND UWNo = ?");
		...
		SQLString sqlObj = new SQLString("LCCUWSubBak");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCCUWSubBak SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , UpReport = ? , CommonReason = ? , CommonReasonCode = ? WHERE  ContNo = ? AND UWNo = ?");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCCUWSubBak(GrpContNo ,ContNo ,ProposalContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea ,UpReport ,CommonReason ,CommonReasonCode) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCCUWSubBak WHERE  ContNo = ? AND UWNo = ?", 
	...
	public LCCUWSubBakSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCCUWSubBakSet aLCCUWSubBakSet = new LCCUWSubBakSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCCUWSubBak");
			LCCUWSubBakSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCCUWSubBakSchema s1 = new LCCUWSubBakSchema();
				s1.setSchema(rs,i);
				aLCCUWSubBakSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCCUWSubBakSet;

	}

	public LCCUWSubBakSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCCUWSubBakSet aLCCUWSubBakSet = new LCCUWSubBakSet();
	...
	public LCCUWSubBakSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCCUWSubBakSet aLCCUWSubBakSet = new LCCUWSubBakSet();
	...
	public LCCUWSubBakSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCCUWSubBakSet aLCCUWSubBakSet = new LCCUWSubBakSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCCUWSubBak " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCCUWSubBakSet
 */
public LCCUWSubBakSet getData()
{
    int tCount = 0;
    LCCUWSubBakSet tLCCUWSubBakSet = new LCCUWSubBakSet();
    LCCUWSubBakSchema tLCCUWSubBakSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCCUWSubBakDB";
    ...
    {
        tCount = 1;
        tLCCUWSubBakSchema = new LCCUWSubBakSchema();
        tLCCUWSubBakSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCCUWSubBakSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCCUWSubBakSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

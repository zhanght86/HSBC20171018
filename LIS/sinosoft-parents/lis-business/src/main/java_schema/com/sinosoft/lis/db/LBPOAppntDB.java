...
/*
 * <p>ClassName: LBPOAppntDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 内部外包流程
 */
public class LBPOAppntDB extends LBPOAppntSchema
{
private static Logger logger = Logger.getLogger(LBPOAppntDB.class);
	...
	**/
	...
	// @Constructor
	public LBPOAppntDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LBPOAppnt" );
		mflag = true;
	}
	...
	{
		LBPOAppntSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LBPOAppnt WHERE  ContNo = ? AND InputNo = ? AND FillNo = ?");
		...
		SQLString sqlObj = new SQLString("LBPOAppnt");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LBPOAppnt SET  GrpContNo = ? , ContNo = ? , InputNo = ? , FillNo = ? , PrtNo = ? , AppntNo = ? , AppntGrade = ? , AppntName = ? , AppntSex = ? , AppntBirthday = ? , AppntType = ? , AddressNo = ? , IDType = ? , IDNo = ? , NativePlace = ? , Nationality = ? , RgtAddress = ? , Marriage = ? , MarriageDate = ? , Health = ? , Stature = ? , Avoirdupois = ? , Degree = ? , CreditGrade = ? , BankCode = ? , BankAccNo = ? , AccName = ? , JoinCompanyDate = ? , StartWorkDate = ? , Position = ? , Salary = ? , OccupationType = ? , OccupationCode = ? , WorkType = ? , PluralityType = ? , SmokeFlag = ? , Operator = ? , ManageCom = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , BMI = ? , License = ? , LicenseType = ? , RelationToInsured = ? , EMail = ? , Mobile = ? , CompanyFax = ? , CompanyPhone = ? , CompanyZipCode = ? , CompanyAddress = ? , HomeFax = ? , HomePhone = ? , HomeZipCode = ? , HomeAddress = ? , Fax = ? , Phone = ? , ZipCode = ? , PostalAddress = ? , SocialInsuFlag = ? , IDExpDate = ? WHERE  ContNo = ? AND InputNo = ? AND FillNo = ?");
			...
			if(this.getAppntBirthday() == null || this.getAppntBirthday().equals("null")) {
				...
				pstmt.setString(10, this.getAppntBirthday());
			}
			...
			if(this.getMarriageDate() == null || this.getMarriageDate().equals("null")) {
				...
				pstmt.setString(19, this.getMarriageDate());
			}
			...
			if(this.getStature() == null || this.getStature().equals("null")) {
				...
				pstmt.setString(21, this.getStature());
			}
			if(this.getAvoirdupois() == null || this.getAvoirdupois().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getAvoirdupois());
			}
			...
			if(this.getJoinCompanyDate() == null || this.getJoinCompanyDate().equals("null")) {
				...
				pstmt.setString(28, this.getJoinCompanyDate());
			}
			if(this.getStartWorkDate() == null || this.getStartWorkDate().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getStartWorkDate());
			}
			...
			if(this.getSalary() == null || this.getSalary().equals("null")) {
				...
				pstmt.setString(31, this.getSalary());
			}
			...
			if(this.getMakeDate() == null || this.getMakeDate().equals("null")) {
				...
				pstmt.setString(39, this.getMakeDate());
			}
			...
			if(this.getModifyDate() == null || this.getModifyDate().equals("null")) {
				...
				pstmt.setString(41, this.getModifyDate());
			}
			...
			if(this.getBMI() == null || this.getBMI().equals("null")) {
				...
				pstmt.setString(43, this.getBMI());
			}
			...
			if(this.getPostalAddress() == null || this.getPostalAddress().equals("null")) {
				...
				pstmt.setString(60, this.getPostalAddress());
			}
			if(this.getSocialInsuFlag() == null || this.getSocialInsuFlag().equals("null")) {
				pstmt.setNull(61, 12);
			} else {
				pstmt.setString(61, this.getSocialInsuFlag());
			}
			...
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				...
				pstmt.setString(63, this.getContNo());
			}
			pstmt.setInt(64, this.getInputNo());
			if(this.getFillNo() == null || this.getFillNo().equals("null")) {
				pstmt.setNull(65, 12);
			} else {
				pstmt.setString(65, this.getFillNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LBPOAppnt(GrpContNo ,ContNo ,InputNo ,FillNo ,PrtNo ,AppntNo ,AppntGrade ,AppntName ,AppntSex ,AppntBirthday ,AppntType ,AddressNo ,IDType ,IDNo ,NativePlace ,Nationality ,RgtAddress ,Marriage ,MarriageDate ,Health ,Stature ,Avoirdupois ,Degree ,CreditGrade ,BankCode ,BankAccNo ,AccName ,JoinCompanyDate ,StartWorkDate ,Position ,Salary ,OccupationType ,OccupationCode ,WorkType ,PluralityType ,SmokeFlag ,Operator ,ManageCom ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,BMI ,License ,LicenseType ,RelationToInsured ,EMail ,Mobile ,CompanyFax ,CompanyPhone ,CompanyZipCode ,CompanyAddress ,HomeFax ,HomePhone ,HomeZipCode ,HomeAddress ,Fax ,Phone ,ZipCode ,PostalAddress ,SocialInsuFlag ,IDExpDate) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBPOAppnt WHERE  ContNo = ? AND InputNo = ? AND FillNo = ?", 
	...
	public LBPOAppntSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOAppntSet aLBPOAppntSet = new LBPOAppntSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBPOAppnt");
			LBPOAppntSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBPOAppntSchema s1 = new LBPOAppntSchema();
				s1.setSchema(rs,i);
				aLBPOAppntSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBPOAppntSet;

	}

	public LBPOAppntSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOAppntSet aLBPOAppntSet = new LBPOAppntSet();
	...
	public LBPOAppntSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOAppntSet aLBPOAppntSet = new LBPOAppntSet();
	...
	public LBPOAppntSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOAppntSet aLBPOAppntSet = new LBPOAppntSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBPOAppnt " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LBPOAppntSet
 */
public LBPOAppntSet getData()
{
    int tCount = 0;
    LBPOAppntSet tLBPOAppntSet = new LBPOAppntSet();
    LBPOAppntSchema tLBPOAppntSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBPOAppntDB";
    ...
    {
        tCount = 1;
        tLBPOAppntSchema = new LBPOAppntSchema();
        tLBPOAppntSchema.setSchema(mResultSet, 1);
        tLBPOAppntSet.add(tLBPOAppntSchema);
...
public boolean closeData()
{
	...
	public LBPOAppntSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBPOAppntSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

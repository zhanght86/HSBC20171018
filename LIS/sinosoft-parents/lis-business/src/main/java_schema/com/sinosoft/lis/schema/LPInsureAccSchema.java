...
/*
 * <p>ClassName: LPInsureAccSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPInsureAccSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPInsureAccSchema.class);
	// @Field
	/** 批单号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPInsureAccSchema cloned = (LPInsureAccSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LPInsureAccSchema 对象给 Schema 赋值
	* @param: aLPInsureAccSchema LPInsureAccSchema
	**/
	public void setSchema(LPInsureAccSchema aLPInsureAccSchema)
	{
		this.EdorNo = aLPInsureAccSchema.getEdorNo();
		this.EdorType = aLPInsureAccSchema.getEdorType();
		this.PolNo = aLPInsureAccSchema.getPolNo();
		this.InsuAccNo = aLPInsureAccSchema.getInsuAccNo();
		this.ContNo = aLPInsureAccSchema.getContNo();
		this.GrpContNo = aLPInsureAccSchema.getGrpContNo();
		this.GrpPolNo = aLPInsureAccSchema.getGrpPolNo();
		this.PrtNo = aLPInsureAccSchema.getPrtNo();
		this.RiskCode = aLPInsureAccSchema.getRiskCode();
		this.AccType = aLPInsureAccSchema.getAccType();
		this.InvestType = aLPInsureAccSchema.getInvestType();
		this.FundCompanyCode = aLPInsureAccSchema.getFundCompanyCode();
		this.InsuredNo = aLPInsureAccSchema.getInsuredNo();
		this.AppntNo = aLPInsureAccSchema.getAppntNo();
		this.Owner = aLPInsureAccSchema.getOwner();
		this.AccComputeFlag = aLPInsureAccSchema.getAccComputeFlag();
		this.AccFoundDate = fDate.getDate( aLPInsureAccSchema.getAccFoundDate());
		this.AccFoundTime = aLPInsureAccSchema.getAccFoundTime();
		this.BalaDate = fDate.getDate( aLPInsureAccSchema.getBalaDate());
		this.BalaTime = aLPInsureAccSchema.getBalaTime();
		this.SumPay = aLPInsureAccSchema.getSumPay();
		this.SumPaym = aLPInsureAccSchema.getSumPaym();
		this.LastAccBala = aLPInsureAccSchema.getLastAccBala();
		this.LastUnitCount = aLPInsureAccSchema.getLastUnitCount();
		this.LastUnitPrice = aLPInsureAccSchema.getLastUnitPrice();
		this.InsuAccBala = aLPInsureAccSchema.getInsuAccBala();
		this.UnitCount = aLPInsureAccSchema.getUnitCount();
		this.UnitPrice = aLPInsureAccSchema.getUnitPrice();
		this.InsuAccGetMoney = aLPInsureAccSchema.getInsuAccGetMoney();
		this.FrozenMoney = aLPInsureAccSchema.getFrozenMoney();
		this.State = aLPInsureAccSchema.getState();
		this.ManageCom = aLPInsureAccSchema.getManageCom();
		this.Operator = aLPInsureAccSchema.getOperator();
		this.MakeDate = fDate.getDate( aLPInsureAccSchema.getMakeDate());
		this.MakeTime = aLPInsureAccSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLPInsureAccSchema.getModifyDate());
		this.ModifyTime = aLPInsureAccSchema.getModifyTime();
		this.Currency = aLPInsureAccSchema.getCurrency();
		this.ComCode = aLPInsureAccSchema.getComCode();
		this.ModifyOperator = aLPInsureAccSchema.getModifyOperator();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPInsureAcc表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPInsureAccSchema";
	...
	public LPInsureAccSchema getSchema()
	{
		LPInsureAccSchema aLPInsureAccSchema = new LPInsureAccSchema();
		aLPInsureAccSchema.setSchema(this);
		return aLPInsureAccSchema;
	}

	public LPInsureAccDB getDB()
	{
		LPInsureAccDB aDBOper = new LPInsureAccDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPInsureAcc描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPInsureAcc>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			AccType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			...
			Owner = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			...
			SumPay = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,21,SysConst.PACKAGESPILTER))).doubleValue();
			SumPaym = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,22,SysConst.PACKAGESPILTER))).doubleValue();
			LastAccBala = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,23,SysConst.PACKAGESPILTER))).doubleValue();
			LastUnitCount = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,24,SysConst.PACKAGESPILTER))).doubleValue();
			LastUnitPrice = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,25,SysConst.PACKAGESPILTER))).doubleValue();
			InsuAccBala = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,26,SysConst.PACKAGESPILTER))).doubleValue();
			UnitCount = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,27,SysConst.PACKAGESPILTER))).doubleValue();
			UnitPrice = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,28,SysConst.PACKAGESPILTER))).doubleValue();
			InsuAccGetMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,29,SysConst.PACKAGESPILTER))).doubleValue();
			FrozenMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,30,SysConst.PACKAGESPILTER))).doubleValue();
			State = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 31, SysConst.PACKAGESPILTER );
			ManageCom = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 32, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPInsureAccSchema other = (LPInsureAccSchema)otherObject;
		return

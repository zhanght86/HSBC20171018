...
/*
 * <p>ClassName: LCCUWErrorSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCCUWErrorSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LCCUWErrorSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LCCUWErrorSchema cloned = (LCCUWErrorSchema)super.clone();
	...
	/**
	* 使用另外一个 LCCUWErrorSchema 对象给 Schema 赋值
	* @param: aLCCUWErrorSchema LCCUWErrorSchema
	**/
	public void setSchema(LCCUWErrorSchema aLCCUWErrorSchema)
	{
		this.GrpContNo = aLCCUWErrorSchema.getGrpContNo();
		this.ContNo = aLCCUWErrorSchema.getContNo();
		this.ProposalContNo = aLCCUWErrorSchema.getProposalContNo();
		this.SerialNo = aLCCUWErrorSchema.getSerialNo();
		this.UWNo = aLCCUWErrorSchema.getUWNo();
		this.InsuredNo = aLCCUWErrorSchema.getInsuredNo();
		this.InsuredName = aLCCUWErrorSchema.getInsuredName();
		this.AppntNo = aLCCUWErrorSchema.getAppntNo();
		this.AppntName = aLCCUWErrorSchema.getAppntName();
		this.ManageCom = aLCCUWErrorSchema.getManageCom();
		this.UWRuleCode = aLCCUWErrorSchema.getUWRuleCode();
		this.UWError = aLCCUWErrorSchema.getUWError();
		this.CurrValue = aLCCUWErrorSchema.getCurrValue();
		this.ModifyDate = fDate.getDate( aLCCUWErrorSchema.getModifyDate());
		this.ModifyTime = aLCCUWErrorSchema.getModifyTime();
		this.UWPassFlag = aLCCUWErrorSchema.getUWPassFlag();
		this.UWGrade = aLCCUWErrorSchema.getUWGrade();
		this.SugPassFlag = aLCCUWErrorSchema.getSugPassFlag();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LCCUWError表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LCCUWErrorSchema";
	...
	public LCCUWErrorSchema getSchema()
	{
		LCCUWErrorSchema aLCCUWErrorSchema = new LCCUWErrorSchema();
		aLCCUWErrorSchema.setSchema(this);
		return aLCCUWErrorSchema;
	}

	public LCCUWErrorDB getDB()
	{
		LCCUWErrorDB aDBOper = new LCCUWErrorDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCCUWError描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCCUWError>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LCCUWErrorSchema other = (LCCUWErrorSchema)otherObject;
		return

...
/*
 * <p>ClassName: LCGrpFeeSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCGrpFeeSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LCGrpFeeSchema.class);
	// @Field
	/** 集体保单险种号码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LCGrpFeeSchema cloned = (LCGrpFeeSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LCGrpFeeSchema 对象给 Schema 赋值
	* @param: aLCGrpFeeSchema LCGrpFeeSchema
	**/
	public void setSchema(LCGrpFeeSchema aLCGrpFeeSchema)
	{
		this.GrpPolNo = aLCGrpFeeSchema.getGrpPolNo();
		this.GrpContNo = aLCGrpFeeSchema.getGrpContNo();
		this.RiskCode = aLCGrpFeeSchema.getRiskCode();
		this.FeeCode = aLCGrpFeeSchema.getFeeCode();
		this.InsuAccNo = aLCGrpFeeSchema.getInsuAccNo();
		this.PayPlanCode = aLCGrpFeeSchema.getPayPlanCode();
		this.PayInsuAccName = aLCGrpFeeSchema.getPayInsuAccName();
		this.FeeCalMode = aLCGrpFeeSchema.getFeeCalMode();
		this.FeeCalModeType = aLCGrpFeeSchema.getFeeCalModeType();
		this.FeeCalCode = aLCGrpFeeSchema.getFeeCalCode();
		this.FeeValue = aLCGrpFeeSchema.getFeeValue();
		this.CompareValue = aLCGrpFeeSchema.getCompareValue();
		this.FeePeriod = aLCGrpFeeSchema.getFeePeriod();
		this.MaxTime = aLCGrpFeeSchema.getMaxTime();
		this.DefaultFlag = aLCGrpFeeSchema.getDefaultFlag();
		this.Operator = aLCGrpFeeSchema.getOperator();
		this.MakeDate = fDate.getDate( aLCGrpFeeSchema.getMakeDate());
		this.MakeTime = aLCGrpFeeSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLCGrpFeeSchema.getModifyDate());
		this.ModifyTime = aLCGrpFeeSchema.getModifyTime();
		this.FeeStartDate = fDate.getDate( aLCGrpFeeSchema.getFeeStartDate());
		this.PeriodType = aLCGrpFeeSchema.getPeriodType();
		this.FeeType = aLCGrpFeeSchema.getFeeType();
		this.InterfaceClassName = aLCGrpFeeSchema.getInterfaceClassName();
		this.FeeBaseType = aLCGrpFeeSchema.getFeeBaseType();
		this.FeeNum = aLCGrpFeeSchema.getFeeNum();
		this.FeeTakePlace = aLCGrpFeeSchema.getFeeTakePlace();
		this.ValPeriod = aLCGrpFeeSchema.getValPeriod();
		this.ValStartDate = aLCGrpFeeSchema.getValStartDate();
		this.ValEndDate = aLCGrpFeeSchema.getValEndDate();
		this.ManageCom = aLCGrpFeeSchema.getManageCom();
		this.ComCode = aLCGrpFeeSchema.getComCode();
		this.ModifyOperator = aLCGrpFeeSchema.getModifyOperator();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LCGrpFee表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LCGrpFeeSchema";
	...
	public LCGrpFeeSchema getSchema()
	{
		LCGrpFeeSchema aLCGrpFeeSchema = new LCGrpFeeSchema();
		aLCGrpFeeSchema.setSchema(this);
		return aLCGrpFeeSchema;
	}

	public LCGrpFeeDB getDB()
	{
		LCGrpFeeDB aDBOper = new LCGrpFeeDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCGrpFee描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCGrpFee>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			FeeCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			InsuAccNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			PayPlanCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			PayInsuAccName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			FeeCalMode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			FeeCalModeType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			FeeCalCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			FeeValue = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,11,SysConst.PACKAGESPILTER))).doubleValue();
			CompareValue = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,12,SysConst.PACKAGESPILTER))).doubleValue();
			FeePeriod = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			MaxTime= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,14,SysConst.PACKAGESPILTER))).intValue();
			DefaultFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			...
			FeeStartDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 21,SysConst.PACKAGESPILTER));
			PeriodType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22, SysConst.PACKAGESPILTER );
			FeeType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 23, SysConst.PACKAGESPILTER );
			InterfaceClassName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 24, SysConst.PACKAGESPILTER );
			FeeBaseType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 25, SysConst.PACKAGESPILTER );
			FeeNum= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,26,SysConst.PACKAGESPILTER))).intValue();
			FeeTakePlace = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 27, SysConst.PACKAGESPILTER );
			ValPeriod = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 28, SysConst.PACKAGESPILTER );
			ValStartDate = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 29, SysConst.PACKAGESPILTER );
			ValEndDate = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 30, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LCGrpFeeSchema other = (LCGrpFeeSchema)otherObject;
		return

...
/*
 * <p>ClassName: OFinaLogSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 财务接口
 */
public class OFinaLogSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(OFinaLogSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		OFinaLogSchema cloned = (OFinaLogSchema)super.clone();
	...
	*/
	...
	/**
	* 0，表示RecordNo对应的数据没有应用操作<p>
	* 1，表示RecordNo对应的数据正在被应用操作<p>
	* <p>
	* 默认为 0
	*/
	...
	*/
	...
	/**
	* 使用另外一个 OFinaLogSchema 对象给 Schema 赋值
	* @param: aOFinaLogSchema OFinaLogSchema
	**/
	public void setSchema(OFinaLogSchema aOFinaLogSchema)
	{
		this.RecordNo = aOFinaLogSchema.getRecordNo();
		this.TransDate = fDate.getDate( aOFinaLogSchema.getTransDate());
		this.BatchNo = aOFinaLogSchema.getBatchNo();
		this.VoucherType = aOFinaLogSchema.getVoucherType();
		this.MakeDate = fDate.getDate( aOFinaLogSchema.getMakeDate());
		this.MakeTime = aOFinaLogSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aOFinaLogSchema.getModifyDate());
		this.ModifyTime = aOFinaLogSchema.getModifyTime();
		this.Operator = aOFinaLogSchema.getOperator();
		this.Remark = aOFinaLogSchema.getRemark();
		this.State = aOFinaLogSchema.getState();
		this.MatchID = aOFinaLogSchema.getMatchID();
		this.ManageCom = aOFinaLogSchema.getManageCom();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的OFinaLog表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "OFinaLogSchema";
	...
	public OFinaLogSchema getSchema()
	{
		OFinaLogSchema aOFinaLogSchema = new OFinaLogSchema();
		aOFinaLogSchema.setSchema(this);
		return aOFinaLogSchema;
	}

	public OFinaLogDB getDB()
	{
		OFinaLogDB aDBOper = new OFinaLogDB();
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(RecordNo)); strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpOFinaLog>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			RecordNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 1, SysConst.PACKAGESPILTER );
			...
			VoucherType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			...
			MatchID= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,12,SysConst.PACKAGESPILTER))).intValue();
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		OFinaLogSchema other = (OFinaLogSchema)otherObject;
		return

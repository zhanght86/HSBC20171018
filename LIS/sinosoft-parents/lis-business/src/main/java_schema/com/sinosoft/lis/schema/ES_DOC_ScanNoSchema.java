...
{
private static Logger logger = Logger.getLogger(ES_DOC_ScanNoSchema.class);
	...
	*/
	...
	{
		if(aScanNo!=null && aScanNo.length()>30)
			throw new IllegalArgumentException("批次号ScanNo值"+aScanNo+"的长度"+aScanNo.length()+"大于最大值30");
		ScanNo = aScanNo;
	}
	...
	{
		if(aScanOperator!=null && aScanOperator.length()>60)
			throw new IllegalArgumentException("扫描操作员ScanOperator值"+aScanOperator+"的长度"+aScanOperator.length()+"大于最大值60");
	...
	*/
	...
	public void setOperator(String aOperator)
	{
		if(aOperator!=null && aOperator.length()>60)
			throw new IllegalArgumentException("操作员Operator值"+aOperator+"的长度"+aOperator.length()+"大于最大值60");
		Operator = aOperator;
	}
	/**
	* 上载日期
	...
	*/
	...
	public void setMakeTime(String aMakeTime)
	{
		if(aMakeTime!=null && aMakeTime.length()>8)
			throw new IllegalArgumentException("生成时间MakeTime值"+aMakeTime+"的长度"+aMakeTime.length()+"大于最大值8");
		MakeTime = aMakeTime;
	}
	...
	**/
	...
	{
		this.ScanNo = aES_DOC_ScanNoSchema.getScanNo();
		this.ScanOperator = aES_DOC_ScanNoSchema.getScanOperator();
		this.ManageCom = aES_DOC_ScanNoSchema.getManageCom();
		this.Operator = aES_DOC_ScanNoSchema.getOperator();
		this.MakeDate = fDate.getDate( aES_DOC_ScanNoSchema.getMakeDate());
		this.MakeTime = aES_DOC_ScanNoSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aES_DOC_ScanNoSchema.getModifyDate());
		this.ModifyTime = aES_DOC_ScanNoSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的ES_DOC_ScanNo表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			ScanNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 1, SysConst.PACKAGESPILTER );
			ScanOperator = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 2, SysConst.PACKAGESPILTER );

...
import com.sinosoft.lis.db.LSQuotPlanDB;

/*
 * <p>ClassName: LSQuotPlanSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LSQuotPlanSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LSQuotPlanSchema.class);
	// @Field
	/** 询价号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LSQuotPlanSchema cloned = (LSQuotPlanSchema)super.clone();
	...
	public void setSysPlanCode(String aSysPlanCode)
	{
		if(aSysPlanCode!=null && aSysPlanCode.length()>10)
			logger.debug("系统方案编码SysPlanCode值"+aSysPlanCode+"的长度"+aSysPlanCode.length()+"大于最大值10");
	...
	public void setPlanCode(String aPlanCode)
	{
		if(aPlanCode!=null && aPlanCode.length()>10)
			logger.debug("方案编码PlanCode值"+aPlanCode+"的长度"+aPlanCode.length()+"大于最大值10");
		PlanCode = aPlanCode;
	}
	...
	public void setPlanDesc(String aPlanDesc)
	{
		if(aPlanDesc!=null && aPlanDesc.length()>30)
			logger.debug("方案描述PlanDesc值"+aPlanDesc+"的长度"+aPlanDesc.length()+"大于最大值30");
		PlanDesc = aPlanDesc;
	}
	/**
	...
	*/
	...
	public void setPlanType(String aPlanType)
	{
		if(aPlanType!=null && aPlanType.length()>2)
			logger.debug("方案类型PlanType值"+aPlanType+"的长度"+aPlanType.length()+"大于最大值2");
		PlanType = aPlanType;
	}
	/**
	* 00-核心方案，01-自选方案
	*/
	...
	public void setPlanFlag(String aPlanFlag)
	{
		if(aPlanFlag!=null && aPlanFlag.length()>2)
			logger.debug("方案标识PlanFlag值"+aPlanFlag+"的长度"+aPlanFlag.length()+"大于最大值2");
		PlanFlag = aPlanFlag;
	}
	/**
	...
	*/
	...
	public void setPremCalType(String aPremCalType)
	{
		if(aPremCalType!=null && aPremCalType.length()>2)
			logger.debug("保费计算方式PremCalType值"+aPremCalType+"的长度"+aPremCalType.length()+"大于最大值2");
	...
	public void setOccupTypeFlag(String aOccupTypeFlag)
	{
		if(aOccupTypeFlag!=null && aOccupTypeFlag.length()>2)
			logger.debug("职业类别标记OccupTypeFlag值"+aOccupTypeFlag+"的长度"+aOccupTypeFlag.length()+"大于最大值2");
	...
	public void setMinOccupType(String aMinOccupType)
	{
		if(aMinOccupType!=null && aMinOccupType.length()>10)
			logger.debug("最低职业类别MinOccupType值"+aMinOccupType+"的长度"+aMinOccupType.length()+"大于最大值10");
	...
	public void setMaxOccupType(String aMaxOccupType)
	{
		if(aMaxOccupType!=null && aMaxOccupType.length()>10)
			logger.debug("最高职业类别MaxOccupType值"+aMaxOccupType+"的长度"+aMaxOccupType.length()+"大于最大值10");
	...
	public void setOccupType(String aOccupType)
	{
		if(aOccupType!=null && aOccupType.length()>10)
			logger.debug("职业类别OccupType值"+aOccupType+"的长度"+aOccupType.length()+"大于最大值10");
	...
	public void setOccupMidType(String aOccupMidType)
	{
		if(aOccupMidType!=null && aOccupMidType.length()>10)
			logger.debug("职业中分类OccupMidType值"+aOccupMidType+"的长度"+aOccupMidType.length()+"大于最大值10");
	...
	public void setOccupCode(String aOccupCode)
	{
		if(aOccupCode!=null && aOccupCode.length()>10)
			logger.debug("工种OccupCode值"+aOccupCode+"的长度"+aOccupCode.length()+"大于最大值10");
	...
	public void setOtherDesc(String aOtherDesc)
	{
		if(aOtherDesc!=null && aOtherDesc.length()>3000)
			logger.debug("其他说明OtherDesc值"+aOtherDesc+"的长度"+aOtherDesc.length()+"大于最大值3000");
	...
	public void setOccupRate(String aOccupRate)
	{
		if(aOccupRate!=null && aOccupRate.length()>40)
			logger.debug("职业比例OccupRate值"+aOccupRate+"的长度"+aOccupRate.length()+"大于最大值40");
		OccupRate = aOccupRate;
	}

	/**
	* 使用另外一个 LSQuotPlanSchema 对象给 Schema 赋值
	* @param: aLSQuotPlanSchema LSQuotPlanSchema
	**/
	public void setSchema(LSQuotPlanSchema aLSQuotPlanSchema)
	{
		this.QuotNo = aLSQuotPlanSchema.getQuotNo();
		this.QuotBatNo = aLSQuotPlanSchema.getQuotBatNo();
		this.SysPlanCode = aLSQuotPlanSchema.getSysPlanCode();
		this.PlanCode = aLSQuotPlanSchema.getPlanCode();
		this.PlanDesc = aLSQuotPlanSchema.getPlanDesc();
		this.PlanType = aLSQuotPlanSchema.getPlanType();
		this.PlanFlag = aLSQuotPlanSchema.getPlanFlag();
		this.PremCalType = aLSQuotPlanSchema.getPremCalType();
		this.InsuPeriod = aLSQuotPlanSchema.getInsuPeriod();
		this.InsuPeriodFlag = aLSQuotPlanSchema.getInsuPeriodFlag();
		this.OccupTypeFlag = aLSQuotPlanSchema.getOccupTypeFlag();
		this.MinOccupType = aLSQuotPlanSchema.getMinOccupType();
		this.MaxOccupType = aLSQuotPlanSchema.getMaxOccupType();
		this.OccupType = aLSQuotPlanSchema.getOccupType();
		this.OccupMidType = aLSQuotPlanSchema.getOccupMidType();
		this.OccupCode = aLSQuotPlanSchema.getOccupCode();
		this.MinAge = aLSQuotPlanSchema.getMinAge();
		this.MaxAge = aLSQuotPlanSchema.getMaxAge();
		this.AvgAge = aLSQuotPlanSchema.getAvgAge();
		this.NumPeople = aLSQuotPlanSchema.getNumPeople();
		this.SocialInsuRate = aLSQuotPlanSchema.getSocialInsuRate();
		this.MaleRate = aLSQuotPlanSchema.getMaleRate();
		this.FemaleRate = aLSQuotPlanSchema.getFemaleRate();
		this.RetireRate = aLSQuotPlanSchema.getRetireRate();
		this.PremMode = aLSQuotPlanSchema.getPremMode();
		this.EnterpriseRate = aLSQuotPlanSchema.getEnterpriseRate();
		this.MinSalary = aLSQuotPlanSchema.getMinSalary();
		this.MaxSalary = aLSQuotPlanSchema.getMaxSalary();
		this.AvgSalary = aLSQuotPlanSchema.getAvgSalary();
		this.OtherDesc = aLSQuotPlanSchema.getOtherDesc();
		this.MakeOperator = aLSQuotPlanSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLSQuotPlanSchema.getMakeDate());
		this.MakeTime = aLSQuotPlanSchema.getMakeTime();
		this.ModifyOperator = aLSQuotPlanSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLSQuotPlanSchema.getModifyDate());
		this.ModifyTime = aLSQuotPlanSchema.getModifyTime();
		this.OccupRate = aLSQuotPlanSchema.getOccupRate();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LSQuotPlan表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LSQuotPlanSchema";
	...
	public LSQuotPlanSchema getSchema()
	{
		LSQuotPlanSchema aLSQuotPlanSchema = new LSQuotPlanSchema();
		aLSQuotPlanSchema.setSchema(this);
		return aLSQuotPlanSchema;
	}

	public LSQuotPlanDB getDB()
	{
		LSQuotPlanDB aDBOper = new LSQuotPlanDB();
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(OccupRate));
		return strReturn.toString();
	}

	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotPlan>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			PlanDesc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			PlanType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			PlanFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			PremCalType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			InsuPeriod= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,9,SysConst.PACKAGESPILTER))).intValue();
			InsuPeriodFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			OccupTypeFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			MinOccupType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			MaxOccupType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			OccupType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			OccupMidType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			OccupCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 16, SysConst.PACKAGESPILTER );
			MinAge= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,17,SysConst.PACKAGESPILTER))).intValue();
			MaxAge= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,18,SysConst.PACKAGESPILTER))).intValue();
			AvgAge= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,19,SysConst.PACKAGESPILTER))).intValue();
			NumPeople= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,20,SysConst.PACKAGESPILTER))).intValue();
			SocialInsuRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,21,SysConst.PACKAGESPILTER))).doubleValue();
			MaleRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,22,SysConst.PACKAGESPILTER))).doubleValue();
			FemaleRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,23,SysConst.PACKAGESPILTER))).doubleValue();
			RetireRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,24,SysConst.PACKAGESPILTER))).doubleValue();
			PremMode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 25, SysConst.PACKAGESPILTER );
			EnterpriseRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,26,SysConst.PACKAGESPILTER))).doubleValue();
			MinSalary = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,27,SysConst.PACKAGESPILTER))).doubleValue();
			MaxSalary = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,28,SysConst.PACKAGESPILTER))).doubleValue();
			AvgSalary = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,29,SysConst.PACKAGESPILTER))).doubleValue();
			OtherDesc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 30, SysConst.PACKAGESPILTER );
			...
			OccupRate = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 37, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LSQuotPlanSchema other = (LSQuotPlanSchema)otherObject;
		return

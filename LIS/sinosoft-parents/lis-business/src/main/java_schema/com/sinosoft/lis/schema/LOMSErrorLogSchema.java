...
/*
 * <p>ClassName: LOMSErrorLogSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LOMSErrorLogSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LOMSErrorLogSchema.class);
	// @Field
	/** 短信流水号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LOMSErrorLogSchema cloned = (LOMSErrorLogSchema)super.clone();
	...
	*/
	...
	{
		if(aErrorLog!=null && aErrorLog.length()>1000)
			throw new IllegalArgumentException("错误原因ErrorLog值"+aErrorLog+"的长度"+aErrorLog.length()+"大于最大值1000");
		ErrorLog = aErrorLog;
	}
	...
	*/
	...
	/**
	* 使用另外一个 LOMSErrorLogSchema 对象给 Schema 赋值
	* @param: aLOMSErrorLogSchema LOMSErrorLogSchema
	**/
	public void setSchema(LOMSErrorLogSchema aLOMSErrorLogSchema)
	{
		this.MSSeq = aLOMSErrorLogSchema.getMSSeq();
		this.PrtNo = aLOMSErrorLogSchema.getPrtNo();
		this.MSType = aLOMSErrorLogSchema.getMSType();
		this.State = aLOMSErrorLogSchema.getState();
		this.AppntNo = aLOMSErrorLogSchema.getAppntNo();
		this.AppntName = aLOMSErrorLogSchema.getAppntName();
		this.ManageCom = aLOMSErrorLogSchema.getManageCom();
		this.AgentCode = aLOMSErrorLogSchema.getAgentCode();
		this.AgentName = aLOMSErrorLogSchema.getAgentName();
		this.AgentMobile = aLOMSErrorLogSchema.getAgentMobile();
		this.MSContent = aLOMSErrorLogSchema.getMSContent();
		this.Operator = aLOMSErrorLogSchema.getOperator();
		this.PatchFlag = aLOMSErrorLogSchema.getPatchFlag();
		this.PatchCount = aLOMSErrorLogSchema.getPatchCount();
		this.ErrorLog = aLOMSErrorLogSchema.getErrorLog();
		this.Remark = aLOMSErrorLogSchema.getRemark();
		this.MakeDate = fDate.getDate( aLOMSErrorLogSchema.getMakeDate());
		this.MakeTime = aLOMSErrorLogSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLOMSErrorLogSchema.getModifyDate());
		this.ModifyTime = aLOMSErrorLogSchema.getModifyTime();
		this.StandbyFlag1 = aLOMSErrorLogSchema.getStandbyFlag1();
		this.StandbyFlag2 = aLOMSErrorLogSchema.getStandbyFlag2();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			if( rs.getString("ErrorLog") == null )
				this.ErrorLog = null;
			else
				this.ErrorLog = rs.getString("ErrorLog").trim();
		...
		{
			logger.debug("数据库中的LOMSErrorLog表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LOMSErrorLogSchema";
	...
	public LOMSErrorLogSchema getSchema()
	{
		LOMSErrorLogSchema aLOMSErrorLogSchema = new LOMSErrorLogSchema();
		aLOMSErrorLogSchema.setSchema(this);
		return aLOMSErrorLogSchema;
	}

	public LOMSErrorLogDB getDB()
	{
		LOMSErrorLogDB aDBOper = new LOMSErrorLogDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOMSErrorLog描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(ErrorLog)); strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOMSErrorLog>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ErrorLog = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			...
			StandbyFlag1 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 21, SysConst.PACKAGESPILTER );
			StandbyFlag2 = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22, SysConst.PACKAGESPILTER );
		}
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("ErrorLog"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(ErrorLog));
		}
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 13:
				...
				break;
			case 14:
				strFieldValue = StrTool.GBKToUnicode(ErrorLog);
				break;
			case 15:
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LOMSErrorLogSchema other = (LOMSErrorLogSchema)otherObject;
		return
			...
			&& ErrorLog.equals(other.getErrorLog())
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("ErrorLog") ) {
			return 14;
		}

...
/*
 * <p>ClassName: LPIssuePolDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPIssuePolDB extends LPIssuePolSchema
{
private static Logger logger = Logger.getLogger(LPIssuePolDB.class);
	...
	**/
	...
	// @Constructor
	public LPIssuePolDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPIssuePolSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPIssuePolDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPIssuePol WHERE  EdorNo = ? AND EdorType = ? AND ProposalContNo = ? AND SerialNo = ?", 
	...
	public LPIssuePolSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPIssuePolSet aLPIssuePolSet = new LPIssuePolSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPIssuePol");
			LPIssuePolSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPIssuePolSchema s1 = new LPIssuePolSchema();
				s1.setSchema(rs,i);
				aLPIssuePolSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPIssuePolSet;

	}

	public LPIssuePolSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPIssuePolSet aLPIssuePolSet = new LPIssuePolSet();
	...
	public LPIssuePolSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPIssuePolSet aLPIssuePolSet = new LPIssuePolSet();
	...
	public LPIssuePolSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPIssuePolSet aLPIssuePolSet = new LPIssuePolSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPIssuePol " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPIssuePolSet
 */
public LPIssuePolSet getData()
{
    int tCount = 0;
    LPIssuePolSet tLPIssuePolSet = new LPIssuePolSet();
    LPIssuePolSchema tLPIssuePolSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPIssuePolDB";
    ...
    try
    {
        ...
        tLPIssuePolSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LPIssuePolSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPIssuePolSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

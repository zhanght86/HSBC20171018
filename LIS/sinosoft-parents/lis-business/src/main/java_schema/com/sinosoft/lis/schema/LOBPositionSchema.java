...
import com.sinosoft.lis.db.LOBPositionDB;

/*
 * 与数据库结构不一致，暂不处理
 * <p>ClassName: LOBPositionSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LOBPositionSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LOBPositionSchema.class);
	// @Field
	/** 业务类型 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LOBPositionSchema cloned = (LOBPositionSchema)super.clone();
	...
	public void setPositionCode(String aPositionCode)
	{
		if(aPositionCode!=null && aPositionCode.length()>10)
			throw new IllegalArgumentException("职级代码PositionCode值"+aPositionCode+"的长度"+aPositionCode.length()+"大于最大值10");
	...
	*/
	...
	public void setPositionCodeName(String aPositionCodeName)
	{
		if(aPositionCodeName!=null && aPositionCodeName.length()>100)
			throw new IllegalArgumentException("职级名称PositionCodeName值"+aPositionCodeName+"的长度"+aPositionCodeName.length()+"大于最大值100");
		PositionCodeName = aPositionCodeName;
	}
	public String getCountType()
	{
		return CountType;
	}
	public void setCountType(String aCountType)
	{
		if(aCountType!=null && aCountType.length()>30)
			throw new IllegalArgumentException("归属年度计算方式CountType值"+aCountType+"的长度"+aCountType.length()+"大于最大值30");
	...
	*/
	...
	/**
	* 使用另外一个 LOBPositionSchema 对象给 Schema 赋值
	* @param: aLOBPositionSchema LOBPositionSchema
	**/
	public void setSchema(LOBPositionSchema aLOBPositionSchema)
	{
		this.BussType = aLOBPositionSchema.getBussType();
		this.BussNo = aLOBPositionSchema.getBussNo();
		this.SerialNo = aLOBPositionSchema.getSerialNo();
		this.GrpContNo = aLOBPositionSchema.getGrpContNo();
		this.RiskCode = aLOBPositionSchema.getRiskCode();
		this.GrpPolNo = aLOBPositionSchema.getGrpPolNo();
		this.PositionCode = aLOBPositionSchema.getPositionCode();
		this.PositionCodeName = aLOBPositionSchema.getPositionCodeName();
		this.CountType = aLOBPositionSchema.getCountType();
		this.Segment1 = aLOBPositionSchema.getSegment1();
		this.Segment2 = aLOBPositionSchema.getSegment2();
		this.Segment3 = aLOBPositionSchema.getSegment3();
		this.ManageCom = aLOBPositionSchema.getManageCom();
		this.ComCode = aLOBPositionSchema.getComCode();
		this.MakeOperator = aLOBPositionSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLOBPositionSchema.getMakeDate());
		this.MakeTime = aLOBPositionSchema.getMakeTime();
		this.ModifyOperator = aLOBPositionSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLOBPositionSchema.getModifyDate());
		this.ModifyTime = aLOBPositionSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			if( rs.getString("PositionCodeName") == null )
				this.PositionCodeName = null;
			else
				this.PositionCodeName = rs.getString("PositionCodeName").trim();

			if( rs.getString("CountType") == null )
				this.CountType = null;
			else
				this.CountType = rs.getString("CountType").trim();
		...
		{
			logger.debug("数据库中的LOBPosition表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LOBPositionSchema";
	...
	public LOBPositionSchema getSchema()
	{
		LOBPositionSchema aLOBPositionSchema = new LOBPositionSchema();
		aLOBPositionSchema.setSchema(this);
		return aLOBPositionSchema;
	}

	public LOBPositionDB getDB()
	{
		LOBPositionDB aDBOper = new LOBPositionDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBPosition描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(PositionCodeName)); strReturn.append(SysConst.PACKAGESPILTER);
		strReturn.append(StrTool.cTrim(CountType)); strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBPosition>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			PositionCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			PositionCodeName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			CountType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("PositionCodeName"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(PositionCodeName));
		}
		if (FCode.equalsIgnoreCase("CountType"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(CountType));
		}
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 6:
				...
				break;
			case 7:
				strFieldValue = StrTool.GBKToUnicode(PositionCodeName);
				break;
			case 8:
				strFieldValue = StrTool.GBKToUnicode(CountType);
				break;
			case 9:
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LOBPositionSchema other = (LOBPositionSchema)otherObject;
		return
			...
			&& PositionCodeName.equals(other.getPositionCodeName())
			&& CountType.equals(other.getCountType())
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("PositionCodeName") ) {
			return 7;
		}
		if( strFieldName.equals("CountType") ) {
			return 8;
		}
	...
	public String getFieldName(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 6:
				...
				break;
			case 7:
				strFieldName = "PositionCodeName";
				break;
			case 8:
				strFieldName = "CountType";
				break;
			case 9:

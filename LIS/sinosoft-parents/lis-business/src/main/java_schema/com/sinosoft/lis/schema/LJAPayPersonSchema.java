...
{
private static Logger logger = Logger.getLogger(LJAPayPersonSchema.class);
	// @Field
	/** 保单号码 */
	...
	**/
	public void setSchema(LJAPayPersonSchema aLJAPayPersonSchema)
	{
		...
		this.Currency = aLJAPayPersonSchema.getCurrency();
		this.EndorsementNo = aLJAPayPersonSchema.getEndorsementNo();
		this.GetFlag = aLJAPayPersonSchema.getGetFlag();
		this.ComCode = aLJAPayPersonSchema.getComCode();
		this.ModifyOperator = aLJAPayPersonSchema.getModifyOperator();
		this.NetAmount = aLJAPayPersonSchema.getNetAmount();
		this.TaxAmount = aLJAPayPersonSchema.getTaxAmount();
		this.Tax = aLJAPayPersonSchema.getTax();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LJAPayPerson表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			MainPolYear= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,38,SysConst.PACKAGESPILTER))).intValue();
			...
			EndorsementNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 41, SysConst.PACKAGESPILTER );
			GetFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 42, SysConst.PACKAGESPILTER );

...
/*
 * <p>ClassName: LMInsuAccRateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_21
 */
public class LMInsuAccRateDB extends LMInsuAccRateSchema
{
	private static Logger logger = Logger.getLogger(LMInsuAccRateDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LMInsuAccRateDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LMInsuAccRateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LMInsuAccRateDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getBalaDate() == null || this.getBalaDate().equals("null")) {
				...
				pstmt.setDate(3, Date.valueOf(this.getBalaDate()));
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRateIntv() == null || this.getRateIntv().equals("null")) {
				...
				pstmt.setString(6, this.getRateIntv());
			}
			...
			if(this.getRateState() == null || this.getRateState().equals("null")) {
				...
				pstmt.setString(11, this.getRateState());
			}
			...
			pstmt.setInt(13, this.getFiscalYear());
			if(this.getFlag() == null || this.getFlag().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getFlag());
			}
			if(this.getBonustype() == null || this.getBonustype().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getBonustype());
			}
			...
			if(this.getRateIntv() == null || this.getRateIntv().equals("null")) {
				...
				pstmt.setString(20, StrTool.space1(this.getRateIntv(), 1));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LMInsuAccRate WHERE  RiskCode = ? AND InsuAccNo = ? AND BalaDate = ? AND RateIntv = ?", 
	...
	public LMInsuAccRateSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LMInsuAccRateSet aLMInsuAccRateSet = new LMInsuAccRateSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LMInsuAccRate");
			LMInsuAccRateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LMInsuAccRateSchema s1 = new LMInsuAccRateSchema();
				s1.setSchema(rs,i);
				aLMInsuAccRateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLMInsuAccRateSet;
	}

	public LMInsuAccRateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMInsuAccRateSet aLMInsuAccRateSet = new LMInsuAccRateSet();
	...
	public LMInsuAccRateSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LMInsuAccRateSet aLMInsuAccRateSet = new LMInsuAccRateSet();
	...
	public LMInsuAccRateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMInsuAccRateSet aLMInsuAccRateSet = new LMInsuAccRateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LMInsuAccRate " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public LMInsuAccRateSet getData()
{
    int tCount = 0;
    LMInsuAccRateSet tLMInsuAccRateSet = new LMInsuAccRateSet();
    LMInsuAccRateSchema tLMInsuAccRateSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LMInsuAccRateDB";
    ...
    try
    {
        ...
        tLMInsuAccRateSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LMInsuAccRateSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LMInsuAccRateSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

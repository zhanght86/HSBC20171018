...
/*
 * <p>ClassName: LZCardTrackDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 单证管理
 */
public class LZCardTrackDB extends LZCardTrackSchema
{
private static Logger logger = Logger.getLogger(LZCardTrackDB.class);
	...
	**/
	...
	// @Constructor
	public LZCardTrackDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LZCardTrack" );
		mflag = true;
	}

	public LZCardTrackDB()
	{
		con = null;
		db = new DBOper( "LZCardTrack" );
		mflag = false;
	}
	...
	{
		LZCardTrackSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LZCardTrackDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LZCardTrack WHERE  CertifyCode = ? AND SubCode = ? AND RiskCode = ? AND RiskVersion = ? AND StartNo = ? AND EndNo = ? AND SendOutCom = ? AND ReceiveCom = ? AND MakeDate = ? AND MakeTime = ?");
			...
			if(this.getMakeTime() == null || this.getMakeTime().equals("null")) {
				...
				pstmt.setString(10, StrTool.space1(this.getMakeTime(), 8));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
		...
		SQLString sqlObj = new SQLString("LZCardTrack");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LZCardTrack SET  CertifyCode = ? , SubCode = ? , RiskCode = ? , RiskVersion = ? , StartNo = ? , EndNo = ? , SendOutCom = ? , ReceiveCom = ? , SumCount = ? , Prem = ? , Amnt = ? , Handler = ? , HandleDate = ? , InvaliDate = ? , TakeBackNo = ? , SaleChnl = ? , StateFlag = ? , OperateFlag = ? , PayFlag = ? , EnterAccFlag = ? , Reason = ? , State = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , ApplyNo = ? , AgentCom = ? WHERE  CertifyCode = ? AND SubCode = ? AND RiskCode = ? AND RiskVersion = ? AND StartNo = ? AND EndNo = ? AND SendOutCom = ? AND ReceiveCom = ? AND MakeDate = ? AND MakeTime = ?");
			if(this.getCertifyCode() == null || this.getCertifyCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCertifyCode());
			...
			if(this.getSendOutCom() == null || this.getSendOutCom().equals("null")) {
				...
				pstmt.setString(36, this.getSendOutCom());
			}
			if(this.getReceiveCom() == null || this.getReceiveCom().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getReceiveCom());
			}
			if(this.getMakeDate() == null || this.getMakeDate().equals("null")) {
				pstmt.setNull(38, 91);
			} else {
			...
			if(this.getMakeTime() == null || this.getMakeTime().equals("null")) {
				...
				pstmt.setString(39, StrTool.space1(this.getMakeTime(), 8));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LZCardTrack(CertifyCode ,SubCode ,RiskCode ,RiskVersion ,StartNo ,EndNo ,SendOutCom ,ReceiveCom ,SumCount ,Prem ,Amnt ,Handler ,HandleDate ,InvaliDate ,TakeBackNo ,SaleChnl ,StateFlag ,OperateFlag ,PayFlag ,EnterAccFlag ,Reason ,State ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,ApplyNo ,AgentCom) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getCertifyCode() == null || this.getCertifyCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCertifyCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LZCardTrack WHERE  CertifyCode = ? AND SubCode = ? AND RiskCode = ? AND RiskVersion = ? AND StartNo = ? AND EndNo = ? AND SendOutCom = ? AND ReceiveCom = ? AND MakeDate = ? AND MakeTime = ?", 
	...
	public LZCardTrackSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardTrackSet aLZCardTrackSet = new LZCardTrackSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LZCardTrack");
			LZCardTrackSchema aSchema = this.getSchema();
			...
			{
				i++;
				LZCardTrackSchema s1 = new LZCardTrackSchema();
				s1.setSchema(rs,i);
				aLZCardTrackSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLZCardTrackSet;

	}

	public LZCardTrackSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardTrackSet aLZCardTrackSet = new LZCardTrackSet();
	...
	public LZCardTrackSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardTrackSet aLZCardTrackSet = new LZCardTrackSet();
	...
	public LZCardTrackSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardTrackSet aLZCardTrackSet = new LZCardTrackSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LZCardTrack " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LZCardTrackSet
 */
public LZCardTrackSet getData()
{
    int tCount = 0;
    ...
    {
        tCount = 1;
        tLZCardTrackSchema = new LZCardTrackSchema();
        tLZCardTrackSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LZCardTrackSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LZCardTrackSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

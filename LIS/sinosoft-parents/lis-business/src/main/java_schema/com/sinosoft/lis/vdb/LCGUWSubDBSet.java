...
/*
 * <p>ClassName: LCGUWSubDBSet </p>
 * <p>Description: DB层多记录数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCGUWSubDBSet extends LCGUWSubSet
{
private static Logger logger = Logger.getLogger(LCGUWSubDBSet.class);
	...
	**/
	...
	// @Constructor
	public LCGUWSubDBSet(Connection tConnection)
	{
		con = tConnection;
		db = new DBOper(con,"LCGUWSub");
		mflag = true;
	}

	public LCGUWSubDBSet()
	{
		db = new DBOper( "LCGUWSub" );
	}
	// @Method
	public boolean deleteSQL()
	{
		...
		else
		{
			...
			tError.moduleName = "LCGUWSubDBSet";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt = con.prepareStatement("DELETE FROM LCGUWSub WHERE  GrpProposalNo = ? AND UWNo = ?");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getGrpProposalNo() == null || this.get(i).getGrpProposalNo().equals("null")) {
				pstmt.setString(1,null);
			} else {
				pstmt.setString(1, this.get(i).getGrpProposalNo());
			}
			pstmt.setInt(2, this.get(i).getUWNo());

		// only for debug purpose
		SQLString sqlObj = new SQLString("LCGUWSub");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.get(i).getUpReport() == null || this.get(i).getUpReport().equals("null")) {
				...
				pstmt.setString(38, this.get(i).getUpReport());
			}
			// set where condition
			if(this.get(i).getGrpProposalNo() == null || this.get(i).getGrpProposalNo().equals("null")) {
				pstmt.setString(39,null);
			} else {
				pstmt.setString(39, this.get(i).getGrpProposalNo());
			}
			pstmt.setInt(40, this.get(i).getUWNo());
...
ex.printStackTrace();
	...
	public boolean insert()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("INSERT INTO LCGUWSub(GrpContNo ,ProposalGrpContNo ,GrpPolNo ,GrpProposalNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,UpReport) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getGrpContNo() == null || this.get(i).getGrpContNo().equals("null")) {
				pstmt.setString(1,null);

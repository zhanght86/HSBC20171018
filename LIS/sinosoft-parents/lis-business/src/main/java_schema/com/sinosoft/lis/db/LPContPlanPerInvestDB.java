...
/*
 * <p>ClassName: LPContPlanPerInvestDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LPContPlanPerInvestDB extends LPContPlanPerInvestSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPContPlanPerInvestDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPContPlanPerInvestSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPContPlanPerInvestDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPContPlanPerInvest WHERE  EdorNo = ? AND EdorType = ? AND PolicyNo = ? AND SysPlanCode = ? AND PlanCode = ? AND RiskCode = ? AND DutyCode = ? AND PayPlanCode = ? AND InsuAccNo = ?", 
	...
	public LPContPlanPerInvestSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPContPlanPerInvestSet aLPContPlanPerInvestSet = new LPContPlanPerInvestSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPContPlanPerInvest");
			LPContPlanPerInvestSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPContPlanPerInvestSchema s1 = new LPContPlanPerInvestSchema();
				s1.setSchema(rs,i);
				aLPContPlanPerInvestSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPContPlanPerInvestSet;
	}

	public LPContPlanPerInvestSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPContPlanPerInvestSet aLPContPlanPerInvestSet = new LPContPlanPerInvestSet();
	...
	public LPContPlanPerInvestSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPContPlanPerInvestSet aLPContPlanPerInvestSet = new LPContPlanPerInvestSet();
	...
	public LPContPlanPerInvestSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPContPlanPerInvestSet aLPContPlanPerInvestSet = new LPContPlanPerInvestSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPContPlanPerInvest " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPContPlanPerInvestSet
 */
public LPContPlanPerInvestSet getData()
{
    int tCount = 0;
    LPContPlanPerInvestSet tLPContPlanPerInvestSet = new LPContPlanPerInvestSet();
    LPContPlanPerInvestSchema tLPContPlanPerInvestSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPContPlanPerInvestDB";
    ...
    {
        tCount = 1;
        tLPContPlanPerInvestSchema = new LPContPlanPerInvestSchema();
        tLPContPlanPerInvestSchema.setSchema(mResultSet, 1);
    ...
    return tLPContPlanPerInvestSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LPContPlanPerInvestSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPContPlanPerInvestSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

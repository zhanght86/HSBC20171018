...
/*
 * <p>ClassName: LOBEdorItemDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LOBEdorItemDB extends LOBEdorItemSchema
{
private static Logger logger = Logger.getLogger(LOBEdorItemDB.class);
	...
	**/
	...
	// @Constructor
	public LOBEdorItemDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LOBEdorItem" );
		mflag = true;
	}

	public LOBEdorItemDB()
	{
		con = null;
		db = new DBOper( "LOBEdorItem" );
		mflag = false;
	}
	...
	{
		LOBEdorItemSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBEdorItemDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LOBEdorItem WHERE  EdorAcceptNo = ? AND EdorNo = ? AND EdorType = ? AND ContNo = ? AND InsuredNo = ? AND PolNo = ?");
		...
		SQLString sqlObj = new SQLString("LOBEdorItem");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LOBEdorItem SET  EdorAcceptNo = ? , EdorNo = ? , EdorAppNo = ? , EdorType = ? , DisplayType = ? , GrpContNo = ? , ContNo = ? , InsuredNo = ? , PolNo = ? , ManageCom = ? , EdorValiDate = ? , EdorAppDate = ? , EdorState = ? , UWFlag = ? , UWOperator = ? , UWDate = ? , UWTime = ? , ChgPrem = ? , ChgAmnt = ? , GetMoney = ? , GetInterest = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Reason = ? , ReasonCode = ? , AppReason = ? , EdorReasonCode = ? , EdorReason = ? , ApproveFlag = ? , ApproveOperator = ? , ApproveDate = ? , ApproveTime = ? , StandbyFlag1 = ? , StandbyFlag2 = ? , StandbyFlag3 = ? , EdorTypeCal = ? , Currency = ? WHERE  EdorAcceptNo = ? AND EdorNo = ? AND EdorType = ? AND ContNo = ? AND InsuredNo = ? AND PolNo = ?");
			if(this.getEdorAcceptNo() == null || this.getEdorAcceptNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorAcceptNo());
			...
			if(this.getEdorTypeCal() == null || this.getEdorTypeCal().equals("null")) {
				...
				pstmt.setString(39, this.getEdorTypeCal());
			}
			...
			if(this.getEdorAcceptNo() == null || this.getEdorAcceptNo().equals("null")) {
				...
				pstmt.setString(41, this.getEdorAcceptNo());
			}
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
			...
			if(this.getInsuredNo() == null || this.getInsuredNo().equals("null")) {
				...
				pstmt.setString(45, this.getInsuredNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(46, 12);
			} else {
				pstmt.setString(46, this.getPolNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LOBEdorItem(EdorAcceptNo ,EdorNo ,EdorAppNo ,EdorType ,DisplayType ,GrpContNo ,ContNo ,InsuredNo ,PolNo ,ManageCom ,EdorValiDate ,EdorAppDate ,EdorState ,UWFlag ,UWOperator ,UWDate ,UWTime ,ChgPrem ,ChgAmnt ,GetMoney ,GetInterest ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,Reason ,ReasonCode ,AppReason ,EdorReasonCode ,EdorReason ,ApproveFlag ,ApproveOperator ,ApproveDate ,ApproveTime ,StandbyFlag1 ,StandbyFlag2 ,StandbyFlag3 ,EdorTypeCal ,Currency) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getEdorAcceptNo() == null || this.getEdorAcceptNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorAcceptNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBEdorItem WHERE  EdorAcceptNo = ? AND EdorNo = ? AND EdorType = ? AND ContNo = ? AND InsuredNo = ? AND PolNo = ?", 
	...
	public LOBEdorItemSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LOBEdorItemSet aLOBEdorItemSet = new LOBEdorItemSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBEdorItem");
			LOBEdorItemSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBEdorItemSchema s1 = new LOBEdorItemSchema();
				s1.setSchema(rs,i);
				aLOBEdorItemSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBEdorItemSet;

	}

	public LOBEdorItemSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBEdorItemSet aLOBEdorItemSet = new LOBEdorItemSet();
	...
	public LOBEdorItemSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LOBEdorItemSet aLOBEdorItemSet = new LOBEdorItemSet();
	...
	public LOBEdorItemSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBEdorItemSet aLOBEdorItemSet = new LOBEdorItemSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBEdorItem " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBEdorItemSet
 */
public LOBEdorItemSet getData()
{
    int tCount = 0;
    LOBEdorItemSet tLOBEdorItemSet = new LOBEdorItemSet();
    LOBEdorItemSchema tLOBEdorItemSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBEdorItemDB";
    ...
    {
        tCount = 1;
        tLOBEdorItemSchema = new LOBEdorItemSchema();
        tLOBEdorItemSchema.setSchema(mResultSet, 1);
        tLOBEdorItemSet.add(tLOBEdorItemSchema);
...
public boolean closeData()
{
	...
	public LOBEdorItemSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBEdorItemSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

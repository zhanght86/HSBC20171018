...
/*
 * <p>ClassName: LPSpecSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPSpecSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPSpecSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPSpecSchema cloned = (LPSpecSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LPSpecSchema 对象给 Schema 赋值
	* @param: aLPSpecSchema LPSpecSchema
	**/
	public void setSchema(LPSpecSchema aLPSpecSchema)
	{
		this.EdorNo = aLPSpecSchema.getEdorNo();
		this.EdorType = aLPSpecSchema.getEdorType();
		this.GrpContNo = aLPSpecSchema.getGrpContNo();
		this.ContNo = aLPSpecSchema.getContNo();
		this.PolNo = aLPSpecSchema.getPolNo();
		this.ProposalNo = aLPSpecSchema.getProposalNo();
		this.PrtSeq = aLPSpecSchema.getPrtSeq();
		this.SerialNo = aLPSpecSchema.getSerialNo();
		this.EndorsementNo = aLPSpecSchema.getEndorsementNo();
		this.SpecType = aLPSpecSchema.getSpecType();
		this.SpecCode = aLPSpecSchema.getSpecCode();
		this.SpecContent = aLPSpecSchema.getSpecContent();
		this.PrtFlag = aLPSpecSchema.getPrtFlag();
		this.BackupType = aLPSpecSchema.getBackupType();
		this.Operator = aLPSpecSchema.getOperator();
		this.MakeDate = fDate.getDate( aLPSpecSchema.getMakeDate());
		this.MakeTime = aLPSpecSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLPSpecSchema.getModifyDate());
		this.ModifyTime = aLPSpecSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPSpec表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPSpecSchema";
	...
	public LPSpecSchema getSchema()
	{
		LPSpecSchema aLPSpecSchema = new LPSpecSchema();
		aLPSpecSchema.setSchema(this);
		return aLPSpecSchema;
	}

	public LPSpecDB getDB()
	{
		LPSpecDB aDBOper = new LPSpecDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPSpec描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPSpec>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			EndorsementNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			SpecType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			SpecCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			SpecContent = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			PrtFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			BackupType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPSpecSchema other = (LPSpecSchema)otherObject;
		return

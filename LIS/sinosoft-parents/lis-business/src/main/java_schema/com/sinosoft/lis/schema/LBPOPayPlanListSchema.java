...
import com.sinosoft.lis.db.LBPOPayPlanListDB;

/*
 * <p>ClassName: LBPOPayPlanListSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LBPOPayPlanListSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBPOPayPlanListSchema.class);
	// @Field
	/** 流水号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBPOPayPlanListSchema cloned = (LBPOPayPlanListSchema)super.clone();
	...
	/**
	* 使用另外一个 LBPOPayPlanListSchema 对象给 Schema 赋值
	* @param: aLBPOPayPlanListSchema LBPOPayPlanListSchema
	**/
	public void setSchema(LBPOPayPlanListSchema aLBPOPayPlanListSchema)
	{
		this.SerialNo = aLBPOPayPlanListSchema.getSerialNo();
		this.GrpPropNo = aLBPOPayPlanListSchema.getGrpPropNo();
		this.ActivityID = aLBPOPayPlanListSchema.getActivityID();
		this.BatchNo = aLBPOPayPlanListSchema.getBatchNo();
		this.OrderNo = aLBPOPayPlanListSchema.getOrderNo();
		this.PayPlanID = aLBPOPayPlanListSchema.getPayPlanID();
		this.InsuredID = aLBPOPayPlanListSchema.getInsuredID();
		this.DutyCode = aLBPOPayPlanListSchema.getDutyCode();
		this.DutyName = aLBPOPayPlanListSchema.getDutyName();
		this.PayPlanCode = aLBPOPayPlanListSchema.getPayPlanCode();
		this.PayPlanName = aLBPOPayPlanListSchema.getPayPlanName();
		this.PayPlanPrem = aLBPOPayPlanListSchema.getPayPlanPrem();
		this.State = aLBPOPayPlanListSchema.getState();
		this.Reason = aLBPOPayPlanListSchema.getReason();
		this.MakeOperator = aLBPOPayPlanListSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLBPOPayPlanListSchema.getMakeDate());
		this.MakeTime = aLBPOPayPlanListSchema.getMakeTime();
		this.ModifyOperator = aLBPOPayPlanListSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLBPOPayPlanListSchema.getModifyDate());
		this.ModifyTime = aLBPOPayPlanListSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBPOPayPlanList表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBPOPayPlanListSchema";
	...
	public LBPOPayPlanListSchema getSchema()
	{
		LBPOPayPlanListSchema aLBPOPayPlanListSchema = new LBPOPayPlanListSchema();
		aLBPOPayPlanListSchema.setSchema(this);
		return aLBPOPayPlanListSchema;
	}

	public LBPOPayPlanListDB getDB()
	{
		LBPOPayPlanListDB aDBOper = new LBPOPayPlanListDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBPOPayPlanList>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBPOPayPlanListSchema other = (LBPOPayPlanListSchema)otherObject;
		return

...
/*
 * <p>ClassName: LCGCUWMasterDBSet </p>
 * <p>Description: DB层多记录数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCGCUWMasterDBSet extends LCGCUWMasterSet
{
private static Logger logger = Logger.getLogger(LCGCUWMasterDBSet.class);
	...
	**/
	...
	// @Constructor
	public LCGCUWMasterDBSet(Connection tConnection)
	{
		con = tConnection;
		db = new DBOper(con,"LCGCUWMaster");
		mflag = true;
	}

	public LCGCUWMasterDBSet()
	{
		db = new DBOper( "LCGCUWMaster" );
	}
	// @Method
	public boolean deleteSQL()
	{
		...
		else
		{
			...
			tError.moduleName = "LCGCUWMasterDBSet";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt = con.prepareStatement("DELETE FROM LCGCUWMaster WHERE  ProposalGrpContNo = ?");
		...
		SQLString sqlObj = new SQLString("LCGCUWMaster");
...
ex.printStackTrace();
	...
	public boolean update()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("UPDATE LCGCUWMaster SET  GrpContNo = ? , ProposalGrpContNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , UpReport = ? WHERE  ProposalGrpContNo = ?");
            for (int i = 1; i <= tCount; i++)
            {
			...
			if(this.get(i).getInsuredName() == null || this.get(i).getInsuredName().equals("null")) {
				...
				pstmt.setString(5, this.get(i).getInsuredName());
			}
			...
			if(this.get(i).getPassFlag() == null || this.get(i).getPassFlag().equals("null")) {
				...
				pstmt.setString(10, this.get(i).getPassFlag());
			}
			...
			if(this.get(i).getUWIdea() == null || this.get(i).getUWIdea().equals("null")) {
				...
				pstmt.setString(18, this.get(i).getUWIdea());
			}
			if(this.get(i).getUpReportContent() == null || this.get(i).getUpReportContent().equals("null")) {
				pstmt.setString(19,null);
			} else {
				pstmt.setString(19, this.get(i).getUpReportContent());
			}
			...
			if(this.get(i).getProposalGrpContNo() == null || this.get(i).getProposalGrpContNo().equals("null")) {
				...
				pstmt.setString(37, this.get(i).getProposalGrpContNo());
			}
...
ex.printStackTrace();
	...
	public boolean insert()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("INSERT INTO LCGCUWMaster(GrpContNo ,ProposalGrpContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,UpReport) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getGrpContNo() == null || this.get(i).getGrpContNo().equals("null")) {
				pstmt.setString(1,null);

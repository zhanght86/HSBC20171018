...
/*
 * <p>ClassName: LCClaimFactorControlDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LCClaimFactorControlDB extends LCClaimFactorControlSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCClaimFactorControlDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCClaimFactorControlSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCClaimFactorControlDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getFactorAttribute() == null || this.getFactorAttribute().equals("null")) {
				...
				pstmt.setString(6, this.getFactorAttribute());
			}
			...
			pstmt.setInt(9, this.getInerSerialNo());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getParams() == null || this.getParams().equals("null")) {
				...
				pstmt.setString(11, this.getParams());
			}
			...
			if(this.getFactorAttribute() == null || this.getFactorAttribute().equals("null")) {
				...
				pstmt.setString(23, this.getFactorAttribute());
			}
			if(this.getFactorType() == null || this.getFactorType().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getFactorType());
			}
			if(this.getFactorCode() == null || this.getFactorCode().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
				pstmt.setString(25, this.getFactorCode());
			}
			pstmt.setInt(26, this.getInerSerialNo());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCClaimFactorControl WHERE  PolicyNo = ? AND SysPlanCode = ? AND PlanCode = ? AND RiskCode = ? AND DutyCode = ? AND FactorAttribute = ? AND FactorType = ? AND FactorCode = ? AND InerSerialNo = ?", 
	...
	public LCClaimFactorControlSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCClaimFactorControlSet aLCClaimFactorControlSet = new LCClaimFactorControlSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCClaimFactorControl");
			LCClaimFactorControlSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCClaimFactorControlSchema s1 = new LCClaimFactorControlSchema();
				s1.setSchema(rs,i);
				aLCClaimFactorControlSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCClaimFactorControlSet;
	}

	public LCClaimFactorControlSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCClaimFactorControlSet aLCClaimFactorControlSet = new LCClaimFactorControlSet();
	...
	public LCClaimFactorControlSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCClaimFactorControlSet aLCClaimFactorControlSet = new LCClaimFactorControlSet();
	...
	public LCClaimFactorControlSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCClaimFactorControlSet aLCClaimFactorControlSet = new LCClaimFactorControlSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCClaimFactorControl " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCClaimFactorControlSet
 */
public LCClaimFactorControlSet getData()
{
    int tCount = 0;
    LCClaimFactorControlSet tLCClaimFactorControlSet = new LCClaimFactorControlSet();
    LCClaimFactorControlSchema tLCClaimFactorControlSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCClaimFactorControlDB";
    ...
    {
        tCount = 1;
        tLCClaimFactorControlSchema = new LCClaimFactorControlSchema();
        tLCClaimFactorControlSchema.setSchema(mResultSet, 1);
    ...
    return tLCClaimFactorControlSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LCClaimFactorControlSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCClaimFactorControlSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

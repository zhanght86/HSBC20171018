...
{
private static Logger logger = Logger.getLogger(FIRuleDealErrLogSchema.class);
	...
	**/
	public void setSchema(FIRuleDealErrLogSchema aFIRuleDealErrLogSchema)
	{
		...
		this.RuleDealBatchNo = aFIRuleDealErrLogSchema.getRuleDealBatchNo();
		this.DataSourceBatchNo = aFIRuleDealErrLogSchema.getDataSourceBatchNo();
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的FIRuleDealErrLog表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			RulePlanID = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			DealState = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			CertificateID = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			businessno = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
		}

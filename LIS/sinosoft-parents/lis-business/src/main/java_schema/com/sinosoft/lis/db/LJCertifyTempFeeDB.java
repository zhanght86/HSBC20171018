...
/*
 * <p>ClassName: LJCertifyTempFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统未整理模型
 */
public class LJCertifyTempFeeDB extends LJCertifyTempFeeSchema
{
private static Logger logger = Logger.getLogger(LJCertifyTempFeeDB.class);
	...
	**/
	...
	// @Constructor
	public LJCertifyTempFeeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJCertifyTempFeeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJCertifyTempFeeDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getIDNo() == null || this.getIDNo().equals("null")) {
				...
				pstmt.setString(2, this.getIDNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPolicyCom() == null || this.getPolicyCom().equals("null")) {
				...
				pstmt.setString(8, this.getPolicyCom());
			}
			if(this.getPayDate() == null || this.getPayDate().equals("null")) {
				pstmt.setNull(9, 91);
			} else {
				pstmt.setDate(9, Date.valueOf(this.getPayDate()));
			}
			...
			if(this.getConfFlag() == null || this.getConfFlag().equals("null")) {
				...
				pstmt.setString(11, this.getConfFlag());
			}
			...
			if(this.getStandbyFlag4() == null || this.getStandbyFlag4().equals("null")) {
				...
				pstmt.setString(20, this.getStandbyFlag4());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJCertifyTempFee WHERE  GetNoticeNo = ? AND IDNo = ?", 
	...
	public LJCertifyTempFeeSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LJCertifyTempFeeSet aLJCertifyTempFeeSet = new LJCertifyTempFeeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJCertifyTempFee");
			LJCertifyTempFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJCertifyTempFeeSchema s1 = new LJCertifyTempFeeSchema();
				s1.setSchema(rs,i);
				aLJCertifyTempFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJCertifyTempFeeSet;

	}

	public LJCertifyTempFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJCertifyTempFeeSet aLJCertifyTempFeeSet = new LJCertifyTempFeeSet();
	...
	public LJCertifyTempFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LJCertifyTempFeeSet aLJCertifyTempFeeSet = new LJCertifyTempFeeSet();
	...
	public LJCertifyTempFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJCertifyTempFeeSet aLJCertifyTempFeeSet = new LJCertifyTempFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJCertifyTempFee " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJCertifyTempFeeSet
 */
public LJCertifyTempFeeSet getData()
{
    int tCount = 0;
    LJCertifyTempFeeSet tLJCertifyTempFeeSet = new LJCertifyTempFeeSet();
    LJCertifyTempFeeSchema tLJCertifyTempFeeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJCertifyTempFeeDB";
    ...
    {
        tCount = 1;
        tLJCertifyTempFeeSchema = new LJCertifyTempFeeSchema();
        tLJCertifyTempFeeSchema.setSchema(mResultSet, 1);
        tLJCertifyTempFeeSet.add(tLJCertifyTempFeeSchema);
...
public boolean closeData()
{
	...
	public LJCertifyTempFeeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LJCertifyTempFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

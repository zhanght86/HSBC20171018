...
/*
 * <p>ClassName: PD_LMRiskRoleDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 产品定义平台_PDM
 */
public class PD_LMRiskRoleDB extends PD_LMRiskRoleSchema
{
private static Logger logger = Logger.getLogger(PD_LMRiskRoleDB.class);
	...
	**/
	...
	// @Constructor
	public PD_LMRiskRoleDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		PD_LMRiskRoleSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "PD_LMRiskRoleDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getRiskRoleSex() == null || this.getRiskRoleSex().equals("null")) {
				...
				pstmt.setString(3, StrTool.space1(this.getRiskRoleSex(), 1));
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getMinRnewAgeFlag() == null || this.getMinRnewAgeFlag().equals("null")) {
				...
				pstmt.setString(21, this.getMinRnewAgeFlag());
			}
			pstmt.setInt(22, this.getMinRnewAge());
			if(this.getMAXRnewAgeFlag() == null || this.getMAXRnewAgeFlag().equals("null")) {
				pstmt.setNull(23, 1);
			} else {
				pstmt.setString(23, this.getMAXRnewAgeFlag());
			}
			pstmt.setInt(24, this.getMAXRnewAge());
			// set where condition
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
			...
			if(this.getRiskRole() == null || this.getRiskRole().equals("null")) {
				...
				pstmt.setString(26, this.getRiskRole());
			}
			if(this.getRiskRoleSex() == null || this.getRiskRoleSex().equals("null")) {
				pstmt.setNull(27, 1);
			} else {
				pstmt.setString(27, StrTool.space1(this.getRiskRoleSex(), 1));
			}
			if(this.getRiskRoleNo() == null || this.getRiskRoleNo().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getRiskRoleNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM PD_LMRiskRole WHERE  RiskCode = ? AND RiskRole = ? AND RiskRoleSex = ? AND RiskRoleNo = ?", 
	...
	public PD_LMRiskRoleSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		PD_LMRiskRoleSet aPD_LMRiskRoleSet = new PD_LMRiskRoleSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("PD_LMRiskRole");
			PD_LMRiskRoleSchema aSchema = this.getSchema();
			...
			{
				i++;
				PD_LMRiskRoleSchema s1 = new PD_LMRiskRoleSchema();
				s1.setSchema(rs,i);
				aPD_LMRiskRoleSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aPD_LMRiskRoleSet;

	}

	public PD_LMRiskRoleSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskRoleSet aPD_LMRiskRoleSet = new PD_LMRiskRoleSet();
	...
	public PD_LMRiskRoleSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		PD_LMRiskRoleSet aPD_LMRiskRoleSet = new PD_LMRiskRoleSet();
	...
	public PD_LMRiskRoleSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMRiskRoleSet aPD_LMRiskRoleSet = new PD_LMRiskRoleSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update PD_LMRiskRole " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return PD_LMRiskRoleSet
 */
public PD_LMRiskRoleSet getData()
{
    int tCount = 0;
    PD_LMRiskRoleSet tPD_LMRiskRoleSet = new PD_LMRiskRoleSet();
    PD_LMRiskRoleSchema tPD_LMRiskRoleSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "PD_LMRiskRoleDB";
    ...
    {
        tCount = 1;
        tPD_LMRiskRoleSchema = new PD_LMRiskRoleSchema();
        tPD_LMRiskRoleSchema.setSchema(mResultSet, 1);
        tPD_LMRiskRoleSet.add(tPD_LMRiskRoleSchema);
...
public boolean closeData()
{
	...
	public PD_LMRiskRoleSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public PD_LMRiskRoleSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

 ...
 * @Database: 再保
 */
public class LPLoanSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPLoanSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPLoanSchema cloned = (LPLoanSchema)super.clone();
	...
	* @param: aLPLoanSchema LPLoanSchema
	**/
	public void setSchema(LPLoanSchema aLPLoanSchema)
	{
		this.ContNo = aLPLoanSchema.getContNo();
		...
		this.PreInterest = aLPLoanSchema.getPreInterest();
		this.CurLoanMoney = aLPLoanSchema.getCurLoanMoney();
		this.LastSumLoanMoney = aLPLoanSchema.getLastSumLoanMoney();
		this.LoanToDate = fDate.getDate( aLPLoanSchema.getLoanToDate());
		this.NewLoanDate = fDate.getDate( aLPLoanSchema.getNewLoanDate());
		this.ZqFlag = aLPLoanSchema.getZqFlag();
		this.ClmNo = aLPLoanSchema.getClmNo();
		this.Currency = aLPLoanSchema.getCurrency();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPLoan表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			PreInterest = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,26,SysConst.PACKAGESPILTER))).doubleValue();
			CurLoanMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,27,SysConst.PACKAGESPILTER))).doubleValue();
			LastSumLoanMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,28,SysConst.PACKAGESPILTER))).doubleValue();
			LoanToDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 29,SysConst.PACKAGESPILTER));
			NewLoanDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 30,SysConst.PACKAGESPILTER));
			ZqFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 31, SysConst.PACKAGESPILTER );
			ClmNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 32, SysConst.PACKAGESPILTER );

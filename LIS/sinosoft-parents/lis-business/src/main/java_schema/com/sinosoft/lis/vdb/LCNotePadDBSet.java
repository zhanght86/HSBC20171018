...
/*
 * <p>ClassName: LCNotePadDBSet </p>
 * <p>Description: DB层多记录数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCNotePadDBSet extends LCNotePadSet
{
private static Logger logger = Logger.getLogger(LCNotePadDBSet.class);
	...
	**/
	...
	// @Constructor
	public LCNotePadDBSet(Connection tConnection)
	{
		con = tConnection;
		db = new DBOper(con,"LCNotePad");
		mflag = true;
	}

	public LCNotePadDBSet()
	{
		db = new DBOper( "LCNotePad" );
	}
	// @Method
	public boolean deleteSQL()
	{
		...
		else
		{
			...
			tError.moduleName = "LCNotePadDBSet";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt = con.prepareStatement("DELETE FROM LCNotePad WHERE  ProposalContNo = ? AND SerialNo = ?");
		...
		SQLString sqlObj = new SQLString("LCNotePad");
...
ex.printStackTrace();
	...
	public boolean update()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("UPDATE LCNotePad SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , SerialNo = ? , OperatePos = ? , PrtNo = ? , NotePadCont = ? , Operator = ? , ManageCom = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? WHERE  ProposalContNo = ? AND SerialNo = ?");
            for (int i = 1; i <= tCount; i++)
            {
...
ex.printStackTrace();
	...
	public boolean insert()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("INSERT INTO LCNotePad(GrpContNo ,ContNo ,ProposalContNo ,SerialNo ,OperatePos ,PrtNo ,NotePadCont ,Operator ,ManageCom ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getGrpContNo() == null || this.get(i).getGrpContNo().equals("null")) {
				pstmt.setString(1,null);

...
/*
 * <p>ClassName: LDCodeExpDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LDCodeExpDB extends LDCodeExpSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDCodeExpDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDCodeExpSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getCodeExp() == null || this.getCodeExp().equals("null")) {
				pstmt.setNull(2, 12);
			} else {
				pstmt.setString(2, this.getCodeExp());
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setInt(9, this.getOrderNo());
			...
			if(this.getCodeType() == null || this.getCodeType().equals("null")) {
				...
				pstmt.setString(16, this.getCodeType());
			}
			if(this.getCodeExp() == null || this.getCodeExp().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getCodeExp());
			}
			if(this.getCode() == null || this.getCode().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
				pstmt.setString(18, this.getCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDCodeExp WHERE  CodeType = ? AND CodeExp = ? AND Code = ?", 
	...
	public LDCodeExpSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDCodeExpSet aLDCodeExpSet = new LDCodeExpSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDCodeExp");
			LDCodeExpSchema aSchema = this.getSchema();
			...
			{
				i++;
				LDCodeExpSchema s1 = new LDCodeExpSchema();
				s1.setSchema(rs,i);
				aLDCodeExpSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLDCodeExpSet;
	}

	public LDCodeExpSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDCodeExpSet aLDCodeExpSet = new LDCodeExpSet();
	...
	public LDCodeExpSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDCodeExpSet aLDCodeExpSet = new LDCodeExpSet();
	...
	public LDCodeExpSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDCodeExpSet aLDCodeExpSet = new LDCodeExpSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDCodeExp " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LDCodeExpSet
 */
public LDCodeExpSet getData()
{
    int tCount = 0;
    LDCodeExpSet tLDCodeExpSet = new LDCodeExpSet();
    LDCodeExpSchema tLDCodeExpSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDCodeExpDB";
    ...
    {
        tCount = 1;
        tLDCodeExpSchema = new LDCodeExpSchema();
        tLDCodeExpSchema.setSchema(mResultSet, 1);
        tLDCodeExpSet.add(tLDCodeExpSchema);
...
public boolean closeData()
{
	...
	public LDCodeExpSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDCodeExpSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

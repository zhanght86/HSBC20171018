...
/*
 * <p>ClassName: LJPremMatchPayLogDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LJPremMatchPayLogDB extends LJPremMatchPayLogSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LJPremMatchPayLogDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJPremMatchPayLogSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJPremMatchPayLogDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getMainBussNo() == null || this.getMainBussNo().equals("null")) {
				...
				pstmt.setString(4, this.getMainBussNo());
			}
			if(this.getSubBussNo() == null || this.getSubBussNo().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getSubBussNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getInsurancecom() == null || this.getInsurancecom().equals("null")) {
				...
				pstmt.setString(8, this.getInsurancecom());
			}
			if(this.getInsurancecomName() == null || this.getInsurancecomName().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getInsurancecomName());
			}
			if(this.getBussDate() == null || this.getBussDate().equals("null")) {
				pstmt.setNull(10, 91);
			} else {
				pstmt.setDate(10, Date.valueOf(this.getBussDate()));
			}
			pstmt.setDouble(11, this.getDuePayMoney());
			pstmt.setDouble(12, this.getCurOutPayMoney());
			...
			if(this.getMainBussNo() == null || this.getMainBussNo().equals("null")) {
				...
				pstmt.setString(24, this.getMainBussNo());
			}
			if(this.getSubBussNo() == null || this.getSubBussNo().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
				pstmt.setString(25, this.getSubBussNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJPremMatchPayLog WHERE  MatchSerialNo = ? AND MatchNode = ? AND BussType = ? AND MainBussNo = ? AND SubBussNo = ?", 
	...
	public LJPremMatchPayLogSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJPremMatchPayLogSet aLJPremMatchPayLogSet = new LJPremMatchPayLogSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJPremMatchPayLog");
			LJPremMatchPayLogSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJPremMatchPayLogSchema s1 = new LJPremMatchPayLogSchema();
				s1.setSchema(rs,i);
				aLJPremMatchPayLogSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJPremMatchPayLogSet;
	}

	public LJPremMatchPayLogSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJPremMatchPayLogSet aLJPremMatchPayLogSet = new LJPremMatchPayLogSet();
	...
	public LJPremMatchPayLogSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJPremMatchPayLogSet aLJPremMatchPayLogSet = new LJPremMatchPayLogSet();
	...
	public LJPremMatchPayLogSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJPremMatchPayLogSet aLJPremMatchPayLogSet = new LJPremMatchPayLogSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJPremMatchPayLog " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJPremMatchPayLogSet
 */
public LJPremMatchPayLogSet getData()
{
    int tCount = 0;
    LJPremMatchPayLogSet tLJPremMatchPayLogSet = new LJPremMatchPayLogSet();
    LJPremMatchPayLogSchema tLJPremMatchPayLogSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJPremMatchPayLogDB";
    ...
    {
        tCount = 1;
        tLJPremMatchPayLogSchema = new LJPremMatchPayLogSchema();
        tLJPremMatchPayLogSchema.setSchema(mResultSet, 1);
        tLJPremMatchPayLogSet.add(tLJPremMatchPayLogSchema);
...
public boolean closeData()
{
	...
	public LJPremMatchPayLogSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LJPremMatchPayLogSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

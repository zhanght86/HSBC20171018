 ...
 * @Database: 承保核心
 */
public class LCBnfSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LCBnfSchema.class);
	// @Field
	/** 合同号码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LCBnfSchema cloned = (LCBnfSchema)super.clone();
	...
	*/
	...
	public void setMobile(String aMobile)
	{
		if(aMobile!=null && aMobile.length()>20)
			throw new IllegalArgumentException("手机Mobile值"+aMobile+"的长度"+aMobile.length()+"大于最大值20");
		Mobile = aMobile;
	}
	...
	* @param: aLCBnfSchema LCBnfSchema
	**/
	public void setSchema(LCBnfSchema aLCBnfSchema)
	{
		...
		this.PostalAddress = aLCBnfSchema.getPostalAddress();
		this.ZipCode = aLCBnfSchema.getZipCode();
		this.Remark = aLCBnfSchema.getRemark();
		this.BankCode = aLCBnfSchema.getBankCode();
		this.BankAccNo = aLCBnfSchema.getBankAccNo();
		this.AccName = aLCBnfSchema.getAccName();
		this.IDExpDate = fDate.getDate( aLCBnfSchema.getIDExpDate());
		this.Mobile = aLCBnfSchema.getMobile();
		this.ManageCom = aLCBnfSchema.getManageCom();
		this.ComCode = aLCBnfSchema.getComCode();
		this.ModifyOperator = aLCBnfSchema.getModifyOperator();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LCBnf表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理

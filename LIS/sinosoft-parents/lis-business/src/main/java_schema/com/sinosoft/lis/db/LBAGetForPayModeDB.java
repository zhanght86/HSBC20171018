...
/*
 * <p>ClassName: LBAGetForPayModeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务付费
 */
public class LBAGetForPayModeDB extends LBAGetForPayModeSchema
{
	private static Logger logger = Logger.getLogger(LBAGetForPayModeDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LBAGetForPayModeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBAGetForPayModeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBAGetForPayModeDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(7, this.getSumGetMoney());
			...
			if(this.getShouldDate() == null || this.getShouldDate().equals("null")) {
				...
				pstmt.setDate(9, Date.valueOf(this.getShouldDate()));
			}
			...
			if(this.getDrawer() == null || this.getDrawer().equals("null")) {
				...
				pstmt.setString(17, this.getDrawer());
			}
			if(this.getDrawerID() == null || this.getDrawerID().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
				pstmt.setString(18, this.getDrawerID());
			}
			...
			if(this.getBankOnTheWayFlag() == null || this.getBankOnTheWayFlag().equals("null")) {
				...
				pstmt.setString(25, this.getBankOnTheWayFlag());
			}
			if(this.getBankSuccFlag() == null || this.getBankSuccFlag().equals("null")) {
				pstmt.setNull(26, 1);
			} else {
				pstmt.setString(26, this.getBankSuccFlag());
			}
			pstmt.setInt(27, this.getSendBankCount());
			...
			if(this.getAgentType() == null || this.getAgentType().equals("null")) {
				...
				pstmt.setString(30, this.getAgentType());
			}
			...
			if(this.getAgentGroup() == null || this.getAgentGroup().equals("null")) {
				...
				pstmt.setString(32, this.getAgentGroup());
			}
			...
			if(this.getStartGetDate() == null || this.getStartGetDate().equals("null")) {
				...
				pstmt.setDate(34, Date.valueOf(this.getStartGetDate()));
			}
			// set where condition
			if(this.getActuGetNo() == null || this.getActuGetNo().equals("null")) {
				pstmt.setNull(35, 12);
			...
			if(this.getChangeSerialNo() == null || this.getChangeSerialNo().equals("null")) {
				...
				pstmt.setString(36, this.getChangeSerialNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBAGetForPayMode WHERE  ActuGetNo = ? AND ChangeSerialNo = ?", 
	...
	public LBAGetForPayModeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LBAGetForPayModeSet aLBAGetForPayModeSet = new LBAGetForPayModeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBAGetForPayMode");
			LBAGetForPayModeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBAGetForPayModeSchema s1 = new LBAGetForPayModeSchema();
				s1.setSchema(rs,i);
				aLBAGetForPayModeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBAGetForPayModeSet;
	}

	public LBAGetForPayModeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBAGetForPayModeSet aLBAGetForPayModeSet = new LBAGetForPayModeSet();
	...
	public LBAGetForPayModeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LBAGetForPayModeSet aLBAGetForPayModeSet = new LBAGetForPayModeSet();
	...
	public LBAGetForPayModeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBAGetForPayModeSet aLBAGetForPayModeSet = new LBAGetForPayModeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBAGetForPayMode " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBAGetForPayModeSet
 */
public LBAGetForPayModeSet getData()
{
    int tCount = 0;
    LBAGetForPayModeSet tLBAGetForPayModeSet = new LBAGetForPayModeSet();
    LBAGetForPayModeSchema tLBAGetForPayModeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBAGetForPayModeDB";
    ...
    {
        tCount = 1;
        tLBAGetForPayModeSchema = new LBAGetForPayModeSchema();
        tLBAGetForPayModeSchema.setSchema(mResultSet, 1);
        tLBAGetForPayModeSet.add(tLBAGetForPayModeSchema);
...
public boolean closeData()
{
	...
	public LBAGetForPayModeSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LBAGetForPayModeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

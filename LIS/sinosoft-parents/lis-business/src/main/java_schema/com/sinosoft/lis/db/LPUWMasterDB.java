...
/*
 * <p>ClassName: LPUWMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPUWMasterDB extends LPUWMasterSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPUWMasterDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LPUWMaster" );
		mflag = true;
	}
	...
	{
		LPUWMasterSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPUWMasterDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LPUWMaster WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ?");
		...
		SQLString sqlObj = new SQLString("LPUWMaster");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LPUWMaster SET  EdorNo = ? , EdorType = ? , GrpContNo = ? , ContNo = ? , ProposalContNo = ? , PolNo = ? , ProposalNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , AutoUWDate = ? , UWConclu = ? , UWDate = ? , UpReport = ? , ComCode = ? , ModifyOperator = ? WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ?");
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorNo());
			...
			if(this.getUWIdea() == null || this.getUWIdea().equals("null")) {
				...
				pstmt.setString(23, this.getUWIdea());
			}
			if(this.getUpReportContent() == null || this.getUpReportContent().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getUpReportContent());
			}
			...
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				...
				pstmt.setString(49, this.getEdorNo());
			}
			if(this.getEdorType() == null || this.getEdorType().equals("null")) {
				pstmt.setNull(50, 12);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LPUWMaster VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPUWMaster WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ?", 
	...
	public LPUWMasterSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPUWMasterSet aLPUWMasterSet = new LPUWMasterSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPUWMaster");
			LPUWMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPUWMasterSchema s1 = new LPUWMasterSchema();
				s1.setSchema(rs,i);
				aLPUWMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPUWMasterSet;
	}

	public LPUWMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPUWMasterSet aLPUWMasterSet = new LPUWMasterSet();
	...
	public LPUWMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPUWMasterSet aLPUWMasterSet = new LPUWMasterSet();
	...
	public LPUWMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPUWMasterSet aLPUWMasterSet = new LPUWMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPUWMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPUWMasterSet
 */
public LPUWMasterSet getData()
{
    int tCount = 0;
    LPUWMasterSet tLPUWMasterSet = new LPUWMasterSet();
    LPUWMasterSchema tLPUWMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPUWMasterDB";
    ...
    try
    {
        ...
        tLPUWMasterSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LPUWMasterSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPUWMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

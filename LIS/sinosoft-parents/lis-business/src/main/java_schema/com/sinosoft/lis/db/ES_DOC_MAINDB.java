...
/*
 * <p>ClassName: ES_DOC_MAINDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: bug4
 */
public class ES_DOC_MAINDB extends ES_DOC_MAINSchema
{
	private static Logger logger = Logger.getLogger(ES_DOC_MAINDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public ES_DOC_MAINDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "ES_DOC_MAIN" );
		mflag = true;
	}

	public ES_DOC_MAINDB()
	{
		con = null;
		db = new DBOper( "ES_DOC_MAIN" );
		mflag = false;
	}
	...
	{
		ES_DOC_MAINSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "ES_DOC_MAINDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM ES_DOC_MAIN WHERE  DocID = ?");
		...
		SQLString sqlObj = new SQLString("ES_DOC_MAIN");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE ES_DOC_MAIN SET  DocID = ? , DocCode = ? , BussType = ? , SubType = ? , NumPages = ? , DocFlag = ? , DocRemark = ? , ScanOperator = ? , ManageCom = ? , InputState = ? , Operator = ? , InputStartDate = ? , InputStartTime = ? , InputEndDate = ? , InputEndTime = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Version = ? , ScanNo = ? , PrintCode = ? WHERE  DocID = ?");
			pstmt.setInt(1, this.getDocID());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO ES_DOC_MAIN VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			pstmt.setInt(1, this.getDocID());
			if(this.getDocCode() == null || this.getDocCode().equals("null")) {
				pstmt.setNull(2, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM ES_DOC_MAIN WHERE  DocID = ?", 
	...
	public ES_DOC_MAINSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("ES_DOC_MAIN");
			ES_DOC_MAINSchema aSchema = this.getSchema();
			...
			{
				i++;
				ES_DOC_MAINSchema s1 = new ES_DOC_MAINSchema();
				s1.setSchema(rs,i);
				aES_DOC_MAINSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aES_DOC_MAINSet;
	}

	public ES_DOC_MAINSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		ES_DOC_MAINSet aES_DOC_MAINSet = new ES_DOC_MAINSet();
	...
	public ES_DOC_MAINSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		ES_DOC_MAINSet aES_DOC_MAINSet = new ES_DOC_MAINSet();
	...
	public ES_DOC_MAINSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		ES_DOC_MAINSet aES_DOC_MAINSet = new ES_DOC_MAINSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update ES_DOC_MAIN " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public ES_DOC_MAINSet getData()
{
    int tCount = 0;
    ES_DOC_MAINSet tES_DOC_MAINSet = new ES_DOC_MAINSet();
    ES_DOC_MAINSchema tES_DOC_MAINSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "ES_DOC_MAINDB";
    ...
    {
        tCount = 1;
        tES_DOC_MAINSchema = new ES_DOC_MAINSchema();
        tES_DOC_MAINSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public ES_DOC_MAINSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public ES_DOC_MAINSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

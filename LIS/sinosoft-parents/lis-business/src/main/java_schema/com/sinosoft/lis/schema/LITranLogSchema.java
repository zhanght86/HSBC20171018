 ...
 * @Database: 泰康未整理PDM
 */
public class LITranLogSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LITranLogSchema.class);
	// @Field
	/** 批次号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LITranLogSchema cloned = (LITranLogSchema)super.clone();
	...
	{
		if(aFileName!=null && aFileName.length()>30)
			throw new IllegalArgumentException("文件名FileName值"+aFileName+"的长度"+aFileName.length()+"大于最大值30");
		FileName = aFileName;
	}
	...
	{
		if(aFilePath!=null && aFilePath.length()>150)
			throw new IllegalArgumentException("文件路径FilePath值"+aFilePath+"的长度"+aFilePath.length()+"大于最大值150");
		FilePath = aFilePath;
	}
	/**
	* 0:传送成功 1:传送失败
	*/
	...
	public void setFlag(String aFlag)
	{
		if(aFlag!=null && aFlag.length()>10)
			throw new IllegalArgumentException("标志Flag值"+aFlag+"的长度"+aFlag.length()+"大于最大值10");
		Flag = aFlag;
	}
	...
	* @param: aLITranLogSchema LITranLogSchema
	**/
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LITranLog表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public String encode()
	{
		...
		strReturn.append( ChgData.chgData(NumBuss));strReturn.append(SysConst.PACKAGESPILTER);
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			NumBuss= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,2,SysConst.PACKAGESPILTER))).intValue();
			...
			Flag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("NumBuss"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(NumBuss));
		}

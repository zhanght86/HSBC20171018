...
/*
 * <p>ClassName: RnewIndUWSubDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 续期续保二核
 */
public class RnewIndUWSubDB extends RnewIndUWSubSchema
{
private static Logger logger = Logger.getLogger(RnewIndUWSubDB.class);
	...
	**/
	...
	// @Constructor
	public RnewIndUWSubDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		RnewIndUWSubSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "RnewIndUWSubDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM RnewIndUWSub WHERE  ContNo = ? AND UWNo = ? AND InsuredNo = ?", 
	...
	public RnewIndUWSubSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		RnewIndUWSubSet aRnewIndUWSubSet = new RnewIndUWSubSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("RnewIndUWSub");
			RnewIndUWSubSchema aSchema = this.getSchema();
			...
			{
				i++;
				RnewIndUWSubSchema s1 = new RnewIndUWSubSchema();
				s1.setSchema(rs,i);
				aRnewIndUWSubSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aRnewIndUWSubSet;

	}

	public RnewIndUWSubSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RnewIndUWSubSet aRnewIndUWSubSet = new RnewIndUWSubSet();
	...
	public RnewIndUWSubSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		RnewIndUWSubSet aRnewIndUWSubSet = new RnewIndUWSubSet();
	...
	public RnewIndUWSubSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RnewIndUWSubSet aRnewIndUWSubSet = new RnewIndUWSubSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update RnewIndUWSub " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return RnewIndUWSubSet
 */
public RnewIndUWSubSet getData()
{
    int tCount = 0;
    RnewIndUWSubSet tRnewIndUWSubSet = new RnewIndUWSubSet();
    RnewIndUWSubSchema tRnewIndUWSubSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "RnewIndUWSubDB";
    ...
    try
    {
        ...
        tRnewIndUWSubSchema.setSchema(mResultSet, 1);
        tRnewIndUWSubSet.add(tRnewIndUWSubSchema);
...
public boolean closeData()
{
	...
	public RnewIndUWSubSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public RnewIndUWSubSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

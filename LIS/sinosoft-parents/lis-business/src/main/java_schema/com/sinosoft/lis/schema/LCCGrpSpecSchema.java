...
/*
 * <p>ClassName: LCCGrpSpecSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCCGrpSpecSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LCCGrpSpecSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LCCGrpSpecSchema cloned = (LCCGrpSpecSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LCCGrpSpecSchema 对象给 Schema 赋值
	* @param: aLCCGrpSpecSchema LCCGrpSpecSchema
	**/
	public void setSchema(LCCGrpSpecSchema aLCCGrpSpecSchema)
	{
		this.GrpContNo = aLCCGrpSpecSchema.getGrpContNo();
		this.ProposalGrpContNo = aLCCGrpSpecSchema.getProposalGrpContNo();
		this.PrtSeq = aLCCGrpSpecSchema.getPrtSeq();
		this.SerialNo = aLCCGrpSpecSchema.getSerialNo();
		this.EndorsementNo = aLCCGrpSpecSchema.getEndorsementNo();
		this.SpecType = aLCCGrpSpecSchema.getSpecType();
		this.SpecCode = aLCCGrpSpecSchema.getSpecCode();
		this.SpecContent = aLCCGrpSpecSchema.getSpecContent();
		this.PrtFlag = aLCCGrpSpecSchema.getPrtFlag();
		this.BackupType = aLCCGrpSpecSchema.getBackupType();
		this.Operator = aLCCGrpSpecSchema.getOperator();
		this.MakeDate = fDate.getDate( aLCCGrpSpecSchema.getMakeDate());
		this.MakeTime = aLCCGrpSpecSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLCCGrpSpecSchema.getModifyDate());
		this.ModifyTime = aLCCGrpSpecSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LCCGrpSpec表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LCCGrpSpecSchema";
	...
	public LCCGrpSpecSchema getSchema()
	{
		LCCGrpSpecSchema aLCCGrpSpecSchema = new LCCGrpSpecSchema();
		aLCCGrpSpecSchema.setSchema(this);
		return aLCCGrpSpecSchema;
	}

	public LCCGrpSpecDB getDB()
	{
		LCCGrpSpecDB aDBOper = new LCCGrpSpecDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCCGrpSpec描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCCGrpSpec>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			EndorsementNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			SpecType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			SpecCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			SpecContent = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			PrtFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			BackupType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LCCGrpSpecSchema other = (LCCGrpSpecSchema)otherObject;
		return

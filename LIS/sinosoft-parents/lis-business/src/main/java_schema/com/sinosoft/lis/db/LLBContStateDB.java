...
/*
 * <p>ClassName: LLBContStateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLBContStateDB extends LLBContStateSchema
{
private static Logger logger = Logger.getLogger(LLBContStateDB.class);
	...
	**/
	...
	// @Constructor
	public LLBContStateDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLBContStateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLBContStateDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getDealState() == null || this.getDealState().equals("null")) {
				...
				pstmt.setString(17, this.getDealState());
			}
			if(this.getClmState() == null || this.getClmState().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
				pstmt.setString(18, this.getClmState());
			}
			// set where condition
			if(this.getBackNo() == null || this.getBackNo().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getBackNo());
			}
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getClmNo());
			}
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
			...
			if(this.getStateType() == null || this.getStateType().equals("null")) {
				...
				pstmt.setString(24, this.getStateType());
			}
			if(this.getStartDate() == null || this.getStartDate().equals("null")) {
				pstmt.setNull(25, 91);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLBContState WHERE  BackNo = ? AND ClmNo = ? AND ContNo = ? AND InsuredNo = ? AND PolNo = ? AND StateType = ? AND StartDate = ?", 
	...
	public LLBContStateSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLBContStateSet aLLBContStateSet = new LLBContStateSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLBContState");
			LLBContStateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLBContStateSchema s1 = new LLBContStateSchema();
				s1.setSchema(rs,i);
				aLLBContStateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLBContStateSet;

	}

	public LLBContStateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLBContStateSet aLLBContStateSet = new LLBContStateSet();
	...
	public LLBContStateSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLBContStateSet aLLBContStateSet = new LLBContStateSet();
	...
	public LLBContStateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLBContStateSet aLLBContStateSet = new LLBContStateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLBContState " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLBContStateSet
 */
public LLBContStateSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLLBContStateSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLBContStateSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLBContStateSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

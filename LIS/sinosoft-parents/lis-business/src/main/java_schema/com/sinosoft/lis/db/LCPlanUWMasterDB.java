...
/*
 * <p>ClassName: LCPlanUWMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统未整理模型
 */
public class LCPlanUWMasterDB extends LCPlanUWMasterSchema
{
private static Logger logger = Logger.getLogger(LCPlanUWMasterDB.class);
	...
	**/
	...
	// @Constructor
	public LCPlanUWMasterDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCPlanUWMaster" );
		mflag = true;
	}

	public LCPlanUWMasterDB()
	{
		con = null;
		db = new DBOper( "LCPlanUWMaster" );
		mflag = false;
	}
	...
	{
		LCPlanUWMasterSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCPlanUWMasterDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCPlanUWMaster WHERE  ProposalContNo = ? AND ContPlanCode = ? AND PlanType = ? AND InsuredNo = ?");
			if(this.getProposalContNo() == null || this.getProposalContNo().equals("null")) {
				pstmt.setNull(1, 1);
			} else {
				pstmt.setString(1, StrTool.space1(this.getProposalContNo(), 20));
			}
			...
			if(this.getInsuredNo() == null || this.getInsuredNo().equals("null")) {
				...
				pstmt.setString(4, StrTool.space1(this.getInsuredNo(), 24));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
		...
		SQLString sqlObj = new SQLString("LCPlanUWMaster");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCPlanUWMaster SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , ContPlanCode = ? , PlanType = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , SugPassFlag = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , SugUWIdea = ? , UpReport = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? WHERE  ProposalContNo = ? AND ContPlanCode = ? AND PlanType = ? AND InsuredNo = ?");
			...
			if(this.getSugPassFlag() == null || this.getSugPassFlag().equals("null")) {
				...
				pstmt.setString(13, this.getSugPassFlag());
			}
			...
			if(this.getSugUWIdea() == null || this.getSugUWIdea().equals("null")) {
				...
				pstmt.setString(35, this.getSugUWIdea());
			}
			...
			if(this.getProposalContNo() == null || this.getProposalContNo().equals("null")) {
				...
				pstmt.setString(42, StrTool.space1(this.getProposalContNo(), 20));
			}
			if(this.getContPlanCode() == null || this.getContPlanCode().equals("null")) {
				pstmt.setNull(43, 12);
			} else {
			...
			if(this.getPlanType() == null || this.getPlanType().equals("null")) {
				...
				pstmt.setString(44, StrTool.space1(this.getPlanType(), 1));
			}
			if(this.getInsuredNo() == null || this.getInsuredNo().equals("null")) {
				pstmt.setNull(45, 1);
			} else {
				pstmt.setString(45, StrTool.space1(this.getInsuredNo(), 24));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCPlanUWMaster(GrpContNo ,ContNo ,ProposalContNo ,ContPlanCode ,PlanType ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,SugPassFlag ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,SugUWIdea ,UpReport ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 1);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPlanUWMaster WHERE  ProposalContNo = ? AND ContPlanCode = ? AND PlanType = ? AND InsuredNo = ?", 
	...
	public LCPlanUWMasterSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPlanUWMasterSet aLCPlanUWMasterSet = new LCPlanUWMasterSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPlanUWMaster");
			LCPlanUWMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCPlanUWMasterSchema s1 = new LCPlanUWMasterSchema();
				s1.setSchema(rs,i);
				aLCPlanUWMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCPlanUWMasterSet;

	}

	public LCPlanUWMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPlanUWMasterSet aLCPlanUWMasterSet = new LCPlanUWMasterSet();
	...
	public LCPlanUWMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPlanUWMasterSet aLCPlanUWMasterSet = new LCPlanUWMasterSet();
	...
	public LCPlanUWMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPlanUWMasterSet aLCPlanUWMasterSet = new LCPlanUWMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPlanUWMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCPlanUWMasterSet
 */
public LCPlanUWMasterSet getData()
{
    int tCount = 0;
    LCPlanUWMasterSet tLCPlanUWMasterSet = new LCPlanUWMasterSet();
    LCPlanUWMasterSchema tLCPlanUWMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCPlanUWMasterDB";
    ...
    {
        tCount = 1;
        tLCPlanUWMasterSchema = new LCPlanUWMasterSchema();
        tLCPlanUWMasterSchema.setSchema(mResultSet, 1);
        tLCPlanUWMasterSet.add(tLCPlanUWMasterSchema);
...
public boolean closeData()
{
	...
	public LCPlanUWMasterSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPlanUWMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LBPOCustomerImpartDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 内部外包流程
 */
public class LBPOCustomerImpartDB extends LBPOCustomerImpartSchema
{
private static Logger logger = Logger.getLogger(LBPOCustomerImpartDB.class);
	...
	**/
	...
	// @Constructor
	public LBPOCustomerImpartDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBPOCustomerImpartSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBPOCustomerImpartDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getImpartContent() == null || this.getImpartContent().equals("null")) {
				...
				pstmt.setString(9, this.getImpartContent());
			}
			if(this.getImpartParamModle() == null || this.getImpartParamModle().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getImpartParamModle());
			}
			...
			if(this.getCustomerNoType() == null || this.getCustomerNoType().equals("null")) {
				...
				pstmt.setString(12, this.getCustomerNoType());
			}
			if(this.getUWClaimFlg() == null || this.getUWClaimFlg().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getUWClaimFlg());
			}
			if(this.getPrtFlag() == null || this.getPrtFlag().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getPrtFlag());
			}
			...
			if(this.getMakeDate() == null || this.getMakeDate().equals("null")) {
				...
				pstmt.setString(16, this.getMakeDate());
			}
			...
			if(this.getModifyDate() == null || this.getModifyDate().equals("null")) {
				...
				pstmt.setString(18, this.getModifyDate());
			}
			...
			pstmt.setInt(20, this.getPatchNo());
			if(this.getInsured1() == null || this.getInsured1().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getInsured1());
			}
			if(this.getInsured2() == null || this.getInsured2().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getInsured2());
			}
			if(this.getinsured3() == null || this.getinsured3().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
				pstmt.setString(23, this.getinsured3());
			}
			// set where condition
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getGrpContNo());
			}
			pstmt.setInt(25, this.getInputNo());
			if(this.getFillNo() == null || this.getFillNo().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getFillNo());
			}
			if(this.getProposalContNo() == null || this.getProposalContNo().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
				pstmt.setString(27, this.getProposalContNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBPOCustomerImpart WHERE  GrpContNo = ? AND InputNo = ? AND FillNo = ? AND ProposalContNo = ?", 
	...
	public LBPOCustomerImpartSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOCustomerImpartSet aLBPOCustomerImpartSet = new LBPOCustomerImpartSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBPOCustomerImpart");
			LBPOCustomerImpartSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBPOCustomerImpartSchema s1 = new LBPOCustomerImpartSchema();
				s1.setSchema(rs,i);
				aLBPOCustomerImpartSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBPOCustomerImpartSet;

	}

	public LBPOCustomerImpartSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOCustomerImpartSet aLBPOCustomerImpartSet = new LBPOCustomerImpartSet();
	...
	public LBPOCustomerImpartSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOCustomerImpartSet aLBPOCustomerImpartSet = new LBPOCustomerImpartSet();
	...
	public LBPOCustomerImpartSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOCustomerImpartSet aLBPOCustomerImpartSet = new LBPOCustomerImpartSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBPOCustomerImpart " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBPOCustomerImpartSet
 */
public LBPOCustomerImpartSet getData()
{
    int tCount = 0;
    LBPOCustomerImpartSet tLBPOCustomerImpartSet = new LBPOCustomerImpartSet();
    LBPOCustomerImpartSchema tLBPOCustomerImpartSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBPOCustomerImpartDB";
    ...
    {
        tCount = 1;
        tLBPOCustomerImpartSchema = new LBPOCustomerImpartSchema();
        tLBPOCustomerImpartSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LBPOCustomerImpartSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBPOCustomerImpartSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCCUWMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: lccuwmaster
 */
public class LCCUWMasterDB extends LCCUWMasterSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCCUWMasterDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCCUWMaster" );
		mflag = true;
	}

	public LCCUWMasterDB()
	{
		con = null;
		db = new DBOper( "LCCUWMaster" );
		mflag = false;
	}
	...
	{
		LCCUWMasterSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCCUWMasterDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCCUWMaster WHERE  ContNo = ?");
		...
		SQLString sqlObj = new SQLString("LCCUWMaster");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCCUWMaster SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , UpReport = ? , CommonReason = ? , BatchNo = ? , CommonReasonCode = ? , UWState = ? , UniteNo = ? , WaitReason = ? , WaitContent = ? WHERE  ContNo = ?");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCCUWMaster VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCCUWMaster WHERE  ContNo = ?", 
	...
	public LCCUWMasterSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCCUWMasterSet aLCCUWMasterSet = new LCCUWMasterSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCCUWMaster");
			LCCUWMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCCUWMasterSchema s1 = new LCCUWMasterSchema();
				s1.setSchema(rs,i);
				aLCCUWMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCCUWMasterSet;
	}

	public LCCUWMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCCUWMasterSet aLCCUWMasterSet = new LCCUWMasterSet();
	...
	public LCCUWMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCCUWMasterSet aLCCUWMasterSet = new LCCUWMasterSet();
	...
	public LCCUWMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCCUWMasterSet aLCCUWMasterSet = new LCCUWMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCCUWMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCCUWMasterSet
 */
public LCCUWMasterSet getData()
{
    int tCount = 0;
    LCCUWMasterSet tLCCUWMasterSet = new LCCUWMasterSet();
    LCCUWMasterSchema tLCCUWMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCCUWMasterDB";
    ...
    {
        tCount = 1;
        tLCCUWMasterSchema = new LCCUWMasterSchema();
        tLCCUWMasterSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCCUWMasterSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCCUWMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
import com.sinosoft.lis.db.PD_LMRiskFeeDB;

/*
 * <p>ClassName: PD_LMRiskFeeSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class PD_LMRiskFeeSchema implements Schema, Cloneable
{
	// @Field
	/** 管理费编码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		PD_LMRiskFeeSchema cloned = (PD_LMRiskFeeSchema)super.clone();
	...
	*/
	...
	/**
	* 01-管理费 02-行政管理费 03-投资管理费 04-帐户转换费
	*/
	...
	/**
	* 01－交费时 02－固定日期 03－生效对应日 04－转个人单对应日 05－每月的首次计价日 06－帐户转换时使用
	*/
	...
	*/
	...
	/**
	* 使用另外一个 PD_LMRiskFeeSchema 对象给 Schema 赋值
	* @param: aPD_LMRiskFeeSchema PD_LMRiskFeeSchema
	**/
	public void setSchema(PD_LMRiskFeeSchema aPD_LMRiskFeeSchema)
	{
		this.FeeCode = aPD_LMRiskFeeSchema.getFeeCode();
		this.FeeName = aPD_LMRiskFeeSchema.getFeeName();
		this.FeeNoti = aPD_LMRiskFeeSchema.getFeeNoti();
		this.InsuAccNo = aPD_LMRiskFeeSchema.getInsuAccNo();
		this.PayPlanCode = aPD_LMRiskFeeSchema.getPayPlanCode();
		this.PayInsuAccName = aPD_LMRiskFeeSchema.getPayInsuAccName();
		this.FeeKind = aPD_LMRiskFeeSchema.getFeeKind();
		this.FeeItemType = aPD_LMRiskFeeSchema.getFeeItemType();
		this.FeeTakePlace = aPD_LMRiskFeeSchema.getFeeTakePlace();
		this.FeeCalMode = aPD_LMRiskFeeSchema.getFeeCalMode();
		this.FeeCalModeType = aPD_LMRiskFeeSchema.getFeeCalModeType();
		this.FeeCalCode = aPD_LMRiskFeeSchema.getFeeCalCode();
		this.FeeValue = aPD_LMRiskFeeSchema.getFeeValue();
		this.CompareValue = aPD_LMRiskFeeSchema.getCompareValue();
		this.FeePeriod = aPD_LMRiskFeeSchema.getFeePeriod();
		this.MaxTime = aPD_LMRiskFeeSchema.getMaxTime();
		this.DefaultFlag = aPD_LMRiskFeeSchema.getDefaultFlag();
		this.FeeStartDate = fDate.getDate( aPD_LMRiskFeeSchema.getFeeStartDate());
		this.FeeNum = aPD_LMRiskFeeSchema.getFeeNum();
		this.FeeBaseType = aPD_LMRiskFeeSchema.getFeeBaseType();
		this.InterfaceClassName = aPD_LMRiskFeeSchema.getInterfaceClassName();
		this.FeeType = aPD_LMRiskFeeSchema.getFeeType();
		this.PeriodType = aPD_LMRiskFeeSchema.getPeriodType();
		this.Operator = aPD_LMRiskFeeSchema.getOperator();
		this.MakeDate = fDate.getDate( aPD_LMRiskFeeSchema.getMakeDate());
		this.MakeTime = aPD_LMRiskFeeSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aPD_LMRiskFeeSchema.getModifyDate());
		this.ModifyTime = aPD_LMRiskFeeSchema.getModifyTime();
		this.Standbyflag1 = aPD_LMRiskFeeSchema.getStandbyflag1();
		this.Standbyflag2 = aPD_LMRiskFeeSchema.getStandbyflag2();
		this.Standbyflag3 = aPD_LMRiskFeeSchema.getStandbyflag3();
		this.Standbyflag4 = aPD_LMRiskFeeSchema.getStandbyflag4();
		this.Standbyflag5 = aPD_LMRiskFeeSchema.getStandbyflag5();
		this.Standbyflag6 = aPD_LMRiskFeeSchema.getStandbyflag6();
		this.RiskCode = aPD_LMRiskFeeSchema.getRiskCode();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			System.out.println("数据库中的PD_LMRiskFee表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "PD_LMRiskFeeSchema";
	...
	public PD_LMRiskFeeSchema getSchema()
	{
		PD_LMRiskFeeSchema aPD_LMRiskFeeSchema = new PD_LMRiskFeeSchema();
		aPD_LMRiskFeeSchema.setSchema(this);
		return aPD_LMRiskFeeSchema;
	}

	public PD_LMRiskFeeDB getDB()
	{
		PD_LMRiskFeeDB aDBOper = new PD_LMRiskFeeDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpPD_LMRiskFee描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpPD_LMRiskFee>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			FeeStartDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 18,SysConst.PACKAGESPILTER));
			FeeNum= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,19,SysConst.PACKAGESPILTER))).intValue();
			FeeBaseType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20, SysConst.PACKAGESPILTER );
			InterfaceClassName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 21, SysConst.PACKAGESPILTER );
			FeeType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22, SysConst.PACKAGESPILTER );
			PeriodType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 23, SysConst.PACKAGESPILTER );
			...
			RiskCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 35, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		PD_LMRiskFeeSchema other = (PD_LMRiskFeeSchema)otherObject;
		return

...
/*
 * <p>ClassName: LBTempFeeClassDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务收费
 */
public class LBTempFeeClassDB extends LBTempFeeClassSchema
{
	private static Logger logger = Logger.getLogger(LBTempFeeClassDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LBTempFeeClassDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBTempFeeClassSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBTempFeeClassDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getBackUpSerialNo() == null || this.getBackUpSerialNo().equals("null")) {
				...
				pstmt.setString(35, this.getBackUpSerialNo());
			}
			// set where condition
			if(this.getTempFeeNo() == null || this.getTempFeeNo().equals("null")) {
				pstmt.setNull(36, 12);
			...
			if(this.getBackUpSerialNo() == null || this.getBackUpSerialNo().equals("null")) {
				...
				pstmt.setString(39, this.getBackUpSerialNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBTempFeeClass WHERE  TempFeeNo = ? AND PayMode = ? AND ChequeNo = ? AND BackUpSerialNo = ?", 
	...
	public LBTempFeeClassSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LBTempFeeClassSet aLBTempFeeClassSet = new LBTempFeeClassSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBTempFeeClass");
			LBTempFeeClassSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBTempFeeClassSchema s1 = new LBTempFeeClassSchema();
				s1.setSchema(rs,i);
				aLBTempFeeClassSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBTempFeeClassSet;
	}

	public LBTempFeeClassSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBTempFeeClassSet aLBTempFeeClassSet = new LBTempFeeClassSet();
	...
	public LBTempFeeClassSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LBTempFeeClassSet aLBTempFeeClassSet = new LBTempFeeClassSet();
	...
	public LBTempFeeClassSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBTempFeeClassSet aLBTempFeeClassSet = new LBTempFeeClassSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBTempFeeClass " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBTempFeeClassSet
 */
public LBTempFeeClassSet getData()
{
    int tCount = 0;
    LBTempFeeClassSet tLBTempFeeClassSet = new LBTempFeeClassSet();
    LBTempFeeClassSchema tLBTempFeeClassSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBTempFeeClassDB";
    ...
    {
        tCount = 1;
        tLBTempFeeClassSchema = new LBTempFeeClassSchema();
        tLBTempFeeClassSchema.setSchema(mResultSet, 1);
        tLBTempFeeClassSet.add(tLBTempFeeClassSchema);
...
public boolean closeData()
{
	...
	public LBTempFeeClassSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LBTempFeeClassSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

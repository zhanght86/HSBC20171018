...
/*
 * <p>ClassName: LBPOPerInvestPlanBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LBPOPerInvestPlanBDB extends LBPOPerInvestPlanBSchema
{
private static Logger logger = Logger.getLogger(LBPOPerInvestPlanBDB.class);
	...
	**/
	...
	// @Constructor
	public LBPOPerInvestPlanBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBPOPerInvestPlanBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBPOPerInvestPlanBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getMakeDate() == null || this.getMakeDate().equals("null")) {
				...
				pstmt.setString(18, this.getMakeDate());
			}
			...
			if(this.getModifyDate() == null || this.getModifyDate().equals("null")) {
				...
				pstmt.setString(21, this.getModifyDate());
			}
			...
			if(this.getTransDate() == null || this.getTransDate().equals("null")) {
				...
				pstmt.setDate(23, Date.valueOf(this.getTransDate()));
			}
			if(this.getTransTime() == null || this.getTransTime().equals("null")) {
				pstmt.setNull(24, 1);
			} else {
				pstmt.setString(24, this.getTransTime());
			}
			if(this.getTransOperator() == null || this.getTransOperator().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
				pstmt.setString(25, this.getTransOperator());
			}
			// set where condition
			if(this.getIDX() == null || this.getIDX().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getIDX());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
			...
			pstmt.setInt(28, this.getInputNo());
			if(this.getFillNo() == null || this.getFillNo().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getFillNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBPOPerInvestPlanB WHERE  IDX = ? AND PolNo = ? AND InputNo = ? AND FillNo = ?", 
	...
	public LBPOPerInvestPlanBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOPerInvestPlanBSet aLBPOPerInvestPlanBSet = new LBPOPerInvestPlanBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBPOPerInvestPlanB");
			LBPOPerInvestPlanBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBPOPerInvestPlanBSchema s1 = new LBPOPerInvestPlanBSchema();
				s1.setSchema(rs,i);
				aLBPOPerInvestPlanBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBPOPerInvestPlanBSet;

	}

	public LBPOPerInvestPlanBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOPerInvestPlanBSet aLBPOPerInvestPlanBSet = new LBPOPerInvestPlanBSet();
	...
	public LBPOPerInvestPlanBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOPerInvestPlanBSet aLBPOPerInvestPlanBSet = new LBPOPerInvestPlanBSet();
	...
	public LBPOPerInvestPlanBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOPerInvestPlanBSet aLBPOPerInvestPlanBSet = new LBPOPerInvestPlanBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBPOPerInvestPlanB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBPOPerInvestPlanBSet
 */
public LBPOPerInvestPlanBSet getData()
{
    int tCount = 0;
    LBPOPerInvestPlanBSet tLBPOPerInvestPlanBSet = new LBPOPerInvestPlanBSet();
    LBPOPerInvestPlanBSchema tLBPOPerInvestPlanBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBPOPerInvestPlanBDB";
    ...
    {
        tCount = 1;
        tLBPOPerInvestPlanBSchema = new LBPOPerInvestPlanBSchema();
        tLBPOPerInvestPlanBSchema.setSchema(mResultSet, 1);
        tLBPOPerInvestPlanBSet.add(tLBPOPerInvestPlanBSchema);
...
public boolean closeData()
{
	...
	public LBPOPerInvestPlanBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBPOPerInvestPlanBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCApplyRecallPolDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统lis5模型
 */
public class LCApplyRecallPolDB extends LCApplyRecallPolSchema
{
private static Logger logger = Logger.getLogger(LCApplyRecallPolDB.class);
	...
	**/
	...
	// @Constructor
	public LCApplyRecallPolDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCApplyRecallPolSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCApplyRecallPolDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getInputState() == null || this.getInputState().equals("null")) {
				...
				pstmt.setString(6, this.getInputState());
			}
			...
			if(this.getApplyType() == null || this.getApplyType().equals("null")) {
				pstmt.setNull(12, 1);
			} else {
				pstmt.setString(12, this.getApplyType());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCApplyRecallPol WHERE  ProposalNo = ?", 
	...
	public LCApplyRecallPolSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCApplyRecallPolSet aLCApplyRecallPolSet = new LCApplyRecallPolSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCApplyRecallPol");
			LCApplyRecallPolSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCApplyRecallPolSchema s1 = new LCApplyRecallPolSchema();
				s1.setSchema(rs,i);
				aLCApplyRecallPolSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCApplyRecallPolSet;

	}

	public LCApplyRecallPolSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCApplyRecallPolSet aLCApplyRecallPolSet = new LCApplyRecallPolSet();
	...
	public LCApplyRecallPolSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCApplyRecallPolSet aLCApplyRecallPolSet = new LCApplyRecallPolSet();
	...
	public LCApplyRecallPolSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCApplyRecallPolSet aLCApplyRecallPolSet = new LCApplyRecallPolSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCApplyRecallPol " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCApplyRecallPolSet
 */
public LCApplyRecallPolSet getData()
{
    int tCount = 0;
    LCApplyRecallPolSet tLCApplyRecallPolSet = new LCApplyRecallPolSet();
    LCApplyRecallPolSchema tLCApplyRecallPolSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCApplyRecallPolDB";
    ...
    try
    {
        ...
        tLCApplyRecallPolSchema.setSchema(mResultSet, 1);
        tLCApplyRecallPolSet.add(tLCApplyRecallPolSchema);
...
public boolean closeData()
{
	...
	public LCApplyRecallPolSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCApplyRecallPolSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

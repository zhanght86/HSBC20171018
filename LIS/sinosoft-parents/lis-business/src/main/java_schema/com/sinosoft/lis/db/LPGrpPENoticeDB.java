...
/*
 * <p>ClassName: LPGrpPENoticeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPGrpPENoticeDB extends LPGrpPENoticeSchema
{
private static Logger logger = Logger.getLogger(LPGrpPENoticeDB.class);
	...
	**/
	...
	// @Constructor
	public LPGrpPENoticeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPGrpPENoticeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPGrpPENoticeDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAgentName() == null || this.getAgentName().equals("null")) {
				...
				pstmt.setString(10, this.getAgentName());
			}
			...
			if(this.getPEResult() == null || this.getPEResult().equals("null")) {
				...
				pstmt.setString(18, this.getPEResult());
			}
			...
			if(this.getEdorAcceptNo() == null || this.getEdorAcceptNo().equals("null")) {
				...
				pstmt.setString(20, this.getEdorAcceptNo());
			}
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
			...
			if(this.getPrtSeq() == null || this.getPrtSeq().equals("null")) {
				...
				pstmt.setString(22, this.getPrtSeq());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPGrpPENotice WHERE  EdorAcceptNo = ? AND EdorNo = ? AND PrtSeq = ?", 
	...
	public LPGrpPENoticeSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGrpPENoticeSet aLPGrpPENoticeSet = new LPGrpPENoticeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPGrpPENotice");
			LPGrpPENoticeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPGrpPENoticeSchema s1 = new LPGrpPENoticeSchema();
				s1.setSchema(rs,i);
				aLPGrpPENoticeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPGrpPENoticeSet;

	}

	public LPGrpPENoticeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGrpPENoticeSet aLPGrpPENoticeSet = new LPGrpPENoticeSet();
	...
	public LPGrpPENoticeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGrpPENoticeSet aLPGrpPENoticeSet = new LPGrpPENoticeSet();
	...
	public LPGrpPENoticeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGrpPENoticeSet aLPGrpPENoticeSet = new LPGrpPENoticeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPGrpPENotice " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPGrpPENoticeSet
 */
public LPGrpPENoticeSet getData()
{
    int tCount = 0;
    LPGrpPENoticeSet tLPGrpPENoticeSet = new LPGrpPENoticeSet();
    LPGrpPENoticeSchema tLPGrpPENoticeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPGrpPENoticeDB";
    ...
    {
        tCount = 1;
        tLPGrpPENoticeSchema = new LPGrpPENoticeSchema();
        tLPGrpPENoticeSchema.setSchema(mResultSet, 1);
        tLPGrpPENoticeSet.add(tLPGrpPENoticeSchema);
...
public boolean closeData()
{
	...
	public LPGrpPENoticeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPGrpPENoticeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

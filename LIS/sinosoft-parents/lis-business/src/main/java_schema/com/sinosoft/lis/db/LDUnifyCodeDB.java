...
/*
 * <p>ClassName: LDUnifyCodeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_2
 */
public class LDUnifyCodeDB extends LDUnifyCodeSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDUnifyCodeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDUnifyCodeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LDUnifyCodeDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getSysCode() == null || this.getSysCode().equals("null")) {
				...
				pstmt.setString(15, this.getSysCode());
			}
			if(this.getCodeType() == null || this.getCodeType().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDUnifyCode WHERE  SysCode = ? AND CodeType = ? AND Code = ? AND State = ?", 
	...
	public LDUnifyCodeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDUnifyCodeSet aLDUnifyCodeSet = new LDUnifyCodeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDUnifyCode");
			LDUnifyCodeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LDUnifyCodeSchema s1 = new LDUnifyCodeSchema();
				s1.setSchema(rs,i);
				aLDUnifyCodeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLDUnifyCodeSet;
	}

	public LDUnifyCodeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDUnifyCodeSet aLDUnifyCodeSet = new LDUnifyCodeSet();
	...
	public LDUnifyCodeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDUnifyCodeSet aLDUnifyCodeSet = new LDUnifyCodeSet();
	...
	public LDUnifyCodeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDUnifyCodeSet aLDUnifyCodeSet = new LDUnifyCodeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDUnifyCode " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDUnifyCodeSet
 */
public LDUnifyCodeSet getData()
{
    int tCount = 0;
    LDUnifyCodeSet tLDUnifyCodeSet = new LDUnifyCodeSet();
    LDUnifyCodeSchema tLDUnifyCodeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDUnifyCodeDB";
    ...
    {
        tCount = 1;
        tLDUnifyCodeSchema = new LDUnifyCodeSchema();
        tLDUnifyCodeSchema.setSchema(mResultSet, 1);
        tLDUnifyCodeSet.add(tLDUnifyCodeSchema);
...
public boolean closeData()
{
	...
	public LDUnifyCodeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDUnifyCodeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

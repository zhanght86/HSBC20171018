...
/*
 * <p>ClassName: LBGrpAppntDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 泰康未整理PDM
 */
public class LBGrpAppntDB extends LBGrpAppntSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LBGrpAppntDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBGrpAppntSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBGrpAppntDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAppntType() == null || this.getAppntType().equals("null")) {
				...
				pstmt.setString(13, this.getAppntType());
			}
			if(this.getRelationToInsured() == null || this.getRelationToInsured().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getRelationToInsured());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBGrpAppnt WHERE  GrpContNo = ?", 
	...
	public LBGrpAppntSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBGrpAppnt");
			LBGrpAppntSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBGrpAppntSchema s1 = new LBGrpAppntSchema();
				s1.setSchema(rs,i);
				aLBGrpAppntSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBGrpAppntSet;
	}

	public LBGrpAppntSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBGrpAppntSet aLBGrpAppntSet = new LBGrpAppntSet();
	...
	public LBGrpAppntSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LBGrpAppntSet aLBGrpAppntSet = new LBGrpAppntSet();
	...
	public LBGrpAppntSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBGrpAppntSet aLBGrpAppntSet = new LBGrpAppntSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBGrpAppnt " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBGrpAppntSet
 */
public LBGrpAppntSet getData()
{
    int tCount = 0;
    LBGrpAppntSet tLBGrpAppntSet = new LBGrpAppntSet();
    LBGrpAppntSchema tLBGrpAppntSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBGrpAppntDB";
    ...
    try
    {
        ...
        tLBGrpAppntSchema.setSchema(mResultSet, 1);
        tLBGrpAppntSet.add(tLBGrpAppntSchema);
...
public boolean closeData()
{
	...
	public LBGrpAppntSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBGrpAppntSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

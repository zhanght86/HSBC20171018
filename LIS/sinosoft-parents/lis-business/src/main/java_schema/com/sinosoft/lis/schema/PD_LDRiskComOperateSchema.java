...
import com.sinosoft.lis.db.PD_LDRiskComOperateDB;

/**
 * <p>ClassName: PD_LDRiskComOperateSchema </p>
 ...
 */
public class PD_LDRiskComOperateSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(PD_LDRiskComOperateSchema.class);
    ...
    // @Constructor
    public PD_LDRiskComOperateSchema()
    {
        mErrors = new CErrors();
    ...
    {
        PD_LDRiskComOperateSchema cloned = (PD_LDRiskComOperateSchema)super.clone();
    ...
    /**
     * 使用另外一个 PD_LDRiskComOperateSchema 对象给 Schema 赋值
     * @param: aPD_LDRiskComOperateSchema PD_LDRiskComOperateSchema
     **/
    public void setSchema(PD_LDRiskComOperateSchema aPD_LDRiskComOperateSchema)
    {
        this.RiskCode = aPD_LDRiskComOperateSchema.getRiskCode();
        this.ManageCom = aPD_LDRiskComOperateSchema.getManageCom();
        this.ComCode = aPD_LDRiskComOperateSchema.getComCode();
        this.OperateType = aPD_LDRiskComOperateSchema.getOperateType();
        this.Remark = aPD_LDRiskComOperateSchema.getRemark();
        this.Operator = aPD_LDRiskComOperateSchema.getOperator();
        this.MakeDate = fDate.getDate(aPD_LDRiskComOperateSchema.getMakeDate());
        this.MakeTime = aPD_LDRiskComOperateSchema.getMakeTime();
        this.ModifyDate = fDate.getDate(aPD_LDRiskComOperateSchema.getModifyDate());
        this.ModifyTime = aPD_LDRiskComOperateSchema.getModifyTime();
        this.Standbyflag1 = aPD_LDRiskComOperateSchema.getStandbyflag1();
        this.Standbyflag2 = aPD_LDRiskComOperateSchema.getStandbyflag2();
        this.Standbyflag3 = aPD_LDRiskComOperateSchema.getStandbyflag3();
        this.Standbyflag4 = aPD_LDRiskComOperateSchema.getStandbyflag4();
        this.Standbyflag5 = aPD_LDRiskComOperateSchema.getStandbyflag5();
        this.Standbyflag6 = aPD_LDRiskComOperateSchema.getStandbyflag6();
    }
    ...
    public boolean setSchema(ResultSet rs, int i)
    {
        try
        {
            ...
            if (rs.getString(2) == null)
                ...
                this.ManageCom = rs.getString(2).trim();
            if (rs.getString(3) == null)
                this.ComCode = null;
            else
                this.ComCode = rs.getString(3).trim();
            if (rs.getString(4) == null)
                this.OperateType = null;
            else
                this.OperateType = rs.getString(4).trim();
        ...
        {
            logger.debug("数据库中表PD_LDRiskComOperate字段个数和Schema中的字段个数不一致，或执行db.executeQuery查询时未使用select * from tables");
            // @@错误处理
            CError tError = new CError();
            tError.moduleName = "PD_LDRiskComOperateSchema";
    ...
    public PD_LDRiskComOperateSchema getSchema()
    {
        PD_LDRiskComOperateSchema aPD_LDRiskComOperateSchema = new PD_LDRiskComOperateSchema();
        aPD_LDRiskComOperateSchema.setSchema(this);
        return aPD_LDRiskComOperateSchema;
    }

    public PD_LDRiskComOperateDB getDB()
    {
        PD_LDRiskComOperateDB aDBOper = new PD_LDRiskComOperateDB();
    ...
    /**
     * 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpPD_LDRiskComOperate>历史记账凭证主表信息</A>表字段
    ...
    public boolean equals(Object otherObject)
    {
        ...
        if (getClass() != otherObject.getClass())
            return false;
        PD_LDRiskComOperateSchema other = (PD_LDRiskComOperateSchema) otherObject;
        return

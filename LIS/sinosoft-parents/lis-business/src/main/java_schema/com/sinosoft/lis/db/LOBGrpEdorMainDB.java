...
import com.sinosoft.lis.vschema.LOBGrpEdorMainSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LOBGrpEdorMainDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LOBGrpEdorMainDB extends LOBGrpEdorMainSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LOBGrpEdorMainDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LOBGrpEdorMain" );
		mflag = true;
	}

	public LOBGrpEdorMainDB()
	{
		con = null;
		db = new DBOper( "LOBGrpEdorMain" );
		mflag = false;
	}
	...
	{
		LOBGrpEdorMainSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBGrpEdorMainDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LOBGrpEdorMain WHERE  EdorAcceptNo = ? AND EdorNo = ?");
		...
		SQLString sqlObj = new SQLString("LOBGrpEdorMain");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LOBGrpEdorMain SET  EdorAcceptNo = ? , EdorNo = ? , GrpContNo = ? , EdorAppNo = ? , EdorAppName = ? , ManageCom = ? , ChgPrem = ? , ChgAmnt = ? , GetMoney = ? , GetInterest = ? , EdorAppDate = ? , EdorValiDate = ? , EdorState = ? , BankCode = ? , BankAccNo = ? , AccName = ? , PostalAddress = ? , ZipCode = ? , Phone = ? , PrintFlag = ? , UWGrade = ? , AppGrade = ? , UWState = ? , UWOperator = ? , UWDate = ? , UWTime = ? , ConfOperator = ? , ConfDate = ? , ConfTime = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , Reason = ? , ReasonCode = ? , ApproveFlag = ? , ApproveOperator = ? , ApproveDate = ? , ApproveTime = ? , Currency = ? WHERE  EdorAcceptNo = ? AND EdorNo = ?");
			if(this.getEdorAcceptNo() == null || this.getEdorAcceptNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorAcceptNo());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LOBGrpEdorMain VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getEdorAcceptNo() == null || this.getEdorAcceptNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorAcceptNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBGrpEdorMain WHERE  EdorAcceptNo = ? AND EdorNo = ?", 
	...
	public LOBGrpEdorMainSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBGrpEdorMainSet aLOBGrpEdorMainSet = new LOBGrpEdorMainSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBGrpEdorMain");
			LOBGrpEdorMainSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBGrpEdorMainSchema s1 = new LOBGrpEdorMainSchema();
				s1.setSchema(rs,i);
				aLOBGrpEdorMainSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBGrpEdorMainSet;
	}

	public LOBGrpEdorMainSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBGrpEdorMainSet aLOBGrpEdorMainSet = new LOBGrpEdorMainSet();
	...
	public LOBGrpEdorMainSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBGrpEdorMainSet aLOBGrpEdorMainSet = new LOBGrpEdorMainSet();
	...
	public LOBGrpEdorMainSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBGrpEdorMainSet aLOBGrpEdorMainSet = new LOBGrpEdorMainSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBGrpEdorMain " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBGrpEdorMainSet
 */
public LOBGrpEdorMainSet getData()
{
    int tCount = 0;
    LOBGrpEdorMainSet tLOBGrpEdorMainSet = new LOBGrpEdorMainSet();
    LOBGrpEdorMainSchema tLOBGrpEdorMainSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBGrpEdorMainDB";
    ...
    {
        tCount = 1;
        tLOBGrpEdorMainSchema = new LOBGrpEdorMainSchema();
        tLOBGrpEdorMainSchema.setSchema(mResultSet, 1);
        tLOBGrpEdorMainSet.add(tLOBGrpEdorMainSchema);
...
public boolean closeData()
{
	...
	public LOBGrpEdorMainSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBGrpEdorMainSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
import com.sinosoft.lis.db.LOBContPlanEngineeringDB;

/*
 * 与数据库结构不一致，暂不处理
 * <p>ClassName: LOBContPlanEngineeringSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LOBContPlanEngineeringSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LOBContPlanEngineeringSchema.class);
	// @Field
	/** 业务类型 */
	...
	// @Constructor
	public LOBContPlanEngineeringSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LOBContPlanEngineeringSchema cloned = (LOBContPlanEngineeringSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LOBContPlanEngineeringSchema 对象给 Schema 赋值
	* @param: aLOBContPlanEngineeringSchema LOBContPlanEngineeringSchema
	**/
	public void setSchema(LOBContPlanEngineeringSchema aLOBContPlanEngineeringSchema)
	{
		this.BussType = aLOBContPlanEngineeringSchema.getBussType();
		this.BussNo = aLOBContPlanEngineeringSchema.getBussNo();
		this.PolicyNo = aLOBContPlanEngineeringSchema.getPolicyNo();
		this.PropNo = aLOBContPlanEngineeringSchema.getPropNo();
		this.SysPlanCode = aLOBContPlanEngineeringSchema.getSysPlanCode();
		this.PlanCode = aLOBContPlanEngineeringSchema.getPlanCode();
		this.EnginName = aLOBContPlanEngineeringSchema.getEnginName();
		this.EnginType = aLOBContPlanEngineeringSchema.getEnginType();
		this.EnginArea = aLOBContPlanEngineeringSchema.getEnginArea();
		this.EnginCost = aLOBContPlanEngineeringSchema.getEnginCost();
		this.EnginPlace = aLOBContPlanEngineeringSchema.getEnginPlace();
		this.EnginStartDate = fDate.getDate( aLOBContPlanEngineeringSchema.getEnginStartDate());
		this.EnginEndDate = fDate.getDate( aLOBContPlanEngineeringSchema.getEnginEndDate());
		this.InsurerName = aLOBContPlanEngineeringSchema.getInsurerName();
		this.InsurerType = aLOBContPlanEngineeringSchema.getInsurerType();
		this.ContractorName = aLOBContPlanEngineeringSchema.getContractorName();
		this.ContractorType = aLOBContPlanEngineeringSchema.getContractorType();
		this.EnginDesc = aLOBContPlanEngineeringSchema.getEnginDesc();
		this.EnginCondition = aLOBContPlanEngineeringSchema.getEnginCondition();
		this.Remark = aLOBContPlanEngineeringSchema.getRemark();
		this.MakeOperator = aLOBContPlanEngineeringSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLOBContPlanEngineeringSchema.getMakeDate());
		this.MakeTime = aLOBContPlanEngineeringSchema.getMakeTime();
		this.ModifyOperator = aLOBContPlanEngineeringSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLOBContPlanEngineeringSchema.getModifyDate());
		this.ModifyTime = aLOBContPlanEngineeringSchema.getModifyTime();
		this.EnginDays = aLOBContPlanEngineeringSchema.getEnginDays();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LOBContPlanEngineering表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LOBContPlanEngineeringSchema";
	...
	public LOBContPlanEngineeringSchema getSchema()
	{
		LOBContPlanEngineeringSchema aLOBContPlanEngineeringSchema = new LOBContPlanEngineeringSchema();
		aLOBContPlanEngineeringSchema.setSchema(this);
		return aLOBContPlanEngineeringSchema;
	}

	public LOBContPlanEngineeringDB getDB()
	{
		LOBContPlanEngineeringDB aDBOper = new LOBContPlanEngineeringDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBContPlanEngineering描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBContPlanEngineering>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			EnginName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			EnginType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			EnginArea = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,9,SysConst.PACKAGESPILTER))).doubleValue();
			EnginCost = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,10,SysConst.PACKAGESPILTER))).doubleValue();
			EnginPlace = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			EnginStartDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12,SysConst.PACKAGESPILTER));
			EnginEndDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13,SysConst.PACKAGESPILTER));
			InsurerName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			InsurerType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			ContractorName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 16, SysConst.PACKAGESPILTER );
			ContractorType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 17, SysConst.PACKAGESPILTER );
			EnginDesc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 18, SysConst.PACKAGESPILTER );
			EnginCondition = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 19, SysConst.PACKAGESPILTER );
			...
			EnginDays = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 27, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LOBContPlanEngineeringSchema other = (LOBContPlanEngineeringSchema)otherObject;
		return

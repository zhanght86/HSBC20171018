...
/*
 * <p>ClassName: LRTemplateTSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 规则引擎
 */
public class LRTemplateTSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LRTemplateTSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LRTemplateTSchema cloned = (LRTemplateTSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LRTemplateTSchema 对象给 Schema 赋值
	* @param: aLRTemplateTSchema LRTemplateTSchema
	**/
	public void setSchema(LRTemplateTSchema aLRTemplateTSchema)
	{
		this.Id = aLRTemplateTSchema.getId();
		this.Version = aLRTemplateTSchema.getVersion();
		this.RuleName = aLRTemplateTSchema.getRuleName();
		this.RuleCh = aLRTemplateTSchema.getRuleCh();
		this.SQLStatement = aLRTemplateTSchema.getSQLStatement();
		this.BOMs = aLRTemplateTSchema.getBOMs();
		this.SQLParameter = aLRTemplateTSchema.getSQLParameter();
		this.ViewParameter = aLRTemplateTSchema.getViewParameter();
		this.Valid = aLRTemplateTSchema.getValid();
		this.State = aLRTemplateTSchema.getState();
		this.TemplateLevel = aLRTemplateTSchema.getTemplateLevel();
		this.Business = aLRTemplateTSchema.getBusiness();
		this.Type = aLRTemplateTSchema.getType();
		this.TableName = aLRTemplateTSchema.getTableName();
		this.StartDate = fDate.getDate( aLRTemplateTSchema.getStartDate());
		this.EndDate = fDate.getDate( aLRTemplateTSchema.getEndDate());
		this.Description = aLRTemplateTSchema.getDescription();
		this.Creator = aLRTemplateTSchema.getCreator();
		this.Modifier = aLRTemplateTSchema.getModifier();
		this.Approver = aLRTemplateTSchema.getApprover();
		this.Deployer = aLRTemplateTSchema.getDeployer();
		this.MakeTime = aLRTemplateTSchema.getMakeTime();
		this.MakeDate = fDate.getDate( aLRTemplateTSchema.getMakeDate());
		this.ModifyTime = aLRTemplateTSchema.getModifyTime();
		this.ModifyDate = fDate.getDate( aLRTemplateTSchema.getModifyDate());
		this.AuthorTime = aLRTemplateTSchema.getAuthorTime();
		this.AuthorDate = fDate.getDate( aLRTemplateTSchema.getAuthorDate());
		this.DeclTime = aLRTemplateTSchema.getDeclTime();
		this.DeclDate = fDate.getDate( aLRTemplateTSchema.getDeclDate());
		this.RuleCalCh = aLRTemplateTSchema.getRuleCalCh();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			String viewParameter=rs.getString("ViewParameter");
			if( viewParameter == null )
				this.ViewParameter = null;
			else
				this.ViewParameter = viewParameter.trim();
		...
		{
			logger.debug("数据库中的LRTemplateT表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LRTemplateTSchema";
	...
	public LRTemplateTSchema getSchema()
	{
		LRTemplateTSchema aLRTemplateTSchema = new LRTemplateTSchema();
		aLRTemplateTSchema.setSchema(this);
		return aLRTemplateTSchema;
	}

	public LRTemplateTDB getDB()
	{
		LRTemplateTDB aDBOper = new LRTemplateTDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLRTemplateT描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLRTemplateT>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ViewParameter = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			Valid = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			State = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			TemplateLevel = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			Business = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			...
			Creator = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 18, SysConst.PACKAGESPILTER );
			Modifier = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 19, SysConst.PACKAGESPILTER );
			Approver = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20, SysConst.PACKAGESPILTER );
			Deployer = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 21, SysConst.PACKAGESPILTER );
			...
			AuthorTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 26, SysConst.PACKAGESPILTER );
			AuthorDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 27,SysConst.PACKAGESPILTER));
			DeclTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 28, SysConst.PACKAGESPILTER );
			DeclDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 29,SysConst.PACKAGESPILTER));
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LRTemplateTSchema other = (LRTemplateTSchema)otherObject;
		return

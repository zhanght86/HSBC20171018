...
/*
 * <p>ClassName: LAAssessDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 代理人管理
 */
public class LAAssessDB extends LAAssessSchema
{
private static Logger logger = Logger.getLogger(LAAssessDB.class);
	...
	**/
	...
	// @Constructor
	public LAAssessDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAAssessSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAgentSeries1() == null || this.getAgentSeries1().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getAgentSeries1());
			}
			if(this.getAgentGrade1() == null || this.getAgentGrade1().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getAgentGrade1());
			}
			if(this.getAgentGroupNew() == null || this.getAgentGroupNew().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getAgentGroupNew());
			}
			if(this.getConfirmer() == null || this.getConfirmer().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getConfirmer());
			}
			if(this.getConfirmDate() == null || this.getConfirmDate().equals("null")) {
				pstmt.setNull(12, 91);
			} else {
				pstmt.setDate(12, Date.valueOf(this.getConfirmDate()));
			}
			...
			if(this.getStandAssessFlag() == null || this.getStandAssessFlag().equals("null")) {
				pstmt.setNull(20, 1);
			} else {
				pstmt.setString(20, this.getStandAssessFlag());
			}
			if(this.getFirstAssessFlag() == null || this.getFirstAssessFlag().equals("null")) {
				pstmt.setNull(21, 1);
			} else {
				pstmt.setString(21, this.getFirstAssessFlag());
			}
			if(this.getBranchAttr() == null || this.getBranchAttr().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getBranchAttr());
			}
			if(this.getCalAgentSeries() == null || this.getCalAgentSeries().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
				pstmt.setString(23, this.getCalAgentSeries());
			}
			if(this.getCalAgentGrade() == null || this.getCalAgentGrade().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getCalAgentGrade());
			}
			// set where condition
			if(this.getIndexCalNo() == null || this.getIndexCalNo().equals("null")) {
				pstmt.setNull(25, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAAssess WHERE  IndexCalNo = ? AND AgentCode = ?", 
	...
	public LAAssessSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAAssess");
			LAAssessSchema aSchema = this.getSchema();
			...
			{
				i++;
				LAAssessSchema s1 = new LAAssessSchema();
				s1.setSchema(rs,i);
				aLAAssessSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LAAssessSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LAAssessSet aLAAssessSet = new LAAssessSet();
	...
	public LAAssessSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LAAssessSet aLAAssessSet = new LAAssessSet();
	...
	public LAAssessSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LAAssessSet aLAAssessSet = new LAAssessSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAAssess " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LAAssessSet
 */
public LAAssessSet getData()
{
    int tCount = 0;
    LAAssessSet tLAAssessSet = new LAAssessSet();
    LAAssessSchema tLAAssessSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAAssessDB";
    ...
    try
    {
        ...
        tLAAssessSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LAAssessSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LAAssessSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
import com.sinosoft.lis.db.LBAirPolDB;

/*
 * <p>ClassName: LBAirPolSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 短期险平台接口表
 */
public class LBAirPolSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBAirPolSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBAirPolSchema cloned = (LBAirPolSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LBAirPolSchema 对象给 Schema 赋值
	* @param: aLBAirPolSchema LBAirPolSchema
	**/
	public void setSchema(LBAirPolSchema aLBAirPolSchema)
	{
		this.PolNo = aLBAirPolSchema.getPolNo();
		this.CertifyCode = aLBAirPolSchema.getCertifyCode();
		this.ManageCom = aLBAirPolSchema.getManageCom();
		this.AgentCom = aLBAirPolSchema.getAgentCom();
		this.Password = aLBAirPolSchema.getPassword();
		this.InsuredName = aLBAirPolSchema.getInsuredName();
		this.InsuredIDType = aLBAirPolSchema.getInsuredIDType();
		this.InsuredIDNo = aLBAirPolSchema.getInsuredIDNo();
		this.RelationToInsured = aLBAirPolSchema.getRelationToInsured();
		this.BnfAddress = aLBAirPolSchema.getBnfAddress();
		this.BnfName = aLBAirPolSchema.getBnfName();
		this.BnfZipCode = aLBAirPolSchema.getBnfZipCode();
		this.BnfPhone = aLBAirPolSchema.getBnfPhone();
		this.Mult = aLBAirPolSchema.getMult();
		this.Prem = aLBAirPolSchema.getPrem();
		this.Amnt = aLBAirPolSchema.getAmnt();
		this.PolState = aLBAirPolSchema.getPolState();
		this.EmplaneDate = fDate.getDate( aLBAirPolSchema.getEmplaneDate());
		this.FlightNo = aLBAirPolSchema.getFlightNo();
		this.AgentCode = aLBAirPolSchema.getAgentCode();
		this.AgentGroup = aLBAirPolSchema.getAgentGroup();
		this.InsuYearFlag = aLBAirPolSchema.getInsuYearFlag();
		this.InsuYear = aLBAirPolSchema.getInsuYear();
		this.EndDate = fDate.getDate( aLBAirPolSchema.getEndDate());
		this.CValiDate = fDate.getDate( aLBAirPolSchema.getCValiDate());
		this.Operator = aLBAirPolSchema.getOperator();
		this.MakeDate = fDate.getDate( aLBAirPolSchema.getMakeDate());
		this.MakeTime = aLBAirPolSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLBAirPolSchema.getModifyDate());
		this.ModifyTime = aLBAirPolSchema.getModifyTime();
		this.TSignDate = fDate.getDate( aLBAirPolSchema.getTSignDate());
		this.RiskCode = aLBAirPolSchema.getRiskCode();
		this.State = aLBAirPolSchema.getState();
		this.RiskType = aLBAirPolSchema.getRiskType();
		this.Destination = aLBAirPolSchema.getDestination();
		this.Reason = aLBAirPolSchema.getReason();
		this.InsuredSex = aLBAirPolSchema.getInsuredSex();
		this.InsuredBirthday = fDate.getDate( aLBAirPolSchema.getInsuredBirthday());
		this.OccupationType = aLBAirPolSchema.getOccupationType();
		this.PrtNo = aLBAirPolSchema.getPrtNo();
		this.StandByFlag1 = aLBAirPolSchema.getStandByFlag1();
		this.StandByFlag2 = aLBAirPolSchema.getStandByFlag2();
		this.StandByFlag3 = aLBAirPolSchema.getStandByFlag3();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBAirPol表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBAirPolSchema";
	...
	public LBAirPolSchema getSchema()
	{
		LBAirPolSchema aLBAirPolSchema = new LBAirPolSchema();
		aLBAirPolSchema.setSchema(this);
		return aLBAirPolSchema;
	}

	public LBAirPolDB getDB()
	{
		LBAirPolDB aDBOper = new LBAirPolDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBAirPol描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBAirPol>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBAirPolSchema other = (LBAirPolSchema)otherObject;
		return

...
/*
 * <p>ClassName: LLDutyCtrllevelDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLDutyCtrllevelDB extends LLDutyCtrllevelSchema
{
private static Logger logger = Logger.getLogger(LLDutyCtrllevelDB.class);
	...
	**/
	...
	// @Constructor
	public LLDutyCtrllevelDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLDutyCtrllevelSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLDutyCtrllevelDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getLevelType() == null || this.getLevelType().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getLevelType());
			}
			pstmt.setDouble(9, this.getUpLimit());
			pstmt.setDouble(10, this.getDownLimit());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLDutyCtrllevel WHERE  GrpContNo = ? AND RiskCode = ? AND DutyCode = ? AND ContPlanCode = ? AND Operator = ? AND MakeDate = ? AND MakeTime = ? AND ModifyDate = ? AND ModifyTime = ?", 
	...
	public LLDutyCtrllevelSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLDutyCtrllevelSet aLLDutyCtrllevelSet = new LLDutyCtrllevelSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLDutyCtrllevel");
			LLDutyCtrllevelSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLDutyCtrllevelSchema s1 = new LLDutyCtrllevelSchema();
				s1.setSchema(rs,i);
				aLLDutyCtrllevelSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLDutyCtrllevelSet;

	}

	public LLDutyCtrllevelSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLDutyCtrllevelSet aLLDutyCtrllevelSet = new LLDutyCtrllevelSet();
	...
	public LLDutyCtrllevelSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLDutyCtrllevelSet aLLDutyCtrllevelSet = new LLDutyCtrllevelSet();
	...
	public LLDutyCtrllevelSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLDutyCtrllevelSet aLLDutyCtrllevelSet = new LLDutyCtrllevelSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLDutyCtrllevel " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLDutyCtrllevelSet
 */
public LLDutyCtrllevelSet getData()
{
    int tCount = 0;
    LLDutyCtrllevelSet tLLDutyCtrllevelSet = new LLDutyCtrllevelSet();
    LLDutyCtrllevelSchema tLLDutyCtrllevelSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLDutyCtrllevelDB";
    ...
    {
        tCount = 1;
        tLLDutyCtrllevelSchema = new LLDutyCtrllevelSchema();
        tLLDutyCtrllevelSchema.setSchema(mResultSet, 1);
        tLLDutyCtrllevelSet.add(tLLDutyCtrllevelSchema);
...
public boolean closeData()
{
	...
	public LLDutyCtrllevelSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLDutyCtrllevelSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LSQuotUWRuleDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LSQuotUWRuleDB extends LSQuotUWRuleSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LSQuotUWRuleDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LSQuotUWRuleSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LSQuotUWRuleDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRuleClass() == null || this.getRuleClass().equals("null")) {
				...
				pstmt.setString(6, this.getRuleClass());
			}
			if(this.getRuleLevel() == null || this.getRuleLevel().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getRuleLevel());
			}
			if(this.getRuleDescription() == null || this.getRuleDescription().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getRuleDescription());
			}
			if(this.getCalSQL() == null || this.getCalSQL().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getCalSQL());
			}
			pstmt.setInt(10, this.getParamsNum());
			...
			if(this.getDefaultValues() == null || this.getDefaultValues().equals("null")) {
				...
				pstmt.setString(12, this.getDefaultValues());
			}
			if(this.getInputValues() == null || this.getInputValues().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getInputValues());
			}
			...
			if(this.getQuotNo() == null || this.getQuotNo().equals("null")) {
				...
				pstmt.setString(20, this.getQuotNo());
			}
			pstmt.setInt(21, this.getQuotBatNo());
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
			...
			if(this.getRuleCode() == null || this.getRuleCode().equals("null")) {
				...
				pstmt.setString(23, this.getRuleCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LSQuotUWRule WHERE  QuotNo = ? AND QuotBatNo = ? AND RiskCode = ? AND RuleCode = ?", 
	...
	public LSQuotUWRuleSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LSQuotUWRuleSet aLSQuotUWRuleSet = new LSQuotUWRuleSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LSQuotUWRule");
			LSQuotUWRuleSchema aSchema = this.getSchema();
			...
			{
				i++;
				LSQuotUWRuleSchema s1 = new LSQuotUWRuleSchema();
				s1.setSchema(rs,i);
				aLSQuotUWRuleSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLSQuotUWRuleSet;
	}

	public LSQuotUWRuleSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LSQuotUWRuleSet aLSQuotUWRuleSet = new LSQuotUWRuleSet();
	...
	public LSQuotUWRuleSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LSQuotUWRuleSet aLSQuotUWRuleSet = new LSQuotUWRuleSet();
	...
	public LSQuotUWRuleSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LSQuotUWRuleSet aLSQuotUWRuleSet = new LSQuotUWRuleSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LSQuotUWRule " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LSQuotUWRuleSet
 */
public LSQuotUWRuleSet getData()
{
    int tCount = 0;
    LSQuotUWRuleSet tLSQuotUWRuleSet = new LSQuotUWRuleSet();
    LSQuotUWRuleSchema tLSQuotUWRuleSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LSQuotUWRuleDB";
    ...
    {
        tCount = 1;
        tLSQuotUWRuleSchema = new LSQuotUWRuleSchema();
        tLSQuotUWRuleSchema.setSchema(mResultSet, 1);
        tLSQuotUWRuleSet.add(tLSQuotUWRuleSchema);
...
public boolean closeData()
{
	...
	public LSQuotUWRuleSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LSQuotUWRuleSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

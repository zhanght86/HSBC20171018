...
/*
 * <p>ClassName: LJSPayDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务收费
 */
public class LJSPayDB extends LJSPaySchema
{
	private static Logger logger = Logger.getLogger(LJSPayDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJSPayDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJSPaySchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getBankOnTheWayFlag() == null || this.getBankOnTheWayFlag().equals("null")) {
				...
				pstmt.setString(7, this.getBankOnTheWayFlag());
			}
			if(this.getBankSuccFlag() == null || this.getBankSuccFlag().equals("null")) {
				pstmt.setNull(8, 1);
			} else {
				pstmt.setString(8, this.getBankSuccFlag());
			}
			pstmt.setInt(9, this.getSendBankCount());
			if(this.getApproveCode() == null || this.getApproveCode().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getApproveCode());
			}
			if(this.getApproveDate() == null || this.getApproveDate().equals("null")) {
				pstmt.setNull(11, 91);
			} else {
				pstmt.setDate(11, Date.valueOf(this.getApproveDate()));
			}
			...
			if(this.getPayForm() == null || this.getPayForm().equals("null")) {
				...
				pstmt.setString(29, this.getPayForm());
			}
			...
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				...
				pstmt.setString(33, this.getGrpContNo());
			}
			pstmt.setInt(34, this.getPayCount());
			pstmt.setDouble(35, this.getSumActuPayMoney());
			...
			if(this.getSendRelaNo() == null || this.getSendRelaNo().equals("null")) {
				...
				pstmt.setString(48, this.getSendRelaNo());
			}
			...
			if(this.getModifyOperator() == null || this.getModifyOperator().equals("null")) {
				...
				pstmt.setString(50, this.getModifyOperator());
			}
			pstmt.setDouble(51, this.getNetAmount());
			pstmt.setDouble(52, this.getTaxAmount());
			pstmt.setDouble(53, this.getTax());
			// set where condition
			if(this.getGetNoticeNo() == null || this.getGetNoticeNo().equals("null")) {
				pstmt.setNull(54, 12);
			} else {
				pstmt.setString(54, this.getGetNoticeNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(55, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJSPay WHERE  GetNoticeNo = ? AND Currency = ?", 
	...
	public LJSPaySet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJSPay");
			LJSPaySchema aSchema = this.getSchema();
			...
			{
				i++;
				LJSPaySchema s1 = new LJSPaySchema();
				s1.setSchema(rs,i);
				aLJSPaySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LJSPaySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSPaySet aLJSPaySet = new LJSPaySet();
	...
	public LJSPaySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSPaySet aLJSPaySet = new LJSPaySet();
	...
	public LJSPaySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSPaySet aLJSPaySet = new LJSPaySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJSPay " + sqlObj.getUpdPart() + " where " + strWherePart;
...
public LJSPaySet getData()
{
    ...
    try
    {
        ...
        tLJSPaySchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LJSPaySet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJSPaySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

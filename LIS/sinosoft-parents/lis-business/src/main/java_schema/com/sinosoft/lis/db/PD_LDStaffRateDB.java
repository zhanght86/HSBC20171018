...
/*
 * <p>ClassName: PD_LDStaffRateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PD_LDStaffRate
 */
public class PD_LDStaffRateDB extends PD_LDStaffRateSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public PD_LDStaffRateDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "PD_LDStaffRate" );
		mflag = true;
	}

	public PD_LDStaffRateDB()
	{
		con = null;
		db = new DBOper( "PD_LDStaffRate" );
		mflag = false;
	}
	...
	{
		PD_LDStaffRateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "PD_LDStaffRateDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM PD_LDStaffRate WHERE  IDNo = ?");
		...
		SQLString sqlObj = new SQLString("PD_LDStaffRate");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE PD_LDStaffRate SET  IDNo = ? , AgentCom = ? , ManageCom = ? , RiskCode = ? , PayYear = ? , Currency = ? , InvestType = ? , PayType = ? , ProtectionPlan = ? , PayYears = ? , CommRate = ? , StartDate = ? , EndDate = ? , ValidFlag = ? , AppState = ? , AppDate = ? , AppTime = ? , Branchtype = ? , ActionType = ? , OrigIDNo = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , StaffRate = ? , InsureAgeStart = ? , InsureAgeEnd = ? , PayToAge = ? , DefaultFlag = ? , SRFlag = ? , PlanCode = ? , Insuyear = ? , InsuyearFlag = ? WHERE  IDNo = ?");
			if(this.getIDNo() == null || this.getIDNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getIDNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM PD_LDStaffRate WHERE  IDNo = ?", 
	...
	public PD_LDStaffRateSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LDStaffRateSet aPD_LDStaffRateSet = new PD_LDStaffRateSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("PD_LDStaffRate");
			PD_LDStaffRateSchema aSchema = this.getSchema();
			...
			{
				i++;
				PD_LDStaffRateSchema s1 = new PD_LDStaffRateSchema();
				s1.setSchema(rs,i);
				aPD_LDStaffRateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aPD_LDStaffRateSet;
	}

	public PD_LDStaffRateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LDStaffRateSet aPD_LDStaffRateSet = new PD_LDStaffRateSet();
	...
	public PD_LDStaffRateSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LDStaffRateSet aPD_LDStaffRateSet = new PD_LDStaffRateSet();
	...
	public PD_LDStaffRateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LDStaffRateSet aPD_LDStaffRateSet = new PD_LDStaffRateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update PD_LDStaffRate " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return PD_LDStaffRateSet
 */
public PD_LDStaffRateSet getData()
{
    int tCount = 0;
    PD_LDStaffRateSet tPD_LDStaffRateSet = new PD_LDStaffRateSet();
    PD_LDStaffRateSchema tPD_LDStaffRateSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "PD_LDStaffRateDB";
    ...
    {
        tCount = 1;
        tPD_LDStaffRateSchema = new PD_LDStaffRateSchema();
        tPD_LDStaffRateSchema.setSchema(mResultSet, 1);

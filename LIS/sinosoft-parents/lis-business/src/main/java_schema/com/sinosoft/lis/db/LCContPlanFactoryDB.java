...
/*
 * <p>ClassName: LCContPlanFactoryDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCContPlanFactoryDB extends LCContPlanFactorySchema
{
private static Logger logger = Logger.getLogger(LCContPlanFactoryDB.class);
	...
	**/
	...
	// @Constructor
	public LCContPlanFactoryDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCContPlanFactory" );
		mflag = true;
	}

	public LCContPlanFactoryDB()
	{
		con = null;
		db = new DBOper( "LCContPlanFactory" );
		mflag = false;
	}
	...
	{
		LCContPlanFactorySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCContPlanFactoryDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCContPlanFactory WHERE  ProposalGrpContNo = ? AND MainRiskCode = ? AND RiskCode = ? AND ContPlanCode = ? AND FactoryType = ? AND OtherNo = ? AND FactoryCode = ? AND FactorySubCode = ? AND InerSerialNo = ? AND PlanType = ?");
			...
			if(this.getPlanType() == null || this.getPlanType().equals("null")) {
				...
				pstmt.setString(10, StrTool.space1(this.getPlanType(), 1));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
		...
		SQLString sqlObj = new SQLString("LCContPlanFactory");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getParams() == null || this.getParams().equals("null")) {
				...
				pstmt.setString(16, this.getParams());
			}
			if(this.getCalRemark() == null || this.getCalRemark().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getCalRemark());
			}
			...
			if(this.getPlanType() == null || this.getPlanType().equals("null")) {
				...
				pstmt.setString(22, this.getPlanType());
			}
			// set where condition
			if(this.getProposalGrpContNo() == null || this.getProposalGrpContNo().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
				pstmt.setString(23, this.getProposalGrpContNo());
			}
			if(this.getMainRiskCode() == null || this.getMainRiskCode().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getMainRiskCode());
			}
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
			...
			if(this.getContPlanCode() == null || this.getContPlanCode().equals("null")) {
				...
				pstmt.setString(26, this.getContPlanCode());
			}
			if(this.getFactoryType() == null || this.getFactoryType().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
				pstmt.setString(27, this.getFactoryType());
			}
			if(this.getOtherNo() == null || this.getOtherNo().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getOtherNo());
			}
			if(this.getFactoryCode() == null || this.getFactoryCode().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getFactoryCode());
			}
			pstmt.setInt(30, this.getFactorySubCode());
			if(this.getInerSerialNo() == null || this.getInerSerialNo().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
				pstmt.setString(31, this.getInerSerialNo());
			}
			if(this.getPlanType() == null || this.getPlanType().equals("null")) {
				pstmt.setNull(32, 1);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCContPlanFactory(GrpContNo ,ProposalGrpContNo ,MainRiskCode ,MainRiskVersion ,RiskCode ,RiskVersion ,ContPlanCode ,ContPlanName ,FactoryType ,OtherNo ,FactoryCode ,FactorySubCode ,FactoryName ,InerSerialNo ,CalSql ,Params ,CalRemark ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,PlanType) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCContPlanFactory WHERE  ProposalGrpContNo = ? AND MainRiskCode = ? AND RiskCode = ? AND ContPlanCode = ? AND FactoryType = ? AND OtherNo = ? AND FactoryCode = ? AND FactorySubCode = ? AND InerSerialNo = ? AND PlanType = ?", 
	...
	public LCContPlanFactorySet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCContPlanFactory");
			LCContPlanFactorySchema aSchema = this.getSchema();
			...
			{
				i++;
				LCContPlanFactorySchema s1 = new LCContPlanFactorySchema();
				s1.setSchema(rs,i);
				aLCContPlanFactorySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCContPlanFactorySet;

	}

	public LCContPlanFactorySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCContPlanFactorySet aLCContPlanFactorySet = new LCContPlanFactorySet();
	...
	public LCContPlanFactorySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCContPlanFactorySet aLCContPlanFactorySet = new LCContPlanFactorySet();
	...
	public LCContPlanFactorySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCContPlanFactorySet aLCContPlanFactorySet = new LCContPlanFactorySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCContPlanFactory " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCContPlanFactorySet
 */
public LCContPlanFactorySet getData()
{
    int tCount = 0;
    LCContPlanFactorySet tLCContPlanFactorySet = new LCContPlanFactorySet();
    LCContPlanFactorySchema tLCContPlanFactorySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCContPlanFactoryDB";
    ...
    try
    {
        ...
        tLCContPlanFactorySchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCContPlanFactorySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCContPlanFactorySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

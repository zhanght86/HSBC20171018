...
/*
 * <p>ClassName: LPPremSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPPremSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPPremSchema.class);
	// @Field
	/** 批单号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPPremSchema cloned = (LPPremSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LPPremSchema 对象给 Schema 赋值
	* @param: aLPPremSchema LPPremSchema
	**/
	public void setSchema(LPPremSchema aLPPremSchema)
	{
		this.EdorNo = aLPPremSchema.getEdorNo();
		this.EdorType = aLPPremSchema.getEdorType();
		this.GrpContNo = aLPPremSchema.getGrpContNo();
		this.ContNo = aLPPremSchema.getContNo();
		this.PolNo = aLPPremSchema.getPolNo();
		this.DutyCode = aLPPremSchema.getDutyCode();
		this.PayPlanCode = aLPPremSchema.getPayPlanCode();
		this.PayPlanType = aLPPremSchema.getPayPlanType();
		this.AppntType = aLPPremSchema.getAppntType();
		this.AppntNo = aLPPremSchema.getAppntNo();
		this.UrgePayFlag = aLPPremSchema.getUrgePayFlag();
		this.NeedAcc = aLPPremSchema.getNeedAcc();
		this.PayTimes = aLPPremSchema.getPayTimes();
		this.Rate = aLPPremSchema.getRate();
		this.PayStartDate = fDate.getDate( aLPPremSchema.getPayStartDate());
		this.PayEndDate = fDate.getDate( aLPPremSchema.getPayEndDate());
		this.PaytoDate = fDate.getDate( aLPPremSchema.getPaytoDate());
		this.PayIntv = aLPPremSchema.getPayIntv();
		this.StandPrem = aLPPremSchema.getStandPrem();
		this.Prem = aLPPremSchema.getPrem();
		this.SumPrem = aLPPremSchema.getSumPrem();
		this.SuppRiskScore = aLPPremSchema.getSuppRiskScore();
		this.FreeFlag = aLPPremSchema.getFreeFlag();
		this.FreeRate = aLPPremSchema.getFreeRate();
		this.FreeStartDate = fDate.getDate( aLPPremSchema.getFreeStartDate());
		this.FreeEndDate = fDate.getDate( aLPPremSchema.getFreeEndDate());
		this.State = aLPPremSchema.getState();
		this.ManageCom = aLPPremSchema.getManageCom();
		this.Operator = aLPPremSchema.getOperator();
		this.MakeDate = fDate.getDate( aLPPremSchema.getMakeDate());
		this.MakeTime = aLPPremSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLPPremSchema.getModifyDate());
		this.ModifyTime = aLPPremSchema.getModifyTime();
		this.AddFeeDirect = aLPPremSchema.getAddFeeDirect();
		this.SecInsuAddPoint = aLPPremSchema.getSecInsuAddPoint();
		this.AddForm = aLPPremSchema.getAddForm();
		this.Currency = aLPPremSchema.getCurrency();
		this.PeriodPrem = aLPPremSchema.getPeriodPrem();
		this.ComCode = aLPPremSchema.getComCode();
		this.ModifyOperator = aLPPremSchema.getModifyOperator();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPPrem表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPPremSchema";
	...
	public LPPremSchema getSchema()
	{
		LPPremSchema aLPPremSchema = new LPPremSchema();
		aLPPremSchema.setSchema(this);
		return aLPPremSchema;
	}

	public LPPremDB getDB()
	{
		LPPremDB aDBOper = new LPPremDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPPrem>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			AddForm = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 36, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPPremSchema other = (LPPremSchema)otherObject;
		return

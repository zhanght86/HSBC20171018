...
/*
 * <p>ClassName: LJAPaySchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务收费
 */
public class LJAPaySchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LJAPaySchema.class);
	// @Field
	/** 交费收据号码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LJAPaySchema cloned = (LJAPaySchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LJAPaySchema 对象给 Schema 赋值
	* @param: aLJAPaySchema LJAPaySchema
	**/
	public void setSchema(LJAPaySchema aLJAPaySchema)
	{
		this.PayNo = aLJAPaySchema.getPayNo();
		this.IncomeNo = aLJAPaySchema.getIncomeNo();
		this.IncomeType = aLJAPaySchema.getIncomeType();
		this.AppntNo = aLJAPaySchema.getAppntNo();
		this.SumActuPayMoney = aLJAPaySchema.getSumActuPayMoney();
		this.PayDate = fDate.getDate( aLJAPaySchema.getPayDate());
		this.EnterAccDate = fDate.getDate( aLJAPaySchema.getEnterAccDate());
		this.ConfDate = fDate.getDate( aLJAPaySchema.getConfDate());
		this.ApproveCode = aLJAPaySchema.getApproveCode();
		this.ApproveDate = fDate.getDate( aLJAPaySchema.getApproveDate());
		this.SerialNo = aLJAPaySchema.getSerialNo();
		this.Operator = aLJAPaySchema.getOperator();
		this.MakeDate = fDate.getDate( aLJAPaySchema.getMakeDate());
		this.MakeTime = aLJAPaySchema.getMakeTime();
		this.GetNoticeNo = aLJAPaySchema.getGetNoticeNo();
		this.ModifyDate = fDate.getDate( aLJAPaySchema.getModifyDate());
		this.ModifyTime = aLJAPaySchema.getModifyTime();
		this.ManageCom = aLJAPaySchema.getManageCom();
		this.AgentCom = aLJAPaySchema.getAgentCom();
		this.AgentType = aLJAPaySchema.getAgentType();
		this.BankCode = aLJAPaySchema.getBankCode();
		this.BankAccNo = aLJAPaySchema.getBankAccNo();
		this.RiskCode = aLJAPaySchema.getRiskCode();
		this.AgentCode = aLJAPaySchema.getAgentCode();
		this.AgentGroup = aLJAPaySchema.getAgentGroup();
		this.AccName = aLJAPaySchema.getAccName();
		this.StartPayDate = fDate.getDate( aLJAPaySchema.getStartPayDate());
		this.PayTypeFlag = aLJAPaySchema.getPayTypeFlag();
		this.OtherNo = aLJAPaySchema.getOtherNo();
		this.OtherNoType = aLJAPaySchema.getOtherNoType();
		this.IDType = aLJAPaySchema.getIDType();
		this.IDNo = aLJAPaySchema.getIDNo();
		this.Currency = aLJAPaySchema.getCurrency();
		this.GrpContNo = aLJAPaySchema.getGrpContNo();
		this.PayCount = aLJAPaySchema.getPayCount();
		this.SumDuePayMoney = aLJAPaySchema.getSumDuePayMoney();
		this.PayMode = aLJAPaySchema.getPayMode();
		this.PayObj = aLJAPaySchema.getPayObj();
		this.PayIntv = aLJAPaySchema.getPayIntv();
		this.GuaranteeFlag = aLJAPaySchema.getGuaranteeFlag();
		this.Insurancecom = aLJAPaySchema.getInsurancecom();
		this.BankOnTheWayFlag = aLJAPaySchema.getBankOnTheWayFlag();
		this.BankSuccFlag = aLJAPaySchema.getBankSuccFlag();
		this.SendBankCount = aLJAPaySchema.getSendBankCount();
		this.MatchSerialNo = aLJAPaySchema.getMatchSerialNo();
		this.State = aLJAPaySchema.getState();
		this.BalanceOnTime = aLJAPaySchema.getBalanceOnTime();
		this.BalanceAllNo = aLJAPaySchema.getBalanceAllNo();
		this.BalanceRelaNo = aLJAPaySchema.getBalanceRelaNo();
		this.SendRelaNo = aLJAPaySchema.getSendRelaNo();
		this.ComCode = aLJAPaySchema.getComCode();
		this.ModifyOperator = aLJAPaySchema.getModifyOperator();
		this.NetAmount = aLJAPaySchema.getNetAmount();
		this.TaxAmount = aLJAPaySchema.getTaxAmount();
		this.Tax = aLJAPaySchema.getTax();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LJAPay表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LJAPaySchema";
	...
	public LJAPaySchema getSchema()
	{
		LJAPaySchema aLJAPaySchema = new LJAPaySchema();
		aLJAPaySchema.setSchema(this);
		return aLJAPaySchema;
	}

	public LJAPayDB getDB()
	{
		LJAPayDB aDBOper = new LJAPayDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLJAPay描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLJAPay>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			PayCount= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,35,SysConst.PACKAGESPILTER))).intValue();
			SumDuePayMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,36,SysConst.PACKAGESPILTER))).doubleValue();
			PayMode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 37, SysConst.PACKAGESPILTER );
			PayObj = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 38, SysConst.PACKAGESPILTER );
			PayIntv= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,39,SysConst.PACKAGESPILTER))).intValue();
			GuaranteeFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 40, SysConst.PACKAGESPILTER );
			Insurancecom = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 41, SysConst.PACKAGESPILTER );
			BankOnTheWayFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 42, SysConst.PACKAGESPILTER );
			BankSuccFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 43, SysConst.PACKAGESPILTER );
			SendBankCount= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,44,SysConst.PACKAGESPILTER))).intValue();
			MatchSerialNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 45, SysConst.PACKAGESPILTER );
			State = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 46, SysConst.PACKAGESPILTER );
			BalanceOnTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 47, SysConst.PACKAGESPILTER );
			BalanceAllNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 48, SysConst.PACKAGESPILTER );
			BalanceRelaNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 49, SysConst.PACKAGESPILTER );
			SendRelaNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 50, SysConst.PACKAGESPILTER );
			...
			NetAmount = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,53,SysConst.PACKAGESPILTER))).doubleValue();
			TaxAmount = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,54,SysConst.PACKAGESPILTER))).doubleValue();
			Tax = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,55,SysConst.PACKAGESPILTER))).doubleValue();
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LJAPaySchema other = (LJAPaySchema)otherObject;
		return

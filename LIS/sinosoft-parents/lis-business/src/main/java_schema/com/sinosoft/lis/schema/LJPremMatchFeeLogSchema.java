...
import com.sinosoft.lis.db.LJPremMatchFeeLogDB;

/*
 * <p>ClassName: LJPremMatchFeeLogSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LJPremMatchFeeLogSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LJPremMatchFeeLogSchema.class);
	// @Field
	/** 保费匹配流水号 */
	...
	// @Constructor
	public LJPremMatchFeeLogSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LJPremMatchFeeLogSchema cloned = (LJPremMatchFeeLogSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LJPremMatchFeeLogSchema 对象给 Schema 赋值
	* @param: aLJPremMatchFeeLogSchema LJPremMatchFeeLogSchema
	**/
	public void setSchema(LJPremMatchFeeLogSchema aLJPremMatchFeeLogSchema)
	{
		this.MatchSerialNo = aLJPremMatchFeeLogSchema.getMatchSerialNo();
		this.MatchNode = aLJPremMatchFeeLogSchema.getMatchNode();
		this.DataSource = aLJPremMatchFeeLogSchema.getDataSource();
		this.SourceNo = aLJPremMatchFeeLogSchema.getSourceNo();
		this.GrpContNo = aLJPremMatchFeeLogSchema.getGrpContNo();
		this.BussType = aLJPremMatchFeeLogSchema.getBussType();
		this.BussNo = aLJPremMatchFeeLogSchema.getBussNo();
		this.CustBankCode = aLJPremMatchFeeLogSchema.getCustBankCode();
		this.CustBankAccNo = aLJPremMatchFeeLogSchema.getCustBankAccNo();
		this.CustAccName = aLJPremMatchFeeLogSchema.getCustAccName();
		this.AppntNo = aLJPremMatchFeeLogSchema.getAppntNo();
		this.GrpName = aLJPremMatchFeeLogSchema.getGrpName();
		this.InBankCode = aLJPremMatchFeeLogSchema.getInBankCode();
		this.InBankAccNo = aLJPremMatchFeeLogSchema.getInBankAccNo();
		this.EnterAccDate = fDate.getDate( aLJPremMatchFeeLogSchema.getEnterAccDate());
		this.Money = aLJPremMatchFeeLogSchema.getMoney();
		this.CurOutPayMoney = aLJPremMatchFeeLogSchema.getCurOutPayMoney();
		this.ManageCom = aLJPremMatchFeeLogSchema.getManageCom();
		this.ComCode = aLJPremMatchFeeLogSchema.getComCode();
		this.MakeOperator = aLJPremMatchFeeLogSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLJPremMatchFeeLogSchema.getMakeDate());
		this.MakeTime = aLJPremMatchFeeLogSchema.getMakeTime();
		this.ModifyOperator = aLJPremMatchFeeLogSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLJPremMatchFeeLogSchema.getModifyDate());
		this.ModifyTime = aLJPremMatchFeeLogSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LJPremMatchFeeLog表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LJPremMatchFeeLogSchema";
	...
	public LJPremMatchFeeLogSchema getSchema()
	{
		LJPremMatchFeeLogSchema aLJPremMatchFeeLogSchema = new LJPremMatchFeeLogSchema();
		aLJPremMatchFeeLogSchema.setSchema(this);
		return aLJPremMatchFeeLogSchema;
	}

	public LJPremMatchFeeLogDB getDB()
	{
		LJPremMatchFeeLogDB aDBOper = new LJPremMatchFeeLogDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLJPremMatchFeeLog描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLJPremMatchFeeLog>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			DataSource = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			SourceNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			...
			CustBankCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			CustBankAccNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			CustAccName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			AppntNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			GrpName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			InBankCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			InBankAccNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			EnterAccDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15,SysConst.PACKAGESPILTER));
			Money = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,16,SysConst.PACKAGESPILTER))).doubleValue();
			CurOutPayMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,17,SysConst.PACKAGESPILTER))).doubleValue();
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LJPremMatchFeeLogSchema other = (LJPremMatchFeeLogSchema)otherObject;
		return

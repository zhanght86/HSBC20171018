...
/*
 * <p>ClassName: LCUWErrorDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCUWErrorDB extends LCUWErrorSchema
{
private static Logger logger = Logger.getLogger(LCUWErrorDB.class);
	...
	**/
	...
	// @Constructor
	public LCUWErrorDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCUWErrorSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getUWType() == null || this.getUWType().equals("null")) {
				...
				pstmt.setString(21, this.getUWType());
			}
			// set where condition
			if(this.getProposalNo() == null || this.getProposalNo().equals("null")) {
				pstmt.setNull(22, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCUWError WHERE  ProposalNo = ? AND UWNo = ? AND SerialNo = ?", 
	...
	public LCUWErrorSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCUWErrorSet aLCUWErrorSet = new LCUWErrorSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCUWError");
			LCUWErrorSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCUWErrorSchema s1 = new LCUWErrorSchema();
				s1.setSchema(rs,i);
				aLCUWErrorSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCUWErrorSet;

	}

	public LCUWErrorSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCUWErrorSet aLCUWErrorSet = new LCUWErrorSet();
	...
	public LCUWErrorSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCUWErrorSet aLCUWErrorSet = new LCUWErrorSet();
	...
	public LCUWErrorSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCUWErrorSet aLCUWErrorSet = new LCUWErrorSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCUWError " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LCUWErrorSet
 */
public LCUWErrorSet getData()
{
    int tCount = 0;
    LCUWErrorSet tLCUWErrorSet = new LCUWErrorSet();
    LCUWErrorSchema tLCUWErrorSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCUWErrorDB";
    ...
    {
        tCount = 1;
        tLCUWErrorSchema = new LCUWErrorSchema();
        tLCUWErrorSchema.setSchema(mResultSet, 1);
        tLCUWErrorSet.add(tLCUWErrorSchema);
...
public boolean closeData()
{
	...
	public LCUWErrorSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCUWErrorSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

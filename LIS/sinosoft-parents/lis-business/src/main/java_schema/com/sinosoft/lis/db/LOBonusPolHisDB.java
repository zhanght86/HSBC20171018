...
/*
 * <p>ClassName: LOBonusPolHisDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 泰康未整理PDM
 */
public class LOBonusPolHisDB extends LOBonusPolHisSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LOBonusPolHisDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LOBonusPolHisSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBonusPolHisDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setInt(6, this.getFiscalYear());
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
			...
			if(this.getBonusDate() == null || this.getBonusDate().equals("null")) {
				pstmt.setNull(18, 91);
			} else {
				pstmt.setDate(18, Date.valueOf(this.getBonusDate()));
			}
			pstmt.setInt(19, this.getBonusInt());
			if(this.getBonusGetMode() == null || this.getBonusGetMode().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getBonusGetMode());
			}
			pstmt.setDouble(21, this.getLastBala());
			pstmt.setDouble(22, this.getLastStock());
			pstmt.setDouble(23, this.getBonusRate());
			pstmt.setDouble(24, this.getInterestRate());
			pstmt.setDouble(25, this.getSumBonus());
			pstmt.setDouble(26, this.getSumInterest());
			pstmt.setDouble(27, this.getInsuAccBala());
			pstmt.setDouble(28, this.getInsuAccStock());
			...
			if(this.getStandbyFlag1() == null || this.getStandbyFlag1().equals("null")) {
				...
				pstmt.setString(35, this.getStandbyFlag1());
			}
			if(this.getStandbyFlag2() == null || this.getStandbyFlag2().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
				pstmt.setString(36, this.getStandbyFlag2());
			}
			if(this.getStandbyFlag3() == null || this.getStandbyFlag3().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getStandbyFlag3());
			}
			// set where condition
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(38, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBonusPolHis WHERE  SerialNo = ?", 
	...
	public LOBonusPolHisSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBonusPolHisSet aLOBonusPolHisSet = new LOBonusPolHisSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBonusPolHis");
			LOBonusPolHisSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBonusPolHisSchema s1 = new LOBonusPolHisSchema();
				s1.setSchema(rs,i);
				aLOBonusPolHisSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBonusPolHisSet;
	}

	public LOBonusPolHisSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBonusPolHisSet aLOBonusPolHisSet = new LOBonusPolHisSet();
	...
	public LOBonusPolHisSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBonusPolHisSet aLOBonusPolHisSet = new LOBonusPolHisSet();
	...
	public LOBonusPolHisSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBonusPolHisSet aLOBonusPolHisSet = new LOBonusPolHisSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBonusPolHis " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBonusPolHisSet
 */
public LOBonusPolHisSet getData()
{
    int tCount = 0;
    LOBonusPolHisSet tLOBonusPolHisSet = new LOBonusPolHisSet();
    LOBonusPolHisSchema tLOBonusPolHisSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBonusPolHisDB";
    ...
    {
        tCount = 1;
        tLOBonusPolHisSchema = new LOBonusPolHisSchema();
        tLOBonusPolHisSchema.setSchema(mResultSet, 1);
        tLOBonusPolHisSet.add(tLOBonusPolHisSchema);
...
public boolean closeData()
{
	...
	public LOBonusPolHisSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBonusPolHisSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

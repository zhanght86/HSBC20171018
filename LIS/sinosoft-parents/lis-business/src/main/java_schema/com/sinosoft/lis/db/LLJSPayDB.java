...
/*
 * <p>ClassName: LLJSPayDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LLJSPayDB extends LLJSPaySchema
{
private static Logger logger = Logger.getLogger(LLJSPayDB.class);
	...
	**/
	...
	// @Constructor
	public LLJSPayDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLJSPaySchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAgentCom() == null || this.getAgentCom().equals("null")) {
				...
				pstmt.setString(20, this.getAgentCom());
			}
			if(this.getAgentType() == null || this.getAgentType().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getAgentType());
			}
			...
			if(this.getStartPayDate() == null || this.getStartPayDate().equals("null")) {
				...
				pstmt.setDate(28, Date.valueOf(this.getStartPayDate()));
			}
			...
			if(this.getGetNoticeNo() == null || this.getGetNoticeNo().equals("null")) {
				...
				pstmt.setString(34, this.getGetNoticeNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLJSPay WHERE  ClmNo = ? AND GetNoticeNo = ? AND Currency = ?", 
	...
	public LLJSPaySet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLJSPaySet aLLJSPaySet = new LLJSPaySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLJSPay");
			LLJSPaySchema aSchema = this.getSchema();
			...
			{
				i++;
				LLJSPaySchema s1 = new LLJSPaySchema();
				s1.setSchema(rs,i);
				aLLJSPaySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LLJSPaySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLJSPaySet aLLJSPaySet = new LLJSPaySet();
	...
	public LLJSPaySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLJSPaySet aLLJSPaySet = new LLJSPaySet();
	...
	public LLJSPaySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLJSPaySet aLLJSPaySet = new LLJSPaySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLJSPay " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LLJSPaySet
 */
public LLJSPaySet getData()
{
    int tCount = 0;
    LLJSPaySet tLLJSPaySet = new LLJSPaySet();
    LLJSPaySchema tLLJSPaySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLJSPayDB";
    ...
    {
        tCount = 1;
        tLLJSPaySchema = new LLJSPaySchema();
        tLLJSPaySchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLJSPaySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLJSPaySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

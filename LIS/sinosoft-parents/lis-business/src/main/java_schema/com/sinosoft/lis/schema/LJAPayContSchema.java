 ...
 * @Database: 寿险业务收费
 */
public class LJAPayContSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LJAPayContSchema.class);
	// @Field
	/** 总单/合同号码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LJAPayContSchema cloned = (LJAPayContSchema)super.clone();
	...
	* @param: aLJAPayContSchema LJAPayContSchema
	**/
	public void setSchema(LJAPayContSchema aLJAPayContSchema)
	{
		...
		this.GrpContNo = aLJAPayContSchema.getGrpContNo();
		this.CustomerName = aLJAPayContSchema.getCustomerName();
		this.IDType = aLJAPayContSchema.getIDType();
		this.IDNo = aLJAPayContSchema.getIDNo();
		this.BankCode = aLJAPayContSchema.getBankCode();
		this.BankAccNo = aLJAPayContSchema.getBankAccNo();
		this.AccName = aLJAPayContSchema.getAccName();
		this.BankOnTheWayFlag = aLJAPayContSchema.getBankOnTheWayFlag();
		this.BankSuccFlag = aLJAPayContSchema.getBankSuccFlag();
		this.SendBankCount = aLJAPayContSchema.getSendBankCount();
		this.State = aLJAPayContSchema.getState();
		this.SendRelaNo = aLJAPayContSchema.getSendRelaNo();
		this.ComCode = aLJAPayContSchema.getComCode();
		this.ModifyOperator = aLJAPayContSchema.getModifyOperator();
		this.NetAmount = aLJAPayContSchema.getNetAmount();
		this.TaxAmount = aLJAPayContSchema.getTaxAmount();
		this.Tax = aLJAPayContSchema.getTax();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LJAPayCont表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			PayNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			...
			LastPayToDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12,SysConst.PACKAGESPILTER));
			CurPayToDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13,SysConst.PACKAGESPILTER));
			ApproveCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			ApproveDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15,SysConst.PACKAGESPILTER));
			SerialNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 16, SysConst.PACKAGESPILTER );
			...
			GetNoticeNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20, SysConst.PACKAGESPILTER );
			...
			AgentCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 27, SysConst.PACKAGESPILTER );
			AgentGroup = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 28, SysConst.PACKAGESPILTER );

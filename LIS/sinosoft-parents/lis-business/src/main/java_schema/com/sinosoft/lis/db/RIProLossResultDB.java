...
/*
 * <p>ClassName: RIProLossResultDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保系统模型
 */
public class RIProLossResultDB extends RIProLossResultSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public RIProLossResultDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		RIProLossResultSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "RIProLossResultDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getCalYear() == null || this.getCalYear().equals("null")) {
				...
				pstmt.setString(3, this.getCalYear());
			}
			...
			pstmt.setDouble(9, this.getProLosAmnt());
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
			...
			if(this.getStandbyString1() == null || this.getStandbyString1().equals("null")) {
				...
				pstmt.setString(11, this.getStandbyString1());
			}
			if(this.getStandbyString2() == null || this.getStandbyString2().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getStandbyString2());
			}
			...
			pstmt.setDouble(19, this.getProfitAmnt());
			// set where condition
			if(this.getBatchNo() == null || this.getBatchNo().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getBatchNo());
			}
			if(this.getRIProfitNo() == null || this.getRIProfitNo().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getRIProfitNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM RIProLossResult WHERE  BatchNo = ? AND RIProfitNo = ? AND Currency = ?", 
	...
	public RIProLossResultSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIProLossResultSet aRIProLossResultSet = new RIProLossResultSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("RIProLossResult");
			RIProLossResultSchema aSchema = this.getSchema();
			...
			{
				i++;
				RIProLossResultSchema s1 = new RIProLossResultSchema();
				s1.setSchema(rs,i);
				aRIProLossResultSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aRIProLossResultSet;
	}

	public RIProLossResultSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIProLossResultSet aRIProLossResultSet = new RIProLossResultSet();
	...
	public RIProLossResultSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIProLossResultSet aRIProLossResultSet = new RIProLossResultSet();
	...
	public RIProLossResultSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIProLossResultSet aRIProLossResultSet = new RIProLossResultSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update RIProLossResult " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return RIProLossResultSet
 */
public RIProLossResultSet getData()
{
    int tCount = 0;
    RIProLossResultSet tRIProLossResultSet = new RIProLossResultSet();
    RIProLossResultSchema tRIProLossResultSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "RIProLossResultDB";
    ...
    {
        tCount = 1;
        tRIProLossResultSchema = new RIProLossResultSchema();
        tRIProLossResultSchema.setSchema(mResultSet, 1);

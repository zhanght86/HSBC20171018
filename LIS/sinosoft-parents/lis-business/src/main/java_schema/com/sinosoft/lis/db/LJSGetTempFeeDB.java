...
/*
 * <p>ClassName: LJSGetTempFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务付费
 */
public class LJSGetTempFeeDB extends LJSGetTempFeeSchema
{
	private static Logger logger = Logger.getLogger(LJSGetTempFeeDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJSGetTempFeeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJSGetTempFeeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJSGetTempFeeDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getFeeOperationType() == null || this.getFeeOperationType().equals("null")) {
				...
				pstmt.setString(14, this.getFeeOperationType());
			}
			if(this.getFeeFinaType() == null || this.getFeeFinaType().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getFeeFinaType());
			}
			...
			if(this.getGetReasonCode() == null || this.getGetReasonCode().equals("null")) {
				...
				pstmt.setString(20, this.getGetReasonCode());
			}
			...
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				...
				pstmt.setString(24, this.getCurrency());
			}
			pstmt.setDouble(25, this.getNetAmount());
			pstmt.setDouble(26, this.getTaxAmount());
			pstmt.setDouble(27, this.getTax());
			// set where condition
			if(this.getGetNoticeNo() == null || this.getGetNoticeNo().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getGetNoticeNo());
			}
			if(this.getTempFeeNo() == null || this.getTempFeeNo().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getTempFeeNo());
			}
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(30, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJSGetTempFee WHERE  GetNoticeNo = ? AND TempFeeNo = ? AND RiskCode = ? AND Currency = ?", 
	...
	public LJSGetTempFeeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSGetTempFeeSet aLJSGetTempFeeSet = new LJSGetTempFeeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJSGetTempFee");
			LJSGetTempFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJSGetTempFeeSchema s1 = new LJSGetTempFeeSchema();
				s1.setSchema(rs,i);
				aLJSGetTempFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJSGetTempFeeSet;
	}

	public LJSGetTempFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSGetTempFeeSet aLJSGetTempFeeSet = new LJSGetTempFeeSet();
	...
	public LJSGetTempFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSGetTempFeeSet aLJSGetTempFeeSet = new LJSGetTempFeeSet();
	...
	public LJSGetTempFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSGetTempFeeSet aLJSGetTempFeeSet = new LJSGetTempFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJSGetTempFee " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJSGetTempFeeSet
 */
public LJSGetTempFeeSet getData()
{
    int tCount = 0;
    LJSGetTempFeeSet tLJSGetTempFeeSet = new LJSGetTempFeeSet();
    LJSGetTempFeeSchema tLJSGetTempFeeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJSGetTempFeeDB";
    ...
    try
    {
        ...
        tLJSGetTempFeeSchema.setSchema(mResultSet, 1);
        tLJSGetTempFeeSet.add(tLJSGetTempFeeSchema);
...
public boolean closeData()
{
	...
	public LJSGetTempFeeSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJSGetTempFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

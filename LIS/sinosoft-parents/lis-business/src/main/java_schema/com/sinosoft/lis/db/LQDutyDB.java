...
/*
 * <p>ClassName: LQDutyDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LQDutyDB extends LQDutySchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LQDutyDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LQDutySchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAskPriceNo() == null || this.getAskPriceNo().equals("null")) {
				...
				pstmt.setString(57, this.getAskPriceNo());
			}
			if(this.getAskNo() == null || this.getAskNo().equals("null")) {
				pstmt.setNull(58, 12);
			} else {
				pstmt.setString(58, this.getAskNo());
			}
			if(this.getClassNo() == null || this.getClassNo().equals("null")) {
				pstmt.setNull(59, 12);
			} else {
				pstmt.setString(59, this.getClassNo());
			}
			if(this.getDutyCode() == null || this.getDutyCode().equals("null")) {
				pstmt.setNull(60, 12);
			} else {
				pstmt.setString(60, this.getDutyCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LQDuty WHERE  AskPriceNo = ? AND AskNo = ? AND ClassNo = ? AND DutyCode = ?", 
	...
	public LQDutySet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LQDutySet aLQDutySet = new LQDutySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LQDuty");
			LQDutySchema aSchema = this.getSchema();
			...
			{
				i++;
				LQDutySchema s1 = new LQDutySchema();
				s1.setSchema(rs,i);
				aLQDutySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LQDutySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LQDutySet aLQDutySet = new LQDutySet();
	...
	public LQDutySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LQDutySet aLQDutySet = new LQDutySet();
	...
	public LQDutySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LQDutySet aLQDutySet = new LQDutySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LQDuty " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LQDutySet
 */
public LQDutySet getData()
{
    int tCount = 0;
    LQDutySet tLQDutySet = new LQDutySet();
    LQDutySchema tLQDutySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LQDutyDB";
    ...
    {
        tCount = 1;
        tLQDutySchema = new LQDutySchema();
        tLQDutySchema.setSchema(mResultSet, 1);
        tLQDutySet.add(tLQDutySchema);
...
public boolean closeData()
{
	...
	public LQDutySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LQDutySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

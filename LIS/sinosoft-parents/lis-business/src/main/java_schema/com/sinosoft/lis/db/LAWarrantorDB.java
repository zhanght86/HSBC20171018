...
/*
 * <p>ClassName: LAWarrantorDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 代理人管理
 */
public class LAWarrantorDB extends LAWarrantorSchema
{
private static Logger logger = Logger.getLogger(LAWarrantorDB.class);
	...
	**/
	...
	// @Constructor
	public LAWarrantorDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAWarrantorSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAWarrantorDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getCautionerName() == null || this.getCautionerName().equals("null")) {
				...
				pstmt.setString(3, this.getCautionerName());
			}
			if(this.getCautionerSex() == null || this.getCautionerSex().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getCautionerSex());
			}
			if(this.getCautionerID() == null || this.getCautionerID().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getCautionerID());
			}
			if(this.getCautionerBirthday() == null || this.getCautionerBirthday().equals("null")) {
				pstmt.setNull(6, 91);
			} else {
				pstmt.setDate(6, Date.valueOf(this.getCautionerBirthday()));
			}
			if(this.getCautionerCom() == null || this.getCautionerCom().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getCautionerCom());
			}
			if(this.getHomeAddressCode() == null || this.getHomeAddressCode().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getHomeAddressCode());
			}
			...
			if(this.getRelation() == null || this.getRelation().equals("null")) {
				...
				pstmt.setString(12, this.getRelation());
			}
			if(this.getCautionData() == null || this.getCautionData().equals("null")) {
				pstmt.setNull(13, 91);
			} else {
				pstmt.setDate(13, Date.valueOf(this.getCautionData()));
			}
			...
			pstmt.setInt(21, this.getSerialNo());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAWarrantor WHERE  AgentCode = ? AND SerialNo = ?", 
	...
	public LAWarrantorSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LAWarrantorSet aLAWarrantorSet = new LAWarrantorSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAWarrantor");
			LAWarrantorSchema aSchema = this.getSchema();
			...
			{
				i++;
				LAWarrantorSchema s1 = new LAWarrantorSchema();
				s1.setSchema(rs,i);
				aLAWarrantorSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAWarrantorSet;

	}

	public LAWarrantorSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LAWarrantorSet aLAWarrantorSet = new LAWarrantorSet();
	...
	public LAWarrantorSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LAWarrantorSet aLAWarrantorSet = new LAWarrantorSet();
	...
	public LAWarrantorSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LAWarrantorSet aLAWarrantorSet = new LAWarrantorSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAWarrantor " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAWarrantorSet
 */
public LAWarrantorSet getData()
{
    int tCount = 0;
    LAWarrantorSet tLAWarrantorSet = new LAWarrantorSet();
    LAWarrantorSchema tLAWarrantorSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAWarrantorDB";
    ...
    try
    {
        ...
        tLAWarrantorSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LAWarrantorSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LAWarrantorSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LPUWErrorSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPUWErrorSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPUWErrorSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPUWErrorSchema cloned = (LPUWErrorSchema)super.clone();
	...
	/**
	* 使用另外一个 LPUWErrorSchema 对象给 Schema 赋值
	* @param: aLPUWErrorSchema LPUWErrorSchema
	**/
	public void setSchema(LPUWErrorSchema aLPUWErrorSchema)
	{
		this.EdorNo = aLPUWErrorSchema.getEdorNo();
		this.EdorType = aLPUWErrorSchema.getEdorType();
		this.GrpContNo = aLPUWErrorSchema.getGrpContNo();
		this.ContNo = aLPUWErrorSchema.getContNo();
		this.ProposalContNo = aLPUWErrorSchema.getProposalContNo();
		this.PolNo = aLPUWErrorSchema.getPolNo();
		this.ProposalNo = aLPUWErrorSchema.getProposalNo();
		this.UWNo = aLPUWErrorSchema.getUWNo();
		this.SerialNo = aLPUWErrorSchema.getSerialNo();
		this.InsuredNo = aLPUWErrorSchema.getInsuredNo();
		this.InsuredName = aLPUWErrorSchema.getInsuredName();
		this.AppntNo = aLPUWErrorSchema.getAppntNo();
		this.AppntName = aLPUWErrorSchema.getAppntName();
		this.ManageCom = aLPUWErrorSchema.getManageCom();
		this.UWRuleCode = aLPUWErrorSchema.getUWRuleCode();
		this.UWError = aLPUWErrorSchema.getUWError();
		this.CurrValue = aLPUWErrorSchema.getCurrValue();
		this.ModifyDate = fDate.getDate( aLPUWErrorSchema.getModifyDate());
		this.ModifyTime = aLPUWErrorSchema.getModifyTime();
		this.UWPassFlag = aLPUWErrorSchema.getUWPassFlag();
		this.UWGrade = aLPUWErrorSchema.getUWGrade();
		this.SugPassFlag = aLPUWErrorSchema.getSugPassFlag();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPUWError表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPUWErrorSchema";
	...
	public LPUWErrorSchema getSchema()
	{
		LPUWErrorSchema aLPUWErrorSchema = new LPUWErrorSchema();
		aLPUWErrorSchema.setSchema(this);
		return aLPUWErrorSchema;
	}

	public LPUWErrorDB getDB()
	{
		LPUWErrorDB aDBOper = new LPUWErrorDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPUWError描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPUWError>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			InsuredName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			...
			UWRuleCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			UWError = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 16, SysConst.PACKAGESPILTER );
			CurrValue = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 17, SysConst.PACKAGESPILTER );
			...
			UWPassFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20, SysConst.PACKAGESPILTER );
			UWGrade = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 21, SysConst.PACKAGESPILTER );
			SugPassFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPUWErrorSchema other = (LPUWErrorSchema)otherObject;
		return

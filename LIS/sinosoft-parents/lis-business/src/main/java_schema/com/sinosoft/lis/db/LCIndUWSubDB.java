...
/*
 * <p>ClassName: LCIndUWSubDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCIndUWSubDB extends LCIndUWSubSchema
{
private static Logger logger = Logger.getLogger(LCIndUWSubDB.class);
	...
	**/
	...
	// @Constructor
	public LCIndUWSubDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCIndUWSub" );
		mflag = true;
	}
	...
	{
		LCIndUWSubSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCIndUWSubDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCIndUWSub WHERE  ContNo = ? AND UWNo = ? AND InsuredNo = ?");
		...
		SQLString sqlObj = new SQLString("LCIndUWSub");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getUWType() == null || this.getUWType().equals("null")) {
				...
				pstmt.setString(40, this.getUWType());
			}
			// set where condition
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(41, 12);
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCIndUWSub(GrpContNo ,ContNo ,ProposalContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea ,UpReport ,UWType) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCIndUWSub WHERE  ContNo = ? AND UWNo = ? AND InsuredNo = ?", 
	...
	public LCIndUWSubSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCIndUWSubSet aLCIndUWSubSet = new LCIndUWSubSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCIndUWSub");
			LCIndUWSubSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCIndUWSubSchema s1 = new LCIndUWSubSchema();
				s1.setSchema(rs,i);
				aLCIndUWSubSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCIndUWSubSet;

	}

	public LCIndUWSubSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCIndUWSubSet aLCIndUWSubSet = new LCIndUWSubSet();
	...
	public LCIndUWSubSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCIndUWSubSet aLCIndUWSubSet = new LCIndUWSubSet();
	...
	public LCIndUWSubSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCIndUWSubSet aLCIndUWSubSet = new LCIndUWSubSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCIndUWSub " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCIndUWSubSet
 */
public LCIndUWSubSet getData()
{
    int tCount = 0;
    LCIndUWSubSet tLCIndUWSubSet = new LCIndUWSubSet();
    LCIndUWSubSchema tLCIndUWSubSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCIndUWSubDB";
    ...
    try
    {
        ...
        tLCIndUWSubSchema.setSchema(mResultSet, 1);
        tLCIndUWSubSet.add(tLCIndUWSubSchema);
...
public boolean closeData()
{
	...
	public LCIndUWSubSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCIndUWSubSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

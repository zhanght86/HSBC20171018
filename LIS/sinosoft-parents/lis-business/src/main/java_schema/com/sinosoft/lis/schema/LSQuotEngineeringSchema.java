...
import com.sinosoft.lis.db.LSQuotEngineeringDB;

/*
 * <p>ClassName: LSQuotEngineeringSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LSQuotEngineeringSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LSQuotEngineeringSchema.class);
	// @Field
	/** 询价号 */
	...
	// @Constructor
	public LSQuotEngineeringSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LSQuotEngineeringSchema cloned = (LSQuotEngineeringSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LSQuotEngineeringSchema 对象给 Schema 赋值
	* @param: aLSQuotEngineeringSchema LSQuotEngineeringSchema
	**/
	public void setSchema(LSQuotEngineeringSchema aLSQuotEngineeringSchema)
	{
		this.QuotNo = aLSQuotEngineeringSchema.getQuotNo();
		this.QuotBatNo = aLSQuotEngineeringSchema.getQuotBatNo();
		this.SysPlanCode = aLSQuotEngineeringSchema.getSysPlanCode();
		this.PlanCode = aLSQuotEngineeringSchema.getPlanCode();
		this.EnginName = aLSQuotEngineeringSchema.getEnginName();
		this.EnginType = aLSQuotEngineeringSchema.getEnginType();
		this.EnginArea = aLSQuotEngineeringSchema.getEnginArea();
		this.EnginCost = aLSQuotEngineeringSchema.getEnginCost();
		this.EnginPlace = aLSQuotEngineeringSchema.getEnginPlace();
		this.EnginStartDate = fDate.getDate( aLSQuotEngineeringSchema.getEnginStartDate());
		this.EnginEndDate = fDate.getDate( aLSQuotEngineeringSchema.getEnginEndDate());
		this.InsurerName = aLSQuotEngineeringSchema.getInsurerName();
		this.InsurerType = aLSQuotEngineeringSchema.getInsurerType();
		this.ContractorName = aLSQuotEngineeringSchema.getContractorName();
		this.ContractorType = aLSQuotEngineeringSchema.getContractorType();
		this.EnginDesc = aLSQuotEngineeringSchema.getEnginDesc();
		this.EnginCondition = aLSQuotEngineeringSchema.getEnginCondition();
		this.Remark = aLSQuotEngineeringSchema.getRemark();
		this.MakeOperator = aLSQuotEngineeringSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLSQuotEngineeringSchema.getMakeDate());
		this.MakeTime = aLSQuotEngineeringSchema.getMakeTime();
		this.ModifyOperator = aLSQuotEngineeringSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLSQuotEngineeringSchema.getModifyDate());
		this.ModifyTime = aLSQuotEngineeringSchema.getModifyTime();
		this.EnginDays = aLSQuotEngineeringSchema.getEnginDays();
		this.MinEnginArea = aLSQuotEngineeringSchema.getMinEnginArea();
		this.MinEnginCost = aLSQuotEngineeringSchema.getMinEnginCost();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LSQuotEngineering表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LSQuotEngineeringSchema";
	...
	public LSQuotEngineeringSchema getSchema()
	{
		LSQuotEngineeringSchema aLSQuotEngineeringSchema = new LSQuotEngineeringSchema();
		aLSQuotEngineeringSchema.setSchema(this);
		return aLSQuotEngineeringSchema;
	}

	public LSQuotEngineeringDB getDB()
	{
		LSQuotEngineeringDB aDBOper = new LSQuotEngineeringDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotEngineering描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotEngineering>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			MinEnginArea = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,26,SysConst.PACKAGESPILTER))).doubleValue();
			MinEnginCost = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,27,SysConst.PACKAGESPILTER))).doubleValue();
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LSQuotEngineeringSchema other = (LSQuotEngineeringSchema)otherObject;
		return

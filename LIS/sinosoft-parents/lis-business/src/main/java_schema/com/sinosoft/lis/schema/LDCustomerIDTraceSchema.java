...
import com.sinosoft.lis.db.LDCustomerIDTraceDB;

/*
 * <p>ClassName: LDCustomerIDTraceSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 客户管理
 */
public class LDCustomerIDTraceSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LDCustomerIDTraceSchema.class);
	// @Field
	/** 轨迹号 */
	...
	// @Constructor
	public LDCustomerIDTraceSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LDCustomerIDTraceSchema cloned = (LDCustomerIDTraceSchema)super.clone();
	...
	{
		if(aTraceID!=null && aTraceID.length()>20)
			throw new IllegalArgumentException("轨迹号TraceID值"+aTraceID+"的长度"+aTraceID.length()+"大于最大值20");
		TraceID = aTraceID;
	}
	...
	*/
	...
	/**
	* 使用另外一个 LDCustomerIDTraceSchema 对象给 Schema 赋值
	* @param: aLDCustomerIDTraceSchema LDCustomerIDTraceSchema
	**/
	public void setSchema(LDCustomerIDTraceSchema aLDCustomerIDTraceSchema)
	{
		this.TraceID = aLDCustomerIDTraceSchema.getTraceID();
		this.CustomerNo = aLDCustomerIDTraceSchema.getCustomerNo();
		this.ContactNo = aLDCustomerIDTraceSchema.getContactNo();
		this.IDFlag = aLDCustomerIDTraceSchema.getIDFlag();
		this.IDType = aLDCustomerIDTraceSchema.getIDType();
		this.IDNo = aLDCustomerIDTraceSchema.getIDNo();
		this.IDValFlag = aLDCustomerIDTraceSchema.getIDValFlag();
		this.IDInitiateDate = fDate.getDate( aLDCustomerIDTraceSchema.getIDInitiateDate());
		this.IDExpiryDate = fDate.getDate( aLDCustomerIDTraceSchema.getIDExpiryDate());
		this.State = aLDCustomerIDTraceSchema.getState();
		this.Remark = aLDCustomerIDTraceSchema.getRemark();
		this.ManageCom = aLDCustomerIDTraceSchema.getManageCom();
		this.ComCode = aLDCustomerIDTraceSchema.getComCode();
		this.MakeOperator = aLDCustomerIDTraceSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLDCustomerIDTraceSchema.getMakeDate());
		this.MakeTime = aLDCustomerIDTraceSchema.getMakeTime();
		this.ModifyOperator = aLDCustomerIDTraceSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLDCustomerIDTraceSchema.getModifyDate());
		this.ModifyTime = aLDCustomerIDTraceSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LDCustomerIDTrace表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LDCustomerIDTraceSchema";
	...
	public LDCustomerIDTraceSchema getSchema()
	{
		LDCustomerIDTraceSchema aLDCustomerIDTraceSchema = new LDCustomerIDTraceSchema();
		aLDCustomerIDTraceSchema.setSchema(this);
		return aLDCustomerIDTraceSchema;
	}

	public LDCustomerIDTraceDB getDB()
	{
		LDCustomerIDTraceDB aDBOper = new LDCustomerIDTraceDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDCustomerIDTrace>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ContactNo= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,3,SysConst.PACKAGESPILTER))).intValue();
			IDFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			...
			IDValFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			IDInitiateDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8,SysConst.PACKAGESPILTER));
			IDExpiryDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9,SysConst.PACKAGESPILTER));
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LDCustomerIDTraceSchema other = (LDCustomerIDTraceSchema)otherObject;
		return

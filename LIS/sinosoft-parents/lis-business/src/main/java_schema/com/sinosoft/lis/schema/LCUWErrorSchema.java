...
/*
 * <p>ClassName: LCUWErrorSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCUWErrorSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LCUWErrorSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LCUWErrorSchema cloned = (LCUWErrorSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LCUWErrorSchema 对象给 Schema 赋值
	* @param: aLCUWErrorSchema LCUWErrorSchema
	**/
	public void setSchema(LCUWErrorSchema aLCUWErrorSchema)
	{
		this.GrpContNo = aLCUWErrorSchema.getGrpContNo();
		this.ContNo = aLCUWErrorSchema.getContNo();
		this.ProposalContNo = aLCUWErrorSchema.getProposalContNo();
		this.PolNo = aLCUWErrorSchema.getPolNo();
		this.ProposalNo = aLCUWErrorSchema.getProposalNo();
		this.UWNo = aLCUWErrorSchema.getUWNo();
		this.SerialNo = aLCUWErrorSchema.getSerialNo();
		this.InsuredNo = aLCUWErrorSchema.getInsuredNo();
		this.InsuredName = aLCUWErrorSchema.getInsuredName();
		this.AppntNo = aLCUWErrorSchema.getAppntNo();
		this.AppntName = aLCUWErrorSchema.getAppntName();
		this.ManageCom = aLCUWErrorSchema.getManageCom();
		this.UWRuleCode = aLCUWErrorSchema.getUWRuleCode();
		this.UWError = aLCUWErrorSchema.getUWError();
		this.CurrValue = aLCUWErrorSchema.getCurrValue();
		this.ModifyDate = fDate.getDate( aLCUWErrorSchema.getModifyDate());
		this.ModifyTime = aLCUWErrorSchema.getModifyTime();
		this.UWPassFlag = aLCUWErrorSchema.getUWPassFlag();
		this.UWGrade = aLCUWErrorSchema.getUWGrade();
		this.SugPassFlag = aLCUWErrorSchema.getSugPassFlag();
		this.UWType = aLCUWErrorSchema.getUWType();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LCUWError表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LCUWErrorSchema";
	...
	public LCUWErrorSchema getSchema()
	{
		LCUWErrorSchema aLCUWErrorSchema = new LCUWErrorSchema();
		aLCUWErrorSchema.setSchema(this);
		return aLCUWErrorSchema;
	}

	public LCUWErrorDB getDB()
	{
		LCUWErrorDB aDBOper = new LCUWErrorDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCUWError描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCUWError>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			UWRuleCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			UWError = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			CurrValue = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			...
			UWPassFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 18, SysConst.PACKAGESPILTER );
			UWGrade = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 19, SysConst.PACKAGESPILTER );
			SugPassFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20, SysConst.PACKAGESPILTER );
			UWType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 21, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LCUWErrorSchema other = (LCUWErrorSchema)otherObject;
		return

...
{
private static Logger logger = Logger.getLogger(LDCodeRelaDB.class);
	...
	**/
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRelaType() == null || this.getRelaType().equals("null")) {
				...
				pstmt.setString(7, this.getRelaType());
			}
			if(this.getCode1() == null || this.getCode1().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getCode1());
			}
			if(this.getCode2() == null || this.getCode2().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getCode2());
			}
			if(this.getCode3() == null || this.getCode3().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getCode3());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDCodeRela WHERE  RelaType = ? AND Code1 = ? AND Code2 = ? AND Code3 = ?", 
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDCodeRela " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDCodeRelaSet
 */
public LDCodeRelaSet getData()
{
    int tCount = 0;
    LDCodeRelaSet tLDCodeRelaSet = new LDCodeRelaSet();
    LDCodeRelaSchema tLDCodeRelaSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDCodeRelaDB";
    ...
    {
        tCount = 1;
        tLDCodeRelaSchema = new LDCodeRelaSchema();
        tLDCodeRelaSchema.setSchema(mResultSet, 1);
        tLDCodeRelaSet.add(tLDCodeRelaSchema);
...
public boolean closeData()
{
	...
	public LDCodeRelaSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDCodeRelaSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

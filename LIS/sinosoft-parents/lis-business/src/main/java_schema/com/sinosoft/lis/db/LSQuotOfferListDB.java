...
/*
 * <p>ClassName: LSQuotOfferListDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LSQuotOfferListDB extends LSQuotOfferListSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LSQuotOfferListDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LSQuotOfferListSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LSQuotOfferListDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOfferListNo() == null || this.getOfferListNo().equals("null")) {
				...
				pstmt.setString(17, this.getOfferListNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LSQuotOfferList WHERE  OfferListNo = ?", 
	...
	public LSQuotOfferListSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LSQuotOfferListSet aLSQuotOfferListSet = new LSQuotOfferListSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LSQuotOfferList");
			LSQuotOfferListSchema aSchema = this.getSchema();
			...
			{
				i++;
				LSQuotOfferListSchema s1 = new LSQuotOfferListSchema();
				s1.setSchema(rs,i);
				aLSQuotOfferListSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLSQuotOfferListSet;
	}

	public LSQuotOfferListSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LSQuotOfferListSet aLSQuotOfferListSet = new LSQuotOfferListSet();
	...
	public LSQuotOfferListSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LSQuotOfferListSet aLSQuotOfferListSet = new LSQuotOfferListSet();
	...
	public LSQuotOfferListSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LSQuotOfferListSet aLSQuotOfferListSet = new LSQuotOfferListSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LSQuotOfferList " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LSQuotOfferListSet
 */
public LSQuotOfferListSet getData()
{
    int tCount = 0;
    LSQuotOfferListSet tLSQuotOfferListSet = new LSQuotOfferListSet();
    LSQuotOfferListSchema tLSQuotOfferListSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LSQuotOfferListDB";
    ...
    {
        tCount = 1;
        tLSQuotOfferListSchema = new LSQuotOfferListSchema();
        tLSQuotOfferListSchema.setSchema(mResultSet, 1);
        tLSQuotOfferListSet.add(tLSQuotOfferListSchema);
...
public boolean closeData()
{
	...
	public LSQuotOfferListSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LSQuotOfferListSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

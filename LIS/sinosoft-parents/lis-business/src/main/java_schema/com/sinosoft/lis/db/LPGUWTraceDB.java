...
import com.sinosoft.lis.schema.LPGUWTraceSchema;
import com.sinosoft.lis.vschema.LPGUWTraceSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LPGUWTraceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LPGUWTraceDB extends LPGUWTraceSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPGUWTraceDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LPGUWTrace" );
		mflag = true;
	}
	...
	{
		LPGUWTraceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPGUWTraceDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LPGUWTrace WHERE  EdorNo = ? AND EdorType = ? AND GrpContNo = ? AND RiskCode = ? AND RuleLevel = ? AND UWNo = ? AND RuleCode = ?");
		...
		SQLString sqlObj = new SQLString("LPGUWTrace");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				...
				pstmt.setString(36, this.getRiskCode());
			}
			if(this.getRuleLevel() == null || this.getRuleLevel().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getRuleLevel());
			}
			pstmt.setInt(38, this.getUWNo());
			if(this.getRuleCode() == null || this.getRuleCode().equals("null")) {
				pstmt.setNull(39, 12);
			} else {
				pstmt.setString(39, this.getRuleCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LPGUWTrace VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPGUWTrace WHERE  EdorNo = ? AND EdorType = ? AND GrpContNo = ? AND RiskCode = ? AND RuleLevel = ? AND UWNo = ? AND RuleCode = ?", 
	...
	public LPGUWTraceSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPGUWTraceSet aLPGUWTraceSet = new LPGUWTraceSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPGUWTrace");
			LPGUWTraceSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPGUWTraceSchema s1 = new LPGUWTraceSchema();
				s1.setSchema(rs,i);
				aLPGUWTraceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPGUWTraceSet;
	}

	public LPGUWTraceSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGUWTraceSet aLPGUWTraceSet = new LPGUWTraceSet();
	...
	public LPGUWTraceSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPGUWTraceSet aLPGUWTraceSet = new LPGUWTraceSet();
	...
	public LPGUWTraceSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGUWTraceSet aLPGUWTraceSet = new LPGUWTraceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPGUWTrace " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPGUWTraceSet
 */
public LPGUWTraceSet getData()
{
    int tCount = 0;
    LPGUWTraceSet tLPGUWTraceSet = new LPGUWTraceSet();
    LPGUWTraceSchema tLPGUWTraceSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPGUWTraceDB";
    ...
    {
        tCount = 1;
        tLPGUWTraceSchema = new LPGUWTraceSchema();
        tLPGUWTraceSchema.setSchema(mResultSet, 1);
        tLPGUWTraceSet.add(tLPGUWTraceSchema);
...
public boolean closeData()
{
	...
	public LPGUWTraceSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPGUWTraceSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LLCUWBatchDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLCUWBatchDB extends LLCUWBatchSchema
{
private static Logger logger = Logger.getLogger(LLCUWBatchDB.class);
	...
	**/
	...
	// @Constructor
	public LLCUWBatchDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LLCUWBatch" );
		mflag = true;
	}
	...
	{
		LLCUWBatchSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLCUWBatchDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LLCUWBatch WHERE  CaseNo = ? AND BatNo = ? AND ContNo = ?");
		...
		SQLString sqlObj = new SQLString("LLCUWBatch");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LLCUWBatch SET  CaseNo = ? , BatNo = ? , GrpContNo = ? , ContNo = ? , ProposalContNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , ClaimRelFlag = ? , HealthImpartNo1 = ? , HealthImpartNo2 = ? , NoImpartDesc = ? , Remark1 = ? , Remark2 = ? , InEffectFlag = ? , UWOperator = ? WHERE  CaseNo = ? AND BatNo = ? AND ContNo = ?");
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCaseNo());
			...
			if(this.getClaimRelFlag() == null || this.getClaimRelFlag().equals("null")) {
				...
				pstmt.setString(39, this.getClaimRelFlag());
			}
			if(this.getHealthImpartNo1() == null || this.getHealthImpartNo1().equals("null")) {
				pstmt.setNull(40, 12);
			} else {
				pstmt.setString(40, this.getHealthImpartNo1());
			}
			if(this.getHealthImpartNo2() == null || this.getHealthImpartNo2().equals("null")) {
				pstmt.setNull(41, 12);
			} else {
				pstmt.setString(41, this.getHealthImpartNo2());
			}
			if(this.getNoImpartDesc() == null || this.getNoImpartDesc().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
				pstmt.setString(42, this.getNoImpartDesc());
			}
			if(this.getRemark1() == null || this.getRemark1().equals("null")) {
				pstmt.setNull(43, 12);
			} else {
				pstmt.setString(43, this.getRemark1());
			}
			if(this.getRemark2() == null || this.getRemark2().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
				pstmt.setString(44, this.getRemark2());
			}
			if(this.getInEffectFlag() == null || this.getInEffectFlag().equals("null")) {
				pstmt.setNull(45, 12);
			} else {
				pstmt.setString(45, this.getInEffectFlag());
			}
			...
			if(this.getBatNo() == null || this.getBatNo().equals("null")) {
				...
				pstmt.setString(48, this.getBatNo());
			}
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(49, 12);
			} else {
				pstmt.setString(49, this.getContNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LLCUWBatch(CaseNo ,BatNo ,GrpContNo ,ContNo ,ProposalContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea ,ClaimRelFlag ,HealthImpartNo1 ,HealthImpartNo2 ,NoImpartDesc ,Remark1 ,Remark2 ,InEffectFlag ,UWOperator) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCaseNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLCUWBatch WHERE  CaseNo = ? AND BatNo = ? AND ContNo = ?", 
	...
	public LLCUWBatchSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLCUWBatchSet aLLCUWBatchSet = new LLCUWBatchSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLCUWBatch");
			LLCUWBatchSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLCUWBatchSchema s1 = new LLCUWBatchSchema();
				s1.setSchema(rs,i);
				aLLCUWBatchSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLCUWBatchSet;

	}

	public LLCUWBatchSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLCUWBatchSet aLLCUWBatchSet = new LLCUWBatchSet();
	...
	public LLCUWBatchSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLCUWBatchSet aLLCUWBatchSet = new LLCUWBatchSet();
	...
	public LLCUWBatchSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLCUWBatchSet aLLCUWBatchSet = new LLCUWBatchSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLCUWBatch " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLCUWBatchSet
 */
public LLCUWBatchSet getData()
{
    int tCount = 0;
    LLCUWBatchSet tLLCUWBatchSet = new LLCUWBatchSet();
    LLCUWBatchSchema tLLCUWBatchSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLCUWBatchDB";
    ...
    {
        tCount = 1;
        tLLCUWBatchSchema = new LLCUWBatchSchema();
        tLLCUWBatchSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLCUWBatchSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLCUWBatchSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LPGrpFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPGrpFeeDB extends LPGrpFeeSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPGrpFeeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPGrpFeeSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPayInsuAccName() == null || this.getPayInsuAccName().equals("null")) {
				...
				pstmt.setString(9, this.getPayInsuAccName());
			}
			...
			if(this.getPeriodType() == null || this.getPeriodType().equals("null")) {
				...
				pstmt.setString(24, this.getPeriodType());
			}
			...
			if(this.getFeeBaseType() == null || this.getFeeBaseType().equals("null")) {
				...
				pstmt.setString(27, this.getFeeBaseType());
			}
			pstmt.setInt(28, this.getFeeNum());
			if(this.getFeeTakePlace() == null || this.getFeeTakePlace().equals("null")) {
				pstmt.setNull(29, 1);
			} else {
				pstmt.setString(29, this.getFeeTakePlace());
			}
			if(this.getValPeriod() == null || this.getValPeriod().equals("null")) {
				pstmt.setNull(30, 12);
			} else {
				pstmt.setString(30, this.getValPeriod());
			}
			if(this.getValStartDate() == null || this.getValStartDate().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
				pstmt.setString(31, this.getValStartDate());
			}
			if(this.getValEndDate() == null || this.getValEndDate().equals("null")) {
				pstmt.setNull(32, 12);
			} else {
				pstmt.setString(32, this.getValEndDate());
			}
			if(this.getManageCom() == null || this.getManageCom().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
				pstmt.setString(33, this.getManageCom());
			}
			...
			if(this.getGrpPolNo() == null || this.getGrpPolNo().equals("null")) {
				...
				pstmt.setString(38, this.getGrpPolNo());
			}
			if(this.getFeeCode() == null || this.getFeeCode().equals("null")) {
				pstmt.setNull(39, 12);
			} else {
			...
			if(this.getInsuAccNo() == null || this.getInsuAccNo().equals("null")) {
				...
				pstmt.setString(40, this.getInsuAccNo());
			}
			if(this.getPayPlanCode() == null || this.getPayPlanCode().equals("null")) {
				pstmt.setNull(41, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPGrpFee WHERE  EdorNo = ? AND EdorType = ? AND GrpPolNo = ? AND FeeCode = ? AND InsuAccNo = ? AND PayPlanCode = ?", 
	...
	public LPGrpFeeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPGrpFeeSet aLPGrpFeeSet = new LPGrpFeeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPGrpFee");
			LPGrpFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPGrpFeeSchema s1 = new LPGrpFeeSchema();
				s1.setSchema(rs,i);
				aLPGrpFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LPGrpFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGrpFeeSet aLPGrpFeeSet = new LPGrpFeeSet();
	...
	public LPGrpFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPGrpFeeSet aLPGrpFeeSet = new LPGrpFeeSet();
	...
	public LPGrpFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGrpFeeSet aLPGrpFeeSet = new LPGrpFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPGrpFee " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LPGrpFeeSet
 */
public LPGrpFeeSet getData()
{
    int tCount = 0;
    LPGrpFeeSet tLPGrpFeeSet = new LPGrpFeeSet();
    LPGrpFeeSchema tLPGrpFeeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPGrpFeeDB";
    ...
    {
        tCount = 1;
        tLPGrpFeeSchema = new LPGrpFeeSchema();
        tLPGrpFeeSchema.setSchema(mResultSet, 1);
        tLPGrpFeeSet.add(tLPGrpFeeSchema);
...
public boolean closeData()
{
	...
	public LPGrpFeeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPGrpFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

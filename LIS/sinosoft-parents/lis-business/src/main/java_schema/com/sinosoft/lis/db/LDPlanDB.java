...
/*
 * <p>ClassName: LDPlanDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 基础信息
 */
public class LDPlanDB extends LDPlanSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDPlanDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDPlanSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPlanRule() == null || this.getPlanRule().equals("null")) {
				...
				pstmt.setString(4, this.getPlanRule());
			}
			if(this.getPlanSql() == null || this.getPlanSql().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getPlanSql());
			}
			...
			pstmt.setInt(12, this.getPeoples3());
			...
			if(this.getPlanKind2() == null || this.getPlanKind2().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getPlanKind2());
			}
			if(this.getPlankind3() == null || this.getPlankind3().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getPlankind3());
			}
			if(this.getPlanKind1() == null || this.getPlanKind1().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getPlanKind1());
			}
			if(this.getPortfolioFlag() == null || this.getPortfolioFlag().equals("null")) {
				pstmt.setNull(22, 1);
			} else {
				pstmt.setString(22, this.getPortfolioFlag());
			}
			// set where condition
			if(this.getContPlanCode() == null || this.getContPlanCode().equals("null")) {
				pstmt.setNull(23, 12);
			...
			if(this.getPlanType() == null || this.getPlanType().equals("null")) {
				...
				pstmt.setString(24, StrTool.space1(this.getPlanType(), 1));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDPlan WHERE  ContPlanCode = ? AND PlanType = ?", 
	...
	public LDPlanSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDPlanSet aLDPlanSet = new LDPlanSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDPlan");
			LDPlanSchema aSchema = this.getSchema();
			...
			{
				i++;
				LDPlanSchema s1 = new LDPlanSchema();
				s1.setSchema(rs,i);
				aLDPlanSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LDPlanSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDPlanSet aLDPlanSet = new LDPlanSet();
	...
	public LDPlanSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDPlanSet aLDPlanSet = new LDPlanSet();
	...
	public LDPlanSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDPlanSet aLDPlanSet = new LDPlanSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDPlan " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LDPlanSet
 */
public LDPlanSet getData()
{
    int tCount = 0;
    LDPlanSet tLDPlanSet = new LDPlanSet();
    LDPlanSchema tLDPlanSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDPlanDB";
    ...
    {
        tCount = 1;
        tLDPlanSchema = new LDPlanSchema();
        tLDPlanSchema.setSchema(mResultSet, 1);
        tLDPlanSet.add(tLDPlanSchema);
...
public boolean closeData()
{
	...
	public LDPlanSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDPlanSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

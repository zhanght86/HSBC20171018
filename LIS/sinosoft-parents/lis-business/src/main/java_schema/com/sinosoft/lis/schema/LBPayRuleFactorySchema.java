...
import com.sinosoft.lis.db.LBPayRuleFactoryDB;

/*
 * <p>ClassName: LBPayRuleFactorySchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统未整理模型
 */
public class LBPayRuleFactorySchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBPayRuleFactorySchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBPayRuleFactorySchema cloned = (LBPayRuleFactorySchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LBPayRuleFactorySchema 对象给 Schema 赋值
	* @param: aLBPayRuleFactorySchema LBPayRuleFactorySchema
	**/
	public void setSchema(LBPayRuleFactorySchema aLBPayRuleFactorySchema)
	{
		this.EdorNo = aLBPayRuleFactorySchema.getEdorNo();
		this.GrpPolNo = aLBPayRuleFactorySchema.getGrpPolNo();
		this.GrpContNo = aLBPayRuleFactorySchema.getGrpContNo();
		this.RiskCode = aLBPayRuleFactorySchema.getRiskCode();
		this.PayRuleCode = aLBPayRuleFactorySchema.getPayRuleCode();
		this.PayRuleName = aLBPayRuleFactorySchema.getPayRuleName();
		this.FactoryType = aLBPayRuleFactorySchema.getFactoryType();
		this.OtherNo = aLBPayRuleFactorySchema.getOtherNo();
		this.FactoryCode = aLBPayRuleFactorySchema.getFactoryCode();
		this.FactorySubCode = aLBPayRuleFactorySchema.getFactorySubCode();
		this.FactoryName = aLBPayRuleFactorySchema.getFactoryName();
		this.InerSerialNo = aLBPayRuleFactorySchema.getInerSerialNo();
		this.CalSql = aLBPayRuleFactorySchema.getCalSql();
		this.Params = aLBPayRuleFactorySchema.getParams();
		this.CalRemark = aLBPayRuleFactorySchema.getCalRemark();
		this.Operator = aLBPayRuleFactorySchema.getOperator();
		this.MakeDate = fDate.getDate( aLBPayRuleFactorySchema.getMakeDate());
		this.MakeTime = aLBPayRuleFactorySchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLBPayRuleFactorySchema.getModifyDate());
		this.ModifyTime = aLBPayRuleFactorySchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBPayRuleFactory表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBPayRuleFactorySchema";
	...
	public LBPayRuleFactorySchema getSchema()
	{
		LBPayRuleFactorySchema aLBPayRuleFactorySchema = new LBPayRuleFactorySchema();
		aLBPayRuleFactorySchema.setSchema(this);
		return aLBPayRuleFactorySchema;
	}

	public LBPayRuleFactoryDB getDB()
	{
		LBPayRuleFactoryDB aDBOper = new LBPayRuleFactoryDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBPayRuleFactory>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			PayRuleCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			PayRuleName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			FactoryType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			OtherNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			FactoryCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			FactorySubCode= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,10,SysConst.PACKAGESPILTER))).intValue();
			FactoryName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			InerSerialNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			CalSql = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			Params = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			CalRemark = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBPayRuleFactorySchema other = (LBPayRuleFactorySchema)otherObject;
		return

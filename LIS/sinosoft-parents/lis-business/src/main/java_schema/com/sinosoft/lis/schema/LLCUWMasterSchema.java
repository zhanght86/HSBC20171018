...
/*
 * <p>ClassName: LLCUWMasterSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLCUWMasterSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLCUWMasterSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLCUWMasterSchema cloned = (LLCUWMasterSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LLCUWMasterSchema 对象给 Schema 赋值
	* @param: aLLCUWMasterSchema LLCUWMasterSchema
	**/
	public void setSchema(LLCUWMasterSchema aLLCUWMasterSchema)
	{
		this.CaseNo = aLLCUWMasterSchema.getCaseNo();
		this.GrpContNo = aLLCUWMasterSchema.getGrpContNo();
		this.BatNo = aLLCUWMasterSchema.getBatNo();
		this.ContNo = aLLCUWMasterSchema.getContNo();
		this.ProposalContNo = aLLCUWMasterSchema.getProposalContNo();
		this.UWNo = aLLCUWMasterSchema.getUWNo();
		this.InsuredNo = aLLCUWMasterSchema.getInsuredNo();
		this.InsuredName = aLLCUWMasterSchema.getInsuredName();
		this.AppntNo = aLLCUWMasterSchema.getAppntNo();
		this.AppntName = aLLCUWMasterSchema.getAppntName();
		this.AgentCode = aLLCUWMasterSchema.getAgentCode();
		this.AgentGroup = aLLCUWMasterSchema.getAgentGroup();
		this.PassFlag = aLLCUWMasterSchema.getPassFlag();
		this.UWGrade = aLLCUWMasterSchema.getUWGrade();
		this.AppGrade = aLLCUWMasterSchema.getAppGrade();
		this.PostponeDay = aLLCUWMasterSchema.getPostponeDay();
		this.PostponeDate = fDate.getDate( aLLCUWMasterSchema.getPostponeDate());
		this.AutoUWFlag = aLLCUWMasterSchema.getAutoUWFlag();
		this.State = aLLCUWMasterSchema.getState();
		this.ManageCom = aLLCUWMasterSchema.getManageCom();
		this.UWIdea = aLLCUWMasterSchema.getUWIdea();
		this.UpReportContent = aLLCUWMasterSchema.getUpReportContent();
		this.Operator = aLLCUWMasterSchema.getOperator();
		this.HealthFlag = aLLCUWMasterSchema.getHealthFlag();
		this.QuesFlag = aLLCUWMasterSchema.getQuesFlag();
		this.SpecFlag = aLLCUWMasterSchema.getSpecFlag();
		this.AddPremFlag = aLLCUWMasterSchema.getAddPremFlag();
		this.AddPremReason = aLLCUWMasterSchema.getAddPremReason();
		this.ReportFlag = aLLCUWMasterSchema.getReportFlag();
		this.PrintFlag = aLLCUWMasterSchema.getPrintFlag();
		this.PrintFlag2 = aLLCUWMasterSchema.getPrintFlag2();
		this.ChangePolFlag = aLLCUWMasterSchema.getChangePolFlag();
		this.ChangePolReason = aLLCUWMasterSchema.getChangePolReason();
		this.SpecReason = aLLCUWMasterSchema.getSpecReason();
		this.MakeDate = fDate.getDate( aLLCUWMasterSchema.getMakeDate());
		this.MakeTime = aLLCUWMasterSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLLCUWMasterSchema.getModifyDate());
		this.ModifyTime = aLLCUWMasterSchema.getModifyTime();
		this.SugPassFlag = aLLCUWMasterSchema.getSugPassFlag();
		this.SugUWIdea = aLLCUWMasterSchema.getSugUWIdea();
		this.ClaimRelFlag = aLLCUWMasterSchema.getClaimRelFlag();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLCUWMaster表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLCUWMasterSchema";
	...
	public LLCUWMasterSchema getSchema()
	{
		LLCUWMasterSchema aLLCUWMasterSchema = new LLCUWMasterSchema();
		aLLCUWMasterSchema.setSchema(this);
		return aLLCUWMasterSchema;
	}

	public LLCUWMasterDB getDB()
	{
		LLCUWMasterDB aDBOper = new LLCUWMasterDB();
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(ClaimRelFlag));
		return strReturn.toString();
	}

	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLCUWMaster>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ClaimRelFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 41, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LLCUWMasterSchema other = (LLCUWMasterSchema)otherObject;
		return

...
/*
 * <p>ClassName: LCUWSubBakDBSet </p>
 * <p>Description: DB层多记录数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LCUWSubBakDBSet extends LCUWSubBakSet
{
private static Logger logger = Logger.getLogger(LCUWSubBakDBSet.class);
	...
	**/
	...
	// @Constructor
	public LCUWSubBakDBSet(Connection tConnection)
	{
		con = tConnection;
		db = new DBOper(con,"LCUWSubBak");
		mflag = true;
	}

	public LCUWSubBakDBSet()
	{
		db = new DBOper( "LCUWSubBak" );
	}
	// @Method
	public boolean deleteSQL()
	{
		...
		else
		{
			...
			tError.moduleName = "LCUWSubBakDBSet";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt = con.prepareStatement("DELETE FROM LCUWSubBak WHERE  ProposalNo = ? AND UWNo = ?");
		...
		SQLString sqlObj = new SQLString("LCUWSubBak");
...
ex.printStackTrace();
	...
	public boolean update()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("UPDATE LCUWSubBak SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , PolNo = ? , ProposalNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , UpReport = ? , UWType = ? WHERE  ProposalNo = ? AND UWNo = ?");
            for (int i = 1; i <= tCount; i++)
            {
			...
			if(this.get(i).getUWType() == null || this.get(i).getUWType().equals("null")) {
				...
				pstmt.setString(42, this.get(i).getUWType());
			}
			// set where condition
			if(this.get(i).getProposalNo() == null || this.get(i).getProposalNo().equals("null")) {
				pstmt.setString(43,null);
			} else {
				pstmt.setString(43, this.get(i).getProposalNo());
			}
			pstmt.setInt(44, this.get(i).getUWNo());
...
ex.printStackTrace();
	...
	public boolean insert()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("INSERT INTO LCUWSubBak(GrpContNo ,ContNo ,ProposalContNo ,PolNo ,ProposalNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea ,UpReport ,UWType) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getGrpContNo() == null || this.get(i).getGrpContNo().equals("null")) {
				pstmt.setString(1,null);

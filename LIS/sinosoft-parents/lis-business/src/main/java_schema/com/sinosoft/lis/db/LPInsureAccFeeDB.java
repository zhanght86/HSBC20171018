...
/*
 * <p>ClassName: LPInsureAccFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPInsureAccFeeDB extends LPInsureAccFeeSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPInsureAccFeeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPInsureAccFeeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPInsureAccFeeDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOwner() == null || this.getOwner().equals("null")) {
				...
				pstmt.setString(16, this.getOwner());
			}
			...
			if(this.getBalaDate() == null || this.getBalaDate().equals("null")) {
				...
				pstmt.setDate(23, Date.valueOf(this.getBalaDate()));
			}
			if(this.getBalaTime() == null || this.getBalaTime().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getBalaTime());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPInsureAccFee WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ? AND InsuAccNo = ?", 
	...
	public LPInsureAccFeeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPInsureAccFee");
			LPInsureAccFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPInsureAccFeeSchema s1 = new LPInsureAccFeeSchema();
				s1.setSchema(rs,i);
				aLPInsureAccFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPInsureAccFeeSet;
	}

	public LPInsureAccFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPInsureAccFeeSet aLPInsureAccFeeSet = new LPInsureAccFeeSet();
	...
	public LPInsureAccFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPInsureAccFeeSet aLPInsureAccFeeSet = new LPInsureAccFeeSet();
	...
	public LPInsureAccFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPInsureAccFeeSet aLPInsureAccFeeSet = new LPInsureAccFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPInsureAccFee " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPInsureAccFeeSet
 */
public LPInsureAccFeeSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLPInsureAccFeeSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LPInsureAccFeeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPInsureAccFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

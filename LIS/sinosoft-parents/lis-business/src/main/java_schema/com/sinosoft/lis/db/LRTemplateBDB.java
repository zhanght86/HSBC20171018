...
/*
 * <p>ClassName: LRTemplateBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LRTemplateBDB extends LRTemplateBSchema
{
	private static Logger logger = Logger.getLogger(LRTemplateBDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LRTemplateBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LRTemplateBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LRTemplateBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setInt(32, this.getVersion());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRTemplateB WHERE  Id = ? AND Version = ?", 
	...
	public LRTemplateBSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRTemplateBSet aLRTemplateBSet = new LRTemplateBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRTemplateB");
			LRTemplateBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LRTemplateBSchema s1 = new LRTemplateBSchema();
				s1.setSchema(rs,i);
				aLRTemplateBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLRTemplateBSet;
	}

	public LRTemplateBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LRTemplateBSet aLRTemplateBSet = new LRTemplateBSet();
	...
	public LRTemplateBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRTemplateBSet aLRTemplateBSet = new LRTemplateBSet();
	...
	public LRTemplateBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LRTemplateBSet aLRTemplateBSet = new LRTemplateBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRTemplateB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LRTemplateBSet
 */
public LRTemplateBSet getData()
{
    int tCount = 0;
    LRTemplateBSet tLRTemplateBSet = new LRTemplateBSet();
    LRTemplateBSchema tLRTemplateBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRTemplateBDB";
    ...
    {
        tCount = 1;
        tLRTemplateBSchema = new LRTemplateBSchema();
        tLRTemplateBSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LRTemplateBSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LRTemplateBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

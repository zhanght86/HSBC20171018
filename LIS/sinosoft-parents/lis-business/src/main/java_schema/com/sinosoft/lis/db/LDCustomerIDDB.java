...
/*
 * <p>ClassName: LDCustomerIDDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 客户管理
 */
public class LDCustomerIDDB extends LDCustomerIDSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDCustomerIDDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDCustomerIDSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LDCustomerIDDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getIDValFlag() == null || this.getIDValFlag().equals("null")) {
				...
				pstmt.setString(6, this.getIDValFlag());
			}
			if(this.getIDInitiateDate() == null || this.getIDInitiateDate().equals("null")) {
				pstmt.setNull(7, 91);
			} else {
				pstmt.setDate(7, Date.valueOf(this.getIDInitiateDate()));
			}
			...
			pstmt.setInt(20, this.getContactNo());
			if(this.getIDType() == null || this.getIDType().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDCustomerID WHERE  CustomerNo = ? AND ContactNo = ? AND IDType = ?", 
	...
	public LDCustomerIDSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDCustomerIDSet aLDCustomerIDSet = new LDCustomerIDSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDCustomerID");
			LDCustomerIDSchema aSchema = this.getSchema();
			...
			{
				i++;
				LDCustomerIDSchema s1 = new LDCustomerIDSchema();
				s1.setSchema(rs,i);
				aLDCustomerIDSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLDCustomerIDSet;
	}

	public LDCustomerIDSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDCustomerIDSet aLDCustomerIDSet = new LDCustomerIDSet();
	...
	public LDCustomerIDSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDCustomerIDSet aLDCustomerIDSet = new LDCustomerIDSet();
	...
	public LDCustomerIDSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDCustomerIDSet aLDCustomerIDSet = new LDCustomerIDSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDCustomerID " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDCustomerIDSet
 */
public LDCustomerIDSet getData()
{
    int tCount = 0;
    LDCustomerIDSet tLDCustomerIDSet = new LDCustomerIDSet();
    LDCustomerIDSchema tLDCustomerIDSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDCustomerIDDB";
    ...
    {
        tCount = 1;
        tLDCustomerIDSchema = new LDCustomerIDSchema();
        tLDCustomerIDSchema.setSchema(mResultSet, 1);
        tLDCustomerIDSet.add(tLDCustomerIDSchema);
...
public boolean closeData()
{
	...
	public LDCustomerIDSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDCustomerIDSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

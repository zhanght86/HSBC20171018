...
/*
 * <p>ClassName: LARateCommisionBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 代理人管理
 */
public class LARateCommisionBDB extends LARateCommisionBSchema
{
private static Logger logger = Logger.getLogger(LARateCommisionBDB.class);
	...
	**/
	...
	// @Constructor
	public LARateCommisionBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LARateCommisionBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LARateCommisionBDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getsex() == null || this.getsex().equals("null")) {
				...
				pstmt.setString(7, StrTool.space1(this.getsex(), 1));
			}
			pstmt.setInt(8, this.getAppAge());
			pstmt.setInt(9, this.getYear());
			if(this.getPayIntv() == null || this.getPayIntv().equals("null")) {
				pstmt.setNull(10, 12);
			} else {
				pstmt.setString(10, this.getPayIntv());
			}
			pstmt.setInt(11, this.getCurYear());
			if(this.getF01() == null || this.getF01().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getF01());
			}
			if(this.getF02() == null || this.getF02().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getF02());
			}
			pstmt.setDouble(14, this.getF03());
			pstmt.setDouble(15, this.getF04());
			if(this.getF05() == null || this.getF05().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
				pstmt.setString(16, this.getF05());
			}
			if(this.getF06() == null || this.getF06().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getF06());
			}
			...
			if(this.getVersionType() == null || this.getVersionType().equals("null")) {
				...
				pstmt.setString(19, this.getVersionType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(18, this.getrate());
			...
			if(this.getVersionType() == null || this.getVersionType().equals("null")) {
				...
				pstmt.setString(25, this.getVersionType());
			}
			// set where condition
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(26, 12);
			...
			if(this.getBranchType() == null || this.getBranchType().equals("null")) {
				...
				pstmt.setString(28, this.getBranchType());
			}
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
			...
			if(this.getsex() == null || this.getsex().equals("null")) {
				...
				pstmt.setString(32, StrTool.space1(this.getsex(), 1));
			}
			pstmt.setInt(33, this.getAppAge());
			pstmt.setInt(34, this.getYear());
			if(this.getPayIntv() == null || this.getPayIntv().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getPayIntv());
			}
			pstmt.setInt(36, this.getCurYear());
			if(this.getF01() == null || this.getF01().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getF01());
			}
			if(this.getF02() == null || this.getF02().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getF02());
			}
			pstmt.setDouble(39, this.getF03());
			pstmt.setDouble(40, this.getF04());
			if(this.getF05() == null || this.getF05().equals("null")) {
				pstmt.setNull(41, 12);
			} else {
				pstmt.setString(41, this.getF05());
			}
			if(this.getF06() == null || this.getF06().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
				pstmt.setString(42, this.getF06());
			}
			if(this.getManageCom() == null || this.getManageCom().equals("null")) {
				pstmt.setNull(43, 12);
			} else {
				pstmt.setString(43, this.getManageCom());
			}
			if(this.getVersionType() == null || this.getVersionType().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
				pstmt.setString(44, this.getVersionType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LARateCommisionB WHERE  EdorNo = ? AND EdorType = ? AND BranchType = ? AND RiskCode = ? AND StartDate = ? AND EndDate = ? AND sex = ? AND AppAge = ? AND Year = ? AND PayIntv = ? AND CurYear = ? AND F01 = ? AND F02 = ? AND F03 = ? AND F04 = ? AND F05 = ? AND F06 = ? AND ManageCom = ? AND VersionType = ?", 
	...
	public LARateCommisionBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LARateCommisionBSet aLARateCommisionBSet = new LARateCommisionBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LARateCommisionB");
			LARateCommisionBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LARateCommisionBSchema s1 = new LARateCommisionBSchema();
				s1.setSchema(rs,i);
				aLARateCommisionBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLARateCommisionBSet;

	}

	public LARateCommisionBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LARateCommisionBSet aLARateCommisionBSet = new LARateCommisionBSet();
	...
	public LARateCommisionBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LARateCommisionBSet aLARateCommisionBSet = new LARateCommisionBSet();
	...
	public LARateCommisionBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LARateCommisionBSet aLARateCommisionBSet = new LARateCommisionBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LARateCommisionB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LARateCommisionBSet
 */
public LARateCommisionBSet getData()
{
    int tCount = 0;
    LARateCommisionBSet tLARateCommisionBSet = new LARateCommisionBSet();
    LARateCommisionBSchema tLARateCommisionBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LARateCommisionBDB";
    ...
    {
        tCount = 1;
        tLARateCommisionBSchema = new LARateCommisionBSchema();
        tLARateCommisionBSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LARateCommisionBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LARateCommisionBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

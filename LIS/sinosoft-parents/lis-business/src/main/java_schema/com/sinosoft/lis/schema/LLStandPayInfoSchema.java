...
import com.sinosoft.lis.db.LLStandPayInfoDB;

/*
 * <p>ClassName: LLStandPayInfoSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LLStandPayInfoSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLStandPayInfoSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLStandPayInfoSchema cloned = (LLStandPayInfoSchema)super.clone();
	...
	/**
	* 使用另外一个 LLStandPayInfoSchema 对象给 Schema 赋值
	* @param: aLLStandPayInfoSchema LLStandPayInfoSchema
	**/
	public void setSchema(LLStandPayInfoSchema aLLStandPayInfoSchema)
	{
		this.CaseNo = aLLStandPayInfoSchema.getCaseNo();
		this.RiskCode = aLLStandPayInfoSchema.getRiskCode();
		this.StandPay = aLLStandPayInfoSchema.getStandPay();
		this.Flag = aLLStandPayInfoSchema.getFlag();
		this.ManageCom = aLLStandPayInfoSchema.getManageCom();
		this.Operator = aLLStandPayInfoSchema.getOperator();
		this.MakeDate = fDate.getDate( aLLStandPayInfoSchema.getMakeDate());
		this.MakeTime = aLLStandPayInfoSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLLStandPayInfoSchema.getModifyDate());
		this.ModifyTime = aLLStandPayInfoSchema.getModifyTime();
		this.Currency = aLLStandPayInfoSchema.getCurrency();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLStandPayInfo表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLStandPayInfoSchema";
	...
	public LLStandPayInfoSchema getSchema()
	{
		LLStandPayInfoSchema aLLStandPayInfoSchema = new LLStandPayInfoSchema();
		aLLStandPayInfoSchema.setSchema(this);
		return aLLStandPayInfoSchema;
	}

	public LLStandPayInfoDB getDB()
	{
		LLStandPayInfoDB aDBOper = new LLStandPayInfoDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLStandPayInfo>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			StandPay = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,3,SysConst.PACKAGESPILTER))).doubleValue();
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LLStandPayInfoSchema other = (LLStandPayInfoSchema)otherObject;
		return

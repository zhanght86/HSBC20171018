...
/*
 * <p>ClassName: LBPOContBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LBPOContBDB extends LBPOContBSchema
{
private static Logger logger = Logger.getLogger(LBPOContBDB.class);
	...
	**/
	...
	// @Constructor
	public LBPOContBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBPOContBSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getProposalContNo() == null || this.getProposalContNo().equals("null")) {
				...
				pstmt.setString(6, this.getProposalContNo());
			}
			...
			if(this.getFamilyType() == null || this.getFamilyType().equals("null")) {
				...
				pstmt.setString(9, this.getFamilyType());
			}
			...
			if(this.getPolType() == null || this.getPolType().equals("null")) {
				...
				pstmt.setString(11, this.getPolType());
			}
			if(this.getCardFlag() == null || this.getCardFlag().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getCardFlag());
			}
			...
			if(this.getExecuteCom() == null || this.getExecuteCom().equals("null")) {
				...
				pstmt.setString(14, this.getExecuteCom());
			}
			...
			if(this.getAgentCode1() == null || this.getAgentCode1().equals("null")) {
				...
				pstmt.setString(18, this.getAgentCode1());
			}
			...
			if(this.getConsignNo() == null || this.getConsignNo().equals("null")) {
				...
				pstmt.setString(32, this.getConsignNo());
			}
			...
			if(this.getPrintCount() == null || this.getPrintCount().equals("null")) {
				...
				pstmt.setString(36, this.getPrintCount());
			}
			if(this.getLostTimes() == null || this.getLostTimes().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getLostTimes());
			}
			...
			if(this.getApproveDate() == null || this.getApproveDate().equals("null")) {
				...
				pstmt.setString(55, this.getApproveDate());
			}
			...
			if(this.getUWDate() == null || this.getUWDate().equals("null")) {
				...
				pstmt.setString(59, this.getUWDate());
			}
			...
			if(this.getMakeDate() == null || this.getMakeDate().equals("null")) {
				...
				pstmt.setString(68, this.getMakeDate());
			}
			...
			if(this.getModifyDate() == null || this.getModifyDate().equals("null")) {
				...
				pstmt.setString(70, this.getModifyDate());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBPOContB WHERE  IDX = ? AND ContNo = ? AND InputNo = ? AND FillNo = ?", 
	...
	public LBPOContBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOContBSet aLBPOContBSet = new LBPOContBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBPOContB");
			LBPOContBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBPOContBSchema s1 = new LBPOContBSchema();
				s1.setSchema(rs,i);
				aLBPOContBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBPOContBSet;

	}

	public LBPOContBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOContBSet aLBPOContBSet = new LBPOContBSet();
	...
	public LBPOContBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOContBSet aLBPOContBSet = new LBPOContBSet();
	...
	public LBPOContBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOContBSet aLBPOContBSet = new LBPOContBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBPOContB " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LBPOContBSet
 */
public LBPOContBSet getData()
{
    int tCount = 0;
    LBPOContBSet tLBPOContBSet = new LBPOContBSet();
    LBPOContBSchema tLBPOContBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBPOContBDB";
    ...
    {
        tCount = 1;
        tLBPOContBSchema = new LBPOContBSchema();
        tLBPOContBSchema.setSchema(mResultSet, 1);
        tLBPOContBSet.add(tLBPOContBSchema);
...
public boolean closeData()
{
	...
	public LBPOContBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBPOContBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

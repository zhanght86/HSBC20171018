...
/*
 * <p>ClassName: LLAccidentDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLAccidentDB extends LLAccidentSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LLAccidentDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLAccidentSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLAccidentDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAccDate() == null || this.getAccDate().equals("null")) {
				...
				pstmt.setDate(2, Date.valueOf(this.getAccDate()));
			}
			...
			if(this.getAccSubject() == null || this.getAccSubject().equals("null")) {
				...
				pstmt.setString(4, this.getAccSubject());
			}
			if(this.getAccDesc() == null || this.getAccDesc().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getAccDesc());
			}
			if(this.getAccPlace() == null || this.getAccPlace().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getAccPlace());
			}
			if(this.getAccGrade() == null || this.getAccGrade().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getAccGrade());
			}
			...
			if(this.getRptNo() == null || this.getRptNo().equals("null")) {
				...
				pstmt.setString(15, this.getRptNo());
			}
			...
			if(this.getAccReason() == null || this.getAccReason().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getAccReason());
			}
			if(this.getDeathDate() == null || this.getDeathDate().equals("null")) {
				pstmt.setNull(18, 91);
			} else {
				pstmt.setDate(18, Date.valueOf(this.getDeathDate()));
			}
			if(this.getDeformityDate() == null || this.getDeformityDate().equals("null")) {
				pstmt.setNull(19, 91);
			} else {
				pstmt.setDate(19, Date.valueOf(this.getDeformityDate()));
			}
			if(this.getAccResult1() == null || this.getAccResult1().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
				pstmt.setString(20, this.getAccResult1());
			}
			if(this.getAccResult2() == null || this.getAccResult2().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
				pstmt.setString(21, this.getAccResult2());
			}
			if(this.getHospitalCode() == null || this.getHospitalCode().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getHospitalCode());
			}
			if(this.getHospitalName() == null || this.getHospitalName().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
				pstmt.setString(23, this.getHospitalName());
			}
			pstmt.setDouble(24, this.getClaimPay());
			if(this.getAccProvince() == null || this.getAccProvince().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
				pstmt.setString(25, this.getAccProvince());
			}
			if(this.getAccCity() == null || this.getAccCity().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getAccCity());
			}
			if(this.getAccCounty() == null || this.getAccCounty().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
				pstmt.setString(27, this.getAccCounty());
			}
			...
			if(this.getAccNo() == null || this.getAccNo().equals("null")) {
				...
				pstmt.setString(30, this.getAccNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLAccident WHERE  AccNo = ?", 
	...
	public LLAccidentSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLAccidentSet aLLAccidentSet = new LLAccidentSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLAccident");
			LLAccidentSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLAccidentSchema s1 = new LLAccidentSchema();
				s1.setSchema(rs,i);
				aLLAccidentSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLAccidentSet;
	}

	public LLAccidentSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLAccidentSet aLLAccidentSet = new LLAccidentSet();
	...
	public LLAccidentSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLAccidentSet aLLAccidentSet = new LLAccidentSet();
	...
	public LLAccidentSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLAccidentSet aLLAccidentSet = new LLAccidentSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLAccident " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLAccidentSet
 */
public LLAccidentSet getData()
{
    int tCount = 0;
    LLAccidentSet tLLAccidentSet = new LLAccidentSet();
    LLAccidentSchema tLLAccidentSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLAccidentDB";
    ...
    {
        tCount = 1;
        tLLAccidentSchema = new LLAccidentSchema();
        tLLAccidentSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLAccidentSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLAccidentSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LPInsureAccFeeTraceSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: bug2
 */
public class LPInsureAccFeeTraceSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPInsureAccFeeTraceSchema.class);
	// @Field
	/** 集体合同号码 */
	...
	// @Constructor
	public LPInsureAccFeeTraceSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPInsureAccFeeTraceSchema cloned = (LPInsureAccFeeTraceSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LPInsureAccFeeTraceSchema 对象给 Schema 赋值
	* @param: aLPInsureAccFeeTraceSchema LPInsureAccFeeTraceSchema
	**/
	public void setSchema(LPInsureAccFeeTraceSchema aLPInsureAccFeeTraceSchema)
	{
		this.GrpContNo = aLPInsureAccFeeTraceSchema.getGrpContNo();
		this.GrpPolNo = aLPInsureAccFeeTraceSchema.getGrpPolNo();
		this.ContNo = aLPInsureAccFeeTraceSchema.getContNo();
		this.PolNo = aLPInsureAccFeeTraceSchema.getPolNo();
		this.SerialNo = aLPInsureAccFeeTraceSchema.getSerialNo();
		this.InsuAccNo = aLPInsureAccFeeTraceSchema.getInsuAccNo();
		this.RiskCode = aLPInsureAccFeeTraceSchema.getRiskCode();
		this.PayPlanCode = aLPInsureAccFeeTraceSchema.getPayPlanCode();
		this.OtherNo = aLPInsureAccFeeTraceSchema.getOtherNo();
		this.OtherType = aLPInsureAccFeeTraceSchema.getOtherType();
		this.AccAscription = aLPInsureAccFeeTraceSchema.getAccAscription();
		this.MoneyType = aLPInsureAccFeeTraceSchema.getMoneyType();
		this.FeeRate = aLPInsureAccFeeTraceSchema.getFeeRate();
		this.Fee = aLPInsureAccFeeTraceSchema.getFee();
		this.FeeUnit = aLPInsureAccFeeTraceSchema.getFeeUnit();
		this.PayDate = fDate.getDate( aLPInsureAccFeeTraceSchema.getPayDate());
		this.State = aLPInsureAccFeeTraceSchema.getState();
		this.FeeCode = aLPInsureAccFeeTraceSchema.getFeeCode();
		this.InerSerialNo = aLPInsureAccFeeTraceSchema.getInerSerialNo();
		this.ManageCom = aLPInsureAccFeeTraceSchema.getManageCom();
		this.Operator = aLPInsureAccFeeTraceSchema.getOperator();
		this.MakeDate = fDate.getDate( aLPInsureAccFeeTraceSchema.getMakeDate());
		this.MakeTime = aLPInsureAccFeeTraceSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLPInsureAccFeeTraceSchema.getModifyDate());
		this.ModifyTime = aLPInsureAccFeeTraceSchema.getModifyTime();
		this.EdorNo = aLPInsureAccFeeTraceSchema.getEdorNo();
		this.EdorType = aLPInsureAccFeeTraceSchema.getEdorType();
		this.PayNo = aLPInsureAccFeeTraceSchema.getPayNo();
		this.Currency = aLPInsureAccFeeTraceSchema.getCurrency();
		this.INvesttype = aLPInsureAccFeeTraceSchema.getINvesttype();
		this.AccAlterNo = aLPInsureAccFeeTraceSchema.getAccAlterNo();
		this.AccAlterType = aLPInsureAccFeeTraceSchema.getAccAlterType();
		this.ConfDate = fDate.getDate( aLPInsureAccFeeTraceSchema.getConfDate());
		this.AllocationDate = fDate.getDate( aLPInsureAccFeeTraceSchema.getAllocationDate());
		this.PricingDate = fDate.getDate( aLPInsureAccFeeTraceSchema.getPricingDate());
		this.ComCode = aLPInsureAccFeeTraceSchema.getComCode();
		this.ModifyOperator = aLPInsureAccFeeTraceSchema.getModifyOperator();
		this.NetAmount = aLPInsureAccFeeTraceSchema.getNetAmount();
		this.TaxAmount = aLPInsureAccFeeTraceSchema.getTaxAmount();
		this.Tax = aLPInsureAccFeeTraceSchema.getTax();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPInsureAccFeeTrace表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPInsureAccFeeTraceSchema";
	...
	public LPInsureAccFeeTraceSchema getSchema()
	{
		LPInsureAccFeeTraceSchema aLPInsureAccFeeTraceSchema = new LPInsureAccFeeTraceSchema();
		aLPInsureAccFeeTraceSchema.setSchema(this);
		return aLPInsureAccFeeTraceSchema;
	}

	public LPInsureAccFeeTraceDB getDB()
	{
		LPInsureAccFeeTraceDB aDBOper = new LPInsureAccFeeTraceDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPInsureAccFeeTrace描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPInsureAccFeeTrace>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			EdorNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 26, SysConst.PACKAGESPILTER );
			...
			INvesttype = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 30, SysConst.PACKAGESPILTER );
			AccAlterNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 31, SysConst.PACKAGESPILTER );
			AccAlterType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 32, SysConst.PACKAGESPILTER );
			ConfDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 33,SysConst.PACKAGESPILTER));
			AllocationDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 34,SysConst.PACKAGESPILTER));
			PricingDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 35,SysConst.PACKAGESPILTER));
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPInsureAccFeeTraceSchema other = (LPInsureAccFeeTraceSchema)otherObject;
		return

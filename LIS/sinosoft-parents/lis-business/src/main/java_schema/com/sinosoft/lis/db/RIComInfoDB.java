...
/*
 * <p>ClassName: RIComInfoDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保系统模型
 */
public class RIComInfoDB extends RIComInfoSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public RIComInfoDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		RIComInfoSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getComPanyNo() == null || this.getComPanyNo().equals("null")) {
				...
				pstmt.setString(1, this.getComPanyNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getComPanyName() == null || this.getComPanyName().equals("null")) {
				...
				pstmt.setString(2, this.getComPanyName());
			}
			...
			if(this.getFax() == null || this.getFax().equals("null")) {
				...
				pstmt.setString(5, this.getFax());
			}
			if(this.getHomePage() == null || this.getHomePage().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getHomePage());
			}
			if(this.getLinkMan() == null || this.getLinkMan().equals("null")) {
				pstmt.setNull(7, 12);
			} else {
				pstmt.setString(7, this.getLinkMan());
			}
			...
			if(this.getInformation() == null || this.getInformation().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getInformation());
			}
			// set where condition
			if(this.getComPanyNo() == null || this.getComPanyNo().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getComPanyNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM RIComInfo WHERE  ComPanyNo = ?", 
	...
	public RIComInfoSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIComInfoSet aRIComInfoSet = new RIComInfoSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("RIComInfo");
			RIComInfoSchema aSchema = this.getSchema();
			...
			{
				i++;
				RIComInfoSchema s1 = new RIComInfoSchema();
				s1.setSchema(rs,i);
				aRIComInfoSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aRIComInfoSet;
	}

	public RIComInfoSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIComInfoSet aRIComInfoSet = new RIComInfoSet();
	...
	public RIComInfoSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIComInfoSet aRIComInfoSet = new RIComInfoSet();
	...
	public RIComInfoSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIComInfoSet aRIComInfoSet = new RIComInfoSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update RIComInfo " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return RIComInfoSet
 */
public RIComInfoSet getData()
{
    int tCount = 0;
    RIComInfoSet tRIComInfoSet = new RIComInfoSet();
    RIComInfoSchema tRIComInfoSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "RIComInfoDB";
    ...
    {
        tCount = 1;
        tRIComInfoSchema = new RIComInfoSchema();
        tRIComInfoSchema.setSchema(mResultSet, 1);
        tRIComInfoSet.add(tRIComInfoSchema);

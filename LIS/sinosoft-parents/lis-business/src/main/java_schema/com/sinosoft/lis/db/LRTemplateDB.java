...
/*
 * <p>ClassName: LRTemplateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LRTemplateDB extends LRTemplateSchema
{
	private static Logger logger = Logger.getLogger(LRTemplateDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LRTemplateDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LRTemplate" );
		mflag = true;
	}
	...
	{
		LRTemplateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LRTemplateDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LRTemplate WHERE  Id = ?");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LRTemplate SET  Id = ? , Version = ? , RuleName = ? , RuleCh = ? , SQLStatement = ? , BOMs = ? , SQLParameter = ? , Valid = ? , State = ? , TemplateLevel = ? , Business = ? , Type = ? , TableName = ? , StartDate = ? , EndDate = ? , Description = ? , Creator = ? , Modifier = ? , Approver = ? , Deployer = ? , MakeTime = ? , MakeDate = ? , ModifyTime = ? , ModifyDate = ? , AuthorTime = ? , AuthorDate = ? , DeclTime = ? , DeclDate = ? , ViewParameter = ? , RuleCalCh = ? WHERE  Id = ?");
			if(this.getId() == null || this.getId().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getId());
			...
			if(this.getValid() == null || this.getValid().equals("null")) {
				...
				pstmt.setString(8, this.getValid());
			}
			...
			if(this.getTemplateLevel() == null || this.getTemplateLevel().equals("null")) {
				...
				pstmt.setString(10, this.getTemplateLevel());
			}
			if(this.getBusiness() == null || this.getBusiness().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getBusiness());
			}
			if(this.getType() == null || this.getType().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getType());
			}
			...
			if(this.getAuthorTime() == null || this.getAuthorTime().equals("null")) {
				...
				pstmt.setString(25, this.getAuthorTime());
			}
			if(this.getAuthorDate() == null || this.getAuthorDate().equals("null")) {
				pstmt.setNull(26, 91);
			} else {
				pstmt.setDate(26, Date.valueOf(this.getAuthorDate()));
			}
			if(this.getDeclTime() == null || this.getDeclTime().equals("null")) {
				pstmt.setNull(27, 1);
			} else {
				pstmt.setString(27, this.getDeclTime());
			}
			if(this.getDeclDate() == null || this.getDeclDate().equals("null")) {
				pstmt.setNull(28, 91);
			} else {
				pstmt.setDate(28, Date.valueOf(this.getDeclDate()));
			}
			pstmt.setString(29, this.getViewParameter());
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LRTemplate VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getId() == null || this.getId().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getId());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LRTemplate WHERE  Id = ?", 
	...
	public LRTemplateSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRTemplateSet aLRTemplateSet = new LRTemplateSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LRTemplate");
			LRTemplateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LRTemplateSchema s1 = new LRTemplateSchema();
				s1.setSchema(rs,i);
				aLRTemplateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLRTemplateSet;
	}

	public LRTemplateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LRTemplateSet aLRTemplateSet = new LRTemplateSet();
	...
	public LRTemplateSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LRTemplateSet aLRTemplateSet = new LRTemplateSet();
	...
	public LRTemplateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LRTemplateSet aLRTemplateSet = new LRTemplateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LRTemplate " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LRTemplateSet
 */
public LRTemplateSet getData()
{
    int tCount = 0;
    LRTemplateSet tLRTemplateSet = new LRTemplateSet();
    LRTemplateSchema tLRTemplateSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LRTemplateDB";
    ...
    {
        tCount = 1;
        tLRTemplateSchema = new LRTemplateSchema();
        tLRTemplateSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LRTemplateSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LRTemplateSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

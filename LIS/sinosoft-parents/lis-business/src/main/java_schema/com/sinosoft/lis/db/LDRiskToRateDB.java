...
/*
 * <p>ClassName: LDRiskToRateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 费率表维护
 */
public class LDRiskToRateDB extends LDRiskToRateSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDRiskToRateDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDRiskToRateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LDRiskToRateDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRateTable() == null || this.getRateTable().equals("null")) {
				...
				pstmt.setString(12, this.getRateTable());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDRiskToRate WHERE  RiskCode = ? AND RateType = ? AND RateTable = ?", 
	...
	public LDRiskToRateSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDRiskToRateSet aLDRiskToRateSet = new LDRiskToRateSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("LDRiskToRate");
			LDRiskToRateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LDRiskToRateSchema s1 = new LDRiskToRateSchema();
				s1.setSchema(rs,i);
				aLDRiskToRateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aLDRiskToRateSet;
	}

	public LDRiskToRateSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDRiskToRateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDRiskToRateSet aLDRiskToRateSet = new LDRiskToRateSet();
	...
	public LDRiskToRateSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDRiskToRateSet aLDRiskToRateSet = new LDRiskToRateSet();
	...
	public LDRiskToRateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDRiskToRateSet aLDRiskToRateSet = new LDRiskToRateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDRiskToRate " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDRiskToRateSet
 */
public LDRiskToRateSet getData()
{
    int tCount = 0;
    LDRiskToRateSet tLDRiskToRateSet = new LDRiskToRateSet();
    LDRiskToRateSchema tLDRiskToRateSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDRiskToRateDB";
    ...
    {
        tCount = 1;
        tLDRiskToRateSchema = new LDRiskToRateSchema();
        tLDRiskToRateSchema.setSchema(mResultSet, 1);
        tLDRiskToRateSet.add(tLDRiskToRateSchema);

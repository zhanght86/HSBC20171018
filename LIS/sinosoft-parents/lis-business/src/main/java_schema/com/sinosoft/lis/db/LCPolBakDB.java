...
/*
 * <p>ClassName: LCPolBakDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LCPolBakDB extends LCPolBakSchema
{
private static Logger logger = Logger.getLogger(LCPolBakDB.class);
	...
	**/
	...
	// @Constructor
	public LCPolBakDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCPolBakSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAscriptionFlag() == null || this.getAscriptionFlag().equals("null")) {
				...
				pstmt.setString(121, this.getAscriptionFlag());
			}
			if(this.getPCValiDate() == null || this.getPCValiDate().equals("null")) {
				pstmt.setNull(122, 91);
			} else {
				pstmt.setDate(122, Date.valueOf(this.getPCValiDate()));
			}
			...
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				...
				pstmt.setString(132, this.getPolNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPolBak WHERE  PolNo = ?", 
	...
	public LCPolBakSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPolBakSet aLCPolBakSet = new LCPolBakSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPolBak");
			LCPolBakSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCPolBakSchema s1 = new LCPolBakSchema();
				s1.setSchema(rs,i);
				aLCPolBakSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LCPolBakSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPolBakSet aLCPolBakSet = new LCPolBakSet();
	...
	public LCPolBakSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPolBakSet aLCPolBakSet = new LCPolBakSet();
	...
	public LCPolBakSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPolBakSet aLCPolBakSet = new LCPolBakSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPolBak " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LCPolBakSet
 */
public LCPolBakSet getData()
{
    int tCount = 0;
    LCPolBakSet tLCPolBakSet = new LCPolBakSet();
    LCPolBakSchema tLCPolBakSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCPolBakDB";
    ...
    {
        tCount = 1;
        tLCPolBakSchema = new LCPolBakSchema();
        tLCPolBakSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCPolBakSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPolBakSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LPDutySubDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LPDutySubDB extends LPDutySubSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPDutySubDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPDutySubSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getSysPlanCode() == null || this.getSysPlanCode().equals("null")) {
				...
				pstmt.setString(8, this.getSysPlanCode());
			}
			if(this.getPlanCode() == null || this.getPlanCode().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getPlanCode());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPDutySub WHERE  EdorNo = ? AND EdorType = ? AND BussType = ? AND BussNo = ? AND PolNo = ? AND DutyCode = ?", 
	...
	public LPDutySubSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPDutySubSet aLPDutySubSet = new LPDutySubSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPDutySub");
			LPDutySubSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPDutySubSchema s1 = new LPDutySubSchema();
				s1.setSchema(rs,i);
				aLPDutySubSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPDutySubSet;
	}

	public LPDutySubSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPDutySubSet aLPDutySubSet = new LPDutySubSet();
	...
	public LPDutySubSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPDutySubSet aLPDutySubSet = new LPDutySubSet();
	...
	public LPDutySubSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPDutySubSet aLPDutySubSet = new LPDutySubSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPDutySub " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LPDutySubSet
 */
public LPDutySubSet getData()
{
    int tCount = 0;
    LPDutySubSet tLPDutySubSet = new LPDutySubSet();
    LPDutySubSchema tLPDutySubSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPDutySubDB";
    ...
    {
        tCount = 1;
        tLPDutySubSchema = new LPDutySubSchema();
        tLPDutySubSchema.setSchema(mResultSet, 1);
        tLPDutySubSet.add(tLPDutySubSchema);
...
public boolean closeData()
{
	...
	public LPDutySubSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPDutySubSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

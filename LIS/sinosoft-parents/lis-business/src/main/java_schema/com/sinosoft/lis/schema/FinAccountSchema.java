...
import com.sinosoft.lis.db.FinAccountDB;

/*
 * <p>ClassName: FinAccountSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class FinAccountSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(FinAccountSchema.class);
	// @Field
	/** 会计科目编码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		FinAccountSchema cloned = (FinAccountSchema)super.clone();
	...
	/**
	* 01-资产，02-负债，03-所有者权益，04-成本，05-损益
	*/
	...
	{
		if(aFinType!=null && aFinType.length()>6)
			throw new IllegalArgumentException("会计科目类型FinType值"+aFinType+"的长度"+aFinType.length()+"大于最大值6");
		FinType = aFinType;
	}
	...
	/**
	* 使用另外一个 FinAccountSchema 对象给 Schema 赋值
	* @param: aFinAccountSchema FinAccountSchema
	**/
	public void setSchema(FinAccountSchema aFinAccountSchema)
	{
		this.FinCode = aFinAccountSchema.getFinCode();
		this.FinName = aFinAccountSchema.getFinName();
		this.FinType = aFinAccountSchema.getFinType();
		this.Remark = aFinAccountSchema.getRemark();
		this.ManageCom = aFinAccountSchema.getManageCom();
		this.ComCode = aFinAccountSchema.getComCode();
		this.MakeOperator = aFinAccountSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aFinAccountSchema.getMakeDate());
		this.MakeTime = aFinAccountSchema.getMakeTime();
		this.ModifyOperator = aFinAccountSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aFinAccountSchema.getModifyDate());
		this.ModifyTime = aFinAccountSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的FinAccount表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "FinAccountSchema";
	...
	public FinAccountSchema getSchema()
	{
		FinAccountSchema aFinAccountSchema = new FinAccountSchema();
		aFinAccountSchema.setSchema(this);
		return aFinAccountSchema;
	}

	public FinAccountDB getDB()
	{
		FinAccountDB aDBOper = new FinAccountDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpFinAccount描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpFinAccount>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			FinCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 1, SysConst.PACKAGESPILTER );
			FinName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 2, SysConst.PACKAGESPILTER );
			FinType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		FinAccountSchema other = (FinAccountSchema)otherObject;
		return

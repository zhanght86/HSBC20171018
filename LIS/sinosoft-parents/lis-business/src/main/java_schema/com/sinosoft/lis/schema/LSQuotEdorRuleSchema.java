...
import com.sinosoft.lis.db.LSQuotEdorRuleDB;

/*
 * <p>ClassName: LSQuotEdorRuleSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LSQuotEdorRuleSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LSQuotEdorRuleSchema.class);
	// @Field
	/** 序号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LSQuotEdorRuleSchema cloned = (LSQuotEdorRuleSchema)super.clone();
	...
	{
		if(aEdorCode!=null && aEdorCode.length()>10)
			throw new IllegalArgumentException("保全项目EdorCode值"+aEdorCode+"的长度"+aEdorCode.length()+"大于最大值10");
		EdorCode = aEdorCode;
	}
	...
	public void setCalCode(String aCalCode)
	{
		if(aCalCode!=null && aCalCode.length()>10)
			throw new IllegalArgumentException("保全算法编码CalCode值"+aCalCode+"的长度"+aCalCode.length()+"大于最大值10");
		CalCode = aCalCode;
	}
	...
	/**
	* 使用另外一个 LSQuotEdorRuleSchema 对象给 Schema 赋值
	* @param: aLSQuotEdorRuleSchema LSQuotEdorRuleSchema
	**/
	public void setSchema(LSQuotEdorRuleSchema aLSQuotEdorRuleSchema)
	{
		this.SerialNo = aLSQuotEdorRuleSchema.getSerialNo();
		this.QuotNo = aLSQuotEdorRuleSchema.getQuotNo();
		this.QuotBatNo = aLSQuotEdorRuleSchema.getQuotBatNo();
		this.RuleType = aLSQuotEdorRuleSchema.getRuleType();
		this.EdorCode = aLSQuotEdorRuleSchema.getEdorCode();
		this.RiskCode = aLSQuotEdorRuleSchema.getRiskCode();
		this.CalCode = aLSQuotEdorRuleSchema.getCalCode();
		this.FeeRate = aLSQuotEdorRuleSchema.getFeeRate();
		this.MakeOperator = aLSQuotEdorRuleSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLSQuotEdorRuleSchema.getMakeDate());
		this.MakeTime = aLSQuotEdorRuleSchema.getMakeTime();
		this.ModifyOperator = aLSQuotEdorRuleSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLSQuotEdorRuleSchema.getModifyDate());
		this.ModifyTime = aLSQuotEdorRuleSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LSQuotEdorRule表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LSQuotEdorRuleSchema";
	...
	public LSQuotEdorRuleSchema getSchema()
	{
		LSQuotEdorRuleSchema aLSQuotEdorRuleSchema = new LSQuotEdorRuleSchema();
		aLSQuotEdorRuleSchema.setSchema(this);
		return aLSQuotEdorRuleSchema;
	}

	public LSQuotEdorRuleDB getDB()
	{
		LSQuotEdorRuleDB aDBOper = new LSQuotEdorRuleDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotEdorRule>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			EdorCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LSQuotEdorRuleSchema other = (LSQuotEdorRuleSchema)otherObject;
		return

...
/*
 * <p>ClassName: LOBContPlanEngineeringDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LOBContPlanEngineeringDB extends LOBContPlanEngineeringSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LOBContPlanEngineeringDB( Connection tConnection )
	{
		con = tConnection;
	...
	public LOBContPlanEngineeringDB()
	{
		con = null;
	...
	{
		LOBContPlanEngineeringSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBContPlanEngineeringDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getEnginDays() == null || this.getEnginDays().equals("null")) {
				...
				pstmt.setString(27, this.getEnginDays());
			}
			// set where condition
			if(this.getBussType() == null || this.getBussType().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getBussType());
			}
			if(this.getBussNo() == null || this.getBussNo().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getBussNo());
			}
			if(this.getPolicyNo() == null || this.getPolicyNo().equals("null")) {
				pstmt.setNull(30, 12);
			} else {
				pstmt.setString(30, this.getPolicyNo());
			}
			if(this.getSysPlanCode() == null || this.getSysPlanCode().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
				pstmt.setString(31, this.getSysPlanCode());
			}
			if(this.getPlanCode() == null || this.getPlanCode().equals("null")) {
				pstmt.setNull(32, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBContPlanEngineering WHERE  BussType = ? AND BussNo = ? AND PolicyNo = ? AND SysPlanCode = ? AND PlanCode = ?", 
	...
	public LOBContPlanEngineeringSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBContPlanEngineeringSet aLOBContPlanEngineeringSet = new LOBContPlanEngineeringSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBContPlanEngineering");
			LOBContPlanEngineeringSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBContPlanEngineeringSchema s1 = new LOBContPlanEngineeringSchema();
				s1.setSchema(rs,i);
				aLOBContPlanEngineeringSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBContPlanEngineeringSet;
	}

	public LOBContPlanEngineeringSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBContPlanEngineeringSet aLOBContPlanEngineeringSet = new LOBContPlanEngineeringSet();
	...
	public LOBContPlanEngineeringSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBContPlanEngineeringSet aLOBContPlanEngineeringSet = new LOBContPlanEngineeringSet();
	...
	public LOBContPlanEngineeringSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBContPlanEngineeringSet aLOBContPlanEngineeringSet = new LOBContPlanEngineeringSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBContPlanEngineering " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBContPlanEngineeringSet
 */
public LOBContPlanEngineeringSet getData()
{
    int tCount = 0;
    LOBContPlanEngineeringSet tLOBContPlanEngineeringSet = new LOBContPlanEngineeringSet();
    LOBContPlanEngineeringSchema tLOBContPlanEngineeringSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBContPlanEngineeringDB";
    ...
    {
        tCount = 1;
        tLOBContPlanEngineeringSchema = new LOBContPlanEngineeringSchema();
        tLOBContPlanEngineeringSchema.setSchema(mResultSet, 1);
        tLOBContPlanEngineeringSet.add(tLOBContPlanEngineeringSchema);
    ...
    return tLOBContPlanEngineeringSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LOBContPlanEngineeringSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBContPlanEngineeringSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

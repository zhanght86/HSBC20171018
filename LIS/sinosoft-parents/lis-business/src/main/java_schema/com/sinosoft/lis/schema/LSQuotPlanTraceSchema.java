...
import com.sinosoft.lis.db.LSQuotPlanTraceDB;

/*
 * <p>ClassName: LSQuotPlanTraceSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LSQuotPlanTraceSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LSQuotPlanTraceSchema.class);
	// @Field
	/** 流水号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LSQuotPlanTraceSchema cloned = (LSQuotPlanTraceSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LSQuotPlanTraceSchema 对象给 Schema 赋值
	* @param: aLSQuotPlanTraceSchema LSQuotPlanTraceSchema
	**/
	public void setSchema(LSQuotPlanTraceSchema aLSQuotPlanTraceSchema)
	{
		this.SerialNo = aLSQuotPlanTraceSchema.getSerialNo();
		this.QuotNo = aLSQuotPlanTraceSchema.getQuotNo();
		this.QuotBatNo = aLSQuotPlanTraceSchema.getQuotBatNo();
		this.SysPlanCode = aLSQuotPlanTraceSchema.getSysPlanCode();
		this.PlanCode = aLSQuotPlanTraceSchema.getPlanCode();
		this.PlanDesc = aLSQuotPlanTraceSchema.getPlanDesc();
		this.PlanType = aLSQuotPlanTraceSchema.getPlanType();
		this.PlanFlag = aLSQuotPlanTraceSchema.getPlanFlag();
		this.PremCalType = aLSQuotPlanTraceSchema.getPremCalType();
		this.InsuPeriod = aLSQuotPlanTraceSchema.getInsuPeriod();
		this.InsuPeriodFlag = aLSQuotPlanTraceSchema.getInsuPeriodFlag();
		this.OccupTypeFlag = aLSQuotPlanTraceSchema.getOccupTypeFlag();
		this.MinOccupType = aLSQuotPlanTraceSchema.getMinOccupType();
		this.MaxOccupType = aLSQuotPlanTraceSchema.getMaxOccupType();
		this.OccupType = aLSQuotPlanTraceSchema.getOccupType();
		this.OccupMidType = aLSQuotPlanTraceSchema.getOccupMidType();
		this.OccupCode = aLSQuotPlanTraceSchema.getOccupCode();
		this.MinAge = aLSQuotPlanTraceSchema.getMinAge();
		this.MaxAge = aLSQuotPlanTraceSchema.getMaxAge();
		this.AvgAge = aLSQuotPlanTraceSchema.getAvgAge();
		this.NumPeople = aLSQuotPlanTraceSchema.getNumPeople();
		this.SocialInsuRate = aLSQuotPlanTraceSchema.getSocialInsuRate();
		this.MaleRate = aLSQuotPlanTraceSchema.getMaleRate();
		this.FemaleRate = aLSQuotPlanTraceSchema.getFemaleRate();
		this.RetireRate = aLSQuotPlanTraceSchema.getRetireRate();
		this.PremMode = aLSQuotPlanTraceSchema.getPremMode();
		this.EnterpriseRate = aLSQuotPlanTraceSchema.getEnterpriseRate();
		this.MinSalary = aLSQuotPlanTraceSchema.getMinSalary();
		this.MaxSalary = aLSQuotPlanTraceSchema.getMaxSalary();
		this.AvgSalary = aLSQuotPlanTraceSchema.getAvgSalary();
		this.OtherDesc = aLSQuotPlanTraceSchema.getOtherDesc();
		this.MakeOperator = aLSQuotPlanTraceSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLSQuotPlanTraceSchema.getMakeDate());
		this.MakeTime = aLSQuotPlanTraceSchema.getMakeTime();
		this.ModifyOperator = aLSQuotPlanTraceSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLSQuotPlanTraceSchema.getModifyDate());
		this.ModifyTime = aLSQuotPlanTraceSchema.getModifyTime();
		this.OccupRate = aLSQuotPlanTraceSchema.getOccupRate();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LSQuotPlanTrace表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LSQuotPlanTraceSchema";
	...
	public LSQuotPlanTraceSchema getSchema()
	{
		LSQuotPlanTraceSchema aLSQuotPlanTraceSchema = new LSQuotPlanTraceSchema();
		aLSQuotPlanTraceSchema.setSchema(this);
		return aLSQuotPlanTraceSchema;
	}

	public LSQuotPlanTraceDB getDB()
	{
		LSQuotPlanTraceDB aDBOper = new LSQuotPlanTraceDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotPlanTrace描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLSQuotPlanTrace>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			PlanDesc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			PlanType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			PlanFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			PremCalType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			InsuPeriod= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,10,SysConst.PACKAGESPILTER))).intValue();
			InsuPeriodFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			OccupTypeFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			MinOccupType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			MaxOccupType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			OccupType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			OccupMidType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 16, SysConst.PACKAGESPILTER );
			OccupCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 17, SysConst.PACKAGESPILTER );
			MinAge= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,18,SysConst.PACKAGESPILTER))).intValue();
			MaxAge= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,19,SysConst.PACKAGESPILTER))).intValue();
			AvgAge= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,20,SysConst.PACKAGESPILTER))).intValue();
			NumPeople= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,21,SysConst.PACKAGESPILTER))).intValue();
			SocialInsuRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,22,SysConst.PACKAGESPILTER))).doubleValue();
			MaleRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,23,SysConst.PACKAGESPILTER))).doubleValue();
			FemaleRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,24,SysConst.PACKAGESPILTER))).doubleValue();
			RetireRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,25,SysConst.PACKAGESPILTER))).doubleValue();
			PremMode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 26, SysConst.PACKAGESPILTER );
			EnterpriseRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,27,SysConst.PACKAGESPILTER))).doubleValue();
			MinSalary = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,28,SysConst.PACKAGESPILTER))).doubleValue();
			MaxSalary = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,29,SysConst.PACKAGESPILTER))).doubleValue();
			AvgSalary = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,30,SysConst.PACKAGESPILTER))).doubleValue();
			OtherDesc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 31, SysConst.PACKAGESPILTER );
			...
			OccupRate = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 38, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LSQuotPlanTraceSchema other = (LSQuotPlanTraceSchema)otherObject;
		return

...
/*
 * <p>ClassName: LCPolOtherDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCPolOtherDB extends LCPolOtherSchema
{
private static Logger logger = Logger.getLogger(LCPolOtherDB.class);
	...
	**/
	...
	// @Constructor
	public LCPolOtherDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCPolOtherSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCPolOtherDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getGrpPolNo() == null || this.getGrpPolNo().equals("null")) {
				...
				pstmt.setString(32, this.getGrpPolNo());
			}
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPolOther WHERE  GrpContNo = ? AND GrpPolNo = ? AND ContNo = ? AND PolNo = ? AND DutyCode = ?", 
	...
	public LCPolOtherSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPolOther");
			LCPolOtherSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCPolOtherSchema s1 = new LCPolOtherSchema();
				s1.setSchema(rs,i);
				aLCPolOtherSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCPolOtherSet;

	}

	public LCPolOtherSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPolOtherSet aLCPolOtherSet = new LCPolOtherSet();
	...
	public LCPolOtherSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPolOtherSet aLCPolOtherSet = new LCPolOtherSet();
	...
	public LCPolOtherSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPolOtherSet aLCPolOtherSet = new LCPolOtherSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPolOther " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCPolOtherSet
 */
public LCPolOtherSet getData()
{
    int tCount = 0;
    ...
    {
        tCount = 1;
        tLCPolOtherSchema = new LCPolOtherSchema();
        tLCPolOtherSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCPolOtherSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPolOtherSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

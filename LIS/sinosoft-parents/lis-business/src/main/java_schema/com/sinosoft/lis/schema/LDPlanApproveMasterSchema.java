...
import com.sinosoft.lis.db.LDPlanApproveMasterDB;

/*
 * <p>ClassName: LDPlanApproveMasterSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 基础信息
 */
public class LDPlanApproveMasterSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LDPlanApproveMasterSchema.class);
	...
	// @Constructor
	public LDPlanApproveMasterSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LDPlanApproveMasterSchema cloned = (LDPlanApproveMasterSchema)super.clone();
	...
	*/
	...
	public void setOperateNo(int aOperateNo)
	{
		OperateNo = aOperateNo;
	}
	public void setOperateNo(String aOperateNo)
	{
		if (aOperateNo != null && !aOperateNo.equals(""))
		{
			Integer tInteger = new Integer(aOperateNo);
	...
	*/
	...
	/**
	* 使用另外一个 LDPlanApproveMasterSchema 对象给 Schema 赋值
	* @param: aLDPlanApproveMasterSchema LDPlanApproveMasterSchema
	**/
	public void setSchema(LDPlanApproveMasterSchema aLDPlanApproveMasterSchema)
	{
		this.ContPlanCode = aLDPlanApproveMasterSchema.getContPlanCode();
		this.ContPlanName = aLDPlanApproveMasterSchema.getContPlanName();
		this.PlanType = aLDPlanApproveMasterSchema.getPlanType();
		this.OperateNo = aLDPlanApproveMasterSchema.getOperateNo();
		this.ManageCom = aLDPlanApproveMasterSchema.getManageCom();
		this.OperateType = aLDPlanApproveMasterSchema.getOperateType();
		this.OperateResult = aLDPlanApproveMasterSchema.getOperateResult();
		this.Remark = aLDPlanApproveMasterSchema.getRemark();
		this.Operator = aLDPlanApproveMasterSchema.getOperator();
		this.MakeDate = fDate.getDate( aLDPlanApproveMasterSchema.getMakeDate());
		this.MakeTime = aLDPlanApproveMasterSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLDPlanApproveMasterSchema.getModifyDate());
		this.ModifyTime = aLDPlanApproveMasterSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			this.OperateNo = rs.getInt("OperateNo");
		...
		{
			logger.debug("数据库中的LDPlanApproveMaster表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LDPlanApproveMasterSchema";
	...
	public LDPlanApproveMasterSchema getSchema()
	{
		LDPlanApproveMasterSchema aLDPlanApproveMasterSchema = new LDPlanApproveMasterSchema();
		aLDPlanApproveMasterSchema.setSchema(this);
		return aLDPlanApproveMasterSchema;
	}

	public LDPlanApproveMasterDB getDB()
	{
		LDPlanApproveMasterDB aDBOper = new LDPlanApproveMasterDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDPlanApproveMaster描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append( ChgData.chgData(OperateNo));strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDPlanApproveMaster>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			OperateNo= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,4,SysConst.PACKAGESPILTER))).intValue();
			ManageCom = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			OperateType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			OperateResult = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("OperateNo"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(OperateNo));
		}
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 2:
				...
				break;
			case 3:
				strFieldValue = String.valueOf(OperateNo);
				break;
			case 4:
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("OperateNo") ) {
			return 3;
		}

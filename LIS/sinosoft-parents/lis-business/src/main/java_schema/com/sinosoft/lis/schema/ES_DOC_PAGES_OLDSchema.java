...
/*
 * <p>ClassName: ES_DOC_PAGES_OLDSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: EasyScanV4
 */
public class ES_DOC_PAGES_OLDSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(ES_DOC_PAGES_OLDSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		ES_DOC_PAGES_OLDSchema cloned = (ES_DOC_PAGES_OLDSchema)super.clone();
	...
	*/
	...
	public void setFNDocKey(String aFNDocKey)
	{
		FNDocKey = aFNDocKey;
	}

	/**
	* 使用另外一个 ES_DOC_PAGES_OLDSchema 对象给 Schema 赋值
	* @param: aES_DOC_PAGES_OLDSchema ES_DOC_PAGES_OLDSchema
	**/
	public void setSchema(ES_DOC_PAGES_OLDSchema aES_DOC_PAGES_OLDSchema)
	{
		this.PageID = aES_DOC_PAGES_OLDSchema.getPageID();
		this.DocID = aES_DOC_PAGES_OLDSchema.getDocID();
		this.PageCode = aES_DOC_PAGES_OLDSchema.getPageCode();
		this.HostName = aES_DOC_PAGES_OLDSchema.getHostName();
		this.PageName = aES_DOC_PAGES_OLDSchema.getPageName();
		this.PageSuffix = aES_DOC_PAGES_OLDSchema.getPageSuffix();
		this.PicPathFTP = aES_DOC_PAGES_OLDSchema.getPicPathFTP();
		this.PageFlag = aES_DOC_PAGES_OLDSchema.getPageFlag();
		this.PicPath = aES_DOC_PAGES_OLDSchema.getPicPath();
		this.PageType = aES_DOC_PAGES_OLDSchema.getPageType();
		this.ManageCom = aES_DOC_PAGES_OLDSchema.getManageCom();
		this.Operator = aES_DOC_PAGES_OLDSchema.getOperator();
		this.MakeDate = fDate.getDate( aES_DOC_PAGES_OLDSchema.getMakeDate());
		this.MakeTime = aES_DOC_PAGES_OLDSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aES_DOC_PAGES_OLDSchema.getModifyDate());
		this.ModifyTime = aES_DOC_PAGES_OLDSchema.getModifyTime();
		this.ScanNo = aES_DOC_PAGES_OLDSchema.getScanNo();
		this.FNDocKey = aES_DOC_PAGES_OLDSchema.getFNDocKey();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			if( rs.getString("FNDocKey") == null )
				this.FNDocKey = null;
			else
				this.FNDocKey = rs.getString("FNDocKey").trim();

		}
		catch(SQLException sqle)
		{
			logger.debug("数据库中的ES_DOC_PAGES_OLD表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "ES_DOC_PAGES_OLDSchema";
	...
	public ES_DOC_PAGES_OLDSchema getSchema()
	{
		ES_DOC_PAGES_OLDSchema aES_DOC_PAGES_OLDSchema = new ES_DOC_PAGES_OLDSchema();
		aES_DOC_PAGES_OLDSchema.setSchema(this);
		return aES_DOC_PAGES_OLDSchema;
	}

	public ES_DOC_PAGES_OLDDB getDB()
	{
		ES_DOC_PAGES_OLDDB aDBOper = new ES_DOC_PAGES_OLDDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpES_DOC_PAGES_OLD描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(FNDocKey));
		return strReturn.toString();
	}

	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpES_DOC_PAGES_OLD>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			PageID= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,1,SysConst.PACKAGESPILTER))).intValue();
			DocID= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,2,SysConst.PACKAGESPILTER))).intValue();
			PageCode= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,3,SysConst.PACKAGESPILTER))).intValue();
			HostName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			PageName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			PageSuffix = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			PicPathFTP = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			PageFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			PicPath = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			PageType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			...
			ScanNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 17, SysConst.PACKAGESPILTER );
			FNDocKey = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 18, SysConst.PACKAGESPILTER );
		}
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("FNDocKey"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(FNDocKey));
		}
		if (strReturn.equals(""))
		{
			strReturn = "null";
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 16:
				...
				break;
			case 17:
				strFieldValue = StrTool.GBKToUnicode(FNDocKey);
				break;
			default:
				strFieldValue = "";
		};
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		ES_DOC_PAGES_OLDSchema other = (ES_DOC_PAGES_OLDSchema)otherObject;
		return
			...
			&& FNDocKey.equals(other.getFNDocKey());
	}
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("FNDocKey") ) {
			return 17;
		}
		return -1;
	}

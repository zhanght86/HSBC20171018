...
/*
 * <p>ClassName: LZCardAppDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 单证管理
 */
public class LZCardAppDB extends LZCardAppSchema
{
private static Logger logger = Logger.getLogger(LZCardAppDB.class);
	...
	**/
	...
	// @Constructor
	public LZCardAppDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LZCardAppSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getApplyCom() == null || this.getApplyCom().equals("null")) {
				...
				pstmt.setString(15, this.getApplyCom());
			}
			if(this.getReplyPerson() == null || this.getReplyPerson().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
				pstmt.setString(16, this.getReplyPerson());
			}
			if(this.getReplyDate() == null || this.getReplyDate().equals("null")) {
				pstmt.setNull(17, 91);
			} else {
				pstmt.setDate(17, Date.valueOf(this.getReplyDate()));
			}
			...
			if(this.getReplyCom() == null || this.getReplyCom().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getReplyCom());
			}
			...
			if(this.getStateFlag() == null || this.getStateFlag().equals("null")) {
				...
				pstmt.setString(25, this.getStateFlag());
			}
			// set where condition
			if(this.getApplyNo() == null || this.getApplyNo().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getApplyNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LZCardApp WHERE  ApplyNo = ?", 
	...
	public LZCardAppSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardAppSet aLZCardAppSet = new LZCardAppSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LZCardApp");
			LZCardAppSchema aSchema = this.getSchema();
			...
			{
				i++;
				LZCardAppSchema s1 = new LZCardAppSchema();
				s1.setSchema(rs,i);
				aLZCardAppSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLZCardAppSet;

	}

	public LZCardAppSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardAppSet aLZCardAppSet = new LZCardAppSet();
	...
	public LZCardAppSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardAppSet aLZCardAppSet = new LZCardAppSet();
	...
	public LZCardAppSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardAppSet aLZCardAppSet = new LZCardAppSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LZCardApp " + sqlObj.getUpdPart() + " where " + strWherePart;
...
public LZCardAppSet getData()
{
    ...
    try
    {
        ...
        tLZCardAppSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LZCardAppSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LZCardAppSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

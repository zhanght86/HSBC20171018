...
/*
 * <p>ClassName: LBTempFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务收费
 */
public class LBTempFeeDB extends LBTempFeeSchema
{
	private static Logger logger = Logger.getLogger(LBTempFeeDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LBTempFeeDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LBTempFee" );
		mflag = true;
	}
	...
	{
		LBTempFeeSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LBTempFee WHERE  TempFeeNo = ? AND TempFeeType = ? AND RiskCode = ? AND BackUpSerialNo = ?");
			...
			if(this.getBackUpSerialNo() == null || this.getBackUpSerialNo().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getBackUpSerialNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
		...
		SQLString sqlObj = new SQLString("LBTempFee");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getBackUpSerialNo() == null || this.getBackUpSerialNo().equals("null")) {
				...
				pstmt.setString(40, this.getBackUpSerialNo());
			}
			if(this.getBackUpReason() == null || this.getBackUpReason().equals("null")) {
				pstmt.setNull(41, 12);
			} else {
				pstmt.setString(41, this.getBackUpReason());
			}
			// set where condition
			if(this.getTempFeeNo() == null || this.getTempFeeNo().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
				pstmt.setString(42, this.getTempFeeNo());
			}
			if(this.getTempFeeType() == null || this.getTempFeeType().equals("null")) {
				pstmt.setNull(43, 1);
			} else {
				pstmt.setString(43, StrTool.space1(this.getTempFeeType(), 2));
			}
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
			...
			if(this.getBackUpSerialNo() == null || this.getBackUpSerialNo().equals("null")) {
				...
				pstmt.setString(45, this.getBackUpSerialNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LBTempFee VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getTempFeeNo() == null || this.getTempFeeNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getTempFeeNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBTempFee WHERE  TempFeeNo = ? AND TempFeeType = ? AND RiskCode = ? AND BackUpSerialNo = ?", 
	...
	public LBTempFeeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LBTempFeeSet aLBTempFeeSet = new LBTempFeeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBTempFee");
			LBTempFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBTempFeeSchema s1 = new LBTempFeeSchema();
				s1.setSchema(rs,i);
				aLBTempFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBTempFeeSet;
	}

	public LBTempFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBTempFeeSet aLBTempFeeSet = new LBTempFeeSet();
	...
	public LBTempFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LBTempFeeSet aLBTempFeeSet = new LBTempFeeSet();
	...
	public LBTempFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBTempFeeSet aLBTempFeeSet = new LBTempFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBTempFee " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LBTempFeeSet
 */
public LBTempFeeSet getData()
{
    int tCount = 0;
    LBTempFeeSet tLBTempFeeSet = new LBTempFeeSet();
    LBTempFeeSchema tLBTempFeeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBTempFeeDB";
    ...
    {
        tCount = 1;
        tLBTempFeeSchema = new LBTempFeeSchema();
        tLBTempFeeSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LBTempFeeSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LBTempFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

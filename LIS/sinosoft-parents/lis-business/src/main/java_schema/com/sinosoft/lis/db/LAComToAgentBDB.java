...
/*
 * <p>ClassName: LAComToAgentBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 代理人管理
 */
public class LAComToAgentBDB extends LAComToAgentBSchema
{
private static Logger logger = Logger.getLogger(LAComToAgentBDB.class);
	...
	**/
	...
	// @Constructor
	public LAComToAgentBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAComToAgentBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAComToAgentBDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getRelaType() == null || this.getRelaType().equals("null")) {
				...
				pstmt.setString(4, StrTool.space1(this.getRelaType(), 1));
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRelaType() == null || this.getRelaType().equals("null")) {
				...
				pstmt.setString(4, this.getRelaType());
			}
			...
			if(this.getRelaType() == null || this.getRelaType().equals("null")) {
				...
				pstmt.setString(17, StrTool.space1(this.getRelaType(), 1));
			}
			if(this.getAgentGroup() == null || this.getAgentGroup().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAComToAgentB WHERE  EdorNo = ? AND EdorType = ? AND AgentCom = ? AND RelaType = ? AND AgentGroup = ?", 
	...
	public LAComToAgentBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LAComToAgentBSet aLAComToAgentBSet = new LAComToAgentBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAComToAgentB");
			LAComToAgentBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LAComToAgentBSchema s1 = new LAComToAgentBSchema();
				s1.setSchema(rs,i);
				aLAComToAgentBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAComToAgentBSet;

	}

	public LAComToAgentBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LAComToAgentBSet aLAComToAgentBSet = new LAComToAgentBSet();
	...
	public LAComToAgentBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LAComToAgentBSet aLAComToAgentBSet = new LAComToAgentBSet();
	...
	public LAComToAgentBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LAComToAgentBSet aLAComToAgentBSet = new LAComToAgentBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAComToAgentB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAComToAgentBSet
 */
public LAComToAgentBSet getData()
{
    int tCount = 0;
    LAComToAgentBSet tLAComToAgentBSet = new LAComToAgentBSet();
    LAComToAgentBSchema tLAComToAgentBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAComToAgentBDB";
    ...
    try
    {
        ...
        tLAComToAgentBSchema.setSchema(mResultSet, 1);
        tLAComToAgentBSet.add(tLAComToAgentBSchema);
...
public boolean closeData()
{
	...
	public LAComToAgentBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LAComToAgentBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

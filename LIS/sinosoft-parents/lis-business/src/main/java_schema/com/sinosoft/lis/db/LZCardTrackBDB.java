...
/*
 * <p>ClassName: LZCardTrackBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 单证管理
 */
public class LZCardTrackBDB extends LZCardTrackBSchema
{
private static Logger logger = Logger.getLogger(LZCardTrackBDB.class);
	...
	**/
	...
	// @Constructor
	public LZCardTrackBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LZCardTrackBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LZCardTrackBDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LZCardTrackB WHERE  CertifyCode = ? AND SubCode = ? AND RiskCode = ? AND RiskVersion = ? AND StartNo = ? AND EndNo = ? AND SendOutCom = ? AND ReceiveCom = ? AND MakeDate = ? AND MakeTime = ?", 
	...
	public LZCardTrackBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardTrackBSet aLZCardTrackBSet = new LZCardTrackBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LZCardTrackB");
			LZCardTrackBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LZCardTrackBSchema s1 = new LZCardTrackBSchema();
				s1.setSchema(rs,i);
				aLZCardTrackBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLZCardTrackBSet;

	}

	public LZCardTrackBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardTrackBSet aLZCardTrackBSet = new LZCardTrackBSet();
	...
	public LZCardTrackBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardTrackBSet aLZCardTrackBSet = new LZCardTrackBSet();
	...
	public LZCardTrackBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardTrackBSet aLZCardTrackBSet = new LZCardTrackBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LZCardTrackB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LZCardTrackBSet
 */
public LZCardTrackBSet getData()
{
    int tCount = 0;
    LZCardTrackBSet tLZCardTrackBSet = new LZCardTrackBSet();
    LZCardTrackBSchema tLZCardTrackBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LZCardTrackBDB";
    ...
    try
    {
        ...
        tLZCardTrackBSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LZCardTrackBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LZCardTrackBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

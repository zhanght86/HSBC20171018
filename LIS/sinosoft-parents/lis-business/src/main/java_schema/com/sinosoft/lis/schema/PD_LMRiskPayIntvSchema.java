...
import com.sinosoft.lis.db.PD_LMRiskPayIntvDB;

/**
 * <p>ClassName: PD_LMRiskPayIntvSchema </p>
 ...
 */
public class PD_LMRiskPayIntvSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(PD_LMRiskPayIntvSchema.class);
    ...
    {
        PD_LMRiskPayIntvSchema cloned = (PD_LMRiskPayIntvSchema)super.clone();
    ...
    /**
     * 使用另外一个 PD_LMRiskPayIntvSchema 对象给 Schema 赋值
     * @param: aPD_LMRiskPayIntvSchema PD_LMRiskPayIntvSchema
     **/
    public void setSchema(PD_LMRiskPayIntvSchema aPD_LMRiskPayIntvSchema)
    {
        this.RiskCode = aPD_LMRiskPayIntvSchema.getRiskCode();
        this.PayIntv = aPD_LMRiskPayIntvSchema.getPayIntv();
        this.ChooseFlag = aPD_LMRiskPayIntvSchema.getChooseFlag();
        this.Operator = aPD_LMRiskPayIntvSchema.getOperator();
        this.MakeDate = fDate.getDate(aPD_LMRiskPayIntvSchema.getMakeDate());
        this.MakeTime = aPD_LMRiskPayIntvSchema.getMakeTime();
        this.ModifyDate = fDate.getDate(aPD_LMRiskPayIntvSchema.getModifyDate());
        this.ModifyTime = aPD_LMRiskPayIntvSchema.getModifyTime();
        this.Standbyflag1 = aPD_LMRiskPayIntvSchema.getStandbyflag1();
        this.Standbyflag2 = aPD_LMRiskPayIntvSchema.getStandbyflag2();
        this.Standbyflag3 = aPD_LMRiskPayIntvSchema.getStandbyflag3();
        this.Standbyflag4 = aPD_LMRiskPayIntvSchema.getStandbyflag4();
        this.Standbyflag5 = aPD_LMRiskPayIntvSchema.getStandbyflag5();
        this.Standbyflag6 = aPD_LMRiskPayIntvSchema.getStandbyflag6();
    }
    ...
    public boolean setSchema(ResultSet rs, int i)
    {
        try
        {
            ...
            this.PayIntv = rs.getInt(2);
            if (rs.getString(3) == null)
                this.ChooseFlag = null;
            else
                this.ChooseFlag = rs.getString(3).trim();
        ...
        {
            logger.debug("数据库中表PD_LMRiskPayIntv字段个数和Schema中的字段个数不一致，或执行db.executeQuery查询时未使用select * from tables");
            // @@错误处理
            CError tError = new CError();
            tError.moduleName = "PD_LMRiskPayIntvSchema";
    ...
    public PD_LMRiskPayIntvSchema getSchema()
    {
        PD_LMRiskPayIntvSchema aPD_LMRiskPayIntvSchema = new PD_LMRiskPayIntvSchema();
        aPD_LMRiskPayIntvSchema.setSchema(this);
        return aPD_LMRiskPayIntvSchema;
    }

    public PD_LMRiskPayIntvDB getDB()
    {
        PD_LMRiskPayIntvDB aDBOper = new PD_LMRiskPayIntvDB();
    ...
    /**
     * 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpPD_LMRiskPayIntv>历史记账凭证主表信息</A>表字段
    ...
    public boolean decode(String strMessage)
    {
        try
        {
            ...
            PayIntv = new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage, 2, SysConst.PACKAGESPILTER))).intValue();
            ChooseFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER);
    ...
    public boolean equals(Object otherObject)
    {
        ...
        if (getClass() != otherObject.getClass())
            return false;
        PD_LMRiskPayIntvSchema other = (PD_LMRiskPayIntvSchema) otherObject;
        return
                (RiskCode == null ? other.getRiskCode() == null : RiskCode.equals(other.getRiskCode()))
                && PayIntv == other.getPayIntv()
                && (ChooseFlag == null ? other.getChooseFlag() == null : ChooseFlag.equals(other.getChooseFlag()))

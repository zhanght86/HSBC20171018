...
/*
 * <p>ClassName: LPIndUWSubDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPIndUWSubDB extends LPIndUWSubSchema
{
private static Logger logger = Logger.getLogger(LPIndUWSubDB.class);
	...
	**/
	...
	// @Constructor
	public LPIndUWSubDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LPIndUWSub" );
		mflag = true;
	}
	...
	{
		LPIndUWSubSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPIndUWSubDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LPIndUWSub WHERE  ContNo = ? AND UWNo = ? AND InsuredNo = ? AND EdorNo = ? AND EdorType = ?");
			...
			if(this.getEdorType() == null || this.getEdorType().equals("null")) {
				...
				pstmt.setString(5, this.getEdorType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
		...
		SQLString sqlObj = new SQLString("LPIndUWSub");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LPIndUWSub SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , EdorNo = ? , EdorType = ? WHERE  ContNo = ? AND UWNo = ? AND InsuredNo = ? AND EdorNo = ? AND EdorType = ?");
			...
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				...
				pstmt.setString(44, this.getEdorNo());
			}
			if(this.getEdorType() == null || this.getEdorType().equals("null")) {
				pstmt.setNull(45, 12);
			} else {
				pstmt.setString(45, this.getEdorType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LPIndUWSub(GrpContNo ,ContNo ,ProposalContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea ,EdorNo ,EdorType) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPIndUWSub WHERE  ContNo = ? AND UWNo = ? AND InsuredNo = ? AND EdorNo = ? AND EdorType = ?", 
	...
	public LPIndUWSubSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPIndUWSubSet aLPIndUWSubSet = new LPIndUWSubSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPIndUWSub");
			LPIndUWSubSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPIndUWSubSchema s1 = new LPIndUWSubSchema();
				s1.setSchema(rs,i);
				aLPIndUWSubSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPIndUWSubSet;

	}

	public LPIndUWSubSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPIndUWSubSet aLPIndUWSubSet = new LPIndUWSubSet();
	...
	public LPIndUWSubSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPIndUWSubSet aLPIndUWSubSet = new LPIndUWSubSet();
	...
	public LPIndUWSubSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPIndUWSubSet aLPIndUWSubSet = new LPIndUWSubSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPIndUWSub " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPIndUWSubSet
 */
public LPIndUWSubSet getData()
{
    int tCount = 0;
    LPIndUWSubSet tLPIndUWSubSet = new LPIndUWSubSet();
    LPIndUWSubSchema tLPIndUWSubSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPIndUWSubDB";
    ...
    try
    {
        ...
        tLPIndUWSubSchema.setSchema(mResultSet, 1);
        tLPIndUWSubSet.add(tLPIndUWSubSchema);
...
public boolean closeData()
{
	...
	public LPIndUWSubSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPIndUWSubSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LPContPlanDetailDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LPContPlanDetailDB extends LPContPlanDetailSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPContPlanDetailDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPContPlanDetailSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPContPlanDetailDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				...
				pstmt.setString(29, this.getEdorNo());
			}
			if(this.getEdorType() == null || this.getEdorType().equals("null")) {
				pstmt.setNull(30, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPContPlanDetail WHERE  EdorNo = ? AND EdorType = ? AND PolicyNo = ? AND SysPlanCode = ? AND PlanCode = ? AND RiskCode = ? AND DutyCode = ?", 
	...
	public LPContPlanDetailSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPContPlanDetailSet aLPContPlanDetailSet = new LPContPlanDetailSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPContPlanDetail");
			LPContPlanDetailSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPContPlanDetailSchema s1 = new LPContPlanDetailSchema();
				s1.setSchema(rs,i);
				aLPContPlanDetailSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPContPlanDetailSet;
	}

	public LPContPlanDetailSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPContPlanDetailSet aLPContPlanDetailSet = new LPContPlanDetailSet();
	...
	public LPContPlanDetailSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LPContPlanDetailSet aLPContPlanDetailSet = new LPContPlanDetailSet();
	...
	public LPContPlanDetailSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPContPlanDetailSet aLPContPlanDetailSet = new LPContPlanDetailSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPContPlanDetail " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPContPlanDetailSet
 */
public LPContPlanDetailSet getData()
{
    int tCount = 0;
    LPContPlanDetailSet tLPContPlanDetailSet = new LPContPlanDetailSet();
    LPContPlanDetailSchema tLPContPlanDetailSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPContPlanDetailDB";
    ...
    {
        tCount = 1;
        tLPContPlanDetailSchema = new LPContPlanDetailSchema();
        tLPContPlanDetailSchema.setSchema(mResultSet, 1);
        tLPContPlanDetailSet.add(tLPContPlanDetailSchema);
...
public boolean closeData()
{
	...
	public LPContPlanDetailSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPContPlanDetailSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

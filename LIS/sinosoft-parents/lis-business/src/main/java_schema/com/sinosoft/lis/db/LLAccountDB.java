...
/*
 * <p>ClassName: LLAccountDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_4
 */
public class LLAccountDB extends LLAccountSchema
{
	private static Logger logger = Logger.getLogger(LLAccountDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LLAccountDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLAccountSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(6, this.getBnfRate());
			...
			if(this.getModefyTime() == null || this.getModefyTime().equals("null")) {
				...
				pstmt.setString(14, this.getModefyTime());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLAccount WHERE  ContNo = ? AND Name = ?", 
	...
	public LLAccountSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLAccountSet aLLAccountSet = new LLAccountSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLAccount");
			LLAccountSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLAccountSchema s1 = new LLAccountSchema();
				s1.setSchema(rs,i);
				aLLAccountSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLAccountSet;
	}

	public LLAccountSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLAccountSet aLLAccountSet = new LLAccountSet();
	...
	public LLAccountSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLAccountSet aLLAccountSet = new LLAccountSet();
	...
	public LLAccountSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLAccountSet aLLAccountSet = new LLAccountSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLAccount " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LLAccountSet
 */
public LLAccountSet getData()
{
    int tCount = 0;
    LLAccountSet tLLAccountSet = new LLAccountSet();
    LLAccountSchema tLLAccountSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLAccountDB";
    ...
    try
    {
        ...
        tLLAccountSchema.setSchema(mResultSet, 1);
        tLLAccountSet.add(tLLAccountSchema);
...
public boolean closeData()
{
	...
	public LLAccountSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LLAccountSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

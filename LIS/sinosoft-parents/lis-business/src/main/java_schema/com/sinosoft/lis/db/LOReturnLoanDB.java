...
/*
 * <p>ClassName: LOReturnLoanDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保
 */
public class LOReturnLoanDB extends LOReturnLoanSchema
{
private static Logger logger = Logger.getLogger(LOReturnLoanDB.class);
	...
	**/
	...
	// @Constructor
	public LOReturnLoanDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LOReturnLoanSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOReturnLoanDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getLoanNo() == null || this.getLoanNo().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getLoanNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getCurrency());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getLoanNo() == null || this.getLoanNo().equals("null")) {
				...
				pstmt.setString(25, this.getLoanNo());
			}
			pstmt.setDouble(26, this.getReturnMoney());
			pstmt.setDouble(27, this.getReturnInterest());
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getClmNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
			...
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				...
				pstmt.setString(30, this.getContNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
			...
			if(this.getLoanNo() == null || this.getLoanNo().equals("null")) {
				...
				pstmt.setString(33, this.getLoanNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(34, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOReturnLoan WHERE  ContNo = ? AND PolNo = ? AND EdorNo = ? AND LoanNo = ? AND Currency = ?", 
	...
	public LOReturnLoanSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LOReturnLoanSet aLOReturnLoanSet = new LOReturnLoanSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOReturnLoan");
			LOReturnLoanSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOReturnLoanSchema s1 = new LOReturnLoanSchema();
				s1.setSchema(rs,i);
				aLOReturnLoanSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOReturnLoanSet;

	}

	public LOReturnLoanSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOReturnLoanSet aLOReturnLoanSet = new LOReturnLoanSet();
	...
	public LOReturnLoanSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LOReturnLoanSet aLOReturnLoanSet = new LOReturnLoanSet();
	...
	public LOReturnLoanSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOReturnLoanSet aLOReturnLoanSet = new LOReturnLoanSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOReturnLoan " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOReturnLoanSet
 */
public LOReturnLoanSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLOReturnLoanSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LOReturnLoanSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOReturnLoanSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LOBonusGrpPolDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 分红管理
 */
public class LOBonusGrpPolDB extends LOBonusGrpPolSchema
{
private static Logger logger = Logger.getLogger(LOBonusGrpPolDB.class);
	...
	**/
	...
	// @Constructor
	public LOBonusGrpPolDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LOBonusGrpPolSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBonusGrpPolDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setInt(6, this.getPolYear());
			if(this.getPolTypeFlag() == null || this.getPolTypeFlag().equals("null")) {
				pstmt.setNull(7, 1);
			} else {
				pstmt.setString(7, this.getPolTypeFlag());
			}
			pstmt.setDouble(8, this.getBonusMoney());
			if(this.getBonusFlag() == null || this.getBonusFlag().equals("null")) {
				pstmt.setNull(9, 1);
			} else {
				pstmt.setString(9, this.getBonusFlag());
			}
			...
			if(this.getSGetDate() == null || this.getSGetDate().equals("null")) {
				...
				pstmt.setDate(11, Date.valueOf(this.getSGetDate()));
			}
			if(this.getAGetDate() == null || this.getAGetDate().equals("null")) {
				pstmt.setNull(12, 91);
			} else {
				pstmt.setDate(12, Date.valueOf(this.getAGetDate()));
			}
			...
			if(this.getInsuAccNo() == null || this.getInsuAccNo().equals("null")) {
				...
				pstmt.setString(21, this.getInsuAccNo());
			}
			pstmt.setInt(22, this.getFiscalYear());
			if(this.getPayPlanCode() == null || this.getPayPlanCode().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBonusGrpPol WHERE  PolNo = ? AND GrpPolNo = ? AND InsuAccNo = ? AND FiscalYear = ? AND PayPlanCode = ?", 
	...
	public LOBonusGrpPolSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LOBonusGrpPolSet aLOBonusGrpPolSet = new LOBonusGrpPolSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBonusGrpPol");
			LOBonusGrpPolSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBonusGrpPolSchema s1 = new LOBonusGrpPolSchema();
				s1.setSchema(rs,i);
				aLOBonusGrpPolSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBonusGrpPolSet;

	}

	public LOBonusGrpPolSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBonusGrpPolSet aLOBonusGrpPolSet = new LOBonusGrpPolSet();
	...
	public LOBonusGrpPolSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LOBonusGrpPolSet aLOBonusGrpPolSet = new LOBonusGrpPolSet();
	...
	public LOBonusGrpPolSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBonusGrpPolSet aLOBonusGrpPolSet = new LOBonusGrpPolSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBonusGrpPol " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBonusGrpPolSet
 */
public LOBonusGrpPolSet getData()
{
    int tCount = 0;
    LOBonusGrpPolSet tLOBonusGrpPolSet = new LOBonusGrpPolSet();
    LOBonusGrpPolSchema tLOBonusGrpPolSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBonusGrpPolDB";
    ...
    {
        tCount = 1;
        tLOBonusGrpPolSchema = new LOBonusGrpPolSchema();
        tLOBonusGrpPolSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LOBonusGrpPolSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBonusGrpPolSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

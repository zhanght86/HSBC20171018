...
{
	private static Logger logger = Logger.getLogger(LATreeDB.class);
	// @Field
	...
	**/
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getIsHandlers() == null || this.getIsHandlers().equals("null")) {
				pstmt.setNull(41, 12);
			} else {
				pstmt.setString(41, this.getIsHandlers());
			}
			// set where condition
			if(this.getAgentCode() == null || this.getAgentCode().equals("null")) {
				pstmt.setNull(42, 12);
...
public boolean closeData()
{
	...
	public LATreeSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LATreeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

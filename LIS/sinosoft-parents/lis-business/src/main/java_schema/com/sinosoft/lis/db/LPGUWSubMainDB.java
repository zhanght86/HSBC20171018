...
/*
 * <p>ClassName: LPGUWSubMainDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPGUWSubMainDB extends LPGUWSubMainSchema
{
private static Logger logger = Logger.getLogger(LPGUWSubMainDB.class);
	...
	**/
	...
	// @Constructor
	public LPGUWSubMainDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LPGUWSubMain" );
		mflag = true;
	}

	public LPGUWSubMainDB()
	{
		con = null;
		db = new DBOper( "LPGUWSubMain" );
		mflag = false;
	}
	...
	{
		LPGUWSubMainSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPGUWSubMainDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LPGUWSubMain WHERE  EdorNo = ? AND GrpContNo = ? AND UWNo = ?");
		...
		SQLString sqlObj = new SQLString("LPGUWSubMain");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LPGUWSubMain(EdorNo ,GrpContNo ,ProposalGrpContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getEdorNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPGUWSubMain WHERE  EdorNo = ? AND GrpContNo = ? AND UWNo = ?", 
	...
	public LPGUWSubMainSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGUWSubMainSet aLPGUWSubMainSet = new LPGUWSubMainSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPGUWSubMain");
			LPGUWSubMainSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPGUWSubMainSchema s1 = new LPGUWSubMainSchema();
				s1.setSchema(rs,i);
				aLPGUWSubMainSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPGUWSubMainSet;

	}

	public LPGUWSubMainSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGUWSubMainSet aLPGUWSubMainSet = new LPGUWSubMainSet();
	...
	public LPGUWSubMainSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGUWSubMainSet aLPGUWSubMainSet = new LPGUWSubMainSet();
	...
	public LPGUWSubMainSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGUWSubMainSet aLPGUWSubMainSet = new LPGUWSubMainSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPGUWSubMain " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPGUWSubMainSet
 */
public LPGUWSubMainSet getData()
{
    int tCount = 0;
    LPGUWSubMainSet tLPGUWSubMainSet = new LPGUWSubMainSet();
    LPGUWSubMainSchema tLPGUWSubMainSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPGUWSubMainDB";
    ...
    {
        tCount = 1;
        tLPGUWSubMainSchema = new LPGUWSubMainSchema();
        tLPGUWSubMainSchema.setSchema(mResultSet, 1);
        tLPGUWSubMainSet.add(tLPGUWSubMainSchema);
...
public boolean closeData()
{
	...
	public LPGUWSubMainSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPGUWSubMainSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LLRgtAffixDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统lis5模型
 */
public class LLRgtAffixDB extends LLRgtAffixSchema
{
private static Logger logger = Logger.getLogger(LLRgtAffixDB.class);
	...
	**/
	...
	// @Constructor
	public LLRgtAffixDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLRgtAffixSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLRgtAffixDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAffixNo() == null || this.getAffixNo().equals("null")) {
				...
				pstmt.setString(5, this.getAffixNo());
			}
			...
			pstmt.setInt(13, this.getSerialNo());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLRgtAffix WHERE  RgtNo = ? AND SerialNo = ?", 
	...
	public LLRgtAffixSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLRgtAffixSet aLLRgtAffixSet = new LLRgtAffixSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLRgtAffix");
			LLRgtAffixSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLRgtAffixSchema s1 = new LLRgtAffixSchema();
				s1.setSchema(rs,i);
				aLLRgtAffixSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLRgtAffixSet;

	}

	public LLRgtAffixSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLRgtAffixSet aLLRgtAffixSet = new LLRgtAffixSet();
	...
	public LLRgtAffixSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLRgtAffixSet aLLRgtAffixSet = new LLRgtAffixSet();
	...
	public LLRgtAffixSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLRgtAffixSet aLLRgtAffixSet = new LLRgtAffixSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLRgtAffix " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLRgtAffixSet
 */
public LLRgtAffixSet getData()
{
    int tCount = 0;
    LLRgtAffixSet tLLRgtAffixSet = new LLRgtAffixSet();
    LLRgtAffixSchema tLLRgtAffixSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLRgtAffixDB";
    ...
    try
    {
        ...
        tLLRgtAffixSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLRgtAffixSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLRgtAffixSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

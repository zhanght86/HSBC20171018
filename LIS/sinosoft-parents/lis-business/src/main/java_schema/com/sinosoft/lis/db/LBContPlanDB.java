...
/*
 * <p>ClassName: LBContPlanDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 泰康未整理PDM
 */
public class LBContPlanDB extends LBContPlanSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LBContPlanDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBContPlanSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBContPlanDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getContPlanName() == null || this.getContPlanName().equals("null")) {
				...
				pstmt.setString(5, this.getContPlanName());
			}
			if(this.getPlanType() == null || this.getPlanType().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
			...
			if(this.getPlanRule() == null || this.getPlanRule().equals("null")) {
				...
				pstmt.setString(7, this.getPlanRule());
			}
			if(this.getPlanSql() == null || this.getPlanSql().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getPlanSql());
			}
			...
			pstmt.setInt(15, this.getPeoples3());
			if(this.getRemark2() == null || this.getRemark2().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
				pstmt.setString(16, this.getRemark2());
			}
			// set where condition
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(17, 12);
			...
			if(this.getContPlanCode() == null || this.getContPlanCode().equals("null")) {
				...
				pstmt.setString(18, this.getContPlanCode());
			}
			if(this.getPlanType() == null || this.getPlanType().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
				pstmt.setString(19, this.getPlanType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBContPlan WHERE  GrpContNo = ? AND ContPlanCode = ? AND PlanType = ?", 
	...
	public LBContPlanSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBContPlan");
			LBContPlanSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBContPlanSchema s1 = new LBContPlanSchema();
				s1.setSchema(rs,i);
				aLBContPlanSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBContPlanSet;
	}

	public LBContPlanSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBContPlanSet aLBContPlanSet = new LBContPlanSet();
	...
	public LBContPlanSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LBContPlanSet aLBContPlanSet = new LBContPlanSet();
	...
	public LBContPlanSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBContPlanSet aLBContPlanSet = new LBContPlanSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBContPlan " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBContPlanSet
 */
public LBContPlanSet getData()
{
    int tCount = 0;
    LBContPlanSet tLBContPlanSet = new LBContPlanSet();
    LBContPlanSchema tLBContPlanSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBContPlanDB";
    ...
    try
    {
        ...
        tLBContPlanSchema.setSchema(mResultSet, 1);
        tLBContPlanSet.add(tLBContPlanSchema);
...
public boolean closeData()
{
	...
	public LBContPlanSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBContPlanSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

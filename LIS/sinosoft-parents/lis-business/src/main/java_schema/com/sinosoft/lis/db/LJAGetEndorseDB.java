...
/*
 * <p>ClassName: LJAGetEndorseDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务收费
 */
public class LJAGetEndorseDB extends LJAGetEndorseSchema
{
	private static Logger logger = Logger.getLogger(LJAGetEndorseDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJAGetEndorseDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJAGetEndorseSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJAGetEndorseDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRiskVersion() == null || this.getRiskVersion().equals("null")) {
				...
				pstmt.setString(22, this.getRiskVersion());
			}
			...
			if(this.getAgentGroup() == null || this.getAgentGroup().equals("null")) {
				...
				pstmt.setString(27, this.getAgentGroup());
			}
			...
			if(this.getHandler() == null || this.getHandler().equals("null")) {
				...
				pstmt.setString(29, this.getHandler());
			}
			if(this.getApproveDate() == null || this.getApproveDate().equals("null")) {
				pstmt.setNull(30, 91);
			} else {
				pstmt.setDate(30, Date.valueOf(this.getApproveDate()));
			}
			if(this.getApproveTime() == null || this.getApproveTime().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
				pstmt.setString(31, this.getApproveTime());
			}
			...
			if(this.getGetFlag() == null || this.getGetFlag().equals("null")) {
				...
				pstmt.setString(39, this.getGetFlag());
			}
			...
			if(this.getSubFeeOperationType() == null || this.getSubFeeOperationType().equals("null")) {
				...
				pstmt.setString(41, this.getSubFeeOperationType());
			}
			if(this.getOperState() == null || this.getOperState().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
				pstmt.setString(42, this.getOperState());
			}
			...
			pstmt.setDouble(44, this.getPeriodPrem());
			...
			if(this.getEndorsementNo() == null || this.getEndorsementNo().equals("null")) {
				...
				pstmt.setString(51, this.getEndorsementNo());
			}
			if(this.getFeeOperationType() == null || this.getFeeOperationType().equals("null")) {
				pstmt.setNull(52, 12);
			} else {
				pstmt.setString(52, this.getFeeOperationType());
			}
			if(this.getFeeFinaType() == null || this.getFeeFinaType().equals("null")) {
				pstmt.setNull(53, 12);
			} else {
				pstmt.setString(53, this.getFeeFinaType());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(54, 12);
			} else {
				pstmt.setString(54, this.getPolNo());
			}
			if(this.getOtherNo() == null || this.getOtherNo().equals("null")) {
				pstmt.setNull(55, 12);
			} else {
				pstmt.setString(55, this.getOtherNo());
			}
			if(this.getDutyCode() == null || this.getDutyCode().equals("null")) {
				pstmt.setNull(56, 12);
			} else {
			...
			if(this.getPayPlanCode() == null || this.getPayPlanCode().equals("null")) {
				...
				pstmt.setString(57, this.getPayPlanCode());
			}
			if(this.getSubFeeOperationType() == null || this.getSubFeeOperationType().equals("null")) {
				pstmt.setNull(58, 12);
			} else {
				pstmt.setString(58, this.getSubFeeOperationType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJAGetEndorse WHERE  ActuGetNo = ? AND EndorsementNo = ? AND FeeOperationType = ? AND FeeFinaType = ? AND PolNo = ? AND OtherNo = ? AND DutyCode = ? AND PayPlanCode = ? AND SubFeeOperationType = ?", 
	...
	public LJAGetEndorseSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJAGetEndorseSet aLJAGetEndorseSet = new LJAGetEndorseSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJAGetEndorse");
			LJAGetEndorseSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJAGetEndorseSchema s1 = new LJAGetEndorseSchema();
				s1.setSchema(rs,i);
				aLJAGetEndorseSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJAGetEndorseSet;
	}

	public LJAGetEndorseSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJAGetEndorseSet aLJAGetEndorseSet = new LJAGetEndorseSet();
	...
	public LJAGetEndorseSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJAGetEndorseSet aLJAGetEndorseSet = new LJAGetEndorseSet();
	...
	public LJAGetEndorseSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJAGetEndorseSet aLJAGetEndorseSet = new LJAGetEndorseSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJAGetEndorse " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJAGetEndorseSet
 */
public LJAGetEndorseSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLJAGetEndorseSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LJAGetEndorseSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJAGetEndorseSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

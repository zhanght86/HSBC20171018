...
import com.sinosoft.lis.db.LCServInfoDB;

/*
 * <p>ClassName: LCServInfoSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCServInfoSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LCServInfoSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LCServInfoSchema cloned = (LCServInfoSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LCServInfoSchema 对象给 Schema 赋值
	* @param: aLCServInfoSchema LCServInfoSchema
	**/
	public void setSchema(LCServInfoSchema aLCServInfoSchema)
	{
		this.GrpContNo = aLCServInfoSchema.getGrpContNo();
		this.ContNo = aLCServInfoSchema.getContNo();
		this.ProposalContNo = aLCServInfoSchema.getProposalContNo();
		this.PrtNo = aLCServInfoSchema.getPrtNo();
		this.ServKind = aLCServInfoSchema.getServKind();
		this.ServDetail = aLCServInfoSchema.getServDetail();
		this.ServChoose = aLCServInfoSchema.getServChoose();
		this.ServRemark = aLCServInfoSchema.getServRemark();
		this.MakeDate = fDate.getDate( aLCServInfoSchema.getMakeDate());
		this.MakeTime = aLCServInfoSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLCServInfoSchema.getModifyDate());
		this.ModifyTime = aLCServInfoSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LCServInfo表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LCServInfoSchema";
	...
	public LCServInfoSchema getSchema()
	{
		LCServInfoSchema aLCServInfoSchema = new LCServInfoSchema();
		aLCServInfoSchema.setSchema(this);
		return aLCServInfoSchema;
	}

	public LCServInfoDB getDB()
	{
		LCServInfoDB aDBOper = new LCServInfoDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLCServInfo>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ServKind = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			ServDetail = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			ServChoose = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			ServRemark = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LCServInfoSchema other = (LCServInfoSchema)otherObject;
		return

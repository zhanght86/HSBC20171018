...
/*
 * <p>ClassName: LJSGetDrawDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务付费
 */
public class LJSGetDrawDB extends LJSGetDrawSchema
{
	private static Logger logger = Logger.getLogger(LJSGetDrawDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJSGetDrawDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LJSGetDraw" );
		mflag = true;
	}
	...
	{
		LJSGetDrawSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJSGetDrawDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LJSGetDraw WHERE  GetNoticeNo = ? AND PolNo = ? AND DutyCode = ? AND GetDutyKind = ? AND GetDutyCode = ?");
		...
		SQLString sqlObj = new SQLString("LJSGetDraw");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getGetFirstFlag() == null || this.getGetFirstFlag().equals("null")) {
				...
				pstmt.setString(29, this.getGetFirstFlag());
			}
			...
			if(this.getRReportFlag() == null || this.getRReportFlag().equals("null")) {
				...
				pstmt.setString(42, this.getRReportFlag());
			}
			if(this.getComeFlag() == null || this.getComeFlag().equals("null")) {
				pstmt.setNull(43, 1);
			} else {
				pstmt.setString(43, this.getComeFlag());
			}
			...
			if(this.getGetDutyKind() == null || this.getGetDutyKind().equals("null")) {
				...
				pstmt.setString(51, this.getGetDutyKind());
			}
			if(this.getGetDutyCode() == null || this.getGetDutyCode().equals("null")) {
				pstmt.setNull(52, 12);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LJSGetDraw VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGetNoticeNo() == null || this.getGetNoticeNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGetNoticeNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJSGetDraw WHERE  GetNoticeNo = ? AND PolNo = ? AND DutyCode = ? AND GetDutyKind = ? AND GetDutyCode = ?", 
	...
	public LJSGetDrawSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSGetDrawSet aLJSGetDrawSet = new LJSGetDrawSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJSGetDraw");
			LJSGetDrawSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJSGetDrawSchema s1 = new LJSGetDrawSchema();
				s1.setSchema(rs,i);
				aLJSGetDrawSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJSGetDrawSet;
	}

	public LJSGetDrawSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSGetDrawSet aLJSGetDrawSet = new LJSGetDrawSet();
	...
	public LJSGetDrawSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJSGetDrawSet aLJSGetDrawSet = new LJSGetDrawSet();
	...
	public LJSGetDrawSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJSGetDrawSet aLJSGetDrawSet = new LJSGetDrawSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJSGetDraw " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJSGetDrawSet
 */
public LJSGetDrawSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLJSGetDrawSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LJSGetDrawSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJSGetDrawSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

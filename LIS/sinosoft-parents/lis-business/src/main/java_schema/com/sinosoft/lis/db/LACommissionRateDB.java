...
/*
 * <p>ClassName: LACommissionRateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data _1
 */
public class LACommissionRateDB extends LACommissionRateSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LACommissionRateDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LACommissionRate" );
		mflag = true;
	}

	public LACommissionRateDB()
	{
		con = null;
		db = new DBOper( "LACommissionRate" );
		mflag = false;
	}
	...
	{
		LACommissionRateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LACommissionRateDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LACommissionRate WHERE  IDNo = ?");
		...
		SQLString sqlObj = new SQLString("LACommissionRate");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LACommissionRate SET  IDNo = ? , AgentCom = ? , ManageCom = ? , RiskCode = ? , PayYear = ? , Currency = ? , InvestType = ? , PayType = ? , ProtectionPlan = ? , PayYears = ? , CommRate = ? , StartDate = ? , EndDate = ? , ValidFlag = ? , AppState = ? , AppDate = ? , AppTime = ? , Branchtype = ? , ActionType = ? , OrigIDNo = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , StaffRate = ? , InsureAgeStart = ? , InsureAgeEnd = ? , PayToAge = ? , DefaultFlag = ? , SRFlag = ? , PlanCode = ? , Insuyear = ? , InsuyearFlag = ? , SectionFlag = ? , FPDFlag = ? WHERE  IDNo = ?");
			if(this.getIDNo() == null || this.getIDNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getIDNo());
			...
			if(this.getSectionFlag() == null || this.getSectionFlag().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getSectionFlag());
			}
			if(this.getFPDFlag() == null || this.getFPDFlag().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
				pstmt.setString(36, this.getFPDFlag());
			}
			// set where condition
			if(this.getIDNo() == null || this.getIDNo().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getIDNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LACommissionRate(IDNo ,AgentCom ,ManageCom ,RiskCode ,PayYear ,Currency ,InvestType ,PayType ,ProtectionPlan ,PayYears ,CommRate ,StartDate ,EndDate ,ValidFlag ,AppState ,AppDate ,AppTime ,Branchtype ,ActionType ,OrigIDNo ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,StaffRate ,InsureAgeStart ,InsureAgeEnd ,PayToAge ,DefaultFlag ,SRFlag ,PlanCode ,Insuyear ,InsuyearFlag ,SectionFlag ,FPDFlag) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getIDNo() == null || this.getIDNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getIDNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LACommissionRate WHERE  IDNo = ?", 
	...
	public LACommissionRateSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LACommissionRateSet aLACommissionRateSet = new LACommissionRateSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("LACommissionRate");
			LACommissionRateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LACommissionRateSchema s1 = new LACommissionRateSchema();
				s1.setSchema(rs,i);
				aLACommissionRateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aLACommissionRateSet;
	}

	public LACommissionRateSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LACommissionRateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LACommissionRateSet aLACommissionRateSet = new LACommissionRateSet();
	...
	public LACommissionRateSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LACommissionRateSet aLACommissionRateSet = new LACommissionRateSet();
	...
	public LACommissionRateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LACommissionRateSet aLACommissionRateSet = new LACommissionRateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LACommissionRate " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LACommissionRateSet
 */
public LACommissionRateSet getData()
{
    int tCount = 0;
    LACommissionRateSet tLACommissionRateSet = new LACommissionRateSet();
    LACommissionRateSchema tLACommissionRateSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LACommissionRateDB";
    ...
    {
        tCount = 1;
        tLACommissionRateSchema = new LACommissionRateSchema();
        tLACommissionRateSchema.setSchema(mResultSet, 1);
        tLACommissionRateSet.add(tLACommissionRateSchema);

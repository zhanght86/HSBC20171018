...
import com.sinosoft.lis.db.LPBalanceRelaDB;

/*
 * <p>ClassName: LPBalanceRelaSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LPBalanceRelaSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPBalanceRelaSchema.class);
	// @Field
	/** 定期结算申请号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPBalanceRelaSchema cloned = (LPBalanceRelaSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LPBalanceRelaSchema 对象给 Schema 赋值
	* @param: aLPBalanceRelaSchema LPBalanceRelaSchema
	**/
	public void setSchema(LPBalanceRelaSchema aLPBalanceRelaSchema)
	{
		this.BalanceAppNo = aLPBalanceRelaSchema.getBalanceAppNo();
		this.BalanceAllNo = aLPBalanceRelaSchema.getBalanceAllNo();
		this.EdorAppNo = aLPBalanceRelaSchema.getEdorAppNo();
		this.GrpContNo = aLPBalanceRelaSchema.getGrpContNo();
		this.BalanceType = aLPBalanceRelaSchema.getBalanceType();
		this.BalanceRelaSum = aLPBalanceRelaSchema.getBalanceRelaSum();
		this.BalanceRelaOperator = aLPBalanceRelaSchema.getBalanceRelaOperator();
		this.BalanceRelaType = aLPBalanceRelaSchema.getBalanceRelaType();
		this.BalanceRelaState = aLPBalanceRelaSchema.getBalanceRelaState();
		this.BalanceRelaDate = fDate.getDate( aLPBalanceRelaSchema.getBalanceRelaDate());
		this.BalanceRelaTime = aLPBalanceRelaSchema.getBalanceRelaTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPBalanceRela表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPBalanceRelaSchema";
	...
	public LPBalanceRelaSchema getSchema()
	{
		LPBalanceRelaSchema aLPBalanceRelaSchema = new LPBalanceRelaSchema();
		aLPBalanceRelaSchema.setSchema(this);
		return aLPBalanceRelaSchema;
	}

	public LPBalanceRelaDB getDB()
	{
		LPBalanceRelaDB aDBOper = new LPBalanceRelaDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPBalanceRela描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPBalanceRela>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			BalanceType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			BalanceRelaSum = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,6,SysConst.PACKAGESPILTER))).doubleValue();
			BalanceRelaOperator = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			BalanceRelaType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			BalanceRelaState = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			BalanceRelaDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10,SysConst.PACKAGESPILTER));
			BalanceRelaTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPBalanceRelaSchema other = (LPBalanceRelaSchema)otherObject;
		return

...
/*
 * <p>ClassName: LLCUWSubDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLCUWSubDB extends LLCUWSubSchema
{
private static Logger logger = Logger.getLogger(LLCUWSubDB.class);
	...
	**/
	...
	// @Constructor
	public LLCUWSubDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLCUWSubSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getBatNo() == null || this.getBatNo().equals("null")) {
				...
				pstmt.setString(43, this.getBatNo());
			}
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLCUWSub WHERE  CaseNo = ? AND BatNo = ? AND ContNo = ? AND UWNo = ?", 
	...
	public LLCUWSubSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLCUWSubSet aLLCUWSubSet = new LLCUWSubSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLCUWSub");
			LLCUWSubSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLCUWSubSchema s1 = new LLCUWSubSchema();
				s1.setSchema(rs,i);
				aLLCUWSubSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LLCUWSubSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLCUWSubSet aLLCUWSubSet = new LLCUWSubSet();
	...
	public LLCUWSubSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLCUWSubSet aLLCUWSubSet = new LLCUWSubSet();
	...
	public LLCUWSubSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLCUWSubSet aLLCUWSubSet = new LLCUWSubSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLCUWSub " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LLCUWSubSet
 */
public LLCUWSubSet getData()
{
    int tCount = 0;
    LLCUWSubSet tLLCUWSubSet = new LLCUWSubSet();
    LLCUWSubSchema tLLCUWSubSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLCUWSubDB";
    ...
    try
    {
        ...
        tLLCUWSubSchema.setSchema(mResultSet, 1);
        tLLCUWSubSet.add(tLLCUWSubSchema);
...
public boolean closeData()
{
	...
	public LLCUWSubSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLCUWSubSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

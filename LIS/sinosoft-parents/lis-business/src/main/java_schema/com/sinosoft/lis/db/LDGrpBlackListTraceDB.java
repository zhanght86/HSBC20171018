...
/*
 * <p>ClassName: LDGrpBlackListTraceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LDGrpBlackListTraceDB extends LDGrpBlackListTraceSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDGrpBlackListTraceDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDGrpBlackListTraceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LDGrpBlackListTraceDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getBusinessType() == null || this.getBusinessType().equals("null")) {
				...
				pstmt.setString(5, this.getBusinessType());
			}
			...
			pstmt.setInt(7, this.getPeoples());
			pstmt.setDouble(8, this.getRgtMoney());
			pstmt.setDouble(9, this.getAsset());
			pstmt.setDouble(10, this.getNetProfitRate());
			if(this.getMainBussiness() == null || this.getMainBussiness().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getMainBussiness());
			}
			...
			if(this.getComAera() == null || this.getComAera().equals("null")) {
				...
				pstmt.setString(13, this.getComAera());
			}
			...
			if(this.getFoundDate() == null || this.getFoundDate().equals("null")) {
				...
				pstmt.setDate(19, Date.valueOf(this.getFoundDate()));
			}
			...
			if(this.getGrpGroupNo() == null || this.getGrpGroupNo().equals("null")) {
				...
				pstmt.setString(22, this.getGrpGroupNo());
			}
			if(this.getBlacklistFlag() == null || this.getBlacklistFlag().equals("null")) {
				pstmt.setNull(23, 1);
			} else {
				pstmt.setString(23, this.getBlacklistFlag());
			}
			...
			if(this.getVIPValue() == null || this.getVIPValue().equals("null")) {
				...
				pstmt.setString(26, this.getVIPValue());
			}
			...
			if(this.getSubCompanyFlag() == null || this.getSubCompanyFlag().equals("null")) {
				...
				pstmt.setString(32, this.getSubCompanyFlag());
			}
			if(this.getSupCustoemrNo() == null || this.getSupCustoemrNo().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
				pstmt.setString(33, this.getSupCustoemrNo());
			}
			if(this.getLevelCode() == null || this.getLevelCode().equals("null")) {
				pstmt.setNull(34, 12);
			} else {
				pstmt.setString(34, this.getLevelCode());
			}
			pstmt.setInt(35, this.getOnWorkPeoples());
			pstmt.setInt(36, this.getOffWorkPeoples());
			pstmt.setInt(37, this.getOtherPeoples());
			if(this.getBusinessBigType() == null || this.getBusinessBigType().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getBusinessBigType());
			}
			if(this.getSocialInsuNo() == null || this.getSocialInsuNo().equals("null")) {
				pstmt.setNull(39, 12);
			} else {
				pstmt.setString(39, this.getSocialInsuNo());
			}
			if(this.getBlackListReason() == null || this.getBlackListReason().equals("null")) {
				pstmt.setNull(40, 12);
			} else {
				pstmt.setString(40, this.getBlackListReason());
			}
			// set where condition
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(41, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDGrpBlackListTrace WHERE  SerialNo = ?", 
	...
	public LDGrpBlackListTraceSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDGrpBlackListTraceSet aLDGrpBlackListTraceSet = new LDGrpBlackListTraceSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDGrpBlackListTrace");
			LDGrpBlackListTraceSchema aSchema = this.getSchema();
			...
			{
				i++;
				LDGrpBlackListTraceSchema s1 = new LDGrpBlackListTraceSchema();
				s1.setSchema(rs,i);
				aLDGrpBlackListTraceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLDGrpBlackListTraceSet;
	}

	public LDGrpBlackListTraceSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDGrpBlackListTraceSet aLDGrpBlackListTraceSet = new LDGrpBlackListTraceSet();
	...
	public LDGrpBlackListTraceSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDGrpBlackListTraceSet aLDGrpBlackListTraceSet = new LDGrpBlackListTraceSet();
	...
	public LDGrpBlackListTraceSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDGrpBlackListTraceSet aLDGrpBlackListTraceSet = new LDGrpBlackListTraceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDGrpBlackListTrace " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDGrpBlackListTraceSet
 */
public LDGrpBlackListTraceSet getData()
{
    int tCount = 0;
    LDGrpBlackListTraceSet tLDGrpBlackListTraceSet = new LDGrpBlackListTraceSet();
    LDGrpBlackListTraceSchema tLDGrpBlackListTraceSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDGrpBlackListTraceDB";
    ...
    {
        tCount = 1;
        tLDGrpBlackListTraceSchema = new LDGrpBlackListTraceSchema();
        tLDGrpBlackListTraceSchema.setSchema(mResultSet, 1);
    ...
    return tLDGrpBlackListTraceSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LDGrpBlackListTraceSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDGrpBlackListTraceSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

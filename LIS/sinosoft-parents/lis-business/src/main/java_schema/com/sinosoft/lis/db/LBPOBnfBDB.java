...
/*
 * <p>ClassName: LBPOBnfBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LBPOBnfBDB extends LBPOBnfBSchema
{
private static Logger logger = Logger.getLogger(LBPOBnfBDB.class);
	...
	**/
	...
	// @Constructor
	public LBPOBnfBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBPOBnfBSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getBnfLot() == null || this.getBnfLot().equals("null")) {
				...
				pstmt.setString(11, this.getBnfLot());
			}
			...
			if(this.getBirthday() == null || this.getBirthday().equals("null")) {
				...
				pstmt.setString(15, this.getBirthday());
			}
			...
			if(this.getMakeDate() == null || this.getMakeDate().equals("null")) {
				...
				pstmt.setString(19, this.getMakeDate());
			}
			...
			if(this.getBnfAddress() == null || this.getBnfAddress().equals("null")) {
				...
				pstmt.setString(23, this.getBnfAddress());
			}
			if(this.getTransDate() == null || this.getTransDate().equals("null")) {
				pstmt.setNull(24, 91);
			} else {
				pstmt.setDate(24, Date.valueOf(this.getTransDate()));
			}
			if(this.getTransTime() == null || this.getTransTime().equals("null")) {
				pstmt.setNull(25, 1);
			} else {
				pstmt.setString(25, this.getTransTime());
			}
			if(this.getTransOperator() == null || this.getTransOperator().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getTransOperator());
			}
			// set where condition
			if(this.getIDX() == null || this.getIDX().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
				pstmt.setString(27, this.getIDX());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBPOBnfB WHERE  IDX = ? AND InputNo = ? AND FillNo = ? AND PolNo = ? AND InsuredNo = ?", 
	...
	public LBPOBnfBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOBnfBSet aLBPOBnfBSet = new LBPOBnfBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBPOBnfB");
			LBPOBnfBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBPOBnfBSchema s1 = new LBPOBnfBSchema();
				s1.setSchema(rs,i);
				aLBPOBnfBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LBPOBnfBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOBnfBSet aLBPOBnfBSet = new LBPOBnfBSet();
	...
	public LBPOBnfBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOBnfBSet aLBPOBnfBSet = new LBPOBnfBSet();
	...
	public LBPOBnfBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOBnfBSet aLBPOBnfBSet = new LBPOBnfBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBPOBnfB " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LBPOBnfBSet
 */
public LBPOBnfBSet getData()
{
    int tCount = 0;
    LBPOBnfBSet tLBPOBnfBSet = new LBPOBnfBSet();
    LBPOBnfBSchema tLBPOBnfBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBPOBnfBDB";
    ...
    {
        tCount = 1;
        tLBPOBnfBSchema = new LBPOBnfBSchema();
        tLBPOBnfBSchema.setSchema(mResultSet, 1);
        tLBPOBnfBSet.add(tLBPOBnfBSchema);
...
public boolean closeData()
{
	...
	public LBPOBnfBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBPOBnfBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

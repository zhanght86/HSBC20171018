 ...
 * @Database: 银行业务
 */
public class LYDupPaySchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LYDupPaySchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LYDupPaySchema cloned = (LYDupPaySchema)super.clone();
	...
	* @param: aLYDupPaySchema LYDupPaySchema
	**/
	public void setSchema(LYDupPaySchema aLYDupPaySchema)
	{
		...
		this.SerNo = aLYDupPaySchema.getSerNo();
		this.BankName = aLYDupPaySchema.getBankName();
		this.InBankCode = aLYDupPaySchema.getInBankCode();
		this.InBankName = aLYDupPaySchema.getInBankName();
		this.InAccType = aLYDupPaySchema.getInAccType();
		this.InAccName = aLYDupPaySchema.getInAccName();
		this.InAccNo = aLYDupPaySchema.getInAccNo();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LYDupPay表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理

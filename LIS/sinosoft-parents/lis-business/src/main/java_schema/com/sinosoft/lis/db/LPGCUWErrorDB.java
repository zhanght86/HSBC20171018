...
/*
 * <p>ClassName: LPGCUWErrorDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPGCUWErrorDB extends LPGCUWErrorSchema
{
private static Logger logger = Logger.getLogger(LPGCUWErrorDB.class);
	...
	**/
	...
	// @Constructor
	public LPGCUWErrorDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPGCUWErrorSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPGCUWErrorDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPGCUWError WHERE  EdorNo = ? AND EdorType = ? AND GrpContNo = ? AND SerialNo = ? AND UWNo = ?", 
	...
	public LPGCUWErrorSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGCUWErrorSet aLPGCUWErrorSet = new LPGCUWErrorSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPGCUWError");
			LPGCUWErrorSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPGCUWErrorSchema s1 = new LPGCUWErrorSchema();
				s1.setSchema(rs,i);
				aLPGCUWErrorSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPGCUWErrorSet;

	}

	public LPGCUWErrorSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGCUWErrorSet aLPGCUWErrorSet = new LPGCUWErrorSet();
	...
	public LPGCUWErrorSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGCUWErrorSet aLPGCUWErrorSet = new LPGCUWErrorSet();
	...
	public LPGCUWErrorSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGCUWErrorSet aLPGCUWErrorSet = new LPGCUWErrorSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPGCUWError " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPGCUWErrorSet
 */
public LPGCUWErrorSet getData()
{
    int tCount = 0;
    LPGCUWErrorSet tLPGCUWErrorSet = new LPGCUWErrorSet();
    LPGCUWErrorSchema tLPGCUWErrorSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPGCUWErrorDB";
    ...
    {
        tCount = 1;
        tLPGCUWErrorSchema = new LPGCUWErrorSchema();
        tLPGCUWErrorSchema.setSchema(mResultSet, 1);
        tLPGCUWErrorSet.add(tLPGCUWErrorSchema);
...
public boolean closeData()
{
	...
	public LPGCUWErrorSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPGCUWErrorSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

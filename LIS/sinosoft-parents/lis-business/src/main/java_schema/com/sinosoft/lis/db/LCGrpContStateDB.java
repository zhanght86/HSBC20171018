...
/*
 * <p>ClassName: LCGrpContStateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCGrpContStateDB extends LCGrpContStateSchema
{
private static Logger logger = Logger.getLogger(LCGrpContStateDB.class);
	...
	**/
	...
	// @Constructor
	public LCGrpContStateDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCGrpContStateSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getStateType() == null || this.getStateType().equals("null")) {
				...
				pstmt.setString(3, this.getStateType());
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getStateReason() == null || this.getStateReason().equals("null")) {
				...
				pstmt.setString(5, this.getStateReason());
			}
			if(this.getStartDate() == null || this.getStartDate().equals("null")) {
				pstmt.setNull(6, 91);
			} else {
			...
			if(this.getStateType() == null || this.getStateType().equals("null")) {
				...
				pstmt.setString(16, this.getStateType());
			}
			if(this.getStartDate() == null || this.getStartDate().equals("null")) {
				pstmt.setNull(17, 91);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCGrpContState WHERE  GrpContNo = ? AND GrpPolNo = ? AND StateType = ? AND StartDate = ?", 
	...
	public LCGrpContStateSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCGrpContState");
			LCGrpContStateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCGrpContStateSchema s1 = new LCGrpContStateSchema();
				s1.setSchema(rs,i);
				aLCGrpContStateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCGrpContStateSet;

	}

	public LCGrpContStateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpContStateSet aLCGrpContStateSet = new LCGrpContStateSet();
	...
	public LCGrpContStateSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCGrpContStateSet aLCGrpContStateSet = new LCGrpContStateSet();
	...
	public LCGrpContStateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpContStateSet aLCGrpContStateSet = new LCGrpContStateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCGrpContState " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCGrpContStateSet
 */
public LCGrpContStateSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLCGrpContStateSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCGrpContStateSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCGrpContStateSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

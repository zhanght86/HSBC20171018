...
/*
 * <p>ClassName: LJSPayClaimSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务收费
 */
public class LJSPayClaimSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LJSPayClaimSchema.class);
	// @Field
	/** 给付通知书号码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LJSPayClaimSchema cloned = (LJSPayClaimSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LJSPayClaimSchema 对象给 Schema 赋值
	* @param: aLJSPayClaimSchema LJSPayClaimSchema
	**/
	public void setSchema(LJSPayClaimSchema aLJSPayClaimSchema)
	{
		this.GetNoticeNo = aLJSPayClaimSchema.getGetNoticeNo();
		this.FeeOperationType = aLJSPayClaimSchema.getFeeOperationType();
		this.SubFeeOperationType = aLJSPayClaimSchema.getSubFeeOperationType();
		this.FeeFinaType = aLJSPayClaimSchema.getFeeFinaType();
		this.OtherNo = aLJSPayClaimSchema.getOtherNo();
		this.OtherNoType = aLJSPayClaimSchema.getOtherNoType();
		this.DutyCode = aLJSPayClaimSchema.getDutyCode();
		this.GetDutyKind = aLJSPayClaimSchema.getGetDutyKind();
		this.GetDutyCode = aLJSPayClaimSchema.getGetDutyCode();
		this.GrpContNo = aLJSPayClaimSchema.getGrpContNo();
		this.ContNo = aLJSPayClaimSchema.getContNo();
		this.GrpPolNo = aLJSPayClaimSchema.getGrpPolNo();
		this.PolNo = aLJSPayClaimSchema.getPolNo();
		this.KindCode = aLJSPayClaimSchema.getKindCode();
		this.RiskCode = aLJSPayClaimSchema.getRiskCode();
		this.RiskVersion = aLJSPayClaimSchema.getRiskVersion();
		this.SaleChnl = aLJSPayClaimSchema.getSaleChnl();
		this.AgentCode = aLJSPayClaimSchema.getAgentCode();
		this.AgentGroup = aLJSPayClaimSchema.getAgentGroup();
		this.GetDate = fDate.getDate( aLJSPayClaimSchema.getGetDate());
		this.Pay = aLJSPayClaimSchema.getPay();
		this.ManageCom = aLJSPayClaimSchema.getManageCom();
		this.AgentCom = aLJSPayClaimSchema.getAgentCom();
		this.AgentType = aLJSPayClaimSchema.getAgentType();
		this.SerialNo = aLJSPayClaimSchema.getSerialNo();
		this.Operator = aLJSPayClaimSchema.getOperator();
		this.MakeDate = fDate.getDate( aLJSPayClaimSchema.getMakeDate());
		this.MakeTime = aLJSPayClaimSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLJSPayClaimSchema.getModifyDate());
		this.ModifyTime = aLJSPayClaimSchema.getModifyTime();
		this.Currency = aLJSPayClaimSchema.getCurrency();
		this.NetAmount = aLJSPayClaimSchema.getNetAmount();
		this.TaxAmount = aLJSPayClaimSchema.getTaxAmount();
		this.Tax = aLJSPayClaimSchema.getTax();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LJSPayClaim表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LJSPayClaimSchema";
	...
	public LJSPayClaimSchema getSchema()
	{
		LJSPayClaimSchema aLJSPayClaimSchema = new LJSPayClaimSchema();
		aLJSPayClaimSchema.setSchema(this);
		return aLJSPayClaimSchema;
	}

	public LJSPayClaimDB getDB()
	{
		LJSPayClaimDB aDBOper = new LJSPayClaimDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLJSPayClaim>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LJSPayClaimSchema other = (LJSPayClaimSchema)otherObject;
		return

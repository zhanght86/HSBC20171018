...
/*
 * <p>ClassName: LogBusinessStateSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LogBusinessStateSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LogBusinessStateSchema.class);
	// @Field
	/** 日志号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LogBusinessStateSchema cloned = (LogBusinessStateSchema)super.clone();
	...
	*/
	...
	public void setKeyNo(String aKeyNo)
	{
		if(aKeyNo!=null && aKeyNo.length()>20)
			throw new IllegalArgumentException("关键号码KeyNo值"+aKeyNo+"的长度"+aKeyNo.length()+"大于最大值20");
		KeyNo = aKeyNo;
	}
	...
	public void setState(String aState)
	{
		if(aState!=null && aState.length()>1)
			throw new IllegalArgumentException("业务状态State值"+aState+"的长度"+aState.length()+"大于最大值1");
		State = aState;
	}
	/**
	* NB,POS,CLM等
	*/
	...
	public void setItemDes(String aItemDes)
	{
		if(aItemDes!=null && aItemDes.length()>100)
			throw new IllegalArgumentException("日志信息ItemDes值"+aItemDes+"的长度"+aItemDes.length()+"大于最大值100");
		ItemDes = aItemDes;
	}
	...
	*/
	...
	public void setModifyTime(String aModifyTime)
	{
		if(aModifyTime!=null && aModifyTime.length()>8)
			throw new IllegalArgumentException("日志修改时间ModifyTime值"+aModifyTime+"的长度"+aModifyTime.length()+"大于最大值8");
		ModifyTime = aModifyTime;
	}

	/**
	* 使用另外一个 LogBusinessStateSchema 对象给 Schema 赋值
	* @param: aLogBusinessStateSchema LogBusinessStateSchema
	**/
	public void setSchema(LogBusinessStateSchema aLogBusinessStateSchema)
	{
		this.LogNo = aLogBusinessStateSchema.getLogNo();
		this.SubjectID = aLogBusinessStateSchema.getSubjectID();
		this.ItemID = aLogBusinessStateSchema.getItemID();
		this.SerialNo = aLogBusinessStateSchema.getSerialNo();
		this.KeyNo = aLogBusinessStateSchema.getKeyNo();
		this.State = aLogBusinessStateSchema.getState();
		this.ItemDes = aLogBusinessStateSchema.getItemDes();
		this.Remark = aLogBusinessStateSchema.getRemark();
		this.MakeDate = fDate.getDate( aLogBusinessStateSchema.getMakeDate());
		this.MakeTime = aLogBusinessStateSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLogBusinessStateSchema.getModifyDate());
		this.ModifyTime = aLogBusinessStateSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LogBusinessState表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LogBusinessStateSchema";
	...
	public LogBusinessStateSchema getSchema()
	{
		LogBusinessStateSchema aLogBusinessStateSchema = new LogBusinessStateSchema();
		aLogBusinessStateSchema.setSchema(this);
		return aLogBusinessStateSchema;
	}

	public LogBusinessStateDB getDB()
	{
		LogBusinessStateDB aDBOper = new LogBusinessStateDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLogBusinessState描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLogBusinessState>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ItemDes = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LogBusinessStateSchema other = (LogBusinessStateSchema)otherObject;
		return

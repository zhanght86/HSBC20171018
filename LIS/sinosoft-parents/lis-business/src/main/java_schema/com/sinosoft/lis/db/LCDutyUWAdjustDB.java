...
/*
 * <p>ClassName: LCDutyUWAdjustDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LCDutyUWAdjustDB extends LCDutyUWAdjustSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCDutyUWAdjustDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCDutyUWAdjustSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCDutyUWAdjustDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(10, this.getUWAmnt());
			pstmt.setDouble(11, this.getPrem());
			pstmt.setDouble(12, this.getUWPrem());
			if(this.getAdjustType() == null || this.getAdjustType().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getAdjustType());
			}
			if(this.getAdjustReson() == null || this.getAdjustReson().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getAdjustReson());
			}
			...
			if(this.getBussType() == null || this.getBussType().equals("null")) {
				...
				pstmt.setString(26, this.getBussType());
			}
			if(this.getBussNo() == null || this.getBussNo().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
				pstmt.setString(27, this.getBussNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCDutyUWAdjust WHERE  BussType = ? AND BussNo = ? AND PolNo = ? AND DutyCode = ?", 
	...
	public LCDutyUWAdjustSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCDutyUWAdjustSet aLCDutyUWAdjustSet = new LCDutyUWAdjustSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCDutyUWAdjust");
			LCDutyUWAdjustSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCDutyUWAdjustSchema s1 = new LCDutyUWAdjustSchema();
				s1.setSchema(rs,i);
				aLCDutyUWAdjustSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCDutyUWAdjustSet;
	}

	public LCDutyUWAdjustSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCDutyUWAdjustSet aLCDutyUWAdjustSet = new LCDutyUWAdjustSet();
	...
	public LCDutyUWAdjustSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCDutyUWAdjustSet aLCDutyUWAdjustSet = new LCDutyUWAdjustSet();
	...
	public LCDutyUWAdjustSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCDutyUWAdjustSet aLCDutyUWAdjustSet = new LCDutyUWAdjustSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCDutyUWAdjust " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCDutyUWAdjustSet
 */
public LCDutyUWAdjustSet getData()
{
    int tCount = 0;
    LCDutyUWAdjustSet tLCDutyUWAdjustSet = new LCDutyUWAdjustSet();
    LCDutyUWAdjustSchema tLCDutyUWAdjustSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCDutyUWAdjustDB";
    ...
    try
    {
        ...
        tLCDutyUWAdjustSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCDutyUWAdjustSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCDutyUWAdjustSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

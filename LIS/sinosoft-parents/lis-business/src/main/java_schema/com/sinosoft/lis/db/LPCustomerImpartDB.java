...
/*
 * <p>ClassName: LPCustomerImpartDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPCustomerImpartDB extends LPCustomerImpartSchema
{
private static Logger logger = Logger.getLogger(LPCustomerImpartDB.class);
	...
	**/
	...
	// @Constructor
	public LPCustomerImpartDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPCustomerImpartSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPCustomerImpartDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getImpartCode() == null || this.getImpartCode().equals("null")) {
				...
				pstmt.setString(25, this.getImpartCode());
			}
			if(this.getImpartVer() == null || this.getImpartVer().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getImpartVer());
			}
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
			...
			if(this.getCustomerNoType() == null || this.getCustomerNoType().equals("null")) {
				...
				pstmt.setString(28, StrTool.space1(this.getCustomerNoType(), 1));
			}
			pstmt.setInt(29, this.getPatchNo());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPCustomerImpart WHERE  EdorNo = ? AND EdorType = ? AND ContNo = ? AND ProposalContNo = ? AND ImpartCode = ? AND ImpartVer = ? AND CustomerNo = ? AND CustomerNoType = ? AND PatchNo = ?", 
	...
	public LPCustomerImpartSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPCustomerImpart");
			LPCustomerImpartSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPCustomerImpartSchema s1 = new LPCustomerImpartSchema();
				s1.setSchema(rs,i);
				aLPCustomerImpartSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPCustomerImpartSet;

	}

	public LPCustomerImpartSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPCustomerImpartSet aLPCustomerImpartSet = new LPCustomerImpartSet();
	...
	public LPCustomerImpartSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPCustomerImpartSet aLPCustomerImpartSet = new LPCustomerImpartSet();
	...
	public LPCustomerImpartSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPCustomerImpartSet aLPCustomerImpartSet = new LPCustomerImpartSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPCustomerImpart " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPCustomerImpartSet
 */
public LPCustomerImpartSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLPCustomerImpartSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LPCustomerImpartSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPCustomerImpartSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

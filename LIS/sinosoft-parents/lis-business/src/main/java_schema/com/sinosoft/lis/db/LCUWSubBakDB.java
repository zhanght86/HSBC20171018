...
/*
 * <p>ClassName: LCUWSubBakDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LCUWSubBakDB extends LCUWSubBakSchema
{
private static Logger logger = Logger.getLogger(LCUWSubBakDB.class);
	...
	**/
	...
	// @Constructor
	public LCUWSubBakDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCUWSubBakSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCUWSubBakDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getUWType() == null || this.getUWType().equals("null")) {
				...
				pstmt.setString(42, this.getUWType());
			}
			// set where condition
			if(this.getProposalNo() == null || this.getProposalNo().equals("null")) {
				pstmt.setNull(43, 12);
			} else {
				pstmt.setString(43, this.getProposalNo());
			}
			pstmt.setInt(44, this.getUWNo());
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
			// @@错误处理
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCUWSubBak WHERE  ProposalNo = ? AND UWNo = ?", 
	...
	public LCUWSubBakSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCUWSubBakSet aLCUWSubBakSet = new LCUWSubBakSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCUWSubBak");
			LCUWSubBakSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCUWSubBakSchema s1 = new LCUWSubBakSchema();
				s1.setSchema(rs,i);
				aLCUWSubBakSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCUWSubBakSet;

	}

	public LCUWSubBakSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCUWSubBakSet aLCUWSubBakSet = new LCUWSubBakSet();
	...
	public LCUWSubBakSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCUWSubBakSet aLCUWSubBakSet = new LCUWSubBakSet();
	...
	public LCUWSubBakSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCUWSubBakSet aLCUWSubBakSet = new LCUWSubBakSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCUWSubBak " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCUWSubBakSet
 */
public LCUWSubBakSet getData()
{
    int tCount = 0;
    LCUWSubBakSet tLCUWSubBakSet = new LCUWSubBakSet();
    LCUWSubBakSchema tLCUWSubBakSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCUWSubBakDB";
    ...
    {
        tCount = 1;
        tLCUWSubBakSchema = new LCUWSubBakSchema();
        tLCUWSubBakSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCUWSubBakSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCUWSubBakSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

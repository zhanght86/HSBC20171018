...
/*
 * <p>ClassName: LJAGetClaimDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务付费
 */
public class LJAGetClaimDB extends LJAGetClaimSchema
{
	private static Logger logger = Logger.getLogger(LJAGetClaimDB.class);
	// @Field
	...
	**/
	...
	// @Constructor
	public LJAGetClaimDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJAGetClaimSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJAGetClaimDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getEnterAccDate() == null || this.getEnterAccDate().equals("null")) {
				...
				pstmt.setDate(21, Date.valueOf(this.getEnterAccDate()));
			}
			if(this.getConfDate() == null || this.getConfDate().equals("null")) {
				pstmt.setNull(22, 91);
			} else {
				pstmt.setDate(22, Date.valueOf(this.getConfDate()));
			}
			...
			if(this.getAgentCom() == null || this.getAgentCom().equals("null")) {
				...
				pstmt.setString(25, this.getAgentCom());
			}
			if(this.getAgentType() == null || this.getAgentType().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getAgentType());
			}
			...
			if(this.getAccPolNo() == null || this.getAccPolNo().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
				pstmt.setString(37, this.getAccPolNo());
			}
			if(this.getOperState() == null || this.getOperState().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getOperState());
			}
			...
			pstmt.setDouble(42, this.getNetAmount());
			pstmt.setDouble(43, this.getTaxAmount());
			pstmt.setDouble(44, this.getTax());
			// set where condition
			if(this.getActuGetNo() == null || this.getActuGetNo().equals("null")) {
				pstmt.setNull(45, 12);
			} else {
				pstmt.setString(45, this.getActuGetNo());
			}
			if(this.getFeeOperationType() == null || this.getFeeOperationType().equals("null")) {
				pstmt.setNull(46, 12);
			} else {
				pstmt.setString(46, this.getFeeOperationType());
			}
			if(this.getSubFeeOperationType() == null || this.getSubFeeOperationType().equals("null")) {
				pstmt.setNull(47, 12);
			} else {
				pstmt.setString(47, this.getSubFeeOperationType());
			}
			if(this.getFeeFinaType() == null || this.getFeeFinaType().equals("null")) {
				pstmt.setNull(48, 12);
			} else {
				pstmt.setString(48, this.getFeeFinaType());
			}
			if(this.getDutyCode() == null || this.getDutyCode().equals("null")) {
				pstmt.setNull(49, 12);
			} else {
			...
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				...
				pstmt.setString(52, this.getPolNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJAGetClaim WHERE  ActuGetNo = ? AND FeeOperationType = ? AND SubFeeOperationType = ? AND FeeFinaType = ? AND DutyCode = ? AND GetDutyKind = ? AND GetDutyCode = ? AND PolNo = ?", 
	...
	public LJAGetClaimSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJAGetClaim");
			LJAGetClaimSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJAGetClaimSchema s1 = new LJAGetClaimSchema();
				s1.setSchema(rs,i);
				aLJAGetClaimSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJAGetClaimSet;
	}

	public LJAGetClaimSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJAGetClaimSet aLJAGetClaimSet = new LJAGetClaimSet();
	...
	public LJAGetClaimSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJAGetClaimSet aLJAGetClaimSet = new LJAGetClaimSet();
	...
	public LJAGetClaimSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJAGetClaimSet aLJAGetClaimSet = new LJAGetClaimSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJAGetClaim " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJAGetClaimSet
 */
public LJAGetClaimSet getData()
{
    int tCount = 0;
    LJAGetClaimSet tLJAGetClaimSet = new LJAGetClaimSet();
    LJAGetClaimSchema tLJAGetClaimSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJAGetClaimDB";
    ...
    try
    {
        ...
        tLJAGetClaimSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LJAGetClaimSet executeQuery(SQLwithBindVariables sqlbv) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public LJAGetClaimSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		logger.debug("ExecSQLwithBV SQL: " + sqlbv.originalSql());
		for(String key : sqlbv.variableKey())

...
/*
 * <p>ClassName: RIBatchLogDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保系统模型
 */
public class RIBatchLogDB extends RIBatchLogSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public RIBatchLogDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		RIBatchLogSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "RIBatchLogDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getBatchType() == null || this.getBatchType().equals("null")) {
				pstmt.setNull(2, 12);
			} else {
				pstmt.setString(2, this.getBatchType());
			}
			if(this.getRunState() == null || this.getRunState().equals("null")) {
				pstmt.setNull(3, 12);
			} else {
				pstmt.setString(3, this.getRunState());
			}
			if(this.getRunDate() == null || this.getRunDate().equals("null")) {
				pstmt.setNull(4, 91);
			} else {
				pstmt.setDate(4, Date.valueOf(this.getRunDate()));
			}
			...
			if(this.getBatchNo() == null || this.getBatchNo().equals("null")) {
				...
				pstmt.setString(14, this.getBatchNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM RIBatchLog WHERE  BatchNo = ?", 
	...
	public RIBatchLogSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIBatchLogSet aRIBatchLogSet = new RIBatchLogSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("RIBatchLog");
			RIBatchLogSchema aSchema = this.getSchema();
			...
			{
				i++;
				RIBatchLogSchema s1 = new RIBatchLogSchema();
				s1.setSchema(rs,i);
				aRIBatchLogSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aRIBatchLogSet;
	}

	public RIBatchLogSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIBatchLogSet aRIBatchLogSet = new RIBatchLogSet();
	...
	public RIBatchLogSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIBatchLogSet aRIBatchLogSet = new RIBatchLogSet();
	...
	public RIBatchLogSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RIBatchLogSet aRIBatchLogSet = new RIBatchLogSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update RIBatchLog " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return RIBatchLogSet
 */
public RIBatchLogSet getData()
{
    int tCount = 0;
    RIBatchLogSet tRIBatchLogSet = new RIBatchLogSet();
    RIBatchLogSchema tRIBatchLogSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "RIBatchLogDB";
    ...
    {
        tCount = 1;
        tRIBatchLogSchema = new RIBatchLogSchema();
        tRIBatchLogSchema.setSchema(mResultSet, 1);
        tRIBatchLogSet.add(tRIBatchLogSchema);

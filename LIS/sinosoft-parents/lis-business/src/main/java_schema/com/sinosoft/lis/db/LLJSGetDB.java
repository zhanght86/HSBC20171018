...
/*
 * <p>ClassName: LLJSGetDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LLJSGetDB extends LLJSGetSchema
{
private static Logger logger = Logger.getLogger(LLJSGetDB.class);
	...
	**/
	...
	// @Constructor
	public LLJSGetDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLJSGetSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getStartGetDate() == null || this.getStartGetDate().equals("null")) {
				...
				pstmt.setDate(11, Date.valueOf(this.getStartGetDate()));
			}
			...
			pstmt.setDouble(13, this.getSumGetMoney());
			...
			if(this.getApproveCode() == null || this.getApproveCode().equals("null")) {
				...
				pstmt.setString(16, this.getApproveCode());
			}
			if(this.getApproveDate() == null || this.getApproveDate().equals("null")) {
				pstmt.setNull(17, 91);
			} else {
				pstmt.setDate(17, Date.valueOf(this.getApproveDate()));
			}
			...
			if(this.getBankOnTheWayFlag() == null || this.getBankOnTheWayFlag().equals("null")) {
				...
				pstmt.setString(24, this.getBankOnTheWayFlag());
			}
			if(this.getBankSuccFlag() == null || this.getBankSuccFlag().equals("null")) {
				pstmt.setNull(25, 1);
			} else {
				pstmt.setString(25, this.getBankSuccFlag());
			}
			pstmt.setInt(26, this.getSendBankCount());
			...
			if(this.getGetNoticeNo() == null || this.getGetNoticeNo().equals("null")) {
				...
				pstmt.setString(32, this.getGetNoticeNo());
			}
			if(this.getCurrency() == null || this.getCurrency().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLJSGet WHERE  ClmNo = ? AND GetNoticeNo = ? AND Currency = ?", 
	...
	public LLJSGetSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLJSGetSet aLLJSGetSet = new LLJSGetSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLJSGet");
			LLJSGetSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLJSGetSchema s1 = new LLJSGetSchema();
				s1.setSchema(rs,i);
				aLLJSGetSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LLJSGetSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLJSGetSet aLLJSGetSet = new LLJSGetSet();
	...
	public LLJSGetSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLJSGetSet aLLJSGetSet = new LLJSGetSet();
	...
	public LLJSGetSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLJSGetSet aLLJSGetSet = new LLJSGetSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLJSGet " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LLJSGetSet
 */
public LLJSGetSet getData()
{
    int tCount = 0;
    LLJSGetSet tLLJSGetSet = new LLJSGetSet();
    LLJSGetSchema tLLJSGetSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLJSGetDB";
    ...
    {
        tCount = 1;
        tLLJSGetSchema = new LLJSGetSchema();
        tLLJSGetSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLJSGetSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLJSGetSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCCUWMasterBakDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LCCUWMasterBakDB extends LCCUWMasterBakSchema
{
private static Logger logger = Logger.getLogger(LCCUWMasterBakDB.class);
	...
	**/
	...
	// @Constructor
	public LCCUWMasterBakDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCCUWMasterBak" );
		mflag = true;
	}

	public LCCUWMasterBakDB()
	{
		con = null;
		db = new DBOper( "LCCUWMasterBak" );
		mflag = false;
	}
	...
	{
		LCCUWMasterBakSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCCUWMasterBakDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCCUWMasterBak WHERE  ContNo = ?");
		...
		SQLString sqlObj = new SQLString("LCCUWMasterBak");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCCUWMasterBak SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , UpReport = ? , CommonReason = ? , BatchNo = ? , CommonReasonCode = ? WHERE  ContNo = ?");
			...
			pstmt.setInt(41, this.getBatchNo());
			if(this.getCommonReasonCode() == null || this.getCommonReasonCode().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
				pstmt.setString(42, this.getCommonReasonCode());
			}
			// set where condition
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(43, 12);
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCCUWMasterBak(GrpContNo ,ContNo ,ProposalContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea ,UpReport ,CommonReason ,BatchNo ,CommonReasonCode) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCCUWMasterBak WHERE  ContNo = ?", 
	...
	public LCCUWMasterBakSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCCUWMasterBakSet aLCCUWMasterBakSet = new LCCUWMasterBakSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCCUWMasterBak");
			LCCUWMasterBakSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCCUWMasterBakSchema s1 = new LCCUWMasterBakSchema();
				s1.setSchema(rs,i);
				aLCCUWMasterBakSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCCUWMasterBakSet;

	}

	public LCCUWMasterBakSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCCUWMasterBakSet aLCCUWMasterBakSet = new LCCUWMasterBakSet();
	...
	public LCCUWMasterBakSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCCUWMasterBakSet aLCCUWMasterBakSet = new LCCUWMasterBakSet();
	...
	public LCCUWMasterBakSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCCUWMasterBakSet aLCCUWMasterBakSet = new LCCUWMasterBakSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCCUWMasterBak " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCCUWMasterBakSet
 */
public LCCUWMasterBakSet getData()
{
    int tCount = 0;
    LCCUWMasterBakSet tLCCUWMasterBakSet = new LCCUWMasterBakSet();
    LCCUWMasterBakSchema tLCCUWMasterBakSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCCUWMasterBakDB";
    ...
    {
        tCount = 1;
        tLCCUWMasterBakSchema = new LCCUWMasterBakSchema();
        tLCCUWMasterBakSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCCUWMasterBakSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCCUWMasterBakSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

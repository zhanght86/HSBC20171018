...
import com.sinosoft.lis.db.LPCUWErrorDB;

/*
 * <p>ClassName: LPCUWErrorSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPCUWErrorSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LPCUWErrorSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LPCUWErrorSchema cloned = (LPCUWErrorSchema)super.clone();
	...
	/**
	* 使用另外一个 LPCUWErrorSchema 对象给 Schema 赋值
	* @param: aLPCUWErrorSchema LPCUWErrorSchema
	**/
	public void setSchema(LPCUWErrorSchema aLPCUWErrorSchema)
	{
		this.EdorNo = aLPCUWErrorSchema.getEdorNo();
		this.EdorType = aLPCUWErrorSchema.getEdorType();
		this.GrpContNo = aLPCUWErrorSchema.getGrpContNo();
		this.ContNo = aLPCUWErrorSchema.getContNo();
		this.ProposalContNo = aLPCUWErrorSchema.getProposalContNo();
		this.SerialNo = aLPCUWErrorSchema.getSerialNo();
		this.UWNo = aLPCUWErrorSchema.getUWNo();
		this.InsuredNo = aLPCUWErrorSchema.getInsuredNo();
		this.InsuredName = aLPCUWErrorSchema.getInsuredName();
		this.AppntNo = aLPCUWErrorSchema.getAppntNo();
		this.AppntName = aLPCUWErrorSchema.getAppntName();
		this.ManageCom = aLPCUWErrorSchema.getManageCom();
		this.UWRuleCode = aLPCUWErrorSchema.getUWRuleCode();
		this.UWError = aLPCUWErrorSchema.getUWError();
		this.CurrValue = aLPCUWErrorSchema.getCurrValue();
		this.ModifyDate = fDate.getDate( aLPCUWErrorSchema.getModifyDate());
		this.ModifyTime = aLPCUWErrorSchema.getModifyTime();
		this.UWPassFlag = aLPCUWErrorSchema.getUWPassFlag();
		this.UWGrade = aLPCUWErrorSchema.getUWGrade();
		this.SugPassFlag = aLPCUWErrorSchema.getSugPassFlag();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPCUWError表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LPCUWErrorSchema";
	...
	public LPCUWErrorSchema getSchema()
	{
		LPCUWErrorSchema aLPCUWErrorSchema = new LPCUWErrorSchema();
		aLPCUWErrorSchema.setSchema(this);
		return aLPCUWErrorSchema;
	}

	public LPCUWErrorDB getDB()
	{
		LPCUWErrorDB aDBOper = new LPCUWErrorDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPCUWError描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLPCUWError>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LPCUWErrorSchema other = (LPCUWErrorSchema)otherObject;
		return

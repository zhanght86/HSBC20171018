...
/*
 * <p>ClassName: LBPODataDetailErrorSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LBPODataDetailErrorSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBPODataDetailErrorSchema.class);
	...
	// @Constructor
	public LBPODataDetailErrorSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBPODataDetailErrorSchema cloned = (LBPODataDetailErrorSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LBPODataDetailErrorSchema 对象给 Schema 赋值
	* @param: aLBPODataDetailErrorSchema LBPODataDetailErrorSchema
	**/
	public void setSchema(LBPODataDetailErrorSchema aLBPODataDetailErrorSchema)
	{
		this.BussNo = aLBPODataDetailErrorSchema.getBussNo();
		this.BussNoType = aLBPODataDetailErrorSchema.getBussNoType();
		this.ErrorCount = aLBPODataDetailErrorSchema.getErrorCount();
		this.SerialNo = aLBPODataDetailErrorSchema.getSerialNo();
		this.ErrorTag = aLBPODataDetailErrorSchema.getErrorTag();
		this.ErrorCode = aLBPODataDetailErrorSchema.getErrorCode();
		this.ErrorContent = aLBPODataDetailErrorSchema.getErrorContent();
		this.Operator = aLBPODataDetailErrorSchema.getOperator();
		this.MakeDate = fDate.getDate( aLBPODataDetailErrorSchema.getMakeDate());
		this.MakeTime = aLBPODataDetailErrorSchema.getMakeTime();
		this.ErrorFlag = aLBPODataDetailErrorSchema.getErrorFlag();
		this.Content = aLBPODataDetailErrorSchema.getContent();
		this.TableName = aLBPODataDetailErrorSchema.getTableName();
		this.OtherNo = aLBPODataDetailErrorSchema.getOtherNo();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBPODataDetailError表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBPODataDetailErrorSchema";
	...
	public LBPODataDetailErrorSchema getSchema()
	{
		LBPODataDetailErrorSchema aLBPODataDetailErrorSchema = new LBPODataDetailErrorSchema();
		aLBPODataDetailErrorSchema.setSchema(this);
		return aLBPODataDetailErrorSchema;
	}

	public LBPODataDetailErrorDB getDB()
	{
		LBPODataDetailErrorDB aDBOper = new LBPODataDetailErrorDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBPODataDetailError>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ErrorFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			Content = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			TableName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			OtherNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBPODataDetailErrorSchema other = (LBPODataDetailErrorSchema)otherObject;
		return

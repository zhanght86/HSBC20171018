...
/*
 * <p>ClassName: LLBExemptDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLBExemptDB extends LLBExemptSchema
{
private static Logger logger = Logger.getLogger(LLBExemptDB.class);
	...
	**/
	...
	// @Constructor
	public LLBExemptDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LLBExempt" );
		mflag = true;
	}
	...
	{
		LLBExemptSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LLBExempt WHERE  BackNo = ? AND ClmNo = ? AND PolNo = ? AND DutyCode = ? AND PayPlanCode = ?");
		...
		SQLString sqlObj = new SQLString("LLBExempt");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LLBExempt SET  BackNo = ? , ClmNo = ? , GrpContNo = ? , ContNo = ? , PolNo = ? , DutyCode = ? , PayPlanCode = ? , PayPlanType = ? , AppntType = ? , AppntNo = ? , UrgePayFlag = ? , NeedAcc = ? , PayTimes = ? , Rate = ? , PayStartDate = ? , PayEndDate = ? , PaytoDate = ? , PayIntv = ? , StandPrem = ? , Prem = ? , SumPrem = ? , SuppRiskScore = ? , FreeFlag = ? , FreeRate = ? , FreeStartDate = ? , FreeEndDate = ? , State = ? , ManageCom = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , AddFeeDirect = ? , ExemptPeriod = ? , NBPolNo = ? , ExemptReason = ? , ExemptDesc = ? WHERE  BackNo = ? AND ClmNo = ? AND PolNo = ? AND DutyCode = ? AND PayPlanCode = ?");
			if(this.getBackNo() == null || this.getBackNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getBackNo());
			...
			if(this.getExemptPeriod() == null || this.getExemptPeriod().equals("null")) {
				...
				pstmt.setString(35, this.getExemptPeriod());
			}
			...
			if(this.getExemptReason() == null || this.getExemptReason().equals("null")) {
				...
				pstmt.setString(37, this.getExemptReason());
			}
			if(this.getExemptDesc() == null || this.getExemptDesc().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getExemptDesc());
			}
			// set where condition
			if(this.getBackNo() == null || this.getBackNo().equals("null")) {
				pstmt.setNull(39, 12);
			} else {
				pstmt.setString(39, this.getBackNo());
			}
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(40, 12);
			} else {
			...
			if(this.getPayPlanCode() == null || this.getPayPlanCode().equals("null")) {
				...
				pstmt.setString(43, this.getPayPlanCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LLBExempt(BackNo ,ClmNo ,GrpContNo ,ContNo ,PolNo ,DutyCode ,PayPlanCode ,PayPlanType ,AppntType ,AppntNo ,UrgePayFlag ,NeedAcc ,PayTimes ,Rate ,PayStartDate ,PayEndDate ,PaytoDate ,PayIntv ,StandPrem ,Prem ,SumPrem ,SuppRiskScore ,FreeFlag ,FreeRate ,FreeStartDate ,FreeEndDate ,State ,ManageCom ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,AddFeeDirect ,ExemptPeriod ,NBPolNo ,ExemptReason ,ExemptDesc) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getBackNo() == null || this.getBackNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getBackNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLBExempt WHERE  BackNo = ? AND ClmNo = ? AND PolNo = ? AND DutyCode = ? AND PayPlanCode = ?", 
	...
	public LLBExemptSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLBExemptSet aLLBExemptSet = new LLBExemptSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLBExempt");
			LLBExemptSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLBExemptSchema s1 = new LLBExemptSchema();
				s1.setSchema(rs,i);
				aLLBExemptSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLBExemptSet;

	}

	public LLBExemptSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLBExemptSet aLLBExemptSet = new LLBExemptSet();
	...
	public LLBExemptSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLBExemptSet aLLBExemptSet = new LLBExemptSet();
	...
	public LLBExemptSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLBExemptSet aLLBExemptSet = new LLBExemptSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLBExempt " + sqlObj.getUpdPart() + " where " + strWherePart;
...
public LLBExemptSet getData()
{
    ...
    try
    {
        ...
        tLLBExemptSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLBExemptSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLBExemptSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

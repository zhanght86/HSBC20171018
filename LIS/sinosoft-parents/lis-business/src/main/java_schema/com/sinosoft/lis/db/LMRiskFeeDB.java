...
/*
 * <p>ClassName: LMRiskFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 管理费描述
 */
public class LMRiskFeeDB extends LMRiskFeeSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LMRiskFeeDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LMRiskFee" );
		mflag = true;
	}
	...
	{
		LMRiskFeeSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LMRiskFee WHERE  FeeCode = ? AND InsuAccNo = ? AND PayPlanCode = ? AND FeeCalMode = ?");
		...
		SQLString sqlObj = new SQLString("LMRiskFee");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getFeeStartDate() == null || this.getFeeStartDate().equals("null")) {
				...
				pstmt.setDate(23, Date.valueOf(this.getFeeStartDate()));
			}
			pstmt.setInt(24, this.getFeeNum());
			...
			if(this.getInterfaceClassName() == null || this.getInterfaceClassName().equals("null")) {
				...
				pstmt.setString(26, this.getInterfaceClassName());
			}
			...
			if(this.getPeriodType() == null || this.getPeriodType().equals("null")) {
				...
				pstmt.setString(28, this.getPeriodType());
			}
			...
			if(this.getFeeCalMode() == null || this.getFeeCalMode().equals("null")) {
				...
				pstmt.setString(33, this.getFeeCalMode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LMRiskFee(FeeCode ,FeeName ,FeeNoti ,InsuAccNo ,PayPlanCode ,PayInsuAccName ,FeeKind ,FeeItemType ,FeeTakePlace ,FeeCalMode ,FeeCalModeType ,FeeCalCode ,FeeValue ,CompareValue ,FeePeriod ,MaxTime ,DefaultFlag ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,FeeStartDate ,FeeNum ,FeeBaseType ,InterfaceClassName ,FeeType ,PeriodType ,RiskCode) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getFeeCode() == null || this.getFeeCode().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getFeeCode());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LMRiskFee WHERE  FeeCode = ? AND InsuAccNo = ? AND PayPlanCode = ? AND FeeCalMode = ?", 
	...
	public LMRiskFeeSet query()
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("LMRiskFee");
			LMRiskFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LMRiskFeeSchema s1 = new LMRiskFeeSchema();
				s1.setSchema(rs,i);
				aLMRiskFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aLMRiskFeeSet;
	}

	public LMRiskFeeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LMRiskFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMRiskFeeSet aLMRiskFeeSet = new LMRiskFeeSet();
	...
	public LMRiskFeeSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMRiskFeeSet aLMRiskFeeSet = new LMRiskFeeSet();
	...
	public LMRiskFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMRiskFeeSet aLMRiskFeeSet = new LMRiskFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LMRiskFee " + sqlObj.getUpdPart() + " where " + strWherePart;
...
public LMRiskFeeSet getData()
{
    ...
    try
    {
        ...
        tLMRiskFeeSchema.setSchema(mResultSet, 1);
        tLMRiskFeeSet.add(tLMRiskFeeSchema);

 ...
 * @Database: 泰康未整理PDM
 */
public class LLNoticeRelaSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLNoticeRelaSchema.class);
	// @Field
	/** 分报案号(事件号) */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLNoticeRelaSchema cloned = (LLNoticeRelaSchema)super.clone();
	...
	* @param: aLLNoticeRelaSchema LLNoticeRelaSchema
	**/
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLNoticeRela表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(NoticeNo));
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			NoticeNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 2, SysConst.PACKAGESPILTER );
		}

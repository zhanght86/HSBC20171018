 ...
 * @Database: 寿险业务系统缺失模型
 */
public class LGWorkTypeSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LGWorkTypeSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LGWorkTypeSchema cloned = (LGWorkTypeSchema)super.clone();
	...
	* @param: aLGWorkTypeSchema LGWorkTypeSchema
	**/
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LGWorkType表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(WorkTypeName)); strReturn.append(SysConst.PACKAGESPILTER);
		strReturn.append(StrTool.cTrim(SuperTypeNo)); strReturn.append(SysConst.PACKAGESPILTER);
		strReturn.append(StrTool.cTrim(IsDeal)); strReturn.append(SysConst.PACKAGESPILTER);
		strReturn.append(StrTool.cTrim(DealPath)); strReturn.append(SysConst.PACKAGESPILTER);
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			WorkTypeNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 1, SysConst.PACKAGESPILTER );
			WorkTypeName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 2, SysConst.PACKAGESPILTER );
			SuperTypeNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			IsDeal = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			DealPath = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			DateLimit = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("WorkTypeName"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(WorkTypeName));
		}
		if (FCode.equalsIgnoreCase("SuperTypeNo"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(SuperTypeNo));
		}
		if (FCode.equalsIgnoreCase("IsDeal"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(IsDeal));
		}
		if (FCode.equalsIgnoreCase("DealPath"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(DealPath));
		}

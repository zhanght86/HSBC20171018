...
/*
 * <p>ClassName: LCGCUWErrorDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCGCUWErrorDB extends LCGCUWErrorSchema
{
private static Logger logger = Logger.getLogger(LCGCUWErrorDB.class);
	...
	**/
	...
	// @Constructor
	public LCGCUWErrorDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCGCUWErrorSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCGCUWErrorDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getGrpNo() == null || this.getGrpNo().equals("null")) {
				...
				pstmt.setString(5, this.getGrpNo());
			}
			...
			if(this.getProposalGrpContNo() == null || this.getProposalGrpContNo().equals("null")) {
				...
				pstmt.setString(19, this.getProposalGrpContNo());
			}
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(20, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCGCUWError WHERE  ProposalGrpContNo = ? AND SerialNo = ? AND UWNo = ?", 
	...
	public LCGCUWErrorSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCGCUWErrorSet aLCGCUWErrorSet = new LCGCUWErrorSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCGCUWError");
			LCGCUWErrorSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCGCUWErrorSchema s1 = new LCGCUWErrorSchema();
				s1.setSchema(rs,i);
				aLCGCUWErrorSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCGCUWErrorSet;

	}

	public LCGCUWErrorSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGCUWErrorSet aLCGCUWErrorSet = new LCGCUWErrorSet();
	...
	public LCGCUWErrorSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCGCUWErrorSet aLCGCUWErrorSet = new LCGCUWErrorSet();
	...
	public LCGCUWErrorSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGCUWErrorSet aLCGCUWErrorSet = new LCGCUWErrorSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCGCUWError " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCGCUWErrorSet
 */
public LCGCUWErrorSet getData()
{
    int tCount = 0;
    LCGCUWErrorSet tLCGCUWErrorSet = new LCGCUWErrorSet();
    LCGCUWErrorSchema tLCGCUWErrorSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCGCUWErrorDB";
    ...
    {
        tCount = 1;
        tLCGCUWErrorSchema = new LCGCUWErrorSchema();
        tLCGCUWErrorSchema.setSchema(mResultSet, 1);
        tLCGCUWErrorSet.add(tLCGCUWErrorSchema);
...
public boolean closeData()
{
	...
	public LCGCUWErrorSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCGCUWErrorSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

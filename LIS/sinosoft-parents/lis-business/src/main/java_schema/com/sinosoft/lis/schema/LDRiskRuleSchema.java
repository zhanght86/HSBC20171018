...
/*
 * <p>ClassName: LDRiskRuleSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LDRiskRuleSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LDRiskRuleSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LDRiskRuleSchema cloned = (LDRiskRuleSchema)super.clone();
	...
	/**
	* 使用另外一个 LDRiskRuleSchema 对象给 Schema 赋值
	* @param: aLDRiskRuleSchema LDRiskRuleSchema
	**/
	public void setSchema(LDRiskRuleSchema aLDRiskRuleSchema)
	{
		this.RiskCode = aLDRiskRuleSchema.getRiskCode();
		this.SaleChnl = aLDRiskRuleSchema.getSaleChnl();
		this.ComGroup = aLDRiskRuleSchema.getComGroup();
		this.StartPolApplyDate = fDate.getDate( aLDRiskRuleSchema.getStartPolApplyDate());
		this.EndPolApplyDate = fDate.getDate( aLDRiskRuleSchema.getEndPolApplyDate());
		this.StartScanDate = fDate.getDate( aLDRiskRuleSchema.getStartScanDate());
		this.StartScanTime = aLDRiskRuleSchema.getStartScanTime();
		this.EndScanDate = fDate.getDate( aLDRiskRuleSchema.getEndScanDate());
		this.EndScanTime = aLDRiskRuleSchema.getEndScanTime();
		this.Operator = aLDRiskRuleSchema.getOperator();
		this.MakeDate = fDate.getDate( aLDRiskRuleSchema.getMakeDate());
		this.MakeTime = aLDRiskRuleSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLDRiskRuleSchema.getModifyDate());
		this.ModifyTime = aLDRiskRuleSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LDRiskRule表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LDRiskRuleSchema";
	...
	public LDRiskRuleSchema getSchema()
	{
		LDRiskRuleSchema aLDRiskRuleSchema = new LDRiskRuleSchema();
		aLDRiskRuleSchema.setSchema(this);
		return aLDRiskRuleSchema;
	}

	public LDRiskRuleDB getDB()
	{
		LDRiskRuleDB aDBOper = new LDRiskRuleDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDRiskRule描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDRiskRule>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LDRiskRuleSchema other = (LDRiskRuleSchema)otherObject;
		return

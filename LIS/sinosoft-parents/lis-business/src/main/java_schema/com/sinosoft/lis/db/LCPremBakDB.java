...
/*
 * <p>ClassName: LCPremBakDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LCPremBakDB extends LCPremBakSchema
{
private static Logger logger = Logger.getLogger(LCPremBakDB.class);
	...
	**/
	...
	// @Constructor
	public LCPremBakDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCPremBak" );
		mflag = true;
	}
	...
	{
		LCPremBakSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCPremBak WHERE  PolNo = ? AND DutyCode = ? AND PayPlanCode = ?");
		...
		SQLString sqlObj = new SQLString("LCPremBak");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCPremBak SET  GrpContNo = ? , ContNo = ? , PolNo = ? , DutyCode = ? , PayPlanCode = ? , PayPlanType = ? , AppntType = ? , AppntNo = ? , UrgePayFlag = ? , NeedAcc = ? , PayTimes = ? , Rate = ? , PayStartDate = ? , PayEndDate = ? , PaytoDate = ? , PayIntv = ? , StandPrem = ? , Prem = ? , SumPrem = ? , SuppRiskScore = ? , FreeFlag = ? , FreeRate = ? , FreeStartDate = ? , FreeEndDate = ? , State = ? , ManageCom = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , AddFeeDirect = ? , SecInsuAddPoint = ? WHERE  PolNo = ? AND DutyCode = ? AND PayPlanCode = ?");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCPremBak(GrpContNo ,ContNo ,PolNo ,DutyCode ,PayPlanCode ,PayPlanType ,AppntType ,AppntNo ,UrgePayFlag ,NeedAcc ,PayTimes ,Rate ,PayStartDate ,PayEndDate ,PaytoDate ,PayIntv ,StandPrem ,Prem ,SumPrem ,SuppRiskScore ,FreeFlag ,FreeRate ,FreeStartDate ,FreeEndDate ,State ,ManageCom ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,AddFeeDirect ,SecInsuAddPoint) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPremBak WHERE  PolNo = ? AND DutyCode = ? AND PayPlanCode = ?", 
	...
	public LCPremBakSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPremBakSet aLCPremBakSet = new LCPremBakSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPremBak");
			LCPremBakSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCPremBakSchema s1 = new LCPremBakSchema();
				s1.setSchema(rs,i);
				aLCPremBakSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCPremBakSet;

	}

	public LCPremBakSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPremBakSet aLCPremBakSet = new LCPremBakSet();
	...
	public LCPremBakSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPremBakSet aLCPremBakSet = new LCPremBakSet();
	...
	public LCPremBakSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPremBakSet aLCPremBakSet = new LCPremBakSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPremBak " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LCPremBakSet
 */
public LCPremBakSet getData()
{
    int tCount = 0;
    LCPremBakSet tLCPremBakSet = new LCPremBakSet();
    LCPremBakSchema tLCPremBakSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCPremBakDB";
    ...
    {
        tCount = 1;
        tLCPremBakSchema = new LCPremBakSchema();
        tLCPremBakSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCPremBakSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPremBakSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

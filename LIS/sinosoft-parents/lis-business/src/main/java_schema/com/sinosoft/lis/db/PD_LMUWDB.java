...
/*
 * <p>ClassName: PD_LMUWDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 产品定义平台_PDM
 */
public class PD_LMUWDB extends PD_LMUWSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public PD_LMUWDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		PD_LMUWSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getUWCode() == null || this.getUWCode().equals("null")) {
				...
				pstmt.setString(1, this.getUWCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRelaPolType() == null || this.getRelaPolType().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getRelaPolType());
			}
			...
			pstmt.setInt(8, this.getUWOrder());
			if(this.getOthCalCode() == null || this.getOthCalCode().equals("null")) {
				pstmt.setNull(9, 12);
			} else {
				pstmt.setString(9, this.getOthCalCode());
			}
			...
			if(this.getUWResult() == null || this.getUWResult().equals("null")) {
				...
				pstmt.setString(12, this.getUWResult());
			}
			...
			if(this.getUWCode() == null || this.getUWCode().equals("null")) {
				...
				pstmt.setString(25, this.getUWCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM PD_LMUW WHERE  UWCode = ?", 
	...
	public PD_LMUWSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMUWSet aPD_LMUWSet = new PD_LMUWSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("PD_LMUW");
			PD_LMUWSchema aSchema = this.getSchema();
			...
			{
				i++;
				PD_LMUWSchema s1 = new PD_LMUWSchema();
				s1.setSchema(rs,i);
				aPD_LMUWSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public PD_LMUWSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMUWSet aPD_LMUWSet = new PD_LMUWSet();
	...
	public PD_LMUWSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMUWSet aPD_LMUWSet = new PD_LMUWSet();
	...
	public PD_LMUWSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMUWSet aPD_LMUWSet = new PD_LMUWSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update PD_LMUW " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return PD_LMUWSet
 */
public PD_LMUWSet getData()
{
    int tCount = 0;
    PD_LMUWSet tPD_LMUWSet = new PD_LMUWSet();
    PD_LMUWSchema tPD_LMUWSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "PD_LMUWDB";
    ...
    {
        tCount = 1;
        tPD_LMUWSchema = new PD_LMUWSchema();
        tPD_LMUWSchema.setSchema(mResultSet, 1);
        tPD_LMUWSet.add(tPD_LMUWSchema);

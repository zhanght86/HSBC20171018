...
/*
 * <p>ClassName: LBPOBnfSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 内部外包流程
 */
public class LBPOBnfSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBPOBnfSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBPOBnfSchema cloned = (LBPOBnfSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LBPOBnfSchema 对象给 Schema 赋值
	* @param: aLBPOBnfSchema LBPOBnfSchema
	**/
	public void setSchema(LBPOBnfSchema aLBPOBnfSchema)
	{
		this.ContNo = aLBPOBnfSchema.getContNo();
		this.InputNo = aLBPOBnfSchema.getInputNo();
		this.FillNo = aLBPOBnfSchema.getFillNo();
		this.PolNo = aLBPOBnfSchema.getPolNo();
		this.InsuredNo = aLBPOBnfSchema.getInsuredNo();
		this.BnfType = aLBPOBnfSchema.getBnfType();
		this.BnfNo = aLBPOBnfSchema.getBnfNo();
		this.BnfGrade = aLBPOBnfSchema.getBnfGrade();
		this.RelationToInsured = aLBPOBnfSchema.getRelationToInsured();
		this.BnfLot = aLBPOBnfSchema.getBnfLot();
		this.CustomerNo = aLBPOBnfSchema.getCustomerNo();
		this.Name = aLBPOBnfSchema.getName();
		this.Sex = aLBPOBnfSchema.getSex();
		this.Birthday = aLBPOBnfSchema.getBirthday();
		this.IDType = aLBPOBnfSchema.getIDType();
		this.IDNo = aLBPOBnfSchema.getIDNo();
		this.Operator = aLBPOBnfSchema.getOperator();
		this.MakeDate = aLBPOBnfSchema.getMakeDate();
		this.MakeTime = aLBPOBnfSchema.getMakeTime();
		this.ModifyDate = aLBPOBnfSchema.getModifyDate();
		this.ModifyTime = aLBPOBnfSchema.getModifyTime();
		this.BnfAddress = aLBPOBnfSchema.getBnfAddress();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBPOBnf表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBPOBnfSchema";
	...
	public LBPOBnfSchema getSchema()
	{
		LBPOBnfSchema aLBPOBnfSchema = new LBPOBnfSchema();
		aLBPOBnfSchema.setSchema(this);
		return aLBPOBnfSchema;
	}

	public LBPOBnfDB getDB()
	{
		LBPOBnfDB aDBOper = new LBPOBnfDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBPOBnf描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(BnfAddress));
		return strReturn.toString();
	}

	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBPOBnf>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			BnfLot = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			...
			Birthday = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			...
			BnfAddress = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBPOBnfSchema other = (LBPOBnfSchema)otherObject;
		return

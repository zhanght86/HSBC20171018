...
import com.sinosoft.lis.db.LBAccountDB;

/*
 * <p>ClassName: LBAccountSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 泰康未整理PDM
 */
public class LBAccountSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBAccountSchema.class);
	// @Field
	/** 批单号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBAccountSchema cloned = (LBAccountSchema)super.clone();
	...
	*/
	...
	public void setAccKind(String aAccKind)
	{
		if(aAccKind!=null && aAccKind.length()>2)
			throw new IllegalArgumentException("是否默认账户AccKind值"+aAccKind+"的长度"+aAccKind.length()+"大于最大值2");
		AccKind = aAccKind;
	}
	...
	/**
	* 使用另外一个 LBAccountSchema 对象给 Schema 赋值
	* @param: aLBAccountSchema LBAccountSchema
	**/
	public void setSchema(LBAccountSchema aLBAccountSchema)
	{
		this.EdorNo = aLBAccountSchema.getEdorNo();
		this.CustomerNo = aLBAccountSchema.getCustomerNo();
		this.AccKind = aLBAccountSchema.getAccKind();
		this.BankCode = aLBAccountSchema.getBankCode();
		this.BankAccNo = aLBAccountSchema.getBankAccNo();
		this.AccName = aLBAccountSchema.getAccName();
		this.Operator = aLBAccountSchema.getOperator();
		this.MakeDate = fDate.getDate( aLBAccountSchema.getMakeDate());
		this.MakeTime = aLBAccountSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLBAccountSchema.getModifyDate());
		this.ModifyTime = aLBAccountSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBAccount表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBAccountSchema";
	...
	public LBAccountSchema getSchema()
	{
		LBAccountSchema aLBAccountSchema = new LBAccountSchema();
		aLBAccountSchema.setSchema(this);
		return aLBAccountSchema;
	}

	public LBAccountDB getDB()
	{
		LBAccountDB aDBOper = new LBAccountDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBAccount描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBAccount>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBAccountSchema other = (LBAccountSchema)otherObject;
		return

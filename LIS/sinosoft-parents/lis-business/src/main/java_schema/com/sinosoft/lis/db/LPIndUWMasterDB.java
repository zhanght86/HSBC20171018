...
/*
 * <p>ClassName: LPIndUWMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPIndUWMasterDB extends LPIndUWMasterSchema
{
private static Logger logger = Logger.getLogger(LPIndUWMasterDB.class);
	...
	**/
	...
	// @Constructor
	public LPIndUWMasterDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LPIndUWMaster" );
		mflag = true;
	}

	public LPIndUWMasterDB()
	{
		con = null;
		db = new DBOper( "LPIndUWMaster" );
		mflag = false;
	}
	...
	{
		LPIndUWMasterSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPIndUWMasterDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LPIndUWMaster WHERE  ContNo = ? AND InsuredNo = ? AND EdorNo = ? AND EdorType = ?");
		...
		SQLString sqlObj = new SQLString("LPIndUWMaster");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LPIndUWMaster SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , EdorNo = ? , EdorType = ? WHERE  ContNo = ? AND InsuredNo = ? AND EdorNo = ? AND EdorType = ?");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LPIndUWMaster(GrpContNo ,ContNo ,ProposalContNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea ,EdorNo ,EdorType) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPIndUWMaster WHERE  ContNo = ? AND InsuredNo = ? AND EdorNo = ? AND EdorType = ?", 
	...
	public LPIndUWMasterSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPIndUWMasterSet aLPIndUWMasterSet = new LPIndUWMasterSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPIndUWMaster");
			LPIndUWMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPIndUWMasterSchema s1 = new LPIndUWMasterSchema();
				s1.setSchema(rs,i);
				aLPIndUWMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPIndUWMasterSet;

	}

	public LPIndUWMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPIndUWMasterSet aLPIndUWMasterSet = new LPIndUWMasterSet();
	...
	public LPIndUWMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPIndUWMasterSet aLPIndUWMasterSet = new LPIndUWMasterSet();
	...
	public LPIndUWMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPIndUWMasterSet aLPIndUWMasterSet = new LPIndUWMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPIndUWMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPIndUWMasterSet
 */
public LPIndUWMasterSet getData()
{
    int tCount = 0;
    LPIndUWMasterSet tLPIndUWMasterSet = new LPIndUWMasterSet();
    LPIndUWMasterSchema tLPIndUWMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPIndUWMasterDB";
    ...
    {
        tCount = 1;
        tLPIndUWMasterSchema = new LPIndUWMasterSchema();
        tLPIndUWMasterSchema.setSchema(mResultSet, 1);
        tLPIndUWMasterSet.add(tLPIndUWMasterSchema);
...
public boolean closeData()
{
	...
	public LPIndUWMasterSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPIndUWMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: RnewScoreRReportSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 续期续保二核
 */
public class RnewScoreRReportSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(RnewScoreRReportSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		RnewScoreRReportSchema cloned = (RnewScoreRReportSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 RnewScoreRReportSchema 对象给 Schema 赋值
	* @param: aRnewScoreRReportSchema RnewScoreRReportSchema
	**/
	public void setSchema(RnewScoreRReportSchema aRnewScoreRReportSchema)
	{
		this.GrpContNo = aRnewScoreRReportSchema.getGrpContNo();
		this.ContNo = aRnewScoreRReportSchema.getContNo();
		this.ProposalContNo = aRnewScoreRReportSchema.getProposalContNo();
		this.CustomerNo = aRnewScoreRReportSchema.getCustomerNo();
		this.Name = aRnewScoreRReportSchema.getName();
		this.ManageCom = aRnewScoreRReportSchema.getManageCom();
		this.SScore = aRnewScoreRReportSchema.getSScore();
		this.AScore = aRnewScoreRReportSchema.getAScore();
		this.Score = aRnewScoreRReportSchema.getScore();
		this.Conclusion = aRnewScoreRReportSchema.getConclusion();
		this.AssessOperator = aRnewScoreRReportSchema.getAssessOperator();
		this.AssessDay = fDate.getDate( aRnewScoreRReportSchema.getAssessDay());
		this.MakeDate = fDate.getDate( aRnewScoreRReportSchema.getMakeDate());
		this.MakeTime = aRnewScoreRReportSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aRnewScoreRReportSchema.getModifyDate());
		this.ModifyTime = aRnewScoreRReportSchema.getModifyTime();
		this.Operator = aRnewScoreRReportSchema.getOperator();
		this.AssessTime = aRnewScoreRReportSchema.getAssessTime();
		this.PrtSeq = aRnewScoreRReportSchema.getPrtSeq();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的RnewScoreRReport表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "RnewScoreRReportSchema";
	...
	public RnewScoreRReportSchema getSchema()
	{
		RnewScoreRReportSchema aRnewScoreRReportSchema = new RnewScoreRReportSchema();
		aRnewScoreRReportSchema.setSchema(this);
		return aRnewScoreRReportSchema;
	}

	public RnewScoreRReportDB getDB()
	{
		RnewScoreRReportDB aDBOper = new RnewScoreRReportDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpRnewScoreRReport>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		RnewScoreRReportSchema other = (RnewScoreRReportSchema)otherObject;
		return

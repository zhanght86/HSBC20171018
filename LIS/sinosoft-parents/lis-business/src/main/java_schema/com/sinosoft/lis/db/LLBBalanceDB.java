...
/*
 * <p>ClassName: LLBBalanceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LLBBalanceDB extends LLBBalanceSchema
{
private static Logger logger = Logger.getLogger(LLBBalanceDB.class);
	...
	**/
	...
	// @Constructor
	public LLBBalanceDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLBBalanceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLBBalanceDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getSubFeeOperationType() == null || this.getSubFeeOperationType().equals("null")) {
				...
				pstmt.setString(4, this.getSubFeeOperationType());
			}
			if(this.getFeeFinaType() == null || this.getFeeFinaType().equals("null")) {
				pstmt.setNull(5, 12);
			} else {
				pstmt.setString(5, this.getFeeFinaType());
			}
			if(this.getBatNo() == null || this.getBatNo().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getBatNo());
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOtherNoType() == null || this.getOtherNoType().equals("null")) {
				...
				pstmt.setString(8, this.getOtherNoType());
			}
			...
			if(this.getGetDutyCode() == null || this.getGetDutyCode().equals("null")) {
				...
				pstmt.setString(16, this.getGetDutyCode());
			}
			...
			if(this.getGetDate() == null || this.getGetDate().equals("null")) {
				...
				pstmt.setDate(22, Date.valueOf(this.getGetDate()));
			}
			pstmt.setDouble(23, this.getPay());
			if(this.getPayFlag() == null || this.getPayFlag().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getPayFlag());
			}
			...
			if(this.getDealFlag() == null || this.getDealFlag().equals("null")) {
				...
				pstmt.setString(26, this.getDealFlag());
			}
			...
			if(this.getAgentCom() == null || this.getAgentCom().equals("null")) {
				...
				pstmt.setString(28, this.getAgentCom());
			}
			if(this.getAgentType() == null || this.getAgentType().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getAgentType());
			}
			...
			if(this.getBackNo() == null || this.getBackNo().equals("null")) {
				...
				pstmt.setString(41, this.getBackNo());
			}
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
			...
			if(this.getFeeOperationType() == null || this.getFeeOperationType().equals("null")) {
				...
				pstmt.setString(43, this.getFeeOperationType());
			}
			if(this.getSubFeeOperationType() == null || this.getSubFeeOperationType().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
				pstmt.setString(44, this.getSubFeeOperationType());
			}
			if(this.getFeeFinaType() == null || this.getFeeFinaType().equals("null")) {
				pstmt.setNull(45, 12);
			} else {
				pstmt.setString(45, this.getFeeFinaType());
			}
			if(this.getBatNo() == null || this.getBatNo().equals("null")) {
				pstmt.setNull(46, 12);
			} else {
				pstmt.setString(46, this.getBatNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(47, 12);
			} else {
			...
			if(this.getGetDutyKind() == null || this.getGetDutyKind().equals("null")) {
				...
				pstmt.setString(49, this.getGetDutyKind());
			}
			if(this.getGetDutyCode() == null || this.getGetDutyCode().equals("null")) {
				pstmt.setNull(50, 12);
			} else {
				pstmt.setString(50, this.getGetDutyCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLBBalance WHERE  BackNo = ? AND ClmNo = ? AND FeeOperationType = ? AND SubFeeOperationType = ? AND FeeFinaType = ? AND BatNo = ? AND PolNo = ? AND DutyCode = ? AND GetDutyKind = ? AND GetDutyCode = ?", 
	...
	public LLBBalanceSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLBBalanceSet aLLBBalanceSet = new LLBBalanceSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLBBalance");
			LLBBalanceSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLBBalanceSchema s1 = new LLBBalanceSchema();
				s1.setSchema(rs,i);
				aLLBBalanceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLBBalanceSet;

	}

	public LLBBalanceSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLBBalanceSet aLLBBalanceSet = new LLBBalanceSet();
	...
	public LLBBalanceSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLBBalanceSet aLLBBalanceSet = new LLBBalanceSet();
	...
	public LLBBalanceSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLBBalanceSet aLLBBalanceSet = new LLBBalanceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLBBalance " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLBBalanceSet
 */
public LLBBalanceSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLLBBalanceSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLBBalanceSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLBBalanceSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

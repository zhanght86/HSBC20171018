...
/*
 * <p>ClassName: LMEdorZT1DB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 新险种定义
 */
public class LMEdorZT1DB extends LMEdorZT1Schema
{
private static Logger logger = Logger.getLogger(LMEdorZT1DB.class);
	...
	**/
	...
	// @Constructor
	public LMEdorZT1DB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LMEdorZT1Schema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPayPlanCode() == null || this.getPayPlanCode().equals("null")) {
				...
				pstmt.setString(13, this.getPayPlanCode());
			}
			if(this.getSurrCalType() == null || this.getSurrCalType().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getSurrCalType());
			}
			if(this.getCycPayIntvType() == null || this.getCycPayIntvType().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getCycPayIntvType());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LMEdorZT1 WHERE  RiskCode = ? AND DutyCode = ? AND PayPlanCode = ? AND SurrCalType = ? AND CycPayIntvType = ?", 
	...
	public LMEdorZT1Set query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LMEdorZT1Set aLMEdorZT1Set = new LMEdorZT1Set();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LMEdorZT1");
			LMEdorZT1Schema aSchema = this.getSchema();
			...
			{
				i++;
				LMEdorZT1Schema s1 = new LMEdorZT1Schema();
				s1.setSchema(rs,i);
				aLMEdorZT1Set.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLMEdorZT1Set;

	}

	public LMEdorZT1Set executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMEdorZT1Set aLMEdorZT1Set = new LMEdorZT1Set();
	...
	public LMEdorZT1Set query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LMEdorZT1Set aLMEdorZT1Set = new LMEdorZT1Set();
	...
	public LMEdorZT1Set executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMEdorZT1Set aLMEdorZT1Set = new LMEdorZT1Set();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LMEdorZT1 " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LMEdorZT1Set
 */
public LMEdorZT1Set getData()
{
    int tCount = 0;
    LMEdorZT1Set tLMEdorZT1Set = new LMEdorZT1Set();
    LMEdorZT1Schema tLMEdorZT1Schema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LMEdorZT1DB";
    ...
    {
        tCount = 1;
        tLMEdorZT1Schema = new LMEdorZT1Schema();
        tLMEdorZT1Schema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LMEdorZT1Set executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LMEdorZT1Set executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCInsureAccFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCInsureAccFeeDB extends LCInsureAccFeeSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCInsureAccFeeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCInsureAccFeeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCInsureAccFeeDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCInsureAccFee WHERE  PolNo = ? AND InsuAccNo = ?", 
	...
	public LCInsureAccFeeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCInsureAccFee");
			LCInsureAccFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCInsureAccFeeSchema s1 = new LCInsureAccFeeSchema();
				s1.setSchema(rs,i);
				aLCInsureAccFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCInsureAccFeeSet;
	}

	public LCInsureAccFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCInsureAccFeeSet aLCInsureAccFeeSet = new LCInsureAccFeeSet();
	...
	public LCInsureAccFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCInsureAccFeeSet aLCInsureAccFeeSet = new LCInsureAccFeeSet();
	...
	public LCInsureAccFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCInsureAccFeeSet aLCInsureAccFeeSet = new LCInsureAccFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCInsureAccFee " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCInsureAccFeeSet
 */
public LCInsureAccFeeSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLCInsureAccFeeSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCInsureAccFeeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCInsureAccFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

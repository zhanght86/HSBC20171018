...
/*
 * <p>ClassName: LLContStateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLContStateDB extends LLContStateSchema
{
private static Logger logger = Logger.getLogger(LLContStateDB.class);
	...
	**/
	...
	// @Constructor
	public LLContStateDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLContStateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLContStateDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getStateType() == null || this.getStateType().equals("null")) {
				...
				pstmt.setString(5, this.getStateType());
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getStateReason() == null || this.getStateReason().equals("null")) {
				...
				pstmt.setString(7, this.getStateReason());
			}
			if(this.getStartDate() == null || this.getStartDate().equals("null")) {
				pstmt.setNull(8, 91);
			} else {
			...
			if(this.getDealState() == null || this.getDealState().equals("null")) {
				...
				pstmt.setString(16, this.getDealState());
			}
			if(this.getClmState() == null || this.getClmState().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getClmState());
			}
			// set where condition
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				pstmt.setNull(18, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLContState WHERE  ClmNo = ? AND ContNo = ? AND InsuredNo = ? AND PolNo = ? AND StateType = ? AND StartDate = ?", 
	...
	public LLContStateSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLContStateSet aLLContStateSet = new LLContStateSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLContState");
			LLContStateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLContStateSchema s1 = new LLContStateSchema();
				s1.setSchema(rs,i);
				aLLContStateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLContStateSet;

	}

	public LLContStateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLContStateSet aLLContStateSet = new LLContStateSet();
	...
	public LLContStateSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLContStateSet aLLContStateSet = new LLContStateSet();
	...
	public LLContStateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLContStateSet aLLContStateSet = new LLContStateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLContState " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLContStateSet
 */
public LLContStateSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLLContStateSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLContStateSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLContStateSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

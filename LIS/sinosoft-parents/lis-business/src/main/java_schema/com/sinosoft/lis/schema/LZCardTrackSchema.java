...
/*
 * <p>ClassName: LZCardTrackSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 单证管理
 */
public class LZCardTrackSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LZCardTrackSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LZCardTrackSchema cloned = (LZCardTrackSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LZCardTrackSchema 对象给 Schema 赋值
	* @param: aLZCardTrackSchema LZCardTrackSchema
	**/
	public void setSchema(LZCardTrackSchema aLZCardTrackSchema)
	{
		this.CertifyCode = aLZCardTrackSchema.getCertifyCode();
		this.SubCode = aLZCardTrackSchema.getSubCode();
		this.RiskCode = aLZCardTrackSchema.getRiskCode();
		this.RiskVersion = aLZCardTrackSchema.getRiskVersion();
		this.StartNo = aLZCardTrackSchema.getStartNo();
		this.EndNo = aLZCardTrackSchema.getEndNo();
		this.SendOutCom = aLZCardTrackSchema.getSendOutCom();
		this.ReceiveCom = aLZCardTrackSchema.getReceiveCom();
		this.SumCount = aLZCardTrackSchema.getSumCount();
		this.Prem = aLZCardTrackSchema.getPrem();
		this.Amnt = aLZCardTrackSchema.getAmnt();
		this.Handler = aLZCardTrackSchema.getHandler();
		this.HandleDate = fDate.getDate( aLZCardTrackSchema.getHandleDate());
		this.InvaliDate = fDate.getDate( aLZCardTrackSchema.getInvaliDate());
		this.TakeBackNo = aLZCardTrackSchema.getTakeBackNo();
		this.SaleChnl = aLZCardTrackSchema.getSaleChnl();
		this.StateFlag = aLZCardTrackSchema.getStateFlag();
		this.OperateFlag = aLZCardTrackSchema.getOperateFlag();
		this.PayFlag = aLZCardTrackSchema.getPayFlag();
		this.EnterAccFlag = aLZCardTrackSchema.getEnterAccFlag();
		this.Reason = aLZCardTrackSchema.getReason();
		this.State = aLZCardTrackSchema.getState();
		this.Operator = aLZCardTrackSchema.getOperator();
		this.MakeDate = fDate.getDate( aLZCardTrackSchema.getMakeDate());
		this.MakeTime = aLZCardTrackSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLZCardTrackSchema.getModifyDate());
		this.ModifyTime = aLZCardTrackSchema.getModifyTime();
		this.ApplyNo = aLZCardTrackSchema.getApplyNo();
		this.AgentCom = aLZCardTrackSchema.getAgentCom();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LZCardTrack表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LZCardTrackSchema";
	...
	public LZCardTrackSchema getSchema()
	{
		LZCardTrackSchema aLZCardTrackSchema = new LZCardTrackSchema();
		aLZCardTrackSchema.setSchema(this);
		return aLZCardTrackSchema;
	}

	public LZCardTrackDB getDB()
	{
		LZCardTrackDB aDBOper = new LZCardTrackDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLZCardTrack描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLZCardTrack>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LZCardTrackSchema other = (LZCardTrackSchema)otherObject;
		return

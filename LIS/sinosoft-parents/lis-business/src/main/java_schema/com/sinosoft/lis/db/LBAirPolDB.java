...
import com.sinosoft.lis.vschema.LBAirPolSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LBAirPolDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 短期险平台接口表
 */
public class LBAirPolDB extends LBAirPolSchema
{
private static Logger logger = Logger.getLogger(LBAirPolDB.class);
	...
	**/
	...
	// @Constructor
	public LBAirPolDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LBAirPol" );
		mflag = true;
	}
	...
	{
		LBAirPolSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LBAirPol WHERE  PolNo = ?");
		...
		SQLString sqlObj = new SQLString("LBAirPol");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LBAirPol(PolNo ,CertifyCode ,ManageCom ,AgentCom ,Password ,InsuredName ,InsuredIDType ,InsuredIDNo ,RelationToInsured ,BnfAddress ,BnfName ,BnfZipCode ,BnfPhone ,Mult ,Prem ,Amnt ,PolState ,EmplaneDate ,FlightNo ,AgentCode ,AgentGroup ,InsuYearFlag ,InsuYear ,EndDate ,CValiDate ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,TSignDate ,RiskCode ,State ,RiskType ,Destination ,Reason ,InsuredSex ,InsuredBirthday ,OccupationType ,PrtNo ,StandByFlag1 ,StandByFlag2 ,StandByFlag3) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getPolNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBAirPol WHERE  PolNo = ?", 
	...
	public LBAirPolSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBAirPolSet aLBAirPolSet = new LBAirPolSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBAirPol");
			LBAirPolSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBAirPolSchema s1 = new LBAirPolSchema();
				s1.setSchema(rs,i);
				aLBAirPolSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LBAirPolSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBAirPolSet aLBAirPolSet = new LBAirPolSet();
	...
	public LBAirPolSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBAirPolSet aLBAirPolSet = new LBAirPolSet();
	...
	public LBAirPolSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBAirPolSet aLBAirPolSet = new LBAirPolSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBAirPol " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LBAirPolSet
 */
public LBAirPolSet getData()
{
    int tCount = 0;
    LBAirPolSet tLBAirPolSet = new LBAirPolSet();
    LBAirPolSchema tLBAirPolSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBAirPolDB";
    ...
    {
        tCount = 1;
        tLBAirPolSchema = new LBAirPolSchema();
        tLBAirPolSchema.setSchema(mResultSet, 1);
        tLBAirPolSet.add(tLBAirPolSchema);
...
public boolean closeData()
{
	...
	public LBAirPolSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBAirPolSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

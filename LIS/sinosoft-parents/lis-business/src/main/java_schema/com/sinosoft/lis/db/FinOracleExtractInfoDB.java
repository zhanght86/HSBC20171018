...
/*
 * <p>ClassName: FinOracleExtractInfoDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class FinOracleExtractInfoDB extends FinOracleExtractInfoSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public FinOracleExtractInfoDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		FinOracleExtractInfoSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "FinOracleExtractInfoDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				...
				pstmt.setString(45, this.getSerialNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM FinOracleExtractInfo WHERE  SerialNo = ?", 
	...
	public FinOracleExtractInfoSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		FinOracleExtractInfoSet aFinOracleExtractInfoSet = new FinOracleExtractInfoSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("FinOracleExtractInfo");
			FinOracleExtractInfoSchema aSchema = this.getSchema();
			...
			{
				i++;
				FinOracleExtractInfoSchema s1 = new FinOracleExtractInfoSchema();
				s1.setSchema(rs,i);
				aFinOracleExtractInfoSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aFinOracleExtractInfoSet;
	}

	public FinOracleExtractInfoSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		FinOracleExtractInfoSet aFinOracleExtractInfoSet = new FinOracleExtractInfoSet();
	...
	public FinOracleExtractInfoSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		FinOracleExtractInfoSet aFinOracleExtractInfoSet = new FinOracleExtractInfoSet();
	...
	public FinOracleExtractInfoSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		FinOracleExtractInfoSet aFinOracleExtractInfoSet = new FinOracleExtractInfoSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update FinOracleExtractInfo " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return FinOracleExtractInfoSet
 */
public FinOracleExtractInfoSet getData()
{
    int tCount = 0;
    FinOracleExtractInfoSet tFinOracleExtractInfoSet = new FinOracleExtractInfoSet();
    FinOracleExtractInfoSchema tFinOracleExtractInfoSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "FinOracleExtractInfoDB";
    ...
    {
        tCount = 1;
        tFinOracleExtractInfoSchema = new FinOracleExtractInfoSchema();
        tFinOracleExtractInfoSchema.setSchema(mResultSet, 1);
        tFinOracleExtractInfoSet.add(tFinOracleExtractInfoSchema);
    ...
    return tFinOracleExtractInfoSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public FinOracleExtractInfoSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public FinOracleExtractInfoSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCPayRuleParamsDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCPayRuleParamsDB extends LCPayRuleParamsSchema
{
private static Logger logger = Logger.getLogger(LCPayRuleParamsDB.class);
	...
	**/
	...
	// @Constructor
	public LCPayRuleParamsDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCPayRuleParamsSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCPayRuleParamsDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPayRuleCode() == null || this.getPayRuleCode().equals("null")) {
				...
				pstmt.setString(20, this.getPayRuleCode());
			}
			if(this.getFactoryType() == null || this.getFactoryType().equals("null")) {
				pstmt.setNull(21, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPayRuleParams WHERE  GrpPolNo = ? AND PayRuleCode = ? AND FactoryType = ? AND OtherNo = ? AND FactoryCode = ? AND FactorySubCode = ? AND InerSerialNo = ? AND ParamName = ?", 
	...
	public LCPayRuleParamsSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPayRuleParamsSet aLCPayRuleParamsSet = new LCPayRuleParamsSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPayRuleParams");
			LCPayRuleParamsSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCPayRuleParamsSchema s1 = new LCPayRuleParamsSchema();
				s1.setSchema(rs,i);
				aLCPayRuleParamsSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCPayRuleParamsSet;

	}

	public LCPayRuleParamsSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPayRuleParamsSet aLCPayRuleParamsSet = new LCPayRuleParamsSet();
	...
	public LCPayRuleParamsSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPayRuleParamsSet aLCPayRuleParamsSet = new LCPayRuleParamsSet();
	...
	public LCPayRuleParamsSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPayRuleParamsSet aLCPayRuleParamsSet = new LCPayRuleParamsSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPayRuleParams " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCPayRuleParamsSet
 */
public LCPayRuleParamsSet getData()
{
    int tCount = 0;
    LCPayRuleParamsSet tLCPayRuleParamsSet = new LCPayRuleParamsSet();
    LCPayRuleParamsSchema tLCPayRuleParamsSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCPayRuleParamsDB";
    ...
    {
        tCount = 1;
        tLCPayRuleParamsSchema = new LCPayRuleParamsSchema();
        tLCPayRuleParamsSchema.setSchema(mResultSet, 1);
        tLCPayRuleParamsSet.add(tLCPayRuleParamsSchema);
...
public boolean closeData()
{
	...
	public LCPayRuleParamsSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPayRuleParamsSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

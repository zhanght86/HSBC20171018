 ...
 */
public class LQClaimCtrlDB extends LQClaimCtrlSchema
{
private static Logger logger = Logger.getLogger(LQClaimCtrlDB.class);
	...
	**/
	...
	// @Constructor
	public LQClaimCtrlDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LQClaimCtrlSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LQClaimCtrlDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAskPriceNo() == null || this.getAskPriceNo().equals("null")) {
				...
				pstmt.setString(27, this.getAskPriceNo());
			}
			if(this.getAskNo() == null || this.getAskNo().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getAskNo());
			}
			if(this.getClaimCtrlCode() == null || this.getClaimCtrlCode().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getClaimCtrlCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LQClaimCtrl WHERE  AskPriceNo = ? AND AskNo = ? AND ClaimCtrlCode = ?", 
	...
	public LQClaimCtrlSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LQClaimCtrlSet aLQClaimCtrlSet = new LQClaimCtrlSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LQClaimCtrl");
			LQClaimCtrlSchema aSchema = this.getSchema();
			...
			{
				i++;
				LQClaimCtrlSchema s1 = new LQClaimCtrlSchema();
				s1.setSchema(rs,i);
				aLQClaimCtrlSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLQClaimCtrlSet;

	}

	public LQClaimCtrlSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LQClaimCtrlSet aLQClaimCtrlSet = new LQClaimCtrlSet();
	...
	public LQClaimCtrlSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LQClaimCtrlSet aLQClaimCtrlSet = new LQClaimCtrlSet();
	...
	public LQClaimCtrlSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LQClaimCtrlSet aLQClaimCtrlSet = new LQClaimCtrlSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LQClaimCtrl " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LQClaimCtrlSet
 */
public LQClaimCtrlSet getData()
{
    int tCount = 0;
    LQClaimCtrlSet tLQClaimCtrlSet = new LQClaimCtrlSet();
    LQClaimCtrlSchema tLQClaimCtrlSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LQClaimCtrlDB";
    ...
    {
        tCount = 1;
        tLQClaimCtrlSchema = new LQClaimCtrlSchema();
        tLQClaimCtrlSchema.setSchema(mResultSet, 1);
        tLQClaimCtrlSet.add(tLQClaimCtrlSchema);
...
public boolean closeData()
{
	...
	public LQClaimCtrlSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LQClaimCtrlSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

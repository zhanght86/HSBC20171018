...
/*
 * <p>ClassName: LAComToAgentDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 代理人管理
 */
public class LAComToAgentDB extends LAComToAgentSchema
{
private static Logger logger = Logger.getLogger(LAComToAgentDB.class);
	...
	**/
	...
	// @Constructor
	public LAComToAgentDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LAComToAgentSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LAComToAgentDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getRelaType() == null || this.getRelaType().equals("null")) {
				...
				pstmt.setString(2, StrTool.space1(this.getRelaType(), 1));
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRelaType() == null || this.getRelaType().equals("null")) {
				...
				pstmt.setString(2, this.getRelaType());
			}
			...
			if(this.getRelaType() == null || this.getRelaType().equals("null")) {
				...
				pstmt.setString(13, StrTool.space1(this.getRelaType(), 1));
			}
			if(this.getAgentGroup() == null || this.getAgentGroup().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LAComToAgent WHERE  AgentCom = ? AND RelaType = ? AND AgentGroup = ?", 
	...
	public LAComToAgentSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LAComToAgentSet aLAComToAgentSet = new LAComToAgentSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LAComToAgent");
			LAComToAgentSchema aSchema = this.getSchema();
			...
			{
				i++;
				LAComToAgentSchema s1 = new LAComToAgentSchema();
				s1.setSchema(rs,i);
				aLAComToAgentSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLAComToAgentSet;

	}

	public LAComToAgentSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LAComToAgentSet aLAComToAgentSet = new LAComToAgentSet();
	...
	public LAComToAgentSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LAComToAgentSet aLAComToAgentSet = new LAComToAgentSet();
	...
	public LAComToAgentSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LAComToAgentSet aLAComToAgentSet = new LAComToAgentSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LAComToAgent " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LAComToAgentSet
 */
public LAComToAgentSet getData()
{
    int tCount = 0;
    LAComToAgentSet tLAComToAgentSet = new LAComToAgentSet();
    LAComToAgentSchema tLAComToAgentSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LAComToAgentDB";
    ...
    try
    {
        ...
        tLAComToAgentSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LAComToAgentSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LAComToAgentSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

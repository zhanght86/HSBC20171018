...
/*
 * <p>ClassName: RnewNotePadDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 续期续保二核
 */
public class RnewNotePadDB extends RnewNotePadSchema
{
private static Logger logger = Logger.getLogger(RnewNotePadDB.class);
	...
	**/
	...
	// @Constructor
	public RnewNotePadDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		RnewNotePadSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "RnewNotePadDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getOperatePos() == null || this.getOperatePos().equals("null")) {
				...
				pstmt.setString(5, this.getOperatePos());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM RnewNotePad WHERE  ProposalContNo = ? AND SerialNo = ?", 
	...
	public RnewNotePadSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		RnewNotePadSet aRnewNotePadSet = new RnewNotePadSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("RnewNotePad");
			RnewNotePadSchema aSchema = this.getSchema();
			...
			{
				i++;
				RnewNotePadSchema s1 = new RnewNotePadSchema();
				s1.setSchema(rs,i);
				aRnewNotePadSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aRnewNotePadSet;

	}

	public RnewNotePadSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RnewNotePadSet aRnewNotePadSet = new RnewNotePadSet();
	...
	public RnewNotePadSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		RnewNotePadSet aRnewNotePadSet = new RnewNotePadSet();
	...
	public RnewNotePadSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		RnewNotePadSet aRnewNotePadSet = new RnewNotePadSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update RnewNotePad " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return RnewNotePadSet
 */
public RnewNotePadSet getData()
{
    int tCount = 0;
    RnewNotePadSet tRnewNotePadSet = new RnewNotePadSet();
    RnewNotePadSchema tRnewNotePadSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "RnewNotePadDB";
    ...
    {
        tCount = 1;
        tRnewNotePadSchema = new RnewNotePadSchema();
        tRnewNotePadSchema.setSchema(mResultSet, 1);
        tRnewNotePadSet.add(tRnewNotePadSchema);
...
public boolean closeData()
{
	...
	public RnewNotePadSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public RnewNotePadSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

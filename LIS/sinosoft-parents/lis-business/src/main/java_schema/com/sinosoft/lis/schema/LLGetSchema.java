...
/*
 * <p>ClassName: LLGetSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLGetSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLGetSchema.class);
	// @Field
	/** 赔案号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLGetSchema cloned = (LLGetSchema)super.clone();
	...
	*/
	...
	public void setDutyCode(String aDutyCode)
	{
		if(aDutyCode!=null && aDutyCode.length()>10)
			throw new IllegalArgumentException("责任编??DutyCode值"+aDutyCode+"的长度"+aDutyCode.length()+"大于最大值10");
		DutyCode = aDutyCode;
	}
	...
	*/
	...
	/**
	* 使用另外一个 LLGetSchema 对象给 Schema 赋值
	* @param: aLLGetSchema LLGetSchema
	**/
	public void setSchema(LLGetSchema aLLGetSchema)
	{
		this.ClmNo = aLLGetSchema.getClmNo();
		this.OperationType = aLLGetSchema.getOperationType();
		this.BnfNo = aLLGetSchema.getBnfNo();
		this.CustomerNo = aLLGetSchema.getCustomerNo();
		this.Name = aLLGetSchema.getName();
		this.Sex = aLLGetSchema.getSex();
		this.Birthday = fDate.getDate( aLLGetSchema.getBirthday());
		this.IDType = aLLGetSchema.getIDType();
		this.IDNo = aLLGetSchema.getIDNo();
		this.GrpContNo = aLLGetSchema.getGrpContNo();
		this.ContNo = aLLGetSchema.getContNo();
		this.PolNo = aLLGetSchema.getPolNo();
		this.DutyCode = aLLGetSchema.getDutyCode();
		this.GetDutyCode = aLLGetSchema.getGetDutyCode();
		this.GetDutyKind = aLLGetSchema.getGetDutyKind();
		this.InsuredNo = aLLGetSchema.getInsuredNo();
		this.GetMode = aLLGetSchema.getGetMode();
		this.GetLimit = aLLGetSchema.getGetLimit();
		this.GetRate = aLLGetSchema.getGetRate();
		this.UrgeGetFlag = aLLGetSchema.getUrgeGetFlag();
		this.LiveGetType = aLLGetSchema.getLiveGetType();
		this.AddRate = aLLGetSchema.getAddRate();
		this.CanGet = aLLGetSchema.getCanGet();
		this.NeedAcc = aLLGetSchema.getNeedAcc();
		this.NeedCancelAcc = aLLGetSchema.getNeedCancelAcc();
		this.StandMoney = aLLGetSchema.getStandMoney();
		this.ActuGet = aLLGetSchema.getActuGet();
		this.SumMoney = aLLGetSchema.getSumMoney();
		this.GetIntv = aLLGetSchema.getGetIntv();
		this.GettoDate = fDate.getDate( aLLGetSchema.getGettoDate());
		this.GetStartDate = fDate.getDate( aLLGetSchema.getGetStartDate());
		this.GetEndDate = fDate.getDate( aLLGetSchema.getGetEndDate());
		this.BalaDate = fDate.getDate( aLLGetSchema.getBalaDate());
		this.State = aLLGetSchema.getState();
		this.ManageCom = aLLGetSchema.getManageCom();
		this.Operator = aLLGetSchema.getOperator();
		this.MakeDate = fDate.getDate( aLLGetSchema.getMakeDate());
		this.MakeTime = aLLGetSchema.getMakeTime();
		this.ModifyTime = aLLGetSchema.getModifyTime();
		this.ModifyDate = fDate.getDate( aLLGetSchema.getModifyDate());
		this.GetEndState = aLLGetSchema.getGetEndState();
		this.Currency = aLLGetSchema.getCurrency();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLGet表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLGetSchema";
	...
	public LLGetSchema getSchema()
	{
		LLGetSchema aLLGetSchema = new LLGetSchema();
		aLLGetSchema.setSchema(this);
		return aLLGetSchema;
	}

	public LLGetDB getDB()
	{
		LLGetDB aDBOper = new LLGetDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLGet描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLGet>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			OperationType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 2, SysConst.PACKAGESPILTER );
			BnfNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			...
			GetMode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 17, SysConst.PACKAGESPILTER );
			GetLimit = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,18,SysConst.PACKAGESPILTER))).doubleValue();
			GetRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,19,SysConst.PACKAGESPILTER))).doubleValue();
			UrgeGetFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20, SysConst.PACKAGESPILTER );
			LiveGetType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 21, SysConst.PACKAGESPILTER );
			AddRate = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,22,SysConst.PACKAGESPILTER))).doubleValue();
			CanGet = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 23, SysConst.PACKAGESPILTER );
			NeedAcc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 24, SysConst.PACKAGESPILTER );
			NeedCancelAcc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 25, SysConst.PACKAGESPILTER );
			StandMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,26,SysConst.PACKAGESPILTER))).doubleValue();
			ActuGet = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,27,SysConst.PACKAGESPILTER))).doubleValue();
			SumMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,28,SysConst.PACKAGESPILTER))).doubleValue();
			GetIntv= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,29,SysConst.PACKAGESPILTER))).intValue();
			GettoDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 30,SysConst.PACKAGESPILTER));
			GetStartDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 31,SysConst.PACKAGESPILTER));
			GetEndDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 32,SysConst.PACKAGESPILTER));
			BalaDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 33,SysConst.PACKAGESPILTER));
			State = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 34, SysConst.PACKAGESPILTER );
			ManageCom = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 35, SysConst.PACKAGESPILTER );
			...
			GetEndState = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 41, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LLGetSchema other = (LLGetSchema)otherObject;
		return

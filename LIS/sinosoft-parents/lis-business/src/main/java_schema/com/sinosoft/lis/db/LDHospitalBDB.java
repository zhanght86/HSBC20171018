...
/*
 * <p>ClassName: LDHospitalBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LDHospitalBDB extends LDHospitalBSchema
{
private static Logger logger = Logger.getLogger(LDHospitalBDB.class);
	...
	**/
	...
	// @Constructor
	public LDHospitalBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDHospitalBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LDHospitalBDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getHospitCode() == null || this.getHospitCode().equals("null")) {
				...
				pstmt.setString(2, this.getHospitCode());
			}
			...
			if(this.getHospitShortName() == null || this.getHospitShortName().equals("null")) {
				...
				pstmt.setString(5, this.getHospitShortName());
			}
			if(this.getFixFlag() == null || this.getFixFlag().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getFixFlag());
			}
			...
			if(this.getHospitLicencNo() == null || this.getHospitLicencNo().equals("null")) {
				...
				pstmt.setString(12, this.getHospitLicencNo());
			}
			...
			if(this.getAccName() == null || this.getAccName().equals("null")) {
				...
				pstmt.setString(14, this.getAccName());
			}
			...
			if(this.getHosState() == null || this.getHosState().equals("null")) {
				...
				pstmt.setString(16, this.getHosState());
			}
			...
			if(this.getLinkMan() == null || this.getLinkMan().equals("null")) {
				...
				pstmt.setString(18, this.getLinkMan());
			}
			if(this.getValidityDate() == null || this.getValidityDate().equals("null")) {
				pstmt.setNull(19, 91);
			} else {
				pstmt.setDate(19, Date.valueOf(this.getValidityDate()));
			}
			...
			if(this.getOperator2() == null || this.getOperator2().equals("null")) {
				...
				pstmt.setString(30, this.getOperator2());
			}
			// set where condition
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(31, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDHospitalB WHERE  SerialNo = ?", 
	...
	public LDHospitalBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LDHospitalBSet aLDHospitalBSet = new LDHospitalBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDHospitalB");
			LDHospitalBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LDHospitalBSchema s1 = new LDHospitalBSchema();
				s1.setSchema(rs,i);
				aLDHospitalBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLDHospitalBSet;

	}

	public LDHospitalBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDHospitalBSet aLDHospitalBSet = new LDHospitalBSet();
	...
	public LDHospitalBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LDHospitalBSet aLDHospitalBSet = new LDHospitalBSet();
	...
	public LDHospitalBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDHospitalBSet aLDHospitalBSet = new LDHospitalBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDHospitalB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDHospitalBSet
 */
public LDHospitalBSet getData()
{
    int tCount = 0;
    LDHospitalBSet tLDHospitalBSet = new LDHospitalBSet();
    LDHospitalBSchema tLDHospitalBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDHospitalBDB";
    ...
    {
        tCount = 1;
        tLDHospitalBSchema = new LDHospitalBSchema();
        tLDHospitalBSchema.setSchema(mResultSet, 1);
        tLDHospitalBSet.add(tLDHospitalBSchema);
...
public boolean closeData()
{
	...
	public LDHospitalBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDHospitalBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
{
private static Logger logger = Logger.getLogger(LBInsuredDBSet.class);
	...
	**/
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt = con.prepareStatement("DELETE FROM LBInsured WHERE  ContNo = ? AND InsuredNo = ?");
...
ex.printStackTrace();
	...
	public boolean update()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("UPDATE LBInsured SET  EdorNo = ? , GrpContNo = ? , ContNo = ? , InsuredNo = ? , PrtNo = ? , AppntNo = ? , ManageCom = ? , ExecuteCom = ? , FamilyID = ? , RelationToMainInsured = ? , RelationToAppnt = ? , AddressNo = ? , SequenceNo = ? , Name = ? , Sex = ? , Birthday = ? , IDType = ? , IDNo = ? , NativePlace = ? , Nationality = ? , RgtAddress = ? , Marriage = ? , MarriageDate = ? , Health = ? , Stature = ? , Avoirdupois = ? , Degree = ? , CreditGrade = ? , BankCode = ? , BankAccNo = ? , AccName = ? , JoinCompanyDate = ? , StartWorkDate = ? , Position = ? , Salary = ? , OccupationType = ? , OccupationCode = ? , WorkType = ? , PluralityType = ? , SmokeFlag = ? , ContPlanCode = ? , Operator = ? , InsuredStat = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , UWFlag = ? , UWCode = ? , UWDate = ? , UWTime = ? , BMI = ? , InsuredPeoples = ? , License = ? , LicenseType = ? , CustomerSeqNo = ? , SocialInsuNo = ? , SendMsgFlag = ? , SendMailFlag = ? , JoinContDate = ? , WorkNo = ? , CertifyCode = ? , StartCode = ? , EndCode = ? , SocialInsuFlag = ? , OrganComCode = ? WHERE  ContNo = ? AND InsuredNo = ?");
            for (int i = 1; i <= tCount; i++)
            {
			...
			if(this.get(i).getExecuteCom() == null || this.get(i).getExecuteCom().equals("null")) {
				...
				pstmt.setString(8, this.get(i).getExecuteCom());
			}
			...
			if(this.get(i).getRelationToMainInsured() == null || this.get(i).getRelationToMainInsured().equals("null")) {
				...
				pstmt.setString(10, this.get(i).getRelationToMainInsured());
			}
			if(this.get(i).getRelationToAppnt() == null || this.get(i).getRelationToAppnt().equals("null")) {
				pstmt.setString(11,null);
			} else {
				pstmt.setString(11, this.get(i).getRelationToAppnt());
			}
			...
			if(this.get(i).getSequenceNo() == null || this.get(i).getSequenceNo().equals("null")) {
				...
				pstmt.setString(13, this.get(i).getSequenceNo());
			}
			if(this.get(i).getName() == null || this.get(i).getName().equals("null")) {
				pstmt.setString(14,null);
			} else {
				pstmt.setString(14, this.get(i).getName());
			}
			if(this.get(i).getSex() == null || this.get(i).getSex().equals("null")) {
				pstmt.setString(15,null);
			} else {
				pstmt.setString(15, this.get(i).getSex());
			}
			if(this.get(i).getBirthday() == null || this.get(i).getBirthday().equals("null")) {
				pstmt.setDate(16,null);
			} else {
				pstmt.setDate(16, Date.valueOf(this.get(i).getBirthday()));
			}
			...
			if(this.get(i).getCreditGrade() == null || this.get(i).getCreditGrade().equals("null")) {
				...
				pstmt.setString(28, this.get(i).getCreditGrade());
			}
			...
			if(this.get(i).getWorkType() == null || this.get(i).getWorkType().equals("null")) {
				...
				pstmt.setString(38, this.get(i).getWorkType());
			}
			if(this.get(i).getPluralityType() == null || this.get(i).getPluralityType().equals("null")) {
				pstmt.setString(39,null);
			} else {
				pstmt.setString(39, this.get(i).getPluralityType());
			}
			...
			if(this.get(i).getContPlanCode() == null || this.get(i).getContPlanCode().equals("null")) {
				...
				pstmt.setString(41, this.get(i).getContPlanCode());
			}
			...
			if(this.get(i).getInsuredStat() == null || this.get(i).getInsuredStat().equals("null")) {
				...
				pstmt.setString(43, this.get(i).getInsuredStat());
			}
			...
			if(this.get(i).getOrganComCode() == null || this.get(i).getOrganComCode().equals("null")) {
				...
				pstmt.setString(66, this.get(i).getOrganComCode());
			}
			// set where condition
			if(this.get(i).getContNo() == null || this.get(i).getContNo().equals("null")) {
				pstmt.setString(67,null);
			} else {
				pstmt.setString(67, this.get(i).getContNo());
			}
			if(this.get(i).getInsuredNo() == null || this.get(i).getInsuredNo().equals("null")) {
				pstmt.setString(68,null);
			} else {
				pstmt.setString(68, this.get(i).getInsuredNo());
			}

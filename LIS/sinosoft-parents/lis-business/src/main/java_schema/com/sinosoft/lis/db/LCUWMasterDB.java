...
/*
 * <p>ClassName: LCUWMasterDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LCUWMasterDB extends LCUWMasterSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCUWMasterDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCUWMasterSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAutoUWDate() == null || this.getAutoUWDate().equals("null")) {
				...
				pstmt.setDate(44, Date.valueOf(this.getAutoUWDate()));
			}
			if(this.getUWConclu() == null || this.getUWConclu().equals("null")) {
				pstmt.setNull(45, 12);
			} else {
				pstmt.setString(45, this.getUWConclu());
			}
			if(this.getUWDate() == null || this.getUWDate().equals("null")) {
				pstmt.setNull(46, 91);
			} else {
				pstmt.setDate(46, Date.valueOf(this.getUWDate()));
			}
			...
			if(this.getProposalNo() == null || this.getProposalNo().equals("null")) {
				...
				pstmt.setString(49, this.getProposalNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCUWMaster WHERE  ProposalNo = ?", 
	...
	public LCUWMasterSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCUWMasterSet aLCUWMasterSet = new LCUWMasterSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCUWMaster");
			LCUWMasterSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCUWMasterSchema s1 = new LCUWMasterSchema();
				s1.setSchema(rs,i);
				aLCUWMasterSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCUWMasterSet;
	}

	public LCUWMasterSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCUWMasterSet aLCUWMasterSet = new LCUWMasterSet();
	...
	public LCUWMasterSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCUWMasterSet aLCUWMasterSet = new LCUWMasterSet();
	...
	public LCUWMasterSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCUWMasterSet aLCUWMasterSet = new LCUWMasterSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCUWMaster " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCUWMasterSet
 */
public LCUWMasterSet getData()
{
    int tCount = 0;
    LCUWMasterSet tLCUWMasterSet = new LCUWMasterSet();
    LCUWMasterSchema tLCUWMasterSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCUWMasterDB";
    ...
    try
    {
        ...
        tLCUWMasterSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCUWMasterSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCUWMasterSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LJPremMatchFeeLogDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LJPremMatchFeeLogDB extends LJPremMatchFeeLogSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LJPremMatchFeeLogDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJPremMatchFeeLogSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJPremMatchFeeLogDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getDataSource() == null || this.getDataSource().equals("null")) {
				...
				pstmt.setString(3, this.getDataSource());
			}
			if(this.getSourceNo() == null || this.getSourceNo().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getSourceNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(16, this.getMoney());
			pstmt.setDouble(17, this.getCurOutPayMoney());
			...
			if(this.getMatchSerialNo() == null || this.getMatchSerialNo().equals("null")) {
				...
				pstmt.setString(26, this.getMatchSerialNo());
			}
			if(this.getMatchNode() == null || this.getMatchNode().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
				pstmt.setString(27, this.getMatchNode());
			}
			if(this.getDataSource() == null || this.getDataSource().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getDataSource());
			}
			if(this.getSourceNo() == null || this.getSourceNo().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getSourceNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJPremMatchFeeLog WHERE  MatchSerialNo = ? AND MatchNode = ? AND DataSource = ? AND SourceNo = ?", 
	...
	public LJPremMatchFeeLogSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJPremMatchFeeLogSet aLJPremMatchFeeLogSet = new LJPremMatchFeeLogSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJPremMatchFeeLog");
			LJPremMatchFeeLogSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJPremMatchFeeLogSchema s1 = new LJPremMatchFeeLogSchema();
				s1.setSchema(rs,i);
				aLJPremMatchFeeLogSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJPremMatchFeeLogSet;
	}

	public LJPremMatchFeeLogSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJPremMatchFeeLogSet aLJPremMatchFeeLogSet = new LJPremMatchFeeLogSet();
	...
	public LJPremMatchFeeLogSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJPremMatchFeeLogSet aLJPremMatchFeeLogSet = new LJPremMatchFeeLogSet();
	...
	public LJPremMatchFeeLogSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJPremMatchFeeLogSet aLJPremMatchFeeLogSet = new LJPremMatchFeeLogSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJPremMatchFeeLog " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJPremMatchFeeLogSet
 */
public LJPremMatchFeeLogSet getData()
{
    int tCount = 0;
    LJPremMatchFeeLogSet tLJPremMatchFeeLogSet = new LJPremMatchFeeLogSet();
    LJPremMatchFeeLogSchema tLJPremMatchFeeLogSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJPremMatchFeeLogDB";
    ...
    {
        tCount = 1;
        tLJPremMatchFeeLogSchema = new LJPremMatchFeeLogSchema();
        tLJPremMatchFeeLogSchema.setSchema(mResultSet, 1);
        tLJPremMatchFeeLogSet.add(tLJPremMatchFeeLogSchema);
...
public boolean closeData()
{
	...
	public LJPremMatchFeeLogSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LJPremMatchFeeLogSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

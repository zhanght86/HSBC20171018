...
/*
 * <p>ClassName: LLUWSubDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLUWSubDB extends LLUWSubSchema
{
private static Logger logger = Logger.getLogger(LLUWSubDB.class);
	...
	**/
	...
	// @Constructor
	public LLUWSubDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LLUWSub" );
		mflag = true;
	}
	...
	{
		LLUWSubSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LLUWSub WHERE  CaseNo = ? AND BatNo = ? AND PolNo = ? AND UWNo = ?");
		...
		SQLString sqlObj = new SQLString("LLUWSub");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LLUWSub SET  CaseNo = ? , GrpContNo = ? , BatNo = ? , ContNo = ? , ProposalContNo = ? , PolNo = ? , ProposalNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? WHERE  CaseNo = ? AND BatNo = ? AND PolNo = ? AND UWNo = ?");
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCaseNo());
			...
			if(this.getBatNo() == null || this.getBatNo().equals("null")) {
				...
				pstmt.setString(44, this.getBatNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(45, 12);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LLUWSub(CaseNo ,GrpContNo ,BatNo ,ContNo ,ProposalContNo ,PolNo ,ProposalNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getCaseNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLUWSub WHERE  CaseNo = ? AND BatNo = ? AND PolNo = ? AND UWNo = ?", 
	...
	public LLUWSubSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLUWSubSet aLLUWSubSet = new LLUWSubSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLUWSub");
			LLUWSubSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLUWSubSchema s1 = new LLUWSubSchema();
				s1.setSchema(rs,i);
				aLLUWSubSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LLUWSubSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWSubSet aLLUWSubSet = new LLUWSubSet();
	...
	public LLUWSubSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLUWSubSet aLLUWSubSet = new LLUWSubSet();
	...
	public LLUWSubSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWSubSet aLLUWSubSet = new LLUWSubSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLUWSub " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LLUWSubSet
 */
public LLUWSubSet getData()
{
    int tCount = 0;
    LLUWSubSet tLLUWSubSet = new LLUWSubSet();
    LLUWSubSchema tLLUWSubSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLUWSubDB";
    ...
    {
        tCount = 1;
        tLLUWSubSchema = new LLUWSubSchema();
        tLLUWSubSchema.setSchema(mResultSet, 1);
        tLLUWSubSet.add(tLLUWSubSchema);
...
public boolean closeData()
{
	...
	public LLUWSubSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLUWSubSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

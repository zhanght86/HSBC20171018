...
/*
 * <p>ClassName: LBAppntIndDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统lis5模型
 */
public class LBAppntIndDB extends LBAppntIndSchema
{
private static Logger logger = Logger.getLogger(LBAppntIndDB.class);
	...
	**/
	...
	// @Constructor
	public LBAppntIndDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBAppntIndSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBAppntIndDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getSequenceNo() == null || this.getSequenceNo().equals("null")) {
				...
				pstmt.setString(4, this.getSequenceNo());
			}
			...
			if(this.getBirthday() == null || this.getBirthday().equals("null")) {
				...
				pstmt.setDate(10, Date.valueOf(this.getBirthday()));
			}
			if(this.getNativePlace() == null || this.getNativePlace().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getNativePlace());
			}
			if(this.getNationality() == null || this.getNationality().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getNationality());
			}
			...
			if(this.getOccupationType() == null || this.getOccupationType().equals("null")) {
				...
				pstmt.setString(15, this.getOccupationType());
			}
			if(this.getStartWorkDate() == null || this.getStartWorkDate().equals("null")) {
				pstmt.setNull(16, 91);
			} else {
				pstmt.setDate(16, Date.valueOf(this.getStartWorkDate()));
			}
			pstmt.setDouble(17, this.getSalary());
			...
			if(this.getBP() == null || this.getBP().equals("null")) {
				...
				pstmt.setString(33, this.getBP());
			}
			if(this.getMobile() == null || this.getMobile().equals("null")) {
				pstmt.setNull(34, 12);
			} else {
				pstmt.setString(34, this.getMobile());
			}
			...
			if(this.getPhone2() == null || this.getPhone2().equals("null")) {
				...
				pstmt.setString(56, this.getPhone2());
			}
			// set where condition
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(57, 12);
			...
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				...
				pstmt.setString(58, this.getCustomerNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBAppntInd WHERE  PolNo = ? AND CustomerNo = ?", 
	...
	public LBAppntIndSet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBAppntInd");
			LBAppntIndSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBAppntIndSchema s1 = new LBAppntIndSchema();
				s1.setSchema(rs,i);
				aLBAppntIndSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBAppntIndSet;

	}

	public LBAppntIndSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBAppntIndSet aLBAppntIndSet = new LBAppntIndSet();
	...
	public LBAppntIndSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBAppntIndSet aLBAppntIndSet = new LBAppntIndSet();
	...
	public LBAppntIndSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBAppntIndSet aLBAppntIndSet = new LBAppntIndSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBAppntInd " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBAppntIndSet
 */
public LBAppntIndSet getData()
{
    int tCount = 0;
    LBAppntIndSet tLBAppntIndSet = new LBAppntIndSet();
    LBAppntIndSchema tLBAppntIndSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBAppntIndDB";
    ...
    try
    {
        ...
        tLBAppntIndSchema.setSchema(mResultSet, 1);
        tLBAppntIndSet.add(tLBAppntIndSchema);
...
public boolean closeData()
{
	...
	public LBAppntIndSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBAppntIndSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LLClaimAccountDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统lis5模型
 */
public class LLClaimAccountDB extends LLClaimAccountSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LLClaimAccountDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLClaimAccountSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLClaimAccountDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getDeclineNo() == null || this.getDeclineNo().equals("null")) {
				...
				pstmt.setString(2, this.getDeclineNo());
			}
			if(this.getAccNo() == null || this.getAccNo().equals("null")) {
				pstmt.setNull(3, 12);
			} else {
				pstmt.setString(3, this.getAccNo());
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getRiskVer() == null || this.getRiskVer().equals("null")) {
				...
				pstmt.setString(9, this.getRiskVer());
			}
			...
			if(this.getPolMngCom() == null || this.getPolMngCom().equals("null")) {
				...
				pstmt.setString(11, this.getPolMngCom());
			}
			...
			pstmt.setDouble(15, this.getAccPayMoney());
			...
			if(this.getOtherNo() == null || this.getOtherNo().equals("null")) {
				...
				pstmt.setString(25, this.getOtherNo());
			}
			...
			if(this.getAccAscription() == null || this.getAccAscription().equals("null")) {
				...
				pstmt.setString(27, this.getAccAscription());
			}
			if(this.getOtherType() == null || this.getOtherType().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getOtherType());
			}
			...
			if(this.getAscriptType() == null || this.getAscriptType().equals("null")) {
				...
				pstmt.setString(33, this.getAscriptType());
			}
			...
			if(this.getInvestType() == null || this.getInvestType().equals("null")) {
				...
				pstmt.setString(35, this.getInvestType());
			}
			if(this.getAccComputeFlag() == null || this.getAccComputeFlag().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
				pstmt.setString(36, this.getAccComputeFlag());
			}
			pstmt.setDouble(37, this.getAccPayUnitCount());
			pstmt.setDouble(38, this.getAccPayInterest());
			...
			if(this.getClmNo() == null || this.getClmNo().equals("null")) {
				...
				pstmt.setString(41, this.getClmNo());
			}
			if(this.getDeclineNo() == null || this.getDeclineNo().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
				pstmt.setString(42, this.getDeclineNo());
			}
			if(this.getAccNo() == null || this.getAccNo().equals("null")) {
				pstmt.setNull(43, 12);
			} else {
				pstmt.setString(43, this.getAccNo());
			}
			if(this.getGetDutyCode() == null || this.getGetDutyCode().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLClaimAccount WHERE  ClmNo = ? AND DeclineNo = ? AND AccNo = ? AND GetDutyCode = ? AND PolNo = ?", 
	...
	public LLClaimAccountSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLClaimAccountSet aLLClaimAccountSet = new LLClaimAccountSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLClaimAccount");
			LLClaimAccountSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLClaimAccountSchema s1 = new LLClaimAccountSchema();
				s1.setSchema(rs,i);
				aLLClaimAccountSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLClaimAccountSet;
	}

	public LLClaimAccountSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLClaimAccountSet aLLClaimAccountSet = new LLClaimAccountSet();
	...
	public LLClaimAccountSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLClaimAccountSet aLLClaimAccountSet = new LLClaimAccountSet();
	...
	public LLClaimAccountSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLClaimAccountSet aLLClaimAccountSet = new LLClaimAccountSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLClaimAccount " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLClaimAccountSet
 */
public LLClaimAccountSet getData()
{
    int tCount = 0;
    LLClaimAccountSet tLLClaimAccountSet = new LLClaimAccountSet();
    LLClaimAccountSchema tLLClaimAccountSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLClaimAccountDB";
    ...
    {
        tCount = 1;
        tLLClaimAccountSchema = new LLClaimAccountSchema();
        tLLClaimAccountSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLClaimAccountSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLClaimAccountSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

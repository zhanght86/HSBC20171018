...
/*
 * <p>ClassName: LCPolOriginalDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCPolOriginalDB extends LCPolOriginalSchema
{
private static Logger logger = Logger.getLogger(LCPolOriginalDB.class);
	...
	**/
	...
	// @Constructor
	public LCPolOriginalDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCPolOriginalSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCPolOriginalDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPolTypeFlag() == null || this.getPolTypeFlag().equals("null")) {
				...
				pstmt.setString(5, this.getPolTypeFlag());
			}
			if(this.getMainPolNo() == null || this.getMainPolNo().equals("null")) {
				pstmt.setNull(6, 12);
			} else {
				pstmt.setString(6, this.getMainPolNo());
			}
			...
			if(this.getPayEndYearFlag() == null || this.getPayEndYearFlag().equals("null")) {
				...
				pstmt.setString(15, this.getPayEndYearFlag());
			}
			pstmt.setInt(16, this.getPayEndYear());
			...
			if(this.getPayMode() == null || this.getPayMode().equals("null")) {
				...
				pstmt.setString(19, this.getPayMode());
			}
			if(this.getPayLocation() == null || this.getPayLocation().equals("null")) {
				pstmt.setNull(20, 1);
			} else {
				pstmt.setString(20, this.getPayLocation());
			}
			pstmt.setInt(21, this.getPayIntv());
			pstmt.setInt(22, this.getPayYears());
			pstmt.setInt(23, this.getYears());
			pstmt.setDouble(24, this.getMult());
			pstmt.setDouble(25, this.getStandPrem());
			pstmt.setDouble(26, this.getPrem());
			pstmt.setDouble(27, this.getSumPrem());
			pstmt.setDouble(28, this.getAmnt());
			pstmt.setDouble(29, this.getRiskAmnt());
			...
			if(this.getRiskSequence() == null || this.getRiskSequence().equals("null")) {
				...
				pstmt.setString(35, this.getRiskSequence());
			}
			pstmt.setDouble(36, this.getJobAddFee());
			pstmt.setDouble(37, this.getInputPrem());
			// set where condition
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPolOriginal WHERE  PolNo = ?", 
	...
	public LCPolOriginalSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPolOriginalSet aLCPolOriginalSet = new LCPolOriginalSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPolOriginal");
			LCPolOriginalSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCPolOriginalSchema s1 = new LCPolOriginalSchema();
				s1.setSchema(rs,i);
				aLCPolOriginalSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCPolOriginalSet;

	}

	public LCPolOriginalSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPolOriginalSet aLCPolOriginalSet = new LCPolOriginalSet();
	...
	public LCPolOriginalSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPolOriginalSet aLCPolOriginalSet = new LCPolOriginalSet();
	...
	public LCPolOriginalSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPolOriginalSet aLCPolOriginalSet = new LCPolOriginalSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPolOriginal " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCPolOriginalSet
 */
public LCPolOriginalSet getData()
{
    int tCount = 0;
    LCPolOriginalSet tLCPolOriginalSet = new LCPolOriginalSet();
    LCPolOriginalSchema tLCPolOriginalSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCPolOriginalDB";
    ...
    {
        tCount = 1;
        tLCPolOriginalSchema = new LCPolOriginalSchema();
        tLCPolOriginalSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCPolOriginalSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPolOriginalSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

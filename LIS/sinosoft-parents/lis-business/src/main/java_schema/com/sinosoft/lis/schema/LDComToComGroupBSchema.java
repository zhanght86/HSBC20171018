...
import com.sinosoft.lis.db.LDComToComGroupBDB;

/*
 * <p>ClassName: LDComToComGroupBSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class LDComToComGroupBSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LDComToComGroupBSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LDComToComGroupBSchema cloned = (LDComToComGroupBSchema)super.clone();
	...
	/**
	* 使用另外一个 LDComToComGroupBSchema 对象给 Schema 赋值
	* @param: aLDComToComGroupBSchema LDComToComGroupBSchema
	**/
	public void setSchema(LDComToComGroupBSchema aLDComToComGroupBSchema)
	{
		this.SerialNo = aLDComToComGroupBSchema.getSerialNo();
		this.ComGroup = aLDComToComGroupBSchema.getComGroup();
		this.ComCode = aLDComToComGroupBSchema.getComCode();
		this.Operator = aLDComToComGroupBSchema.getOperator();
		this.MakeDate = fDate.getDate( aLDComToComGroupBSchema.getMakeDate());
		this.MakeTime = aLDComToComGroupBSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLDComToComGroupBSchema.getModifyDate());
		this.ModifyTime = aLDComToComGroupBSchema.getModifyTime();
		this.BatchNo = aLDComToComGroupBSchema.getBatchNo();
		this.BakDate = fDate.getDate( aLDComToComGroupBSchema.getBakDate());
		this.BakTime = aLDComToComGroupBSchema.getBakTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LDComToComGroupB表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LDComToComGroupBSchema";
	...
	public LDComToComGroupBSchema getSchema()
	{
		LDComToComGroupBSchema aLDComToComGroupBSchema = new LDComToComGroupBSchema();
		aLDComToComGroupBSchema.setSchema(this);
		return aLDComToComGroupBSchema;
	}

	public LDComToComGroupBDB getDB()
	{
		LDComToComGroupBDB aDBOper = new LDComToComGroupBDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDComToComGroupB描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLDComToComGroupB>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			BatchNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			BakDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10,SysConst.PACKAGESPILTER));
			BakTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LDComToComGroupBSchema other = (LDComToComGroupBSchema)otherObject;
		return

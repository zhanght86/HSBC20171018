...
/*
 * <p>ClassName: LJDebtsPayDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LJDebtsPayDB extends LJDebtsPaySchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LJDebtsPayDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJDebtsPaySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJDebtsPayDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getDebtsNo() == null || this.getDebtsNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getDebtsNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(2, this.getDebtsMoney());
			pstmt.setDouble(3, this.getActualMoney());
			if(this.getDebtsType() == null || this.getDebtsType().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getDebtsType());
			}
			...
			if(this.getBussType() == null || this.getBussType().equals("null")) {
				...
				pstmt.setString(7, this.getBussType());
			}
			if(this.getBussNo() == null || this.getBussNo().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getBussNo());
			}
			...
			if(this.getRelaNo() == null || this.getRelaNo().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getRelaNo());
			}
			if(this.getAppOperator() == null || this.getAppOperator().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getAppOperator());
			}
			if(this.getAppDate() == null || this.getAppDate().equals("null")) {
				pstmt.setNull(13, 91);
			} else {
				pstmt.setDate(13, Date.valueOf(this.getAppDate()));
			}
			if(this.getAppTime() == null || this.getAppTime().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getAppTime());
			}
			if(this.getAppDesc() == null || this.getAppDesc().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getAppDesc());
			}
			...
			if(this.getDebtsNo() == null || this.getDebtsNo().equals("null")) {
				...
				pstmt.setString(32, this.getDebtsNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJDebtsPay WHERE  DebtsNo = ?", 
	...
	public LJDebtsPaySet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJDebtsPaySet aLJDebtsPaySet = new LJDebtsPaySet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJDebtsPay");
			LJDebtsPaySchema aSchema = this.getSchema();
			...
			{
				i++;
				LJDebtsPaySchema s1 = new LJDebtsPaySchema();
				s1.setSchema(rs,i);
				aLJDebtsPaySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJDebtsPaySet;
	}

	public LJDebtsPaySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJDebtsPaySet aLJDebtsPaySet = new LJDebtsPaySet();
	...
	public LJDebtsPaySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LJDebtsPaySet aLJDebtsPaySet = new LJDebtsPaySet();
	...
	public LJDebtsPaySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJDebtsPaySet aLJDebtsPaySet = new LJDebtsPaySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJDebtsPay " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJDebtsPaySet
 */
public LJDebtsPaySet getData()
{
    int tCount = 0;
    LJDebtsPaySet tLJDebtsPaySet = new LJDebtsPaySet();
    LJDebtsPaySchema tLJDebtsPaySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJDebtsPayDB";
    ...
    {
        tCount = 1;
        tLJDebtsPaySchema = new LJDebtsPaySchema();
        tLJDebtsPaySchema.setSchema(mResultSet, 1);
        tLJDebtsPaySet.add(tLJDebtsPaySchema);
...
public boolean closeData()
{
	...
	public LJDebtsPaySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LJDebtsPaySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

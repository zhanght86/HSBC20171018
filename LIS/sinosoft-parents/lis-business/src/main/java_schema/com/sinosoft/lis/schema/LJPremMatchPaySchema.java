...
import com.sinosoft.lis.db.LJPremMatchPayDB;

/*
 * <p>ClassName: LJPremMatchPaySchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LJPremMatchPaySchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LJPremMatchPaySchema.class);
	// @Field
	/** 保费匹配流水号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LJPremMatchPaySchema cloned = (LJPremMatchPaySchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LJPremMatchPaySchema 对象给 Schema 赋值
	* @param: aLJPremMatchPaySchema LJPremMatchPaySchema
	**/
	public void setSchema(LJPremMatchPaySchema aLJPremMatchPaySchema)
	{
		this.MatchSerialNo = aLJPremMatchPaySchema.getMatchSerialNo();
		this.BussType = aLJPremMatchPaySchema.getBussType();
		this.MainBussNo = aLJPremMatchPaySchema.getMainBussNo();
		this.SubBussNo = aLJPremMatchPaySchema.getSubBussNo();
		this.AppntNo = aLJPremMatchPaySchema.getAppntNo();
		this.GrpName = aLJPremMatchPaySchema.getGrpName();
		this.Insurancecom = aLJPremMatchPaySchema.getInsurancecom();
		this.InsurancecomName = aLJPremMatchPaySchema.getInsurancecomName();
		this.BussDate = fDate.getDate( aLJPremMatchPaySchema.getBussDate());
		this.DuePayMoney = aLJPremMatchPaySchema.getDuePayMoney();
		this.CurOutPayMoney = aLJPremMatchPaySchema.getCurOutPayMoney();
		this.ManageCom = aLJPremMatchPaySchema.getManageCom();
		this.ComCode = aLJPremMatchPaySchema.getComCode();
		this.MakeOperator = aLJPremMatchPaySchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLJPremMatchPaySchema.getMakeDate());
		this.MakeTime = aLJPremMatchPaySchema.getMakeTime();
		this.ModifyOperator = aLJPremMatchPaySchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLJPremMatchPaySchema.getModifyDate());
		this.ModifyTime = aLJPremMatchPaySchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LJPremMatchPay表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LJPremMatchPaySchema";
	...
	public LJPremMatchPaySchema getSchema()
	{
		LJPremMatchPaySchema aLJPremMatchPaySchema = new LJPremMatchPaySchema();
		aLJPremMatchPaySchema.setSchema(this);
		return aLJPremMatchPaySchema;
	}

	public LJPremMatchPayDB getDB()
	{
		LJPremMatchPayDB aDBOper = new LJPremMatchPayDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLJPremMatchPay>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			MainBussNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			SubBussNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			...
			Insurancecom = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			InsurancecomName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			BussDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9,SysConst.PACKAGESPILTER));
			DuePayMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,10,SysConst.PACKAGESPILTER))).doubleValue();
			CurOutPayMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,11,SysConst.PACKAGESPILTER))).doubleValue();
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LJPremMatchPaySchema other = (LJPremMatchPaySchema)otherObject;
		return

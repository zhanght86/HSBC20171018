...
/*
 * <p>ClassName: LJTempFeeClassBakDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LJTempFeeClassBakDB extends LJTempFeeClassBakSchema
{
private static Logger logger = Logger.getLogger(LJTempFeeClassBakDB.class);
	...
	**/
	...
	// @Constructor
	public LJTempFeeClassBakDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LJTempFeeClassBak" );
		mflag = true;
	}

	public LJTempFeeClassBakDB()
	{
		con = null;
		db = new DBOper( "LJTempFeeClassBak" );
		mflag = false;
	}
	...
	{
		LJTempFeeClassBakSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LJTempFeeClassBakDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LJTempFeeClassBak WHERE  TempFeeNo = ? AND PayMode = ? AND ChequeNo = ?");
		...
		SQLString sqlObj = new SQLString("LJTempFeeClassBak");
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getTempFeeNo() == null || this.getTempFeeNo().equals("null")) {
				...
				pstmt.setString(36, this.getTempFeeNo());
			}
			if(this.getPayMode() == null || this.getPayMode().equals("null")) {
				pstmt.setNull(37, 1);
			} else {
				pstmt.setString(37, StrTool.space1(this.getPayMode(), 1));
			}
			if(this.getChequeNo() == null || this.getChequeNo().equals("null")) {
				pstmt.setNull(38, 12);
			} else {
				pstmt.setString(38, this.getChequeNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LJTempFeeClassBak(TempFeeNo ,PayMode ,ChequeNo ,PayMoney ,AppntName ,PayDate ,ConfDate ,ApproveDate ,EnterAccDate ,ConfFlag ,SerialNo ,ManageCom ,PolicyCom ,BankCode ,BankAccNo ,AccName ,ConfMakeDate ,ConfMakeTime ,ChequeDate ,Operator ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,InBankCode ,InBankAccNo ,InAccName ,ContCom ,OtherNo ,OtherNoType ,TempFeeNoType ,OperState ,IDType ,IDNo ,OriBankAccNo) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getTempFeeNo() == null || this.getTempFeeNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getTempFeeNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJTempFeeClassBak WHERE  TempFeeNo = ? AND PayMode = ? AND ChequeNo = ?", 
	...
	public LJTempFeeClassBakSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LJTempFeeClassBakSet aLJTempFeeClassBakSet = new LJTempFeeClassBakSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJTempFeeClassBak");
			LJTempFeeClassBakSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJTempFeeClassBakSchema s1 = new LJTempFeeClassBakSchema();
				s1.setSchema(rs,i);
				aLJTempFeeClassBakSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJTempFeeClassBakSet;

	}

	public LJTempFeeClassBakSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJTempFeeClassBakSet aLJTempFeeClassBakSet = new LJTempFeeClassBakSet();
	...
	public LJTempFeeClassBakSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LJTempFeeClassBakSet aLJTempFeeClassBakSet = new LJTempFeeClassBakSet();
	...
	public LJTempFeeClassBakSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJTempFeeClassBakSet aLJTempFeeClassBakSet = new LJTempFeeClassBakSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJTempFeeClassBak " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LJTempFeeClassBakSet
 */
public LJTempFeeClassBakSet getData()
{
    int tCount = 0;
    LJTempFeeClassBakSet tLJTempFeeClassBakSet = new LJTempFeeClassBakSet();
    LJTempFeeClassBakSchema tLJTempFeeClassBakSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJTempFeeClassBakDB";
    ...
    {
        tCount = 1;
        tLJTempFeeClassBakSchema = new LJTempFeeClassBakSchema();
        tLJTempFeeClassBakSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LJTempFeeClassBakSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LJTempFeeClassBakSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCUWMasterBakDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LCUWMasterBakDB extends LCUWMasterBakSchema
{
private static Logger logger = Logger.getLogger(LCUWMasterBakDB.class);
	...
	**/
	...
	// @Constructor
	public LCUWMasterBakDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCUWMasterBak" );
		mflag = true;
	}

	public LCUWMasterBakDB()
	{
		con = null;
		db = new DBOper( "LCUWMasterBak" );
		mflag = false;
	}
	...
	{
		LCUWMasterBakSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCUWMasterBakDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCUWMasterBak WHERE  ProposalNo = ?");
		...
		SQLString sqlObj = new SQLString("LCUWMasterBak");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCUWMasterBak SET  GrpContNo = ? , ContNo = ? , ProposalContNo = ? , PolNo = ? , ProposalNo = ? , UWNo = ? , InsuredNo = ? , InsuredName = ? , AppntNo = ? , AppntName = ? , AgentCode = ? , AgentGroup = ? , PassFlag = ? , UWGrade = ? , AppGrade = ? , PostponeDay = ? , PostponeDate = ? , AutoUWFlag = ? , State = ? , ManageCom = ? , UWIdea = ? , UpReportContent = ? , Operator = ? , HealthFlag = ? , QuesFlag = ? , SpecFlag = ? , AddPremFlag = ? , AddPremReason = ? , ReportFlag = ? , PrintFlag = ? , PrintFlag2 = ? , ChangePolFlag = ? , ChangePolReason = ? , SpecReason = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , SugPassFlag = ? , SugUWIdea = ? , UpReport = ? , BatchNo = ? , UWType = ? WHERE  ProposalNo = ?");
			...
			if(this.getUpReport() == null || this.getUpReport().equals("null")) {
				...
				pstmt.setString(41, this.getUpReport());
			}
			pstmt.setInt(42, this.getBatchNo());
			if(this.getUWType() == null || this.getUWType().equals("null")) {
				pstmt.setNull(43, 12);
			} else {
				pstmt.setString(43, this.getUWType());
			}
			// set where condition
			if(this.getProposalNo() == null || this.getProposalNo().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
				pstmt.setString(44, this.getProposalNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCUWMasterBak(GrpContNo ,ContNo ,ProposalContNo ,PolNo ,ProposalNo ,UWNo ,InsuredNo ,InsuredName ,AppntNo ,AppntName ,AgentCode ,AgentGroup ,PassFlag ,UWGrade ,AppGrade ,PostponeDay ,PostponeDate ,AutoUWFlag ,State ,ManageCom ,UWIdea ,UpReportContent ,Operator ,HealthFlag ,QuesFlag ,SpecFlag ,AddPremFlag ,AddPremReason ,ReportFlag ,PrintFlag ,PrintFlag2 ,ChangePolFlag ,ChangePolReason ,SpecReason ,MakeDate ,MakeTime ,ModifyDate ,ModifyTime ,SugPassFlag ,SugUWIdea ,UpReport ,BatchNo ,UWType) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getGrpContNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCUWMasterBak WHERE  ProposalNo = ?", 
	...
	public LCUWMasterBakSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCUWMasterBakSet aLCUWMasterBakSet = new LCUWMasterBakSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCUWMasterBak");
			LCUWMasterBakSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCUWMasterBakSchema s1 = new LCUWMasterBakSchema();
				s1.setSchema(rs,i);
				aLCUWMasterBakSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCUWMasterBakSet;

	}

	public LCUWMasterBakSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCUWMasterBakSet aLCUWMasterBakSet = new LCUWMasterBakSet();
	...
	public LCUWMasterBakSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCUWMasterBakSet aLCUWMasterBakSet = new LCUWMasterBakSet();
	...
	public LCUWMasterBakSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCUWMasterBakSet aLCUWMasterBakSet = new LCUWMasterBakSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCUWMasterBak " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCUWMasterBakSet
 */
public LCUWMasterBakSet getData()
{
    int tCount = 0;
    LCUWMasterBakSet tLCUWMasterBakSet = new LCUWMasterBakSet();
    LCUWMasterBakSchema tLCUWMasterBakSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCUWMasterBakDB";
    ...
    {
        tCount = 1;
        tLCUWMasterBakSchema = new LCUWMasterBakSchema();
        tLCUWMasterBakSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCUWMasterBakSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCUWMasterBakSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

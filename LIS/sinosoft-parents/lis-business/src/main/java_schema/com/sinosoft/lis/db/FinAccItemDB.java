...
/*
 * <p>ClassName: FinAccItemDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class FinAccItemDB extends FinAccItemSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public FinAccItemDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		FinAccItemSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "FinAccItemDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getAccItemCode() == null || this.getAccItemCode().equals("null")) {
				...
				pstmt.setString(1, this.getAccItemCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAccItemCode() == null || this.getAccItemCode().equals("null")) {
				...
				pstmt.setString(19, this.getAccItemCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM FinAccItem WHERE  AccItemCode = ?", 
	...
	public FinAccItemSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		FinAccItemSet aFinAccItemSet = new FinAccItemSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("FinAccItem");
			FinAccItemSchema aSchema = this.getSchema();
			...
			{
				i++;
				FinAccItemSchema s1 = new FinAccItemSchema();
				s1.setSchema(rs,i);
				aFinAccItemSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aFinAccItemSet;
	}

	public FinAccItemSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		FinAccItemSet aFinAccItemSet = new FinAccItemSet();
	...
	public FinAccItemSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		FinAccItemSet aFinAccItemSet = new FinAccItemSet();
	...
	public FinAccItemSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		FinAccItemSet aFinAccItemSet = new FinAccItemSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update FinAccItem " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return FinAccItemSet
 */
public FinAccItemSet getData()
{
    int tCount = 0;
    FinAccItemSet tFinAccItemSet = new FinAccItemSet();
    FinAccItemSchema tFinAccItemSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "FinAccItemDB";
    ...
    {
        tCount = 1;
        tFinAccItemSchema = new FinAccItemSchema();
        tFinAccItemSchema.setSchema(mResultSet, 1);
        tFinAccItemSet.add(tFinAccItemSchema);
...
public boolean closeData()
{
	...
	public FinAccItemSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public FinAccItemSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

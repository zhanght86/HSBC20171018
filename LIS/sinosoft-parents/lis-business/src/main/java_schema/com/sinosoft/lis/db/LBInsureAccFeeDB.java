...
/*
 * <p>ClassName: LBInsureAccFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LBInsureAccFeeDB extends LBInsureAccFeeSchema
{
private static Logger logger = Logger.getLogger(LBInsureAccFeeDB.class);
	...
	**/
	...
	// @Constructor
	public LBInsureAccFeeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBInsureAccFeeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBInsureAccFeeDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(20, this.getFee());
			pstmt.setDouble(21, this.getFeeUnit());
			if(this.getBalaDate() == null || this.getBalaDate().equals("null")) {
				pstmt.setNull(22, 91);
			} else {
				pstmt.setDate(22, Date.valueOf(this.getBalaDate()));
			}
			if(this.getBalaTime() == null || this.getBalaTime().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
				pstmt.setString(23, this.getBalaTime());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBInsureAccFee WHERE  EdorNo = ? AND PolNo = ? AND InsuAccNo = ?", 
	...
	public LBInsureAccFeeSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBInsureAccFee");
			LBInsureAccFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBInsureAccFeeSchema s1 = new LBInsureAccFeeSchema();
				s1.setSchema(rs,i);
				aLBInsureAccFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBInsureAccFeeSet;

	}

	public LBInsureAccFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBInsureAccFeeSet aLBInsureAccFeeSet = new LBInsureAccFeeSet();
	...
	public LBInsureAccFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBInsureAccFeeSet aLBInsureAccFeeSet = new LBInsureAccFeeSet();
	...
	public LBInsureAccFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBInsureAccFeeSet aLBInsureAccFeeSet = new LBInsureAccFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBInsureAccFee " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBInsureAccFeeSet
 */
public LBInsureAccFeeSet getData()
{
    int tCount = 0;
    LBInsureAccFeeSet tLBInsureAccFeeSet = new LBInsureAccFeeSet();
    LBInsureAccFeeSchema tLBInsureAccFeeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBInsureAccFeeDB";
    ...
    try
    {
        ...
        tLBInsureAccFeeSchema.setSchema(mResultSet, 1);
        tLBInsureAccFeeSet.add(tLBInsureAccFeeSchema);
...
public boolean closeData()
{
	...
	public LBInsureAccFeeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBInsureAccFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

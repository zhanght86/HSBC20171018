...
import com.sinosoft.lis.db.LOBContPlanDetailDB;

/*
 * 与数据库结构不一致，暂不处理
 * <p>ClassName: LOBContPlanDetailSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LOBContPlanDetailSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LOBContPlanDetailSchema.class);
	// @Field
	/** 业务类型 */
	...
	// @Constructor
	public LOBContPlanDetailSchema()
	{
		mErrors = new CErrors();
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LOBContPlanDetailSchema cloned = (LOBContPlanDetailSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LOBContPlanDetailSchema 对象给 Schema 赋值
	* @param: aLOBContPlanDetailSchema LOBContPlanDetailSchema
	**/
	public void setSchema(LOBContPlanDetailSchema aLOBContPlanDetailSchema)
	{
		this.BussType = aLOBContPlanDetailSchema.getBussType();
		this.BussNo = aLOBContPlanDetailSchema.getBussNo();
		this.PolicyNo = aLOBContPlanDetailSchema.getPolicyNo();
		this.PropNo = aLOBContPlanDetailSchema.getPropNo();
		this.SysPlanCode = aLOBContPlanDetailSchema.getSysPlanCode();
		this.PlanCode = aLOBContPlanDetailSchema.getPlanCode();
		this.RiskCode = aLOBContPlanDetailSchema.getRiskCode();
		this.DutyCode = aLOBContPlanDetailSchema.getDutyCode();
		this.AmntType = aLOBContPlanDetailSchema.getAmntType();
		this.FixedAmnt = aLOBContPlanDetailSchema.getFixedAmnt();
		this.SalaryMult = aLOBContPlanDetailSchema.getSalaryMult();
		this.MaxAmnt = aLOBContPlanDetailSchema.getMaxAmnt();
		this.MinAmnt = aLOBContPlanDetailSchema.getMinAmnt();
		this.ExceptPremType = aLOBContPlanDetailSchema.getExceptPremType();
		this.ExceptPrem = aLOBContPlanDetailSchema.getExceptPrem();
		this.StandValue = aLOBContPlanDetailSchema.getStandValue();
		this.InitPrem = aLOBContPlanDetailSchema.getInitPrem();
		this.ExceptYield = aLOBContPlanDetailSchema.getExceptYield();
		this.FinalValue = aLOBContPlanDetailSchema.getFinalValue();
		this.ChangeType = aLOBContPlanDetailSchema.getChangeType();
		this.RelaShareFlag = aLOBContPlanDetailSchema.getRelaShareFlag();
		this.Remark = aLOBContPlanDetailSchema.getRemark();
		this.MakeOperator = aLOBContPlanDetailSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLOBContPlanDetailSchema.getMakeDate());
		this.MakeTime = aLOBContPlanDetailSchema.getMakeTime();
		this.ModifyOperator = aLOBContPlanDetailSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLOBContPlanDetailSchema.getModifyDate());
		this.ModifyTime = aLOBContPlanDetailSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LOBContPlanDetail表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LOBContPlanDetailSchema";
	...
	public LOBContPlanDetailSchema getSchema()
	{
		LOBContPlanDetailSchema aLOBContPlanDetailSchema = new LOBContPlanDetailSchema();
		aLOBContPlanDetailSchema.setSchema(this);
		return aLOBContPlanDetailSchema;
	}

	public LOBContPlanDetailDB getDB()
	{
		LOBContPlanDetailDB aDBOper = new LOBContPlanDetailDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBContPlanDetail描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBContPlanDetail>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			AmntType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			FixedAmnt = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			SalaryMult = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
			MaxAmnt = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 12, SysConst.PACKAGESPILTER );
			MinAmnt = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			ExceptPremType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			ExceptPrem = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			StandValue = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 16, SysConst.PACKAGESPILTER );
			InitPrem = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 17, SysConst.PACKAGESPILTER );
			ExceptYield = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 18, SysConst.PACKAGESPILTER );
			FinalValue = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 19, SysConst.PACKAGESPILTER );
			ChangeType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20, SysConst.PACKAGESPILTER );
			RelaShareFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 21, SysConst.PACKAGESPILTER );
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LOBContPlanDetailSchema other = (LOBContPlanDetailSchema)otherObject;
		return

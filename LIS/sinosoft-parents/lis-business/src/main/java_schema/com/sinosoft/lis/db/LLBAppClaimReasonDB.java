...
/*
 * <p>ClassName: LLBAppClaimReasonDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLBAppClaimReasonDB extends LLBAppClaimReasonSchema
{
private static Logger logger = Logger.getLogger(LLBAppClaimReasonDB.class);
	...
	**/
	...
	// @Constructor
	public LLBAppClaimReasonDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLBAppClaimReasonSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLBAppClaimReasonDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getReasonType() == null || this.getReasonType().equals("null")) {
				...
				pstmt.setString(6, this.getReasonType());
			}
			...
			if(this.getAffixGetDate() == null || this.getAffixGetDate().equals("null")) {
				...
				pstmt.setDate(8, Date.valueOf(this.getAffixGetDate()));
			}
			...
			if(this.getBackNo() == null || this.getBackNo().equals("null")) {
				...
				pstmt.setString(15, this.getBackNo());
			}
			// set where condition
			if(this.getEditNo() == null || this.getEditNo().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
				pstmt.setString(16, this.getEditNo());
			}
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getCaseNo());
			}
			if(this.getRgtNo() == null || this.getRgtNo().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
				pstmt.setString(18, this.getRgtNo());
			}
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				pstmt.setNull(19, 12);
			} else {
			...
			if(this.getReasonCode() == null || this.getReasonCode().equals("null")) {
				...
				pstmt.setString(20, this.getReasonCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLBAppClaimReason WHERE  EditNo = ? AND CaseNo = ? AND RgtNo = ? AND CustomerNo = ? AND ReasonCode = ?", 
	...
	public LLBAppClaimReasonSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLBAppClaimReasonSet aLLBAppClaimReasonSet = new LLBAppClaimReasonSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLBAppClaimReason");
			LLBAppClaimReasonSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLBAppClaimReasonSchema s1 = new LLBAppClaimReasonSchema();
				s1.setSchema(rs,i);
				aLLBAppClaimReasonSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLBAppClaimReasonSet;

	}

	public LLBAppClaimReasonSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLBAppClaimReasonSet aLLBAppClaimReasonSet = new LLBAppClaimReasonSet();
	...
	public LLBAppClaimReasonSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLBAppClaimReasonSet aLLBAppClaimReasonSet = new LLBAppClaimReasonSet();
	...
	public LLBAppClaimReasonSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLBAppClaimReasonSet aLLBAppClaimReasonSet = new LLBAppClaimReasonSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLBAppClaimReason " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLBAppClaimReasonSet
 */
public LLBAppClaimReasonSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLLBAppClaimReasonSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLBAppClaimReasonSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLBAppClaimReasonSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

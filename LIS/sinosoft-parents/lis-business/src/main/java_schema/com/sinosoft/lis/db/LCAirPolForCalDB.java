...
/*
 * <p>ClassName: LCAirPolForCalDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 短期险平台接口表
 */
public class LCAirPolForCalDB extends LCAirPolForCalSchema
{
private static Logger logger = Logger.getLogger(LCAirPolForCalDB.class);
	...
	**/
	...
	// @Constructor
	public LCAirPolForCalDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCAirPolForCalSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCAirPolForCalDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCAirPolForCal WHERE  PolNo = ?", 
	...
	public LCAirPolForCalSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCAirPolForCalSet aLCAirPolForCalSet = new LCAirPolForCalSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCAirPolForCal");
			LCAirPolForCalSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCAirPolForCalSchema s1 = new LCAirPolForCalSchema();
				s1.setSchema(rs,i);
				aLCAirPolForCalSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCAirPolForCalSet;

	}

	public LCAirPolForCalSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCAirPolForCalSet aLCAirPolForCalSet = new LCAirPolForCalSet();
	...
	public LCAirPolForCalSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCAirPolForCalSet aLCAirPolForCalSet = new LCAirPolForCalSet();
	...
	public LCAirPolForCalSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCAirPolForCalSet aLCAirPolForCalSet = new LCAirPolForCalSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCAirPolForCal " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCAirPolForCalSet
 */
public LCAirPolForCalSet getData()
{
    int tCount = 0;
    LCAirPolForCalSet tLCAirPolForCalSet = new LCAirPolForCalSet();
    LCAirPolForCalSchema tLCAirPolForCalSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCAirPolForCalDB";
    ...
    try
    {
        ...
        tLCAirPolForCalSchema.setSchema(mResultSet, 1);
        tLCAirPolForCalSet.add(tLCAirPolForCalSchema);
...
public boolean closeData()
{
	...
	public LCAirPolForCalSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCAirPolForCalSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

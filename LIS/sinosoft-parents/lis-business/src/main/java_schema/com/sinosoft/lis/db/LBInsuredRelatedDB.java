...
/*
 * <p>ClassName: LBInsuredRelatedDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统未整理模型
 */
public class LBInsuredRelatedDB extends LBInsuredRelatedSchema
{
private static Logger logger = Logger.getLogger(LBInsuredRelatedDB.class);
	...
	**/
	...
	// @Constructor
	public LBInsuredRelatedDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBInsuredRelatedSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBInsuredRelatedDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getSequenceNo() == null || this.getSequenceNo().equals("null")) {
				...
				pstmt.setString(17, this.getSequenceNo());
			}
			// set where condition
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(18, 12);
			...
			if(this.getMainCustomerNo() == null || this.getMainCustomerNo().equals("null")) {
				...
				pstmt.setString(20, this.getMainCustomerNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBInsuredRelated WHERE  PolNo = ? AND CustomerNo = ? AND MainCustomerNo = ?", 
	...
	public LBInsuredRelatedSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBInsuredRelated");
			LBInsuredRelatedSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBInsuredRelatedSchema s1 = new LBInsuredRelatedSchema();
				s1.setSchema(rs,i);
				aLBInsuredRelatedSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBInsuredRelatedSet;

	}

	public LBInsuredRelatedSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBInsuredRelatedSet aLBInsuredRelatedSet = new LBInsuredRelatedSet();
	...
	public LBInsuredRelatedSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBInsuredRelatedSet aLBInsuredRelatedSet = new LBInsuredRelatedSet();
	...
	public LBInsuredRelatedSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBInsuredRelatedSet aLBInsuredRelatedSet = new LBInsuredRelatedSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBInsuredRelated " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBInsuredRelatedSet
 */
public LBInsuredRelatedSet getData()
{
    int tCount = 0;
    LBInsuredRelatedSet tLBInsuredRelatedSet = new LBInsuredRelatedSet();
    LBInsuredRelatedSchema tLBInsuredRelatedSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBInsuredRelatedDB";
    ...
    try
    {
        ...
        tLBInsuredRelatedSchema.setSchema(mResultSet, 1);
        tLBInsuredRelatedSet.add(tLBInsuredRelatedSchema);
...
public boolean closeData()
{
	...
	public LBInsuredRelatedSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBInsuredRelatedSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

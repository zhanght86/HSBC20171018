...
/*
 * <p>ClassName: LZCardTempDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 单证管理
 */
public class LZCardTempDB extends LZCardTempSchema
{
private static Logger logger = Logger.getLogger(LZCardTempDB.class);
	...
	**/
	...
	// @Constructor
	public LZCardTempDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LZCardTempSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LZCardTempDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getQueryOper() == null || this.getQueryOper().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getQueryOper());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getQueryOper() == null || this.getQueryOper().equals("null")) {
				...
				pstmt.setString(28, this.getQueryOper());
			}
			// set where condition
			if(this.getCertifyCode() == null || this.getCertifyCode().equals("null")) {
				pstmt.setNull(29, 12);
			} else {
				pstmt.setString(29, this.getCertifyCode());
			}
			if(this.getSubCode() == null || this.getSubCode().equals("null")) {
				pstmt.setNull(30, 12);
			} else {
				pstmt.setString(30, this.getSubCode());
			}
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(31, 12);
			} else {
			...
			if(this.getRiskVersion() == null || this.getRiskVersion().equals("null")) {
				...
				pstmt.setString(32, this.getRiskVersion());
			}
			if(this.getStartNo() == null || this.getStartNo().equals("null")) {
				pstmt.setNull(33, 12);
			} else {
				pstmt.setString(33, this.getStartNo());
			}
			if(this.getEndNo() == null || this.getEndNo().equals("null")) {
				pstmt.setNull(34, 12);
			} else {
				pstmt.setString(34, this.getEndNo());
			}
			if(this.getSendOutCom() == null || this.getSendOutCom().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getSendOutCom());
			}
			if(this.getReceiveCom() == null || this.getReceiveCom().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
				pstmt.setString(36, this.getReceiveCom());
			}
			if(this.getMakeDate() == null || this.getMakeDate().equals("null")) {
				pstmt.setNull(37, 91);
			} else {
			...
			if(this.getMakeTime() == null || this.getMakeTime().equals("null")) {
				...
				pstmt.setString(38, StrTool.space1(this.getMakeTime(), 8));
			}
			if(this.getQueryOper() == null || this.getQueryOper().equals("null")) {
				pstmt.setNull(39, 12);
			} else {
				pstmt.setString(39, this.getQueryOper());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LZCardTemp WHERE  CertifyCode = ? AND SubCode = ? AND RiskCode = ? AND RiskVersion = ? AND StartNo = ? AND EndNo = ? AND SendOutCom = ? AND ReceiveCom = ? AND MakeDate = ? AND MakeTime = ? AND QueryOper = ?", 
	...
	public LZCardTempSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardTempSet aLZCardTempSet = new LZCardTempSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LZCardTemp");
			LZCardTempSchema aSchema = this.getSchema();
			...
			{
				i++;
				LZCardTempSchema s1 = new LZCardTempSchema();
				s1.setSchema(rs,i);
				aLZCardTempSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLZCardTempSet;

	}

	public LZCardTempSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardTempSet aLZCardTempSet = new LZCardTempSet();
	...
	public LZCardTempSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LZCardTempSet aLZCardTempSet = new LZCardTempSet();
	...
	public LZCardTempSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LZCardTempSet aLZCardTempSet = new LZCardTempSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LZCardTemp " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LZCardTempSet
 */
public LZCardTempSet getData()
{
    int tCount = 0;
    LZCardTempSet tLZCardTempSet = new LZCardTempSet();
    LZCardTempSchema tLZCardTempSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LZCardTempDB";
    ...
    {
        tCount = 1;
        tLZCardTempSchema = new LZCardTempSchema();
        tLZCardTempSchema.setSchema(mResultSet, 1);
        tLZCardTempSet.add(tLZCardTempSchema);
...
public boolean closeData()
{
	...
	public LZCardTempSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LZCardTempSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

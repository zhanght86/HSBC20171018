...
/*
 * <p>ClassName: LPUWErrorDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPUWErrorDB extends LPUWErrorSchema
{
private static Logger logger = Logger.getLogger(LPUWErrorDB.class);
	...
	**/
	...
	// @Constructor
	public LPUWErrorDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPUWErrorSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getInsuredName() == null || this.getInsuredName().equals("null")) {
				...
				pstmt.setString(11, this.getInsuredName());
			}
			...
			if(this.getUWRuleCode() == null || this.getUWRuleCode().equals("null")) {
				...
				pstmt.setString(15, this.getUWRuleCode());
			}
			if(this.getUWError() == null || this.getUWError().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
				pstmt.setString(16, this.getUWError());
			}
			if(this.getCurrValue() == null || this.getCurrValue().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getCurrValue());
			}
			...
			if(this.getUWPassFlag() == null || this.getUWPassFlag().equals("null")) {
				...
				pstmt.setString(20, this.getUWPassFlag());
			}
			...
			if(this.getSugPassFlag() == null || this.getSugPassFlag().equals("null")) {
				...
				pstmt.setString(22, this.getSugPassFlag());
			}
			// set where condition
			if(this.getEdorNo() == null || this.getEdorNo().equals("null")) {
				pstmt.setNull(23, 12);
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPUWError WHERE  EdorNo = ? AND EdorType = ? AND PolNo = ? AND UWNo = ? AND SerialNo = ?", 
	...
	public LPUWErrorSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPUWErrorSet aLPUWErrorSet = new LPUWErrorSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPUWError");
			LPUWErrorSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPUWErrorSchema s1 = new LPUWErrorSchema();
				s1.setSchema(rs,i);
				aLPUWErrorSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPUWErrorSet;

	}

	public LPUWErrorSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPUWErrorSet aLPUWErrorSet = new LPUWErrorSet();
	...
	public LPUWErrorSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPUWErrorSet aLPUWErrorSet = new LPUWErrorSet();
	...
	public LPUWErrorSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPUWErrorSet aLPUWErrorSet = new LPUWErrorSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPUWError " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LPUWErrorSet
 */
public LPUWErrorSet getData()
{
    int tCount = 0;
    LPUWErrorSet tLPUWErrorSet = new LPUWErrorSet();
    LPUWErrorSchema tLPUWErrorSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPUWErrorDB";
    ...
    {
        tCount = 1;
        tLPUWErrorSchema = new LPUWErrorSchema();
        tLPUWErrorSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LPUWErrorSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPUWErrorSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCProposalDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 新单管理
 */
public class LCProposalDB extends LCProposalSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCProposalDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCProposalSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCProposalDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPolicyFlag() == null || this.getPolicyFlag().equals("null")) {
				...
				pstmt.setString(9, this.getPolicyFlag());
			}
			...
			if(this.getGrpPropNo() == null || this.getGrpPropNo().equals("null")) {
				...
				pstmt.setString(26, this.getGrpPropNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCProposal WHERE  GrpPropNo = ?", 
	...
	public LCProposalSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCProposalSet aLCProposalSet = new LCProposalSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCProposal");
			LCProposalSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCProposalSchema s1 = new LCProposalSchema();
				s1.setSchema(rs,i);
				aLCProposalSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCProposalSet;
	}

	public LCProposalSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCProposalSet aLCProposalSet = new LCProposalSet();
	...
	public LCProposalSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCProposalSet aLCProposalSet = new LCProposalSet();
	...
	public LCProposalSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCProposalSet aLCProposalSet = new LCProposalSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCProposal " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCProposalSet
 */
public LCProposalSet getData()
{
    int tCount = 0;
    LCProposalSet tLCProposalSet = new LCProposalSet();
    LCProposalSchema tLCProposalSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCProposalDB";
    ...
    {
        tCount = 1;
        tLCProposalSchema = new LCProposalSchema();
        tLCProposalSchema.setSchema(mResultSet, 1);
        tLCProposalSet.add(tLCProposalSchema);
...
public boolean closeData()
{
	...
	public LCProposalSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCProposalSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

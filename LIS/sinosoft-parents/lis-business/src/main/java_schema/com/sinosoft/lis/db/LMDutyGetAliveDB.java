...
/*
 * <p>ClassName: LMDutyGetAliveDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 新险种定义
 */
public class LMDutyGetAliveDB extends LMDutyGetAliveSchema
{
private static Logger logger = Logger.getLogger(LMDutyGetAliveDB.class);
	...
	**/
	...
	// @Constructor
	public LMDutyGetAliveDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LMDutyGetAliveSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LMDutyGetAliveDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getGetDutyName() == null || this.getGetDutyName().equals("null")) {
				...
				pstmt.setString(2, this.getGetDutyName());
			}
			if(this.getGetDutyKind() == null || this.getGetDutyKind().equals("null")) {
				pstmt.setNull(3, 12);
			} else {
			...
			pstmt.setInt(4, this.getGetIntv());
			...
			if(this.getGetDutyKind() == null || this.getGetDutyKind().equals("null")) {
				...
				pstmt.setString(36, this.getGetDutyKind());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LMDutyGetAlive WHERE  GetDutyCode = ? AND GetDutyKind = ?", 
	...
	public LMDutyGetAliveSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LMDutyGetAliveSet aLMDutyGetAliveSet = new LMDutyGetAliveSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LMDutyGetAlive");
			LMDutyGetAliveSchema aSchema = this.getSchema();
			...
			{
				i++;
				LMDutyGetAliveSchema s1 = new LMDutyGetAliveSchema();
				s1.setSchema(rs,i);
				aLMDutyGetAliveSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLMDutyGetAliveSet;

	}

	public LMDutyGetAliveSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMDutyGetAliveSet aLMDutyGetAliveSet = new LMDutyGetAliveSet();
	...
	public LMDutyGetAliveSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LMDutyGetAliveSet aLMDutyGetAliveSet = new LMDutyGetAliveSet();
	...
	public LMDutyGetAliveSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LMDutyGetAliveSet aLMDutyGetAliveSet = new LMDutyGetAliveSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LMDutyGetAlive " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 */
public LMDutyGetAliveSet getData()
{
    int tCount = 0;
    LMDutyGetAliveSet tLMDutyGetAliveSet = new LMDutyGetAliveSet();
    LMDutyGetAliveSchema tLMDutyGetAliveSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LMDutyGetAliveDB";
    ...
    {
        tCount = 1;
        tLMDutyGetAliveSchema = new LMDutyGetAliveSchema();
        tLMDutyGetAliveSchema.setSchema(mResultSet, 1);
        tLMDutyGetAliveSet.add(tLMDutyGetAliveSchema);
...
public boolean closeData()
{
	...
	public LMDutyGetAliveSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LMDutyGetAliveSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

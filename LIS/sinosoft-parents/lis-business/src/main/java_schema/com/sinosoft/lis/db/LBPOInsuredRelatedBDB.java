...
/*
 * <p>ClassName: LBPOInsuredRelatedBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LBPOInsuredRelatedBDB extends LBPOInsuredRelatedBSchema
{
private static Logger logger = Logger.getLogger(LBPOInsuredRelatedBDB.class);
	...
	**/
	...
	// @Constructor
	public LBPOInsuredRelatedBDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LBPOInsuredRelatedBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LBPOInsuredRelatedBDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getMainCustomerNo() == null || this.getMainCustomerNo().equals("null")) {
				...
				pstmt.setString(4, this.getMainCustomerNo());
			}
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAddressNo() == null || this.getAddressNo().equals("null")) {
				...
				pstmt.setString(8, this.getAddressNo());
			}
			...
			if(this.getBirthday() == null || this.getBirthday().equals("null")) {
				...
				pstmt.setString(11, this.getBirthday());
			}
			...
			if(this.getMakeDate() == null || this.getMakeDate().equals("null")) {
				...
				pstmt.setString(15, this.getMakeDate());
			}
			...
			if(this.getModifyDate() == null || this.getModifyDate().equals("null")) {
				...
				pstmt.setString(17, this.getModifyDate());
			}
			...
			if(this.getSequenceNo() == null || this.getSequenceNo().equals("null")) {
				...
				pstmt.setString(19, this.getSequenceNo());
			}
			if(this.getTransDate() == null || this.getTransDate().equals("null")) {
				pstmt.setNull(20, 91);
			} else {
				pstmt.setDate(20, Date.valueOf(this.getTransDate()));
			}
			if(this.getTransTime() == null || this.getTransTime().equals("null")) {
				pstmt.setNull(21, 1);
			} else {
				pstmt.setString(21, this.getTransTime());
			}
			if(this.getTransOperator() == null || this.getTransOperator().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getTransOperator());
			}
			// set where condition
			if(this.getIDX() == null || this.getIDX().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
				pstmt.setString(23, this.getIDX());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
			...
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				...
				pstmt.setString(25, this.getCustomerNo());
			}
			if(this.getMainCustomerNo() == null || this.getMainCustomerNo().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getMainCustomerNo());
			}
			pstmt.setInt(27, this.getInputNo());
			if(this.getFillNo() == null || this.getFillNo().equals("null")) {
				pstmt.setNull(28, 12);
			} else {
				pstmt.setString(28, this.getFillNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LBPOInsuredRelatedB WHERE  IDX = ? AND PolNo = ? AND CustomerNo = ? AND MainCustomerNo = ? AND InputNo = ? AND FillNo = ?", 
	...
	public LBPOInsuredRelatedBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOInsuredRelatedBSet aLBPOInsuredRelatedBSet = new LBPOInsuredRelatedBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LBPOInsuredRelatedB");
			LBPOInsuredRelatedBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LBPOInsuredRelatedBSchema s1 = new LBPOInsuredRelatedBSchema();
				s1.setSchema(rs,i);
				aLBPOInsuredRelatedBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLBPOInsuredRelatedBSet;

	}

	public LBPOInsuredRelatedBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOInsuredRelatedBSet aLBPOInsuredRelatedBSet = new LBPOInsuredRelatedBSet();
	...
	public LBPOInsuredRelatedBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LBPOInsuredRelatedBSet aLBPOInsuredRelatedBSet = new LBPOInsuredRelatedBSet();
	...
	public LBPOInsuredRelatedBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LBPOInsuredRelatedBSet aLBPOInsuredRelatedBSet = new LBPOInsuredRelatedBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LBPOInsuredRelatedB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LBPOInsuredRelatedBSet
 */
public LBPOInsuredRelatedBSet getData()
{
    int tCount = 0;
    LBPOInsuredRelatedBSet tLBPOInsuredRelatedBSet = new LBPOInsuredRelatedBSet();
    LBPOInsuredRelatedBSchema tLBPOInsuredRelatedBSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LBPOInsuredRelatedBDB";
    ...
    {
        tCount = 1;
        tLBPOInsuredRelatedBSchema = new LBPOInsuredRelatedBSchema();
        tLBPOInsuredRelatedBSchema.setSchema(mResultSet, 1);
        tLBPOInsuredRelatedBSet.add(tLBPOInsuredRelatedBSchema);
    ...
    return tLBPOInsuredRelatedBSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LBPOInsuredRelatedBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LBPOInsuredRelatedBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
/*
 * <p>ClassName: LCPrem_1DB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCPrem_1DB extends LCPrem_1Schema
{
private static Logger logger = Logger.getLogger(LCPrem_1DB.class);
	...
	**/
	...
	// @Constructor
	public LCPrem_1DB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCPrem_1Schema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPayStartDate() == null || this.getPayStartDate().equals("null")) {
				...
				pstmt.setDate(4, Date.valueOf(this.getPayStartDate()));
			}
			if(this.getPaytoDate() == null || this.getPaytoDate().equals("null")) {
				pstmt.setNull(5, 91);
			} else {
				pstmt.setDate(5, Date.valueOf(this.getPaytoDate()));
			}
			if(this.getUrgePayFlag() == null || this.getUrgePayFlag().equals("null")) {
				pstmt.setNull(6, 1);
			} else {
				pstmt.setString(6, this.getUrgePayFlag());
			}
			...
			pstmt.setInt(10, this.getBankPayInterval());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCPrem_1 WHERE  PolNo = ? AND DutyCode = ? AND PayPlanCode = ?", 
	...
	public LCPrem_1Set query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPrem_1Set aLCPrem_1Set = new LCPrem_1Set();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCPrem_1");
			LCPrem_1Schema aSchema = this.getSchema();
			...
			{
				i++;
				LCPrem_1Schema s1 = new LCPrem_1Schema();
				s1.setSchema(rs,i);
				aLCPrem_1Set.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LCPrem_1Set executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPrem_1Set aLCPrem_1Set = new LCPrem_1Set();
	...
	public LCPrem_1Set query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCPrem_1Set aLCPrem_1Set = new LCPrem_1Set();
	...
	public LCPrem_1Set executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCPrem_1Set aLCPrem_1Set = new LCPrem_1Set();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCPrem_1 " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LCPrem_1Set
 */
public LCPrem_1Set getData()
{
    int tCount = 0;
    LCPrem_1Set tLCPrem_1Set = new LCPrem_1Set();
    LCPrem_1Schema tLCPrem_1Schema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCPrem_1DB";
    ...
    {
        tCount = 1;
        tLCPrem_1Schema = new LCPrem_1Schema();
        tLCPrem_1Schema.setSchema(mResultSet, 1);
        tLCPrem_1Set.add(tLCPrem_1Schema);
...
public boolean closeData()
{
	...
	public LCPrem_1Set executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCPrem_1Set executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

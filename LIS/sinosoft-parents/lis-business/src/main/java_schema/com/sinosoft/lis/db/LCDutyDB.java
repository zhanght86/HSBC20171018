...
/*
 * <p>ClassName: LCDutyDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCDutyDB extends LCDutySchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCDutyDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LCDuty" );
		mflag = true;
	}
	...
	{
		LCDutySchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LCDuty WHERE  PolNo = ? AND DutyCode = ?");
		...
		SQLString sqlObj = new SQLString("LCDuty");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LCDuty SET  PolNo = ? , DutyCode = ? , ContNo = ? , Mult = ? , StandPrem = ? , Prem = ? , SumPrem = ? , Amnt = ? , RiskAmnt = ? , PayIntv = ? , PayYears = ? , Years = ? , FloatRate = ? , FirstPayDate = ? , FirstMonth = ? , PaytoDate = ? , PayEndDate = ? , PayEndYearFlag = ? , PayEndYear = ? , GetYearFlag = ? , GetYear = ? , InsuYearFlag = ? , InsuYear = ? , AcciYearFlag = ? , AcciYear = ? , EndDate = ? , AcciEndDate = ? , FreeFlag = ? , FreeRate = ? , FreeStartDate = ? , FreeEndDate = ? , GetStartDate = ? , GetStartType = ? , LiveGetMode = ? , DeadGetMode = ? , BonusGetMode = ? , SSFlag = ? , PeakLine = ? , GetLimit = ? , GetRate = ? , CalRule = ? , PremToAmnt = ? , StandbyFlag1 = ? , StandbyFlag2 = ? , StandbyFlag3 = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , CValiDate = ? , GetIntv = ? , AscriptionRuleCode = ? , PayRuleCode = ? , Currency = ? , ContPlanCode = ? , PlanCode = ? , RiskCode = ? , PeriodPrem = ? , ConfDate = ? , State = ? , ManageCom = ? , ComCode = ? , ModifyOperator = ? WHERE  PolNo = ? AND DutyCode = ?");
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getPolNo());
			...
			if(this.getContPlanCode() == null || this.getContPlanCode().equals("null")) {
				...
				pstmt.setString(56, this.getContPlanCode());
			}
			if(this.getPlanCode() == null || this.getPlanCode().equals("null")) {
				pstmt.setNull(57, 12);
			} else {
				pstmt.setString(57, this.getPlanCode());
			}
			if(this.getRiskCode() == null || this.getRiskCode().equals("null")) {
				pstmt.setNull(58, 12);
			} else {
				pstmt.setString(58, this.getRiskCode());
			}
			pstmt.setDouble(59, this.getPeriodPrem());
			if(this.getConfDate() == null || this.getConfDate().equals("null")) {
				pstmt.setNull(60, 91);
			} else {
				pstmt.setDate(60, Date.valueOf(this.getConfDate()));
			}
			...
			if(this.getManageCom() == null || this.getManageCom().equals("null")) {
				...
				pstmt.setString(62, this.getManageCom());
			}
			if(this.getComCode() == null || this.getComCode().equals("null")) {
				pstmt.setNull(63, 12);
			} else {
				pstmt.setString(63, this.getComCode());
			}
			if(this.getModifyOperator() == null || this.getModifyOperator().equals("null")) {
				pstmt.setNull(64, 12);
			} else {
				pstmt.setString(64, this.getModifyOperator());
			}
			// set where condition
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(65, 12);
			} else {
				pstmt.setString(65, this.getPolNo());
			}
			if(this.getDutyCode() == null || this.getDutyCode().equals("null")) {
				pstmt.setNull(66, 12);
			} else {
				pstmt.setString(66, this.getDutyCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LCDuty VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getPolNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCDuty WHERE  PolNo = ? AND DutyCode = ?", 
	...
	public LCDutySet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCDuty");
			LCDutySchema aSchema = this.getSchema();
			...
			{
				i++;
				LCDutySchema s1 = new LCDutySchema();
				s1.setSchema(rs,i);
				aLCDutySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LCDutySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCDutySet aLCDutySet = new LCDutySet();
	...
	public LCDutySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCDutySet aLCDutySet = new LCDutySet();
	...
	public LCDutySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCDutySet aLCDutySet = new LCDutySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCDuty " + sqlObj.getUpdPart() + " where " + strWherePart;
...
public LCDutySet getData()
{
    ...
    try
    {
        ...
        tLCDutySchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LCDutySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCDutySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
import com.sinosoft.lis.vschema.LQGetSet;
import com.sinosoft.lis.pubfun.PubFun;
import com.sinosoft.utility.*;

/*
 * <p>ClassName: LQGetDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LQGetDB extends LQGetSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LQGetDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LQGet" );
		mflag = true;
	}
	...
	{
		LQGetSchema tSchema = this.getSchema();
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LQGet WHERE  AskPriceNo = ? AND AskNo = ? AND ClassNo = ? AND DutyCode = ? AND GetDutyCode = ?");
		...
		SQLString sqlObj = new SQLString("LQGet");
	...
	public boolean update()
	{
		...
		try
		{
			pstmt = con.prepareStatement("UPDATE LQGet SET  AskPriceNo = ? , AskNo = ? , ClassNo = ? , DutyCode = ? , GetDutyCode = ? , GetDutyKind = ? , InsuredNo = ? , GetMode = ? , GetLimit = ? , GetRate = ? , UrgeGetFlag = ? , LiveGetType = ? , AddRate = ? , CanGet = ? , NeedAcc = ? , NeedCancelAcc = ? , StandMoney = ? , ActuGet = ? , SumMoney = ? , GetIntv = ? , GettoDate = ? , GetStartDate = ? , GetEndDate = ? , BalaDate = ? , State = ? , ManageCom = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyTime = ? , ModifyDate = ? , GetEndState = ? , Currency = ? WHERE  AskPriceNo = ? AND AskNo = ? AND ClassNo = ? AND DutyCode = ? AND GetDutyCode = ?");
			if(this.getAskPriceNo() == null || this.getAskPriceNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getAskPriceNo());
			...
			if(this.getAskPriceNo() == null || this.getAskPriceNo().equals("null")) {
				...
				pstmt.setString(34, this.getAskPriceNo());
			}
			if(this.getAskNo() == null || this.getAskNo().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getAskNo());
			}
			if(this.getClassNo() == null || this.getClassNo().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
				pstmt.setString(36, this.getClassNo());
			}
			if(this.getDutyCode() == null || this.getDutyCode().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LQGet VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getAskPriceNo() == null || this.getAskPriceNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getAskPriceNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LQGet WHERE  AskPriceNo = ? AND AskNo = ? AND ClassNo = ? AND DutyCode = ? AND GetDutyCode = ?", 
	...
	public LQGetSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LQGetSet aLQGetSet = new LQGetSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LQGet");
			LQGetSchema aSchema = this.getSchema();
			...
			{
				i++;
				LQGetSchema s1 = new LQGetSchema();
				s1.setSchema(rs,i);
				aLQGetSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
	...
	public LQGetSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LQGetSet aLQGetSet = new LQGetSet();
	...
	public LQGetSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LQGetSet aLQGetSet = new LQGetSet();
	...
	public LQGetSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LQGetSet aLQGetSet = new LQGetSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LQGet " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LQGetSet
 */
public LQGetSet getData()
{
    int tCount = 0;
    LQGetSet tLQGetSet = new LQGetSet();
    LQGetSchema tLQGetSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LQGetDB";
    ...
    {
        tCount = 1;
        tLQGetSchema = new LQGetSchema();
        tLQGetSchema.setSchema(mResultSet, 1);
        tLQGetSet.add(tLQGetSchema);
...
public boolean closeData()
{
	...
	public LQGetSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LQGetSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

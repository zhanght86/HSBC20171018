...
import com.sinosoft.lis.db.LLCUWErrorDB;

/*
 * <p>ClassName: LLCUWErrorSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 泰康未整理PDM
 */
public class LLCUWErrorSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLCUWErrorSchema.class);
	// @Field
	/** 分案号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLCUWErrorSchema cloned = (LLCUWErrorSchema)super.clone();
	...
	/**
	* 使用另外一个 LLCUWErrorSchema 对象给 Schema 赋值
	* @param: aLLCUWErrorSchema LLCUWErrorSchema
	**/
	public void setSchema(LLCUWErrorSchema aLLCUWErrorSchema)
	{
		this.CaseNo = aLLCUWErrorSchema.getCaseNo();
		this.GrpContNo = aLLCUWErrorSchema.getGrpContNo();
		this.ContNo = aLLCUWErrorSchema.getContNo();
		this.ProposalContNo = aLLCUWErrorSchema.getProposalContNo();
		this.SerialNo = aLLCUWErrorSchema.getSerialNo();
		this.UWNo = aLLCUWErrorSchema.getUWNo();
		this.InsuredNo = aLLCUWErrorSchema.getInsuredNo();
		this.InsuredName = aLLCUWErrorSchema.getInsuredName();
		this.AppntNo = aLLCUWErrorSchema.getAppntNo();
		this.AppntName = aLLCUWErrorSchema.getAppntName();
		this.ManageCom = aLLCUWErrorSchema.getManageCom();
		this.UWRuleCode = aLLCUWErrorSchema.getUWRuleCode();
		this.UWError = aLLCUWErrorSchema.getUWError();
		this.CurrValue = aLLCUWErrorSchema.getCurrValue();
		this.ModifyDate = fDate.getDate( aLLCUWErrorSchema.getModifyDate());
		this.ModifyTime = aLLCUWErrorSchema.getModifyTime();
		this.UWPassFlag = aLLCUWErrorSchema.getUWPassFlag();
		this.UWGrade = aLLCUWErrorSchema.getUWGrade();
		this.SugPassFlag = aLLCUWErrorSchema.getSugPassFlag();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLCUWError表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLCUWErrorSchema";
	...
	public LLCUWErrorSchema getSchema()
	{
		LLCUWErrorSchema aLLCUWErrorSchema = new LLCUWErrorSchema();
		aLLCUWErrorSchema.setSchema(this);
		return aLLCUWErrorSchema;
	}

	public LLCUWErrorDB getDB()
	{
		LLCUWErrorDB aDBOper = new LLCUWErrorDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLCUWError>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			SugPassFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 19, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LLCUWErrorSchema other = (LLCUWErrorSchema)otherObject;
		return

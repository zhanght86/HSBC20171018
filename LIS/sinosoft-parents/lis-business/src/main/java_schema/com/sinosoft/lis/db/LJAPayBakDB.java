...
/*
 * <p>ClassName: LJAPayBakDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class LJAPayBakDB extends LJAPayBakSchema
{
private static Logger logger = Logger.getLogger(LJAPayBakDB.class);
	...
	**/
	...
	// @Constructor
	public LJAPayBakDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LJAPayBakSchema tSchema = this.getSchema();
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getPayNo() == null || this.getPayNo().equals("null")) {
				...
				pstmt.setString(33, this.getPayNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LJAPayBak WHERE  PayNo = ?", 
	...
	public LJAPayBakSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LJAPayBakSet aLJAPayBakSet = new LJAPayBakSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LJAPayBak");
			LJAPayBakSchema aSchema = this.getSchema();
			...
			{
				i++;
				LJAPayBakSchema s1 = new LJAPayBakSchema();
				s1.setSchema(rs,i);
				aLJAPayBakSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLJAPayBakSet;

	}

	public LJAPayBakSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJAPayBakSet aLJAPayBakSet = new LJAPayBakSet();
	...
	public LJAPayBakSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LJAPayBakSet aLJAPayBakSet = new LJAPayBakSet();
	...
	public LJAPayBakSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LJAPayBakSet aLJAPayBakSet = new LJAPayBakSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LJAPayBak " + sqlObj.getUpdPart() + " where " + strWherePart;
 ...
 * @return LJAPayBakSet
 */
public LJAPayBakSet getData()
{
    int tCount = 0;
    LJAPayBakSet tLJAPayBakSet = new LJAPayBakSet();
    LJAPayBakSchema tLJAPayBakSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LJAPayBakDB";
    ...
    {
        tCount = 1;
        tLJAPayBakSchema = new LJAPayBakSchema();
        tLJAPayBakSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LJAPayBakSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LJAPayBakSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

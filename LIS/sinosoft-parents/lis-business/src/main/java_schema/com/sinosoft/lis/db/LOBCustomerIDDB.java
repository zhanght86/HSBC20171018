...
/*
 * <p>ClassName: LOBCustomerIDDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LOBCustomerIDDB extends LOBCustomerIDSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LOBCustomerIDDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LOBCustomerIDSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LOBCustomerIDDB";
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LOBCustomerID WHERE  BussType = ? AND BussNo = ? AND PolicyNo = ? AND CustomerNo = ? AND IDType = ?", 
	...
	public LOBCustomerIDSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBCustomerIDSet aLOBCustomerIDSet = new LOBCustomerIDSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LOBCustomerID");
			LOBCustomerIDSchema aSchema = this.getSchema();
			...
			{
				i++;
				LOBCustomerIDSchema s1 = new LOBCustomerIDSchema();
				s1.setSchema(rs,i);
				aLOBCustomerIDSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLOBCustomerIDSet;
	}

	public LOBCustomerIDSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBCustomerIDSet aLOBCustomerIDSet = new LOBCustomerIDSet();
	...
	public LOBCustomerIDSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LOBCustomerIDSet aLOBCustomerIDSet = new LOBCustomerIDSet();
	...
	public LOBCustomerIDSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LOBCustomerIDSet aLOBCustomerIDSet = new LOBCustomerIDSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LOBCustomerID " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LOBCustomerIDSet
 */
public LOBCustomerIDSet getData()
{
    int tCount = 0;
    LOBCustomerIDSet tLOBCustomerIDSet = new LOBCustomerIDSet();
    LOBCustomerIDSchema tLOBCustomerIDSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LOBCustomerIDDB";
    ...
    {
        tCount = 1;
        tLOBCustomerIDSchema = new LOBCustomerIDSchema();
        tLOBCustomerIDSchema.setSchema(mResultSet, 1);
        tLOBCustomerIDSet.add(tLOBCustomerIDSchema);
...
public boolean closeData()
{
	...
	public LOBCustomerIDSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LOBCustomerIDSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

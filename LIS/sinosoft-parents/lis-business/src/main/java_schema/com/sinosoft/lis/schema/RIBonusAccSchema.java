...
/*
 * <p>ClassName: RIBonusAccSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 再保系统模型
 */
public class RIBonusAccSchema implements Schema, Cloneable
{
	// @Field
	/** 保单险种号码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		RIBonusAccSchema cloned = (RIBonusAccSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 RIBonusAccSchema 对象给 Schema 赋值
	* @param: aRIBonusAccSchema RIBonusAccSchema
	**/
	public void setSchema(RIBonusAccSchema aRIBonusAccSchema)
	{
		this.PolNo = aRIBonusAccSchema.getPolNo();
		this.InsuAccNo = aRIBonusAccSchema.getInsuAccNo();
		this.ContNo = aRIBonusAccSchema.getContNo();
		this.GrpContNo = aRIBonusAccSchema.getGrpContNo();
		this.GrpPolNo = aRIBonusAccSchema.getGrpPolNo();
		this.PrtNo = aRIBonusAccSchema.getPrtNo();
		this.RiskCode = aRIBonusAccSchema.getRiskCode();
		this.AccType = aRIBonusAccSchema.getAccType();
		this.BonusType = aRIBonusAccSchema.getBonusType();
		this.InsuredNo = aRIBonusAccSchema.getInsuredNo();
		this.AppntNo = aRIBonusAccSchema.getAppntNo();
		this.AccComputeFlag = aRIBonusAccSchema.getAccComputeFlag();
		this.SumBonus = aRIBonusAccSchema.getSumBonus();
		this.ManageCom = aRIBonusAccSchema.getManageCom();
		this.Operator = aRIBonusAccSchema.getOperator();
		this.MakeDate = fDate.getDate( aRIBonusAccSchema.getMakeDate());
		this.MakeTime = aRIBonusAccSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aRIBonusAccSchema.getModifyDate());
		this.ModifyTime = aRIBonusAccSchema.getModifyTime();
		this.Currency = aRIBonusAccSchema.getCurrency();
		this.State = aRIBonusAccSchema.getState();
		this.StartDate = fDate.getDate( aRIBonusAccSchema.getStartDate());
		this.EndDate = fDate.getDate( aRIBonusAccSchema.getEndDate());
		this.LastPayDate = fDate.getDate( aRIBonusAccSchema.getLastPayDate());
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			System.out.println("数据库中的RIBonusAcc表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "RIBonusAccSchema";
	...
	public RIBonusAccSchema getSchema()
	{
		RIBonusAccSchema aRIBonusAccSchema = new RIBonusAccSchema();
		aRIBonusAccSchema.setSchema(this);
		return aRIBonusAccSchema;
	}

	public RIBonusAccDB getDB()
	{
		RIBonusAccDB aDBOper = new RIBonusAccDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpRIBonusAcc描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpRIBonusAcc>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			SumBonus = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,13,SysConst.PACKAGESPILTER))).doubleValue();
			...
			StartDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22,SysConst.PACKAGESPILTER));
			EndDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 23,SysConst.PACKAGESPILTER));
			LastPayDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 24,SysConst.PACKAGESPILTER));
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		RIBonusAccSchema other = (RIBonusAccSchema)otherObject;
		return

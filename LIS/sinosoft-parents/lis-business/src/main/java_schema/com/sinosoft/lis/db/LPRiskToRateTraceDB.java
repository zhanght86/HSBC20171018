...
/*
 * <p>ClassName: LPRiskToRateTraceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 费率表维护
 */
public class LPRiskToRateTraceDB extends LPRiskToRateTraceSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LPRiskToRateTraceDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPRiskToRateTraceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPRiskToRateTraceDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAuditFlag() == null || this.getAuditFlag().equals("null")) {
				...
				pstmt.setString(10, this.getAuditFlag());
			}
			...
			if(this.getAuditor() == null || this.getAuditor().equals("null")) {
				...
				pstmt.setString(12, this.getAuditor());
			}
			...
			if(this.getRateType() == null || this.getRateType().equals("null")) {
				...
				pstmt.setString(16, this.getRateType());
			}
			if(this.getRateTable() == null || this.getRateTable().equals("null")) {
				pstmt.setNull(17, 12);
			} else {
				pstmt.setString(17, this.getRateTable());
			}
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPRiskToRateTrace WHERE  RiskCode = ? AND RateType = ? AND RateTable = ? AND SerialNo = ?", 
	...
	public LPRiskToRateTraceSet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPRiskToRateTraceSet aLPRiskToRateTraceSet = new LPRiskToRateTraceSet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("LPRiskToRateTrace");
			LPRiskToRateTraceSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPRiskToRateTraceSchema s1 = new LPRiskToRateTraceSchema();
				s1.setSchema(rs,i);
				aLPRiskToRateTraceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aLPRiskToRateTraceSet;
	}

	public LPRiskToRateTraceSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPRiskToRateTraceSet aLPRiskToRateTraceSet = new LPRiskToRateTraceSet();
	...
	public LPRiskToRateTraceSet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPRiskToRateTraceSet aLPRiskToRateTraceSet = new LPRiskToRateTraceSet();
	...
	public LPRiskToRateTraceSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPRiskToRateTraceSet aLPRiskToRateTraceSet = new LPRiskToRateTraceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPRiskToRateTrace " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPRiskToRateTraceSet
 */
public LPRiskToRateTraceSet getData()
{
    int tCount = 0;
    LPRiskToRateTraceSet tLPRiskToRateTraceSet = new LPRiskToRateTraceSet();
    LPRiskToRateTraceSchema tLPRiskToRateTraceSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPRiskToRateTraceDB";
    ...
    {
        tCount = 1;
        tLPRiskToRateTraceSchema = new LPRiskToRateTraceSchema();
        tLPRiskToRateTraceSchema.setSchema(mResultSet, 1);
        tLPRiskToRateTraceSet.add(tLPRiskToRateTraceSchema);

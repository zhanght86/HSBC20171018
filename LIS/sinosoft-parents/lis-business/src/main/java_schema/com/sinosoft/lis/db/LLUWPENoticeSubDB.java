...
/*
 * <p>ClassName: LLUWPENoticeSubDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLUWPENoticeSubDB extends LLUWPENoticeSubSchema
{
private static Logger logger = Logger.getLogger(LLUWPENoticeSubDB.class);
	...
	**/
	...
	// @Constructor
	public LLUWPENoticeSubDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLUWPENoticeSubSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLUWPENoticeSubDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getAgentName() == null || this.getAgentName().equals("null")) {
				...
				pstmt.setString(17, this.getAgentName());
			}
			...
			if(this.getBatNo() == null || this.getBatNo().equals("null")) {
				...
				pstmt.setString(34, this.getBatNo());
			}
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(35, 12);
			} else {
				pstmt.setString(35, this.getSerialNo());
			}
			if(this.getContNo() == null || this.getContNo().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
				pstmt.setString(36, this.getContNo());
			}
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLUWPENoticeSub WHERE  ClmNo = ? AND BatNo = ? AND SerialNo = ? AND ContNo = ? AND CustomerNo = ?", 
	...
	public LLUWPENoticeSubSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLUWPENoticeSubSet aLLUWPENoticeSubSet = new LLUWPENoticeSubSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLUWPENoticeSub");
			LLUWPENoticeSubSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLUWPENoticeSubSchema s1 = new LLUWPENoticeSubSchema();
				s1.setSchema(rs,i);
				aLLUWPENoticeSubSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLUWPENoticeSubSet;

	}

	public LLUWPENoticeSubSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWPENoticeSubSet aLLUWPENoticeSubSet = new LLUWPENoticeSubSet();
	...
	public LLUWPENoticeSubSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LLUWPENoticeSubSet aLLUWPENoticeSubSet = new LLUWPENoticeSubSet();
	...
	public LLUWPENoticeSubSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLUWPENoticeSubSet aLLUWPENoticeSubSet = new LLUWPENoticeSubSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLUWPENoticeSub " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLUWPENoticeSubSet
 */
public LLUWPENoticeSubSet getData()
{
    int tCount = 0;
    LLUWPENoticeSubSet tLLUWPENoticeSubSet = new LLUWPENoticeSubSet();
    LLUWPENoticeSubSchema tLLUWPENoticeSubSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLUWPENoticeSubDB";
    ...
    {
        tCount = 1;
        tLLUWPENoticeSubSchema = new LLUWPENoticeSubSchema();
        tLLUWPENoticeSubSchema.setSchema(mResultSet, 1);
        tLLUWPENoticeSubSet.add(tLLUWPENoticeSubSchema);
...
public boolean closeData()
{
	...
	public LLUWPENoticeSubSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLUWPENoticeSubSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

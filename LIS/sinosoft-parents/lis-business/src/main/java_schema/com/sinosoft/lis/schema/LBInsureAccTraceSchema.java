...
/*
 * <p>ClassName: LBInsureAccTraceSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 投连改造
 */
public class LBInsureAccTraceSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LBInsureAccTraceSchema.class);
	// @Field
	/** 批单号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LBInsureAccTraceSchema cloned = (LBInsureAccTraceSchema)super.clone();
	...
	*/
	...
	public void setMoneyType(String aMoneyType)
	{
		if(aMoneyType!=null && aMoneyType.length()>2)
			throw new IllegalArgumentException("金额类型MoneyType值"+aMoneyType+"的长度"+aMoneyType.length()+"大于最大值2");
	...
	*/
	...
	/**
	* 使用另外一个 LBInsureAccTraceSchema 对象给 Schema 赋值
	* @param: aLBInsureAccTraceSchema LBInsureAccTraceSchema
	**/
	public void setSchema(LBInsureAccTraceSchema aLBInsureAccTraceSchema)
	{
		this.EdorNo = aLBInsureAccTraceSchema.getEdorNo();
		this.GrpContNo = aLBInsureAccTraceSchema.getGrpContNo();
		this.GrpPolNo = aLBInsureAccTraceSchema.getGrpPolNo();
		this.ContNo = aLBInsureAccTraceSchema.getContNo();
		this.PolNo = aLBInsureAccTraceSchema.getPolNo();
		this.SerialNo = aLBInsureAccTraceSchema.getSerialNo();
		this.InsuAccNo = aLBInsureAccTraceSchema.getInsuAccNo();
		this.RiskCode = aLBInsureAccTraceSchema.getRiskCode();
		this.PayPlanCode = aLBInsureAccTraceSchema.getPayPlanCode();
		this.OtherNo = aLBInsureAccTraceSchema.getOtherNo();
		this.OtherType = aLBInsureAccTraceSchema.getOtherType();
		this.AccAscription = aLBInsureAccTraceSchema.getAccAscription();
		this.MoneyType = aLBInsureAccTraceSchema.getMoneyType();
		this.Money = aLBInsureAccTraceSchema.getMoney();
		this.UnitCount = aLBInsureAccTraceSchema.getUnitCount();
		this.PayDate = fDate.getDate( aLBInsureAccTraceSchema.getPayDate());
		this.State = aLBInsureAccTraceSchema.getState();
		this.ManageCom = aLBInsureAccTraceSchema.getManageCom();
		this.Operator = aLBInsureAccTraceSchema.getOperator();
		this.MakeDate = fDate.getDate( aLBInsureAccTraceSchema.getMakeDate());
		this.MakeTime = aLBInsureAccTraceSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLBInsureAccTraceSchema.getModifyDate());
		this.ModifyTime = aLBInsureAccTraceSchema.getModifyTime();
		this.FeeCode = aLBInsureAccTraceSchema.getFeeCode();
		this.AccAlterNo = aLBInsureAccTraceSchema.getAccAlterNo();
		this.AccAlterType = aLBInsureAccTraceSchema.getAccAlterType();
		this.PayNo = aLBInsureAccTraceSchema.getPayNo();
		this.Currency = aLBInsureAccTraceSchema.getCurrency();
		this.EdorType = aLBInsureAccTraceSchema.getEdorType();
		this.BusyType = aLBInsureAccTraceSchema.getBusyType();
		this.ShouldValueDate = fDate.getDate( aLBInsureAccTraceSchema.getShouldValueDate());
		this.ValueDate = fDate.getDate( aLBInsureAccTraceSchema.getValueDate());
		this.DutyCode = aLBInsureAccTraceSchema.getDutyCode();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LBInsureAccTrace表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LBInsureAccTraceSchema";
	...
	public LBInsureAccTraceSchema getSchema()
	{
		LBInsureAccTraceSchema aLBInsureAccTraceSchema = new LBInsureAccTraceSchema();
		aLBInsureAccTraceSchema.setSchema(this);
		return aLBInsureAccTraceSchema;
	}

	public LBInsureAccTraceDB getDB()
	{
		LBInsureAccTraceDB aDBOper = new LBInsureAccTraceDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBInsureAccTrace描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLBInsureAccTrace>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			MoneyType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 13, SysConst.PACKAGESPILTER );
			Money = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,14,SysConst.PACKAGESPILTER))).doubleValue();
			UnitCount = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,15,SysConst.PACKAGESPILTER))).doubleValue();
			...
			FeeCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 24, SysConst.PACKAGESPILTER );
			AccAlterNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 25, SysConst.PACKAGESPILTER );
			AccAlterType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 26, SysConst.PACKAGESPILTER );
			PayNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 27, SysConst.PACKAGESPILTER );
			Currency = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 28, SysConst.PACKAGESPILTER );
			EdorType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 29, SysConst.PACKAGESPILTER );
			BusyType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 30, SysConst.PACKAGESPILTER );
			ShouldValueDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 31,SysConst.PACKAGESPILTER));
			ValueDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 32,SysConst.PACKAGESPILTER));
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LBInsureAccTraceSchema other = (LBInsureAccTraceSchema)otherObject;
		return

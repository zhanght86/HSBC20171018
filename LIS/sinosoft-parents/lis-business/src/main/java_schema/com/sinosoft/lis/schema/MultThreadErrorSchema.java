...
/*
 * <p>ClassName: MultThreadErrorSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保流程
 */
public class MultThreadErrorSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(MultThreadErrorSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		MultThreadErrorSchema cloned = (MultThreadErrorSchema)super.clone();
	...
	* 2-
	*/
	public String getDealPos()
	{
		return DealPos;
	}
	public void setDealPos(String aDealPos)
	{
		DealPos = aDealPos;
	}
	...
	/**
	* 使用另外一个 MultThreadErrorSchema 对象给 Schema 赋值
	* @param: aMultThreadErrorSchema MultThreadErrorSchema
	**/
	public void setSchema(MultThreadErrorSchema aMultThreadErrorSchema)
	{
		this.Serino = aMultThreadErrorSchema.getSerino();
		this.BatchNo = aMultThreadErrorSchema.getBatchNo();
		this.DealPos = aMultThreadErrorSchema.getDealPos();
		this.PrtNo = aMultThreadErrorSchema.getPrtNo();
		this.SignNo = aMultThreadErrorSchema.getSignNo();
		this.MakeDate = fDate.getDate( aMultThreadErrorSchema.getMakeDate());
		this.MakeTime = aMultThreadErrorSchema.getMakeTime();
		this.Operator = aMultThreadErrorSchema.getOperator();
		this.Reason = aMultThreadErrorSchema.getReason();
		this.ErrType = aMultThreadErrorSchema.getErrType();
		this.ErrCode = aMultThreadErrorSchema.getErrCode();
		this.State = aMultThreadErrorSchema.getState();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			if( rs.getString("DealPos") == null )
				this.DealPos = null;
			else
				this.DealPos = rs.getString("DealPos").trim();
		...
		{
			logger.debug("数据库中的MultThreadError表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "MultThreadErrorSchema";
	...
	public MultThreadErrorSchema getSchema()
	{
		MultThreadErrorSchema aMultThreadErrorSchema = new MultThreadErrorSchema();
		aMultThreadErrorSchema.setSchema(this);
		return aMultThreadErrorSchema;
	}

	public MultThreadErrorDB getDB()
	{
		MultThreadErrorDB aDBOper = new MultThreadErrorDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpMultThreadError描述/A>表字段
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(DealPos)); strReturn.append(SysConst.PACKAGESPILTER);
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpMultThreadError>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			DealPos = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER );
			PrtNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			SignNo= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,5,SysConst.PACKAGESPILTER))).intValue();
			...
			ErrType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 10, SysConst.PACKAGESPILTER );
			ErrCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 11, SysConst.PACKAGESPILTER );
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("DealPos"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(DealPos));
		}
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 1:
				...
				break;
			case 2:
				strFieldValue = StrTool.GBKToUnicode(DealPos);
				break;
			case 3:
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		MultThreadErrorSchema other = (MultThreadErrorSchema)otherObject;
		return
			Serino.equals(other.getSerino())
			&& BatchNo.equals(other.getBatchNo())
			&& DealPos.equals(other.getDealPos())
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("DealPos") ) {
			return 2;
		}

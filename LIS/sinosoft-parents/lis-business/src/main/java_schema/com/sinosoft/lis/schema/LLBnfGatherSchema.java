...
/*
 * <p>ClassName: LLBnfGatherSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 多币种改造业务表
 */
public class LLBnfGatherSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLBnfGatherSchema.class);
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLBnfGatherSchema cloned = (LLBnfGatherSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LLBnfGatherSchema 对象给 Schema 赋值
	* @param: aLLBnfGatherSchema LLBnfGatherSchema
	**/
	public void setSchema(LLBnfGatherSchema aLLBnfGatherSchema)
	{
		this.ClmNo = aLLBnfGatherSchema.getClmNo();
		this.CaseNo = aLLBnfGatherSchema.getCaseNo();
		this.BatNo = aLLBnfGatherSchema.getBatNo();
		this.BnfKind = aLLBnfGatherSchema.getBnfKind();
		this.InsuredNo = aLLBnfGatherSchema.getInsuredNo();
		this.BnfNo = aLLBnfGatherSchema.getBnfNo();
		this.BnfType = aLLBnfGatherSchema.getBnfType();
		this.BnfGrade = aLLBnfGatherSchema.getBnfGrade();
		this.RelationToInsured = aLLBnfGatherSchema.getRelationToInsured();
		this.CustomerNo = aLLBnfGatherSchema.getCustomerNo();
		this.Name = aLLBnfGatherSchema.getName();
		this.Sex = aLLBnfGatherSchema.getSex();
		this.Birthday = fDate.getDate( aLLBnfGatherSchema.getBirthday());
		this.IDType = aLLBnfGatherSchema.getIDType();
		this.IDNo = aLLBnfGatherSchema.getIDNo();
		this.RelationToPayee = aLLBnfGatherSchema.getRelationToPayee();
		this.PayeeNo = aLLBnfGatherSchema.getPayeeNo();
		this.PayeeName = aLLBnfGatherSchema.getPayeeName();
		this.PayeeSex = aLLBnfGatherSchema.getPayeeSex();
		this.PayeeBirthday = fDate.getDate( aLLBnfGatherSchema.getPayeeBirthday());
		this.PayeeIDType = aLLBnfGatherSchema.getPayeeIDType();
		this.NBPolNo = aLLBnfGatherSchema.getNBPolNo();
		this.PayeeIDNo = aLLBnfGatherSchema.getPayeeIDNo();
		this.BnfLot = aLLBnfGatherSchema.getBnfLot();
		this.GetMoney = aLLBnfGatherSchema.getGetMoney();
		this.CaseGetMode = aLLBnfGatherSchema.getCaseGetMode();
		this.CasePayMode = aLLBnfGatherSchema.getCasePayMode();
		this.CasePayFlag = aLLBnfGatherSchema.getCasePayFlag();
		this.BankCode = aLLBnfGatherSchema.getBankCode();
		this.BankAccNo = aLLBnfGatherSchema.getBankAccNo();
		this.AccName = aLLBnfGatherSchema.getAccName();
		this.ModiReasonCode = aLLBnfGatherSchema.getModiReasonCode();
		this.ModiReasonDesc = aLLBnfGatherSchema.getModiReasonDesc();
		this.OBankCode = aLLBnfGatherSchema.getOBankCode();
		this.OBankAccNo = aLLBnfGatherSchema.getOBankAccNo();
		this.OAccName = aLLBnfGatherSchema.getOAccName();
		this.Operator = aLLBnfGatherSchema.getOperator();
		this.MakeDate = fDate.getDate( aLLBnfGatherSchema.getMakeDate());
		this.MakeTime = aLLBnfGatherSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLLBnfGatherSchema.getModifyDate());
		this.ModifyTime = aLLBnfGatherSchema.getModifyTime();
		this.OtherNo = aLLBnfGatherSchema.getOtherNo();
		this.OtherNoType = aLLBnfGatherSchema.getOtherNoType();
		this.Currency = aLLBnfGatherSchema.getCurrency();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLBnfGather表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLBnfGatherSchema";
	...
	public LLBnfGatherSchema getSchema()
	{
		LLBnfGatherSchema aLLBnfGatherSchema = new LLBnfGatherSchema();
		aLLBnfGatherSchema.setSchema(this);
		return aLLBnfGatherSchema;
	}

	public LLBnfGatherDB getDB()
	{
		LLBnfGatherDB aDBOper = new LLBnfGatherDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLBnfGather>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			BnfKind = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 4, SysConst.PACKAGESPILTER );
			InsuredNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			BnfNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			...
			RelationToPayee = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 16, SysConst.PACKAGESPILTER );
			PayeeNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 17, SysConst.PACKAGESPILTER );
			PayeeName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 18, SysConst.PACKAGESPILTER );
			PayeeSex = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 19, SysConst.PACKAGESPILTER );
			PayeeBirthday = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20,SysConst.PACKAGESPILTER));
			PayeeIDType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 21, SysConst.PACKAGESPILTER );
			NBPolNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 22, SysConst.PACKAGESPILTER );
			PayeeIDNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 23, SysConst.PACKAGESPILTER );
			BnfLot = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,24,SysConst.PACKAGESPILTER))).doubleValue();
			GetMoney = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,25,SysConst.PACKAGESPILTER))).doubleValue();
			CaseGetMode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 26, SysConst.PACKAGESPILTER );
			CasePayMode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 27, SysConst.PACKAGESPILTER );
			CasePayFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 28, SysConst.PACKAGESPILTER );
			...
			ModiReasonCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 32, SysConst.PACKAGESPILTER );
			ModiReasonDesc = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 33, SysConst.PACKAGESPILTER );
			OBankCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 34, SysConst.PACKAGESPILTER );
			OBankAccNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 35, SysConst.PACKAGESPILTER );
			OAccName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 36, SysConst.PACKAGESPILTER );
			...
			OtherNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 42, SysConst.PACKAGESPILTER );
			OtherNoType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 43, SysConst.PACKAGESPILTER );
			Currency = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 44, SysConst.PACKAGESPILTER );
		}
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LLBnfGatherSchema other = (LLBnfGatherSchema)otherObject;
		return

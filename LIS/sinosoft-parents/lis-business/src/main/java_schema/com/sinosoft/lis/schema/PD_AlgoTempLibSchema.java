...
import com.sinosoft.lis.db.PD_AlgoTempLibDB;

/**
 * <p>ClassName: PD_AlgoTempLibSchema </p>
 ...
 */
public class PD_AlgoTempLibSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(PD_AlgoTempLibSchema.class);
    ...
    {
        PD_AlgoTempLibSchema cloned = (PD_AlgoTempLibSchema)super.clone();
    ...
    /**
     * 使用另外一个 PD_AlgoTempLibSchema 对象给 Schema 赋值
     * @param: aPD_AlgoTempLibSchema PD_AlgoTempLibSchema
     **/
    public void setSchema(PD_AlgoTempLibSchema aPD_AlgoTempLibSchema)
    {
        this.Code = aPD_AlgoTempLibSchema.getCode();
        this.Name = aPD_AlgoTempLibSchema.getName();
        this.Content = aPD_AlgoTempLibSchema.getContent();
        this.Type = aPD_AlgoTempLibSchema.getType();
        this.Description = aPD_AlgoTempLibSchema.getDescription();
        this.Operator = aPD_AlgoTempLibSchema.getOperator();
        this.MakeDate = fDate.getDate(aPD_AlgoTempLibSchema.getMakeDate());
        this.MakeTime = aPD_AlgoTempLibSchema.getMakeTime();
        this.ModifyDate = fDate.getDate(aPD_AlgoTempLibSchema.getModifyDate());
        this.ModifyTime = aPD_AlgoTempLibSchema.getModifyTime();
        this.Standbyflag1 = aPD_AlgoTempLibSchema.getStandbyflag1();
        this.Standbyflag2 = aPD_AlgoTempLibSchema.getStandbyflag2();
        this.Standbyflag3 = aPD_AlgoTempLibSchema.getStandbyflag3();
        this.Standbyflag4 = aPD_AlgoTempLibSchema.getStandbyflag4();
        this.Standbyflag5 = aPD_AlgoTempLibSchema.getStandbyflag5();
        this.Standbyflag6 = aPD_AlgoTempLibSchema.getStandbyflag6();
    }
    ...
    public boolean setSchema(ResultSet rs, int i)
    {
        try
        {
            ...
            if (rs.getString(3) == null)
                ...
                this.Content = rs.getString(3).trim();
            if (rs.getString(4) == null)
                this.Type = null;
            else
                this.Type = rs.getString(4).trim();
            if (rs.getString(5) == null)
                this.Description = null;
            else
                this.Description = rs.getString(5).trim();
        ...
        {
            logger.debug("数据库中表PD_AlgoTempLib字段个数和Schema中的字段个数不一致，或执行db.executeQuery查询时未使用select * from tables");
            // @@错误处理
            CError tError = new CError();
            tError.moduleName = "PD_AlgoTempLibSchema";
    ...
    public PD_AlgoTempLibSchema getSchema()
    {
        PD_AlgoTempLibSchema aPD_AlgoTempLibSchema = new PD_AlgoTempLibSchema();
        aPD_AlgoTempLibSchema.setSchema(this);
        return aPD_AlgoTempLibSchema;
    }

    public PD_AlgoTempLibDB getDB()
    {
        PD_AlgoTempLibDB aDBOper = new PD_AlgoTempLibDB();
        aDBOper.setSchema(this);
        return aDBOper;
    }

    /**
     * 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpPD_AlgoTempLib描述/A>表字段
    ...
    /**
     * 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpPD_AlgoTempLib>历史记账凭证主表信息</A>表字段
    ...
    public boolean decode(String strMessage)
    {
        try
        {
            ...
            Content = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 3, SysConst.PACKAGESPILTER);
    ...
    public boolean equals(Object otherObject)
    {
        ...
        if (getClass() != otherObject.getClass())
            return false;
        PD_AlgoTempLibSchema other = (PD_AlgoTempLibSchema) otherObject;
        return

...
/*
 * <p>ClassName: LYReturnFromBankBDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 银行业务
 */
public class LYReturnFromBankBDB extends LYReturnFromBankBSchema
{
private static Logger logger = Logger.getLogger(LYReturnFromBankBDB.class);
	...
	**/
	...
	// @Constructor
	public LYReturnFromBankBDB( Connection tConnection )
	{
		con = tConnection;
		db = new DBOper( con, "LYReturnFromBankB" );
		mflag = true;
	}

	public LYReturnFromBankBDB()
	{
		con = null;
		db = new DBOper( "LYReturnFromBankB" );
		mflag = false;
	}
	...
	{
		LYReturnFromBankBSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LYReturnFromBankBDB";
	...
	public boolean delete()
	{
		...
		try
		{
			pstmt = con.prepareStatement("DELETE FROM LYReturnFromBankB WHERE  SerialNo = ? AND DealType = ? AND PayCode = ? AND PolNo = ?");
		...
		SQLString sqlObj = new SQLString("LYReturnFromBankB");
	...
	public boolean insert()
	{
		...
		try
		{
			pstmt = con.prepareStatement("INSERT INTO LYReturnFromBankB(SerialNo ,DealType ,DataType ,PayCode ,PayType ,Name ,BankCode ,AccType ,AccName ,AccNo ,PolNo ,NoType ,ComCode ,AgentCode ,PayMoney ,SendDate ,BankDealDate ,BankDealTime ,BankSuccFlag ,VertifyFlag ,Remark ,ConvertFlag ,DoType ,ModifyDate ,ModifyTime ,RiskCode ,SerNo ,BankName ,InBankCode ,InBankName ,InAccType ,InAccName ,InAccNo ,IDType ,IDNo ,StandByFlag1 ,StandByFlag2 ,StandByFlag3 ,StandByFlag4 ,StandByFlag5) VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
			if(this.getSerialNo() == null || this.getSerialNo().equals("null")) {
				pstmt.setNull(1, 12);
			} else {
				pstmt.setString(1, this.getSerialNo());
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LYReturnFromBankB WHERE  SerialNo = ? AND DealType = ? AND PayCode = ? AND PolNo = ?", 
	...
	public LYReturnFromBankBSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LYReturnFromBankBSet aLYReturnFromBankBSet = new LYReturnFromBankBSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LYReturnFromBankB");
			LYReturnFromBankBSchema aSchema = this.getSchema();
			...
			{
				i++;
				LYReturnFromBankBSchema s1 = new LYReturnFromBankBSchema();
				s1.setSchema(rs,i);
				aLYReturnFromBankBSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLYReturnFromBankBSet;

	}

	public LYReturnFromBankBSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LYReturnFromBankBSet aLYReturnFromBankBSet = new LYReturnFromBankBSet();
	...
	public LYReturnFromBankBSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LYReturnFromBankBSet aLYReturnFromBankBSet = new LYReturnFromBankBSet();
	...
	public LYReturnFromBankBSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LYReturnFromBankBSet aLYReturnFromBankBSet = new LYReturnFromBankBSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LYReturnFromBankB " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LYReturnFromBankBSet
 */
public LYReturnFromBankBSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLYReturnFromBankBSchema.setSchema(mResultSet, 1);
        tLYReturnFromBankBSet.add(tLYReturnFromBankBSchema);
...
public boolean closeData()
{
	...
	public LYReturnFromBankBSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LYReturnFromBankBSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

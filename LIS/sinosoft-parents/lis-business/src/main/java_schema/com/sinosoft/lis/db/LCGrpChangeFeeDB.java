...
/*
 * <p>ClassName: LCGrpChangeFeeDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LCGrpChangeFeeDB extends LCGrpChangeFeeSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCGrpChangeFeeDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCGrpChangeFeeSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCGrpChangeFeeDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			pstmt.setDouble(5, this.getChangeFee());
			...
			if(this.getPolicyNo() == null || this.getPolicyNo().equals("null")) {
				...
				pstmt.setString(14, this.getPolicyNo());
			}
			if(this.getAgentCom() == null || this.getAgentCom().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCGrpChangeFee WHERE  PolicyNo = ? AND AgentCom = ? AND RiskCode = ?", 
	...
	public LCGrpChangeFeeSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGrpChangeFeeSet aLCGrpChangeFeeSet = new LCGrpChangeFeeSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCGrpChangeFee");
			LCGrpChangeFeeSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCGrpChangeFeeSchema s1 = new LCGrpChangeFeeSchema();
				s1.setSchema(rs,i);
				aLCGrpChangeFeeSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCGrpChangeFeeSet;
	}

	public LCGrpChangeFeeSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpChangeFeeSet aLCGrpChangeFeeSet = new LCGrpChangeFeeSet();
	...
	public LCGrpChangeFeeSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LCGrpChangeFeeSet aLCGrpChangeFeeSet = new LCGrpChangeFeeSet();
	...
	public LCGrpChangeFeeSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCGrpChangeFeeSet aLCGrpChangeFeeSet = new LCGrpChangeFeeSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCGrpChangeFee " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCGrpChangeFeeSet
 */
public LCGrpChangeFeeSet getData()
{
    int tCount = 0;
    LCGrpChangeFeeSet tLCGrpChangeFeeSet = new LCGrpChangeFeeSet();
    LCGrpChangeFeeSchema tLCGrpChangeFeeSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LCGrpChangeFeeDB";
    ...
    {
        tCount = 1;
        tLCGrpChangeFeeSchema = new LCGrpChangeFeeSchema();
        tLCGrpChangeFeeSchema.setSchema(mResultSet, 1);
        tLCGrpChangeFeeSet.add(tLCGrpChangeFeeSchema);
...
public boolean closeData()
{
	...
	public LCGrpChangeFeeSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCGrpChangeFeeSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

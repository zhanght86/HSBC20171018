...
/*
 * <p>ClassName: PD_LMDutyPayDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_3
 */
public class PD_LMDutyPayDB extends PD_LMDutyPaySchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public PD_LMDutyPayDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		PD_LMDutyPaySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "PD_LMDutyPayDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getCalCode() == null || this.getCalCode().equals("null")) {
				...
				pstmt.setString(3, this.getCalCode());
			}
			if(this.getPayPlanName() == null || this.getPayPlanName().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getPayPlanName());
			}
			...
			pstmt.setInt(36, this.getStandbyflag3());
			pstmt.setInt(37, this.getStandbyflag4());
			pstmt.setDouble(38, this.getStandbyflag5());
			pstmt.setDouble(39, this.getStandbyflag6());
			pstmt.setInt(40, this.getPayStartYear());
			if(this.getPayStartYearFlag() == null || this.getPayStartYearFlag().equals("null")) {
				pstmt.setNull(41, 1);
			} else {
				pstmt.setString(41, this.getPayStartYearFlag());
			}
			if(this.getPayStartDateCalRef() == null || this.getPayStartDateCalRef().equals("null")) {
				pstmt.setNull(42, 1);
			} else {
				pstmt.setString(42, this.getPayStartDateCalRef());
			}
			if(this.getPayStartDateCalMode() == null || this.getPayStartDateCalMode().equals("null")) {
				pstmt.setNull(43, 1);
			} else {
				pstmt.setString(43, this.getPayStartDateCalMode());
			}
			if(this.getInvestType() == null || this.getInvestType().equals("null")) {
				pstmt.setNull(44, 12);
			} else {
				pstmt.setString(44, this.getInvestType());
			}
			if(this.getPCalCode() == null || this.getPCalCode().equals("null")) {
				pstmt.setNull(45, 12);
			} else {
				pstmt.setString(45, this.getPCalCode());
			}
			if(this.getRCalPremFlag() == null || this.getRCalPremFlag().equals("null")) {
				pstmt.setNull(46, 12);
			} else {
				pstmt.setString(46, this.getRCalPremFlag());
			}
			if(this.getRCalPremCode() == null || this.getRCalPremCode().equals("null")) {
				pstmt.setNull(47, 12);
			} else {
				pstmt.setString(47, this.getRCalPremCode());
			}
			// set where condition
			if(this.getPayPlanCode() == null || this.getPayPlanCode().equals("null")) {
				pstmt.setNull(48, 12);
			} else {
				pstmt.setString(48, this.getPayPlanCode());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM PD_LMDutyPay WHERE  PayPlanCode = ?", 
	...
	public PD_LMDutyPaySet query()
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMDutyPaySet aPD_LMDutyPaySet = new PD_LMDutyPaySet();
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			SQLString sqlObj = new SQLString("PD_LMDutyPay");
			PD_LMDutyPaySchema aSchema = this.getSchema();
			...
			{
				i++;
				PD_LMDutyPaySchema s1 = new PD_LMDutyPaySchema();
				s1.setSchema(rs,i);
				aPD_LMDutyPaySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
		...
		return aPD_LMDutyPaySet;
	}

	public PD_LMDutyPaySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMDutyPaySet aPD_LMDutyPaySet = new PD_LMDutyPaySet();
	...
	public PD_LMDutyPaySet query(int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMDutyPaySet aPD_LMDutyPaySet = new PD_LMDutyPaySet();
	...
	public PD_LMDutyPaySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		PD_LMDutyPaySet aPD_LMDutyPaySet = new PD_LMDutyPaySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update PD_LMDutyPay " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return PD_LMDutyPaySet
 */
public PD_LMDutyPaySet getData()
{
    int tCount = 0;
    PD_LMDutyPaySet tPD_LMDutyPaySet = new PD_LMDutyPaySet();
    PD_LMDutyPaySchema tPD_LMDutyPaySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "PD_LMDutyPayDB";
    ...
    {
        tCount = 1;
        tPD_LMDutyPaySchema = new PD_LMDutyPaySchema();
        tPD_LMDutyPaySchema.setSchema(mResultSet, 1);
        tPD_LMDutyPaySet.add(tPD_LMDutyPaySchema);

...
/*
 * <p>ClassName: LLCasePolicyDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 寿险业务系统lis5模型
 */
public class LLCasePolicyDB extends LLCasePolicySchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LLCasePolicyDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLCasePolicySchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLCasePolicyDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getCasePolType() == null || this.getCasePolType().equals("null")) {
				...
				pstmt.setString(1, this.getCasePolType());
			}
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				pstmt.setNull(2, 12);
			} else {
			...
			if(this.getPolMngCom() == null || this.getPolMngCom().equals("null")) {
				...
				pstmt.setString(10, this.getPolMngCom());
			}
			...
			if(this.getInsuredSex() == null || this.getInsuredSex().equals("null")) {
				...
				pstmt.setString(16, this.getInsuredSex());
			}
			if(this.getInsuredBirthday() == null || this.getInsuredBirthday().equals("null")) {
				pstmt.setNull(17, 91);
			} else {
				pstmt.setDate(17, Date.valueOf(this.getInsuredBirthday()));
			}
			...
			if(this.getPolState() == null || this.getPolState().equals("null")) {
				...
				pstmt.setString(21, this.getPolState());
			}
			if(this.getCasePolState() == null || this.getCasePolState().equals("null")) {
				pstmt.setNull(22, 1);
			} else {
				pstmt.setString(22, this.getCasePolState());
			}
			if(this.getClmFlag() == null || this.getClmFlag().equals("null")) {
				pstmt.setNull(23, 1);
			} else {
				pstmt.setString(23, this.getClmFlag());
			}
			...
			if(this.getDeclineNo() == null || this.getDeclineNo().equals("null")) {
				...
				pstmt.setString(25, this.getDeclineNo());
			}
			if(this.getEndCaseDate() == null || this.getEndCaseDate().equals("null")) {
				pstmt.setNull(26, 91);
			} else {
				pstmt.setDate(26, Date.valueOf(this.getEndCaseDate()));
			}
			...
			if(this.getCaseRelaNo() == null || this.getCaseRelaNo().equals("null")) {
				...
				pstmt.setString(35, this.getCaseRelaNo());
			}
			// set where condition
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				pstmt.setNull(36, 12);
			} else {
				pstmt.setString(36, this.getCaseNo());
			}
			if(this.getPolNo() == null || this.getPolNo().equals("null")) {
				pstmt.setNull(37, 12);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLCasePolicy WHERE  CaseNo = ? AND PolNo = ?", 
	...
	public LLCasePolicySet query()
	{
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLCasePolicy");
			LLCasePolicySchema aSchema = this.getSchema();
			...
			{
				i++;
				LLCasePolicySchema s1 = new LLCasePolicySchema();
				s1.setSchema(rs,i);
				aLLCasePolicySet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLCasePolicySet;
	}

	public LLCasePolicySet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLCasePolicySet aLLCasePolicySet = new LLCasePolicySet();
	...
	public LLCasePolicySet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLCasePolicySet aLLCasePolicySet = new LLCasePolicySet();
	...
	public LLCasePolicySet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLCasePolicySet aLLCasePolicySet = new LLCasePolicySet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLCasePolicy " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLCasePolicySet
 */
public LLCasePolicySet getData()
{
    int tCount = 0;
    LLCasePolicySet tLLCasePolicySet = new LLCasePolicySet();
    LLCasePolicySchema tLLCasePolicySchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LLCasePolicyDB";
    ...
    try
    {
        ...
        tLLCasePolicySchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLCasePolicySet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLCasePolicySet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

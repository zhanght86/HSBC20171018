...
import com.sinosoft.lis.db.LOBCustomerIDDB;

/*
 * 与数据库结构不一致，暂不处理
 * <p>ClassName: LOBCustomerIDSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: Physical Data Model 1
 */
public class LOBCustomerIDSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LOBCustomerIDSchema.class);
	// @Field
	/** 业务类型 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LOBCustomerIDSchema cloned = (LOBCustomerIDSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 LOBCustomerIDSchema 对象给 Schema 赋值
	* @param: aLOBCustomerIDSchema LOBCustomerIDSchema
	**/
	public void setSchema(LOBCustomerIDSchema aLOBCustomerIDSchema)
	{
		this.BussType = aLOBCustomerIDSchema.getBussType();
		this.BussNo = aLOBCustomerIDSchema.getBussNo();
		this.PolicyNo = aLOBCustomerIDSchema.getPolicyNo();
		this.CustomerNo = aLOBCustomerIDSchema.getCustomerNo();
		this.IDFlag = aLOBCustomerIDSchema.getIDFlag();
		this.IDType = aLOBCustomerIDSchema.getIDType();
		this.IDNo = aLOBCustomerIDSchema.getIDNo();
		this.StartDate = fDate.getDate( aLOBCustomerIDSchema.getStartDate());
		this.EndDate = fDate.getDate( aLOBCustomerIDSchema.getEndDate());
		this.State = aLOBCustomerIDSchema.getState();
		this.Remark = aLOBCustomerIDSchema.getRemark();
		this.Segment1 = aLOBCustomerIDSchema.getSegment1();
		this.Segment2 = aLOBCustomerIDSchema.getSegment2();
		this.Segment3 = aLOBCustomerIDSchema.getSegment3();
		this.ManageCom = aLOBCustomerIDSchema.getManageCom();
		this.ComCode = aLOBCustomerIDSchema.getComCode();
		this.MakeOperator = aLOBCustomerIDSchema.getMakeOperator();
		this.MakeDate = fDate.getDate( aLOBCustomerIDSchema.getMakeDate());
		this.MakeTime = aLOBCustomerIDSchema.getMakeTime();
		this.ModifyOperator = aLOBCustomerIDSchema.getModifyOperator();
		this.ModifyDate = fDate.getDate( aLOBCustomerIDSchema.getModifyDate());
		this.ModifyTime = aLOBCustomerIDSchema.getModifyTime();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LOBCustomerID表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LOBCustomerIDSchema";
	...
	public LOBCustomerIDSchema getSchema()
	{
		LOBCustomerIDSchema aLOBCustomerIDSchema = new LOBCustomerIDSchema();
		aLOBCustomerIDSchema.setSchema(this);
		return aLOBCustomerIDSchema;
	}

	public LOBCustomerIDDB getDB()
	{
		LOBCustomerIDDB aDBOper = new LOBCustomerIDDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLOBCustomerID>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LOBCustomerIDSchema other = (LOBCustomerIDSchema)otherObject;
		return

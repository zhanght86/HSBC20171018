...
/*
 * <p>ClassName: LWProcessTransSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 工作流模型
 */
public class LWProcessTransSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LWProcessTransSchema.class);
	// @Field
	/** 转移id */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LWProcessTransSchema cloned = (LWProcessTransSchema)super.clone();
	...
	{
		if(aTransitionID!=null && aTransitionID.length()>10)
			throw new IllegalArgumentException("转移idTransitionID值"+aTransitionID+"的长度"+aTransitionID.length()+"大于最大值10");
	...
	public void setTransitionCond(String aTransitionCond)
	{
		if(aTransitionCond!=null && aTransitionCond.length()>1000)
			throw new IllegalArgumentException("转移条件TransitionCond值"+aTransitionCond+"的长度"+aTransitionCond.length()+"大于最大值1000");
	...
	*/
	...
	{
		if(aTransitionModel!=null && aTransitionModel.length()>1)
			throw new IllegalArgumentException("转移时方式TransitionModel值"+aTransitionModel+"的长度"+aTransitionModel.length()+"大于最大值1");
	...
	*/
	...
	/**
	* 使用另外一个 LWProcessTransSchema 对象给 Schema 赋值
	* @param: aLWProcessTransSchema LWProcessTransSchema
	**/
	public void setSchema(LWProcessTransSchema aLWProcessTransSchema)
	{
		this.TransitionID = aLWProcessTransSchema.getTransitionID();
		this.ProcessID = aLWProcessTransSchema.getProcessID();
		this.TransitionStart = aLWProcessTransSchema.getTransitionStart();
		this.TransitionEnd = aLWProcessTransSchema.getTransitionEnd();
		this.TransitionCond = aLWProcessTransSchema.getTransitionCond();
		this.TransitionCondT = aLWProcessTransSchema.getTransitionCondT();
		this.TransitionModel = aLWProcessTransSchema.getTransitionModel();
		this.StartType = aLWProcessTransSchema.getStartType();
		this.Version = aLWProcessTransSchema.getVersion();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LWProcessTrans表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LWProcessTransSchema";
	...
	public LWProcessTransSchema getSchema()
	{
		LWProcessTransSchema aLWProcessTransSchema = new LWProcessTransSchema();
		aLWProcessTransSchema.setSchema(this);
		return aLWProcessTransSchema;
	}

	public LWProcessTransDB getDB()
	{
		LWProcessTransDB aDBOper = new LWProcessTransDB();
	...
	**/
	public String encode()
	{
		...
		strReturn.append(StrTool.cTrim(StartType)); strReturn.append(SysConst.PACKAGESPILTER);
		strReturn.append(StrTool.cTrim(Version));
		return strReturn.toString();
	}

	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLWProcessTrans>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LWProcessTransSchema other = (LWProcessTransSchema)otherObject;
		return

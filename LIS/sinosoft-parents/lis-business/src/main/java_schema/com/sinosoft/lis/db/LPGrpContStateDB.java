...
/*
 * <p>ClassName: LPGrpContStateDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全核心
 */
public class LPGrpContStateDB extends LPGrpContStateSchema
{
private static Logger logger = Logger.getLogger(LPGrpContStateDB.class);
	...
	**/
	...
	// @Constructor
	public LPGrpContStateDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPGrpContStateSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPGrpContStateDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getEdorType() == null || this.getEdorType().equals("null")) {
				...
				pstmt.setString(17, this.getEdorType());
			}
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(18, 12);
			} else {
			...
			if(this.getStateType() == null || this.getStateType().equals("null")) {
				...
				pstmt.setString(20, this.getStateType());
			}
			if(this.getStartDate() == null || this.getStartDate().equals("null")) {
				pstmt.setNull(21, 91);
			} else {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPGrpContState WHERE  EdorNo = ? AND EdorType = ? AND GrpContNo = ? AND GrpPolNo = ? AND StateType = ? AND StartDate = ?", 
	...
	public LPGrpContStateSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPGrpContState");
			LPGrpContStateSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPGrpContStateSchema s1 = new LPGrpContStateSchema();
				s1.setSchema(rs,i);
				aLPGrpContStateSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPGrpContStateSet;

	}

	public LPGrpContStateSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGrpContStateSet aLPGrpContStateSet = new LPGrpContStateSet();
	...
	public LPGrpContStateSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGrpContStateSet aLPGrpContStateSet = new LPGrpContStateSet();
	...
	public LPGrpContStateSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGrpContStateSet aLPGrpContStateSet = new LPGrpContStateSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPGrpContState " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPGrpContStateSet
 */
public LPGrpContStateSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLPGrpContStateSchema.setSchema(mResultSet, 1);
        tLPGrpContStateSet.add(tLPGrpContStateSchema);
...
public boolean closeData()
{
	...
	public LPGrpContStateSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPGrpContStateSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

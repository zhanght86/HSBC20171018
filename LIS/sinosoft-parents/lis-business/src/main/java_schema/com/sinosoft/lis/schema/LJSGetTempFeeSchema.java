 ...
 * @Database: 寿险业务付费
 */
public class LJSGetTempFeeSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LJSGetTempFeeSchema.class);
	// @Field
	/** 给付通知书号码 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LJSGetTempFeeSchema cloned = (LJSGetTempFeeSchema)super.clone();
	...
	* @param: aLJSGetTempFeeSchema LJSGetTempFeeSchema
	**/
	public void setSchema(LJSGetTempFeeSchema aLJSGetTempFeeSchema)
	{
		...
		this.Currency = aLJSGetTempFeeSchema.getCurrency();
		this.NetAmount = aLJSGetTempFeeSchema.getNetAmount();
		this.TaxAmount = aLJSGetTempFeeSchema.getTaxAmount();
		this.Tax = aLJSGetTempFeeSchema.getTax();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LJSGetTempFee表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			FeeOperationType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 14, SysConst.PACKAGESPILTER );
			FeeFinaType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 15, SysConst.PACKAGESPILTER );
			...
			GetReasonCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 20, SysConst.PACKAGESPILTER );
			...
			Currency = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 24, SysConst.PACKAGESPILTER );
			NetAmount = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,25,SysConst.PACKAGESPILTER))).doubleValue();
			TaxAmount = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,26,SysConst.PACKAGESPILTER))).doubleValue();
			Tax = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,27,SysConst.PACKAGESPILTER))).doubleValue();
		}

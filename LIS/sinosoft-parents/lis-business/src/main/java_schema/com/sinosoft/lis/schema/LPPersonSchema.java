...
{
private static Logger logger = Logger.getLogger(LPPersonSchema.class);
	// @Field
	/** 批单号 */
	...
	**/
	public void setSchema(LPPersonSchema aLPPersonSchema)
	{
		...
		this.VIPValue = aLPPersonSchema.getVIPValue();
		...
		this.License = aLPPersonSchema.getLicense();
		this.LicenseType = aLPPersonSchema.getLicenseType();
		this.SocialInsuNo = aLPPersonSchema.getSocialInsuNo();
		this.ReasonType = aLPPersonSchema.getReasonType();
		this.SocialInsuFlag = aLPPersonSchema.getSocialInsuFlag();
		this.IDExpDate = fDate.getDate( aLPPersonSchema.getIDExpDate());
		this.LastName = aLPPersonSchema.getLastName();
		this.FirstName = aLPPersonSchema.getFirstName();
		this.LastNameEn = aLPPersonSchema.getLastNameEn();
		this.FirstNameEn = aLPPersonSchema.getFirstNameEn();
		this.NameEn = aLPPersonSchema.getNameEn();
		this.LastNamePY = aLPPersonSchema.getLastNamePY();
		this.FirstNamePY = aLPPersonSchema.getFirstNamePY();
		this.Language = aLPPersonSchema.getLanguage();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LPPerson表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ReasonType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 48, SysConst.PACKAGESPILTER );
			SocialInsuFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 49, SysConst.PACKAGESPILTER );
			IDExpDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 50,SysConst.PACKAGESPILTER));
			LastName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 51, SysConst.PACKAGESPILTER );
			FirstName = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 52, SysConst.PACKAGESPILTER );
			LastNameEn = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 53, SysConst.PACKAGESPILTER );
			FirstNameEn = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 54, SysConst.PACKAGESPILTER );
			NameEn = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 55, SysConst.PACKAGESPILTER );
			LastNamePY = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 56, SysConst.PACKAGESPILTER );
			FirstNamePY = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 57, SysConst.PACKAGESPILTER );
			Language = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 58, SysConst.PACKAGESPILTER );
		}

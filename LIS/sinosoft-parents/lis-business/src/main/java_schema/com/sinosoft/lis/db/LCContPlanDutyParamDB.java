...
/*
 * <p>ClassName: LCContPlanDutyParamDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 承保核心
 */
public class LCContPlanDutyParamDB extends LCContPlanDutyParamSchema
{
private static Logger logger = Logger.getLogger(LCContPlanDutyParamDB.class);
	...
	**/
	...
	// @Constructor
	public LCContPlanDutyParamDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LCContPlanDutyParamSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LCContPlanDutyParamDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getSysContPlanCode() == null || this.getSysContPlanCode().equals("null")) {
				...
				pstmt.setString(16, this.getSysContPlanCode());
			}
			// set where condition
			if(this.getGrpContNo() == null || this.getGrpContNo().equals("null")) {
				pstmt.setNull(17, 12);
			...
			if(this.getCalFactor() == null || this.getCalFactor().equals("null")) {
				...
				pstmt.setString(22, this.getCalFactor());
			}
			if(this.getPlanType() == null || this.getPlanType().equals("null")) {
				pstmt.setNull(23, 1);
			} else {
				pstmt.setString(23, StrTool.space1(this.getPlanType(), 1));
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LCContPlanDutyParam WHERE  GrpContNo = ? AND MainRiskCode = ? AND RiskCode = ? AND ContPlanCode = ? AND DutyCode = ? AND CalFactor = ? AND PlanType = ?", 
	...
	public LCContPlanDutyParamSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCContPlanDutyParamSet aLCContPlanDutyParamSet = new LCContPlanDutyParamSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LCContPlanDutyParam");
			LCContPlanDutyParamSchema aSchema = this.getSchema();
			...
			{
				i++;
				LCContPlanDutyParamSchema s1 = new LCContPlanDutyParamSchema();
				s1.setSchema(rs,i);
				aLCContPlanDutyParamSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLCContPlanDutyParamSet;

	}

	public LCContPlanDutyParamSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCContPlanDutyParamSet aLCContPlanDutyParamSet = new LCContPlanDutyParamSet();
	...
	public LCContPlanDutyParamSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LCContPlanDutyParamSet aLCContPlanDutyParamSet = new LCContPlanDutyParamSet();
	...
	public LCContPlanDutyParamSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LCContPlanDutyParamSet aLCContPlanDutyParamSet = new LCContPlanDutyParamSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LCContPlanDutyParam " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LCContPlanDutyParamSet
 */
public LCContPlanDutyParamSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLCContPlanDutyParamSchema.setSchema(mResultSet, 1);
    ...
    return tLCContPlanDutyParamSet;
}
/**
 * 关闭数据集
 * @return boolean
...
public boolean closeData()
{
	...
	public LCContPlanDutyParamSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LCContPlanDutyParamSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

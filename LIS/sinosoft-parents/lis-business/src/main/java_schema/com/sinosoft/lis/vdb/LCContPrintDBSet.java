...
/*
 * <p>ClassName: LCContPrintDBSet </p>
 * <p>Description: DB层多记录数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 数据模型修改
 */
public class LCContPrintDBSet extends LCContPrintSet
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LCContPrintDBSet(Connection tConnection)
	{
		con = tConnection;
		db = new DBOper(con,"LCContPrint");
		mflag = true;
	}

	public LCContPrintDBSet()
	{
		db = new DBOper( "LCContPrint" );
	...
	public boolean deleteSQL()
	{
		...
		else
		{
			...
			tError.moduleName = "LCContPrintDBSet";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			pstmt = con.prepareStatement("DELETE FROM LCContPrint WHERE  ContNo = ?");
		...
		SQLString sqlObj = new SQLString("LCContPrint");
...
ex.printStackTrace();
	...
	public boolean update()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("UPDATE LCContPrint SET  ContNo = ? , PrtFlag = ? , PrtTimes = ? , ManageCom = ? , Operator = ? , MakeDate = ? , MakeTime = ? , ModifyDate = ? , ModifyTime = ? , ContInfo = ? , ContType = ? WHERE  ContNo = ?");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getContNo() == null || this.get(i).getContNo().equals("null")) {
				pstmt.setString(1,null);
			...
			if(this.get(i).getContType() == null || this.get(i).getContType().equals("null")) {
				...
				pstmt.setString(11, this.get(i).getContType());
			}
			// set where condition
			if(this.get(i).getContNo() == null || this.get(i).getContNo().equals("null")) {
...
ex.printStackTrace();
	...
	public boolean insert()
	{
		...
		{
            int tCount = this.size();
			pstmt = con.prepareStatement("INSERT INTO LCContPrint VALUES( ? , ? , ? , ? , ? , ? , ? , ? , ? , ? , ?)");
            for (int i = 1; i <= tCount; i++)
            {
			if(this.get(i).getContNo() == null || this.get(i).getContNo().equals("null")) {
				pstmt.setString(1,null);

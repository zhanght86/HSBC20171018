...
/*
 * <p>ClassName: LLCaseReceiptDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 理赔管理
 */
public class LLCaseReceiptDB extends LLCaseReceiptSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LLCaseReceiptDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LLCaseReceiptSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LLCaseReceiptDB";
	...
	public boolean delete()
	{
		...
		try
		{
			...
			if(this.getFeeDetailNo() == null || this.getFeeDetailNo().equals("null")) {
				pstmt.setNull(4, 12);
			} else {
				pstmt.setString(4, this.getFeeDetailNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getFeeItemCode() == null || this.getFeeItemCode().equals("null")) {
				...
				pstmt.setString(7, this.getFeeItemCode());
			}
			if(this.getFeeItemName() == null || this.getFeeItemName().equals("null")) {
				pstmt.setNull(8, 12);
			} else {
				pstmt.setString(8, this.getFeeItemName());
			}
			pstmt.setDouble(9, this.getFee());
			pstmt.setDouble(10, this.getAdjSum());
			if(this.getAdjReason() == null || this.getAdjReason().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getAdjReason());
			}
			if(this.getAdjRemark() == null || this.getAdjRemark().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getAdjRemark());
			}
			...
			if(this.getDayCount() == null || this.getDayCount().equals("null")) {
				pstmt.setNull(15, 12);
			} else {
				pstmt.setString(15, this.getDayCount());
			}
			if(this.getDealFlag() == null || this.getDealFlag().equals("null")) {
				pstmt.setNull(16, 12);
			} else {
				pstmt.setString(16, this.getDealFlag());
			}
			if(this.getAvaliFlag() == null || this.getAvaliFlag().equals("null")) {
				pstmt.setNull(17, 1);
			} else {
				pstmt.setString(17, this.getAvaliFlag());
			}
			...
			pstmt.setDouble(24, this.getPreAmnt());
			pstmt.setDouble(25, this.getSelfAmnt());
			pstmt.setDouble(26, this.getRefuseAmnt());
			pstmt.setDouble(27, this.getCutApartAmnt());
			pstmt.setInt(28, this.getHospNum());
			pstmt.setDouble(29, this.getHospLineAmnt());
			if(this.getDiseaseCode() == null || this.getDiseaseCode().equals("null")) {
				pstmt.setNull(30, 12);
			} else {
				pstmt.setString(30, this.getDiseaseCode());
			}
			pstmt.setDouble(31, this.getSecurityAmnt());
			if(this.getDiseaseName() == null || this.getDiseaseName().equals("null")) {
				pstmt.setNull(32, 12);
			} else {
				pstmt.setString(32, this.getDiseaseName());
			}
			...
			pstmt.setDouble(35, this.getPartlyselfAmnt());
			pstmt.setDouble(36, this.getUnreasonAmnt());
			...
			if(this.getComCode() == null || this.getComCode().equals("null")) {
				...
				pstmt.setString(38, this.getComCode());
			}
			...
			if(this.getCaseNo() == null || this.getCaseNo().equals("null")) {
				...
				pstmt.setString(41, this.getCaseNo());
			}
			if(this.getMainFeeNo() == null || this.getMainFeeNo().equals("null")) {
				pstmt.setNull(42, 12);
			} else {
				pstmt.setString(42, this.getMainFeeNo());
			}
			if(this.getFeeDetailNo() == null || this.getFeeDetailNo().equals("null")) {
				pstmt.setNull(43, 12);
			} else {
				pstmt.setString(43, this.getFeeDetailNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LLCaseReceipt WHERE  ClmNo = ? AND CaseNo = ? AND MainFeeNo = ? AND FeeDetailNo = ?", 
	...
	public LLCaseReceiptSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLCaseReceiptSet aLLCaseReceiptSet = new LLCaseReceiptSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LLCaseReceipt");
			LLCaseReceiptSchema aSchema = this.getSchema();
			...
			{
				i++;
				LLCaseReceiptSchema s1 = new LLCaseReceiptSchema();
				s1.setSchema(rs,i);
				aLLCaseReceiptSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLLCaseReceiptSet;
	}

	public LLCaseReceiptSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLCaseReceiptSet aLLCaseReceiptSet = new LLCaseReceiptSet();
	...
	public LLCaseReceiptSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LLCaseReceiptSet aLLCaseReceiptSet = new LLCaseReceiptSet();
	...
	public LLCaseReceiptSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LLCaseReceiptSet aLLCaseReceiptSet = new LLCaseReceiptSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LLCaseReceipt " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LLCaseReceiptSet
 */
public LLCaseReceiptSet getData()
{
    int tCount = 0;
    ...
    try
    {
        ...
        tLLCaseReceiptSchema.setSchema(mResultSet, 1);
...
public boolean closeData()
{
	...
	public LLCaseReceiptSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LLCaseReceiptSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
import com.sinosoft.lis.db.ldstaffrateDB;

/*
 * <p>ClassName: ldstaffrateSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_1
 */
public class ldstaffrateSchema implements Schema, Cloneable
{
	// @Field
	/** 流水號 */
	private String IDNo;
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		ldstaffrateSchema cloned = (ldstaffrateSchema)super.clone();
	...
	*/
	...
	/**
	* 使用另外一个 ldstaffrateSchema 对象给 Schema 赋值
	* @param: aldstaffrateSchema ldstaffrateSchema
	**/
	public void setSchema(ldstaffrateSchema aldstaffrateSchema)
	{
		this.IDNo = aldstaffrateSchema.getIDNo();
		this.AgentCom = aldstaffrateSchema.getAgentCom();
		this.ManageCom = aldstaffrateSchema.getManageCom();
		this.RiskCode = aldstaffrateSchema.getRiskCode();
		this.PayYear = aldstaffrateSchema.getPayYear();
		this.Currency = aldstaffrateSchema.getCurrency();
		this.InvestType = aldstaffrateSchema.getInvestType();
		this.PayType = aldstaffrateSchema.getPayType();
		this.ProtectionPlan = aldstaffrateSchema.getProtectionPlan();
		this.PayYears = aldstaffrateSchema.getPayYears();
		this.CommRate = aldstaffrateSchema.getCommRate();
		this.StartDate = fDate.getDate( aldstaffrateSchema.getStartDate());
		this.EndDate = fDate.getDate( aldstaffrateSchema.getEndDate());
		this.ValidFlag = aldstaffrateSchema.getValidFlag();
		this.AppState = aldstaffrateSchema.getAppState();
		this.AppDate = fDate.getDate( aldstaffrateSchema.getAppDate());
		this.AppTime = aldstaffrateSchema.getAppTime();
		this.Branchtype = aldstaffrateSchema.getBranchtype();
		this.ActionType = aldstaffrateSchema.getActionType();
		this.OrigIDNo = aldstaffrateSchema.getOrigIDNo();
		this.Operator = aldstaffrateSchema.getOperator();
		this.MakeDate = fDate.getDate( aldstaffrateSchema.getMakeDate());
		this.MakeTime = aldstaffrateSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aldstaffrateSchema.getModifyDate());
		this.ModifyTime = aldstaffrateSchema.getModifyTime();
		this.StaffRate = aldstaffrateSchema.getStaffRate();
		this.InsureAgeStart = aldstaffrateSchema.getInsureAgeStart();
		this.InsureAgeEnd = aldstaffrateSchema.getInsureAgeEnd();
		this.PayToAge = aldstaffrateSchema.getPayToAge();
		this.DefaultFlag = aldstaffrateSchema.getDefaultFlag();
		this.SRFlag = aldstaffrateSchema.getSRFlag();
		this.PlanCode = aldstaffrateSchema.getPlanCode();
		this.Insuyear = aldstaffrateSchema.getInsuyear();
		this.InsuyearFlag = aldstaffrateSchema.getInsuyearFlag();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			System.out.println("数据库中的ldstaffrate表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			sqle.printStackTrace();
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "ldstaffrateSchema";
	...
	public ldstaffrateSchema getSchema()
	{
		ldstaffrateSchema aldstaffrateSchema = new ldstaffrateSchema();
		aldstaffrateSchema.setSchema(this);
		return aldstaffrateSchema;
	}

	public ldstaffrateDB getDB()
	{
		ldstaffrateDB aDBOper = new ldstaffrateDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#Prpldstaffrate描述/A>表字段
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#Prpldstaffrate>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		ldstaffrateSchema other = (ldstaffrateSchema)otherObject;
		return

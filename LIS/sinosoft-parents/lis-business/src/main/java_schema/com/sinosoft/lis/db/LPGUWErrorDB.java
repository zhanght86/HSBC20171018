...
/*
 * <p>ClassName: LPGUWErrorDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 保全流程
 */
public class LPGUWErrorDB extends LPGUWErrorSchema
{
private static Logger logger = Logger.getLogger(LPGUWErrorDB.class);
	...
	**/
	...
	// @Constructor
	public LPGUWErrorDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LPGUWErrorSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LPGUWErrorDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getGrpNo() == null || this.getGrpNo().equals("null")) {
				...
				pstmt.setString(9, this.getGrpNo());
			}
			...
			if(this.getUWPassFlag() == null || this.getUWPassFlag().equals("null")) {
				...
				pstmt.setString(15, this.getUWPassFlag());
			}
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LPGUWError WHERE  EdorNo = ? AND EdorType = ? AND GrpPolNo = ? AND SerialNo = ? AND UWNo = ?", 
	...
	public LPGUWErrorSet query()
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGUWErrorSet aLPGUWErrorSet = new LPGUWErrorSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LPGUWError");
			LPGUWErrorSchema aSchema = this.getSchema();
			...
			{
				i++;
				LPGUWErrorSchema s1 = new LPGUWErrorSchema();
				s1.setSchema(rs,i);
				aLPGUWErrorSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLPGUWErrorSet;

	}

	public LPGUWErrorSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGUWErrorSet aLPGUWErrorSet = new LPGUWErrorSet();
	...
	public LPGUWErrorSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
 		ResultSet rs = null;
		LPGUWErrorSet aLPGUWErrorSet = new LPGUWErrorSet();
	...
	public LPGUWErrorSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LPGUWErrorSet aLPGUWErrorSet = new LPGUWErrorSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LPGUWError " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LPGUWErrorSet
 */
public LPGUWErrorSet getData()
{
    int tCount = 0;
    LPGUWErrorSet tLPGUWErrorSet = new LPGUWErrorSet();
    LPGUWErrorSchema tLPGUWErrorSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LPGUWErrorDB";
    ...
    {
        tCount = 1;
        tLPGUWErrorSchema = new LPGUWErrorSchema();
        tLPGUWErrorSchema.setSchema(mResultSet, 1);
        tLPGUWErrorSet.add(tLPGUWErrorSchema);
...
public boolean closeData()
{
	...
	public LPGUWErrorSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LPGUWErrorSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

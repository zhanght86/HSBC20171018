...
/*
 * <p>ClassName: LDGrpTraceDB </p>
 * <p>Description: DB层数据库操作类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: 客户管理
 */
public class LDGrpTraceDB extends LDGrpTraceSchema
{
	// @Field
	...
	**/
	...
	// @Constructor
	public LDGrpTraceDB( Connection tConnection )
	{
		con = tConnection;
	...
	{
		LDGrpTraceSchema tSchema = this.getSchema();
		...
		else
		{
			...
			tError.moduleName = "LDGrpTraceDB";
	...
	public boolean update()
	{
		...
		try
		{
			...
			if(this.getServicePwd() == null || this.getServicePwd().equals("null")) {
				pstmt.setNull(3, 12);
			} else {
				pstmt.setString(3, this.getServicePwd());
			}
			...
			if(this.getGrpNature() == null || this.getGrpNature().equals("null")) {
				...
				pstmt.setString(9, this.getGrpNature());
			}
			pstmt.setInt(10, this.getSumNumPeople());
			if(this.getMainBusiness() == null || this.getMainBusiness().equals("null")) {
				pstmt.setNull(11, 12);
			} else {
				pstmt.setString(11, this.getMainBusiness());
			}
			if(this.getCorporation() == null || this.getCorporation().equals("null")) {
				pstmt.setNull(12, 12);
			} else {
				pstmt.setString(12, this.getCorporation());
			}
			if(this.getCorIDType() == null || this.getCorIDType().equals("null")) {
				pstmt.setNull(13, 12);
			} else {
				pstmt.setString(13, this.getCorIDType());
			}
			if(this.getCorlID() == null || this.getCorlID().equals("null")) {
				pstmt.setNull(14, 12);
			} else {
				pstmt.setString(14, this.getCorlID());
			}
			if(this.getIDExpiryDate() == null || this.getIDExpiryDate().equals("null")) {
				pstmt.setNull(15, 91);
			} else {
				pstmt.setDate(15, Date.valueOf(this.getIDExpiryDate()));
			}
			pstmt.setInt(16, this.getOnJobNumber());
			pstmt.setInt(17, this.getRetireNumber());
			pstmt.setInt(18, this.getOtherNumber());
			pstmt.setDouble(19, this.getRgtCapital());
			pstmt.setDouble(20, this.getTotalAssets());
			pstmt.setDouble(21, this.getNetProfitRate());
			if(this.getSatrap() == null || this.getSatrap().equals("null")) {
				pstmt.setNull(22, 12);
			} else {
				pstmt.setString(22, this.getSatrap());
			}
			if(this.getActuCtrl() == null || this.getActuCtrl().equals("null")) {
				pstmt.setNull(23, 12);
			} else {
				pstmt.setString(23, this.getActuCtrl());
			}
			if(this.getLicense() == null || this.getLicense().equals("null")) {
				pstmt.setNull(24, 12);
			} else {
				pstmt.setString(24, this.getLicense());
			}
			if(this.getSocialInsuCode() == null || this.getSocialInsuCode().equals("null")) {
				pstmt.setNull(25, 12);
			} else {
				pstmt.setString(25, this.getSocialInsuCode());
			}
			if(this.getOrganizationCode() == null || this.getOrganizationCode().equals("null")) {
				pstmt.setNull(26, 12);
			} else {
				pstmt.setString(26, this.getOrganizationCode());
			}
			if(this.getTaxCode() == null || this.getTaxCode().equals("null")) {
				pstmt.setNull(27, 12);
			} else {
				pstmt.setString(27, this.getTaxCode());
			}
			...
			if(this.getFax() == null || this.getFax().equals("null")) {
				...
				pstmt.setString(29, this.getFax());
			}
			...
			if(this.getMakeOperator() == null || this.getMakeOperator().equals("null")) {
				...
				pstmt.setString(40, this.getMakeOperator());
			}
			...
			if(this.getTraceID() == null || this.getTraceID().equals("null")) {
				...
				pstmt.setString(46, this.getTraceID());
			}
			if(this.getCustomerNo() == null || this.getCustomerNo().equals("null")) {
				pstmt.setNull(47, 12);
			} else {
				pstmt.setString(47, this.getCustomerNo());
			}
			pstmt.executeUpdate();
			pstmt.close();
		} catch (Exception ex) {
	...
	public boolean getInfo()
	{
		...
		try
		{
			pstmt = con.prepareStatement("SELECT * FROM LDGrpTrace WHERE  TraceID = ? AND CustomerNo = ?", 
	...
	public LDGrpTraceSet query()
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDGrpTraceSet aLDGrpTraceSet = new LDGrpTraceSet();
	  ...
	  if( !mflag ) {
		...
		{
			SQLString sqlObj = new SQLString("LDGrpTrace");
			LDGrpTraceSchema aSchema = this.getSchema();
			...
			{
				i++;
				LDGrpTraceSchema s1 = new LDGrpTraceSchema();
				s1.setSchema(rs,i);
				aLDGrpTraceSet.add(s1);
			}
			try{ rs.close(); } catch( Exception ex ) {}
			try{ pstmt.close(); } catch( Exception ex1 ) {}
		}
		...
		return aLDGrpTraceSet;
	}

	public LDGrpTraceSet executeQuery(String sql)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDGrpTraceSet aLDGrpTraceSet = new LDGrpTraceSet();
	...
	public LDGrpTraceSet query(int nStart, int nCount)
	{
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		LDGrpTraceSet aLDGrpTraceSet = new LDGrpTraceSet();
	...
	public LDGrpTraceSet executeQuery(String sql, int nStart, int nCount)
	{
		Statement stmt = null;
		ResultSet rs = null;
		LDGrpTraceSet aLDGrpTraceSet = new LDGrpTraceSet();
	...
	public boolean update(String strWherePart)
	{
	  ...
	  if( !mflag ) {
		...
		try
		{
			...
			String sql = "update LDGrpTrace " + sqlObj.getUpdPart() + " where " + strWherePart;
...
/**
 * 获取定量数据
 * @return LDGrpTraceSet
 */
public LDGrpTraceSet getData()
{
    int tCount = 0;
    LDGrpTraceSet tLDGrpTraceSet = new LDGrpTraceSet();
    LDGrpTraceSchema tLDGrpTraceSchema = null;
    if (null == mResultSet)
    {
        CError tError = new CError();
        tError.moduleName = "LDGrpTraceDB";
    ...
    {
        tCount = 1;
        tLDGrpTraceSchema = new LDGrpTraceSchema();
        tLDGrpTraceSchema.setSchema(mResultSet, 1);
        tLDGrpTraceSet.add(tLDGrpTraceSchema);
...
public boolean closeData()
{
	...
	public LDGrpTraceSet executeQuery(SQLwithBindVariables sqlbv) {
	...
	public LDGrpTraceSet executeQuery(SQLwithBindVariables sqlbv, int nStart, int nCount) {

...
import com.sinosoft.lis.db.LQInsuredDB;

/*
 * <p>ClassName: LQInsuredSchema </p>
 ...
 */
public class LQInsuredSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LQInsuredSchema.class);
	...
	public LQInsuredSchema()
	{
		...
		PK = pk;
            ...
            public Object clone()
                    throws CloneNotSupportedException
            {
                LQInsuredSchema cloned = (LQInsuredSchema)super.clone();
	...
	/**
	* 使用另外一个 LQInsuredSchema 对象给 Schema 赋值
	* @param: aLQInsuredSchema LQInsuredSchema
	**/
	public void setSchema(LQInsuredSchema aLQInsuredSchema)
	{
		this.AskPriceNo = aLQInsuredSchema.getAskPriceNo();
		this.AskNo = aLQInsuredSchema.getAskNo();
		this.ClassNo = aLQInsuredSchema.getClassNo();
		this.ManageCom = aLQInsuredSchema.getManageCom();
		this.ExecuteCom = aLQInsuredSchema.getExecuteCom();
		this.FamilyID = aLQInsuredSchema.getFamilyID();
		this.RelationToMainInsured = aLQInsuredSchema.getRelationToMainInsured();
		this.RelationToAppnt = aLQInsuredSchema.getRelationToAppnt();
		this.AddressNo = aLQInsuredSchema.getAddressNo();
		this.SequenceNo = aLQInsuredSchema.getSequenceNo();
		this.Name = aLQInsuredSchema.getName();
		this.Sex = aLQInsuredSchema.getSex();
		this.Birthday = fDate.getDate( aLQInsuredSchema.getBirthday());
		this.IDType = aLQInsuredSchema.getIDType();
		this.IDNo = aLQInsuredSchema.getIDNo();
		this.NativePlace = aLQInsuredSchema.getNativePlace();
		this.Nationality = aLQInsuredSchema.getNationality();
		this.RgtAddress = aLQInsuredSchema.getRgtAddress();
		this.Marriage = aLQInsuredSchema.getMarriage();
		this.MarriageDate = fDate.getDate( aLQInsuredSchema.getMarriageDate());
		this.Health = aLQInsuredSchema.getHealth();
		this.Stature = aLQInsuredSchema.getStature();
		this.Avoirdupois = aLQInsuredSchema.getAvoirdupois();
		this.Degree = aLQInsuredSchema.getDegree();
		this.CreditGrade = aLQInsuredSchema.getCreditGrade();
		this.BankCode = aLQInsuredSchema.getBankCode();
		this.BankAccNo = aLQInsuredSchema.getBankAccNo();
		this.AccName = aLQInsuredSchema.getAccName();
		this.JoinCompanyDate = fDate.getDate( aLQInsuredSchema.getJoinCompanyDate());
		this.StartWorkDate = fDate.getDate( aLQInsuredSchema.getStartWorkDate());
		this.Position = aLQInsuredSchema.getPosition();
		this.Salary = aLQInsuredSchema.getSalary();
		this.OccupationType = aLQInsuredSchema.getOccupationType();
		this.OccupationCode = aLQInsuredSchema.getOccupationCode();
		this.WorkType = aLQInsuredSchema.getWorkType();
		this.PluralityType = aLQInsuredSchema.getPluralityType();
		this.SmokeFlag = aLQInsuredSchema.getSmokeFlag();
		this.ContPlanCode = aLQInsuredSchema.getContPlanCode();
		this.Operator = aLQInsuredSchema.getOperator();
		this.InsuredStat = aLQInsuredSchema.getInsuredStat();
		this.MakeDate = fDate.getDate( aLQInsuredSchema.getMakeDate());
		this.MakeTime = aLQInsuredSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLQInsuredSchema.getModifyDate());
		this.ModifyTime = aLQInsuredSchema.getModifyTime();
		this.UWFlag = aLQInsuredSchema.getUWFlag();
		this.UWCode = aLQInsuredSchema.getUWCode();
		this.UWDate = fDate.getDate( aLQInsuredSchema.getUWDate());
		this.UWTime = aLQInsuredSchema.getUWTime();
		this.BMI = aLQInsuredSchema.getBMI();
		this.InsuredPeoples = aLQInsuredSchema.getInsuredPeoples();
		this.License = aLQInsuredSchema.getLicense();
		this.LicenseType = aLQInsuredSchema.getLicenseType();
		this.CustomerSeqNo = aLQInsuredSchema.getCustomerSeqNo();
		this.WorkNo = aLQInsuredSchema.getWorkNo();
		this.JoinContDate = fDate.getDate( aLQInsuredSchema.getJoinContDate());
		this.SendMsgFlag = aLQInsuredSchema.getSendMsgFlag();
		this.SendMailFlag = aLQInsuredSchema.getSendMailFlag();
		this.SocialInsuNo = aLQInsuredSchema.getSocialInsuNo();
		this.CertifyCode = aLQInsuredSchema.getCertifyCode();
		this.StartCode = aLQInsuredSchema.getStartCode();
		this.EndCode = aLQInsuredSchema.getEndCode();
		this.SocialInsuFlag = aLQInsuredSchema.getSocialInsuFlag();
		this.IDExpDate = fDate.getDate( aLQInsuredSchema.getIDExpDate());
		this.Identity = aLQInsuredSchema.getIdentity();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		try
		{
			...
			if( rs.getString("Identity") == null )
				this.Identity = null;
			else
				this.Identity = rs.getString("Identity").trim();

		}
		catch(SQLException sqle)
		{
			logger.debug("数据库中的LQInsured表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LQInsuredSchema";
	...
	public LQInsuredSchema getSchema()
	{
		LQInsuredSchema aLQInsuredSchema = new LQInsuredSchema();
		aLQInsuredSchema.setSchema(this);
		return aLQInsuredSchema;
	}

	public LQInsuredDB getDB()
	{
		LQInsuredDB aDBOper = new LQInsuredDB();
	...
	/**
	* 数据打包，按 XML 格式打包，顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLQInsured描述/A>表字段
...
strReturn.append(StrTool.cTrim(Identity));
		return strReturn.toString();
	}

	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLQInsured>历史记账凭证主表信息</A>表字段
	...
	**/
	public boolean decode(String strMessage)
	{
		try
		{
			...
			ExecuteCom = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 5, SysConst.PACKAGESPILTER );
			FamilyID = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 6, SysConst.PACKAGESPILTER );
			RelationToMainInsured = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 7, SysConst.PACKAGESPILTER );
			RelationToAppnt = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 8, SysConst.PACKAGESPILTER );
			AddressNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 9, SysConst.PACKAGESPILTER );
			...
			ContPlanCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 38, SysConst.PACKAGESPILTER );
			Operator = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 39, SysConst.PACKAGESPILTER );
			InsuredStat = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 40, SysConst.PACKAGESPILTER );
			...
			UWFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 45, SysConst.PACKAGESPILTER );
			UWCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 46, SysConst.PACKAGESPILTER );
			UWDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 47,SysConst.PACKAGESPILTER));
			UWTime = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 48, SysConst.PACKAGESPILTER );
			BMI = new Double(ChgData.chgNumericStr(StrTool.getStr(strMessage,49,SysConst.PACKAGESPILTER))).doubleValue();
			InsuredPeoples= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,50,SysConst.PACKAGESPILTER))).intValue();
			License = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 51, SysConst.PACKAGESPILTER );
			LicenseType = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 52, SysConst.PACKAGESPILTER );
			CustomerSeqNo= new Integer(ChgData.chgNumericStr(StrTool.getStr(strMessage,53,SysConst.PACKAGESPILTER))).intValue();
			WorkNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 54, SysConst.PACKAGESPILTER );
			JoinContDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 55,SysConst.PACKAGESPILTER));
			SendMsgFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 56, SysConst.PACKAGESPILTER );
			SendMailFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 57, SysConst.PACKAGESPILTER );
			SocialInsuNo = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 58, SysConst.PACKAGESPILTER );
			CertifyCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 59, SysConst.PACKAGESPILTER );
			StartCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 60, SysConst.PACKAGESPILTER );
			EndCode = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 61, SysConst.PACKAGESPILTER );
			SocialInsuFlag = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 62, SysConst.PACKAGESPILTER );
			IDExpDate = fDate.getDate(StrTool.getStr(StrTool.GBKToUnicode(strMessage), 63,SysConst.PACKAGESPILTER));
			Identity = StrTool.getStr(StrTool.GBKToUnicode(strMessage), 64, SysConst.PACKAGESPILTER );
		}
	...
	**/
	public String getV(String FCode)
	{
		...
		if (FCode.equalsIgnoreCase("Identity"))
		{
			strReturn = StrTool.GBKToUnicode(String.valueOf(Identity));
		}
		if (strReturn.equals(""))
		{
			strReturn = "null";
	...
	public String getV(int nFieldIndex)
	{
		...
		switch(nFieldIndex) {
			...
			case 62:
				...
				break;
			case 63:
				strFieldValue = StrTool.GBKToUnicode(Identity);
				break;
			default:
				strFieldValue = "";
		};
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LQInsuredSchema other = (LQInsuredSchema)otherObject;
		return
	...
	**/
	public int getFieldIndex(String strFieldName)
	{
		...
		if( strFieldName.equals("Identity") ) {
			return 63;
		}
		return -1;
	}

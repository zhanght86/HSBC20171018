...
/*
 * <p>ClassName: LLUWSpecMasterSchema </p>
 * <p>Description: DB层 Schema 类文件 </p>
 * <p>Copyright: Copyright (c) 2007</p>
 * <p>Company: sinosoft </p>
 * @Database: PhysicalDataModel_20
 */
public class LLUWSpecMasterSchema implements Schema, Cloneable
{
private static Logger logger = Logger.getLogger(LLUWSpecMasterSchema.class);
	// @Field
	/** 赔案号 */
	...
	*/
	public Object clone()
		throws CloneNotSupportedException
	{
		LLUWSpecMasterSchema cloned = (LLUWSpecMasterSchema)super.clone();
	...
	public void setProposalContNo(String aProposalContNo)
	{
		if(aProposalContNo!=null && aProposalContNo.length()>20)
			throw new IllegalArgumentException("投保单号码ProposalContNo值"+aProposalContNo+"的长度"+aProposalContNo.length()+"大于最大值20");
	...
	*/
	...
	{
		if(aSpecReason!=null && aSpecReason.length()>1000)
			throw new IllegalArgumentException("特约原因SpecReason值"+aSpecReason+"的长度"+aSpecReason.length()+"大于最大值1000");
	...
	public void setNeedPrint(String aNeedPrint)
	{
		if(aNeedPrint!=null && aNeedPrint.length()>1)
			throw new IllegalArgumentException("下发标记NeedPrint值"+aNeedPrint+"的长度"+aNeedPrint.length()+"大于最大值1");
		NeedPrint = aNeedPrint;
	}

	/**
	* 使用另外一个 LLUWSpecMasterSchema 对象给 Schema 赋值
	* @param: aLLUWSpecMasterSchema LLUWSpecMasterSchema
	**/
	public void setSchema(LLUWSpecMasterSchema aLLUWSpecMasterSchema)
	{
		this.ClmNo = aLLUWSpecMasterSchema.getClmNo();
		this.BatNo = aLLUWSpecMasterSchema.getBatNo();
		this.GrpContNo = aLLUWSpecMasterSchema.getGrpContNo();
		this.ContNo = aLLUWSpecMasterSchema.getContNo();
		this.ProposalContNo = aLLUWSpecMasterSchema.getProposalContNo();
		this.PrtSeq = aLLUWSpecMasterSchema.getPrtSeq();
		this.SerialNo = aLLUWSpecMasterSchema.getSerialNo();
		this.EndorsementNo = aLLUWSpecMasterSchema.getEndorsementNo();
		this.SpecType = aLLUWSpecMasterSchema.getSpecType();
		this.SpecCode = aLLUWSpecMasterSchema.getSpecCode();
		this.SpecContent = aLLUWSpecMasterSchema.getSpecContent();
		this.PrtFlag = aLLUWSpecMasterSchema.getPrtFlag();
		this.BackupType = aLLUWSpecMasterSchema.getBackupType();
		this.Operator = aLLUWSpecMasterSchema.getOperator();
		this.MakeDate = fDate.getDate( aLLUWSpecMasterSchema.getMakeDate());
		this.MakeTime = aLLUWSpecMasterSchema.getMakeTime();
		this.ModifyDate = fDate.getDate( aLLUWSpecMasterSchema.getModifyDate());
		this.ModifyTime = aLLUWSpecMasterSchema.getModifyTime();
		this.CustomerNo = aLLUWSpecMasterSchema.getCustomerNo();
		this.SpecReason = aLLUWSpecMasterSchema.getSpecReason();
		this.NeedPrint = aLLUWSpecMasterSchema.getNeedPrint();
	}
	...
	**/
	public boolean setSchema(ResultSet rs,int i)
	{
		...
		{
			logger.debug("数据库中的LLUWSpecMaster表字段个数和Schema中的字段个数不一致，或者执行db.executeQuery查询时没有使用select * from tables");
			// @@错误处理
			CError tError = new CError();
			tError.moduleName = "LLUWSpecMasterSchema";
	...
	public LLUWSpecMasterSchema getSchema()
	{
		LLUWSpecMasterSchema aLLUWSpecMasterSchema = new LLUWSpecMasterSchema();
		aLLUWSpecMasterSchema.setSchema(this);
		return aLLUWSpecMasterSchema;
	}

	public LLUWSpecMasterDB getDB()
	{
		LLUWSpecMasterDB aDBOper = new LLUWSpecMasterDB();
	...
	**/
	...
	/**
	* 数据解包，解包顺序参见<A href ={@docRoot}/dataStructure/tb.html#PrpLLUWSpecMaster>历史记账凭证主表信息</A>表字段
	...
	**/
	...
	public boolean equals(Object otherObject)
	{
		...
		LLUWSpecMasterSchema other = (LLUWSpecMasterSchema)otherObject;
		return

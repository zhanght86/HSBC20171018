/***************************************************************
 * <p>ProName：LSQuotUWQueryInput.js</p>
 * <p>Title：总公司核保</p>
...
function queryClick() {
	...
	tSQLInfo.setSqlId("LSQuotProcessSql3");
...
function perQueryClick() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_quot.LSQuotProcessSql");
	tSQLInfo.setSqlId("LSQuotProcessSql4");
	tSQLInfo.addSubPara(fm.QuotName.value);
	tSQLInfo.addSubPara(fm.QuotNo.value);
	tSQLInfo.addSubPara(fm.QuotType.value);
	tSQLInfo.addSubPara(tManageCom);
	...
	turnPage2.queryModal(tSQLInfo.getString(), PerQuotGrid, 2,1);
}
...
function reApplyClick() {
	...
	var tSelNo = PerQuotGrid.getSelNo();
	if (tSelNo==0) {
		alert("请选择需要操作的信息！");
		return false;
	}
	
	var tMissionID = PerQuotGrid.getRowColData(tSelNo-1, 3);
	var tSubMissionID = PerQuotGrid.getRowColData(tSelNo-1, 4);
	var tActivityID = PerQuotGrid.getRowColData(tSelNo-1, 5);
	
	fm.action = "./LSQuotAppSave.jsp?MissionID="+tMissionID+"&SubMissionID="+tSubMissionID+"&ActivityID="+tActivityID+"&Operate=RETURN";
	submitForm();
}

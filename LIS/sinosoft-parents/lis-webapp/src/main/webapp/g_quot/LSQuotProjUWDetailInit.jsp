<%
/***************************************************************
 * <p>ProName：LSQuotProjUWDetailInit.jsp</p>
 * <p>Title：项目询价核保明细</p>
...
function initComListGrid() {
	...
	try {
		...
		ComListGrid = new MulLineEnter("fm3", "ComListGrid");
		ComListGrid.mulLineCount = 0;
		ComListGrid.displayTitle = 1;
		ComListGrid.locked = 1;
		ComListGrid.canSel = 0;
		ComListGrid.canChk = 0;
		ComListGrid.hiddenSubtraction = 1;
		ComListGrid.hiddenPlus = 1;
		ComListGrid.loadMulLine(iArray);
	} catch (ex) {
		alert("初始化ComListGrid时出错："+ ex);
	}
}
...
function initRelaQuotListGrid() {
	...
	try {
		...
		RelaQuotListGrid = new MulLineEnter("fm3", "RelaQuotListGrid");
		RelaQuotListGrid.mulLineCount = 0;
		RelaQuotListGrid.displayTitle = 1;
		RelaQuotListGrid.locked = 1;
		RelaQuotListGrid.canSel = 0;
		RelaQuotListGrid.canChk = 0;
		RelaQuotListGrid.hiddenSubtraction = 1;
		RelaQuotListGrid.hiddenPlus = 1;
		RelaQuotListGrid.selBoxEventFuncName = "";
		RelaQuotListGrid.loadMulLine(iArray);
	} catch (ex) {
		alert("初始化RelaQuotListGrid时出错："+ ex);
	}
}

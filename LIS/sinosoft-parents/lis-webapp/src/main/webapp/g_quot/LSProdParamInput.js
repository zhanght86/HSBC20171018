...
function checkSubmit() {
	...
	if (fm2.FeeType.value=="0") {
		...
		if (Number(tYearValue)<0) {
			alert("年度管理费(元)/比例应大于等于0！");
			return false;
		}
		if (tYearFeeType == "201") {
			if (Number(tYearValue)>1) {
				alert("年度管理费(元)/比例应小于等于1！");
				return false;
			}
		}
	}

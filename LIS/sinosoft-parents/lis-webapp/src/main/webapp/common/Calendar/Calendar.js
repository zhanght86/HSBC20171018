...
{
    var e = getEvenSrc();   writeIframe();
    var o = WebCalendar.calendar.style; WebCalendar.eventSrc = e;
    if (arguments.length == 0) WebCalendar.objExport = e;
    else WebCalendar.objExport = eval(arguments[0]);
	WebCalendar.dateId = arguments[1];
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    ...
    {
        var returnValue;
        var a = (arguments.length==0 || typeof(arguments[0]) != "string") ? WebCalendar.day[this.id.substr(8)].split("/") : arguments[0].split("/");
        ...
        //如果日期显示框ID不为空，则通过ID赋值
		if(WebCalendar.dateId != null && WebCalendar.dateId != "null" && WebCalendar.dateId !="" 
			&& WebCalendar.dateId != undefined && WebCalendar.dateId != "undefined"){
			document.getElementById(WebCalendar.dateId).value = returnValue;
		} else {
...
{
    if (WebCalendar.eventSrc != getEvenSrc()) hiddenCalendar();
};

function getEvenSrc(){
	var event;
	if(jQuery.browser.msie || navigator.userAgent.match(/Trident\/(\d+)/i)){
 		event= window.event;//如果是ie
 	}
 	func=getEvenSrc.caller;
 	while(func!=null){
 		var arg0=func.arguments[0];
 		if(arg0){
 			if( arg0.constructor && ((arg0.constructor==Event || arg0.constructor ==MouseEvent))

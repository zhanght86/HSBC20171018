/*!
 * XqkChinese - 中文拼音处理javascript库
 *
 * @author 周杰忻<zjx@xqkblog.com>
 * @site http://www.xqkblog.com/
 *
 * @Version: 0.9 (build 110925)
 */
...
var xqk_pinyinlist = [
	...
	XqkChinese.checkPinyin = function(pinyin) {
		var chars = "abcdefghijklmnopqrstuvwxyz";
		for(var i=0; i<pinyin.length; i++) if( chars.indexOf(pinyin.charAt(i))<0) return false;
		return true;
	};
	XqkChinese.getPinyinOfChar = function(ch) {
		var count = xqk_pinyinlist.length / 2;
		for(var i=0; i<count; i++)
		{
			if( xqk_pinyinlist[i*2+1].indexOf(ch)>= 0) return xqk_pinyinlist[i*2];
		}
		return ch;
	};
	XqkChinese.getPinyinsOfCC = function(ch) {
		var count = xqk_pinyinlist.length / 2;
		var result = [];
		for(var i=0; i<count; i++)
		{
			if( xqk_pinyinlist[i*2+1].indexOf(ch)>= 0) result.push(xqk_pinyinlist[i*2]);
		}
		return result;
	};
	XqkChinese.getPinyin = function(chinese,sp){
	...
	XqkChinese.getPinyin2 = function(chinese){
		var s = "";
	...
	XqkChinese.getPinyinText = function(chinese, charSplit, sameSplit){
		var split1 = "";
		var split2 = null;
	...
	XqkChinese.createCTextArray = function(ctexts){
		var arr = [];
		for(var i=0; i<ctexts.length; i++) arr.push(this.getPinyin2(ctexts[i]));
		return arr;
	};
	
	//tongmeng 2012-04-12 add
	//for Lis
	XqkChinese.createCTextArrayForLis = function(ctexts){
		/*var arr = [];
		for(var i=0; i<ctexts.length; i++) 
		{

			var tempArr = [];
			var tempArray =  ctexts[i];
			for(var m=0;m<tempArray.length;m++)
  		{
 
  			tempArr.push(this.getPinyin2(tempArray[m]));	
  		}
		...
		{ 
			var temp = new Array();
			temp [0] = new Array();
			temp[0][0] = this.getPinyinForLis(ctexts[i]);
			temp[0][1] = "";
			arr.push(temp);
		}
		return arr;
	...
	XqkChinese.getPinyinForLis = function(chinese){
		var s = "";
		var tCode = chinese[0];
		var tCodeName = chinese[1];
		for(var i=1; i<2; i++){
			//s += ((i==0) ? "" : "'"); 
			
			
			for(var m=0;m<chinese[i].length;m++)
 
			{
				//s += chinese[i].charAt(m) ;
				var pys = this.getPinyinsOfCC(chinese[i].charAt(m));
				if(pys.length == 0) 
				{s += chinese[i].charAt(m);}
				else {
					s += pys[0];
		...
		//return {key:tCode, text: chinese, pinyin: s };
		return "key:"+tCode+";text:"+s +";value:" +chinese.join('&') +";" ;
	};
	
	XqkChinese.testPinyinAbbr = function(chinese, pinyin){
	...
	XqkChinese.testPinyinAbbr2 = function(chinese, pinyin){
		for(var i=0; i<chinese.length; i++) if( this.testPinyinAbbr(chinese.substring(i),pinyin) ) return true;
		return false;
	};
	
	XqkChinese.testPinyinAbbrByPinyin = function(fullPinyin, pinyin){
	...
	XqkChinese._testPinyinAbbrByPinyin = function(pinyin, pyarr, idx){
		if(pinyin.length == 0) return true;

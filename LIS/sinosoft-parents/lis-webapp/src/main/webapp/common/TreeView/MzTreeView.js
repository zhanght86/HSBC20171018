...
function MzTreeView(Tname)
{
  ...
  this.icons    = {
    ...
    folder    : 'folder_blue.png', //缺省的文件夹图标
  ...
  this.iconsExpand = {  //存放节点图片在展开时的对应图片
    PM0       : 'M0.gif',     //－┏
    PM1       : 'folderopen_blue.png',     //－┣
    PM2       : 'folderopen_blue.png',     //－┗
    PM3       : 'M3.gif',     //－━
...
MzTreeView.prototype.nodeToHTML = function(node, AtEnd)
{
  ...
  var HTML = "<DIV  style=\"width:170;text-overflow : clip ;\" noWrap='True'><NOBR>";
  if(!isRoot)
  {
    ...
    {
      node.iconExpand  = HCN ? AtEnd ? "PM2" : "PM1" :"file";
  ...
  //add by chenjianguo， 把之前左边那条线去掉，但由于跟节点需要那个房子的图片，所以判断如果是root节点的话，则增加《img》图片，反正就把之前左边的图片都删掉，这样显示的效果比较好看
  //好像可以再优化，希望高手来优化这段代码吧吧吧
  if(isRoot){
	  
	  HTML += "<IMG "+
	    "align='absMiddle' "+
	    "id='"+ this.name +"_icon_"+ id +"' "+
	    //sunsheng add next row for get icon.src 20110527
	    "name='s"+node.sourceIndex.substring(node.sourceIndex.indexOf("_",0)+1,node.sourceIndex.length)+"'" +
	    "src='"+ this.icons[node.icon].src +"'><A "+
	    //sunsheng add next row for get link.src 20110527
	     "name='"+node.sourceIndex.substring(node.sourceIndex.indexOf("_",0)+1,node.sourceIndex.length)+"'" +
	  ...
	  HTML += "<A style=\"vertical-align:middle;height:24px; line-height:24px;\""+
	    //sunsheng add next row for get link.src 20110527
	     "name='"+node.sourceIndex.substring(node.sourceIndex.indexOf("_",0)+1,node.sourceIndex.length)+"'" +
	    "class='MzTreeview' hideFocus "+
	    "id='"+ this.name +"_link_"+ id +"' "+

...
function getQueryResult()
{
	...
	var strSQL = ""	;
	/**
	strSQL = "select a.VersionNo,a.VersionState, decode(a.VersionState,'01','正常','02','维护','03','删除','其他')";
	strSQL = strSQL + " from FIRulesVersion a "// where a.VersionNo ='"+RulesVersionGrid.getRowColData(tRow-1,1)+"' ";
	*/
	var mySql1=new SqlClass();
		mySql1.setResourceName("fininterface.VersionRuleQueryForOtherSql"); //指定使用的properties文件名
		mySql1.setSqlId("VersionRuleQueryForOtherSql1");//指定使用的Sql的id
...
{
	// 初始化表格
	//20110512 by Cuizhe 页面异常，注掉alert
	//	alert("============");
  ...
  //   + getWherePart('VersionState')
     ...
     */
     var mySql2=new SqlClass();
		 mySql2.setResourceName("fininterface.VersionRuleQueryForOtherSql"); //指定使用的properties文件名
////20110512 by Cuizhe 传入参数个数与sql不一致，将VersionRuleQueryForOtherSql2改为VersionRuleQueryForOtherSql1
		 mySql2.setSqlId("VersionRuleQueryForOtherSql2");//指定使用的Sql的id
		 mySql2.addSubPara(document.all('VersionNo').value);//指定传入的参数
		 mySql2.addSubPara(document.all('AppDate').value);//指定传入的参数
		 mySql2.addSubPara(document.all('StartDate').value);//指定传入的参数
		 mySql2.addSubPara(document.all('EndDate').value);//指定传入的参数
		 mySql2.addSubPara(document.all('VersionState').value);//指定传入的参数

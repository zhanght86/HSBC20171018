...
function getQueryResult()
{
	...
	*/
	var mySql1=new SqlClass();
		mySql1.setResourceName("fininterface.AssociatedItemDefQuerySql"); //指定使用的properties文件名
		mySql1.setSqlId("AssociatedItemDefQuerySql1");//指定使用的Sql的id
		mySql1.addSubPara(AssociatedItemDefGrid.getRowColData(tRow-1,1));//指定传入的参数
		mySql1.addSubPara(AssociatedItemDefGrid.getRowColData(tRow-1,2));//指定传入的参数
...
//alert(strSQL);
	var mySql2=new SqlClass();
		mySql2.setResourceName("fininterface.AssociatedItemDefQuerySql"); //指定使用的properties文件名
		mySql2.setSqlId("AssociatedItemDefQuerySql2");//指定使用的Sql的id
		mySql2.addSubPara(fm.VersionNo.value);//指定传入的参数
		mySql2.addSubPara(fm.AssociatedID.value);//指定传入的参数
		mySql2.addSubPara(fm.AssociatedName.value);//指定传入的参数
		mySql2.addSubPara(fm.ColumnID.value);//指定传入的参数
		mySql2.addSubPara(fm.SourceColumnID.value);//指定传入的参数
		mySql2.addSubPara(fm.TransFlag.value);//指定传入的参数

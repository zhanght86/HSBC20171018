...
{                               
    ...
    try
    {
      ...
      iArray[11][11]="0|^0|简易模式^1|专家模式"; 
      
      iArray[12]=new Array();
      iArray[12][0]="失败后动作";      	   		//列名
      iArray[12][1]="80px";            		//列宽
      iArray[12][2]=20;            			//列最大值
      iArray[12][3]=2; 
      iArray[12][10]="RunAfterFlag"; 
      iArray[12][11]="0|^00|无动作^01|挂起"; 
...
//		var tSQL = " select serverip||':'||serverport,'有效结点' from LDTaskServer where validflag='1' ";
		
	/*mySql = new SqlClass();
	mySql.setResourceName("taskservice.TaskServiceSql");
	mySql.setSqlId("TaskServiceSql12");  
	mySql.addSubPara("有效节点"); 
...
{                               
	...
	try
	{
		...
		iArray[5]=new Array();                                                    
		iArray[5][0]="是否保存";         			//列名（此列为顺序号，列名无意义，而且不
		iArray[5][1]="0px";            		//列宽                                
		iArray[5][2]=10;            			//列最大值                              
		iArray[5][3]=2;              			//是否允许输入,1表示允许，0表示不允许   		

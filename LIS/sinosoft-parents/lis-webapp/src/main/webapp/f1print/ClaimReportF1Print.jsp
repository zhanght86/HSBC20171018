...
<%
	...
	loggerDebug("ClaimReportF1Print","报表类型代码:" + Code);
	//时间
	...
	loggerDebug("ClaimReportF1Print","案件阶段"+CaseSection);
	String CommonDate = request.getParameter("CommonDate");
	loggerDebug("ClaimReportF1Print","统计时效类型"+CommonDate);
	...
	if (!tClaimReportUI.submitData(tVData, "PRINTPAY")) {
		...
		if (txmlExport == null) {
			...
			loggerDebug("ClaimReportF1Print","没有得到要显示的数据文件");
		}
	}
	if (FlagStr.equals("Succ")) {
		...
		//把dataStream存储到磁盘文件
		loggerDebug("ClaimReportF1Print","存储文件到" + strVFPathName);

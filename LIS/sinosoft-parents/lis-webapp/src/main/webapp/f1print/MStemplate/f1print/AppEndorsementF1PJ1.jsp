...
<%
    ...
    loggerDebug("AppEndorsementF1PJ1","prtType: " + strType);
    loggerDebug("AppEndorsementF1PJ1",strEdorAcceptNo);
    ...
    if( strEdorAcceptNo != null && !strEdorAcceptNo.equals("") ) {  // 合并VTS模板文件与数据文件存入服务器磁盘中
        ...
        if(a > 1){
            //多个批单(一个模板)合并
            loggerDebug("AppEndorsementF1PJ1","============= 多个批单合并 ===========");
            loggerDebug("AppEndorsementF1PJ1","=========== 合并个数 ：" + a);
            ...
            for (int j = 1; j <= aSSRS.getMaxRow(); j++) 
            {
               try{
                  ...
                  if( conn == null ) 
                  {
                      ...
                      loggerDebug("AppEndorsementF1PJ1","SELECT * FROM LPEDORPRINT WHERE EdorNo = '" + aSSRS.GetText(j, 1) + "'");             
                      if( rs.next() )
                      {
                          //输出数据文件
                          ...
                          String tSQL = " and EdorNo = '" + aSSRS.GetText(j, 1) + "'";
	                        ...
	                        loggerDebug("AppEndorsementF1PJ1","==> 个人批单");
                          tBlob = tCOracleBlob.SelectBlob("LPEDORPRINT","edorinfo",tSQL,conn);
                          
                          o = new Object();
                          o = (InputStream)tBlob.getBinaryStream();
                          //ins=tBlob.getBinaryStream();
                          loggerDebug("AppEndorsementF1PJ1","get stream object");              
                      } 
                      else{
                          loggerDebug("AppEndorsementF1PJ1","can't get stream object");
                      }
                      rs.close();
                      stmt.close();
                      conn.close();                
                      ...
                      loggerDebug("AppEndorsementF1PJ1",strTemplatePath); 
            ...
            try
            {  
               loggerDebug("AppEndorsementF1PJ1","========79679=========" + strName);
            ...
            //单个模板
            loggerDebug("AppEndorsementF1PJ1","单个模板");
            CombineVts tcombineVts=null;
            InputStream ins = null;
            try
            {
                ...
                { 
                   ...
                   loggerDebug("AppEndorsementF1PJ1","SELECT * FROM LPEDORPRINT WHERE EdorNo = '" + aSSRS.GetText(1, 1) + "'");  
                   if( rs.next() )
                   {
                       //输出数据文件
                       COracleBlob tCOracleBlob = new COracleBlob();
                   ...
                   loggerDebug("AppEndorsementF1PJ1","==> Write VTS file to disk ");   
                   session.putValue("RealPath", strName);
                   response.sendRedirect("GetF1PrintJ1.jsp");               
                }
            }

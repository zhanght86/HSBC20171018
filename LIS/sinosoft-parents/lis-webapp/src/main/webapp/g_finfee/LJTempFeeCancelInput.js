...
function queryApplyTempFee() {
	...
	tSQLInfo.setSqlId("LJTempFeeSql4");
...
function cancelTempFee() {
	...
	tOperate = "CANCELTEMPFEE";
	fm.action = "./LJTempFeeCancelSave.jsp?Operate="+ tOperate +"&TempFeeNo="+ tTempFeeNo;
	submitForm(fm);
}
...
function checkData() {
	...
	var tCancelDesc = trim(fm.CancelDesc.value);

	if (tCancelDesc==null || tCancelDesc=="") {
		alert("结论描述不能为空！");
		return false;
	}
	
	if (tCancelDesc.length>300) {
		alert("结论描述长度应在300字内！");
		return false;
	}
...
function expData() {
	...
	var tTitle = "暂交费号^|交费方式^|客户开户行^|客户银行账号^|客户账户名^|金额^|投保单位名称^|客户经理" +
			"^|申请人^|申请日期^|到账日期^|收款银行^|收款账号^|付款单位^|支票号^|录入人" +
			"^|录入日期^|审核人^|审核日期";
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_finfee.LJTempFeeSql");
	tSQLInfo.setSqlId("LJTempFeeSql12");
	tSQLInfo.addSubPara(tManageCom);
	tSQLInfo.addSubPara(fm.QueryPayType.value);
	tSQLInfo.addSubPara(fm.QueryCustBankCode.value);
	tSQLInfo.addSubPara(fm.QueryCustBankAccNo.value);

...
function queryInfo() {
	...
	tSQLInfo.setSqlId("DebtsPaySql2");
...
function confirmClick() {
	...
	var tConfirmConlusion = fm.ConfirmConlusion.value;
	var tConfirmDesc = fm.ConfirmDesc.value;
	if (tConfirmConlusion==null || tConfirmConlusion=="") {
		alert("审核结论不能为空！");
		return false;
	}
	
	if (tConfirmConlusion=='01') {
		
		if (tConfirmDesc=="" || tConfirmDesc==null) {
	...
	fm.action = "./LJDebtsPayConfirmSave.jsp?Operate="+ tOperate +"&DebtsPayNo="+ tDebtsPayNo;
	submitForm(fm);
}
...
function dealAfterSubmit(cFlag) {
	...
	if (cFlag!='Fail') {
		...
		initDebtsPayInfoGrid();
		fm.ConfirmConlusion.value = "";
		fm.ConfirmConlusionName.value = "";

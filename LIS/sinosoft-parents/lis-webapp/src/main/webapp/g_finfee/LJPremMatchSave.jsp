<%
/***************************************************************
 * <p>ProName：LJPremMatchSave.jsp</p>
 * <p>Title：保费匹配</p>
...
<%
	...
	if (tGI==null) {
		...
		try {
			...
			if ("PREMMATCH".equals(tOperate)) {
				...
				if (tMatchFeeChk!=null && tMatchFeeChk.length>0) {
					...
					for (int i=0; i<tMatchFeeChk.length; i++) {
						...
						if ("1".equals(tMatchFeeChk[i])) {
							...
							tLJPremMatchFeeSchema.setMatchSerialNo(tMatchSerialNo);
				...
				if (tBussChk!=null && tBussChk.length>0) {
					...
					for (int i=0; i<tBussChk.length; i++) {
						...
						if ("1".equals(tBussChk[i])) {
							...
							tLJPremMatchPaySchema.setMatchSerialNo(tMatchSerialNo);
			...
			} else if ("MATCHCONFIRM".equals(tOperate)) {
				...
				LJPremMatchConfirmSchema tLJPremMatchConfirmSchema = new LJPremMatchConfirmSchema();
				tLJPremMatchConfirmSchema.setMatchSerialNo(tMatchSerialNo);
				tLJPremMatchConfirmSchema.setInputConclusion(tInputConclusion);
				tLJPremMatchConfirmSchema.setInputDesc(tInputDesc);
				
				tVData.add(tLJPremMatchConfirmSchema);
			} 
			
			BusinessDelegate tBusinessDelegate = BusinessDelegate.getBusinessDelegate();
			if (!tBusinessDelegate.submitData(tVData, tOperate, "LJPremMatchUI")) {

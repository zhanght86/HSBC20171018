...
//  	var risksql="select SubRiskFlag from lmriskapp where riskcode='"+document.all('RiskCode').value+"'"
		...
		var sqlid0="CombinationConfigSql0";
		var mySql0=new SqlClass();
		mySql0.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
...
//			+ "and a.DutyCode in "+ getWhere + " and a.ChooseFlag in ('2') "
//			+ "and  a.RiskCode = '"+document.all('RiskCode').value+"' order by a.RiskCode,a.DutyCode,a.FactorOrder";
		
		var sqlid1="CombinationConfigSql1";
		var mySql1=new SqlClass();
		mySql1.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
...
//			+ "from LMRiskDutyFactor a, LMRisk b, LMDuty c "
//			+ "where a.RiskCode = b.RiskCode and a.DutyCode = c.DutyCode  "
//			+ "and a.DutyCode in "+ getWhere + " and a.ChooseFlag in ('2') "//3为个险产品组合
//			+ " and a.RiskCode = '"+document.all('RiskCode').value+"' order by a.RiskCode,a.DutyCode,a.FactorOrder";
		
		var sqlid2="CombinationConfigSql2";
		var mySql2=new SqlClass();
		mySql2.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
...
function submitForm(){
	...
	if (document.all('mOperate').value == "INSERT||MAIN"){
		if (!confirm('组合 '+document.all('ContPlanCode').value+' 下的全部险种要素信息是否已录入完毕，您是否要确认操作？')){
			return false;
		}
	}
...
//   var tSql ="select count(1) from ldplan where ContPlanCode='"+tContPlanCode+"'";
	...
	var sqlid3="CombinationConfigSql3";
	var mySql3=new SqlClass();
	mySql3.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
	mySql3.setSqlId(sqlid3);//指定使用的Sql的id
	mySql3.addSubPara(tContPlanCode);//指定传入的参数
	var tSql=mySql3.getString();
   
   var tCount = easyExecSql(tSql);
   if(tCount>0){
       alert("编码为["+tContPlanCode+"]的组合已经存在,请查看“已存在的产品组合”！");
       return false;
   }
   return true;
}
...
function  clearData(){
	...
	document.all("Peoples3").value="";
	document.all("PlanKind1").value="";
...
function beforeSubmit(){
	...
	if (document.all('mOperate').value != "UPDATE||MAIN"){
		...
		var MainRiskCode = document.all('MainRiskCode').value;
//		var tSql = "select 1 from lmriskapp where riskcode='"+MainRiskCode+"'"
//					+ " and SubRiskFlag='M'";
		
		var sqlid4="CombinationConfigSql4";
		var mySql4=new SqlClass();
		mySql4.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
		mySql4.setSqlId(sqlid4);//指定使用的Sql的id
		mySql4.addSubPara(tContPlanCode);//指定传入的参数
		var tSql=mySql4.getString();
		
		turnPage.strQueryResult  = easyQueryVer3(tSql, 1, 0, 1);//prompt("",tCheckSql);
		//如果没数据也无异常
		if (!turnPage.strQueryResult) {
			alert("请先保存主险");
			return false;
	...
	if(document.all('mOperate').value == "UPDATE||MAIN"){
		//校验主附险能否搭配
		var tMainRisk = ContPlanGrid.getRowColData(1,2);
		//alert(tMainRisk);
//		var tCheckSql = "select 1 from lmriskrela where riskcode='"
//						+ tMainRisk +"'"
//						+ " and relariskcode='"+fm.RiskCode.value+"'";
		
		var sqlid5="CombinationConfigSql5";
		var mySql5=new SqlClass();
		mySql5.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
		mySql5.setSqlId(sqlid5);//指定使用的Sql的id
		mySql5.addSubPara(tMainRisk);//指定传入的参数
		mySql5.addSubPara(fm.RiskCode.value);//指定传入的参数
		var tCheckSql=mySql5.getString();
		
		turnPage.strQueryResult  = easyQueryVer3(tCheckSql, 1, 0, 1);//prompt("",tCheckSql);
		//如果没数据也无异常
		if (!turnPage.strQueryResult) {
			alert("主险"+tMainRisk+"与附加险"+fm.RiskCode.value+"不能搭配");
			return false;
		}
...
//		   + " and (RiskCode = '"+tRiskCode+"' or RiskCode ='000000' )";
	...
	var sqlid6="CombinationConfigSql6";
	var mySql6=new SqlClass();
	mySql6.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
	mySql6.setSqlId(sqlid6);//指定使用的Sql的id
	mySql6.addSubPara(tRiskCode);//指定传入的参数
	mySql6.addSubPara(tRiskCode);//指定传入的参数
...
//		+ "and a.RiskCode = '"+document.all('RiskCode').value+"' order by a.DutyCode";
	...
	var sqlid7="CombinationConfigSql7";
	var mySql7=new SqlClass();
	mySql7.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
	mySql7.setSqlId(sqlid7);//指定使用的Sql的id
	mySql7.addSubPara(document.all('RiskCode').value);//指定传入的参数
...
//	  tSql="select choflag from lmriskduty where riskcode='"+document.all('RiskCode').value+"' and dutycode='"+cDutyCode+"'";
		...
		var sqlid8="CombinationConfigSql8";
		var mySql8=new SqlClass();
		mySql8.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
		mySql8.setSqlId(sqlid8);//指定使用的Sql的id
		mySql8.addSubPara(document.all('RiskCode').value);//指定传入的参数
		mySql8.addSubPara(cDutyCode);//指定传入的参数
...
//		+ " order by ContPlanCode";
	...
	var sqlid9="CombinationConfigSql9";
	var mySql9=new SqlClass();
	mySql9.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
...
function ShowContPlan(parm1,parm2){
	if(document.all(parm1).all('InpContPlanCodeGridSel').value == '1'){
		...
		var cRemark=document.all(parm1).all('ContPlanCodeGrid5').value;//特别约定
		var cPlanKind1=document.all(parm1).all('ContPlanCodeGrid6').value;//套餐层级1
		var cPlanKind2=document.all(parm1).all('ContPlanCodeGrid7').value;//套餐层级2		
        var cPlanKind3=document.all(parm1).all('ContPlanCodeGrid8').value;//套餐层级3
        var cManageCom=document.all(parm1).all('ContPlanCodeGrid9').value;//管理机构
...
//			+ "  order by a.RiskCode,d.MainRiskCode,a.DutyCode";
		...
		var sqlid10="CombinationConfigSql10";
		var mySql10=new SqlClass();
		mySql10.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
		mySql10.setSqlId(sqlid10);//指定使用的Sql的id
		mySql10.addSubPara(document.all('ContPlanCode').value);//指定传入的参数
		strSQL=mySql10.getString();
		
		//document.all('PlanSql').value = strSQL;
		turnPage.strQueryResult  = easyQueryVer3(strSQL, 1, 0, 1);//prompt("",strSQL);
		//原则上不会失败，嘿嘿
		if (!turnPage.strQueryResult) {
			alert("查询失败！");
			return false;
		...
		if(arrResult!=null){
			fm.MainRiskCode.value = arrResult[0][11];
		}
	}
}
...
//	var sql="select RiskCode, RiskName from LMRiskApp where (enddate is null or enddate>'"+fm.sysdate.value+"') and riskprop in ('I','C','D') "
//    +" union select riskcode,(select riskname from lmrisk where riskcode=lmriskcomctrl.riskcode) from LMRiskComCtrl where "
//    +" startdate<='"+fm.sysdate.value+"' and (enddate is null or enddate>'"+fm.sysdate.value+"') and ManageComGrp='"+tManageCom+"' "
//    +"  and (select distinct(riskprop) from lmriskapp where riskcode =lmriskcomctrl.riskcode) in ('I','C','D') order by RiskCode";
	
	var sqlid11="CombinationConfigSql11";
	var mySql11=new SqlClass();
	mySql11.setResourceName("config.CombinationConfigSql"); //指定使用的properties文件名
	mySql11.setSqlId(sqlid11);//指定使用的Sql的id
	mySql11.addSubPara(fm.sysdate.value);//指定传入的参数
	mySql11.addSubPara(fm.sysdate.value);//指定传入的参数
	mySql11.addSubPara(fm.sysdate.value);//指定传入的参数
	mySql11.addSubPara(tManageCom);//指定传入的参数

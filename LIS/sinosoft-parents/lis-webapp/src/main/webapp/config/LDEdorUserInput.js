...
function submitForm()
{
 ...
 if(document.all('UWPopedom').value == document.all('MaxGrade').value)//当该核保师的核保权限为E-当前为最高是 上级核保级别置为'B' modify by liuqh 2008-12-12
   {
      alert("此核保师的核保权限为最高:"+document.all('UWPopedom').value+", 其上级核保级别将仍然为B！");
      document.all('UpUwPopedom').value ="B";
...
function getMaxGrade(){
	...
	mySql1.setResourceName("config.LDEdorUserInputSql");//指定使用的properties文件名
	mySql1.setSqlId(sqlid1);//指定使用的Sql的id
	mySql1.addSubPara();//指定传入的参数
	var tMaxGradeSQL=mySql1.getString();
  //var tMaxGradeSQL = "select max(code) from ldcode where codetype = 'bqgrppopedom'";
...
function updateClick()
{
  ...
  var sqlid2="DSHomeContSql1";
	var mySql2=new SqlClass();
	mySql2.setResourceName("config.LDEdorUserInputSql");//指定使用的properties文件名
	mySql2.setSqlId(sqlid2);//指定使用的Sql的id
	mySql2.addSubPara(fm.UserCode.value);//指定传入的参数
	//这儿添加usercode和uwtype两个才能确定唯一要修改的一条记录
	mySql2.addSubPara(document.getElementById("UWType1").value);//指定传入的参数
	var tSql=mySql2.getString();
	
 // var tSql="select count(*) from LDUWUser where uwtype='3' and usercode='"+fm.UserCode.value+"'";
  var tCount= easyExecSql(tSql);
  if(tCount == null)          //if(tCount=="0") 
  {
  ...
  //当该核保师的核保权限为最高时 上级核保级别置为'B' modify by liuqh 2008-12-12
...
function queryClick()
{
 ...
 { 
  ...
  var sqlid3="DSHomeContSql1";
	var mySql3=new SqlClass();
	mySql3.setResourceName("config.LDEdorUserInputSql");//指定使用的properties文件名
	mySql3.setSqlId(sqlid3);//指定使用的Sql的id
	mySql3.addSubPara(fm.UserCode.value);//指定传入的参数
	mySql3.addSubPara("3");//指定传入的参数,因为增加的时候是:tLDUWUserSchema.setUWType("3")且UWType为联合主键其中的一个、理论上不能为空
	var strSql=mySql3.getString();
//  var strSql = " select uwpopedom,(select max(codename) from ldcode where codetype='bqgrppopedom' "
//              +" and trim(code)=a.uwpopedom),upuwpopedom,(select max(codename) from ldcode where "
//              +" codetype='bqgrppopedom' and trim(code)=a.upuwpopedom) from LDUWUser a "
//              +" where usercode='"+fm.UserCode.value+"' and uwtype='3' ";
	var arrResult = easyExecSql(strSql);
  if(arrResult != null)
  {
	//alert(arrResult);
	fm.UserCode.value=arrResult[0][0];
	fm.UWPopedom.value=arrResult[0][1];
	...
	fm.UWPopedom1.value=arrResult[0][1];
...
function afterQuery( arrQueryResult )
{
	...
	{   
		...
		mySql4.setResourceName("config.LDEdorUserInputSql");//指定使用的properties文件名
		mySql4.setSqlId(sqlid4);//指定使用的Sql的id
		mySql4.addSubPara(type);//指定传入的参数
		mySql4.addSubPara(arrQueryResult[0][0]);//指定传入的参数
		var strSQL=mySql4.getString();
		
//		var strSQL="select UserCode,UWType,UpUserCode,Upuwpopedom,uwpopedom,Remark,Operator,ManageCom,MakeDate,MakeTime,ModifyDate,ModifyTime, AddPoint,ClaimPopedom,uwpopedom,riskrate,specjob,lowestamnt,overage from LDUWUser where uwtype='"+type+"' and UserCode='"+arrQueryResult[0][0]+"'";
	 	//alert(strSQL);
	 	...
	 	document.all('UserCode1').value= arrResult[0][0];
               ...
               document.all('Remark').value= arrResult[0][5];
                ...
                var sqlid5="DSHomeContSql5";
								...
								mySql5.setResourceName("config.LDEdorUserInputSql");//指定使用的properties文件名
...
//                var strSQL2="select UWState,UWStateName from LDUWGrade where uwtype='"+type+"' and UserCode='"+arrQueryResult[0][0]+"'";
                ...
                //alert(arrQueryResult[0][1]);
              	...
              	var sqlid6="DSHomeContSql6";
								...
								mySql6.setResourceName("config.LDEdorUserInputSql");//指定使用的properties文件名
								mySql6.setSqlId(sqlid6);//指定使用的Sql的id
								mySql6.addSubPara(type);//指定传入的参数
								mySql6.addSubPara(arrQueryResult[0][0]);//指定传入的参数
								strSQL2=mySql6.getString();
...
function getMaxAmntInfo()
{
  ...
  //      var strSQL ="select Uwkind,(select codename from ldcode a where trim(a.codetype)='uwkind' and trim(a.code)=trim(Uwkind)),MaxAmnt from LDUWGradePerson where UWType='"+UWType+"' and UWPopedom='"+UWPopedom+"'";
       ...
       var sqlid7="DSHomeContSql7";
			...
			mySql7.setResourceName("config.LDEdorUserInputSql");//指定使用的properties文件名
			mySql7.setSqlId(sqlid7);//指定使用的Sql的id
			mySql7.addSubPara(UWType);//指定传入的参数
			mySql7.addSubPara(UWPopedom);//指定传入的参数

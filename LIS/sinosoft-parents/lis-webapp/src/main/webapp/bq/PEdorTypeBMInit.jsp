...
function initInpBox()
{
  ...
  {
    alert("在PEdorTypeBMInit.jsp-->InitInpBox函数中发生异常:初始化界面错误!");
  }
}
...
function initSelBox()
{
  ...
  {
    alert("在PEdorTypeBMInit.jsp-->InitSelBox函数中发生异常:初始化界面错误!");
  }
}
*/
...
function initForm()
{
  ...
  {
    alert("PEdorTypeBMInit.jsp-->InitForm函数中发生异常:初始化界面错误!");
  }
}
...
// 										+" Left Join (Select code,codename From ldcode Where codetype='sex') sex On sex.code = i.sex Where contno='"+tContNo+"'";
	//alert("-----------"+strSQL+"------------");
	  sql_id = "PEdorTypeBMInputSql2";
	  my_sql = new SqlClass();
	  my_sql.setResourceName("bq.PEdorTypeBMInputSql"); //指定使用的properties文件名
...
//       var tSql = "SELECT distinct RiskCode,"
...
function initPolGrid()
{
    var iArray = new Array();
      ...
      try
      {
        ...
        iArray[5]=new Array();
        iArray[5][0]="原红利领取方式编码";                    //列名（此列为顺序号，列名无意义，而且不显示）
        iArray[5][1]="0px";                  //列宽
        iArray[5][2]=100;                      //列最大值
        iArray[5][3]=0;

        iArray[6]=new Array();
        iArray[6][0]="原红利领取方式";                    //列名（此列为顺序号，列名无意义，而且不显示）
        iArray[6][1]="70px";                  //列宽
        iArray[6][2]=100;                      //列最大值
        iArray[6][3]=0;
        ...
        iArray[7]=new Array();
        iArray[7][0]="现红利领取方式编码";                    //列名（此列为顺序号，列名无意义，而且不显示）
        iArray[7][1]="90px";                  //列宽
        iArray[7][2]=100;                      //列最大值
        iArray[7][3]=2;
        iArray[7][4]="bonusgetriskmode";
        iArray[7][5]="7|8";
        iArray[7][6]="0|1";
        iArray[7][9]="红利领取方式|len<=50";
        iArray[7][15]= "riskcode";  //要依赖的列的名称        
		iArray[7][17]= "1";     //该列的列号
        
        
        iArray[8]=new Array();
        iArray[8][0]="现红利领取方式";                    //列名（此列为顺序号，列名无意义，而且不显示）
        iArray[8][1]="70px";                  //列宽
        iArray[8][2]=100;                      //列最大值
        iArray[8][3]=0;
        ...
        PolGrid.canChk=1;       //1-显示checkbox选项，0-不显示checkbox选项
...
// 		 		         +" case when (select bonusgetmode from LPPol  where polno=a.polno and EdorType='BM' and EdorNo='"+fm.EdorNo.value+"') is not null then "
// 		 		         +" (select bonusgetmode from LPPol  where polno=a.polno and EdorType='BM' and EdorNo='"+fm.EdorNo.value+"') else bonusgetmode end,"
// 		 		         +" case when (select codename from ldcode,LPPol d  where codetype='bonusgetmode' and d.polno=a.polno and code=d.bonusgetmode and d.EdorNo='"+fm.EdorNo.value+"') is not null then "
// 		 		         +" (select codename from ldcode,LPPol d  where codetype='bonusgetmode' and d.polno=a.polno and code=d.bonusgetmode and d.EdorNo='"+fm.EdorNo.value+"') else (select codename from ldcode where codetype='bonusgetmode' and code=a.bonusgetmode) end"
// 		      + " FROM LCPol a"
// 		      + " WHERE a.appflag='1' and ContNo='" + document.all('ContNo').value + "' and exists (select 'X' from lmriskapp b where  a.RiskCode=b.RiskCode and b.bonusflag='1')";
		 		      sql_id = "PEdorTypeBMInputSql4";
		 		 	  my_sql = new SqlClass();
		 		 	  my_sql.setResourceName("bq.PEdorTypeBMInputSql"); //指定使用的properties文件名
		 		 	  my_sql.setSqlId(sql_id);//指定使用的Sql的id
...
//           +" c.polno,"
//           +" c.rnewflag,"
//           +" decode(c.rnewflag,'-1','自动续保','不续保'),"
//           +" nvl((select p1.rnewflag from lppol p1 where p1.polno = c.polno and p1.edorno = '"+document.all("EdorNo").value+"'),'-2'), "
//           +" decode((select p2.rnewflag from lppol p2 where p2.polno = c.polno and p2.edorno = '"+document.all("EdorNo").value+"'),'-1','自动续保','不续保'), "

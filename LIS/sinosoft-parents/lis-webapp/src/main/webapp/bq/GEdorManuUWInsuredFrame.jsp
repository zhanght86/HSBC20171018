...
<frameset name="fraMain" framespacing="0" frameborder="no" rows="0,0,0,0,*" cols="*" border="1">
    ...
    <frameset name="fraSet" framespacing="0" frameborder="no" rows="*" cols="0%,*,0%" border="1">
        ...
        <!-- 实现交互的区域 -->
        <%
            String sInterface = request.getParameter("Interface");
            sInterface += "?EdorAcceptNo=" + request.getParameter("EdorAcceptNo");
            sInterface += "&GrpContNo=" + request.getParameter("GrpContNo");
            sInterface += "&MissionID=" + request.getParameter("MissionID");
            sInterface += "&SubMissionID=" + request.getParameter("SubMissionID");
            sInterface += "&ActivityStatus=" + request.getParameter("ActivityStatus");
        %>
        <frame name="fraInterface" src="<%=sInterface%>" scrolling="auto">
        <!-- 下一步操作区域 -->

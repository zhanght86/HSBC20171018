<!--用户校验类-->
 ...
 <%@page import="com.sinosoft.lis.bq.PEdorLGDetailUI"%>
...
<%
    ...
    tG = (GlobalInput)session.getValue("GI");  
        ...
        tVData.addElement(tLPEdorItemSchema);
            PEdorLGDetailUI tPEdorLGDetailUI = new PEdorLGDetailUI();
            if (!tPEdorLGDetailUI.submitData(tVData, "EdorQuery"))
            {
                tErrors.copyAllErrors(tPEdorLGDetailUI.mErrors);
            }
            VData tResult = new VData();
            tResult = tPEdorLGDetailUI.getResult();
            if(tResult.size()<=0)
            {
            	tErrors.addOneError("获取帐户信息失败！");
            }
            else
            {

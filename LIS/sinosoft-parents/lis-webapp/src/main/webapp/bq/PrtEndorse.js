...
//	              +" order by a.EdorAppDate,a.modifytime";
	...
	mySql1.setResourceName("bq.PrtEndorseSql"); // 指定使用的properties文件名
	mySql1.setSqlId(sqlid1);// 指定使用的Sql的id
	mySql1.addSubPara(comcode.substring(0,6));// 指定传入的参数
...
//	       + " order by EdorAppDate,modifytime";
	...
	mySql2.setResourceName("bq.PrtEndorseSql"); // 指定使用的properties文件名
	mySql2.setSqlId(sqlid2);// 指定使用的Sql的id
	mySql2.addSubPara(window.document.getElementsByName(trim('ContNo'))[0].value);// 指定传入的参数
	mySql2.addSubPara(window.document.getElementsByName(trim('EdorNo'))[0].value);// 指定传入的参数
	mySql2.addSubPara(window.document.getElementsByName(trim('EdorAcceptNo'))[0].value);// 指定传入的参数
	mySql2.addSubPara(comcode.substring(0,6));// 指定传入的参数
	mySql2.addSubPara(window.document.getElementsByName(trim('ManageCom'))[0].value);// 指定传入的参数
...
function initManageCom()
{
	if(comcode.substring(0,6) !=null && comcode.substring(0,6) != "")
	{
	   ...
	   // var strSQL = "select comcode,name from ldcom where comcode like '"+comcode.substring(1,6)+"%%'";
		...
		mySql3.setResourceName("bq.PrtEndorseSql"); // 指定使用的properties文件名
		mySql3.setSqlId(sqlid3);// 指定使用的Sql的id
		mySql3.addSubPara(comcode.substring(0,6));// 指定传入的参数

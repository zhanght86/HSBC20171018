...
function getQueryResult()
{
	...
	mySql1.setResourceName("bq.BqGrpNoticeQueryInputSql");//指定使用的properties文件名
	mySql1.setSqlId(sqlid1);//指定使用的Sql的id
	mySql1.addSubPara(AgentGrid.getRowColData(tRow-1,1));//指定传入的参数
...
function replyAsk()
{
	...
	if(tSelNo != null && tSelNo >= 0)
	{
		...
		var tPrtSeq = AgentGrid.getRowColData(tSelNo,1); 
	...
	showInfo.focus();
    fm.action = "GEdorNoticeSave.jsp?AskOperate=REPLY";
    fm.target = "fraSubmit";
...
function ShowAskInfo()
{
    ...
    if (i != '0')
    {
        ...
        var tAskInfo = AgentGrid.getRowColData(i,7); 
...
function easyQueryClick()
{
	...
	mySql2.setResourceName("bq.BqGrpNoticeQueryInputSql");//指定使用的properties文件名
	mySql2.setSqlId(sqlid2);//指定使用的Sql的id
	mySql2.addSubPara(fm.PrtSeq11.value);//指定传入的参数
	mySql2.addSubPara(fm.GrpContNo.value);//指定传入的参数
	mySql2.addSubPara(fm.EdorAcceptNo.value);//指定传入的参数
	mySql2.addSubPara(fm.AskType.value);//指定传入的参数
	mySql2.addSubPara(fm.DispPer.value);//指定传入的参数
	mySql2.addSubPara(fm.Asker.value);//指定传入的参数
	
	strSQL=mySql2.getString();
	
//	strSQL = "select PrtSeq,OtherNo,StandbyFlag1,StandbyFlag2,decode(StandbyFlag3,'1','人工核保问题件','2','保全审批问题件','其它'),MakeDate,StandbyFlag4 from LOPRTManager where 1=1 "
//	+ "and StateFlag = 'A' and othernotype='04' " 
//	+ getWherePart('PrtSeq','PrtSeq11',strOperate)
//	+ getWherePart('OtherNo','GrpContNo',strOperate)
//	+ getWherePart('StandbyFlag1','EdorAcceptNo',strOperate)
//	+ getWherePart('StandbyFlag3','AskType',strOperate2)
//	+ getWherePart('StandbyFlag5','DispPer',strOperate2)
//	+ getWherePart('StandbyFlag2','Asker',strOperate2)
...
function getEdorInfo()
{
	...
	mySql3.setResourceName("bq.BqGrpNoticeQueryInputSql");//指定使用的properties文件名
	mySql3.setSqlId(sqlid3);//指定使用的Sql的id
	mySql3.addSubPara();//指定传入的参数

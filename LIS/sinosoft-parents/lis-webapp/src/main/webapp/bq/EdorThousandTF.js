...
function fileDownload() { 
 ...
 if( verifyInput() == false ) 
  ...
  var name='提示';   //网页名称，可为空; 
	...
	showInfo.focus(); 
  var filename="EdorthousandTF_"+document.all('ManageCom').value+"_"+document.all('StartDate').value+"_"+document.all('EndDate').value+"_"+document.all('SaleChnl').value+".xls";
...
//	var strSql = "select SysVarValue from LDSysVar where SysVar = 'TKReportCreat'";
	var strSql = "";
	var sqlid1="EdorThousandTFSql1";
	var mySql1=new SqlClass();
	mySql1.setResourceName("bq.EdorThousandTFSql"); //指定使用的properties文件名
   ...
   fm.action="./EdorThousandTFSave.jsp";
   fm.submit(); 
}
...
//	var strSql = "select SysVarValue from LDSysVar where SysVar = 'TKReportCreat'";
	var strSql2 = "";
	var sqlid2="EdorThousandTFSql2";
	var mySql2=new SqlClass();
	mySql2.setResourceName("bq.EdorThousandTFSql"); //指定使用的properties文件名
	mySql2.setSqlId(sqlid2);//指定使用的Sql的id
	strSql2=mySql2.getString();	
   filePath = easyExecSql(strSql2);
   //filePath="d:/zhujc/";
   fm.Url.value=filePath;
...
function ConfirmSelect()
{
	 ...
	 if(confirm("数据已经生成，是否重新计算?如果重算将覆盖原始数据。"))
	 {
       ...
       var urlStr="../common/jsp/MessagePage.jsp?picture=C&content=" + showStr ;  
		...
		showInfo.focus();	 	   //点确定之后将其值变为1 起到开关的作用

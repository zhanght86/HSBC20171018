<%
//程序名称：InsuAccYearBillPrintInit.jsp
//程序功能：清单打印
...
function initInpBox()
{
  ...
  document.all('StartDate').value = '<%=dateArr[0]%>';
	...
	document.all('CurTime').value='<%=PubFun.getCurrentTime()%>'
	document.all('SaleChnl').value = "";
	document.all('ManageCom').value = "<%=strManageCom%>";
  }
  catch(ex)
  {
    alert("在InsuAccYearBillPrintInit.jsp-->InitInpBox函数中发生异常:初始化界面错误!");
  }
}
...
function initSelBox()
{
  ...
  {
    alert("在InsuAccYearBillPrintInit.jsp-->InitSelBox函数中发生异常:初始化界面错误!");
  }
}
...
function initForm()
{
  ...
  {
    alert("InsuAccYearBillPrintInit.jsp-->InitForm函数中发生异常:初始化界面错误!");
  }
}

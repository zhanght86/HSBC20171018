...
function printBill()
{
  ...
  if((StartDate == "" || EndDate == "") && EdorType=='2')
  {
        alert("请输入起始日期和结束日期!");
        return;
  }
   if((CountDate == "" || CountDate == null) && EdorType=='1')
  {
        alert("请输入统计止期!");
        return;
  }
  if(EdorType == "" || EdorType == "")
  {
        alert("请选择类型!");
        return;
  }
...
function afterCodeSelect(sCodeListType, oCodeListField)
{
    ...
    if (sCodeListType == "lntype")
    {
        ...
        if (oCodeListField.value == "2")
        {
            try
            {
                document.all("divDateBy").style.display = "none";
                document.all("divDateCount").style.display = "";  

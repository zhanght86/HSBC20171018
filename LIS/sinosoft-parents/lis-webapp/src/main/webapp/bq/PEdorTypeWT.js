...
function edorTypeCTSave()
{
 ...
 if(tPolFLag&&fm.FeeGetFlag.value=='1')
 {
 	...
 	{
 		if(!confirm("银代险种犹豫期退保默认不扣除工本费，您选择扣除是否继续"))
 		{
 			return ;
 			}
 	}else if(fm.SaleChnl.value!='03' && fm.FeeGetFlag.value=='0')
 		{
 		 if(!confirm("险种犹豫期退保没有选择扣除工本费，是否继续"))
 		{
 			return ;
 		}		
 	  }	
 	  
 	    //如果单退附加险，对每个附加险是否有犹豫期进行判断，如果没有，则不可以进行退保
      ...
      for(var k = 0; k < row; k++ )
      {
        if(PolGrid.getChkNo(k))
        {
         ...
         {
             alert("险种"+PolGrid.getRowColData(k,1)+"没有犹豫期，不可以进行犹豫期退保");
             return false;
         }         
        }
      }	  	    	
 	    }
 	     //如果整单退，对主险是否有犹豫期进行判断，如果没有，则不可以进行退保
...
//	var tSQL="select 1 from LMEdorWT where riskcode='"+tRiskCode+"'";
	...
	mySql17.addSubPara(tRiskCode);//指定传入的参数

...
function initForm(tPolNo,tPrtNo,tEdorNo,tMissionID,tEdorType)
{
  var str = "";
  try
  {
	...
	//initUWSpecGrid(str);
	initPolAddGrid();
	initHide(tPolNo,tPrtNo,tEdorNo,tMissionID,tEdorType);
	QueryPolAddGrid(tPrtNo,tEdorNo);	
	//QueryGrid(tPolNo,tEdorNo);	
    //QueryAddReason(tPolNo,tEdorNo);
...
function initUWSpec2Grid(tstr)
  {                              
    var iArray = new Array();
     ...
     Spec2Grid = new MulLineEnter( "fm" , "Spec2Grid" ); 
      //这些属性必须在loadMulLine前                            
      Spec2Grid.mulLineCount = 1;
      Spec2Grid.displayTitle = 1;
      Spec2Grid.hiddenPlus = 1;        //隐藏加号
     Spec2Grid.hiddenSubtraction = 1; //隐藏减号
      Spec2Grid.loadMulLine(iArray);  
    }
      catch(ex)
      {
        alert(ex);
...
function initHide(tPolNo,tPrtNo,tEdorNo,tMissionID,tEdorType)
{        

...
<frameset name="fraMain" framespacing="0" frameborder="no" rows="0,0,0,0,*" cols="*" border="1">
    ...
    <frameset name="fraSet" framespacing="0" frameborder="no" rows="*" cols="0%,*,0%" border="1">
        ...
        <!-- 实现交互的区域 -->
        <frame id="fraInterface" name="fraInterface" src="EdorUWManuSpecInput.jsp?EdorNo=<%=request.getParameter("EdorNo")%>&EdorType=<%=request.getParameter("EdorType")%>&MissionID=<%=request.getParameter("MissionID")%>&SubMissionID=<%=request.getParameter("SubMissionID")%>&ContNo=<%=request.getParameter("ContNo")%>&PolNo=<%=request.getParameter("PolNo")%>&PrtNo=<%=request.getParameter("PrtNo")%>" scrolling="auto">
        <!-- 下一步操作区域 -->

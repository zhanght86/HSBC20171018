<%
//程序名称：EdorBonusBillPrintInit.jsp
//程序功能：清单打印
...
function initInpBox()
{
  ...
  document.all('StartDate').value = '<%=dateArr[0]%>';
	...
	document.all('FinaYear').value = '<%=PubFun.getCurrentDate().substring(0,4)%>';
	document.all('ManageCom').value = "<%=strManageCom%>";
  }
  catch(ex)
  {
    alert("在EdorBonusBillPrintInit.jsp-->InitInpBox函数中发生异常:初始化界面错误!");
  }
}
...
function initSelBox()
{
  ...
  {
    alert("在EdorBonusBillPrintInit.jsp-->InitSelBox函数中发生异常:初始化界面错误!");
  }
}
...
function initForm()
{
  ...
  {
    alert("EdorBonusBillPrintInit.jsp-->InitForm函数中发生异常:初始化界面错误!");
  }
}

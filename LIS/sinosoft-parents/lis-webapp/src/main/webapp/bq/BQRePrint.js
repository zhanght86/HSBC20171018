...
//	    + getWherePart('LWMission.MissionProp4','CodeS')	  ; 
  //alert(aResult);
	...
	var  CodeS1 = window.document.getElementsByName(trim("CodeS"))[0].value;
	 ...
	 mySql1.addSubPara(strManageCom);//指定传入参数
	 mySql1.addSubPara(StartDay1);//指定传入参数
	 mySql1.addSubPara(EndDay1);//指定传入参数
	 mySql1.addSubPara(ContNoS1);//指定传入参数
	 mySql1.addSubPara(PrtSeqS1);//指定传入参数
	 mySql1.addSubPara(CodeS1);//指定传入参数
...
function checkdata(aContNo,aPrtSeq,aCode)
{
	...
	if(aCode == "BQ90")
	 ...
	 mySql2.addSubPara(aContNo);//指定传入参数
	 mySql2.addSubPara(aPrtSeq);//指定传入参数
	 mySql2.addSubPara(wherepart);//指定传入参数

...
function initForm()
{
  try
  {
    ...
    initMainPolGrid();
    queryLostTimes();
    initLRQuery(); //查询已录入补发信息
    //showmoney();
  }
  catch(re)
  {
    alert("PEdorTypeLRInit.jsp-->InitForm函数中发生异常:初始化界面错误!");
  }
}
...
function initInpBox()
{
  ...
  {
    alert("在PEdorTypeLRInit.jsp-->InitInpBox函数中发生异常:初始化界面错误!" + ex);
  }
}
...
// 										+" Left Join (Select code,codename From ldcode Where codetype='sex') sex On sex.code = i.sex Where contno='"+tContNo+"'";
	//alert("-----------"+strSQL+"------------");
	  ...
	  mySql1.setResourceName("bq.PEdorTypeLRInputSql"); //指定使用的properties文件名

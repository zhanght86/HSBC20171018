...
<%
	...
	if (tG == null) {
		loggerDebug("ClaimRgtConclusionSave","未获取到登录信息!");
		return;
	}
	...
	tTransferData.setNameAndValue("CustomerNo", request.getParameter("CustomerNo")); //出险人
	...
	if (tLLGrpClaimPrtUI.submitData(tVData, "RgtConclusion") == false) {
		...
		if (txmlExport == null) {
			...
			loggerDebug("ClaimRgtConclusionSave","没有得到要显示的数据文件");
		}
	}
	...
	if (FlagStr.equals("Succ")) {
		...
		loggerDebug("ClaimRgtConclusionSave","strVFFileName=" + strVFFileName);

		//获取存放临时文件的路径
		String strRealPath = application.getRealPath("/").replace('\\', '/');
		loggerDebug("ClaimRgtConclusionSave","strRealPath=" + strRealPath);
		...
		//把dataStream存储到磁盘文件
		loggerDebug("ClaimRgtConclusionSave","存储文件到" + strVFPathName);

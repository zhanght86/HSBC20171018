...
function displayDicTable(destination,jsonData) {
   mdestination = destination;
	...
	for ( var i = 0; i < ColumnHeadArray.length; i++) {
		var newJson = null;
		width+=200;
		//alert(ColumnDataTypeArray[i]+":"+TableColumnNameArray[i]);
		if (ColumnDataTypeArray[i]== "String") {
			...
			newJson = {
		 	...
		 	//alert('button:'+TableColumnNameArray[i]+":ColumnHeadArray[i]:"+ColumnHeadArray[i]+":"+TableColumnNameArray[i]);
			newJson = {
				header :ColumnHeadArray[i],
				dataIndex :TableColumnNameArray[i],
				width :200,
	...
	var newJson1 = null;
		...
		strJson[strJson.length] = newJson1;
		dataId[dataId.length] = { name : 'MultMsg'
			};
			*/
	//alert("2");
...
//
function  setRuleNameListener(sm, rowIndex, r) {
	if (!InputNodeEnabled) {
		return;
	}
	getInputNodes();
...
function renderDescn(value, cellmeta, record, rowIndex, columnIndex, store) {
	 ...
	 if(tRES_DEFINE==null||tRES_DEFINE=='')
	 {
	 	tRES_DEFINE = "定义提示信息";
			...
			if(mdestination=="init")
			{
				  str = "<input type='button' value='定义提示信息' onclick='addMsgResult(\""+mRuleName+value+"\",\"0\")'>";
			}
			else
			{
				 str = "<input type='button' value='定义提示信息' onclick='addMsgResult(\""+mRuleName+value+"\",\"1\")'>";
			}*/
...
function checkNodesState(flag)
{
   ...
   case 0:
   {
	   ...
	   disableSpanNodes();
	   hideButtons();
	   document.getElementById('cTableName').disabled = true;
	   document.getElementById('cTableButton').style.display = 'none';
	   break;
   }
   case 1:
   {

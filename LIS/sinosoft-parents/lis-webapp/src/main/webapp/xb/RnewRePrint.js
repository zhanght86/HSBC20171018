...
//	    + getWherePart('LWMission.MissionProp4','CodeS')	  ; 
  //alert(aResult);
	...
	mySql1.setResourceName("xb.RnewRePrintSql"); //指定使用的properties文件名
	mySql1.setSqlId(sqlid1);//指定使用的Sql的id
	mySql1.addSubPara(strManageCom);//指定传入的参数
	mySql1.addSubPara(window.document.getElementsByName(trim('StartDay'))[0].value);//指定传入的参数
	mySql1.addSubPara(window.document.getElementsByName(trim('EndDay'))[0].value);//指定传入的参数
	mySql1.addSubPara(window.document.getElementsByName(trim('ContNoS'))[0].value);//指定传入的参数
	mySql1.addSubPara(window.document.getElementsByName(trim('PrtSeqS'))[0].value);//指定传入的参数
	mySql1.addSubPara(window.document.getElementsByName(trim('CodeS'))[0].value);//指定传入的参数
...
//	var tResult = easyExecSql("select 1 from lwmission where missionprop2 = '" + aContNo + "' and missionprop3 = '" + aPrtSeq + "'" + wherepart,1,0);
	...
	mySql2.setResourceName("xb.RnewRePrintSql"); //指定使用的properties文件名
	mySql2.setSqlId(sqlid1);//指定使用的Sql的id
	mySql2.addSubPara(aContNo);//指定传入的参数
	mySql2.addSubPara(aPrtSeq);//指定传入的参数

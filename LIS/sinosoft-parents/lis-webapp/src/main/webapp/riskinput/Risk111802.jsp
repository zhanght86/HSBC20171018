			    ...
			    sqlVData.add(sqlTransferData);
			  	BusinessDelegate tsqlBusinessDelegate=BusinessDelegate.getBusinessDelegate();
			  	  ...
			  	  {    
			  	       ...
			  	       { 
			  				loggerDebug("Risk111802","查询失败，原因是:" + tsqlBusinessDelegate.getCErrors().getFirstError());
			  		   }
			  		   else
			  		   {
			  			 	loggerDebug("Risk111802","查询失败");				
			  		   }
			  	  }
	riskFlagSSRS=(SSRS)tsqlBusinessDelegate.getResult().get(0);
    loggerDebug("Risk111802","祝福险标记====="+riskFlagSSRS.GetText(1,1));
...
<table>
	<tr>
		...
		<td class=titleImg>保单信息 <INPUT VALUE="查询责任信息" TYPE=button class=cssButton
			onclick="showDuty();"> <INPUT VALUE="关联暂交费信息" TYPE=button class=cssButton
			onclick="showFee();"> <!--<INPUT id="butChooseDuty" VALUE="选择责任" TYPE=button class=cssButton onclick="chooseDuty();" disabled >
<INPUT id="butBack" VALUE="返回" TYPE=button class=cssButton disabled >--></td>
	</tr>
</table>
...
<TABLE class=common>
	...
	<TR CLASS=common>
		...
		<TD CLASS=input COLSPAN=3><Input id= NAME=Remark VALUE="" CLASS=common3
			MAXLENGTH=255></TD>
...
<table>
	<tr>
		...
		<td class=titleImg>投保人信息（客户号：<Input class=common
			...
			TYPE=button class=cssButton onclick="queryAppntNo();"> 首次投保客户无需填写客户号）</td>
	</tr>
</table>
...
<TABLE class=common>
	...
	<TR CLASS=common>
		...
		<TD CLASS=input COLSPAN=1><Input class=wid id=AppGrpNo class=wid id= NAME=AppGrpNo></TD>
...
<table>
	<tr>
		...
		<INPUT id="butBack" VALUE="查询" TYPE=button class=cssButton onclick="queryInsuredNo();">
		首次投保客户无需填写客户号）
...
<TABLE class=common>
	...
	<TR CLASS=common>
		...
		<TD CLASS=input COLSPAN=1><Input id=Mult NAME=Mult MAXLENGTH=12
			CLASS="wid common" verify="份数|notnull"></TD>
			<td class=title>币种</td>
    					<td class=input><Input class="wid common" id=CurrencyCode name=CurrencyCode readonly=true value='01'></td>
	</TR>
	<TR CLASS=common>
...
<TABLE class=common>
	<TR CLASS=common>
		...
		<TD CLASS=input COLSPAN=1><Input class=codeno name=InsuYear id=InsuYear
			verify="保险期间|code:!InsuYear&notnull" style="background:url(../common/images/guanliyuan-bg.png) no-repeat right center"
		...
		<TD CLASS=input COLSPAN=1><Input id=InsuYearFlag NAME=InsuYearFlag VALUE="A"
			class="readonly wid" readonly TABINDEX=-1 verify="保险期间单位|notnull">
		</TD>
	</TR>
	...
	<TR CLASS=common>
		...
		<TD CLASS=input COLSPAN=1><Input class=codeno name=GetYear id=GetYear
			verify="起领期间|code:!GetYear&notnull" style="background:url(../common/images/guanliyuan-bg.png) no-repeat right center"
		...
		<TD CLASS=input COLSPAN=1><Input id= NAME=GetYearFlag id=GetYearFlag VALUE="A"
			class="readonly wid" readonly TABINDEX=-1 verify="起领期间单位|notnull">
		</TD>
	</TR>
	...
	<TR CLASS=common>
		...
		<TD CLASS=input COLSPAN=1><Input class=codeno name=PayIntv id=PayIntv
			verify="缴费方式|code:!PayIntv&notnull"
		...
		<TD CLASS=input COLSPAN=1><Input class=codeno name=GetDutyKind id=GetDutyKind
			verify="给付方法|code:!GetDutyKind&notnull" style="background:url(../common/images/guanliyuan-bg.png) no-repeat right center"
	...
	<TR CLASS=common>
		...
		<TD CLASS=input COLSPAN=1><Input class=codeno name=PayEndYear id=PayEndYear
			verify="交费期间|code:!PayEndYear&notnull" style="background:url(../common/images/guanliyuan-bg.png) no-repeat right center"
		...
		<TD CLASS=input COLSPAN=1><Input id= NAME=PayEndYearFlag VALUE="Y"
...
window.top.opener.returnFromRiskInput('111802');
}

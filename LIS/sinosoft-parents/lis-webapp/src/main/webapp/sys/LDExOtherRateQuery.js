...
//var strSQL = "";
//
//strSQL = "select currcode,per,destcurrcode,decimal(exchrate,14,4),startdate,enddate from LDExOtherRate where 1 = 1"
//				+ getWherePart('CurrCode')
//				+ getWherePart('DestCurrCode')
//				+ getWherePart('StartDate')
//				+ getWherePart('EndDate');
//				
//if(document.all('Per').value!=null&&!document.all('Per').value=='')
//{
//	strSQL=strSQL+" and Per="+document.all('Per').value;
//}
//
//if(document.all('ExchRate').value!=null&&!document.all('ExchRate').value=='')
//{
//	strSQL=strSQL+" and ExchRate="+document.all('ExchRate').value;
//}
	
	var sqlid1	= "ExRateSql5";
	var mySql1=new SqlClass();
	mySql1.setResourceName(sqlresourcename); //指定使用的properties文件名
	mySql1.setSqlId(sqlid1);//指定使用的Sql的id
	mySql1.addSubPara(fm.CurrCode.value);
	mySql1.addSubPara(fm.DestCurrCode.value);		
	mySql1.addSubPara(fm.StartDate.value);
	mySql1.addSubPara(fm.EndDate.value);		
	mySql1.addSubPara(fm.Per.value);
	mySql1.addSubPara(fm.ExchRate.value);		
...
function resetForm()
{
  ...
  {
  	alert("在LDCurrencyQuery.js-->resetForm函数中发生异常:初始化界面错误!");
  }
} 

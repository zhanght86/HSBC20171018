 ...
 * //得到被选中的记录字符串 tRow--; //alert(tRow); var
 * str=document.all("AgentInformation"+tRow).value; var arrRecord = str.split("|");
 * //拆分字符串，形成返回的数组 for(var i=0; i<63;i++) { if(arrRecord[i]=='null') {
...
function returnParent() {
	...
	if (tSel == 0 || tSel == null) {
		...
		try {
			...
			//火狐浏览器兼容性问题解决,火狐浏览器不支持window.close()
			if(navigator.userAgent.indexOf("Firefox") > 0){
				window.top.close();
			}else{ //其它浏览器、比如IE、chrome
				top.close();
			}
		} catch (ex) {
			alert("要返回的页面函数出错！错误信息是：" + ex.name + ":" + ex.message);
		}
	}
}
...
function getQueryResult() {
	...
	mySql3.setResourceName("sys.LDPersonQueryNewSql"); // 指定使用的properties文件名
	mySql3.setSqlId(sqlid3);// 指定使用的Sql的id
	mySql3.addSubPara(AgentGrid.getRowColData(tRow - 1, 1));// 指定传入的参数
...
function easyQueryClick() {
	...
	mySql4.setResourceName("sys.LDPersonQueryNewSql"); // 指定使用的properties文件名
	mySql4.setSqlId(sqlid4);// 指定使用的Sql的id
	mySql4.addSubPara(fm.AgentCode.value);// 指定传入的参数
	mySql4.addSubPara(fm.AgentGroup.value);// 指定传入的参数
	mySql4.addSubPara(fm.ManageCom.value);// 指定传入的参数
	mySql4.addSubPara(fm.Name.value);// 指定传入的参数
	mySql4.addSubPara(fm.Sex.value);// 指定传入的参数
	mySql4.addSubPara(fm.IDNo.value);// 指定传入的参数
	var addStr1 = " and 1=1 ";
	var addStr2 = " and 2=2 ";
	...
	mySql4.addSubPara(addStr1);// 指定传入的参数
	mySql4.addSubPara(addStr2);// 指定传入的参数
	strSQL = mySql4.getString();
	turnPage.queryModal(strSQL, AgentGrid);
	/*
	 * turnPage.strQueryResult = easyQueryVer3(strSQL, 1, 1, 1);
	 * 
	 * //判断是否查询成功 if (!turnPage.strQueryResult) { alert("未查询到满足条件的数据！");
	 * unlockScreen('lkscreen'); return false; } //查询成功则拆分字符串，返回二维数组 arrDataSet =
	 * decodeEasyQueryResult(turnPage.strQueryResult); //tArr =
	 * decodeEasyQueryResult(turnPage.strQueryResult); turnPage.arrDataCacheSet =
	 * arrDataSet; //设置初始化过的MULTILINE对象，VarGrid为在初始化页中定义的全局变量
	 * turnPage.pageDisplayGrid = AgentGrid;
	 * 
	 * //保存SQL语句 turnPage.strQuerySql = strSQL;
	 * 
	 * //设置查询起始位置 turnPage.pageIndex = 0;
	 * 
	 * //在查询结果数组中取出符合页面显示大小设置的数组 //arrDataSet =
	 * turnPage.getData(turnPage.arrDataCacheSet, turnPage.pageIndex,
	 * MAXSCREENLINES); var tArr = new Array(); tArr =
	 * turnPage.getData(turnPage.arrDataCacheSet, turnPage.pageIndex,
	 * MAXSCREENLINES); //调用MULTILINE对象显示查询结果
	 * 

...
function easyQueryClick()
{
	...
	mySql41.setResourceName("sys.ProposalQuerySql"); //指定使用的properties文件名
	mySql41.setSqlId(sqlid41); //指定使用的Sql的id
	mySql41.addSubPara(tActuGetNo); //指定传入的参数
	strSQL=mySql41.getString();
	
	//strSQL = "select ActuGetNo,contno,OtherNoType,BonusYear,(select codename from ldcode where codetype='paymode' and code=PayMode),GetMoney,GetDate,decode(FeeOperationType,'XJTF','红利现金给付','LJTF','红利累计生息给付','DJTF','红利抵交保费给付','HLTF','红利增额交清给付','红利给付'),(select codename from ldcode where codetype='feefinatype' and code=FeeFinaType and rownum=1),State from LJABonusGet where ActuGetNo='" + tActuGetNo + "'";			 	 

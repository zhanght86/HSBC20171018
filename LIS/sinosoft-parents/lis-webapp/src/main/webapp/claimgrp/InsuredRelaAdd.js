...
function QueryGrpCont()
{
	...
	mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql1");
...
{   
   ...
   // var brrResult = easyExecSql("select to_date(max(enteraccdate)) - (-1) from ljtempfee where othernotype='4' and otherno='"+prtNo+"'");
    mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql2");
...
function QueryChargeDate()
{
   ...
   // var arrResult = easyExecSql("select to_date(max(enteraccdate)) from ljtempfee where othernotype='4' and otherno='"+prtNo+"'");
     mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql3");
	mySql.addSubPara(prtNo); 
	var arrResult = easyExecSql(mySql.getString());
    var ChargeDate =""; 
    if(arrResult[0][0]=="")
...
function QueryGrpAppnt()
{
   ...
   /* var strSQL = "select a.GrpName,a.Asset,a.GrpNature,a.BusinessType,a.Peoples,a.Fax,b.AddressNo,b.CustomerNo,a.GetFlag,a.BankCode,a.BankAccNo,a.Remark "
     ...
     mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql4");
...
function QueryDetailGrpaddress(){
    ...
    mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql5");
...
function QueryAgent()
{
  ...
  /*  var strSQL = "";
    ...
    mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql6");
   ...
   mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql7");
	mySql.addSubPara(fm.GrpContNo.value);  
   turnPage.queryModal(mySql.getString(), MultiAgentGrid);
...
function QueryRisk(){
    ...
    /*var  strSql=" select a.riskcode, b.riskname,a.payintv,a.exppeoples,"
	 ...
	 mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql8");
...
function grpFillList()
{
	delGrpVar();
	addGrpVar();
	top.fraInterface.window.location = "../claimgrp/InsuredRelaInput.jsp?LoadFlag=18&ContType=2&PrtNo=" + fm.PrtNo.value + "&scantype=" + fm.ScanType.value + "&ContNo=" + fm.ContNo.value + "&GrpContNo=" + fm.GrpContNo.value + "&checktype=2&display=1";
}
...
function grpRiskPlanInfo()
{
  ...
  showInfo = window.open("../app/ContPlan.jsp?GrpContNo="+fm.GrpContNo.value+"&LoadFlag=18","","toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, status=yes");
...
function getaddresscodedata()
{
   ...
   // strsql = "select AddressNo,GrpAddress from LCGrpAddress where CustomerNo ='"+fm.GrpNo.value+"'";
    ...
    mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql9");
	mySql.addSubPara(fm.GrpNo.value);  
...
function GrpQuestQuery()
{
	...
	showInfo=window.open("./GrpQuestQueryMain.jsp?GrpContNo="+cGrpContNo+"&Flag=18","",,"toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, status=yes");
}     
...
function afterCodeSelect( cCodeName, Field )
{
    if(cCodeName=="GetGrpAddressNo"){
        // var strSQL="select b.AddressNo,b.GrpAddress,b.GrpZipCode,b.LinkMan1,b.Department1,b.HeadShip1,b.Phone1,b.E_Mail1,b.Fax1,b.LinkMan2,b.Department2,b.HeadShip2,b.Phone2,b.E_Mail2,b.Fax2 from LCGrpAddress b where b.AddressNo='"+fm.GrpAddressNo.value+"' and b.CustomerNo='"+fm.GrpNo.value+"'";
         mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql10");
	mySql.addSubPara(fm.GrpAddressNo.value);  
	mySql.addSubPara(fm.GrpNo.value);  
...
function queryAgent()
{
    ...
    if(document.all('AgentCode').value != "")
    {
        ...
        mySql = new SqlClass();
	mySql.setResourceName("claimgrp.InsuredRelaAddSql");
	mySql.setSqlId("InsuredRelaAddSql11");

...
else
{
    ...
    //数据区--准备往当前业务表中写入的赔案号                    
    tLLCaseSchema.setRgtNo(request.getParameter("ClmNo")); //申请登记号
    tLLCaseSchema.setCaseNo(request.getParameter("ClmNo")); //立案号(赔案号)
    ...
    tLLCaseSchema.setAccDate(request.getParameter("newAccidentDate2")); //出险时间
    tLLCaseSchema.setMedAccDate(request.getParameter("newAccidentDate2")); //出险时间
    ...
    if(null!=newtClaimType){
        for (int i = 0; i < newtClaimType.length; i++)
        {
		    ...
		    tLLAppClaimReasonSchema.setCaseNo(request.getParameter("ClmNo")); //赔案号
            tLLAppClaimReasonSchema.setRgtNo(request.getParameter("ClmNo")); //申请登记号
            tLLAppClaimReasonSchema.setCustomerNo(request.getParameter("customerNo")); //出险人编码/出险人客户号
    ...
    mTransferData.setNameAndValue("tPayGetMode", "1"); //赔付金领取方式: 1 一次性支付

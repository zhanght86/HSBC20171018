...
function initParam()
{
    ...
    mManageCom = nullToEmpty("<%= tG.ManageCom %>"); //取到登陆机构的全部代码 用于查询保单处理机构
    document.all('ManageCom').value = mManageCom.substring(0,2); //取到登陆机构的前两个代码 用于查询医院机构
    document.all('ALLManageCom').value = mManageCom;
}
...
function initInpBox()
{
  ...
  {
    alert("在LLClaimAuditInit.jsp-->InitInpBox函数中发生异常:初始化界面错误!");
  }
}
...
//      iArray[7][3]=0;
      ...
      LPEdorItemGrid = new MulLineEnter("document","LPEdorItemGrid");
      LPEdorItemGrid.mulLineCount = 5;       // 显示行数

...
<%
	...
	loggerDebug("BQSendAllNoticeInit","ContNo:"+tContNo);
	loggerDebug("BQSendAllNoticeInit","Flag:"+tFlag);
	loggerDebug("BQSendAllNoticeInit","tNoticeType:"+tNoticeType);
...
function initForm(tContNo,tMissionID,tSubMissionID,tEdorNo,tEdorType)
{
  try
  {
	...
	initAddFeeGrid();	
	querySpec(tContNo);
	queryAddFee(tContNo,tEdorNo,tEdorType);
	queryAllNeedQuestion(tEdorNo,tContNo);
	...
	initAddFeeDGrid();
	//initUWPlanGrid();
	queryQuestD(tEdorNo,tContNo);
	queryUWSpecD(tContNo);
	queryUWHealthD(tEdorNo,tContNo);
	queryUWExistD(tEdorNo,tContNo);
	queryUWHealth(tEdorNo,tContNo);
	queryUWExist(tEdorNo,tContNo);
	queryAddFeeD(tContNo,tEdorNo,tEdorType);
...
function initAddFeeDGrid()
  {                               
    var iArray = new Array();
      ...
      UWAddFeeDGrid = new MulLineEnter( "fm" , "UWAddFeeDGrid" ); 
      //这些属性必须在loadMulLine前  
      UWAddFeeDGrid.displayTitle = 1;
      //PolAddGrid.locked = 1;
      UWAddFeeDGrid.canSel = 0;
      UWAddFeeDGrid.canChk = 0;
      UWAddFeeDGrid.hiddenPlus = 1;
      UWAddFeeDGrid.hiddenSubtraction = 1;
      UWAddFeeDGrid.loadMulLine(iArray);       

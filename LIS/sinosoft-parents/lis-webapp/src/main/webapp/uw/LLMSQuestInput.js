...
var tResourceName="uw.LLMSQuestInputSql";
...
function QuestQuery(tContNo, tFlag,tBatNo,tCaseNo)
{
	//var tSql = "select count(*) from llquestionnaire where clmno='"+tCaseNo+"' and batno='"+tBatNo+"' and contno='"+tContNo+"'";
	var tSql = wrapSql(tResourceName,"querysqldes1",[tCaseNo,tBatNo,tContNo]);
	var questionFlag=easyExecSql(tSql);
	if(questionFlag==0){
		fm.ImpartQuest.value="N";
	}else{
		fm.ImpartQuest.value="Y";
	}
//	prompt("",tSql);
	// 初始化表格
	var i,j,m,n;
	...
	//			 + " and code <> '99'";
		...
		/*strSQL = "select code,cont from ldcodemod where "+k+"="+k				 	
				 + " and codetype = 'Question'";*/
		strSQL = wrapSql(tResourceName,"querysqldes2",["1"]);
	//}
...
//  		alert("M:"+m);
//  		alert(turnPage.arrDataCacheSet[i][j]);
  		if (m > 0)
  		{
  			for( j = 0; j< m; j++)
  			{
...
{	
	...
	//{
		/*strSQL = "select issuecont from lcissuepol where "+k+"="+k				 	
				 + " and ContNo = '"+tContNo+"'";*/
		strSQL = wrapSql(tResourceName,"querysqldes3",[tContNo]);
	//}
...
function afterCodeSelect( cCodeName, Field )
{
	...
	var tSQL = wrapSql(tResourceName,"querysqldes4",[tObjFlag]);
...
//	alert(tContNo+","+document.all('QuestionObj').value);
	...
	strsql = wrapSql(tResourceName,"querysqldes6",[tContNo]);
	//prompt("",strsql);
  //查询SQL，返回结果字符串
  turnPage.strQueryResult = easyQueryVer3(strsql, 1, 1, 1);  
...
//初始化查询所有需要回复的问题件
function questAllNeedQuestion(tContNo,tFlag,tBatNo,tCaseNo)
{
	initQuestGrid();
	/*strsql = " select proposalcontno,issuetype,issuecont,replyresult,(select username from lduser where usercode=a.operator),makedate,(select codename from ldcode where codetype='operatepos' and code=a.operatepos), "
	       + " backobjtype,needprint,serialno,(select codename from ldcode where codetype='backobj' and comcode=a.BackObjType and othersign=a.standbyflag2),nvl(state,'x'),QuestionObjType from llissuepol a where contno='"+tContNo+"' "
	       + " and state is null "
	       //+ " and prtseq is null "
	       ;*/
	strsql = wrapSql(tResourceName,"querysqldes7",[tContNo]);
...
function queryone(parm1,parm2)
{
	...
	var tSql = wrapSql(tResourceName,"querysqldes8",[QuestGrid.getRowColData(QuestGrid.getSelNo() - 1, 11)]);
	var arrResult =  easyExecSql(tSql);
	...
	var tSql2 = wrapSql(tResourceName,"querysqldes9",[QuestGrid.getRowColData(QuestGrid.getSelNo() - 1, 13)]);
	var arrResult2 =  easyExecSql(tSql2);//prompt("",tSql2);
...
function queryquestionnaire()
{
    ...
    var tSQL_Sub = wrapSql(tResourceName,"querysqldes10",[fm.hiddenProposalContNo.value]);

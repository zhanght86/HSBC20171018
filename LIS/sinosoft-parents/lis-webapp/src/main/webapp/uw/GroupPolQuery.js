...
function showApp()
{
  ...
  if (cProposalNo != "")
  {
    var tSelNo = PolAddGrid.getSelNo()-1;
  	...
  	window.open("../uw/UWAppMain.jsp?ProposalNo1="+cProposalNo+"&InsureNo1="+cInsureNo);
  	showInfo.close();
  }
  else
  {
...
function showReport()
{
  ...
  if (cContNo != "")
  {
  	window.open("../uw/UWManuReportMain.jsp?ContNo="+cContNo,"window1");
  	showInfo.close();
  }
  else
  {
...
function SendFirstNotice()
{
  ...
  if (cProposalNo != "")
  {
	...
	var iTop = (window.screen.availHeight - iHeight) / 2; //获得窗口的垂直位置 
	var iLeft = (window.screen.availWidth - iWidth) / 2;  //获得窗口的水平位置 
	showInfo1 = window.open ("./UWSendPrintMain.jsp?ProposalNo1="+cProposalNo+"&OtherNoType="+cOtherNoType+"&Code="+cCode,name, "status=no,toolbar=no,menubar=no,location=no,resizable=no,scrollbars=0,titlebar=no,height="+ iHeight+",width="+iWidth+",innerHeight="+iHeight+",innerWidth=" +iWidth+",left="+iLeft+",top="+iTop,false);
...
function easyQueryAddClick(parm1,parm2)
{
	...
	if(document.all(parm1).all('InpPolGridSel').value == '1' ){
		...
		tPEdorNo = document.all(parm1).all('PolGrid1').value;
	}

	document.all('ContNo').value = document.all(parm1).all('PolGrid2').value;
	document.all('MissionID').value = document.all(parm1).all('PolGrid7').value;
	document.all('PrtNo').value = document.all(parm1).all('PolGrid1').value;
	document.all('SubMissionID').value = document.all(parm1).all('PolGrid8').value;
...
function showNotePad() {
  ...
  if (cContNo!="" && PrtNo!="") {
  	window.open("../uw/UWNotePadMain.jsp?ContNo="+cContNo+"&PrtNo="+PrtNo+"&OperatePos=3", "window1");
  }
  else {
  	alert("请先选择保单!");
  }

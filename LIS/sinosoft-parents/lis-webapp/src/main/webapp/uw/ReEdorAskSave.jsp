...
//
//  //把dataStream存储到磁盘文件
//  //loggerDebug("ReEdorAskSave","存储文件到"+strVFPathName);
//  AccessVtsFile.saveToFile(dataStream,strVFPathName);
//  loggerDebug("ReEdorAskSave","==> Write VTS file to disk ");
...
boolean getFile(VData tVData,String operate,String templateFile ) throws Exception
{
        ...
        if(!tEdorAskPrintUI.submitData(tVData,operate))
        {
  ...
  loggerDebug("ReEdorAskSave","Operate:"+operate+" before Combine:"+templateFile);
//  txmlExport.outputDocumentToFile("c:\\",templateFile+".xml");
  ...
  ByteArrayOutputStream outStream = new ByteArrayOutputStream();
	...
	loggerDebug("ReEdorAskSave","before output...");
  tcombineVts.output(outStream);
	loggerDebug("ReEdorAskSave","after output...");
	tcombineVts.destroy();
//	System.gc();

  byte[] bytes = outStream.toByteArray();
  loggerDebug("ReEdorAskSave","byte.length="+bytes.length);
...
// AccessVtsFile.saveToFile(outStream,"c:\\"+templateFile);
 outStream.close();
  ...
  try
  {  
  		loggerDebug("ReEdorAskSave","before new newFile");
  		newFile = new BookModelImpl();
//      newFile.read(new FileInputStream("c:\\"+templateFile));
  		loggerDebug("ReEdorAskSave","before read");
      newFile.read(inputStream);
  		loggerDebug("ReEdorAskSave","after read");			
...
//      targetFile = newFile;  //同一个对象指针错误，切切
	  loggerDebug("ReEdorAskSave","before new target...");
	  targetFile = new BookModelImpl();
	  loggerDebug("ReEdorAskSave","after new target...");
	  targetFile.copyAll(newFile);
      loggerDebug("ReEdorAskSave","init new file");
  }
  else
  {
	loggerDebug("ReEdorAskSave","before combine");
	targetFile = vfc.dataCombine(newFile,targetFile);
//	 loggerDebug("ReEdorAskSave","after combine:"+targetFile.getSelection());
  }
...
{
	loggerDebug("ReEdorAskSave",Content);
...
loggerDebug("ReEdorAskSave","--------------------start------------------");
tPath = application.getRealPath("f1print/NCLtemplate") + "/";
loggerDebug("ReEdorAskSave","标准路径==="+tPath);
...
loggerDebug("ReEdorAskSave","合同号==="+tContNo);
...
loggerDebug("ReEdorAskSave",PrtSeq);
loggerDebug("ReEdorAskSave","tCode:"+tCode);
...
loggerDebug("ReEdorAskSave","问卷的行数==="+testSSRS.MaxRow);
...
loggerDebug("ReEdorAskSave","put session value,ContNo:" + tContNo);
loggerDebug("ReEdorAskSave","put session value==" + tMissionID);
loggerDebug("ReEdorAskSave","put session value,SubMissionID:" + tSubMissionID);
loggerDebug("ReEdorAskSave","put session value,PrtSeq=="+PrtSeq);
session.putValue("RealPath",strVFPathName );
loggerDebug("ReEdorAskSave","before sendredirect..............");

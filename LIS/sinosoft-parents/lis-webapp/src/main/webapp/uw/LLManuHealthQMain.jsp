...
<!--标题与状态区域-->
	...
	<frameset name="fraSet" cols="*" frameborder="yes" border="1" framespacing="0" rows="0,50%,*,0">
		...
		<!--交互区域-->
		<frame id="fraPic" name="fraPic" scrolling="auto" noresize src="../common/EasyScanQuery/EasyScanQuery.jsp?prtNo=<%=request.getParameter("ContNo")%>&BussNoType=11&BussType=TB&SubType=CA102&QueryType=0&DocID=<%=request.getParameter("DocID") %>">
		<frame id="fraInterface" name="fraInterface" scrolling="auto" src="./LLManuHealthQ.jsp?ContNo=<%=request.getParameter("ContNo")%>&MissionID=<%=request.getParameter("MissionID")%>&SubMissionID=<%=request.getParameter("SubMissionID")%>&PrtNo=<%=request.getParameter("PrtNo")%>&PrtSeq=<%=request.getParameter("PrtSeq")%>&Flag=<%=request.getParameter("Flag")%>&CustomerNo=<%=request.getParameter("CustomerNo")%>&ActivityID=<%=request.getParameter("ActivityID")%>&ClmNo=<%=request.getParameter("ClmNo")%>&BatNo=<%=request.getParameter("BatNo")%>">
    <!--下一步页面区域-->

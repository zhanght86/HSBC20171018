...
<%
	...
	//loggerDebug("EdorUWManuRReportInit1","1:"+strOperator);
%>
...
function initForm(tContNo,tPrtNo,tMissionID,tSubMissionID,tEdorNo,tEdorType){
  var str = "";
  try
  {	
  	//alert("tSubMissionID"+tSubMissionID);
	initInpBox(tPrtNo); //alert(87);
  	initCustomerNo(tContNo); //alert(88);
	initHide(tContNo,tMissionID,tPrtNo,tSubMissionID,tEdorNo,tEdorType); //alert(89);
	initInvestigateGrid();
...
function initHide(tContNo,tMissionID,tPrtNo,tSubMissionID,tEdorNo,tEdorType){

	document.all('ProposalNoHide').value=tContNo;
	document.all('MissionID').value=tMissionID;
	document.all('SubMissionID').value=tSubMissionID;	//alert(tSubMissionID);
	document.all('ContNo').value=tContNo;
	document.all('Operator').value = '<%= strOperator %>';
	document.all('Flag').value=<%=tFlag%>;
	document.all('EdorNo').value=tEdorNo; //alert("tEdorNo=="+tEdorNo);
	document.all('EdorType').value=tEdorType; //alert("tEdorType=="+tEdorType);
...
function initInvestigateGrid()
{
    var iArray = new Array();
      ...
      //这些属性必须在loadMulLine前                            
      InvestigateGrid.mulLineCount = 5;

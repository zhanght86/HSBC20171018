...
%>  
		...
		<!--交互区域-->
		<frame id="fraInterface" name="fraInterface" noresize scrolling="auto" src="./LLUWChangeRiskPlanInput.jsp?ContNo=<%=request.getParameter("ContNo")%>&MissionID=<%=request.getParameter("MissionID")%>&SubMissionID=<%=request.getParameter("SubMissionID")%>&PrtNo=<%=request.getParameter("PrtNo")%>&QueryFlag=<%=request.getParameter("QueryFlag")%>&InsuredNo=<%=request.getParameter("InsuredNo")%>&InsuredSumLifeAmnt=<%=request.getParameter("InsuredSumLifeAmnt")%>&InsuredSumHealthAmnt=<%=request.getParameter("InsuredSumHealthAmnt")%>&BatNo=<%=request.getParameter("BatNo") %>&ClmNo=<%=request.getParameter("ClmNo") %>">
  	<!--险种界面区域-->

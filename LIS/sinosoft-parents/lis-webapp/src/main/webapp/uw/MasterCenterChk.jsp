...
<%
  ...
  if(tG == null) 
  {
		loggerDebug("MasterCenterChk","session has expired");
		return;
  }
  ...
  loggerDebug("MasterCenterChk","== sMissionID == " + sMissionID);    	   
  loggerDebug("MasterCenterChk","== sSubMissionID == " + sSubMissionID); 
  loggerDebug("MasterCenterChk","== sActivityID == " + sActivityID); 
  ...
  try{
		...
		if (tBusinessDelegate.submitData(tVData,"",busiName) == false)
		{
			...
			for (int i = 0; i < n; i++)
			{
			  loggerDebug("MasterCenterChk","Error: "+tBusinessDelegate.getCErrors().getError(i).errorMessage);
		...
		if (FlagStr == "Fail")
		{
		    ...
		    loggerDebug("MasterCenterChk","tError.getErrorCount:"+tError.getErrorCount());

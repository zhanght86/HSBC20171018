...
//  arrResult = easyExecSql("select * from lwmission where missionprop1 = '" + tContno +"' and activityId in (select activityId from Lwactivity where functionId in( '10020302','10020303')) and missionprop3 in (select trim(prtseq) from LPPENotice where contno='" + tContno + "' and CustomerNo='" + tCustomerNo + "')", 1, 0);
	...
	mySql1.setResourceName("uw.BQManuHealthSql"); //指定使用的properties文件名         
	mySql1.setSqlId(sqlid1);//指定使用的Sql的id                                
	mySql1.addSubPara(tContno);//传入参数     
	mySql1.addSubPara(tContno);//传入参数     
	mySql1.addSubPara(tCustomerNo);//传入参数     
...
//	    arrResult = easyExecSql("select * from lwmission where missionprop2 = '" + tContno +"' and activityId in (select activityId from Lwactivity where functionId in( '10020302','10020303')) and missionprop3 in (select trim(prtseq) from LPPENotice where contno='" + tContno + "' and CustomerNo='" + tCustomerNo + "')", 1, 0);
		...
		mySql2.setResourceName("uw.BQManuHealthSql"); //指定使用的properties文件名         
		mySql2.setSqlId(sqlid2);//指定使用的Sql的id                                
		mySql2.addSubPara(tContno);//传入参数     
		mySql2.addSubPara(tContno);//传入参数     
		mySql2.addSubPara(tCustomerNo);//传入参数     
...
//	
		...
		mySql3.setResourceName("uw.BQManuHealthSql"); //指定使用的properties文件名         
		mySql3.setSqlId(sqlid3);//指定使用的Sql的id                                
		mySql3.addSubPara(tProposalContno);//传入参数     
		mySql3.addSubPara(tInsuredNo);//传入参数       
...
//	//execEasyQuery( strSQL );
	...
	mySql4.setResourceName("uw.BQManuHealthSql"); //指定使用的properties文件名         
	mySql4.setSqlId(sqlid4);//指定使用的Sql的id                                
	mySql4.addSubPara(tProposalContno);//传入参数     
	mySql4.addSubPara(tInsuredNo);//传入参数       
...
//             + " and (c.uwno = (select max(d.uwno) from LPCUWError d where d.ContNo = '" + tContNo + "'))"
            ...
            var sqlid5="BQManuHealthSql5";                              
         	...
         	mySql5.setResourceName('uw.BQManuHealthSql'); //指定使用的properties文件名         
         	mySql5.setSqlId(sqlid5);//指定使用的Sql的id                                
         	mySql5.addSubPara(tContNo);//传入参数     
         	mySql5.addSubPara(tContNo);//传入参数       
         	mySql5.addSubPara(tContNo);//传入参数     
         	mySql5.addSubPara(tContNo);//传入参数       
...
//           + " from"
//           + " (select InsuredNo as a,('被保人') as b,name as c,'I' d,sequenceno e from lpinsured where 1=1 "
...
//           + " union select appntno as a,'投保人' as b ,appntname as c,'A' d,'-1' e from lpcont where edorno='"+tEdorNo+"' and contno = '" +tContNo + "'"
//           + " union select CustomerNo as a,'第二被保人' as b , Name as c,'I' d,'0' e from LPInsuredRelated where polno in (select polno from lppol where edorno='"+tEdorNo+"' and contno = '" +tContNo+"')"
//		   + " union select u.AppntNo as a, '变更投保人' as b, u.AppntName as c,'A' d,'0' e from LPAppnt u where 1 = 1 and u.EdorNo = '" + tEdorNo + "' and u.ContNo='"+tContNo+"' and not exists (select 'X' from LPAppnt where edorno=u.edorno and ContNo = u.ContNo and AppntNo = u.AppntNo)"
//           +  " union select v.InsuredNo as a, '变更被保人' as b, v.Name as c,'I' d,'0' e from LPInsured v where 1 = 1 and v.EdorNo = '" + tEdorNo + "' and v.ContNo='"+tContNo+"' and not exists (select 'X' from LPInsured where edorno=v.edorno and ContNo = v.ContNo and InsuredNo = v.InsuredNo)"
//		   + ") temp"
//           +  " order by e";
//           //prompt("",strSql);
    ...
    var sqlid6="BQManuHealthSql6";                              
 	...
 	mySql6.setResourceName('uw.BQManuHealthSql'); //指定使用的properties文件名         
 	mySql6.setSqlId(sqlid6);//指定使用的Sql的id                                
 	mySql6.addSubPara(tEdorNo);//传入参数     
 	mySql6.addSubPara(tContNo);//传入参数       
 	mySql6.addSubPara(tEdorNo);//传入参数     
 	mySql6.addSubPara(tEdorNo);//传入参数    
 	mySql6.addSubPara(tContNo);//传入参数    
 	mySql6.addSubPara(tEdorNo);//传入参数     
...
//alert(340);
  ...
  /*if (turnPage.strQueryResult == "")
  {
    ...
    var strSql1 = "select b,a,c,d,e from (select InsuredNo as a,('第'||sequenceno||'被保人') as b,name as c,'I' d, e from lpinsured where 1=1 "
...
//           + "and mngcom like concat(concat('' , substr((select managecom from lpcont where EdorNo='"+tEdorNo+"' and ContNo = '"+tContNo+"'),1,6)) , '%%') ";
     ...
     mySql7.setResourceName('uw.BQManuHealthSql'); //指定使用的properties文件名         
     mySql7.setSqlId(sqlid7);//指定使用的Sql的id                                
     mySql7.addSubPara(tEdorNo);//传入参数     
     mySql7.addSubPara(tContNo);//传入参数       
...
{
//	var tSQL = "select name,(case customernotype when 'A' then '投保人' else '被保人' end),(select codename from ldcode where codetype='printstate' and code=(case when printflag is not null then printflag else 'x' end)),"
...
//	         + " order by customerno,makedate,maketime ";
    ...
    mySql8.setResourceName('uw.BQManuHealthSql'); //指定使用的properties文件名         
    mySql8.setSqlId(sqlid8);//指定使用的Sql的id                                
    mySql8.addSubPara(document.all('EdorNo').value);//传入参数     
    mySql8.addSubPara(document.all('ContNo').value);//传入参数       
...
//	       + " and contno = '"+ document.all('ContNo').value +"' and prtseq = '"+turnPage.arrDataCacheSet[i][4]+"' order by peitemcode" ;
    ...
    mySql9.setResourceName('uw.BQManuHealthSql'); //指定使用的properties文件名         
    mySql9.setSqlId(sqlid9);//指定使用的Sql的id                                
    mySql9.addSubPara(document.all('ContNo').value);//传入参数     
    mySql9.addSubPara(turnPage.arrDataCacheSet[i][4]);//传入参数       
...
//		            ;
	    ...
	    mySql10.setResourceName('uw.BQManuHealthSql'); //指定使用的properties文件名         
	    mySql10.setSqlId(sqlid10);//指定使用的Sql的id                                
	    mySql10.addSubPara(document.all('ContNo').value);//传入参数     
	    mySql10.addSubPara(tPrtSeq);//传入参数    
	    mySql10.addSubPara(tCustomerNo);//传入参数   
	    mySql10.addSubPara(tCustomerType);//传入参数   
...
//                 + " and prtseq = '"+tPrtSeq+"' ";
    ...
    mySql11.setResourceName('uw.BQManuHealthSql'); //指定使用的properties文件名         
    mySql11.setSqlId(sqlid11);//指定使用的Sql的id                                
    mySql11.addSubPara(document.all('ContNo').value);//传入参数     
    mySql11.addSubPara(tPrtSeq);//传入参数    

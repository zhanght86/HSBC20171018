...
//1-得到所有纪录，显示纪录值
  ...
  loggerDebug("BQQuestQueryChk","radiolength:"+temp);
  ...
  if(tTempClassNum!=null) //如果不是空纪录	
  {
  	TempCount = tTempClassNum.length; //记录数      
   	loggerDebug("BQQuestQueryChk","Start query Count="+TempCount);   
  	while(index<TempCount)
  	{
  		loggerDebug("BQQuestQueryChk","----3-----");
  		loggerDebug("BQQuestQueryChk","polcode:"+tPolCode[index]);
  		loggerDebug("BQQuestQueryChk","radio:"+tRadio[index]);
  		if (!tPolCode[index].equals("")&&tRadio[index].equals("1"))
  		{
  			loggerDebug("BQQuestQueryChk","GridNO="+tTempClassNum[index]);
  			loggerDebug("BQQuestQueryChk","Grid 1="+tPolCode[index]);
  			loggerDebug("BQQuestQueryChk","Radio:"+tRadio[index]);
    			loggerDebug("BQQuestQueryChk","this radio is selected");
...
<%    			
    		...
    		index=index+1;
    		loggerDebug("BQQuestQueryChk","index:"+index);          
	}
}

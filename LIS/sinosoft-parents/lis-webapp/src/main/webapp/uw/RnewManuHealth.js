...
function submitForm()
{
  ...
  arrResult = easyExecSql("select * from lwmission where missionprop1 = '" + tContno +"' and activityid in ('0000007007','0000007009') and missionprop3 in (select trim(prtseq) from RnewPENotice where contno='" + tContno + "' and CustomerNo='" + tCustomerNo + "')", 1, 0);

  if(arrResult == null)
  {
    arrResult = easyExecSql("select * from lwmission where missionprop2 = '" + tContno +"' and activityid in ('0000007007','0000007009') and missionprop3 in (select trim(prtseq) from RnewPENotice where contno='" + tContno + "' and CustomerNo='" + tCustomerNo + "')", 1, 0);
  }
...
//
  ...
  fm.action= "./RnewManuHealthChk.jsp";
...
function easyQueryClickSingle()
{
  ...
  {
//	strsql = "select proposalno,insuredname,pedate,peaddress,PEBeforeCond,remark,printflag from RnewPENotice where 1=1"
//				 + " and proposalno = '"+ tProposalContno + "'"
//				 + " and insuredno = '"+ tInsuredNo + "'";
	
	    var  sqlid1="RnewManuHealthSql0";
	 	var  mySql1=new SqlClass();
	 	mySql1.setResourceName("uw.RnewManuHealthSql"); //指定使用的properties文件名
...
function easyQueryClick()
{
  ...
  {
//	strsql = "select peitemcode,peitemname from RnewPENotice where 1=1"
//				 + " and proposalcontno = '"+ tProposalContno + "'"
//				 + " and customerno = '"+ tInsuredNo + "'";
	    var  sqlid2="RnewManuHealthSql1";
	 	var  mySql2=new SqlClass();
	 	mySql2.setResourceName("uw.RnewManuHealthSql"); //指定使用的properties文件名
	 	mySql2.setSqlId(sqlid2);//指定使用的Sql的id
	 	mySql2.addSubPara(tProposalContno);//指定传入的参数
...
//             + " and (c.uwno = (select max(d.uwno) from LCCUWError d where d.ContNo = '" + tContNo + "'))"
             ...
             var  sqlid3="RnewManuHealthSql2";
     	 	 var  mySql3=new SqlClass();
     	 	 mySql3.setResourceName("uw.RnewManuHealthSql"); //指定使用的properties文件名
...
function initInsureNo(tContNo)
{
    ...
    var strSql = "select b,a,c,d,e from (select InsuredNo as a,concat(concat('第',sequenceno),'被保人') as b,name as c,'I' d,sequenceno e from lcinsured where 1=1 "
           + " and contno = '" +tContNo + "' and not exists(select 'x' from lcinsuredrelated where customerno = insuredno and polno in (select polno from lcpol where contno = '" +tContNo + "'))"
           + " union select appntno as a,'投保人' as b ,appntname as c,'A' d,'-1' e from lccont where contno = '" +tContNo + "'"
           + " union select CustomerNo as a,'第二被保人' as b , Name as c,'I' d,'0' e from LCInsuredRelated where polno in (select polno from lcpol where contno = '" +tContNo + "')";
...
{
//	var tSQL = "select name,(case customertype when 'A' then '投保人' else '被保人' end),(select codename from ldcode where codetype='printstate' and code=(case when printflag is not null then printflag else 'x' end)),"
//	         + " operator,prtseq,(case when printflag is not null then printflag else 'x' end),customerno,customertype,'',makedate from RnewPENotice "
//	         + " where contno='"+document.all('ContNo').value+"' "
//	         + " order by customerno,makedate,maketime ";
	     var  sqlid4="RnewManuHealthSql3";
	 	 var  mySql4=new SqlClass();
	 	 mySql4.setResourceName("uw.RnewManuHealthSql"); //指定使用的properties文件名
...
//	       + " and contno = '"+ document.all('ContNo').value +"' and prtseq = '"+turnPage.arrDataCacheSet[i][4]+"' order by peitemcode" ;
     var  sqlid5="RnewManuHealthSql4";
	 var  mySql5=new SqlClass();
	 mySql5.setResourceName("uw.RnewManuHealthSql"); //指定使用的properties文件名
	 mySql5.setSqlId(sqlid5);//指定使用的Sql的id
	 mySql5.addSubPara(document.all('ContNo').value);//指定传入的参数
	 mySql5.addSubPara(turnPage.arrDataCacheSet[i][4]);//指定传入的参数
...
//		var tSQL = "select customerno,customertype,pereason,pebeforecond,PEDate,(case when PrintFlag is not null then PrintFlag else 'x' end),remark from RnewPENotice "
//		            + " where contno='"+document.all('ContNo').value+"' "
//		            + " and prtseq='"+tPrtSeq+"' and customerno='"+tCustomerNo+"' "
//		            + " and customertype='"+tCustomerType+"' "
//		            ;
		 var  sqlid6="RnewManuHealthSql5";
		 var  mySql6=new SqlClass();
		 mySql6.setResourceName("uw.RnewManuHealthSql"); //指定使用的properties文件名
		 mySql6.setSqlId(sqlid6);//指定使用的Sql的id
		 mySql6.addSubPara(document.all('ContNo').value);//指定传入的参数
		 mySql6.addSubPara(tPrtSeq);//指定传入的参数
		 mySql6.addSubPara(tCustomerNo);//指定传入的参数
		 mySql6.addSubPara(tCustomerType);//指定传入的参数
    ...
    //查询体检子项目
//    var tSQL_Sub = "select peitemcode,peitemname from RnewPENoticeitem "
//                 + " where contno='"+document.all('ContNo').value+"' "
//                 + " and prtseq = '"+tPrtSeq+"' ";
     var  sqlid7="RnewManuHealthSql6";
	 var  mySql7=new SqlClass();
	 mySql7.setResourceName("uw.RnewManuHealthSql"); //指定使用的properties文件名
	 mySql7.setSqlId(sqlid7);//指定使用的Sql的id
	 mySql7.addSubPara(document.all('ContNo').value);//指定传入的参数
	 mySql7.addSubPara(tPrtSeq);//指定传入的参数

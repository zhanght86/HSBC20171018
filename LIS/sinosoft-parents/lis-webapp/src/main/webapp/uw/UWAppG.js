...
function easyQueryClick()
{
	...
	var strSQL = "";
	var i, j, m, n;
	var proposalno = document.all('ProposalNoHide').value;
...
//				 + " order by 1";		
		...
		mySql1.setResourceName("uw.UWAppGSql"); // 指定使用的properties文件名
		mySql1.setSqlId(sqlid1);// 指定使用的Sql的id
		mySql1.addSubPara(insureno);// 指定传入的参数
		mySql1.addSubPara(proposalno);// 指定传入的参数
		mySql1.addSubPara(insureno);// 指定传入的参数
		mySql1.addSubPara(proposalno);// 指定传入的参数
...
//				 + " order by 1";		
		...
		mySql2.setResourceName("uw.UWAppGSql"); // 指定使用的properties文件名

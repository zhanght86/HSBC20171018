...
{	
	...
	var tPrtSeq = QuestGrid.getRowColData(tSelNo,8);
		...
		if (tContNo != " ")
	{
		strSQL = "select RReportItemCode,RReportItemName,Remark from lcrreportitem where contno = '"+tContNo+"' and prtseq = '"+tPrtSeq+"'";
		turnPage.strQueryResult  = easyQueryVer3(strSQL, 1, 1, 1);  
  ...
  displayMultiline(arrDataSet, turnPage.pageDisplayGrid);
	...
	//查询SQL，返回结果字符串
  	...
  	var strsql= "select rreportreason,ReplyContente from LCRReport where 1=1"	
				+ " and ContNo = '"+tContNo+"'"
				+ " and PrtSeq = '"+tPrtSeq+"'"; 	
...
function easyQueryClickItem()
{
  ...
  displayMultiline(arrDataSet, turnPage.pageDisplayGrid);
	...
	//查询SQL，返回结果字符串
  	...
  	var strsql= "select ReplyContente, rreportreason from LCRReport where 1=1"	
				+ " and ContNo = '"+tContNo+"'"
				+ " and PrtSeq='"+tPrtSeq+"'"; 	
	...
	var arrReturn = new Array();
       ...
       if(arrReturn!=null)
       {
		...
		document.all('Contente').value = arrReturn[0][1];
}

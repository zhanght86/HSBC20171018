/***************************************************************
 * <p>ProName：LCContPlanDetailInput.js</p>
 * <p>Title：方案信息录入</p>
...
function queryPlanDetail() {
	...
	tSQLInfo.setSqlId("LCContPlanTradSql6");
...
function returnShowCode(value1, value2, value3, returnType) {
	...
	if (value1=='contplan') {
		...
		var tSql = "1";
		tSql = "1 and substr(a.grpcontno,1,18)=substr(#"+ tGrpPropNo + "#,1,18) ";
		if (returnType=='0') {
	...
	if (value1=='quotrisk') {
		...
		if (tContPlanType=='00') {//如果是普通险种,看方案是否为公共保额方案,如果是,那么仅下拉出公共保额,如果不是,那么下拉出不含公共 保额的其他普通险种
			...
			if (tPlanType=="00") {//非公共保额
				...
				//tSql = "1 and a.insuaccflag!=#Y# and b.riskprop!=#I# and not exists (select 1 from ldcodeexp t where t.codetype=#quotriskexp# and t.codeexp=#2# and t.code=a.riskcode)";
				tSql = "1 and b.startdate<=#"+tCurrentDate+"# and (CASE WHEN (b.enddate is null or b.enddate =##) THEN	#9999-12-31# ELSE  to_char(b.enddate,#YYYY-MM-DD#)	END) >#"+tCurrentDate+"# and a.insuaccflag!=#Y# and b.riskprop!=#I# and not exists (select 1 from ldcodeexp t where t.codetype=#quotriskexp# and t.codeexp=#2# and t.code=a.riskcode and t.othersign = #1#)";
				...
				//tSql = "1 and exists (select 1 from ldcodeexp t where t.codetype=#quotriskexp# and t.codeexp=#1# and t.code=a.riskcode)";
				tSql = "1 and b.startdate<=#"+tCurrentDate+"# and (CASE WHEN (b.enddate is null or b.enddate =##) THEN	#9999-12-31# ELSE  to_char(b.enddate,#YYYY-MM-DD#)	END) >#"+tCurrentDate+"# and exists (select 1 from ldcodeexp t where t.codetype=#quotriskexp# and t.codeexp=#1# and t.code=a.riskcode)";
			} else {
				return null;
			}
		} else if (tContPlanType=='01') {//建工险
			
			//tSql = "1 and exists (select 1 from ldcodeexp t where t.codetype=#quotriskexp# and t.codeexp=#2# and t.code=a.riskcode)";
			tSql = "1 and b.startdate<=#"+tCurrentDate+"# and (CASE WHEN (b.enddate is null or b.enddate =##) THEN	#9999-12-31# ELSE  to_char(b.enddate,#YYYY-MM-DD#)	END) >#"+tCurrentDate+"# and exists (select 1 from ldcodeexp t where t.codetype=#quotriskexp# and t.codeexp=#2# and t.code=a.riskcode)";
		} else if (tContPlanType=='02') {//账户型
		
			//tSql = "1 and a.insuaccflag=#Y# and b.riskprop!=#I# ";
			tSql = "1 and b.startdate<=#"+tCurrentDate+"# and (CASE WHEN (b.enddate is null or b.enddate =##) THEN	#9999-12-31# ELSE  to_char(b.enddate,#YYYY-MM-DD#) END) >#"+tCurrentDate+"# and a.insuaccflag=#Y# and b.riskprop!=#I# ";
		} else if (tContPlanType=='03') {//个人产品
			//tSql = "1 and b.riskprop=#I# ";
			tSql = "1 and b.startdate<=#"+tCurrentDate+"# and (CASE WHEN (b.enddate is null or b.enddate =##) THEN	#9999-12-31# ELSE  to_char(b.enddate,#YYYY-MM-DD#) END) >#"+tCurrentDate+"# and b.riskprop=#I# ";
		}
...
function afterCodeSelect(cCodeType, Field) {
	...
	if (cCodeType=="contplan") {//下拉方案后,如果方案信息与所选方案信息不一致,那么重置方案信息
...
function showPlanDetailInfo() {
	...
	if(tContPlanType=='02'){
		...
		divInfo7.style.display='';
		divInfo8.style.display='none';
		
		tSQLInfo = new SqlClass();
		tSQLInfo.setResourceName("g_app.LCContPlanTradSql");
		tSQLInfo.setSqlId("LCContPlanTradSql15");
		...
		initPayFeeGrid();
		turnPage4.queryModal(tSQLInfo.getString(), PayFeeGrid, 1);
		initTZFeeGrid();
	}
}
...
function addPlanDetail() {
	...
	fm2.action = "./LCContPlanDetailSave.jsp?Operate=ADDPLANDETAIL&GrpPropNo="+ tGrpPropNo +"&MissionID="+ tMissionID +"&SubMissionID="+ tSubMissionID +"&ActivityID="+ tActivityID +"&ContPlanType="+ tContPlanType +"&PolicyNo="+ tPolicyNo;
	submitForm(fm2);
}
...
function modifyPlanDetail() {
	...
	fm2.action = "./LCContPlanDetailSave.jsp?Operate=MODIFYPLANDETAIL&GrpPropNo="+ tGrpPropNo +"&MissionID="+ tMissionID +"&SubMissionID="+ tSubMissionID +"&ActivityID="+ tActivityID +"&OSysPlanCode="+ tSysPlanCode +"&OPlanCode="+ tPlanCode +"&ORiskCode="+ tRiskCode +"&ODutyCode="+ tDutyCode +"&ContPlanType="+ tContPlanType +"&PolicyNo="+ tPolicyNo;
	submitForm(fm2);
}
...
function delPlanDetail() {
	...
	fm2.action = "./LCContPlanDetailSave.jsp?Operate=DELPLANDETAIL&GrpPropNo="+ tGrpPropNo +"&MissionID="+ tMissionID +"&SubMissionID="+ tSubMissionID +"&ActivityID="+ tActivityID +"&OSysPlanCode="+ tSysPlanCode +"&OPlanCode="+ tPlanCode +"&ORiskCode="+ tRiskCode +"&ODutyCode="+ tDutyCode +"&ContPlanType="+ tContPlanType +"&PolicyNo="+ tPolicyNo;
	submitForm(fm2);
}
...
function dealRiskDuty(cRiskCode, cDutyCode, cFlag, cObj, cDivID, cSysPlanCode, cPlanCode, cPlanType) {
	...
	*/
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_app.LCContPlanTradSql");
	tSQLInfo.setSqlId("LCContPlanTradSql7");
	tSQLInfo.addSubPara(tGrpPropNo);
	...
	if (tArr==null || tArr.length==0) {
		...
		tSQLInfo = new SqlClass();
		tSQLInfo.setResourceName("g_app.LCContPlanTradSql");
		tSQLInfo.setSqlId("LCContPlanTradSql8");
			...
			for (var i=0; i<tArr.length; i++) {
				...
				if (tFieldType=="1") {
					...
					if (tFactorCode == "P16") {//公共保额 保费计算方式
						document.all("PremCalWay").value = tArr1[0][i];
						auotContShowCodeName1(tValueType, tArr1[0][i], cObj, "PremCalWayName");
					} else {
					...
					if (tFactorCode == "P17") {
						document.all("PerPrem").value = tArr1[0][i];
					} else if (tFactorCode == "P18") {
						document.all("StandPerPrem").value = tArr1[0][i];
					} else {
						document.all(tFactorCode).value = tArr1[0][i];
					}
				}
			}
			//add by dianj 20150320 处理公共保额 保费计算方式，人均保费展示
	...
	if (document.getElementById("trRelation").style.display=="" && document.getElementById("RelaShareFlag").value=="1") {//只有当附属方案配置展示且被勾选了,才查询附属信息
		...
		tSQLInfo = new SqlClass();
		tSQLInfo.setResourceName("g_app.LCContPlanTradSql");
		tSQLInfo.setSqlId("LCContPlanTradSql19");
		...
		if (tArr2==null) {
			...
			for (var i=0; i<tArr.length; i++) {
				...
				if (tFieldType=="1") {
					...
					document.getElementById(tFactorCode).value = tArr2[0][j];
...
function auotContShowCodeName1(cCodeType, cCode, cObj, cCodeName) {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_app.LCContPlanTradSql");
	tSQLInfo.setSqlId("LCContPlanTradSql14");
	...
	if (tArr==null) {
		...
		document.getElementById(cCodeName).value = tArr[0];
	}
}
/**
 * 提交时处理冗余数据
...
function beforeSubmit(cObj, cContPlanType) {
 ...
 tPolicyNo = fm2.PolicyNo.value
	...
	if (cContPlanType=="02") {//对于账户型产品,无保额类型的相关数据,校验期望初始保费及期望收益率
		...
		//modify by dianj 20150320  添加保费计算方式、人均保费，修改校验
				...
				if (fmPub.Operate.value!="TRYCAL") {
					...
					if (tExceptPremType=="01") {//保费,12,2
						...
						if (!checkDecimalFormat(tExceptPrem, 4, 8)) {
							...
							alert("当期望保费类型为期望费率时，期望费率整数位不应超过4位,小数位不应超过6位！");
							return false;
						}
...
function queryPubAmntRelaPlanInfo(){
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_app.LCContPlanTradSql");
	tSQLInfo.setSqlId("LCContPlanTradSql10");
	tSQLInfo.addSubPara(tGrpPropNo);
...
function showRelaDuty(){
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_app.LCContPlanTradSql");
	tSQLInfo.setSqlId("LCContPlanTradSql11");
	tSQLInfo.addSubPara(tGrpPropNo);
	tSQLInfo.addSubPara(tSysPlanCode);
	tSQLInfo.addSubPara(tPlanCode);
...
function saveClick() {
	...
	fm2.action = "./LCContPlanPubAmntRelaSave.jsp?Operate=SAVEPUBAMNT&GrpPropNo="+ tGrpPropNo +"&SysPlanCode="+ tSysPlanCode +"&PlanCode="+ tPlanCode+"&LimitAmnt="+ tLimitAmnt +"&PolicyNo="+ tPolicyNo;
	submitForm(fm2);
}
/**
 * 缴费金额--保存
...
function saveInfoClick(){
	...
	fm2.action = "./LCContPlanPubAmntRelaSave.jsp?Operate=SAVEFEE&GrpPropNo="+ tGrpPropNo +"&SysPlanCode="+ tSysPlanCode +"&PlanCode="+ tPlanCode+"&RiskCode="+ tRiskCode +"&PolicyNo="+ tPolicyNo;
	submitForm(fm2);
}
/**
 * 公共保额使用关联--取消
...
function cancelClick() {
	...
	fm2.action = "./LCContPlanPubAmntRelaSave.jsp?Operate=DELETEPUBAMNT&GrpPropNo="+ tGrpPropNo +"&SysPlanCode="+ tSysPlanCode +"&PlanCode="+ tPlanCode +"&PolicyNo="+ tPolicyNo;
	submitForm(fm2);
}
...
function checkPubAmnt() {
	...
	var tDutyTypeArr = [];//记录门诊、住院责任数组 add by dianj
	var tCount = 0;
	//校验责任限额
	for(var i=0;i < PubAmntRelaDutyGrid.mulLineCount;i++){
	...
	if (tArr==null) {
		...
		for (var i=0; i<tArr.length; i++) {
			...
			if (tDefaultValue==null || tDefaultValue=="") {
				...
				tFieldType = tArr[i][3];
				if(document.all(tFactorCode) != null){
				...
				if (tFieldType=="1") {//下拉框,而外清空codename
					if(document.all(tFactorCode+"Name") != null){
...
function showTZInfo(){
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_app.LCContPlanTradSql");
	tSQLInfo.setSqlId("LCContPlanTradSql16");
	tSQLInfo.addSubPara(tRiskCode);
...
function saveAccClick(){
	...
	var tSelNo = PayFeeGrid.getSelNo();
	if (tSelNo==0) {
		alert("请选择一条缴费项信息！");
		return false;
	}
	
	var tPayPlanPrem = PayFeeGrid.getRowColData(tSelNo-1, 5);
	
	var tCount = PayFeeGrid.mulLineCount;
	if (tCount==0) {
		alert("没有需要修改的投资账户信息！");
		return false;
	}
	...
	for (var i=0;i < TZFeeGrid.mulLineCount;i++) {
		
		var tInvestMoney = TZFeeGrid.getRowColData(i,5);
		var tInvestRate = TZFeeGrid.getRowColData(i,6);
	...
	fm2.action = "./LCContPlanPubAmntRelaSave.jsp?Operate=SAVEACC&GrpPropNo="+ tGrpPropNo +"&SysPlanCode="+ tSysPlanCode +"&PlanCode="+ tPlanCode+"&RiskCode="+ tRiskCode+"&DutyCode="+tDutyCode+"&PayPlanCode="+tPayPlanCode+"&PolicyNo="+tPolicyNo;
	submitForm(fm2);
}
//校验缴费金额需等于初始保费
function beforeSub(){
...
function tryCal() {
	...
	fm2.action = "./LCContTryCalSave.jsp?Operate=TRYCAL&PolicyNo="+ tPolicyNo +"&MissionID="+ tMissionID +"&SubMissionID="+ tSubMissionID +"&ActivityID="+ tActivityID +"&ContPlanType="+ tContPlanType;
	submitForm(fm2); 
}
...
function dealAfterSubmit(o) {
	...
	if (tOperate=="ADDPLANDETAIL" || tOperate=="MODIFYPLANDETAIL" || tOperate=="DELPLANDETAIL") {
		...
		document.getElementById("divInfo7").style.display = "none";

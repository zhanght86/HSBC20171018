...
function queryMain(tFlag) {
	...
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveAppSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveAppSql1");
...
function querySelf(type) {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveAppSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveAppSql2");
...
function applay() {
	...
	if (mOperator==tLastOperator) {
		alert("案件审批人与案件复核人不可为同一人!");
		return false;
	}	
...
function enterCase(parm1) {
	...
	window.location.href="LLClaimCaseApproveInput.jsp?GrpRgtNo="+tGrpRgtNo+"&Mode=0";
}
/**
 * 释放案件
 */

...
function queryAcceptInfo() {
	...
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql1");
	...
	if (tArr!=null && tArr.length==1) {
		...
		+ "已受理人数【"+ tArr[0][11] +"】,未受理人数【"+ tArr[0][12] +"】,待审批人数【"+ tArr[0][13] +"】,";
...
function queryCustomerList() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql2");
	tSQLInfo.addSubPara(mGrpRegisterNo);
...
function showSelectCustomer() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql3");
...
/*	//统计个人已赔付事件数、赔付金额
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql4");
	tSQLInfo.addSubPara(tCustomerNo);
...
function queryLastClaimInfo() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql18");
...
function showSelectCase() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql6");
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	tSQLInfo.addSubPara(fm.CustomerNo.value);
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql7");
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	tSQLInfo.addSubPara(fm.CaseNo.value);
	tSQLInfo.addSubPara(fm.CustomerNo.value);	
...
function queryCaseDutyInfo() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql8");
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	tSQLInfo.addSubPara(fm.CaseNo.value);
	tSQLInfo.addSubPara(fm.CustomerNo.value);
...
function queryClaimInfo() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql10");
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	tSQLInfo.addSubPara(fm.CaseNo.value);
	tSQLInfo.addSubPara(fm.CustomerNo.value);
...
function queryCustomerCaseList() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql5");
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	tSQLInfo.addSubPara(fm.CustomerNo.value);
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	tSQLInfo.addSubPara(fm.CustomerNo.value);
...
function queryReviewInfo() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql12");
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	...
	if (tArr==null || tArr.length<=0) {
		...
		if (tArr.length<=1) {	
			...
			fm.Conclusion.value = tArr[1][0];
			fm.ConclusionName.value = tArr[1][1];
			fm.ReviewAdvice.value = tArr[0][2];
			fm.AgainReviewAdvice.value = tArr[1][2];
		}
	}
}
/**
...
function queryCheckInfo() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql16");
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	...
	if (tArr==null || tArr.length<=0) {
		...
		if (tArr.length>=1) {
			...
			fm.ChkConclusion.value = tArr[0][0];
			fm.ChkConclusionName.value = tArr[0][1];
			fm.CheckAdvice.value = tArr[0][2];
			fm.ChkAdvice.value = tArr[0][3];
			fm.ChkAdviceName.value = tArr[0][4];
		}
	}
}
/**
...
function queryApproveInfo() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql17");
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	...
	if (tArr==null || tArr.length<=0) {
		...
		if (tArr.length>=1) {
			...
			var tCheckType = tArr[0][5];
			if (tCheckType=="2") {
				tApproveInfo.style.display = "";
				fm.AppConclusion.value = tArr[0][0];
				fm.AppConclusionName.value = tArr[0][1];
				fm.ApproveAdvice.value = tArr[0][2];
				fm.AppAdvice.value = tArr[0][3];
				fm.AppAdviceName.value = tArr[0][4];				
			}			
		}
	}
}
...
function showSurvey() {
   ...
   if(tInsuredNo == "" || tInsuredNo == null) {
    ...
    alert("出险人编码为空，请核查信息！");
		...
		var strUrl="LLClaimPreinvestInputMain.jsp?GrpRgtNo="+tGrpRgtNo+"&RgtNo="+tRgtNo+"&custNo="+tInsuredNo+"&surveyType=01&initPhase=04&AppntNo="+tAppntNo+"&ManageCom="+tManageCom+"&Mode=1";
...
function blackList() {
	...
	var strUrl="./LLClaimBlackListMain.jsp?GrpRgtNo="+fm.GrpRgtNo.value+"&Mode=1&ClmState=50";
	var tWidth=1100;
	var tHeight=700;
	var tTop=(window.screen.availHeight-30-tHeight)/2;       //获得窗口的垂直位置;
	var tLeft=(window.screen.availWidth-10-tWidth)/2;        //获得窗口的水平位置;	
...
function showAjustInfo() {
	...
	//if (fm.CaseType.value!="01" && fm.CaseType.value!="02") {
		...
		tSQLInfo = new SqlClass();
		tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
		tSQLInfo.setSqlId("LLClaimCaseApproveSql11");
		tSQLInfo.addSubPara(fm.RegisterNo.value);
		tSQLInfo.addSubPara(fm.CustomerNo.value);
		tSQLInfo.addSubPara(fm.CaseNo.value);
...
function goToBack() {
	...
	if (mMode==1 || mMode=="1") {
		...
		window.location.href="./LLClaimCaseApproveAppInput.jsp";
	}	
}
/**
 * 核赔履历查看
 */
function showCaseTrace() {
...
function approveConfirm() {
	...
	if(!notNull(fm.AppConclusion,"审批结论")){return false;};
	if(!notNull(fm.ApproveAdvice,"审批意见")){return false;};
	//检查是否存在未处理的问题件
	/*if (!checkQuestion()) {		
		return false;
	}*/
	...
	fm.Operate.value = "APPROVECONFIRM";
	if(submit != 0){
		return false;
	}
	submit = submit+1;
...
/*function checkQuestion() {
	...
	if (tCaseType!=null && tCaseType=="02") {
		...
		tSQLInfo = new SqlClass();
		tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
		tSQLInfo.setSqlId("LLClaimCaseApproveSql13");
		tSQLInfo.addSubPara(fm.RegisterNo.value);
...
function checkQueryInq() {
	...
	tSQLInfo = new SqlClass();
	tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
	tSQLInfo.setSqlId("LLClaimCaseApproveSql14");
	tSQLInfo.addSubPara(mGrpRegisterNo);
	tSQLInfo.addSubPara(fm.RegisterNo.value);
	tSQLInfo.addSubPara(fm.CustomerNo.value);
	var tExistArr = easyExecSql(tSQLInfo.getString(), 1, 0, 1);
	if (tExistArr!=null) {
		...
		if (tInqState!="06" && tInqState!="07" && tInqState!="00") {
...
function afterSubmit(FlagStr, content) {
	...
	if (FlagStr=="Fail") {
		...
		showInfo.focus();
		if (fm.Operate.value == "APPROVECONFIRM" || fm.Operate.value == "ALLCONFIRM") {
			
			//校验是否还存在待复核的出险人，若存在停留当前页，否则跳出
			tSQLInfo = new SqlClass();
			tSQLInfo.setResourceName("g_claim.LLClaimCaseApproveSql");
			tSQLInfo.setSqlId("LLClaimCaseApproveSql15");
			tSQLInfo.addSubPara(mGrpRegisterNo);
			var tExistArr = easyExecSql(tSQLInfo.getString(), 1, 0, 1);
...
function afterCodeSelect(cCodeName, Field) {
	...
	if (cCodeName == "givetype"){
		...
		var tChkConclusion = fm.AppConclusion.value;

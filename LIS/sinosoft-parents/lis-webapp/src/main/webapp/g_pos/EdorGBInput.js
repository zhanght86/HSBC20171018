...
function queryOldClick(){
	...
	tSQLInfo.setResourceName("g_pos.EdorGBSql");
	tSQLInfo.setSqlId("EdorGBSql1");
	...
	tSQLInfo.addSubPara(fm.OldGetStartType.value);
...
function queryUpClick(o){
	...
	tSQLInfo.addSubPara(fm.NewGetStartType.value);
...
function deleteOperate(){
	...
	mOperate="DELETE";
	fm.action = "./EdorGBSave.jsp?Operate="+ mOperate+"&EdorType="+tEdorType+"&EdorAppNo="+tEdorAppNo+"&GrpContNo="+tGrpContNo+"&MissionID="+tMissionID+"&SubMissionID="+tSubMissionID+"&ActivityID="+tActivityID;
	submitFunc();
}

//领取年龄变更
function updateGetYear(){
	...
	for (var i=0;i<rowNum;i++) {
		...
		if (OldInsuredInfoGrid.getChkNo(i)) {
			...
			var tGetYear = OldInsuredInfoGrid.getRowColData(i, 11);
			var tGetStartType = OldInsuredInfoGrid.getRowColData(i, 12);
			if (tGetYear==fm.GetYear.value && tGetStartType==fm.GetStartType.value) {
				
				alert("第"+(i+1)+"行被保险人领取年龄及起领日期计算类型未变更！");
				return false;
			}
		}
	}

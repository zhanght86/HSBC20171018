...
function GrpPerPolDefine(){
	...
	mySql = new SqlClass();
	mySql.setResourceName("cardgrp.AscriptionRuleInputSql");
	mySql.setSqlId("AscriptionRuleSql1");
	mySql.addSubPara(tGrpContNo); 
	var tImpAscriptionRuleCode  = easyQueryVer3(mySql.getString(), 1, 0, 1);
...
function GrpPerPolDefineOld(){
	...
	mySql = new SqlClass();
	mySql.setResourceName("cardgrp.AscriptionRuleInputSql");
	mySql.setSqlId("AscriptionRuleSql2");
	mySql.addSubPara(tGrpContNo); 
...
function initAscriptionRuleOld(tGrpContNo){
	...
	mySql = new SqlClass();
	mySql.setResourceName("cardgrp.AscriptionRuleInputSql");
	mySql.setSqlId("AscriptionRuleSql3"); 
	var str  = easyQueryVer3(mySql.getString(), 1, 0, 1);
	//alert(str);
    return str;
}
//单选框点击触发事件
function ShowAscriptionRule(parm1,parm2){
	...
	*/
	...
	mySql = new SqlClass();
	mySql.setResourceName("cardgrp.AscriptionRuleInputSql");
	mySql.setSqlId("AscriptionRuleSql4"); 
	mySql.addSubPara(document.all('GrpContNo').value ); 
	mySql.addSubPara(cAscriptionRuleCode); 
	turnPage.strQueryResult  = easyQueryVer3(mySql.getString(), 1, 0, 1);
	if (turnPage.strQueryResult) {
	         // alert("result is right");
		//查询成功则拆分字符串，返回二维数组
...
function queryrelate()
{
	...
	mySql = new SqlClass();
	mySql.setResourceName("cardgrp.AscriptionRuleInputSql");
	mySql.setSqlId("AscriptionRuleSql5"); 
	mySql.addSubPara(fm.GrpContNo.value );  
	turnPage.queryModal(mySql.getString() , AscriptionRuleOldGrid);
}

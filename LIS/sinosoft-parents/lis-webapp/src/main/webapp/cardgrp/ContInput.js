...
var sqlresourcename = "cardgrp.ContInputSql";
...
function afterSubmit( FlagStr, content,contract )
{
    ...
    var uwflag = easyExecSql(mySql2.getString());
    if(uwflag!="9")
    {
    	mark="fail";
    }
    //strSql = "select uwflag From lcpol where contno='"+contract+"'"; 
		...
		mySql3.addSubPara(contract);
    
    var uwflag2 = easyExecSql(mySql3.getString());
    var i;
...
function afterQuery( arrQueryResult ) {
	...
	if( arrQueryResult != null ) {
		...
		mySql6.addSubPara( arrQueryResult[0][0]);
...
arrResult=easyExecSql(mySql8.getString(),1,0);
 ...
 var sqlid10="ContInputSql10";
		...
		mySql10.addSubPara(arrResult[0][19]);
...
*********************/
    ...
    arrResult = easyExecSql(mySql11.getString(),1,0);
    ...
    arrResult1 = easyExecSql(mySql12.getString(),1,0);
...
/***********************************/
	...
	arrResult=easyExecSql(mySql14.getString(),1,0)	
    if(arrResult==null){
      //alert("未得到主业务员信息");
      //return;
    }else{
...
//查询业务员告知
   ...
   turnPage.queryModal(mySql15.getString(), AgentImpartGrid);
...
function displayLCCont() {
    ...
    try { fm.all('SecPayMode').value = arrResult[0][33]; } catch(ex) { };
...
function queryAppntNo() {
  ...
  {
   // arrResult = easyExecSql("select a.*,(select GrpName from LDGrp where CustomerNo=(select GrpNo from LDPerson where CustomerNo=a.CustomerNo) ) from LDPerson a where 1=1  and a.CustomerNo = '" + fm.all("AppntNo").value + "'", 1, 0);
    
     var sqlid17="ContInputSql17";
		var mySql17=new SqlClass();
		mySql17.setResourceName(sqlresourcename);
		mySql17.setSqlId(sqlid17);
		mySql17.addSubPara(fm.all("AppntNo").value);
    arrResult = easyExecSql(mySql17.getString(), 1, 0);
    if (arrResult == null) {
      alert("未查到投保人信息");
...
function queryAgent2()
{
    ...
    var sqlid19="ContInputSql19";
		...
		mySql19.addSubPara(cAgentCode);
		mySql19.addSubPara(fm.all('ManageCom').value);
...
var sqlid20="ContInputSql20";
		...
		mySql20.addSubPara(fm.AppntBankAccNo.value);
		mySql20.addSubPara(fm.AppntNo.value);
...
function getdetailwork()
{
  ...
  var sqlid21="ContInputSql21";
		...
		mySql21.addSubPara(fm.AppntOccupationCode.value);
  
  var arrResult = easyExecSql(mySql21.getString());
  if (arrResult != null) {
...
function getdetailaddress(){
 	//var strSQL="select b.* from LCAddress b where b.AddressNo='"+fm.AppntAddressNo.value+"' and b.CustomerNo='"+fm.AppntNo.value+"'";
      ...
      var sqlid22="ContInputSql22";
		...
		mySql22.addSubPara(fm.AppntAddressNo.value);
...
function afterCodeSelect( cCodeName, Field )
{
 ...
 if(cCodeName=="GetAddressNo"){
 	//var strSQL="select b.* from LCAddress b where b.AddressNo='"+fm.AppntAddressNo.value+"' and b.CustomerNo='"+fm.AppntNo.value+"'";
		...
		mySql24.addSubPara(fm.AppntAddressNo.value); 
		mySql24.addSubPara(fm.AppntNo.value);
        
        
        arrResult=easyExecSql(mySql24.getString());
   try{fm.all('AppntNo').value= arrResult[0][0];}catch(ex){};
   try{fm.all('AppntAddressNo').value= arrResult[0][1];}catch(ex){};
   try{fm.all('AppntPostalAddress').value= arrResult[0][2];}catch(ex){};
   try{fm.all('AppntZipCode').value= arrResult[0][3];}catch(ex){};
...
*/
		...
		mySql26.addSubPara(fm.AppntName.value); 
		mySql26.addSubPara(Sex);
		mySql26.addSubPara(fm.AppntBirthday.value);
		mySql26.addSubPara(fm.AppntNo.value);
		mySql26.addSubPara(fm.AppntIDType.value);
		mySql26.addSubPara(fm.AppntIDNo.value);
		mySql26.addSubPara(fm.AppntNo.value);
...
function getCompanyCode()
{
   ...
   // strsql = "select CustomerNo,GrpName from LDgrp ";
    ...
    fm.all("AppntGrpNo").CodeData=tCodeData+easyQueryVer3(mySql27.getString(), 1, 0, 1);
}
...
function afterSubmit2( FlagStr, content )
{
  ...
  if(InsuredGrid.mulLineCount>0){
    fm.all('spanInsuredGrid').all('InsuredGridSel').checked='true'
  }
...
function intoRiskInfo()
{
  ...
  var sqlid29="ContInputSql29";
		...
		mySql29.addSubPara(fm.ProposalContNo.value); 
		mySql29.addSubPara(tInsuredNo);
  
  //alert(sqlstr);
  var sequence = easyExecSql(mySql29.getString(),1,0);
 ...
 function deleteRecord()
{
   ...
   // alert("ContType"+ContType);
    ...
    //alert("fm.all('fmAction').value"+fm.all('fmAction').value);
    fm.action="ContInsuredSave.jsp";
   ...
   // arrResult=easyExecSql("select * from LCInsured where ContNo='"+tContNo+"'",1,0);
    ...
    else
    {
      ...
      var sqlid37="ContInputSql37";
		...
		mySql37.addSubPara(tCustomerNo);
...
function getImpartInfo()
{
    ...
    if(InsuredNo!=null&&InsuredNo!="")
    {
	   ...
	   // var strSQL1="select ImpartParam from LCCustomerImpartparams where CustomerNoType='1' and CustomerNo='"+InsuredNo+"' and ContNo='"+ContNo+"' and impartver='01' and impartcode='001' and ImpartParamNo = '2'";
	    ...
	    arrResult = easyExecSql(mySql40.getString(),1,0);
	    ...
	    arrResult1 = easyExecSql(mySql41.getString(),1,0);
...
function checkrelation()
{
    ...
    else
    {
		...
		mySql45.addSubPara(fm.all('ContNo').value);
        ...
        ***/
		...
		mySql46.addSubPara(fm.all('ContNo').value);
...
function queryInsuredNo()
{
    ...
    else
    {
		...
		mySql47.addSubPara(fm.all("InsuredNo").value);
...
function getdetailwork2()
{
   // var strSql = "select OccupationType from LDOccupation where OccupationCode='" + fm.OccupationCode.value+"'";
    ...
    var arrResult = easyExecSql(mySql50.getString());
    if (arrResult != null)
    {
...
function inputConfirm2(wFlag)
{
    ...
    if (wFlag ==1 ) //录入完毕确认
    {
       // var tStr= "	select * from lwmission where 1=1 and lwmission.activityid in ('0000001001','0000001002','0000001220','0000001230') and lwmission.missionprop1 = '"+fm.ContNo.value+"'";
		...
		mySql51.addSubPara(fm.OccupationCode.value);
...
function getdetailaddress2()
{
   ...
   // var strSQL="select b.AddressNo,b.PostalAddress,b.ZipCode,b.Phone,b.Mobile,b.EMail,b.CompanyPhone,b.CompanyAddress,b.CompanyZipCode,b.fax,b.HomePhone,b.grpName,b.province,b.city,b.County from LCAddress b where b.AddressNo='"+fm.InsuredAddressNo.value+"' and b.CustomerNo='"+fm.InsuredNo.value+"'";
    ...
    arrResult=easyExecSql(sqlid52.getString());
  ...
  var sqlstr="select * from ldperson where Name='"+tInsuredName
    ...
    */
     var sqlid58="ContInputSql58";
		...
		mySql58.addSubPara(fm.IDType.value);
		mySql58.addSubPara(fm.IDNo.value);
		mySql58.addSubPara(fm.tInsuredNo.value);
...
arrResult = easyExecSql(mySql60.getString());
if (arrResult != null) {
...
function InsuredChkNew(){
  ...
  */
    var sqlid61="ContInputSql61";
		...
		mySql61.addSubPara(fm.IDType.value);
        mySql61.addSubPara(fm.IDNo.value);
        mySql61.addSubPara(fm.tInsuredNo.value);
...
function getImpartInitInfo()
{
  ...
  */
  var sqlid65="ContInputSql65";
		...
		mySql65.setResourceName(sqlresourcename);
		mySql65.setSqlId(sqlid65);
		mySql65.addSubPara(tContNo);
	turnPage.queryModal(mySql65.getString(),AgentImpartGrid);
	return true;
}

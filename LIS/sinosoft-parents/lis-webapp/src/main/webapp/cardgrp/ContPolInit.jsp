...
function initInpBox()
{
	try 
	{
		fm.all('ProposalGrpContNo').value = prtNo;
		fm.all('GrpContNo').value = prtNo;
...
//alert("a");		
		...
		if(this.ScanFlag == "1"||this.ScanFlag == "0")
		{
			...
			if(arrResult1 != null)
			{
			fm.all('GetFlag').value=arrResult1[0][0];
			fm.all('BankCode').value=arrResult1[0][1];
...
//alert("c");		
		...
		if(this.LoadFlag == "16")
		{
		  ...
		  //团险卡单查询
		  if(cardflag=="1"){
		    fm.buttonScanDoc.disabled =true;
		  }
	    try
	    {
	    	if(scantype=="scan")
		...
		if(this.LoadFlag == "18")
		{
			...
			divnopertoperbtn.style.display="";
			divquerybtn.style.display="";
			getapproveinfo();
		  //代码汉化的反显函数
		  showCodeName();

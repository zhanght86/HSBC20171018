...
<%
	//程序名称：CertifyDestroyConfirmSave.jsp
	//程序功能：
	//创建日期：2002-09-23
	//创建人  ：mw
...
<%
	...
	try {
		...
		String tStartNo[] = request.getParameterValues("CertifyList6"); //单证起始号
		String tEndNo[] = request.getParameterValues("CertifyList7"); //单证终止号
		...
		loggerDebug("CertReveTakeBackSave","CertReveTakeBackSave.jsp传入数据：");
		loggerDebug("CertReveTakeBackSave","核销修订申请号码=" + tApplyNo[i]);
			}
		}
		...
		// 设置操作字符串
		String szOperator = "CONFIRM";

		// 数据传输
		/*CertReveTakeBackUI tCertReveTakeBackUI = new CertReveTakeBackUI();
		if (!tCertReveTakeBackUI.submitData(vData, szOperator)) {
			Content = " 保存失败，原因是: "
			+ tCertReveTakeBackUI.mErrors.getFirstError();
			FlagStr = "Fail";
		} else {
			Content = " 保存成功 ";
			FlagStr = "Succ";
		...
		String busiName="CertReveTakeBackUI";
		  String mDescType="保存";

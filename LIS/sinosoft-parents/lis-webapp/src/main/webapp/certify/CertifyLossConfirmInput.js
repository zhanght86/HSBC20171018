...
var tResourceName="certify.CertifyLossConfirmInputSql";
//查询待批复的增领申请
function queryClick()
{
  initCertifyList();
...
function submitForm()
{
  ...
  if(fm.PublishDate.value=='' || fm.Newspaper.value==''){
    alert("请输入【登报日期】和【报纸名称】！");
    return false;
  }
...
function afterSubmit( FlagStr, content )
{
  ...
  if(FlagStr == "Fail" ) {             
	...
	s//howModalDialog(urlStr,window,"status:no;help:0;close:0;dialogWidth:550px;dialogHeight:350px");
	var name='提示';   //网页名称，可为空; 
	var iWidth=550;      //弹出窗口的宽度; 
	var iHeight=350;     //弹出窗口的高度; 
	var iTop = (window.screen.availHeight - iHeight) / 2; //获得窗口的垂直位置 

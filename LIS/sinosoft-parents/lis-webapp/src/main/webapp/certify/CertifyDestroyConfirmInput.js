...
var tResourceName="certify.CertifyDestroyConfirmInputSql";
function myonfocus()
{
	if(showInfo!=null)
	{
...
function queryClick()
{
  ...
  /*var strSql = "select a.applyno, a.certifycode, (select certifyname from lmcertifydes b where b.certifycode = a.certifycode),"
			 +"a.sendoutcom, a.receivecom,"
			 +"a.startno, a.endno, a.sumcount, a.reason from lzcardapp a where a.OperateFlag='4' and a.stateflag='1' "
			 + getWherePart('a.handler', 'handler')
			 + getWherePart('a.handledate', 'handledate')
			 + getWherePart('a.ApplyCom', 'ComCode', 'like')
			 + getWherePart('a.ApplyCom', 'MngCom', 'like')
			 +" order by a.certifycode,a.startno";*/
  var strSql = wrapSql(tResourceName,"querysqldes5",[fm.Handler.value,fm.HandleDate.value,fm.ComCode.value,fm.MngCom.value]);
  turnPage.strQueryResult  = easyQueryVer3(strSql, 1, 0, 1); 
  if (!turnPage.strQueryResult) {
 	 alert("没有销毁提交记录,请检查查询条件！");
     return false;
    }else{

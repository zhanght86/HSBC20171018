...
var tResourceName="certify.CardSendOutReceiveSql";
//单证状态查询
function certifyQuery()
{
    ...
    CardSendOutReceiveGrid.clearData();//清除表格中所有数据

    var ManageCom = fm.ManageCom.value;
    var strSQL="";//查寻SQL语句字串
    /*strSQL="select  a.sendoutcom, a.receivecom,"
		...
		+" else	'未知' end),a.certifycode,(select certifyname from lmcertifydes where certifycode = a.certifycode),"
    ...
    strSQL = wrapSql(tResourceName,"querysqldes1",[ManageCom,ManageCom,document.all('CertifyCode').value,document.all('sendoutcom').value
   	...
   	turnPage.queryModal(strSQL, CardSendOutReceiveGrid, 1);
   	//prompt("strSQL：",strSQL);
   	if(CardSendOutReceiveGrid.mulLineCount==0)
   	{
   		alert("没有查询到任何单证信息！");
   		return false;
   	}
...
function certifyPrint()
{
	...
	easyQueryPrint(2,'CardSendOutReceiveGrid','turnPage');	
}

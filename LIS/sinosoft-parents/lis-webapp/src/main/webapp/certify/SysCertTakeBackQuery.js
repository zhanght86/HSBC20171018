...
//           ;
        	...
        	mySql1.setResourceName("certify.SysCertTakeBackQuerySql"); //指定使用的properties文件名
        	mySql1.setSqlId(sqlid1);//指定使用的Sql的id
        	mySql1.addSubPara(ManageCom);//指定传入的参数
        	mySql1.addSubPara(tAgentCode);//指定传入的参数
        	mySql1.addSubPara(CertifyNo);//指定传入的参数
        	mySql1.addSubPara(SendOutCom);//指定传入的参数
        	mySql1.addSubPara(ReceiveCom);//指定传入的参数
...
//			}
		...
		mySql2.setResourceName("certify.SysCertTakeBackQuerySql"); //指定使用的properties文件名
		mySql2.setSqlId(sqlid2);//指定使用的Sql的id
		mySql2.addSubPara(ManageCom);//指定传入的参数
		mySql2.addPara(fm.sql_where.value);//指定传入的参数
		mySql2.addSubPara(CertifyCode);//指定传入的参数
		mySql2.addSubPara(CertifyNo);//指定传入的参数
		mySql2.addSubPara(SendOutCom);//指定传入的参数
		mySql2.addSubPara(ReceiveCom);//指定传入的参数
		mySql2.addSubPara(Handler);//指定传入的参数
		mySql2.addSubPara(HandleDate);//指定传入的参数

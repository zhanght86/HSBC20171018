...
<%
  CardPrintUI cardPrintUI = new CardPrintUI();
	...
	try {
	  ...
	  // 数据传输
	  loggerDebug("CardPrintQuerySubmit","before cardPrintUI.submitData");
		if (!cardPrintUI.submitData(tVData, "QUERY||MAIN")) {
			out.println( buildMsg(false, "查询失败，原因是:" + cardPrintUI.mErrors.getFirstError()));
			return;
		
		} else {
			tVData.clear();
			tVData = cardPrintUI.getResult();

...
* <p>Description: EasyScan查询中心的单证索引数据处理</p>
...
<%
loggerDebug("QueryIndex","-------------QueryIndex.jsp Begin---------------------");
...
try{
  ...
  loggerDebug("QueryIndex",xmlBuffer.toString());
  ...
  //提交UI处理
  QueryIndexUI tQueryIndexUI   = new QueryIndexUI();
  tQueryIndexUI.submitData(nVData,"");

  //获得返回数据
  VData resultData = new VData();
  resultData = tQueryIndexUI.getResult();
  ...
  //如果出现异常，返回异常信息
  String strErr = "QueryIndex.jsp--查询中心的单证索引数据处理失败！\n" + e.toString();
  ...
  loggerDebug("QueryIndex",strErr);
}
finally{
  //输出返回数据
  out.println("<IndexXML>" + bufXML.toString() + "</IndexXML>");
  loggerDebug("QueryIndex","-------------QueryIndex.jsp End---------------------");
}
%>
<HTML>
  <title>查询中心的单证索引数据处理</title>
  <BODY>
    wellhi
  </BODY>
</HTML>

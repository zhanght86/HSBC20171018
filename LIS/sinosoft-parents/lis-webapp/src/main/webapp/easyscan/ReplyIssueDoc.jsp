...
* <p>Description: EasyScan问题卷修改保存处理</p>
...
<%
loggerDebug("ReplyIssueDoc","-------------ReplyIssueDoc.jsp Begin---------------------");
...
try{
  ...
  loggerDebug("ReplyIssueDoc",xmlBuffer);
...
//  nVData.add(tGI);
  ...
  //如果出现异常，返回异常信息
  String strErr = "ReplyIssueDoc.jsp--问题卷修改保存处理失败！\n" + e.toString();
  ...
  loggerDebug("ReplyIssueDoc",strErr);
}
finally{
  //输出返回数据
  out.println("<IndexXML>" + bufXML.toString() + "</IndexXML>");
  loggerDebug("ReplyIssueDoc","-------------ReplyIssueDoc.jsp End---------------------");
}
%>
<HTML>
  <title>问题卷修改保存处理</title>
  <BODY>
    wellhi
  </BODY>
</HTML>

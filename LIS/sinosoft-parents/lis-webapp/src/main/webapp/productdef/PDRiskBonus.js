...
function button294(calType)
{
	...
	if( calType == '1' )
	{
		calCode = Mulline10Grid.getRowColData(selNo-1,3);
	}else{
		calCode = Mulline10Grid.getRowColData(selNo-1,4);
	}
...
{
   var strSQL = "";
   strSQL = "select DECODE(standbyflag1, 'N', '[*]' || fieldname,  fieldname),fieldcode,fieldtype,null,officialdesc,busidesc from Pd_Basefield where tablecode = upper('PD_LMRiskBonus') and isdisplay = '1' order by displayorder";Mulline9GridTurnPage.pageLineNum  = 3215;
   Mulline9GridTurnPage.queryModal(strSQL,Mulline9Grid);
}
function queryMulline10Grid()
{
   var strSQL = "";
   strSQL = "SELECT RiskCode,RiskVer,Bonuscoefcode,Addamntcoefcode,Bonuscalstart,Bonuscaldend FROM PD_LMRiskBonus WHERE RiskCode = '"+fm.all("RiskCode").value+"'";

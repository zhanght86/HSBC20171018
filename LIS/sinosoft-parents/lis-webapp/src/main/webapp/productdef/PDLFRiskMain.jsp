...
<!--标题与状态区域-->
 ...
 <frameset name="fraSet" cols="0%,*,0%" frameborder="no" border="1" framespacing="0" rows="*">
  ...
  <!--交互区域-->
  <frame id="fraInterface" name="fraInterface" scrolling="auto" src="./PDLFRiskInput.jsp?riskcode=<%=request.getParameter("riskcode")%>&pdflag=<%=request.getParameter("pdflag")%>&missionid=<%=request.getParameter("missionid")%>&submissionid=<%=request.getParameter("submissionid")%>&activityid=<%=request.getParameter("activityid")%>&requdate=<%=request.getParameter("requdate")%>">
     <!--下一步页面区域-->

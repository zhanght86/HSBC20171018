...
function initForm()
{
	try{
		...
		myAlert("PDRiskPayIntvInit.jsp-->"+"初始化界面错误!");
	}
}
function updateDisplayState()
{
 ...
 for(var i = 0; i < rowCount; i++)
 {
	 ...
	 else 
	 {
 		...
 		{
 			if(Mulline9Grid.getRowColData(i,2) == "PAYINTV")
	 		{
		 		Mulline9Grid.setRowColDataCustomize(i,4,Mulline10Grid.getRowColData(selNo-1,2),null,"readonly"); 		 
	 		}else
	 		{ 			
	 	 		var tDefaultValueSQL = "select "+Mulline9Grid.getRowColData(i,2)+" from "+fm.all("tableName").value+" where riskcode = '"+fm.all('RiskCode').value+"' and PAYINTV = '"+Mulline10Grid.getRowColData(selNo-1,2)+"'";

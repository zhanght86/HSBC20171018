...
var cValue;
var dValue;
var sourcename="productdef.PDRiskCommission";
...
function addClick()
{
      ...
      fm.all("AgentOrgan").value="000000";
      fm.all("Risk").value="";
      ...
      fm.all("IDNo").value="";
      fm.all("STAFFRATE").value="0";
      fm.all("SRFlag").value="N";
...
//	}
	...
	if(fm.all("CommissionRate").value==""){
		myAlert(""+"佣金比例不能为空!"+"");
		return false;
	}
	if(!decimal.test(fm.all("CommissionRate").value)){
		myAlert(""+"佣金比例不是有效的数字，小数点后面最多只有五位!"+"");
		fm.all("CommissionRate").value="";
		return false;
	}
...
function checkInsureAgeInterval(){
	...
	mySql.setSqlId("LABankChargeRateSql3");	
	mySql.addSubPara(date.getTime());	       		
	mySql.addSubPara(trim(fm.all('ManageOrgan').value));
	mySql.addSubPara(trim(fm.all('AgentOrgan').value));
...
function easyQueryClick()
{
	...
	var mySql = new SqlClass();
        mySql.setResourceName(sourcename);
	mySql.setSqlId("LABankChargeRateSql1");		       		
	mySql.addSubPara(trim(fm.all('ManageCom').value));
	mySql.addSubPara(trim(fm.all('AgentCom').value));
...
function checkRisk(RiskCode){
	...
	mySql = new SqlClass();
	mySql.setResourceName(sourcename);
	mySql.setSqlId("LABankChargeRateSql2");	
	mySql.addSubPara(date.getTime());	       		
...
function checkRiskCode(){
	...
	if(mRiskCode != checkRiskCodeValue){
		myAlert(""+"只能新增、修改、刪除当前正在定义的产品："+""+mRiskCode+""+"。"+"");
		return false;
	}
	return true;
}

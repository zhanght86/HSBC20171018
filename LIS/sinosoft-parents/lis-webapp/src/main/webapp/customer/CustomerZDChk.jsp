...
loggerDebug("CustomerZDChk","Auto-begin:");
...
<%
	...
	loggerDebug("CustomerZDChk","cout:"+Count);
	...
	loggerDebug("CustomerZDChk","tFICustomerAccTraceSet:" + tFICustomerAccTraceSet.encode());
	...
	try
	{
	  	if (flag == true)
	  	{
            ...
            //调用客户账户收费接口，传入标志ZD
            //if(tFICustomerMain.submitData(tVData, "ZD"))
            //{
            //}
            //else
            //{FlagStr = "Fail";
            //	Content = " 重新指定失败，原因是: " + tFICustomerMain.mErrors.getError(0).errorMessage;
            //}
            if(tBusinessDelegate.submitData(tVData, "ZD","FICustomerMain"))
            {
            }
            else
            {FlagStr = "Fail";
            	Content = " 重新指定失败，原因是: " + tBusinessDelegate.getCErrors().getError(0).errorMessage;
            }

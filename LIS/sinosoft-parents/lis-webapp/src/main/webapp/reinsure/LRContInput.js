...
function veriryInput3()
{
	if(typeRadio=='1'){
		if(fm.ModRIContNo.value==null||fm.ModRIContNo.value==''||fm.ModRIContName.value==null||fm.ModRIContName.value==''){
			myAlert(''+"合同修改模式"+'"修改合同编码""+"、"+""修改合同名称""+"不能为空"+" ');
			return false;
		}
	}
	if(typeRadio=='1'){
...
function checkContState(){
	...
	if(tContState=="0"){
		...
		var mySql100=new SqlClass();
			mySql100.setResourceName("reinsure.LRContInputSql"); //指定使用的properties文件名
			mySql100.setSqlId("LRContInputSql100");//指定使用的Sql的id
			mySql100.addSubPara(tRIContNo);//指定传入的参数
...
function nextStep()
{
	...
	var mySql101=new SqlClass();
			mySql101.setResourceName("reinsure.LRContInputSql"); //指定使用的properties文件名
			mySql101.setSqlId("LRContInputSql101");//指定使用的Sql的id

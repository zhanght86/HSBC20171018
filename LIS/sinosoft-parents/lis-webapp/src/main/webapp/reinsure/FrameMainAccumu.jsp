...
<!--标题与状态区域-->
	...
	<frameset name="fraSet" cols="0%,*,0%" frameborder="no" border="0" framespacing="0" rows="*">
		...
		<!--交互区域-->
		<%
			String szSrc = request.getParameter("Interface")+"?AccNo="+request.getParameter("AccNo")
					+"&AccName="+request.getParameter("AccName")+"&RiskCode="+request.getParameter("RiskCode")
					+"&RiskName="+request.getParameter("RiskName")+"&DutyCode="+request.getParameter("DutyCode")
					+"&DutyName="+request.getParameter("DutyName");
			szSrc = new String((szSrc).getBytes("ISO-8859-1"),"UTF-8");

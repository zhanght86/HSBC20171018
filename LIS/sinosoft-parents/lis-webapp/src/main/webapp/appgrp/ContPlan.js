...
function afterCodeSelect( cCodeName, Field ){
    ...
    {
    //通过险种修改保额或是保费
    var mult = document.all('InsuMult').value;
    var tRiskCode=getRiskCount();
    //查看计划类别
    var tPlanType = document.all('RiskPlan1').value;
    ...
    for(j=0;j<tRiskCode.length;j++){
        var ttRiskCode = tRiskCode[j];
        var tsqlprem=" select (calfactorvalue / remark2),a.dutycode  from lccontplandutyparam a ,lccontplan b where a.grpcontno = b.grpcontno and a.contplancode=b.contplancode and a.calfactor = 'Prem' and a.contplancode = '"+document.all('ContPlanCode').value+"' and a.grpcontno = '"+document.all('GrpContNo').value+"' and a.riskcode='"+ttRiskCode+"'";      
        var tsqlamnt=" select (calfactorvalue / remark2),a.dutycode  from lccontplandutyparam a ,lccontplan b where a.grpcontno = b.grpcontno and a.contplancode=b.contplancode and a.calfactor = 'Amnt' and a.contplancode = '"+document.all('ContPlanCode').value+"' and a.grpcontno = '"+document.all('GrpContNo').value+"' and a.riskcode='"+ttRiskCode+"'";      
        var tPremInfo = easyExecSql(tsqlprem);
        var tAmntInfo = easyExecSql(tsqlamnt);
       // alert("tPrem:"+tPrem);
        if(tPremInfo!=null&&tPremInfo!=""&&tPremInfo!="null"){
    ...
    //var premsql = " select (calfactorvalue / remark2)  from lccontplandutyparam a ,lccontplan b where a.grpcontno = b.grpcontno and a.contplancode=b.contplancode and a.calfactor = 'Prem' and a.contplancode = '"+document.all('ContPlanCode').value+"' and a.grpcontno = '"+document.all('GrpContNo').value+"'";
    //var tPerPrem = easyExecSql(premsql);
    //var amntsql = " select (calfactorvalue / remark2)  from lccontplandutyparam a ,lccontplan b where a.grpcontno = b.grpcontno and a.contplancode=b.contplancode and a.calfactor = 'Amnt' and a.contplancode = '"+document.all('ContPlanCode').value+"' and a.grpcontno = '"+document.all('GrpContNo').value+"'";
	...
	if (cCodeName=="GrpRisk"){
		...
		if (tRiskFlag!="S"){
			...
			document.all('MainRiskCode').value = fm.RiskCode.value;//edit by yaory 20050914
			//alert(document.all('MainRiskCode').value);
		}
	}
	//判断是否选择保险套餐
...
function submitForm()
{
	...
	fm.action="../appgrp/ContPlanSave.jsp";
...
function beforeSubmit(){
	...
	if (document.all('ContPlanName').value == ""){
		alert("请输入保险计划名称！");
		document.all('ContPlanName').focus();
		return false;
	}
	if (document.all('Peoples3').value == ""){
		alert("请输入投保人数！");
	...
	for(var i=0;i<lineCount;i++){
		...
		if (sCalMode == "PayIntv")
      	{
			if (sValue!=""){
				if (sValue!="1" && sValue!="3" && sValue!="6" && sValue!="0" && sValue!="12" && sValue!="-1"){
					alert("请录入正确的交费方式！");
					ContPlanGrid.setFocus(i,8);
					return false;
				}
...
function nextstep()
{
	var newWindow = window.open("../appgrp/ContPlanNextInput.jsp?GrpContNo="+document.all('GrpContNo').value+"&LoadFlag="+LoadFlag,"AgentCommonQueryMain",'width='+screen.availWidth+',height='+screen.availHeight+',top=0,left=0,toolbar=0,location=0,directories=0,menubar=0,scrollbars=1,resizable=1,status=0');
}
...
function QueryDutyClick(){
	...
	//查询该险种下的险种计算要素
	var sql="select risktype6 from lmriskapp where riskcode='"+document.all('RiskCode').value+"'";
...
function easyQueryClick(){
	...
	strSQL = "select ContPlanCode,ContPlanName,PlanSql,Peoples3 "
		     + "from LCContPlan "
		     + "where 1=1 "
		     + "and GrpContNo = '"+document.all('GrpContNo').value+"' and ContPlanCode <> '00' order by ContPlanCode";
	//document.all('ContPlanName').value = strSQL;
	turnPage.strQueryResult  = easyQueryVer3(strSQL, 1, 0, 1);
	//如果没数据也无异常
	if (!turnPage.strQueryResult) {
...
{
	var tPlanKind1=document.all('PlanKind1').value;
...
{
	var tPlanKind1=document.all('PlanKind1').value;
	var tPlanKind2=document.all('PlanKind2').value;
	//alert ("PlanKind1:"+tPlanKind1);
...
function initRiskPlan(cObjCode,cObjName)
{
	var tPlanKind1=document.all('PlanKind1').value;
	var tPlanKind2=document.all('PlanKind2').value;
	var tPlanKind3=document.all('PlanKind3').value;
	mRiskPlan="1"
	if(tPlanKind1!='')
	{
...
{
		if(document.all('RiskPlan1').value=="1")
	{
		//alert("riskplan:"+document.all('RiskPlan1').value);
  document.all('mOperate').value="INSERT||GROUPRISK";
  
  //var showStr="正在添加团单险种信息，请您稍候并且不要修改屏幕上的值或链接其他页面";
	//var urlStr="../common/jsp/MessagePage.jsp?picture=C&content=" + showStr ;  
	//showInfo=window.open(urlStr,"newwindow","height=250;width=550;top=0;left=0;toolbar=no;menubar=no;scrollbars=no;resizable=no;alwaysRaised:yes;");
  
  fm.action="../appgrp/GroupPlanRiskSave.jsp";
  document.getElementById("fm").submit();
  return true;
  //alert("返回结果："+document.all("mFlagStr").value);
  //if(document.all("mFlagStr").value=="Succ")
  //{
  //return true;
  //}
  //else
...
function ImportContPlan()
{
	//查询保单基本信息
	var strSQL = "select GrpContNo,ProposalGrpContNo,ManageCom,AppntNo,GrpName from LCGrpCont where 1 = 1 and GrpContNo = '" + GrpContNo + "'";
	turnPage.strQueryResult   = easyQueryVer3(strSQL, 1, 0, 1);
	turnPage.arrDataCacheSet  = decodeEasyQueryResult(turnPage.strQueryResult);
	if(turnPage.arrDataCacheSet != null)
	{
	...
	turnPage.strQueryResult  = easyQueryVer3(strSQL, 1, 0, 1);
	//如果没数据也无异常
	if (!turnPage.strQueryResult && tImport==0) {
			tImport = 1;
		//alert("导入询价数据");
			var showStr="正在导入保单计划数据，请您稍候并且不要修改屏幕上的值或链接其他页面";
			var urlStr="../common/jsp/MessagePage.jsp?picture=C&content=" + showStr ; 
        var name='提示';   //网页名称，可为空; 
		var iWidth=550;      //弹出窗口的宽度; 
		var iHeight=250;     //弹出窗口的高度; 
		...
		showInfo.focus();			
		  lockScreen('lkscreen');   
		  fm.action="../appgrp/ImportContPlanSave.jsp"
		  document.getElementById("fm").submit(); //提交
	}

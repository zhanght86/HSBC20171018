	...
	if(document.all('RiskCode').value=='211801')
	{
		...
		for(i=0;i<=8;i++)
		{
			...
			if(DutyGrid.getChkNo(i)==true)
			{
				...
				DutyGrid.setRowColData(i, 9, document.all('StandByFlag1').value);//是否在职  0在职 |1退休
				//alert("StandByFlag2"+ document.all('StandByFlag2').value);
				DutyGrid.setRowColData(i, 10, document.all('StandByFlag2').value);//公共责任7档保额
				//alert("StandByFlag2"+ document.all('StandByFlag2').value);
				DutyGrid.setRowColData(i, 11, document.all('StandByFlag3').value);//公共责任8档保额
				//alert("StandByFlag3"+ document.all('StandByFlag3').value);
				//DutyGrid.setRowColData(i, 13, document.all('Prem').value);//保费
				//alert("Prem"+ document.all('Prem').value);
				//DutyGrid.setRowColData(i, 17, document.all('FloatRate').value);//费率/折扣
				//alert("FloatRate"+ document.all('FloatRate').value);
				//DutyGrid.setRowColData(i, 19, document.all('GetLimit').value);//免赔额
				//alert("GetLimit"+ document.all('GetLimit').value);
				//DutyGrid.setRowColData(i, 20, document.all('GetRate').value);//赔付比例
				//alert("GetRate"+ document.all('GetRate').value);
				DutyGrid.setRowColData(i, 18, document.all('PayIntv').value);//缴费方式
			}
			else
		{
				strChooseDuty=strChooseDuty+"0";
 ...
 if(document.all('RiskCode').value=="221601")
       {
        ...
        iArray[4][14]="M";
		    ...
		    iArray[16][3]=2;
		    iArray[16][4]="PolCalRule";
		    iArray[16][11]="0|^1|表定费率^2|表定费率折扣^3|约定保费保额"
		    iArray[16][14] = "0";
  ...
  if (arr.length>0){
   ...
   document.all('PayRuleCode').CodeData=tCodeData;
    ...
    if(document.all('RiskCode').value=="211701")
    {
        ...
        {
            document.all('PayRuleCode').CodeData="0|^错误|没有录入缴费规则，请检查！";
        }
        else
        {
            if (arr.length>0)

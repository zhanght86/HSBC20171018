...
function getcodedata2()
{
	...
	mySql1.setResourceName("appgrp.AppGrpPolQuerySql"); // 指定使用的properties文件名
	mySql1.setSqlId(sqlid1);// 指定使用的Sql的id
	mySql1.addSubPara(fm.sysdate.value);// 指定传入的参数
	mySql1.addSubPara(fm.sysdate.value);// 指定传入的参数
	mySql1.addSubPara(fm.sysdate.value);// 指定传入的参数
...
function returnParent()
{
	...
	mySql2.setResourceName("appgrp.AppGrpPolQuerySql"); // 指定使用的properties文件名
	mySql2.setSqlId(sqlid2);// 指定使用的Sql的id
	mySql2.addSubPara(polNo);// 指定传入的参数
...
function easyQueryClick()
{
	...
	if(tRiskCode == "")
	{
		...
		mySql3.setResourceName("appgrp.AppGrpPolQuerySql"); // 指定使用的properties文件名
		mySql3.setSqlId(sqlid3);// 指定使用的Sql的id
		mySql3.addSubPara(fm.PrtNo.value);// 指定传入的参数
		mySql3.addSubPara(fm.GrpProposalNo.value);// 指定传入的参数
		mySql3.addSubPara(fm.ManageCom.value);// 指定传入的参数
		mySql3.addSubPara(fm.CManagerCode.value);// 指定传入的参数
		mySql3.addSubPara(fm.SaleChnl.value);// 指定传入的参数
	...
	else
	{
		...
		mySql4.setResourceName("appgrp.AppGrpPolQuerySql"); // 指定使用的properties文件名
		mySql4.setSqlId(sqlid4);// 指定使用的Sql的id
		mySql4.addSubPara(fm.PrtNo.value);// 指定传入的参数
		mySql4.addSubPara(fm.GrpProposalNo.value);// 指定传入的参数
		mySql4.addSubPara(fm.ManageCom.value);// 指定传入的参数
		mySql4.addSubPara(fm.CManagerCode.value);// 指定传入的参数
		mySql4.addSubPara(fm.SaleChnl.value);// 指定传入的参数
...
function ScanQuery()
{
	...
	else
	{
		...
		// and subtype!='3000' ";
		// //这里排出查协议的扫描件，如果既有协议又有"团险投保书_年金险种投保单"扫描件那么会有问题。
		// var crr = easyExecSql(tsql);
		// var tsubtype="";
		// //alert(crr);
		// if(crr!=null)
		...
		mySql5.setResourceName("appgrp.AppGrpPolQuerySql"); // 指定使用的properties文件名

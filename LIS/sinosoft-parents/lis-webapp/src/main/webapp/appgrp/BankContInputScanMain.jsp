...
<!--标题与状态区域-->
	...
	<frameset name="fraSet" cols="*" frameborder="yes" border="1" framespacing="0" rows="0,50%,*,0">
		...
		<frame id="fraInterface" name="fraInterface" scrolling="auto" noresize src="./BankContInput.jsp?ScanFlag=<%=request.getParameter("ScanFlag")%>&prtNo=<%=request.getParameter("prtNo")%>&ManageCom=<%=request.getParameter("ManageCom")%>&scantype=<%=request.getParameter("scantype")%>&MissionID=<%=request.getParameter("MissionID")%>&SubMissionID=<%=request.getParameter("SubMissionID")%>">
    <!--下一步页面区域-->

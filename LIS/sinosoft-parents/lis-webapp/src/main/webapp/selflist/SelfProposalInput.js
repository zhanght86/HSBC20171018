...
//	var strSQL="select OccupationType,OccupationName,WorkName,Occupationcode from LDOccupation where occupationver='002'";
	...
	var sqlid1="SelfProposalInputSql1";
 	...
 	mySql1.addSubPara(document.all( 'AppntOccupationName' ).value);//指定传入参数
...
//	var strSQL="select OccupationType,OccupationName,WorkName,Occupationcode from LDOccupation where occupationver='002'";
	...
	var sqlid2="SelfProposalInputSql2";
 	...
 	mySql2.addSubPara(document.all( 'LCInsuredOccupationName' ).value);//指定传入参数
...
function vertifyInput()
{
	...
	if(document.all( 'RelationToLCInsured' ).value!='00')
	{
		...
		//如果投保人与被保人的关系是父母,则姓名,性别,出生日期不能全部相同,否则认定是同一人
		...
		{
		    alert("当与被保人关系为父母时,被保人信息不能与投保人相同!");
		    return false; 
		}	
		
		//如果投保险人年龄>=18岁,则被保人必须是投保人本人
	    var age = calAge(document.all("LCInsuredBirthday").value);
	    //alert(age);
	    if(age>=18)
	    {
	        alert("被保人年龄大于18岁,被保人必须是投保人本人");
	        return false;
	    }	
	}
    else
    {
        //当投,被保人是同一人时,由于只输入投保人信息,所以需要按照被保人必须录入的项目进行校验
	    if(document.all( 'AppntName' ).value==null||document.all( 'AppntName' ).value=="")
		{
	    	alert("请输入投保人姓名!");
	    	return false;

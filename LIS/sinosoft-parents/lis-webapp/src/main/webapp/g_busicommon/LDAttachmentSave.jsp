<%
/***************************************************************
 * <p>ProName：LDAttachmentSave.jsp</p>
 * <p>Title：附件管理</p>
...
<%
	...
	if (tGI==null) {
		...
		try {
			...
			String tAttachID = request.getParameter("AttachID");
			String tAttachType = request.getParameter("AttachType");
			String tSubAttachType = request.getParameter("SubAttachType");
			String tAttachFlag = request.getParameter("AttachFlag");
			String tAttachNum = request.getParameter("AttachNum");
			...
			if ("INSERT".equals(tOperate)) {
				...
				if (tSysUploadPath==null || "".equals(tSysUploadPath)) {
					...
					File tFile = new File(tSysUploadPath+"temp/");
					if (!tFile.exists()) {
						tFile.mkdirs();
			...
			if ("Succ".equals(tFlagStr) || "DELETE".equals(tOperate)) {
				
				LDAttachmentSchema tLDAttachmentSchema = new LDAttachmentSchema();
				
				tLDAttachmentSchema.setAttachID(tAttachID);
				...
				tLDAttachmentSchema.setSubAttachType(tSubAttachType);
				...
				tLDAttachmentSchema.setAttachFlag(tAttachFlag);
				tLDAttachmentSchema.setAttachNum(tAttachNum);
				tLDAttachmentSchema.setRemark(tRemark);
				
				TransferData tTransferData = new TransferData();
				if ("QUOT".equals(tOtherNoType)) {
				
					tTransferData.setNameAndValue("OtherNoType", tOtherNoType);
					tTransferData.setNameAndValue("QuotNo", tOtherNo);
					tTransferData.setNameAndValue("QuotBatNo", tSubOtherNo);
					tTransferData.setNameAndValue("ActivityID", tUploadNode);
				}
				...
				if (!tBusinessDelegate.submitData(tVData, tOperate, "LDAttachmentUI")) {

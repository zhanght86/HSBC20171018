...
//				 + " and codetype = 'GrpQuestion' order by to_number(code)";
	//}
		 ...
		 var sqlid32="ContQuerySQL32";
		var mySql32=new SqlClass();
		mySql32.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql32.setSqlId(sqlid32);//指定使用的Sql的id
		mySql32.addSubPara(k);//指定传入的参数
		mySql32.addSubPara(k);//指定传入的参数
...
{	
	...
	//{
			 ...
			 var sqlid33="ContQuerySQL33";
		var mySql33=new SqlClass();
		mySql33.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql33.setSqlId(sqlid33);//指定使用的Sql的id
		mySql33.addSubPara(k);//指定传入的参数
		mySql33.addSubPara(k);//指定传入的参数
		mySql33.addSubPara(tProposalNo);//指定传入的参数
...
function query()
{
window.open("./GrpQuestQueryMain.jsp?GrpContNo="+document.all('GrpContNoHide').value+"&Flag="+tFlag);
}

...
//	var strSql = "select bussno,errorcount,errorcontent,"
//	             +"(select bponame from BPOServerInfo where BPOServerInfo.Lisoperator = BPOMissionDetailError.operator and bussNoType='"+BussNoType+"') bponame,"
//	             +"MakeDate,MakeTime"
...
//	             +"  order by bussno,errorcount";
	...
	mySql1.setResourceName("app.AbnormityErrAndRecordErrSql"); //指定使用的properties文件名
...
//	             +" and bussNoType='"+BussNoType+"'";
	...
	var sqlid1="AbnormityErrAndRecordErrSql1";
	var mySql1=new SqlClass();
	mySql1.setResourceName("app.AbnormityErrAndRecordErrSql"); //指定使用的properties文件名
	mySql1.setSqlId(sqlid1);//指定使用的Sql的id
	mySql1.addSubPara(prtNo);//指定传入的参数
...
function submitForm()
{
  ...
  var name='提示';   //网页名称，可为空; 
	...
	showInfo.focus();
  //alert(prtNo);
  fm.action = "./AbnormityErrAndRecordErrSave.jsp?prtNo="+prtNo;
  fm.submit(); //
}

...
var mAction ="";
var sqlresourcename = "app.DSEasyContSql";
/*********************************************************************
...
function displayContInfo(arrResultCont){
	try{
		...
		fm.RnewFlag.value 		= arrResultCont[0][87];
		...
		fm.Lang.value				= arrResultCont[0][36];
		fm.Currency.value			= arrResultCont[0][37];
	}catch(ex){
		alert("显示合同信息时出错！");
	}
}
...
function saveRisk(wFlag){
	...
	mAction="UPDATE";
	document.all( 'fmAction' ).value = mAction;
	fm.action = "DSEasyProposalSave.jsp";
	//showInfo=window.showModelessDialog(urlStr,window,"status:no;help:0;close:0;dialogWidth:550px;dialogHeight:250px");
	var name='提示';   //网页名称，可为空; 
	var iWidth=550;      //弹出窗口的宽度; 
	var iHeight=250;     //弹出窗口的高度; 
...
mySql6.addSubPara(prtNo);
		...
		/*var AgentSql="select impartcode,impartver,impartcontent,impartparammodle,Insured1,PrtFlag,Insured2,insured3,fillno from LBPOCustomerImpart where contno='"+prtNo+"' and ImpartVer='A03' and InputNo='3' order by impartver,impartcode";
		var arrResult2;
...
mySql9.addSubPara(InputNo);
		...
		/*var AgentSql="select impartcode,impartver,impartcontent,impartparammodle,Insured1,PrtFlag,Insured2,insured3,fillno from LBPOCustomerImpart where contno='"+prtNo+"' and ImpartVer='A03' and InputNo='"+InputNo+"' order by impartver,impartcode";
		var arrResult2;
		arrResult2=easyExecSql(AgentSql);
		if(arrResult2!=null){
			turnPage.queryModal(AgentSql, AgentImpartGrid);
...
//		var tBnfSql2 = "select 1,1,b.name,b.sex,b.idno,b.relationtoinsured,b.bnflot,b.bnfgrade,b.bnfaddress,b.FillNo,b.insuredno,0 from lbpobnf b where b.contno='"+prtNo+"' and b.inputno='3'"   
var sqlid14="DSEasyContSql14";
...
mySql18.addSubPara(arrResultErrorCont[k][0]);
  		...
  		//处理投保人信息
  	//	var tErrorAppnt = "select errortag from lbpodatadetailerror where bussno='"+prtNo+"'and bussnotype='TB' and tablename='LBPOAppnt' ";
 var sqlid19="DSEasyContSql19";
  				...
  				try{
	  				...
	  				if(arrResultErrorAppnt[k][0]=="CompanyAddress"||arrResultErrorAppnt[k][0]=="CompanyPhone"
	  				   ...
	  				   ||arrResultErrorAppnt[k][0]=="ZipCode"
	  				   	...
	  				   	ele="Appnt"+arrResultErrorAppnt[k][0];//alert(ele);
	  				   }else{
...
mySql32.addSubPara(tFillNo);
	     			...
	     			//alert(tTitle);
		  			...
		  			var tempOrder="document.all('PolGrid"+tLineCount+"').disabled=false;";
		  			//alert("tempOrder"+tempOrder);
		  			eval(tempOrder);
	  			}
	  		}
...
mySql39.addSubPara(InputNo);
	...
	//	var tBnfSql = "select 1,1,b.name,b.sex,b.idno,b.relationtoinsured,b.bnflot,b.bnfgrade,b.bnfaddress,b.FillNo,b.insuredno,0 from lbpobnf b where b.contno='"+prtNo+"' and b.inputno='"+InputNo+"'"   
var sqlid40="DSEasyContSql40";
...
mySql43.addSubPara(InputNo);
	...
	//		var tBnfSql = "select a.sequenceno,b.name,b.sex,b.idno,b.relationtoinsured,b.bnflot,b.bnfgrade,b.bnfaddress from lbpoinsured a,lbpobnf b where a.contno=b.contno and a.inputno=b.inputno and a.contno='"+prtNo+"' and a.inputno='"+InputNo+"' and a.SequenceNo='"+Field.value+"'";
var sqlid44="DSEasyContSql44";
...
mySql46.addSubPara(prtNo);
	...
	for(var k=0;k<errorMessageResult.length;k++){
  		...
  		//投保人特殊处理
  	//	var tErrorAppnt = "select errortag,errorcontent from lbpodatadetailerror where bussno='"+prtNo+"'and bussnotype='TB' and tablename='LBPOAppnt' order by errortag";
var sqlid47="DSEasyContSql47";

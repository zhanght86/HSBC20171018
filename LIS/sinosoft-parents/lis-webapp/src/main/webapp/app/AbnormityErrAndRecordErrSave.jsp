...
<%
  ...
  //操作类型是插入，查询，更新
  loggerDebug("AbnormityErrAndRecordErrSave","印刷号是"+rgtNo);
  String BussNoType=request.getParameter("BussNoType");
  //操作类型是插入，查询，更新
  loggerDebug("AbnormityErrAndRecordErrSave","bussnotype: "+BussNoType);
      ...
      for (int i = 0; i < ImpartCount; i++)	
  	   {
		  ...
		  loggerDebug("AbnormityErrAndRecordErrSave","印刷号是"+rgtNo);
		  mBPOErrLogSchema.setBussNoType(BussNoType);
		  loggerDebug("AbnormityErrAndRecordErrSave","业务号码类型是"+BussNoType);
	      mBPOErrLogSchema.setErrCode(terrorCode[i]);
	      loggerDebug("AbnormityErrAndRecordErrSave","差错编码terrorCode["+i+"]="+terrorCode[i]);
		  //mBPOErrLogSchema.setBussNoType("TB");
 ...
 // AbnormityErrAndRecordErrUI mAbnormityErrAndRecordErrUI = new AbnormityErrAndRecordErrUI();
  String busiName="tbAbnormityErrAndRecordErrUI";

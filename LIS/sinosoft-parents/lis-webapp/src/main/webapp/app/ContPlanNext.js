...
//		+ "order by ContPlanCode,FactoryType, OtherNo,FactoryCode,FactorySubCode ";
	...
	mySql0.setResourceName("app.ContPlanNextSql"); //指定使用的properties文件名
	mySql0.setSqlId(sqlid0);//指定使用的Sql的id
	mySql0.addSubPara(tGrpContNo);//指定传入的参数
...
//	strSQL = "select GrpContNo,ProposalGrpContNo,ManageCom,AppntNo,GrpName from LCGrpCont where GrpContNo = '" +GrpContNo+ "'";
	...
	mySql1.setResourceName("app.ContPlanNextSql"); //指定使用的properties文件名
...
function initImpContPlanCode(tGrpContNo){
	...
	*/
	...
	mySql2.setResourceName("app.ContPlanNextSql"); //指定使用的properties文件名

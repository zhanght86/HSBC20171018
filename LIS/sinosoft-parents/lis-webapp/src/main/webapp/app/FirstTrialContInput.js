...
{	
		...
		var sqlid1="FristTrialContinputSql1";
		var mySql1=new SqlClass();
		mySql1.setResourceName("app.FristTrialContinputSql"); //指定使用的properties文件名
	...
	if(arrResult!=null)
	{
		...
		var sqlid2="FristTrialContinputSql2";
		var mySql2=new SqlClass();
		mySql2.setResourceName("app.FristTrialContinputSql"); //指定使用的properties文件名
...
function afterQuery( arrQueryResult ) {
  //alert("here:" + arrQueryResult + "\n" + mOperate);
	if( arrQueryResult != null ) {
		...
		var sqlid3="FristTrialContinputSql3";
		var mySql3=new SqlClass();
		mySql3.setResourceName("app.FristTrialContinputSql"); //指定使用的properties文件名
		mySql3.setSqlId(sqlid3);//指定使用的Sql的id
		mySql3.addSubPara(arrQueryResult[0][0]);//指定传入的参数
	    var strSql3 =mySql3.getString();	
		
		    arrResult = easyExecSql(strSql3, 1, 0);
		...
		var sqlid4="FristTrialContinputSql4";
		var mySql4=new SqlClass();
		mySql4.setResourceName("app.FristTrialContinputSql"); //指定使用的properties文件名
		mySql4.setSqlId(sqlid4);//指定使用的Sql的id
		mySql4.addSubPara(arrQueryResult[0][0]);//指定传入的参数
	    var strSql4 =mySql4.getString();	
		
		arrResult = easyExecSql(strSql4, 1, 0);
			//arrResult = easyExecSql("select a.*,(select GrpName from LDGrp where CustomerNo=(select GrpNo from LDPerson where CustomerNo=a.CustomerNo) ) from LDPerson a,LCAddress b where 1=1 and a.CustomerNo=b.CustomerNo and a.CustomerNo = '" + arrQueryResult[0][0] + "'", 1, 0);
			if (arrResult == null) {
			  alert("未查到投保人信息");
			} else {
...
try{
  if(PrtNo==null)
    return;
		...
		var sqlid5="FristTrialContinputSql5";
		var mySql5=new SqlClass();
		mySql5.setResourceName("app.FristTrialContinputSql"); //指定使用的properties文件名
...
//  arrResult=easyExecSql("select * from LCCont where PrtNo='"+PrtNo+"'",1,0);
  if(arrResult==null){
    ...
    displayLCCont();       //显示投保单详细内容
  		...
  		var sqlid6="FristTrialContinputSql6";
		var mySql6=new SqlClass();
		mySql6.setResourceName("app.FristTrialContinputSql"); //指定使用的properties文件名
...
function queryAgent()
{
	...
	var cAgentCode = fm.AgentCode.value;  //保单号码	
		  ...
		  var sqlid2="ContQuerySQL2";
		var mySql2=new SqlClass();
		mySql2.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql2.setSqlId(sqlid2);//指定使用的Sql的id
		mySql2.addSubPara(cAgentCode);//指定传入的参数
		mySql2.addSubPara(document.all('ManageCom').value);//指定传入的参数
	    var strSql=mySql2.getString();	
...
function queryAgent2()
{
	...
	var cAgentCode = fm.AgentCode.value;  //保单号码	
		...
		var sqlid4="ContQuerySQL4";
		var mySql4=new SqlClass();
		mySql4.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql4.setSqlId(sqlid4);//指定使用的Sql的id
...
function getdetail()
{
			...
			var sqlid5="ContQuerySQL5";
		var mySql5=new SqlClass();
		mySql5.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql5.setSqlId(sqlid5);//指定使用的Sql的id
		mySql5.addSubPara(fm.AppntBankAccNo.value);//指定传入的参数
...
function getdetailwork()
{
		...
		var sqlid6="ContQuerySQL6";
		var mySql6=new SqlClass();
		mySql6.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql6.setSqlId(sqlid6);//指定使用的Sql的id
		mySql6.addSubPara(fm.AppntOccupationCode.value);//指定传入的参数
...
function getdetailaddress(){
			...
			var sqlid7="ContQuerySQL7";
		var mySql7=new SqlClass();
		mySql7.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql7.setSqlId(sqlid7);//指定使用的Sql的id
		mySql7.addSubPara(fm.AddressNo.value);//指定传入的参数
		mySql7.addSubPara(fm.AppntNo.value);//指定传入的参数
...
function getaddresscodedata()
{
    ...
    var tCodeData = "0|";
				...
				var sqlid8="ContQuerySQL8";
		var mySql8=new SqlClass();
		mySql8.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
...
function afterCodeSelect( cCodeName, Field )
{
 if(cCodeName=="GetAddressNo"){
		...
		var sqlid9="ContQuerySQL9";
		var mySql9=new SqlClass();
		mySql9.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql9.setSqlId(sqlid9);//指定使用的Sql的id
		mySql9.addSubPara(fm.AddressNo.value);//指定传入的参数
		mySql9.addSubPara(fm.AppntNo.value);//指定传入的参数
...
function AppntChk()
{
	...
	Sex=Sex.substring(0,i);
			...
			var sqlid10="ContQuerySQL10";
		var mySql10=new SqlClass();
		mySql10.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql10.setSqlId(sqlid10);//指定使用的Sql的id
		mySql10.addSubPara(fm.AppntName.value);//指定传入的参数
		mySql10.addSubPara(Sex);//指定传入的参数
		mySql10.addSubPara(fm.AppntBirthday.value);//指定传入的参数
		mySql10.addSubPara(fm.AppntNo.value);//指定传入的参数
	    var sqlstr =mySql10.getString();	
	...
	window.open("../uw/AppntChkMain.jsp?ProposalNo1="+fm.ContNo.value+"&Flag=A","window1",sFeatures);
}                                                                             
...
function showHealth()
{
  ...
  MakeWorkFlow();
  			...
  			var sqlid11="ContQuerySQL11";
		var mySql11=new SqlClass();
		mySql11.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql11.setSqlId(sqlid11);//指定使用的Sql的id
		mySql11.addSubPara(fm.MissionID.value);//指定传入的参数
  ...
  if (cContNo != "")
  {
  	...
  	window.open("../uw/UWManuHealthMain.jsp?ContNo1="+cContNo+"&MissionID="+cMissionID+"&SubMissionID="+cSubMissionID+"&PrtNo="+cPrtNo,"window1",sFeatures);
  	showInfo.close();
  }
  else
  {
...
function showReport()
{
	...
	MakeWorkFlow();
	    ...
	    var sqlid12="ContQuerySQL12";
		var mySql12=new SqlClass();
		mySql12.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql12.setSqlId(sqlid12);//指定使用的Sql的id
		mySql12.addSubPara(fm.MissionID.value);//指定传入的参数
  ...
  window.open("../uw/UWManuRReportMain.jsp?ContNo="+cContNo+"&MissionID="+cMissionID+"&PrtNo="+cPrtNo+"&SubMissionID="+cSubMissionID,"window2",sFeatures);  	
  showInfo.close();  
}
//自动工作流初始化
...
function initDetail()
{
		    ...
		    var sqlid13="ContQuerySQL13";
		var mySql13=new SqlClass();
		mySql13.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
    ...
    if(fm.ContNo.value!='')
    {
				    ...
				    var sqlid14="ContQuerySQL14";
		var mySql14=new SqlClass();
		mySql14.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
...
function PrintHealth()
{
		 ...
		 var sqlid15="ContQuerySQL15";
		var mySql15=new SqlClass();
		mySql15.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql15.setSqlId(sqlid15);//指定使用的Sql的id
		mySql15.addSubPara( fm.PrtNo.value);//指定传入的参数
	...
	window.open( "../uw/BodyCheckPrintSave.jsp?PrtSeq="+arrResult[0][2]+"&MissionID="+arrResult[0][0]+"&SubMissionID="+arrResult[0][1],"",sFeatures ); 
}                                                                           
//打印生调通知书
function PrintReport()
{
	
		var sqlid16="ContQuerySQL16";
		var mySql16=new SqlClass();
		mySql16.setResourceName("app.ContQuerySQL"); //指定使用的properties文件名
		mySql16.setSqlId(sqlid16);//指定使用的Sql的id
		mySql16.addSubPara( fm.PrtNo.value);//指定传入的参数

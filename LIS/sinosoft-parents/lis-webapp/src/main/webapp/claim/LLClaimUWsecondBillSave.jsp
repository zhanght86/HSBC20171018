...
<%
	...
	if (tG == null) {
		loggerDebug("LLClaimUWsecondBillSave","未获取到登录信息!");
		return;
	}
	...
	if (tLLClaimBillUI.submitData(tVData, "UWsecond") == false) {
		...
		if (txmlExport == null) {
			...
			loggerDebug("LLClaimUWsecondBillSave","没有得到要显示的数据文件");
		}
	}
	...
	if (FlagStr.equals("Succ")) {
		...
		loggerDebug("LLClaimUWsecondBillSave","strVFFileName=" + strVFFileName);

		//获取存放临时文件的路径
		String strRealPath = application.getRealPath("/").replace('\\', '/');
		loggerDebug("LLClaimUWsecondBillSave","strRealPath=" + strRealPath);
		...
		//把dataStream存储到磁盘文件
		loggerDebug("LLClaimUWsecondBillSave","存储文件到" + strVFPathName);

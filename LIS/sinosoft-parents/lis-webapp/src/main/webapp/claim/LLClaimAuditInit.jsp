...
function initForm()
{
  try
  {
    ...
    * 审核界面初始化时自动重新匹配并理算,防止预付案件在回退时llbalance中正常给付信息被删除,而预付金额在该表中是负值,如果不重新理算,则在受益人分配时的初始金额就是负值
    */
    initShowMatchDutyPay(); //自动匹配并理算
  ...
  {
    alert("在LLClaimAuditInit.jsp-->InitForm函数中发生异常:初始化界面错误!");
  }
}
...
function initInpBox()
{
  try
  {
    ...
    fm.printAuditRpt.disabled = true;
  }
  catch(ex)
  {
...
//        iArray[10][3]=3;
        ...
        SubReportGrid.mulLineCount = 1;
        SubReportGrid.displayTitle = 1;
        SubReportGrid.locked = 0;
       // SubReportGrid.canChk =0;
...
function initPolDutyCodeGrid()
{
    ...
    try
    {
      ...
      iArray[16][5]="15|16";              //引用代码信息分别放在第13列和第14列，'|'为分割符
      ...
      iArray[20][1]="60px";
      iArray[20][2]=60;
      iArray[20][3]=0;                    //1表示允许，0表示不允许,2表示代码选择,3隐藏
      ...
      iArray[22][1]="60px";
      iArray[22][2]=60;
      iArray[22][3]=0;                    //1表示允许，0表示不允许,2表示代码选择,3隐藏

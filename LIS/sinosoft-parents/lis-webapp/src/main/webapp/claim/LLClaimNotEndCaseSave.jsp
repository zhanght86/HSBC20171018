...
<%
	...
	if (tG == null) {
		loggerDebug("LLClaimNotEndCaseSave","未获取到登录信息!");
		return;
	}
	...
	tTransferData.setNameAndValue("ClaimType", request.getParameter("ClaimType")); //统计类型
	// 准备传输数据 VData
	...
	if (tLLClaimBillUI.submitData(tVData, "NotEndCase") == false) {
		...
		if (txmlExport == null) {
			...
			loggerDebug("LLClaimNotEndCaseSave","没有得到要显示的数据文件");
		}
	}
	...
	if (FlagStr.equals("Succ")) {
		...
		loggerDebug("LLClaimNotEndCaseSave","strVFFileName=" + strVFFileName);

		//获取存放临时文件的路径
		String strRealPath = application.getRealPath("/").replace('\\', '/');
		loggerDebug("LLClaimNotEndCaseSave","strRealPath=" + strRealPath);
		...
		//把dataStream存储到磁盘文件
		loggerDebug("LLClaimNotEndCaseSave","存储文件到" + strVFPathName);

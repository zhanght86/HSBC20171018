...
function queryGrid1()
{
   ...
   mySql = new SqlClass();
	mySql.setResourceName("claim.LLColQueryMedicalFeeInpInputSql");
...
function queryDeformityRate()
{
    if(fm.DefoCode.value != "" && fm.DefoCode.value !=null)
    {
        /*var strSql = " select t.deforate from LLParaDeformity t where "
         ...
         mySql = new SqlClass();
	mySql.setResourceName("claim.LLColQueryMedicalFeeInpInputSql");
	mySql.setSqlId("LLColQueryMedicalFeeInpSql8");
...
function getMedFeeClinicInpGrid()
{
    ...
    fm.ClinicMedFeeSum.value = MedFeeClinicInpGrid.getRowColData(tNo - 1,9);//费用金额
    ...
    fm.selfMedFeeSum.value = MedFeeClinicInpGrid.getRowColData(tNo - 1,16); //自费/自付金额
    //alert("自费/自付金额:"+fm.selfMedFeeSum.value);
    
    fm.DealFlag.value = MedFeeClinicInpGrid.getRowColData(tNo - 1,18); //处理标记:1-正常费用,0-出险前费用
    //alert("费用处理标记:"+fm.DealFlag.value);
    
    fm.ClinicDayCount2.value = MedFeeClinicInpGrid.getRowColData(tNo - 1,19); //距离事故日期天数
    
    fm.theReason1.value = MedFeeClinicInpGrid.getRowColData(tNo - 1,21);
    fm.theReasonName1.value = MedFeeClinicInpGrid.getRowColData(tNo - 1,20);
    fm.Remark1.value = MedFeeClinicInpGrid.getRowColData(tNo - 1,22);
...
function getMedFeeInHosInpGrid()
{
    ...
    fm.InHosMedFeeSum.value = MedFeeInHosInpGrid.getRowColData(tNo - 1,9); //费用金额
    fm.selfInHosFeeSum.value = MedFeeInHosInpGrid.getRowColData(tNo - 1,10); //自费/自付金额  
    ...
    fm.DealFlag.value = MedFeeInHosInpGrid.getRowColData(tNo - 1,17); //处理标记:1-正常费用,0-出险前费用
    fm.InHosDayCount2.value = MedFeeInHosInpGrid.getRowColData(tNo - 1,18); //距离事故日期天数
    
    fm.theReason2.value = MedFeeInHosInpGrid.getRowColData(tNo - 1,20); 
    fm.theReasonName2.value = MedFeeInHosInpGrid.getRowColData(tNo - 1,19); 
    fm.Remark2.value = MedFeeInHosInpGrid.getRowColData(tNo - 1,21); 
...
function getMedFeeCaseInfoGrid()
{
    ...
    showOneCodeName('lldefotype','DefoType','DefoTypeName');
    showOneCodeName('lldefocode','DefoCode','DefoCodeName');
}
...
function getMedFeeOperGrid()
{
    ...
    fm.OpFee.value = MedFeeOperGrid.getRowColData(tNo - 1,6);
    fm.selfOpFeeSum.value = MedFeeOperGrid.getRowColData(tNo - 1,7);

    fm.UnitName.value = MedFeeOperGrid.getRowColData(tNo - 1,13);
    //alert("机构名称:"+fm.UnitName.value);
    
    fm.DiagnoseDate.value = MedFeeOperGrid.getRowColData(tNo - 1,14);
    //alert("确认日期:"+fm.DiagnoseDate.value);
    
    //赋值关键信息
    fm.claimNo.value = MedFeeOperGrid.getRowColData(tNo - 1,9);       //赔案号
    fm.caseNo.value = MedFeeOperGrid.getRowColData(tNo - 1,10);        //分案号
    fm.SerialNo4.value = MedFeeOperGrid.getRowColData(tNo - 1,11);     //流水号
    //alert("手术序号:"+fm.SerialNo4.value);
    
    fm.custNo.value = MedFeeOperGrid.getRowColData(tNo - 1,12);        //客户号
...
function getMedFeeOtherGrid()
{
    ...
    fm.FactorValue.value = MedFeeOtherGrid.getRowColData(tNo - 1,4);//费用金额
    fm.selfFactorFeeSum.value = MedFeeOtherGrid.getRowColData(tNo - 1,5);//自费/自付金额
    //alert("fm.selfFactorFeeSum.value"+fm.selfFactorFeeSum.value);
    
    //赋值关键信息
    fm.claimNo.value = MedFeeOtherGrid.getRowColData(tNo - 1,6);       //赔案号
    fm.caseNo.value = MedFeeOtherGrid.getRowColData(tNo - 1,7);        //分案号
    //alert("fm.caseNo.value"+fm.caseNo.value);
    fm.SerialNo5.value = MedFeeOtherGrid.getRowColData(tNo - 1,8);     //流水号
    fm.custNo.value = MedFeeOtherGrid.getRowColData(tNo - 1,9);        //客户号
    //alert("fm.custNo.value"+fm.custNo.value);
    
    
    fm.FactorUnitName.value = MedFeeOtherGrid.getRowColData(tNo - 1,10);
    showOneCodeName('llfactype','FactorType','FactorTypeName');
    fm.FactorStateDate.value = MedFeeOtherGrid.getRowColData(tNo - 1,11);
    fm.FactorEndDate.value = MedFeeOtherGrid.getRowColData(tNo - 1,12);
    
    fm.theReason4.value = MedFeeOtherGrid.getRowColData(tNo - 1,14);
    fm.theReasonName4.value = MedFeeOtherGrid.getRowColData(tNo - 1,13);
    fm.Remark4.value = MedFeeOtherGrid.getRowColData(tNo - 1,15);
...
function getMedFeeCompensateInpGrid()
{
    ...
    fm.MedFeeCompensateSum.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,9);

    
    //赋值关键信息
    fm.claimNo.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,11);       //赔案号
    fm.caseNo.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,12);        //分案号
    fm.custNo.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,13);        //客户号
    fm.CompensateMainFeeNo.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,14);     //帐单号
    fm.feeDetailNo.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,15);   //帐单明细号
    

    fm.selfMedFeeCompensateFeeSum.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,16); //自费/自付金额  
    fm.DealFlag.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,18); //处理标记:1-正常费用,0-出险前费用
    fm.MedFeeCompensateEndDateInHosDayCount2.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,19); //距离事故日期天数

    fm.theReason3.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,21); //
    fm.theReasonName3.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,20); //
    fm.Remark3.value = MedFeeCompensateInpGrid.getRowColData(tNo - 1,22); //    
...
function queryHospitalxx(tCaseNo,tCustNo)
{
	...
	mySql = new SqlClass();
	mySql.setResourceName("claim.LLColQueryMedicalFeeInpInputSql");
	mySql.setSqlId("LLColQueryMedicalFeeInpSql9");

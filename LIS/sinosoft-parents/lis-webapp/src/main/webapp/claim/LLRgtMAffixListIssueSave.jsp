...
<%
    ...
    if(tG == null)
    {
        //loggerDebug("LLRgtMAffixListIssueSave","登录信息没有获取!!!");
        return;
     } 
     else if (tOperate == null || tOperate == "") 
     {
        //loggerDebug("LLRgtMAffixListIssueSave","没有获取操作符!!!");
        return;
    }
  ...
  //输入参数
    ...
    //准备数据容器信息
     ...
     String tGrid12[] = request.getParameterValues("AffixGrid12"); //发起阶段
    ...
    if (tChk!=null&&tChk.length>0)
    {
	    for(int index=0;index<tChk.length;index++)
	    {
	      ...
	      {          
	          ...
	          tLLAffixSchema.setCustomerNo(tClmNo); //客户号   	 
		      ...
		      tLLAffixSchema.setSupplyStage(tGrid12[index]); //发起阶段
		      tLLAffixSchema.setReAffixDate(tReplyDate); //补充材料受理日期
		      tLLAffixSchema.setAffixState("03"); //所处阶段
...
//            for (int i = 0; i < n; i++)
//            {
//                Content = Content + "单证信息保存错误，原因是: " + tLLRgtMAffixListIssueUI.mErrors.getError(i).errorMessage;
//                FlagStr = "FAIL";
//            }
//        } 

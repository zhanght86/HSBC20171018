...
function initQueryAffixGrid()
{
    try
    {
	   ...
	   /* strSQL="select affixcode,affixname,subflag,needflag,readycount,property,shortcount,returnflag,affixno from LLAffix where "
	    ...
	    mySql = new SqlClass();
		mySql.setResourceName("claim.LLRgtAddMAffixListInputSql");
...
function AffixSave()
{
	//var strSQL="select 1 from LOPRTManager where code='PCT003' and Otherno='"+document.all('ClmNo').value+"'";
        mySql = new SqlClass();
		mySql.setResourceName("claim.LLRgtAddMAffixListInputSql");
		mySql.setSqlId("LLRgtAddMAffixListSql4");
		mySql.addSubPara(document.all('ClmNo').value);  
    var arr=easyExecSql(mySql.getString());

    if((arr!=null)&&(arr!=""))
    {
    	alert("已发起过补充材料，不能重复发起！");
    	return;
    }

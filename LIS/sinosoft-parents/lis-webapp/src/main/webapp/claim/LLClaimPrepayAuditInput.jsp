...
<%
	...
	String mAuditPopedom =request.getParameter("AuditPopedom");  //案件审核级别
	String mRgtClass =request.getParameter("RgtClass");  //申请类型
	String mRgtObj =request.getParameter("RgtObj"); //号码类型
...
<form  method=post name=fm id=fm target="fraSubmit">
   ...
   <Div id= "divLLPrepayDetailInfo" style= "display: ''" align = center>
    ...
    <table >
        <TR class= common>
            <TD> <Input TYPE=button class=cssButton name="LLPrepayAdd" id=LLPrepayAdd disabled VALUE="增  加" onclick="LLPrepayDetailAdd();"></TD>
            <TD> <Input TYPE=button class=cssButton name="LLPrepayCancel" id=LLPrepayCancel disabled VALUE="取  消" onclick="LLPrepayDetailCancel();"></TD>
        </TR>
    </Table>      
   <Div id= "divLLPrepayDetail" style= "display: none" >
        ...
        <table>
	     <TR class=common>
            	<TD> <Input TYPE=button class=cssButton name="PrsSave" id=PrsSave VALUE="保  存" onclick="LLPrepayDetailSave();"></TD>    
         </TR>
        </table>
   </Div>      
    <!--预付审核管理信息-->
    ...
    <Div id= "divLLAudit" class=maxbox1 style= "display: ''">
        ...
        <table>
            <tr>
                ...
                <td><INPUT class=cssButton name="printAuditRpt" id=printAuditRpt VALUE="打印审核报告"  TYPE=button onclick="LLPRTClaimAuditiReport()" ></td>                                 
            </tr>
        </table>
    </Div> 
	<hr class=line>
	<Table>
        <TR>	
            <TD> <Input TYPE=button class=cssButton name="PrepayCofirm" id=PrepayCofirm VALUE="审核确认" onclick="LLClaimPrepayCofirm();"></TD>
    ...
    <Input type=hidden name="ComCode" id=ComCode>     <!--//当前机构-->
    <input type=hidden id="fmtransact" name="fmtransact"><!--操作代码‘insert,delete’等-->
    <Input type=hidden name="PrepayNo" id=PrepayNo >     <!--//预付批次号,每次进入预付页面初始化时生成，在处理过程中一直不变-->

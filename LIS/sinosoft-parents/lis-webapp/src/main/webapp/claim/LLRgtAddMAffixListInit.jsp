...
//	alert("document.all('ClmNo').value"+document.all('ClmNo').value);
    document.all('CustomerNo').value =nullToEmpty("<%=tCustomerNo%>");
//    alert("document.all('CustomerNo').value"+document.all('CustomerNo').value);
    document.all('Proc').value =nullToEmpty("<%=tProc%>");
//    alert("document.all('Proc').value"+document.all('Proc').value);
}
...
function initInpBox()
{
    ...
    {    
    	//判断是否需要生成单证----查询如果有单证，则无需生成，否则则需生成单证
         //var strSql="select * from llaffix where rgtno='"+fm.ClmNo.value+"' and customerno='"+fm.CustomerNo.value+"' ";
         var tResourceName="claim.LLRgtAddMAffixListInputSql";
	     var strSql = wrapSql(tResourceName,"querysqldes1",[fm.ClmNo.value,fm.CustomerNo.value]);
//         alert(strSql);

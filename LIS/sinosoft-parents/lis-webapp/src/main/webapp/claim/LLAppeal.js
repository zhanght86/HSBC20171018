...
function afterMatchDutyPayQuery()
{
   ...
   /* tSql = " select a.StandPay ,a.BeforePay,a.BalancePay,a.RealPay,a.DeclinePay,"
	...
	mySql = new SqlClass();
	mySql.setResourceName("claim.LLAppealInputSql");
	mySql.setSqlId("LLAppealSql1");
...
//    alert(LLAccident);
    ...
    /*var strSQL2 = " select CustomerNo,Name,"
                         ...
                         + " Birthday,"
                         + " nvl(VIPValue,0) as VIPValue, "
                         + " (case when trim(nvl(VIPValue, 0)) = '1' then '是' else '否' end) as VIPValueName "
                         + " from LDPerson where "
                         + " CustomerNo in("
                         + " select CustomerNo from LLSubReport where SubRptNo = '"+ arr[0] +"')";*/
	mySql = new SqlClass();
	mySql.setResourceName("claim.LLAppealInputSql");
	mySql.setSqlId("LLAppealSql8");
	mySql.addSubPara(arr[0]);    
//    alert(strSQL2);                 
...
function AddUpdate()
{
    ...
    var tPolNo = PolDutyCodeGrid.getRowColData(i,1);

    //检查保项结论:1 不允许从给付改为拒付, 2 不允许做过合同处理的拒付改为给付
   /* var strSQL = "select count(1) from llcontstate where 1=1 "
                + " and contno = (select contno from llclaimdetail where clmno = '" + fm.RptNo.value + "' and polno = '" + tPolNo + "' and givetype = '1') "
                + " and clmno = (select caseno from llappeal where appealno = '" + fm.RptNo.value + "') ";
   */
    mySql = new SqlClass();
	mySql.setResourceName("claim.LLAppealInputSql");
	mySql.setSqlId("LLAppealSql11");
	mySql.addSubPara(fm.RptNo.value);   
...
function queryAppeal()
{
   ...
   /* var strSql = " select AppealType,AppealState,WaitDate,AppealName,AppealSex,Address,Phone,Mobile,AppealMode,PostCode,Relation,ReturnMode,IDNo,IDType,AppealRDesc,(case AppealState when '0' then '未回复' when '1' then '已回复' end ) from LLAppeal where "
    ...
    mySql = new SqlClass();
	mySql.setResourceName("claim.LLAppealInputSql");
	mySql.setSqlId("LLAppealSql21");
	mySql.addSubPara(fm.ClmNo.value );           
//    alert(strSql);
    var tAppeal = easyExecSql(mySql.getString());
//    alert(tAppeal);
    if (tAppeal != null)
    {
...
function OriClmLook()
{
  ...
  {
  	var tClmNo = arr[0];
  	//alert("tClmNo="+tClmNo);//调用综合查询中的公用页面

...
function initQuery()
{
    ...
    //       +"(case (select conbalflag from llclaim where clmno = '"+fm.ClmNo.value+"') when '1' then '保单已结算' else '保单未结算' end) "
    //显示赔付涉及的合同信息
...
function getContCalResultGrid(spanID,parm)
{
    ...
    var tNewPay = ContCalResultGrid.getRowColData(tNo - 1,9);//当前金额
    var tOldPay = ContCalResultGrid.getRowColData(tNo - 1,10);//原始金额
    var tOldAdjRemark = ContCalResultGrid.getRowColData(tNo - 1,11);//历史调整原因

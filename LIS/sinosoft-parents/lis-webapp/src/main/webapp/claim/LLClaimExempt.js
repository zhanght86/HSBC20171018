...
function LLClaimExemptGridQuery()
{
	...
	mySql = new SqlClass();
	mySql.setResourceName("claim.LLClaimExemptInputSql");
	mySql.setSqlId("LLClaimExemptSql1");
	mySql.addSubPara(fm.ClmNo.value); 
	//prompt("豁免信息初始化查询",strSQL);
...
//	alert(row);
	if(row>0)
	{
		...
		document.getElementById("exemptquery").disabled =true;//[提取豁免信息]按钮
		//end
	}
...
function FreeFlagClick()
{
	...
	{
		divLLClaimExemptFreeInfo.style.display="";
		fm.FreeRate.disabled=true;
		//fm.FreeStartDate.disabled=true;
		//fm.FreeEndDate.disabled=true;
	}
	else
	{
...
function LLExemptQueryClick()
{
	...
	document.getElementById("exemptquery").disabled=true;
	//end

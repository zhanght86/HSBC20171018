...
<%
	//程序名称：LLClaimCancelBillSave.jsp
	//程序功能：个险理赔撤件案件清单
	//创建日期：2009-04-16
	//创建人  ：mw
	//更新记录：
...
<%
	...
	if (tG == null) {
		loggerDebug("LLClaimCancelBillSave","未获取到登录信息!");
		return;
	}
	...
	if (tLLClaimBillUI.submitData(tVData, "Cancel") == false) {
		...
		if (txmlExport == null) {
			...
			loggerDebug("LLClaimCancelBillSave","没有得到要显示的数据文件");
		}
	}
	...
	if (FlagStr.equals("Succ")) {
		...
		loggerDebug("LLClaimCancelBillSave","strVFFileName=" + strVFFileName);

		//获取存放临时文件的路径
		String strRealPath = application.getRealPath("/").replace('\\', '/');
		loggerDebug("LLClaimCancelBillSave","strRealPath=" + strRealPath);
		...
		//把dataStream存储到磁盘文件
		loggerDebug("LLClaimCancelBillSave","存储文件到" + strVFPathName);

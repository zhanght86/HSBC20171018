...
<%
	...
	if (tG == null) {
		loggerDebug("LLClaimNoRgtBillSave","未获取到登录信息!");
		return;
	}
	...
	if (tLLClaimBillUI.submitData(tVData, "NoRgtBill") == false) {
		...
		if (txmlExport == null) {
			...
			loggerDebug("LLClaimNoRgtBillSave","没有得到要显示的数据文件");
		}
	}
	...
	if (FlagStr.equals("Succ")) {
		...
		loggerDebug("LLClaimNoRgtBillSave","strVFFileName=" + strVFFileName);

		//获取存放临时文件的路径
		String strRealPath = application.getRealPath("/").replace('\\', '/');
		loggerDebug("LLClaimNoRgtBillSave","strRealPath=" + strRealPath);
		...
		//把dataStream存储到磁盘文件
		loggerDebug("LLClaimNoRgtBillSave","存储文件到" + strVFPathName);

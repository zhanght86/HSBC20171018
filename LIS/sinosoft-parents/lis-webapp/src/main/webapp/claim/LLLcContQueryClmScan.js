...
function queryGrid()
{
	...
	/*var strSQLA="select "
        ...
        +" where a.contno=b.contno and b.InsuredNo = '"+ fm.all('Customerno').value +"'"
        ;
    var strSQLB = "select "
        +" a.grpcontno,a.ContNo,a.ManageCom,substr(to_char(a.CValiDate),0,10),"
        +" substr(to_char(a.PaytoDate),0,10),"
        ...
        +" where a.contno=b.contno and b.appntno = '"+ fm.all('Customerno').value +"'";   */  
        
    //var strSQLC = strSQLA + " union " + strSQLB
     mySql = new SqlClass();
		mySql.setResourceName("claim.LLLcContQueryInputClmScanSql");
		mySql.setSqlId("LLLcContQueryInputClmScanSql1");
...
//	//首先检验是否选择记录  
    ...
    var tGrpContNo = LLLcContSuspendGrid.getRowColData(row-1,1);//Modify by zhaorx 2006-11-24
	 // var tSQLF = " select PrtNo from LCGrpCont where grpcontno='"+tGrpContNo+"'"; //appflag='1'承保条件没加
	    mySql = new SqlClass();
		mySql.setResourceName("claim.LLLcContQueryInputClmScanSql");
		mySql.setSqlId("LLLcContQueryInputClmScanSql2");

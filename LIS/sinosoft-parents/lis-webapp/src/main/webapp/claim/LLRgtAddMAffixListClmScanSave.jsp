...
<%
//程序名称：LLRgtAddMAffixListClmScanSave.jsp
//程序功能：单证回销
...
<%
    ...
    if(tG == null)
    {
        //loggerDebug("LLRgtAddMAffixListClmScanSave","登录信息没有获取!!!");
        return;
     } 
     else if (tOperate == null || tOperate == "") 
     {
        //loggerDebug("LLRgtAddMAffixListClmScanSave","没有获取操作符!!!");
        return;
    }
    ...
    if (tChk!=null&&tChk.length>0)
    {
	    for(int index=0;index<tChk.length;index++)
	    {
	      ...
	      {          
		      ...
		      tLLAffixSchema.setCustomerNo(tClmNo); //有扫描初审存入立案号 
             ...
             //缺少件数  
		      ...
		      tLLAffixSchema.setSupplyStage("01"); //补充材料发起阶段  01-立案初审核 03-无扫描立案录入
		      tLLAffixSchema.setAffixState("01"); //所处阶段 01-发起补充材料 02-补充材料打印完毕 03-补充材料回销完毕
...
//        LLRgtAddMAffixListClmScanUI tLLRgtAddMAffixListClmScanUI = new LLRgtAddMAffixListClmScanUI();
//       if (tLLRgtAddMAffixListClmScanUI.submitData(tVData,tOperate) == false)
//        {
//            int n = tLLRgtAddMAffixListClmScanUI.mErrors.getErrorCount();
//            for (int i = 0; i < n; i++)
//            {
//                Content = Content + "单证信息保存错误，原因是: " + tLLRgtAddMAffixListClmScanUI.mErrors.getError(i).errorMessage;
//                FlagStr = "FAIL";
//            }
//        } 
		String busiName="LLRgtAddMAffixListClmScanUI";
		...
		if(!tBusinessDelegate.submitData(tVData,tOperate,busiName))
		    {    
		        ...
		        { 
			        ...
			        for (int i = 0; i < n; i++)
			        {
			            //loggerDebug("LLRgtAddMAffixListClmScanSave","Error: "+tBusinessDelegate.getCErrors().getError(i).errorMessage);

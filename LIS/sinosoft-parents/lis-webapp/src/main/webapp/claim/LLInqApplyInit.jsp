...
function initParam()
{
    ...
    document.all('InitPhase').value = nullToEmpty("<%= tInitPhase %>");
...
{ 
    try
    {                                   
		//var strSql="select customerno,name from ldperson  where customerno='"+document.all('CustomerNo').value+"' ";
		var tResourceName="claim.LLInqApplyInputSql";
		var strSql = wrapSql(tResourceName,"querysqldes1",[document.all('CustomerNo').value]);
		var arr = easyExecSql(strSql);
		document.all('CustomerName').value=arr[0][1];
    }
    catch(ex)
    {

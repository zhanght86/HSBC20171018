...
<%
	...
	if (tG == null) {
		loggerDebug("LLClaimEndCaseBillSave","未获取到登录信息!");
		return;
	}
	...
	if (tLLClaimBillUI.submitData(tVData, "EndCaseBill") == false) {
		...
		if (txmlExport == null) {
			...
			loggerDebug("LLClaimEndCaseBillSave","没有得到要显示的数据文件");
		}
	}
	...
	if (FlagStr.equals("Succ")) {
		...
		loggerDebug("LLClaimEndCaseBillSave","strVFFileName=" + strVFFileName);

		//获取存放临时文件的路径
		String strRealPath = application.getRealPath("/").replace('\\', '/');
		loggerDebug("LLClaimEndCaseBillSave","strRealPath=" + strRealPath);
		...
		//把dataStream存储到磁盘文件
		loggerDebug("LLClaimEndCaseBillSave","存储文件到" + strVFPathName);

<%
//文件名称：LLClaimEndCaseProtestPrint.jsp
//文件功能：结案单证打印-----传入 “单证代码”、“赔案号、客户号”
//由于"PCT005--理赔决定通知书－拒付[赔案层面]"和"PCT006--'理赔决定通知书－拒付[保项层面]"
//只需要打印一联,和别的需要打印两联的不一样,故特殊处理
//创建人：niuzj
//创建日期:2005-09-17
%>
...
<%
	...
	if(tGI == null)
	{
	    FlagStr = "Fail";
	    Content = "页面失效,请重新登陆";
	    loggerDebug("LLClaimEndCaseProtestPrint","页面失效,请重新登陆");    
	}
	   ...
	   {    
		...
		else
		{
		    ...
		    if(txmlExport==null)
		    {
	            ...
	            loggerDebug("LLClaimEndCaseProtestPrint","没有得到要显示的数据文件");
		    }
		}
...
//			loggerDebug("LLClaimEndCaseProtestPrint","临时文件名strFilePath="+strFilePath);
//			String strVFFileName = strFilePath+"new.vts";
			
			//获取存放临时文件的路径
			String strRealPath = application.getRealPath("/").replace('\\','/');
			loggerDebug("LLClaimEndCaseProtestPrint","临时文件的路径strRealPath="+strRealPath);
			...
			loggerDebug("LLClaimEndCaseProtestPrint","存储文件到"+strVFPathName);
			...
			loggerDebug("LLClaimEndCaseProtestPrint","==流水号 : "+session.getValue("PrtSeq"));

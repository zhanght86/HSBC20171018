...
<%
	...
	if (tG == null) {
		loggerDebug("LLClaimPrepayBillSave","未获取到登录信息!");
		return;
	}
	...
	if (tLLClaimBillUI.submitData(tVData, "PrepayBill") == false) {
		...
		if (txmlExport == null) {
			...
			loggerDebug("LLClaimPrepayBillSave","没有得到要显示的数据文件");
		}
	}
	...
	if (FlagStr.equals("Succ")) {
		...
		loggerDebug("LLClaimPrepayBillSave","strVFFileName=" + strVFFileName);

		//获取存放临时文件的路径
		String strRealPath = application.getRealPath("/").replace('\\', '/');
		loggerDebug("LLClaimPrepayBillSave","strRealPath=" + strRealPath);
		...
		//把dataStream存储到磁盘文件
		loggerDebug("LLClaimPrepayBillSave","存储文件到" + strVFPathName);

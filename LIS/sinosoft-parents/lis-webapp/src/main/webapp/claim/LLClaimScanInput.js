...
function beforeSubmit()
{
  ...
  if(document.all('BussNo').value==null || trim(document.all('BussNo').value)=="")
  {
	...
	if(document.all('BussType').value==null || trim(document.all('BussType').value)=="")
  {
    alert("请输入业务类型！");
    return false;
  }
  ...
  if(document.all('IssueDesc').value==null || trim(document.all('IssueDesc').value)=="")
  {
    alert("请输入问题描述内容！");
    return false;
  }
...
function queryClick()
{
  ...
  //showInfo=window.open("./LATrainQuery.html");
   showInfo=window.open("./LATrainQuery.html","","toolbar=no, menubar=no, scrollbars=yes, resizable=yes, location=no, status=yes");
}           
...
{ 
  ...
  if (getWherePart('AgentCode')!='')
  {
	...
	mySql = new SqlClass();
	mySql.setResourceName("claim.LLClaimScanInputSql");
	mySql.setSqlId("LLClaimScanSql1");
	mySql.addSubPara(fm.AgentCode.value ); 
	mySql.addSubPara(tBranchType); 
  ...
  mySql = new SqlClass();
	mySql.setResourceName("claim.LLClaimScanInputSql");
	mySql.setSqlId("LLClaimScanSql2"); 
	mySql.addSubPara(old_AgentGroup);  
     var strQueryResult_AgentGroup = easyQueryVer3(mySql.getString(), 1, 1, 1);

...
//	strSQL="select t.usercode,t.username,t.comcode,t.userdescription,t.userstate from LDUser t";
	...
	my_sql = new SqlClass();
	my_sql.setResourceName("claim.LLUserQuerySql"); //指定使用的properties文件名
...
//alert(strSQL);
	...
	my_sql.setResourceName("claim.LLUserQuerySql"); //指定使用的properties文件名
	my_sql.setSqlId(sql_id);//指定使用的Sql的id
	my_sql.addSubPara(document.getElementsByName(trim('UserCodeQ'))[0].value);//指定传入的参数
	my_sql.addSubPara(document.getElementsByName(trim('UserNameQ'))[0].value);//指定传入的参数
	my_sql.addSubPara(document.getElementsByName(trim('ComCodeQ'))[0].value);//指定传入的参数
	str_sql = my_sql.getString();
	turnPage.queryModal(str_sql, LLQueryUserGrid);  
}
...
//	       + " and t.usercode = '" + LLQueryUserGrid.getRowColData(tRow, 1) + "'";
	...
	my_sql.setResourceName("claim.LLUserQuerySql"); //指定使用的properties文件名
	my_sql.setSqlId(sql_id);//指定使用的Sql的id
	my_sql.addSubPara(LLQueryUserGrid.getRowColData(tRow, 1));//指定传入的参数
	str_sql = my_sql.getString();
